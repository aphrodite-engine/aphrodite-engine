...
def parse_config(value: str) -> Config:
    try:
        ...
        raise argparse.ArgumentTypeError("config must be BLOCK,OUTPUTS,K_UNROLL[,VECTOR_WIDTH]") from error
    if len(parts) == 3:
        return Config(*parts)
    if len(parts) == 4:
        return Config(*parts)
    raise argparse.ArgumentTypeError("config must be BLOCK,OUTPUTS,K_UNROLL[,VECTOR_WIDTH]")
...
def compile_kernel(kernel_class, m: int, config: Config, max_registers: int):
    ...
    return cute.compile(
        ...
        stream(),
        options=(
            f"--enable-tvm-ffi --keep-cubin --ptxas-options -maxrregcount={max_registers} --ptxas-options -lineinfo"
        ),
    )
...
def resource_usage(compiled) -> dict[str, Any]:
    ...
    registers = [attribute(cuda.CUfunction_attribute.CU_FUNC_ATTRIBUTE_NUM_REGS, function) for function in functions]
    local_bytes = [
        attribute(
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=("baseline", "sweep", "selected"), default="baseline")
    ...
    with args.output.open("w", encoding="utf-8") as output_file:
        for m in token_counts:
            ...
            weights = [torch.randn((N, K), device="cuda", dtype=torch.bfloat16) for _ in range(count)]
            residuals = [torch.randn((m, N), device="cuda", dtype=torch.bfloat16) for _ in range(count)]
            ...
            for backend, config in candidates:
                ...
                try:
                    if backend == "cublas_addmm":
                        launch = lambda a, b, residual, c: torch.addmm(residual, a, b.t(), out=c)
                    else:
                        if config is None:
                            raise AssertionError("missing CuTe config")
                        compiled = compile_kernel(kernel_class, m, config, args.max_registers)
                        launch = lambda a, b, residual, c, fn=compiled: fn(a, b, residual, c, stream())
                    ...
                    row.update(correctness(outputs[0], activation, weights[0], residuals[0]))
