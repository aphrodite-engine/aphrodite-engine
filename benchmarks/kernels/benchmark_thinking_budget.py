...
def create_case(
    ...
    active_req_indices = [
        req_idx for req_num, req_idx in enumerate(req_indices) if budget_type != "mixed" or req_num % 2 == 0
    ]
...
def benchmark_cached(case: BenchmarkCase, warmup_ms: int, rep_ms: int) -> tuple[float, float, float]:
...
def main() -> None:
        ...
        for batch_size in mode_batch_sizes:
            ...
            for history_len in history_lengths:
                if mode == "cached":
                    ...
                    budget_type = "forced" if mode == "forced-end" else "mixed" if mode == "batched-mixed" else "active"
                    case = create_case(
                        history_len,
                        device,
    ...
    if args.max_slowdown is not None:
        for mode in ("cached", "incremental-decode"):
            ...
            baseline_ms = mode_results[history_lengths[0]][0]
            worst_history_len = max(mode_results, key=lambda length: mode_results[length][0])
