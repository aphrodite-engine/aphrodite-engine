...
def _bench_graph_layers(calls: list) -> float:
    ...
    with torch.cuda.graph(graph):
        for call in calls:
            call()
    total = triton.testing.do_bench(graph.replay, warmup=50, rep=300, return_mode="median")
...
class Inputs:
    def __init__(self, num_tokens: int, num_heads: int, conv_layout: str) -> None:
        ...
        self.conv_weights = torch.randn(3 * dim, CONV_WIDTH, device=device, dtype=torch.float32)
        self.decode_conv1d_weight = torch.stack(
            [self.conv_weights[i * dim : (i + 1) * dim].transpose(0, 1).contiguous() for i in range(3)]
        )
        if conv_layout == "DS":
            self.conv_state_t = torch.randn(num_slots, 3 * dim, CONV_WIDTH - 1, device=device, dtype=DTYPE)
        else:
            conv_state = torch.randn(num_slots, CONV_WIDTH - 1, 3 * dim, device=device, dtype=DTYPE)
            self.conv_state_t = conv_state.transpose(-1, -2)
        self.recurrent_state = torch.randn(num_slots, num_heads, HEAD_DIM, HEAD_DIM, device=device, dtype=torch.float32)
        self.g1 = torch.randn(1, num_tokens, num_heads, HEAD_DIM, device=device, dtype=DTYPE)
        self.g2 = torch.randn(num_tokens, num_heads, HEAD_DIM, device=device, dtype=DTYPE)
        ...
        self.state_indices = torch.arange(1, num_tokens + 1, device=device, dtype=torch.int32)
        self.out = torch.empty(1, num_tokens, num_heads, HEAD_DIM, device=device, dtype=DTYPE)
...
def _gated_rmsnorm(x: torch.Tensor, gate: torch.Tensor, weight: torch.Tensor, eps: float) -> torch.Tensor:
    x_float = x.float()
...
def _triton_gated_norm(inp: Inputs, core_attn_out: torch.Tensor) -> torch.Tensor:
    from aphrodite.third_party.flash_linear_attention.ops.kda import rms_norm_gated
...
def triton_chain(inp: Inputs, fused_norm: bool = False) -> None:
    ...
    if fused_norm:
        ...
        inp.out.copy_(_gated_rmsnorm(core_attn_out, inp.g2, inp.norm_weight_bf16, NORM_EPS))
...
def main() -> None:
    ...
    if not hasattr(torch.ops._C, "fused_kda_decode"):
        raise SystemExit("Aphrodite was built without the fused KDA decode kernel")

    props = torch.cuda.get_device_properties(0)
    bench = _bench_graph if args.graph else _bench
    mode = "cuda-graph replay" if args.graph else "eager dispatch"
    print(f"device: {props.name} ({props.gcnArchName})  timing: {mode}  layout: {args.layout}")
    print(
    ...
    for num_heads in args.heads:
        for num_tokens in args.tokens:
            if args.graph and args.layers > 1:
                layers = [Inputs(num_tokens, num_heads, args.layout) for _ in range(args.layers)]
                eager_ms = _bench_graph_layers([functools.partial(triton_chain, i, False) for i in layers])
                triton_ms = _bench_graph_layers([functools.partial(triton_chain, i, True) for i in layers])
                fused_ms = _bench_graph_layers([functools.partial(fused, i) for i in layers])
                inp = layers[0]
                del layers
