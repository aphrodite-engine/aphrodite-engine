...
from aphrodite.models.kimi_k3.nvidia.ops.cute_dsl.gemm_rs_ar import GemmRsAr
...
def benchmark_shape(
    ...
    inputs = [torch.randn(M, K, dtype=torch.bfloat16, device=device, generator=rng) for _ in range(num_workspaces)]
    weights = [torch.randn(N, K, dtype=torch.bfloat16, device=device, generator=rng) for _ in range(num_workspaces)]
    ...
    for _ in range(num_workspaces):
        ...
        symm_collective_handles.append(symm_mem.rendezvous(collective_input, device_group))
    ...
    def make_torch_ring_ll_gemm_collective(x: torch.Tensor, weight: torch.Tensor) -> Callable[[], torch.Tensor]:
    ...
    def make_torch_ldmc_gemm_collective(x: torch.Tensor, weight: torch.Tensor) -> Callable[[], torch.Tensor]:
        def run() -> torch.Tensor:
    ...
    def make_fused_gemm_collective(x: torch.Tensor, weight: torch.Tensor) -> Callable[[], torch.Tensor]:
        def run() -> torch.Tensor:
    ...
    def make_ring_ll_collective(
        ...
        def run() -> torch.Tensor:
            ...
            dist.reduce_scatter_single(torch_output, collective_input, group=device_group)
            return torch_output
    ...
    def make_ldmc_collective(
        ...
        def run() -> torch.Tensor:
            if all_reduce:
                dist.all_reduce(collective_input, group=device_group)
                return collective_input
            dist.reduce_scatter_single(symm_output, collective_input, group=device_group)
            return symm_output
...
def print_results(results: list[dict[str, float | int | str]]) -> None:
    ...
    print(f"\nNCCL {collective} (best) is the faster of RING_LL and LDMC for each shape.\n")
