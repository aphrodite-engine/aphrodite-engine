...
NCCL_NVLS_ENABLE=1 NCCL_CUMEM_ENABLE=1 APHRODITE_USE_NCCL_SYMM_MEM=1, otherwise NCCL does
    ...
    def benchmark_allreduce(
        ...
        if self.pynccl_comm is not None:
            ...
            communicators.append(
                (
                    "pynccl-symm",
                    lambda t: torch.ops.aphrodite.all_reduce_symmetric_with_copy(t),
                    lambda t: True,  # Always available if initialized
                    nullcontext(),
                    None,  # no env variable needed
                )
        ...
        for name, allreduce_fn, should_use_fn, context, env_var in communicators:
            ...
            if env_var is not None:
                os.environ["APHRODITE_CUSTOM_ALLREDUCE_ALGO"] = env_var
            else:
                # Clear the environment variable to avoid interference
                os.environ.pop("APHRODITE_CUSTOM_ALLREDUCE_ALGO", None)
