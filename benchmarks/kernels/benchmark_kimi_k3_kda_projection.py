...
def main() -> None:
    ...
    packed_weights = [torch.randn(6288, 7168, dtype=torch.bfloat16, device="cuda") for _ in range(args.weight_sets)]
    f_b_weights = [torch.randn(1536, 128, dtype=torch.bfloat16, device="cuda") for _ in range(args.weight_sets)]
    ...
    for num_tokens in args.tokens:
        ...
        def sequential(
            ...
            return [
                ...
                for x, weight, f_b_weight in zip(hidden_states, packed_weights, f_b_weights)
            ]

        candidates: list[tuple[str, Callable[[], list[ProjectionOutput]]]] = [("sequential", sequential)]
        if num_tokens <= KDA_PROJECTION_OVERLAP_MAX_TOKENS:
            aux_stream = torch.cuda.Stream()
            events = [(torch.cuda.Event(), torch.cuda.Event()) for _ in range(args.weight_sets)]
            ...
            def overlap(
                ...
                return [
                    ...
                    for x, weight, f_b_weight, pair in zip(hidden_states, packed_weights, f_b_weights, events)
                ]
        ...
        for trial in range(args.trials):
            ...
            for name, graph in ordered_graphs:
                samples[name].append(_benchmark_graph(graph, args.weight_sets, args.repeats))
        ...
        multistream_text = f"{multistream_us:18.3f}" if multistream_us is not None else "                 -"
        fastest = "multistream" if multistream_us is not None and multistream_us < sequential_us else "sequential"
