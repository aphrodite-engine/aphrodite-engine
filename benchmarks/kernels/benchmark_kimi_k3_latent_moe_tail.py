...
from aphrodite.models.kimi_k3.nvidia.ops.cute_dsl.latent_moe_tail import (
...
def parse_up_projection_config(
    ...
    try:
        ...
        raise argparse.ArgumentTypeError("config must be BLOCK,OUTPUTS,K_UNROLL[,VECTOR_WIDTH[,PREFETCH_B]]") from error
    ...
    raise argparse.ArgumentTypeError(
        "config must be BLOCK,OUTPUTS,K_UNROLL[,VECTOR_WIDTH[,PREFETCH_B]], where PREFETCH_B is 0 or 1"
    )
...
def parse_tail_skinny_config(
    ...
    raise argparse.ArgumentTypeError(
        "config must be M,BLOCK,OUTPUTS,K_UNROLL[,VECTOR_WIDTH[,PREFETCH_B]], where PREFETCH_B is 0 or 1"
    )
...
def rotating_weight_count(
    ...
    properties = torch.cuda.get_device_properties(torch.accelerator.current_device_index())
...
def make_up_projection_launches(
    ...
    return [lambda weight=weight: launch(latent, weight, shared_shard) for weight in weights]
...
def benchmark_up_projection(args: argparse.Namespace) -> None:
    ...
    dynamic_kernel = DynamicKernel(shard_size, mailbox, shared_shard) if use_dynamic else None
...
def make_deferred_input(
    ...
    gemm2_permuted = (
        routed[:, None, :].expand(num_tokens, top_k, LATENT_SIZE).contiguous().view(num_tokens * top_k, LATENT_SIZE)
    )
...
def benchmark_whole_tail(args: argparse.Namespace) -> None:
    ...
    if args.skinny_max_num_tokens is not None and any(not 0 <= cutoff <= 8 for cutoff in args.skinny_max_num_tokens):
    ...
    routed_input_modes = ("finalized", "deferred") if args.routed_input == "both" else (args.routed_input,)
    fused_ops = []
    if use_fused:
                ...
                fused_ops.append(
                    (
                        ...
                        latent_moe_tail.KimiK3LatentMoETailOp.initialize(
                            ...
                            device=device,
                            rms_eps=RMS_EPS,
                            experts_per_token=(args.top_k if routed_input_mode == "deferred" else 0),
                        ),
                    )
                )
        cutedsl_warmup()
    ...
    for num_tokens in args.num_tokens:
        ...
        routed_inputs: dict[str, torch.Tensor | UnfinalizedMoEOutput] = {"finalized": routed}
        ...
        for cutoff, routed_input_mode, fused_op in fused_ops:
            ...
            if "reference" in result:
                speedup = result["reference"]["median_us"] / result[fused_key]["median_us"]
                if fused_key == "fused":
    ...
    report = {
        ...
        "skinny_configs": {str(num_tokens): asdict(config) for num_tokens, config in skinny_configs.items()},
        "timing_scope": {
            "reference": (
                "two input copies, two AllReduces, RMSNorm, full replicated up-projection GEMM, and final add"
            ),
            "fused": (
                "routed AllReduce/RMSNorm plus shared ReduceScatter, sharded up-projection/multicast, and Lamport copy"
            ),
        },
        "results": results,
    }
