...
from aphrodite import _custom_ops as aphrodite_ops
...
def embedding_fused(input_ids, weight, shard_indices):
    return aphrodite_ops.vocab_parallel_embedding(
        input_ids,
        weight,
        shard_indices.org_vocab_start_index,
        shard_indices.org_vocab_end_index,
...
def calculate_diff(vocab_size, hidden_size, tp_size, num_tokens):
    ...
    for tp_rank in range(tp_size):
        input_ids, weight, shard_indices = make_inputs(vocab_size, hidden_size, tp_size, num_tokens, dtype, tp_rank)
...
def get_benchmark(vocab_size, hidden_size, dtype, use_cuda_graph):
    ...
    def benchmark(tp_size, num_tokens, provider):
        input_ids, weight, shard_indices = make_inputs(vocab_size, hidden_size, tp_size, num_tokens, dtype)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hidden-size", type=int, default=4096, help="Embedding dimension")
    parser.add_argument("--dtype", type=str, default="bfloat16", help="Embedding table dtype")
    parser.add_argument(
        "--tp-size",
        type=int,
        default=4,
    ...
    benchmark = get_benchmark(args.vocab_size, args.hidden_size, dtype, args.use_cuda_graph)
