...
def benchmark_config(
    ...
    try:
        with override_ssm_config((block_size_m, num_warps_val)):
            ...
            graph = torch.cuda.CUDAGraph() if current_platform.is_cuda_alike() else torch.xpu.XPUGraph()
        ...
        if "OutOfResources" not in str(e):
            print(f"    Warning: config M={block_size_m},w={num_warps_val} raised {type(e).__name__}: {e}")
        return None
...
def expand_batch_x_nheads(
    ...
    if skipped_grid:
        print(f"  Note: skipping (batch, nheads) pairs exceeding CUDA grid dim {_CUDA_MAX_GRID_DIM}: {skipped_grid}")
    if skipped_ngroups:
        print(
    ...
    if skipped_eb:
        print(
            f"  Note: skipping (batch, nheads) pairs whose effective_batch exceeds {_MAX_EFFECTIVE_BATCH}: {skipped_eb}"
        )
...
def tune_dstate(
    ...
    for eb, batch, nheads in active:
        ...
        if not best_cfg:
            print(f"{eb:>8} | {'-':>7} | {'-':>5} | {'-':>10} | no working config (skipped)")
            continue

        if not verbose:
            print(f"{eb:>8} | {best_cfg['BLOCK_SIZE_M']:>7} | {best_cfg['num_warps']:>5} | {best_time:>10.2f} | best")
...
def compare_heuristic_vs_tuned(
    ...
    print(f"\n{'=' * 74}")
    print(f"Comparison  headdim={headdim}  dstate={dstate}  ngroups={ngroups}  —  heuristic vs tuned")
    print(f"Heuristic: BLOCK_SIZE_M={heur_cfg['BLOCK_SIZE_M']}, num_warps={heur_cfg['num_warps']}")
    print(f"{'=' * 74}")
    hdr = f"{'EffBatch':>8} | {'Heur(us)':>10} | {'Tuned(us)':>10} | {'Speedup':>8} | Best config"
    print(hdr)
    print("-" * len(hdr))
...
def save_results(device_name: str, output: str, results_file: str | None = None) -> str:
    ...
    if results_file is None:
        results_file = os.path.join(_RESULTS_DIR, f"ssm_benchmark_results_{device_name}.txt")
...
def main():
    parser = argparse.ArgumentParser(description="Tune selective_state_update kernel for Mamba SSM")
    parser.add_argument(
        "--dstate",
        type=int,
        default=128,
        ...
        for dstate in dstates:
            ...
            if args.validate:
                ...
                if failed:
                    ...
                    tuned = {eb: cfg for eb, cfg in tuned.items() if validity.get(eb, True)}
