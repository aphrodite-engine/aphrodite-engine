...
def bench(
    ...
    print(f"{'seq_len':>8} {'BF16 ms':>12} {'FP8 ms':>12} {'speedup':>10} {'FP8/BF16':>12}")
    print("-" * 60)
    for seq_len in seq_lens:
        ...
        bf16_ms = triton.testing.do_bench(
            lambda q=q, k=k, v=v, cu=cu_seqlens, ms=max_seqlen: (bf16_attention._forward_fa(q, k, v, cu, ms)),
            warmup=warmup_ms,
            rep=repeat_ms,
        )
        fp8_ms = triton.testing.do_bench(
            lambda q=q, k=k, v=v, cu=cu_seqlens, ms=max_seqlen: (fp8_attention._forward_aiter_fp8(q, k, v, cu, ms)),
            warmup=warmup_ms,
            rep=repeat_ms,
        )
        speedup = bf16_ms / fp8_ms
        ratio = fp8_ms / bf16_ms
        print(f"{seq_len:>8} {bf16_ms:>12.3f} {fp8_ms:>12.3f} {speedup:>9.2f}x {ratio:>12.3f}")


if __name__ == "__main__":
    parser = FlexibleArgumentParser(description="Benchmark AITER BF16 vs FP8 ViT attention.")
    parser.add_argument("--seq-lens", type=int, nargs="+", default=[2304, 4096, 8192, 16384])
