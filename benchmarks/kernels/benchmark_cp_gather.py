...
def make_page_table(
    ...
    blocks_per_req = [math.ceil((start + length) / block_size) for start, length in zip(seq_starts, seq_lens)]
    total_blocks = sum(blocks_per_req)
    block_table = torch.zeros((len(seq_lens), max(blocks_per_req)), dtype=torch.int32, device="cuda")
    ...
    for req_id, num_blocks in enumerate(blocks_per_req):
        block_table[req_id, :num_blocks] = physical_blocks[block_offset : block_offset + num_blocks]
        block_offset += num_blocks

    cu_seq_lens = torch.zeros(len(seq_lens) + 1, dtype=torch.int32, device="cuda")
    cu_seq_lens[1:] = torch.tensor(seq_lens, dtype=torch.int32, device="cuda").cumsum(dim=0)
...
def make_cache_gather(
    ...
    seq_starts_list = [13 + (17 * req_id) % block_size for req_id in range(len(seq_lens))]
    block_table, cu_seq_lens, total_blocks = make_page_table(seq_lens, block_size, seq_starts_list)
    src_cache = torch.empty((total_blocks, block_size, entry_size), dtype=dtype, device="cuda")
...
def make_fp8_upconvert(
    ...
    dst = torch.empty((sum(seq_lens), output_elements), dtype=torch.bfloat16, device="cuda")
    seq_starts = torch.tensor(seq_starts_list, dtype=torch.int32, device="cuda")
...
def run_scenario(
    ...
    latency_ms = triton.testing.do_bench(run, warmup=warmup_ms, rep=rep_ms, return_mode="median")
...
def main() -> None:
    ...
    variants = ("cache", "fp8-upconvert", "maybe-dequant") if args.variant == "all" else (args.variant,)
