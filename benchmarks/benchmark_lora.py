...
if HAS_TRITON:
    from aphrodite.lora.ops.triton_ops import (  ## added fused_moe_lora
        LoRAKernelMeta,
...
class OpType(Enum):
    ...
    def matmul_shapes_fused_moe_lora(
        ...
        if self.is_fused_moe_lora_shrink_fn():
            input_shape = (m * top_k_num, n) if self in [OpType.FUSED_MOE_LORA_DOWN_SHRINK] else (m, n)
...
class BenchmarkTensors:
    ...
    def get_num_tokens(self, size: int, top_k_num: int, op_type: OpType):
        return size * top_k_num if op_type in [OpType.FUSED_MOE_LORA_DOWN_SHRINK] else size
    ...
    def sanity_check(self, ctx: BenchmarkContext, op_type: OpType) -> None:
        ...
        """
        num_tokens = self.input.shape[1] if op_type.is_fused_moe_lora_expand_fn() else self.input.shape[-2]
        # check metadata tensors
        ## In down shrink case, each token is repeated top_k_num times
        assert num_tokens == self.get_num_tokens(torch.sum(self.seq_lens), ctx.top_k_num, op_type), (
        ...
        assert self.get_num_tokens(self.lora_kernel_meta.token_lora_mapping.shape[0], ctx.top_k_num, op_type)
    ...
    def metadata(self, ctx: BenchmarkContext, op_type: OpType) -> tuple[int, int, int]:
        ...
        """
        num_seqs = self.seq_lens.shape[0]
        num_tokens = self.get_num_tokens(self.lora_kernel_meta.token_lora_mapping.shape[0], ctx.top_k_num, op_type)
    ...
    def fused_moe_lora_data_prepare(
        ...
        (sorted_token_ids_lora, expert_ids_lora, num_tokens_post_padded_lora) = moe_lora_align_block_size(
    ...
    def as_lora_shrink_kwargs(self, ctx: BenchmarkContext, op_type: OpType) -> dict[str, Any]:
    ...
    def as_lora_expand_kwargs(self, ctx: BenchmarkContext, op_type: OpType, add_inputs: bool) -> dict[str, Any]:
        self.sanity_check(ctx, op_type)
        self.to_device(self.input.device)
    ...
    def as_fused_moe_lora_shrink_kwargs(self, ctx: BenchmarkContext, op_type: OpType) -> dict[str, Any]:
        self.sanity_check(ctx, op_type)
        self.to_device(self.input.device)
        ...
        assert (
            o_shape == (num_slices, num_tokens // ctx.top_k_num, ctx.top_k_num, lora_rank)
        ...
        (topk_weights, sorted_token_ids, expert_ids, num_tokens_post_padded) = self.fused_moe_lora_data_prepare(
    ...
    def as_fused_moe_lora_expand_kwargs(self, ctx: BenchmarkContext, op_type: OpType) -> dict[str, Any]:
        self.sanity_check(ctx, op_type)
        self.to_device(self.input.device)
    ...
    def bench_fn_kwargs(self, ctx: BenchmarkContext, op_type: OpType, add_inputs: bool | None = None) -> dict[str, Any]:
...
def bench_optype(
    ...
    if test_correctness:
        ...
        assert all([bt.test_correctness(ctx, op_type, expand_fn_add_inputs) for bt in bench_tensors])

    # BenchmarkTensors -> dict (kwargs)
    kwargs_list = [bt.bench_fn_kwargs(ctx, op_type, add_inputs=expand_fn_add_inputs) for bt in bench_tensors]
