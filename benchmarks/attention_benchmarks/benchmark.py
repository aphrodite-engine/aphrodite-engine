...
"""
Universal Aphrodite Attention Benchmark
...
def run_model_parameter_sweep(
    ...
    sweep_desc = f"{sweep.param_name} = {sweep.values}" if sweep.param_name else f"{len(sweep.values)} configurations"
        ...
        for backend in backends:
            for spec in batch_specs:
                for value in sweep.values:
                    ...
                    clean_config = BenchmarkConfig(backend=backend, batch_spec=spec, **config_args)
                    ...
                    if not result.success:
                        err_label = f"{sweep.param_name}={value}" if sweep.param_name else f"{value}"
                        console.print(f"[red]Error {backend} {spec} {err_label}: {result.error}[/]")
    ...
    sorted_param_values = sorted(by_param_value.keys(), key=lambda x: int(x) if x.isdigit() else x)

    for param_value in sorted_param_values:
        label = f"{sweep.param_name} = {param_value}" if sweep.param_name else param_value
        console.print(f"\n[bold cyan]{label}[/]")
    ...
    for param_value, spec in sorted_keys:
        ...
        if param_value != current_param_value:
            header = f"{sweep.param_name}={param_value}" if sweep.param_name else param_value
            console.print(f"\n  [bold]{header}:[/]")
        ...
        console.print(f"    {spec:12s} -> [bold green]{backend_name:15s}[/] ({times_str})")
...
def run_parameter_sweep(
        ...
        for backend in backends:
            for spec in batch_specs:
                for value in sweep_values:
                    # Create config with original backend for running
                    config = BenchmarkConfig(backend=backend, batch_spec=spec, **base_config_args)
                    ...
                    if not result.success:
                        console.print(f"[red]Error {backend} {spec} {sweep.param_name}={value}: {result.error}[/]")
    ...
    for spec in sorted(by_spec.keys(), key=batch_spec_sort_key):
        ...
        best = min(results, key=lambda r: r.mean_time)
        console.print(f"  {spec}: [bold green]{best.config.backend}[/] ({best.mean_time:.6f}s)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Universal Aphrodite attention benchmark",
    ...
    parser.add_argument(
        "--prefill-backends",
        nargs="+",
        help="Prefill backends to compare (fa2, fa3, fa4). Uses the first decode backend for impl construction.",
    )
    parser.add_argument(
        "--fp8-output-scale",
        type=float,
    ...
    parser.add_argument("--kv-lora-rank", type=int, default=None, help="MLA KV LoRA rank")
    parser.add_argument("--qk-nope-head-dim", type=int, default=None, help="MLA non-RoPE QK head dim")
    parser.add_argument("--qk-rope-head-dim", type=int, default=None, help="MLA RoPE QK head dim")
    ...
    console = Console()
    console.print("[bold cyan]Aphrodite Attention Benchmark[/]")
    ...
    if args.config:
        ...
        if not cli_batch_specs_provided:
            if "batch_spec_ranges" in yaml_config:
                ...
                generated_specs = generate_batch_specs_from_ranges(yaml_config["batch_spec_ranges"])
                ...
                console.print(f"[dim]Generated {len(generated_specs)} batch specs from ranges[/]")
        ...
        if "model" in yaml_config:
            ...
            args.max_model_len = model.get("max_model_len", getattr(args, "max_model_len", None))
        ...
        args.sparse_mla_topk_pattern = yaml_config.get("sparse_mla_topk_pattern", "random")
        args.sparse_mla_dense_mha_max_seq_len = yaml_config.get("sparse_mla_dense_mha_max_seq_len", None)
        args.sparse_mla_mha_variants = yaml_config.get("sparse_mla_mha_variants", args.sparse_mla_mha_variants)
        args.sparse_mla_masked_mha_max_seq_len = yaml_config.get("sparse_mla_masked_mha_max_seq_len", None)
        ...
        if "parameter_sweep" in yaml_config:
            ...
            args.parameter_sweep = ParameterSweep(
                ...
                label_format=sweep_config.get("label_format", "{backend}_{param_name}_{value}"),
            )
        else:
    ...
    if fp8_output_scale is not None:
        ...
        console.print(
            f"[yellow]FP8 output comparison @ scale={fp8_output_scale} (prefill=fa4, decode impl={decode_backend})[/]"
        )
        fp8_results = []
        ...
        with tqdm(total=total, desc="FP8 output benchmarking") as pbar:
            for spec in args.batch_specs:
                for fuse in fuse_variants:
                    ...
                    result = run_benchmark(config, output_scale=fp8_output_scale, fuse_quant_op=fuse)
                    label = label_of[fuse]
    ...
    # Handle special mode: decode_vs_prefill comparison
        ...
        console.print("[dim]For each query length, testing both decode and prefill pipelines[/]")
        ...
        with tqdm(total=total, desc="Benchmarking") as pbar:
            for batch_size in batch_sizes:
                ...
                for spec in args.batch_specs:
                    ...
                    if not requests:
                        console.print(f"[red]Error: Could not parse batch spec '{spec}'[/]")
                        continue
                    ...
                    if query_length > 1:
                        ...
                        configs_with_thresholds.append((config_prefill, prefill_threshold))
                ...
                try:
                    ...
                    for (config, _), timing in zip(configs_with_thresholds, timing_results):
                    ...
                    console.print(f"[red]Error running batched benchmarks for batch_size={batch_size}: {e}[/]")
        ...
        for bs in sorted(by_batch_size.keys()):
            ...
            for qlen in sorted(by_qlen.keys()):
                ...
                if "decode" in pipelines and "prefill" in pipelines:
                    ...
                    speedup = prefill_time / decode_time if decode_time < prefill_time else decode_time / prefill_time
            ...
            if last_decode_faster is not None:
                ...
                console.print(f"\n  [bold green]Optimal threshold for batch_size={bs}: {optimal_threshold}[/]")
                console.print(f"  [dim](Use decode pipeline for query_length <= {optimal_threshold})[/]")
            else:
    ...
    # Handle MHA vs MQA comparison mode for sparse MLA
        ...
        if requested_variants is not None:
            ...
            variants = [variant for variant in available_variants if variant[0] in requested_variant_set]
        else:
        ...
        if model_sweep is None:
            ...
            raise ValueError("mha_vs_mqa mode only supports a num_q_heads model parameter sweep")
        benchmark_specs = [(spec, num_q_heads) for spec in args.batch_specs for num_q_heads in num_q_heads_values]
        masked_mha_max_seq_len = getattr(args, "sparse_mla_masked_mha_max_seq_len", None)
        formatter = ResultsFormatter(console)
        total = 0
        for spec, _ in benchmark_specs:
            ...
            for variant_label, _, _ in variants:
                if (
                    variant_label == "dense_mha" and dense_mha_max_seq_len is not None and q_len > dense_mha_max_seq_len
                ) or (
                ...
                for backend in backends:
                    for variant_label, force_mqa, mha_mode in variants:
                        ...
                        if model_sweep is not None:
                            result_backend = model_sweep.get_label(result_backend, num_q_heads)
                        config = BenchmarkConfig(
                        ...
                        if not result.success:
                            console.print(f"[red]Error {backend}_{variant_label} {spec}: {result.error}[/]")
    ...
    # Handle parameter sweep mode (unified)
        ...
        all_results = run_parameter_sweep(backends, args.batch_specs, base_config_args, args.parameter_sweep, console)
        ...
        if prefill_backends:
            ...
            console.print(f"[yellow]Prefill comparison mode: using {decode_backend} for decode impl[/]")
