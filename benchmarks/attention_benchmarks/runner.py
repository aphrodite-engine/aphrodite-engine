...
(FlashAttention, Triton, FlashInfer) with real Aphrodite integration.
"""
...
def _get_backend_config(backend: str) -> dict:
    ...
    try:
        ...
        raise ValueError(f"Unknown backend: {backend}. Valid backends: {valid_backends}") from e
...
def log_warnings_and_errors_only():
    """Temporarily set Aphrodite logger to WARNING level."""
...
def _build_common_attn_metadata(
    ...
    query_start_loc[1:] = torch.tensor(q_lens, dtype=torch.int32, device=device).cumsum(0)
    ...
    block_table_tensor = torch.arange(num_blocks, dtype=torch.int32, device=device).view(batch_size, max_blocks)
...
def _create_aphrodite_config(
    ...
    """Create a AphroditeConfig for benchmarking with mock model methods."""
    ...
    model_config.get_num_layers = types.MethodType(lambda self: config.num_layers, model_config)
    ...
    model_config.get_sm_scale_for_layer = types.MethodType(lambda self, i: 1.0 / config.head_dim**0.5, model_config)
    ...
    model_config.get_head_size = types.MethodType(lambda self: config.head_dim, model_config)
...
def _create_input_tensors(
    ...
    v_dim = config.v_head_dim if config.v_head_dim is not None else config.head_dim
    q_list = [
        torch.randn(total_q, config.num_q_heads, config.head_dim, device=device, dtype=dtype).to(q_dtype)
        for _ in range(config.num_layers)
    ]
    k_list = [
        torch.randn(total_q, config.num_kv_heads, config.head_dim, device=device, dtype=dtype)
        for _ in range(config.num_layers)
    ]
    v_list = [
        torch.randn(total_q, config.num_kv_heads, v_dim, device=device, dtype=dtype) for _ in range(config.num_layers)
    ]
...
def _create_kv_cache(
    ...
    total_bytes = prod(compute_layer_kv_cache_shape_bytes(spec, max_num_blocks)) * config.num_layers
    buf = torch.zeros(total_bytes, device=device, dtype=torch.int8)
    layer_stride, block_stride, _, _, _ = compute_layout_strides(spec, max_num_blocks, config.num_layers, layout)
...
def run_attention_benchmark(config: BenchmarkConfig) -> BenchmarkResult:
    ...
    # Suppress Aphrodite logs during setup to reduce spam
    with log_warnings_and_errors_only():
        # Create aphrodite_config first - uses model's native dtype via "auto"
        aphrodite_config = _create_aphrodite_config(config, max_num_blocks)
        dtype = aphrodite_config.model_config.dtype

        # Wrap everything in set_current_aphrodite_config context
        # This is required for backends like flashinfer that need global config
        with set_current_aphrodite_config(aphrodite_config):
            backend_class, impl, layer = _create_backend_impl(backend_cfg, config, device, dtype)
            ...
            layout = resolve_kv_cache_layout(aphrodite_config, [[m.name for m in supported]])

            common_metadata = _build_common_attn_metadata(q_lens, kv_lens, config.block_size, device)
            ...
            builder = _create_metadata_builder(backend_class, kv_cache_spec, aphrodite_config, device, config.backend)
            ...
            if config.num_splits is not None and hasattr(attn_metadata, "max_num_splits"):
            ...
            cache_list = _create_kv_cache(config, max_num_blocks, backend_class, device, dtype, layout)
