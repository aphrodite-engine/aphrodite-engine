...
needing full AphroditeConfig integration.
"""
...
def _create_kv_b_proj(
    ...
    kv_b_proj = ColumnParallelLinear(
        ...
        mla_dims["num_q_heads"] * (mla_dims["qk_nope_head_dim"] + mla_dims["v_head_dim"]),
        bias=False,
...
def _ensure_single_rank_model_parallel() -> None:
    ...
    if not dist.is_initialized():
        with tempfile.NamedTemporaryFile(prefix="aphrodite_bench_dist_", delete=False) as init_file:
...
def _add_mock_methods_to_model_config(model_config: ModelConfig) -> None:
    ...
    model_config.get_sliding_window_for_layer = types.MethodType(lambda self, _i: None, model_config)
    model_config.get_logits_soft_cap_for_layer = types.MethodType(lambda self, _i: None, model_config)
    model_config.get_sm_scale_for_layer = types.MethodType(
        lambda self, _i: 1.0 / model_config.get_head_size() ** 0.5, model_config
    )


def create_minimal_aphrodite_config(
    ...
    """
    Create minimal AphroditeConfig for MLA benchmarks.
    ...
    Returns:
        AphroditeConfig for benchmarking
    """
    ...
    temp_dir = tempfile.mkdtemp(prefix="aphrodite_bench_")
    ...
    if prefill_backend is not None:
        ...
        aphrodite_config.attention_config.mla_prefill_backend = prefill_cfg["mla_prefill_backend"]
        if prefill_cfg["flash_attn_version"] is not None:
            aphrodite_config.attention_config.flash_attn_version = prefill_cfg["flash_attn_version"]

    if sparse_mla_force_mqa:
        aphrodite_config.attention_config.sparse_mla_force_mqa = True
...
def get_prefill_backend_config(prefill_backend: str) -> dict:
    ...
    if prefill_backend not in _PREFILL_BACKEND_CONFIG:
        raise ValueError(
            f"Unknown prefill backend: {prefill_backend!r}. Available: {list(_PREFILL_BACKEND_CONFIG.keys())}"
        )
...
def _get_backend_config(backend: str) -> dict:
    ...
    try:
        ...
        raise ValueError(
            f"Unknown backend: {backend}. Valid MLA backends: {[b for b in valid_backends if 'MLA' in b]}"
        ) from e
...
def _build_attention_metadata(
    ...
    # Build slot mapping
    slot_mapping_list = []
    for i, (q_len, kv_len, num_blocks) in enumerate(zip(q_lens, kv_lens, num_blocks_per_req)):
...
def _create_backend_impl(
    ...
    Args:
        ...
        mla_dims: MLA dimension configuration
        aphrodite_config: AphroditeConfig instance
        device: Target device
    ...
    kv_cache_spec = FullAttentionSpec(
        block_size=backend_cfg["block_size"] or aphrodite_config.cache_config.block_size,
    ...
    if builder_class:
        ...
        aphrodite_config.compilation_config.static_forward_context = {"placeholder": layer}
...
def _run_single_benchmark(
    ...
    # Build metadata
    metadata, num_blocks = _build_attention_metadata(requests, block_size, device, builder_instance)
    ...
    num_decode = metadata.num_decode_tokens if (has_decode and has_prefill) else total_q if has_decode else 0
    ...
    quantize_query = is_fp8_kvcache and getattr(impl, "supports_quant_query_input", False)
    ...
    if has_decode:
        decode_inputs, _ = _create_input_tensors(num_decode, mla_dims, query_fmt, device, torch.bfloat16)
    ...
    if has_prefill:
        _, prefill_inputs = _create_input_tensors(num_prefill, mla_dims, query_fmt, device, torch.bfloat16)
    ...
    if has_prefill and output_scale is not None:
        ...
        prefill_output_scale = torch.tensor([output_scale], device=device, dtype=torch.float32)
        if fuse_quant_op:
            prefill_fp8_output = torch.empty_like(prefill_inputs["output"], dtype=current_platform.fp8_dtype())
        else:
    ...
    def forward_fn():
        ...
        if has_prefill:
            ...
            if fused_output:
                ...
                out, _ = prefill_quant_op(prefill_inputs["output"], prefill_output_scale)
            results.append(out)
    ...
    if config.torch_profile:
        profile_dir = Path(config.torch_profile_dir or "benchmark_outputs/torch_profiles")
...
def _run_mla_benchmark_batched(
    ...
    max_total_q = max(sum(r.q_len for r in parse_batch_spec(cfg.batch_spec)) for cfg, *_ in configs_with_params)
    max_model_len = max(
        max_total_q,
        max(getattr(cfg, "max_model_len", None) or 32768 for cfg, *_ in configs_with_params),
    )

    # Create and set Aphrodite config for MLA (reused across all benchmarks)
    aphrodite_config = create_minimal_aphrodite_config(
    ...
    with set_current_aphrodite_config(aphrodite_config):
        ...
        if prefill_backend is not None:
            ...
            if prefill_backend in {"fa2", "fa3", "fa4"}:
                ...
                actual_fa_version = getattr(layer.prefill_backend, "flash_attn_version", None)
        ...
        for config, threshold, num_splits in configs_with_params:
            force_masked_mha = is_sparse and getattr(config, "sparse_mla_mha_mode", "auto") == "masked"
            if force_masked_mha:
                ...
                mask_shape = _topk_mask_shape(len(requests), max_query_len, max_query_len)
                mask_words = max(
