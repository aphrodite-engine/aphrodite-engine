...
def parse_args():
    ...
    parser.add_argument("--max-concurrent", type=int, default=100, help="Maximum concurrent requests to backend services")
    ...
    parser.add_argument("--prefill-url", type=str, default="http://localhost:8100/v1/completions", help="Prefill service endpoint URL")
    parser.add_argument("--decode-url", type=str, default="http://localhost:8200/v1/completions", help="Decode service endpoint URL")
...
def main():
    ...
    async def process_request():
        try:
            ...
            logger.info("   Pulling KV from: %s (%s:%d)", 
                       PREFILL_ENGINE_ID, PREFILL_NIXL_HOST, PREFILL_NIXL_PORT)
    ...
    logger.info("Prefill: %s (engine_id=%s, nixl=%s:%d)", 
                PREFILL_SERVICE_URL, PREFILL_ENGINE_ID, PREFILL_NIXL_HOST, PREFILL_NIXL_PORT)
    logger.info("Decode: %s (engine_id=%s, nixl=%s:%d)", 
                DECODE_SERVICE_URL, DECODE_ENGINE_ID, DECODE_NIXL_HOST, DECODE_NIXL_PORT)
