...
try:
    ...
    from aphrodite.kernels.helion import get_kernel_by_name, get_registered_kernels
    from aphrodite.logger import init_logger
    from aphrodite.utils.import_utils import has_helion
except ImportError as e:
    print(f"Error importing Aphrodite: {e}")
    print("Please ensure Aphrodite is installed and in your Python path")
    sys.exit(1)
...
def import_all_kernels() -> None:
    ...
    Current Aphrodite registers every Helion kernel as a side effect of importing
    ``aphrodite.kernels.helion.ops``; some builds instead expose an explicit importer
    ...
    """
    try:
        import aphrodite.kernels.helion.ops as ops
    except ImportError:
        return
...
logger = init_logger("aphrodite.scripts.benchmark_helion_kernels")
...
def make_cuda_baseline(kernel_name: str) -> Callable:
    ...
    if cuda_op is None:
        logger.error(
            "torch.ops._C.%s is not available. Ensure the Aphrodite C extension is built and loaded.",
            cuda_op_name,
        )
        sys.exit(1)
...
def _assert_close(actual: object, expected: object, atol: float, rtol: float) -> None:
    ...
    if actual_spec != expected_spec:
        raise AssertionError(f"Output structure mismatch: {actual_spec} != {expected_spec}")

    for actual_leaf, expected_leaf in zip(actual_flat, expected_flat, strict=True):
        is_fp8 = isinstance(actual_leaf, torch.Tensor) and is_fp8_dtype(actual_leaf.dtype)
        helion_assert_close(
            actual_leaf,
            expected_leaf,
...
def do_bench_cudagraph_l2_clear(fn: Callable, rep: int = 100, return_mode: str = "mean") -> float:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=("Compare numerics against the selected performance baseline instead of the eager reference"),
    )
            ...
            json.dump(
                {
                    ...
                    "numerics_baseline": (args.baseline if args.numerics_with_perf_baseline else "eager"),
