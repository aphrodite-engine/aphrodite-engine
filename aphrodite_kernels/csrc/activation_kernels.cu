...
__global__ void act_and_mul_kernel(
  ...
  if (aligned && d >= VEC_SIZE) {
    ...
    for (int i = threadIdx.x; i < num_vecs; i += blockDim.x) {
      int4 x = APHRODITE_LDG(&x_vec[i]), y = APHRODITE_LDG(&y_vec[i]), r;
...
#pragma unroll
    ...
    for (int i = vec_end + threadIdx.x; i < d; i += blockDim.x) {
      out_ptr[i] = compute<scalar_t, ACT_FN, act_first>(
          APHRODITE_LDG(&x_ptr[i]), APHRODITE_LDG(&y_ptr[i]));
    }
  } else {
...
#pragma unroll
    ...
    for (int i = vec_end + threadIdx.x; i < d; i += blockDim.x) {
      out_ptr[i] =
          ACT_FN(APHRODITE_LDG(&x_ptr[i]), param) * APHRODITE_LDG(&y_ptr[i]);
    }
  } else {
    // Scalar fallback for unaligned data or small d
    for (int64_t idx = threadIdx.x; idx < d; idx += blockDim.x) {
...
__global__ void activation_kernel(
  ...
  if (aligned && d >= VEC_SIZE) {
    ...
    for (int i = threadIdx.x; i < num_vecs; i += blockDim.x) {
      int4 v = APHRODITE_LDG(&in_vec[i]), r;
      auto* vp = reinterpret_cast<scalar_t*>(&v);
      auto* rp = reinterpret_cast<scalar_t*>(&r);
#pragma unroll
      for (int j = 0; j < VEC_SIZE; j++) {
    ...
    // Scalar cleanup for remaining elements
    for (int i = vec_end + threadIdx.x; i < d; i += blockDim.x) {
      out_ptr[i] = ACT_FN(APHRODITE_LDG(&in_ptr[i]));
    }
  } else {
    // Scalar fallback for unaligned data or small d
    for (int64_t idx = threadIdx.x; idx < d; idx += blockDim.x) {
