...
std::tuple<torch::Tensor, torch::Tensor> grouped_topk(
  ...
  TORCH_CHECK(scoring_func == aphrodite::moe::SCORING_NONE ||
...
#define LAUNCH_KERNEL(T, IdxT)                                               \
  do {                                                                       \
    switch (bias_type) {                                                     \
      case torch::kFloat16:                                                  \
        aphrodite::moe::invokeNoAuxTc<T, half, IdxT>(                        \
        ...
        break;                                                               \
      case torch::kFloat32:                                                  \
        aphrodite::moe::invokeNoAuxTc<T, float, IdxT>(                       \
            reinterpret_cast<T*>(scores.mutable_data_ptr()),                 \
            reinterpret_cast<T*>(group_scores.mutable_data_ptr()),           \
            reinterpret_cast<float*>(topk_values.mutable_data_ptr()),        \
            reinterpret_cast<IdxT*>(topk_indices.mutable_data_ptr()),        \
      ...
      case torch::kBFloat16:                                                 \
        aphrodite::moe::invokeNoAuxTc<T, __nv_bfloat16, IdxT>(               \
            reinterpret_cast<T*>(scores.mutable_data_ptr()),                 \
            reinterpret_cast<T*>(group_scores.mutable_data_ptr()),           \
            reinterpret_cast<float*>(topk_values.mutable_data_ptr()),        \
            reinterpret_cast<IdxT*>(topk_indices.mutable_data_ptr()),        \
