#ifndef APHRODITE_MOBILE_THREADING_HPP
#define APHRODITE_MOBILE_THREADING_HPP
...
inline float16x8_t accum_f16_dot(float16x8_t acc, float16x8_t a_low,
                                 float16x8_t a_high, float16x8_t b_low,
                                 float16x8_t b_high) {
  acc = vfmaq_f16(acc, a_low, b_low);
  return vfmaq_f16(acc, a_high, b_high);
}

inline float32x4_t accum_f32_dot(float32x4_t acc, float32x4_t a_low,
                                 float32x4_t a_high, float32x4_t b_low,
                                 float32x4_t b_high) {
 ...
 public:
  ...
  void wait_all() {
    ...
    finish_condition.wait(
        lock, [this] { return tasks.empty() && active_workers == 0; });
  }
...
inline size_t get_optimal_thread_count(size_t total_work,
                                       size_t min_work_per_thread) {
  if (total_work < min_work_per_thread) return 1;
  size_t pool_size = get_thread_pool().num_workers();
  return std::min(pool_size, std::max(static_cast<size_t>(1),
                                      total_work / min_work_per_thread));
}
...
class TaskHandle {
 ...
 public:
  ...
  void add_future(std::future<void>&& f) { futures_.push_back(std::move(f)); }
...
TaskHandle parallel_for(size_t total_work, size_t threshold, WorkFunc work_func,
                        bool wait = true) {
  ...
  if (num_threads == 1) {
    ...
    handle.add_future(
        pool.enqueue([work_func, total_work]() { work_func(0, total_work); }));
    return handle;
  }
  ...
  for (size_t t = 0; t < num_threads; ++t) {
    handle.add_future(pool.enqueue(
        [work_func, t, num_threads, work_per_thread, total_work]() {
          const size_t start_idx = t * work_per_thread;
          const size_t end_idx =
              (t == num_threads - 1) ? total_work : (t + 1) * work_per_thread;
...
template <typename WorkFunc>
void parallel_for_2d(size_t outer_size, size_t inner_size, size_t threshold,
                     WorkFunc work_func) {
...
ResultType parallel_reduce(size_t total_work, size_t threshold,
                           WorkFunc work_func, ResultType init_value,
  ...
  for (size_t t = 0; t < num_threads; ++t) {
    futures.push_back(pool.enqueue([work_func, t, num_threads, work_per_thread,
                                    total_work]() -> ResultType {
      const size_t start_idx = t * work_per_thread;
      const size_t end_idx =
          (t == num_threads - 1) ? total_work : (t + 1) * work_per_thread;
...
inline size_t compute_gemm_parallelism(size_t M, size_t K, size_t N,
                                       size_t element_size) {
  ...
  size_t cache_tiles = (bytes_accessed + Thresholds::L2_CACHE_SIZE - 1) /
                       Thresholds::L2_CACHE_SIZE;

  size_t compute_threads =
      std::sqrt(static_cast<double>(total_ops) / Thresholds::GEMM_SMALL);
...
template <typename WorkFunc>
void parallel_for_2d_tiled(size_t rows, size_t cols, size_t tile_rows,
                           size_t tile_cols, WorkFunc work_func) {
  ...
  parallel_for(total_tiles, Thresholds::GEMM_TILED,
               [=](size_t start_tile, size_t end_tile) {
                 for (size_t tile_idx = start_tile; tile_idx < end_tile;
                      ++tile_idx) {
...
#endif  // APHRODITE_MOBILE_THREADING_HPP