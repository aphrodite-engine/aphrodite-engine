...
static void matmul_int8_worker(const int8_t* a, const int8_t* b_transposed,
                               int8_t* c, size_t M, size_t K, size_t N,
                               size_t start_row, size_t end_row, float a_scale,
                               float b_scale, float c_scale) {
  ...
  for (size_t row_block = start_row; row_block < end_row; row_block += TILE_M) {
    for (size_t col_block = 0; col_block < N; col_block += TILE_N) {
      ...
      for (int m = 0; m < TILE_M; ++m)
        for (int n = 0; n < TILE_N; ++n) accumulators[m][n] = vdupq_n_s32(0);

      for (size_t k_block = 0; k_block < K_aligned;
           k_block += VECTOR_WIDTH * 2) {
        ...
        for (int n = 0; n < TILE_N; ++n) {
          ...
          if (col < N) {
            ...
            b_vec_high[n] =
                vld1q_s8(&b_transposed[col * K + k_block + VECTOR_WIDTH]);
          } else {
        ...
        accumulators[0][0] =
            accum_i8mm(accumulators[0][0], a_vec_low[0], b_vec_low[0]);
        accumulators[0][1] =
            accum_i8mm(accumulators[0][1], a_vec_low[0], b_vec_low[1]);
        accumulators[0][2] =
            accum_i8mm(accumulators[0][2], a_vec_low[0], b_vec_low[2]);
        accumulators[0][3] =
            accum_i8mm(accumulators[0][3], a_vec_low[0], b_vec_low[3]);
        accumulators[1][0] =
            accum_i8mm(accumulators[1][0], a_vec_low[1], b_vec_low[0]);
        accumulators[1][1] =
            accum_i8mm(accumulators[1][1], a_vec_low[1], b_vec_low[1]);
        accumulators[1][2] =
            accum_i8mm(accumulators[1][2], a_vec_low[1], b_vec_low[2]);
        accumulators[1][3] =
            accum_i8mm(accumulators[1][3], a_vec_low[1], b_vec_low[3]);
        accumulators[2][0] =
            accum_i8mm(accumulators[2][0], a_vec_low[2], b_vec_low[0]);
        accumulators[2][1] =
            accum_i8mm(accumulators[2][1], a_vec_low[2], b_vec_low[1]);
        accumulators[2][2] =
            accum_i8mm(accumulators[2][2], a_vec_low[2], b_vec_low[2]);
        accumulators[2][3] =
            accum_i8mm(accumulators[2][3], a_vec_low[2], b_vec_low[3]);
        accumulators[3][0] =
            accum_i8mm(accumulators[3][0], a_vec_low[3], b_vec_low[0]);
        accumulators[3][1] =
            accum_i8mm(accumulators[3][1], a_vec_low[3], b_vec_low[1]);
        accumulators[3][2] =
            accum_i8mm(accumulators[3][2], a_vec_low[3], b_vec_low[2]);
        accumulators[3][3] =
            accum_i8mm(accumulators[3][3], a_vec_low[3], b_vec_low[3]);

        accumulators[0][0] =
            accum_i8mm(accumulators[0][0], a_vec_high[0], b_vec_high[0]);
        accumulators[0][1] =
            accum_i8mm(accumulators[0][1], a_vec_high[0], b_vec_high[1]);
        accumulators[0][2] =
            accum_i8mm(accumulators[0][2], a_vec_high[0], b_vec_high[2]);
        accumulators[0][3] =
            accum_i8mm(accumulators[0][3], a_vec_high[0], b_vec_high[3]);
        accumulators[1][0] =
            accum_i8mm(accumulators[1][0], a_vec_high[1], b_vec_high[0]);
        accumulators[1][1] =
            accum_i8mm(accumulators[1][1], a_vec_high[1], b_vec_high[1]);
        accumulators[1][2] =
            accum_i8mm(accumulators[1][2], a_vec_high[1], b_vec_high[2]);
        accumulators[1][3] =
            accum_i8mm(accumulators[1][3], a_vec_high[1], b_vec_high[3]);
        accumulators[2][0] =
            accum_i8mm(accumulators[2][0], a_vec_high[2], b_vec_high[0]);
        accumulators[2][1] =
            accum_i8mm(accumulators[2][1], a_vec_high[2], b_vec_high[1]);
        accumulators[2][2] =
            accum_i8mm(accumulators[2][2], a_vec_high[2], b_vec_high[2]);
        accumulators[2][3] =
            accum_i8mm(accumulators[2][3], a_vec_high[2], b_vec_high[3]);
        accumulators[3][0] =
            accum_i8mm(accumulators[3][0], a_vec_high[3], b_vec_high[0]);
        accumulators[3][1] =
            accum_i8mm(accumulators[3][1], a_vec_high[3], b_vec_high[1]);
        accumulators[3][2] =
            accum_i8mm(accumulators[3][2], a_vec_high[3], b_vec_high[2]);
        accumulators[3][3] =
            accum_i8mm(accumulators[3][3], a_vec_high[3], b_vec_high[3]);
      }
      ...
      for (size_t k_block = K_aligned; k_block < K; k_block += VECTOR_WIDTH) {
        ...
        for (int m = 0; m < TILE_M; ++m)
          for (int n = 0; n < TILE_N; ++n)
            accumulators[m][n] =
                accum_i8mm(accumulators[m][n], a_vec[m], b_vec[n]);
      }
      ...
      for (int m = 0; m < TILE_M; ++m) {
        ...
        for (int n = 0; n < TILE_N; ++n) {
          ...
          int32_t quantized_result = static_cast<int32_t>(
              scaled_result + (scaled_result >= 0 ? 0.5f : -0.5f));
  ...
  aphrodite::mobile::parallel_for(
      ...
      [=](size_t start_block, size_t end_block) {
        for (size_t block_idx = start_block; block_idx < end_block;
          ...
          matmul_int8_worker(a, b_transposed, c, M, K, N, start_row, end_row,
                             a_scale, b_scale, c_scale);
        }
      });
}

static void matmul_f16_worker(const __fp16* a, const __fp16* b_transposed,
  ...
  for (size_t row_block = start_row; row_block < end_row; row_block += TILE_M) {
    for (size_t col_block = 0; col_block < N; col_block += TILE_N) {
      ...
      for (int m = 0; m < TILE_M; ++m)
        for (int n = 0; n < TILE_N; ++n) accumulators[m][n] = vdupq_n_f16(0.0);
      ...
      for (size_t k_block = 0; k_block < K_aligned;
        ...
        for (int n = 0; n < TILE_N; ++n) {
          ...
          if (col < N) {
            ...
            b_vec_high[n] =
                vld1q_f16(&b_transposed[col * K + k_block + VECTOR_WIDTH]);
          } else {
        ...
        for (int m = 0; m < TILE_M; ++m)
          for (int n = 0; n < TILE_N; ++n) {
            accumulators[m][n] =
                accum_f16_dot(accumulators[m][n], a_vec_low[m], a_vec_high[m],
      ...
      for (size_t k_block = K_aligned; k_block < K; k_block += VECTOR_WIDTH) {
        ...
        for (int n = 0; n < TILE_N; ++n) {
          ...
          if (col < N) {
            if (remaining >= VECTOR_WIDTH) {
              ...
              __fp16 tmp[VECTOR_WIDTH] = {0.0};
              memcpy(tmp, &b_transposed[col * K + k_block],
                     remaining * sizeof(__fp16));
        ...
        for (int m = 0; m < TILE_M; ++m)
          for (int n = 0; n < TILE_N; ++n)
            accumulators[m][n] =
                vfmaq_f16(accumulators[m][n], a_vec[m], b_vec[n]);
      }
  ...
  aphrodite::mobile::parallel_for(
      ...
      [=](size_t start_block, size_t end_block) {
        for (size_t block_idx = start_block; block_idx < end_block;
          ...
          matmul_f16_worker(a, b_transposed, c, M, K, N, start_row, end_row);
        }
      });
}

static void matmul_f32_worker(const float* a, const float* b_transposed,
  ...
  for (size_t row_block = start_row; row_block < end_row; row_block += TILE_M) {
    for (size_t col_block = 0; col_block < N; col_block += TILE_N) {
      ...
      for (int m = 0; m < TILE_M; ++m)
        for (int n = 0; n < TILE_N; ++n) accumulators[m][n] = vdupq_n_f32(0.0f);
      ...
      for (size_t k_block = 0; k_block < K_aligned;
        ...
        for (int n = 0; n < TILE_N; ++n) {
          ...
          if (col < N) {
            ...
            b_vec_high[n] =
                vld1q_f32(&b_transposed[col * K + k_block + VECTOR_WIDTH]);
          } else {
        ...
        for (int m = 0; m < TILE_M; ++m)
          for (int n = 0; n < TILE_N; ++n) {
            accumulators[m][n] =
                accum_f32_dot(accumulators[m][n], a_vec_low[m], a_vec_high[m],
                              b_vec_low[n], b_vec_high[n]);
          }
      }
      ...
      for (size_t k_block = K_aligned; k_block < K; k_block += VECTOR_WIDTH) {
        ...
        for (int m = 0; m < TILE_M; ++m)
          for (int n = 0; n < TILE_N; ++n)
            accumulators[m][n] =
                vfmaq_f32(accumulators[m][n], a_vec[m], b_vec[n]);
      }
  ...
  aphrodite::mobile::parallel_for(
      ...
      [=](size_t start_block, size_t end_block) {
        for (size_t block_idx = start_block; block_idx < end_block;
          ...
          matmul_f32_worker(a, b_transposed, c, M, K, N, start_row, end_row);
        }
      });
}

#if !defined(__ARM_FEATURE_MATMUL_INT8)
static void matmul_int8_to_int32_worker(const int8_t* a,
                                        const int8_t* b_transposed, int32_t* c,
                                        size_t M, size_t K, size_t N,
                                        size_t start_row, size_t end_row) {
  constexpr int TILE_M = 4;
  constexpr int TILE_N = 4;
  ...
  const size_t K_aligned =
      (K / (VECTOR_WIDTH * VECTOR_UNROLL)) * (VECTOR_WIDTH * VECTOR_UNROLL);
  ...
  for (size_t row_block = start_row; row_block < end_row; row_block += TILE_M) {
    for (size_t col_block = 0; col_block < N; col_block += TILE_N) {
      ...
      for (size_t k_block = 0; k_block < K_aligned;
           k_block += VECTOR_WIDTH * VECTOR_UNROLL) {
        ...
        for (int m = 0; m < TILE_M; ++m) {
          for (int n = 0; n < TILE_N; ++n) {
            accumulators[m][n] =
                accum_i8mm(accumulators[m][n], a_vec[0][m], b_vec[0][n]);
            accumulators[m][n] =
                accum_i8mm(accumulators[m][n], a_vec[1][m], b_vec[1][n]);
          }
        }
      }
...
void matmul_int8_to_int32(const int8_t* a, const int8_t* b_transposed,
  ...
  size_t num_threads =
      aphrodite::mobile::compute_gemm_parallelism(M, K, N, sizeof(int8_t));

  if (num_threads == 1) {
    matmul_int8_to_int32_worker(a, b_transposed, c, M, K, N, 0, M);
    return;
  }

  size_t optimal_tile_m =
      std::min(aphrodite::mobile::Thresholds::GEMM_TILE_M, (M + 1) / 2 * 2);
  size_t optimal_tile_n =
      std::min(aphrodite::mobile::Thresholds::GEMM_TILE_N, (N + 1) / 2 * 2);

  size_t k_cache_footprint = K * sizeof(int8_t);
  if (k_cache_footprint > aphrodite::mobile::Thresholds::L2_CACHE_SIZE) {
    optimal_tile_m = aphrodite::mobile::Thresholds::GEMM_TILE_M_SMALL;
    optimal_tile_n = aphrodite::mobile::Thresholds::GEMM_TILE_N_SMALL;
  }

  aphrodite::mobile::parallel_for_2d_tiled(
      M, N, optimal_tile_m, optimal_tile_n,
      [=](size_t row_start, size_t row_end, size_t col_start, size_t col_end) {
        ...
        const size_t K_aligned = (K / (VECTOR_WIDTH * VECTOR_UNROLL)) *
                                 (VECTOR_WIDTH * VECTOR_UNROLL);

        for (size_t row_block = row_start; row_block < row_end;
             row_block += MICRO_TILE_M) {
          for (size_t col_block = col_start; col_block < col_end;
            ...
            for (size_t k_block = 0; k_block < K_aligned;
              ...
              for (int m = 0; m < MICRO_TILE_M; ++m) {
                ...
                if (row < row_end) {
                  ...
                  a_vec[1][m] = vld1q_s8(&a[row * K + k_block + VECTOR_WIDTH]);
                  a_vec[2][m] =
                      vld1q_s8(&a[row * K + k_block + VECTOR_WIDTH * 2]);
                  a_vec[3][m] =
                      vld1q_s8(&a[row * K + k_block + VECTOR_WIDTH * 3]);
                } else {
                  a_vec[0][m] = vdupq_n_s8(0);
                  a_vec[1][m] = vdupq_n_s8(0);
              ...
              for (int n = 0; n < MICRO_TILE_N; ++n) {
                ...
                if (col < col_end) {
                  ...
                  b_vec[1][n] =
                      vld1q_s8(&b_transposed[col * K + k_block + VECTOR_WIDTH]);
                  b_vec[2][n] = vld1q_s8(
                      &b_transposed[col * K + k_block + VECTOR_WIDTH * 2]);
                  b_vec[3][n] = vld1q_s8(
                      &b_transposed[col * K + k_block + VECTOR_WIDTH * 3]);
                } else {
                  b_vec[0][n] = vdupq_n_s8(0);
                  b_vec[1][n] = vdupq_n_s8(0);
              ...
              accumulators[0][0] =
                  accum_i8mm(accumulators[0][0], a_vec[0][0], b_vec[0][0]);
              accumulators[0][1] =
                  accum_i8mm(accumulators[0][1], a_vec[0][0], b_vec[0][1]);
              accumulators[1][0] =
                  accum_i8mm(accumulators[1][0], a_vec[0][1], b_vec[0][0]);
              accumulators[1][1] =
                  accum_i8mm(accumulators[1][1], a_vec[0][1], b_vec[0][1]);

              accumulators[0][0] =
                  accum_i8mm(accumulators[0][0], a_vec[1][0], b_vec[1][0]);
              accumulators[0][1] =
                  accum_i8mm(accumulators[0][1], a_vec[1][0], b_vec[1][1]);
              accumulators[1][0] =
                  accum_i8mm(accumulators[1][0], a_vec[1][1], b_vec[1][0]);
              accumulators[1][1] =
                  accum_i8mm(accumulators[1][1], a_vec[1][1], b_vec[1][1]);

              accumulators[0][0] =
                  accum_i8mm(accumulators[0][0], a_vec[2][0], b_vec[2][0]);
              accumulators[0][1] =
                  accum_i8mm(accumulators[0][1], a_vec[2][0], b_vec[2][1]);
              accumulators[1][0] =
                  accum_i8mm(accumulators[1][0], a_vec[2][1], b_vec[2][0]);
              accumulators[1][1] =
                  accum_i8mm(accumulators[1][1], a_vec[2][1], b_vec[2][1]);

              accumulators[0][0] =
                  accum_i8mm(accumulators[0][0], a_vec[3][0], b_vec[3][0]);
              accumulators[0][1] =
                  accum_i8mm(accumulators[0][1], a_vec[3][0], b_vec[3][1]);
              accumulators[1][0] =
                  accum_i8mm(accumulators[1][0], a_vec[3][1], b_vec[3][0]);
              accumulators[1][1] =
                  accum_i8mm(accumulators[1][1], a_vec[3][1], b_vec[3][1]);
            }

            for (size_t k_block = K_aligned; k_block < K;
                 k_block += VECTOR_WIDTH) {
              size_t remaining = K - k_block;
...
static void matmul_int8_to_int32_smmla_worker(
    const int8_t* a, const int8_t* b_transposed, int32_t* output, size_t M,
    size_t K, size_t N, size_t start_row, size_t end_row, size_t start_col,
    size_t end_col) {
  ...
  for (size_t row_block = start_row; row_block < end_row; row_block += 4) {
    for (size_t col_block = start_col; col_block < end_col; col_block += 4) {
      int32x4_t acc[2][2] = {{vdupq_n_s32(0), vdupq_n_s32(0)},
      ...
      for (size_t k_block = 0; k_block < K_aligned; k_block += 8) {
        if (row_block + 3 < M && col_block + 3 < N && k_block + 8 <= K) {
          ...
          if (k_block + prefetch_distance < K) {
            __builtin_prefetch(&a[row_block * K + k_block + prefetch_distance],
                               0, 1);
            __builtin_prefetch(
                &a[(row_block + 1) * K + k_block + prefetch_distance], 0, 1);
            __builtin_prefetch(
                &a[(row_block + 2) * K + k_block + prefetch_distance], 0, 1);
            __builtin_prefetch(
                &a[(row_block + 3) * K + k_block + prefetch_distance], 0, 1);
          }
          ...
          asm volatile(
              ...
              "smmla %2.4s, %5.16b, %6.16b\n"  // acc[1][0] += a_tiles[1] *
                                               // b_tiles[0]
              "smmla %3.4s, %5.16b, %7.16b"    // acc[1][1] += a_tiles[1] *
                                               // b_tiles[1]
              : "+w"(acc[0][0]), "+w"(acc[0][1]), "+w"(acc[1][0]),
                "+w"(acc[1][1])
              : "w"(a_tiles[0]), "w"(a_tiles[1]), "w"(b_tiles[0]),
                "w"(b_tiles[1]));
        } else {
          for (size_t r = 0; r < 4; r += 2) {
            for (size_t c = 0; c < 4; c += 2) {
              if (row_block + r < M && col_block + c < N) {
                ...
                const int8_t* b_col_base =
                    &b_transposed[(col_block + c) * K + k_block];
                ...
                asm volatile("smmla %0.4s, %1.16b, %2.16b"
                             : "+w"(acc[r / 2][c / 2])
                             : "w"(a_tile), "w"(b_tile));
              }
            }
      ...
      for (size_t k = K_aligned; k < K; k++) {
        for (size_t r = 0; r < 4 && row_block + r < M; r++) {
          for (size_t c = 0; c < 4 && col_block + c < N; c++) {
            ...
            results[tile_idx][elem_idx] +=
                a[(row_block + r) * K + k] *
                b_transposed[(col_block + c) * K + k];
          }
        }
      }
      ...
      for (size_t r = 0; r < 4 && row_block + r < M; r++) {
        for (size_t c = 0; c < 4 && col_block + c < N; c++) {
          size_t tile_idx = (r / 2) * 2 + (c / 2);
          size_t elem_idx = (r % 2) * 2 + (c % 2);
          output[(row_block + r) * N + (col_block + c)] =
              results[tile_idx][elem_idx];
        }
      }
    }
...
void matmul_int8_to_int32_i8mm(const int8_t* a, const int8_t* b_transposed,
  ...
  if (total_ops < aphrodite::mobile::Thresholds::GEMM_SMALL) {
    cactus_matmul_int8_to_int32_smmla_worker(a, b_transposed, c, M, K, N, 0, M,
                                             0, N);
    return;
  }
  ...
  size_t optimal_tile_m =
      std::min(aphrodite::mobile::Thresholds::GEMM_TILE_M, (M + 3) / 4 * 4);
  size_t optimal_tile_n =
      std::min(aphrodite::mobile::Thresholds::GEMM_TILE_N, (N + 3) / 4 * 4);
  ...
  aphrodite::mobile::parallel_for_2d_tiled(
      M, N, optimal_tile_m, optimal_tile_n,
      [=](size_t row_start, size_t row_end, size_t col_start, size_t col_end) {
        cactus_matmul_int8_to_int32_smmla_worker(a, b_transposed, c, M, K, N,
