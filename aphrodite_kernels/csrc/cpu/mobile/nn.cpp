      ...
      [&](size_t start_idx, size_t end_idx) {
        for (size_t i = start_idx; i < end_idx; ++i) {
          ...
          output[i] = static_cast<int8_t>(
              std::max(-128.0f, std::min(127.0f, roundf(scaled))));
        }
      });
}
  ...
  aphrodite::mobile::parallel_for(
      ...
      [&](size_t start_idx, size_t end_idx) {
        ...
        for (size_t i = start_idx; i < vectorized_end; i += SIMD_WIDTH) {
          ...
          float32x4_t gelu =
              vmulq_f32(vmulq_f32(half, x), vaddq_f32(one, tanh_result));
  ...
  aphrodite::mobile::parallel_for(
      ...
      [&](size_t start_idx, size_t end_idx) {
        ...
        for (size_t i = start_idx; i < vectorized_end; i += SIMD_WIDTH) {
          ...
          float32x4_t x_cubed_high =
              vmulq_f32(vmulq_f32(x_high, x_high), x_high);
          ...
          float32x4_t gelu_low =
              vmulq_f32(vmulq_f32(half, x_low), one_plus_tanh_low);
          float32x4_t gelu_high =
              vmulq_f32(vmulq_f32(half, x_high), one_plus_tanh_high);
...
void kernel_softmax_neon_optimized_single(const float* input, float* output,
                                          size_t vocab_size) {
  constexpr size_t SIMD_WIDTH = 4;
  constexpr size_t UNROLL_FACTOR = 8;
  constexpr size_t VECTORIZED_WIDTH = SIMD_WIDTH * UNROLL_FACTOR;
  const size_t vocab_vectorized =
      (vocab_size / VECTORIZED_WIDTH) * VECTORIZED_WIDTH;
  ...
  aphrodite::mobile::parallel_for(
      ...
      [&](size_t start_idx, size_t end_idx) {
        for (size_t idx = start_idx; idx < end_idx; ++idx) {
          ...
          CactusSoftmax::kernel_softmax_neon_optimized_single(
              input + offset, output + offset, vocab_size);
        }
      });
}

void kernel_softmax_f16_single(const __fp16* input, __fp16* output,
                               size_t vocab_size) {
  constexpr size_t SIMD_WIDTH = 8;
  ...
  const float32x4_t max_broadcast = vdupq_n_f32(max_val);
  const float16x8_t max_broadcast_f16 =
      vcombine_f16(vcvt_f16_f32(max_broadcast), vcvt_f16_f32(max_broadcast));
  ...
  for (size_t i = 0; i < vocab_vectorized; i += VECTORIZED_WIDTH) {
    for (size_t u = 0; u < UNROLL_FACTOR; u++) {
      ...
      float16x8_t exp_f16 =
          vcombine_f16(vcvt_f16_f32(exp_low), vcvt_f16_f32(exp_high));
...
void softmax_f16(const __fp16* input, __fp16* output, size_t batch_size,
  ...
  aphrodite::mobile::parallel_for(
      ...
      [&](size_t start_idx, size_t end_idx) {
        for (size_t idx = start_idx; idx < end_idx; ++idx) {
          const size_t offset = idx * vocab_size;
          kernel_softmax_f16_single(input + offset, output + offset,
                                    vocab_size);
        }
      });
}

void sample_f32(const float* logits, uint32_t* output, size_t vocab_size,
  ...
  if (min_p > 0.0f) {
    ...
    if (!std::isinf(max_logit)) {
      ...
      if (sum > 0.0f) {
        ...
        float max_prob =
            *std::max_element(temp_probs.begin(), temp_probs.end());
  ...
  if (top_p > 0.0f && top_p < 1.0f) {
    ...
    for (size_t i = 0; i < sorted_logits.size(); ++i) {
      if (indices_to_remove[i]) {
        filtered_logits[sorted_logits[i].second] =
  ...
  uint32_t actual_seed =
      (random_seed == 0) ? std::random_device{}() : random_seed;
...
void sample_f16(const __fp16* logits, uint32_t* output, size_t vocab_size,
                float temperature, float top_p, size_t top_k,
                size_t random_seed) {
  ...
  if (!token_history.empty() && REPETITION_PENALTY != 1.0f) {
    ...
    for (uint32_t prev_token : token_history) {
      if (prev_token < vocab_size) {
        filtered_logits[prev_token] =
            (filtered_logits[prev_token] > static_cast<__fp16>(0))
  ...
  if (top_k > 0) {
    ...
    std::partial_sort(
        logit_pairs.begin(), logit_pairs.begin() + std::min(top_k, vocab_size),
        logit_pairs.end(),
        [](const auto& a, const auto& b) { return a.first > b.first; });
  ...
  if (top_p > 0.0f && top_p < 1.0f) {
    ...
    __fp16 max_logit = sorted_logits.empty() ? static_cast<__fp16>(0.0f)
                                             : sorted_logits[0].first;
    std::vector<float> temp_probs;
    temp_probs.reserve(sorted_logits.size());
    float sum = 0.0f;
    for (const auto& pair : sorted_logits) {
