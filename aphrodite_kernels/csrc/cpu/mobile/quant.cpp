...
void int8_to_fp32(const int8_t* src, float* dst, size_t count, float scale) {
  aphrodite::mobile::parallel_for(
      count, aphrodite::mobile::Thresholds::ELEMENT_WISE,
  ...
  aphrodite::mobile::parallel_for(
      ...
      [src, dst, inv_scale](size_t start, size_t end) {
        ...
        for (size_t i = start; i < simd_end; i += 16) {
          ...
          int16x8_t int16_low =
              vcombine_s16(vqmovn_s32(int_0), vqmovn_s32(int_1));
          int16x8_t int16_high =
              vcombine_s16(vqmovn_s32(int_2), vqmovn_s32(int_3));

          int8x16_t result =
              vcombine_s8(vqmovn_s16(int16_low), vqmovn_s16(int16_high));
...
void dynamic_quantize_fp32_to_int8(const float* src, int8_t* dst, size_t count,
  ...
  fp32_to_int8(src, dst, count, scale);
  ...
  aphrodite::mobile::parallel_for(
      ...
      [src, dst, scale](size_t start, size_t end) {
        ...
        for (size_t i = start; i < simd_end; i += 8) {
          ...
          float16x8_t output =
              vcombine_f16(vcvt_f16_f32(float_low), vcvt_f16_f32(float_high));
          vst1q_f16(&dst[i], output);
        }
  ...
  aphrodite::mobile::parallel_for(
      ...
      [src, dst, inv_scale](size_t start, size_t end) {
        ...
        for (size_t i = start; i < simd_end; i += 8) {
          ...
          int16x8_t int16_combined =
              vcombine_s16(vqmovn_s32(int_low), vqmovn_s32(int_high));
...
void int32_to_fp16_scaled(const int32_t* src, __fp16* dst, size_t count,
  ...
  for (size_t i = 0; i < simd_end; i += 8) {
    ...
    float16x8_t result =
        vcombine_f16(vcvt_f16_f32(scaled_low), vcvt_f16_f32(scaled_high));
