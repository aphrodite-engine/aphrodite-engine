...
void large_context_topk(
  ...
  const auto params = aphrodite::get_params(logits, seq_lens, row_starts, indices);
  const int64_t batch_size = logits.size(0);

  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  const dim3 grid(static_cast<uint32_t>(batch_size));
  const dim3 block(aphrodite::kThreadsPerBlock);

  aphrodite::setup_kernel_smem_once<aphrodite::topk_kernel, aphrodite::kSmem>();
  aphrodite::topk_kernel<<<grid, block, aphrodite::kSmem, stream>>>(params);
