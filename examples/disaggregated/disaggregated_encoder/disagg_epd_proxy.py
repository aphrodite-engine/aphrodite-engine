...
def validate_ec_consumer_routing(prefill_urls: list[str], consumer_addrs: list[str]) -> None:
...
def rewrite_for_decode(req_data: dict, item_meta: dict[int, dict]) -> dict:
    ...
    for msg in req_data.get("messages", []):
        ...
        for item in content:
            ...
            new_content.append({"type": embeds_type, embeds_type: metadata, "uuid": item_uuid})
            if transfer_id is not None:
                transfer_items.append({"mm_hash": ec_mm_hash, "transfer_id": transfer_id})
            rewritten += 1
...
async def on_startup() -> None:
    ...
    # Aphrodite closes an idle keep-alive connection after
    # APHRODITE_HTTP_TIMEOUT_KEEP_ALIVE seconds (5 by default), while aiohttp keeps
    ...
    server_keep_alive = float(os.getenv("APHRODITE_HTTP_TIMEOUT_KEEP_ALIVE", "5"))
    connector = aiohttp.TCPConnector(
        limit=0,
        **({"keepalive_timeout": server_keep_alive / 2} if server_keep_alive > 0 else {"force_close": True}),
    )
...
async def prepare_for_decode(
    ...
    item_meta, ec_params = await fanout_encoder_primer(req_data, e_urls, req_id, consumer_zmq)
