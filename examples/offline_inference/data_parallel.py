...
def parse_args():
    ...
    parser.add_argument("--model", type=str, default="ibm-research/PowerMoE-3b", help="Model name or path")
...
def main(model, dp_size, local_dp_rank, global_dp_rank, dp_master_ip, dp_master_port, GPUs_per_dp_rank):
    os.environ["APHRODITE_DP_RANK"] = str(global_dp_rank)
    os.environ["APHRODITE_DP_RANK_LOCAL"] = str(local_dp_rank)
    os.environ["APHRODITE_DP_SIZE"] = str(dp_size)
    os.environ["APHRODITE_DP_MASTER_IP"] = dp_master_ip
    os.environ["APHRODITE_DP_MASTER_PORT"] = str(dp_master_port)
    ...
    # Create an LLM.
    llm = LLM(model=model, tensor_parallel_size=GPUs_per_dp_rank, enforce_eager=True, enable_expert_parallel=True)
...
if __name__ == "__main__":
    ...
    for local_dp_rank, global_dp_rank in enumerate(range(node_rank * dp_per_node, (node_rank + 1) * dp_per_node)):
        proc = Process(
            target=main,
            args=(args.model, dp_size, local_dp_rank, global_dp_rank, dp_master_ip, dp_master_port, tp_size),
        )
        proc.start()
        procs.append(proc)
    exit_code = 0
