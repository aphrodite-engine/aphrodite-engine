...
"""
Trace-replay with Aphrodite offline inference.
...
def run_trace_replay(llm: LLM, prompt: str, decode_token_ids: list[int]) -> None:
    ...
    assert replayed_ids == decode_token_ids, f"Mismatch!\n  expected: {decode_token_ids}\n  got:      {replayed_ids}"
    ...
    if result.logprobs:
        ...
        for step, (token_id, logprob_dict) in enumerate(zip(replayed_ids, result.logprobs)):
            ...
            print(f"    step {step:2d}: token_id={token_id:6d}  logprob={lp_value}  rank={rank}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Trace-replay with Aphrodite offline inference")
    parser.add_argument(
        "--prompt",
        type=str,
