...
"""Driver-side helpers to run the inference fleet as a standard ``aphrodite serve``
process for the sharded-RDT examples.

The trainer reaches the server's weight-sync control plane over the RLHF HTTP
routes (``HTTPAphroditeWeightSyncClient``); generation uses ``/v1/completions``. RDT
...
def launch_aphrodite_serve(
    ...
    *,
    ...
    """Start ``aphrodite serve`` for the RDT inference fleet.

    ``APHRODITE_SERVER_DEV_MODE`` exposes the RLHF weight-sync routes; the Ray v2
    ...
    and picks up the editable Aphrodite install via the venv interpreter.
    """
    aphrodite_bin = os.path.join(os.path.dirname(sys.executable), "aphrodite")
    if not os.path.exists(aphrodite_bin):
        aphrodite_bin = shutil.which("aphrodite") or "aphrodite"
    cmd = [
        aphrodite_bin,
        "serve",
        model,
    ...
    # data_parallel_backend=ray forces Aphrodite's DP-placement path, which fails.
    ...
    env = dict(
        ...
        APHRODITE_USE_RAY_V2_EXECUTOR_BACKEND="1",
    )
    # Pin the child to THIS Ray cluster. Aphrodite's ray DP backend reaches Ray
    ...
    # which shadows an editable Aphrodite install (its compiled .so is gitignored, so
...
def wait_for_server(endpoint: str, proc: subprocess.Popen, timeout: float = 1800) -> None:
    """Block until ``/health`` returns 200, failing fast if the server exits."""
    deadline = time.time() + timeout
    while time.time() < deadline:
        if proc.poll() is not None:
            raise RuntimeError(f"aphrodite serve exited early (code {proc.returncode})")
        try:
            ...
            pass
        time.sleep(3)
    raise RuntimeError("aphrodite serve did not become healthy in time")


def http_generate(endpoint: str, model: str, prompts: list[str], max_tokens: int = 16) -> list[str]:
...
def shutdown_server(proc: subprocess.Popen) -> None:
    print("[serve] shutting down aphrodite serve", flush=True)
    proc.terminate()
    try:
