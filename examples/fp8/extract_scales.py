...
def _prepare_hf_weights(
    ...
    if not os.path.isdir(quantized_model_dir):
        raise FileNotFoundError(f"The quantized model directory `{quantized_model_dir}` does not exist.")
    ...
    if len(hf_weights_files) == 0:
        raise RuntimeError(f"Cannot find any model weights with `{quantized_model_dir}`")

    return hf_weights_files, use_safetensors


# Adapted from aphrodite/modeling/model_loader/weight_utils.py
def _hf_tensorfile_iterator(filename: str, load_format: str, use_safetensors: bool):
    if load_format == "npz":
...
def _kv_scales_extractor(
    hf_tensor_files: list[str], use_safetensors: bool, rank_keyword: str = "rank", expected_tp_size: int | None = None
    ...
    for tensor_file in hf_tensor_files:
        try:
            ...
            print(f"Unable to determine TP rank corresponding to file '{tensor_file}'")
            raise
        ...
        if rank not in rank_scales_map:
            ...
            raise RuntimeError(f"Tensor file '{tensor_file}' shares TP rank {rank} with another tensor file.")

        module_delimiter = ":" if args.load_format == "npz" else "."
        for name, param in _hf_tensorfile_iterator(tensor_file, args.load_format, use_safetensors):
            if "kv_cache_scaling_factor" in name:
                ...
                layer_idx = nums[0]
                assert layer_idx not in layer_scales_map, f"Duplicate scaling factor corresponding to layer {layer_idx}"
                try:
...
def _metadata_extractor(
    quantized_model_dir: str, metadata_extract_fns: dict[str, Callable[[dict[str, Any]], Any]]
) -> dict[str, Any]:
    """
    ...
    for file in metadata_files:
        with open(file) as f:
            ...
            if not isinstance(metadata, dict):
                print(f"The file `{file}` does not correspond to a JSON-serialized dictionary, skipping it.")
                continue
    ...
    for metadata_name in metadata_extract_fns:
        if metadata_name not in result:
            print(f"WARNING: Unable to find requested metadata field `{metadata_name}`, setting it to None.")
...
def main(args):
    ...
    rank_scales_map = _kv_scales_extractor(hf_tensor_files, use_safetensors, rank_keyword, expected_tp_size)
    ...
    rank_scales_map = {rank: {k: scale[k] for k in sorted(scale.keys())} for rank, scale in rank_scales_map.items()}
    ...
    schema = QuantParamSchema(
        ...
        kv_cache={
            "dtype": ("float8_e4m3fn" if len(rank_scales_map) > 0 else recovered_metadata["model_dtype"]),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--load_format",
        help="Optionally specify the format of the model's tensor files containing the KV cache scaling factors.",
