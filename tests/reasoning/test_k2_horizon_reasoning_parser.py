...
from aphrodite.reasoning.k2_horizon_reasoning_parser import K2HorizonReasoningParser
...
class K2Tokenizer:
    def __init__(self) -> None:
        special_tokens = [token for token_pair in EFFORT_TOKENS.values() for token in token_pair] + [TOOL_CALLS_START]
...
def test_reasoning_effort_precedence_and_default():
    ...
    resolved_kwargs = (
        request.build_chat_params(None, "auto").with_defaults({"reasoning_effort": "high"}).chat_template_kwargs
    )
...
def test_non_streaming_explicit_and_implicit_boundaries():
    ...
    assert parser.extract_reasoning(f"plan{TOOL_CALLS_START}<ifm|tool_call>ping</ifm|tool_call>", request) == (
        "plan",
...
def test_reasoning_parser_registered():
    assert ReasoningParserManager.get_reasoning_parser("k2_horizon") is K2HorizonReasoningParser
