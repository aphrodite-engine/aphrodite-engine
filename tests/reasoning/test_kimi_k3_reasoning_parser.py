...
def test_delegating_parser_thinking_false_streams_response_content():
    parser = ReasoningOnlyParser(DummyTokenizer(), chat_template_kwargs={"thinking": False})
...
def test_reasoning_end_matches_reference_over_marker_dense_sequences(seed):
    ...
    for _ in range(200):
        ...
        if not _reference_is_reasoning_end(head):
            assert parser.is_reasoning_end_streaming(full, delta) == _reference_is_reasoning_end(full), (head, delta)
