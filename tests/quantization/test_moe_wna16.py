...
from aphrodite.model_executor.layers.quantization import moe_wna16
...
def test_moe_wna16_humming_adapter_repacks_uint8_tensors():
    ...
    expected_qzeros = qzeros.transpose(-1, -2).contiguous().view(torch.int32).transpose(-1, -2).contiguous()
...
def test_moe_wna16_uses_humming_quant_config(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda actual_layer, *args, **kwargs: (quant_config if actual_layer is layer else None),
    )
...
def test_moe_wna16_forwards_packed_modules_mapping_to_linear_delegate(monkeypatch):
    ...
    monkeypatch.setattr(
        AutoGPTQConfig,
        "get_quant_method",
        lambda self, layer, prefix: seen.setdefault("mapping", self.packed_modules_mapping),
    )
...
def test_xpu_platform_supports_moe_wna16():
    ...
    try:
        ...
        pytest.skip("aphrodite_xpu_kernels not importable outside an XPU stack")
