...
from aphrodite.model_executor.kernels.linear import _POSSIBLE_MXFP4_KERNELS
...
from aphrodite.model_executor.layers.quantization.quark.quark_moe import (
    QuarkOCP_MX_MoEMethod,
)
from aphrodite.model_executor.layers.quantization.quark.schemes.quark_ocp_mx import (
    QuarkOCP_MX,
)
...
def fix_negative_zeros(tensor: torch.Tensor) -> torch.Tensor:
    ...
    """
    assert tensor.dtype == torch.uint8, f"Expected a torch.uint8 tensor, got {tensor.dtype}"

    low_nibble = tensor & 0x0F
    high_nibble = tensor & 0xF0

    low_nibble = torch.where(low_nibble == 0x08, torch.zeros_like(low_nibble), low_nibble)
    high_nibble = torch.where(high_nibble == 0x80, torch.zeros_like(high_nibble), high_nibble)
...
def assert_quantized_weights_equal(
    ...
    for key in keys:
        ...
        assert torch.equal(checkpoint_tensor, online_tensor), (
            f"{key}: {num_mismatched}/{total} ({100 * num_mismatched / total:.4f}%) mismatched bytes"
        )
...
@pytest.mark.skipif(not current_platform.is_cuda_alike(), reason="Only tested on ROCm/CUDA.")
...
def test_online_mxfp4_tp_weight_quant_matches_unsharded(tp_size: int):
    ...
    dense_weight = torch.randn(hidden_size, intermediate_size, dtype=torch.bfloat16, device=device)
    ...
    for tp_rank in range(tp_size):
        ...
        column_quantized, column_scales = mxfp4_quantize(dense_weight.narrow(0, output_start, output_per_partition))
        assert torch.equal(
            column_quantized,
        ...
        row_quantized, row_scales = mxfp4_quantize(dense_weight.narrow(1, input_start, input_per_partition))
        assert torch.equal(
            row_quantized,
        ...
        w2_quantized_shard, w2_scales_shard = mxfp4_quantize(w2_weight.narrow(2, input_start, input_per_partition))
        assert torch.equal(
            w2_quantized_shard,
...
def test_online_mxfp4_moe_matches_quark(
    ...
    if moe_backend == "aiter":
        ...
        monkeypatch.setenv("APHRODITE_ROCM_USE_AITER_MOE", "1")
        monkeypatch.setattr("aphrodite.platforms.rocm.on_gfx950", lambda: True)
    ...
    with torch.device(device):
        ...
        hidden_size, intermediate_size = online_method.maybe_roundup_sizes(**roundup_arguments)
        assert (
            hidden_size,
            intermediate_size,
        ...
        for layer in (checkpoint_layer, online_layer):
            ...
            assert layer.moe_config.intermediate_size_per_partition_unpadded == unpadded_intermediate_size
        ...
        checkpoint_w13, checkpoint_w2 = scatter_into_padded(gate_up_weight, down_weight, padding_value=0.0)
        ...
        online_w13, online_w2 = scatter_into_padded(gate_up_weight, down_weight, padding_value=float("nan"))
...
def test_online_mxfp4_dense_matches_quark(linear_backend: str, default_aphrodite_config, dist_init, monkeypatch):
    """
    ...
    if linear_backend == "marlin" and not current_platform.is_cuda():
        ...
        pytest.skip("MarlinMxFp4LinearKernel.process_weights_after_loading is CUDA-only.")
    ...
    if linear_backend == "aiter":
        ...
        monkeypatch.setattr(
            "aphrodite.model_executor.kernels.linear.mxfp4.emulation.has_quark",
            lambda: True,
        )

    default_aphrodite_config.model_config = ModelConfig()
    default_aphrodite_config.kernel_config.linear_backend = linear_backend
...
def test_quark_ocp_mx_dense_rejects_unaligned_input_partition():
    ...
    with pytest.raises(
        ValueError,
        match=("input size per partition of 80, which must be divisible by the OCP MX group size 32"),
    ):
