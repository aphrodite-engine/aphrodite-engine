...
def test_opt_125m_int8wo_model_loading_with_params(pt_load_map_location, monkeypatch, dist_init, workspace_init):
    torch._dynamo.reset()
    model_name = "jerryzh168/opt-125m-int8wo-partial-quant"
    model, aphrodite_config = load_model_without_aphrodite_runner(
        model_name,
        quantization="torchao",
        aphrodite_config_kwargs={"load_config": LoadConfig(pt_load_map_location=pt_load_map_location)},
    )
...
@pytest.mark.skipif(not TORCHAO_AVAILABLE, reason="torchao is not available")
def test_qwenvl_int8wo_model_loading_with_params(monkeypatch, dist_init, workspace_init):
    torch._dynamo.reset()
    model_name = "mobicham/Qwen2.5-VL-3B-Instruct_int8wo_ao"
    model, aphrodite_config = load_model_without_aphrodite_runner(
        model_name,
        quantization="torchao",
        aphrodite_config_kwargs={"load_config": LoadConfig(pt_load_map_location=f"{DEVICE_TYPE}:0")},
    )
