...
def test_is_equal_or_regex_match():
    assert is_equal_or_regex_match("model.layers.0.mlp.down_proj", "model.layers.0.mlp.down_proj")
...
def test_online_ignore_supports_fnmatch_patterns():
    layer_name = "model.layers.0.mlp.experts"
    ignore = ["*mlp.experts*"]
    config = OnlineQuantizationConfig(QuantizationConfigArgs(targets={"*mlp.experts*": "mxfp4"}, ignore=ignore))
...
def test_find_matching_patterns_distinguishes_direct_and_shard_matches(patterns, expected):
    layer_name = "model.layers.0.self_attn.qkv_proj"
    fused_mapping = {"qkv_proj": ["q_proj", "k_proj", "v_proj"]}
