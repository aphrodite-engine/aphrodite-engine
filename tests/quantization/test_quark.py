...
@pytest.mark.parametrize("tp", [1])
def test_quark_w4a8_fp8_moe(aphrodite_runner, monkeypatch, tp):
...
WIKITEXT_ACCURACY_CONFIGS = [
    ...
    AccuracyTestConfig(model_name="fxmarty/qwen_1.5-moe-a2.7b-mxfp4", excepted_value=12.45),
]
...
@pytest.mark.parametrize("tp_size", [pytest.param(val, id=f"tp_size:{val}") for val in [1, 2]])
