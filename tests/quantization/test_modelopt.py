...
def test_modelopt_fp8_updates_weight_dims_after_transpose():
    ...
    layer.register_parameter("weight", torch.nn.Parameter(torch.empty(3, 2), requires_grad=False))
    layer.register_parameter("weight_scale", torch.nn.Parameter(torch.ones(1), requires_grad=False))
    layer.register_parameter("input_scale", torch.nn.Parameter(torch.ones(1), requires_grad=False))
    ...
    config = m.ModelOptMixedPrecisionConfig.from_config(
        {
            "quantization": {
                ...
                "exclude_modules": [],
                "group_size": 16,
                "quantized_layers": {"model.layers.0.mlp.down_proj": {"quant_algo": algo}},
            }
        }
    )
    method = config.get_quant_method(MagicMock(spec=LinearBase), "model.layers.0.mlp.down_proj")
...
def test_modelopt_mixed_precision_does_not_quantize_unlisted_fused_sibling():
    ...
    assert config._resolve_quant_algo("model.layers.0.linear_attn.in_proj_qkvz") == "FP8"
...
def test_modelopt_mixed_precision_composes_gemma4_mappers():
    ...
    from aphrodite.model_executor.models.gemma4_mm import (
    ...
    config.apply_aphrodite_mapper(Gemma4ForConditionalGeneration.hf_to_aphrodite_mapper.get_rename_mapper())
    config.apply_aphrodite_mapper(Gemma4ForCausalLM.hf_to_aphrodite_mapper.get_rename_mapper())
...
def test_modelopt_mixed_precision_does_not_infer_missing_sibling_linear(quantized_prefix, missing_prefix):
    from aphrodite.model_executor.layers.linear import LinearBase
...
def test_modelopt_nvfp4_config_dispatches_w4a4_method():
    ...
    method = config.get_quant_method(MagicMock(spec=LinearBase), "model.layers.0.fake_proj")
    assert isinstance(method, ModelOptLinearMethod)
    assert method.spec.weight is kNvfp4Static
    assert method.spec.activation is kNvfp4Dynamic
...
def test_modelopt_linear_exposes_humming_layer_attrs(dist_init, monkeypatch):
    ...
    """
    from aphrodite.config.quantization import QuantSpec
    from aphrodite.model_executor.layers.quantization import modelopt as mo

    monkeypatch.setattr(mo, "select_linear_kernel", lambda spec, layer, rt, **kwargs: Mock())
    ...
    def build(layer):
        ...
        method.create_weights(layer, 64, [32, 32], 64, 64, torch.bfloat16, weight_loader=Mock())
...
def test_modelopt_nvfp4_moe_dispatches_to_marlin_when_w4a16(quant_method, expected_use_a16, act_key_is_none):
    ...
    with (
        patch(
            "aphrodite.model_executor.layers.quantization.modelopt.select_nvfp4_moe_backend",
            mock_select,
        ),
        patch(
...
def test_modelopt_mixed_precision_dispatches_linear_layer(per_layer_algo, expected_weight, expected_activation):
...
def test_modelopt_fp8_pb_wo_hides_output_padding(monkeypatch):
    ...
    with (
        ...
        method.create_weights(layer, 128, [2048, 576], 128, 2624, torch.bfloat16, weight_loader=Mock())
...
def test_modelopt_fp8_pb_wo_rejects_non_128_input():
    ...
    with pytest.raises(ValueError, match="in divisible by 128"):
        scheme.create_weights(torch.nn.Module(), mo.WEIGHT, mo.CkptCtx(), shapes, Mock())
