...
from aphrodite.tool_parsers.k2_horizon_tool_parser import K2HorizonToolParser
...
def _request(
    ...
    chat_template_kwargs = {"tool_call_format": tool_format} if tool_format is not None else None
...
def test_invalid_request_format_rejected_before_generation(tokenizer, tool_format):
    parser = K2HorizonToolParser(tokenizer, _request().tools)
    request = _request().model_copy(update={"chat_template_kwargs": {"tool_call_format": tool_format}})
...
def test_unknown_or_wrong_named_tool_fails_closed(tokenizer):
    named_request = _request("json", {"type": "function", "function": {"name": "lookup"}})
    parser = K2HorizonToolParser(tokenizer, named_request.tools)

    unknown = parser.extract_tool_calls(_group(_json_call("missing", {})), _request("json"))
    wrong_named = parser.extract_tool_calls(_group(_json_call("ping", {})), named_request)
...
def test_whitespace_only_surrounding_content_is_not_preserved(tokenizer):
    ...
    result = parser.extract_tool_calls(_group(_xml_call("ping", []), prefix=" \n", suffix="\t "), request)
