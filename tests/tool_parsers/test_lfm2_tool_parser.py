...
NESTED_QUOTE_FUNCTION_OUTPUT = "bash(command='sed -n '360,450p' /testbed/sympy/matrices/common.py')"
...
MULTILINE_FUNCTION_OUTPUT = "exec(command='cat > f.py << EOF\nimport csv\nprint(1)\nEOF')"
...
def test_empty_tool_call_block(body: str, lfm2_tokenizer: TokenizerLike):
    ...
    content, tool_calls = run_tool_extraction(cls(lfm2_tokenizer), model_output, streaming=False)
    assert len(tool_calls) == 0

    reconstructor = run_tool_extraction_streaming(cls(lfm2_tokenizer), [model_output], assert_one_tool_per_delta=False)
