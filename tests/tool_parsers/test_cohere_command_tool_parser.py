...
from aphrodite.tool_parsers.cohere_command_tool_parser import (
...
class TestExtractToolCalls:
    ...
    def test_streaming_nonstreaming_agree(
        ...
        for i, actual_tc in enumerate(result.tool_calls):
            ...
            assert json.loads(streamed.tool_calls[i]["arguments"]) == json.loads(actual_tc.function.arguments)
        ...
        for token_str in token_strings:
            ...
            if delta is not None:
                ...
                assert sum(populated) == 1, (
                    f"A single streaming delta must carry exactly one of content/reasoning/tool_calls, got {delta!r}"
                )
                if delta.reasoning is not None:
                    saw_reasoning = True
                if delta.tool_calls:
