# Test the AphroditeEngine with multi-step-decoding
...
def test_multi_step_llm_w_prompt_logprobs(
    ...
    Set up a Aphrodite engine instance w/ single-step scheduling as a ground-truth
    reference.
    ...
    * All generated logprobs are all very close
    ...
    with monkeypatch.context() as m:
        ...
        with aphrodite_runner(
            ...
            single_step_aphrodite_outputs = aphrodite_model.generate_greedy_logprobs(
                prompts,
                max_tokens,
                num_logprobs,
                num_prompt_logprobs=num_prompt_logprobs)
...
def test_multi_step_llm_chunked_prefill_prefix_cache(
    ...
    """Test Aphrodite engine with multi-step+"single-step chunked prefill"+APC.
    ...
    "single-step chunked prefill" here refers to the current Aphrodite multi-step+
    ...
    * (If logprobs are enabled) check logprobs are close enough
    ...
    with monkeypatch.context() as m:
        ...
        challenge_prompts[0] = (
            'Aphrodite is a high-throughput and memory-efficient '
        ...
        with aphrodite_runner(
            ...
            outputs_baseline = (
                aphrodite_model.generate_greedy(challenge_prompts, max_tokens) if
                num_logprobs is None else aphrodite_model.generate_greedy_logprobs(
