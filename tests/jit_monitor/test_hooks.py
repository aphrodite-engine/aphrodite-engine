...
import aphrodite.triton_utils as triton_utils
from aphrodite.utils import jit_monitor
...
def _patch_jit_modules(fake_knobs, *, cute_compile=_fake_cute_compile):
    ...
    fake_triton.runtime = SimpleNamespace(
        jit=SimpleNamespace(serialize_specialization_data=lambda *args, **kwargs: None)
    )
    with (
        mock.patch.dict(
            sys.modules,
        ...
        mock.patch.object(triton_utils, "triton", fake_triton),
    ):
        yield
