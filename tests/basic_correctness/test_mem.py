...
import aphrodite.device_allocator.cumem as cumem
import aphrodite.envs as envs
from aphrodite import LLM, AsyncEngineArgs, AsyncLLMEngine, SamplingParams
...
def mapped_usage(allocator) -> int:
    ...
    """
    return sum(data.handle[1] for data in allocator.pointer_to_data.values() if not data.is_asleep)
...
def test_level2_discards_ordinary_tensor_with_weights_tag():
    ...
    with allocator.use_memory_pool("weights"):
        ...
        ordinary_tensor = torch.full((4096,), 0x55, dtype=torch.uint8, device=DEVICE_TYPE)
...
def _lora_logits_mapping_present(model) -> bool:
    ...
    return any(
        isinstance(m, LogitsProcessorWithLoRA) and m.sharded_to_full_mapping_gpu is not None for m in model.modules()
    )
