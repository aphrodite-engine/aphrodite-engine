...
def _multimodal_wrapper_mtp_config(model_type: str, mtp_layers: int = 1) -> PretrainedConfig:
...
def test_mtp_override_recognizes_text_only_types(model_type: str, expected_arch: str) -> None:
...
def test_mtp_override_extracts_n_predict_from_multimodal_wrapper(model_type: str, expected_arch: str) -> None:
    ...
    cfg = SpeculativeConfig.hf_config_override(_multimodal_wrapper_mtp_config(model_type, mtp_layers=2))
    assert cfg.model_type == "qwen3_5_mtp"
    assert cfg.architectures == [expected_arch]
...
def test_mtp_override_top_level_precedence_over_nested_text_config(model_type: str, expected_arch: str) -> None:
...
def test_mtp_override_downloads_real_hf_hub_configs(model_id: str, expected_arch: str) -> None:
