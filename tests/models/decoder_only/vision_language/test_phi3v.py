...
def run_test(
    ...
    *,
    ...
    For phrodite runner, we provide MultiModalDataDict objects
...
def test_input_mapper_override(model: str, image_assets: _ImageAssets,
    ...
    aphrodite_result = mm_registry.map_input(
        ctx.model_config,
        {"image": image},
    )

    assert torch.all(
        hf_result["image_sizes"] == aphrodite_result["image_sizes"])
    assert torch.all(
        hf_result["num_img_tokens"] == aphrodite_result["num_img_tokens"])

    # For pixel values, the second axis should be the num_crops + 1
    # for the rescaled original image. The default value in APHRODITE falls
    # back to the HF config, which is why we compare to the processor num_crops
    assert torch.all(
        hf_result["pixel_values"] == aphrodite_result["pixel_values"])
    assert aphrodite_result[
        "pixel_values"].shape[1] == hf_processor.num_crops + 1
