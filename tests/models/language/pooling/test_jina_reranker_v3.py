...
def test_online():
    with RemoteOpenAIServer(model_name, ["--runner", "pooling", "--enforce-eager"]) as server:
...
def _test_online_instruction(server):
    ...
    assert [d.score for d in kwargs_score.data] == pytest.approx([d.score for d in instruction_score.data], abs=TOL)
