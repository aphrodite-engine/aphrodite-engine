...
def _set_conv_state_layout(monkeypatch, layout: str) -> None:
    ...
    from aphrodite.model_executor.layers.mamba import mamba_utils

    monkeypatch.setenv("APHRODITE_SSM_CONV_STATE_LAYOUT", layout)
...
def _get_Aphrodite_output(
    ...
    runner_context = (
        _owned_aphrodite_runner(aphrodite_runner, kwargs) if aphrodite_model is None else nullcontext(aphrodite_model)
    )
    with runner_context as runner:
        ...
        for _ in range(num_repetitions):
            if num_logprobs < 0:
                ...
                aphrodite_output = runner.generate_greedy_logprobs(prompts, max_tokens, num_logprobs)
...
def test_same_mamba_output_apc_on_vs_off(
    ...
    base_kwargs.update(enforce_eager=True, block_size=16, seed=42, gpu_memory_utilization=0.8)
