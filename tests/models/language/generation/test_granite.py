...
from aphrodite.model_executor.models.granite import granite_layer_attn_params
...
def test_granite_swa_features_are_off_without_swa_config():
    ...
    assert [granite_layer_attn_params(config, i) for i in range(4)] == [(None, theta, False)] * 4


@pytest.mark.parametrize("attention_sinks, expected_sink", [(None, True), (False, False)])
