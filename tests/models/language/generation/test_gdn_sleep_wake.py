...
def test_gdn_sleep_wake_no_stale_state():
    ...
    for output in after:
        ...
        assert len(set(token_ids)) > 1, f"degenerate single-token output after wake_up: {token_ids[:16]}"
        # NaN logits surface as NaN log-probs.
        for step_logprobs in completion.logprobs or []:
            for logprob in step_logprobs.values():
                assert logprob.logprob == logprob.logprob, "NaN log-prob after wake_up (stale GDN state)"
