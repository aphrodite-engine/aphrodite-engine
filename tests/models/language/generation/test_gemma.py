...
from aphrodite.model_executor.models import gemma
...
def test_checkpoint_lm_head_can_override_tied_config(monkeypatch) -> None:
    ...
    model = gemma.GemmaForCausalLM(aphrodite_config=cast(AphroditeConfig, aphrodite_config))
...
def test_gemma4_kv_shared_layer_loads_plain_q_proj() -> None:
    ...
    shards: list[str] = []
    model.layers[0].self_attn.qkv_proj.weight.weight_loader = lambda param, weight, shard_id: shards.append(shard_id)

    q_weight = torch.full((2, 2), 2.0)
    weights = [
        (f"layers.{i}.self_attn.{tensor}.weight", q_weight) for i in (0, 1) for tensor in ("q_proj", "k_proj", "v_proj")
...
def test_gemma3n_kv_shared_layer_mapper() -> None:
    ...
    mapper = Gemma3nTextModel.hf_to_aphrodite_mapper | _kv_sharing_weights_mapper(config)
    weights = [
    ...
    mapped = [(name, getattr(weight, "shard_id", None)) for name, weight in mapper.apply(weights)]
