...
from aphrodite.model_executor.models.conformer_encoder import RelPosMultiHeadAttention
...
class _ReferenceRelPosMultiHeadAttention(RelPosMultiHeadAttention):
    ...
    def _reference_rel_shift(x: torch.Tensor) -> torch.Tensor:
        ...
        zero_pad = torch.zeros((batch, heads, query_len, 1), device=x.device, dtype=x.dtype)
...
@pytest.fixture
def tp1_cpu(tmp_path, default_aphrodite_config):
...
def test_relative_attention_matches_reference(tp1_cpu, sequence: int) -> None:
    ...
    expected_output, expected_attention = reference(query, key, value, pos_emb, mask=mask)
