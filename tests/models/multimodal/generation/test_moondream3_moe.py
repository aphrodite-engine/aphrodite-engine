...
from aphrodite.model_executor.models import moondream3
...
def test_all_reduce_failure_is_not_retried(moe, monkeypatch):
    ...
    monkeypatch.setattr(moondream3, "tensor_model_parallel_all_reduce", _failing_all_reduce)
    x = torch.randn(NUM_TOKENS, HIDDEN_SIZE)
    ...
    assert len(calls) == 1, (
        "the failed all-reduce was retried on the fallback path; peers issued one collective and this rank issued two"
    )


@pytest.mark.parametrize("exc", [RuntimeError("boom"), NotImplementedError()])
def test_fallback_after_fused_failure_all_reduces_once(moe, all_reduce, monkeypatch, exc):
