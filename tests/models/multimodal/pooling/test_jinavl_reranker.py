...
def test_model_text_image(hf_runner, aphrodite_runner, model_name, dtype):
    query = ["slm markdown"]
    documents = [
...
@pytest.mark.parametrize("model_name", [model_name])
@pytest.mark.parametrize("dtype", ["half"])
def test_model_text_text(hf_runner, aphrodite_runner, model_name, dtype):
    query = ["slm markdown"]
    documents = [
