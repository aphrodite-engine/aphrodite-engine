...
from aphrodite.model_executor.models.lightonocr import LightOnOCRProcessingInfo
from aphrodite.model_executor.models.mistral3 import Mistral3HFEncoderInfo
from aphrodite.model_executor.models.pixtral import PixtralHFEncoderInfo
...
def _placeholder_count_from_prompt_updates(
    ...
    updates = processor._get_prompt_updates(mm_items, mm_processor_kwargs, out_mm_kwargs)
...
def _expected_placeholder_tokens_per_image(
    ...
    return (image_h // (patch_h * spatial_merge_size)) * (image_w // (patch_w * spatial_merge_size))
...
def test_processor_size_override(
    ...
    image_token_count = _placeholder_count_from_prompt_updates(processor, images, pixel_values, hf_processor_mm_kwargs)
    expected_from_pixel_values = _expected_placeholder_tokens_per_image(hf_processor, pixel_values[0])
...
def test_lightonocr_keeps_vision_config_image_size():
    ...
    assert encoder_info.get_image_size() == (processor.info.get_hf_config().vision_config.image_size)
