...
(issue https://github.com/vllm-project/vllm/issues/35909).
"""
...
def _probe_mm_token_budgets(model_id: str, mm_processor_kwargs: dict[str, Any] | None) -> tuple[int, int]:
    ctx = build_model_context(
        model_id,
    ...
    video = info.get_max_video_tokens(seq_len=500000, mm_counts={"video": 1, "image": 1})
...
def test_processor_kwargs_videos_kwargs_does_not_leak_into_image_budget(
    ...
    scoped_video, scoped_image = _probe_mm_token_budgets(model_id, {"videos_kwargs": {"size": _LONG_VIDEO_SIZE}})
    flat_video, flat_image = _probe_mm_token_budgets(model_id, {"size": _LONG_VIDEO_SIZE})
...
def test_processor_video_embeds_with_timestamps(model_id: str) -> None:
    ...
    grid_thw = (2, 4, 4)
    mm_data = _build_video_embeds_mm_data(model_config.get_inputs_embeds_size(), grid_thw)
    ...
    timestamp_tokens = sum(
        len(tokenizer.encode(f"<{float(i):.1f} seconds>", add_special_tokens=False)) for i in range(t)
    )
...
def test_dummy_video_spreads_budget_when_frame_cap_enabled(model_id: str) -> None:
    ...
    assert capped_frames == 16, f"Expected the dummy to spread the budget over 16 frames, got {capped_frames}"
    ...
    assert uncapped_frames == 2, f"Expected the uncapped dummy to keep 2 frames, got {uncapped_frames}"
