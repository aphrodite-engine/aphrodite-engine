...
def _assert_image_outputs(processor, processed, num_images: int) -> None:
    ...
    assert [placeholder.length for placeholder in image_placeholders] == (expected_tokens)
...
def _assert_video_outputs(processor, processed) -> None:
    ...
    assert prompt_token_ids.count(hf_processor.vision_start_token_id) == expected_frame_wrappers
    assert prompt_token_ids.count(hf_processor.vision_end_token_id) == expected_frame_wrappers
