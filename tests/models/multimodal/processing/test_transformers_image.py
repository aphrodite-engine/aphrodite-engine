...
from aphrodite.model_executor.models.transformers.multimodal import (
...
def _process_one_gemma3_image(processor_cls):
    ...
    result = mm_processor(
        ...
        mm_items=mm_processor.info.parse_mm_data({"image": ImageAsset("cherry_blossom").pil_image}),
        hf_processor_mm_kwargs={},
    )
...
def test_tokens_structuring_an_image_are_masked_not_dropped(processor_cls):
    ...
    mm_processor = create_processor("HuggingFaceTB/SmolVLM-256M-Instruct", processor_cls)
    result = mm_processor(
