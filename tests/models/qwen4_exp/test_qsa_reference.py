...
from aphrodite.models.qwen4_exp.common import qsa_cache
from aphrodite.models.qwen4_exp.common.qsa_cache import QSAMetadataBuilder
from aphrodite.models.qwen4_exp.nvidia import indexer_qsa
from aphrodite.models.qwen4_exp.nvidia import (
    model as _qwen4_exp_model,  # noqa: F401
)
from aphrodite.models.qwen4_exp.nvidia.ops import qsa as qsa_ops
from aphrodite.models.qwen4_exp.nvidia.ops import qsa_indexer as qsa_indexer_ops
...
def _qsa_relative_topk_reference(
    ...
    for row in range(logits.shape[0]):
        ...
        if width:
            output[row, :width] = torch.topk(logits[row, start : start + length], width).indices.to(torch.int32)
    return output
...
def _qsa_sparse_paged_attention_reference(
    ...
    for row in range(q.shape[0]):
        ...
        output[row] = torch.einsum("hk,khd->hd", probabilities, values.float()).to(q.dtype)
    return output
...
def _qsa_key_cache(block_size: int, compress_ratio: int) -> qsa_cache.QSAKeyStateCache:
    return qsa_cache.QSAKeyStateCache(
        ...
        aphrodite_config=SimpleNamespace(compilation_config=SimpleNamespace(static_forward_context={})),
...
def test_qsa_ring_capacity_covers_one_speculative_step(compress_ratio: int, num_spec: int, expected: int) -> None:
    """Capacity spans the open group plus one speculative step, in whole groups."""
    spec = _qsa_key_cache(block_size=48, compress_ratio=compress_ratio).get_kv_cache_spec(
...
def test_qsa_compressed_metadata_keeps_dummy_slots_inert() -> None:
    ...
    token_to_req = torch.tensor([0, 0, 0, 2, 2, 2, 2, 2], dtype=torch.int32, device=device)
    common = SimpleNamespace(
        num_actual_tokens=8,
        num_reqs=3,
        max_query_len=5,
...
def test_qsa_unfused_cache_update_ignores_padded_qk() -> None:
    ...
    indexer = SimpleNamespace(
        ...
        rotary_emb=rope,
        compress_ratio=4,
        raw_key_cache=SimpleNamespace(kv_cache=raw_cache, key_cache=raw_cache, rope_position_cache=None),
        compressed_key_cache=SimpleNamespace(kv_cache=compressed_cache),
    )
    keys = torch.arange(1, 6, dtype=torch.bfloat16, device=device)[:, None].expand(5, 64)
...
def test_qsa_triton_metadata_matches_pytorch(compress_ratio: int, num_reqs: int) -> None:
    device = torch.device("cuda")
    num_tokens = 8
    query_start_loc = torch.tensor([0, 3, *([3] * (num_reqs - 2)), 8], dtype=torch.int32, device=device)
    token_to_req = torch.tensor(
        [0, 0, 0, *([num_reqs - 1] * 5)],
        dtype=torch.int32,
        device=device,
    ...
    block_table_storage = block_table_rows[torch.arange(num_reqs, device=device) % block_table_rows.shape[0]]
    ...
    common = SimpleNamespace(
        ...
        query_start_loc_cpu=query_start_loc.cpu(),
        seq_lens=seq_lens,
        slot_mapping=torch.tensor([0, 1, -1, 3, 4, -1, -1, -1], dtype=torch.int64, device=device),
    ...
    max_num_work = (num_tokens + (compress_ratio - 1) * num_reqs) // compress_ratio if compress_ratio != 1 else 0
    actual_k_work = torch.empty(max_num_work, 2, dtype=torch.int32, device=device) if max_num_work else None
    ...
    expected_k_work = torch.empty_like(actual_k_work) if actual_k_work is not None else None
...
def test_qsa_fused_metadata_matches_pytorch_for_large_padded_prefill() -> None:
    ...
    num_tokens = 4224
    query_start_loc = torch.tensor([0, num_mapped_tokens], dtype=torch.int32, device=device)
    common = SimpleNamespace(
        num_actual_tokens=num_tokens,
        query_start_loc=query_start_loc,
        query_start_loc_cpu=query_start_loc.cpu(),
        seq_lens=torch.tensor([num_mapped_tokens + 32], dtype=torch.int32, device=device),
...
def test_qsa_decode_selection_correctness(decode_query_len: int, num_requests: int, dtype: torch.dtype) -> None:
    torch.manual_seed(1)
    heads, head_dim = 4, 128
    rows = num_requests * decode_query_len
    q = torch.randn(rows, heads, head_dim, device="cuda", dtype=torch.bfloat16).to(dtype)
    page_size, pages_per_request, max_sequence_length = (16, 40, 2560) if num_requests > 32 else (4, 20, 320)
    ...
    page_table = torch.randperm(num_pages, device="cuda", dtype=torch.int32).reshape(num_requests, pages_per_request)
    token_to_req = torch.repeat_interleave(
        torch.arange(num_requests, device="cuda", dtype=torch.int32),
        decode_query_len,
    )
    sequence_lengths = max_sequence_length - 4 * (torch.arange(num_requests, device="cuda", dtype=torch.int32) % 8)
    ...
    actual = torch.empty((rows, token_topk // compress_ratio), device="cuda", dtype=torch.int32)
    ...
    if dtype == torch.float8_e4m3fn:
        ...
        logits = _qsa_mqa_paged_reference(q, cache, page_table, token_to_req, visible_blocks)
...
def test_qsa_prefill_selection_correctness(
    ...
    if force_chunk:
        monkeypatch.setenv("APHRODITE_SPARSE_INDEXER_MAX_LOGITS_MB", "0")
    torch.manual_seed(2)
    query_lens = [3, 33]
    rows, heads, head_dim = sum(query_lens), 4, 128
    q = torch.randn(rows, heads, head_dim, device="cuda", dtype=torch.bfloat16).to(dtype)
    cache = torch.randn(128, 24, 1, head_dim, device="cuda", dtype=torch.bfloat16).to(dtype)
    ...
    query_positions = torch.cat(
        [
            ...
            for query_len, length in zip(query_lens, sequence_lengths.tolist(), strict=True)
        ]
    )
    token_topk, compress_ratio = 2048, 4
    visible_blocks = torch.minimum(
...
def test_qsa_sparse_paged_attention_correctness(
    ...
    block_table = (
        torch.randperm(num_cache_blocks, device="cuda").reshape(num_requests, num_pages_per_request).to(torch.int32)
    )
    ...
    request_row_counts = torch.full((num_requests,), rows_per_request, device="cuda", dtype=torch.int32)
    ...
    block_indices = (selected_pages * compressed_blocks_per_page + selected_offsets).to(torch.int32)
    rows_within_request = row_indices % rows_per_request
    query_positions = (
        context_lengths[token_to_req.long()] - request_row_counts[token_to_req.long()] + rows_within_request
    ).to(torch.int64)
    ...
    logical_indices = torch.empty((num_rows, selection_width + 1), device="cuda", dtype=torch.int32)
    qsa_indexer_ops.expand_qsa_block_indices(
        block_indices,
        query_positions,
        visible_blocks,
...
def test_qsa_split_selection_correctness(workspace_init, decode_query_len: int) -> None:
    ...
    query_positions = torch.cat([torch.arange(2560 - query_len, 2560, device="cuda") for query_len in query_lens])
...
def test_qsa_streaming_compression_and_compressor_state_store_match_reference() -> None:
    ...
    def key_row(request: int, position: int) -> torch.Tensor:
        return torch.arange(head_dim, dtype=torch.float32) + request * 1000 + position * 10
    ...
    raw_positions = (
        torch.stack([position_row(request, position) for request, position in current_pairs])
        .unsqueeze(1)
        .to(device="cuda")
    )
    token_to_req = torch.tensor(
    ...
    compressor_state_block_table = torch.tensor([[1], [0]], dtype=torch.int32, device="cuda")
    compressor_state_cache = torch.zeros(2, 4, 1, head_dim, dtype=torch.bfloat16, device="cuda")
    ...
    compressed_slots = torch.full((len(current_pairs),), -1, dtype=torch.int64, device="cuda")
    ...
    pooled_without_rope, scalar_first_positions = qsa_ops.qsa_compress_groups_with_ratio(
        raw_keys,
        raw_positions,
        compressor_state_cache,
        compressor_state_block_table,
    ...
    expected_pooled = (
        torch.stack([torch.stack([key_row(*pair) for pair in group]).mean(dim=0) for group in groups])
        .unsqueeze(1)
        .to(device="cuda", dtype=torch.bfloat16)
    )
    expected_positions = torch.stack([position_row(0, 0), position_row(0, 4), position_row(1, 4)]).to("cuda")
    ...
    torch.testing.assert_close(scalar_first_positions[valid_rows], expected_scalar_positions)
    ...
    qsa_ops.qsa_store_cache_rows(compressor_state_cache, compressor_state_slots, raw_keys)
