...
from aphrodite.models.qwen4_exp.config import (
    Qwen4ExpConfig,
    Qwen4ExpTextConfig,
)
from aphrodite.models.qwen4_exp.nvidia.model_state import Qwen4ExpModelState
...
def test_qwen4_exp_mtp_returns_sample_and_multi_streams() -> None:
    from aphrodite.models.qwen4_exp.nvidia.mtp import (
    ...
    with patch(
        "aphrodite.models.qwen4_exp.nvidia.mtp.get_pp_group",
...
def test_qwen4_exp_rejects_pipeline_parallel_only_with_ple(ple_layer_ids) -> None:
    ...
    aphrodite_config = SimpleNamespace(
        ...
        parallel_config=SimpleNamespace(pipeline_parallel_size=2, enable_dbo=False, ubatch_size=1),
        speculative_config=None,
    )
    with patch.object(Qwen3_5ForConditionalGenerationConfig, "verify_and_update_config"):
        if ple_layer_ids:
...
def test_qwen4_exp_model_state_prepares_ngram_context() -> None:
    ...
    req_states = SimpleNamespace(
        ...
        all_token_ids=SimpleNamespace(gpu=torch.tensor([[1, 2, 3, 4], [20, 21, 22, 23]], dtype=torch.int32)),
    )
...
def test_qwen4_exp_model_state_prepares_stable_dummy_ngram_inputs() -> None:
    ...
    torch.testing.assert_close(second["query_start_loc"], torch.tensor([0, 2, 5, 8], dtype=torch.int32))
    torch.testing.assert_close(second["ngram_context"], torch.full((3, 3), 99, dtype=torch.int32))
