...
from aphrodite.models.qwen4_exp.nvidia.ops.hc import (
...
def test_hc_gate_mix() -> None:
    ...
    expected = (
        torch.sigmoid(gate.float().unflatten(-1, (HC, HIDDEN_SIZE))) * x.float().unflatten(-1, (HC, HIDDEN_SIZE))
    ).mean(-2)
...
def test_hc_combine() -> None:
    ...
    expected = expected + block_output.float().unsqueeze(-2) * injection_weight.unsqueeze(-1)
...
def test_hc_combine_norm() -> None:
    ...
    actual, actual_norm = hc_combine_norm(residual, block_output, injection, weight, EPS, HC)
...
def test_hc_combine_norm_unit_injection(num_tokens: int) -> None:
    torch.manual_seed(0)
    embedding = torch.randn(num_tokens, HIDDEN_SIZE, dtype=torch.bfloat16, device="cuda")
    hidden = torch.randn(num_tokens, HC, HIDDEN_SIZE, dtype=torch.bfloat16, device="cuda")
    weight = torch.randn(HYPER_HIDDEN_SIZE, dtype=torch.bfloat16, device="cuda")

    actual, actual_norm = hc_combine_norm(hidden.flatten(1), embedding, None, weight, EPS, HC)
