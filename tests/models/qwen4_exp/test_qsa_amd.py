...
from aphrodite.models.qwen4_exp.amd import (
    model as _qwen4_exp_model,  # noqa: F401
)
from aphrodite.models.qwen4_exp.amd import ple_layer as ple_layer_module
from aphrodite.models.qwen4_exp.amd.indexer_qsa import (
    apply_qsa_rmsnorm,
    apply_qsa_rope,
)
from aphrodite.models.qwen4_exp.amd.ops import qsa as qsa_ops
...
def test_qsa_rmsnorm_uses_portable_implementation(default_aphrodite_config) -> None:
...
def test_qsa_selection_uses_portable_topk_on_rocm(
    ...
    selection_output = torch.empty((rows, token_topk + compress_ratio - 1), dtype=torch.int32)
...
def test_qsa_sparse_paged_attention_matches_reference(
    ...
    query_positions = (context_length - request_row_counts[token_to_req.long()] + rows_within_request).to(torch.int64)
    sequence_lengths = torch.full((num_requests,), context_length, device="cuda", dtype=torch.int32)
