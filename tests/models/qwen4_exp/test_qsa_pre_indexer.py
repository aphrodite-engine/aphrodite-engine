...
from aphrodite.models.qwen4_exp.common.qsa_cache import (
    canonical_qsa_rope_positions,
    circular_qsa_slot_mapping,
    compressed_qsa_slot_mapping,
)
from aphrodite.models.qwen4_exp.nvidia.indexer_qsa import apply_qsa_rope
from aphrodite.models.qwen4_exp.nvidia.ops.qsa import (
    qsa_compress_groups_with_ratio,
    qsa_store_cache_rows,
)
from aphrodite.models.qwen4_exp.nvidia.ops.qsa_pre_indexer import (
    qsa_pre_indexer,
)
...
def assert_fp8_within_one_ulp(actual: torch.Tensor, expected: torch.Tensor) -> None:
    ...
    code_diff = (actual.view(torch.uint8).int() - expected.view(torch.uint8).int()).abs()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(False, False, True, 4, *MIXED_BATCH, id="text-with-position-cache"),
...
def test_qsa_fused_pre_indexer_matches_unfused(
    ...
    token_to_req = torch.cat(
        [torch.full((length,), request, dtype=torch.int32) for request, length in enumerate(query_lens)]
    ).to(device)
    ...
    query_start_loc = torch.tensor([0, *torch.tensor(query_lens).cumsum(0).tolist()], dtype=torch.int32).to(device)
    positions = (
        torch.stack(
            [
                logical_positions,
    ...
    compressed_block_counts = [(seq_len // CR + COMP_PAGE - 1) // COMP_PAGE for seq_len in seq_lens]
    raw_block_table, num_raw_blocks = _make_block_table([1] * len(seq_lens))
    compressed_block_table, num_compressed_blocks = _make_block_table(compressed_block_counts)
    ...
    group_counts = torch.tensor(
        [seq_len // CR - (seq_len - query_len) // CR for seq_len, query_len in zip(seq_lens, query_lens)],
        dtype=torch.int32,
        device=device,
    )
    ...
    local_work = torch.arange(int(k_start_loc[-1]), dtype=torch.int32, device=device) - torch.repeat_interleave(
        k_start_loc[:-1], k_work_counts
    )
    max_k_work = (num_tokens + (CR - 1) * len(query_lens)) // CR
    k_work_metadata = torch.full((max_k_work, 2), -1, dtype=torch.int32, device=device)
    k_work_metadata[: work_requests.numel()] = torch.stack((work_requests, local_work), dim=1)

    raw_width = D + 12 if cache_rope_positions else D
    # Match Aphrodite's padded-page cache layout: rows are contiguous, while physical
    ...
    projected_qk = torch.randn(num_tokens, (HQ + 1) * D, dtype=torch.bfloat16, device=device)
    ...
    unfused_query = gemma_rmsnorm(unfused_query.reshape(-1, D), q_weight, EPS).reshape_as(unfused_query)
    unfused_query = apply_qsa_rope(rope, positions, unfused_query)

    raw_keys = unfused_raw[..., :D]
    rope_positions = unfused_raw[..., ROPE_POS_OFFSET:].view(torch.int64) if cache_rope_positions else None
    ...
    compressed_rows = gemma_rmsnorm(pooled.reshape(-1, D), k_weight, EPS).reshape(-1, 1, D)
    group_positions = first_positions.transpose(0, 1) if mrope else first_positions[:, 0]
    ...
    if indexer_dtype == torch.float8_e4m3fn:
        ...
        torch.testing.assert_close(fused_compressed, unfused_compressed, rtol=RTOL, atol=ATOL)
