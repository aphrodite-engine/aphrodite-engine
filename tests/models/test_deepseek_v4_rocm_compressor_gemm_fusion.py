...
import aphrodite.model_executor.offloader as offloader
...
def test_prepare_compressor_gemm_fusion_aliases_original_weights():
    ...
    torch.testing.assert_close(state_dict["compressor.fused_wkv_wgate.weight"], main_value)
...
def test_prepare_compressor_gemm_fusion_skips_weight_offloading(monkeypatch):
    ...
    assert main_weight.untyped_storage().data_ptr() != (indexer_weight.untyped_storage().data_ptr())
...
def test_fused_input_projection_uses_one_mm_and_returns_expected_tuple(
    ...
    hidden_states = torch.tensor([[2.0, 3.0, 5.0], [7.0, 11.0, 13.0]], dtype=torch.float32)
    ...
    qr_out, kv_score, indexer_kv_score, indexer_weights = attention._run_parallel_input_projections(hidden_states)
