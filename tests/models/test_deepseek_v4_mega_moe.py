...
from aphrodite.models.deepseek_v4.nvidia.dspark import DSparkDeepseekV4ForCausalLM
...
from aphrodite.models.deepseek_v4.nvidia.mtp import DeepSeekV4MTP
...
def test_deep_gemm_mega_moe_capture_precedes_eplb(monkeypatch, use_kimi):
    ...
    if use_kimi:
        from aphrodite.models.kimi_k3.nvidia.model import KimiK3MegaMoEExperts
    ...
    monkeypatch.setattr("aphrodite.utils.deep_gemm._import_deep_gemm", lambda: SimpleNamespace())
...
def test_deepseek_v4_mega_moe_finalizes_native_shared_expert_weights(monkeypatch):
    ...
    def fp8_parameter(*shape):
        return torch.nn.Parameter(torch.empty(*shape, dtype=torch.float8_e4m3fn), requires_grad=False)
    ...
    monkeypatch.setattr("aphrodite.utils.deep_gemm._import_deep_gemm", lambda: FakeDeepGemm)
    ...
    assert experts._transformed_shared_l1_weights[0].data_ptr() == shared_experts.gate_up_proj.weight.data_ptr()
    assert experts._transformed_shared_l2_weights[0].data_ptr() == shared_experts.down_proj.weight.data_ptr()


@pytest.mark.parametrize("fused", [False, True])
def test_deepseek_v4_mega_moe_does_not_double_add_fused_shared_expert(monkeypatch, fused):
    ...
    monkeypatch.setattr(
        "aphrodite.models.deepseek_v4.nvidia.model.fused_topk_bias",
        lambda **kwargs: (
...
def test_deepseek_v4_mega_moe_fused_input_staging_is_bitwise_exact():
    from aphrodite.third_party.deep_gemm.utils import per_token_cast_to_fp8
...
def test_deepseek_v4_mega_moe_stages_shared_scale_tma_layout(shared_block_m):
    ...
    num_shared_rows = ((num_tokens + shared_block_m - 1) // shared_block_m) * (aligned_block_m)
    ...
    for token_id in range(num_tokens):
        ...
        transposed_m = (m_in_block // 128) * 128 + (m_in_block % 32) * 4 + (m_in_block % 128) // 32
