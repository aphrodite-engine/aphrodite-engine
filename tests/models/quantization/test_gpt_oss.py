...
if current_platform.is_rocm():
    from aphrodite.platforms.rocm import on_gfx950, on_gfx1250
else:
...
def test_aiter_mxfp4_moe_ignores_padded_rows(
    ...
    weight_backend = Mxfp4MoeBackend.AITER_MXFP4_FP8 if is_w4a8 else Mxfp4MoeBackend.TRITON
    ...
    layer.w13_bias = torch.randn((num_experts, 2 * intermediate_size), dtype=torch.float32, device=device)
    layer.w2_bias = torch.randn((num_experts, hidden_size), dtype=torch.float32, device=device)
    # Read off `layer`, not off the arguments, by the AITER_MXFP4_FP8 branch.
    layer.w13_input_scale = torch.full((num_experts,), 0.1, dtype=torch.float32, device=device)
    layer.w2_input_scale = torch.full((num_experts,), 0.1, dtype=torch.float32, device=device)
    ...
    quant_config_factory = mxfp4_w4a8_moe_quant_config if is_w4a8 else mxfp4_w4a16_moe_quant_config
    ...
    hidden_states = torch.randn((TOKENS_PADDED, hidden_size), dtype=torch.bfloat16, device=device)
    gating_output = torch.randn((TOKENS_PADDED, num_experts), dtype=torch.bfloat16, device=device)
    ...
    with set_forward_context(None, get_current_aphrodite_config(), is_padding=is_padding):
        if is_w4a8:
