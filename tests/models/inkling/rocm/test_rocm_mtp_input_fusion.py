...
from aphrodite.models.inkling.amd.ops.norm import (
...
EPS = 1e-6
VOCAB = 4096
BF16_ATOL = torch.finfo(torch.bfloat16).eps


def _assert_bf16_close(actual: torch.Tensor, expected: torch.Tensor) -> None:
    # Different Triton reduction schedules can move a small number of outputs
    # by one bf16 epsilon while remaining numerically equivalent.
...
def test_embed_dual_rmsnorm_cat(n: int, t: int, ids_dtype: torch.dtype) -> None:
    ...
    _assert_bf16_close(out, _ref(hidden, w_h, w_e, emb, w_pre))
...
def test_embed_rmsnorm(n: int, t: int, ids_dtype: torch.dtype) -> None:
    ...
    out = cast(torch.Tensor, embed_rmsnorm(ids, table, w, EPS))
    assert out.shape == (t, n)
    _assert_bf16_close(out, rmsnorm(ref_emb, w, EPS) if t else ref_emb)
    ...
    res, attn_in = cast(
        ...
        embed_rmsnorm(ids, table, w, EPS, chain_weight=w_chain),
    )
    ref_res = rmsnorm(ref_emb, w, EPS) if t else ref_emb
    _assert_bf16_close(res, ref_res)
    _assert_bf16_close(attn_in, rmsnorm(ref_res, w_chain, EPS) if t else ref_res)

    # Chained without embed_norm (use_embed_norm=False).
    res, attn_in = cast(
        tuple[torch.Tensor, torch.Tensor],
        embed_rmsnorm(ids, table, None, EPS, chain_weight=w_chain),
    )
    assert torch.equal(res, ref_emb)
    _assert_bf16_close(attn_in, rmsnorm(ref_emb, w_chain, EPS) if t else ref_emb)
