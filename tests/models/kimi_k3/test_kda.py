...
from aphrodite.model_executor.layers.mamba.ops.causal_conv1d import causal_conv1d_update
from aphrodite.model_executor.layers.mamba.ops.gather_initial_states import (
    gather_initial_states,
)
from aphrodite.models.kimi_k3.amd.ops.third_party.kda import (
    fused_recurrent_kda_packed_decode as fused_recurrent_kda_packed_decode_amd,
)
from aphrodite.models.kimi_k3.nvidia import kda as nvidia_kda
from aphrodite.models.kimi_k3.nvidia.kda import (
    _flashkda_prefill,
...
from aphrodite.models.kimi_k3.nvidia.ops import recoverssm as recoverssm_ops
...
def test_kda_recoverssm_config_state_layout():
    ...
    assert KimiLinearForCausalLM.get_mamba_state_dtype_from_config(aphrodite_config) == (
        torch.bfloat16,
        torch.float32,
        torch.float32,
        torch.bfloat16,
    )
    assert KimiLinearForCausalLM.get_mamba_state_shape_from_config(aphrodite_config)[2:] == (
        (4, 3, 32),
        (4, 3, 64),
    )
...
def test_chunk_kda_fused_gate_cumsum_matches_unfused(
    ...
    if lower_bound is not None:
        expected_gate = lower_bound * torch.sigmoid(
            A_log.exp()[None, :, None] * (raw_g.float().view(T, H, D) + dt_bias.view(H, D))
        )
...
def test_packed_kda_decode_correctness(
    ...
    q, k, v = (x.contiguous().view(1, num_seqs, H, D) for x in mixed_qkv.split(H * D, dim=-1))
    raw_g = torch.randn(
        1,
        num_seqs,
        H,
...
def test_kda_spec_decode_correctness(
    ...
    for seq, accepted in enumerate(num_accepted_tokens.tolist()):
        recurrent_state = expected_state[state_indices[seq, accepted - 1]].transpose(-1, -2)
        start = seq * query_len
        for token in range(query_len):
            ...
            expected_state[state_indices[seq, token]] = recurrent_state.transpose(-1, -2)
...
def test_kda_recoverssm_verify_and_group_commit(
    ...
    state_indices = torch.tensor([5, 6] if align_mode else [1, 2], dtype=torch.int32, device=DEVICE)
    accepted = [2, 8]
    ...
    if align_mode:
        ...
        rows = request_indices if request_indices is not None else torch.arange(num_seqs, device=DEVICE)
    ...
    for layer_idx in range(num_layers):
        A_log = (0.2 * torch.randn(num_heads, dtype=torch.float32, device=DEVICE) + layer_idx * 0.03).contiguous()
        dt_bias = (0.1 * torch.randn(num_heads, dim, dtype=torch.float32, device=DEVICE)).contiguous()
        ...
        normalized_q = q.float() * torch.rsqrt(q.float().square().sum(dim=-1, keepdim=True) + 1e-6)
        normalized_k = k.float() * torch.rsqrt(k.float().square().sum(dim=-1, keepdim=True) + 1e-6)
        gate_input = raw_g.float() + dt_bias.view(1, 1, num_heads, dim)
        if lower_bound is None:
            gate = -A_log.exp().view(1, 1, num_heads, 1) * F.softplus(gate_input)
        else:
            gate = lower_bound * torch.sigmoid(A_log.exp().view(1, 1, num_heads, 1) * gate_input)
        beta = raw_beta.float().sigmoid()
    ...
    for layer_idx, layer in enumerate(layers):
        ...
        for seq_idx, commit_len in enumerate(accepted):
            ...
            if align_mode and 4 + commit_len >= 8:
                ...
                if conv_state_dim_first:
                    actual_boundary_conv = layer.kv_cache[0][boundary_block, :, :history_len]
                    expected_boundary_conv = initial_conv_states[layer_idx][
                        state_indices[seq_idx], :, 3 : 3 + history_len
                    ]
                else:
                    actual_boundary_conv = layer.kv_cache[0][boundary_block, :history_len]
                    expected_boundary_conv = initial_conv_states[layer_idx][state_indices[seq_idx], 3 : 3 + history_len]
...
def test_fused_kda_decode_correctness(
    ...
    dim = num_heads * D
    slots = num_seqs + 2
    packed_x_storage = torch.randn(num_seqs, 3 * dim + 17, dtype=torch.bfloat16, device=DEVICE)
    ...
    if conv_layout == "DS":
        ...
        # (stride (W-1, 1)), matching APHRODITE_SSM_CONV_STATE_LAYOUT=DS.
...
def test_flashkda_correctness():
    ...
    q, k, v, raw_g = [torch.randn(B, T, H, D, dtype=torch.bfloat16, device=DEVICE) for _ in range(4)]
    ...
    gate = lower_bound * torch.sigmoid(A_log.exp()[None, None, :, None] * (raw_g.float() + dt_bias[None, None, :, :]))
    ...
    checkpoint_state_indices = torch.tensor([1, NULL_BLOCK_ID], dtype=torch.int32, device=DEVICE)
