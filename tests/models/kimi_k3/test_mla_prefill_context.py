...
pytestmark = pytest.mark.skipif(not current_platform.is_cuda(), reason="Kimi-K3 fused MLA requires CUDA")
...
class _RecordingPrefillBackend:
    ...
    def run_prefill_context_chunk(self, *, chunk, q, k, v, out=None):
        ...
        num_q = q.shape[0]
        if out is None:
            out = torch.empty((num_q, _NUM_HEADS, _V_HEAD_DIM), device=q.device, dtype=torch.bfloat16)
        else:
    ...
    * a bf16 weight -- what a stock K3 checkpoint carries -- is a plain
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, None]:
        if self.weight.dtype == torch.bfloat16 and x.dtype != torch.bfloat16:
            raise RuntimeError(
                f"a bfloat16 kv_b_proj cannot consume the gathered latent as {x.dtype}; it must be cast first"
            )
        return torch.nn.functional.linear(x.to(torch.bfloat16), self.weight.to(torch.bfloat16)), None
...
def _build_prefill_metadata(
    ...
    workspace = torch.empty((_WORKSPACE_TOKENS, _ENTRY), dtype=workspace_dtype, device=device)
    ...
    block_table = torch.arange(num_blocks, dtype=torch.int32, device=device).view(len(_CONTEXT_LENS), max_blocks)
...
@pytest.mark.parametrize("kv_b_proj_quantized", [True, False], ids=["fp8_kv_b_proj", "bf16_kv_b_proj"])
@torch.inference_mode()
def test_fused_context_matches_generic_impl(kv_b_proj_quantized: bool, kv_cache_dtype: str, honors_out: bool) -> None:
    ...
    prefill_fused, num_blocks = _build_prefill_metadata(device, workspace_dtype, q_data_type, backend_fused)
    prefill_ref, _ = _build_prefill_metadata(device, workspace_dtype, q_data_type, backend_ref)

    cache = torch.randn((num_blocks, _BLOCK_SIZE, _ENTRY), device=device, dtype=torch.bfloat16)
    ...
    fused_out, fused_lse = layer._compute_prefill_context(q, SimpleNamespace(prefill=prefill_fused))

    impl = _ReferenceImpl(kv_b_proj, kv_cache_dtype)
    ref_out, ref_lse = impl._compute_prefill_context(q, kv_cache, SimpleNamespace(prefill=prefill_ref), k_scale)

    assert len(backend_fused.calls) == len(backend_ref.calls)
    for chunk_idx, (fused_call, ref_call) in enumerate(zip(backend_fused.calls, backend_ref.calls, strict=True)):
        for name, fused_t, ref_t in zip(("q", "k", "v"), fused_call, ref_call, strict=True):
...
def test_fused_context_rejects_an_unquantized_query() -> None:
    ...
    device = torch.device("cuda")
    fp8 = current_platform.fp8_dtype()
    prefill, num_blocks = _build_prefill_metadata(device, fp8, fp8, _RecordingPrefillBackend())
    layer = _FusedLayer(
