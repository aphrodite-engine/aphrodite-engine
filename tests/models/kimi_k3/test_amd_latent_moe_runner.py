...
from aphrodite.model_executor.layers.fused_moe.runner import moe_runner
from aphrodite.model_executor.layers.layernorm import RMSNorm
from aphrodite.model_executor.layers.linear import ReplicatedLinear
from aphrodite.models.kimi_k3.amd.latent_moe_runner import ROCmLatentMoERunner
from aphrodite.models.kimi_k3.amd.linear import KimiRoutedOutputTransform
...
def _tail_runner(transform: KimiRoutedOutputTransform, tp_size: int) -> ROCmLatentMoERunner:
...
def _rank_partials(num_tokens: int, device: torch.device) -> tuple[torch.Tensor, torch.Tensor]:
...
def _check_writes_only_its_own_shard(device: torch.device, tp_size: int, rank: int) -> None:
    ...
    torch.testing.assert_close(local[:, start:end], before[:, start:end] + projected, atol=8e-2, rtol=3e-2)
...
def test_forward_shards_only_when_the_tail_is_valid(
    ...
    monkeypatch.setattr(ROCmLatentMoERunner, "_fused_forward", lambda *a, **k: sharded_output)
    monkeypatch.setattr(ROCmLatentMoERunner, "_fused_output_is_reduced", property(lambda _: pre_reduced))
...
def build_runner(monkeypatch: pytest.MonkeyPatch):
    ...
    def _base_init(
        ...
        *,
        ...
        self.moe_config = SimpleNamespace(tp_size=tp_size, is_sequence_parallel=is_sequence_parallel)
        self.routed_output_transform = SimpleNamespace(
            norm=None,
            up_proj=SimpleNamespace(weight=torch.zeros(hidden, LATENT_SIZE)) if has_up_proj else None,
        )
...
def test_falls_back_when_the_config_breaks_the_shard(build_runner, override: dict) -> None:
