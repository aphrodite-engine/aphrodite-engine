...
from aphrodite.models.kimi_k3.nvidia import model as k3_model
    ...
    for i in range(END_LAYER):
        consumers.append(
            SimpleNamespace(
                ...
                self_attention_res_proj=SimpleNamespace(weight=torch.full((1, 2), float(i))),
                prev_valid_blocks=i,
            )
        )
    return SimpleNamespace(
...
@pytest.mark.parametrize("enabled,use_attn_res", [(False, True), (True, False), (False, False)])
def test_disabled_reproduces_the_plain_residual_sum(recorder, monkeypatch, enabled, use_attn_res):
...
def test_last_layer_on_the_final_rank_uses_the_output_aggregation(recorder, monkeypatch):
    """Nothing downstream but the model's own output-side mixture."""
    _set_last_rank(monkeypatch, True)

    _call(_stub_model(enabled=True), END_LAYER - 1, torch.zeros(2), None, torch.zeros(2))
...
def test_aux_layer_at_non_final_pp_boundary_is_rejected(monkeypatch):
    ...
    model.end_layer = 72
    monkeypatch.setattr("aphrodite.distributed.parallel_state.model_parallel_is_initialized", lambda: False)
...
def test_pending_mlp_output_is_folded_in_rather_than_passed_as_delta(recorder, monkeypatch):
