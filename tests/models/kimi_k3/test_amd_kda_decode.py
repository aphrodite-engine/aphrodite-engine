...
def _requires_kernel() -> None:
    if not hasattr(torch.ops._C, "fused_kda_decode"):
        pytest.skip("Aphrodite was built without the fused KDA decode kernel")
...
class KdaDecodeInputs:
    ...
    def __init__(
        ...
        self.mixed_qkv = torch.randn(num_tokens, 3 * dim, device=device, dtype=DTYPE) * 0.5
        # conv1d weight as the layer holds it: [3 * dim, width] fp32.
        self.conv_weights = torch.randn(3 * dim, CONV_WIDTH, device=device, dtype=torch.float32) * 0.3
        ...
        self.conv_state = torch.randn(num_slots, CONV_WIDTH - 1, 3 * dim, device=device, dtype=DTYPE) * 0.5
        ...
        self.g1 = torch.randn(1, num_tokens, num_heads, HEAD_DIM, device=device, dtype=DTYPE) * 0.5
        self.g2 = torch.randn(num_tokens, num_heads, HEAD_DIM, device=device, dtype=DTYPE) * 0.5
        ...
        self.norm_weight_bf16 = 1 + 0.1 * torch.randn(HEAD_DIM, device=device, dtype=DTYPE)
        ...
        self.state_indices = (torch.randperm(num_slots - 1, device=device)[:num_tokens] + 1).to(torch.int32)
...
def test_fused_kda_decode_leaves_untouched_slots_alone() -> None:
    ...
    torch.testing.assert_close(actual_conv[untouched], inp.conv_state[untouched], atol=0, rtol=0)
    torch.testing.assert_close(actual_state[untouched], inp.recurrent_state[untouched], atol=0, rtol=0)
