...
def test_amd_attn_res_fused_contract(
    ...
    delta = _randn_with_row_padding(num_tokens, hidden_size, padding=11) if has_delta else None
    blocks = _randn_with_row_padding(num_tokens, block_capacity, hidden_size, padding=13)
    norm_weight = 1 + 0.1 * torch.randn(hidden_size, device="cuda", dtype=torch.bfloat16)
    qk_weight = torch.randn(hidden_size, device="cuda", dtype=torch.bfloat16) / hidden_size**0.5
    output_norm_weight = (
        1 + 0.1 * torch.randn(hidden_size, device="cuda", dtype=torch.bfloat16) if apply_output_norm else None
    )
    expected_prefix = prefix.clone()
    if delta is not None:
        expected_prefix = expected_prefix + delta
    values = torch.cat((blocks[:, :num_blocks].clone(), expected_prefix.unsqueeze(1)), dim=1)
    ...
    if output_norm_weight is not None:
        expected = F.rms_norm(expected, (hidden_size,), output_norm_weight.float(), output_eps)
