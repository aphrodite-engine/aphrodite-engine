...
from aphrodite.models.kimi_k3.nvidia.model import KimiLinearForCausalLM
from aphrodite.v1.attention.backend import AttentionMetadataBuilder
from aphrodite.v1.attention.backends.gdn_attn import (
    GDNAttentionBackend,
    GDNAttentionMetadata,
    GDNAttentionMetadataBuilder,
)
from aphrodite.v1.attention.backends.recoverssm_metadata import (
...
from aphrodite.v1.worker.mamba_utils import validate_mamba_state_copy_funcs
...
def _assert_matches_shared_gdn(reference: GDNAttentionMetadata, actual: KimiK3KDAMetadata):
    ...
    for field in fields(GDNAttentionMetadata):
        ...
        if isinstance(actual_value, torch.Tensor):
            ...
            if actual_value is not None:
                ...
                torch.testing.assert_close(actual_value[8]["nums"], expected_value[8]["nums"])
        else:
...
def _make_builder(
    ...
    aphrodite_config.cache_config.use_replayssm = use_recoverssm
    aphrodite_config.cache_config.use_kda_recoverssm = use_recoverssm
    builder = builder_cls(
...
def test_kda_recoverssm_startup_metadata_flow_without_model(monkeypatch):
    ...
    monkeypatch.setattr("aphrodite.utils.torch_utils.PIN_MEMORY", False)
    monkeypatch.setattr("aphrodite.v1.attention.backends.utils.PIN_MEMORY", False)
    ...
    validate_mamba_state_copy_funcs(
        {kv_cache_spec: [0]},
        {MambaAttentionBackendEnum.GDN_ATTN: (KimiLinearForCausalLM.get_mamba_state_copy_func())},
    )
    assert len(kv_cache_spec.shapes) == 4

    builder_config = SimpleNamespace(
        model_config=SimpleNamespace(hf_text_config=SimpleNamespace(linear_key_head_dim=32)),
        cache_config=SimpleNamespace(
            mamba_cache_mode="align",
            use_kda_recoverssm=True,
        ),
    ...
    builder = KimiK3KDAMetadataBuilder(
        ...
        layer_names=["layer.0"],
        aphrodite_config=builder_config,
        device=DEVICE,
    )
    ...
    forward_layer = Mock()
    builder.aphrodite_config.compilation_config.static_forward_context["layer.0"] = forward_layer
    ...
    with patch(
        "aphrodite.models.kimi_k3.nvidia.ops.recoverssm.KDARecoverSSMCommitContext.create",
    ...
    create_context.assert_called_once_with(
        [forward_layer],
        spec_query_len=3,
        max_num_reqs=builder.aphrodite_config.scheduler_config.max_num_seqs,
    )
...
def test_internal_checkpoint_metadata_targets_last_aligned_boundary():
    ...
    common_attn_metadata = create_common_attn_metadata(batch, BLOCK_SIZE, device, arange_block_indices=True).replace(
...
def test_kimi_k3_kda_metadata_matches_shared_gdn(
    ...
    if num_decode_draft_tokens is not None:
        kwargs = {
            "num_decode_draft_tokens_cpu": torch.tensor(num_decode_draft_tokens, dtype=torch.int32),
            "num_accepted_tokens": torch.ones(batch.batch_size, dtype=torch.int32, device=DEVICE),
        }

    common_attn_metadata = create_common_attn_metadata(batch, BLOCK_SIZE, DEVICE).replace(
...
def test_recoverssm_spec_uses_one_state_slot_and_current_window(
    ...
    if mamba_cache_mode == "none":
        ...
        assert postprocess.num_computed_tokens is common_attn_metadata.compute_num_computed_tokens()
...
def test_cudagraph_capture_metadata_avoids_device_to_host_copy(
    ...
    common_attn_metadata = create_common_attn_metadata(batch, BLOCK_SIZE, device).replace(
        is_prefilling=torch.zeros(batch.batch_size, dtype=torch.bool)
    )
    builder = _make_builder(builder_cls, num_speculative_tokens, full_cuda_graph=True, device=device)
...
def test_recoverssm_spec_cudagraph_stages_one_checkpoint_per_request():
    ...
    assert builder.spec_state_indices_tensor.shape == (
        builder.aphrodite_config.scheduler_config.max_num_seqs,
        1,
    )
    actual = builder.build_for_cudagraph_capture(common_attn_metadata)
...
def test_stage_spec_decode_metadata_matches_pytorch():
    ...
    query_start_loc = torch.arange(num_spec_decodes + 1, dtype=torch.int32, device=device) * num_state_slots
    num_accepted_tokens = torch.arange(num_spec_decodes, dtype=torch.int32, device=device) % num_state_slots + 1

    staged_state_indices = torch.empty((batch_size, num_state_slots), dtype=torch.int32, device=device)
    staged_query_start_loc = torch.empty(batch_size + 1, dtype=torch.int32, device=device)
    staged_num_accepted_tokens = torch.empty(batch_size, dtype=torch.int32, device=device)
    stage_spec_decode_metadata(
        state_indices,
        query_start_loc,
        num_accepted_tokens,
    ...
    expected_num_accepted_tokens = torch.ones(batch_size, dtype=torch.int32, device=device)
