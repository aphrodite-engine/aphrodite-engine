...
from aphrodite.config.kernel import (
...
def test_native_deep_gemm_mega_moe_not_arch_gated(architectures):
    """Aphrodite's own deep_gemm mega path is not DSv4-only (Kimi K3 uses it);
...
def test_all_mega_backends_get_sequence_parallel_moe(moe_backend):
    ...
    from aphrodite.models.deepseek_v4.nvidia.model import _use_sequence_parallel
    ...
    assert _use_sequence_parallel(aphrodite_config)
...
def test_megakernel_runtime_requirements(fake_flashinfer):
    dg = megakernel_runtime_requirements(fi_moe_ep_backend_spec("flashinfer_moe_ep_mega_deep_gemm"))
    assert dg == frozenset({"torch_dist"})

    cd = megakernel_runtime_requirements(fi_moe_ep_backend_spec("flashinfer_moe_ep_mega_cutedsl"))
    assert cd == frozenset({"torch_dist", "nvshmem"})


def test_bootstrap_pins_the_device_aphrodite_bound(fake_flashinfer, monkeypatch):
    ...
    (CUDA_ERROR_ILLEGAL_ADDRESS in the weight transforms). Aphrodite passes the
    device it already bound via BootstrapConfig.device."""
    import aphrodite.utils.flashinfer_moe_ep as mod
...
def test_ckpt_uses_nvfp4_experts_reads_moe_quant_algo():
    from aphrodite.models.deepseek_v4.nvidia.fi_moe import ckpt_uses_nvfp4_experts
