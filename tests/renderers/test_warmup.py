...
def _make_renderer_mock(mm_limits: dict[str, int]) -> MagicMock:
    ...
    renderer.start_mm_warmup_in_background = BaseRenderer.start_mm_warmup_in_background.__get__(renderer, BaseRenderer)
    renderer._join_mm_warmup = BaseRenderer._join_mm_warmup.__get__(renderer, BaseRenderer)
...
class TestMmWarmupZeroLimitFiltering:
    ...
    def test_zero_limit_modality_excluded_from_mm_counts(self):
        ...
        with patch("aphrodite.multimodal.processing.TimingContext", autospec=True):
...
class TestWarmupFaultIsolation:
    ...
    def test_mm_failure_does_not_abort_readonly_warmup(self):
        ...
        renderer.mm_processor.dummy_inputs.get_dummy_processor_inputs.side_effect = RuntimeError("mm boom")
    ...
    def test_start_submits_future_to_mm_executor(self):
        ...
        try:
            with patch("aphrodite.multimodal.processing.TimingContext", autospec=True):
                ...
                with patch.object(renderer._mm_executor, "submit", wraps=renderer._mm_executor.submit) as spy:
...
class TestEngineStartWarmupHook:
    ...
    def _mock_config(self):
        ...
        from aphrodite.v1.engine.core_client import EngineCoreClient  # noqa: F401
    ...
    def test_inproc_client_rejects_renderer(self):
        ...
        from aphrodite.v1.engine import core_client as cc
    ...
    def test_inproc_client_forwards_fail_callback_but_not_renderer(self):
        ...
        assert len(args) == 3  # aphrodite_config, executor_class, log_stats
    ...
    def test_make_async_mp_client_passes_renderer(self, client_cls, dp_size, external_lb):
        from aphrodite.v1.engine import core_client as cc
