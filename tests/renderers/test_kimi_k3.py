...
def test_kimi_k3_registered():
    ...
    assert TokenizerRegistry.load_tokenizer_cls("kimi_k3").__name__ == "CachedHfTokenizer"
...
def test_k3_media_io_defaults_preserve_original_mode():
    ...
    assert _merge_k3_media_io_kwargs({"image": {"image_mode": "RGB"}}) == {"image": {"image_mode": "RGB"}}

    # Unrelated image kwargs are merged with the default.
    assert _merge_k3_media_io_kwargs({"image": {"rgba_background_color": (0, 0, 0)}}) == {
...
def test_apply_chat_template_forces_tokenize_and_pins_return_dict():
    ...
    params = ChatParams(chat_template_kwargs={"tools": tools, "tokenize": False, "thinking": True})

    token_ids = renderer._apply_chat_template([{"role": "user", "content": "hi"}], params)
...
def test_apply_chat_template_translates_standard_thinking_kwargs():
    ...
    tokenizer = StubTokenizer([7, 8, 9])
    renderer = _make_renderer(tokenizer)
    params = ChatParams(chat_template_kwargs={"enable_thinking": False, "reasoning_effort": "none"})
...
def test_apply_chat_template_omits_tool_choice_without_tools():
    ...
    renderer._apply_chat_template([{"role": "user", "content": "hi"}], ChatParams(tool_choice=None))
...
def test_render_messages_returns_token_prompt():
    ...
    conversation, prompt = renderer.render_messages([{"role": "user", "content": "hi"}], ChatParams())
...
async def test_render_messages_async_returns_token_prompt():
    ...
    conversation, prompt = await renderer.render_messages_async([{"role": "user", "content": "hi"}], ChatParams())
