...
"""Unit tests for ``aphrodite/renderers/cohere.py``.
...
class TestContentBlocks:
    ...
    def test_document_block_dict_passthrough(self):
        out = _content_blocks([{"type": "document", "document": {"data": {"text": "doc"}}}])
        assert out == [
            {
...
class TestV2ToolContentToMelodyBlock:
    ...
    def test_document_block_with_missing_data_yields_empty_payload(self):
        ...
        result = _v2_tool_content_to_melody_block({"type": "document", "document": {"id": "d1"}})
    ...
    def test_json_parseable_text_becomes_document_dict(self):
        ...
        result = _v2_tool_content_to_melody_block({"type": "text", "text": '{"temperature": "20C"}'})
    ...
    def test_non_json_text_wrapped_under_content_key(self):
        ...
        result = _v2_tool_content_to_melody_block({"type": "text", "text": "K2 is 8611m."})
    ...
    def test_document_block_with_non_dict_document_returns_none(self):
        ...
        assert _v2_tool_content_to_melody_block({"type": "document", "document": "not a dict"}) is None
...
class TestConversationToMelody:
    ...
    def test_tool_calls_normalized(self):
        ...
        out = _conversation_to_melody_messages(conv)  # type: ignore[arg-type]
        assert out[0]["tool_calls"] == [{"id": "c1", "name": "f", "parameters": '{"a":1}'}]
    ...
    def test_tool_message_v2_content_skips_unrecognized_blocks(self):
        ...
        assert out[0]["content"] == [{"type": "document", "document": {"content": "keep me"}}]
...
class TestBuildRenderConfig:
    ...
    def test_default_format_is_cmd4(self):
        ...
        # Mirrors ``_DEFAULT_FORMAT`` in ``aphrodite/renderers/cohere.py`` and
    ...
    def test_available_tools_take_precedence_over_tools(self):
        _, cfg = _build_render_config(
            self._conv(),
            {
                ...
                "available_tools": [{"type": "function", "function": {"name": "preferred"}}],
            },
        )  # type: ignore[arg-type]
    ...
    def test_response_format_json_object_sets_json_mode(self):
        _, cfg = _build_render_config(self._conv(), {"response_format": {"type": "json_object"}})  # type: ignore[arg-type]
    ...
    def test_json_schema_kwarg_direct(self):
        ...
        _, cfg = _build_render_config(self._conv(), {"json_schema": "raw-string-schema"})  # type: ignore[arg-type]
    ...
    def test_cohere_only_template_kwargs_are_rejected(self, cohere_only_key):
        ...
        # accepted at Cohere's own API surface but not at Aphrodite's -- raw
        # template source in Aphrodite must flow through the standard
    ...
    def test_chat_template_arg_populates_template_jinja(self):
        # The standard Aphrodite ``chat_template`` request field is the sole
    ...
    def test_tool_message_v2_content_key_picked_up_from_kwargs(self):
        ...
        # message.
        from aphrodite.renderers.cohere import TOOL_MESSAGE_V2_CONTENT_KEY
        ...
        assert TOOL_MESSAGE_V2_CONTENT_KEY not in cfg.get("additional_template_fields", {})
...
async def test_async_cohere_renderer_does_not_block_event_loop():
    ...
    renderer = CohereRenderer(
        _MockAphroditeConfig(_MockModelConfig(), _MockParallelConfig()),
...
class TestRequestCitationsReachRenderedPrompt:
    ...
    def _melody():
        ...
        #   ``aphrodite.entrypoints.cohere.{protocol,serving}`` (both
    ...
    def test_document_citation_survives_to_prompt(self):
        ...
        from aphrodite.entrypoints.cohere.protocol import CohereChatV2Request
        from aphrodite.entrypoints.cohere.serving import CohereServingChatV2
        from aphrodite.renderers.cohere import _build_render_config
        ...
        assert "citations" in assistant_msg, "citations were not attached to the melody assistant message dict"
        ...
        assert "<co>Shakespeare</co: 0:[0]>" in rendered, (
            f"expected inline citation markup around the cited span; tail of rendered prompt: {rendered[-400:]!r}"
        )
    ...
    def test_no_markup_when_no_citations(self):
        ...
        conversation = self._openai_msgs_to_conversation(chat_req.messages)
        fmt, config = _build_render_config(conversation, chat_req.chat_template_kwargs or {})
