...
from aphrodite.model_executor.models.interfaces import SupportsLoRA
...
def maybe_enable_lora_dual_stream(monkeypatch: pytest.MonkeyPatch):
    if current_platform.is_cuda():
        monkeypatch.setenv("APHRODITE_LORA_ENABLE_DUAL_STREAM", "1")
        import aphrodite.lora.layers.base_linear

        if not hasattr(aphrodite.lora.layers.base_linear, "lora_linear_async"):
            # Reload the module to ensure the environment variable takes effect.
            reload(aphrodite.lora.layers.base_linear)
    yield
...
def qwen2vl_language_lora_files():
    return hf_api().snapshot_download(repo_id="prashanth058/qwen2vl-flickr-lora-language")


@pytest.fixture(scope="session")
def qwen2vl_vision_tower_connector_lora_files():
    return hf_api().snapshot_download(repo_id="prashanth058/qwen2vl-flickr-lora-tower-connector")
...
def qwen25vl_vision_lora_files():
    return hf_api().snapshot_download(repo_id="EpochEcho/qwen2.5-3b-vl-lora-vision-connector")


@pytest.fixture(scope="session")
def qwen3vl_vision_lora_files():
    return hf_api().snapshot_download(repo_id="EpochEcho/qwen3-4b-vl-lora-vision-connector")
...
def gptoss20b_lora_files():
    return hf_api().snapshot_download(repo_id="jeeejeee/gpt-oss-20b-lora-adapter-text2sql")
...
def whisper_lora_files():
    return hf_api().snapshot_download(repo_id="chengyili2005/whisper-small-mandarin-lora")
...
def qwen35_vl_lora_files():
    return hf_api().snapshot_download(repo_id="jeeejeee/qwen35-4b-all-linear-pokemon-lora")


@pytest.fixture(scope="session")
def qwen36_moe_2d_lora_files():
    return hf_api().snapshot_download(repo_id="jeeejeee/qwen36-35ba3b-2d-weights-poken-lora")


@pytest.fixture(scope="session")
def qwen36_moe_3d_lora_files():
    return hf_api().snapshot_download(repo_id="jeeejeee/qwen36-35ba3b-moe-all-linear-poken-lora")
