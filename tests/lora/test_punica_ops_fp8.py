...
from aphrodite.lora.ops.triton_ops.lora_expand_fp8_op import (
    _EXPAND_LORA_SCALE_PTR_DICT,
)
from aphrodite.lora.ops.triton_ops.lora_shrink_fp8_op import (
...
def quantize_to_fp8_blockwise(
    ...
    if tensor.ndim == 2:
        ...
        fp8_tensor = (blocks / scale.unsqueeze(-1)).clamp(FP8_MIN, FP8_MAX).to(FP8_DTYPE)
...
def generate_fp8_expand_data(
    ...
    if quant_mode == "blockwise":
        ...
        fp8_blocks = (blocks / a_scale[None, :, :, None]).clamp(FP8_MIN, FP8_MAX).to(FP8_DTYPE)
        inputs_fp8 = fp8_blocks.view(nslices, total_tokens, -1)[:, :, :rank].contiguous()
        inputs_dequant = (
