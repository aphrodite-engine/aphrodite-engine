...
def generate_and_test(llm: aphrodite.LLM, lora_path: str, lora_id: int) -> None:
    ...
    sampling_params = aphrodite.SamplingParams(temperature=0, max_tokens=128)
    ...
    for generated, expected in zip(generated_texts, EXPECTED_OUTPUTS_VISION):
        assert generated.startswith(expected), f"Generated text {generated!r} does not match expected {expected!r}"
