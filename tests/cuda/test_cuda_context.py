...
def test_get_device_capability_uses_visible_device_ordinal(monkeypatch):
    import aphrodite.platforms.interface as platform_interface
    from aphrodite.platforms.cuda import NvmlCudaPlatform, pynvml
...
def _stub_nvml(monkeypatch) -> dict[str, int]:
    ...
    monkeypatch.setattr(pynvml, "nvmlInit", lambda: calls.__setitem__("init", calls["init"] + 1))
    monkeypatch.setattr(
        pynvml,
        "nvmlShutdown",
    ...
    monkeypatch.setattr(pynvml, "nvmlDeviceGetHandleByIndex", lambda index: f"handle-{index}")
    monkeypatch.setattr(pynvml, "nvmlDeviceGetCudaComputeCapability", lambda _handle: (9, 0))
    NvmlCudaPlatform.get_device_capability.cache_clear()
    return calls
...
def test_has_device_capability_does_not_reinit_nvml(monkeypatch):
    ...
    """
    from aphrodite.platforms.cuda import NvmlCudaPlatform
