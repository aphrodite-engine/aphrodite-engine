...
from aphrodite.reasoning.muse_glimmer_reasoning_parser import MuseGlimmerReasoningParser
from aphrodite.tool_parsers.muse_glimmer_tool_parser import MuseGlimmerToolParser
...
def _req(*names):
    ...
    return SimpleNamespace(tools=[SimpleNamespace(function=SimpleNamespace(name=n)) for n in names])
...
def test_plain_answer_yields_no_tool_calls():
    out = MuseGlimmerToolParser.extract_tool_calls(T, "to=user<|message|>Just a plain answer.<|eot|>", None)
...
def test_reasoning_then_user_answer():
    raw = " to=self<|message|>thinking<|eom|><|start|>assistant to=user<|message|>The answer is 42.<|eot|>"
    reasoning, content = MuseGlimmerReasoningParser.extract_reasoning(R, raw, None)
...
def test_plain_content_without_framing_passes_through():
    reasoning, content = MuseGlimmerReasoningParser.extract_reasoning(R, "Just a direct answer.", None)
...
def test_reasoning_then_parallel_calls():
    ...
    out = MuseGlimmerToolParser.extract_tool_calls(T, content, None)
    assert [t.function.name for t in out.tool_calls] == ["math.add", "math.mul"], out.tool_calls
...
def _stream(raw: str, chunk: int):
    ...
    while i < len(raw):
        cur = raw[: i + chunk]
        delta = cur[len(prev) :]
        dm = MuseGlimmerReasoningParser.extract_reasoning_streaming(R, prev, cur, delta, [], [], [])
        if dm is not None:
        ...
        dt = MuseGlimmerToolParser.extract_tool_calls_streaming(T, prev, cur, delta, [], [], [], _FakeReq())
...
RAW_ANSWER = " to=self<|message|>Think about it.<|eom|><|start|>assistant to=user<|message|>The answer is 42.<|eot|>"
...
def test_doubled_bare_name_collapses():
    out = MuseGlimmerToolParser.extract_tool_calls(T, _call("get_weather.get_weather"), _req("get_weather"))
    assert out.tools_called and out.tool_calls[0].function.name == "get_weather", out.tool_calls[0].function.name


def test_namespaced_name_preserved():
    out = MuseGlimmerToolParser.extract_tool_calls(T, _call("weather.get"), _req("weather.get"))
    assert out.tool_calls[0].function.name == "weather.get"


def test_unregistered_namespace_is_preserved():
    # Suffix-only matching can silently dispatch a tool from the wrong namespace.
    out = MuseGlimmerToolParser.extract_tool_calls(T, _call("foo.get_weather"), _req("get_weather"))
...
def test_trailing_segment_ambiguous_left_alone():
    ...
    out = MuseGlimmerToolParser.extract_tool_calls(T, _call("x.get"), _req("weather.get", "time.get"))
    assert out.tool_calls[0].function.name == "x.get"


def test_no_registered_tools_passthrough():
    out = MuseGlimmerToolParser.extract_tool_calls(T, _call("get_weather.get_weather"), None)
    assert out.tool_calls[0].function.name == "get_weather.get_weather"


def test_exact_match_kept():
    out = MuseGlimmerToolParser.extract_tool_calls(T, _call("get_weather"), _req("get_weather"))
