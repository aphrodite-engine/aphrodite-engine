...
pytestmark = pytest.mark.skipif(not os.path.isdir(CKPT), reason=f"MuseGlimmer checkpoint not found at {CKPT}")
...
def _drive(parser_cls, tok, gen_text, req=None):
    ...
    from aphrodite.parser.abstract_parser import StreamState
...
def test_parse_delta_reasoning_suppressed_when_not_requested(parser_cls, tok):
    ...
    reasoning, content, tools = _drive(
        parser_cls,
        tok,
        " to=self<|message|>secret thoughts<|eom|><|start|>assistant to=user<|message|>Public answer.<|eot|>",
        req=_NoReasonReq(),
    )
