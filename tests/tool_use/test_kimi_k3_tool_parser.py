...
def test_responses_chat_params_keeps_template_tool_choice_when_api_auto():
    request = _responses_request().model_copy(update={"chat_template_kwargs": {"tool_choice": "required"}})
...
def test_responses_required_tool_choice_uses_xtml_parser():
    ...
    reasoning, content, tool_calls = parser.parse(output, request, enable_auto_tools=True, model_output_token_ids=[])
...
def test_responses_named_tool_choice_uses_xtml_parser():
    parser = KimiK3DelegatingParser(DummyTokenizer(), chat_template_kwargs={"thinking": False})
    request = _responses_request(tool_choice={"type": "function", "name": "calc"}).model_copy(
        update={"chat_template_kwargs": {"thinking": False}}
    )
    output = RESPONSE_CLOSE + _tools(_call("calc", 1, _arg("x", "number", "1")))
