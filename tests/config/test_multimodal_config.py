...
from aphrodite.config.ec_transfer import ECRole, ECTransferConfig
...
def test_mm_encoder_attn_backend_hash_updates():
    ...
    overridden_hash = MultiModalConfig(mm_encoder_attn_backend=AttentionBackendEnum.FLASH_ATTN).compute_hash()
...
def test_use_gpu_video_backend_from_media_io_kwargs(backend_arg: str):
    config = MultiModalConfig(media_io_kwargs={"video": {backend_arg: "pynvvideocodec"}})
...
def _make_mm_prefix_model_config(
    *,
    ...
    model_config.multimodal_config = MultiModalConfig(language_model_only=language_model_only)
    # Bind real helper methods onto the mock.
    model_config._supports_multimodal_for_mm_prefix = ModelConfig._supports_multimodal_for_mm_prefix.__get__(
...
def test_supports_multimodal_for_mm_prefix_uses_registry(supports_mm: bool):
    ...
    with patch(
        "aphrodite.multimodal.MULTIMODAL_REGISTRY.supports_multimodal_inputs",
...
def test_sticky_cache_survives_text_subconfig_regeneration():
    ...
    text_config._supports_multimodal_inputs_cached = model_config._supports_multimodal_inputs_cached
    with patch(
        "aphrodite.multimodal.MULTIMODAL_REGISTRY.supports_multimodal_inputs",
...
def _validate_mm_processor_device(*, device: str, ec_role: ECRole | None) -> None:
    ...
    MultiModalConfig(mm_processor_kwargs={"device": device}).validate_mm_processor_device(ec_config)
...
def test_bad_mm_processor_device_rejected(ec_role: ECRole | None):
    ...
    """
    with (
        patch("aphrodite.platforms.current_platform.device_type", "cpu"),
...
def test_accelerator_mm_processor_rejected_outside_encoder_instance(
    ...
    """
    with (
        patch("aphrodite.platforms.current_platform.device_type", "cuda"),
...
def test_accelerator_mm_processor_allowed_on_encoder_instance():
    with patch("aphrodite.platforms.current_platform.device_type", "cuda"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # device behind for `AphroditeConfig` to mistake for an explicit request.
        ("auto", None, None),
        (None, None, None),
        ("cpu", None, "cpu"),
        ("cuda", None, "cuda"),
...
def test_fold_mm_processor_device(flag: str | None, kwargs: dict | None, expected: str | None):
    with patch("aphrodite.platforms.current_platform.device_type", "cuda"):
...
def _resolve_mm_processor_device(
    *,
    ...
    aphrodite_config.ec_transfer_config = (
        None if ec_role is None else ECTransferConfig(ec_connector="ECExampleConnector", ec_role=ec_role)
    )

    with patch("aphrodite.platforms.current_platform.device_type", "cuda"):
        AphroditeConfig._resolve_mm_processor_device(aphrodite_config)
...
def test_auto_mm_processor_device_needs_a_device_capable_transport():
    ...
    assert _resolve_mm_processor_device(ec_role="ec_producer", mm_tensor_ipc="direct_rpc") is None
...
def test_ec_producer_only_enables_mm_encoder_only(ec_role: ECRole | None, expected: bool):
    mm_config = MultiModalConfig()
    model_config = MagicMock(spec=ModelConfig)
    model_config.multimodal_config = mm_config
    aphrodite_config = MagicMock(spec=AphroditeConfig)
    ...
    AphroditeConfig._resolve_mm_encoder_only(aphrodite_config)

    assert mm_config.mm_encoder_only is expected


def test_aphrodite_config_runs_the_mm_processor_device_check():
    ...
    """
    model_config = MagicMock(spec=ModelConfig)
    model_config.multimodal_config = MultiModalConfig(mm_processor_kwargs={"device": "cuda"})
    aphrodite_config = MagicMock(spec=AphroditeConfig)
    aphrodite_config.model_config = model_config
    aphrodite_config.ec_transfer_config = None

    with (
        patch("aphrodite.platforms.current_platform.device_type", "cuda"),
        pytest.raises(ValueError, match="also runs the language model"),
    ):
        AphroditeConfig._validate_mm_processor_device(aphrodite_config)
