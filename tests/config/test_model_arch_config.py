...
from aphrodite.config.model_arch import ModelArchitectureConfig
...
def test_qk_rope_head_dim_recovery_uses_model_revision(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        "aphrodite.transformers_utils.repo_utils.get_hf_file_to_dict",
        get_hf_file_to_dict,
    )
    ...
    get_hf_file_to_dict.assert_called_once_with("config.json", "org/model", "pinned-revision")
...
def test_num_experts_per_tok_aliases(attribute: str):
    ...
    model_config = cast(
        ModelConfig,
        SimpleNamespace(model_arch_config=SimpleNamespace(num_experts_per_token=convertor.get_num_experts_per_token())),
    )
...
def test_from_layers_collapses_to_the_largest_layer():
    ...
    arch = ModelArchitectureConfig.from_layers([_layer(), _layer(), _layer(head_size=32, total_num_kv_heads=8)])
...
def test_layer_view_follows_later_edits_to_the_whole_model_config():
    ...
    arch = ModelArchitectureConfig.from_layers([_layer(), _layer(), _layer(head_size=32)])
...
def test_uniform_layers_stay_homogeneous():
    """A checkpoint can vary attributes Aphrodite never reads."""
...
def test_getitem_rejects_negative_indices():
    ...
    homogeneous = _layer()
    heterogeneous = ModelArchitectureConfig.from_layers([_layer(), _layer(), _layer(head_size=32)])
...
def test_gemma4_head_dims_vary_by_layer_type():
    ...
    """
    text_config = _gemma4_text_config(num_global_key_value_heads=8, attention_k_eq_v=True)
    ...
    # Aphrodite allocates and the projections the model builds cannot disagree.
    assert [gemma4_layer_config(text_config, i).head_dim for i in range(6)] == [arch[i].head_size for i in range(6)]
...
class _HeterogeneousConfig(PretrainedConfig):
    ...
    Mirrors the parts Aphrodite uses: per-layer configs are shallow copies with the
...
def test_heterogeneous_config_varying_nothing_aphrodite_reads():
...
def test_deepseek_v4_convertor_splits_vision_architecture():
    ...
    raw attribute)."""
    from aphrodite.transformers_utils.configs.deepseek_v4 import DeepseekV4Config
    from aphrodite.transformers_utils.model_arch_config_convertor import (
    ...
    vl_cfg = DeepseekV4Config(architectures=["DeepseekV4ForCausalLM"], vision_n_layers=32)
