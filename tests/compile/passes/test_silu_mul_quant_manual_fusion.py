...
from aphrodite.model_executor.layers.fusion.fused_act_quant import (
    _FUSED_ACT_QUANT,
...
def test_manual_fusion_fp8_static_with_linear(
    ...
    with set_current_aphrodite_config(config):
        ...
        if not hasattr(fp8_linear, "input_quant_key"):
            pytest.skip(f"Kernel {force_kernel.__name__} doesn't support input_quant_key")
...
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
@pytest.mark.skipif(not current_platform.is_cuda_alike(), reason="Dynamic block quant CUDA only")
@pytest.mark.skipif(envs.APHRODITE_TARGET_DEVICE not in ["cuda", "rocm"], reason="Only test on CUDA and ROCm")
def test_manual_fusion_fp8_dynamic_128(dtype: torch.dtype):
    ...
    with set_current_aphrodite_config(config):
        ...
        dequant_data = result_fused.data.to(dtype).view(num_tokens, expected_num_groups, group_size)
        scales_expanded = result_fused.scale.unsqueeze(-1)  # (num_tokens, num_groups, 1)
        dequant_result = (dequant_data * scales_expanded).view(num_tokens, hidden_size).to(dtype)
...
@pytest.mark.skipif(not current_platform.has_device_capability(100), reason="NVFP4 requires SM100+")
@pytest.mark.skipif(envs.APHRODITE_TARGET_DEVICE not in ["cuda", "rocm"], reason="Only test on CUDA and ROCm")
def test_manual_fusion_nvfp4_dynamic(dtype: torch.dtype):
    ...
    with set_current_aphrodite_config(config):
        ...
        mock_linear_with_key = MockLinearForFusion(kNvfp4Dynamic, input_global_scale=input_global_scale)
        result_fused = maybe_fused_act_quant(silu_and_mul, x, mock_linear_with_key)
...
def test_manual_fusion_fallback_unsupported_key(dtype: torch.dtype):
    ...
    with set_current_aphrodite_config(config):
        ...
        unsupported_key = QuantKey(
            dtype=torch.int8,
            scale=ScaleDesc(dtype=torch.float32, static=False, group_shape=GroupShape(1, 1)),
        )
