...
def _make_workspace_hidden_config(target_hidden: int, draft_hidden: int | None) -> SimpleNamespace:
...
def test_fused_ar_workspace_hidden_dim(target_hidden: int, draft_hidden: int | None, expected: int):
...
class TestAllReduceRMSNormModel(torch.nn.Module):
    ...
    def ops_in_model_after(self):
        ...
        return [torch.ops.aphrodite.flashinfer_trtllm_fused_allreduce_norm.default]
...
class TestAllReduceRMSNormStaticQuantFP8Model(torch.nn.Module):
    ...
    def ops_in_model_before(self):
        return [
            torch.ops.aphrodite.all_reduce.default,
...
class TestAllReduceGemmaRMSNormStaticQuantFP8Model(TestAllReduceRMSNormStaticQuantFP8Model):
    def __init__(
        self,
        hidden_size=16,
        token_num=16,
...
class TestAiterAllReduceRMSNormGroupQuantFP8Model(torch.nn.Module):
    ...
    * ``norm[2..3]``: ``all_reduce -> fused_add_rms_norm
    ...
    def forward(self, hidden_states):
        ...
        idx2 = torch.ops.aphrodite.rocm_unquantized_gemm(rms2, self.indexer_w[0], None)
        ...
        idx3 = torch.ops.aphrodite.rocm_unquantized_gemm(rms3, self.indexer_w[1], None)
    ...
    def ops_in_model_before(self):
        return [
            torch.ops.aphrodite.all_reduce.default,
            torch.ops.aphrodite.rocm_aiter_group_fp8_quant.default,
        ]
...
def test_all_reduce_fusion_pass_replace(
    ...
    if use_aiter:
        with monkeypatch.context() as m:
            m.setenv("APHRODITE_ROCM_USE_AITER", str(use_aiter))
...
def all_reduce_fusion_pass_on_test_model(
    ...
    with set_current_aphrodite_config(aphrodite_config):
        ...
        if test_model_cls in (
            ...
            fused_op = torch.ops.aphrodite.flashinfer_trtllm_fused_allreduce_norm.default
...
def test_rocm_aiter_all_reduce_rmsnorm_group_quant_fp8_fusion_pass_replace(
    ...
    Validates the three new ``AphroditePatternReplacement`` patterns added to
    ``RocmAiterAllReduceFusionPass``:
    ...
    * ``AiterAllreduceFusedAddRMSNormGroupQuantWithIndexerPattern`` (with-
      ...
      ``rocm_aiter_group_fp8_quant`` producer).
    """
    with monkeypatch.context() as m:
...
def rocm_aiter_group_quant_fusion_pass_on_test_model(
    ...
    with set_current_aphrodite_config(aphrodite_config):
        ...
        for site, (reference, actual) in enumerate(zip(results_unfused[:4], results_fused[:4])):
            close = torch.isclose(reference, actual, atol=5e-2, rtol=5e-2)
            mismatch_ratio = float((~close).float().mean().item())
            assert mismatch_ratio <= 0.05, f"FP8 site {site} mismatch ratio {mismatch_ratio:.2%} exceeds 5%"

        # Residual and bf16 indexer outputs do not incur FP8 quantization noise.
        torch.testing.assert_close(results_unfused[4:], results_fused[4:], atol=1e-2, rtol=1e-2)
