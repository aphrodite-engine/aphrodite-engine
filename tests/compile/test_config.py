...
def test_use_cudagraphs_dynamic():
    ...
    # blanket default.
    assert aphrodite_config.compilation_config.use_cudagraph
...
def test_use_cudagraphs(aphrodite_runner, monkeypatch, enabled):
    # Disable multiprocessing so that the counter is in the same process
    monkeypatch.setenv("APHRODITE_ENABLE_V1_MULTIPROCESSING", "0")
    ...
    if is_torch_equal_or_newer("2.9.0.dev"):
        config = AphroditeConfig(
            compilation_config=CompilationConfig(
                ...
                splitting_ops=["aphrodite::unified_attention"],
            )
        )
        # with inductor partition we use splitting_ops directly for
        # partition rules
        assert config.compilation_config.splitting_ops == ["aphrodite::unified_attention"]
...
def test_resolve_operator_overload():
    ...
    from aphrodite.compilation.partition_rules import resolve_defined_ops
