...
@pytest.mark.parametrize(
    ("model_id", "settings"),
    [pytest.param(model_id, settings, id=model_id) for model_id, settings in SP_TEXT_GENERATION_MODELS.items()],
)
...
def test_tp_sp_generation(
    ...
    comparisons = []
    for _, parallel_setup, backend, runner, test_options in settings.iter_params(model_id):
