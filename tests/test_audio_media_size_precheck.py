...
def test_audio_base64_rejects_before_decode(monkeypatch: pytest.MonkeyPatch):
    ...
    with (
        patch("aphrodite.multimodal.media.audio.pybase64.b64decode") as decode,
        pytest.raises(APHRODITEValidationError, match="Maximum file size exceeded"),
    ):
...
def test_audio_load_bytes_rejects_oversized(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(APHRODITEValidationError, match="Maximum file size exceeded"):
        AudioMediaIO().load_bytes(b"\x00" * (MiB_bytes + 1))


def test_audio_load_file_rejects_oversized(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    monkeypatch.setattr(envs, "APHRODITE_MAX_AUDIO_CLIP_FILESIZE_MB", 1)
...
async def test_audio_connector_threads_byte_limit_to_http_reader(
    ...
    class _Connection:
        async def async_get_bytes(self, _url: str, **kwargs):
            ...
            raise APHRODITEValidationError("Maximum file size exceeded")
