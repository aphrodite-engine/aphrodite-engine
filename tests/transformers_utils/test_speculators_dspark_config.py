...
def test_dspark_updater_preserves_qwen3_omni_contract() -> None:
    config = SpeculatorsConfig.extract_transformers_pre_trained_config(_make_dspark_config("Qwen3OmniDSparkModel"))
...
def test_dspark_updater_keeps_legacy_qwen3_checkpoint_loadable() -> None:
    config = SpeculatorsConfig.extract_transformers_pre_trained_config(_make_dspark_config("DSparkSpeculator"))
