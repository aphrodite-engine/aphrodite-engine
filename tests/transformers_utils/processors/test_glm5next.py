...
"""Unit tests for the Aphrodite-native GLM-5.3-Flash multimodal processor."""
...
def image_processor():
    return Glm5NextImageProcessor(min_image_tokens=MIN_TOKENS, max_image_tokens=MAX_TOKENS)
...
@pytest.fixture(scope="module")
def video_processor():
    return Glm5NextVideoProcessor(min_image_tokens=MIN_TOKENS, max_image_tokens=MAX_TOKENS)
...
def test_video_preprocess_pads_content_not_distort(video_processor):
    ...
    out = run_video_preprocess(video_processor, frames, do_rescale=False, do_normalize=False)
...
def test_glm_sample_frame_indices_behaviour(total_frames, fps, duration, expected_len, first, last):
...
def test_sample_frames_fps_interval(video_processor):
    indices = video_processor.sample_frames(VideoMetadata(total_num_frames=900, fps=30.0, duration=30.0))
...
def test_video_config_fields_land():
    ...
    indices = proc.sample_frames(VideoMetadata(total_num_frames=900, fps=30.0, duration=30.0))
