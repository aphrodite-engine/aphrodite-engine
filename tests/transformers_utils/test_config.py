...
from aphrodite.transformers_utils import config as config_module
from aphrodite.transformers_utils.config import (
    get_safetensors_params_metadata,
    try_get_generation_config,
)
from aphrodite.transformers_utils.configs.glm5_next import (
    Glm5NextConfig,
    Glm5NextTextConfig,
...
def test_glm5_next_accepts_deepseek_sparse_attention_layers():
    ...
    config = Glm5NextTextConfig(num_hidden_layers=len(layer_types), layer_types=layer_types)
...
def test_safetensors_metadata_of_repo_without_safetensors():
    ...
    get_safetensors_metadata = MagicMock(side_effect=NotASafetensorsRepoError("not a safetensors repo"))
    api = SimpleNamespace(
