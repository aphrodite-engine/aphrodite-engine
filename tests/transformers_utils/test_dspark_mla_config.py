...
from aphrodite.transformers_utils.configs.k3_dspark import K3DSparkConfig
...
def test_dspark_mla_uses_latent_kv_geometry(tmp_path, num_attention_heads, expected_local_heads):
    draft_path = tmp_path / "draft"
    ...
    parallel_config = ParallelConfig(tensor_parallel_size=8, distributed_executor_backend="external_launcher")
...
def test_dspark_mla_speculative_config_preserves_architecture(tmp_path):
    ...
    _write_dspark_config(draft_path)
    target_config = ModelConfig(model=str(target_path), tokenizer_mode="skip", max_model_len=32768)
