...
class TestGemma4ChannelLessOutputConsistency:
    ...
    def test_streaming_channel_less_output_is_content(self, plain_parser, plain_tokenizer, request_obj):
        ...
        assert content == _PLAIN_ANSWER_TEXT, f"Channel-less output missing from content: {content!r}"
        assert not reasoning, f"Channel-less output misclassified as reasoning: {reasoning!r}"

    def test_streaming_matches_non_streaming(self, plain_parser, plain_tokenizer, request_obj):
        reasoning, content = plain_parser.extract_reasoning(_PLAIN_ANSWER_TEXT, request_obj)
