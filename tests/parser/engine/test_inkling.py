...
from aphrodite.parser.inkling import InklingParser, _inkling_arg_converter, inkling_config
...
class TestNonStreaming:
    ...
    def test_plain_text(self, parser, mock_request):
        reasoning, content, tools = parser.parse(f"{TEXT_START}hello world{END_MESSAGE}", mock_request)
        assert reasoning is None
        assert content == "hello world"
        assert tools is None
    ...
    def test_invoke_tool_text_is_visible_text(self, parser, mock_request):
        reasoning, content, tools = parser.parse(f"{TOOL_TEXT}do something{END_MESSAGE}", mock_request)
    ...
    def test_incomplete_tool_call_at_eos(self, parser, mock_request):
        ...
        _, _, tools = parser.parse(f'{TOOL_JSON}{{"name":"d","args":{{"k":"v"', mock_request)
    ...
    def test_prose_marker_without_token_ids_is_structural(self, parser, mock_request):
        ...
        _, content, _ = parser.parse(f"{TEXT_START}see {TEXT_START} token{END_MESSAGE}", mock_request)
...
class TestStreaming:
    ...
    def test_chunk_invariance_tool_call_text_only(self, mock_tokenizer, mock_request, chunk_size):
    ...
    def test_chunk_invariance_reasoning(self, mock_tokenizer, mock_request, chunk_size):
        parser = InklingParser(mock_tokenizer)
        text = f"{THINK_START}thinking...{END_MESSAGE}{MSG_MODEL}{TEXT_START}answer{END_MESSAGE}"
        results = _stream(parser, mock_request, text, chunk_size)
    ...
    def test_combined_parser_reasoning_to_tool_handoff_uses_text_markers(self, mock_tokenizer, mock_request):
        parser_cls = ParserManager.get_parser(
            tool_parser_name="inkling",
            reasoning_parser_name="inkling",
            enable_auto_tools=True,
        ...
        body = f'get_weather{TOOL_JSON}{{"name":"get_weather","args":{{"city":"Seattle"}}}}{END_MESSAGE}'
...
class TestPromptSeededState:
    ...
    def test_generation_prompt_tail_starts_message_header(self, parser):
        parser.adjust_initial_state_from_prompt([_TML_VOCAB[END_MESSAGE], _TML_VOCAB[MSG_MODEL]])
    ...
    def test_generation_prompt_header_flushes_bare_text_at_finish(self, parser, mock_request):
        prompt_token_ids = [_TML_VOCAB[END_MESSAGE], _TML_VOCAB[MSG_MODEL]]
        first = parser.parse_delta(
            "plain ",
...
class TestToolCallFiltering:
    ...
    def test_tool_choice_none_non_streaming(self, mock_tokenizer, none_request):
        parser = InklingParser(mock_tokenizer)
        text = f"{THINK_START}plan{END_MESSAGE}{MSG_MODEL}{TEXT_START}visible{END_MESSAGE}{MSG_MODEL}" + _tool_block(
            "f", '{"a":1}'
        )
    ...
    def test_tool_choice_none_streaming(self, mock_tokenizer, none_request):
        parser = InklingParser(mock_tokenizer)
        text = f"{TEXT_START}visible{END_MESSAGE}{MSG_MODEL}" + _tool_block("f", '{"a":1}')
...
class TestDelegatingTwoPass:
    ...
    def test_reasoning_then_text(self, mock_tokenizer, mock_request):
        ...
        reasoning, content, _ = _delegating(mock_tokenizer, tools).parse(
            f"{THINK_START}plan{END_MESSAGE}{MSG_MODEL}{TEXT_START}Let me look.{END_MESSAGE}",
            mock_request,
            enable_auto_tools=True,
        )
        assert reasoning == "plan"
    ...
    def test_two_tools_then_text(self, mock_tokenizer, mock_request):
        _, content, tools = _delegating(mock_tokenizer).parse(
            _tool_block("f", "{}") + MSG_MODEL + _tool_block("g", "{}") + MSG_MODEL + f"{TEXT_START}after{END_MESSAGE}",
            mock_request,
            enable_auto_tools=True,
        )
    ...
    def test_reasoning_then_tool_streaming(self, mock_tokenizer, mock_request, chunk_size):
        content, _, tools, _ = _stream_delegating(
            _delegating(mock_tokenizer),
            mock_request,
            f"{THINK_START}plan{END_MESSAGE}{MSG_MODEL}" + _tool_block("get_weather", '{"city":"SF"}'),
            chunk_size,
            self.GEN_PROMPT,
        )
    ...
    @pytest.mark.parametrize("chunk_size", [1, 3, 64])
    def test_visible_text_then_tool_streaming(self, mock_tokenizer, mock_request, chunk_size, opener):
        ...
        content, _, names, args = _stream_delegating(
            _delegating(mock_tokenizer, tools),
            mock_request,
            f"{opener}let me check{END_MESSAGE}{MSG_MODEL}" + _tool_block("get_weather", '{"city":"SF"}'),
            chunk_size,
            self.GEN_PROMPT,
        )
        assert content == "let me check"
    ...
    @pytest.mark.parametrize("chunk_size", [1, 3, 7, 64])
    def test_tool_start_from_message_header_streaming(self, mock_tokenizer, mock_request, chunk_size):
    ...
    def test_function_name_header_before_tool_start_streaming(self, mock_tokenizer, mock_request):
    ...
    def test_content_state_tool_start_streaming(self, mock_tokenizer, mock_request):
        ...
        content, _, names, args = _stream_delegating(
            _delegating(mock_tokenizer, tools),
            mock_request,
            f"{TEXT_START}intro{END_MESSAGE}" + _tool_block("get_weather", '{"city":"Seattle"}'),
            1,
            self.GEN_PROMPT,
        )
        assert names == ["get_weather"]
...
def test_skip_reasoning_parsing_inert_for_shared_markers():
    ...
    engine.reset(initial_state=ParserState.CONTENT)
    events = engine.parse_complete(f'{TOOL_JSON}{{"name":"f","args":{{}}}}{END_MESSAGE}after')
...
class TestToolParserWithoutReasoningParser:
    ...
    def test_thinking_then_tool_still_promotes(self, mock_tokenizer, mock_request):
        _, content, calls = self._tool_only(mock_tokenizer).parse(
            f"{THINK_START}think{END_MESSAGE}{MSG_MODEL}" + _tool_block("get_weather", '{"x":1}'),
            mock_request,
            enable_auto_tools=True,
        )
        assert [c.name for c in calls] == ["get_weather"]
