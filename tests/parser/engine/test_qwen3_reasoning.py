...
class TestIsReasoningEndTurnBoundaries:
    ...
    def test_injected_think_end_in_history_not_end(self, parser):
        assert not parser.is_reasoning_end([_IM_START_ID, _TEXT_ID, _THINK_END_ID, _IM_END_ID, _IM_START_ID])
    ...
    def test_think_end_in_current_turn_is_end(self, parser):
        assert parser.is_reasoning_end([_IM_START_ID, _TEXT_ID, _IM_END_ID, _IM_START_ID, _THINK_END_ID])

    def test_unpaired_tool_call_in_history_not_end(self, parser):
        assert not parser.is_reasoning_end([_TOOL_CALL_ID, _TEXT_ID, _IM_END_ID, _IM_START_ID])

    def test_continued_turn_with_closed_thinking_is_end(self, parser):
        assert parser.is_reasoning_end([_IM_START_ID, _THINK_START_ID, _TEXT_ID, _THINK_END_ID, _TEXT_ID])
    ...
    def test_empty_boundary_config_keeps_global_walk(self):
        ...
        parser = Qwen3Parser(make_mock_tokenizer(_QWEN3_VOCAB), parser_engine_config=cfg)
        assert parser.is_reasoning_end([_IM_START_ID, _TEXT_ID, _THINK_END_ID, _IM_END_ID, _IM_START_ID])
    ...
    def test_boundary_tokens_missing_from_vocab_keep_global_walk(self):
        ...
        vocab = {token: token_id for token, token_id in _QWEN3_VOCAB.items() if not token.startswith("<|im_")}
...
class TestDelegatingPromptDetection:
    ...
    def test_injected_think_end_in_history_keeps_streaming_reasoning(self, mock_tokenizer, mock_request):
        parser = _Qwen3DelegatingParser(mock_tokenizer)
        prompt_ids = [
            _IM_START_ID,
            _TEXT_ID,
    ...
    def test_think_end_in_generation_prompt_still_seeds_reasoning_ended(self, mock_tokenizer, mock_request):
        parser = _Qwen3DelegatingParser(mock_tokenizer)
        prompt_ids = [
            _IM_START_ID,
            _TEXT_ID,
