...
class TestArgConverter:
    ...
    def test_next_parameter_prefix_implicitly_closes_previous(self):
        raw = f"<{_PARAM_OPEN.format(name='city', is_str='true')}Tokyo\n<｜DSML｜parameter name="
        result = json.loads(_dsml_arg_converter(raw, partial=False))
        assert result == {"city": "Tokyo\n"}
...
class TestImplicitParameterClose:
    ...
    def test_streaming_split_next_parameter_tag_is_buffered(self, mock_tokenizer, mock_request):
        chunks = [
            DSML_TOOL_START,
            f"{DSML_INVOKE_PREFIX}get_weather{DSML_INVOKE_NAME_END}\n",
            f"<{_PARAM_OPEN.format(name='location', is_str='true')}Paris a<b><｜DSML｜parameter",
        ...
        results = simulate_tool_streaming(DeepSeekV4Parser(mock_tokenizer), mock_request, chunks)
...
class TestThinkTagAbsorption:
    ...
    def test_duplicate_think_start_absorbed(self, mock_tokenizer):
        parser = DeepSeekV4Parser(mock_tokenizer, chat_template_kwargs={"thinking": True})
        chunks = [
            "<think>\n",
            "Some reasoning.\n",
            "</think>\n",
...
class TestThinkingModeConfig:
    ...
    def test_parser_thinking_mode_matches_tokenizer_default(self, mock_tokenizer, chat_template_kwargs, expected_state):
...
class TestImplicitReasoningEnd:
    ...
    def test_non_streaming_extract_tool_calls_implicit_end(self, thinking_parser, mock_request):
        text = self._reasoning_then_tool("Let me look up the weather.\n\n")
    ...
    def test_streaming_tool_extraction_implicit_end(self, thinking_parser, mock_request):
        chunks = [
            "Let me check.\n\n",
            DSML_TOOL_START,
            DSML_INVOKE_PREFIX
    ...
    def test_non_streaming_parallel_tools_after_implicit_end(self, thinking_parser, mock_request):
        text = "I need both.\n\n" + _tool_calls(
            _invoke("get_weather", ("location", "true", "NYC")),
...
def _make_tool(name, properties):
    from aphrodite.entrypoints.openai.chat_completion.protocol import (  # noqa: E501
        ChatCompletionToolsParam,
    )
...
def _invoke(name, *params):
    ...
    return f"{DSML_INVOKE_PREFIX}{name}{DSML_INVOKE_NAME_END}\n{body}\n{DSML_INVOKE_END}"
...
class TestParallelUnwrapping:
    ...
    def test_unwrap_parallel_streaming(self, mock_tokenizer, mock_request, weather_tool, time_tool):
        tools = [weather_tool, time_tool]
        parser = DeepSeekV4Parser(mock_tokenizer, tools=tools)
        mock_request.tools = tools
    ...
    def test_no_unwrap_parallel_when_no_match(self, mock_tokenizer, mock_request, weather_tool, time_tool):
        tools = [weather_tool, time_tool]
        parser = DeepSeekV4Parser(mock_tokenizer, tools=tools)
        mock_request.tools = tools
...
class TestStreamingWrapperConsistency:
    ...
    def test_streaming_wrapper_unwrap_consistency(self, mock_tokenizer, mock_request):
        ...
        assert final_args.startswith(streamed_args), (
            f"Extracted args {final_args!r} should start with streamed args {streamed_args!r}"
        )
...
class TestDelegatingParserLargeDelta:
    ...
    def test_tool_calls_extracted_at_all_chunk_sizes(self, dsv4_tokenizer, dsv4_tokens, chunk_size):
    ...
    def test_default_thinking_extracts_tool_call_without_think_end(self, dsv4_tokens):
        tokens = [token for token in dsv4_tokens if token[0] != _DSV4_FULL_VOCAB[DSML_THINK_END]]
