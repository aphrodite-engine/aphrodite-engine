...
class TestReasoningTokenCounts:
    ...
    def test_deferred_reasoning_tokens_stay_before_next_end_terminal(self):
        ...
        assert sum(e.token_count for e in events if e.type == EventType.REASONING_CHUNK) == 2
    ...
    return ParserEngineConfig(
        ...
        transitions={
            ...
            (ParserState.MESSAGE_HEADER, "TOOL_START"): Transition(
                ParserState.TOOL_ARGS,
                (EventType.REASONING_END, EventType.TOOL_CALL_START) if reasoning_end else (EventType.TOOL_CALL_START,),
            ),
        },
    )
...
class TestSkipToolParsingFromMessageHeader:
    ...
    def _skip_events(reasoning_end: bool):
        engine = StreamingParserEngine(_message_header_config(reasoning_end), tokenizer=None)
        engine.skip_tool_parsing = True
