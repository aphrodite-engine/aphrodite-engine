...
from aphrodite.parser.deepseek_v32 import (
    DSML_FUNC_END,
    DSML_FUNC_START,
    DeepSeekV32Parser,
)
...
class TestNonStreaming:
    ...
    def test_content_before_tool_call(self, mock_tokenizer, mock_request):
        text = "Let me check. " + _func_calls(
            _invoke("search", _param("q", "true", "aphrodite")),
        )
        parser = DeepSeekV32Parser(mock_tokenizer)
        result = parser.extract_tool_calls(text, mock_request)
        assert result.tools_called
...
class TestStreaming:
    def test_split_parameter_start_is_buffered(self, mock_tokenizer, mock_request):
        ...
        results = simulate_tool_streaming(DeepSeekV32Parser(mock_tokenizer), mock_request, chunks)
