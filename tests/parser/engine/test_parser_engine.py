...
def test_parser_manager_preserves_tool_only_adapter(monkeypatch):
    ...
    parser_cls = ParserManager.get_parser(tool_parser_name="combined", enable_auto_tools=True)
...
class TestSkipToolSpanForwarding:
    ...
    def test_content_after_transitionless_closer_observable(self):
        ...
        cfg = ParserEngineConfig(
            ...
            transitions={
                (ParserState.CONTENT, "TOOL_START"): Transition(ParserState.TOOL_ARGS, (EventType.TOOL_CALL_START,)),
                (ParserState.TOOL_ARGS, "CLOSE_A"): Transition(ParserState.CONTENT, (EventType.TOOL_CALL_END,)),
                (ParserState.TOOL_ARGS, "CLOSE_B"): Transition(ParserState.CONTENT, (EventType.TOOL_CALL_END,)),
...
class TestTruncatedToolOpenerStreamParity:
    ...
    def _stream_content(self, request, chunks: list[str]) -> str:
        ...
        for i, chunk in enumerate(chunks):
            delta_token_ids = [tid for text, tid in self._QWEN3_VOCAB.items() if text in chunk]
    ...
    def test_truncated_opener_dropped_in_both_paths(self, mock_request, chunks):
        ...
        _, content, tool_calls = self._make_parser().parse(text, mock_request, enable_auto_tools=True)
...
class TestThinkMarkupWithoutReasoningParser:
    ...
    def _stream(self, tool_parser_name, request, chunks):
        ...
        content, reasoning = "", ""
        for i, chunk in enumerate(chunks):
            delta_token_ids = [tid for text, tid in self._VOCAB.items() if text in chunk]
            delta = parser.parse_delta(
                chunk,
                delta_token_ids,
                request,
    ...
    def test_think_block_passes_through_both_paths(self, mock_request, tool_parser_name):
        ...
        streamed_content, streamed_reasoning = self._stream(tool_parser_name, mock_request, chunks)
    ...
    def test_think_block_preserved_alongside_tool_call(self, mock_request):
        ...
        _, content, tool_calls = self._make_parser("qwen3_coder").parse(text, mock_request, enable_auto_tools=True)
        assert tool_calls is not None and len(tool_calls) == 1
        assert tool_calls[0].name == "get_weather"
