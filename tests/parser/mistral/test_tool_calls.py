...
from aphrodite.parser.mistral import _DEFAULT_JSON_SCHEMA, MistralParser, mistral_config
...
def assert_tool_calls(
    ...
    for actual_tool_call, expected_tool_call in zip(actual_tool_calls, expected_tool_calls):
        ...
        if isinstance(actual_tool_call, DeltaToolCall) and actual_args is not None:
            actual_args = partial_json_parser.ensure_json(actual_args, Allow.OBJ | Allow.STR)
...
def test_extract_tool_calls_malformed_name_before_marker_emits_empty_name(
    ...
    assert result.tool_calls[0].function.arguments == json.dumps({"command": "cat file.py"})
...
def test_extract_tool_calls_parallel_malformed_then_well_formed(
    ...
    model_output = 'bash[TOOL_CALLS]{"command": "ls"}[TOOL_CALLS]submit[ARGS]{"answer": {"nested": 1}}'
    result = mistral_tool_parser.extract_tool_calls(model_output, request=_DUMMY_REQUEST)
    assert result.tools_called
    ...
    assert result.tool_calls[1].function.arguments == json.dumps({"answer": {"nested": 1}})
    assert result.content == "bash"
            ...
            for flush_tc in flush_delta.tool_calls:
                for existing_tc in delta_message.tool_calls:
                    if existing_tc.index == flush_tc.index and flush_tc.function:
                        existing_tc.function = existing_tc.function or DeltaFunctionCall()
                        existing_tc.function.arguments = (existing_tc.function.arguments or "") + (
...
def test_extract_tool_calls_streaming_exception_returns_none(
    ...
    with patch.object(parser._parser_engine, patched_method, side_effect=RuntimeError("boom")):
...
def test_content_tool_calls_transition_emits_reasoning_end(
    ...
    engine = StreamingParserEngine(config=cfg, tokenizer=mistral_tokenizer, vocab=mistral_tokenizer.get_vocab())
    ...
    assert EventType.REASONING_END in event_types, (
        f"REASONING_END missing for reasoning_encoding={reasoning_encoding!r}; got events: {event_types}"
    )
...
def test_adjust_request_non_mistral_tokenizer_required_injects_schema(
    ...
    request = _make_request(tool_choice="required", response_format={"type": "json_object"})
    result = non_mistral_parser.adjust_request(request)
...
def test_legacy_extract_tool_calls_guided_bare_array_required(
    ...
    result = mistral_pre_v11_tool_parser.extract_tool_calls(model_output, request=request)
...
def test_legacy_extract_tool_calls_none_with_tools(
    ...
    model_output = '[TOOL_CALLS] [{"name": "get_current_weather", "arguments": {"city": "Dallas"}}]'
...
def test_guided_streaming_required_pre_v11(
    ...
    model_output = '[{"name": "get_current_weather", "arguments": {"city": "Dallas"}}]'
    request = _make_request(tool_choice="required")
    all_token_ids = mistral_pre_v11_tokenizer.encode(model_output, add_special_tokens=False)
...
_MALFORMED_MATRIX_CASES = [
    ...
    pytest.param(
        ...
        '[TOOL_CALLS]bash[ARGS]{"command": "ls"}[TOOL_CALLS]submit[ARGS]{"answer": {"nested": 1}}',
        ["bash", "submit"],
    ...
    pytest.param(
        ...
        'bash[TOOL_CALLS][ARGS]{"command": "ls"}[TOOL_CALLS][ARGS]{"answer": {"nested": 1}}',
        ["", ""],
        [json.dumps({"command": "ls"}), json.dumps({"answer": {"nested": 1}})],
        "bash",
    ...
    pytest.param(
        ...
        'bash[TOOL_CALLS]{"command": "ls"}[TOOL_CALLS]submit[ARGS]{"answer": {"nested": 1}}',
        ["", "submit"],
        [json.dumps({"command": "ls"}), json.dumps({"answer": {"nested": 1}})],
        "bash",
    ...
    pytest.param(
        ...
        'bash[TOOL_CALLS][ARGS]{"command": "ls"}[TOOL_CALLS]submit[ARGS]{"answer": {"nested": 1}}',
        ["", "submit"],
        [json.dumps({"command": "ls"}), json.dumps({"answer": {"nested": 1}})],
        "bash",
...
def test_malformed_tool_call_matrix_streaming(
    ...
    content, names, args = _stream_via_parse_delta(parser, mistral_tokenizer, model_output, request)
