...
def _encode_v13(tokenizer: MistralTokenizer, output: str) -> list[int]:
    ...
    if index_start != -1:
        ...
        if index_end != -1:
            ...
            out += tokenizer.tokenizer.encode(output[index_start + len_start :], False, False)
    elif index_end != -1:
...
def test_mistral_reasoning_v13(
    ...
    parser: ReasoningParser = ReasoningParserManager.get_reasoning_parser(_PARSER_NAME)(mistral_tokenizer)
...
def test_reasoning_with_tool_calls(
    ...
    for streaming in (False, True):
        parser: ReasoningParser = ReasoningParserManager.get_reasoning_parser(_PARSER_NAME)(mistral_tokenizer)
        reasoning, _content = run_reasoning_extraction_mistral(parser, output_tokens, streaming=streaming)
        assert reasoning == "r"
        assert "[TOOL_CALLS]" not in (reasoning or "")


@pytest.mark.parametrize("streaming", [False, True], ids=["nonstream", "stream"])
def test_reasoning_v11_plain_text_think(mistral_v11_tokenizer: MistralTokenizer, streaming: bool) -> None:
    """v11 tokenizers use plain-text `<think>`/`</think>` (no special tokens)."""
    parser: ReasoningParser = ReasoningParserManager.get_reasoning_parser(_PARSER_NAME)(mistral_v11_tokenizer)

    tokens = mistral_v11_tokenizer.tokenizer.encode("<think>r</think>c", False, False)
    reasoning, content = run_reasoning_extraction_mistral(parser, tokens, streaming=streaming)
    assert reasoning == "r"
    assert content == "c"
...
def test_parse_delta_keeps_reasoning_open_after_closed_think_prompt(
    ...
    reasoning, content = _stream_parse_delta(parser, mistral_tokenizer, gen_ids, request, prompt_ids)
...
def _stream_turn(
    ...
    tool_calls = [(tool_call_acc[k]["name"], tool_call_acc[k]["args"]) for k in sorted(tool_call_acc)]
...
def test_parse_delta_multi_turn_reason_and_tool(
    ...
    for i, city in enumerate(cities):
        ...
        prompt_ids = mistral_tokenizer.instruct.encode_instruct(InstructRequest(messages=conversation)).tokens

        gen_str = f'[THINK]I should check {city} weather.[/THINK][TOOL_CALLS]get_weather[ARGS]{{"city": "{city}"}}'
        gen_ids = _encode_gen(mistral_tokenizer, gen_str)

        reasoning, content, tool_calls = _stream_turn(parser, mistral_tokenizer, gen_ids, request, prompt_ids)

        assert f"check {city} weather" in reasoning, f"turn {i}: reasoning not captured"
        assert f"check {city} weather" not in content, f"turn {i}: reasoning leaked into content"
