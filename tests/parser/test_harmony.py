...
def xgrammar_backend(gpt_oss_tokenizer) -> XgrammarBackend:
    aphrodite_config = AphroditeConfig(structured_outputs_config=StructuredOutputsConfig(backend="xgrammar"))
...
def assistant(content: str, channel: str, content_type: str | None = None) -> Message:
    message = Message.from_role_and_content(Role.ASSISTANT, content).with_channel(channel)
    ...
    def test_cross_channel_with_tool(self, gpt_oss_tokenizer, chat_request):
        ...
        delta = parser.parse_delta(
            ...
            delta_token_ids=encode_output(
                ...
                '<|constrain|>json<|message|>{"query": "aphrodite"}<|call|>'
        ...
        assert tool_call_entries(delta) == [(0, "search", '{"query": "aphrodite"}')]
...
class TestAdjustRequest:
    ...
    FINAL_TEXT_ONLY = get_model_output_str([assistant("reasoning", "analysis"), assistant("any", "final")])
    FINAL_REGEX = get_model_output_str([assistant("reasoning", "analysis"), assistant("regex", "final")])
    FINAL_CHOICE = get_model_output_str([assistant("reasoning", "analysis"), assistant("choice1", "final")])
    FINAL_GRAMMAR = get_model_output_str([assistant("reasoning", "analysis"), assistant("grammar", "final")])
    FINAL_STRUCTURAL_TAG = get_model_output_str([assistant("reasoning", "analysis"), assistant("tag content", "final")])
    ADMISSION_SAMPLES = (
        "COMMENTARY",
        "TOOL_CALL_1_CHANNEL_FIRST",
    ...
    def _assert_structured_outputs_admission(
        ...
        for sample_name in cls.ADMISSION_SAMPLES:
            ...
            assert admitted is should_admit, (
                f"Expected structured_outputs admission for {sample_name} to be {should_admit}, got {admitted}."
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                {"structured_outputs": StructuredOutputsParams(choice=["choice1", "choice2"])},
                ["FINAL_CHOICE"],
            ),
            (
                {"structured_outputs": StructuredOutputsParams(grammar='root ::= "grammar"')},
                ["FINAL_GRAMMAR"],
            ),
            (
                {
