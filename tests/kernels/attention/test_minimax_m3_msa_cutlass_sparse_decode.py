...
from aphrodite.config import AttentionConfig, CUDAGraphMode
from aphrodite.forward_context import ForwardContext, override_forward_context
...
from aphrodite.models.minimax_m3.nvidia import (
    sparse_attention_msa as sparse_attention_msa_module,
)
from aphrodite.models.minimax_m3.nvidia.model import MiniMaxM3SparseAttention
from aphrodite.models.minimax_m3.nvidia.msa_cutlass_sparse_decode import (
    MSACutlassDecodePlanCache,
    msa_cutlass_sparse_decode,
    prepare_decode_metadata,
    should_prepare_decode_metadata,
)
from aphrodite.models.minimax_m3.nvidia.sparse_attention_msa import (
...
def test_msa_metadata_builder_prepares_cutlass_for_regular_decode(
    ...
    metadata = builder.build(
        0,
        SimpleNamespace(seq_lens_cpu_upper_bound=torch.full((batch,), 257, dtype=torch.int32)),
    )
...
def test_msa_cutlass_plan_cache_keys_query_len(
    ...
    def fake_build_plan(**kwargs):
        ...
        return (
            ...
            None,
            {
                "kv_segment_lens": torch.empty(num_rows, dtype=torch.int32, device="cuda"),
...
def test_query_fp8_stays_valid_when_cutlass_plan_appears_on_replay(
    ...
    monkeypatch.setenv("APHRODITE_USE_BREAKABLE_CUDAGRAPH", "1")
...
def _make_topk(
    ...
    for request, seq_len in enumerate(seq_lens):
        for local_query in range(query_len):
            ...
            topk[token, :, :visible_pages] = torch.arange(visible_pages, dtype=torch.int32, device="cuda")
    return topk
...
def test_msa_cutlass_decode_matches_triton_with_interleaved_cache(
    ...
    block_table = torch.zeros(len(seq_lens_list), max_pages, dtype=torch.int32, device="cuda")
    ...
    for request, request_pages in enumerate(pages_per_request):
        block_table[request, :request_pages] = physical_pages[offset : offset + request_pages]
    ...
    seq_lens.copy_(torch.tensor(updated_seq_lens_list, dtype=torch.int32, device="cuda"))
    ...
    if capture_graph:
        ...
        topk_token_major.copy_(_make_topk(replay_seq_lens_list, num_kv_heads, query_len))
        prepare_decode_metadata(
            block_table,
            seq_lens,
