...
def test_deepseek_v4_c128a_adaptive_width_has_capture_stable_stride(
    ...
    from aphrodite.models.deepseek_v4 import sparse_mla
    ...
    capacity_width = 512
    global_decode_buffer = torch.empty((2, capacity_width), dtype=torch.int32, device=device)
    prefill_buffer = torch.empty_like(global_decode_buffer)
    kwargs = dict(
        positions=torch.tensor([255, 511, 383, 639], device=device),
        compress_ratio=128,
        num_decode_tokens=2,
        token_to_req_indices=torch.tensor([0, 1, 0, 1], dtype=torch.int32, device=device),
...
def test_sparse_flashmla_metadata_smoke():
    import aphrodite.v1.attention.ops.flashmla as fm
...
def test_flashinfer_sparse_indices_cache(monkeypatch):
    from aphrodite.models.deepseek_v4.nvidia import flashinfer_sparse as flashinfer_mod
    from aphrodite.models.deepseek_v4.sparse_mla import DeepseekV4FlashMLAMetadata
    from aphrodite.v1.attention.backends.mla.sparse_swa import DeepseekSparseSWAMetadata
...
def test_flashinfer_mixed_sparse_indices_separates_window_and_padded_width():
    ...
    sparse_indices, sparse_lens = build_flashinfer_mixed_sparse_indices(
        decode_swa_indices=torch.empty((0, padded_width), dtype=torch.int32, device=device),
