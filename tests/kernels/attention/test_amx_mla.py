...
from aphrodite import _custom_ops as ops  # noqa: E402
from aphrodite.utils.torch_utils import set_random_seed  # noqa: E402
from aphrodite.v1.attention.backends.mla.amx_mla import (  # noqa: E402
    AMXMLAImpl,
...
def test_decode_attention_cpu_matches_reference(seq_lens):
    ...
    attn_logits = torch.zeros(num_seqs, num_heads, 4, KV_LORA_RANK + 1, dtype=torch.float32)
    ops.cpu_mla_decode(
        q,
        kv_cache_flat,
        v_buffer,
...
def test_extend_attention_cpu_matches_reference(prefix_lens, extend_lens):
    ...
    extend_start_loc = torch.cumsum(torch.tensor([0, *extend_lens[:-1]], dtype=torch.int64), dim=0)
    scale = HEAD_SIZE**-0.5
...
def _make_amx_mla_impl(num_heads, qk_nope_head_dim, v_head_dim, kv_lora_rank):
    weight = torch.randn(num_heads * (qk_nope_head_dim + v_head_dim), kv_lora_rank, dtype=DTYPE)
...
class _FakePrefillMetadata:
    def __init__(self, block_table, cpu_seq_lens, query_start_loc, req_to_token, req_pool_indices):
...
class _FakeDecodeMetadata:
    def __init__(self, block_table, seq_lens, req_to_token, req_pool_indices):
        ...
        self.req_to_token = req_to_token
        self.req_pool_indices = req_pool_indices
        self.num_kv_splits = _compute_num_kv_splits(int(seq_lens.max().item()), current_platform.num_compute_units())
...
def test_amx_mla_impl_forward_mqa_matches_reference(default_aphrodite_config):
    ...
    impl, kv_b_weight = _make_amx_mla_impl(num_heads, qk_nope_head_dim, v_head_dim, kv_lora_rank)
    w_uk, w_uv = kv_b_weight.T.view(kv_lora_rank, num_heads, qk_nope_head_dim + v_head_dim).split(
    ...
    for i, seq_len in enumerate(seq_lens):
        ...
        k_nope_latent, k_pe = keys_latent.split([kv_lora_rank, QK_ROPE_HEAD_DIM], dim=-1)
        ...
        q_full = torch.cat([q_nope[i], q_pe[i]], dim=-1).float()  # (num_heads, qk_head_dim)
...
def test_amx_mla_impl_forward_mha_matches_reference(default_aphrodite_config):
    ...
    impl.forward_mha(q, kv_c_normed, k_pe, kv_cache, attn_metadata, k_scale=None, output=output)
    ...
    for i in range(num_seqs):
        ...
        for t in range(ext_len):
            ...
            k_nope_latent, k_pe_ref = keys_latent.split([kv_lora_rank, QK_ROPE_HEAD_DIM], dim=-1)
            k_real = torch.einsum("lp,pnd->nld", k_nope_latent.float(), w_uk.float())
            ...
            v_real = torch.einsum("lp,pnv->nlv", k_nope_latent.float(), w_uv.float())
            ref = torch.einsum("nl,nlv->nv", probs, v_real)
            torch.testing.assert_close(out_view[start + t], ref, atol=_IMPL_ATOL, rtol=_IMPL_RTOL)
