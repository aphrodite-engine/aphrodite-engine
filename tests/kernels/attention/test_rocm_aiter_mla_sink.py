...
def _assert_sink_output_close(actual, expected, value_scale, score_error, probability_dtype, native):
...
def test_sparse_mla_sink_matches_ragged_reference(real_heads: int, cache_kind: str) -> None:
    ...
    from aphrodite.v1.attention.backends.mla.rocm_aiter_mla_sparse import (
    ...
    output, lse = impl._forward_mla(SimpleNamespace(_q_scale=q_scale, _k_scale=kv_scale), q, kv, metadata)
    kv_flat = kv_ref[:, 0]
    references = []
    start = 0
    for batch_idx, seq_len in enumerate(seq_lens):
        ...
        start += seq_len
    expected, expected_lse, value_scale, score_error = (torch.stack(values) for values in zip(*references))
...
def test_sparse_mla_sink_rejects_unsupported_aiter_dtypes(q_dtype: torch.dtype, kv_dtype: torch.dtype) -> None:
    from aphrodite.v1.attention.backends.mla.rocm_aiter_mla_sparse import (
        ROCMAiterMLASparseImpl,
    )
...
def test_sparse_mla_sink_matches_dense_attention_with_empty_rows_and_paged_cache(
    ...
    from aphrodite.v1.attention.backends.mla.sparse_utils import flat_kv_row_view
    ...
    q_cpu = torch.randn(len(selected_rows), num_heads, head_dim, generator=generator).to(dtype)
    ...
    indices = [page * num_layers * block_size + offset for rows in selected_rows for page, offset in rows]
    ...
    metadata = SimpleNamespace(
        ...
        max_query_len=1,
        qo_indptr=torch.arange(len(selected_rows) + 1, dtype=torch.int32, device="cuda"),
        paged_kv_last_page_len=torch.ones(len(selected_rows), dtype=torch.int32, device="cuda"),
        work_meta_data=None,
    ...
    for query_idx, rows in enumerate(selected_rows):
        ...
        references.append(_sink_reference(q_cpu[query_idx], keys, sinks_cpu, impl.scale))
    expected, _, value_scale, score_error = (torch.stack(values) for values in zip(*references))
...
def test_sparse_mla_backend_resolves_only_contiguous_layer_layouts(monkeypatch, layout):
    ...
    monkeypatch.setenv("APHRODITE_KV_CACHE_LAYOUT", layout)
    config = SimpleNamespace(cache_config=CacheConfig())
    supported = [[x.name for x in ROCMAiterMLASparseBackend.supported_kv_cache_layouts()]]
    if layout == "BLNHC":
...
def test_sparse_mla_sink_forward_mqa_preserves_split_query(dtype):
    ...
    impl.scale = SM_SCALE
    impl.sinks = sinks
    impl.topk_indices_buffer = torch.full((num_tokens, 128), -1, dtype=torch.int32, device="cuda")
    ...
    metadata = SimpleNamespace(
        ...
        paged_kv_indices=torch.empty(selected.numel(), dtype=torch.int32, device="cuda"),
    ...
    references = [_sink_reference(q[i], kv[:, 0][selected[i].long()], sinks, SM_SCALE) for i in range(num_tokens)]
    expected, _, value_scale, score_error = (torch.stack(values) for values in zip(*references))
    assert actual.dtype == dtype
    _assert_sink_output_close(
        actual,
