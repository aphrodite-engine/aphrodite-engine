...
def test_reshape_and_cache_flash(
    ...
    if kv_cache_dtype == "nvfp4":
        ...
        if (head_size // 16) % 4 != 0:
            pytest.skip("NVFP4 requires (head_size // 16) divisible by 4 for 4x4 block scale swizzle.")
        if block_size % 4 != 0:
    ...
    if kv_cache_dtype == "fp8":
        ...
        convert_fp8_local(cloned_value_cache, value_cache_compact, v_scale, kv_cache_dtype)
    ...
    if kv_cache_dtype == "nvfp4":
        ...
        def dequant_nvfp4_cache(data_cache, scale_cache, global_scale):
            ...
            result_hnd = dequant_nvfp4_kv_cache(data_hnd, scale_hnd, global_scale, head_size, block_size)
            return result_hnd.permute(0, 2, 1, 3)  # back to [B, N, H, dim]

        result_key_cache = dequant_nvfp4_cache(nvfp4_key_data, key_scale_cache, k_scale.item())
        result_value_cache = dequant_nvfp4_cache(nvfp4_value_data, value_scale_cache, v_scale.item())
        ...
        torch.testing.assert_close(result_key_flat[slot_mapping], key.float(), atol=1.5, rtol=0.5)
        torch.testing.assert_close(result_value_flat[slot_mapping], value.float(), atol=1.5, rtol=0.5)
        return
...
def test_concat_and_cache_nvfp4_ds_mla(
    ...
    for i in range(num_tokens):
        ...
        # differ by 1).
        nope_sf_bytes = _nvfp4_sf_element_order(entry[nope_sf_off : nope_sf_off + num_nope_sf])
        ref_nope_sf_bytes = _ref_nvfp4_sf_bytes(kv_c_ref, 6.0)
        assert ((nope_sf_bytes.to(torch.int16) - ref_nope_sf_bytes.to(torch.int16)).abs() <= 1).all()
...
def test_gather_and_maybe_dequant_cache_mla(
    ...
    seq_len_tensor = torch.randint(max_seq_len, max_seq_len + 1, (batch_size,), device=device)
...
def test_gather_and_maybe_dequant_cache_mla_with_seq_starts(
    ...
    token_to_seq = torch.repeat_interleave(torch.arange(batch_size, dtype=torch.int32, device=device), seq_lens)
...
def test_gather_and_maybe_dequant_cache_mla_large_uneven_sequences(
    ...
    cu_seq_lens[1:] = seq_lens.cumsum(dim=0)
    token_to_seq = torch.repeat_interleave(torch.arange(batch_size, dtype=torch.int32, device=device), seq_lens)
    block_table = torch.stack([torch.randperm(num_blocks, device=device) for _ in range(batch_size)]).to(torch.int32)
    ...
    for req_id in range(batch_size):
        ...
        source_tokens = torch.arange(start, start + seq_lens[req_id].item(), device=device)
        physical_blocks = block_table[req_id, source_tokens // block_size].long()
        expected_batches.append(dequant_src_cache[physical_blocks, source_tokens % block_size])
