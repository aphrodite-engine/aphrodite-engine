...
def test_clamped_mm_prefix_prunes_sliding_window_tiles() -> None:
    ...
    mm_prefix_ranges = torch.tensor([[[1024, 2303], [0, 0]]], dtype=torch.int32, device=DEVICE_TYPE)
...
@pytest.mark.parametrize(("use_causal", "use_per_seq_causal"), [(False, False), (True, True)])
def test_clamped_mm_prefix_preserves_noncausal_right_window(use_causal: bool, use_per_seq_causal: bool) -> None:
    """Union the non-causal right window with intersecting image ranges."""
    mm_prefix_ranges = torch.tensor([[[1024, 1500], [0, 0]]], dtype=torch.int32, device=DEVICE_TYPE)
    bounds = torch.empty(3, dtype=torch.int32, device=DEVICE_TYPE)
...
def ref_paged_clamped_mm_attn(
    ...
    for req_idx, (query_len, kv_len) in enumerate(zip(query_lens, kv_lens)):
        ...
        keep = (delta >= 0) & (query_pos[:, None] // chunk_size - key_pos[None, :] // chunk_size <= chunk_lookback)
...
def test_triton_unified_attn_clamped_mm_matches_dense_reference() -> None:
    ...
    num_blocks_per_req = [(kv_len + block_size - 1) // block_size for kv_len in kv_lens_list]
    ...
    cu_seqlens_q = torch.tensor([0] + query_lens, dtype=torch.int32, device=DEVICE_TYPE).cumsum(0, dtype=torch.int32)
    kv_lens = torch.tensor(kv_lens_list, dtype=torch.int32, device=DEVICE_TYPE)
    mm_prefix_range = torch.tensor([[[32, 351]], [[160, 287]]], dtype=torch.int32, device=DEVICE_TYPE)
