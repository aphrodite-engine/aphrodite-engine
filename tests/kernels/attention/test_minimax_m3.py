...
from aphrodite.config.cache import _layout_from_name
from aphrodite.models.minimax_m3.common.ops.index_topk import (
    minimax_m3_index_decode,
    minimax_m3_index_score,
    minimax_m3_index_topk,
)
from aphrodite.models.minimax_m3.common.ops.sparse_attn import (
    _FP8_DTYPES,
    minimax_m3_sparse_attn,
    minimax_m3_sparse_attn_decode,
)
from aphrodite.models.minimax_m3.common.sparse_attention import MiniMaxM3SparseTritonImpl
from aphrodite.platforms import current_platform
from aphrodite.utils.torch_utils import set_random_seed
from aphrodite.v1.attention.backends.utils import record_kv_cache_layout
...
def test_amd_decode_score_split_launch_policy(
    ...
    from aphrodite.models.minimax_m3.amd.ops.index_topk import (
...
def test_amd_optimized_decode_score_bitwise_and_graph_replay(
    ...
    def score_masks(
        ...
        write_mask = block_ids[None, :] < request_blocks.repeat_interleave(decode_query_len)[:, None]
...
def _reference_decode_index_score(
    ...
    for req_id, seq_len in enumerate(seq_lens.tolist()):
        ...
        out[:, token_start : token_start + decode_query_len, :num_blocks] = (
            score.reshape(num_idx_heads, decode_query_len, num_blocks, BLOCK_SIZE).max(dim=3).values
        )
    return out
...
def _fmha_indexer_topk(
    ...
    from aphrodite.third_party.fmha_sm100.api import _fmha_sm100, _fmha_sm100_plan
...
def test_msa_indexer_impl_matches_triton(topk, index_dtype, monkeypatch):
    ...
    common.positions = torch.cat(
        [torch.arange(s - q, s, device=device, dtype=torch.int64) for s, q in zip(batch.seq_lens, batch.query_lens)]
    )
    ...
    index_cache = torch.zeros(num_pages, BLOCK_SIZE, head_dim, device=device, dtype=index_dtype)
    ...
    index_q = torch.ones(num_tokens, num_idx_heads * head_dim, device=device, dtype=index_dtype)
    ...
    # Exercise the shared persistent top-k buffer for BOTH impls: each writes
    # into its token-major buffer. Separate buffers so the two forwards don't
    # clobber each other.
...
def test_amd_decode_fused_topk_total_order_and_replay(
    ...
    if num_topk_chunks == 16 and max_blocks >= 8192:
        ...
        boundary_limit = 4
    if boundary_limit:
        boundary_blocks = [(active_chunks - 1) * 512 + 1 for active_chunks in range(3, boundary_limit + 1)]
        assert [(num_blocks + 511) // 512 for num_blocks in boundary_blocks] == list(range(3, boundary_limit + 1))
        long_seq_lens_list.extend(num_blocks * SPARSE_BLOCK_SIZE for num_blocks in boundary_blocks)
        short_seq_lens_list.extend(num_blocks * SPARSE_BLOCK_SIZE for num_blocks in reversed(boundary_blocks))
    ...
    actual = torch.full((num_idx_heads, batch, topk), -2, device="cuda", dtype=torch.int32)
    dummy_block_table = torch.zeros((batch, max_blocks), device="cuda", dtype=torch.int32)
    addresses = (partial.data_ptr(), counter.data_ptr(), actual.data_ptr())
    long_seq_lens = torch.tensor(long_seq_lens_list, device="cuda", dtype=torch.int32)
    ...
    assert torch.equal(actual.cpu(), reference(long_seq_lens_list))
    assert torch.count_nonzero(counter).item() == 0

    seq_lens.copy_(short_seq_lens)
    launch()
    torch.accelerator.synchronize()
...
def test_amd_decode_index_topk_end_to_end(
    ...
    from aphrodite.models.minimax_m3.amd.ops.index_topk import (
        minimax_m3_index_decode as amd_minimax_m3_index_decode,
    )

    torch.manual_seed(0)
    topk = 16
    head_dim = 16
    seq_lens = torch.tensor([65537], device="cuda", dtype=torch.int32)
    q_lens = torch.tensor([decode_query_len], device="cuda", dtype=torch.int32)
    prefix_lens = seq_lens - q_lens
    max_seq_len = int(seq_lens.max())
    max_blocks = (max_seq_len + BLOCK_SIZE - 1) // BLOCK_SIZE
    block_table = torch.randperm(max_blocks, device="cuda", dtype=torch.int32).unsqueeze(0)
    idx_q = torch.randn(
        decode_query_len,
        num_idx_heads,
        head_dim,
    ...
    completion_counter = torch.zeros((num_idx_heads, decode_query_len), device="cuda", dtype=torch.int32)
...
def test_decode_index_score_cutedsl_correctness(
    ...
    from aphrodite.models.minimax_m3.nvidia.ops import (
    ...
    index_kv_cache = torch.randn(num_pages, BLOCK_SIZE, head_dim, device="cuda").to(dtype)
...
def test_aiter_sparse_pa_layout_contract(monkeypatch):
    ...
    import aphrodite.models.minimax_m3.common.sparse_attention as sparse_attn_mod
...
def test_aiter_sparse_block_table_handles_padded_decode_rows():
    ...
    from aphrodite.models.minimax_m3.amd.ops.sparse_pa import (
    ...
    sparse_bt, sparse_ctx = minimax_m3_build_sparse_block_table(topk_idx, block_table, seq_lens)
    torch.accelerator.synchronize()
...
def test_aiter_decode_sparse_block_table_supports_spec_decode():
    ...
    for row, selected_blocks in enumerate(topk_cpu):
        ...
        if tail_blocks:
            expected_ctx[row] = len(full_blocks) * BLOCK_SIZE + causal_len - self_block * BLOCK_SIZE
        else:
...
@pytest.mark.parametrize("block_page_stride", [8, 16])
@pytest.mark.parametrize(
    "seq_lens_list",
    [
        [4097],
        [65537],
        [1, 129, 4097, 0],
        [127, 128, 129, 256, 4097, 0],
        [1, 127, 128, 129, 255, 256, 257, 4097, 0],
    ],
)
def test_amd_decode_fused_topk_emits_sparse_table(block_page_stride: int, seq_lens_list: list[int]):
    """The fused selector must match top-k followed by table construction."""
    from aphrodite.models.minimax_m3.amd.ops.index_topk import (
        minimax_m3_index_decode as amd_minimax_m3_index_decode,
    )
    from aphrodite.models.minimax_m3.amd.ops.sparse_pa import (
    ...
    index_block_table = torch.zeros((batch, max_blocks), device="cuda", dtype=torch.int32)
    for req_id, seq_len in enumerate(seq_lens.tolist()):
        num_blocks = (seq_len + BLOCK_SIZE - 1) // BLOCK_SIZE
        pages = torch.arange(max_blocks, device="cuda", dtype=torch.int32)
        index_block_table[req_id, :num_blocks] = torch.roll(pages, req_id)[:num_blocks]

    # The index cache and main-attention cache have independent physical page
    # mappings in production. Disjoint values make using the wrong table fail.
    attention_block_table = torch.full_like(index_block_table, -31337)
    for req_id, seq_len in enumerate(seq_lens.tolist()):
        num_blocks = (seq_len + BLOCK_SIZE - 1) // BLOCK_SIZE
        first_page = 10_000 + req_id * max_blocks
        attention_block_table[req_id, :num_blocks] = torch.arange(
            first_page + num_blocks - 1,
            first_page - 1,
            -1,
            device="cuda",
            dtype=torch.int32,
        )
    assert not torch.equal(index_block_table, attention_block_table)

    idx_q = torch.randn(total_q, 1, HEAD_DIM, device="cuda", dtype=torch.bfloat16)
    index_kv_cache = torch.randn(
        max_blocks,
        BLOCK_SIZE,
        HEAD_DIM,
    ...
    expected_topk = amd_minimax_m3_index_decode(
        idx_q,
        index_kv_cache,
        index_block_table,
        seq_lens,
    ...
    expected_sparse_bt, expected_sparse_ctx = minimax_m3_build_sparse_block_table_decode(
        expected_topk,
    ...
    topk_out = torch.full_like(expected_topk, -27182)
    sparse_bt_out = torch.full(
        (total_q, TOPK * PAGES_PER_SPARSE_BLOCK),
        -27182,
        device="cuda",
        dtype=torch.int32,
    )
    sparse_ctx_out = torch.full((total_q,), -27182, device="cuda", dtype=torch.int32)
    actual_topk = amd_minimax_m3_index_decode(
        idx_q,
        index_kv_cache,
        index_block_table,
        seq_lens,
        ...
        sparse_block_table_out=sparse_bt_out,
        sparse_context_lens_out=sparse_ctx_out,
    ...
    assert actual_topk.data_ptr() == topk_out.data_ptr()
    assert torch.equal(actual_topk, expected_topk)
    assert torch.equal(sparse_bt_out, expected_sparse_bt)
    assert torch.equal(sparse_ctx_out, expected_sparse_ctx)
    assert torch.count_nonzero(completion_counter).item() == 0

    if seq_lens_list[-1] == 0:
        padded_rows = slice(total_q - decode_query_len, total_q)
        assert torch.all(actual_topk[:, padded_rows] == -1)
        assert torch.count_nonzero(sparse_bt_out[padded_rows]) == 0
        assert torch.count_nonzero(sparse_ctx_out[padded_rows]) == 0
