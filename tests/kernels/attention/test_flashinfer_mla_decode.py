...
def test_flashinfer_mla_decode(dtype: torch.dtype, bs: int, block_size: int):
    ...
    q, kv_cache, block_tables, seq_lens_tensor, max_seq_len = _make_decode_inputs(bs, block_size, dtype)
...
def test_flashinfer_trtllm_sparse_mla_decode_without_rope():
    ...
    slot_tables = torch.stack([torch.randperm(num_slots)[:sparse_topk] for _ in range(batch_size)]).to(torch.int32)
...
def test_flashinfer_mla_decode_workspace_supports_autotune():
    """Aphrodite's FlashInfer MLA decode workspace must be int8 for autotuning.
    ...
    ``workspace_buffer must be torch.int8`` (https://github.com/vllm-project/vllm/issues/46646).
    """
    from flashinfer.autotuner import autotune

    from aphrodite.v1.attention.backends.mla.flashinfer_mla import _get_workspace_buffer
