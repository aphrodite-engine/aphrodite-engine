...
def test_mask_empty_context() -> None:
    ...
    query_start_loc = torch.cat((torch.zeros(1, dtype=torch.int32), query_lens.cumsum(0))).cuda()
    context_lens = torch.tensor([4] * 32 + [0, 3], dtype=torch.int32)
    context_start_loc = torch.cat((torch.zeros(1, dtype=torch.int32), context_lens.cumsum(0))).cuda()
...
def test_merge_attn_states_both_empty(merge_fn, output_dtype) -> None:
    ...
    prefix_output = torch.zeros(num_tokens, num_heads, head_size, device="cuda", dtype=output_dtype)
    prefix_lse = torch.randn(num_heads, num_tokens, device="cuda")
    suffix_output = torch.zeros(num_tokens, num_heads, head_size, device="cuda", dtype=output_dtype)
...
def test_merge_attn_states(
    ...
    torch.testing.assert_close(output_lse_cuda.float(), output_lse_ref.float(), atol=atol, rtol=rtol)
