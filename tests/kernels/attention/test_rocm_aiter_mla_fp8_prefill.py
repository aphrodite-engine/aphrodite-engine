...
pytestmark = pytest.mark.skipif(
    ...
    reason="FP8 MLA prefill needs gfx950 (MI355) + an AITER build exporting mla_prefill_ps_asm_fwd / mla_reduce_v1",
)
...
def _make_impl(num_heads: int):
    ...
    from aphrodite.v1.attention.backends.mla.rocm_aiter_mla import AiterMLAImpl
...
def _build_prefill_metadata(
    ...
    from aphrodite.v1.attention.backends.mla.rocm_aiter_mla import AiterMLAMetadataBuilder
...
def test_fp8_prefill_matches_reference(seq_len: int, num_heads: int) -> None:
    ...
    out = torch.zeros(total_q, num_heads * V_HEAD_DIM, dtype=ATTN_OUT_DTYPE, device=device)
...
def test_fp8_prefill_metadata_width_matches_forward(num_heads: int) -> None:
    ...
    from aphrodite.v1.attention.backends.mla.rocm_aiter_mla import _FP8_PREFILL_TILE_Q
