...
def _on_split_decode_arch() -> bool:
    if not current_platform.is_rocm():
        return False
    try:
        from aphrodite.platforms.rocm import _ON_GFX942, _ON_GFX950
...
def _on_gfx950() -> bool:
    if not current_platform.is_rocm():
        return False
    try:
        from aphrodite.platforms.rocm import _ON_GFX950
...
def _pack_fp8_ds_mla_cache(kv: torch.Tensor, block_size: int, use_fnuz: bool) -> torch.Tensor:
    assert kv.shape[-1] == HEAD_DIM
    from aphrodite.models.deepseek_v4.common.ops.cache_utils import (
...
def _poison_fp8_ds_mla_cache_row(cache: torch.Tensor, block_size: int, slot: int = 0) -> None:
    flat = cache.flatten()
    ...
    flat[token_base + NOPE_HEAD_DIM : token_base + 576].view(torch.bfloat16)[0] = float("nan")
...
def _read_fp8_ds_mla_cache_rows(
    ...
    scales = torch.exp2(cache_flat[scale_base[:, None] + scale_offsets].to(torch.float32) - 127.0)
...
def _ref_sparse_decode_ragged(
    ...
    for query_idx in range(q.shape[0]):
        ...
        if main_rows[query_idx]:
            main_slots = torch.tensor(main_rows[query_idx], dtype=torch.int64, device=q.device)
            row_kv.append(_read_fp8_ds_mla_cache_rows(main_cache, main_slots, block_size, main_use_fnuz))
        if extra_cache is not None and extra_rows is not None and extra_rows[query_idx]:
            extra_slots = torch.tensor(extra_rows[query_idx], dtype=torch.int64, device=q.device)
            row_kv.append(_read_fp8_ds_mla_cache_rows(extra_cache, extra_slots, block_size, extra_use_fnuz))
...
def _launch_sparse_decode_reduce(
    ...
    from aphrodite.v1.attention.ops import rocm_aiter_mla_sparse as mod
...
def test_paged_mqa_logits_do_not_contain_nan(monkeypatch) -> None:
    ...
    class FakeWorkspaceManager:
        def get_simultaneous(self, *shapes_and_dtypes):
            return [torch.empty(shape, dtype=dtype, device=device) for shape, dtype in shapes_and_dtypes]
    ...
    monkeypatch.setattr(mod, "current_workspace_manager", lambda: FakeWorkspaceManager())
...
def test_extra_cache_nan_free_provenance_gate(monkeypatch) -> None:
    from aphrodite.models.deepseek_v4.amd import rocm as mod
...
@torch.inference_mode()
def test_rocm_ragged_graph_buffer_view_tracks_source_width(monkeypatch, on_gfx950: bool) -> None:
    from aphrodite.models.deepseek_v4.amd import rocm as rocm_mod
    ...
    expected_first_entries = first_indices.numel() if on_gfx950 else indices_buffer.numel()
    expected_second_entries = second_indices.numel() if on_gfx950 else indices_buffer.numel()
...
def test_rocm_capture_metadata_sets_adaptive_marker(monkeypatch) -> None:
    from aphrodite.models.deepseek_v4.amd import rocm as rocm_mod
    from aphrodite.models.deepseek_v4.sparse_mla import (
...
def test_sparse_attn_decode_split_k_kernel(monkeypatch, num_splits: int, with_extra: bool, with_sink: bool) -> None:
    ...
    other_split_fn = "_decode_num_splits" if _on_gfx950() else "_decode_gfx950_num_splits"
    monkeypatch.setattr(mod, split_fn, lambda *args, **kwargs: num_splits)
    monkeypatch.setattr(mod, other_split_fn, lambda *args, **kwargs: pytest.fail("wrong selector"))
...
@torch.inference_mode()
def test_sparse_attn_decode_gfx950_outer64_boundaries(monkeypatch, extra_len: int) -> None:
    from aphrodite.v1.attention.ops import rocm_aiter_mla_sparse as mod
    ...
    extra_cache = _pack_fp8_ds_mla_cache(
        torch.randn(num_extra_rows, HEAD_DIM, dtype=torch.bfloat16, device=device) * 0.125,
        block_size,
        use_fnuz=False,
    )
    _poison_fp8_ds_mla_cache_row(extra_cache, block_size)
...
def test_sparse_attn_decode_gfx950_graph_replay(monkeypatch) -> None:
    ...
    extra_rows = [
        list(range(query_idx * extra_per_query, (query_idx + 1) * extra_per_query)) for query_idx in range(num_queries)
    ]
    main_indices, main_indptr = _ragged_from_rows(main_rows, device)
...
def test_get_cached_wo_a_bf16_plain_caches() -> None:
    from aphrodite.v1.attention.ops.rocm_aiter_mla_sparse import _get_cached_wo_a_bf16
