...
def _read_fp8_ds_mla_cache(cache: torch.Tensor, slot: int, block_size: int, use_fnuz: bool) -> torch.Tensor:
    ...
    scales = torch.exp2(cache_flat[scale_base : scale_base + 7].to(torch.float32) - 127.0)
...
def _ref_sparse_decode_ragged(
    ...
    for query_idx in range(q.shape[0]):
        row_kv = [
            _read_fp8_ds_mla_cache(main_cache, int(slot), block_size, main_use_fnuz) for slot in main_rows[query_idx]
        ]
        if extra_cache is not None and extra_rows is not None:
            row_kv.extend(
                _read_fp8_ds_mla_cache(extra_cache, int(slot), block_size, extra_use_fnuz)
