...
if not is_aiter_found_and_supported():
    pytest.skip(
        "ROCm AITER sparse MLA metadata sync test requires a supported AITER installation.",
        allow_module_level=True,
    )

from aphrodite.v1.attention.backend import CommonAttentionMetadata
from aphrodite.v1.attention.backends.mla import rocm_aiter_mla_sparse as sparse_mod
...
def _make_builder():
    ...
    builder.req_id_per_token_buffer = torch.zeros(max_num_batched_tokens, dtype=torch.int32, device="cpu")
    builder.qo_indptr = torch.arange(max_num_batched_tokens + 1, dtype=torch.int32, device="cpu")
    builder.paged_kv_last_page_len = torch.ones(max_num_batched_tokens, dtype=torch.int32, device="cpu")
    builder.paged_kv_indices = torch.zeros(max_num_batched_tokens * topk_tokens, dtype=torch.int32, device="cpu")
    builder.paged_kv_indptr = torch.zeros(max_num_batched_tokens + 1, dtype=torch.int32, device="cpu")
...
def _patch_build_deps(monkeypatch, events=None):
    ...
    def fake_generate_sparse_seqlen_triton(query_lens, seq_lens, cu_query_lens, topk_token, num_tokens, max_query_len):
    ...
    monkeypatch.setattr(sparse_mod, "generate_sparse_seqlen_triton", fake_generate_sparse_seqlen_triton)
    monkeypatch.setattr(
        sparse_mod.torch.cuda,
        "current_stream",
        lambda device=None: SimpleNamespace(synchronize=lambda: events.append("sync") if events is not None else None),
    )
...
def test_build_populates_decode_only_split_fields(monkeypatch):
    ...
    md = builder.build(common_prefix_len=0, common_attn_metadata=_make_common_metadata())
...
def test_build_populates_mixed_split_fields(monkeypatch):
    ...
    md = builder.build(common_prefix_len=0, common_attn_metadata=_make_mixed_common_metadata())
