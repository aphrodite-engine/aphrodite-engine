...
def _apply_gptj_rope(x: torch.Tensor, positions: torch.Tensor, cos_sin_cache: torch.Tensor) -> torch.Tensor:
...
def test_context_kv_concat_accepts_strided_inputs(num_tokens: int, num_heads: int, input_dtype: torch.dtype) -> None:
    k_nope, k_pe, _ = _strided_context_inputs(num_tokens, num_heads, input_dtype, k_pe_fp8=False)
    ...
    torch.testing.assert_close(k[..., 128:], k_pe.expand(-1, num_heads, -1), atol=0, rtol=0)
...
def test_context_kv_pack_quantizes_strided_inputs(
    ...
    k_nope, k_pe, v = _strided_context_inputs(num_tokens, num_heads, input_dtype, k_pe_fp8=k_pe_dtype == "fp8")
...
def test_prefill_epilogue_fuses_gptj_rope(cache_kind: str) -> None:
    ...
    q_expected = q.clone()
    q_expected[..., 128:] = _apply_gptj_rope(q_expected[..., 128:], positions, cos_sin_cache)
    k_pe_expected = _apply_gptj_rope(k_pe, positions, cos_sin_cache)
    k_expected = torch.cat((k_nope, k_pe_expected[:, None, :].expand(-1, _NUM_HEADS, -1)), dim=-1)
    ...
    if cache_kind == "bf16":
        ...
        cache = torch.zeros(2, _BLOCK_SIZE, 576, device="cuda", dtype=torch.float8_e4m3fn)
...
def test_decode_epilogue_fuses_gptj_rope(cache_kind: str) -> None:
    ...
    if cache_kind == "bf16":
        cache = torch.zeros(2, _BLOCK_SIZE, 576, device="cuda", dtype=_DTYPE)
        q_actual = fused_mla_decode_q_concat_kv_cache_insert(ql_nope, q_pe, kv_c, k_pe, cache, slots, **kwargs)
        torch.testing.assert_close(q_actual, q_expected)
        torch.testing.assert_close(_cache_rows(cache, slots), cache_expected)
    elif cache_kind == "fp8":
        cache = torch.zeros(2, _BLOCK_SIZE, 576, device="cuda", dtype=torch.float8_e4m3fn)
...
def test_decode_epilogue_preserves_nope_path() -> None:
    ...
    q_actual = fused_mla_decode_q_concat_kv_cache_insert(ql_nope, q_pe, kv_c, k_pe, cache, slots)

    torch.testing.assert_close(q_actual, torch.cat((ql_nope, q_pe), dim=-1))
    torch.testing.assert_close(_cache_rows(cache, slots), torch.cat((kv_c, k_pe), dim=-1))
