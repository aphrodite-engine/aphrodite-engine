...
pytestmark = pytest.mark.skipif(not current_platform.is_cuda(), reason="Kimi-K3 fused MLA ops require CUDA")
...
def test_prefill_concat_ignores_negative_slots_for_cache(cache_format: str) -> None:
    ...
    op = fused_mla_key_concat_kv_cache_insert if cache_format == "bf16" else fused_mla_key_concat_ds_mla_insert
    output = op(
        inputs["q"],
        inputs["k_nope"],
        inputs["k_pe"],
    ...
    torch.testing.assert_close(output, _full_key(inputs["k_nope"], inputs["k_pe"]), rtol=0, atol=0)
