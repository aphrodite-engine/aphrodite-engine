...
from aphrodite.v1.attention.backends.mla import rocm_aiter_mla
...
def _disable_native_h24(monkeypatch):
    ...
    monkeypatch.setattr(rocm_aiter_mla, "_aiter_mla_native_h24_supported", lambda: False)
...
def _make_h12_decode_metadata(device: torch.device) -> AiterMLAMetadata:
    ...
    decode = AiterMLADecodeMetadata(
        ...
        paged_kv_indptr=torch.tensor([0, CONTEXT_LEN], dtype=torch.int32, device=device),
...
def test_h24_reducer_without_metadata_still_pads_to_h32(monkeypatch):
    monkeypatch.setattr(rocm_aiter_mla, "_aiter_mla_native_h24_reducer_supported", lambda: True)
    monkeypatch.setattr(rocm_aiter_mla, "_aiter_mla_native_h24_metadata_supported", lambda: False)
    monkeypatch.setattr(
        rocm_aiter_mla,
...
def test_native_h24_requires_reducer_and_metadata(monkeypatch):
    monkeypatch.setattr(rocm_aiter_mla, "_aiter_mla_native_h24_reducer_supported", lambda: True)
    monkeypatch.setattr(rocm_aiter_mla, "_aiter_mla_native_h24_metadata_supported", lambda: True)
    monkeypatch.setattr(
        rocm_aiter_mla,
        "_aiter_mla_native_h24_supported",
        lambda: (
...
def test_asm_padding_env_default_is_auto(monkeypatch):
    monkeypatch.delenv("APHRODITE_ROCM_AITER_MLA_ASM_PADDING", raising=False)
    import aphrodite.envs as envs

    assert envs.APHRODITE_ROCM_AITER_MLA_ASM_PADDING == "auto"


def test_asm_padding_env_force_asm_disables_gluon(monkeypatch):
    monkeypatch.setenv("APHRODITE_ROCM_AITER_MLA_ASM_PADDING", "asm")
...
def test_asm_padding_env_force_gluon_follows_arch(monkeypatch):
    monkeypatch.setenv("APHRODITE_ROCM_AITER_MLA_ASM_PADDING", "gluon")
    on_gfx950 = _on_gfx950()
...
def test_asm_padding_env_auto_matches_arch_gate(monkeypatch):
    monkeypatch.setenv("APHRODITE_ROCM_AITER_MLA_ASM_PADDING", "auto")
    on_gfx950 = _on_gfx950()
...
def test_h12_aiter_mla_decode_matches_reference():
    ...
    kv_cache = torch.randn(CONTEXT_LEN, 1, QK_HEAD_DIM, dtype=torch.bfloat16, device=device)
