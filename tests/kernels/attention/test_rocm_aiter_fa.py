...
wired through ``aphrodite.v1.attention.backends.rocm_aiter_fa``:
...
def _assert_aiter_supported() -> None:
    ...
    assert is_aiter_found_and_supported(), "aiter is required on supported ROCm hardware for this test"
...
def _run_direct_flash_attn_case(
    *,
    ...
    from aphrodite.v1.attention.backends.rocm_aiter_fa import cp_mha_gather_cache
...
def test_aiter_mha_backend_contract():
    ...
    assert not AiterFlashAttentionBackend.supports_attn_type(AttentionType.ENCODER_DECODER)
...
def test_aiter_mha_backend_validates_kv_cache_block_size():
    ...
    with pytest.raises(ValueError, match="Block size must be a multiple of 16"):
        AiterFlashAttentionBackend.customize_spec(
            FullAttentionSpec(block_size=15, num_kv_heads=8, head_size=128, dtype=torch.bfloat16)
        )
    # block_size == 1 is the per-token page-size probe and must pass through.
    AiterFlashAttentionBackend.customize_spec(
        FullAttentionSpec(block_size=1, num_kv_heads=8, head_size=128, dtype=torch.bfloat16)
    )
...
def test_aiter_mha_backend_supports_compute_capability_matches_mi3xx_probe():
    ...
    assert AiterFlashAttentionBackend.supports_compute_capability(DeviceCapability(0, 0)) is on_mi3xx()
...
def test_aiter_mha_env_flags_control_enablement(use_aiter, use_mha, expected, monkeypatch):
    ...
    with monkeypatch.context() as mp:
        mp.setenv("APHRODITE_ROCM_USE_AITER", "1" if use_aiter else "0")
        mp.setenv("APHRODITE_ROCM_USE_AITER_MHA", "1" if use_mha else "0")
        _reload_envs()
...
def test_aiter_mha_platform_gate_matches_install_and_arch():
    ...
    from aphrodite._aiter_ops import IS_AITER_FOUND, is_aiter_found_and_supported

    assert is_aiter_found_and_supported() is (current_platform.is_rocm() and on_mi3xx() and IS_AITER_FOUND)
...
def _run_single_seq_flash_attn_case(
    *,
    ...
    cu_query_lens = torch.tensor([0, query_len], dtype=torch.int32).cumsum(dim=0, dtype=torch.int32)
    cu_seq_lens = torch.tensor([0, kv_len], dtype=torch.int32).cumsum(dim=0, dtype=torch.int32)
...
def test_aiter_fa_head_sizes(head_size, dtype, num_heads, seq_lens):
    ...
    _print_close_stats(
        f"head_sizes dtype={dtype} head_size={head_size} num_heads={num_heads} seq_lens={seq_lens}",
        output,
        ref,
        atol=atol,
        rtol=rtol,
...
def test_aiter_mha_varlen_paged_kv(head_size, num_heads, seq_lens, dtype):
    ...
    Exercises: APHRODITE_ROCM_USE_AITER, APHRODITE_ROCM_USE_AITER_MHA
    """
    atol = 1.5e-2
    rtol = 1e-2
    _assert_aiter_supported()
    ...
    _print_close_stats(
        f"varlen_paged_kv dtype={dtype} head_size={head_size} num_heads={num_heads} seq_lens={seq_lens}",
        output,
        ref,
        atol=atol,
        rtol=rtol,
...
def test_aiter_mha_multi_batch(num_heads, head_size, dtype):
    ...
    _print_close_stats(
        f"multi_batch dtype={dtype} head_size={head_size} num_heads={num_heads} seq_lens={seq_lens}",
        output,
        ref,
        atol=atol,
        rtol=rtol,
...
def test_aiter_mha_varlen_fp8_kv(dtype):
    ...
    Exercises: APHRODITE_ROCM_USE_AITER, APHRODITE_ROCM_USE_AITER_MHA, FP8 KV path.
    """
    atol = 0.15
    rtol = 0.05
    _assert_aiter_supported()
    ...
    key_cache_fp8 = torch.clamp(torch.randn(NUM_BLOCKS, BLOCK_SIZE, num_kv_heads, head_size), -1.0, 1.0).to(FP8_DTYPE)
    value_cache_fp8 = torch.clamp(torch.randn(NUM_BLOCKS, BLOCK_SIZE, num_kv_heads, head_size), -1.0, 1.0).to(FP8_DTYPE)
