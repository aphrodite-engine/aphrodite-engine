...
from aphrodite._aiter_ops import is_aiter_found
...
_GLUON_SUPPORTED_TARGET = "aphrodite.v1.attention.backends.mla.rocm_aiter_mla._gluon_mla_decode_supported"
...
def _run_verify_block():
    ...
    aphrodite_config.speculative_config = SpeculativeConfig(method="ngram", num_speculative_tokens=QLEN - 1)
    aphrodite_config.model_config.get_num_attention_heads = types.MethodType(
    ...
    q_nope = torch.zeros(num_rows, NUM_QUERY_HEADS, KV_LORA_RANK, dtype=dtype, device=device)
    q_pe = torch.zeros(num_rows, NUM_QUERY_HEADS, QK_ROPE_HEAD_DIM, dtype=dtype, device=device)
    kv_cache = torch.zeros(num_gpu_blocks, PAGE_SIZE, HEAD_SIZE, dtype=dtype, device=device)
    ...
    with (
        patch(_GLUON_SUPPORTED_TARGET, lambda: True),
        patch(
            "aphrodite.v1.attention.backends.mla.rocm_aiter_mla._get_mla_gluon",
            lambda: spy,
        ),
    ):
...
def test_verify_mtp_uses_native_4d_gluon_entry():
    ...
    assert q_nope.dim() == 4, f"verify must pass 4-D q to mla_gluon, got {q_nope.dim()}-D shape {tuple(q_nope.shape)}"
    assert q_nope.shape[:2] == (num_reqs, QLEN), (
        f"expected q shape [num_reqs={num_reqs}, qlen={QLEN}, ...], got {tuple(q_nope.shape)}"
    )
    ...
    assert page_table.dim() == 1, f"verify MTP passes the 1-D paged_kv_indices buffer, got {page_table.dim()}-D"
    assert seq_info.dim() == 1 and seq_info.numel() == num_reqs + 1, (
        f"verify MTP passes paged_kv_indptr [num_reqs + 1], got shape {tuple(seq_info.shape)}"
    )
