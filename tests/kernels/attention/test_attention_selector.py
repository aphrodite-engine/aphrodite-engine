...
def test_env(
    ...
    with monkeypatch.context() as m:
        ...
        if device == "cpu":
            with patch("aphrodite.attention.selector.current_platform",
                       CpuPlatform()):
                ...
                if use_mla:
                    if name == "FLASHMLA" and block_size == 64:
                        ...
                        backend = get_attn_backend(16,
                                                   ...
                                                   False,
                                                   use_mla=use_mla)
                        expected = ("TRITON_MLA_APHRODITE_V1"
...
def test_flash_attn(monkeypatch: pytest.MonkeyPatch):
    ...
    with monkeypatch.context() as m:
        ...
        if original_module is not None:
            monkeypatch.setitem(sys.modules, 'aphrodite_flash_attn',
                                original_module)
        else:
...
def test_invalid_env(use_v1: bool, monkeypatch: pytest.MonkeyPatch):
    ...
    with monkeypatch.context() as m, patch(
            "aphrodite.attention.selector.current_platform", CudaPlatform()):
        m.setenv("APHRODITE_USE_V1", "1" if use_v1 else "0")
