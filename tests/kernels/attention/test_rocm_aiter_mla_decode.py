...
def _ref_mla_decode(
    ...
    output = torch.zeros(batch_size, num_heads, V_HEAD_DIM, dtype=q.dtype, device=q.device)
...
def _make_inputs(
    ...
    *,
    ...
    q = torch.randn(batch_size, actual_nhead, Q_HEAD_DIM, dtype=torch.bfloat16, device="cuda")
    kv_buffer = torch.randn(pool_size, 1, Q_HEAD_DIM, dtype=torch.bfloat16, device="cuda")
    o = torch.zeros(batch_size, actual_nhead, V_HEAD_DIM, dtype=torch.bfloat16, device="cuda")
...
def _ref_output(inputs: dict) -> torch.Tensor:
    ...
    return _ref_mla_decode(inputs["q"], kv_flat, inputs["kv_indptr"], inputs["kv_indices"])
