...
def test_maybe_fused_act_quant_fp8_dynamic_block(
    ...
    ref_out, ref_scales = per_token_group_quant_fp8(silu_out, group_size=group_size, use_ue8m0=False)

    torch.testing.assert_close(result.scale, ref_scales, rtol=1e-5, atol=1e-5)

    ref_deq = ref_out.to(torch.float32) * ref_scales.repeat_interleave(group_size, dim=1)
    result_deq = result.data.to(torch.float32) * result.scale.repeat_interleave(group_size, dim=1)
