...
introduce regressions, independent of Aphrodite's fusion/compilation passes.
...
import aphrodite._aiter_ops  # noqa: F401 - ensure ops are registered
...
def _dequantize_grouped(quantized: torch.Tensor, scales: torch.Tensor, group_size: int) -> torch.Tensor:
...
def test_rocm_aiter_rmsnorm_with_add_vs_torch():
    ...
    assert max_ulp_normed <= 1, f"rmsnorm_with_add normed: max ULP distance {max_ulp_normed} > 1"

    ulp_residual = bf16_ulp_distance(res_out, ref_residual)
    max_ulp_residual = int(ulp_residual.max().item())
    assert max_ulp_residual <= 1, f"rmsnorm_with_add residual: max ULP distance {max_ulp_residual} > 1"
...
def test_rocm_aiter_triton_rotary_embedding_vs_torch():
    ...
    q_aiter = query.clone()
    k_aiter = key.clone()
    torch.ops.aphrodite.rocm_aiter_triton_rotary_embedding(
        positions,
        q_aiter,
        k_aiter,
        head_size,
...
def test_rocm_aiter_act_mul_fp8_group_quant_roundtrip():
    ...
    x_quant, scale = torch.ops.aphrodite.rocm_aiter_act_mul_and_fp8_group_quant(x, group_size)
...
def test_rocm_aiter_rmsnorm_fused_dynamic_quant_vs_sequential():
    ...
    # Fused op
    fused_q, fused_scale = torch.ops.aphrodite.rocm_aiter_rmsnorm_fused_dynamic_quant(x, weight, eps, fp8_dtype)
...
def test_rocm_aiter_rmsnorm_fused_add_dynamic_quant_vs_reference():
    ...
    fused_q, fused_res_out, fused_scale = torch.ops.aphrodite.rocm_aiter_rmsnorm_fused_add_dynamic_quant(
...
def test_rocm_aiter_rmsnorm_fp8_group_quant_vs_sequential():
    ...
    fused_q, fused_scales = torch.ops.aphrodite.rocm_aiter_rmsnorm_fp8_group_quant(x, weight, eps, group_size)
...
def test_rocm_aiter_rmsnorm_with_add_fp8_group_quant_residual_accuracy():
    ...
    fused_q, fused_res, fused_scales = torch.ops.aphrodite.rocm_aiter_rmsnorm_with_add_fp8_group_quant(
...
def test_rocm_aiter_mla_dual_rmsnorm_group_quant_strided_split_view():
    ...
    def run(q, kv):
        return rocm_aiter_ops.fused_qk_rmsnorm_group_quant(q, q_weight, eps, kv, kv_weight, eps, group_size=group_size)
