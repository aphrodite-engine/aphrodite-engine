...
def test_moe_silu_clamp_uses_native_xpu_fallback(default_aphrodite_config, monkeypatch) -> None:
    ...
    expected = SiluAndMulWithClamp(clamp_limit, compile_native=False).forward_native(input)
...
def _assert_masked_moe_activation(
    ...
    *,
    ...
    if mask_layout == "flat":
        ...
        valid_token_counts = torch.tensor([max_num_tokens // 2], dtype=torch.int32, device=device)
        output = torch.full((max_num_tokens, d), 42.0, dtype=dtype, device=device)
    else:
        input = torch.randn(num_experts, max_num_tokens, input_dim, dtype=dtype, device=device)
        valid_token_counts = torch.tensor(
            [0, 1, max_num_tokens // 2, max_num_tokens],
            dtype=torch.int32,
            device=device,
        )
        output = torch.full((num_experts, max_num_tokens, d), 42.0, dtype=dtype, device=device)
...
def test_swiglu_limit_func_without_routing_uses_output_buffer() -> None:
    ...
    expected = torch.nn.functional.silu(gate.clamp(max=7.0)) * up.clamp(min=-7.0, max=7.0)
...
def test_masked_situ_and_mul(
    ...
    torch.ops._C.masked_situ_and_mul(output, input, expert_num_tokens, beta, linear_beta)
...
@torch.inference_mode()
def test_masked_moe_activation_opcheck(default_aphrodite_config) -> None:
    device = CUDA_DEVICES[0]
