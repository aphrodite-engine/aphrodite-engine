...
def _worker_forward_qk(
    ...
    from aphrodite.model_executor.layers.minimax_rms_norm.lamport_workspace import (
...
def _worker_forward_qk_rocm_aiter(
    ...
    try:
        ...
        for num_tokens, dtype in test_cases:
            ...
            q_norm.weight = nn.Parameter(q_weight[local_rank * hq : (local_rank + 1) * hq])
            k_norm.weight = nn.Parameter(k_weight[local_rank * hk : (local_rank + 1) * hk])
            ...
            with (
                ...
                patch.object(
                    ...
                    side_effect=AssertionError("ROCm AITER test unexpectedly used the fallback"),
                ),
            ):
                fused_q, fused_k, fused_v = MiniMaxText01RMSNormTP.forward_qkv(q_norm, k_norm, qkv.clone(), hq, hk)
...
def test_minimax_reduce_rms_qk_rocm_aiter(
    ...
    monkeypatch.setenv("APHRODITE_ROCM_USE_AITER_CUSTOM_AR", "1")
    monkeypatch.setenv("APHRODITE_ROCM_QUICK_REDUCE_QUANTIZATION", "NONE")
