...
(``aphrodite.model_executor.layers.fused_embed_norm``).

The guarantee: enabling ``APHRODITE_REPLICATE_EMBED`` (replicated table + fused
...
def test_fused_embed_norm_matches_reference():
    ...
    torch.testing.assert_close(normed.float(), _rmsnorm(embeds, weight, EPS), atol=1e-3, rtol=1e-2)
