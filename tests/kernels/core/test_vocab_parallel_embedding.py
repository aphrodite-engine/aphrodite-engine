...
(``aphrodite._custom_ops.vocab_parallel_embedding``).
...
def _shard(full_table, si, rows_per_rank, dtype):
    ...
    weight[added_at : added_at + num_added] = full_table[si.added_vocab_start_index : si.added_vocab_end_index]
    return weight
...
@pytest.mark.parametrize("vocab_size,org_vocab_size", [(128256, 128256), (32256, 32000)])
...
def test_matches_masked_gather(tp_size, vocab_size, org_vocab_size, hidden, id_dtype):
    ...
    for tp_rank in range(tp_size):
        ...
        torch.testing.assert_close(_fused(ids, weight, si), _reference(ids, weight, si), atol=0.0, rtol=0.0)
...
def test_rank_sum_is_full_lookup(tp_size):
    ...
    ids[:16] = torch.randint(org_vocab_size, vocab_size, (16,), dtype=torch.int32, device="cuda")
