...
import aphrodite._custom_ops as ops  # noqa: E402
...
def _deterministic_expert_routes(
    ...
    topk_ids = torch.cat([torch.full((size,), expert, dtype=torch.int32) for expert, size in enumerate(block_sizes)])
...
def test_mxfp4_cpu_zero_codes_stay_zero(zero_byte, e8m0):
    ...
    assert torch.equal(out, torch.zeros_like(out)), (
        f"zero code 0x{zero_byte:02x} with e8m0={e8m0} produced max |out| = {out.abs().max().item()}"
    )
