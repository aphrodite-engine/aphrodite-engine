...
from aphrodite.model_executor.layers.fused_moe.experts.trtllm_bf16_moe import (
    TrtLlmBf16ExpertsMonolithic,
)
from aphrodite.model_executor.layers.fused_moe.experts.trtllm_fp8_moe import (
    TrtLlmFp8ExpertsMonolithic,
)
from aphrodite.model_executor.layers.fused_moe.experts.trtllm_nvfp4_moe import (
...
def _shuffle_bf16_weights_block_major_k(w: torch.Tensor, epilogue_tile_m: int = 64, block_k: int = 128) -> torch.Tensor:
...
def test_trtllm_bf16_monolithic_routing_replay_records_valid_experts(
    ...
    if top_k > _DSV3_N_GROUP * _DSV3_TOPK_GROUP:
        pytest.skip(f"DSV3 requires top_k <= n_group * topk_group ({_DSV3_N_GROUP * _DSV3_TOPK_GROUP})")
    torch.manual_seed(0)
    ...
    hidden_states = torch.randn(num_tokens, hidden_size, device=device, dtype=torch.bfloat16) * 0.1
    router_logits = torch.rand(num_tokens, num_experts, device=device, dtype=torch.float32)
    ...
    for t in range(num_tokens):
        ...
        assert unique.numel() == top_k, (
            f"token {t}: expected {top_k} distinct experts, got {unique.numel()} ({replay[t].tolist()})"
        )
...
def test_routed_experts_capturer_e2e_via_monolithic_experts() -> None:
    ...
    import aphrodite.model_executor.layers.fused_moe.routed_experts_capturer as rec
    ...
    for t in range(num_tokens):
        ...
        assert unique.numel() == top_k, (
            f"token {t}: expected {top_k} distinct experts at layer {layer_id}, got {unique.numel()}"
        )
...
# ----------------------------------------------------------------------------
# FP8 block-scale (DeepSeekFp8) — Aphrodite's ``TrtLlmFp8ExpertsMonolithic``
# ----------------------------------------------------------------------------
...
def _make_fp8_block_scale_monolithic_experts(
    ...
    (same helper the Aphrodite weight loader uses for DeepSeek-FP8 models).
    """
    ...
    gemm1 = torch.randn(num_experts, 2 * intermediate_size, hidden_size, device=device).to(torch.float8_e4m3fn)
    gemm2 = torch.randn(num_experts, hidden_size, intermediate_size, device=device).to(torch.float8_e4m3fn)
    ...
    experts = TrtLlmFp8ExpertsMonolithic(moe_config=moe_config, quant_config=quant_config)
...
def test_trtllm_fp8_block_scale_monolithic_routing_replay_records_valid_experts(
    ...
    hidden_states = (torch.randn(num_tokens, hidden_size, device=device, dtype=torch.bfloat16) * 0.1).to(
        torch.float8_e4m3fn
    )
    hidden_states_scale = torch.ones(num_tokens, hidden_size // 128, device=device, dtype=torch.float32)
    router_logits = torch.rand(num_tokens, num_experts, device=device, dtype=torch.float32)
    routing_bias = _make_dsv3_routing_bias(num_experts, device)
...
# ----------------------------------------------------------------------------
# NVFP4 — Aphrodite's ``TrtLlmNvFp4ExpertsMonolithic``
# ----------------------------------------------------------------------------
...
def _make_nvfp4_monolithic_experts(
    ...
    w2_scale = w2_scale.view(torch.float8_e4m3fn).reshape(num_experts, hidden_size, intermediate_size // block_size)
    ...
    experts = TrtLlmNvFp4ExpertsMonolithic(moe_config=moe_config, quant_config=quant_config)
...
def test_trtllm_nvfp4_monolithic_routing_replay_records_valid_experts(
    ...
    # The Aphrodite apply() does the .view(fp8_e4m3fn).reshape itself, so leave
