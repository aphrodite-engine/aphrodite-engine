...
def _run_single_group_topk(
    ...
    *,
    ...
    return fused_grouped_topk(
        hidden_states=torch.empty((logits.shape[0], 0), dtype=logits.dtype, device=logits.device),
...
def _single_group_reference(
    ...
    *,
    ...
    indices = torch.argsort(scores + bias.float(), dim=-1, descending=True, stable=True)[:, :topk]
...
def test_grouped_topk_single_group_large_batch():
    ...
    expected_values, expected_ids = _single_group_reference(logits, bias, 16, scoring_func="sigmoid", renormalize=True)
    actual_values, actual_ids = _run_single_group_topk(logits, bias, 16, scoring_func="sigmoid", renormalize=True)
...
def test_grouped_topk_single_group_nonfinite_scores(num_experts: int, num_finite: int, renormalize: bool):
    logits = torch.full((1, num_experts), float("nan"), dtype=torch.bfloat16, device="cuda")
    if num_finite:
        logits[0, :num_finite] = torch.arange(num_finite, dtype=torch.bfloat16, device="cuda")
    ...
    if num_finite == 0:
        expected_ids = torch.arange(16, dtype=torch.int32, device="cuda")[None]
        if renormalize:
            expected_values = torch.full((1, 16), 1 / 16, dtype=torch.float32, device="cuda")
        else:
        ...
        finite_values *= 2.5
        expected_values = torch.cat((finite_values, torch.zeros(1, dtype=torch.float32, device="cuda")))[None]
