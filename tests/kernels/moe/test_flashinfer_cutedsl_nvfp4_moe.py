...
from aphrodite.model_executor.layers.fused_moe.experts.flashinfer_cutedsl_moe import (
...
from aphrodite.utils.flashinfer import has_flashinfer_cutedsl_moe_nvfp4
...
if not has_flashinfer_cutedsl_moe_nvfp4() or not (current_platform.is_device_capability_family(100)):
    pytest.skip(
...
def _reference_activation(
    ...
    """Aphrodite's own op for this activation, so the reference is not re-derived."""
...
def _torch_moe_reference(
    ...
    return (out.view(m, topk, -1).float() * weights.view(m, topk, 1)).sum(dim=1).to(a.dtype)
...
def _quantize_nvfp4_linear(
    ...
    for expert_weight in weight:
        global_scale = (FLOAT8_E4M3_MAX * FLOAT4_E2M1_MAX / expert_weight.abs().max()).to(torch.float32)
