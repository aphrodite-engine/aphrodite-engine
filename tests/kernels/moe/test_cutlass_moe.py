...
from aphrodite.model_executor.layers.fused_moe.oracle import nvfp4 as nvfp4_oracle
...
def test_cutlass_moe_forwards_shared_activation_parameters():
    ...
    assert ApplyMoEActivationConfig.from_configs(moe_config, quant_config) == ApplyMoEActivationConfig(
        clamp_limit=7.0,
        alpha=1.75,
        beta=0.25,
...
@pytest.mark.parametrize(
    ("backend", "expected"),
    [
        ("cutlass", nvfp4_oracle.NvFp4MoeBackend.APHRODITE_CUTLASS),
...
def test_nvfp4_clamp_allows_shared_activation_backends(
    ...
    monkeypatch.setattr(nvfp4_oracle, "backend_to_kernel_cls", lambda backend: [SupportedExperts])
    moe_config = make_dummy_moe_config()
    moe_config.moe_backend = backend
    moe_config.swiglu_limit = 7.0

    selected, _ = nvfp4_oracle.select_nvfp4_moe_backend(moe_config, weight_key=None, activation_key=None)
...
def test_cutlass_moe_permutation_maps_padding_to_zero():
    ...
    num_experts = 2
    topk_ids = torch.tensor([[0, -1], [1, num_experts]], device="cuda", dtype=torch.int32)
...
def test_cutlass_fp4_moe_padded_routes_do_not_change_valid_output(quantization: str):
    ...
    w1 = torch.full((num_experts, 2 * n, k // 2), 0x11, device=device, dtype=torch.uint8)
    ...
    def run_moe(
        ...
        workspace2 = torch.full((m * topk, n), workspace_value, device=device, dtype=dtype)
