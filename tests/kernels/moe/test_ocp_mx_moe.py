...
def swigluoai(x, alpha: float = 1.702, limit: float = 7.0):
    ...
    # See SwigluOAIAndMul in aphrodite/model_executor/layers/activation.py
    gate, up = x[..., ::2], x[..., 1::2]
...
def test_rocm_mxfp4_moe_oracle(
    ...
    w13_dq = upcast_from_mxfp(w13_quant.view(torch.uint8), w13_scale, torch.bfloat16, axis=-1)
    w2_dq = upcast_from_mxfp(w2_quant.view(torch.uint8), w2_scale, torch.bfloat16, axis=-1)
