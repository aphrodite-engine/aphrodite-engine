...
from aphrodite.model_executor.layers.quantization.utils.ocp_mx_utils import OCP_MX_Scheme
from aphrodite.platforms import current_platform
from aphrodite.utils.flashinfer import has_flashinfer
from tests.kernels.moe.utils import check_accuracy

TRTLLM_GEN_MXFP4_AVAILABLE = current_platform.is_cuda() and current_platform.is_device_capability_family(100)
...
def test_mxfp4_loading_and_execution_moe(aphrodite_runner, model_case: ModelCase):
    if torch.accelerator.device_count() < model_case.tp:
        pytest.skip(f"This test requires >={model_case.tp} gpus, got only {torch.accelerator.device_count()}")
    ...
    with aphrodite_runner(
        ...
        #         QuarkLinearMethod)
        #     from aphrodite.model_executor.layers.quantization.quark.schemes.quark_ocp_mx import QuarkOCP_MX  # noqa: E501
...
def mxfp4_quant_dequant(x: torch.Tensor) -> torch.Tensor:
    ...
    return upcast_from_mxfp(quantized.view(torch.uint8), scale, torch.bfloat16, axis=-1).reshape(shape).float()
...
def reference_moe(
    ...
    if activation in ("swiglu", "silu"):
        t = swiglu(t, alpha=alpha, beta=beta, limit=limit, interleaved=use_interleaved_layout)
...
def test_trtllm_gen_mxfp4_fused_moe(
    ...
    if act_type == "mxfp8":
        ...
        hidden_states_scale = hidden_states_scale.view(torch.float8_e4m3fn).reshape(*hidden_states.shape[:-1], -1)
    else:
...
def test_flashinfer_cutlass_mxfp4_fused_moe(
    ...
    w13_q_swapped = interleave_moe_weights_for_sm90_mixed_gemm(w13_q_swapped, quant_type="fp4")
...
@pytest.mark.parametrize("alpha,beta,limit", [(1.0, 1.0, None), (1.702, 1.0, 7.0)])
@pytest.mark.skipif(
    not (current_platform.is_cuda() and current_platform.is_device_capability_family(100) and has_flashinfer()),
...
def test_convert_standard_mxfp4_weights_for_flashinfer_cutlass(monkeypatch, backend_name, interleaves_weights):
    ...
    expected_w13_bias = torch.cat([w13_bias[:, 2:], w13_bias[:, :2]], dim=1).to(torch.bfloat16)
...
def test_gpt_oss_quant_config_supplies_clamped_swiglu_params():
    ...
    from aphrodite.model_executor.layers.quantization.mxfp4 import GptOssMxfp4MoEMethod
    ...
    quant_config = GptOssMxfp4MoEMethod.get_fused_moe_quant_config(fake_method, fake_layer)
...
def test_flashinfer_experts_swiglu_params_follow_quant_config(supplied: bool):
    ...
    from aphrodite.model_executor.layers.fused_moe.experts.flashinfer_cutlass_moe import (
        FlashInferExperts,
    )
    ...
    extra = {"gemm1_alpha": 0.5, "gemm1_beta": 0.25, "gemm1_clamp_limit": 3.0} if supplied else {}
    quant_config = mxfp4_w4a16_moe_quant_config(w1_scale=w1_scale, w2_scale=w2_scale, **extra)
...
def test_trtllm_gen_mxfp8_block_scale_moe(
    ...
    hidden_states = torch.randn(num_tokens, hidden_size, device=device, dtype=torch.bfloat16) / 20
    w13 = (
        torch.randn(
            num_experts,
            inter_size,
    ...
    hidden_states_q, hidden_states_scale = mxfp8_quantize(hidden_states, is_sf_swizzled_layout=False)
    ...
    hidden_states_ref = mxfp8_dequantize(hidden_states_q, hidden_states_scale).to(torch.float32)
    bias13 = torch.zeros(
        num_experts,
    ...
    for i in range(num_experts):
        ...
        if is_gated:
            ...
            w13_scale_interleaved = reorder_rows_for_gated_act_gemm(w13_scale_interleaved)
        gemm1_weights_shuffled.append(
            shuffle_matrix_a(w13_interleaved.view(torch.uint8), epilogue_tile_m).contiguous().view(w13_q.dtype)
        )
        gemm2_weights_shuffled.append(
            shuffle_matrix_a(w2_q[i].view(torch.uint8), epilogue_tile_m).contiguous().view(w2_q.dtype)
        )
...
def test_rocm_mxfp4_moe_oracle(
    ...
    import aphrodite.distributed.parallel_state as ps
    ...
    from aphrodite.model_executor.layers.fused_moe import FusedMoEConfig
    from aphrodite.model_executor.layers.fused_moe.config import (
        FusedMoEParallelConfig,
        RoutingMethodType,
    )
    ...
    w13_float = torch.randn(num_experts, 2 * intermediate_size, hidden_size, dtype=dtype, device=device)
    w2_float = torch.randn(num_experts, hidden_size, intermediate_size, dtype=dtype, device=device)
    ...
    w13_bias = torch.randn(num_experts, 2 * intermediate_size, dtype=dtype, device=device)
    ...
    w13_dq = upcast_from_mxfp(w13_quant_ref.view(torch.uint8), w13_scale_ref, torch.bfloat16, axis=-1)
    w2_dq = upcast_from_mxfp(w2_quant_ref.view(torch.uint8), w2_scale_ref, torch.bfloat16, axis=-1)
    ...
    use_interleaved = bool(config.get("interleaved_layout", activation == MoEActivation.SWIGLUOAI))
    ...
    print(f"  Reference: min={ref.min():.4f}, max={ref.max():.4f}, mean={ref.mean():.4f}")
    print(f"  Output:    min={out.min():.4f}, max={out.max():.4f}, mean={out.mean():.4f}")
    print(f"  Abs diff:  min={diff.min():.4f}, max={diff.max():.4f}, mean={diff.mean():.4f}")
    print(f"  Rel diff:  min={rel_diff.min():.4f}, max={rel_diff.max():.4f}, mean={rel_diff.mean():.4f}")
...
@pytest.mark.skipif(not is_aiter_found_and_supported(), reason="AITER is not installed or supported")
...
def test_mxfp4_emulation_rounds_up_to_block_size(
    ...
    rounded_hidden, rounded_intermediate = mxfp4_round_up_hidden_size_and_intermediate_size(
...
def test_select_mxfp4_moe_backend_raises_with_unsupported_reasons(
    ...
    """
    import aphrodite.model_executor.layers.fused_moe.oracle.mxfp4 as mxfp4_oracle
    from aphrodite.model_executor.layers.fused_moe import FusedMoEConfig
    from aphrodite.model_executor.layers.fused_moe.activation import MoEActivation
    from aphrodite.model_executor.layers.fused_moe.config import (
        FusedMoEParallelConfig,
    ...
    monkeypatch.setattr(mxfp4_oracle, "backend_to_kernel_cls", lambda backend: [UnsupportedExperts])
...
def test_emulation_activation_quant_dtype_is_dispatchable(ocp_mx_scheme):
    ...
    out, _ = moe_kernel_quantize_input(a, a_scale, quant_dtype, False, None, quantization_emulation=True)
...
def test_emulation_a_mxfp6_moe_forward_quantizes_activations():
    ...
    w13, w13_scale = mxfp4_quantize(
        torch.randn(num_experts, 2 * intermediate_size, hidden_size, dtype=dtype, device=device) / 8
    )
    w2, w2_scale = mxfp4_quantize(
        torch.randn(num_experts, hidden_size, intermediate_size, dtype=dtype, device=device) / 8
    )
