...
def _make_experts(
    ...
    if experts_backend != "trtllm_fp8":
        ...
        experts_cls = next(cls for cls in backend_to_kernel_cls(backend) if issubclass(cls, mk.FusedMoEExpertsModular))
    ...
    reference_topk_weights = test_tensors.topk_weights.to(torch.bfloat16).to(torch.float32)
    torch_combined = torch_experts(
        test_tensors.rank_tokens,
        qw.w13_weight,
        qw.w2_weight,
...
def test_deep_ep_v2_moe_backends(
    ...
    world_size, dp_size = world_dp_size
    config = TestConfig(
        dtype=torch.float8_e4m3fn if experts_backend == "trtllm_fp8" else torch.bfloat16,
        topk=topk,
        m=m,
        k=k,
        n=n,
