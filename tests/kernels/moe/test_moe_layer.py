...
from aphrodite.model_executor.layers.fused_moe import FusedMoEFactory, MoERunner, fused_experts
...
from aphrodite.utils.import_utils import has_deep_ep, has_mori, has_nixl_ep
...
def on_gfx950() -> bool:
    if current_platform.is_rocm():
        from aphrodite.platforms.rocm import on_gfx950 as rocm_on_gfx950
...
def maybe_roundup_layer_hidden_size(
    ...
    if backend == "deepep_high_throughput":
        from aphrodite.model_executor.layers.fused_moe.prepare_finalize.deepep_ht import (
...
def test_group_deepep_ll_configs_by_buffer_requirements():
    ...
    assert _group_deepep_ll_configs([config_a, config_b, config_a_later, config_c, config_d]) == [
...
def is_valid_config(config: MoETestConfig) -> tuple[bool, str | None]:
    ...
    if config.quantization == "modelopt_fp4" and not (
        (current_platform.is_rocm() and on_gfx950()) or current_platform.has_device_capability(100)
    ):
    ...
    if config.backend is not None:
        ...
        if on_gfx950() and config.backend == "deepep_low_latency" and config.quantization == "modelopt_fp4":
            return (
                False,
                "DeepEP low latency requires a batched NVFP4 MoE backend, which is not available on gfx950.",
            )

        if config.backend in MORI_BACKENDS:
            if os.environ.get("APHRODITE_TEST_ENABLE_MORI_MOE_LAYER") != "1":
        ...
        if config.backend == "deepep_low_latency":
            from aphrodite.model_executor.layers.fused_moe.prepare_finalize.deepep_ll import (  # noqa: E501
                DeepEPLLPrepareAndFinalize,
            )
        ...
        if config.backend == "nixl_ep":
            from aphrodite.model_executor.layers.fused_moe.prepare_finalize.nixl_ep import (  # noqa: E501
...
def _run_one_config(
    ...
    **kwargs,
    ...
    - When is_sequence_parallel=True (EP + sequence splitting):
      ...
      * tp_size: Number of ranks to split sequence across (becomes sp_size in FusedMoEFactory)
...
def test_moe_layer_no_parallel(
    ...
    if os.environ.get("APHRODITE_LOGGING_LEVEL") is None:
        monkeypatch.setenv("APHRODITE_LOGGING_LEVEL", "ERROR")
...
def _parallel_worker(
    ...
    **kwargs,
    ...
    for test_config in test_configs:
        ...
        any_failed_tensor = torch.tensor([int(local_failed)], device=pgi.device, dtype=torch.int32)
        torch.distributed.all_reduce(any_failed_tensor, op=torch.distributed.ReduceOp.MAX)
        ...
        if any_failed:
            ...
            torch.distributed.all_gather_object(gathered_errors, local_error, group=cpu_group)
            first_error = next(
    ...
    report = f"============= {fails}{skips}{passes} of {total} total subcases ============="
...
def _parallel_worker_rocm_deepep(
    ...
    **kwargs,
    ...
    try:
        ...
        try:
            torch.accelerator.synchronize()
            # Do not run Aphrodite cleanup: it explicitly destroys the DeepEP
...
def test_moe_layer(
    ...
    if backend in MORI_BACKENDS and os.environ.get("APHRODITE_TEST_ENABLE_MORI_MOE_LAYER") == "1":
        monkeypatch.setenv("APHRODITE_ROCM_USE_AITER", "1")
        monkeypatch.setenv("APHRODITE_ROCM_USE_AITER_MOE", "1")
        monkeypatch.setenv("APHRODITE_ROCM_USE_AITER_FUSION_SHARED_EXPERTS", "0")
        from aphrodite._aiter_ops import rocm_aiter_ops
    ...
    # (BF16, FP8 and NVFP4 paths), and APHRODITE_USE_FLASHINFER_MOE_INT4=1.
    ...
    eplb_config = EPLBConfig(communicator="torch_gloo" if enable_eplb and on_gfx950() else None)
    parallel_config = ParallelConfig(
    ...
    with tempfile.NamedTemporaryFile(prefix="moe-layer-failures-", delete=False) as failure_report_file:
