...
pytestmark = [
    ...
    pytest.mark.skipif(
        current_platform.is_cuda_alike() and not current_platform.has_device_capability(80),
...
def test_apply_moe_activation_supported_contract():
    supported = {activation for activation in MoEActivation if apply_moe_activation_supported(activation)}
...
@pytest.mark.parametrize(
    "activation",
    [activation for activation in MoEActivation if apply_moe_activation_supported(activation)],
)
...
def test_triton_activation_metadata_tracks_shared_apply(activation: MoEActivation):
    assert TritonExperts._supports_activation(activation) == apply_moe_activation_supported(activation)
