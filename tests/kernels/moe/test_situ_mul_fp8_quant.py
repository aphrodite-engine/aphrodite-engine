...
from aphrodite.model_executor.layers.fused_moe.activation import situ_and_mul_quant
...
def test_situ_and_mul_quant_pipelined(num_tokens: int, valid: int | None, dtype: torch.dtype) -> None:
    set_random_seed(42)
    d = SITU_D
    ...
    scale = torch.full((num_tokens, d // GROUP_SIZE), -1.0, dtype=torch.float32, device=DEVICE)
    out_sentinel = out.clone()
    num_valid_tokens = None if valid is None else torch.tensor(valid, dtype=torch.int32, device=DEVICE)
    ...
    if valid is not None and valid < num_tokens:
        ...
        assert torch.equal(out[valid:].view(torch.uint8), out_sentinel[valid:].view(torch.uint8))

    rows = num_tokens if valid is None else valid
    if rows == 0:
        return

    ref_q, ref_s = ref_situ_block_fp8_quant(inp[:rows], SITU_BETA, SITU_LINEAR_BETA, GROUP_SIZE)
...
def test_situ_and_mul_quant_topk_row_bound(tokens: int, topk: int) -> None:
    ...
    assert torch.equal(out[rows:].view(torch.uint8), out_sentinel[rows:].view(torch.uint8))
