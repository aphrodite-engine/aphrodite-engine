...
def torch_moe_align_block_size(
    ...
    expert_ids = torch.full((max_num_blocks,), -1, dtype=torch.int32, device=topk_ids.device)
...
def test_moe_align_block_size_with_expert_map(
    ...
    for i in range(m):
        experts = torch.randperm(num_experts, device="cuda")[:topk]
        for k in range(topk):
            topk_ids[i, k] = experts[k] if (experts[k] in local_experts) or not mask_inactive_experts else -1
    topk_ids[0, 0] = -1
