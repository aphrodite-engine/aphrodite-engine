...
def test_zen_int8_support_predicates(mock_zentorch_fused_moe, monkeypatch):
    ...
    assert ZenCPUExpertsInt8._supports_quant_scheme(kInt8StaticChannelSym, kInt8DynamicTokenSym)
    ...
    assert not ZenCPUExpertsInt8._supports_activation(MoEActivation.SWIGLUOAI_UNINTERLEAVE)
    assert not ZenCPUExpertsInt8._supports_no_act_and_mul()
    assert ZenCPUExpertsInt8._supports_parallel_config(FusedMoEParallelConfig.make_no_parallel())
...
def test_zen_int8_dispatch_contract(
    ...
    input = torch.randn((BATCH_SIZE, HIDDEN_SIZE), dtype=torch.bfloat16) / (0.5 * HIDDEN_SIZE**0.5)
