...
def test_x86_cpu_unquantized_kernel_selection(
    ...
    expected_kernel_cls = X86CPUUnquantizedExperts if expect_amx_kernel else CPUUnquantizedExperts
...
def test_arm_cpu_unquantized_kernel_selection(
    ...
    with (
        ...
        patch(
            "aphrodite.model_executor.layers.fused_moe.experts.cpu_moe.sys.platform",
            platform,
        ),
    ):
        moe_config = make_dummy_moe_config(
    ...
    expected_kernel_cls = ArmCPUUnquantizedExperts if expect_arm_kernel else CPUUnquantizedExperts
    assert kernel_cls is expected_kernel_cls
...
@patch(
    "aphrodite.utils.flashinfer.has_flashinfer",
    return_value=False,
)
@patch(
...
@patch(
    "aphrodite.model_executor.layers.fused_moe.experts.trtllm_bf16_moe.TrtLlmBf16ExpertsMonolithic.is_supported_config",
    return_value=(True, None),
)
@pytest.mark.skipif(not current_platform.is_cuda(), reason="Only supported on NVIDIA platforms.")
def test_select_cuda_flashinfer_trtllm_backend(mock_is_supported_trtllm_monolithic):
...
@patch(
    "aphrodite.model_executor.layers.fused_moe.experts.trtllm_bf16_moe.TrtLlmBf16ExpertsModular.is_supported_config",
    return_value=(True, None),
)
@pytest.mark.skipif(not current_platform.is_cuda(), reason="Only supported on NVIDIA platforms.")
...
@patch(
    "aphrodite.model_executor.layers.fused_moe.experts.trtllm_bf16_moe.TrtLlmBf16ExpertsBase._supports_current_device",
    return_value=True,
)
@pytest.mark.parametrize(
    "all2all_backend",
...
def test_select_cuda_flashinfer_trtllm_modular_for_standard_all2all(
    ...
    with (
        ...
        patch.object(current_platform, "is_tpu", return_value=False),
        patch.object(current_platform, "is_out_of_tree", return_value=False),
        patch.object(current_platform, "is_device_capability_family", return_value=False),
    ):
