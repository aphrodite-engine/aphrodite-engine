...
def test_invalid_slots_excluded(num_tokens: int, top_k: int, use_expert_map: bool):
    ...
    inputs = torch.randn(num_tokens, top_k, HIDDEN_SIZE, dtype=torch.bfloat16, device=device)
...
def test_nonlocal_row_is_zeroed():
    ...
    topk_ids = torch.randint(local, NUM_EXPERTS, (num_tokens, top_k), dtype=torch.int32, device=device)

    outputs = torch.full((num_tokens, HIDDEN_SIZE), SENTINEL, dtype=torch.bfloat16, device=device)
    out = moe_fused_mul_sum(
        inputs=inputs,
        topk_weights=topk_weights,
        outputs=outputs,
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_weighted_sum_matches_reference(num_tokens: int, top_k: int, hidden_size: int, dtype: torch.dtype):
...
def test_expert_map_is_required_for_nonlocal_ids(hidden_size: int, dtype: torch.dtype):
    ...
    topk_ids[:, :half] = torch.randint(0, local, (num_tokens, half), dtype=torch.int32, device=device)
    topk_ids[:, half:] = torch.randint(local, NUM_EXPERTS, (num_tokens, top_k - half), dtype=torch.int32, device=device)
...
def test_num_valid_tokens_bounds_stale_padding(dtype: torch.dtype):
    ...
    bounded = moe_fused_mul_sum(
        inputs=inputs,
        topk_weights=topk_weights,
        outputs=torch.full((num_tokens, hidden_size), SENTINEL, dtype=dtype, device=device),
        topk_ids=topk_ids,
        num_valid_tokens=num_valid,
    )
    assert torch.all(bounded[num_recv:] == SENTINEL), "padding tail past num_recv written"
