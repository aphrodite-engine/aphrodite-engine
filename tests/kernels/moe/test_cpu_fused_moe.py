...
def quantize_per_channel(
    ...
    quantized = (weight_fp32 / scale.unsqueeze(-1)).round().clamp(-127, 127).to(torch.int8)
...
def ref_fused_moe_int8(
    ...
    return routed_output.view(*topk_ids.shape, -1).mul_(topk_weights.unsqueeze(-1)).sum(dim=1).to(input.dtype)
...
def test_cpu_fused_moe_int8(
    ...
    input = torch.randn((batch_size, hidden_size), dtype=dtype) / (0.5 * hidden_size**0.5)
    w13_fp = torch.randn((expert_num, up_dim, hidden_size), dtype=dtype) / (0.5 * hidden_size**0.5)
    w2_fp = torch.randn((expert_num, hidden_size, intermediate_size), dtype=dtype) / (0.5 * intermediate_size**0.5)
...
def test_cpu_fused_moe_unaligned_intermediate_size(
    ...
    moe_config = _make_moe_config(expert_num, hidden_size, intermediate_size, topk_num, dtype, act, use_bias)
    experts_cls = {
    ...
    assert supported, f"expected the padded intermediate size to be supported, got {reason}"
    ...
    quant_config = biased_moe_quant_config(layer.w13_bias, layer.w2_bias) if use_bias else FusedMoEQuantConfig.make()
