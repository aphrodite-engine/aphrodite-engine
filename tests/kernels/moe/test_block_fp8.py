...
def torch_w8a8_block_fp8_moe(a, w1, w2, w1_s, w2_s, topk_weight, topk_ids, block_shape, silu_fp32=False):
    ...
    for i in range(w1.shape[0]):
        ...
        if mask.sum():
            ...
            act_out = SiluAndMul().forward_native(inter_out.float() if silu_fp32 else inter_out)
            act_out_q, act_out_s = per_token_group_quant_fp8(act_out, block_k, dtype=current_platform.fp8_dtype())
...
def test_w8a8_block_fp8_fused_moe(M, N, K, E, topk, block_size, dtype, seed, monkeypatch, workspace_init):
    if topk > E:
...
def _dequant_block_fp8(w_fp8: torch.Tensor, w_s: torch.Tensor, block_shape: list[int]) -> torch.Tensor:
...
def test_w8a8_block_fp8_fused_moe_refined_block_scales(tp_rank, workspace_init):
    ...
    w1 = torch.cat([w1_full[:, lo:hi], w1_full[:, inter_full + lo : inter_full + hi]], dim=1)
    w2 = w2_full[:, :, lo:hi]
    ...
    w1_s32 = w1_s_full.repeat_interleave(factor, dim=-2).repeat_interleave(factor, dim=-1)
    ...
    w2_s32 = w2_s_full.repeat_interleave(factor, dim=-2).repeat_interleave(factor, dim=-1)
    ...
    with set_current_aphrodite_config(aphrodite_config):
        ref_out = torch_w8a8_block_fp8_moe(a, w1, w2, w1_s, w2_s, topk_weights, topk_ids, refined_block)
        out = fused_experts(a, w1, w2, topk_weights, topk_ids, quant_config=quant_config)
