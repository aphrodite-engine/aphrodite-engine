...
def test_reorder_w13_swigluoai_interleaved():
    ...
    out, out_scale = reorder_w13_to_w31_for_flashinfer_cutedsl(MoEActivation.SWIGLUOAI, w13, w13 + 100)

    torch.testing.assert_close(out, _EXPECTED)
    torch.testing.assert_close(out_scale, _EXPECTED + 100)


@pytest.mark.parametrize("activation", [MoEActivation.SILU, MoEActivation.SWIGLUOAI_UNINTERLEAVE])
def test_reorder_w13_packed_layouts(activation: MoEActivation):
    w13 = torch.cat([_GATE, _UP], dim=1)

    out, out_scale = reorder_w13_to_w31_for_flashinfer_cutedsl(activation, w13, w13 + 100)
