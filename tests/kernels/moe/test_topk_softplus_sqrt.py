...
from aphrodite.model_executor.layers.fused_moe.router.dsv4_topk import dsv4_topk
...
def _torch_topk_softplus_sqrt(
    ...
    if hash_indices_table is not None:
        ...
        if bias_vl is None or image_sentinel_lo == 0:
            ...
            image_mask = (input_ids.long() >= image_sentinel_lo) & (input_ids.long() < image_sentinel_lo + 5)
            topk_ids = hash_indices_table[input_ids.long()]
            vl_ids = (scores + bias_vl).topk(topk, dim=-1).indices
            topk_ids = torch.where(image_mask.unsqueeze(-1), vl_ids.to(topk_ids.dtype), topk_ids)
    else:
        if bias_vl is not None and image_sentinel_lo > 0:
            ...
            image_mask = (
                (input_ids.long() >= image_sentinel_lo) & (input_ids.long() < image_sentinel_lo + 5)
            ).unsqueeze(-1)
        ...
        topk_ids = torch.argsort(scores_for_choice, dim=-1, descending=True, stable=True)[:, :topk]
...
def test_fused_topk_softplus_sqrt_padding(
    ...
    if use_bias:
        e_score_correction_bias = -torch.rand((num_experts,), dtype=torch.float32, device="cuda") - 1.0
    ...
    if use_hash:
        ...
        hash_indices_table = torch.stack([torch.randperm(num_experts)[:topk] for _ in range(vocab_size)]).to(
            device="cuda", dtype=indices_dtype
        )
        input_ids = torch.randint(0, vocab_size, (num_tokens,), dtype=indices_dtype, device="cuda")
    ...
    if use_padding_mask:
        ...
        assert (pad_weights == 0).all(), f"Explicit pad rows should have all-zero weights, got {pad_weights.tolist()}"
    ...
    if pad_with_nan:
        ...
        assert (nan_pad_weights == 0).all(), (
            f"NaN-padded rows should have all-zero weights, got {nan_pad_weights.tolist()}"
        )
    ...
    torch.testing.assert_close(sorted_ref_ids, sorted_ids.to(sorted_ref_ids.dtype), atol=0, rtol=0)
...
def test_dsv4_fast_topk(
    ...
    topk_weights, topk_ids = dsv4_topk(gating_output, correction_bias, indices_type, 1.5)
...
def test_fused_topk_softplus_sqrt_hash(
    ...
    input_ids = torch.randint(0, vocab_size, (num_tokens,), dtype=torch.long, device="cuda")
...
def _make_mixed_input_ids(num_tokens: int, image_sentinel_lo: int, dtype: torch.dtype = torch.long) -> torch.Tensor:
    ...
    """
    input_ids = torch.randint(0, image_sentinel_lo, (num_tokens,), dtype=torch.long, device="cuda")
...
def test_fused_topk_softplus_sqrt_hash_bias_vl(
    ...
    torch.testing.assert_close(topk_ids[~image_rows].to(text_ids.dtype), text_ids, atol=0, rtol=0)
