...
def _td_supported() -> bool:
    return current_platform.is_xpu() or (current_platform.is_cuda() and current_platform.has_device_capability(90))
...
def test_batched_mm_td_matches_plain(num_experts, max_tokens_per_expert, K, N):
    ...
    set_random_seed(42)
    device = current_platform.device_type
    A = torch.randn(num_experts, max_tokens_per_expert, K, device=device, dtype=torch.bfloat16) / 10
...
def test_batched_triton_supports_masked_activations(monkeypatch, activation: MoEActivation):
...
def _torch_experts_with_shared_activation(
    ...
    for expert in range(w1.shape[0]):
        ...
        activated = torch.empty(projected.shape[0], w2.shape[2], dtype=a.dtype, device=a.device)
        apply_moe_activation(
            activation,
            activated,
            projected,
    ...
    return (routed_output.view(num_tokens, topk, hidden_dim).float() * topk_weights[..., None]).sum(dim=1).to(a.dtype)
...
@pytest.mark.parametrize(("activation", "activation_config"), _MASKED_BATCHED_ACTIVATION_CASES)
...
def test_batched_experts_masked_activation_end_to_end(
    ...
    moe_config.activation_situ_linear_beta = activation_config.activation_situ_linear_beta
    ...
    with set_current_aphrodite_config(aphrodite_config):
        ...
        triton_output = batched_moe(a, w1, w2, topk_weights, topk_ids, moe_config=moe_config)
...
def test_batched_experts_end_to_end(m, n, k, e, topk):
    """Validate BatchedTritonExperts against a torch reference."""
...
def test_batched_triton_backend_mapping():
    ...
    assert map_unquantized_backend("batched_triton") == UnquantizedMoeBackend.BATCHED_TRITON
