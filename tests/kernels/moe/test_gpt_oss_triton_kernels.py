...
def test_routing_data_from_sparse_topk_parity(n_tokens, n_experts, topk):
    ...
    from aphrodite.model_executor.layers.fused_moe.experts import (
    ...
    rd_new, gather_new, scatter_new = routing_data_from_sparse_topk(sparse_topk, n_experts)
    rd_ref, gather_ref, scatter_ref = make_routing_data(sparse_topk.indx.to(torch.long), sparse_topk.vals, n_experts)
    ...
    def assert_equivalent(new, ref, path="RoutingData"):
        ...
        if isinstance(ref, torch.Tensor):
            ...
            for f in fields(ref):
                assert_equivalent(getattr(new, f.name), getattr(ref, f.name), f"{path}.{f.name}")
        elif callable(ref):
