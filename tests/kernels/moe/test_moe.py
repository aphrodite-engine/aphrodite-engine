...
import aphrodite.model_executor.layers.fused_moe.fused_moe as fused_moe_module
...
def test_fused_shared_expert_alignment(workspace_init):
    ...
    topk_ids = torch.tensor([[0, 8], [1, 8], [2, 8], [3, 8]], device=DEVICE_TYPE, dtype=torch.int32)
    topk_weights = torch.tensor([[0.5, 1.0]] * m, device=DEVICE_TYPE, dtype=torch.float32)
...
def test_fused_moe_wn16(
    ...
    w1_ref = w1.clone()
    w2_ref = w2.clone()
    w1_qweight = torch.empty((e, 2 * n, k // pack_factor), device=DEVICE_TYPE, dtype=torch.uint8)
    w2_qweight = torch.empty((e, k, n // pack_factor), device=DEVICE_TYPE, dtype=torch.uint8)
    w1_scales = torch.empty((e, 2 * n, k // group_size), device=DEVICE_TYPE, dtype=dtype)
    ...
    w2_qzeros = torch.empty((e, k // pack_factor, n // group_size), device=DEVICE_TYPE, dtype=torch.uint8)
...
def _make_humming_indexed_experts(activation: MoEActivation):
    ...
    from aphrodite.model_executor.layers.quantization.utils import humming_utils
    from aphrodite.utils import humming
...
def test_humming_activation_metadata_tracks_shared_apply(activation: MoEActivation):
    ...
    assert HummingExpertsBase._supports_activation(activation) == apply_moe_activation_supported(activation)
...
def test_humming_selects_gemm_from_parallelism_and_override(
    ...
    if env_override is None:
        monkeypatch.delenv("APHRODITE_HUMMING_MOE_GEMM_TYPE", raising=False)
    else:
        monkeypatch.setenv("APHRODITE_HUMMING_MOE_GEMM_TYPE", env_override)
    moe_config = make_dummy_moe_config()
...
def test_humming_global_valid_shape_m(
    ...
    import aphrodite.model_executor.layers.fused_moe.experts.fused_humming_moe as humming

    dp_metadata = (
        None
        if dp_token_counts is None
        else SimpleNamespace(num_tokens_across_dp_cpu=torch.tensor(dp_token_counts, dtype=torch.int32))
    )
    monkeypatch.setattr(
        humming,
        "get_forward_context",
...
def test_humming_delegates_to_instance_activation():
    ...
    HummingExpertsBase.apply_activation(experts, MoEActivation.SWIGLUOAI_UNINTERLEAVE, output, input)
...
def test_batched_marlin_activation_uses_expert_token_counts(
    ...
    import aphrodite.model_executor.layers.fused_moe.experts.marlin_moe as marlin
    from aphrodite.model_executor.layers.fused_moe.experts.marlin_moe import (
    ...
    experts = SimpleNamespace(
        ...
        input_dtype=None,
        is_k_full=True,
        activation_config=ApplyMoEActivationConfig(clamp_limit=3.0, alpha=1.3, beta=0.5),
    )
    ...
    monkeypatch.setattr(marlin, "batched_fused_marlin_moe", fake_batched_fused_marlin_moe)
    output = torch.empty_like(hidden_states)
