...
def test_fused_moe(
    ...
    monkeypatch.setenv("APHRODITE_FUSED_MOE_CHUNK_SIZE", str(chunk_size))
    ...
    with set_current_aphrodite_config(aphrodite_config), set_forward_context(None, aphrodite_config):
        ...
        if padding:
            ...
            aphrodite_moe.experts.w2_weight = Parameter(
                F.pad(aphrodite_moe.experts.w2_weight, (0, 128), "constant", 0)[..., 0:-128],
                requires_grad=False,
            )
            torch.cuda.synchronize()
...
def test_cpu_fused_moe_basic(m, n, k, e, topk, dtype, with_bias, activation):
    from aphrodite.modeling.layers.fused_moe.cpu_fused_moe import CPUFusedMOE
