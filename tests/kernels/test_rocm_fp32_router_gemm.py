...
from aphrodite.model_executor.layers.fused_moe.router.gate_linear import (
    fp32_router_gemm_dispatch_impl,
)
from aphrodite.model_executor.layers.fused_moe.router.rocm_fp32_router_gemm import (
...
pytestmark = pytest.mark.skipif(not _on_gfx950(), reason="rocm_fp32_router_gemm requires ROCm gfx950")
...
@pytest.mark.parametrize(("hidden_size", "num_experts"), SHAPES)
@torch.inference_mode()
def test_rocm_fp32_router_gemm_preserves_topk(num_tokens: int, hidden_size: int, num_experts: int) -> None:
...
def test_rocm_fp32_router_gemm_cuda_graph_observes_input_mutation() -> None:
    ...
    torch.testing.assert_close(first_output, _reference(first_input, weight), atol=ATOL, rtol=RTOL)
    torch.testing.assert_close(second_output, _reference(second_input, weight), atol=ATOL, rtol=RTOL)
...
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float32])
@torch.inference_mode()
def test_rocm_fp32_router_gemm_custom_op_dispatch(num_tokens: int, dtype: torch.dtype) -> None:
    ...
    output = torch.ops.aphrodite.fp32_router_gemm_dispatch(x, weight, False)
...
def test_rocm_fp32_router_gemm_dynamic_compile_dispatch() -> None:
    ...
    def dispatch(x: torch.Tensor) -> torch.Tensor:
        return torch.ops.aphrodite.fp32_router_gemm_dispatch(x, weight, False)
