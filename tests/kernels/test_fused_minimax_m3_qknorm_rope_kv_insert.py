...
as the unfused path, followed by Aphrodite CUDA rotary_embedding-style NeoX RoPE.
"""
...
def assert_fp8_cache_close(kv_cache, expected_kv_cache):
    ...
    assert bool(ok.all()), f"fp8 cache differs by more than 1 ulp in {int((~ok).sum())} elements"
