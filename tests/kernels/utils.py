...
def _assert_accurate(
    ...
    assert rate >= pass_rate, f"Accuracy pass rate {rate:.6f} < {pass_rate} (atol={atol}, rtol={rtol})"
    ...
    assert mean_err <= atol * 0.25, f"Mean absolute error {mean_err:.6f} >= atol * 0.25 = {atol * 0.25:.6f}"
    ...
    for i in range(num_experts):
        ...
        if mask.sum():
            if quant_dtype is None:
                ...
                tmp2, tmp2_scale = moe_kernel_quantize_input(tmp2, None, quant_dtype, per_act_token_quant)
