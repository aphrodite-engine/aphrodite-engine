...
def test_emulation_kernel_accepts_any_supported_config(weight_quant_key, activation_quant_key):
    """EmulationMxfp6LinearKernel is the only backend today: it must accept
    every supported weight/activation format combination."""
    config = MxFp6LinearLayerConfig(weight_quant_key=weight_quant_key, activation_quant_key=activation_quant_key)
    can_implement, reason = EmulationMxfp6LinearKernel.can_implement(config)
...
def test_emulation_kernel_rejects_non_mxfp4_or_mxfp6_activation(weight_quant_key):
    config = MxFp6LinearLayerConfig(weight_quant_key=weight_quant_key, activation_quant_key=kMxfp4Static)
    can_implement, reason = EmulationMxfp6LinearKernel.can_implement(config)
    assert not can_implement
    assert reason
...
@patch("aphrodite.model_executor.kernels.linear.current_platform")
def test_init_mxfp6_linear_kernel_dispatches_to_registered_kernel(platform_mock):
    ...
    kernel = init_mxfp6_linear_kernel(weight_quant_key=kMxfp6E3M2Static, activation_quant_key=kMxfp6E3M2Dynamic)
...
def test_init_mxfp6_linear_kernel_raises_when_no_kernel_matches(platform_mock):
    ...
    register_linear_kernel(UnsupportedMxFp6LinearKernel, PlatformEnum.UNSPECIFIED, "mxfp6")
