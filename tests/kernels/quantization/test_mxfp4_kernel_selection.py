...
def test_weight_only_kernels_accept_unquantized_or_mxfp4_activation(kernel_cls, activation_quant_key):
...
def test_emulation_kernel_accepts_any_config(activation_quant_key, monkeypatch):
    ...
    config = MxFp4LinearLayerConfig(activation_quant_key=activation_quant_key)
    with patch(
        "aphrodite.model_executor.kernels.linear._get_linear_backend",
