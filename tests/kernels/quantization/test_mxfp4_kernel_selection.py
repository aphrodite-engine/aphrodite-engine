...
def test_aiter_kernel_is_supported_requires_native_mx_support():
    ...
    with patch(
        "aphrodite.model_executor.kernels.linear.mxfp4.aiter.current_platform.supports_mx",
        return_value=False,
    ):
...
def test_init_mxfp4_linear_kernel_raises_when_no_kernel_matches(platform_mock):
    ...
    register_linear_kernel(UnsupportedMxFp4LinearKernel, PlatformEnum.UNSPECIFIED, "mxfp4")

    with pytest.raises(ValueError, match="Failed to find a kernel"):
        init_mxfp4_linear_kernel()


@patch("aphrodite.model_executor.kernels.linear.mxfp4.aiter.is_aiter_found_and_supported")
@patch("aphrodite.model_executor.kernels.linear.mxfp4.aiter.current_platform")
@patch("aphrodite.model_executor.kernels.linear.current_platform")
def test_init_mxfp4_linear_kernel_raises_on_rocm_without_aiter(
    ...
    with pytest.raises(
        ValueError,
        match="(?s)Failed to find a kernel.*AITER not found or not supported on the current platform",
    ):
        init_mxfp4_linear_kernel()
