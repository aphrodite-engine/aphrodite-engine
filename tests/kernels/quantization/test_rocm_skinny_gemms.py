...
@pytest.mark.parametrize("seed", SEEDS)
@pytest.mark.skipif(not current_platform.is_rocm(), reason="only test for rocm")
def test_rocm_wvsplitk_kernel(n, k, m, dtype, padded_a, padded_b, bias_mode, xnorm, seed):
    torch.manual_seed(seed)
    cu_count = num_compute_units()
...
def test_rocm_wvsplitk_fp8_kernel(n, k, m, dtype, padded_a, padded_b, biased, xnorm, seed):
    torch.manual_seed(seed)
