...
def test_triton_w4a16_gemm_fake_impl():
    with FakeTensorMode():
        ...
        graph = make_fx(lambda a, b_q, scales: triton_w4a16_gemm(a, b_q, scales, qzeros=None, group_size=128))(
            a, b_q, scales
        )

        assert any(
            node.op == "call_function" and node.target == torch.ops.aphrodite.triton_w4a16_gemm.default
...
def test_triton_w4a16_process_weights_after_loading_repacks_layout():
    ...
    # Build dummy layer with Aphrodite parameter wrappers.
