...
- large-shape MXFP4 wrapper checks through Aphrodite's public helper op
...
PRESHUFFLED_SHAPES = [
    ...
    pytest.param(
        (32, 8192, 8192),
        marks=pytest.mark.skip(reason="aiter 0.1.20 preshuffled fp4 GEMM OOB (M32,N8192,K8192, gfx950)"),
    ),
]
...
def test_fp4_env_defaults():
    ...
    assert envs.APHRODITE_ROCM_USE_AITER_FP4BMM is True
...
def test_rocm_aiter_fp4_enablement_follows_env_and_arch(
    ...
    with monkeypatch.context() as mp:
        ...
        mp.setenv("APHRODITE_ROCM_USE_AITER_FP4BMM", "1" if use_fp4bmm else "0")
        _reload_envs()
        rocm_aiter_ops.refresh_env_variables()

        assert rocm_aiter_ops.is_asm_fp4_gemm_dynamic_quant_enabled() is expected_asm_gemm
...
def test_aphrodite_quant_dequant_mxfp4_matches_reference_on_large_shapes(shape, dtype):
    """The public Aphrodite MXFP4 QDQ helper should stay exact against the torch
...
def test_aphrodite_quant_dequant_mxfp4_is_deterministic():
    """The public Aphrodite MXFP4 QDQ helper should stay bitwise deterministic."""
    from aphrodite.model_executor.layers.quantization.utils.mxfp4_utils import (
        quant_dequant_mxfp4,
    )
...
def test_aiter_dynamic_mxfp4_quant_output_format():
    ...
    assert x_fp4.shape == (M, K // 2), f"Expected fp4 shape ({M}, {K // 2}), got {x_fp4.shape}"
    assert x_fp4.dtype == torch.uint8

    # One E8M0 scale byte per 32-element block; shape (M, K // 32)
    assert x_scale.shape == (M, K // 32), f"Expected scale shape ({M}, {K // 32}), got {x_scale.shape}"
...
def test_aiter_fp4_gemm_preshuffled_tuned_shapes(shape):
    ...
    packed inputs Aphrodite uses in production.
...
def test_aiter_hardware_fp4_dynamic_quant_format(shape):
    ...
    assert out_fp4.shape == (M, K // 2), f"Shape {shape}: expected fp4 ({M}, {K // 2}), got {out_fp4.shape}"
...
def test_aiter_hardware_fp4_quant_vs_triton():
    ...
    dynamic_per_group_scaled_quant_fp4(out_hw, x, scales_hw, group_size, shuffle_scale=False)
