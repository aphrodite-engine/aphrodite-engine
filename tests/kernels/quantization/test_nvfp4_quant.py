...
@torch.inference_mode()
def test_swizzled_scale_padding_initialized_in_kernel(shape: tuple[int, int], padded_n: int | None) -> None:
    ...
    out_scale = torch.full((rounded_m, rounded_scale_n // 4), 0x7F7F7F7F, dtype=torch.int32)
    ...
    full_scale = recover_swizzled_scales(out_scale.view(torch.float8_e4m3fn), m, physical_n, include_padding=True)
