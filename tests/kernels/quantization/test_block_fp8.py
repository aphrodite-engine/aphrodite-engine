...
def test_w8a8_block_fp8_torch_scaled_mm_matmul():
    ...
    A_fp8_cuda, As_cuda = per_token_group_quant_fp8(A_fp32, block_size[1], column_major_scales=True)

    stub = BlockWiseTorchFP8ScaledMMLinearKernel.__new__(BlockWiseTorchFP8ScaledMMLinearKernel)
    stub.config = types.SimpleNamespace(out_dtype=out_dtype)
    out = stub.apply_block_scaled_mm(A_fp8_cuda.cuda(), B_fp8.cuda(), As_cuda.cuda(), Bs.cuda())
