...
def _run_topk_backend(
    ...
    if backend == "top_k_per_row_decode":
        ...
        if indices.shape[0] > 64:
            pytest.skip("cooperative_topk supports <=64 rows; persistent_topk covers larger batches")
...
def test_top_k_per_row_decode_gfx950_long_c4a(
    ...
    seq_lens = min_seq_len + offsets * (max_seq_len - min_seq_len) // max(num_rows - 1, 1)
...
def test_aiter_c4a_prefill_topk_returns_sequence_local_indices() -> None:
    ...
    for row_idx, (row_start, row_end) in enumerate(zip(row_starts.tolist(), row_ends.tolist())):
...
def test_aiter_c4a_decode_topk_uses_exact_mtp_lengths(
    ...
    logits = torch.arange(num_rows * 16, dtype=torch.float32, device="cuda").reshape(num_rows, 16)
    top_k = 4
    indices = torch.empty((num_rows, top_k), dtype=torch.int32, device="cuda")
...
def test_aiter_c4a_topk_kernel_selection(monkeypatch) -> None:
    from aphrodite.v1.attention.ops import rocm_aiter_mla_sparse
