...
from aphrodite.models.deepseek_v32.nvidia import kernels as K
...
def test_platform_capability_queries_are_constant_during_compile(monkeypatch):
    monkeypatch.setattr(K.current_platform, "has_device_capability", lambda capability: True)
...
def test_pdl_is_disabled_before_blackwell(monkeypatch):
    monkeypatch.setattr(K.current_platform, "has_device_capability", lambda capability: False)
    monkeypatch.setattr(K.current_platform, "is_arch_support_pdl", lambda: True)
