...
from aphrodite.models.deepseek_v32.common import kernels as K
...
def test_fused_norm_rope_materializes_qk_without_local_cache_slots():
    ...
    mla_cache = torch.zeros(1, max_pos, KV_LORA + ROPE_DIM, device=dev, dtype=torch.bfloat16)
...
def test_fused_norm_rope_materializes_pcp_cache_inputs(has_indexer: bool):
    ...
    qw = torch.randn(Q_LORA, device=dev, dtype=torch.bfloat16)
    kvw = torch.randn(KV_LORA, device=dev, dtype=torch.bfloat16)
    ik = torch.randn(num_tokens, INDEX_HEAD_DIM, device=dev, dtype=torch.bfloat16) if has_indexer else None
    ikw = torch.randn(INDEX_HEAD_DIM, device=dev, dtype=torch.float32) if has_indexer else None
    ikb = torch.randn(INDEX_HEAD_DIM, device=dev, dtype=torch.float32) if has_indexer else None
    mla_cos_sin = make_cos_sin(max_pos, ROPE_DIM, dev)
...
def test_fused_q(num_tokens: int, num_q_heads: int, index_interleave: bool):
    ...
    q_pe = torch.randn(num_tokens, num_q_heads, ROPE_DIM, device=dev, dtype=torch.bfloat16)
