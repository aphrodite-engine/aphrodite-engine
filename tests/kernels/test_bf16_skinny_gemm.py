...
from aphrodite.model_executor.kernels.linear.cute_dsl.skinny_gemm import (
    SkinnyGemmConfig,
)
from aphrodite.models.deepseek_v32.nvidia import glm52_low_latency_gemm as glm52_gemm
from aphrodite.models.kimi_k3.nvidia import low_latency_gemm as k3_gemm
from aphrodite.models.kimi_k3.nvidia.low_latency_gemm import KIMI_K3_PROJECTIONS
from aphrodite.models.qwen4_exp.nvidia import low_latency_gemm as qwen4_exp_gemm
...
GLM_CUTE_CASES = [(spec, config) for spec in glm52_gemm.GLM52_PROJECTIONS.values() for _, config in spec.cute_configs]
...
def test_every_dsv3_routed_shape_is_instantiated() -> None:
    ...
    """
    source = (Path(__file__).resolve().parents[2] / "csrc" / "libtorch_stable" / "dsv3_fused_a_gemm.cu").read_text(
        encoding="utf-8"
    )
    # Benchmark-only shapes live behind APHRODITE_K3_BENCH_SHAPES and are not built
    # by default, so they must not count as available.
    production_macros = source.split("#ifdef APHRODITE_K3_BENCH_SHAPES")[0]
    explicit = source.split("#undef DISPATCH_DSV3_SHAPE")[1].split("#ifdef APHRODITE_K3_BENCH_SHAPES")[0]
    compiled = {
        (int(hd_in), int(hd_out))
        for hd_in, hd_out in re.findall(r"DISPATCH_DSV3_SHAPE\((\d+),\s*(\d+)\)", production_macros)
    } | {
        (int(hd_in), int(hd_out))
    ...
    missing = sorted((spec.n, spec.k) for spec in specs if spec.dsv3_tokens and (spec.k, spec.n) not in compiled)
    assert not missing, f"routed to dsv3 with no instantiation: {missing}; add DISPATCH_DSV3_SHAPE(K, N) for each"
...
def test_cute_configs_match_measured_table() -> None:
    ...
    actual = {(n, k, num_tokens): _config_tuple(config) for n, k, num_tokens, config in configs}
    assert actual == EXPECTED_CUTE_CONFIGS
    static_k_configs = {
        (n, k, num_tokens, config.static_k) for n, k, num_tokens, config in configs if config.static_k is not None
    }
...
def test_kda_overlap_configs_match_measured_table() -> None:
    ...
    assert {num_tokens: _config_tuple(config) for num_tokens, config in k3_gemm.KDA_QKVG_CONFIGS.items()} == {
        1: (64, 4, 2, 8),
        2: (64, 3, 2, 8),
    }
...
def test_kda_qkvg_autotune_enables_full_overlap(
    ...
    assert kda._projection_overlap_max_tokens == k3_gemm.KDA_PROJECTION_OVERLAP_MAX_TOKENS
...
def test_glm52_projection_plans_are_separate() -> None:
    ...
    assert {num_tokens for num_tokens, (backend, _) in qkv_a_plan.items() if backend == "cute"} == {1, 2}
    assert {num_tokens for num_tokens, (backend, _) in qkv_a_plan.items() if backend == "dsv3_fused_a"} == set(
        range(3, 17)
    )
    assert {num_tokens for num_tokens, (backend, _) in q_b_plan.items() if backend == "cute"} == {1, 2}
    assert {num_tokens for num_tokens, (backend, _) in q_b_plan.items() if backend == "dsv3_fused_a"} == set(
        range(3, 17)
    )
...
def test_glm52_installer_maps_only_selected_unquantized_shapes(
    ...
    root = nn.Module()
    root.attn = nn.Module()
    root.attn.fused_qkv_a_proj = FakeLinearBase(qkv_a.n, qkv_a.k, glm52_gemm.UnquantizedLinearMethod())
    root.attn.q_b_proj = FakeLinearBase(q_b.n, q_b.k, glm52_gemm.UnquantizedLinearMethod())
    root.same_shape_other_name = FakeLinearBase(qkv_a.n, qkv_a.k, glm52_gemm.UnquantizedLinearMethod())
    ...
    root.wrong_shape.fused_qkv_a_proj = FakeLinearBase(qkv_a.n + 1, qkv_a.k, glm52_gemm.UnquantizedLinearMethod())
    ...
    assert type(root.wrong_shape.fused_qkv_a_proj.quant_method) is glm52_gemm.UnquantizedLinearMethod
...
def test_low_latency_table_capability_routing(
    ...
    monkeypatch.setattr(k3_gemm.current_platform, "is_device_capability", lambda cc: False)
...
def test_installation_is_shape_specific_and_unquantized(
    ...
    assert isinstance(root.lm_head.quant_method, k3_gemm.KimiK3LowLatencyEmbeddingMethod)
    ...
    assert residual_warmup_configs == {
        config for _, config in k3_gemm.KIMI_K3_PROJECTIONS[(7168, 3584)].residual_configs
    }
...
def test_qwen4_exp_sm90_selected_shapes(
    ...
    cosine = torch.nn.functional.cosine_similarity(output.float().flatten(), reference.float().flatten(), dim=0).item()
    assert cosine > 0.999
...
def test_cute_selected_shapes(capability: tuple[int, int], n: int, k: int, num_tokens: int) -> None:
...
def test_kda_projection_overlap_cuda_graph(num_tokens: int) -> None:
    ...
    expected_qkvg = torch.nn.functional.linear(hidden_states.float(), packed_weight[:6144].float())
    expected_fab = torch.nn.functional.linear(hidden_states.float(), packed_weight[6144:6284].float())
    ...
    for actual, expected in (
        ...
        cosine = torch.nn.functional.cosine_similarity(actual.float().flatten(), expected.flatten(), dim=0).item()
        assert cosine > 0.999
...
def test_dsv3_selected_shapes(capability: tuple[int, int], num_tokens: int, n: int, k: int) -> None:
...
def test_kda_f_b_nonpacked_dsv3_dispatches(num_tokens: int) -> None:
    ...
    method = k3_gemm.KimiK3LowLatencyLinearMethod(k3_gemm._build_plan(spec), k3_gemm._build_residual_plan(spec))
...
def test_selected_kernels_cuda_graph_capture(capability: tuple[int, int], table: K3ProjectionTable) -> None:
    _require_capability_and_cute(capability)
    _require_capability_and_dsv3(capability)
    cute_spec = table[(6288, 7168)]
    dsv3_spec = table[(1536, 128)]
    cute_x = torch.randn(1, cute_spec.k, dtype=torch.bfloat16, device="cuda")
    cute_weight = torch.randn(cute_spec.n, cute_spec.k, dtype=torch.bfloat16, device="cuda")
    dsv3_x = torch.randn(1, dsv3_spec.k, dtype=torch.bfloat16, device="cuda")
    dsv3_weight = torch.randn(dsv3_spec.n, dsv3_spec.k, dtype=torch.bfloat16, device="cuda")
...
def test_latent_moe_production_layout_residual(
    ...
    buf = torch.randn(num_tokens, latent_dim + shared_dim, dtype=torch.bfloat16, device="cuda")
    ...
    assert not spy.calls, "non-packed buf-slice residual must fall back to addmm at every M"
...
@pytest.mark.skipif(
    ...
    "goes through the aphrodite::rocm_unquantized_gemm custom op, which has no CPU "
