...
from aphrodite.model_executor.kernels.linear.cute_dsl.skinny_gemm import SkinnyGemmConfig
...
CUTE_CASES = [
    (spec.n, spec.k, num_tokens) for spec in k3_gemm.KIMI_K3_PROJECTIONS.values() for num_tokens, _ in spec.cute_configs
]
...
def test_every_dsv3_routed_shape_is_instantiated() -> None:
    ...
    } | {(int(hd_in), int(hd_out)) for hd_in, hd_out in re.findall(r"hd_in == (\d+) && hd_out == (\d+)", explicit)}
