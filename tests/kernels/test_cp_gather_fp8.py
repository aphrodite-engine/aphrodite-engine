...
def test_cp_gather_and_upconvert_fp8_kv_cache(seq_lens, block_size):
    ...
    ops.cp_gather_and_upconvert_fp8_kv_cache(cache, dst, block_table, workspace_starts_t, num_reqs)
...
def test_cp_gather_fp8_shuffled_blocks():
    ...
    ops.cp_gather_and_upconvert_fp8_kv_cache(cache, dst, block_table, workspace_starts, len(seq_lens))
...
def test_cp_gather_fp8_with_sequence_starts(gather_seq_lens, seq_starts):
    ...
    expected = torch.cat(
        [
            full_expected[
                full_workspace_starts[i] + seq_starts[i] : full_workspace_starts[i] + seq_starts[i] + gather_seq_lens[i]
            ]
...
def test_cp_gather_fp8_large_uneven_sequences_with_starts():
    ...
    seq_starts = [3, 17, 5]
    full_seq_lens = [start + length for start, length in zip(seq_starts, gather_seq_lens)]
    (
        cache,
        block_table,
    ...
    workspace_starts[1:] = torch.tensor(gather_seq_lens[:-1], dtype=torch.int32, device="cuda").cumsum(dim=0)
    seq_starts_t = torch.tensor(seq_starts, dtype=torch.int32, device="cuda")
    dst = torch.empty(
        sum(gather_seq_lens),
        NOPE_DIM + ROPE_DIM,
    ...
    expected = torch.cat(
        [
            full_expected[
                full_workspace_starts[req_id] + seq_starts[req_id] : full_workspace_starts[req_id]
                + seq_starts[req_id]
