...
# Guard the ROCm-only import: `aphrodite.platforms.rocm` pulls in amdsmi and other
...
from aphrodite.platforms.rocm import on_gfx950  # noqa: E402
...
from aphrodite.v1.attention.ops.flydsl_turboquant_decode import (  # noqa: E402
...
def _build_cache(num_seqs, num_kv_heads, seq_len, qg, kv_block_size, seed=0xC0FFEE, device="cuda"):
    ...
    centroids = torch.randn(N_CENTROIDS, generator=g, dtype=torch.float32, device=device) * 0.5
    q = torch.randn(num_seqs, hq, HEAD_SIZE, generator=g, dtype=torch.float32, device=device) * 0.1
    ...
    block_table[:, :blocks_per_seq] = (
        torch.arange(num_seqs * blocks_per_seq, dtype=torch.int32, device=device).reshape(num_seqs, blocks_per_seq) + 1
    )
    ...
    knorm = (torch.rand(num_seqs, hk, seq_len, generator=g, device=device) * 0.5 + 0.5).to(torch.float16)
    vscale = (torch.rand(num_seqs, hk, seq_len, generator=g, device=device) * 0.05 + 0.01).to(torch.float16)
    vzero = ((torch.rand(num_seqs, hk, seq_len, generator=g, device=device) - 0.5) * 0.1).to(torch.float16)
    ...
    kv_cache = torch.zeros(total_blocks, bytes_per_block, dtype=torch.uint8, device=device)

    seq_ax = torch.arange(num_seqs, device=device).view(num_seqs, 1).expand(num_seqs, seq_len)
    tok_ax = torch.arange(seq_len, device=device).view(1, seq_len).expand(num_seqs, seq_len)
    blk_for_tok = block_table[seq_ax, tok_ax // kv_block_size]
    slot_for_tok = tok_ax % kv_block_size

    h_ax = torch.arange(hk, device=device)
    base_data = slot_for_tok.unsqueeze(1) * hk * DATA_BYTES_PER_SLOT + h_ax.view(1, hk, 1) * DATA_BYTES_PER_SLOT
    ...
    common = h_ax.view(1, hk, 1) * NUM_SOA_FIELDS * kv_block_size + slot_for_tok.unsqueeze(1)
