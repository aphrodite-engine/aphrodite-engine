...
from aphrodite.model_executor.layers.quantization.utils.mxfp8_utils import (  # noqa: E402
    _mxfp8_e4m3_quantize_torch,
    _mxfp8_e4m3_quantize_triton,
    dequant_mxfp8_to_bf16,
)
from aphrodite.models.minimax_m3.amd.ops import (  # noqa: E402
    gemma_fused_add_rmsnorm,
    gemma_rmsnorm,
    swiglu_oai_split,
)
from aphrodite.models.minimax_m3.amd.ops.gemma_rmsnorm import _num_warps  # noqa: E402
...
def test_mxfp8_grouped_gemm_native(T, N, K, E, top_k, weighted):
    ...
    sorted_ids, expert_ids, num_post = moe_align_block_size(topk_ids, block_m, E, None, ignore_invalid_experts=False)
...
# rather than the global APHRODITE_ROCM_USE_AITER switch. ``AiterMxfp8Experts.apply``
...
def _capture_expert_mask(expert_mask, *, global_num_experts):
    ...
    from aphrodite.model_executor.layers.fused_moe.experts.aiter_mxfp8_moe import (
        AiterMxfp8Experts,
    )
...
def test_aiter_mxfp8_apply_forwards_expert_mask_unchanged():
    ...
    ep_mask = torch.tensor([1, 1, 1, 1, 0, 0, 0, 0, 0], dtype=torch.int32, device=DEVICE)
...
def test_routed_experts_expert_map_delegates_to_kernel():
    ...
    def resolve(consumes_mask, *, has_moe_kernel=True):
        moe_kernel = (
            SimpleNamespace(fused_experts=SimpleNamespace(consumes_expert_mask=consumes_mask))
            if has_moe_kernel
            else None
        )
