...
if current_platform.is_rocm():
    from aphrodite.models.glm5next.amd.ops.kpool_compress import (
        kpool_compress_and_write_cache,
        kpool_decode_update_and_maybe_write_cache_batched,
        kpool_seed_tail_cache,
    )
else:
    from aphrodite.models.glm5next.nvidia.ops.kpool_compress import (
        kpool_compress_and_write_cache,
        kpool_decode_update_and_maybe_write_cache_batched,
        kpool_seed_tail_cache,
    )
...
def _make_caches():
    kv = torch.zeros(NUM_BLOCKS, PAGE_SIZE, HEAD_DIM + 4, dtype=torch.uint8, device="cuda")
    tail = torch.zeros(NUM_BLOCKS, 2, POOL_SIZE, HEAD_DIM, dtype=torch.bfloat16, device="cuda")
    return kv, tail
...
def _assert_eq(r_ref, r_kern):
    ...
    assert torch.equal(kv_ref, kv_kern), (
        f"kv_cache differs: max diff {(kv_ref.int() - kv_kern.int()).abs().max().item()}"
    )
    assert torch.equal(tail_ref, tail_kern), (
        f"tail_kv_cache differs: max diff {(tail_ref.float() - tail_kern.float()).abs().max().item()}"
    )
...
def test_amd_prefill_writer_uses_preshuffled_cache_layout():
    ...
    offsets = (token_offset // 16) * 16 * HEAD_DIM + (dim // 16) * 16 * 16 + (token_offset % 16) * 16 + dim % 16
...
def test_decode_writer_matches_prefill_writer(pool_size):
    ...
    differing = [
        p for p in range(n_pools) if not torch.equal(kv_prefill[p // page, p % page], kv_decode[p // page, p % page])
    ]
...
def test_leading_invalid_tail_slot():
    ...
    pos = torch.tensor([[-1, 13, 14, 15], [4, 5, 6, 7]], dtype=torch.int32, device="cuda")
...
def test_amd_prefill_seed_honors_padded_tail_block_stride():
    ...
    tail_slot = torch.tensor([block * kpool + ring_slot], dtype=torch.int32, device="cuda")
...
def test_batched_matches_reference(case_id):
    ...
    if case_id == "no_completion":
        ...
        pos = torch.arange(next_n, device="cuda", dtype=torch.int32).unsqueeze(0).expand(B, -1).contiguous()
        slot_map = torch.full((B, next_n), -1, dtype=torch.int32, device="cuda")
        n_prior = 0
    elif case_id == "completion_at_end":
        B, next_n, blocks = 2, 4, [0, 1]
        pos = torch.tensor([[12, 13, 14, 15], [12, 13, 14, 15]], dtype=torch.int32, device="cuda")
        slot_map = torch.full((B, next_n), -1, dtype=torch.int32, device="cuda")
        slot_map[:, 3] = torch.tensor([15, PAGE_SIZE + 15], dtype=torch.int32, device="cuda")
        n_prior = POOL_SIZE - next_n
    elif case_id == "completion_mid_batch":
        B, next_n, blocks = 3, 4, [0, 1, 2]
        pos = torch.tensor([[13, 14, 15, 16]] * B, dtype=torch.int32, device="cuda")
        slot_map = torch.full((B, next_n), -1, dtype=torch.int32, device="cuda")
        slot_map[:, 2] = torch.tensor([15, PAGE_SIZE + 15, 2 * PAGE_SIZE + 15], dtype=torch.int32, device="cuda")
        n_prior = 13
    elif case_id == "non_uniform_padding":
        B, next_n, blocks = 2, 4, [0, 1]
        pos = torch.tensor([[12, 13, 14, 15], [12, 13, -1, -1]], dtype=torch.int32, device="cuda")
        slot_map = torch.full((B, next_n), -1, dtype=torch.int32, device="cuda")
        slot_map[0, 3] = 15
        n_prior = POOL_SIZE - 4
    else:  # plain_decode
...
def test_batched_matches_reference_fuzz(seed):
    ...
    pos = starts.unsqueeze(1) + torch.arange(next_n, device="cuda", dtype=torch.int32).unsqueeze(0)
    tail_slot = _tail_slot_for(blocks, pos)
    ...
    key = torch.randn(B, next_n, HEAD_DIM, dtype=torch.bfloat16, device="cuda", generator=g)
    score = torch.randn(B, next_n, HEAD_DIM, dtype=torch.bfloat16, device="cuda", generator=g)
    ape = torch.randn(POOL_SIZE, HEAD_DIM, dtype=torch.float32, device="cuda", generator=g)
