...
import aphrodite.model_executor.kernels.mhc  # noqa: F401
import aphrodite.model_executor.layers.mhc as mhc_layers
...
def test_mhc_pre_tilelang(num_tokens, hidden_size, hc_mult):
    ...
    out = torch.ops.aphrodite.mhc_pre_tilelang(
        residual,
        fn,
        hc_scale,
        hc_base,
...
def test_mhc_post_tilelang(num_tokens, hidden_size, hc_mult):
    ...
    out = torch.ops.aphrodite.mhc_post_tilelang(
        x,
        residual,
        post_layer_mix,
        comb_res_mix,
...
def test_mhc_fused_post_pre(num_tokens, hidden_size, hc_mult):
    ...
    residual, post_mix, res_mix, x = torch.ops.aphrodite.mhc_fused_post_pre_tilelang(
        x,
        residual,
        post_layer_mix,
        comb_res_mix,
...
def _rocm_mhc_inputs(num_tokens=2, hidden_size=256, hc_mult=4):
    ...
    hc_mult3 = 2 * hc_mult + hc_mult * hc_mult
    fn = torch.randn((hc_mult3, hc_mult * hidden_size), dtype=torch.float32, device=DEVICE) * 1e-4
...
def test_mhc_pre_rocm_fallback_applies_norm(monkeypatch):
    ...
    expected_layer_input = F.rms_norm(ref[2], (ref[2].shape[-1],), norm_weight, norm_eps)
...
def test_mhc_fused_rocm_fallback_applies_norm(monkeypatch):
    ...
    expected_layer_input = F.rms_norm(pre_ref[2], (pre_ref[2].shape[-1],), norm_weight, norm_eps)
...
def test_mhc_pre_rocm_aiter_fuses_norm():
    set_random_seed(0)
    residual, fn, hc_scale, hc_base, norm_weight = _rocm_mhc_inputs(num_tokens=2, hidden_size=7168)
    rms_eps = hc_pre_eps = hc_sinkhorn_eps = norm_eps = 1e-6
    sinkhorn_repeat = 20
    hc_post_alpha = 1.0
    ref = mhc_pre_ref(
...
@pytest.mark.skipif(
    not (current_platform.is_rocm() and HAS_AITER_MHC_FUSED and HAS_AITER_MHC_FUSED_NORM),
...
def test_hc_head_triton(num_tokens, hidden_size, hc_mult):
    ...
    result = torch.ops.aphrodite.hc_head_triton(
        residual,
        fn,
        hc_scale,
        hc_base,
...
def test_hc_head_tilelang(num_tokens, hidden_size, hc_mult):
    ...
    out = torch.ops.aphrodite.hc_head_fused_kernel_tilelang(
        residual,
        fn,
        hc_scale,
        hc_base,
...
def _patch_first_rank_pp_group(monkeypatch):
    monkeypatch.setattr(
        "aphrodite.models.deepseek_v4.nvidia.model.get_pp_group",
