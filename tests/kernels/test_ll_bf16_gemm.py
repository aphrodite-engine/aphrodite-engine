...
def _assert_close(out, ref, *, min_cos_sim=0.99, context=""):
    ...
    assert cos > min_cos_sim, (
        f"{context}: cos_sim {cos:.6f} < {min_cos_sim} (abs_err={(out.float() - ref.float()).abs().max().item():.2e})"
    )
...
def _gemm(a, b):
    ...
    if _can_precompile(a, b):
        compile_key = ll_bf16_gemm_kernel.dispatch(M=a.shape[0], K=a.shape[1], N=b.shape[0])
...
@pytest.mark.parametrize("M,K", [(1, 4096), (4, 4096), (5, 4096), (8, 4096), (16, 4096)])
...
def test_cudagraph_20x_replay():
    ...
    for i in range(1, len(results)):
        torch.testing.assert_close(results[0], results[i], atol=0, rtol=0, msg=f"Replay {i} differs")
...
def _make_gate_linear(monkeypatch, *, params_dtype, out_dtype=torch.float32):
    ...
    monkeypatch.setattr(
        "aphrodite.model_executor.kernels.linear.cute_dsl.ll_bf16.is_available",
        lambda: True,
    )
    return GateLinear(
        input_size=2048,
...
def test_gate_linear_uses_ll_bf16_for_bf16_fast_path(monkeypatch):
    ...
    monkeypatch.setattr(
        "aphrodite.model_executor.kernels.linear.cute_dsl.ll_bf16.ll_bf16_gemm",
        fake_ll_bf16_gemm,
    )
    out, bias = gate(x)
    assert bias is None
...
def test_invalid_output_dtype():
    ...
    with pytest.raises(ValueError, match="output_dtype=torch.float32"):
        from aphrodite.model_executor.kernels.linear.cute_dsl.ll_bf16 import ll_bf16_gemm

        ll_bf16_gemm(a, b, output_dtype=torch.bfloat16)


def test_cache_miss_compiles_dotprod():
    from aphrodite.model_executor.kernels.linear.cute_dsl.ll_bf16 import LLBf16Gemm
