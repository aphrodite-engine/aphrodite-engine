...
from aphrodite.models.deepseek_v4.common.ops.fused_compress_quant_cache import (
    _fused_kv_compress_norm_rope_insert_indexer_attn,
    _fused_kv_compress_norm_rope_insert_indexer_mxfp4_attn,
    _launch_two_stage_sparse_attn_compressor,
    compress_norm_rope_store_triton,
)
from aphrodite.models.deepseek_v4.compressor import _get_c128_boundary
...
def test_compute_global_topk_reuses_output_buffers():
    device = "cuda"
    topk_indices = torch.tensor([[0, 3, -1], [1, 2, -1]], dtype=torch.int32, device=device)
...
def test_get_dspark_swa_index_width(window_size: int, num_speculative_tokens: int, expected: int):
...
def _decode_dsv4_cache_row(cache: torch.Tensor, block_size: int, scrub_nan: bool) -> torch.Tensor:
    flat = cache.flatten()
...
def _assert_nan_free_cache_matches_legacy_scrub(cache: torch.Tensor, block_size: int) -> None:
    flat = cache.flatten()
...
def test_indexer_gather_accepts_upper_bound_output():
    ...
    kv_cache = torch.zeros(num_blocks, block_size, cache_stride, dtype=torch.uint8, device=device)
    slot_mapping = torch.tensor([0, 1, 2, 16, 17, 18, 32, 33, 34], dtype=torch.int64, device=device)
    ops.indexer_k_quant_and_cache(k, kv_cache, slot_mapping, quant_block_size, "ue8m0")

    block_table = torch.arange(num_blocks, dtype=torch.int32, device=device).unsqueeze(1)
    ...
    if current_platform.is_rocm():
        ...
        token_to_seq = torch.cat(
            (
                torch.repeat_interleave(torch.arange(num_seqs, dtype=torch.int32, device=device), 3),
                torch.full(
    ...
    if current_platform.is_rocm():
        triton_recovered = triton_dst_k[:valid_tokens].view(current_platform.fp8_dtype()).float() * triton_dst_scale[
            :valid_tokens
        ].view(torch.float32)
        triton_error = (triton_recovered - k.float()).abs().amax(dim=1)
        max_triton_error = 16.0 * triton_dst_scale[:valid_tokens].view(torch.float32).flatten()
...
def test_cutedsl_full_cache_store(compress_ratio: int, store_fp8: bool):
    ...
    from aphrodite.models.deepseek_v4.nvidia.ops.sparse_attn_compress_cutedsl import (
...
def test_fused_kv_insert_split(num_tokens: int, kv_block_size: int):
    ...
    kv_n_blocks = (num_tokens + kv_block_size - 1) // kv_block_size + 1
    kv_cache = torch.zeros(kv_n_blocks, kv_block_size, HEAD_BYTES, dtype=torch.uint8, device=device)
    compress_scratch = torch.empty(num_tokens, HEAD_DIM, dtype=torch.float32, device=device)
    ...
    out = torch.zeros(1, num_tokens, HEAD_DIM, dtype=torch.bfloat16, device=device)
    seq_lens = torch.tensor([num_tokens], dtype=torch.int32, device=device)
    gather_block_table = torch.arange(kv_n_blocks, dtype=torch.int32, device=device).unsqueeze(0)
    dequantize_and_gather_k_cache(out, kv_cache, seq_lens, None, gather_block_table, kv_block_size, offset=0)
    recovered = out[0, :num_tokens]
