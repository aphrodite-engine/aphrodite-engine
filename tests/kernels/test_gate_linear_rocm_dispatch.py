...
import aphrodite.model_executor.layers.fused_moe.router.gate_linear as gate_linear_mod
...
def _make_gate(
    ...
    *,
    ...
    for target in (
        "aphrodite.model_executor.layers.linear",
        "aphrodite.model_executor.parameter",
    ):
        monkeypatch.setattr(
    ...
    if is_rocm:
        import aphrodite.platforms.rocm as rocm_platform
...
def test_rocm_gfx950_enables_fp32_router_gemm(monkeypatch, input_size: int, output_size: int) -> None:
    gate = _make_gate(
        monkeypatch,
        is_rocm=True,
...
def test_rocm_fp32_router_gemm_is_replicated_across_ep_sizes(monkeypatch, ep_size: int) -> None:
    gate = _make_gate(
        monkeypatch,
        is_rocm=True,
        params_dtype=torch.float32,
