...
from aphrodite._custom_ops import fp32_router_gemm
    ...
    for seed in range(5):
        ...
        for t in range(num_tokens):
            ...
            for e in got.symmetric_difference(want):
                ...
                assert gap < 1e-3, f"top-{top_k} mismatch beyond tie tolerance: token {t}, expert {e}, gap {gap:.3e}"
...
def test_rejects_invalid_inputs():
    ...
    with pytest.raises(Exception, match="num_tokens"):
        fp32_router_gemm(torch.randn(33, hidden_dim, dtype=torch.float32, device=device), mat_b)
    ...
    with pytest.raises(Exception, match="float32 or bfloat16"):
        fp32_router_gemm(torch.randn(4, hidden_dim, dtype=torch.float16, device=device), mat_b)
