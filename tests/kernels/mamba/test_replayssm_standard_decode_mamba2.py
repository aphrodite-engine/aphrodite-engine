...
from aphrodite.model_executor.layers.mamba.ops.mamba_ssm import selective_state_update
...
def _run_standard_decode(
    *,
    ...
    state0 = torch.randn(batch, nheads, headdim, dstate, dtype=state_dtype, device=device)
    ...
    bc_pre = torch.empty(batch, ngroups, max_cache_len, device=device, dtype=torch.float32)

    if desync_write_pos:
        # Rows start at different ring positions so they flush on different
        # steps, exercising per-row write-position handling.
        write_pos = torch.arange(batch, device=device, dtype=torch.int32) % max_cache_len
    else:
    ...
    for _ in range(num_steps):
        ...
        selective_state_update_replayssm_output_only(state_cached, x, dt, A, B, C, bc_pre=bc_pre, **common)
        ...
        if bool(is_flush.any()):
            torch.testing.assert_close(state_cached[is_flush], state_ref[is_flush], rtol=rtol, atol=atol)
            if state_dtype == torch.float32:
...
def test_replayssm_standard_decode_with_batch_indices(
    ...
    state = torch.randn(total_state_slots, nheads, headdim, dstate, dtype=state_dtype, device=device)
    state_baseline = state.clone()
    state_cached = state.clone()
    state_before = state.clone()

    state_indices = (torch.randperm(total_state_slots - 1, device=device)[:batch] + 1).to(torch.int32)
    ...
    dt_cache = torch.zeros(total_state_slots, nheads, max_cache_len, device=device, dtype=torch.float32)
    B_cache = torch.zeros(
        total_state_slots,
        ngroups,
        max_cache_len,
    ...
    bc_pre = torch.empty(padded_batch, ngroups, max_cache_len, device=device, dtype=torch.float32)
    ...
    for _ in range(num_steps):
        ...
        torch.testing.assert_close(out_cached[:batch], out_baseline[:batch], rtol=rtol, atol=atol)
        if with_padding:
            assert torch.equal(out_cached[batch:], torch.full_like(out_cached[batch:], 42))
...
def test_replayssm_standard_decode_tp_head_shard_equivalence(
    ...
    state_full = torch.randn(batch, nheads, headdim, dstate, dtype=state_dtype, device=device)
    x_cache, dt_cache, B_cache, _ = allocate_update_caches(
        batch,
        nheads,
        ngroups,
    ...
    for r in range(tp):
        ...
        bcp = torch.empty(batch, ng_s, max_cache_len, device=device, dtype=torch.float32)
    ...
    for _ in range(num_steps):
        ...
        for r in range(tp):
            ...
            out_shard = torch.empty(batch, nh_s, headdim, device=device, dtype=act_dtype)
            selective_state_update_replayssm_output_only(
                shard_state[r],
            ...
            torch.testing.assert_close(out_full[:, h0 : h0 + nh_s], out_shard, rtol=rtol, atol=atol)
            if bool(is_flush.any()):
                torch.testing.assert_close(
