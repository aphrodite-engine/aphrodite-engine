...
from aphrodite.config import SpeculativeConfig, set_current_aphrodite_config  # noqa: E402
...
from aphrodite.third_party.flash_linear_attention.ops.layernorm_guard import (  # noqa: E402
    rmsnorm_fn,
)
from aphrodite.utils.torch_utils import _encode_layer_name  # noqa: E402
from aphrodite.v1.attention.backends.gdn_attn import (  # noqa: E402
    GDNAttentionMetadata,
    GDNAttentionMetadataBuilder,
)
from aphrodite.v1.kv_cache_interface import MambaSpec  # noqa: E402
...
def _make_aphrodite_config():
    ...
    config.speculative_config = SpeculativeConfig(method="ngram", num_speculative_tokens=NUM_SPEC)
    return config
...
def test_fused_mtp_head_ratio_guard(num_v_heads: int, expected: bool) -> None:
    ...
    attn_metadata = types.SimpleNamespace(
        spec_state_indices_tensor=torch.ones(1, SPEC_TOKENS, dtype=torch.int32, device="cuda"),
...
def test_fused_forward_uses_packed_entrypoint() -> None:
    ...
    mixed_qkvz = torch.randn(num_tokens, CONV_DIM + HV * V, dtype=torch.bfloat16, device=device)
    ba = torch.randn(num_tokens, 2 * HV, dtype=torch.bfloat16, device=device)
    layer = types.SimpleNamespace(
        prefix=PREFIX,
        enable_fused_gdn_decode=True,
        norm=types.SimpleNamespace(weight=torch.empty(V, dtype=torch.bfloat16, device=device)),
        num_v_heads=HV,
        tp_size=1,
        head_v_dim=V,
    ...
    layer.forward_cuda = types.MethodType(QwenGatedDeltaNetAttention.forward_cuda, layer)
    ...
    with (
        ...
        patch.object(torch.ops.aphrodite, "qwen_gdn_attention_core") as triton_mock,
    ):
...
def test_fused_model_path_matches_reference(
    ...
    conv_state_shape, temporal_state_shape = MambaStateShapeCalculator.gated_delta_net_state_shape(
        1, H, HV, K, V, CONV_KERNEL, NUM_SPEC
    )
    conv_state_seed = 0.05 * torch.randn(pool_size, *conv_state_shape, dtype=torch.bfloat16, device=device)
    ssm_state_seed = 0.01 * torch.randn(pool_size, *temporal_state_shape, dtype=torch.float32, device=device)
    a_log = 0.1 * torch.randn(HV, dtype=torch.float32, device=device)
    dt_bias = 0.1 * torch.randn(HV, dtype=torch.float32, device=device)
    conv_weight = 0.1 * torch.randn(CONV_DIM, 1, CONV_KERNEL, dtype=torch.bfloat16, device=device)
    norm_weight = torch.randn(V, dtype=torch.float32, device=device)
    num_tokens = batch.compute_num_tokens()
    mixed_qkv = 0.1 * torch.randn(num_tokens, CONV_DIM, dtype=torch.bfloat16, device=device)
    b = 0.1 * torch.randn(num_tokens, HV, dtype=torch.bfloat16, device=device)
    a = 0.1 * torch.randn_like(b)
    output_gate = 0.1 * torch.randn(num_tokens, HV, V, dtype=torch.bfloat16, device=device)
    ...
    with patch.object(qwen_gdn_linear_attn, "get_forward_context", return_value=context):
    ...
    torch.testing.assert_close(fused_layer.kv_cache[0], reference_layer.kv_cache[0], atol=0, rtol=0)
