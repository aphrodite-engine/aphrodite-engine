...
from aphrodite.model_executor.layers.mamba.ops.cpu import gdn_attention
...
# decode path
@pytest.mark.parametrize("batch_size,num_heads,head_dims,state_dtype", DECODE_DTYPE_CASES)
@torch.inference_mode()
def test_fused_sigmoid_gating_delta_rule_update_cpu(
    ...
    state = tensor_cache(batch_size * num_v_heads * head_dim * v_head_dim, state_dtype).view(state_shape)
...
def test_chunk_gated_delta_rule_cpu(
    ...
    initial_state = tensor_cache(len(seq_lens) * num_v_heads * head_dim * v_head_dim, state_dtype).view(
        initial_state_shape
    )
...
def _sd_conv_states(num_slots: int, state_len: int, dim: int = CONV_DIM) -> torch.Tensor:
...
def test_spec_aware_mixed_routing_preserves_token_order(
    ...
    layer = types.SimpleNamespace(kv_cache=[torch.empty(1, 4, 6), torch.empty(1, 1, 1, 1)])
...
def test_spec_aware_nonspec_materializes_state_indices(
    ...
    monkeypatch.setattr(gdn_attention.ops, "causal_conv1d_fwd_cpu", causal_conv1d_fwd_cpu)
    ...
    torch.testing.assert_close(recorded_indices, torch.tensor([0, 4], dtype=torch.int32))
...
def test_spec_forward_prepares_native_conv_metadata(
    ...
    monkeypatch.setattr(torch.cpu, "_is_amx_tile_supported", lambda: True)
    monkeypatch.setattr(gdn_attention, "is_conv_state_dim_first", lambda: False)
    monkeypatch.setattr(gdn_attention.ops, "causal_conv1d_update_cpu", causal_conv1d_update_cpu)
    monkeypatch.setattr(
        gdn_attention.ops,
...
def test_causal_conv1d_update_cpu_accepts_wide_state() -> None:
    ...
    narrow_state.copy_(tensor_cache(narrow_state.numel(), torch.bfloat16).view_as(narrow_state))
    ...
    torch.testing.assert_close(wide_state[:, :, :state_len], narrow_state, atol=0, rtol=0)
...
def test_causal_conv1d_update_cpu_multi_token_matches_python(
    ...
    dim = 96
    state_len = seq_len + 2
    x = tensor_cache(batch_size * seq_len * dim, torch.bfloat16).view(batch_size, seq_len, dim)
    ...
    conv_states_ref.copy_(tensor_cache(conv_states_ref.numel(), torch.bfloat16).view_as(conv_states_ref))
...
def test_batch_memcpy_cpu_fallback() -> None:
    ...
    """
    from aphrodite.utils.cpu_triton_utils import batch_memcpy_kernel
...
def _run_prefill_torch(x, weight, bias, seq_lens):
    ...
    qsl = torch.tensor([0, *torch.tensor(seq_lens).cumsum(0).tolist()], dtype=torch.int32)
...
def _run_prefill_cpp(x, weight, bias, seq_lens, is_vnni=False):
    ...
    if is_vnni:
        ...
        conv_state = torch.zeros(num_seqs, _STATE_LEN, CONV_DIM, dtype=x.dtype).transpose(1, 2)
    else:
...
@pytest.mark.skipif(not _HAS_AVX512_BF16, reason="C++ causal_conv1d requires AVX-512BF16")
...
def test_conv_cpp_no_worse_than_torch_vs_fp32(seq_lens):
    ...
    assert err_cpp <= err_torch + 1e-3, (
        f"C++ conv less accurate than torch: err_cpp={err_cpp:.2e} err_torch={err_torch:.2e}"
    )
...
def test_conv_update_cpp_matches_torch(batch):
    ...
    weight = tensor_cache(CONV_DIM * CONV_KERNEL, torch.bfloat16).view(CONV_DIM, CONV_KERNEL)
    bias = tensor_cache(CONV_DIM, torch.bfloat16)
    conv_state = tensor_cache(batch * CONV_DIM * _STATE_LEN, torch.bfloat16).view(batch, CONV_DIM, _STATE_LEN)
...
def test_spec_decode_unpacked_conv_weight_stash():
    ...
    conv.weight = torch.nn.Parameter(orig_2d.view(CONV_DIM, 1, CONV_KERNEL).clone(), requires_grad=False)
    ...
    assert hasattr(conv, "_cpu_unpacked_conv_weight"), (
        "dispatch_cpu_unquantized_gemm did not stash _cpu_unpacked_conv_weight on an AVX-512BF16 CPU"
    )
