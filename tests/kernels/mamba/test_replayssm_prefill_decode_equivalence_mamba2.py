...
dt_bias, applied inside each kernel) to:
  ...
  * the ReplaySSM output_only decode kernel,
...
def _run_prefill_decode_equivalence(
    *,
    ...
    cu_chunk_seqlens, last_chunk_indices, seq_idx = compute_varlen_chunk_metadata(cu_seqlens, chunk_size)
    ...
    state_ref = torch.zeros(1, nheads, headdim, dstate, device=device, dtype=torch.float32)
    state_base = torch.zeros(1, nheads, headdim, dstate, device=device, dtype=state_dtype)
    state_dec = torch.zeros(1, nheads, headdim, dstate, device=device, dtype=state_dtype)
    x_cache = torch.zeros(1, nheads, max_cache_len, headdim, device=device, dtype=act_dtype)
    dt_cache = torch.zeros(1, nheads, max_cache_len, device=device, dtype=torch.float32)
    B_cache = torch.zeros(1, ngroups, max_cache_len, dstate, device=device, dtype=act_dtype)
    bc_pre = torch.empty(1, ngroups, max_cache_len, device=device, dtype=torch.float32)
    ...
    torch.testing.assert_close(final_state_prefill[0].float(), state_ref[0], rtol=rtol, atol=atol)
