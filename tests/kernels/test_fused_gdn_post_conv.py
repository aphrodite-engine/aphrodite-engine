...
def test_fused_gdn_decode_post_conv_mtp_head_ratios(
    ...
    state_indices = torch.arange(1, num_slots, dtype=torch.int32, device=device).view(num_reqs, state_width)
    ...
    for step, accepted_tokens in enumerate((1, min(2, state_width), state_width)):
        ...
        assert output_relative_l2 < 5e-4, (
            f"MTP output relative L2 mismatch at step {step}: {output_relative_l2.item():.6g}"
        )
