...
class _TestTritonKernel(AphroditeTritonJitKernel["_TestTritonKernel.CompileKey"]):
...
def test_triton_launcher_supports_compile_and_runtime_adapters() -> None:
    ...
    assert owner.kernel.warmup_calls == [{"grid": (1,), "first": "warmup", "second": 1, "CONST": 7}]
...
def test_compute_slot_mapping_uses_named_launcher_inputs(monkeypatch) -> None:
    ...
    from aphrodite.v1.worker.block_table import ComputeSlotMappingKernel
...
def test_compute_slot_mapping_supports_cpu_adapter(monkeypatch) -> None:
    from types import SimpleNamespace

    from aphrodite.utils import cpu_triton_utils
    from aphrodite.v1.worker.block_table import ComputeSlotMappingKernel
    ...
    monkeypatch.setattr(
        ...
        SimpleNamespace(
            ops=SimpleNamespace(
                _C=SimpleNamespace(compute_slot_mapping_kernel_impl=lambda *args: calls.append(args)),
            ),
        ),
    )
    owner = ComputeSlotMappingKernel()
    owner.kernel = cpu_triton_utils.compute_slot_mapping_kernel
    owner(
        num_reqs=1,
        num_tokens=2,
        max_num_tokens=8,
    ...
    assert calls == [("query", "positions", "blocks", "slots", 16)]
