...
def _sleep_test_hf_overrides(hf_config, *, model_arch: str):
    ...
    if model_arch == "Ernie4_5_VLMoeForConditionalGeneration" and getattr(hf_config, "im_patch_id", None) is None:
...
def _get_sleep_tensor_targets(
    ...
    if model_name == "fireredasr2":
        from aphrodite.model_executor.models.conformer_encoder import (
            RelPositionalEncoding,
        )

        return ((RelPositionalEncoding, ("pe",)),)
    if model_name == "gemma3n":
        from aphrodite.model_executor.models.gemma3n import (
            Gemma3nAltUp,
            Gemma3nSelfDecoder,
        )
    ...
    if model_name == "voxtral":
        from aphrodite.model_executor.models.voxtral import VoxtralEncoderModel

        return ((VoxtralEncoderModel, ("mel_filters",)),)
    if model_name == "ernie45_vl":
        from aphrodite.model_executor.models.ernie45_vl import (
...
def _snapshot_sleep_tensors(
    ...
    *,
    ...
    for module_path, module in model.named_modules():
        for module_type, target_names in targets:
            ...
            for tensor_name in target_names:
                ...
                qualified_name = f"{module_path}.{tensor_name}" if module_path else tensor_name
    ...
    assert found_tensor_names == set(tensor_names), (
        f"{model_name}: expected tensors {tensor_names}, found {tuple(sorted(found_tensor_names))}"
    )
    return snapshot
...
def _generation_signature(output) -> tuple[tuple[int, ...], tuple[float, ...]]:
    ...
    chosen_logprobs = tuple(
        step[token_id].logprob for token_id, step in zip(token_ids, completion.logprobs, strict=True)
    )
...
def test_static_model_tensors_survive_level2_restore(
    ...
    if case.name == "fireredasr2":
        ...
        from aphrodite.model_executor.models.fireredasr2 import (
    ...
    if case.model is not None:
        llm_kwargs["hf_overrides"] = partial(_sleep_test_hf_overrides, model_arch=case.architecture)
    if case.modalities:
        llm_kwargs["limit_mm_per_prompt"] = {modality: 0 for modality in case.modalities}
    ...
    assert after_generation[1] == pytest.approx(before_generation[1], rel=1e-5, abs=1e-5), case.name
