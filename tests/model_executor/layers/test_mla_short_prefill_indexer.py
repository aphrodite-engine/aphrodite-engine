...
import aphrodite.model_executor.layers.sparse_attn_indexer as sparse_indexer
from aphrodite.config import CUDAGraphMode
from aphrodite.models.deepseek_v32 import attention as deepseek_v32_attention
...
def test_short_prefill_updates_k_cache_before_scoring_decision(
    ...
    runtime_mode = CUDAGraphMode.FULL if batch_kind == "full" else CUDAGraphMode.PIECEWISE
    ...
    monkeypatch.setattr(sparse_indexer.current_platform, "fp8_dtype", lambda: torch.float16)
    monkeypatch.setattr(
        torch.cuda,
    ...
    monkeypatch.setattr(sparse_indexer.ops, "indexer_k_quant_and_cache", record_cache_update)
    ...
    monkeypatch.setattr(sparse_indexer, "kv_cache_as_quant_view", scoring_decode)
