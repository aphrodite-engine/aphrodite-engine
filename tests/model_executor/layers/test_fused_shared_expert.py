...
import aphrodite.config as aphrodite_config_module
from aphrodite.config import AphroditeConfig, set_current_aphrodite_config
from aphrodite.model_executor.layers.fused_moe import utils as fused_moe_utils
from aphrodite.model_executor.layers.fused_moe.layer import determine_expert_counts
from aphrodite.model_executor.layers.quantization.fp8 import Fp8Config
from aphrodite.model_executor.layers.quantization.quark.quark import QuarkConfig
...
from aphrodite.model_executor.models.qwen3_next import (
    _should_replicate_misaligned_shared_expert,
)
from aphrodite.model_executor.models.utils import PPMissingLayer
from aphrodite.models.deepseek_v4 import quant_config as deepseek_v4_quant_config
from aphrodite.models.minimax_m3.amd import model as minimax_m3_model
from aphrodite.platforms import current_platform
from aphrodite.transformers_utils.configs.minimax_m3 import MiniMaxM3TextConfig
from aphrodite.transformers_utils.configs.qwen3_5_moe import Qwen3_5MoeTextConfig
...
def get_deepseek_v4_quark_config(exclude: list[str]) -> dict[str, Any]:
    ...
    mxfp4_config = cast(dict[str, Any], deepcopy(quantization_config["global_quant_config"]))
...
def get_fse_test_model_config(
    ...
    if model_type == "deepseek_v4":
        from aphrodite.models.deepseek_v4.amd.model import DeepseekV4Model
    ...
    if model_type == "qwen3_5":
        from aphrodite.model_executor.models.qwen3_5 import Qwen3_5Model
    ...
    if model_type == "deepseek_v2":
        ...
        from aphrodite.model_executor.models.deepseek_v2 import DeepseekV2Model
    ...
    if model_type == "glm4_moe":
        ...
        from aphrodite.model_executor.models.glm4_moe import Glm4MoeModel
...
def test_resolve_layer_fused_shared_expert_skips_compatibility_when_disabled(
    ...
    assert not fused_moe_utils.resolve_layer_fused_shared_expert(object(), "model.layers.0.mlp")
...
def test_resolve_layer_fused_shared_expert_normalizes_unavailable_aiter(
    ...
    assert fused_moe_utils.resolve_layer_fused_shared_expert(object(), "model.layers.0.mlp") is False
...
def test_resolve_layer_fused_shared_expert_passes_module_prefixes(
    ...
    def check_compatibility(config: object, expert_prefix: str, shared_expert_prefix: str) -> tuple[bool, str | None]:
        calls.append((config, expert_prefix, shared_expert_prefix))
        return True, None

    monkeypatch.setattr(fused_moe_utils, "is_shared_expert_quant_fse_compatible", check_compatibility)
...
def test_deepseek_v4_shared_expert_fse_uses_mtp_quantization_config_prefix(
    ...
    hf_config = SimpleNamespace(
        num_hidden_layers=2,
        quantization_config={
            "layer_quant_config": {r"re:mtp\.0\.ffn\.shared_experts\.w1": {"weight": {"dtype": "fp4"}}},
            "global_quant_config": {"weight": {"dtype": "fp8"}},
        },
    )
    monkeypatch.setattr(deepseek_v4_quant_config, "DeepseekV4FP8Config", DeepseekV4Config)
    monkeypatch.setattr(
...
def test_deepseek_v4_heterogeneous_fhmoe_keeps_native_intermediate_width() -> None:
    from aphrodite.models.deepseek_v4.amd.model import _prepare_native_fp8_shared_expert
    ...
    w13_scale_bytes = (torch.arange(6 * 56).reshape(6, 56).remainder(20) + 0x60).to(torch.uint8)
    w2_scale_bytes = (torch.arange(56 * 3).reshape(56, 3).remainder(30) + 0x50).to(torch.uint8)
    w13_scale = w13_scale_bytes.view(torch.float8_e8m0fnu)
    w2_scale = w2_scale_bytes.view(torch.float8_e8m0fnu)

    prepared = _prepare_native_fp8_shared_expert(w13, w2, w13_scale, w2_scale, intermediate_size)
    ...
    expected_w13_scale = w13_scale_bytes.repeat_interleave(128, dim=0).repeat_interleave(4, dim=1)
    expected_w2_scale = w2_scale_bytes.repeat_interleave(128, dim=0).repeat_interleave(4, dim=1)
...
def test_deepseek_v4_heterogeneous_fhmoe_token_policy(
    ...
    from aphrodite.models.deepseek_v4.amd import model as deepseek_v4_model
...
def test_deepseek_v4_heterogeneous_fhmoe_aiter_capability(
    ...
    assert rocm_aiter_ops._probe_dsv4_i384_fhmoe_capability(cast(int, num_tokens)) is expected
...
def test_deepseek_v4_heterogeneous_fhmoe_aiter_capability_catches_signature_error(
    ...
    _install_fake_aiter_fhmoe(monkeypatch, _supports_fhmoe_through_2048, _supported_fhmoe_signature)
...
def test_deepseek_v4_heterogeneous_fhmoe_compatibility_gates(
    ...
    monkeypatch.setattr(
        ...
        "APHRODITE_ROCM_USE_AITER_FUSION_SHARED_EXPERTS",
        True,
    )
    ...
    assert deepseek_v4_model._heterogeneous_shared_expert_enabled(cast(AphroditeConfig, aphrodite_config)) is expected
...
def test_deepseek_v4_heterogeneous_fhmoe_rejects_invalid_routes(
    ...
    from aphrodite.models.deepseek_v4.amd.model import _validate_heterogeneous_routes
...
def test_aiter_fused_moe_validates_shared_expert_arguments() -> None:
    from aphrodite._aiter_ops import (
...
def test_is_model_fused_shared_expert_compatible() -> None:
    ...
    assert fused_moe_utils.is_model_fused_shared_expert_compatible(enabled_layers, MoE, "mlp")
    assert not fused_moe_utils.is_model_fused_shared_expert_compatible(disabled_layers, MoE, "mlp")
    assert not fused_moe_utils.is_model_fused_shared_expert_compatible(empty_layers, MoE, "mlp")
    assert fused_moe_utils.is_model_fused_shared_expert_compatible(pipeline_layers, MoE, "mlp")
    with pytest.raises(NotImplementedError, match="1 enabled and 1 disabled layers"):
        fused_moe_utils.is_model_fused_shared_expert_compatible(mixed_layers, MoE, "mlp")
...
def test_models_fse_init(
    ...
    config, model_constructor = get_fse_test_model_config(model_type, quantization_config)
    ...
    aphrodite_config.parallel_config.enable_expert_parallel = False
    if model_type == "deepseek_v4":
        from aphrodite.models.deepseek_v4.quant_config import DeepseekV4FP8Config

        aphrodite_config.cache_config.cache_dtype = "fp8_ds_mla"
        aphrodite_config.quant_config = DeepseekV4FP8Config(
            is_checkpoint_fp8_serialized=True,
            weight_block_size=[128, 128],
        )
    else:
        aphrodite_config.quant_config = QuarkConfig(quantization_config)
    ...
    with monkeypatch.context() as mp:
        mp.setenv("APHRODITE_ROCM_USE_AITER", str(use_fse))
        mp.setenv("APHRODITE_ROCM_USE_AITER_MOE", str(use_fse))
        mp.setenv("APHRODITE_ROCM_USE_AITER_FUSION_SHARED_EXPERTS", str(use_fse))
        mp.setattr(
            "aphrodite.model_executor.layers.fused_moe.experts.ocp_mx_emulation_moe.has_quark",
            lambda: True,
        )
        importlib.reload(envs)
        mp.setattr(envs, "APHRODITE_ROCM_USE_AITER_FUSION_SHARED_EXPERTS", use_fse)
        ...
        fse_enabled = use_fse and (aiter_fse_enabled or model_type in {"deepseek_v4", "minimax_m3"})

        with (
            patch.object(warning_logger, "warning") as warning,
            set_current_aphrodite_config(aphrodite_config),
        ):
            model = model_constructor(aphrodite_config=aphrodite_config)
            mtp = None
            if model_type == "deepseek_v4" and use_fse and not exclude:
                from aphrodite.models.deepseek_v4.amd.mtp import (
                    DeepSeekV4MTP,
                )

                aphrodite_config.speculative_config = SimpleNamespace(
                ...
                mtp = DeepSeekV4MTP(aphrodite_config=aphrodite_config)
        ...
        if mtp is not None:
            assert not mtp.model.layers["1"].mtp_block.ffn.is_fused_shared_expert_enabled
            warning.assert_called_once()
            assert "DeepSeek-V4 shared experts at mtp.0.ffn.shared_experts" in (warning.call_args.args[1])
        if aiter_fse_enabled and exclude:
            warning.assert_called_once()
            assert "APHRODITE_ROCM_USE_AITER_FUSION_SHARED_EXPERTS is enabled" in (warning.call_args.args[0])
...
def test_quark_shared_expert_fse_compatibility(exclude: list[str], expected: bool) -> None:
    compatible, reason = is_shared_expert_quant_fse_compatible(
        QuarkConfig(
            {
                "exclude": exclude,
...
def test_quark_shared_expert_fse_exclude_is_scoped_to_the_layer() -> None:
    ...
    assert reason == "Quark excludes shared experts at model.layers.78.mlp.shared_experts"
...
def test_quark_shared_expert_fse_rejects_partially_excluded_packed_projection() -> None:
    ...
    with pytest.raises(ValueError, match="different quantization schemes"):
        is_shared_expert_quant_fse_compatible(quant_config, "model.layers.0.mlp.experts", shared_expert_prefix)
...
def test_quark_shared_expert_fse_ignores_sibling_gate_exclusions() -> None:
    ...
    assert is_shared_expert_quant_fse_compatible(quant_config, expert_prefix, shared_expert_prefix) == (True, None)
    ...
    compatible, reason = is_shared_expert_quant_fse_compatible(quant_config, expert_prefix, shared_expert_prefix)
    assert compatible is False
...
def test_quark_ocp_mx_group_size_for_layer() -> None:
    ...
    quant_config.quant_config["exclude"] = ["model.layers.0.mlp.shared_expert.down_proj"]
    assert (
        get_quark_ocp_mx_group_size(
            quant_config,
            "model.layers.0.mlp.shared_expert.down_proj",
    ...
    quant_config = QuarkConfig(
        {
            ...
            "layer_quant_config": {
                "model.layers.0.mlp.shared_expert.gate_up_proj": {"weight": {"dtype": "fp8"}},
                "model.layers.0.mlp.shared_expert.down_proj": {"weight": {"dtype": "fp8"}},
            },
        }
    )
...
def test_quark_layer_config_from_name_checks_packed_projections() -> None:
    ...
    with pytest.raises(ValueError, match="requires all to use the same scheme"):
        quant_config.get_layer_quant_config_from_name("model.layers.0.mlp.shared_expert.gate_up_proj")
...
def test_quark_packed_layer_config_must_match_global_config() -> None:
    ...
    with pytest.raises(ValueError, match="requires all to use the same scheme"):
        quant_config._find_matched_config("model.layers.0.mlp.shared_expert.gate_up_proj", nn.Module())
...
def test_non_quark_shared_expert_fse_is_incompatible() -> None:
    ...
    assert not compatible
    assert reason == ("shared-expert FSE quantization compatibility is not implemented for object")
...
def test_per_tensor_fp8_shared_expert_fse_is_incompatible() -> None:
    ...
    assert not compatible
    assert reason == ("FP8 shared-expert FSE is only implemented for block-quantized checkpoints")
...
def test_fp8_shared_expert_fse_rejects_asymmetric_ignored_layers(
    ...
    assert not compatible
    assert reason == ("FP8 ignores routed and shared experts inconsistently at model.layers.0.mlp.shared_experts")
...
def test_fp8_shared_expert_fse_propagates_partial_shard_exclusion() -> None:
    ...
    quant_config = _fp8_config(ignored_layers=["model.layers.0.mlp.shared_experts.gate_proj"])
    quant_config.packed_modules_mapping = {"gate_up_proj": ["gate_proj", "up_proj"]}
