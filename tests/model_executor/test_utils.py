...
from aphrodite.model_executor.parameter import ModelWeightParameter, PackedAphroditeParameter
from aphrodite.model_executor.utils import replace_parameter


@pytest.fixture
def single_rank_tp(monkeypatch: pytest.MonkeyPatch) -> None:
    """`BaseAphroditeParameter.__init__` queries the TP group, which is not
    initialized in a unit test. Pin it to a single rank.
    """
    monkeypatch.setattr("aphrodite.model_executor.parameter.get_tensor_model_parallel_rank", lambda: 0)
    monkeypatch.setattr("aphrodite.model_executor.parameter.get_tensor_model_parallel_world_size", lambda: 1)


@pytest.mark.parametrize("prefer_copy", [False, True])
@pytest.mark.parametrize("wrap_in_parameter", [False, True])
def test_replace_parameter_preserves_custom_attribute(prefer_copy: bool, wrap_in_parameter: bool) -> None:
    ...
    layer.register_parameter("weight", torch.nn.Parameter(torch.zeros(4, 4), requires_grad=False))
...
def test_replace_parameter_weight_loader_comes_from_old_parameter() -> None:
    ...
    of `BaseAphroditeParameter.weight_loader`, so leaving it in would smuggle a
...
def test_replace_parameter_attributes_from_the_layers_own_parameter(single_rank_tp: None, param_kind: str) -> None:
    ...
    `BaseAphroditeParameter` subclass rather than a plain `Parameter`.
    ...
    For the Aphrodite parameter classes only the private backing fields come along.
    ...
    if param_kind == "plain":
        ...
        old_param = ModelWeightParameter(data=data, input_dim=1, output_dim=0, weight_loader=loader)
        source_attrs = dict(aphrodite_param_attrs)
    else:
        old_param = PackedAphroditeParameter(
            data=data,
            input_dim=1,
            output_dim=0,
            packed_dim=0,
    ...
    # Pin what the caller hands back: on the Aphrodite classes `weight_loader` is a
