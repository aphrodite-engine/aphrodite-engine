...
from aphrodite.model_executor.layers.fused_moe.config import RoutingMethodType
...
_REC_MODULE = "aphrodite.model_executor.layers.fused_moe.routed_experts_capturer"
...
def _capturer_with_buffer(
    *,
    ...
    # AphroditeConfig dependency. The CUDA device-tensor allocation in the
...
def _make_model_config(hf_config):
    num_experts_per_token = ModelArchConfigConvertorBase(hf_config, hf_config).get_num_experts_per_token()
    ...
    model_config.get_num_experts_per_tok = lambda: (ModelConfig.get_num_experts_per_tok(model_config))
...
def test_routed_experts_manager_uses_gemma4_top_k_experts():
    ...
    aphrodite_config = SimpleNamespace(model_config=_make_model_config(hf_config))
    ...
    manager = RoutedExpertsManager(aphrodite_config, kv_cache_config)
...
def test_public_binding_only_visits_target_model(monkeypatch):
    ...
    import aphrodite.model_executor.layers.fused_moe.layer as fused_moe_layer
    ...
    bind_routed_experts_capturer(types.SimpleNamespace(modules=lambda: [target_module]), capturer)
...
def test_public_binding_rejects_monolithic_without_replay_support(monkeypatch):
        ...
        def __init__(self):
            ...
            self.routed_experts = types.SimpleNamespace(
                quant_method=types.SimpleNamespace(
                    is_monolithic=True,
                    moe_kernel=types.SimpleNamespace(impl=types.SimpleNamespace(fused_experts=fused_experts)),
                )
            )
    ...
    with pytest.raises(ValueError, match="monolithic MoE kernel"):
        bind_routed_experts_capturer(types.SimpleNamespace(modules=lambda: [dummy_module]), DummyCapturer())
...
def test_routed_experts_attention_group_unwraps_uniform_type_specs():
    ...
    config = SimpleNamespace(
        kv_cache_groups=[
            KVCacheGroupSpec(
                ["swa_layer"],
                UniformTypeKVCacheSpecs(block_size=4, kv_cache_specs={"swa_layer": swa_spec}),
            ),
            KVCacheGroupSpec(
                ["mla_layer"],
                UniformTypeKVCacheSpecs(block_size=4, kv_cache_specs={"mla_layer": mla_spec}),
            ),
        ]
    )
...
def test_mrv2_async_output_returns_existing_routed_experts_field():
    from aphrodite.v1.outputs import ModelRunnerOutput, RoutedExpertsTensors
    from aphrodite.v1.worker.gpu.async_utils import AsyncOutput
...
def test_all_tp_ranks_initialize_capture(monkeypatch, rank):
    pytest.importorskip("aphrodite.vllm_flash_attn", exc_type=ImportError)
    import aphrodite.v1.worker.gpu.model_runner as model_runner
    ...
    runner.aphrodite_config = SimpleNamespace(parallel_config=SimpleNamespace(rank=rank))
...
def test_v2_model_runner_accepts_routed_experts(monkeypatch):
    ...
    unsupported = AphroditeConfig._get_v2_model_runner_unsupported_features(config)
