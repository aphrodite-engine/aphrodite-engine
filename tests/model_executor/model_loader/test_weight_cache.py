...
class WeightCacheDaemon:
    ...
    def __init__(self, model: str, tp_size: int):
        ...
        self.socket_dir = tempfile.mkdtemp(prefix="aphrodite_ipc_")
        self._cmd = [
            sys.executable,
            "-m",
            "aphrodite.model_executor.model_loader.weight_cache.daemon",
            "--model",
            model,
    ...
    def _wait_ready(self, timeout_s: float) -> None:
        ...
        while time.monotonic() < deadline:
            if self._proc.poll() is not None:
                raise RuntimeError(f"Weight cache daemon exited with {self._proc.returncode}:\n{self._logs()}")
            if self._READY_MARKER in self._logs():
                return
            time.sleep(1.0)
        raise TimeoutError(f"Weight cache daemon not ready after {timeout_s}s:\n{self._logs()}")
...
def generate(aphrodite_runner, socket_dir: str | None, fallback: bool, **kwargs):
    extra_config = {} if socket_dir is None else {"socket_dir": socket_dir, "fallback": fallback}
...
def test_ipc_cache_cold_start_and_warm_restart(aphrodite_runner):
    ...
    baseline_outputs = generate(aphrodite_runner, None, fallback=True, **llm_kwargs)
    ...
    with tempfile.TemporaryDirectory(prefix="aphrodite_ipc_empty_") as empty_socket_dir:
        cold_outputs = generate(aphrodite_runner, empty_socket_dir, fallback=True, **llm_kwargs)

    with WeightCacheDaemon(MODEL, tp_size=1) as daemon:
        warm_outputs = generate(aphrodite_runner, daemon.socket_dir, fallback=False, **llm_kwargs)
        # Warm restart: a second engine lifetime against the same daemon.
        restart_outputs = generate(aphrodite_runner, daemon.socket_dir, fallback=False, **llm_kwargs)
