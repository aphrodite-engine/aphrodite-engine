...
from aphrodite.model_executor.models.gpt_oss import (
    GptOssForCausalLM,
    GptOssModel,
    GptOssRoutedExperts,
)
...
class _TestExperts(GptOssRoutedExperts):
    def __init__(
        ...
        *,
        ...
        self.moe_config = SimpleNamespace(moe_parallel_config=SimpleNamespace(tp_size=2, tp_rank=tp_rank))
        self.quant_method = type(quant_method_name, (), {"weight_dtype": weight_dtype})()
...
def test_streamed_expert_resolves_wrapped_parameter(base_layer_prefix: str) -> None:
    ...
    mapped_name = GptOssForCausalLM.hf_to_aphrodite_mapper.apply_list([checkpoint_name])[0]
    fused_name = f"model.layers.2.mlp.experts.{base_layer_prefix}routed_experts.w13_weight"
    ...
    assert GptOssModel._try_load_streamed_expert(mapped_name, loaded_weight, params_dict, loaded_params)
