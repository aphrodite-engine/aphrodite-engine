...
def registered_paths(supported_tasks: tuple[SupportedTask, ...], *, tokens_only: bool = False) -> set[str]:
    app = FastAPI()
...
def test_disabled_scale_out_routes_are_logged(monkeypatch, caplog):
    ...
    with caplog.at_level("INFO", logger="aphrodite.entrypoints.scale_out.factories"):
        registered_paths(("generate",))

    assert any("APHRODITE_ENABLE_SCALE_OUT_ENDPOINTS=1" in record.message for record in caplog.records)
...
def test_render_server_rejects_explicitly_disabled_scale_out_routes(monkeypatch):
    ...
    with pytest.raises(ValueError, match="APHRODITE_ENABLE_SCALE_OUT_ENDPOINTS=0"):
