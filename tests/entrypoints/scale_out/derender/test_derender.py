...
async def test_derender_chat_too_many_choices_rejected(client):
    """choices count exceeding APHRODITE_MAX_N_SEQUENCES returns 400."""
    # Default APHRODITE_MAX_N_SEQUENCES is 16384; use a larger count.
    oversized_choices = [{"index": i, "token_ids": [42], "finish_reason": "stop"} for i in range(20_000)]
    response = await client.post(
        "/v1/chat/completions/derender",
        json={
            "model": MODEL_NAME,
...
async def test_derender_chat_negative_token_ids_rejected(client):
    ...
    # Aphrodite's validation_exception_handler converts Pydantic errors to 400.
    assert response.status_code == 400
...
async def test_derender_chat_oversized_logprobs_rejected(client):
    ...
    oversized_logprobs: dict = {
        "content": [{"token": "x", "logprob": -1.0, "bytes": None, "top_logprobs": []} for _ in range(1_000_000)]
    }
    response = await client.post(
        "/v1/chat/completions/derender",
        json={
        ...
        "content": [
            {
                ...
                "logprob": -1.0,
                "bytes": None,
                "top_logprobs": [{"token": f"t{i}", "logprob": -float(i), "bytes": None} for i in range(25)],
            }
        ]
    }
    response = await client.post(
