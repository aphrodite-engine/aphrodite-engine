...
class TestDetokenizeDelta:
    ...
    def _chunked(self, derenderer, tokenizer, chunks: list[list[int]]) -> str:
        ...
        parts: list[str] = []
        for delta in chunks:
            text, state = derenderer._detokenize_delta(tokenizer, delta, state, skip_special_tokens=True)
            parts.append(text)
        return "".join(parts)
    ...
    def test_single_chunk(self, derenderer, tokenizer):
        ...
        assert self._chunked(derenderer, tokenizer, [token_ids]) == self._one_shot(tokenizer, token_ids)
    ...
    def test_two_equal_chunks(self, derenderer, tokenizer):
        ...
        assert self._chunked(derenderer, tokenizer, chunks) == self._one_shot(tokenizer, token_ids)
    ...
    def test_empty_delta_passthrough(self, derenderer, tokenizer):
        ...
        _, state = derenderer._detokenize_delta(tokenizer, token_ids, DerenderStreamState(), skip_special_tokens=True)
        text, new_state = derenderer._detokenize_delta(tokenizer, [], state, skip_special_tokens=True)
        assert text == ""
    ...
    def test_n_independent_streams_same_result(self, derenderer, tokenizer):
        ...
        for _ in range(3):
            state = DerenderStreamState()
            text, state = derenderer._detokenize_delta(tokenizer, token_ids[:mid], state)
...
class TestDerenderCompletionStream:
    ...
    async def test_prompt_tokens_in_usage(self, derenderer, tokenizer):
        ...
        usage_chunk = _make_usage_chunk(completion_tokens=len(token_ids), prompt_tokens=7)
    ...
    async def test_none_state_initialises_correctly(self, derenderer, tokenizer):
        ...
        assert chunk.choices[0].text == tokenizer.decode(token_ids, skip_special_tokens=True)
    ...
    async def test_skip_special_tokens_threaded(self, derenderer, tokenizer):
        ...
        async def _text(skip: bool) -> str:
            req = CompletionRequest(model=MODEL_NAME, prompt="x", skip_special_tokens=skip)
            chunk, _ = await derenderer.derender_completion_stream(
                model=MODEL_NAME,
                generate_chunk=_make_stream_chunk(token_ids),
...
class TestDerenderChatStream:
    ...
    async def test_chunked_equals_oneshot(self, derenderer, tokenizer):
        ...
        streamed = (chunk1.choices[0].delta.content or "") + (chunk2.choices[0].delta.content or "")
        one_shot = tokenizer.decode(token_ids, skip_special_tokens=True)
...
class TestServingDerenderStreamErrorHandling:
    ...
    def _make_serving(self, side_effect: Exception):
        ...
        from aphrodite.entrypoints.scale_out.derender.serving import ServingDerender
        ...
        online_derenderer.derender_completion_stream = AsyncMock(side_effect=side_effect)
...
async def test_streaming_completion_derender_roundtrip(client):
    ...
    r1 = await client.post(
        ...
        json={
            ...
            "generate_chunk": {
                "request_id": "test-s",
                "choices": [{"index": 0, "token_ids": chunk1_ids, "finish_reason": None}],
            },
            "stream_state": None,
        },
    )
    ...
    r2 = await client.post(
        ...
        json={
            ...
            "generate_chunk": {
                "request_id": "test-s",
                "choices": [{"index": 0, "token_ids": chunk2_ids, "finish_reason": "stop"}],
            },
            "stream_state": state1,
        },
    )
