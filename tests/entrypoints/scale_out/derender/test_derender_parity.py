...
out of process). A standard ``aphrodite run`` server mounts both, so one
...
def server():
    with RemoteOpenAIServer(
        MODEL,
        ARGS,
        env_dict={"APHRODITE_ENABLE_SCALE_OUT_ENDPOINTS": "1"},
    ) as remote_server:
        yield remote_server
...
async def client(server):
    async with httpx.AsyncClient(base_url=server.url_for(""), timeout=60.0) as http_client:
        yield http_client
...
def _tool_sig(response_choice: dict) -> list[tuple[str, dict]]:
    """Normalize tool signatures so argument ordering does not matter."""
    return [
...
async def _run_parity_case(client: httpx.AsyncClient, messages: list[dict], **extra) -> tuple[dict, dict]:
    """Run coupled generation, then feed its tokens into derender."""
...
async def test_parity_reasoning(client):
    ...
    coupled, disagg = await _run_parity_case(client, messages, include_reasoning=True, max_tokens=256)
    _assert_parity(coupled, disagg)
...
async def test_parity_reasoning_and_tool_call(client):
    """Exercise reasoning and tool parser branches on the same output."""
    messages = [{"role": "user", "content": "What's the weather in Paris?"}]
    coupled, disagg = await _run_parity_case(
        client,
        messages,
...
async def test_parity_logprobs(client):
    ...
    placeholder = await _coupled(client, messages, return_tokens_as_token_ids=True, **extra)
    ...
    for d_entry, r_entry in zip(d_content, r_content, strict=True):
