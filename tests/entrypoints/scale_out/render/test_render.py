...
from aphrodite.entrypoints.scale_out.render.api_router import router
...
def _build_responses_serving_render() -> ServingRender:
    ...
    serving.online_renderer.create_error_response = OnlineRenderer.create_error_response.__get__(
        serving.online_renderer
    )
    serving.online_renderer.validate_chat_template = OnlineRenderer.validate_chat_template.__get__(
        serving.online_renderer
    )
...
def test_responses_render_route_documents_not_implemented():
    route = next(route for route in router.routes if route.path == "/v1/responses/render")
...
async def test_render_responses_rejects_empty_token_ids():
    ...
    response = await serving.render_responses_request(ResponsesRequest(model=MODEL_NAME, input="Test prompt"))
