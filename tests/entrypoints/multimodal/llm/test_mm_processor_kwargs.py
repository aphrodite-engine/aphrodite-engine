...
def test_preprocess_cmpl_applies_mm_processor_kwargs_to_renderer(
    ...
    monkeypatch.setattr(
        "aphrodite.entrypoints.offline_utils.parse_model_prompt",
...
def test_preprocess_chat_defaults_add_special_tokens_to_false() -> None:
    ...
    renderer.default_chat_tok_params.with_kwargs.assert_called_once_with(add_special_tokens=False)
...
class TestChatAddSpecialTokensDefault:
    ...
    BOS_TEMPLATE = "{{ bos_token }}{% for m in messages %}{{ m['content'] }}{% endfor %}"
    MESSAGES = [[{"role": "user", "content": "hi"}]]
    ...
    def test_chat_matches_online_chat_api(self, llava_llm):
        from aphrodite.renderers.params import ChatParams, TokenizeParams
        ...
        _, (online_input,) = llava_llm.renderer.render_chat(self.MESSAGES, chat_params, online_tok_params)
    ...
    def test_explicit_tokenization_kwargs_override_default(self, llava_llm):
        bos = llava_llm.renderer.tokenizer.bos_token_id
        prompt_token_ids = self._chat_token_ids(llava_llm, tokenization_kwargs={"add_special_tokens": True})
