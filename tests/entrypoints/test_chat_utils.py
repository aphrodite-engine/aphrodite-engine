...
from aphrodite.endpoints.chat_utils import (_try_extract_ast, load_chat_template,
                                         parse_chat_messages,
                                         parse_chat_messages_futures,
                                         resolve_chat_template_content_format,
                                         resolve_hf_chat_template)
from aphrodite.endpoints.llm import apply_hf_chat_template
...
def test_multimodal_image_parsing_matches_hf(model, image_url):
    ...
    # Now parse with Aphrodites chat utils & apply the template
    aphrodite_conversation = get_conversation(is_hf=False)
    conversation, _ = parse_chat_messages(
        aphrodite_conversation,
        model_config,
        tokenizer_group,
        content_format="openai",
    )

    aphrodite_result = apply_hf_chat_template(
        tokenizer,
    ...
    assert hf_result == aphrodite_result
