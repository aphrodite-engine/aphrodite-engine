...
from aphrodite.entrypoints.openai.responses.context import (
    ConversationContext,
    HarmonyContext,
    SimpleContext,
)
...
from aphrodite.entrypoints.openai.responses.serving import (
...
from aphrodite.renderers.online_renderer import (
    OnlineRenderer,
...
def test_extract_tool_types(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # When envs.APHRODITE_GPT_OSS_SYSTEM_TOOL_MCP_LABELS is not set,
    # mcp tool types are all ignored.
    assert extract_tool_types(tools) == {"local_shell", "auto"}

    # container is allowed, it would be extracted
    monkeypatch.setenv("APHRODITE_GPT_OSS_SYSTEM_TOOL_MCP_LABELS", "container")
...
async def test_online_renderer_rejects_harmony_history_for_non_harmony_model():
    ...
    result = await renderer.render_responses(
        request,
        previous_messages=[OpenAIHarmonyMessage.from_role_and_content(Role.USER, "first")],
    )
...
async def test_online_renderer_adjusts_harmony_tool_choice(
    ...
    monkeypatch.setattr(
        "aphrodite.renderers.online_renderer.render_for_completion",
        lambda messages: [1],
    )
    request = ResponsesRequest(
        input="next",
...
async def test_online_renderer_applies_responses_token_budget_to_harmony_prompt(
    ...
    renderer.renderer = SimpleNamespace(tokenizer=SimpleNamespace(truncation_side="left"))
    monkeypatch.setattr(
        "aphrodite.renderers.online_renderer.render_for_completion",
...
class TestInitializeToolSessions:
    ...
    @pytest.mark.asyncio
    async def test_render_next_turn_reuses_shared_responses_renderer(self, serving_responses_instance):
        serving_responses_instance.online_renderer.render_responses = AsyncMock(
            return_value=SimpleNamespace(
                messages=[],
        ...
        serving_responses_instance.online_renderer.preprocess_chat = AsyncMock(return_value=([], [tokens_input([1])]))
        request = ResponsesRequest(
            input="first",
        ...
        render_request = serving_responses_instance.online_renderer.render_responses.call_args.args[0]
    ...
    async def test_harmony_tool_followup_preserves_render_params(
        ...
        serving_responses_instance.engine_client.generate.side_effect = lambda *args, **kwargs: generate_output()
        monkeypatch.setattr(
            "aphrodite.renderers.online_renderer.render_for_completion",
            lambda messages: [1, 2, 3, 4],
        )
        online_renderer = serving_responses_instance.online_renderer
        online_renderer.renderer = SimpleNamespace(tokenizer=SimpleNamespace(truncation_side="left"))
        ...
        followup_engine_input = serving_responses_instance.engine_client.generate.call_args_list[1].args[0]
