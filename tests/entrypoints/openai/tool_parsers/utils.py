...
class StreamingToolReconstructor:
    ...
    def append_delta(self, delta: DeltaMessage):
        if delta.content is not None:
            ...
            assert delta.tool_calls, "Streaming results should have either content or tool calls (or both)"
        if self._assert_one_tool_per_delta:
            ...
            assert len(delta.tool_calls) < 2, "Streaming should include only one tool call per update."
        for call_delta in delta.tool_calls:
            assert call_delta.type is None or call_delta.type == "function", (
                f"Streaming tool calls should only emit function calls. Got {call_delta.type}"
            )
            current_tool_call = self.tool_calls[call_delta.index] if call_delta.index < len(self.tool_calls) else None
            if current_tool_call:
                ...
                assert not call_delta.id, f"Streaming tool calls must emit function id only once. Got {call_delta.id}"
                assert call_delta.index == len(self.tool_calls) - 1, (
                    f"Incorrect index for tool delta. Got {call_delta.index}, expected {len(self.tool_calls) - 1}"
                )
                current_tool_call.function.arguments += call_delta.function.arguments
            else:
                assert call_delta.id is not None, "Streaming tool calls must have an id on first appearance"
                ...
                assert call_delta.index == len(self.tool_calls), (
                    f"Incorrect index for tool delta. Got {call_delta.index}, expected {len(self.tool_calls)}"
                )
...
def run_tool_extraction_streaming(
    ...
    if isinstance(model_deltas, str):
        model_deltas = split_string_into_token_deltas(tool_parser.model_tokenizer, model_deltas)
