...
async def _assert_parity(
    ...
    *,
    ...
    assert chat_params.chat_template_content_format == responses_params.chat_template_content_format
    ...
    assert dict(chat_params.chat_template_kwargs) == dict(responses_params.chat_template_kwargs)


def _weather_tools(*, overrides: dict[str, Any] | None = None) -> tuple[list[dict], list[dict]]:
...
def serving_responses(
    ...
    engine_client.aphrodite_config = MagicMock()
...
class TestToolsRenderParity:
    ...
    async def test_tools_with_tool_choice_none(self, online_renderer, serving_responses):
        chat_tools, responses_tools = _weather_tools()
        await _assert_parity(
            online_renderer,
            serving_responses,
    ...
    async def test_function_tool_optional_fields(self, online_renderer, serving_responses):
...
class TestReasoningRenderParity:
    ...
    async def test_reasoning_effort(self, online_renderer, serving_responses, effort: str):
        await _assert_parity(
            online_renderer,
            serving_responses,
            chat_kwargs={
    ...
    async def test_explicit_enable_thinking_not_overridden(self, online_renderer, serving_responses):
        await _assert_parity(
            online_renderer,
            serving_responses,
            chat_kwargs={
