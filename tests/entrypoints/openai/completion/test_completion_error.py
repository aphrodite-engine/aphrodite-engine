...
from aphrodite.v1.metrics.stats import RequestSpecDecodeMetrics, RequestStateStats
...
def test_completion_per_request_metrics_follow_server_flag():
    ...
    disabled_serving = _build_minimal_metrics_serving_completion(enable_per_request_metrics=False)
    ...
    enabled_serving = _build_minimal_metrics_serving_completion(enable_per_request_metrics=True)
...
def _make_spec_decode_request_output(num_seqs: int = 1, with_metrics: bool = True) -> RequestOutput:
    outputs = [
        CompletionOutput(
            index=i,
            text="Hello",
...
class TestCompletionPromptListLimit:
    """Regression tests for unbounded prompt list fan-out."""
    ...
    def test_bounded_text_prompt_list_allowed(self, monkeypatch):
        monkeypatch.setenv("APHRODITE_MAX_COMPLETION_PROMPTS", "10")
    ...
    def test_oversized_text_prompt_list_rejected(self, monkeypatch):
        monkeypatch.setenv("APHRODITE_MAX_COMPLETION_PROMPTS", "5")
        from aphrodite import envs

        if hasattr(envs.__getattr__, "cache_clear"):
            envs.__getattr__.cache_clear()

        with pytest.raises(Exception, match="prompt list length 10 exceeds the maximum"):
            CompletionRequest(
                model=MODEL_NAME,
                prompt=["x"] * 10,
                max_tokens=1,
