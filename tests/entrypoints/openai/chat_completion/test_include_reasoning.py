...
async def test_include_reasoning_false_non_streaming(client: openai.AsyncOpenAI):
    ...
    msg = response.choices[0].message
    reasoning = getattr(msg, "reasoning", None) or getattr(msg, "reasoning_content", None)
    assert not reasoning, f"Expected no reasoning when include_reasoning=False, got: {reasoning}"
...
async def test_include_reasoning_false_streaming(client: openai.AsyncOpenAI):
    ...
    assert not reasoning_text, f"Expected no reasoning deltas when include_reasoning=False, got: {reasoning_text[:100]}"
