...
def test_structured_output(
    ...
    with aphrodite_runner(
        ...
        enforce_eager=True,
        max_model_len=1024,
        structured_outputs_config=dict(backend=backend, disable_any_whitespace=backend in {"xgrammar", "guidance"}),
        seed=120,
        ...
        **platform_args,
        ...
        if backend.startswith("xgrammar"):
            with pytest.raises(
                ...
                match="The provided JSON schema contains features not supported by xgrammar.",
            ):
                prompt = (
        ...
        if backend not in ["outlines", "lm-format-enforcer"]:
            ...
            with pytest.raises(APHRODITEValidationError, match="Failed to convert the grammar "):
                runner.llm.generate(
                    (
                        "Generate a sql statement that selects col_1 from "
        ...
        prompt = (
            f"Give an example IPv4 address with this regex: {sample_regex}. Make the response as short as possible."
        )
        outputs = runner.llm.generate(
            [prompt] * 2,
            sampling_params=sampling_params,
        ...
        sampling_params = SamplingParams(
            temperature=0.8,
            top_p=0.95,
            structured_outputs=StructuredOutputsParams(choice=sample_structured_outputs_choices),
        )

        outputs = runner.llm.generate(
            ("The best language for type-safe systems programming is (Make the response as short as possible.) "),
            sampling_params=sampling_params,
            use_tqdm=True,
        )
        assert outputs is not None
        ...
        outputs = runner.llm.generate(
            ("Generate a description of a frog using 50 characters. Make the response as short as possible."),
            sampling_params=sampling_params,
            use_tqdm=True,
        )
        ...
        if backend not in ["outlines", "lm-format-enforcer"]:
            ...
            sampling_params = SamplingParams(
                temperature=0.0,
                max_tokens=4096,
                structured_outputs=StructuredOutputsParams(structural_tag=json.dumps(structural_tag_config)),
            )
...
"""
            ...
            outputs = runner.llm.generate(prompt, sampling_params=sampling_params, use_tqdm=True)
            assert outputs is not None
            ...
            for output in outputs:
                ...
                if not matches:
                    print(f"Warning: No function calls found in response: {generated_text!r}")
                    continue
                ...
                try:
                    ...
                    pytest.fail(f"Invalid function call format: {generated_text!r}\nError: {str(e)}")
...
def test_structured_output_with_reasoning_matrices(
    ...
    with aphrodite_runner(
        ...
        reasoner = ReasoningParserManager.get_reasoning_parser(reasoning_parser)(tokenizer=tokenizer)
...
def test_structured_output_auto_mode(
    ...
    with aphrodite_runner(
        ...
        outputs = runner.llm.generate(prompts, sampling_params=sampling_params, use_tqdm=True)
        # Make sure `auto` backend handling doesn't mess up sampling_params
        # and that we can reuse it without error.
        outputs.extend(runner.llm.generate(prompts, sampling_params=sampling_params, use_tqdm=True))
...
def test_guidance_no_additional_properties(aphrodite_runner):
    with aphrodite_runner(
        "Qwen/Qwen2.5-1.5B-Instruct",
        max_model_len=1024,
        structured_outputs_config=dict(
...
def test_structured_output_batched_with_non_structured_outputs_requests(
    ...
    with aphrodite_runner(
        ...
        outputs = runner.llm.generate(prompts=prompts, sampling_params=sampling_params, use_tqdm=True)
...
def test_structured_output_with_structural_tag(backend: str, aphrodite_runner):
    with aphrodite_runner(
        "Qwen/Qwen2.5-1.5B-Instruct",
