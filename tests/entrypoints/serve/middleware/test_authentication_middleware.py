...
from aphrodite.entrypoints.serve.middleware.authenticate import (
    GUARDED_PREFIX,
...
def task_routes(request, monkeypatch) -> tuple[str, list[tuple[str, list[str]]]]:
    ...
    monkeypatch.setenv("APHRODITE_SERVER_DEV_MODE", "1")
    ...
    register_api_routers(args, app, supported_tasks=(task,), model_config=MockModelConfig())
...
def test_auto_discovered_protected_routes_require_auth(task_routes):
    ...
    for path_template, methods in routes:
        ...
        assert resp.status_code == 401, f"[{task}] {test_method} {test_path} should reject missing token"

        resp = client.request(test_method, test_path, headers={"Authorization": "Bearer wrong"})
        assert resp.status_code == 401, f"[{task}] {test_method} {test_path} should reject invalid token"

        resp = client.request(test_method, test_path, headers={"Authorization": "Bearer valid-token"})
        assert resp.status_code == 200, f"[{task}] {test_method} {test_path} should accept valid token"
...
def test_auto_discovered_unprotected_routes_no_auth(task_routes):
    ...
    for path_template, methods in routes:
        ...
        resp = client.request(test_method, test_path)
        assert resp.status_code == 200, f"[{task}] {test_method} {test_path} should be accessible without token"
