...
MODEL_NAME = os.environ.get("APHRODITE_TEST_MODEL", "Qwen/Qwen3-0.6B")
...
def server(
    ...
    """Launch a Aphrodite server with the dev router; yield its base URL.
    ...
    """
    env = {**os.environ, "APHRODITE_SERVER_DEV_MODE": "1"}
    base = _DUMMY_ARGS if dummy_weights else _BASE_ARGS
    cmd = [
        sys.executable,
        "-m",
        "aphrodite.entrypoints.openai.api_server",
        "--model",
        MODEL_NAME,
        "--port",
        str(port),
    ...
    try:
        ...
        while time.time() < deadline:
            if proc.poll() is not None:
                ...
                raise RuntimeError(f"aphrodite server exited during startup:\n{err}")
            ...
            proc.terminate()
            raise RuntimeError("aphrodite server did not start in time")
        _warm_up(url)
        yield url
    finally:
        proc.terminate()
...
def poll_until(
    ...
    Workaround for the Aphrodite sleep/wake "200-lie" — the HTTP endpoints may
...
def start_stream(url: str, max_tokens: int) -> tuple[StreamResult, threading.Thread]:
    ...
    assert started, f"request did not start generating within {STREAM_START_TIMEOUT}s (stream error: {result.error})"
...
def sleep_metrics(url):
    ...
    for family in text_string_to_metric_families(r.text):
        if family.name == "aphrodite:engine_sleep_state":
