...
"""End-to-end tests for the Aphrodite RL pause/resume lifecycle."""
...
def server_url(use_v2):
    env_vars = {
        "APHRODITE_USE_V2_MODEL_RUNNER": "1" if use_v2 else "0",
    }
    ...
    def test_mode_request_lifecycle(
        ...
        try:
            ...
            if mode in ("abort", "wait"):
                ...
                assert len(inflight.chunks) == chunks_after_pause, "in-flight request continued generating in keep mode"

            new_thread.start()
            assert not new_done.wait(timeout=0.3), "new request completed while generation was paused"
        finally:
            assert resume(server_url) == 200
