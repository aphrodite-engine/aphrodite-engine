...
``aphrodite/entrypoints/cohere/serving.py``.
...
from aphrodite.entrypoints.cohere.serving import (
    _DONE_FRAME,
    CohereServingChatV2,
    ContentBlockType,
    _emit,
...
async def _drain(serving: CohereServingChatV2, items: list[str]) -> list[str]:
    ...
    request = CohereChatV2Request(model="m", messages=[{"role": "user", "content": "hi"}], stream=True)
...
class TestSSEHelpers:
    ...
    def test_emit_serializes_event_with_type_discriminator(self):
        frame = _emit(MessageStartEvent(id="abc", delta={"message": {"role": "assistant"}}))
...
class TestHandleToolCallDeltas:
        ...
        deltas = [
            type(
                ...
                (),
                {
                    "index": 0,
                    "id": "c1",
                    "function": type("Fn", (), {"name": "calc", "arguments": '{"x":'})(),
                },
            )()
        ]
    ...
    def test_subsequent_arguments_emit_delta(self):
        ...
        assert payload["index"] == 0
        assert payload["delta"]["message"]["tool_calls"]["function"]["arguments"] == "1}"
...
class TestHandleCitationDeltas:
    """Test the per-delta emitter. Sources are pre-resolved by the
    reasoning parser via the position map forwarded through
    ``chat_template_kwargs`` (see :func:`_melody_sources_to_aphrodite` in
    ``aphrodite/reasoning/cohere_command_reasoning_parser.py``), so the
    handler only has to coerce to the SDK wire shape, apply the
    ``THINKING_CONTENT`` -> ``PLAN`` rewrite, and drop citations with
    no attributable sources.
    """

    @staticmethod
    def _resolved_source(**overrides: Any) -> CitationSource:
        base = dict(
            type="document",
            id="doc_hamlet",
    ...
    def test_multiple_sources_survive_to_wire(self):
        serving = _serving()
        state = _StreamState()
        citations = [
            APHRODITECitation(
                ...
                text="works",
                sources=[
                    self._resolved_source(id="d0", document={"id": "d0"}),
                    self._resolved_source(id="d1", document={"id": "d1"}),
                ],
                type="TEXT_CONTENT",
            )
        ]
    ...
    def test_source_without_id_streamed_with_payload(self):
        # Match the cohere api: id-less tool sources (from text-only
        # tool results) still ride through with their ``tool_output``
        # payload; only the ``id`` field is omitted.
        serving = _serving()
        state = _StreamState()
        citations = [
            APHRODITECitation(
                ...
                text="hello",
                sources=[CitationSource(type="tool", tool_output={"content": "hi"})],
            )
        ]
        events = serving._handle_citation_deltas(state, citations)
        assert len(events) >= 1
        cit_payload = _parse_event(events[0])["delta"]["message"]["citations"]
        assert cit_payload["sources"] == [{"type": "tool", "tool_output": {"content": "hi"}}]
        assert "id" not in cit_payload["sources"][0]

    def test_citation_with_no_sources_streamed_dropped(self):
        # Citations whose source list is entirely empty are still
        # dropped so we don't emit an unattributed anchor.
        serving = _serving()
        state = _StreamState()
        citations = [
            APHRODITECitation(
...
class TestBuildMessageEndEvent:
    ...
    def test_with_usage(self):
        ...
        frame = serving._build_message_end_event(chunk_id="abc", finish_reason="length", usage_chunk=chunk)
        payload = _parse_event(frame)
    ...
    def test_with_cached_tokens(self):
        ...
        frame = serving._build_message_end_event(chunk_id="abc", finish_reason="stop", usage_chunk=chunk)
        payload = _parse_event(frame)
...
class TestChatCompletionStreamToV2:
    ...
    async def test_error_message_end_is_sanitized(self):
        ...
        async def _raises_mid_stream() -> AsyncGenerator[str, None]:
            ...
            raise RuntimeError("boom <Foo at 0x7fabc0deface> reading /Users/dev/aphrodite/x.py")
        ...
        frames = [f async for f in serving._chat_completion_stream_to_v2(_raises_mid_stream(), request)]
        assert frames[-1] == _DONE_FRAME
        error_end = _parse_event(frames[-2])
        assert error_end["delta"]["finish_reason"] == "ERROR"
        ...
        assert "/Users/dev/aphrodite/x.py" not in err_msg
    ...
    async def test_citations_in_delta_emit_citation_events(self):
        # End-to-end streaming citation flow. The parser hands us
        # already-resolved sources (see ``_melody_sources_to_aphrodite``
        # in ``aphrodite/reasoning/cohere_command_reasoning_parser.py``);
        # the streaming loop just has to coerce and emit.
        serving = _serving()
        items = [
            _make_chunk(role="assistant"),
