...
implemented in ``aphrodite/entrypoints/cohere/serving.py``.
        ...
        "citations": [
            {
                ...
                "end": 5,
                "text": "Paris",
                "sources": [{"type": "tool", "id": "doc-9", "tool_output": {"text": "Paris"}}],
            }
        ],
    },
...
def _build_chat_completion_response(
    *,
    ...
    if usage is None:
        ...
        resp = ChatCompletionResponse.model_validate({**resp.model_dump(), "usage": usage})
        if kv_transfer_params is not None:
...
class TestConvertUserMessage:
    ...
    def test_image_url_without_detail_omits_field(self):
        ...
        result = _convert(req)
        assert result.messages[0]["content"][0]["image_url"] == {"url": "https://x/i.png"}
...
class TestConvertToolMessage:
    ...
    def test_text_only_list_preserves_per_block_text_parts(self):
        ...
        # for multi-part user content. Aphrodite's
        ...
        # ``aphrodite/entrypoints/chat_utils.py``), so downstream chat
    ...
    def test_document_blocks_serialized_as_text_parts(self):
        ...
        Aphrodite's shared ``_parse_chat_message_content_parts`` runs a
    ...
    def test_converted_tool_message_content_survives_openai_validation(self):
        ...
        ``aphrodite/entrypoints/chat_utils.py:_parse_chat_message_content_parts``
        ...
        req = self._request_with_tool_message(
            [
                {
                    "type": "document",
                    "document": {"data": {"result": "The second tallest mountain is K2."}},
                }
            ]
        )
        result = _convert(req)
...
class TestResponseFormat:
    ...
    def test_json_schema(self):
        ...
        result = _convert(_make_request(response_format={"type": "json_object", "json_schema": schema}))
        assert result.response_format is not None
...
class TestApplyCohereTemplateKwargs:
    ...
    def test_thinking_forwarded_as_dict(self):
        result = _convert(_make_request(thinking={"type": "enabled", "token_budget": 16}))
...
class TestEngineChatTemplateKwargs:
    ...
    def test_engine_kwargs_are_msgpack_encodable(self):
        ...
        round_tripped = decoder.decode(encoder.encode({"chat_template_kwargs": engine_kwargs})[0])
    ...
    def test_base_implementation_is_a_passthrough(self):
        ...
        passed_through = OpenAIServingChat._engine_chat_template_kwargs(_FakeServing(), kwargs)
...
def _forwarded_v2_content(
    ...
    assert TOOL_MESSAGE_V2_CONTENT_KEY in kwargs, (
        f"expected {TOOL_MESSAGE_V2_CONTENT_KEY} in chat_template_kwargs, got keys {list(kwargs)}"
    )
...
class TestMessageCitations:
    ...
    def test_absent_when_no_assistant_citations(self):
        ...
        assert result.chat_template_kwargs is None or ("_messages_citations" not in (result.chat_template_kwargs or {}))
...
class TestChatCompletionToV2:
    ...
    def test_stop_sequence_finish_reason(self):
        serving = _serving()
        resp = _build_chat_completion_response(finish_reason="stop", stop_reason="<END>")
        v2 = serving._chat_completion_to_v2(resp, _make_request())
    ...
    def test_non_reasoning_model_keeps_thinking_when_no_tool_calls(self):
        ...
        serving = _serving(is_reasoning_model=False)
        resp = _build_chat_completion_response(content="answer", reasoning="plan", tool_calls=None)
        v2 = serving._chat_completion_to_v2(resp, _make_request())
        assert v2.message.tool_plan is None
        types = [c.type for c in v2.message.content]
        assert types == ["thinking", "text"]
    ...
    def test_kv_transfer_params_propagated(self):
        serving = _serving()
        resp = _build_chat_completion_response(content="hi", kv_transfer_params={"k": 1})
        v2 = serving._chat_completion_to_v2(resp, _make_request())
...
class TestExtractCitations:
    ...
    ``_melody_sources_to_aphrodite`` in
    ``aphrodite/reasoning/cohere_command_reasoning_parser.py``, which
    ...
    @staticmethod
    def _make_citation(**overrides: Any) -> APHRODITECitation:
        base = dict(
            start=0,
            end=11,
            text="Shakespeare",
        ...
        return APHRODITECitation(**base)

    def test_none_or_empty_returns_none(self):
        serving = _serving()
        assert serving._extract_citations_if_any(type("M", (), {"citations": None})()) is None
        assert serving._extract_citations_if_any(type("M", (), {"citations": []})()) is None
