...
async def test_shutdown_on_engine_failure(tmp_path: Path):
    ...
    Starts a Aphrodite server, kills it to simulate a crash, then verifies that
    ...
    with server_log.open("wb") as log_file:
        proc = subprocess.Popen(
            [
                ...
                sys.executable,
                "-m",
                "aphrodite.entrypoints.launchers.api_server.entry",
                "--model",
                MODEL_NAME,
                "--dtype",
                "bfloat16",
...
async def test_abort_timeout_exits_within_cleanup_grace(
    ...
    with RemoteOpenAIServer(MODEL_NAME, server_args) as remote_server:
        ...
        try:
            ...
            proc.kill()
            proc.wait(timeout=5)
            pytest.fail(f"Process did not exit within the resource cleanup grace ({termination_timeout}s)")
