...
Lifecycle integration tests replace child Aphrodite servers with lightweight
...
Port allocation (kept far from default Aphrodite ports to avoid conflicts):
  Supervisor : 19256
...
import aphrodite.entrypoints.launchers.dp_supervisor as dp_sup
...
async def test_shutdown_children_uses_engine_process_timeout(
    ...
    supervisor._processes = [SimpleNamespace(name="APIServer_DPRank_4", pid=None, is_alive=lambda: False)]
    calls = []
    timeout_calls = []
...
async def _await_supervisor_health(expected_status: int, use_ssl: bool = False, retries: int = 20) -> bool:
...
async def _poll_until_api_server_running(port: int, timeout_s: float = 30.0, use_ssl: bool = False) -> None:
    ...
    raise TimeoutError(f"API server on port {port} did not start within {timeout_s}s") from last_exc
...
async def test_failed_startup(monkeypatch):
    """
    A) One of the Aphrodite servers crashes during startup.
