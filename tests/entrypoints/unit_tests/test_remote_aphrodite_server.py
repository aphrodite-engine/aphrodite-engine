...
@pytest.mark.parametrize(
    ("server_cls", "command"),
    [
        (RemoteOpenAIServer, ["aphrodite", "serve"]),
        (RemoteLaunchRenderServer, ["aphrodite", "launch", "render"]),
    ],
)
def test_server_redacts_sensitive_values(
    ...
    server._start_server("test-model", serve_args, {"OVERRIDE_SECRET": "override-secret"})
...
def test_openai_server_shutdown_wait_covers_engine_cleanup(
    ...
    monkeypatch.setattr(test_utils, "get_engine_process_shutdown_timeout", get_engine_timeout)
