...
def test_parse_chat_messages_video_and_video_embeds_mixed(
    ...
    with pytest.raises(APHRODITEValidationError, match="Mixing raw video"):
        parse_chat_messages(
            [
                {
                    "role": "user",
...
def test_parse_chat_messages_interleave_placeholder_overcount_raises(
    ...
    with pytest.raises(APHRODITEValidationError, match="placeholders in input prompt"):
        parse_chat_messages(
            [
                {
                    "role": "user",
...
async def test_resolve_items_does_not_leak_tasks_on_partial_failure():
    ...
    assert not leaked_tasks, f"resolve_items left {len(leaked_tasks)} task(s) running after raising: {leaked_tasks}"
