...
from aphrodite.model_executor.models.qwen3_asr import Qwen3ASRForConditionalGeneration
...
def test_qwen3_asr_stream_processor_passes_plain_text_without_prefix():
    post_processor = Qwen3ASRForConditionalGeneration.get_streaming_post_processor_cls()()
...
def test_qwen3_asr_stream_processor_emits_finished_incomplete_prefix():
    ...
    assert post_processor.process_delta(" language English", True) == " language English"
...
async def test_transcription_stream_generator_english_inserts_space_between_chunks():
    ...
    serving.streaming_post_processor_cls = _StubTranscriptionModel.get_streaming_post_processor_cls()
...
async def test_transcription_stream_generator_strips_qwen3_asr_prefix_per_chunk():
    ...
    serving.streaming_post_processor_cls = Qwen3ASRForConditionalGeneration.get_streaming_post_processor_cls()
    serving.task_type = "transcribe"
    request = SimpleNamespace(
...
async def test_create_transcription_non_streaming_joins_chunks_by_language():
    ...
    preprocess_mock = AsyncMock(return_value=([MagicMock(), MagicMock()], 1.0, [0.0, 29.5]))
