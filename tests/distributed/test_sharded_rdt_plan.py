...
from aphrodite.distributed.weight_transfer.base import layerwise_groups
from aphrodite.distributed.weight_transfer.sharded_rdt_common import (
    ALLOWED_OPS,
    SUPPORTED_OPS,
    RdtRouter,
...
from aphrodite.distributed.weight_transfer.sharded_rdt_engine import (
    ShardedRDTWeightTransferEngine,
    ShardedRDTWeightTransferInitInfo,
    _dtype_from_name,
)
from aphrodite.distributed.weight_transfer.sharded_rdt_fake import (
    BakeSink,
    FakeRDTTensor,
    _Scatter,
    _UnsupportedFakeOp,
...
def _planner(baked, *, name_meta, live=None, held_by=None, group_lens=None):
    ...
    router = RdtRouter(n_prod, 1, owner_sets, classes, names, group_lens or [1] * len(names))
...
def _router(
    ...
    if group_lens is None:
        group_lens = [1] * len(names)
    return RdtRouter(num_producers, num_consumers, owner_sets, name_owner_class, names, group_lens)
...
class TestRdtRouter:
    ...
    def test_fan_in_shares_one_producer(self):
        ...
        assert [{r.producer_for(c, n) for n in names} for c in range(8)] == [{c // 4} for c in range(8)]
    ...
    def test_consumers_spread_over_an_owner_set(self):
        ...
        r = _router(4, 4, owner_sets=[[0, 1]], name_owner_class=[0] * 6, group_lens=[1] * 6)
    ...
    def test_validate_rejects_an_empty_owner_set(self):
        with pytest.raises(ValueError, match="empty"):
            RdtRouter(4, 2, [[0, 1], [], [2, 3]], [0, 1, 2], ["a", "b", "c"], [3]).validate()
...
class TestReplicaOverlay:
    ...
    def test_every_replica_of_a_worker_resolves_the_same_producer(self):
        ...
        for worker in range(8):
            for name in names:
                ...
                assert len(picks) == 1, f"worker {worker} split across producers {picks}"
    ...
    def test_one_deployment_is_the_historical_carve(self):
        ...
        for consumer_id in range(8):
            ...
            for name in names:
                assert plain.producer_for(consumer_id, name) == explicit.producer_for(consumer_id, name)
...
class TestPackedLayout:
    def test_consumer_layout_matches_the_producer_rule(self):
        ...
        for name, dtype_name, shape in specs:
            ...
            copies.append(_copy(name, shape=tuple(shape), dtype=_dtype_from_name(dtype_name)))
        ...
        want_offsets, want_bytes = _producer_pack_offsets([(s[0], _dtype_from_name(d)) for _n, d, s in specs])
...
class TestChunkModuleScatters:
    ...
    table, so Aphrodite's expert placement needs no cases."""
...
class TestBuildCallPlan:
    ...
    def test_a_group_with_nothing_local_is_signaled_at_sync_start(self):
        ...
        eng = _planner(baked, name_meta=name_meta, live=set(names) - {"model.layers.1.w"})
        plan = eng._build_call_plan(names, group_lens)
        assert len(plan.chunks) == 3
...
class TestCallPlanRouting:
    ...
    def _routed_planner(self, held_by_group, num_consumers, consumer_id, *, held_by=None):
        """``held_by_group[g]`` = the ranks holding group g's names, so a test
        can still speak in groups; ``held_by`` overrides individual names."""
        baked, name_meta, names, group_lens = _one_module_per_layer(len(held_by_group) - 2)
        ...
        for n in names:
            ...
            classes.append(seen[key])
        eng = _planner(baked, name_meta=name_meta)
        router = RdtRouter(n_prod, num_consumers, owner_sets, classes, names, group_lens)
        router.validate()
...
class TestSignalCompleteness:
    ...
    def _moe_planner(self, worker_experts, *, ep_size=4, n_experts=8, n_layers=2):
        ...
        names = ["embed.weight"]
        group_lens = [1]
        baked = {"embed.weight": _module(_FakeLayer("embed"), [_copy("embed.weight", shape=(4,))])}
        ...
        for li in range(n_layers):
            ...
            fused = _module(
                _FakeLayer(f"moe{li}"),
                [_copy(f"model.layers.{li}.experts.{e}.w", shape=(4,)) for e in worker_experts],
            )
        ...
        group_lens.append(1)
        baked["lm_head.weight"] = _module(_FakeLayer("head"), [_copy("lm_head.weight", shape=(4,))])
    ...
    def test_placement_changes_only_the_chunk_count_never_the_signals(self):
        ...
        assert len(plans["linear"].chunks) == 2 + 2 * 3  # embed+head + 2 layers x (1 + 2)
    ...
    def test_the_fused_module_materializes_first_and_quants_last_across_chunks(self):
        ...
        eng, names, group_lens = self._moe_planner(worker_experts=[0, 2, 4, 6], n_layers=1)
        plan = eng._build_call_plan(names, group_lens)
        # group 1 = [norm chunk, coord 0..3 chunks] at chunk indices 1..5
        fused_chunks = [c for c in plan.chunks[1:6] if any("experts" in sc.src[0] for sc in c.scatters)]
...
BAKE_MODELS = [
    ...
    #
    # Not an fp8 checkpoint: `process_weights_after_loading` swaps the Aphrodite
...
def _bake_probe(self, model_name):  # noqa: PLR0915  (runs in the Aphrodite worker)
    ...
    from aphrodite.distributed.weight_transfer.sharded_rdt_common import ALLOWED_OPS
    from aphrodite.distributed.weight_transfer.sharded_rdt_engine import (
        ShardedRDTWeightTransferEngine,
        ShardedRDTWeightTransferInitInfo,
    )
    from aphrodite.model_executor.model_loader.reload.utils import get_layer_tensors
    ...
    loaded = {n for n, prm in model.named_parameters() if not (prm.is_floating_point() and torch.isnan(prm).any())}
    # On the host: the weights themselves already fill the GPU.
    reference = {n: prm.detach().to("cpu", copy=True) for n, prm in model.named_parameters() if n in loaded}
    ...
    eng._name_meta = {}
    eng._live_names = set()
    eng._bake(ShardedRDTWeightTransferInitInfo(names=names, dtype_names=dtype_names, shapes=shapes))
    ...
    for mod in model.modules():
        for pname, tensor in get_layer_tensors(mod).items():
            ...
            assert not hasattr(loader, "_rdt_stamp_inner"), f"recording stamp left on {param_name_of.get(key, key)}"
            assert loader is loaders_before[key], f"weight_loader not restored on {param_name_of.get(key, key)}"
            assert not tensor.is_meta, f"{param_name_of.get(key, key)} left on meta after the dry run"
    ...
    from aphrodite.model_executor.layers.attention import is_deferred_attention_layer

    attn_in_plan = sorted(
        {type(c.layer).__name__ for plan in scatters.values() for c in plan if is_deferred_attention_layer(c.layer)}
    )
    ...
    post_processed = {id(mod) for mod in model.modules() if hasattr(mod, "process_weights_after_loading")}
    ...
    assert not unexplained, (
        f"the plan misses {len(unexplained)} param(s) load_weights wrote that nothing derives: {unexplained[:5]}"
    )
    mismatched = [
        n for n, prm in model.named_parameters() if n in loaded and not torch.equal(prm.detach().cpu(), reference[n])
    ]
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="the bake drives a real model on GPU")
class TestBakeOnARealModel:
    """`_bake` against a real Aphrodite model, which the CPU tests above cannot reach:
    ...
    def test_bake_records_a_replayable_plan_and_restores_the_model(self, aphrodite_runner, monkeypatch, model):
        monkeypatch.setenv("APHRODITE_ALLOW_INSECURE_SERIALIZATION", "1")
        with aphrodite_runner(model, load_format="dummy", enforce_eager=True, max_model_len=1024) as llm:
