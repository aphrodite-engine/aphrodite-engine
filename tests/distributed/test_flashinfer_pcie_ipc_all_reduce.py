...
from aphrodite.distributed.device_communicators import cuda_communicator
...
def _uninitialized_comm() -> pcie_ipc.FlashInferPcieIpcAllReduce:
    comm = pcie_ipc.FlashInferPcieIpcAllReduce.__new__(pcie_ipc.FlashInferPcieIpcAllReduce)
    comm.disabled = False
    comm.group = object()
...
def test_setup_uses_exact_graph_capacity(monkeypatch, tmp_path):
    ...
    workspace.tune.assert_called_once_with([4096], dtype=torch.bfloat16, tune_group=comm.tune_group)
    workspace.prepare.assert_called_once_with([(8, 4096), (16, 4096), (32, 4096)], dtype=torch.bfloat16)
...
def test_cuda_dispatch_prefers_pcie_ipc_before_existing_flashinfer(monkeypatch):
    communicator = cuda_communicator.CudaCommunicator.__new__(cuda_communicator.CudaCommunicator)
    ...
    monkeypatch.setattr(cuda_communicator, "should_nccl_symm_mem_allreduce", lambda *_: False)
...
def test_group_destroy_releases_communicator_before_process_groups(monkeypatch):
    ...
    coordinator.device_communicator = SimpleNamespace(destroy=lambda: events.append("communicator"))
