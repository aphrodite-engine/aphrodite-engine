...
from aphrodite.distributed.device_communicators import flashinfer_all_reduce
...
def _make_group_for_unit_test(rank_in_group: int = 0, world_size: int = 2) -> GroupCoordinator:
...
def test_cuda_communicator_checkpoints_flashinfer_workspaces(
    ...
    unique_workspaces = [normal_workspace] if aliased else [normal_workspace, quant_workspace]

    monkeypatch.setattr(flashinfer_all_reduce, "_fi_ar_workspace", normal_workspace)
    monkeypatch.setattr(flashinfer_all_reduce, "_fi_ar_quant_workspace", quant_workspace)
    monkeypatch.setattr(
        flashinfer_all_reduce,
        "_fi_ar_workspace_groups",
        {id(workspace): group for workspace in unique_workspaces},
    )
    monkeypatch.setattr(flashinfer_all_reduce, "TorchDistBackend", lambda group: group, raising=False)
...
def test_flashinfer_standalone_size_tuning(
    ...
    assert flashinfer_all_reduce._get_tuned_standalone_max_size(world_size, backend, Mock()) == expected


@pytest.mark.parametrize(("enabled", "expected"), [(True, 4681), (False, 128)])
def test_flashinfer_standalone_workspace_size(monkeypatch: pytest.MonkeyPatch, enabled: bool, expected: int) -> None:
    create_workspace = Mock(return_value=Mock(backend="mnnvl"))
    monkeypatch.setattr(flashinfer_all_reduce.envs, "APHRODITE_ALLREDUCE_USE_FLASHINFER", enabled)
...
def test_flashinfer_all_reduce_precedes_nccl(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "aphrodite.distributed.device_communicators.cuda_communicator.should_nccl_symm_mem_allreduce",
        nccl_selector,
    )
