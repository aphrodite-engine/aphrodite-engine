...
from aphrodite.model_executor.layers.quantization.compressed_tensors.compressed_tensors_moe.compressed_tensors_moe_w4a4_nvfp4 import (  # noqa: E501
...
def _make_raw_weights(device: torch.device, generator: torch.Generator) -> dict[str, torch.Tensor]:
    ...
    def block_scale(*shape: int) -> torch.Tensor:
        return torch.randint(0, 128, shape, dtype=torch.uint8, device=device, generator=generator).view(
            torch.float8_e4m3fn
        )
...
def _assert_tensors_equal(actual: torch.Tensor, expected: torch.Tensor, name: str) -> None:
...
def test_nvfp4_eplb_rearrangement_matches_reload(backend: str) -> None:
    ...
    with set_current_aphrodite_config(AphroditeConfig()):
        ...
        for name, param in layer.named_parameters():
            ...
            assert param.shape[0] == NUM_EXPERTS, f"{name} is not expert-major: {tuple(param.shape)}"
