...
def _supports_multimem():
    ...
    return current_platform.is_cuda() and capability is not None and capability.major >= 9
...
def _prepare_rank_state(fa, epoch_row, region_offset, epoch, stage, peer_rank, peer_payload):
    ...
    slot = _stage_slot(buffer, fa.mnnvl_buffer_size, region_offset, stage, peer_rank, slot_size)
...
def _run_all_gather_retention_case(fa, rank, local_rank, base_epoch, device):
    ...
    checks = _all_ranks_agree([fast_output_ok, publication_ok, slow_output_ok, retained], device)
...
def _run_epoch_wrap_case(fa, rank, local_rank, seed_epoch, device):
    ...
    rs_input = torch.cat((_payload(rank + 1, device), _payload((rank + 1) * 10, device)))
...
def _run_stage_cleanup_test(
    ...
    with monkeypatch.context() as m:
        ...
        m.setenv("APHRODITE_ALLREDUCE_USE_SYMM_MEM", "1")
