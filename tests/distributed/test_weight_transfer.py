...
from aphrodite.distributed.weight_transfer.ipc_engine import (
    IPCTrainerInitInfo,
...
from aphrodite.distributed.weight_transfer.nccl_engine import (
    NCCLTrainerInitInfo,
...
from aphrodite.distributed.weight_transfer.packed_tensor import (
    DEFAULT_PACKED_BUFFER_SIZE_BYTES,
    DEFAULT_PACKED_NUM_BUFFERS,
)
from aphrodite.distributed.weight_transfer.sparse_nccl_engine import (
...
def inference_receive_tensor(
    ...
    recorder = Recorder()
    engine = NCCLWeightTransferEngine(config, aphrodite_config, torch.device("cuda"), recorder)
...
def test_sparse_nccl_checkpoint_chunks_to_ep_local_experts_cpu(monkeypatch):
    ...
    wire_payloads = []
    sender.model_update_group.broadcast.side_effect = lambda tensor, **_: (wire_payloads.append(tensor.clone()))
    ...
    for ep_rank, expected_gate in enumerate(expected_gates):
        ...
        receiver = SparseNCCLWeightTransferEngine(
            ...
            create_mock_aphrodite_config(rank=ep_rank, world_size=2),
            torch.device("cpu"),
            model,
        )
        ...
        receiver.model_update_group.broadcast.side_effect = lambda tensor, payloads=payloads, **_: tensor.copy_(
            next(payloads)
        )
...
class TestIPCWeightTransferUpdateInfoValidation:
    ...
    def test_pickled_requires_insecure_serialization_flag(self, monkeypatch):
        monkeypatch.setenv("APHRODITE_ALLOW_INSECURE_SERIALIZATION", "0")

        with pytest.raises(ValueError, match="APHRODITE_ALLOW_INSECURE_SERIALIZATION=1"):
            IPCWeightTransferUpdateInfo(
                names=[],
                dtype_names=[],
                shapes=[],
...
def inference_receive_ipc_tensor(
    ...
    if mode == "http":
        os.environ["APHRODITE_ALLOW_INSECURE_SERIALIZATION"] = "1"
    ...
    recorder = Recorder()
    engine = IPCWeightTransferEngine(config, aphrodite_config, device, recorder)
    # Transport-only test: bypass the set_current_aphrodite_config context that
    # receive_weights enters, since aphrodite_config here is a mock.
    import aphrodite.config as _aphrodite_config_mod
...
def _module_with(*pairs):
    """A tiny nn.Module exposing parameters for ModuleSource tests."""
...
class TestTrainerClients:
    ...
    def test_http_client_pickles_ipc_handles_for_json(self, monkeypatch):
        ...
        assert pickle.loads(base64.b64decode(sent["ipc_handles_pickled"])) == [{"gpu": ("args",)}]

        client.update_weights([{"ipc_handles": {"gpu": ("args",)}}, {"names": []}])
        sent = captured["json"]["update_info"]
        assert sent[1] == {"names": []}
        assert pickle.loads(base64.b64decode(sent[0]["ipc_handles_pickled"])) == {"gpu": ("args",)}
...
class TestModuleSource:
    ...
    def test_metadata_reads_shape_and_dtype(self):
        source = ModuleSource(_module_with(("w", torch.zeros(2, 3)), ("b", torch.zeros(3))))
...
class TestWeightSourceGroupContract:
    ...
    def test_groups_order_follows_metadata_not_the_declaration(self):
        ...
        names = ["embed.w", "model.layers.0.a", "model.layers.1.a", "norm.w"]
        source = self._source(names, held=["model.layers.1.a", "model.layers.0.a", "model.layers.1.a"])
...
class TestDeferredProcessingContract:
    ...
    def _engines(self):
        return {name: loader() for name, loader in WeightTransferEngineFactory._registry.items()}
...
def test_nccl_trainer_init_ships_worker_init_info(monkeypatch):
    ...
    import aphrodite.distributed.weight_transfer.nccl_engine as nccl_engine_mod

    # Bypass the real NCCL rendezvous.
    monkeypatch.setattr(nccl_engine_mod, "open_trainer_endpoint", lambda info: MagicMock())
...
def test_nccl_worker_learns_wire_params_from_init_handshake(monkeypatch):
    ...
    monkeypatch.setattr(nccl_engine_mod, "worker_init_process_group", lambda info, pc: MagicMock())
...
def _mock_group_engine(source, monkeypatch, **kwargs):
    ...
    engine = NCCLTrainerWeightTransferEngine(client=RecordingClient(), source=source, packed=False, **kwargs)
    engine.model_update_group = MagicMock()
...
def test_nccl_trainer_init_requires_source():
    ...
    with pytest.raises(ValueError, match="requires a WeightSource"):
        NCCLTrainerWeightTransferEngine.trainer_init(
            NCCLTrainerInitInfo(master_address="127.0.0.1", master_port=29500, world_size=2, rank=0),
            client=RecordingClient(),
        )
...
def test_nccl_trainer_send_weights_rejects_truncated_source(monkeypatch):
    ...
    engine = _mock_group_engine(_ScriptedSource(meta, [("w", torch.zeros(4))]), monkeypatch)
...
def test_nccl_trainer_send_weights_raises_instead_of_hanging(monkeypatch):
    ...
    engine = NCCLTrainerWeightTransferEngine(client=_BlockingClient(), source=_FailingSource(), packed=False)
    engine.model_update_group = MagicMock()
    monkeypatch.setattr(torch.cuda, "current_stream", MagicMock())
...
def test_sparse_nccl_trainer_init_ships_worker_init_info(monkeypatch):
    ...
    import aphrodite.distributed.weight_transfer.sparse_nccl_engine as sparse_mod
