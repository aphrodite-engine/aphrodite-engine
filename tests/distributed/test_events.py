...
def test_replay_mechanism(publisher, subscriber):
    ...
    assert len(replayed) == 9, f"Expected 9 replayed messages (seq 10-18), got {len(replayed)}"
...
def test_buffer_limit(publisher, subscriber, publisher_config):
    ...
    assert len(replayed) == buffer_size, f"Expected {buffer_size} replayed messages, got {len(replayed)}"

    seqs = [seq for seq, _ in replayed]
    assert seqs == list(range(10, buffer_size + 10)), "Should replay seq 11 through buffer_size+10"
