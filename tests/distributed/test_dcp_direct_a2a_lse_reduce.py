...
import aphrodite.v1.attention.ops.cp_common as cp_common
...
def _assert_q_gather_matches_reference(
    ...
    remaining_storage_bytes = actual.untyped_storage().nbytes() - actual.storage_offset() * actual.element_size()
    required_storage_bytes = local_query.shape[0] * storage_num_heads * local_query.shape[2] * actual.element_size()
...
class TestDirectDCPGating:
    @pytest.mark.parametrize(
        ("flag_name", "factory_name", "workspace_name", "factory_args", "gate_name"),
        [
            (
                "APHRODITE_USE_DIRECT_DCP_A2A",
            ...
            (
                "APHRODITE_USE_DIRECT_DCP_Q_GATHER",
            ...
            (
                "APHRODITE_USE_DIRECT_DCP_KV_GATHER",
    ...
    def test_auto_workspace_init_failure_falls_back(
        self,
        monkeypatch,
        flag_name,
        factory_name,
        ...
        monkeypatch.setattr(dcp, gate_name, lambda *_args, **_kwargs: True)
        monkeypatch.setattr(dcp, workspace_name, MagicMock(side_effect=RuntimeError("unsupported")))
        factory = getattr(dcp, factory_name)
        factory.cache_clear()

        assert factory(_FakeGroupCoordinator(), torch.device("cpu"), *factory_args) is None

    def test_forced_workspace_init_failure_raises(self, monkeypatch):
        monkeypatch.setenv("APHRODITE_USE_DIRECT_DCP_A2A", "1")
        monkeypatch.setattr(dcp, "direct_cp_enabled", lambda *_args, **_kwargs: True)
        monkeypatch.setattr(
            dcp,
            "DirectDCPA2AWorkspace",
            MagicMock(side_effect=RuntimeError("unsupported")),
        )
    ...
    def test_env_disabled_returns_none(self, monkeypatch):
        monkeypatch.setenv("APHRODITE_USE_DIRECT_DCP_A2A", "0")
        dcp.get_direct_dcp_a2a_workspace.cache_clear()
    ...
    def test_zero_ubatches_raises(self):
        with pytest.raises(ValueError, match="ubatch"):
            dcp.DirectDCPA2AWorkspace(None, torch.device("cpu"), 16, 2, 32, torch.bfloat16, num_ubatches=0)

    def test_auto_with_unsupported_dtype_returns_none(self, monkeypatch):
        monkeypatch.delenv("APHRODITE_USE_DIRECT_DCP_A2A", raising=False)
        dcp.get_direct_dcp_a2a_workspace.cache_clear()
        workspace = dcp.get_direct_dcp_a2a_workspace(
    ...
    def test_q_gather_env_disabled_returns_none(self, monkeypatch):
        monkeypatch.setenv("APHRODITE_USE_DIRECT_DCP_Q_GATHER", "0")
        monkeypatch.setenv("APHRODITE_USE_DIRECT_DCP_A2A", "1")
    ...
    def test_q_gather_flag_is_independent(self, monkeypatch):
        monkeypatch.setenv("APHRODITE_USE_DIRECT_DCP_Q_GATHER", "1")
        monkeypatch.setenv("APHRODITE_USE_DIRECT_DCP_A2A", "0")
    ...
    def test_kv_gather_env_disabled_returns_none(self, monkeypatch):
        monkeypatch.setenv("APHRODITE_USE_DIRECT_DCP_KV_GATHER", "0")
        monkeypatch.setenv("APHRODITE_USE_DIRECT_DCP_A2A", "1")
    ...
    def test_kv_gather_flag_is_independent(self, monkeypatch):
        monkeypatch.setenv("APHRODITE_USE_DIRECT_DCP_KV_GATHER", "1")
        monkeypatch.setenv("APHRODITE_USE_DIRECT_DCP_A2A", "0")
        monkeypatch.setattr(cp_common, "_symm_mem_spans_group", lambda group: True)
        dcp.get_direct_dcp_kv_gather_workspace.cache_clear()
        workspace = object()
    ...
    def test_kv_gather_rejects_invalid_workspace_geometry(self):
        with pytest.raises(ValueError, match="ubatch"):
            dcp.DirectDCPKVGatherWorkspace(None, torch.device("cpu"), 64, 576, num_ubatches=0)
        with pytest.raises(ValueError, match="divide evenly"):
            dcp.DirectDCPKVGatherWorkspace(_FakeProcessGroup(), torch.device("cpu"), 63, 576)
        with pytest.raises(ValueError, match="16-byte"):
            dcp.DirectDCPKVGatherWorkspace(_FakeProcessGroup(), torch.device("cpu"), 64, 3)

    def test_q_gather_rejects_invalid_workspace_geometry(self):
        with pytest.raises(ValueError, match="ubatch"):
            dcp.DirectDCPQGatherWorkspace(None, torch.device("cpu"), 16, 2, 32, num_ubatches=0)
...
def test_mla_dcp_manager_selects_direct_backends(monkeypatch):
    import aphrodite.v1.attention.ops.dcp as dcp_manager
    ...
    direct_kv = MagicMock()
    monkeypatch.setattr(dcp_manager, "get_direct_dcp_a2a_workspace", MagicMock(return_value=direct_a2a))
    monkeypatch.setattr(
        dcp_manager,
        "get_direct_dcp_q_gather_workspace",
    ...
    manager = dcp_manager.MLADCPManager(
        aphrodite_config=_manager_config(),
    ...
    assert manager.query_gather(query) == direct_query.gather.return_value
...
def test_mla_dcp_manager_selects_fallback_backends(monkeypatch):
    ...
    monkeypatch.setattr(dcp_manager, "get_dcp_group", lambda: group)
    monkeypatch.setattr(dcp_manager, "get_direct_dcp_a2a_workspace", MagicMock(return_value=None))
    monkeypatch.setattr(dcp_manager, "get_direct_dcp_q_gather_workspace", MagicMock(return_value=None))
    monkeypatch.setattr(dcp_manager, "get_direct_dcp_kv_gather_workspace", MagicMock(return_value=None))
...
def test_mla_dcp_manager_selects_pcp_combine(monkeypatch):
    ...
    manager = dcp_manager.MLADCPManager(
        aphrodite_config=_manager_config(dcp_comm_backend="ag_rs"),
        device=torch.device("cpu"),
        num_heads=2,
        query_head_dim=8,
        output_head_dim=4,
...
def test_sparse_mla_builder_initializes_dcp_manager(monkeypatch):
    import aphrodite.model_executor.layers.attention.sparse_mla_attention as sparse_mla
...
def test_sparse_mla_workspace_preserves_non_dcp_size():
    from aphrodite.model_executor.layers.attention.sparse_mla_attention import (
    ...
    assert SparseMLACommonMetadataBuilder.determine_chunked_prefill_workspace_size(config) == 21
...
def _distributed_direct_q_gather_worker(env: dict[str, str]) -> None:
    ...
    try:
        ...
        for dtype_idx, dtype_name in enumerate(("bfloat16", "float8_e4m3fn", "float32")):
            ...
            cases = ((1, False), (128, True), (17, True), (5, False)) if dtype == torch.bfloat16 else ((17, True),)
            for iteration, (num_tokens, noncontiguous) in enumerate(cases):
                ...
                source_num_heads = heads_per_rank + 2 if noncontiguous else heads_per_rank
                ...
                assert actual.data_ptr() == workspace.final_query[active_ubatch[0]].data_ptr()
                assert workspace.completion[active_ubatch[0]].numel() == 1
                _assert_q_gather_matches_reference(actual, local_query, world_size, padded_num_heads)
            ...
            torch.accelerator.synchronize()
            _assert_q_gather_matches_reference(eager_output, eager_input, world_size, padded_num_heads)
            ...
            for replay in range(2):
                ...
                graph.replay()
                torch.accelerator.synchronize()
                assert int(workspace.epoch[1].item()) == (epoch_before_replays + replay + 1)
            ...
            _assert_q_gather_matches_reference(captured_output, captured_input, world_size, padded_num_heads)
    finally:
...
def _distributed_direct_kv_gather_worker(env: dict[str, str]) -> None:
    ...
    try:
        ...
        for dtype_idx, dtype_name in enumerate(("bfloat16", "float16")):
            ...
            storage = torch.zeros((world_size + 1) * 128, token_dim, device=device, dtype=dtype)
...
def _distributed_direct_a2a_worker(env: dict[str, str]) -> None:
    ...
    try:
        ...
        def check(num_tokens: int, iteration: int, padded: bool) -> None:
            ...
            gathered_output = [torch.empty_like(reference_output) for _ in range(world_size)]
            ...
            lses = torch.stack(
                [value[:, rank * heads_per_rank : (rank + 1) * heads_per_rank] for value in gathered_lse]
            )
            expected = _lse_weighted_combine(outputs, lses, is_lse_base_on_e=is_lse_base_on_e)
        ...
        def check_empty_shards(query_lens: list[int], iteration: int) -> None:
            ...
            seq_lens = torch.tensor(
                [0 if is_empty(seq_idx, rank) else seq_idx + 3 for seq_idx in range(num_seqs)],
                dtype=torch.int32,
                device=device,
            )
            ...
            outputs = torch.stack([value[:, head_slice, :] for value in gathered_output]).float()
            ...
            all_empty = torch.tensor(
                [all(is_empty(seq_idx, r) for r in range(world_size)) for seq_idx in range(num_seqs)],
                device=device,
            ).repeat_interleave(query_lens_tensor)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            2,
            marks=pytest.mark.skipif(torch.accelerator.device_count() < 2, reason="Need at least 2 GPUs."),
        ),
        pytest.param(
            4,
            marks=pytest.mark.skipif(torch.accelerator.device_count() < 4, reason="Need at least 4 GPUs."),
        ),
    ],
)
