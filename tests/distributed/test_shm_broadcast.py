...
def worker_fn_tensor_broadcast():
    rank = dist.get_rank()
    writer_rank = 0
    message_queue = MessageQueue.create_from_process_group(dist.group.WORLD, 8 * 1024 * 1024, 4, writer_rank)
    ...
    if rank == writer_rank:
        with mock.patch(
            "aphrodite.distributed.device_communicators.shm_broadcast._reduce_tensor",
...
def _dumps_oob(obj) -> tuple[bytes, list]:
    """Pickle `obj` like `MessageQueue.enqueue`: tensor dispatch + OOB buffers."""
    buffers = []
    ...
    bio = io.BytesIO()
    pickler = pickle.Pickler(bio, protocol=pickle.HIGHEST_PROTOCOL, buffer_callback=callback)
...
def test_reduce_tensor_fallback(case: str):
    """Tensors without safe aliases must fall back to torch's default reduction."""
    if case == "cuda":
...
def test_reader_timeout_caps_indefinite_waits(should_warn):
    with (
        mock.patch(
            "aphrodite.distributed.device_communicators.shm_broadcast.SHM_READER_RECHECK_INTERVAL_MS",
            new=7,
        ),
        mock.patch(
            "aphrodite.distributed.device_communicators.shm_broadcast.APHRODITE_RINGBUFFER_WARNING_INTERVAL",
            new=60,
        ),
    ):
...
def test_check_shm_free_space_raises_when_insufficient(tmp_path):
    with (
        mock.patch.object(shm_broadcast.shutil, "disk_usage", return_value=_fake_disk_usage(32 << 20)),
...
def test_check_shm_free_space_passes_when_sufficient(tmp_path):
    with mock.patch.object(shm_broadcast.shutil, "disk_usage", return_value=_fake_disk_usage(512 << 20)):
        check_shm_free_space(240 << 20, shm_path=str(tmp_path))
...
def test_shm_ring_buffer_creation_checks_free_space():
    with (
        mock.patch.object(shm_broadcast.shutil, "disk_usage", return_value=_fake_disk_usage(1 << 20)),
...
def test_remote_subscribe_addr_unique_concurrent_writers(
    ...
    monkeypatch.setattr(shm_broadcast, "get_open_port", lambda: colliding_port, raising=False)
