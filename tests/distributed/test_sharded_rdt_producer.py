...
import aphrodite.distributed.weight_transfer.sharded_rdt_trainer as trainer_mod
from aphrodite.distributed.weight_transfer.sharded_rdt_common import (
    buffer_alloc_bytes,
)
from aphrodite.distributed.weight_transfer.sharded_rdt_trainer import (
...
def server_factory(monkeypatch):
    ...
    """
    monkeypatch.setattr(torch.accelerator, "current_device_index", lambda *a, **kw: 0, raising=False)
...
class TestFreeBarrier:
    ...
    def test_a_signal_that_beats_its_publish_is_completed_at_publish(self, server_factory):
    ...
    def test_early_signals_from_every_consumer_complete_at_publish(self, server_factory):
        server = server_factory()
        server.begin_sync(3)
        for _ in range(3):
            server.free_group(GI_A)
...
class TestGatherCredit:
    ...
    def test_lookahead_one_holds_at_most_two_groups_resident(self, server_factory, gather_engine):
        ...
        def _consumer():
            for gi in range(n_groups):
                ...
                while time.monotonic() < deadline:
                    ...
                    failures.append(f"group {target} never published while {gi} was unfreed")
                    return
        ...
        assert engine._inflight.max_len <= 2, f"{engine._inflight.max_len} groups were resident at lookahead=1"
        assert engine._inflight == {} and server._cache == {}, "everything must be freed by end_sync"

    def test_lookahead_zero_holds_exactly_one_group(self, server_factory, gather_engine):
    ...
    def test_the_residency_bound_scales_with_the_lookahead(self, server_factory, gather_engine):
    ...
    def test_a_gather_error_releases_a_blocked_wait_freed(self, server_factory):
        ...
        assert not done.wait(timeout=0.3), "wait_freed must block while nothing is freed"
...
class TestEndSync:
    def test_end_sync_returns_immediately_when_everything_is_freed(self, server_factory):
        server = server_factory()
        server.begin_sync(1)
        _publish(server, GI_A, GROUP_A)
        server.free_group(GI_A)
...
class TestBeginSync:
    ...
    def test_a_straggler_signal_cannot_credit_the_next_sync(self, server_factory):
        ...
        _publish(server, GI_A, GROUP_A)
        server.free_group(GI_A)
        assert server._inflight_groups == [GI_A], "one signal after the reset must not release"
    ...
    def test_the_packed_destination_cache_survives_the_sync_boundary(self, server_factory):
...
class TestStallWatchdog:
    ...
    def test_a_blocked_wait_freed_fails_instead_of_hanging(self, server_factory):
        ...
        assert done.wait(timeout=10), "wait_freed must give up rather than block forever"
...
class TestConcurrentPublishAndFree:
    def test_the_lifecycle_survives_concurrent_publishes_and_signals(self, server_factory):
    ...
    def test_concurrent_signals_of_one_group_release_it_exactly_once(self, server_factory):
...
class TestExportRing:
    ...
    def test_a_reused_slot_reallocates_and_still_rebuilds(self):
        ...
        rebuilt = torch.as_strided(eng._export_ring[0].view(getattr(torch, dt)), shape, stride, off)
