...
def test_sp16_dispatches_only_to_mnnvl_lamport(
    ...
    assert not comm.should_custom_all_gather(torch.empty((131073, 8), dtype=torch.bfloat16))

    comm.mnnvl_only = False
    assert not comm.should_custom_all_gather(torch.empty((8, 8), dtype=torch.bfloat16))
    assert not comm.should_custom_reduce_scatter(torch.empty((16, 8), dtype=torch.bfloat16))
...
def graph_allreduce(
    ...
    with monkeypatch.context() as m:
        ...
        with graph_capture(device=device) as graph_capture_context:
            local = torch.full((512, 4096), tp_rank + 1, dtype=torch.bfloat16, device=device)
        ...
        assert gathered is not None
        assert scattered is not None
        expected_gather = torch.cat([torch.full_like(local, peer_rank + 1) for peer_rank in range(tp_size)])
...
def eager_allreduce(
    ...
    with monkeypatch.context() as m:
        ...
        for dtype in [torch.float32, torch.float16, torch.bfloat16]:
            ...
            expected_gather = torch.empty((64 * tp_size, 4096), dtype=dtype, device=device)
            dist.all_gather_into_tensor(expected_gather, local, group=group)
            gathered = fa.custom_all_gather(local)
            assert gathered is not None
            torch.testing.assert_close(gathered, expected_gather)

            reduce_input = torch.full((64 * tp_size, 4096), tp_rank + 1, dtype=dtype, device=device)
            expected_scatter = torch.empty((64, 4096), dtype=dtype, device=device)
            dist.reduce_scatter_tensor(expected_scatter, reduce_input.clone(), group=group)
            scattered = fa.custom_reduce_scatter(reduce_input)
            assert scattered is not None
            torch.testing.assert_close(scattered, expected_scatter)
