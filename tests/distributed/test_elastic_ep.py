...
def _scale_with_traffic(
    ...
    with ThreadPoolExecutor(max_workers=len(clients)) as executor:
        futures = [
            executor.submit(_traffic_loop, server, rank, ready, stop, finished, is_probe) for rank, is_probe in clients
        ]
        try:
            ready.wait(timeout=120)
...
def test_elastic_ep_scaling(enforce_eager: bool, traffic_mode: str):
    from aphrodite.distributed.eplb.eplb_communicator import has_nixl

    if not has_nixl():
        pytest.skip("Async EPLB with elastic EP requires NIXL (not installed)")

    initial_dp_size = int(os.getenv("APHRODITE_TEST_ELASTIC_EP_INITIAL_DP", "2"))
    target_dp_size = int(os.getenv("APHRODITE_TEST_ELASTIC_EP_TARGET_DP", "4"))
    assert target_dp_size > initial_dp_size
    aphrodite_serve_args = _base_serve_args(initial_dp_size, enforce_eager)
...
def test_elastic_ep_scaling_uneven():
    ...
    aphrodite_serve_args = _base_serve_args()
