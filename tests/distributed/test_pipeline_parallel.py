...
class PPTestSettings:
    ...
    # aphrodite_major_versions should be the same, and they
    # are first zipped together to iterate over all
    # test settings.
    distributed_backends: list[str]
    # aphrodite major version: "0" for V0, "1" for V1
    aphrodite_major_versions: list[str]
    task: TaskOption
    test_options: PPTestOptions

    def __post_init__(self):
        if len(self.distributed_backends) != len(self.aphrodite_major_versions):
            raise ValueError(
                f"Length mismatch: distributed_backends "
                f"({len(self.distributed_backends)}) != "
                f"aphrodite_major_versions ({len(self.aphrodite_major_versions)})")
    ...
    def detailed(
        *,
        ...
        return PPTestSettings(
            ...
            aphrodite_major_versions=["0", "0", "1"],
            task=task,
    ...
    def iter_params(self, model_id: str):
        ...
        for parallel_setup in self.parallel_setups:
            for backend, aphrodite_major_version in zip(self.distributed_backends,
                                                   self.aphrodite_major_versions):
                yield (model_id, parallel_setup, backend, aphrodite_major_version,
                       self.task, opts)
...
def _compare_tp(
    ...
    *,
    ...
    if distributed_backend == "ray" and (aphrodite_major_version == "1"
                                         or specific_case):
        # For V1, test Ray Compiled Graph for all the tests
        # For V0, test Ray Compiled Graph for a subset of the tests
        pp_env = {
            "APHRODITE_USE_V1": aphrodite_major_version,
    ...
    try:
        ...
        if testing_ray_compiled_graph and aphrodite_major_version == "0":
...
@pytest.mark.parametrize(
    ("model_id", "parallel_setup", "distributed_backend", "aphrodite_major_version",
     "task", "test_options"),
    [
