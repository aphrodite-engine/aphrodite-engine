...
def run_evalscope(eval_config: dict[str, Any], base_url: str, work_dir: Path) -> dict[str, float]:
...
def test_qwen4_exp_accuracy(config_filename: Path, tmp_path: Path):
    ...
    for dataset, metric_config in eval_config["datasets"].items():
        ...
        print(f"{dataset}: measured={score:.4f}, expected={threshold:.4f}, tolerance={tolerance:.4f}")
        assert score >= minimum_score, (
            f"{dataset} score too low: {score:.4f} < {threshold:.4f} - {tolerance:.4f} = {minimum_score:.4f}"
        )
