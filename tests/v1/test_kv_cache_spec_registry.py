...
from aphrodite.v1.kv_cache_spec_registry import (
...
class TestGetKVCacheSpecKind:
    ...
    def test_homogeneous_group_keeps_member_kind(self, spec_cls, expected_kind):
        ...
        spec = make_spec(spec_cls)
        group = UniformTypeKVCacheSpecs(block_size=64, kv_cache_specs={"layer_0": spec, "layer_1": spec})
    ...
    def test_mixed_kinds_sharing_full_attention_base(self, other_cls):
        ...
        assert len({get_kv_cache_spec_kind(s) for s in group.kv_cache_specs.values()}) == 2
        assert {KVCacheSpecRegistry.get_uniform_type_base_spec(s) for s in group.kv_cache_specs.values()} == {
            FullAttentionSpec
        }
    ...
    def test_mixed_base_specs_report_unknown(self):
        ...
        assert len({KVCacheSpecRegistry.get_uniform_type_base_spec(s) for s in group.kv_cache_specs.values()}) == 2
