...
def run_chat_completion(
    ...
    Args:
        base_url: Base URL of the Aphrodite server
    ...
    """
    with openai.OpenAI(api_key="EMPTY", base_url=base_url, timeout=120, max_retries=0) as client:
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The Aphrodite service URL (e.g., http://localhost:8000)",
    )
    ...
    if args.use_mm_prompts:
        test_prompts = SAMPLE_PROMPTS_MM[:1] if args.skip_two_image_prompt else SAMPLE_PROMPTS_MM
    ...
    if args.mm_smoke_test:
        ...
        cases = [
            (
                "Single image",
                [image],
                "What word is on the red road sign? Reply with only that word in uppercase.",
                "STOP",
            ),
            (
                "Two different images",
                image_pair,
                "Do these pictures contain both flowers and birds? Reply with only YES or NO.",
                "YES",
            ),
            (
                "Same image twice",
    ...
    def complete(prompt_data):
        ...
        if args.mm_smoke_test:
            ...
            assert output == prompt_data["expected"], (
                f"{prompt_data['description']}: expected {prompt_data['expected']!r}, got {output!r}"
            )
        return output
