...
def _started(data: MagicMock | None = None, deadline: float | None = None) -> ConsumerXfer:
...
def _xfer_req(mm_hash: str = "h1", session_id: str = "sess-1") -> XferReq:
    return XferReq(mm_hash=mm_hash, compatibility_hash="hash-abc", session_id=session_id)
...
def test_consumer_xfer_handle_ack_nack_returns_false():
    data = _make_data()
    x = _xfer(data)
    assert x.handle_ack(XferAck(mm_hash="h1", status=XferStatus.NACK_MISSING), "a") is False
...
def test_consumer_xfer_poll_waiting_ack_timeout():
    assert _xfer(deadline=time.monotonic() - 1).poll(time.monotonic()) == XferState.ACK_TIMEOUT
