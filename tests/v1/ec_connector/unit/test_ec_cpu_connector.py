...
from aphrodite.config import ECTransferConfig
...
def _latency_test(llm: LLM) -> None:
    ...
    assert num_times_ec_better_than_cold >= 0.8 * num_tests, (
        f"EC hit was faster only {num_times_ec_better_than_cold}/{num_tests} times (expected >= 80%)"
    )
...
def _accuracy_test(llm: LLM) -> None:
    ...
    assert success_count >= 0.5 * test_count, (
        f"Only {success_count}/{test_count} outputs matched baseline (expected >= 50%). Baseline: {baseline_text!r}"
    )
