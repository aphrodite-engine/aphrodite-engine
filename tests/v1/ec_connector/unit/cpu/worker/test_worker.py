...
from aphrodite.distributed.ec_transfer.ec_connector.cpu.ec_shared_region import (
    ECSharedRegion,
)
from aphrodite.distributed.ec_transfer.ec_connector.cpu.worker import ECCPUWorker
from aphrodite.platforms import current_platform
from tests.v1.ec_connector.unit.utils import create_ec_aphrodite_config
...
_requires_swap_blocks_batch = pytest.mark.skipif(
    ...
    reason=(
        "installed aphrodite C++ extension predates the swap_blocks_batch op "
...
def _aphrodite_config(rank: int = 0) -> Mock:
    return create_ec_aphrodite_config(rank=rank, dtype=_DTYPE)
...
def _meta(*, saves: dict | None = None, loads: dict | None = None) -> ECCPUConnectorMetadata:
    ...
    return ECCPUConnectorMetadata(
        saves=saves or {},
        loads={mm_hash: (idx, block_ids) for idx, (mm_hash, block_ids) in enumerate((loads or {}).items())},
    )
...
def make_worker():
    ...
    def factory(
        *,
        ...
        with (
            patch(
                "aphrodite.distributed.ec_transfer.ec_connector.cpu.worker.create_ec_shared_region",
                return_value=region,
            ),
            patch(
                "aphrodite.distributed.ec_transfer.ec_connector.cpu.worker.is_pin_memory_available",
                return_value=pin_memory_available,
            ),
            patch(
                "aphrodite.distributed.ec_transfer.ec_connector.cpu.worker.get_tensor_model_parallel_rank",
                return_value=tp_rank,
            ),
            patch(
                "aphrodite.distributed.ec_transfer.ec_connector.cpu.worker.get_pcp_group",
                return_value=pcp_group,
            ),
        ):
            worker = ECCPUWorker(_aphrodite_config(rank=rank))
...
def _wait_for_completion(worker: ECCPUWorker, expected, direction: str, timeout_s: float = 30.0) -> None:
...
def test_save_caches_coalesces_only_adjacent_blocks(make_worker, n_elements, block_ids, expected_sizes):
...
def test_save_descriptors_hold_high_bit_source_addresses(make_worker):
    ...
    worker.save_caches({"a": _HighAddress(real, address)}, "a", _meta(saves={"a": [1, 5]}))
...
def test_start_load_caches_copies_with_correct_shape_dtype_and_bytes(make_worker):
    ...
    out = encoder_cache["h"]
    assert out.device.type == DEVICE_TYPE, "consumer worker must place the tensor on the accelerator"
...
def test_start_load_caches_coalesces_across_entries(make_worker, loads, expected_sizes):
    ...
    with patch(
        "aphrodite.distributed.ec_transfer.ec_connector.cpu.worker.swap_blocks_batch",
...
def test_save_then_load_round_trips_bytes(make_worker):
    ...
    src = torch.arange(n_blocks * _HIDDEN_DIM, dtype=_DTYPE, device=DEVICE_TYPE).reshape(n_blocks, _HIDDEN_DIM)
    worker.save_caches({"h": src}, "h", _meta(saves={"h": block_ids}))
    worker.flush_saves()
    _wait_for_completion(worker, "h", "saves")
...
def test_save_survives_encoder_cache_free_before_copy_runs(make_worker):
    ...
    block_ids = [0, 1, 2]
    saved = torch.full((len(block_ids), _HIDDEN_DIM), 1.0, dtype=_DTYPE, device=DEVICE_TYPE)
    ...
    overwrite = torch.empty((len(block_ids), _HIDDEN_DIM), dtype=_DTYPE, device=DEVICE_TYPE)
...
def test_init_pins_memory_when_available(pin_available, expected_pinned):
    ...
    with (
        ...
        ECCPUWorker(_aphrodite_config(rank=0))
...
def test_shutdown_calls_region_cleanup_and_swallows_errors(caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="aphrodite"):
...
def test_e2e_scheduler_worker_save_then_load(make_worker, monkeypatch):
    ...
    """
    import aphrodite.distributed.ec_transfer.ec_connector.cpu.scheduler as sched_mod
    from aphrodite.distributed.ec_transfer.ec_connector.cpu.scheduler import (
        ECCPUScheduler,
    )
    ...
    scheduler = ECCPUScheduler(_aphrodite_config())
