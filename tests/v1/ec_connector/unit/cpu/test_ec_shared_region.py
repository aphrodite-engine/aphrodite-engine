...
def test_madvise_success_selects_madvise_population(monkeypatch):
    ...
    """
    from aphrodite.distributed.ec_transfer.ec_connector.cpu import ec_shared_region as esr
...
def test_non_creator_does_not_probe_or_populate(monkeypatch):
    ...
    instance_id = str(uuid.uuid4())
    r1 = ECSharedRegion(engine_id=instance_id, num_blocks=4, block_size_bytes=mmap.PAGESIZE)
    try:
        calls: list[str] = []
        monkeypatch.setattr(
            esr,
        ...
        r2 = ECSharedRegion(engine_id=instance_id, num_blocks=4, block_size_bytes=mmap.PAGESIZE)
        try:
            assert not r2._is_creator
            assert calls == []
        finally:
...
def test_madvise_unexpected_oserror_propagates(monkeypatch):
    ...
    try:
        with pytest.raises(OSError) as exc_info:
            ECSharedRegion(engine_id=engine_id, num_blocks=4, block_size_bytes=mmap.PAGESIZE)
        ...
        with contextlib.suppress(FileNotFoundError):
            os.unlink(f"/dev/shm/aphrodite_ec_{engine_id}.mmap")
