...
def test_prefix_cache_hit_uses_per_group_dcp_geometry():
    ...
    blocks = manager.allocate_slots(req0, len(req0.prompt_token_ids), 0, computed_blocks)
...
def test_pure_swa_retention_interval_caches_sparse_tails():
    ...
    block_size = 16
    manager = _make_pure_swa_manager(block_size, sliding_window=block_size, retention_interval=64)
...
def test_pure_swa_retention_latest_only():
    ...
    block_size = 16
    manager = _make_pure_swa_manager(block_size, sliding_window=block_size, retention_interval=0)
...
def test_swa_reachable_block_mask_final_partial_segment():
    ...
    def reachable_blocks(final_segment_end_block: int, *, use_eagle: bool = False) -> set[int]:
...
def test_swa_reachable_block_mask_with_dcp_scaling():
    ...
    assert mask_no_dcp != mask_with_dcp, "DCP scaling should change retention granularity"
...
def test_swa_reachable_block_mask_sub_block_alignment_is_dense(block_size, dcp_world_size, alignment_tokens):
