...
def make_kv_cache_config_hybrid_model(block_size: int, num_blocks: int) -> KVCacheConfig:
    return KVCacheConfig(
        ...
        kv_cache_groups=[
            ...
            KVCacheGroupSpec(
                ["layer2"],
                SlidingWindowSpec(block_size, 1, 1, torch.float32, sliding_window=2 * block_size),
            ),
            KVCacheGroupSpec(
                ["layer3"],
                SlidingWindowSpec(block_size, 1, 1, torch.float32, sliding_window=2 * block_size),
...
def test_prefill(hash_fn):
    ...
    assert blocks is not None and blocks.get_block_ids() == ([7, 8, 9, 10, 4, 5, 6, 3, 2, 1],)
...
def test_prefill_hybrid_model():
    ...
    def test_partial_request_hit(
        ...
        req = make_request(request_id, common_token_ids + unique_token_ids, block_size, sha256)
...
def test_mm_prefix_caching():
    ...
    assert block_hashes[0] == sha256((kv_cache_utils.NONE_HASH, tuple(all_token_ids[:block_size]), ("aaa",)))
    ...
    assert block_hashes[3] == sha256((block_hashes[2], tuple(all_token_ids[3 * block_size :] + [8] * 5), ("ccc",)))
