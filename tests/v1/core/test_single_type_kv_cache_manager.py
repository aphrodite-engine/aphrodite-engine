...
def test_circular_buffer_allocates_one_block_for_the_request_lifetime():
    ...
    blocks = manager.allocate_new_blocks(request_id, num_tokens=128, num_tokens_main_model=128)
    assert len(blocks) == 1
...
def test_sliding_window_records_new_blocks_for_zeroing():
    ...
    block_pool = BlockPool(num_gpu_blocks=10, enable_caching=False, hash_block_size=block_size)
    ...
    blocks = manager.allocate_new_blocks("request", num_tokens=4, num_tokens_main_model=4)
...
def test_sliding_window_cache_hit_with_finer_hash_alignment():
    ...
    for block_hash, block in zip((block_hashes[1], block_hashes[3]), block_pool.blocks[10:12]):
        block_pool.cached_block_hash_to_block.insert(make_block_hash_with_group_id(block_hash, 0), block)
...
def test_get_num_blocks_to_allocate():
    ...
    assert manager.get_num_blocks_to_allocate("1", 20 * block_size, cached_blocks_1, 0, 0, 20 * block_size) == 20
    assert manager.get_num_blocks_to_allocate("2", 20 * block_size, cached_blocks_2, 0, 0, 20 * block_size) == 15
...
def test_predictor_matches_allocator_blocks_calculation_with_admission_cap():
    ...
    for num_tokens in (4, 8, 12, 16):
        predicted = manager.get_num_blocks_to_allocate(
            ...
            num_local_computed_tokens=total_computed,
            num_tokens_main_model=num_tokens,
        )
