...
def _split(
    ...
    stub = SimpleNamespace(
        ...
        hash_block_size=ATTN_BLOCK_SIZE,
        mamba_has_prefill_checkpoint_blocks=(num_prefill_checkpoint_blocks > 0 and not use_eagle),
    )
...
def test_internal_checkpoint_split(prompt_len: int, num_new_tokens: int, use_eagle: bool, expected: int) -> None:
...
def _run_chunked_prefill(manager: KVCacheManager, request: Request, budgets: list[int]) -> dict[int, int]:
    ...
    for step in range(len(budgets) + 64):
        ...
        if num_new == 0:
            continue
        assert manager.allocate_slots(request, num_new, num_lookahead_tokens=NUM_SPEC) is not None
...
def _count_cached_boundary_states(manager: KVCacheManager, request: Request, state_at: dict[int, int]) -> int:
    ...
    for pos, block in enumerate(mamba_manager.req_to_blocks[request.request_id]):
        ...
        assert state_at.get(block.block_id) == claimed, (
            f"mamba slot {pos} is hashed as state@{claimed} but holds state@{state_at.get(block.block_id)}"
        )
        checked += 1
    return checked
...
def test_unaligned_resume_never_runs_past_its_block(
    ...
    for end in ends[:-1]:
        ...
        assert aligned or (partial_hit and end == tail_boundary), (
            f"intermediate chunk end {end} is neither block-aligned nor the partial-tail boundary"
        )
