...
def _packing_by_layer(tensors: list[KVCacheTensor]) -> dict[str, tuple[int, int]]:
    return {layer_name: (tensor.offset, tensor.block_stride) for tensor in tensors for layer_name in tensor.shared_by}
...
def _make_page_group(prefix: str, page_sizes: list[int]) -> KVCacheGroupSpec:
    specs = {f"{prefix}.{i}": MagicMock(page_size_bytes=page_size) for i, page_size in enumerate(page_sizes)}
...
class TestInterleavedPacking:
    def test_compact_cache_overlays_fp32_state_group(self):
        ...
        assert {full_specs[f"indexer.{i}"].real_page_size_bytes for i in range(2)} == {4352}
        assert {state_specs[f"indexer_state.{i}"].page_size_bytes for i in range(2)} == {8192}

        full_group_bytes = 2 * (32768 + 4608)
        state_group_bytes = 2 * (32768 + 8192)
        bytes_per_block = max(full_group_bytes, state_group_bytes)
        num_blocks, tensors = _get_kv_cache_config_packed(_mock_aphrodite_config(), groups, bytes_per_block * 32)
    ...
    def test_deepseek_v4_pro_stride(self):
        ...
        num_blocks, tensors = _get_kv_cache_config_packed(_mock_aphrodite_config(), groups, expected_stride * 32)
    ...
    def test_offsets_within_one_block(self):
        ...
        _, tensors = _get_kv_cache_config_packed(_mock_aphrodite_config(), groups, 100 * 1024 * 1024)
        page_sizes = _page_sizes_by_layer(groups)
        packing = _packing_by_layer(tensors)
        for layer_name, page_size in page_sizes.items():
    ...
    def test_layouts_are_disjoint_within_each_group(self):
        ...
        for group in groups:
            ranges = sorted((packing[name][0], packing[name][0] + page_sizes[name]) for name in group.layer_names)
