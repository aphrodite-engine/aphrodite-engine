...
def _mla(head_size: int) -> MLAAttentionSpec:
    return MLAAttentionSpec(block_size=64, num_kv_heads=1, head_size=head_size, dtype=torch.uint8)


def _full() -> FullAttentionSpec:
    return FullAttentionSpec(block_size=16, num_kv_heads=2, head_size=64, dtype=torch.float16)
...
def _mock_aphrodite_config(layout: str | None):
    config = MagicMock()
...
def _shared_layout_config():
    config = _mock_aphrodite_config("BLNHC")
...
class TestCSALinearGrouping:
    ...
    def test_roles_land_in_separate_groups(self):
        ...
        assert scratch.layer_names == [_compressor_state_name(i) for i in range(NUM_CACHE_TUPLES)]
    ...
    def test_every_group_fits_one_packed_block(self):
        ...
        kv_cache_config = get_kv_cache_config_from_groups(config, groups, available_memory=bytes_per_block * 32)
    ...
    def test_scratch_group_survives_computed_block_truncation(self):
        ...
        groups = get_kv_cache_groups(config, _make_csa_linear_specs())
        kv_cache_config = get_kv_cache_config_from_groups(config, groups, available_memory=BYTES_PER_BLOCK * 64)
        ...
        blocks = manager.create_kv_cache_blocks(
            tuple(
                manager.block_pool.get_new_blocks(3) if group.kv_cache_spec.prefix_cacheable else []
                for group in scheduler_config.kv_cache_groups
            )
        )
        truncated = manager.truncate_computed_blocks(blocks, 48)
        scratch_index = next(
            i for i, group in enumerate(scheduler_config.kv_cache_groups) if not group.kv_cache_spec.prefix_cacheable
        )
    ...
    def test_prefix_hits_respect_compression_alignment(self):
        ...
        kv_cache_config = get_kv_cache_config_from_groups(config, groups, available_memory=8 * MAIN_KV_PAGE_BYTES)
    ...
    def test_mamba_split_measures_the_block_the_other_groups_already_force(self, wide):
        ...
        for i in range(10):
            specs[f"narrow.a.{i}"] = FullAttentionSpec(block_size=16, num_kv_heads=1, head_size=8, dtype=torch.uint8)
            specs[f"narrow.b.{i}"] = FullAttentionSpec(block_size=16, num_kv_heads=1, head_size=16, dtype=torch.uint8)
        for i in range(100):
            specs[f"gdn.{i}"] = MambaSpec(block_size=16, shapes=((512,),), dtypes=(torch.bfloat16,))
...
class TestDensePacking:
    def test_bytes_per_block_is_largest_group(self):
        ...
        assert _get_kv_cache_bytes_per_block(groups) == _expected_bytes_per_block(groups)

        config = get_kv_cache_config_from_groups(_mock_aphrodite_config("BLHNC"), groups, MEMORY)
    ...
    def test_allocation_is_layout_invariant(self, layout):
        ...
        config = get_kv_cache_config_from_groups(_mock_aphrodite_config(layout), groups, MEMORY)
    ...
    def test_single_group_mixed_pages_follows_layout(self, layout):
        ...
        assert {t.size for t in config.kv_cache_tensors} == {block_stride * config.num_blocks}
        if layout == "LBNHC":
            assert idx_tensor.offset == 2 * _mla(512).page_size_bytes * config.num_blocks
    ...
    def test_overlaid_groups_alias_and_stay_isolated(self):
        ...
        config = get_kv_cache_config_from_groups(_mock_aphrodite_config("BLNHC"), groups, MEMORY)
    ...
    def test_layer_compact_layout_rejected_for_overlaid_groups(self):
        ...
        # silent flip.
        groups, _, _ = _mixed_page_groups()
        with pytest.raises(ValueError, match="cannot express this model's mixed page sizes"):
            get_kv_cache_config_from_groups(_mock_aphrodite_config("LBNHC"), groups, MEMORY)

    def test_unresolved_layout_rejected(self):
        groups, _, _ = _mixed_page_groups()
        with pytest.raises(ValueError, match="has not been resolved"):
            get_kv_cache_config_from_groups(_mock_aphrodite_config(None), groups, MEMORY)

    def test_head_outer_layout_rejected_for_mixed_pages(self):
        groups, _, _ = _mixed_page_groups()
        with pytest.raises(ValueError, match="cannot express this model's mixed page sizes"):
            get_kv_cache_config_from_groups(_mock_aphrodite_config("LHBNC"), groups, MEMORY)
