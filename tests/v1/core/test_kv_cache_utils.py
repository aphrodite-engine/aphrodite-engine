...
import aphrodite.v1.core.kv_cache_utils as kv_cache_utils
...
from aphrodite.utils.hashing import sha256, sha256_cbor, xxhash, xxhash_cbor
...
from aphrodite.v1.metrics.stats import CachingMetrics, PrefixCacheStats
...
def test_none_hash(monkeypatch, hash_fn):
    import aphrodite.v1.core.kv_cache_utils
    ...
    with monkeypatch.context() as m:
        ...
        reloaded_kv_cache_utils = importlib.reload(aphrodite.v1.core.kv_cache_utils)
...
def test_get_none_hash_seed_reports_effective_seed(monkeypatch):
    ...
    with monkeypatch.context() as m:
        m.delenv("PYTHONHASHSEED", raising=False)
        reloaded = importlib.reload(aphrodite.v1.core.kv_cache_utils)
...
def test_free_kv_cache_block_queue_append_n():
    ...
    assert invalid_queue.fake_free_list_head.next_free_block == invalid_queue.fake_free_list_tail
...
def test_free_kv_cache_block_queue_popleft_n():
    blocks = [KVCacheBlock(block_id=i) for i in range(6)]
    # Create an empty FreeKVCacheBlockQueue with these blocks
    queue = FreeKVCacheBlockQueue([blocks[1], blocks[3], blocks[5], blocks[4], blocks[0], blocks[2]])
    assert queue.num_free_blocks == 6
    assert queue.fake_free_list_head.next_free_block is blocks[1]
...
def test_generate_block_hash_extra_keys_lora():
    ...
    request.lora_request = LoRARequest(lora_name="test_lora_adapter", lora_int_id=1, lora_path="/path/to/lora")
...
def test_request_block_hasher(hash_fn):
    ...
    assert block_hashes[0] == hash_fn((kv_cache_utils.NONE_HASH, (0, 1, 2), (("hash1", 0),)))
...
def test_get_kv_cache_configs_multiple_workers():
    ...
    aphrodite_config.cache_config.kv_cache_layout = "LBNHC"
    aphrodite_config.cache_config.prefix_cache_retention_interval = None
...
def test_get_kv_cache_configs_pp_sharding(asymmetric_memory):
    ...
    available_memory = [avail_memory, avail_memory * 2] if asymmetric_memory else [avail_memory] * 2
        ...
        KVCacheConfig(
            ...
            kv_cache_tensors=[
                KVCacheTensor(
                    ...
                    layers=["layer1"],
                    layer_stride=ref_kv_cache_spec.page_size_bytes * expected_num_blocks,
                    block_stride=ref_kv_cache_spec.page_size_bytes,
                ),
            ],
...
def test_project_kv_cache_groups_to_worker():
    ...
    projected = kv_cache_utils._project_kv_cache_groups_to_worker(global_groups, worker_spec)
    assert len(projected) == 1
    assert projected[0].layer_names == ["layer1", "layer2"]
    assert projected[0].kv_cache_spec is spec_a

    projected = kv_cache_utils._project_kv_cache_groups_to_worker(global_groups, {"layer4": spec_a})
    assert len(projected) == 1
...
def test_uniform_type_spec_block_table_width_matches_layer_spec(layer_type, dcp_size, expected_width):
    ...
    aphrodite_config = AphroditeConfig(model_config=ModelConfig(max_model_len=1024))
    aphrodite_config.parallel_config.decode_context_parallel_size = dcp_size
    ...
    assert layer_spec.max_num_blocks_per_req(aphrodite_config, 1024) == expected_width
    assert uniform_spec.max_num_blocks_per_req(aphrodite_config, 1024) == expected_width
...
from aphrodite.v1.core.kv_cache_utils import is_kv_cache_spec_uniform
...
def test_estimate_max_model_len(model_id, max_model_len, want_estimated_max_len):
    ...
    estimated_max_len = estimate_max_model_len(aphrodite_config, kv_cache_spec, 8 * GiB_bytes)
...
def test_get_max_concurrency_for_kv_cache_config():
    ...
    assert aphrodite_config.max_concurrent_batches == 1
    ...
    kv_cache_config_hybrid_model = KVCacheConfig(
        ...
        kv_cache_groups=[
            KVCacheGroupSpec([f"layer_{i}" for i in range(32)], full_attention_spec),
            KVCacheGroupSpec([f"layer_{i}" for i in range(32, 64)], sliding_window_spec),
        ],
    )
    ...
    num_tokens, max_concurrency = get_kv_cache_capacity(aphrodite_config, kv_cache_config_hybrid_model)
    ...
    assert get_max_concurrency_for_kv_cache_config(aphrodite_config, kv_cache_config_unequal_groups) == 3
    ...
    assert get_max_concurrency_for_kv_cache_config(aphrodite_config, kv_cache_config_uniform_group) == 3
    ...
    kv_cache_config_scheduler_shape = generate_scheduler_kv_cache_config([copy.deepcopy(kv_cache_config_uniform_group)])
    assert get_max_concurrency_for_kv_cache_config(
        aphrodite_config, kv_cache_config_scheduler_shape
    ) == get_max_concurrency_for_kv_cache_config(aphrodite_config, kv_cache_config_uniform_group)
...
def test_get_kv_cache_config_one_worker():
    ...
    assert kv_cache_config_sliding == KVCacheConfig(
        ...
        kv_cache_groups=[KVCacheGroupSpec(["layer_1", "layer_2"], new_sliding_window_spec())],
    )
    ...
    assert kv_cache_config_hybrid == KVCacheConfig(
        ...
        kv_cache_groups=[
            KVCacheGroupSpec(["layer_1", "layer_2"], new_kv_cache_spec(sliding_window=1)),
        ],
    )
    ...
    assert kv_cache_config_hybrid == KVCacheConfig(
        ...
        kv_cache_groups=[
            ...
            KVCacheGroupSpec(["layer_4", "layer_7", "layer_10"], new_sliding_window_spec()),
    ...
    assert kv_cache_config_hybrid == KVCacheConfig(
        ...
        kv_cache_groups=[
            KVCacheGroupSpec(
                ["layer_1", "layer_2"],
                UniformTypeKVCacheSpecs(block_size=16, kv_cache_specs=kv_cache_specs_hybrid),
            )
        ],
    )
    ...
    assert kv_cache_config_hybrid == KVCacheConfig(
        ...
        kv_cache_groups=[
            ...
            KVCacheGroupSpec(["layer_2"], new_sliding_window_spec(head_size=32, block_size=32)),
        ],
    )
...
def test_generate_uniform_type_kv_cache_specs():
    ...
    assert uniform_spec == UniformTypeKVCacheSpecs(block_size=16, kv_cache_specs=kv_cache_specs)
    ...
    kv_cache_configs = [
        KVCacheConfig(
            ...
            kv_cache_groups=[
                KVCacheGroupSpec(
                    ["layer_1", "layer_2"],
                    UniformTypeKVCacheSpecs(block_size=16, kv_cache_specs=kv_cache_specs),
                ),
            ],
        )
    ]
...
def _tensor_by_layer(kv_cache_config: KVCacheConfig) -> dict[str, KVCacheTensor]:
    return {layer_name: tensor for tensor in kv_cache_config.kv_cache_tensors for layer_name in tensor.layers}
...
def test_get_kv_cache_config_balanced_mamba_hybrid():
    ...
    groups = kv_cache_utils.get_kv_cache_groups(aphrodite_config, kv_cache_spec)
    uniform_groups = [group for group in groups if isinstance(group.kv_cache_spec, UniformTypeKVCacheSpecs)]
    mamba_groups = [group for group in groups if isinstance(group.kv_cache_spec, MambaSpec)]
    ...
    attn_blocks = uniform_groups[0].kv_cache_spec.max_memory_usage_pages(aphrodite_config)
    mamba_blocks_per_group = 1 + new_mamba_spec().num_speculative_blocks
    blocks_per_request = attn_blocks + 4 * mamba_blocks_per_group
    assert (
        kv_cache_utils._max_memory_usage_bytes_from_groups(aphrodite_config, groups)
        == blocks_per_request * bytes_per_block
    )

    available_memory = bytes_per_block * 100 + 1
    kv_cache_config = kv_cache_utils.get_kv_cache_config_from_groups(aphrodite_config, groups, available_memory)
    ...
    assert get_max_concurrency_for_kv_cache_config(aphrodite_config, kv_cache_config) == pytest.approx(
        100 / blocks_per_request
    )
...
def test_get_kv_cache_config_kpool_tail_coowns_indexer_tensor():
    ...
    tail_group = next(
        ...
        and all(isinstance(spec, KpoolTailSpec) for spec in group.kv_cache_spec.kv_cache_specs.values())
    )
    ...
    tail_inner = cast(KpoolTailSpec, tail_group.kv_cache_spec.kv_cache_specs["layers.3.tail"])
    ...
    kv_cache_config = kv_cache_utils.get_kv_cache_config_from_groups(
        aphrodite_config, groups, bytes_per_block * 100 + 1
    )
    assert kv_cache_config.num_blocks == 100
    ...
    attn_group = next(
        group
        for group in groups
        if isinstance(group.kv_cache_spec, UniformTypeKVCacheSpecs) and group is not tail_group
    )
    attn_blocks = attn_group.kv_cache_spec.max_memory_usage_pages(aphrodite_config)
    mamba_blocks_per_group = 1 + new_mamba_spec().num_speculative_blocks
...
def test_glm5_kpool_tail_does_not_drag_hash_block_size():
    ...
    groups = kv_cache_utils.get_kv_cache_groups(aphrodite_config, _glm5_like_kv_cache_spec_with_tail(align_mamba))
    kv_cache_config = KVCacheConfig(
        num_blocks=1,
        kv_cache_tensors=[],
        kv_cache_groups=groups,
    )
    hash_aphrodite_config = SimpleNamespace(
    ...
    assert kv_cache_utils.resolve_kv_cache_block_sizes(kv_cache_config, hash_aphrodite_config) == (1024, 16)
...
def test_get_kv_cache_config_mamba_hybrid_sharing_infeasible():
    ...
    with pytest.raises(ValueError, match="does not fit the MLA page"):
        kv_cache_utils.get_kv_cache_groups(aphrodite_config, kv_cache_spec)
...
def test_get_kv_cache_config_mamba_hybrid_sharing_infeasible_no_indexer():
    ...
    model_config = ModelConfig(max_model_len=8192)
    aphrodite_config = AphroditeConfig(model_config=model_config)
    aphrodite_config.cache_config.kv_cache_layout = "LBHNC"
    ...
    for i in range(27):
        if i % 4 == 3 or i == 26:
            ...
            kv_cache_spec[f"layers.{i}.linear_attn"] = new_mamba_spec(shapes=((512, 1024),), dtypes=(torch.float32,))
    mla_page = kv_cache_spec["layers.3.attn"].page_size_bytes
...
def test_get_kv_cache_config_mamba_hybrid_sharing_pp_balanced_projection():
    ...
    global_groups = kv_cache_utils.get_kv_cache_groups(aphrodite_config, kv_cache_spec)
    mla_page = kv_cache_spec["layers.3.attn"].page_size_bytes
    idx_page = kv_cache_spec["layers.3.indexer"].page_size_bytes
    ...
    groups = kv_cache_utils._project_kv_cache_groups_to_worker(global_groups, worker_spec)
    mamba_groups = [group for group in groups if isinstance(group.kv_cache_spec, MambaSpec)]
...
def test_get_kv_cache_config_mamba_hybrid_sharing_pp_group_count_bump(monkeypatch):
    ...
    for rank in range(4):
        ...
        worker_spec = {n: s for n, s in kv_cache_spec.items() if start <= int(n.split(".")[1]) < end}
        projected = kv_cache_utils._project_kv_cache_groups_to_worker(groups, worker_spec)
...
def test_get_kv_cache_config_mamba_hybrid_sharing_pp_starved_stage(monkeypatch):
    ...
    monkeypatch.setattr(ModelConfig, "get_total_num_hidden_layers", lambda self: 45)
    monkeypatch.setenv("APHRODITE_PP_LAYER_PARTITION", "3,42")
    aphrodite_config = AphroditeConfig(
        model_config=ModelConfig(max_model_len=8192),
    ...
    kv_cache_spec, _ = _glm5_like_kv_cache_spec()
    with pytest.raises(ValueError, match="APHRODITE_PP_LAYER_PARTITION"):
        kv_cache_utils.get_kv_cache_groups(aphrodite_config, kv_cache_spec)
...
def test_get_kv_cache_config_mamba_hybrid_sharing_beats_cross_layers_flag():
    ...
    aphrodite_config = AphroditeConfig(model_config=model_config, kv_transfer_config=kv_transfer_config)
    ...
    assert kv_cache_config.num_blocks == 100
    assert len(kv_cache_config.kv_cache_tensors) == 56
    assert all(tensor.block_stride in (mla_page, idx_page) for tensor in kv_cache_config.kv_cache_tensors)
    assert {t.size for t in kv_cache_config.kv_cache_tensors} == {bytes_per_block * 100}
...
def test_get_kv_cache_capacity_after_scheduler_unwrap():
    ...
    assert not any(isinstance(g.kv_cache_spec, UniformTypeKVCacheSpecs) for g in scheduler_config.kv_cache_groups)

    unwrapped_groups = scheduler_config.kv_cache_groups
    expected_max_mem = kv_cache_utils._max_memory_usage_bytes_from_groups(aphrodite_config, unwrapped_groups)
    expected_pool = kv_cache_utils._pool_bytes_per_block(unwrapped_groups)
    expected_blocks_per_request = (expected_max_mem + expected_pool - 1) // expected_pool

    _, max_concurrency = get_kv_cache_capacity(aphrodite_config, scheduler_config)
    assert max_concurrency > 0
    assert max_concurrency == pytest.approx(scheduler_config.num_blocks / expected_blocks_per_request)
...
def test_hidden_state_group_preserves_hybrid_prefix_cache_granularity():
    ...
    hidden_group = next(group for group in groups if isinstance(group.kv_cache_spec, HiddenStateCacheSpec))
    ...
    assert kv_cache_utils.resolve_kv_cache_block_sizes(kv_cache_config, aphrodite_config) == (544, 136)
...
def test_resolve_dcp_kv_block_size_unwraps_uniform_type_specs():
    ...
    assert all(spec.block_size == 64 for spec in scaled_attention.kv_cache_specs.values())
...
def test_multi_run_layer_compact_strides_place_hoisted_heads():
    ...
    aphrodite_config = AphroditeConfig(model_config=ModelConfig(max_model_len=16))
    aphrodite_config.cache_config.kv_cache_layout = "LHBNC"
...
def test_mla_draft_prefers_standard_layout_when_pages_can_be_unified():
    ...
    assert len(groups) == 2
    assert all(not isinstance(group.kv_cache_spec, UniformTypeKVCacheSpecs) for group in groups)


def test_mla_with_incompatible_swa_uses_one_full_allocation_group(caplog_aphrodite):
    ...
    assert "attention compute is unchanged" in caplog_aphrodite.text
...
def test_get_kv_cache_spec_kind_prefers_specific_attention_subclasses():
    ...
    assert get_kv_cache_spec_kind(sliding_window_mla_spec) == KVCacheSpecKind.SLIDING_WINDOW_MLA
    ...
    assert get_kv_cache_spec_kind(sink_full_attention_spec) == KVCacheSpecKind.SINK_FULL_ATTENTION
...
def test_get_kv_cache_spec_kind_unwraps_uniform_type_specs():
    ...
    assert get_kv_cache_spec_kind(uniform_swa_mla_spec) == KVCacheSpecKind.SLIDING_WINDOW_MLA
...
def test_request_block_hasher_with_prompt_embeds(hash_fn: Callable[[Any], bytes]):
    ...
    block1_embeds_hash = hashlib.sha256(tensor_data(prompt_embeds[:block_size])).digest()
    ...
    block2_embeds_hash = hashlib.sha256(tensor_data(prompt_embeds[block_size:num_tokens])).digest()
...
def test_auto_fit_max_model_len():
    ...
    _kv_cache_configs = get_kv_cache_configs(aphrodite_config, [kv_cache_specs], [large_available_memory])
    ...
    _kv_cache_configs = get_kv_cache_configs(aphrodite_config, [kv_cache_specs], [limited_memory])
...
def test_auto_fit_max_model_len_with_hybrid():
    ...
    _kv_cache_configs = get_kv_cache_configs(aphrodite_config, [kv_cache_specs], [available_memory])
    assert aphrodite_config.model_config.max_model_len == 1024
...
def test_auto_fit_max_model_len_not_triggered():
    ...
    _kv_cache_configs = get_kv_cache_configs(aphrodite_config, [kv_cache_specs], [mem_per_block_per_layer * 2 * 32])
...
def test_unpadded_page_size_includes_per_token_head_scales():
    ...
    spec = TritonAttentionBackend.customize_spec(
        new_kv_cache_spec(dtype=torch.uint8, kv_quant_mode=KVQuantMode.FP8_PER_TOKEN_HEAD)
    )
...
def test_unify_hybrid_kv_cache_specs():
    ...
    before_spec_2 = new_sliding_window_spec(block_size=16, page_size_padded=32 * 1024, sliding_window=1024)
    kv_cache_spec = {
    ...
    expected_spec_2 = new_kv_cache_spec(block_size=64, page_size_padded=64 * 1024, sliding_window=1024)
    ...
    before_spec_2 = new_chunked_local_attention_spec(page_size_padded=32 * 1024, attention_chunk_size=512)
    kv_cache_spec = {
        "layer_1": before_spec_1,
        "layer_2": before_spec_2,
    }
    ...
    expected_spec_2 = new_kv_cache_spec(page_size_padded=32 * 1024, attention_chunk_size=512)
    ...
    before_spec_1 = new_kv_cache_spec()
    before_spec_2 = new_sliding_window_spec(page_size_padded=32 * 1024, sliding_window=1024)
    before_spec_3 = new_chunked_local_attention_spec(page_size_padded=32 * 1024, attention_chunk_size=512)
    kv_cache_spec = {
        "layer_1": before_spec_1,
        "layer_2": before_spec_2,
    ...
    expected_spec_3 = new_kv_cache_spec(page_size_padded=32 * 1024, attention_chunk_size=512)
    assert kv_cache_spec["layer_1"] == expected_spec_1
    assert kv_cache_spec["layer_2"] == expected_spec_2
...
def test_unify_kv_cache_spec_page_size_mamba():
    ...
    padded_mamba_spec = new_mamba_spec(shapes=((2, 256), (3, 32, 32)), page_size_padded=16384)
...
def test_hma_not_disabled_when_kv_events_enabled():
    ...
    This test guards against that regression by verifying that a AphroditeConfig
...
def test_resolve_block_hashes_gate():
    ...
    assert resolve_block_hashes(raw, 2, 2, supports_fine_grained_hash_lookup=True, alignment_tokens=2) is raw
    ...
    assert resolve_block_hashes(raw, 2, 4, supports_fine_grained_hash_lookup=True, alignment_tokens=2) is raw
    ...
    off = resolve_block_hashes(raw, 2, 4, supports_fine_grained_hash_lookup=True, alignment_tokens=4)
    ...
    swa = resolve_block_hashes(raw, 2, 4, supports_fine_grained_hash_lookup=False, alignment_tokens=2)
...
def test_kv_cache_reserves_null_block_for_max_model_len(use_override):
    ...
    aphrodite_config = AphroditeConfig(model_config=ModelConfig(max_model_len=max_model_len))
    ...
    with pytest.raises(ValueError, match="max seq len"):
        get_kv_cache_configs(aphrodite_config, [{"layer1": spec}], available_memory)
...
def test_auto_fit_max_model_len_reserves_null_block():
    ...
    assert aphrodite_config.model_config.max_model_len == 63 * block_size
...
def test_check_enough_kv_cache_memory_reserves_null_block():
    ...
    # 32 blocks -> only 31 usable after the null block: one short -> reject.
    with pytest.raises(ValueError, match="max seq len"):
        check_enough_kv_cache_memory(aphrodite_config, {"layer1": spec}, spec.page_size_bytes * 32)

    # 33 blocks -> 32 usable after the null block -> accept.
    check_enough_kv_cache_memory(aphrodite_config, {"layer1": spec}, spec.page_size_bytes * 33)
...
def test_iter_layer_specs_returns_group_members():
    ...
    wrapped = UniformTypeKVCacheSpecs(block_size=4, kv_cache_specs={"a": full, "b": mla})
...
def _spec_decode_grouping_config(method="dspark", model_type=None):
    ...
    return SimpleNamespace(
        scheduler_config=SimpleNamespace(disable_hybrid_kv_cache_manager=False),
        cache_config=SimpleNamespace(get_resolved_kv_cache_layout=lambda: SimpleNamespace(is_block_outermost=True)),
...
def test_draft_group_annotated_on_hybrid_general_path():
    ...
    groups = get_kv_cache_groups(_spec_decode_grouping_config(), _hybrid_specs_with_draft(draft=True))
...
def test_unidentifiable_draft_with_mamba_warns(caplog_aphrodite):
    ...
    groups = get_kv_cache_groups(_spec_decode_grouping_config(), _hybrid_specs_with_draft(draft=False))

    assert not any(g.is_eagle_group for g in groups)
    assert "no KV cache group could be identified as the draft model's" in (caplog_aphrodite.text)
    assert "Mamba groups" in caplog_aphrodite.text


def test_no_warning_when_draft_group_is_identified(caplog_aphrodite):
    get_kv_cache_groups(_spec_decode_grouping_config(), _hybrid_specs_with_draft(draft=True))

    assert "could be identified as the draft model's" not in caplog_aphrodite.text
...
def _deepseek_v4_specs(model_version="deepseek_v4"):
    ...
    return {
        ...
        "model.layers.2.self_attn.attn": new_swa_mla_spec(head_size=1024, model_version=model_version),
