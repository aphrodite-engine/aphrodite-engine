...
def test_get_kv_cache_configs_multiple_workers():
    ...
    assert kv_cache_configs == [
        KVCacheConfig(
            num_blocks=10,
            kv_cache_tensors=[
                KVCacheTensor(size=ref_kv_cache_spec.page_size_bytes * 10, shared_by=["layer1"]),
                KVCacheTensor(size=ref_kv_cache_spec.page_size_bytes * 10, shared_by=["layer2"]),
            ],
            kv_cache_groups=[
    ...
    assert kv_cache_configs == [
        ...
        KVCacheConfig(
            ...
            kv_cache_tensors=[
                KVCacheTensor(size=ref_kv_cache_spec.page_size_bytes * 10, shared_by=["layer2"]),
                KVCacheTensor(size=ref_kv_cache_spec.page_size_bytes * 10, shared_by=["layer3"]),
            ],
            kv_cache_groups=[
    ...
    assert kv_cache_configs == [
        ...
        KVCacheConfig(
            ...
            kv_cache_tensors=[
                KVCacheTensor(size=ref_kv_cache_spec.page_size_bytes * 10, shared_by=["layer3"]),
                KVCacheTensor(size=ref_kv_cache_spec.page_size_bytes * 10, shared_by=["layer4"]),
            ],
            kv_cache_groups=[
...
def test_get_max_concurrency_for_kv_cache_config():
    ...
    # spec's memory/page ratio equals a single layer's page count, so the group
    # needs 1024 blocks and the request 1153 in total. The previous formula
    # normalized both groups' memory by the first group's page size, reporting
    # 3459/1057 = 3.27 here instead of 3 -- and a different value again for
    # the scheduler-config shape below.
    ...
    # uniform-type group's spec with a representative per-layer spec. Capacity
    # must not change between the two shapes (the engine computes on the
    # scheduler config, the worker loop on the worker config).
...
def test_get_max_concurrency_packed_kv_cache_config():
    ...
    assert _use_packed_kv_cache_config(aphrodite_config, kv_cache_groups)
    num_blocks, kv_cache_tensors = _get_kv_cache_config_packed(aphrodite_config, kv_cache_groups, 2 * GiB_bytes)
    assert num_blocks > 0
    ...
    assert get_max_concurrency_for_kv_cache_config(aphrodite_config, kv_cache_config_packed) == num_blocks / (1024 + 73)
...
def test_get_kv_cache_config_one_worker():
    ...
    assert kv_cache_config_hybrid == KVCacheConfig(
        num_blocks=64,
        kv_cache_tensors=[
            KVCacheTensor(size=mem_per_block_per_layer * 64, shared_by=["layer_1", "layer_2"]),
        ],
        kv_cache_groups=[
    ...
    assert kv_cache_config_hybrid == KVCacheConfig(
        ...
        kv_cache_tensors=[
            ...
            KVCacheTensor(size=mem_per_block_per_layer * 32, shared_by=["layer_3", "layer_10"]),
        ],
        kv_cache_groups=[
    ...
    assert kv_cache_config_hybrid == KVCacheConfig(
        num_blocks=32,
        kv_cache_tensors=[
            KVCacheTensor(size=mem_per_block_per_layer * 32, shared_by=["layer_1", "layer_2"]),
        ],
        kv_cache_groups=[
...
def test_mixed_precision_kv_cache_with_uniform_type_specs():
    ...
    worker_config = KVCacheConfig(
        ...
        kv_cache_groups=[
            KVCacheGroupSpec(
                ["fp8_layer"],
                UniformTypeKVCacheSpecs(block_size=16, kv_cache_specs={"fp8_layer": fp8_spec}),
            ),
            KVCacheGroupSpec(
                ["bf16_layer"],
                UniformTypeKVCacheSpecs(block_size=16, kv_cache_specs={"bf16_layer": bf16_spec}),
            ),
        ],
    )
...
def test_unpadded_page_size_includes_per_token_head_scales():
    ...
    spec = new_kv_cache_spec(dtype=torch.uint8, kv_quant_mode=KVQuantMode.FP8_PER_TOKEN_HEAD)
