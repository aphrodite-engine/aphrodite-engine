...
from aphrodite.distributed.kv_transfer.kv_connector.v1.base import SupportsHMA
...
def _free_block_ids(manager):
    ...
    return {block.block_id for block in manager.block_pool.free_block_queue.get_all_free_blocks()}
...
def test_mamba_align_split_when_block_exceeds_long_prefill_threshold():
    ...
    mock = SimpleNamespace(
        cache_config=SimpleNamespace(block_size=block_size),
        max_num_scheduled_tokens=token_budget,
        scheduler_config=SimpleNamespace(long_prefill_token_threshold=long_prefill_threshold),
        use_eagle_block_drop=False,
        hash_block_size=32,
        mamba_partial_cache_hit=False,
        mamba_has_prefill_checkpoint_blocks=False,
...
def test_hybrid_mamba_align_partial_hash_hit():
    ...
    partial_mamba_block = manager.block_pool.get_cached_block(partial_mamba_hash, kv_cache_group_ids=[1])
...
def test_hybrid_mamba_partial_tail_owner_uses_cow_on_continue():
    ...
    moved = manager.block_pool.get_cached_block(partial_mamba_hash, kv_cache_group_ids=[1])
...
def test_external_mamba_hit_same_block_uses_running_cow_on_continue():
    ...
    partial_block = manager.block_pool.get_cached_block(partial_hash, kv_cache_group_ids=[1])
    assert partial_block is not None
...
def test_boundary_state_offloads_returns_cow_target():
    ...
    ((group_id, block_id, boundary_tokens),) = drain_boundary_state_offloads(manager)["0"]
    assert group_id == 1
    assert boundary_tokens == 4
    aligned_hash = req0.block_hashes[4 // hash_block_size - 1]
    aligned_block = manager.block_pool.get_cached_block(aligned_hash, kv_cache_group_ids=[1])
    assert aligned_block is not None
    assert block_id == aligned_block[0].block_id

    partial_mamba_hash = req0.block_hashes[6 // hash_block_size - 1]
    source_block = manager.block_pool.get_cached_block(partial_mamba_hash, kv_cache_group_ids=[1])
...
def test_finished_partial_tail_uses_table_source_once():
    ...
    assert advanced_manager.allocate_slots(req1, 6, num_computed, computed_blocks) is not None
...
def test_connector_finish_registers_partial_tail_before_cleanup():
    ...
    scheduler.aphrodite_config = SimpleNamespace(kv_transfer_config=SimpleNamespace(is_kv_producer=True))
    scheduler.kv_cache_manager = MagicMock()
    scheduler.kv_cache_manager.finalize_partial_tail_offloads.return_value = [(1, 9, 12)]
...
def test_block_pool_touch_pins_released_cow_target():
    ...
    new_blocks = manager.block_pool.get_new_blocks(manager.block_pool.get_num_free_blocks())
...
def test_truncate_computed_blocks_preserves_sparse_prefix_positions():
    ...
    consumer = make_request("consumer", [0, 0, 1, 1, 2, 2, 3, 3], hash_block_size, sha256)
...
def test_truncate_computed_blocks_allows_short_mamba_group_only():
    ...
    short_full_attention = manager.create_kv_cache_blocks((list(blocks.blocks[0][:1]), list(blocks.blocks[1])))
...
def test_hybrid_full_attention_partial_hash_hit_uses_cow():
    ...
    partial_full_block = manager.block_pool.get_cached_block(partial_full_hash, kv_cache_group_ids=[0])
...
def test_hybrid_partial_hit_cow_target_starts_uncached():
    ...
    partial_full_block = manager.block_pool.get_cached_block(partial_hash, kv_cache_group_ids=[0])
    partial_mamba_block = manager.block_pool.get_cached_block(partial_hash, kv_cache_group_ids=[1])
    assert partial_full_block is not None
    assert partial_mamba_block is not None
...
def test_free_cow_retained_blocks_defers_until_copy_step_processed():
    ...
    mock = SimpleNamespace(
        kv_cache_manager=SimpleNamespace(block_pool=SimpleNamespace(free_blocks=freed.extend)),
...
def test_full_attention_eagle_drops_one_hash_unit():
    ...
    from aphrodite.v1.core.single_type_kv_cache_manager import FullAttentionManager

    hash_block_size = 2
    block_size = 4
    pool = BlockPool(num_gpu_blocks=10, enable_caching=True, hash_block_size=hash_block_size)
    ...
    pool2 = BlockPool(num_gpu_blocks=10, enable_caching=True, hash_block_size=hash_block_size)
    pool = pool2
    blocks = pool.get_new_blocks(2)
    pool.cache_full_blocks(
        request=req,
...
def test_hybrid_partial_hit_with_eagle_stays_within_group_blocks():
    ...
    assert all(len(group) * block_size >= num_computed for group in computed_blocks.blocks)
...
def test_hybrid_sliding_window_group_disables_partial_hash_hits():
    ...
    assert manager.allocate_slots(request, mamba_block_size, num_computed, computed_blocks) is not None
    ...
    cached_request = make_request("1", tokens + [len(tokens), len(tokens) + 1], hash_block_size, sha256)
...
def test_kpool_tail_supports_128_token_partial_hash_hits():
    ...
    computed_blocks, num_computed, _ = manager.get_computed_blocks(req0)
    assert manager.allocate_slots(req0, len(shared_prefix), num_computed, computed_blocks) is not None
    manager.free(req0)
    manager.new_step_starts()
...
def test_hybrid_partial_hash_hit_uses_cow_under_dcp(dcp_world_size: int):
    ...
    assert KVCacheBlockCopy(partial_mamba_block[0].block_id, mamba_new_block_id) in copies
    manager.block_pool.free_blocks(retained)
...
def test_dcp_partial_hit_resumes_on_replicated_mamba_snapshot(
    ...
    assert manager.coordinator.single_type_managers[0].block_size == (block_size * dcp_world_size)
    ...
    req1 = make_request("snapshot-replay", prefix + list(range(12, 16)), block_size, sha256)
    ...
    assert all(copy.src_block_id != computed_blocks.blocks[1][-1].block_id for copy in copies)
    manager.block_pool.free_blocks(retained)
...
def _snapshot_offload_kv_cache_config(hash_block_size: int, block_size: int, num_blocks: int = 24):
...
def _run_chunked_prefill(manager, req, chunk_size, num_chunks):
    ...
    for step in range(num_chunks):
        ...
        if step == 0:
            allocated = manager.allocate_slots(req, chunk_size, num_computed, computed_blocks)
        else:
...
def test_boundary_states_offered_past_prompt_for_resumed_prefill():
    ...
    assert [
        boundary for entries in _run_chunked_prefill(manager, req0, block_size, 1) for _, _, boundary in entries
    ] == [prompt_len]
