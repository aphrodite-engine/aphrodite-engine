...
from aphrodite.multimodal.utils import strip_covered_mm_data
...
def test_strip_covered_mm_data() -> None:
    ...
    stripped = strip_covered_mm_data([covered, boundary, uncovered, already_none], num_computed_tokens=250)
...
def _mm_feature_mixed(offset: int, length: int) -> MultiModalFeatureSpec:
    data = MultiModalKwargsItem(
        {
            "pixel_values": MultiModalFieldElem(data=torch.empty(4), field=MultiModalBatchedField()),
...
def test_strip_covered_mm_data_xdrope() -> None:
    ...
    stripped = strip_covered_mm_data([covered, uncovered], num_computed_tokens=250, uses_xdrope=True)
...
def test_strip_covered_mm_data_mrope() -> None:
    ...
    stripped = strip_covered_mm_data([covered, uncovered], num_computed_tokens=250, uses_mrope=True)
...
def test_strip_covered_mm_data_shm_address_item() -> None:
    ...
    the second identical request (https://github.com/vllm-project/vllm/issues/54994)."""
