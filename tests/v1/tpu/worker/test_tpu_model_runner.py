...
def get_model_runner(aphrodite_config):
    device = "xla:0"  # Mocking TPU device
    return TPUModelRunner(aphrodite_config, device)
...
def model_runner():
    ...
    return get_model_runner(aphrodite_config)
...
def test_init_kv_cache_with_kv_sharing_invalid_target_layer_order(model_runner):
    ...
    aphrodite_config = model_runner.aphrodite_config
    with (
...
def test_init_kv_cache_without_kv_sharing():
    ...
    aphrodite_config.model_config.max_model_len = 1_000_000
    aphrodite_ctx = aphrodite_config.compilation_config.static_forward_context
    model_runner = get_model_runner(aphrodite_config)
...
def test_most_model_len(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("APHRODITE_TPU_MOST_MODEL_LEN", "2048")
    aphrodite_config = get_aphrodite_config()
    aphrodite_config.model_config.max_model_len = 32000
    aphrodite_config.scheduler_config.max_num_seqs = 1200
    model_runner = get_model_runner(aphrodite_config)
