...
def test_tpu_model_loader(model):
    ...
    aphrodite_config = _setup_environment(model)
    loader = TPUModelLoader(load_config=aphrodite_config.load_config)
    mesh = _get_spmd_mesh()
    model = loader.load_model(aphrodite_config, aphrodite_config.model_config, mesh)
    del model
    gc.collect()
