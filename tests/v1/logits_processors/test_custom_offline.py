...
def test_fake_entrypoint_preserves_other_groups(monkeypatch):
    ...
    monkeypatch.setattr(logitproc_test_utils, "requires_spawn_multiprocessing", lambda: False)

    setup_fake_entrypoint(monkeypatch)

    logitproc_entrypoints = importlib.metadata.entry_points(group=logitproc_test_utils.LOGITSPROCS_GROUP)
    assert logitproc_entrypoints.names == [logitproc_test_utils.DUMMY_LOGITPROC_ENTRYPOINT]
    assert importlib.metadata.entry_points(group="another.entrypoint.group") is other_group_entrypoints
...
def _run_test(kwargs: dict, logitproc_loaded: bool) -> None:
    ...
    # Create a Aphrodite instance and load custom logitproc
    llm_logitproc = LLM(
        model=MODEL_NAME,
    ...
    # Create a reference Aphrodite instance without custom logitproc
