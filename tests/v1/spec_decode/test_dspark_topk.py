...
def _markov_head(weight: torch.Tensor) -> DSparkMarkovHead:
    ...
    head.markov_w2 = nn.Linear(weight.shape[1], weight.shape[0], bias=False, dtype=weight.dtype)
...
def test_gathered_markov_bias_overwrites_dense_logits():
    ...
    result = _markov_head(weight).apply_bias_gathered(markov_embed, logits, values, index)
...
def test_gathered_markov_bias_matches_dense_at_full_vocab():
    ...
    result = _markov_head(weight).apply_bias_gathered(markov_embed, logits, values, index, scale)
