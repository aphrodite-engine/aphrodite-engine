...
from aphrodite.v1.worker.gpu.async_utils import StepTimingSample
from aphrodite.v1.worker.gpu.attn_utils import AttentionCGSupportInfo
from aphrodite.v1.worker.gpu.spec_decode import adaptive_verification as adaptive_module
from aphrodite.v1.worker.gpu.spec_decode.adaptive_verification import (
    AdaptiveVerificationManager,
    maybe_create_adaptive_verification_manager,
)
from aphrodite.v1.worker.gpu.structured_outputs import _build_grammar_mapping


def make_manager(confidences: np.ndarray, verify_cost_ms: np.ndarray) -> AdaptiveVerificationManager:
...
def test_manager_scopes_varlen_check_without_weakening_runner_cg_mode(monkeypatch):
    ...
    runner_support = AttentionCGSupportInfo(AttentionCGSupport.UNIFORM_BATCH, "DraftBackend")
    created = object()
    monkeypatch.setattr(
        adaptive_module,
        "AdaptiveVerificationManager",
...
def test_profiled_batches_seed_cost_curves_via_consumer():
    ...
    manager.set_cost_curves = lambda draft, verify: curves.update(draft=draft, verify=verify)
...
def test_zero_budget_rebuilds_cpu_cu_num_logits():
    ...
    compacted, cu_num_logits_np = manager.compact_batch(drafts, scheduled, scheduled_cu_num_logits)
...
def test_zero_budget_keeps_one_grammar_row_per_scheduled_draft():
    ...
    manager.get_num_tokens({"low": 3, "high": 3, "prefill": 40}, scheduled_spec_decode_tokens)
    ...
    num_bitmask_rows = sum(len(scheduled_spec_decode_tokens.get(req_id, ())) + 1 for req_id in req_ids)
