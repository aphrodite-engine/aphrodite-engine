...
import aphrodite.v1.spec_decode.llm_base_proposer as llm_base_proposer
...
class _FakeAttentionGroup:
    ...
    def create_metadata_builders(self, aphrodite_config, device, kernel_block_size=None):
...
def _make_proposer(monkeypatch: pytest.MonkeyPatch, layer_names: set[str]) -> EagleProposer:
    fake_layers = {}
    for name in layer_names:
        backend = SimpleNamespace(full_cls_name=lambda: "FakeBackend")
        fake_layers[name] = SimpleNamespace(get_attn_backend=lambda backend=backend: backend)
    monkeypatch.setattr(
        llm_base_proposer,
    ...
    proposer.aphrodite_config = None
...
def test_block_size_falls_back_to_kv_cache_spec(
    monkeypatch: pytest.MonkeyPatch,
):
    layer_names = {"draft.0.self_attn.attn"}
    proposer = _make_proposer(monkeypatch, layer_names)

    proposer.initialize_attn_backend(_make_kv_cache_config(layer_names), kernel_block_sizes=None)

    assert proposer.block_size == SCHEDULER_BLOCK_SIZE


def test_draft_layer_iteration_is_deterministic(
    monkeypatch: pytest.MonkeyPatch,
):
