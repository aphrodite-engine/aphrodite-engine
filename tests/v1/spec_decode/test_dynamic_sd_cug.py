...
def _create_aphrodite_config_for_dsd(
    max_num_seqs: int,
    max_spec_tokens: int,
    *,
    ...
    aphrodite_config.num_speculative_tokens = max_spec_tokens
    ...
    if use_dynamic_sd:
        ...
        if num_spec_per_batch_size is None:
            num_spec_per_batch_size = [(qlen, qlen, qlen - 1) for qlen in range(1, max_decode_query_len + 1)]
        speculative_config.num_speculative_tokens_per_batch_size = num_spec_per_batch_size
    else:
...
def test_dynamic_sd_full_cudagraph_covers_all_uniform_decode_shapes(monkeypatch):
    ...
    aphrodite_config = _create_aphrodite_config_for_dsd(
        max_num_seqs=max_num_seqs,
...
def test_dynamic_sd_cudagraphs_use_clamped_query_length(monkeypatch):
    ...
    full_query_lens = {desc.uniform_token_count for desc in manager._capture_descs[CUDAGraphMode.FULL]}
...
def test_prompt_chunks_shaped_like_spec_decode_miss_the_full_graph(monkeypatch):
    ...
    assert get_uniform_decode_token_count(num_reqs, num_tokens, decode_query_len, has_prefill=False) == decode_query_len

    # Two of the requests are decode_query_len tokens into a longer prompt.
    uniform_tok_count = get_uniform_decode_token_count(num_reqs, num_tokens, decode_query_len, has_prefill=True)
    ...
    uniform_tok_count = get_uniform_decode_token_count(num_reqs, num_tokens, decode_query_len, has_prefill=False)
