...
def _run_prepare(
    *,
    ...
    input_buffers = SimpleNamespace(
        ...
        query_start_loc=torch.full((max_num_reqs + 1,), -1, dtype=torch.int32, device=device),
    ...
    query_slot_mapping = torch.full((max_num_tokens,), -2, dtype=torch.int64, device=device)
    context_positions = torch.full((max_num_tokens,), -1, dtype=torch.int64, device=device)
    context_slot_mapping = torch.full((max_num_tokens,), -2, dtype=torch.int64, device=device)
    ...
    sample_idx_mapping = torch.full(sample_indices.shape, -1, dtype=torch.int32, device=device)
    ...
    input_temperature = torch.tensor([0.0, 0.0, 1.0, 0.0], dtype=torch.float32, device=device)
