...
def _rms_norm_reference(
    ...
    output = input_fp32 * torch.rsqrt(input_fp32.square().mean(dim=-1, keepdim=True) + eps)
...
def test_fused_add_rms_norm_batch_invariant_residual_path(
    ...
    torch.testing.assert_close(
        ...
        rtol=rtol,
        atol=atol,
        msg="Fused add RMSNorm output should stay numerically close to the PyTorch RMSNorm reference",
    )
...
def test_rms_norm_3d_input(default_aphrodite_config, batch_size: int, seq_len: int, hidden_size: int):
    """
...
def test_rms_norm_numerical_stability(default_aphrodite_config):
    ...
    for idx, input_tensor in enumerate(test_cases):
        ...
        assert not torch.isnan(reference_output).any(), f"Reference RMS norm produced NaN for test case {idx}"
        assert not torch.isinf(reference_output).any(), f"Reference RMS norm produced Inf for test case {idx}"
...
def test_rms_norm_batch_invariance(dtype):
    ...
    assert torch.equal(out_single[0], out_batch[4]), "rms_norm output for a row differs when batch context changes"
