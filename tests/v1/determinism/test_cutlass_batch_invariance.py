...
The NVFP4 CUTLASS tests in this file must run with ``APHRODITE_BATCH_INVARIANT=1``
...
def test_cutlass_nvfp4_scaled_mm_batch_invariant(
    ...
    for i in range(m):
        ...
        assert torch.equal(out_full[i], out_row[0]), (
            f"APHRODITE_BATCH_INVARIANT: row {i} differs between M={m} and M=1: "
...
def _make_cutlass_fp4_moe_batch_invariant_case(
    ...
    hidden_states = torch.randn((case_config["m"], case_config["k"]), device="cuda", dtype=dtype) / 10
...
def _run_cutlass_fp4_moe(
    ...
    topk_weights, topk_ids, _ = fused_topk(hidden_states, score, case["topk"], renormalize=False)
...
def test_cutlass_nvfp4_moe_batch_invariant(
    ...
    case = _make_cutlass_fp4_moe_batch_invariant_case(case_config, activation, e, topk, dtype)
    ...
    for perm_name, perm in (
        ...
        torch.testing.assert_close(
            ...
            atol=0,
            rtol=0,
            msg=(f"{case_id}: permutation '{perm_name}' changed outputs relative to the baseline batch order."),
        )
    ...
    for idx in range(case["hidden_states"].size(0)):
        ...
        torch.testing.assert_close(
            ...
            atol=0,
            rtol=0,
            msg=(f"{case_id}: batch-row {idx} changed between full-batch and batch-size-1 execution."),
        )
    ...
    for subset_ids in case["config"]["subset_indices"]:
        ...
        torch.testing.assert_close(
            ...
            atol=0,
            rtol=0,
            msg=(f"{case_id}: sub-batch {list(subset_ids)} changed relative to full-batch execution."),
        )
