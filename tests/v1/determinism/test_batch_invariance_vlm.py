...
VLM_TEST_MODEL = os.getenv("APHRODITE_VLM_TEST_MODEL", "Qwen/Qwen3-VL-2B-Instruct")

IMAGE_PROMPT = (
    "<|im_start|>user\n<|vision_start|><|image_pad|><|vision_end|>Describe this image.<|im_end|><|im_start|>assistant\n"
)
VIDEO_PROMPT = (
    "<|im_start|>user\n<|vision_start|><|video_pad|><|vision_end|>Describe this video.<|im_end|><|im_start|>assistant\n"
)
...
def _assert_batch_invariant(llm: LLM, inputs: list[dict], sampling) -> None:
    bs1 = [_extract_step_logprobs(llm.generate([inp], sampling, use_tqdm=False)[0]) for inp in inputs]
    ...
    if failures:
        pytest.fail(f"Batch invariance violated for {len(failures)}/{len(inputs)} requests:\n" + "\n".join(failures))
