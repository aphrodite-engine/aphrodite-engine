...
def test_update_state_after_alloc_tracks_cached_blocks_per_group():
    ...
    scheduler.kv_cache_config = MagicMock(select_transfer_block_ids=lambda block_ids: block_ids)
...
def test_sync_device_after_mamba_recv_gates(
    ...
    from aphrodite.distributed.kv_transfer.kv_connector.v1.nixl import base_worker
    from aphrodite.distributed.kv_transfer.kv_connector.v1.nixl.worker import (
        NixlConnectorWorker,
    )
...
def test_apply_prefix_caching_ssm_unpairable_slots_rejected():
    ...
    with pytest.raises(AssertionError, match="unpairable SSM state slots"):
        worker._apply_prefix_caching([list(range(10)), [4, 5, 6, 7]], [list(range(10)), [8, 9]], 10, 10)
...
def _make_mock_worker_for_desc_ids(
    ...
    if has_mamba:
        from aphrodite.distributed.kv_transfer.kv_connector.v1.ssm_conv_transfer_utils import (  # noqa: E501
            MambaConvSplitInfo,
        )
...
def test_derive_mamba_conv_split(
    ...
    and calls Aphrodite's derive_mamba_conv_split directly.
    """
    from aphrodite.distributed.kv_transfer.kv_connector.v1.ssm_conv_transfer_utils import (
