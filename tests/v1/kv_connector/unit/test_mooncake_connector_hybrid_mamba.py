...
GDN is represented as a MambaSpec in Aphrodite, so these tests exercise the
...
def test_hybrid_gdn_remote_decode_truncates_prefill_before_cache_lookup():
    ...
    connector = MooncakeConnector(
        ...
        make_hybrid_gdn_kv_cache_config(aphrodite_config.cache_config.block_size),
    )
...
def test_register_kv_caches_emits_fa_and_gdn_regions(monkeypatch):
    ...
    with set_current_aphrodite_config(aphrodite_config), patch_worker_dependencies():
        ...
        (fa_cache,) = dense_kv_cache_views(fa_raw, fa_spec, num_blocks, 1, KVCacheLayout.LBHNC)
        (gdn_cache,) = dense_kv_cache_views(gdn_raw, gdn_spec, num_blocks, 1, KVCacheLayout.LBHNC)
...
async def test_build_transfer_params_uses_non_overlapping_physical_pages():
    ...
    worker._layer_specs = {
        layer_name: group.kv_cache_spec for group in kv_cache_config.kv_cache_groups for layer_name in group.layer_names
    }
    ...
    assert src_ptrs == [fa_cache.data_ptr() + local_logical_block * ratio * physical_page_bytes]
    assert dst_ptrs == [remote_base_addr + remote_logical_block * ratio * physical_page_bytes]
    assert lengths == [ratio * physical_page_bytes]

    def split_contiguous_blocks(src_ids, dst_ids):
        return [[block_id] for block_id in src_ids], [[block_id] for block_id in dst_ids]

    with patch(
        "aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.mooncake_connector.group_concurrent_contiguous",
    ...
    expected_src_ptrs = [
        fa_cache.data_ptr() + (local_logical_block * ratio + offset) * physical_page_bytes for offset in range(ratio)
    ]
    expected_dst_ptrs = [
        remote_base_addr + (remote_logical_block * ratio + offset) * physical_page_bytes for offset in range(ratio)
    ]
    assert err_reqs == []
    assert err_msg is None
    ...
    assert all(next_ptr - ptr == physical_page_bytes for ptr, next_ptr in zip(src_ptrs, src_ptrs[1:]))
    assert all(next_ptr - ptr == physical_page_bytes for ptr, next_ptr in zip(dst_ptrs, dst_ptrs[1:]))
