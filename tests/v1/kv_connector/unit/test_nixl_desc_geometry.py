...
class _RecordingNixl:
    ...
    def make_prepped_xfer(self, op, local_handle, local_ids, remote_handle, remote_ids, notif_msg=None):
        handle = self._next_handle
        self._next_handle += 1
        self.xfers.append(
            (
    ...
    def get_xfer_telemetry(self, handle):
        ...
        return SimpleNamespace(xferDuration=1.0, postDuration=1.0, totalBytes=1, descCount=1)
...
def _make_mla_hybrid_worker(local_block_size, kernel_block_size, num_logical_blocks):
    ...
    aphrodite_config = create_aphrodite_config(block_size=local_block_size)
    ...
    aphrodite_config.kv_transfer_config.kv_buffer_device = "cuda"
    ...
    with (
        ...
        patch(
            "aphrodite.model_executor.layers.mamba.mamba_utils.get_conv_state_layout",
            return_value="DS",
        ),
        set_current_aphrodite_config(aphrodite_config),
    ):
        worker = NixlConnectorWorker(aphrodite_config, "local-engine", kv_cache_config)
        worker.use_mla = True
        ...
        tensors = [torch.zeros(num_logical_blocks * ppl, unified_page // ppl, dtype=torch.uint8) for _ in range(2)]
...
def test_register_compressed_indexer_uses_virtual_transfer_pages(logical_block_size, tail_first):
    ...
    aphrodite_config = create_aphrodite_config(block_size=logical_block_size)
    ...
    num_transfer_blocks = num_logical_blocks * (logical_block_size // transfer_block_size)
    ...
    assert expected_descs[-1, 0] + expected_descs[-1, 1] == (raw.data_ptr() + raw.nbytes)
...
def _assert_local_writes_within(worker, owned_ranges):
    ...
    assert not violations, (
        f"{len(violations)}/{total_descs} local descriptors write outside the request's own blocks: {violations[:10]}"
    )
    return total_descs
...
def test_hetero_ppl_multi_read_writes_stay_within_request_blocks():
    ...
    worker = _make_mla_hybrid_worker(local_block_size=12, kernel_block_size=4, num_logical_blocks=8)
...
def _run_hetero_case(local_block, kernel, remote_block, num_tokens, tp_size=2, remote_kernel=None):
    ...
    for op, lh, lids, rh, rids in nixl.xfers:
        ...
        for li, ri in zip(lids, rids):
            ...
            assert lkind == rkind, (
                f"pair kind mismatch: local {lkind} vs remote {rkind} (local desc {li}, remote desc {ri})"
            )
            assert lreg == rreg, f"region mismatch: local {lreg} vs remote {rreg} for tokens {ltok} vs {rtok}"
            if lkind == "attn":
                assert ltok == rtok, (
...
def test_hetero_ppl_token_alignment_sweep(local_block, remote_block, num_tokens):
    ...
    _run_hetero_case(local_block, kernel=4, remote_block=remote_block, num_tokens=num_tokens)
...
def test_mismatched_mla_kernel_page_rejected_for_mla_hybrid():
    """Reject MLA hybrid handshakes with mismatched per-token pages."""
    worker = _make_mla_hybrid_worker(local_block_size=12, kernel_block_size=4, num_logical_blocks=8)
    meta_r = _make_remote_meta(
        worker,
        remote_block_size=8,
...
def _make_csa_linear_ple_worker(scratch_aliases: str = "compressed"):
    ...
    from aphrodite.config import set_current_aphrodite_config
    from aphrodite.distributed.kv_transfer.kv_connector.v1.nixl import base_worker as bw
    from aphrodite.distributed.kv_transfer.kv_connector.v1.nixl.worker import (
        NixlConnectorWorker,
    )
    from aphrodite.v1.attention.backends.registry import MambaAttentionBackendEnum
    ...
    aphrodite_config = create_aphrodite_config(block_size=4)
    aphrodite_config.cache_config.enable_prefix_caching = False
    aphrodite_config.kv_transfer_config.kv_buffer_device = "cuda"
    fake_backend = MagicMock()
