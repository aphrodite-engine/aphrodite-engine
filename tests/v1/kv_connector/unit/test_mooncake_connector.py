...
def patch_worker_dependencies():
    ...
    with (
        ...
        patch(
            "aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.mooncake_connector.get_ip",
            return_value="127.0.0.1",
        ),
        patch(
            "aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.mooncake_connector.get_tensor_model_parallel_rank",
            return_value=0,
        ),
        patch(
        ...
        patch(
            "aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.mooncake_connector.make_zmq_socket"
        ) as mock_make_zmq,
        ...
        with patch.object(prefill_worker, "_send_blocks", return_value=0) as mock_send_blocks:
            for d_rank in target_d_ranks:
                ...
                for blk_idx, (lblk, rblk) in enumerate(zip(flat_local, flat_remote)):
                    assert src_ptrs[blk_idx] == (local_region_base + lblk * local_block_len + expected_src_off)
                    assert dst_ptrs[blk_idx] == (remote_region_base + rblk * remote_block_len + expected_dst_off)
