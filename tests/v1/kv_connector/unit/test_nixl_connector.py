...
@patch(
    "aphrodite.distributed.kv_transfer.kv_connector.v1.nixl_connector.NixlWrapper",
    FakeNixlWrapper,
)
def test_kv_transfer_handshake(dist_init):
    ...
    # metadata.
    prefill_connector = NixlConnector(aphrodite_config, KVConnectorRole.WORKER)
    kv_cache_shape = FlashAttentionBackend.get_kv_cache_shape(num_blocks=2, block_size=16, num_kv_heads=4, head_size=64)
    ...
    delay, kv_connector_metadata = scheduler.get_kv_connector().request_finished(request, [0, 1, 2])
    assert delay

    # Decode connector will be able to create handshake with the prefill connector.
    decode_connector = NixlConnector(aphrodite_config, KVConnectorRole.WORKER)
...
class FakeNixlConnectorWorker(NixlConnectorWorker):
    ...
    def __init__(self, *args, hand_shake_latency: float = 1.8, kv_cache_layout="HND", **kwargs):
...
class TestNixlHandshake:
    ...
    def test_multi_xfer_one_engine(
        ...
        connector = NixlConnector(aphrodite_config, KVConnectorRole.WORKER)
    ...
    def test_handshake_fails_on_kv_cache_layout_mismatch(self, dist_init):
        ...
        # remote_tp_size=1
        with patch(
            "aphrodite.distributed.kv_transfer.kv_connector.v1.nixl_connector.get_tensor_model_parallel_world_size",  # noqa: E501
            return_value=2,
        ):
    ...
    def test_handshake_succeed_on_kv_cache_layout_mismatch_with_experimental(self, dist_init):
        """
        Verify that adding a remote agent fails if kv_cache_layout differs.
        This test is only relevant for heterogeneous TP.
        """
...
def test_abort_timeout_on_prefiller(monkeypatch, distributed_executor_backend):
    ...
    monkeypatch.setenv("APHRODITE_NIXL_ABORT_REQUEST_TIMEOUT", str(timeout))
    ...
    if distributed_executor_backend == "ray":
        with _make_fake_nixl_pkg() as working_dir:
            runtime_env = {
                ...
                "env_vars": {
                    "APHRODITE_NIXL_ABORT_REQUEST_TIMEOUT": str(timeout),
...
def test_register_kv_caches(dist_init):
    ...
    with (
        patch("aphrodite.distributed.kv_transfer.kv_connector.v1.nixl_connector.NixlWrapper") as mock_nixl_wrapper,
        patch("aphrodite.distributed.kv_transfer.kv_connector.v1.nixl_connector.threading.Event"),
        patch("aphrodite.distributed.kv_transfer.kv_connector.v1.nixl_connector.threading.Thread") as mock_thread,
    ):  # noqa: E501
        # Create connector
        connector = NixlConnector(aphrodite_config, KVConnectorRole.WORKER)
        connector.connector_worker = FakeNixlConnectorWorker(
...
def test_kv_buffer_to_nixl_memory_types(dist_init, kv_buffer_device, nixl_memory_type):
    ...
    from aphrodite.distributed.kv_transfer.kv_connector.v1.nixl_connector import _NIXL_SUPPORTED_DEVICE

    _NIXL_SUPPORTED_DEVICE.update(FakePlatform.get_nixl_supported_devices())

    with (
        patch("aphrodite.distributed.kv_transfer.kv_connector.v1.nixl_connector.NixlWrapper"),
        patch("aphrodite.distributed.kv_transfer.kv_connector.v1.nixl_connector.threading.Event"),
        patch("aphrodite.distributed.kv_transfer.kv_connector.v1.nixl_connector.threading.Thread"),
        patch(
            "aphrodite.distributed.kv_transfer.kv_connector.v1.nixl_connector.current_platform",
            FakePlatform,
        ),
        patch(
            "aphrodite.distributed.kv_transfer.kv_connector.v1.nixl_connector._NIXL_SUPPORTED_DEVICE",
...
def test_shutdown_cleans_up_resources(dist_init):
    ...
    scheduler = NixlConnectorScheduler(aphrodite_config, aphrodite_config.kv_transfer_config.engine_id)

    worker = NixlConnectorWorker(aphrodite_config, aphrodite_config.kv_transfer_config.engine_id)
