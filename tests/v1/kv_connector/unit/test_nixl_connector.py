        ...
        for remote_tp_rank in range(num_handshakes):
            remote_agent_name = self.add_remote_agent(
                NixlAgentMetadata(
                    ...
                    # `self.kv_cache_layout` is only forced to HND when aphrodite engine
...
class TestNixlHandshake:
    ...
    def test_hybrid_mamba_attention_remote_descs_use_packed_head_slices(self, default_aphrodite_config, dist_init):
        ...
        assert worker.get_backend_aware_kv_block_len(0, mamba_view=False) == (local_block_len)
        assert worker.get_backend_aware_kv_block_len(0, first_split=True, mamba_view=True) == worker._mamba_ssm_size[0]
        assert worker.get_backend_aware_kv_block_len(0, first_split=False, mamba_view=True) == worker._mamba_ssm_size[1]
...
def test_register_kv_caches(default_aphrodite_config, dist_init, attn_backend, enable_cross_layers):
    ...
    nixl_connector = "aphrodite.distributed.kv_transfer.kv_connector.v1.nixl.connector"
    with (
