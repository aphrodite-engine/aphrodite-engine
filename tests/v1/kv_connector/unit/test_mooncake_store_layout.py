...
def _rank_local_layout(num_regions: int, num_block_lens: int) -> RankLocalStoreLayout:
    ...
    layout.set_kv_caches_base_addr([0x7F00_0000_0000 + i * (1 << 30) for i in range(num_regions)])
...
def test_rank_local_descriptors_match_reference(num_regions: int, num_block_lens: int):
    ...
    addrs, sizes, selected_blocks = layout.prepare_values(chunks, block_ids, [0] * len(chunks))

    for chunk, chunk_addrs, chunk_sizes, block_id in zip(chunks, addrs, sizes, selected_blocks, strict=True):
        assert (chunk_addrs, chunk_sizes, block_id) == _rank_local_reference(layout, *chunk, block_ids)
...
def test_tp_shared_layout_owns_store_namespace(layout_cls, store_format):
    assert layout_cls.shared_namespace(4, 2) == f"@store_tp:4@store_pp:2@store_format:{store_format}"
...
def test_tp_shared_layout_round_trip_across_tp_sizes(layout_cls, producer_strides, consumer_strides):
    block_size = 16
    ...
    for tp_rank in range(4):
        ...
        tensor.copy_(torch.arange(128, dtype=torch.float16).view(shape) + tp_rank * 1000)
        ...
        stored[tp_rank] = b"".join(ctypes.string_at(addr, size) for addr, size in zip(addrs[0], sizes[0], strict=True))

    for tp_rank in range(2):
        tensor = torch.empty_strided((1, 4, block_size, 4), consumer_strides, dtype=torch.float16)
        tensor.zero_()
        metadata = KeyMetadata("test-model", tp_rank, 0, 0, 0)
        layout = layout_cls(
            metadata,
        ...
        addrs, sizes, _ = layout.prepare_values([(0, block_size)] * len(shard_ids), [0], shard_ids)
        for shard_id, shard_addrs, shard_sizes in zip(shard_ids, addrs, sizes, strict=True):
            offset = 0
