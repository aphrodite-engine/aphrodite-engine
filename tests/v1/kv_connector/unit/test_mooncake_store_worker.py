...
from aphrodite.distributed.kv_events import KVEventAggregator
from aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake import (
    rdma_utils,
)
from aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.store import (
    worker,
)
from aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.store import (
    worker as mooncake_store_worker,
)
from aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.store.data import (
    BlobBlockHashes,
...
from aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.store.metrics import (
...
def _tp_shared_prefix(tp_rank: int) -> str:
    return f"test-model{_TP_SHARED_NAMESPACE}@tp_rank:{tp_rank}@pcp0@dcp0@pp_rank:0@group:0"
...
def _make_aphrodite_config(
    *,
    ...
    return SimpleNamespace(
        ...
        kv_transfer_config=_FakeKVTransferConfig(kv_role=kv_role, extra_config=extra_config),
...
def _make_kv_cache_config(*, block_size: int = 16, prefix_cache_retention_interval: int | None = 0) -> object:
...
def test_partial_tail_offload_skips_null_source_blocks():
    ...
    keys, addrs, _sizes, _replicate_config = store.batch_put_from_multi_buffers.call_args.args
    assert keys == [
...
def test_partial_tail_offload_skips_cap_omitted_mamba_group():
    ...
    full_spec = FullAttentionSpec(block_size=4, num_kv_heads=8, head_size=64, dtype=torch.float32)
...
def test_partial_tail_offload_replaces_stale_group_ids_after_filtering():
    ...
    assert config.group_ids == [
        "aphrodite-mooncake-store:test-model@6131",
        "aphrodite-mooncake-store:test-model@6132",
        "aphrodite-mooncake-store:test-model@6132",
    ]
...
def test_normal_save_excludes_mamba_group_and_null_blocks():
    ...
    full_spec = FullAttentionSpec(block_size=16, num_kv_heads=8, head_size=64, dtype=None)
    mamba_spec = MambaSpec(
        block_size=16,
        shapes=((1, 1),),
        dtypes=(torch.float32,),
    ...
    db_full = ChunkedTokenDatabase(KeyMetadata("test-model", 0, 0, 0, 0, group_id=0), block_size=16)
    db_full.set_kv_caches_base_addr([0x1000])
    db_full.set_block_len([256])
    db_mamba = ChunkedTokenDatabase(KeyMetadata("test-model", 0, 0, 0, 0, group_id=1), block_size=16)
    db_mamba.set_kv_caches_base_addr([0x2000])
    db_mamba.set_block_len([256])
    thread = _make_store_sending_thread(store, coord=coord, token_databases=[db_full, db_mamba])
...
def test_store_sending_thread_prepares_missing_chunks_once_per_group():
    ...
    db0.store_layout.prepare_values.assert_called_once_with([(0, 16), (32, 48)], [1, 2, 3], [0, 0])
    db1.store_layout.prepare_values.assert_called_once_with([(16, 32), (32, 48)], [3, 2, 1], [0, 0])
...
@pytest.mark.parametrize("failing_call", ["batch_is_exist", "batch_put_from_multi_buffers"])
...
def test_store_sending_thread_sets_group_ids_when_enabled():
    ...
    assert config is replicate_config
    assert config.group_ids == [
        "aphrodite-mooncake-store:test-model@6130",
        "aphrodite-mooncake-store:test-model@6131",
    ]
...
def test_mooncake_group_id_uses_logical_chunk_and_excludes_physical_ranks():
    ...
    assert group_id == "aphrodite-mooncake-store:test-model@abcdef"
...
def test_mooncake_group_id_uses_cache_prefix_namespace():
    ...
    assert group_id_a == "aphrodite-mooncake-store:deployment-a@test-model@abcdef"
    assert group_id_b == "aphrodite-mooncake-store:deployment-b@test-model@abcdef"
...
def test_store_sending_thread_group_ids_share_across_kv_cache_groups():
    ...
    assert sizes == [[256], [256], [256], [256]]
    # Different Aphrodite KV cache groups for the same prefix chunk share the
...
def test_store_sending_thread_group_ids_follow_missing_key_filter():
    ...
    assert config.group_ids == ["aphrodite-mooncake-store:test-model@6131"]
...
def test_recv_thread_uses_single_batch_when_no_disk_offload_budget(monkeypatch):
    monkeypatch.delenv("APHRODITE_MOONCAKE_STORE_TIER_LOG", raising=False)
    store = MagicMock()
...
def test_requester_worker_init_prefers_local_hostname_override(
    ...
    worker.MooncakeStoreWorker(_make_aphrodite_config(rank=1), _make_kv_cache_config())
...
def test_worker_enables_lbhnc_store_tp_layout(tmp_path, monkeypatch):
    ...
    w = worker.MooncakeStoreWorker(
        _make_aphrodite_config(extra_config={"store_tp_size": 4}),
...
def test_lcm_store_tp_gives_prefill_and_decode_common_namespace(tmp_path, monkeypatch):
    ...
    for tp_size, tp_rank, kv_role, expected_shards in topologies:
        ...
        store_worker = worker.MooncakeStoreWorker(
            _make_aphrodite_config(extra_config=extra_config, kv_role=kv_role),
            _make_kv_cache_config(),
        )
    ...
    shard_keys = [store_worker.token_dbs[0].store_layout.key_for(2, BlockHash(b"h")) for store_worker in workers]
...
def test_lcm_store_tp_falls_back_when_topology_is_incompatible(
    ...
    assert "@store_format:rank_local_tp" in store_worker.token_dbs[0].key_for(BlockHash(b"h"))
...
def test_worker_scales_uniform_attention_group_under_dcp(tmp_path, monkeypatch):
    ...
    inner_specs = {
        "layer0": FullAttentionSpec(block_size=16, num_kv_heads=8, head_size=64, dtype=None),
        "layer1": FullAttentionSpec(block_size=16, num_kv_heads=4, head_size=64, dtype=None),
    }
    ...
    w = worker.MooncakeStoreWorker(
        _make_aphrodite_config(decode_context_parallel_size=4),
        kv_cache_config,
    )
...
def test_mqa_p4_to_d2_uses_shared_rank_zero_namespace(tmp_path, monkeypatch):
    ...
    assert (
        put_keys
        == get_keys
        == {("test-model@store_pp:1@store_format:tp_shared_mqa@tp_rank:0@pcp0@dcp0@pp_rank:0@group:0@68617368")}
    )
...
def test_requester_worker_group_semantics_falls_back_without_group_ids(
    ...
    assert any("does not support ReplicateConfig.group_ids" in call.args[0] for call in warning.call_args_list)
...
def test_requester_worker_group_semantics_string_false_does_not_enable(
    ...
    assert not any("does not support ReplicateConfig.group_ids" in call.args[0] for call in warning.call_args_list)
...
def test_store_sending_thread_kv_events_use_request_chain_parents(
    ...
    assert [event.parent_block_hash for event in events] == [
        (None if i is None else maybe_convert_block_hash(BlockHash(f"a{i}".encode()))) for i in parent_indices
    ]
...
def _register_with_mocked_threads(
    ...
    prefix = "aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.store.worker."
    with (
...
def _refresh_group_tp_replication_factors(
    ...
    worker._group_tp_replication_factors = worker._compute_group_tp_replication_factors()
...
def test_lookup_key_prefixes_cover_store_tp_shards():
    ...
    assert worker._lookup_key_prefixes[0] == (*(_tp_shared_prefix(tp_rank) for tp_rank in range(4)),)
...
def test_group_tp_replication_factors_mixed_mla_gqa_mamba():
    ...
    worker._kv_cache_groups = [
        KVCacheGroupSpec(
            ["l0"],
            MLAAttentionSpec(block_size=16, num_kv_heads=1, head_size=64, dtype=None),
        ),
        KVCacheGroupSpec(
            ["l1"],
    ...
    assert tuple(len(prefixes) for prefixes in worker._lookup_key_prefixes) == (
        1,
        2,
        4,
    )
...
def test_lookup_plan_resolves_group_tail_keys_from_existing_hashes():
    ...
    def exists(keys):
        return [
            int(any(f"@group:{group_id}@{block_hash.hex()}" in key for group_id, block_hash in present)) for key in keys
        ]
...
def test_lookup_applies_swa_mask_before_accessing_hashes():
    ...
    # Full-attention chunks (compact hashes at 3, 7, 11) plus only the
    # reachable SWA boundary tails (chunks 3, 7, 11) are hash-accessed. Every
    # masked SWA chunk in between is skipped before both the hash access and
    # key build.
...
def test_consumer_starts_send_thread_only_when_put_is_enabled(save_decode_cache):
    ...
    worker = _make_bare_worker(kv_role="kv_consumer", save_decode_cache=save_decode_cache)
...
def test_register_kv_caches_shared_storage(layout: KVCacheLayout):
    ...
    if not layout.is_block_compact:
        ...
        assert db.kv_caches_base_addr == [
            cache[:, head_idx].data_ptr() for cache in caches for head_idx in range(cache.shape[1])
        ]
        assert db.block_len == [head_cache.stride(0) * head_cache.element_size()] * (num_layers * caches[0].shape[1])
...
def test_register_kv_caches_separate_head_groups():
    ...
    expected_addrs = [cache[:, head_idx].data_ptr() for cache in caches for head_idx in range(cache.shape[1])]
    db = worker.token_dbs[0]
...
def test_non_default_tenant_preserves_setup_type_error(tmp_path, monkeypatch):
    store = MagicMock()
    setup_error = TypeError("setup(): incompatible function arguments; supported signature includes tenant_id")
