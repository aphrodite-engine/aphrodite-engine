...
class MockKVConnector(KVConnectorBase_V1):
    ...
    def __init__(
        ...
        self.config = MockKVConfig(
            ...
            num_defers_before_matching=extra_config.get("num_defers_before_matching", 0),
            supports_divergent_local_hybrid_hits=extra_config.get("supports_divergent_local_hybrid_hits", False),
        )
        self._defers_left: defaultdict[str, int] = defaultdict(lambda: self.config.num_defers_before_matching)
