...
from aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.store.scheduler import (
    MooncakeStoreScheduler,
)
from aphrodite.v1.core.block_pool import BlockPool
from aphrodite.v1.core.sched.output import KVConnectorBlockState
...
def _make_bare_scheduler(
    *,
    ...
    scheduler.kv_cache_config = SimpleNamespace(select_transfer_block_ids=lambda block_ids: tuple(block_ids))
    ...
    scheduler._gpu_block_pool = BlockPool(num_gpu_blocks=64, enable_caching=True, hash_block_size=hash_block_size)
...
def _make_decode_scheduler_output(*, num_computed_tokens: int, num_scheduled_tokens: int = 1) -> SimpleNamespace:
    return SimpleNamespace(
        finished_req_ids=set(),
        preempted_req_ids=set(),
        scheduled_new_reqs=[],
...
def _make_worker_output(completed_saves: dict[int, int]) -> SimpleNamespace:
    return SimpleNamespace(kv_connector_worker_meta=MooncakeStoreWorkerMetadata(completed_saves=completed_saves))
...
def test_update_state_excludes_nontransfer_groups():
    ...
    scheduler = _make_bare_scheduler()
    scheduler.kv_cache_config = SimpleNamespace(select_transfer_block_ids=lambda block_ids: (block_ids[0],))
...
def _setup_decode_request(
    *,
    ...
    scheduler = _make_bare_scheduler(kv_role=kv_role, save_decode_cache=save_decode_cache)
...
def test_decode_tracking_is_skipped_by_default(kv_role):
    ...
    meta = scheduler.build_connector_meta(_make_decode_scheduler_output(num_computed_tokens=47))
...
def test_fresh_consumer_first_decode_save_can_backfill_missing_prompt():
    ...
    [prompt_meta] = scheduler.build_connector_meta(_make_decode_scheduler_output(num_computed_tokens=32)).requests
...
def test_consumer_saves_only_full_decode_blocks(token_len, saved_tokens):
    scheduler, tracker = _setup_decode_request(save_decode_cache=True, token_len=token_len)

    meta = scheduler.build_connector_meta(_make_decode_scheduler_output(num_computed_tokens=token_len))
...
def test_sub_block_prompt_looks_up_with_fine_grained():
    ...
    need_to_allocate, load_async = scheduler.get_num_new_matched_tokens(request, num_computed_tokens=0)
...
def test_boundary_state_job_pins_exact_blocks_once():
    ...
    out = _make_offload_only_output([(1, 7, 16), (1, 8, 32), (1, 9, 48)], block_ids=([7],))
