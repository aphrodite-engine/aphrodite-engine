...
def _stub_sw_clipping(scheduler) -> None:
    """Make ``get_exchange_clipped_blocks`` a passthrough so tests don't need
...
class TestPushScheduler:
    ...
    def test_d_side_update_state_after_alloc_stages_registration(self):
        ...
        # ``request_id`` is D's own Aphrodite request id; plus our own (D) coords.
    ...
    def test_registration_watchdog_expires(self, caplog):
        ...
        caplog.set_level(
            logging.CRITICAL,
            logger=("aphrodite.distributed.kv_transfer.kv_connector.v1.nixl.push_scheduler"),
        )
        sched = make_nixl_push_scheduler()
        _stub_sw_clipping(sched)
...
class TestPushWriterMatching:
    ...
    def test_handle_push_reg_drops_malformed(self, caplog):
        ...
        caplog.set_level(
            logging.CRITICAL,
            logger=("aphrodite.distributed.kv_transfer.kv_connector.v1.nixl.push_worker"),
        )
        w = _StubWriterWorker.fresh()
...
class TestPushPipelineParallel:
    ...
    def test_req_meta_reads_pp_size_from_kv_transfer_params(self):
        """D learns the producer's pp_size from kv_transfer_params and
    ...
    def test_add_remote_agent_slices_remote_regions_to_local_pp_window(self):
        ...
        this worker holds only a contiguous layer slice."""
        block_len = 4096 * 16
        w = _StubWriterWorker.fresh()
        w.pp_size = 2
        w._remote_region_offset = 2
        w.block_len_per_layer = [block_len, block_len]
        w.nixl_wrapper = MagicMock()
        ...
        meta = NixlAgentMetadata(
            ...
            kv_caches_base_addr=[10, 11, 12, 13],
            device_id=0,
            num_blocks=4,
...
class TestPushWriterMlaReplication:
    ...
    the producer must WRITE the latent into every D rank it handshook."""
...
class TestPushPrefixCaching:
    ...
    def test_partial_prefix_hit_end_trims_producer_blocks(self):
        ...
        NixlPushConnectorWorker._do_start_push_kv(w, "req-pc", ([10, 11, 12, 13, 14],), reg)
