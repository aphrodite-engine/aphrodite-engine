...
class DecodeBenchTestRunner:
    ...
    def __init__(self, block_size: int, num_gpu_blocks: int):
        ...
        self.kv_caches = {
            layer_name: torch.zeros(num_gpu_blocks, 2, num_heads, block_size, head_dim) for layer_name in layer_names
        }
...
def test_decode_bench_connector_fills_each_hma_group():
    ...
    connector.bind_connector_metadata(DecodeBenchConnectorMetadata(reqs_to_fill={"request": (([1], [5]), block_size)}))

    connector.start_load_kv(ForwardContext(no_compile_layers={}, attn_metadata={}, slot_mapping={}))
...
def test_decode_bench_connector_zero_fills_circular_buffer_groups():
    ...
    connector.bind_connector_metadata(DecodeBenchConnectorMetadata(reqs_to_fill={"request": (([5],), 1)}))
    ...
    blocks = KVCacheBlocks(
        (
            [
                ...
                KVCacheBlock(
                    block_id=2,
                    _block_hash=make_block_hash_with_group_id(BlockHash(b"external-full-block"), 0),
