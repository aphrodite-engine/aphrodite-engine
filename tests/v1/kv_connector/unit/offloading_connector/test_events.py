...
def _lookup_chunk() -> tuple[OffloadingEventsTracker, MagicMock, GroupOffloadConfig, OffloadKey]:
    tracker = _tracker()
...
def test_partial_tail_event_describes_hash_aligned_physical_block_prefix():
    tracker = _tracker()
    group_config = _group_config(block_size=16, blocks_per_chunk=1)._replace(hashes_per_chunk=4)
...
@pytest.mark.parametrize("record_method", ["record_partial_store", "record_partial_lookup"])
...
def test_event_hashes_use_group_block_size(blocks_per_chunk: int, expected_hash_indices: list[int]):
    tokens_per_hash = 4
    block_size = 256
...
def test_lookup_promotion_factor_gt_1_store_and_remove():
    ...
    blocks_per_chunk = 2
    tracker = _tracker()
    group_config = _group_config(block_size=block_size, blocks_per_chunk=blocks_per_chunk)
    req = _request(
...
def test_event_metadata_skips_non_full_attention_group(
    ...
    tracker = _tracker()
    group_config = _group_config(sliding_window_size_in_chunks=sliding_window_size_in_chunks)
    req = _request(block_hashes=[_hash(i) for i in range(3)], token_count=12)
    keys = _record_chunks(tracker, req, group_config, num_chunks=3)
    _record_lookup_chunks(tracker, req, group_config, num_chunks=3)
...
def test_pending_cpu_removal_consumes_hit_backfill_until_next_hit():
    ...
    list(tracker.take_events([_stored_event([key], Medium.STORAGE, ownership="custom")]))
...
def test_reoffload_preserves_secondary_residency(record_method, position):
    ...
    [removed] = tracker.take_events([_removed_event([key], Medium.STORAGE, ownership="custom")])
...
def test_tiering_accepts_self_describing_kv_events():
    ...
    spec = TieringOffloadingSpec(build_offloading_config(aphrodite_config, kv_cache_config))
