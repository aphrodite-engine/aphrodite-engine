...
def _make_partial_tail_scheduler() -> OffloadingConnectorScheduler:
    aphrodite_config = _make_aphrodite_config(extra_config={"self_describing_kv_events": True})
    aphrodite_config.cache_config.prefix_match_unit = 4
...
def _reduce_kv_connector_stats(runner):
    ...
    for payload in runner.kv_connector_stats:
        stats = payload if hasattr(payload, "reduce") else OffloadingConnectorStats(data=payload)
...
def test_aligned_boundary_store_uses_exact_source_with_partial_tail():
    ...
    assert len(jobs) == 2
    src_spec = next(job.src_spec for job in jobs.values() if len(job.src_spec.block_ids) == 1)
    assert isinstance(src_spec, GPULoadStoreSpec)
...
def test_recurrent_group_unhashed_block_does_not_truncate_load_boundary():
    ...
    full_blocks[0].set_block_hash(make_block_hash_with_group_id(BlockHash(b"h0"), 0), 16)
...
def test_final_sampled_token_does_not_complete_an_offloaded_block(
    ...
    runner.manager.prepare_store.side_effect = lambda keys, req_context: (generate_store_output(list(keys)))
...
def test_store_jobs_wait_for_missing_offload_key(request_runner):
    ...
    scheduler_output = SimpleNamespace(num_scheduled_tokens={req_id: 0}, finished_req_ids=set())
...
def test_offloading_connector(request_runner, async_scheduling: bool):
    ...
    runner.new_request(token_ids=[0] * block_size + [1] * (tokens_per_chunk - block_size))
    runner.manager.prepare_store.side_effect = lambda keys, req_context: (generate_store_output([]))
    runner.run(decoded_tokens=[EOS_TOKEN_ID])
...
@pytest.mark.parametrize("async_scheduling", [True, False])
def test_on_request_finished_not_deferred_until_store_completion(request_runner, async_scheduling: bool):
...
@pytest.mark.parametrize("async_scheduling", [True, False])
def test_on_request_finished_fires_after_final_block_store(request_runner, async_scheduling: bool):
    ...
    def prepare_store(keys, req_context):
        ...
        calls.append(("stored_after_finish" if finished else "prepare_store", req_context.req_id))
    ...
    finished_idx = calls.index(("on_request_finished", req_id))
    store_indices = [
        i for i, c in enumerate(calls) if c in (("prepare_store", req_id), ("stored_after_finish", req_id))
    ]
...
def test_sliding_window_demand_is_store_reachable(
    ...
    sched = _make_scheduler_with_lookup({i: LookupResult.HIT_PENDING for i in range(storable_chunk_count)})
    sched._sliding_window_lookup(to_keys(range(storable_chunk_count)), required_window, _EMPTY_REQ_CTX)

    demanded = {
        int(get_offload_block_hash(lookup_call.args[0]).decode()) for lookup_call in sched.manager.lookup.call_args_list
    }
    ...
    assert all(0 <= chunk_idx < storable_chunk_count for chunk_idx in demanded), (
        f"Lookup path demanded chunks {demanded} outside valid range [0, {storable_chunk_count})"
    )
...
def test_fence_at_build_store_jobs(request_runner):
    ...
    runner.scheduler.reset_prefix_cache()
    runner.new_request(token_ids=[1] * 4)
    runner.connector_scheduler._maximal_prefix_lookup = lambda key, req_context, *_: 0
    runner.manager.prepare_store.side_effect = lambda keys, req_context: (generate_store_output([]))
    runner.run(
        decoded_tokens=[EOS_TOKEN_ID],
        expected_stored=(0,),
...
def test_reset_cache_finalizes_finished_request_with_pending_store(request_runner, async_scheduling: bool):
    """reset_cache finalizes a finished request whose stores it discards."""
    block_size = 4
    blocks_per_chunk = 3
    tokens_per_chunk = block_size * blocks_per_chunk
    ...
    # reset_cache issues the deferred notification before dropping the state.
    cs.reset_cache()
...
def test_async_preempt_readmit_before_transfer_output_is_deferred(request_runner):
    ...
    runner.connector_scheduler._maximal_prefix_lookup = lambda key, req_context, *_: len(key)
...
def test_swa_alignment_skip(request_runner, async_scheduling: bool):
    ...
    runner.manager.lookup.return_value = LookupResult.HIT
    runner.connector_scheduler._maximal_prefix_lookup = lambda key, req_context, *_: 2
    runner.run(
        decoded_tokens=[EOS_TOKEN_ID],
...
def _full_and_swa_groups(full_attn_block_size: int, swa_block_size: int, sliding_window: int) -> list[KVCacheGroupSpec]:
...
def _demanded_keys(runner, offload_keys_per_group: list[list], num_tokens: int) -> set:
    ...
    for group_config, offload_keys in zip(runner.connector_scheduler.config.kv_group_configs, offload_keys_per_group):
...
def test_request_level_supply_covers_consumer_demand(request_runner, warmth: str, with_swa_group: bool):
    ...
    kv_cache_groups = (
        _full_and_swa_groups(full_attn_block_size, swa_block_size, sliding_window=8) if with_swa_group else None
    )
    runner = request_runner(
    ...
    def capture_offload_keys() -> None:
        for req_status in runner.connector_scheduler._req_status.values():
            ...
            offload_keys_per_group.extend(list(group_state.offload_keys) for group_state in req_status.group_states)
    ...
    supplied = {key for store_call in runner.manager.prepare_store.call_args_list for key in store_call.args[0]}
...
class TestEagle:
    ...
    def test_sw_lookup_inflates_query_max(self, request_runner):
        ...
        def capturing_sw_lookup(self_arg, keys, window, req_context, *args):
            captured_keys.append(list(keys))
            return orig_sw_lookup(self_arg, keys, window, req_context, *args)

        sched._sliding_window_lookup = lambda keys, window, req_ctx, *args: capturing_sw_lookup(
            sched,
            keys,
            window,
            req_ctx,
    ...
    @pytest.mark.parametrize("async_scheduling", [True, False])
    def test_full_attn_store_excludes_trailing_decode_block(self, request_runner, async_scheduling: bool):
        ...
        # so the held-back eagle chunk becomes storable.
        runner.run(
            decoded_tokens=[EOS_TOKEN_ID],
            expected_stored=((1, 3),),
        )
    ...
    @pytest.mark.parametrize("async_scheduling", [True, False])
    def test_sw_store_excludes_trailing_decode_block(self, request_runner, async_scheduling: bool):
    ...
    @pytest.mark.parametrize("async_scheduling", [True, False])
    def test_single_block_stored_at_end_of_prefill(self, request_runner, async_scheduling: bool):
    ...
    @pytest.mark.parametrize("async_scheduling", [True, False])
    def test_multichunk_store_no_interior_holes(self, request_runner, async_scheduling: bool):
        ...
        offsets = sorted(b.request_block_offset for t in runner.completed_stores for b in t.gpu_blocks)
        ...
        assert offsets == list(range(len(offsets))), f"interior hole in stored blocks: {offsets}"
    ...
    def test_chunked_prefill_uses_final_swa_store_horizon(
        ...
        def abort_after_prefill_step() -> None:
            if runner.scheduler.running:
                runner.scheduler.finish_requests((str(runner.req_id),), RequestStatus.FINISHED_ABORTED)
    ...
    def test_dcp_abort_reconsiders_final_swa_tail(self):
        ...
        first_output.scheduled_new_reqs = [SimpleNamespace(req_id="req", block_ids=(full_block_ids, swa_block_ids))]
...
def test_reachable_block_mask_chunk_to_block_index_conversion(
    ...
    for chunk_idx in range(num_chunks):
        if any(block_mask[chunk_idx * blocks_per_chunk + b] for b in range(blocks_per_chunk)):
...
@pytest.mark.parametrize("async_scheduling", [True, False])
def test_retention_interval_sparsifies_swa_stores(request_runner, async_scheduling: bool):
...
@pytest.mark.parametrize("async_scheduling", [True, False])
def test_retention_interval_zero_stores_only_replay_boundary(request_runner, async_scheduling: bool):
...
class TestMambaHybridOffloadServing:
    ...
    def _make_scheduler(self, speculative_config, mamba_eagle=False):
        aphrodite_config = _make_aphrodite_config(extra_config={"self_describing_kv_events": True})
        aphrodite_config.cache_config.block_size = self.BLOCK
        aphrodite_config.cache_config.prefix_match_unit = self.BLOCK
    ...
    def _roundtrip_served_tokens(self, scheduler) -> int:
        ...
        scheduler.manager.prepare_store.side_effect = lambda keys, ctx: (generate_store_output(list(keys)))
        ...
        def complete_all_jobs():
            ...
            if job_ids:
                scheduler.update_connector_output(
                    KVConnectorOutput(
                        kv_connector_worker_meta=OffloadingWorkerMetadata(completed_jobs={jid: 1 for jid in job_ids})
                    )
                )
        ...
        def make_request(req_id):
            ...
            request.num_computed_tokens = 0
            request.block_hashes = [BlockHash(f"h{i}".encode()) for i in range(self.PROMPT_TOKENS // self.BLOCK)]
    ...
    def test_shared_group_mtp_serves_like_non_speculative(self):
        ...
        assert baseline == 16
        served = self._roundtrip_served_tokens(self._make_scheduler(_shared_kv_mtp_config()))
