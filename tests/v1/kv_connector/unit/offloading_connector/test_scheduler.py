...
def test_last_block_offloaded_at_request_finish(request_runner, async_scheduling: bool, prompt_offset: int):
...
def test_on_request_finished_fires_after_final_block_store(request_runner, async_scheduling: bool):
    ...
    assert calls.count(("on_request_finished", req_id)) == 1, calls
    finished_idx = calls.index(("on_request_finished", req_id))
    prepare_indices = [i for i, call_ in enumerate(calls) if call_ == ("prepare_store", req_id)]
