...
from aphrodite.v1.kv_offload.base import CanonicalPageMapping, CopyRun
...
def _mla_spec(**kwargs) -> MLAAttentionSpec:
    ...
    return MLAAttentionSpec(block_size=4, num_kv_heads=1, head_size=64, dtype=torch.int8, **kwargs)
...
def test_pcp_tokens_and_tp_heads_compose():
    ...
    per_rank = [_mapping(spec, cache, _ctx(rank, tp=2, pcp=2, total=2)) for rank in range(4)]
...
def test_interleave_chunks_stay_contiguous():
    ...
    _verify_tiling(
        "interleave",
        [_mapping(spec, None, _ctx(rank, tp=2, dcp=2, interleave=2)) for rank in range(2)],
    )
...
def test_all_ranks_tile_canonical_page(form):
    spec = _full_spec()
    per_rank = [_mapping(spec, CACHE_BUILDERS[form](spec), _ctx(rank, tp=4)) for rank in range(4)]
...
def test_cross_tp_store_load(form):
    ...
    def mappings_at(tp: int):
        ...
        return [_mapping(spec, cache, _ctx(rank, tp=tp, total=total_heads)) for rank in range(tp)]
...
def test_cp_round_trip():
    ...
    spec = _full_spec(num_kv_heads=1)
    cache = _split_nhd_cache(spec)
    mappings = [_mapping(spec, cache, _ctx(rank, tp=4, dcp=2, total=2)) for rank in range(4)]
...
def test_fail_closed_cases():
    ...
    assert _try_mapping(one_head, _split_nhd_cache(one_head), _ctx(0, tp=3, total=2)) is None
...
def _aphrodite_config(tp=1, dcp=1, pcp=1, pp=1, interleave=1, total_kv_heads=2):
    config = MagicMock()
...
def test_derive_mixed_model_with_dcp():
    ...
    mla_spec = _mla_spec()
    quant_spec = _full_spec(num_kv_heads=1, kv_quant_mode=KVQuantMode.FP8_PER_TOKEN_HEAD)
    ...
    mappings = derive_canonical_mappings(_aphrodite_config(tp=4, dcp=2, total_kv_heads=2), kv_cache_config, kv_caches)
    ...
    assert mappings["quant"].canonical_page_size_bytes == 4 * quant_spec.unpadded_page_size_bytes


def test_derive_refuses_foreign_worker_groups():
    attn_spec = _full_spec()
    kv_cache_config = _kv_cache_config([KVCacheGroupSpec(layer_names=["attn"], kv_cache_spec=attn_spec)])
    kv_caches = {"attn": _split_nhd_cache(attn_spec)}
    assert derive_canonical_mappings(_aphrodite_config(tp=2, pp=2, total_kv_heads=4), kv_cache_config, kv_caches) == {}
