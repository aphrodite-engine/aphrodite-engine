...
from aphrodite.v1.engine.tensor_ipc import (
    TensorIpcData,
    TensorIpcReceiver,
    TensorIpcSender,
)
...
def _collect_process_results(
    ...
    for _ in range(count):
        ...
        if remaining <= 0:
            states = ", ".join(f"{p.name}(pid={p.pid}, alive={p.is_alive()}, exit={p.exitcode})" for p in processes)
...
@dataclass
# Use a typed container so the test covers the real Aphrodite path where tensor IPC
...
def test_cuda_tensor_queue_basic():
    ...
    try:
        for process in processes:
            process.start()
        results = {result["role"]: result for result in _collect_process_results(result_queue, 2, processes)}
    finally:
        retrieval_done.set()
    ...
    try:
        ...
        for sender_idx in range(num_senders):
            ...
            assert len(messages) == num_messages_per_sender, (
                f"Sender {sender_idx}: expected {num_messages_per_sender} messages, got {len(messages)}"
            )
            for msg_idx, message in enumerate(messages):
                ...
                assert torch.allclose(message.t1, torch.full_like(message.t1, float(msg_idx)))
                assert torch.allclose(message.t2, torch.full_like(message.t2, float(msg_idx + 100)))
    finally:
        retrieval_done.set()
        process_failures = _cleanup_processes(processes)
