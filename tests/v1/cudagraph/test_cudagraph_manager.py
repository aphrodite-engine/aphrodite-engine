...
def test_piecewise_capture_uses_pcp_dummy_slot_mappings():
    ...
    block_tables.get_dummy_slot_mappings.return_value = torch.zeros(1, num_tokens, dtype=torch.int64)
...
def _create_decode_aphrodite_config(
    ...
    aphrodite_config = MagicMock(spec=AphroditeConfig)
    aphrodite_config.compilation_config = compilation_config
    aphrodite_config.scheduler_config = SchedulerConfig.default_factory(max_num_seqs=8)
    aphrodite_config.parallel_config = ParallelConfig()
    aphrodite_config.num_speculative_tokens = num_speculative_tokens
...
def _make_spec_decode_manager(
    ...
    manager = gpu_cudagraph_utils.CudaGraphManager(
        aphrodite_config=_create_decode_aphrodite_config(
...
def test_uniform_decode_pads_up_to_full_graph(monkeypatch):
    ...
    assert [(desc.cg_mode, desc.num_tokens) for desc in manager._candidates[(12, 0)]] == [
...
def test_divisor_query_len_dispatch_is_unchanged(monkeypatch, decode_query_len, capture_sizes):
    manager = _make_spec_decode_manager(
        monkeypatch,
        decode_query_len=decode_query_len,
