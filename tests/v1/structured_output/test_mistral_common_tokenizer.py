...
def _backend(cls, name, tokenizer):
    aphrodite_config = AphroditeConfig(structured_outputs_config=StructuredOutputsConfig(backend=name))
    return cls(aphrodite_config, tokenizer=tokenizer, vocab_size=tokenizer.vocab_size)
...
def test_wrapping_fixes_the_outlines_vocabulary(loaded_tokenizer, wrapped_tokenizer):
    ...
    mismapped = {token for token, ids in unwrapped_vocab.items() if ids != wrapped_vocab[token]}
    assert mismapped
