...
from aphrodite.v1.simple_kv_offload.copy_backend import DmaCopyBackend
from aphrodite.v1.simple_kv_offload.cuda_mem_ops import (
    CU_MEMCPY_SRC_ACCESS_ORDER_ANY,
    CU_MEMCPY_SRC_ACCESS_ORDER_STREAM,
    build_params,
    pin_tensor,
)
from aphrodite.v1.simple_kv_offload.metadata import SimpleCPUOffloadMetadata
from aphrodite.v1.simple_kv_offload.worker import SimpleCPUOffloadWorker
...
def test_register_shared_kv_cache_storage(monkeypatch, layout: KVCacheLayout):
    ...
    monkeypatch.setattr("aphrodite.v1.simple_kv_offload.worker.PIN_MEMORY", False)

    worker.register_kv_caches({f"layer.{layer_idx}": cache for layer_idx, cache in enumerate(caches)})
    ...
    assert {cache.shape for cache in worker.gpu_kv_caches.values()} == {(num_blocks, expected_block_bytes)}
...
def test_register_separate_kv_head_groups(monkeypatch):
    ...
    per_group_block_bytes = spec.num_kv_heads * spec.block_size * spec.head_size * spec.dtype.itemsize
    assert {cache.shape for cache in worker.gpu_kv_caches.values()} == {(num_blocks, per_group_block_bytes)}
