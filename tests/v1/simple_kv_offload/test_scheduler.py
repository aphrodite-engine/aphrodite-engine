...
from aphrodite.distributed.kv_transfer.kv_connector.v1.simple_cpu_offload_connector import (
...
from aphrodite.v1.simple_kv_offload.manager import SimpleCPUOffloadScheduler
from aphrodite.v1.simple_kv_offload.metadata import SimpleCPUOffloadWorkerMetadata
...
def test_eager_store_preserves_secondary_block_hashes() -> None:
    ...
    assert secondary_hash in fix.gpu_block_pool.cached_block_hashes_by_block[gpu_block.block_id]
    ...
    cpu_block = sched.cpu_block_pool.cached_block_hash_to_block.get_one_block(secondary_hash)
    ...
    assert secondary_hash in sched.cpu_block_pool.cached_block_hashes_by_block[cpu_block.block_id]
...
def test_finished_eager_stores_are_batched() -> None:
    ...
    hit_tokens, is_async = sched.get_num_new_matched_tokens(cached_request, num_computed_tokens=0)
    ...
    load_meta = sched.build_connector_meta(
        make_scheduler_output(
            ...
            new_reqs={cached_request.request_id: ([block.block_id for block in cached_gpu_blocks],)},
        )
    )
...
def test_finished_eager_store_caches_all_groups() -> None:
    ...
    request = make_request(num_blocks=2)
    group_blocks = tuple(
        _allocate_gpu_blocks(fix.gpu_block_pool, request, 2, group_id=group_id) for group_id in range(2)
    )
...
def test_toctou_cpu_hit_evicted_between_phases_no_crash() -> None:
    """Regression for https://github.com/vllm-project/vllm/issues/39702.
