...
from aphrodite.v1.simple_kv_offload.manager import (
...
def _make_mixed_kv_cache_config(
    ...
    register_all_kvcache_specs(aphrodite_config=None)
    groups = []
    tensors = []
...
def make_events_scheduler(
    ...
    if enable_events:
        aphrodite_config.kv_events_config = KVEventsConfig(enable_kv_cache_events=True)
...
def test_block_stored_per_group_metadata_full_attention() -> None:
    ...
    expected_hash_0 = maybe_convert_block_hash(get_block_hash(make_block_hash_with_group_id(req.block_hashes[0], 0)))
    ...
    ev1 = stored[1]
    expected_hash_1 = maybe_convert_block_hash(get_block_hash(make_block_hash_with_group_id(req.block_hashes[1], 0)))
...
def test_eager_store_lora_metadata() -> None:
    ...
    assert ev.lora_name == "test-lora", f"expected lora_name='test-lora', got {ev.lora_name!r}"
...
def test_finished_eager_store_emits_all_storage_events() -> None:
    ...
    fix.gpu_block_pool._insert_block_hash(secondary_hash, gpu_blocks[0], num_tokens=BLOCK_SIZE // 2)
    ...
    primary_cpu_block = sched.cpu_block_pool.cached_block_hash_to_block.get_one_block(primary_hashes[0])
    secondary_cpu_block = sched.cpu_block_pool.cached_block_hash_to_block.get_one_block(secondary_hash)
    ...
    primary_event_hashes = [maybe_convert_block_hash(get_block_hash(block_hash)) for block_hash in primary_hashes]
    ...
    assert by_hash[primary_event_hashes[0]].token_ids == req.prompt_token_ids[:BLOCK_SIZE]
    assert by_hash[primary_event_hashes[1]].token_ids == req.prompt_token_ids[BLOCK_SIZE : 2 * BLOCK_SIZE]
    ...
    removed = [event for event in sched.take_events() if isinstance(event, BlockRemoved)]
...
def test_secondary_hash_block_stored_metadata() -> None:
    ...
    fix.gpu_block_pool._insert_block_hash(primary_hash, gpu_block, num_tokens=BLOCK_SIZE)
    fix.gpu_block_pool._insert_block_hash(secondary_hash, gpu_block, num_tokens=BLOCK_SIZE // 2)
    ...
    assert by_hash[secondary_event_hash].token_ids == req.prompt_token_ids[: BLOCK_SIZE // 2]

    primary_cpu_block = sched.cpu_block_pool.cached_block_hash_to_block.get_one_block(primary_hash)
    secondary_cpu_block = sched.cpu_block_pool.cached_block_hash_to_block.get_one_block(secondary_hash)
    assert primary_cpu_block is not None
    assert secondary_cpu_block is primary_cpu_block
...
def test_block_removed_relabeled_medium_cpu() -> None:
    ...
    assert len(removed) >= 2, f"expected >=2 BlockRemoved from eviction, got {len(removed)}"
    for ev in removed:
        assert ev.medium == MEDIUM_CPU, f"BlockRemoved should be relabeled CPU, got {ev.medium}"
        assert ev.locality == "LOCAL", f"BlockRemoved should be LOCAL, got {ev.locality}"
    stored = [e for e in events if isinstance(e, BlockStored)]
    assert len(stored) == 2
...
def test_mamba_block_size_not_scaled() -> None:
    ...
    """
    kv_cfg = _make_mixed_kv_cache_config(num_blocks=16, with_mamba=True, mamba_block_size=32)
    fix = make_events_scheduler(
        num_cpu_blocks=8,
        num_gpu_blocks=16,
    ...
    assert ev.kv_cache_spec_sliding_window is None
    assert ev.locality == "LOCAL"
    assert ev.block_size == 32, f"Mamba block_size should be unscaled (32), got {ev.block_size}"
...
def test_disk_mode_block_removed_relabeled_storage() -> None:
    ...
    assert len(removed) >= 2, f"expected >=2 BlockRemoved from eviction, got {len(removed)}"
    for ev in removed:
        assert ev.medium == MEDIUM_STORAGE, f"BlockRemoved should be relabeled STORAGE, got {ev.medium}"
        assert ev.locality == "LOCAL", f"BlockRemoved should be LOCAL, got {ev.locality}"
    stored = [e for e in events if isinstance(e, BlockStored)]
    assert len(stored) == 2
    for ev in stored:
...
def test_mamba_dcp_metadata_guard() -> None:
    ...
    events = list(sched.take_events())
    stored = [e for e in events if isinstance(e, BlockStored)]
    assert len(stored) == 4, f"expected 4 BlockStored (2 FA + 2 Mamba), got {len(stored)}"
    ...
    for ev in fa_events:
        ...
        assert len(ev.token_ids) == vbs, f"FA token_ids should be {vbs}, got {len(ev.token_ids)}"
    ...
    for ev in mamba_events:
        ...
        assert ev.token_ids == [], f"Mamba token_ids should be [] (guard), got {ev.token_ids}"
        assert ev.parent_block_hash is None, "Mamba parent_block_hash should be None (guard)"
