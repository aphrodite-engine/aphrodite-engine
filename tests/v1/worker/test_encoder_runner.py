...
def _embeds_item(embeds: torch.Tensor) -> MultiModalKwargsItem:
    ...
    return MultiModalKwargsItem(
        {"embedding": MultiModalFieldElem(data=embeds, field=MultiModalSharedField(batch_size=1))}
    )
...
def test_prepare_mm_inputs_passes_prompt_embeds_through():
    ...
    runner = _make_runner([_pe_feature("hash_pe", prompt_embeds), image_feature], cached=[])
