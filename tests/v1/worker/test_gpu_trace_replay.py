...
from aphrodite.v1.worker.gpu.sample.trace_replay import (
    TraceReplayState,
    apply_trace_tokens,
)
...
def _trace_state(max_num_reqs: int) -> TraceReplayState:
    ...
    req_states = cast(
        ...
        SimpleNamespace(
            ...
            total_len=SimpleNamespace(gpu=torch.zeros(max_num_reqs, dtype=torch.int32, device=DEVICE)),
            prompt_len=SimpleNamespace(gpu=torch.zeros(max_num_reqs, dtype=torch.int32, device=DEVICE)),
        ),
    )
...
def test_replay_overwrites_sampled_at_each_step():
    ...
    trace_len = _i32([3, 3])
    trace_token_ids = torch.zeros(2, TEST_MAX_MODEL_LEN, dtype=torch.int32, device=DEVICE)
...
def test_past_end_of_trace_leaves_sampled_untouched():
    ...
    trace_token_ids = torch.zeros(1, TEST_MAX_MODEL_LEN, dtype=torch.int32, device=DEVICE)
