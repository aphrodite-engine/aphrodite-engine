...
"""Tests for the V2 model runner's InputBatch (aphrodite.v1.worker.gpu.input_batch)."""
...
def test_make_dummy_distributes_remainder(num_reqs: int, num_tokens: int):
    ...
    CI failure).
    """
    buffers = InputBuffers(max_num_reqs=num_reqs, max_num_tokens=num_tokens, device=torch.device(DEVICE))
    ...
    assert torch.equal(batch.seq_lens, torch.from_numpy(batch.num_scheduled_tokens).to(DEVICE))
    assert batch.query_start_loc_np[-1] == num_tokens
    assert torch.equal(batch.query_start_loc.cpu(), torch.from_numpy(batch.query_start_loc_np))
