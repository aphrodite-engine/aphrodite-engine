...
import aphrodite.v1.worker.gpu.kv_connector as kv_connector_module
from aphrodite.config import KVTransferConfig
from aphrodite.v1.worker.gpu.kv_connector import ActiveKVConnector
...
def _make_connector(
    ...
    monkeypatch.setattr(kv_connector_module, "is_forward_context_available", lambda: True)
...
def test_load_start_phase(
    ...
    assert events == (["handle", "bind", "start"] if has_sync_kv_loads else ["handle", "bind"])
