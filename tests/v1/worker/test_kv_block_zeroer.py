...
from aphrodite.v1.worker import utils as worker_utils
...
def test_large_dsv4_launch_geometry(monkeypatch):
    ...
    zeroer = KVBlockZeroer(
        device,
        attn_groups_iter=[AttentionGroup(None, [name], spec, group_id) for group_id, name in enumerate(layer_names)],
        kernel_block_sizes=[1] * n_segs,
        static_forward_context={name: SimpleNamespace(kv_cache=storage) for name, storage in storages.items()},
        num_blocks=1,
    )
...
def test_warmup_compiles_for_all_block_counts():
    ...
    def compiled_variants() -> set:
        return {key for caches in _zero_kv_blocks_kernel.device_caches.values() for key in caches[0]}
