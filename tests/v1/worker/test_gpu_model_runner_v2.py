...
import aphrodite.v1.worker.gpu.model_runner as model_runner_module
...
def test_append_block_ids_rejects_write_past_row_capacity():
    ...
    block_tables.num_blocks = SimpleNamespace(np=torch.tensor([[0, 3]], dtype=torch.int32))
...
def test_capture_model_locks_workspace_after_capture(monkeypatch):
    ...
    monkeypatch.setattr(model_runner_module, "freeze_gc_for_cudagraph_capture", contextlib.nullcontext)
    monkeypatch.setattr(torch.accelerator, "empty_cache", lambda: None)
    monkeypatch.setattr(torch.accelerator, "get_memory_info", lambda: (1 << 30, 1 << 30))
    lock_calls = []
    monkeypatch.setattr(model_runner_module, "lock_workspace", lambda: lock_calls.append("lock"))
