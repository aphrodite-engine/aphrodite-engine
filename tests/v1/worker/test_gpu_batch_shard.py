...
from aphrodite.v1.worker.gpu.sample import batch_shard
...
def _make_batch(
    ...
    logits_indices_np = np.concatenate(
        [np.arange(qsl[i + 1] - num_logits_per_req[i], qsl[i + 1]) for i in range(num_reqs)]
    ).astype(np.int64)
    expanded_idx_np = np.repeat(slots, num_logits_per_req)
    expanded_pos_np = np.concatenate([np.arange(n, dtype=np.int32) for n in num_logits_per_req])
...
def _shard_all_ranks(batch: InputBatch, max_num_reqs: int, tp_size: int):
    ...
    for rank in range(tp_size):
        ...
        local_batch, sorted_logits_indices, _, metadata = sharder.shard_sampler_inputs(batch, None)
...
def test_local_batch_partition(tp_size: int, seed: int):
    ...
    for rank, (local, _, metadata) in enumerate(results):
        ...
        assert metadata.num_local_logits == len(rows) == metadata0.num_logits_per_rank[rank]
        ...
        assert (_np(local.expanded_idx_mapping) == _np(batch.expanded_idx_mapping)[rows]).all()
        assert (_np(local.expanded_local_pos) == _np(batch.expanded_local_pos)[rows]).all()
        ...
        assert (local.num_draft_tokens_per_req == batch.num_draft_tokens_per_req[owned]).all()
        assert local.num_draft_tokens == int(batch.num_draft_tokens_per_req[owned].sum())
    ...
    rows_all = np.concatenate([_owned_rows(cu, _owned_batch_indices(local)) for local, _, _ in results])
    assert sorted_logits_indices.tolist() == _np(batch.logits_indices)[rows_all].tolist()
...
def test_all_to_all_reassembly_simulated(tp_size: int):
    ...
    sends = [token_logits[sorted_logits_indices][:, r * shard_w : (r + 1) * shard_w] for r in range(tp_size)]
    for rank, (local, _, metadata) in enumerate(results):
        recv = torch.cat([sends[src][bounds[rank] : bounds[rank + 1]] for src in range(tp_size)])
...
def test_shard_no_spec():
    ...
    for local, _, metadata in results:
        ...
        owned = _owned_batch_indices(local)
        assert _np(local.logits_indices).tolist() == (_np(batch.logits_indices)[owned].tolist())
...
def test_shard_grammar_output(tp_size: int):
    ...
    for rank in range(tp_size):
        ...
        expected_rows = np.concatenate(
            [np.arange(row_offsets[i], row_offsets[i] + cu[i + 1] - cu[i]) for i in expected_idx]
        )
...
def test_gather_sampler_output_kernels(tp_size: int):
    ...
    def run(rank: int) -> SamplerOutput:
        ...
        def fake_all_gather(x: torch.Tensor, dim: int = 0) -> torch.Tensor:
            ...
            return torch.zeros(tp_size * x.shape[0], x.shape[1], dtype=x.dtype, device=x.device)

        with mock.patch.object(batch_shard, "tensor_model_parallel_all_gather", fake_all_gather):
...
def test_gather_sampler_output_logprobs_and_nans():
    ...
    expected_logprobs = torch.full((num_logits, num_cols), float("-inf"), dtype=torch.float32)
    ...
    for rank, (local, _, metadata) in enumerate(results):
        ...
        local_outputs.append(
            SamplerOutput(
                sampled_token_ids=torch.zeros(metadata.num_local_reqs, width, dtype=torch.int64, device=device),
                ...
                num_sampled=torch.ones(metadata.num_local_reqs, dtype=torch.int32, device=device),
                num_rejected=torch.zeros(metadata.num_local_reqs, dtype=torch.int32, device=device),
            )
        )
    ...
    for rank in range(tp_size):
        run(rank)
    gathered_full.extend(torch.cat([recorded[r][i] for r in range(tp_size)]) for i in range(3))
