...
def test_drafting_uses_a_separate_noise_stream():
    ...
    for name, tail in (("target", target_tail), ("draft", draft_tail)):
        ...
        z = _z_score(tail_count, NUM_SAMPLES * tail_prob, NUM_SAMPLES)
        assert abs(z) < Z_TOLERANCE, f"{name} tail mass {tail_count / NUM_SAMPLES:.3e} != {tail_prob:.3e} (z={z:.2f})"
...
def test_greedy_temperature_zero_returns_argmax():
    ...
    sampled = gumbel_sample(logits, idx_mapping, temp, seed, pos, apply_temperature=True, is_drafting=False)
...
def test_logits_cache_stores_input_logits_bitwise(dtype: torch.dtype, per_token_col: bool):
