...
import aphrodite.v1.worker.workspace as workspace
from aphrodite.config import AphroditeConfig
from aphrodite.v1.worker.gpu_worker import _num_workspace_lanes
...
def test_workspace_lane_count_is_dspark_only(
    ...
    config = cast(AphroditeConfig, _AphroditeConfig(spec_config))
...
def test_workspace_lanes_do_not_alias_and_restore_context(monkeypatch) -> None:
    ...
    manager = workspace.WorkspaceManager(torch.device("cpu"), num_ubatches=2, num_lanes=2)
