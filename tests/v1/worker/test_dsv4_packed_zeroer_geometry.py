...
from aphrodite.v1.kv_cache_interface import KVCacheLayout, MLAAttentionSpec
...
def test_packed_dsv4_zeroer_zeroes_only_each_layers_page():
    ...
    zeroer = KVBlockZeroer(
        ...
        static_forward_context={f"layer.{i}": SimpleNamespace(kv_cache=views[i]) for i in range(NUM_LAYERS)},
    ...
    assert sorted(a - base for a in seg_addrs.tolist()) == [i * padded_page for i in range(NUM_LAYERS)]
    # Block 99 of the highest-offset layer stays within the packed backing.
    last_end = max(seg_addrs.tolist()) + (NUM_BLOCKS - 1) * int(seg_block_strides[0]) * 4 + int(seg_page_sizes[0]) * 4
...
def test_overlaid_zeroer_dedups_segments_with_max_span():
    ...
    from aphrodite.v1.core.kv_cache_utils import get_kv_cache_config_from_groups
    ...
    aphrodite_config.cache_config = CacheConfig()
    aphrodite_config.cache_config.num_gpu_blocks_override = None
    aphrodite_config.cache_config.kv_cache_layout = "BLHNC"
    config = get_kv_cache_config_from_groups(aphrodite_config, groups, 8 * 1024 * 1024)
    ...
    zeroer = KVBlockZeroer(
        ...
        static_forward_context={name: SimpleNamespace(kv_cache=views[name]) for name in views},
    ...
    by_offset = {a - buf_ptr: p * 4 for a, p in zip(seg_addrs.tolist(), seg_page_sizes.tolist())}
