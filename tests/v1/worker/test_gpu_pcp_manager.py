...
from aphrodite.v1.worker.gpu import pcp_manager as pcp_manager_module
...
def test_replicated_decode_piecewise_graph_padding(monkeypatch):
    ...
    request_indices = [[segment.global_batch_req_idx for segment in rank] for rank in segments_by_rank]
...
def test_num_tokens_for_dispatch_uses_largest_pcp_rank(pcp_world_size, num_scheduled_tokens, is_prefilling, expected):
    manager = PCPManager(
