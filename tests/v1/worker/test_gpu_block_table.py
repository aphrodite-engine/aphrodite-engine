...
def test_dcp_slot_mapping_with_smaller_kernel_blocks(cp_rank: int):
    ...
    expected[first_start : first_start + 128] = torch.arange(5 * 128, 6 * 128, dtype=torch.int64, device=device)
    expected[second_start : second_start + 128] = torch.arange(9 * 128, 10 * 128, dtype=torch.int64, device=device)
