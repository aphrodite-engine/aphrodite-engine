...
from aphrodite.v1.worker.gpu.warmup import (
...
pytestmark = pytest.mark.skipif(not current_platform.is_cuda(), reason="warmup synchronizes on the accelerator")
...
def _make_runner(
    ...
    return SimpleNamespace(
        ...
        kv_cache_config=SimpleNamespace(kv_cache_groups=kv_cache_groups, num_blocks=1024),
        aphrodite_config=SimpleNamespace(num_lookahead_tokens=num_lookahead_tokens, is_mm_encoder_only=False),
    ...
    def execute_model(self, scheduler_output) -> None:
        ...
        for i, req_id in enumerate(cached.req_ids):
            ...
            if new_block_ids is not None:
                self._held[req_id] = [held + len(ids) for held, ids in zip(self._held[req_id], new_block_ids)]
...
def _assert_covers_lookahead(steps: list[tuple[list[int], int, int]], num_lookahead_tokens: int) -> None:
    assert steps, "warmup ran no steps"
    for num_blocks, num_computed, num_scheduled in steps:
        num_tokens = min(num_computed + num_scheduled + num_lookahead_tokens, MAX_MODEL_LEN)
...
def test_warmup_reserves_mamba_speculative_blocks(mamba_cache_mode):
    ...
    warmup_kernels(
        _make_runner([_attention_group(), _mamba_group(mamba_cache_mode)], NUM_SPEC_STEPS),
        recorder.execute_model,
        recorder.sample_tokens,
    )
...
def test_reserved_block_count_matches_real_kv_cache_manager():
    ...
    (request,) = create_requests(num_requests=1, num_tokens=decode_query_len + 1, block_size=BLOCK_SIZE)
...
def test_num_lookahead_tokens_per_method(method: str, expected: int):
    """`AphroditeConfig.num_lookahead_tokens` is the single source of the reservation.
    ...
    class _Config:
        ...
        num_speculative_tokens = AphroditeConfig.num_speculative_tokens
        num_lookahead_tokens = AphroditeConfig.num_lookahead_tokens
