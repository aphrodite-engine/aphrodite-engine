...
from aphrodite.v1.worker.gpu import cudagraph_utils as cgu
from aphrodite.v1.worker.gpu import model_runner as mrv2
...
class _FakeCudaGraphManager:
    def __init__(self, needs_capture: bool, num_full_descs: int, piecewise_only: bool = False) -> None:
...
def _make_profiling_runner(
    ...
    *,
    ...
    runner.cudagraph_manager = _FakeCudaGraphManager(needs_capture, num_full_descs, piecewise_only)
...
class _FakePlatform:
    ...
    matching aphrodite.platforms.Platform's lazy singleton."""
...
def _patch_module(monkeypatch) -> None:
    ...
    def _fake_set_current_aphrodite_config(_cfg):
        yield

    _FakePlatform._global_graph_pool = GLOBAL_POOL
    monkeypatch.setattr(cgu, "set_current_aphrodite_config", _fake_set_current_aphrodite_config)
    monkeypatch.setattr(cgu, "current_platform", _FakePlatform())
    monkeypatch.setattr(cgu, "_init_minimal_kv_cache_for_profiling", lambda r: r.events.append("init"))
    monkeypatch.setattr(cgu, "_teardown_profiling_state", lambda r: r.events.append("teardown"))
    ...
    monkeypatch.setattr(cgu.torch.accelerator, "get_memory_info", lambda: (1 << 30, 1 << 30))
...
def test_profile_cudagraph_memory_samples_and_extrapolates(monkeypatch):
    ...
    assert runner.cudagraph_manager._max_full_descs_to_capture == cgu._FULL_GRAPH_PROFILING_SAMPLES
...
def test_extrapolate_full_graph_memory():
    ...
    assert cgu._extrapolate_full_graph_memory([100 * mib, 20 * mib], 5) == (100 + 4 * 20) * mib
...
def test_profile_cudagraph_memory_frees_throwaway_pool(monkeypatch):
    ...
    def _init(r):
        ...
        r.compilation_config.static_forward_context = {"layer": SimpleNamespace(kv_cache=kv_cache)}
        manager = cgu.CudaGraphManager.__new__(cgu.CudaGraphManager)
        manager.pool = cgu.current_platform.get_global_graph_pool()
        r.speculator = SimpleNamespace(cudagraph_manager=manager)
