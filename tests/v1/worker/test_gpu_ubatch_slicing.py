...
from aphrodite.config import AphroditeConfig, CUDAGraphMode, ModelConfig, ParallelConfig
from aphrodite.forward_context import create_forward_context
from aphrodite.v1.kv_cache_interface import KVCacheConfig
from aphrodite.v1.worker.gpu import dp_utils
...
from aphrodite.v1.worker.ubatching import dbo_current_ubatch_id, dbo_yield
...
def _make_input_batch(
    ...
    buffers.query_start_loc[: num_reqs_padded + 1] = torch.from_numpy(query_start_loc_np)
    ...
    return replace(
        ...
        num_computed_tokens_np=np.array(seq_lens, dtype=np.int32) - np.array(query_lens, dtype=np.int32),
...
def test_slicing_matches_v1_split_attn_metadata(batch_name: str):
    ...
    for i, (ubatch_slice, v1_ubatch) in enumerate(zip(ubatch_slices, v1_ubatches)):
        ...
        torch.testing.assert_close(v2_ubatch.seq_lens_cpu_upper_bound, v1_ubatch.seq_lens_cpu_upper_bound)
...
def test_trailing_microbatch_absorbs_cudagraph_padding():
    ...
    last = _slice_input_batch(input_batch, ubatch_slices_padded[-1], *_make_ubatch_buffers()[1])
    ...
    torch.testing.assert_close(last.query_start_loc, torch.tensor([0, 1, 2, 2, 2], dtype=torch.int32))
    torch.testing.assert_close(last.seq_lens, torch.tensor([64, 64, 0, 0], dtype=torch.int32))
...
def test_dummy_batch_can_be_microbatched():
    ...
    for ubatch in ubatches:
        torch.testing.assert_close(ubatch.query_start_loc, torch.tensor([0, 16], dtype=torch.int32))
...
def test_all_padding_microbatch_has_no_work_to_do():
    ...
    buffers = _make_buffers()
    input_batch = _make_input_batch(query_lens, seq_lens, buffers, num_tokens_padded=num_tokens_padded)
    ubatch_slices_padded = create_ubatch_slices(input_batch, num_ubatches=2)
    ubatch_buffers = _make_ubatch_buffers()
    ...
    torch.testing.assert_close(last.query_start_loc, torch.tensor([0, 0], dtype=torch.int32))
    ...
    torch.testing.assert_close(last.seq_lens_cpu_upper_bound, torch.tensor([64], dtype=torch.int32))
...
def _make_dbo_config() -> AphroditeConfig:
    return AphroditeConfig(
        model_config=ModelConfig(model="facebook/opt-125m", dtype="float16", seed=0),
        parallel_config=ParallelConfig(enable_dbo=True, all2all_backend="deepep_low_latency"),
    )
...
def _no_comm_sm_reservation(monkeypatch):
    ...
    monkeypatch.setenv("APHRODITE_DBO_COMM_SMS", "0")


def _make_execution_runner(aphrodite_config: AphroditeConfig) -> UBatchRunner:
...
def _make_ubatch_state(aphrodite_config: AphroditeConfig, ubatch_slices: list[UBatchSlice]) -> UBatchState:
    return UBatchState(
        slices=ubatch_slices,
        forward_contexts=[create_forward_context(None, aphrodite_config) for _ in ubatch_slices],
    )
...
def test_runner_allocates_one_buffer_pair_per_microbatch():
    ...
    aphrodite_config = _make_dbo_config()
    runner = _make_execution_runner(aphrodite_config)
...
def test_ubatch_runner_overlaps_and_matches_single_batch():
    ...
    expected = model_inputs["input_ids"].float().unsqueeze(-1) * 2 + model_inputs["positions"].float().unsqueeze(-1)
