...
def test_chunked_scores_match_full_batch(logprobs_mode: str):
    ...
    input_batch = SimpleNamespace(
        ...
        expanded_idx_mapping=torch.from_numpy(np.repeat(idx_mapping_np, num_logits_per_req)).to(device),
        expanded_local_pos=torch.from_numpy(
            np.concatenate([np.arange(count, dtype=np.int32) for count in num_logits_per_req])
        ).to(device),
    )
    ...
    def fake_verify(
        ...
        *_mappings,
        ...
        sampled = (idx_mapping.to(torch.int64).unsqueeze(1) + torch.arange(4, device=device)) % logits.shape[1]
    ...
    assert chunked_logprobs.cu_num_generated_tokens == full_logprobs.cu_num_generated_tokens
