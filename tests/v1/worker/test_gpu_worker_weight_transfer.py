...
from aphrodite.v1.worker.gpu_model_runner import _get_parameter_for_reload
...
def test_rank_local_update_selects_worker_payload(rank, expected):
    ...
    Worker.update_weights(worker, [{"names": ["rank-0"]}, {"names": ["rank-1"]}, {"names": []}])
...
def test_rank_local_update_includes_data_parallel_rank():
    ...
    worker = _make_worker(engine)
    worker.rank = 0
    worker.aphrodite_config.parallel_config.data_parallel_size = 4
    worker.aphrodite_config.parallel_config.data_parallel_rank = 2
    Worker.start_weight_update(worker)
