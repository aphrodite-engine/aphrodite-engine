...
def test_bind_kv_cache_shares_replayssm_trackers_by_cache_group():
    ...
    assert mixers[0]._replayssm_ring_start.data_ptr() == mixers[2]._replayssm_ring_start.data_ptr()
    assert mixers[0]._replayssm_prev_num_accepted.data_ptr() == mixers[2]._replayssm_prev_num_accepted.data_ptr()
    assert mixers[1]._replayssm_ring_start.data_ptr() != mixers[0]._replayssm_ring_start.data_ptr()
...
def test_bind_kv_cache_draft_model(default_aphrodite_config):
    ...
    ctx = {layer_name: Attention(32, 128, 0.1, prefix=layer_name) for layer_name in layer_names}
    ...
    assert ctx["draft_model.layers.0.attn"].kv_cache is kv_cache["draft_model.layers.0.attn"]
    assert ctx["draft_model.layers.1.attn"].kv_cache is kv_cache["draft_model.layers.1.attn"]
