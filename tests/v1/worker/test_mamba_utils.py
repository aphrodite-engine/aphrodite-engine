...
from aphrodite.v1.worker.mamba_utils import (
    MambaCopyBuffers,
...
def test_resumed_req_ids_cleared_from_mamba_state_idx():
    ...
    with patch(
        "aphrodite.v1.worker.mamba_utils.get_mamba_groups",
        return_value=([0], spec),
    ):
        preprocess_mamba(
            sched,
...
def test_gpu_context_reinterprets_high_data_ptrs_for_int64_metadata():
    ...
    gpu_ctx.initialize_from_forward_context(kv_cache_config, forward_context, _COPY_FUNCS, [block_table])

    assert gpu_ctx.state_base_addrs.tolist() == [
        _reinterpret_u64_as_i64(conv_ptr),
        _reinterpret_u64_as_i64(temporal_ptr),
    ]
    assert gpu_ctx.block_table_ptrs.tolist() == [_reinterpret_u64_as_i64(block_table_ptr)]
...
def test_stage_postprocess_inputs_to_gpu_fills_pinned_views():
    ...
    np.testing.assert_array_equal(ctx.mamba_state_idx_buf.np[:num_reqs], [100, 200, 300])
    np.testing.assert_array_equal(ctx.num_scheduled_tokens_buf.np[:num_reqs], [5, 12, 3])
    np.testing.assert_array_equal(ctx.num_computed_tokens_buf.np[:num_reqs], [10, 20, 30])
...
def test_gpu_context_ignores_auxiliary_cache_tensors() -> None:
    ...
    temporal_state = torch.empty(config.num_blocks, config.temporal_state_dim, dtype=config.dtype)
    attention = MagicMock()
    attention.kv_cache = [
        conv_state,
        temporal_state,
...
class TestPostprocessMambaFusedKernel:
    ...
    def test_batch_memcpy_left_overlap_has_memmove_semantics(self, device):
        ...
        row_offsets = torch.arange(batch, dtype=torch.int64, device=device) * row_stride_bytes
    ...
    def test_same_block_idx_with_offset_copies_then_sets_accepted_to_1(self, device, test_config):
        ...
        expected_conv_state[dest_block_id, :-1].copy_(conv_state_orig[dest_block_id, 1:])
        torch.testing.assert_close(
            conv_state_py,
            expected_conv_state,
            rtol=0,
    ...
    def test_temporal_copy_with_bias_ge_2(self, device, test_config):
        ...
        expected_conv_state = conv_state_orig.clone()
        expected_conv_state[dest_block_id, :-2].copy_(conv_state_orig[dest_block_id, 2:])
    ...
    @pytest.mark.parametrize("same_physical_block", [True, False], ids=["same", "distinct"])
    ...
    def test_sd_and_ds_conv_layouts_match_snapshot(
        ...
        # SD GPU path.
        monkeypatch.delenv("APHRODITE_SSM_CONV_STATE_LAYOUT", raising=False)
        ...
        expected_conv[dest_block_id, :-accept_token_bias].copy_(sd_source_conv[src_block_id, accept_token_bias:])
        torch.testing.assert_close(
            sd_conv,
            expected_conv,
            rtol=0,
        ...
        expected_temporal[dest_block_id].copy_(sd_source_temporal[actual_temporal_src_id])
        torch.testing.assert_close(
            sd_temporal,
            expected_temporal,
            rtol=0,
        ...
        expected_accepted_tensor = torch.tensor([expected_accepted], dtype=torch.int32, device=device)
        torch.testing.assert_close(
