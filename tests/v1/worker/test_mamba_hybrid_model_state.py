...
from aphrodite.v1.worker.gpu.model_states import mamba_hybrid
from aphrodite.v1.worker.gpu.model_states.mamba_hybrid import MambaHybridModelState
from aphrodite.v1.worker.gpu.model_states.recoverssm import RecoverSSMState


def test_prepare_attn_forwards_positions(monkeypatch: pytest.MonkeyPatch) -> None:
    state = object.__new__(MambaHybridModelState)
    state.aphrodite_config = SimpleNamespace(num_speculative_tokens=0, speculative_config=None)
...
def test_postprocess_state_scalar_with_int32_mapping(num_sampled: int, expected_value: int) -> None:
    state = object.__new__(MambaHybridModelState)
    state.num_accepted_tokens_gpu = torch.full((4,), 9, dtype=torch.int32, device="cuda")
    state._align_mode = False
    state.recoverssm = None
    ...
    expected = torch.tensor([expected_value, 9, expected_value, 9], dtype=torch.int32, device="cuda")
...
def test_recoverssm_align_tracks_mixed_batch_state_and_neutralizes_copy_bias() -> None:
    ...
    state.num_accepted_tokens_gpu = torch.full((5,), 9, dtype=torch.int32, device="cuda")
    metadata = Mock(spec=RecoverSSMMetadata)
