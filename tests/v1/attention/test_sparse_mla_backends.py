...
from aphrodite.model_executor.layers.attention.mla_attention import _use_masked_mha
...
import aphrodite.v1.attention.backends.mla.flashinfer_mla_sparse as flashinfer_sparse_mod
...
from aphrodite.v1.attention.backends.mla.indexer import split_indexer_prefill_chunks
from aphrodite.v1.attention.backends.utils import split_decodes_and_prefills, split_prefill_chunks
from aphrodite.v1.attention.ops import flashmla
...
def _quantize_dequantize_nvfp4_ds_mla(
    ...
    num_tokens = kv_c.shape[0]
    num_blocks = max(1, math.ceil(num_tokens / block_size))
    entry_size = math.ceil((kv_lora_rank // 2 + rope_dim + kv_lora_rank // 16) / 16) * 16
    sf_nope_off = kv_lora_rank // 2 + rope_dim

    tmp_cache = torch.zeros(num_blocks, block_size, entry_size, dtype=torch.uint8, device=kv_c.device)
    slot_mapping = torch.arange(num_tokens, dtype=torch.long, device=kv_c.device)
    ops.concat_and_cache_mla(kv_c, k_pe, tmp_cache, slot_mapping, kv_cache_dtype=kv_cache_dtype, scale=scale)
    ...
    dequant_kv_c = ((nope_vals.unflatten(-1, (-1, 16)) * nope_sf.unsqueeze(-1)).flatten(-2)).to(kv_c.dtype)
...
def test_sparse_backend_decode_correctness(
    ...
    if kv_cache_dtype == "nvfp4_ds_mla":
        ...
        assert _canonicalize_sparse_mla_kv_cache_dtype(backend_cls, kv_cache_dtype) == kv_cache_dtype
    ...
    prefill_backend = get_mla_prefill_backend(aphrodite_config)(
        num_heads=num_heads,
        scale=scale,
        kv_lora_rank=kv_lora_rank,
        qk_nope_head_dim=qk_nope_head_dim,
    ...
    aphrodite_config.compilation_config.static_forward_context["placeholder"] = SimpleNamespace(
...
def test_flashmla_forward_bf16_kv_slices_req_id_to_mqa_tokens():
    ...
    out = FlashMLASparseImpl._forward_bf16_kv(stub_impl, q, kv_cache, topk_indices, attn_metadata, q.shape[1])
...
def test_masked_mha_workspace_guards_long_routing_policy(backend_name, tensor_parallel_size, query_len):
...
def test_masked_mha_workspace_fits_accounts_for_batch_and_context():
    ...
    assert not _masked_mha_workspace_fits(**{**base, "batch_size": 3}, workspace_numel=exact)
    assert not _masked_mha_workspace_fits(**{**base, "max_context_chunk_seq_len": 4096}, workspace_numel=exact)
...
def test_sparse_backend_prefill_correctness(
    ...
    total_query_tokens = sum(query_lens)
    sparse_indices = torch.full((total_query_tokens, topk_tokens), -1, dtype=torch.int32, device=device)
    ...
    for i in range(batch_spec.batch_size):
        ...
        kv_b_weight = torch.cat([W_UK, W_UV], dim=-1).view(kv_lora_rank, num_heads * (qk_nope_head_dim + v_head_dim))
        kv_decompressed = (kv_c_full @ kv_b_weight).view(s_len, num_heads, qk_nope_head_dim + v_head_dim)
        k_nope_all, v_all = kv_decompressed.split([qk_nope_head_dim, v_head_dim], dim=-1)
        ...
        for j in range(q_len):
            ...
            out = torch.nn.functional.scaled_dot_product_attention(q_sdpa, k_sdpa, v_sdpa, scale=scale)
    ...
    kv_b_proj_weight = torch.cat([W_UK, W_UV], dim=-1).view(kv_lora_rank, num_heads * (qk_nope_head_dim + v_head_dim))
    ...
    out_buffer = torch.empty(total_query_tokens, num_heads * v_head_dim, dtype=dtype, device=device)
...
def test_flashmla_cache_dtype_aliases_use_ds_layout():
    ...
    for alias in ("fp8", "fp8_e4m3"):
        assert _canonicalize_sparse_mla_kv_cache_dtype(FlashMLASparseBackend, alias) == "fp8_ds_mla"


def test_flashmla_fp8_metadata_reuses_common_batch_split():
    builder = SimpleNamespace(
        device=torch.device(DEVICE_TYPE),
        aphrodite_config=SimpleNamespace(model_config=SimpleNamespace(max_model_len=8)),
    )
...
def test_flashmla_fp8_metadata_excludes_zero_token_decode_padding(monkeypatch):
    monkeypatch.setattr(
        "aphrodite.v1.attention.backends.mla.flashmla_sparse.get_mla_metadata",
        lambda: (object(), None),
    )
    builder = SimpleNamespace(
        device=torch.device(DEVICE_TYPE),
    ...
    torch.testing.assert_close(fp8_metadata.decode.seq_lens, torch.arange(6, device=DEVICE_TYPE))
...
def test_flashmla_fp8_paths_accept_decode_subset(monkeypatch, use_mixed_batch: bool):
    ...
    def convert_indices(*args, **kwargs):  # noqa: ARG001
        ...
        if not kwargs.get("return_valid_counts", False):
            return topk_indices
        valid_counts = torch.full((num_decode_tokens,), 4, dtype=torch.int32, device=DEVICE_TYPE)
        return topk_indices, valid_counts

    monkeypatch.setattr(
        "aphrodite.v1.attention.backends.mla.flashmla_sparse.triton_convert_req_index_to_global_index",
        convert_indices,
    )
    ...
    if use_mixed_batch:
        ...
        fp8_metadata = FP8Meta(
            ...
            prefill=FP8Meta.Prefill(
                request_ids=torch.empty(num_batch_tokens, dtype=torch.int32, device=DEVICE_TYPE),
    ...
    metadata = SimpleNamespace(
        ...
        req_id_per_token=torch.empty(num_batch_tokens, dtype=torch.int32, device=DEVICE_TYPE),
    ...
    impl._forward_fp8_kv_mixed_batch = MethodType(FlashMLASparseImpl._forward_fp8_kv_mixed_batch, impl)
    impl._forward_fp8_kv_separate_prefill_decode = MethodType(
        FlashMLASparseImpl._forward_fp8_kv_separate_prefill_decode,
        impl,
    )
...
def _build_sparse_dcp_aphrodite_config(
    local_heads: int,
    dcp_world_size: int,
    comm_backend: str = "ag_rs",
):
    """Minimal sparse-MLA AphroditeConfig for the FlashMLASparse DCP head-envelope
    ...
    model_config.get_num_attention_heads = MethodType(lambda self, parallel_config: local_heads, model_config)
    model_config.get_num_kv_heads = MethodType(lambda self, parallel_config: 1, model_config)
    model_config.get_head_size = MethodType(lambda self: head_size, model_config)
    model_config.get_sliding_window = MethodType(lambda self: None, model_config)

    aphrodite_config.cache_config.cache_dtype = "fp8_ds_mla"
    aphrodite_config.parallel_config.decode_context_parallel_size = dcp_world_size
    aphrodite_config.parallel_config.dcp_comm_backend = comm_backend
    ...
    aphrodite_config.compilation_config.static_forward_context["placeholder"] = SimpleNamespace(prefill_backend=None)
...
def test_fp8_dcp_head_envelope_guard(local_heads, dcp_world_size, should_raise):
    ...
    """
    device = torch.device(DEVICE_TYPE)
    aphrodite_config = _build_sparse_dcp_aphrodite_config(local_heads, dcp_world_size)
    kv_cache_spec = create_standard_kv_cache_spec(aphrodite_config)
    builder_cls = FlashMLASparseBackend.get_builder_cls()

    if should_raise:
        with pytest.raises(NotImplementedError, match="envelope"):
            builder_cls(kv_cache_spec, ["placeholder"], aphrodite_config, device)
    else:
        builder = builder_cls(kv_cache_spec, ["placeholder"], aphrodite_config, device)
...
def test_fp8_mixed_batch_dcp_neutralizes_empty_rows(monkeypatch):
    ...
    monkeypatch.setattr(
        "aphrodite.v1.attention.backends.mla.flashmla_sparse.triton_filter_and_convert_dcp_index",
        lambda *args, **kwargs: local_indices,
    )

    def run_kernel(**kwargs):
        out = torch.full((1, num_tokens, num_heads, 1), float("nan"), device=DEVICE_TYPE)
