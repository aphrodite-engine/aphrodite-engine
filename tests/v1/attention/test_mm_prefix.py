...
requires_fa4 = pytest.mark.skipif(not _fa4_available(), reason="FA4 mm_prefix mask_mod requires FA4")
...
def _query_ranges(mm_ranges, query_lens, seq_lens):
    ...
    query_start_loc = torch.tensor([0, *torch.tensor(query_lens).cumsum(0).tolist()], dtype=torch.int32)
...
def test_buffer_reuse_does_not_leak_previous_rows():
    ...
    num_tokens = fill_mm_prefix_query_ranges(out, {0: [(0, 7)]}, query_start_loc, seq_lens)
    assert num_tokens == 8
    np.testing.assert_array_equal(out[:8], np.tile([0, 7], (8, 1)))

    # Same rows scheduled, but now only tokens 2..3 sit inside a range.
    num_tokens = fill_mm_prefix_query_ranges(out, {0: [(2, 3)]}, query_start_loc, seq_lens)
    assert num_tokens == 8
...
def _run_kernel(q, k, v, query_lens, seq_lens, mm_ranges, sliding_window_left, scale, mm_clamp_sw=0):
    cu_q = _cu_seqlens(query_lens)
    cu_k = _cu_seqlens(seq_lens)

    staging = torch.full((int(cu_q[-1]), 2), 12345, dtype=torch.int32).numpy()
    num_rows = fill_mm_prefix_query_ranges(staging, mm_ranges, cu_q, torch.tensor(seq_lens, dtype=torch.int32))
    ...
    flash_attn_varlen_func(
        ...
        causal=False,
        fa_version=4,
        mask_mod=_make_mm_prefix_mask_mod(sliding_window=mm_clamp_sw, sliding_window_left=sliding_window_left),
...
def test_mm_prefix_mask_mod_matches_dense_reference(case):
    ...
    torch.testing.assert_close(_run_kernel(*args).float(), _reference(*args), atol=2e-2, rtol=2e-2)
...
def test_window_modes_on_a_range_wider_than_the_window(window_mode):
    ...
    if mm_clamp_sw:
        assert not torch.allclose(expected, _reference(*args, mm_clamp_sw=0), atol=2e-2, rtol=2e-2), (
...
def _enable_mm_prefix(aphrodite_config):
...
def test_decode_only_batch_reports_no_ranges():
    ...
    """
    aphrodite_config = _enable_mm_prefix(
        create_aphrodite_config(
            model_name=MODEL,
            max_model_len=1024,
    ...
    decode_batch = BatchSpec(seq_lens=[513, 200, 97], query_lens=[1, 1, 1], name="decode_only")
    ...
    builder = FlashAttentionMetadataBuilder(
        create_standard_kv_cache_spec(aphrodite_config),
...
def test_mm_prefix_kv_cache_path(head_size: int):
    ...
    assert aphrodite_config.model_config.is_mm_prefix_lm
    ...
    num_heads = mc.get_num_attention_heads(aphrodite_config.parallel_config)
    num_kv_heads = mc.get_num_kv_heads(aphrodite_config.parallel_config)
    scale = head_size**-0.5

    batch = BatchSpec(seq_lens=PAGED_SEQ_LENS, query_lens=PAGED_QUERY_LENS, name="mm_prefix_mixed")
    kv_cache_spec = FullAttentionSpec(
        ...
        kv_quant_mode=get_kv_quant_mode(aphrodite_config.cache_config.cache_dtype),
    )
    ...
    with set_current_aphrodite_config(aphrodite_config):
        builder = FlashAttentionMetadataBuilder(kv_cache_spec, layer_names, aphrodite_config, DEVICE)
        ...
        assert impl.flash_attn_version == 4, f"expected FA4, got FA{impl.flash_attn_version} for head_size={head_size}"
        ...
        actual = impl.forward(mock_layer, query, key, value, kv_cache, attn_metadata, output=output)
