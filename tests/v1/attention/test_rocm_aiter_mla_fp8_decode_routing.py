...
from aphrodite.v1.attention.backends.mla.rocm_aiter_mla import AiterMLAHelper  # noqa: E402
...
def gluon_available(monkeypatch):
    ...
    Gluon ships a gfx950 build only, and APHRODITE_ROCM_AITER_MLA_ASM_PADDING can
...
def test_fp8_never_routes_to_gluon_under_any_mode(monkeypatch, kv_cache_dtype, num_heads, mode):
    """APHRODITE_ROCM_AITER_MLA_ASM_PADDING cannot force a non-DCP fp8 cache onto Gluon.
...
def test_dcp_multitoken_verify_never_uses_gluon(gluon_available, kv_cache_dtype, num_heads, max_qo_len):
    assert not AiterMLAHelper.use_gluon_verify(num_heads, max_qo_len, kv_cache_dtype, dcp_world_size=8)
...
def test_unquantized_divisor_heads_keep_gluon_decode(gluon_available, kv_cache_dtype, num_heads):
...
def test_unquantized_small_head_verify_keeps_gluon(gluon_available, kv_cache_dtype, num_heads, max_qo_len):
