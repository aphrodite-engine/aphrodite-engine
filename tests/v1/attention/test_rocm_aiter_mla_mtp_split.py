...
from aphrodite.v1.attention.ops.rocm_aiter_mla_merge import (  # noqa: E402
    ...
    def __call__(
        ...
        *,
        ...
        for req_idx in range(self.grid[0]):
            ...
            seq_len = int((paged_kv_indptr[req_idx + 1] - paged_kv_indptr[req_idx]).item())
            for token_idx in range(seq_len):
                block_id = int(block_table_tensor[req_idx, token_idx // KERNEL_BLOCK_SIZE].item())
                page_indices[out_start + token_idx] = block_id * KERNEL_BLOCK_SIZE + token_idx % KERNEL_BLOCK_SIZE
...
def _builder(
    *,
    ...
    stub = SimpleNamespace(
        ...
        _supports_segmented_dcp_verify=rocm_aiter_mla._segmented_dcp_verify_supported(dcp_world_size, 1),
    ...
    stub._build_dcp_verify_row_view = AiterMLAMetadataBuilder._build_dcp_verify_row_view.__get__(stub)
    stub._fill_dcp_verify_page_table = AiterMLAMetadataBuilder._fill_dcp_verify_page_table.__get__(stub)
    return stub
...
def test_backend_declares_uniform_batch_support():
    ...
    assert AiterMLAMetadataBuilder.query_len_support == rocm_aiter_mla.QueryLenSupport.UNIFORM
    assert AiterMLAMetadataBuilder._cudagraph_support == rocm_aiter_mla.AttentionCGSupport.UNIFORM_BATCH
...
def test_dcp_verify_rows_sum_to_the_global_window_across_ranks():
    ...
    expected = [int(seq_len) - qlen + t + 1 for seq_len in seq_lens.tolist() for t in range(qlen)]
...
def test_single_token_dcp_decode_returns_unpadded_lse(monkeypatch):
    ...
    This is the path every DCP step takes outside verification. The Aphrodite custom
    ...
    monkeypatch.setattr(rocm_aiter_mla, "_get_aiter_mla_decode", lambda: fake_aiter_decode)
...
def test_verify_partial_attention_merge():
    ...
    normalizer = torch.logaddexp(lse_a, lse_b)
    expected = out_a * torch.exp(lse_a - normalizer).unsqueeze(-1) + out_b * torch.exp(lse_b - normalizer).unsqueeze(-1)
...
def test_segmented_dcp_verify_matches_causal_attention(monkeypatch):
    ...
    q_nope = torch.randn(qlen, num_heads, kv_lora_rank, dtype=torch.bfloat16, device=device)
    q_pe = torch.randn(qlen, num_heads, rope_dim, dtype=torch.bfloat16, device=device)
    kv_source = torch.randn(global_seq_len, head_dim, dtype=torch.float32, device=device)
    ...
    for dcp_rank in range(dcp_world_size):
        ...
        decode = SimpleNamespace(
            max_qo_len=qlen,
            paged_kv_indptr=torch.tensor([0, local_kv.shape[0]], dtype=torch.int32, device=device),
            paged_kv_indices=torch.arange(local_kv.shape[0], dtype=torch.int32, device=device),
            use_gluon_decode=False,
            use_gluon_verify=False,
            dcp_verify=view,
            attn_out_dtype=torch.bfloat16,
        ...
        partials.append(impl.forward_mqa((q_nope, q_pe), kv_cache, attn_metadata, layer))

    output, lse = partials[0]
    assert lse is not None
    for rank_output, rank_lse in partials[1:]:
        assert rank_lse is not None
        output, lse = _lse_combine_natural(output.float(), lse, rank_output.float(), rank_lse, torch.float32)
    ...
    for query_pos in range(qlen):
        ...
        scores = torch.einsum("hd,nd->hn", q_nope_fp32[query_pos], keys[:, :kv_lora_rank])
        scores += torch.einsum("hd,nd->hn", q_pe_fp32[query_pos], keys[:, kv_lora_rank:])
...
def test_mtp_builder_init_sizes_native_fp8_metadata(monkeypatch, num_heads, spec_method, parallel_drafting):
    ...
    def init_common_builder(self, *args, **kwargs):
        ...
        self.reorder_batch_threshold = 1 + (2 if spec.parallel_drafting else 1) * spec.num_speculative_tokens
...
def test_min_kv_seq_len_ignores_cudagraph_padding_rows(monkeypatch):
    ...
    query_start_loc = torch.tensor([0, mtp_qlen] + [mtp_qlen] * (num_reqs - 1), dtype=torch.int32)
...
def test_persistent_metadata_gate(monkeypatch, mtp_decode_qlen, qo_len, num_heads, kv_cache_dtype, expect_persistent):
    ...
    num_reqs = 2
    query_start_loc = torch.arange(0, (num_reqs + 1) * qo_len, step=qo_len, dtype=torch.int32)
    metadata = AiterMLAMetadataBuilder._build_decode(
        _builder(
