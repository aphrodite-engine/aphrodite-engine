...
def _metadata(query_start_loc: list[int], num_tokens: int | None = None) -> CommonAttentionMetadata:
    ...
    return CommonAttentionMetadata(
        ...
        max_query_len=max(end - start for start, end in zip(query_start_loc, query_start_loc[1:])),
        max_seq_len=num_reqs * 100 + 8,
        block_table_tensor=torch.arange(num_reqs * 3, dtype=torch.int32).view(num_reqs, 3),
...
def _builder(marked: bool = True) -> _NonCausalMLAMetadataBuilder:
    ...
    builder.model_config = SimpleNamespace(dtype=torch.bfloat16, get_head_size=lambda: 576)
    return builder
...
def test_noncausal_decode_metadata_keeps_live_request_buffers():
    ...
    assert metadata.decode.block_table.data_ptr() == common_metadata.block_table_tensor.data_ptr()
...
def test_mla_cache_marker_is_promoted_to_group_capability():
    ...
    assert not MLAAttentionSpec.merge([unmarked, unmarked]).non_causal_multi_token_decode
