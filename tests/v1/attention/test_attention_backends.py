    ...
    for backend_name in backend_to_test:
        ...
        if backend_cls is not None:
            ...
            if stride_order != tuple(range(kv_cache.ndim)):
                ...
                kv_cache_for_backend = kv_cache.permute(*stride_order).contiguous().permute(*inv_order)
...
def test_flashinfer_sm90_xqa_decode_correctness(default_aphrodite_config):
...
@pytest.mark.parametrize("model", ["meta-llama/Meta-Llama-3-8B"])
def test_non_causal_backend_correctness(default_aphrodite_config, batch_spec_name: str, model: str):
