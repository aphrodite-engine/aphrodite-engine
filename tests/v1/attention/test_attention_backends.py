...
from aphrodite.config import ModelConfig, set_current_aphrodite_config
...
from aphrodite.v1.kv_cache_interface import FullAttentionSpec, KVCacheLayout
...
def _clone_kv_cache_in_layout(kv_cache: torch.Tensor, layout: KVCacheLayout) -> torch.Tensor:
...
def run_attention_backend(
    ...
    # Impls capture the current aphrodite config at construction, as in model loading.
...
def _test_backend_correctness(
    ...
    *,
    ...
    if max_num_seqs is not None:
        aphrodite_config.scheduler_config.max_num_seqs = max_num_seqs
    if max_num_batched_tokens is not None:
        aphrodite_config.scheduler_config.max_num_batched_tokens = max_num_batched_tokens
    if num_speculative_tokens > 0:
        aphrodite_config.speculative_config = SimpleNamespace(num_speculative_tokens=num_speculative_tokens)
    ...
    num_kv_heads = aphrodite_config.model_config.get_num_kv_heads(aphrodite_config.parallel_config)
    sinks = torch.linspace(-1.0, 1.0, num_q_heads, dtype=torch.float32, device=device) if use_sinks else None
    head_size = aphrodite_config.model_config.get_head_size()
    ...
    for i in range(batch_size):
        ...
        if sinks is None:
            ...
            scores = torch.matmul(q_sdpa_in.float(), k_sdpa_in.float().transpose(-2, -1)) * scale
            ...
            weights = torch.softmax(torch.cat((scores, sink_logits), dim=-1), dim=-1)[..., :kv_len]
    ...
    declared = [
        ...
        if (supported := _actual_backend(backend).get_class().supported_kv_cache_layouts()) is not None
    ]
    if layout is None:
    ...
    for backend_name in backend_to_test:
        ...
        if is_quantized_kv_cache(kv_cache_dtype) and (not backend_cls.supports_kv_cache_dtype(kv_cache_dtype)):
            continue
        ...
        if backend_name == AttentionBackendEnum.B12X:
            cache_dtype = (
                FP8_KV_CACHE_DTYPES[kv_cache_dtype] if is_quantized_kv_cache(kv_cache_dtype) else kv_cache.dtype
            )
            ...
            kv_cache_for_backend = _clone_kv_cache_in_layout(packed_cache, backend_layout)

        # FlashInfer reads the layout at plan time; set it to match
        # the physical order of the test cache.
        aphrodite_config.cache_config.kv_cache_layout = backend_layout.name
...
def test_causal_backend_correctness(
    ...
    batch_spec = BATCH_SPECS[batch_spec_name]
    LARGE_BLOCK_BACKENDS = [AttentionBackendEnum.FLEX_ATTENTION] if is_torch_equal_or_newer("2.9.0.dev0") else []
...
def test_flashinfer_xqa_draft_masks():
    ...
    ragged = flashinfer_backend._make_xqa_ragged_draft_block_mask([2, 3], 3, True, device)

    assert torch.equal(
        causal.view(torch.int16),
        torch.tensor([[1, 0], [3, 0], [7, 0]], dtype=torch.int16),
    )
    assert torch.equal(full.view(torch.int16), torch.tensor([[7, 0]] * 3, dtype=torch.int16))
    assert torch.equal(
...
def test_flashinfer_native_prefill_with_sinks(default_aphrodite_config):
    if not (current_platform.is_cuda() and current_platform.is_device_capability_family(120)):
...
def test_flashinfer_xqa_decode_correctness(default_aphrodite_config):
    ...
    from aphrodite.utils.flashinfer import can_use_trtllm_attention
    from aphrodite.v1.attention.backends import flashinfer as flashinfer_backend
    ...
    with set_current_aphrodite_config(aphrodite_config):
        if not can_use_trtllm_attention(
            aphrodite_config.model_config.get_num_attention_heads(aphrodite_config.parallel_config),
        ...
        with unittest.mock.patch(
            ...
            builder = flashinfer_backend.FlashInferMetadataBuilder(
                kv_cache_spec, ["placeholder"], aphrodite_config, device
            )
    ...
    assert (
        flashinfer_backend.FlashInferMetadataBuilder.get_cudagraph_support(aphrodite_config, kv_cache_spec)
        == expected_cg_support
    )
    assert isinstance(
...
SLIDING_WINDOW_ENCODER_BACKENDS_TO_TEST = [
    backend for backend in SLIDING_WINDOW_BACKENDS_TO_TEST if backend != "FLEX_ATTENTION_SLOW"
]
...
def test_sliding_window_encoder_backend_correctness(
    ...
    model_config = ModelConfig(model=model, max_model_len=max(batch_spec.seq_lens))
    sliding_window = model_config.get_sliding_window()
    sliding_window_mask_mod_fn = partial(bidi_sliding_window_mask_mod, sliding_window=sliding_window)
...
if current_platform.is_rocm():
    NON_CAUSAL_BACKENDS_TO_TEST = [x for x in NON_CAUSAL_BACKENDS_TO_TEST if x is not AttentionBackendEnum.FLASH_ATTN]
...
def test_non_causal_backend_correctness(
    ...
    SMALL_BLOCK_BACKENDS = [x for x in NON_CAUSAL_BACKENDS_TO_TEST if x not in LARGE_BLOCK_BACKENDS]
