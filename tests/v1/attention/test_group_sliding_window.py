...
def _layers(layer_windows: list[int]):
    ...
    return {
        f"layer_{i}": SimpleNamespace(impl=MagicMock(spec=CPUAttentionBackendImpl, sliding_window=window))
...
def test_cpu_group_sliding_window(layer_windows, expected):
    ...
    builder = SimpleNamespace(aphrodite_config=None, layer_names=list(layers))
    with patch(
