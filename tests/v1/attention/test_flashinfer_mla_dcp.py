...
def test_mla_dcp_gathered_query_reserves_backend_head_storage():
    from aphrodite.v1.attention.ops.dcp import reserve_query_head_storage
...
def test_flashinfer_mla_selects_backend_for_gathered_heads():
    from aphrodite.v1.attention.backends.mla.flashinfer_mla import (
...
def test_flashinfer_mla_forward_uses_gathered_head_count(monkeypatch):
    import aphrodite.v1.attention.backends.mla.flashinfer_mla as flashinfer_mla
    ...
    output, lse = flashinfer_mla.FlashInferMLAImpl.forward_mqa(impl, query, kv_cache, attn_metadata, layer)
...
def test_flashinfer_mla_forward_uses_native_dcp_api(monkeypatch, causal):
    ...
    output, lse = flashinfer_mla.FlashInferMLAImpl.forward_mqa(impl, query, kv_cache, attn_metadata, MagicMock())

    call = kernel.call_args.kwargs
    assert call["query"].shape == (kernel_batch, kernel_query_len, 24, 576)
    expected_block_table = block_table if causal else block_table.repeat_interleave(query_len, dim=0)
    expected_seq_lens = seq_lens if causal else seq_lens.repeat_interleave(query_len)
    expected_global_seq_lens = global_causal_seq_lens if causal else global_causal_seq_lens.repeat_interleave(query_len)
    ...
    torch.testing.assert_close(call["causal_seqlens_kv_global"], expected_global_seq_lens)
