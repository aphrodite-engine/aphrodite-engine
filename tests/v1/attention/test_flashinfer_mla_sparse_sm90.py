...
# isort: off
import aphrodite.v1.attention.backends.mla.flashinfer_mla_sparse_sm90 as sm90_mod
from aphrodite.v1.attention.backends.mla.flashinfer_mla_sparse_sm90 import (
...
class FakeWrapper:
    ...
    def run(self, *args, **kwargs):
        ...
        return torch.zeros(q_nope.shape[0], q_nope.shape[1], ckv.shape[-1], dtype=torch.bfloat16)
...
def make_batch(rows, topk_rows, own_blocks):
    ...
    return SimpleNamespace(req_id_per_token=req_id, block_table=block_table, block_size=BLOCK_SIZE)
...
def test_forward_wiring(monkeypatch, qk_rope, kv_dtype):
    ...
    monkeypatch.setattr(sm90_mod, "triton_convert_req_index_to_global_index", ref_convert)
    ...
    out, lse = impl.forward_mqa((q_nope, q_rope), cache, meta, SimpleNamespace(_k_scale_float=0.5))
    ...
    ref_slots, ref_counts = ref_convert(meta.req_id_per_token, meta.block_table, impl.topk_indices_buffer)
    width = TOPK
...
def test_supports_combination_gates(monkeypatch, default_aphrodite_config):
    ...
    import aphrodite.config as cfg
