...
in aphrodite/v1/worker/gpu/block_table.py), so every token at ``pos >= kpool``
...
from aphrodite.v1.kv_cache_interface import KpoolTailSpec, compute_layout_strides
...
def circular_tail_slots(slot_mapping, block_table, query_start_loc, positions, num_actual, num_reqs):
...
def make_batch(per_req_positions, padded_len=None):
    positions = torch.cat([torch.tensor(p, dtype=torch.int64) for p in per_req_positions])
...
def test_legacy_generic_mapping_collapses_onto_block_zero():
    ...
    for req, prompt in enumerate(per_req):
        ...
        for pos in range(len(prompt)):
            ...
            if pos >= KPOOL:
                assert slot // KPOOL == 0, f"expected collapse onto block 0 at pos {pos}"
...
def test_circular_mapping_isolates_requests():
    ...
    for req, prompt in enumerate(per_req):
        ...
        for pos in range(len(prompt)):
            ...
            assert slot // KPOOL == own_blocks[req], f"req {req} pos {pos} left its tail block"
...
def test_circular_mapping_preserves_padding_and_empty_batch():
    ...
    positions, qsl, slot_mapping, num_actual, num_reqs = make_batch(per_req, padded_len=padded_len)
    bt = make_tail_block_table(own_blocks)
...
def make_common_metadata(per_req_positions, own_blocks, with_positions=True):
    ...
    bt = make_tail_block_table(own_blocks)
    seq_lens = torch.tensor([max(p) + 1 if p else 1 for p in per_req_positions], dtype=torch.int64)
...
def test_builder_build_uses_circular_mapping():
    ...
    assert torch.equal(out[cam.num_actual_tokens :], torch.full_like(out[cam.num_actual_tokens :], -1))
...
def test_builder_reuses_slot_mapping_storage():
    ...
    assert second_meta.slot_mapping[:12].tolist() == [9 * KPOOL + pos % KPOOL for pos in range(12)]
...
class TailRingMirror:
    ...
    def complete(self, tail_slot, pos, k, s):
        ...
        kk = torch.stack([self.k[blk, (start + i) % self.kpool] for i in range(self.kpool)])
        ss = torch.stack([self.s[blk, (start + i) % self.kpool] for i in range(self.kpool)])
...
def test_interleaved_decode_pollution_legacy_vs_circular():
    ...
    assert not torch.allclose(legacy, ground_truth), f"legacy mapping unexpectedly clean: {legacy} vs {ground_truth}"
