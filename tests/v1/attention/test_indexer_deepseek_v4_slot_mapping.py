...
def test_indexer_warmup_normalizes_zero_compress_ratios():
    config = SimpleNamespace(
        ...
        model_config=SimpleNamespace(hf_config=SimpleNamespace(compress_ratios=[0, 0, 4, 128, 0], index_kpool=32)),
    ...
    assert {(key.query_slice_start, key.query_slice_stop) for key in keys} == {
        (query_slice_start, query_slice_stop) for query_slice_start in (1, 2, 16) for query_slice_stop in (1, 2, 16)
    }
...
def test_indexer_builder_deepseek_v4_compressed_slot_mapping_uses_num_states():
    ...
    max_num_blocks = kv_cache_spec.max_num_blocks_per_req(aphrodite_config, 1024)
