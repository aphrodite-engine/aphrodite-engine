...
def create_aphrodite_config(
    ...
    """Create a AphroditeConfig for testing with reasonable defaults."""
...
class MockMambaBuilder(BaseMambaAttentionMetadataBuilder[BaseMambaAttentionMetadata]):
    ...
    def build_mamba_metadata(
        ...
        *,
        ...
        mamba_spec = MambaSpec(block_size=block_size, shapes=((1,), (1,)), dtypes=(torch.float32,))
        ...
        common_metadata = common_metadata.replace(is_prefilling=torch.tensor(is_prefilling, dtype=torch.bool))
...
def dense_kv_cache_views(
    ...
    layer_stride, block_stride, _, _, _ = compute_layout_strides(spec, num_blocks, num_layers, layout)
