...
from aphrodite.v1.kv_cache_interface import SlidingWindowMLASpec
...
def make_batch(
    ...
    block_table = torch.arange(num_reqs * max_blocks, dtype=torch.int32, device=device).view(num_reqs, max_blocks)
...
def ref_swa_slot_rows(
    ...
    lefts, rights = ref_left_right(seq_lens, query_lens, spans_per_req, max_image_tokens)
    rows: list[list[int]] = []
    lens: list[int] = []
    token = 0
...
def run_swa_kernel(
    ...
    query_start_loc, seq_lens_t, token_to_req, slot_mapping, block_table = make_batch(seq_lens, query_lens, device)
    num_tokens = int(query_start_loc[-1])
...
def test_swa_indices_kernel_with_image_spans(case):
    ...
    indices, actual_lens = run_swa_kernel(case["seq_lens"], case["query_lens"], case["spans"], with_image=True)
...
def test_swa_indices_kernel_without_image_unchanged(case):
    ...
    indices, actual_lens = run_swa_kernel(case["seq_lens"], case["query_lens"], case["spans"], with_image=False)
...
def test_image_visibility_kernel():
    ...
    query_lens = [30, 12]
    spans = [[(4, 12), (20, 24)], []]
    query_start_loc, seq_lens_t, token_to_req, _, _ = make_batch(seq_lens, query_lens, device)
    num_tokens = int(query_start_loc[-1])
...
def combine_case(
    ...
    topk_indices = torch.randint(0, 4096, (num_tokens, max(topk, 1)), generator=gen, dtype=torch.int32).to(device)
...
def test_flashinfer_mixed_sparse_indices_with_image_spans():
    ...
    seq_lens_t = torch.tensor(seq_lens, dtype=torch.int32, device=device)
    token_to_req = torch.tensor([0] + [1] * 30 + [2] * 9, dtype=torch.int32, device=device)
    block_table = torch.arange(3, dtype=torch.int32, device=device).view(3, 1)
    decode_swa = torch.tensor([[100, 101, 102, 103, 104, 105, 106, 107]], dtype=torch.int32, device=device)
    ...
    exp_rows, _ = ref_swa_slot_rows(seq_lens, query_lens, spans, block_table, WINDOW, MAX_IMG, swa_total)
...
def build_metadata(
    ...
    device = torch.device("cuda")
    query_start_loc, seq_lens_t, _, slot_mapping, block_table = make_batch(seq_lens, query_lens, device)
...
def test_builder_in_image_visibility():
    ...
    rows, lens = ref_swa_slot_rows(seq_lens, query_lens, spans, block_table, WINDOW, MAX_IMG, WIDTH)
...
def test_builder_no_image_spans_fast_path():
    ...
    _, _, _, _, block_table = make_batch(seq_lens, query_lens, torch.device("cuda"))
    rows, lens = ref_swa_slot_rows(seq_lens, query_lens, [[], []], block_table, WINDOW, MAX_IMG, WIDTH)
    assert md.prefill_swa_lens.cpu().tolist() == lens
    assert md.prefill_swa_indices[:, 0].cpu().tolist() == rows
...
def test_builder_text_model_unchanged():
    ...
    _, _, _, _, block_table = make_batch(seq_lens, query_lens, torch.device("cuda"))
    rows, lens = ref_swa_slot_rows(seq_lens, query_lens, [[], []], block_table, WINDOW, MAX_IMG, WINDOW)
    assert md.prefill_swa_lens.cpu().tolist() == lens
    assert md.prefill_swa_indices[:, 0].cpu().tolist() == rows
