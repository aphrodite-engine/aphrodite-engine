...
def _build_dspark_noncausal_indices(
    ...
    sparse_indices = torch.full((total_query_tokens, topk_width), -1, dtype=torch.int32, device=device)
    gt = 0
    for s_len, q_len in zip(seq_lens, query_lens):
        ...
        n = idx_list.numel()
        assert n <= topk_width, f"index list ({n}) exceeds aligned topk width ({topk_width})"
...
def _run_sparse_backend_vs_sdpa(
    ...
    if stale_cpu_query_lens is not None:
        ...
        common_attn_metadata.query_start_loc_cpu = torch.tensor(cpu_query_start_loc, dtype=torch.int32)
    ...
    builder = backend_cls.get_builder_cls()(kv_cache_spec, ["placeholder"], aphrodite_config, device)
    ...
    with torch.inference_mode():
        backend_output = mock_layer.forward_impl(
            query_aphrodite, kv_c_aphrodite, k_pe_aphrodite, kv_cache, metadata, out_buffer
        )
...
def _skip_if_backend_unavailable(backend_cls, kv_cache_dtype: str, block_size: int):
    ...
    if backend_cls is FlashMLASparseBackend and kv_cache_dtype.startswith("fp8") and kv_cache_dtype != "fp8_ds_mla":
...
def test_flashinfer_sparse_mla_adaptive_varlen_matches_sdpa(
    ...
    assert backend_cls.get_builder_cls().get_cudagraph_support(None, None) == AttentionCGSupport.ALWAYS
...
def test_dspark_noncausal_sparse_mla_matches_sdpa(
    ...
    sparse_indices = _build_dspark_noncausal_indices(seq_lens, query_lens, window, topk_width, device)
