...
from aphrodite.v1.attention.backends import b12x
from aphrodite.v1.attention.backends.b12x import (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            "auto",
            None,
            "Install the b12x backend with `pip install aphrodite[b12x]`",
...
def test_b12x_attention_fp8_descales_follow_request_batch() -> None:
    ...
    k_descale, v_descale = impl._prepare_fp8_descales(layer, num_reqs=2, device=torch.device("cpu"))
...
def test_b12x_causal_sliding_window(
    ...
    sliding_window = ModelConfig(model=model, max_model_len=max(batch_spec.seq_lens)).get_sliding_window()
