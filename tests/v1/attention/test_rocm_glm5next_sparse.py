...
def test_fit_kpool_indices_rejects_narrow_input():
    with pytest.raises(ValueError, match="at least topk_tokens"):
        fit_kpool_indices_to_aiter(torch.zeros((1, 3), dtype=torch.int32), topk_tokens=4)
...
def test_rocm_sparse_triton_route_preserves_padded_sinks(monkeypatch, num_heads):
    ...
    def fake_rocm_sparse_attn_prefill(**kwargs):
        ...
        output.copy_(captured["attn_sink"].to(output.dtype).view(1, -1, 1).expand_as(output))

    monkeypatch.setattr(sparse_mod, "rocm_sparse_attn_prefill", fake_rocm_sparse_attn_prefill)
