...
from aphrodite.v1.attention.backends.mla import flashmla as flashmla_module
from aphrodite.v1.attention.backends.mla import tokenspeed_mla as tokenspeed_mla_module
...
from aphrodite.v1.attention.ops.flashmla import is_flashmla_dense_supported
...
def test_glm5_masked_mha_pure_prefill_routing(tensor_parallel_size, query_len, expected):
    assert (
        _use_masked_mha(
...
def test_glm5_masked_mha_context_routing(tensor_parallel_size, query_len, seq_len, expected):
    assert (
        _use_masked_mha(
            backend_name="FLASHMLA_SPARSE",
            tensor_parallel_size=tensor_parallel_size,
...
def test_glm5_flashinfer_masked_mha_routing(tensor_parallel_size, query_len, seq_len, has_context, expected):
    assert (
        _use_masked_mha(
...
def test_mla_kv_cache_spec_uses_layer_cache_dtype(cache_dtype: str, expected_quant_mode: KVQuantMode):
    ...
    aphrodite_config = SimpleNamespace(cache_config=SimpleNamespace(block_size=64), model_config=None)

    spec = MLAAttention.get_kv_cache_spec(layer, aphrodite_config)
...
def test_mla_post_load_preserves_runtime_weight_addresses(monkeypatch):
    ...
    layer.kv_b_proj.weight = torch.nn.Parameter(torch.arange(28.0, dtype=torch.float16).reshape(14, 2))
    ...
    monkeypatch.setattr(mla_attention_module, "set_default_quant_scales", lambda *_, **__: None)
...
def _prefill_backend_dimension_params():
    ...
    for prefill_backend in PREFILL_BACKENDS_TO_TEST:
        for dimensions_id, qk_nope_head_dim, v_head_dim in MLA_DIMENSIONS_TO_TEST:
            ...
            if invalid_reasons:
                marks.append(
                    pytest.mark.skip(reason=(f"Prefill backend {prefill_backend.name} unavailable: {invalid_reasons}"))
                )
            params.append(
                pytest.param(
                    prefill_backend,
...
for backend in BACKENDS_TO_TEST:
    ...
    if supported_sizes:
        ...
        block_size = default_size if isinstance(default_size, int) else default_size.base
    else:
        block_size = 16
...
def create_and_prepopulate_kv_cache(
    ...
    fp8_attention = bool(kv_cache_dtype and kv_cache_dtype.startswith("fp8")) or use_nvfp4_ds_mla
    ...
    if fp8_attention:
        if use_fp8_ds_mla:
            ...
            # padded to 16B
            kv_entry_size = cdiv(kv_lora_rank // 2 + rope_dim + kv_lora_rank // 16, 16) * 16
        else:
            kv_entry_size = head_size

        kv_cache = torch.zeros(num_blocks, 1, block_size, kv_entry_size, dtype=torch.uint8, device=device)
        scale_tensor = (
        ...
        kv_cache = torch.zeros(num_blocks, 1, block_size, head_size, dtype=dtype, device=device)
...
class MockSparseMLAAttentionLayer:
    ...
    def forward_impl(
        ...
        mqa_q_nope, mqa_q_pe = q.split([self.qk_nope_head_dim, self.qk_rope_head_dim], dim=-1)
        ...
        output[:num_tokens] = decode_output.reshape(num_tokens, self.num_heads * self.v_head_dim)
...
class MockMLAAttentionLayer(MLAAttention):
    ...
    def forward_impl(
        ...
        if has_decode:
            ...
            # Split q into nope and pe parts
            mqa_q_nope, mqa_q_pe = decode_q.split([self.qk_nope_head_dim, self.qk_rope_head_dim], dim=-1)
            ...
            # output shape: (B, N, V) -> flatten to (B, N*V)
            decode_output = torch.bmm(attn_out.transpose(0, 1), self.W_UV).transpose(0, 1)
            output[:num_decode_tokens] = decode_output.reshape(num_decode_tokens, self.num_heads * self.v_head_dim)

        return output


def test_mock_mla_dcp_fp8_decode_gathers_quantized_query(monkeypatch, default_aphrodite_config):
    ...
    k_pe = torch.randn(num_tokens, 1, qk_rope_head_dim, device=device, dtype=torch.float32)
    kv_cache = torch.empty(0, device=device, dtype=torch.float32)
    output = torch.empty(num_tokens, num_heads * v_head_dim, device=device, dtype=torch.float32)
...
def test_flashinfer_mla_dspark_dcp_supports_target_and_draft(monkeypatch):
    flashinfer_mla_module = pytest.importorskip("aphrodite.v1.attention.backends.mla.flashinfer_mla")
...
def test_tokenspeed_mla_decode_contract(monkeypatch, causal, tokens_per_decode, dcp_world_size, dcp_rank):
    decode_call = None
    num_decodes = 2
    ...
    if dcp_world_size > 1:
        torch.testing.assert_close(decode_call["causal_seqs"], metadata.decode.dcp_tot_seq_lens)
    else:
...
def test_flashmla_dcp_decode_metadata_uses_gathered_query_heads(monkeypatch, is_fp8_kvcache):
    ...
    def fake_get_mla_metadata_dense_fp8(seq_lens_device, num_q_tokens_per_head_k, num_heads_k):
        nonlocal fp8_call
        fp8_call = (
            seq_lens_device,
            num_q_tokens_per_head_k,
...
def run_attention_backend(
    ...
    with set_current_aphrodite_config(aphrodite_config):
        ...
        for layer_name in layer_names:
            aphrodite_config.compilation_config.static_forward_context[layer_name] = mock_layer
        ...
        if chunked_prefill_workspace_size is not None:
            ...
            builder.chunked_prefill_workspace = builder.chunked_prefill_workspace.new_empty(
        ...
        # Run forward pass
        output = mock_layer.forward_impl(query, kv_c, k_pe, kv_cache, attn_metadata, output)
...
def _run_backend_correctness(
    ...
    3. Simulating Aphrodite's paged KV cache: It takes the context portion of the
       K/V tensors and manually places them into a paged buffer according to
       the test's (randomly generated) block table.
    4. Running each Aphrodite attention backend with the new queries and the
       simulated paged KV cache.
    5. Comparing the Aphrodite backend's output to the ground-truth SDPA output.
    ...
    backends_to_test = [b for b in BACKENDS_TO_TEST if kv_cache_dtype in b.get_class().supported_kv_cache_dtypes]
    if (q_scale != 1.0 or k_scale != 1.0) and AttentionBackendEnum.CUTLASS_MLA in backends_to_test:
    ...
    required_blocks = sum((seq_len + default_block_size - 1) // default_block_size for seq_len in batch_spec.seq_lens)
    ...
    for block_size in unique_block_sizes:
        ...
        required_blocks_for_size = sum((seq_len + block_size - 1) // block_size for seq_len in batch_spec.seq_lens)
