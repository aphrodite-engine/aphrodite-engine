...
from aphrodite.v1.attention.ops.dcp import CPTritonContext, correct_attn_out
...
def test_dcp_filter_compacts_valid_slots_for_sparse_kernel(
    ...
    assert set(out[0, :valid].cpu().tolist()) == set(range(expected_base, expected_base + block_size))
...
def test_dcp_filter_compaction_matches_reference(interleave: int, dcp_rank: int, num_topk: int):
