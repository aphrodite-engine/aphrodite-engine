...
def test_chunks_gather_every_context_row_exactly_once(context_lens, workspace_size):
    ...
    for chunk in metadata.chunks:
        ...
        for offset, (start, length) in enumerate(zip(starts, seq_lens)):
            ...
            gathered.setdefault(chunk.request_slice.start + offset, []).append((start, length))
...
def test_continuation_is_confined_to_a_chunks_first_request():
    ...
    for chunk in metadata.chunks:
        ...
        assert chunk.token_slice == slice(query_start_loc[request_start], query_start_loc[request_end])
        assert chunk.query_start_loc.tolist() == [
            offset - chunk.token_slice.start for offset in query_start_loc[request_start : request_end + 1]
        ]
...
def test_prefills_without_context_are_skipped_and_neutralized():
    ...
    covered = {
        request for chunk in metadata.chunks for request in range(chunk.request_slice.start, chunk.request_slice.stop)
    }
    assert covered == {0, 2}
...
def test_dcp_chunks_fit_the_per_rank_row_budget():
    ...
    for chunk in metadata.chunks:
        ...
        assert chunk.padded_local_token_to_seq.shape[0] == chunk.num_local_context_tokens
        for offset, (start, length) in enumerate(zip(chunk.local_starts, chunk.padded_local_seq_lens)):
    ...
    for chunk in metadata.chunks:
        ...
        for request, (padded_len, local_lens, local_start) in enumerate(
            ...
            for rank, local_len in enumerate(local_lens):
                ...
                values = (request * 100_000 + rank * 10_000 + torch.arange(local_start, local_start + actual_len)).view(
                    -1, 1, 1
                )
