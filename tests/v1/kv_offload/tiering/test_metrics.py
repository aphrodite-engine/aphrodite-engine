...
from aphrodite.v1.kv_offload.base import LookupResult, ReqContext, make_offload_key
from aphrodite.v1.kv_offload.tiering.base import (
    JobResult,
    TieringOffloadingMetrics,
    TransferJob,
)
from aphrodite.v1.kv_offload.tiering.manager import JobMetadata
...
def test_tiering_metrics_tracker_records_finished_job_metrics():
    ...
    cascade_key, promotion_key, failed_cascade_key, failed_promotion_key = to_keys(range(4))
    jobs = [
        JobMetadata(
    ...
    tracker.on_job_finished(jobs[0], JobResult(job_id=0, success=True, transfer_time=0.5))
    tracker.on_job_finished(jobs[1], JobResult(job_id=1, success=True, transfer_time=0.25))
...
def test_tiering_metrics_tracker_reports_active_job_and_primary_usage_gauges():
    ...
    fs_label = ("1:fs",)
    p2p_label = ("2:p2p",)
    assert values[TieringOffloadingMetrics.PRIMARY_READ_USAGE_PERC][fs_label] == pytest.approx(2 / 6)
    assert values[TieringOffloadingMetrics.ACTIVE_CASCADE_JOBS][fs_label] == 1
    assert values[TieringOffloadingMetrics.PRIMARY_WRITE_USAGE_PERC][p2p_label] == pytest.approx(3 / 6)
