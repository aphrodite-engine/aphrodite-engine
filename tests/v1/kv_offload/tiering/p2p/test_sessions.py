...
from aphrodite.v1.kv_offload.tiering.base import TransferJob
from aphrodite.v1.kv_offload.tiering.p2p.session import (
    LoadResult,
    P2PSession,
    StoreResult,
)
from aphrodite.v1.kv_offload.tiering.p2p.session.client import (
...
def _srv_lookups(session: P2PSession) -> list:
    ...
    return [lu for st in session._server._requests.values() for lu in st.lookups.values()]
...
class TestClientFlows:
    ...
    def test_finish_preserves_active_load_until_terminal_result(self):
        session, conn, _ = _make_session()
        _activate(session, conn)
        session.request_blocks(job_id=7, kv_request_id="req-1", keys=[b"k"], block_ids=[0])
    ...
    def test_finish_abort_ack_emits_one_failure(self):
        session, conn, _ = _make_session()
        _activate(session, conn)
        session.request_blocks(job_id=8, kv_request_id="req-1", keys=[b"k"], block_ids=[0])
        session.finish_request("req-1")
    ...
    def test_finish_abort_timeout_emits_one_failure(self):
        session, conn, _ = _make_session()
        _activate(session, conn)
        session.request_blocks(job_id=9, kv_request_id="req-1", keys=[b"k"], block_ids=[0])
        session.finish_request("req-1")
        _client_load(session, "req-1").aborted_at = time.monotonic() - _ABORT_ACK_TIMEOUT_S - 1.0
    ...
    def test_late_transfer_done_after_abort_ack_is_ignored(self):
        session, conn, _ = _make_session()
        _activate(session, conn)
        session.request_blocks(job_id=10, kv_request_id="req-1", keys=[b"k"], block_ids=[0])
        session.finish_request("req-1")
        ...
        assert session.poll().loads == [LoadResult(job_id=10, kv_request_id="req-1", success=False)]
...
class TestLookupFlow:
    ...
    def test_aggregate_flush_resolve_round_trip(self):
        ...
        sent_before = len(conn._sent)
        session.flush_pending_lookups()
        assert [m for m in conn._sent[sent_before:] if m[TYPE_KEY] == LookupMsg.TYPE] == []
    ...
    def test_request_blocks_clears_probe_cache(self):
        ...
        session.request_blocks(job_id=1, kv_request_id="req-1", keys=[b"hA"], block_ids=[0])
    ...
    def test_finish_after_real_fetch_sends_no_second_fetch(self):
        ...
        session.poll()
        session.request_blocks(job_id=1, kv_request_id="req-1", keys=[b"hA"], block_ids=[7])
        sent_before = len(conn._sent)
    ...
    def test_server_lookup_deferred_until_serve_then_all_misses(self):
        ...
        assert [m for m in conn._sent[sent_before:] if m[TYPE_KEY] == LookupRespMsg.TYPE] == []

        _serve(session, FakeParent())

        resps = [m for m in conn._sent[sent_before:] if m[TYPE_KEY] == LookupRespMsg.TYPE]
        assert len(resps) == 1
        resp = resps[0]
