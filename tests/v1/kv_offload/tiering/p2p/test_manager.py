...
from aphrodite.v1.core.kv_cache_utils import DEFAULT_NONE_HASH_SEED, init_none_hash
...
from aphrodite.v1.kv_offload.tiering.p2p import manager as manager_module
from aphrodite.v1.kv_offload.tiering.p2p.manager import (
    _UNBOUND_STORE_TIMEOUT_S,
    P2PSecondaryTierManager,
    _annotate_req_context,
)
from aphrodite.v1.kv_offload.tiering.p2p.session import (
    LoadResult,
    SessionCloseResult,
    SessionPollResult,
    StoreResult,
...
class TestInitHashSeed:
    ...
    def test_seed_resolved_after_init_none_hash(self, monkeypatch):
        ...
        monkeypatch.setattr(manager_module, "get_none_hash_seed", lambda: "random-seed-abc")
...
class TestPeerIdFromParams:
    def test_valid_params(self):
        result = manager_module._peer_id_from_params({"remote_host": "10.0.0.1", "remote_port": 8000})
...
class _RecordingParent:
    ...
    def on_new_request(self, ctx):
        from aphrodite.v1.kv_offload.base import RequestOffloadingContext
...
class TestSubmitLoad:
    ...
    def test_empty_keys_succeeds_immediately(self):
        ...
        mgr = _make_manager()
        job = _job_metadata(job_id=1, keys=[], block_ids=[], kv_params=_remote_prefiller_kv_params())
        mgr.submit_load(job)
        assert mgr._finished_jobs == [JobResult(job_id=1, success=True)]
...
class TestGetFinished:
    ...
    def test_submit_store_parks_unbound_batch(self):
        ...
        mgr = _make_manager()
        job = _job_metadata(job_id=1, kv_params=_remote_decoder_kv_params(kv_request_id="req-1"))
...
class _FakeData:
    ...
    def poll(self, peer_id=None):
        from aphrodite.v1.kv_offload.tiering.p2p.data.base import PollResult
...
class TestDrainJobs:
    def test_returns_immediately_when_quiescent(self):
        ...
        import aphrodite.v1.kv_offload.tiering.p2p.manager as m
...
class TestBindHostPortDefaults:
    """host/port fall back to APHRODITE_P2P_SIDE_CHANNEL_* when not in config."""
    ...
    def _construct(monkeypatch, dp_index=0, **kwargs) -> P2PSecondaryTierManager:
        ...
        monkeypatch.setattr(
            manager_module,
            "FileMapper",
            SimpleNamespace(from_offloading_spec=lambda **_: SimpleNamespace(get_run_config=lambda: {})),
        )
        calls: dict = {}
        monkeypatch.setattr(
            manager_module,
            "NixlTransport",
            lambda agent_name, *a, **k: (calls.update(nixl_name=agent_name) or SimpleNamespace()),
        )
        monkeypatch.setattr(
            manager_module,
            "ZmqTransport",
            lambda local_id, host, port, *a, **k: (
                calls.update(zmq_id=local_id, zmq_host=host, zmq_port=port) or SimpleNamespace()
            ),
        )
        spec = SimpleNamespace(
            blocks_per_chunk=1,
            config=SimpleNamespace(parallel=SimpleNamespace(data_parallel_index=dp_index)),
        )
    ...
    def test_env_override(self, monkeypatch):
        monkeypatch.setenv("APHRODITE_P2P_SIDE_CHANNEL_HOST", "10.1.2.3")
    ...
    @pytest.mark.parametrize("bind_host", ["localhost", "127.0.0.1", "::1", "0.0.0.0", "::", "192.0.2.5"])
