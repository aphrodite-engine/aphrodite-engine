...
from aphrodite.v1.kv_offload.tiering.base import (
    JobResult,
    SecondaryTierManager,
    TieringOffloadingMetrics,
    TransferJob,
)
from aphrodite.v1.kv_offload.tiering.example.manager import ExampleSecondaryTierManager
...
def has_histogram_count(reduced: dict[str, int | float], metric_name: str) -> bool:
    return any(key.startswith(f"{metric_name}:") and key.endswith("_count") for key in reduced)
...
def test_tiering_spec_collects_secondary_metric_definitions(monkeypatch):
    ...
    metrics = TieringOffloadingSpec.build_metric_definitions({"secondary_tiers": [{"type": "test_metrics"}]})
    ...
    assert metrics[TieringOffloadingMetrics.PROMOTION_JOB_FAILURES].labelnames == ("tier",)
    assert isinstance(
...
def test_tiering_manager_aggregates_secondary_stats():
    ...
    primary_tier = CPUPrimaryTierOffloadingManager(num_blocks=5, mmap_region=mock_region)
    ...
    secondary_stats.increase_counter(MetricsSecondaryTierManager.MY_TIER_METRIC, 7, ("test_metrics",))
    ...
    assert stats.data["data"][MetricsSecondaryTierManager.MY_TIER_METRIC][("test_metrics",)] == 7
...
class TestTieringOffloadingManager:
    ...
    def manager_setup(self):
        ...
        mock_region = _mock_mmap_region(5)
        self.primary_tier = CPUPrimaryTierOffloadingManager(num_blocks=5, mmap_region=mock_region)
    ...
    def test_failed_promotion_finalizes_primary_with_failure(self, manager_setup):
        ...
        from aphrodite.v1.kv_offload.tiering.base import JobResult
        ...
        with (
            patch.object(self.secondary_tier1, "get_finished_jobs", return_value=[failed]),
    ...
    def test_cascade_to_all_secondary_tiers(self, manager_setup):
        ...
        self.secondary_tier1.submit_store = MagicMock(wraps=self.secondary_tier1.submit_store)
        self.secondary_tier2.submit_store = MagicMock(wraps=self.secondary_tier2.submit_store)
        ...
        assert all(self.secondary_tier1.lookup(b, _CTX) is LookupResult.HIT for b in blocks)
        assert all(self.secondary_tier2.lookup(b, _CTX) is LookupResult.HIT for b in blocks)
    ...
    def test_failed_promotion_keeps_only_successful_blocks(self, manager_setup, successful_indices, expected_results):
        blocks = to_keys(range(3))
        for block in blocks:
            self.secondary_tier1.blocks[block] = True

        def submit_partial(job_metadata: TransferJob) -> None:
            successful_keys = None if successful_indices is None else tuple(blocks[i] for i in successful_indices)
        ...
        assert [self.primary_tier.lookup(block, _CTX) for block in blocks] == expected_results
    ...
    def test_lookup_reports_sync_delay_for_resolved_lookups(self, manager_setup):
        ...
        assert reduced[histogram_count_key(TieringOffloadingMetrics.LOOKUP_SYNC_DELAY, ("0:primary",))] == 2
        assert reduced[histogram_count_key(TieringOffloadingMetrics.LOOKUP_SYNC_DELAY, ("1:example",))] == 2
        assert reduced[histogram_count_key(TieringOffloadingMetrics.LOOKUP_SYNC_DELAY, ("2:example",))] == 2
        assert not has_histogram_count(reduced, TieringOffloadingMetrics.LOOKUP_ASYNC_DELAY)
    ...
    def test_lookup_does_not_report_async_delay_for_promotion(self, manager_setup):
        ...
        assert stats is not None
        reduced = stats.reduce()
        assert reduced[histogram_count_key(TieringOffloadingMetrics.LOOKUP_SYNC_DELAY, ("0:primary",))] == 1
        assert reduced[histogram_count_key(TieringOffloadingMetrics.LOOKUP_SYNC_DELAY, ("1:example",))] == 1
        assert not has_histogram_count(reduced, TieringOffloadingMetrics.LOOKUP_ASYNC_DELAY)
        ...
        if stats is not None:
            assert not has_histogram_count(stats.reduce(), TieringOffloadingMetrics.LOOKUP_ASYNC_DELAY)

    def test_lookup_reports_async_delay_when_deferred_lookup_resolves(self, manager_setup):
        """Async delay is observed when a RETRY lookup later resolves."""
        ctx = ReqContext(req_id="req_lookup_finish")
        self._start_request(ctx)
        block = to_keys(range(1))[0]
        self.secondary_tier1.lookup = MagicMock(side_effect=[LookupResult.RETRY, LookupResult.HIT])
        ...
        reduced = stats.reduce()
        assert reduced[histogram_count_key(TieringOffloadingMetrics.LOOKUP_SYNC_DELAY, ("1:example",))] == 1
        assert reduced[histogram_count_key(TieringOffloadingMetrics.LOOKUP_ASYNC_DELAY, ("1:example",))] == 1
    ...
    def test_lookup_batches_submit_load_per_request(self, manager_setup):
        ...
        self.secondary_tier1.submit_load = MagicMock(wraps=self.secondary_tier1.submit_load)
    ...
    def test_on_request_finished_delays_secondary_until_store_submitted(self, manager_setup):
        ...
        self.primary_tier.on_request_finished = MagicMock(
            side_effect=lambda req_context: calls.append(("primary_finish", req_context.req_id))
        )
        ...
        self.secondary_tier1.on_request_finished = MagicMock(
            side_effect=lambda req_context: calls.append(("secondary_finish_1", req_context.req_id))
        )
        self.secondary_tier2.on_request_finished = MagicMock(
            side_effect=lambda req_context: calls.append(("secondary_finish_2", req_context.req_id))
        )
    ...
    def test_failed_store_finalizes_finished_request(self, manager_setup):
        ...
        self.secondary_tier1.submit_store = MagicMock(wraps=self.secondary_tier1.submit_store)
        self.secondary_tier2.submit_store = MagicMock(wraps=self.secondary_tier2.submit_store)
        self.secondary_tier1.on_request_finished = MagicMock(wraps=self.secondary_tier1.on_request_finished)
        self.secondary_tier2.on_request_finished = MagicMock(wraps=self.secondary_tier2.on_request_finished)
    ...
    def test_reset_cache_clears_pending_primary_stores_for_active_request(self, manager_setup):
    ...
    def test_prepare_store_cascades_existing_blocks_to_request_level_tiers(self, manager_setup, new_ids):
    ...
    def test_cascade_drops_deferred_keys_whose_primary_write_failed(self, manager_setup):
    ...
    def test_reset_cache_clears_orchestrator_state(self, manager_setup):
        ...
        assert self.manager.lookup(promo_block, ReqContext(req_id="pending")) is LookupResult.HIT_PENDING
    ...
    def test_reset_cache_drains_all_tiers(self, manager_setup):
        ...
        reallocated.
        """
        self.secondary_tier1.drain_jobs = MagicMock(wraps=self.secondary_tier1.drain_jobs)
        self.secondary_tier2.drain_jobs = MagicMock(wraps=self.secondary_tier2.drain_jobs)
    ...
    def test_tier_filter_skips_filtered_secondary(self, manager_setup, load_tier_filter):
    ...
    def test_tier_filter_allows_matching_secondary(self, manager_setup, load_tier_filter):
...
class TestTieringOffloadingWithoutSecondaryTiers:
    ...
    def test_works_without_secondary_tiers(self):
        ...
        primary_tier = CPUPrimaryTierOffloadingManager(num_blocks=5, mmap_region=_mock_mmap_region(5))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            TierFilter(matchers=(TierMatcher(medium=Medium.STORAGE, locality=Locality.LOCAL),)),
        ),
        (
