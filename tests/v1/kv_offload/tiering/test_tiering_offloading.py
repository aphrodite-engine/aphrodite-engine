...
class TestTieringOffloadingManager:
    ...
    def test_lookup_reports_sync_delay_for_resolved_lookups(self, manager_setup):
        ...
        if stats is not None:
            assert f"{TieringOffloadingMetrics.LOOKUP_SYNC_DELAY}_count" not in stats.reduce()
    ...
    def test_lookup_reports_async_delay_across_promotion(self, manager_setup):
        ...
        stats = self.manager.get_stats()
        if stats is not None:
            assert f"{TieringOffloadingMetrics.LOOKUP_ASYNC_DELAY}_count" not in stats.reduce()
    ...
    def test_lookup_reports_async_delay_on_request_finish(self, manager_setup):
        ...
        if stats is not None:
            reduced = stats.reduce()
            assert f"{TieringOffloadingMetrics.LOOKUP_SYNC_DELAY}_count" not in reduced
            assert f"{TieringOffloadingMetrics.LOOKUP_ASYNC_DELAY}_count" not in reduced
    ...
    def test_on_new_request_lifecycle(self, manager_setup):
        ...
        assert self.manager._req_state[ctx.req_id].request_level_tiers == {self.secondary_tier1}

        # Cleanup
        self.manager.on_request_finished(ctx)
        assert ctx.req_id not in self.manager._req_state

    def test_prepare_store_cascades_existing_blocks_to_request_level_tiers(self, manager_setup):
    ...
    def test_reset_cache_clears_orchestrator_state(self, manager_setup):
        ...
        assert self.manager.lookup(promo_block, ReqContext(req_id="pending")) is LookupResult.RETRY
        ...
        assert self.manager._req_state[rl_ctx.req_id].request_level_tiers == {self.secondary_tier1}
