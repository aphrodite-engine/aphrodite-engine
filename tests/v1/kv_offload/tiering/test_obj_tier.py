...
from aphrodite.v1.kv_offload.tiering.obj.config import ObjStoreConfig
from aphrodite.v1.kv_offload.tiering.obj.manager import ObjectStoreSecondaryTierManager
...
def _make_tier(
    ...
    **tier_kwargs,
    ...
    with (
        patch("aphrodite.v1.kv_offload.tiering.obj.manager.nixl_agent_config"),
        patch(
            "aphrodite.v1.kv_offload.tiering.obj.manager.nixl_agent",
...
class TestMockObjTierFailures:
    ...
    def test_release_xfer_failure_retries_without_losing_result(self, monkeypatch):
        ...
        release_xfer = MagicMock(side_effect=[RuntimeError("transfer is still active"), None])
    ...
    @pytest.mark.parametrize("cleanup_method", ["release_dlist_handle", "deregister_memory"])
    def test_post_transfer_cleanup_failure_does_not_lose_result(self, monkeypatch, cleanup_method):
        tier, agent = _make_tier(num_blocks=4)
        monkeypatch.setattr(
            agent,
            cleanup_method,
    ...
    def test_xfer_cleanup_retry_finalizes_parent_job_and_primary_pin(self, monkeypatch):
        ...
        primary_tier = CPUPrimaryTierOffloadingManager(num_blocks=num_blocks, mmap_region=mmap_region)
        obj_tier, agent = _make_tier(num_blocks=num_blocks, primary_kv_view=primary_kv_view)
        manager = TieringOffloadingManager(primary_tier=primary_tier, secondary_tiers=[obj_tier])
