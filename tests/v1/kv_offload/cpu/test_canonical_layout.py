...
from aphrodite.v1.kv_offload.cpu.gpu_worker import (
...
def test_writer_rotation_matches_is_writer():
    ...
    ids = _canonical_page_ids(np.array([3, 7, 9]), blocks_per_chunk=4, count=10, skip_count=2)
...
def _transfer(handler, num_blocks: int, gpu_to_cpu: bool) -> None:
    ...
    gpu_spec = GPULoadStoreSpec(block_ids, group_sizes=(num_blocks,), block_indices=(0,))
...
def test_gpu_roundtrip_assembles_canonical_page_across_ranks():
    ...
    torch.manual_seed(0)
    num_blocks = 4
    gpu_rank = [torch.randint(-128, 128, (num_blocks, 1024), dtype=torch.int8, device="cuda") for _ in range(2)]
    cpu_canonical = torch.zeros(num_blocks, 2048, dtype=torch.int8, pin_memory=True)

    for rank in (0, 1):
        store = _canonical_handler(gpu_rank[rank], cpu_canonical, _tp2_rank_mapping(rank), gpu_to_cpu=True)
    ...
    for rank in (0, 1):
        ...
        for run in _tp2_rank_mapping(rank).runs:
            for i in range(run.num_fragments):
                ...
                expected[:, co : co + run.fragment_size] = local[:, lo : lo + run.fragment_size]
    ...
    load = _canonical_handler(gpu_back, cpu_canonical, _tp2_rank_mapping(0), gpu_to_cpu=False)
    ...
    load_full = _canonical_handler(gpu_full, cpu_canonical, _whole_page_mapping(), gpu_to_cpu=False)
...
def _nhd_shard_mapping(tp: int, rank: int) -> CanonicalPageMapping:
    ...
    return CanonicalPageMapping(_CANONICAL_PAGE, 2 * _BLOCK_TOKENS * frag, (k_run, v_run), 1, 0, True)
    ...
    try:
        ...
        for rank in range(reader_tp):
            ...
            assert torch.equal(gpu_out.cpu(), expected), (
                f"reader tp={reader_tp} rank={rank} bytes diverge from the tp={writer_tp} writers' ground truth"
            )
    finally:
