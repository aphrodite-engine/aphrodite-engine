...
def test_worker_logs_handler_error_and_cleans_region(
    caplog_aphrodite, monkeypatch: pytest.MonkeyPatch, failing_handler
) -> None:
    worker = CPUOffloadingWorker.__new__(CPUOffloadingWorker)
    calls: list[str] = []
    store_handler = MagicMock()
    ...
    with caplog_aphrodite.at_level(logging.ERROR, logger="aphrodite.v1.kv_offload.cpu.gpu_worker"):
        worker.shutdown()
    ...
    mmap_region.cleanup.assert_called_once_with()
    assert worker._mmap_region is None
    assert f"Failed to shut down {failing_handler} offloading handler" in caplog_aphrodite.text
    assert calls[-2:] == ["sync", "region"]


def test_handler_shutdown_skips_transfers_after_event_sync_failure() -> None:
    handler = gpu_worker.SingleDirectionOffloadingHandler.__new__(gpu_worker.SingleDirectionOffloadingHandler)
...
def test_worker_syncs_before_cleanup_after_handler_failure(
    caplog_aphrodite, monkeypatch: pytest.MonkeyPatch, device_sync_fails: bool
) -> None:
    worker = CPUOffloadingWorker.__new__(CPUOffloadingWorker)
    calls: list[str] = []
    store_handler = MagicMock()
    ...
    with caplog_aphrodite.at_level(logging.WARNING, logger="aphrodite.v1.kv_offload.cpu.gpu_worker"):
        worker.shutdown()

    assert calls == ["sync", "region"]
    mmap_region.cleanup.assert_called_once_with()
    if device_sync_fails:
        assert "Device sync before mmap cleanup failed" in caplog_aphrodite.text


@pytest.mark.parametrize("gpu_to_cpu", [True, False])
@pytest.mark.parametrize("num_mappings", NUM_MAPPINGS)
@pytest.mark.parametrize("gpu_page_size_bytes", GPU_PAGE_SIZES)
@pytest.mark.parametrize("blocks_per_chunk", BLOCK_SIZE_FACTORS)
...
def test_transfer(
    ...
    cpu_blocks_expanded = [
        cpu_block * blocks_per_chunk + j for cpu_block in cpu_blocks for j in range(blocks_per_chunk)
    ]
    ...
    if gpu_to_cpu:
        ...
        src_spec = GPULoadStoreSpec(gpu_blocks, group_sizes=(len(gpu_blocks),), block_indices=(blocks_to_skip,))
        ...
        dst_spec = GPULoadStoreSpec(gpu_blocks, group_sizes=(len(gpu_blocks),), block_indices=(blocks_to_skip,))
    ...
    while time.time() < end_time:
        ...
        if finished:
            ...
            assert finished[0].transfer_size == (
                len(gpu_blocks) * sum([x.page_size_bytes for x in handler.layer_refs_per_group[0]])
            )
...
def test_transfer_multi_group(
    ...
    canonical_kv_caches = CanonicalKVCaches(tensors=kv_cache_tensors, group_data_refs=kv_cache_groups_data_refs)
    ...
    if sub_blocks_to_skip > 0:
        gpu_blocks_per_group[2] = gpu_blocks_per_group[2][sub_blocks_to_skip:-sub_blocks_to_skip]
        cpu_blocks_expanded_per_group[2] = cpu_blocks_expanded_per_group[2][sub_blocks_to_skip:-sub_blocks_to_skip]
    ...
    if gpu_to_cpu:
        handler = worker._store_handler
        src_spec = GPULoadStoreSpec(gpu_blocks, group_sizes=group_sizes, block_indices=block_indices)
        dst_spec = CPULoadStoreSpec(cpu_blocks)
        # per-group mapping: cpu sub-block -> gpu sub-block
        dst_to_src_per_group = [
            dict(zip(expanded, gpu_blks))
            for expanded, gpu_blks in zip(cpu_blocks_expanded_per_group, gpu_blocks_per_group)
        ]
        num_dst_sub_blocks = num_cpu_blocks * blocks_per_chunk
    else:
        handler = worker._load_handler
        src_spec = CPULoadStoreSpec(cpu_blocks)
        dst_spec = GPULoadStoreSpec(gpu_blocks, group_sizes=group_sizes, block_indices=block_indices)
        # per-group mapping: gpu sub-block -> cpu sub-block
        dst_to_src_per_group = [
            dict(zip(gpu_blks, expanded))
            for gpu_blks, expanded in zip(gpu_blocks_per_group, cpu_blocks_expanded_per_group)
        ]
        num_dst_sub_blocks = num_gpu_blocks
    ...
    while time.time() < end_time:
        ...
        if finished:
            ...
            expected_bytes = sum(
                ...
                for group_size, data_refs in zip(group_sizes, handler.layer_refs_per_group)
            )
...
@torch.inference_mode()
def test_load_waits_for_pending_compute_stream_writes(default_aphrodite_config) -> None:
    ...
    gpu_tensor = torch.zeros((num_blocks, page_size_bytes), dtype=torch.int8, device=device)
    loaded_block_ids = torch.tensor(loaded_blocks, dtype=torch.long, device=device)
    worker = CPUOffloadingWorker(
        kv_caches=CanonicalKVCaches(
            tensors=[CanonicalKVCacheTensor(tensor=gpu_tensor, page_size_bytes=page_size_bytes)],
