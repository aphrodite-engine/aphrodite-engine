...
import aphrodite.platforms as platforms
from aphrodite.v1.engine import core as core_module
from aphrodite.v1.engine import utils as engine_utils
...
def test_freeze_gc_after_clean_rocm_engine_core_shutdown(
    ...
    calls: list[str] = []
    aphrodite_config = SimpleNamespace(shutdown_timeout=shutdown_timeout)
    ...
    monkeypatch.setattr(platforms, "current_platform", SimpleNamespace(is_rocm=lambda: is_rocm))
    monkeypatch.setattr(core_module.gc, "freeze", lambda: calls.append("freeze"))

    with pytest.raises(SystemExit):
        EngineCoreProc.run_engine_core(aphrodite_config=aphrodite_config)
...
def test_wait_for_engine_startup_reports_watched_process_exit():
    ...
    assert "Frontend process failed during engine core initialization" in str(exc_info.value)
