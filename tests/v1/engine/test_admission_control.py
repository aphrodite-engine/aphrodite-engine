...
from aphrodite.utils.argparse_utils import human_readable_int
...
def test_graceful_http_error_is_aphrodite_error():
    err = GracefulHTTPError("msg", HTTPStatus.TOO_MANY_REQUESTS)
    assert isinstance(err, APHRODITEError)
...
def test_admission_exceptions_are_aphrodite_errors(exc_cls):
    """Admission rejections must reach the APHRODITEError HTTP handler."""
    assert issubclass(exc_cls, APHRODITEError)
...
async def test_concurrent_single_request_admission_respects_limit():
    ...
    llm.output_processor.get_num_unfinished_requests.side_effect = lambda: len(request_states)
    llm.output_processor.has_request.side_effect = request_states.__contains__

    def add_request(request, *_args):
        request_states[request.request_id] = _make_req_state(len(request.prompt_token_ids))
    ...
    requests = [SimpleNamespace(request_id=f"request-{idx}", prompt_token_ids=[idx]) for idx in range(2)]
    results = await asyncio.gather(
        *(llm._add_request(request, None, None, 0, MagicMock()) for request in requests),
