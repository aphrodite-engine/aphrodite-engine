...
def test_suffix_decoding_acceptance(
    ...
    for i in range(10):  # Run multiple times to warm up the cache.
        ...
        for metric in metrics:
            if metric.name == "aphrodite:spec_decode_num_draft_tokens":
                num_draft.append(metric.value)
            if metric.name == "aphrodite:spec_decode_num_accepted_tokens":
...
def test_speculators_model_integration(
    ...
    This verifies the `aphrodite serve <speculator-model>` use case where
    ...
    with aphrodite_runner(
        ...
        evaluate_llm_for_gsm8k(spec_runner.llm, expected_accuracy_threshold=expected_accuracy_threshold)
        spec_outputs = spec_runner.llm.chat(test_prompts, sampling_config)

        # Verify speculative config was auto-detected
        assert spec_runner.llm.llm_engine.aphrodite_config.speculative_config is not None, (
            f"Speculative config should be auto-detected for {model_path}"
        )

        spec_config = spec_runner.llm.llm_engine.aphrodite_config.speculative_config
        ...
        verifier_model = spec_runner.llm.llm_engine.aphrodite_config.model_config.model
    ...
    with monkeypatch.context() as m:
        ...
        if attn_backend == "ROCM_AITER_FA" and current_platform.is_rocm():
            if "deepseek" in model_setup[1].lower():
                ...
                m.delenv("APHRODITE_MLA_DISABLE", raising=False)
...
def test_mtp_correctness(
    ...
    with monkeypatch.context() as m:
        ...
        if "Qwen3.5" in model_name and os.environ.get("APHRODITE_USE_V2_MODEL_RUNNER"):
            pytest.skip(
...
def test_acceptance_rates(
    ...
    """
    monkeypatch.setenv("APHRODITE_USE_V2_MODEL_RUNNER", "1" if use_mrv2 else "0")
    ...
    for dataset_name, expected_len in expected_acceptance_lengths.items():
        ...
        print(f"acceptance_len for {dataset_name}: {mean_acceptance_length:.2f} (expected at least {expected_len:.2f})")
