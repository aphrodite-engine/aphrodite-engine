...
def assert_request_outputs_match(
    ...
    *,
    ...
    assert len(ref_outputs) == len(spec_outputs), (
        f"{context}: output count differs: reference={len(ref_outputs)}, speculative={len(spec_outputs)}"
    )
    assert 0 <= required_matches <= len(ref_outputs), (
        f"{context}: invalid required_matches={required_matches} for {len(ref_outputs)} outputs"
    )

    mismatches: list[str] = []
    matches = 0
    for index, (ref_output, spec_output) in enumerate(zip(ref_outputs, spec_outputs)):
        assert ref_output.outputs, f"{context}: reference output {index} has no candidate"
        assert spec_output.outputs, f"{context}: speculative output {index} has no candidate"
    ...
    print(f"{context}: exact text matches={matches}/{len(ref_outputs)} (required={required_matches})")
...
def compute_acceptance_rate(metrics: list[Metric], prev_metrics: list[Metric] | None = None) -> float:
    n_draft_toks = get_spec_decode_metric_value(metrics, "aphrodite:spec_decode_num_draft_tokens")
    if n_draft_toks == 0:
        return float("nan")
    n_accepted_toks = get_spec_decode_metric_value(metrics, "aphrodite:spec_decode_num_accepted_tokens")
    if prev_metrics is not None:
        n_draft_toks -= get_spec_decode_metric_value(prev_metrics, "aphrodite:spec_decode_num_draft_tokens")
        n_accepted_toks -= get_spec_decode_metric_value(prev_metrics, "aphrodite:spec_decode_num_accepted_tokens")
        if n_draft_toks <= 0:
            return float("nan")
...
def compute_acceptance_len(metrics: list[Metric], prev_metrics: list[Metric] | None = None) -> float:
    n_drafts = get_spec_decode_metric_value(metrics, "aphrodite:spec_decode_num_drafts")
    n_accepted_toks = get_spec_decode_metric_value(metrics, "aphrodite:spec_decode_num_accepted_tokens")
    if n_drafts == 0:
        return 1
    if prev_metrics is not None:
        n_drafts -= get_spec_decode_metric_value(prev_metrics, "aphrodite:spec_decode_num_drafts")
        n_accepted_toks -= get_spec_decode_metric_value(prev_metrics, "aphrodite:spec_decode_num_accepted_tokens")
        if n_drafts <= 0:
            return 1
