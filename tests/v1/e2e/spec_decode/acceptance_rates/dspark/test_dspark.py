...
def test_dspark_correctness_and_acceptance_rate(
    ...
    with aphrodite_runner(config.model, **runner_config) as spec_runner:
        ...
        metrics_summary = (
            f"gsm8k_accuracy={accuracy:.3f}, acceptance_rate={acceptance_rate:.3f}, acceptance_len={acceptance_len:.3f}"
        )
        print(f"{context}: {metrics_summary}")

        assert accuracy >= config.reference_accuracy * REGRESSION_TOLERANCE, f"{context}: {metrics_summary}"
        assert acceptance_rate >= config.reference_acceptance_rate * REGRESSION_TOLERANCE, (
            f"{context}: {metrics_summary}"
        )
        assert acceptance_len >= config.reference_acceptance_len * REGRESSION_TOLERANCE, f"{context}: {metrics_summary}"
