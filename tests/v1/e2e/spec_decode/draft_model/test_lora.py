...
@pytest.mark.skipif(not current_platform.is_cuda_alike(), reason="Requires CUDA or ROCm")
def test_batch_inference_correctness(
    ...
    with monkeypatch.context() as m:
        m.setenv("APHRODITE_BATCH_INVARIANT", "1")
        ...
        with aphrodite_runner(
            ...
            ref_outputs = ref_runner.llm.generate(prompts, sampling_params, lora_request=lora_request)
        ...
        with aphrodite_runner(
            ...
            lora_spec_outputs = spec_runner.llm.generate(prompts, sampling_params, lora_request=lora_request)
        ...
        assert_request_outputs_match(
            ...
            context=(f"LoRA + {method}: target={model_name}, draft={spec_model_name}, adapter={lora_path}"),
        )
