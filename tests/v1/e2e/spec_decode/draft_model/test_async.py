...
def test_no_sync_with_spec_decode(
    ...
    generation.
    """
    # Import Aphrodite AFTER sync_tracker fixture has applied the patch
    ...
    with aphrodite_runner(
        ...
        **extra_kwargs,
        ...
        assert len(outputs) == 1, f"{method} target={model}: expected one request output, got {len(outputs)}"
        assert outputs[0].outputs, f"{method} target={model}: request output has no completion candidates"
        assert outputs[0].outputs[0].text, f"{method} target={model}: generated completion text is empty"
