...
def _build_access_code_manual(tokenizer, target_tokens: int) -> tuple[str, list[str]]:
    ...
    facts = "".join(f"The access code for vault-{i:02d} is {code}.\n" for i, code in enumerate(codes))
...
def test_qwen_mtp_mamba_prefix_cache_hit_is_bounded(aphrodite_runner, monkeypatch):
    ...
    for arm, outputs in (("warm", warm_outputs), ("cold", cold_outputs)):
        missed = [i for i, (code, output) in enumerate(zip(codes, outputs)) if code not in output.outputs[0].text]
