...
# a chosen step. Gated on APHRODITE_FT_TEST_INJECT_FAULT.
_FAULT_INJECT_SITECUSTOMIZE = """\
import builtins
import os
import sys

_SPEC = os.environ.get("APHRODITE_FT_TEST_INJECT_FAULT")
_MODULE = "aphrodite.v1.worker.gpu.dp_utils"
...
def _install_fault_injection(monkeypatch, tmp_path, rank: int, step: int) -> None:
    ...
    monkeypatch.setenv("APHRODITE_FT_TEST_INJECT_FAULT", f"rank={rank},step={step}")
