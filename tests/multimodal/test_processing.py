...
from aphrodite.multimodal.processing.context import (
...
def test_find_token_matches(
    ...
    result = {key: list(update.iter_token_matches(prompt)) for key, update in prompt_updates.items()}
...
def test_apply_token_matches_with_placeholders(
    ...
    for update_type, expected_by_mm_count in expected_by_update_type_mm_count.items():
        for mm_count, expected in expected_by_mm_count.items():
            ...
            if any(update_idx is None for update_idxs in result.values() for update_idx in update_idxs):
                continue

            expected_placeholders = expected_placeholders_by_update_type_mm_count[update_type][mm_count]
            ...
            assert new_prompt == expected
            assert {modality: ph_list for modality, ph_list in placeholders.items() if ph_list} == expected_placeholders
...
def test_apply_matches_many_shared_targets_scales_linearly():
    ...
    def measure(item_count: int) -> float:
        mm_prompt_updates = {"image": [[update.resolve(item_idx)] for item_idx in range(item_count)]}
        prompt = [0] * item_count
...
def test_find_mm_placeholders_avoids_quadratic_false_prefixes():
    ...
    mm_prompt_updates = {
        ...
        "frequent_false_prefix": [[PromptReplacement("frequent_false_prefix", [0], [1, 2]).resolve(0)]],
    }
...
def test_apply_prompt_updates_falls_back_to_text_matching():
    ...
    new_token_ids, placeholders = processor._apply_prompt_updates(
        [1000],  # "food"
        {"image": [[PromptReplacement("image", [101, 111, 111], [200, 201]).resolve(0)]]},
    )
...
def test_processor_inputs_hashes_partial_uuids():
    ...
    assert inputs.get_mm_hashes("test-model", "blake3") == {
        "image": [
            "image-uuid",
            MultiModalHasher.hash_kwargs("blake3", model_id="test-model", image=images[1]),
        ]
    }
