...
def test_clip_image_processor(image_assets, mm_registry, dtype, size_factor):
    ...
    for asset in image_assets:
        ...
        assert hf_result.keys() == aphrodite_result.keys()
        for key, hf_tensor in hf_result.items():
            hf_arr: np.ndarray = hf_tensor.numpy()
            aphrodite_arr: np.ndarray = aphrodite_result[key].numpy()

            assert hf_arr.shape == aphrodite_arr.shape, f"Failed for key={key}"
            assert np.allclose(hf_arr, aphrodite_arr), f"Failed for key={key}"
