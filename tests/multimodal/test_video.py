...
from aphrodite.multimodal.video_decoders import decode_video, resolve_video_backend_kwargs
...
from aphrodite.transformers_utils.processor import get_video_processor_cls_name_from_config
...
def test_video_decoder_backends_are_lazy_imported():
    code = """
import sys
import aphrodite.multimodal.video  # noqa: F401

backend_modules = {
    f"aphrodite.multimodal.video_decoders.{backend}"
...
def test_decode_video_imports_only_selected_backend(
    ...
    monkeypatch.setattr("aphrodite.multimodal.video_decoders.import_module", fake_import_module)
    result = decode_video(
        backend,
        loader_cls=None,
        data=b"",
    ...
    assert imports == [(f".{backend}", "aphrodite.multimodal.video_decoders")]
...
def test_video_backend_rejects_options_for_another_decoder():
    with pytest.raises(ValueError, match="num_ffmpeg_threads is not supported by the 'opencv' backend"):
...
def test_video_decoder_spec_validates_frame_recovery(backend: str, error: type[Exception]):
...
def test_pynvvideocodec_decoder_slots_are_bounded(
    ...
    with _fresh_decoder_pool():
        ...
        with ExitStack() as stack:
            retained_slots = [
                stack.enter_context(PyNvVideoCodecVideoBackendMixin._borrow_decoder_slot()) for _ in range(hw_decoders)
            ]

            def borrow_extra_slot():
                with PyNvVideoCodecVideoBackendMixin._borrow_decoder_slot() as extra_slot:
...
def test_pynvvideocodec_cross_subclass_shares_single_pool():
    ...
    with _fresh_decoder_pool() as pool:
        ...
        PyNvVideoCodecVideoBackendMixin._create_decoder_slot = classmethod(fake_create_slot)
        try:
            with ExitStack() as stack:
...
def test_video_processor_from_model_repo(
    ...
    also verifies that the Aphrodite backend produces identical frame indices.
    """
...
def test_video_backend_handles_broken_frames(monkeypatch: pytest.MonkeyPatch):
    ...
    with monkeypatch.context() as m:
        m.setenv("APHRODITE_VIDEO_LOADER_BACKEND", "opencv")
...
def test_video_backend_handles_edit_list_trimmed_video(
    ...
    with monkeypatch.context() as m:
        ...
        video_data, num_visible = create_edit_list_trimmed_video(num_frames=90, trim_start_frame=60)

        loader = VIDEO_LOADER_REGISTRY.load("opencv")
        for backend in ["opencv"]:
            frames, metadata = loader.load_bytes(video_data, num_frames=-1, backend=backend)
...
def test_torchcodec_backend_loads_frames(dummy_video_path, monkeypatch: pytest.MonkeyPatch):
    ...
    with monkeypatch.context() as m:
        ...
        loader = VIDEO_LOADER_REGISTRY.load("opencv")
        frames, metadata = loader.load_bytes(video_data, num_frames=8, backend="torchcodec")
...
def test_torchcodec_dynamic_backend_loads_frames(dummy_video_path, monkeypatch: pytest.MonkeyPatch):
    ...
    with monkeypatch.context() as m:
        ...
        loader = VIDEO_LOADER_REGISTRY.load("opencv_dynamic")
        frames, metadata = loader.load_bytes(video_data, fps=2, max_duration=10, backend="torchcodec")
...
def test_torchcodec_backend_rejects_frame_recovery(dummy_video_path):
    ...
    loader = VIDEO_LOADER_REGISTRY.load("opencv")
    with pytest.raises(AssertionError):
        loader.load_bytes(video_data, num_frames=8, backend="torchcodec", frame_recovery=True)
...
def test_torchcodec_backend_returns_target_frames_not_keyframes():
    ...
    loader = VIDEO_LOADER_REGISTRY.load("opencv")
    frames, metadata = loader.load_bytes(video_bytes, num_frames=num_sampled, backend="torchcodec")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("opencv", {"fps": 2, "backend": "torchcodec"}, 120, id="torchcodec-fps"),
        pytest.param(
            "opencv",
...
def test_video_loader_frames_sampling(
    ...
    if kwargs.get("backend") == "torchcodec":
        pytest.importorskip("torchcodec")
    monkeypatch.setenv("APHRODITE_VIDEO_LOADER_BACKEND", loader_key)
...
class TestGLMGASamplingCaps:
    ...
    @staticmethod
    def _source(total_frames: int, fps: float = 30.0, duration: float = 0) -> VideoSourceMetadata:
...
def test_glm5next_backend_indices_match_sampler(total_frames, original_fps, duration, fps, max_frames):
    """The loader must select exactly the frames the processor's sampler
    would, with target.fps mapping onto the raw-fps override."""
    from aphrodite.transformers_utils.processors.glm5next import (
        glm_sample_frame_indices,
    )

    source = VideoSourceMetadata(total_frames_num=total_frames, original_fps=original_fps, duration=duration)
    target = VideoTargetMetadata(num_frames=-1, fps=fps, max_duration=-1)

    indices = Glm5NextVideoBackend.compute_frames_index_to_sample(source, target, max_frames=max_frames)
...
def test_glm5next_backend_metadata_contract():
    ...
    metadata = Glm5NextVideoBackend.create_hf_metadata(source, indices, video_backend="glm5next")
...
def _write_gray_video(tmp_path, total_frames, fps, size=(32, 32)):
    ...
    writer = cv2.VideoWriter(str(path), cv2.VideoWriter_fourcc(*"mp4v"), fps, size, isColor=False)
...
def test_glm5next_backend_codec_parity(tmp_path, backend):
    ...
    for max_frames in (None, 6):
        ...
        expected = glm_sample_frame_indices(total_frames, float(fps), 12.0, max_frame_count=max_frames)

        frames, metadata = Glm5NextVideoBackend.load_bytes(path.read_bytes(), backend=backend, **kwargs)
