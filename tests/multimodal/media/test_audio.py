...
from aphrodite.multimodal.media import AudioMediaIO
from aphrodite.multimodal.media import audio as audio_module
from aphrodite.multimodal.media.audio import (
    load_audio,
...
def test_memory_guard_rejects_large_allocation():
    ...
    with pytest.raises(ValueError, match="APHRODITE_MAX_AUDIO_DECODE_BYTES"):
        load_audio_soundfile(
            BytesIO(payload),
            sr=None,
            max_duration_s=600,
...
def test_torchcodec_max_duration_rejected(dummy_audio_bytes):
    ...
    pytest.importorskip("torchcodec")
    with pytest.raises(ValueError, match="exceeds maximum allowed duration"):
        load_audio_torchcodec(BytesIO(dummy_audio_bytes), sr=None, max_duration_s=0.0001)
