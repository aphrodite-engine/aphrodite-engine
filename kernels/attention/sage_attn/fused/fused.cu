...
#pragma unroll
  ...
  const float block_amax_val = aphrodite::blockReduceMax(amax_val);
  if (thread_id == 0)
  {
...
#pragma unroll
  ...
  float block_max_val = aphrodite::blockReduceMax(max_val);
  float block_min_val = aphrodite::blockReduceMin(min_val);
  float block_sum_val;

  if constexpr (sub_mean)
  {
    block_sum_val = aphrodite::blockReduceSum(sum_val);
  }
