...
__device__ __forceinline__ void thread_reduce_(
    Tensor<Engine0, Layout0> const& tensor, Tensor<Engine1, Layout1>& summary,
    Operator& op) {
...
#pragma unroll
  for (int mi = 0; mi < size<0>(tensor); ++mi) {
    // If max is -inf, then all elements must have been -inf (possibly due to
    // masking). We don't want (-inf - (-inf)) since that would give NaN. If we
    // don't have float around M_LOG2E the multiplication is done in fp64.
...
__forceinline__ __device__ void max_scale_exp2_sum(
    Tensor<Engine0, Layout0>& tensor, Tensor<Engine1, Layout1>& max,
    Tensor<Engine1, Layout1>& sum, const float scale) {
  static_assert(Layout0::rank == 2, "Only support 2D Tensor");
  static_assert(Layout1::rank == 1, "Only support 1D Tensor");
  CUTE_STATIC_ASSERT_V(size<0>(max) == size<0>(tensor));
#pragma unroll
...
#pragma unroll
    ...
    // If max is -inf, then all elements must have been -inf (possibly due to
    // masking). We don't want (-inf - (-inf)) since that would give NaN.
...
struct Softmax {
  ...
  template <bool Is_first, bool Check_inf = false, typename Tensor0,
            typename Tensor1>
  __forceinline__ __device__ void softmax_rescale_o(Tensor0& acc_s,
                                                    Tensor1& acc_o,
                                                    float softmax_scale_log2) {
    // Reshape acc_s from (MMA=4, MMA_M, MMA_N) to (nrow=(2, MMA_M), ncol=(2,
    // MMA_N))
    Tensor scores =
    ...
    if (Is_first) {
      ...
      FLASH_NAMESPACE::template reduce_max</*zero_init=*/false>(scores,
                                                                row_max);
...
#pragma unroll
      ...
      FLASH_NAMESPACE::scale_apply_exp2(scores, row_max, softmax_scale_log2);
      // We don't do the reduce across threads here since we don't need to use
      // the row_sum. We do that reduce at the end when we need to normalize the
      // softmax.
