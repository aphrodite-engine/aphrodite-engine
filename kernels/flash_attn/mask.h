...
__forceinline__ __device__ void apply_mask_local(
    ...
    const int max_seqlen_k, const int row_idx_offset, const int max_seqlen_q,
    const int warp_row_stride, const int window_size_left,
...
__forceinline__ __device__ void apply_mask_causal_w_idx(
    ...
    Tensor<Engine1, Layout1> const& idx_rowcol, const int col_idx_offset_,
...
struct Mask {
  ...
  template <bool Causal_mask = false, bool Is_even_MN = true, typename Engine,
            typename Layout>
  __forceinline__ __device__ void apply_mask(Tensor<Engine, Layout>& tensor_,
    ...
    // if (cute::thread0()) { printf("Has_alibi = %d, Causal_mask=%d,
    // Is_local=%d, Is_even_MN = %d, Need_masking = %d\n", Has_alibi,
