...
template <typename scalar_t, aphrodite::ScalarTypeId w_type_id>
...
dequant<half, aphrodite::kU4B8.id()>(int q) {
...
dequant<nv_bfloat16, aphrodite::kU4B8.id()>(int q) {
...
template <>
__device__ inline typename ScalarType<half>::FragB
dequant<half, aphrodite::kU4.id()>(int q) {
  const int LO = 0x000f000f;
  const int HI = 0x00f000f0;
  const int EX = 0x64006400;
  // Guarantee that the `(a & b) | c` operations are LOP3s.
...
template <>
__device__ inline typename ScalarType<nv_bfloat16>::FragB
dequant<nv_bfloat16, aphrodite::kU4.id()>(int q) {
  static constexpr uint32_t MASK = 0x000f000f;
  static constexpr uint32_t EX = 0x43004300;
...
template <>
__device__ inline typename ScalarType<half>::FragB
dequant<half, aphrodite::kU8B128.id()>(int q) {
...
template <>
__device__ inline typename ScalarType<nv_bfloat16>::FragB
dequant<nv_bfloat16, aphrodite::kU8B128.id()>(int q) {
  typename ScalarType<nv_bfloat16>::FragB frag_b;
...
template <>
__device__ inline typename ScalarType<half>::FragB
dequant<half, aphrodite::kU8.id()>(int q) {
  static constexpr uint32_t mask_for_elt_01 = 0x5250;
  static constexpr uint32_t mask_for_elt_23 = 0x5351;
  static constexpr uint32_t start_byte_for_fp16 = 0x64646464;
...
template <>
__device__ inline typename ScalarType<nv_bfloat16>::FragB
dequant<nv_bfloat16, aphrodite::kU8.id()>(int q) {
  typename ScalarType<nv_bfloat16>::FragB frag_b;
...
void marlin_mm(const void* A, const void* B, void* C, void* C_tmp, void* s,
  ...
  for (int i = 0; i < tot_m_blocks; i += exec_cfg.max_m_blocks) {
    ...
    GPTQ_CALL_IF(aphrodite::kU4B8, 16, 4, 256)
    GPTQ_CALL_IF(aphrodite::kU4B8, 8, 8, 256)
    GPTQ_CALL_IF(aphrodite::kU4B8, 8, 4, 128)
    GPTQ_CALL_IF(aphrodite::kU4B8, 4, 8, 128)
    GPTQ_CALL_IF(aphrodite::kU8B128, 16, 4, 256)
    GPTQ_CALL_IF(aphrodite::kU8B128, 8, 8, 256)
    GPTQ_CALL_IF(aphrodite::kU8B128, 8, 4, 128)
    GPTQ_CALL_IF(aphrodite::kU8B128, 4, 8, 128)

    AWQ_CALL_IF(aphrodite::kU4, 16, 4, 256)
    AWQ_CALL_IF(aphrodite::kU4, 8, 8, 256)
    AWQ_CALL_IF(aphrodite::kU4, 8, 4, 128)
    AWQ_CALL_IF(aphrodite::kU4, 4, 8, 128)
    AWQ_CALL_IF(aphrodite::kU8, 16, 4, 256)
