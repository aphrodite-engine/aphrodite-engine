...
__global__ void Marlin(
  ...
  auto dequant_data = [&](int q, scalar_t2* frag_b_ptr) {
    if constexpr (has_zp && is_zp_float || !has_zp) {
      ...
      static_assert(w_type_id == aphrodite::kU4.id() || w_type_id == aphrodite::kU8.id());
      ...
      if constexpr (w_type_id == aphrodite::kU4.id()) {
        dequant<scalar_t2, aphrodite::kU4B8.id()>(q, frag_b_ptr);
      } else if constexpr (w_type_id == aphrodite::kU8.id()) {
        dequant<scalar_t2, aphrodite::kU8B128.id()>(q, frag_b_ptr);
      }
    }
  };
