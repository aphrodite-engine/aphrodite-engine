...
def get_entries_at_depth(
    depth: int, entries_and_traces: list[tuple[Any, Any]], node: dict, curr_depth: int = 0, trace=()
):
    ...
    for child in node["children"]:
        get_entries_at_depth(depth, entries_and_traces, child, curr_depth=curr_depth + 1, trace=trace)
...
def attempt_to_make_names_unique(entries_and_traces):
    ...
    for name in non_unique_names:
        entries_and_traces_with_name = [(entry, trace) for entry, trace in entries_and_traces if entry["name"] == name]

        zipped_traces = list(zip(*[trace for _, trace in entries_and_traces_with_name]))
        first_trace_difference = next(
            (i for i, trace_eles in enumerate(zipped_traces) if not all_the_same(trace_eles)), None
        )
        ...
        for entry, trace in entries_and_traces_with_name:
            entry["name"] = " <- ".join((entry["name"],) + trace[: first_trace_difference + 1])
...
def group_trace_by_operations(trace_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def is_gemm_op(op_name: str):
        if is_quant(op_name):
            return False
        if (
            "xmma_gemm" in op_name
            or "gemv2T_kernel" in op_name
            or "splitKreduce" in op_name
            or "void cutlass::Kernel" in op_name
    ...
    def is_nccl_all_reduce(op_name: str):
        return is_nccl_op(op_name) and ("all_reduce" in op_name.lower() or "allreduce" in op_name.lower())
    ...
    cross_device_reduce_1stage_ops = list(filter(lambda x: is_cross_device_reduce_1stage(x), ops))
    ops = list(filter(lambda x: x not in cross_device_reduce_1stage_ops, ops))

    cross_device_reduce_2stage_ops = list(filter(lambda x: is_cross_device_reduce_2stage(x), ops))
    ops = list(filter(lambda x: x not in cross_device_reduce_2stage_ops, ops))

    custom_ar_all_reduce_unreg_ops = list(filter(lambda x: is_custom_ar_all_reduce_unreg(x), ops))
    ...
    if len(nccl_all_reduce_ops):
        trace_df["nccl_all_reduce_ops"] = trace_df[nccl_all_reduce_ops].agg("sum", axis=1)
    ...
    if len(cross_device_reduce_1stage_ops):
        trace_df["cross_device_reduce_1stage_ops"] = trace_df[cross_device_reduce_1stage_ops].agg("sum", axis=1)
    if len(cross_device_reduce_2stage_ops):
        trace_df["cross_device_reduce_2stage_ops"] = trace_df[cross_device_reduce_2stage_ops].agg("sum", axis=1)
    if len(custom_ar_all_reduce_unreg_ops):
        trace_df["custom_ar_all_reduce_unreg_ops"] = trace_df[custom_ar_all_reduce_unreg_ops].agg("sum", axis=1)
    ...
    trace_df.drop(
        ...
        + custom_ar_all_reduce_unreg_ops
        + reduce_kernel_ops,
        axis=1,
        inplace=True,
    )
...
def plot_trace_df(traces_df: pd.DataFrame, plot_metric: str, plot_title: str, output: Path | None = None):
    phases = traces_df["phase"].unique()
    traces_df = traces_df.pivot_table(index="phase", columns="name", values=plot_metric, aggfunc="sum")
...
def main(
    ...
    def prepare_data(profile_json: dict, step_keys: list[str]) -> pd.DataFrame:
        ...
        # To pandas dataframe
        trace_dfs = list(map(lambda t: pd.DataFrame([entry for entry, _ in t]).fillna(0), traces))

        # Respect top_k
        if top_k:
            trace_dfs = list(map(lambda trace_df: keep_only_top_entries(trace_df, "cuda_time_us", top_k), trace_dfs))
    ...
    plot_trace_df(prefill_traces, plot_metric, "prefill " + plot_title_suffix, output_directory / Path("prefill.png"))
    plot_trace_df(
        decode_traces, plot_metric, "decodes " + plot_title_suffix, output_directory / Path("decode_steps.png")
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--json-trace", type=str, required=True, help="json trace file output by examples/offline_profile.py"
    )
    parser.add_argument("--output-directory", type=str, required=False, help="Directory to output plots")
    parser.add_argument("--level", type=str, default="module", choices=["module", "kernel"])
    parser.add_argument("--top-k", type=int, default=12, help="Only graph the top `top_k` entries by time.")
    parser.add_argument(
        "--fold-json-node",
        nargs="+",
    ...
    parser.add_argument(
        "--step-plot-interval", type=int, default=4, help="For every `step_plot_interval` steps, plot 1 step"
    )
