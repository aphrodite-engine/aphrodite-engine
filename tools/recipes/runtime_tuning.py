...
"""Optional runtime refinement for configs produced from Aphrodite Recipes.
...
def _resolve_max_num_batched_tokens(
    ...
    # Use Aphrodite's scheduler defaults/constraints as floors rather than local
    ...
    # recipe explicitly disables it, Aphrodite requires the token budget to cover
...
def _resolve_data_parallel_size(
    ...
    result.notes.append(
        "data-parallel-size: target QPS supplied; kept recipe value because per-replica capacity is not known yet"
    )
...
def get_runtime_tuning_policies(
    ...
    if not isinstance(recipe_hardware, str) or not recipe_hardware:
        raise ValueError("Runtime tuning requires the resolved Recipes JSON to declare a non-empty `hardware` field.")
