...
With TTFT/TPOT objectives, the benchmark uses Aphrodite `--goodput`. The recommender
...
```bash
source env.sh
aphrodite serve --config sweep/recommended-config.yml
```

## Aphrodite CPU Docker Shell

Use the common CPU container setup in
[RUNTIME_TUNING.md](RUNTIME_TUNING.md#aphrodite-cpu-docker-shell). It ensures
...
`run_sweep.sh` starts and stops its own Aphrodite servers.
...
```bash
source /output/env.sh
aphrodite serve --config /output/sweep/recommended-config.yml
```
