...
"""CPU hardware discovery for optional Aphrodite Recipes runtime tuning.
...
def detect_hardware() -> HardwareInfo:
    ...
    Aphrodite's CPU helpers already honor process CPU affinity and cgroup-aware
    ...
    if not cpus:
        raise RuntimeError("Aphrodite reported no CPUs available to this process.")
    ...
    physical_cores = {(cpu.numa_node, cpu.physical_core) for cpu in cpus if cpu.physical_core >= 0}

    numa_nodes = []
    for node_id in effective_numa_nodes:
        node_cpus = [cpu for cpu in cpus if cpu.numa_node == node_id]
        node_physical_cores = {cpu.physical_core for cpu in node_cpus if cpu.physical_core >= 0}
    ...
    socket_ids = {socket_id for cpu in cpus if (socket_id := _read_socket_id(cpu.id)) is not None}
