...
"""Select one measured scheduler recommendation from Aphrodite sweep results."""
...
def _percentile_summary(rows: list[dict[str, Any]], key: str) -> tuple[float | None, float | None, float | None]:
    values = [_number(row.get(key)) for row in rows]
    numbers = [value for value in values if value is not None]
    if not numbers:
...
def _aggregate_candidates(
    ...
    *,
    ...
    for (seqs, batch), runs in sorted(grouped.items()):
        ...
        mean_p99_ttft_ms, median_p99_ttft_ms, worst_p99_ttft_ms = _percentile_summary(runs, "p99_ttft_ms")
        mean_p99_tpot_ms, median_p99_tpot_ms, worst_p99_tpot_ms = _percentile_summary(runs, "p99_tpot_ms")

        completed_requests = sum(int(_number(run.get("completed")) or 0) for run in runs)
        estimated_compliant_requests = sum(
            (_number(run.get("request_goodput")) or 0.0) * (_number(run.get("duration")) or 0.0) for run in runs
        )
        combined_compliance_ratio = (
            estimated_compliant_requests / completed_requests if completed_requests > 0 else None
        )
        ...
        if ttft_sla_ms is not None:
            p99_sla_eligible = bool(
                p99_sla_eligible and median_p99_ttft_ms is not None and median_p99_ttft_ms <= ttft_sla_ms
            )
        if tpot_sla_ms is not None:
            p99_sla_eligible = bool(
                p99_sla_eligible and median_p99_tpot_ms is not None and median_p99_tpot_ms <= tpot_sla_ms
            )
        compliance_eligible = bool(
            valid and combined_compliance_ratio is not None and combined_compliance_ratio >= minimum_compliance
        )
        ...
        candidates.append(
            {
                ...
                "combined_compliance_percent": (
                    combined_compliance_ratio * 100 if combined_compliance_ratio is not None else None
                ),
...
def _select_candidate(
    ...
    *,
    ...
    if not valid:
        raise ValueError(
            "No valid sweep configuration was found. Inspect the sweep results for failed requests or missing metrics."
        )
    ...
    if use_goodput:
        ...
        if best_effort["mean_request_goodput"] <= 0:
            raise ValueError(
                "No sweep configuration produced non-zero request goodput for the supplied TTFT/TPOT objectives."
            )
...
def _write_config(
    ...
    *,
    ...
    header = (
        "# Generated from the initial recipe config after a Aphrodite parameter sweep.\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Select one recommended max-num-seqs/max-num-batched-tokens pair from generated Aphrodite sweep results."
        )
    )
    parser.add_argument(
        "--results-dir",
...
def main() -> int:
    ...
    if not results_dir.exists():
        raise ValueError(f"Sweep results were not found at {results_dir}. Run the sweep first.")
    ...
    winner, best_effort, objective = _select_candidate(candidates, use_goodput=use_goodput)
    ...
    recommendation = {
        "status": ("sla_feasible" if winner is not None else "no_sla_feasible_configuration"),
    ...
    print(f"  aphrodite serve --config {output_config}")
    return 0
