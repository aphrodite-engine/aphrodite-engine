...
"""
Convert a Aphrodite Recipes per-hardware JSON rendering into:

  1) config.yml - native `aphrodite serve --config` YAML
  2) env.sh     - environment variables required by the recipe

Examples:
  # Interactive discovery
  python3 recipe_json_to_aphrodite_config.py
...
Then:
  source env.sh
  aphrodite serve --config config.yml
...
The converter intentionally targets a single `aphrodite serve` process. If the
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Convert Aphrodite Recipes JSON to native aphrodite serve config.yml + env.sh"
    )
    p.add_argument(
        "source",
        nargs="?",
        help=("Per-hardware recipe JSON URL or local JSON file. If omitted, discover a recipe from the Recipes API."),
    )
    p.add_argument(
        "--model",
        help=(
    ...
    p.add_argument(
        "--hardware",
        help=("Hardware ID for recipe discovery, for example xeon6, b200, or arc_pro_b70."),
    )
    p.add_argument(
        "--strategy",
        help=(
    ...
    p.add_argument(
        "--config-out",
        default="config.yml",
        help="Output Aphrodite YAML config (default: config.yml)",
    )
    p.add_argument(
        "--env-out",
        default="env.sh",
    ...
    sweep.add_argument(
        "--generate-sweep",
        action="store_true",
        help=("Generate an optional aphrodite bench sweep package around the single initial runtime suggestion."),
    )
    sweep.add_argument(
        "--sweep-out-dir",
        default="sweep",
...
def load_json(source: str) -> dict[str, Any] | list[Any]:
    if source.startswith(("http://", "https://")):
        req = urllib.request.Request(
            source,
            headers={"User-Agent": "aphrodite-recipe-config-converter/1.0"},
        )
...
def search_models(models: list[dict[str, Any]], query: str, limit: int = 20) -> list[dict[str, Any]]:
    q = query.strip().lower()
    if not q:
        return []
...
def select_hardware(by_hardware: dict[str, str], requested: str | None) -> tuple[str, str]:
    ...
    if requested:
        ...
        if selected is None:
            raise ValueError(
                f"Hardware {requested!r} is not available for this model. Available: {', '.join(hardware_ids)}"
            )
...
def strategy_sources(api_base: str, hardware_json_url: str, recipe: dict[str, Any]) -> tuple[str, dict[str, str]]:
    recommended = recipe.get("strategy")
    if not isinstance(recommended, str) or not recommended:
        raise ValueError("Hardware recipe JSON does not contain a usable `strategy` field.")

    sources = {recommended: hardware_json_url}
    raw_alternatives = recipe.get("alternatives") or {}
    if not isinstance(raw_alternatives, dict):
        raise ValueError("Hardware recipe JSON `alternatives` must be an object when present.")
...
def select_strategy(
    ...
    if requested:
        selected = next(
            (strategy for strategy in strategies if strategy.lower() == requested.lower()),
            None,
        )
        if selected is None:
            raise ValueError(
                f"Strategy {requested!r} is not available for this model/hardware. Available: {', '.join(strategies)}"
            )
...
def discover_recipe_source(
    ...
    if not isinstance(recommended, dict):
        raise ValueError(f"Model {model.get('hf_id')!r} has no rendered recommended_command in the Recipes API.")

    raw_by_hardware = recommended.get("by_hardware")
    if not isinstance(raw_by_hardware, dict) or not raw_by_hardware:
        raise ValueError(
            f"Model {model.get('hf_id')!r} has no per-hardware renderings in recommended_command.by_hardware."
        )

    by_hardware = {str(hw): path for hw, path in raw_by_hardware.items() if isinstance(path, str) and path}
    if not by_hardware:
        raise ValueError(f"Model {model.get('hf_id')!r} has no usable hardware JSON paths.")
...
def merge_value(dst: dict[str, Any], path: list[str], value: Any) -> None:
    ...
    for part in path[:-1]:
        ...
        if existing is None:
            ...
            raise ValueError(f"Cannot merge nested option {'.'.join(path)!r}: {part!r} is already a scalar")
        cur = cur[part]
...
def argv_to_config(argv: list[Any]) -> dict[str, Any]:
    ...
    if len(argv) < 3 or argv[0:2] != ["aphrodite", "serve"]:
        raise ValueError(f"Expected recipe argv to start with: ['aphrodite', 'serve', MODEL, ...]. Got: {argv[:4]!r}")

    model = argv[2]
    if model.startswith("-"):
        raise ValueError(f"Expected model after 'aphrodite serve', got {model!r}")
    ...
    while i < len(argv):
        ...
        if not token.startswith("--"):
            raise ValueError(
                ...
                "The converter accepts Aphrodite serve long options and known short aliases."
            )
...
def write_config(
    ...
    metadata = [
        "# Generated from Aphrodite Recipes JSON.",
        f"# Source: {source}",
    ]
...
def main() -> int:
    ...
    try:
        source = args.source
        if source is None:
            source = discover_recipe_source(args.api_base, args.model, args.hardware, args.strategy)
        ...
        sweep_writer = None
        if tuning_requested:
            # Keep plain Recipes conversion lightweight. Aphrodite-specific modules
    ...
    print(f"  aphrodite serve --config {shlex.quote(args.config_out)}")
    if sweep_files:
