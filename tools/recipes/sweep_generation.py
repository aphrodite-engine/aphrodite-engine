...
"""Generate optional Aphrodite benchmark sweep files from one initial suggestion."""
...
def _benchmark_models(config: dict[str, Any]) -> tuple[str, str]:
    ...
    if isinstance(served_model_name, list) and served_model_name and isinstance(served_model_name[0], str):
...
def _write_run_script(
    ...
    *,
    ...
    if goodput_pairs:
        bench_parts.append("--goodput " + " ".join(shlex.quote(pair) for pair in goodput_pairs))
...
aphrodite bench sweep serve \
  --serve-cmd "aphrodite serve --config '${{CONFIG_PATH}}'" \
  --bench-cmd "{bench_cmd}" \
...
def _write_recommend_script(
    ...
    *,
    ...
    replacements = {
        "DEFAULT_CONFIG_PATH: str | None = None": (f"DEFAULT_CONFIG_PATH: str | None = {config_rel!r}"),
        "DEFAULT_ENV_PATH: str | None = None": (f"DEFAULT_ENV_PATH: str | None = {env_rel!r}"),
        "DEFAULT_TTFT_SLA_MS: float | None = None": (f"DEFAULT_TTFT_SLA_MS: float | None = {workload.ttft_sla_ms!r}"),
        "DEFAULT_TPOT_SLA_MS: float | None = None": (f"DEFAULT_TPOT_SLA_MS: float | None = {workload.tpot_sla_ms!r}"),
    }
...
def _write_guide(path: Path, workload: WorkloadHints) -> None:
    ...
    if sla_text:
        sla_text = (
            ...
            + sla_text
            + "\n\nThe generated benchmark uses these values with Aphrodite "
            "`--goodput`.\n"
        )
...
By default, Aphrodite benchmarks each parameter combination three times.
{sla_text}
## Outputs
