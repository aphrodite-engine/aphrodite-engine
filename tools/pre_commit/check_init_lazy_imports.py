...
def _is_internal(name: str | None, *, level: int = 0) -> bool:
    ...
    if name is None:
        return False
    return name.startswith("aphrodite.") or name == "aphrodite"


def _fail(violations: Iterable[tuple[int, str]]) -> None:
    print("ERROR: Disallowed eager imports in aphrodite/__init__.py:\n", file=sys.stderr)
    ...
    class Visitor(ast.NodeVisitor):
        ...
        def visit_If(self, node: ast.If) -> None:
            ...
            if isinstance(test, ast.Attribute) and isinstance(test.value, ast.Name):
                guard_is_type_checking = test.value.id == "typing" and test.attr == "TYPE_CHECKING"
        ...
        def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
            ...
            if _is_internal(node.module, level=node.level) and module_as_written not in ALLOWED_FROM_MODULES:
                violations.append(
                    (
                        node.lineno,
