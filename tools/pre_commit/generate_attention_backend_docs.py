...
RELEVANT_PATTERNS = [
    "aphrodite/v1/attention/backends/*.py",
    "aphrodite/v1/attention/backends/**/*.py",
    "aphrodite/v1/attention/backends/fa_utils.py",
    "aphrodite/model_executor/layers/attention/mla_attention.py",
    "aphrodite/platforms/cuda.py",
    "tools/pre_commit/generate_attention_backend_docs.py",
    "docs/design/attention_backends.md",
]

BACKENDS_DIR = REPO_ROOT / "aphrodite" / "v1" / "attention" / "backends"
REGISTRY_FILE = BACKENDS_DIR / "registry.py"
CUDA_PLATFORM_FILE = REPO_ROOT / "aphrodite" / "platforms" / "cuda.py"
...
def _parse_return_list(method: ast.FunctionDef | None, handle_multiple_of: bool = False) -> list[str]:
...
def _resolve_import_to_file(tree: ast.AST, class_name: str, source_file: Path | None = None) -> Path | None:
    """Try to resolve a class name to its source file via imports in the AST.

    Handles both absolute imports (from aphrodite.foo import Bar) and relative
    ...
    if method:
        for n in ast.walk(method):
            if (
                ...
                dtypes = [
                    e.value for e in n.comparators[0].elts if isinstance(e, ast.Constant) and isinstance(e.value, str)
                ]
                if dtypes:
                    return ", ".join(dtypes)
...
def parse_compute_capability(node: ast.ClassDef) -> str:
    ...
    for n in ast.walk(method):
        ...
        if isinstance(n.left, ast.Attribute) and n.left.attr == "major" and len(n.ops) == 1 and len(n.comparators) == 1:
            ...
            if isinstance(n.ops[0], ast.Eq) and isinstance(comp, ast.Constant):
                ...
                major_list.extend(
                    e.value for e in comp.elts if isinstance(e, ast.Constant) and isinstance(e.value, int)
                )
...
def parse_impl_bool_attr(
    ...
    if parent_name:
        ...
        if parent_node is not None:
            return parse_impl_bool_attr(tree, parent_name, attr_name, default, source_file, _visited)
...
def analyze_backend(backend_name: str, class_path: str) -> dict[str, Any] | None:
    ...
    is_mla_backend = parent in mla_parents or ".mla." in class_path.lower() or "_mla" in backend_name.lower()
    ...
    supports_dcp = False
    if impl_class_name:
        supports_dcp = parse_impl_bool_attr(tree, impl_class_name, "can_return_lse_for_decode", False, file_path)
    ...
    if backend_name in BACKEND_KV_DTYPE_EXCLUDES:
        ...
        kv_cache_dtypes = ", ".join(d for d in (d.strip() for d in kv_cache_dtypes.split(",")) if d not in excluded)
...
def _parse_fa4_supported_caps() -> str | None:
    ...
    """
    fa_interface_file = REPO_ROOT / "aphrodite" / "vllm_flash_attn" / "flash_attn_interface.py"
    ...
    for node in ast.walk(tree):
        ...
        for n in ast.walk(node):
            if not (
                ...
                continue
            caps: list[int] = [
                e.value for e in n.comparators[0].elts if isinstance(e, ast.Constant) and isinstance(e.value, int)
            ]
            if caps:
                caps.sort()
        ...
        if node.name == "flash_attn_supports_sinks":
            for n in ast.walk(node):
                if (
                    ...
                    is_version_compare = (isinstance(n.left, ast.Name) and n.left.id == "fa_version") or (
...
def parse_cuda_priority_lists() -> dict[str, list[str]]:
    ...
    for node in ast.walk(tree):
        ...
        for stmt in node.body:
            ...
            is_mla_branch = isinstance(stmt.test, ast.Name) and stmt.test.id == "use_mla"
        ...
        if isinstance(stmt, ast.Assign) and isinstance(stmt.value, ast.List):
            for target in stmt.targets:
                if isinstance(target, ast.Name):
                    var_assigns[target.id] = [e.attr for e in stmt.value.elts if isinstance(e, ast.Attribute)]
        elif isinstance(stmt, ast.If):
            for branch in (stmt.body, stmt.orelse):
                for branch_stmt in branch:
                    if isinstance(branch_stmt, ast.Assign) and isinstance(branch_stmt.value, ast.List):
                        for target in branch_stmt.targets:
                            if isinstance(target, ast.Name):
                                var_assigns[target.id] = [
                                    e.attr for e in branch_stmt.value.elts if isinstance(e, ast.Attribute)
                                ]
    ...
    for stmt in stmts:
        if isinstance(stmt, ast.Return) and isinstance(stmt.value, ast.List):
            ...
            for e in stmt.value.elts:
                ...
                elif isinstance(e, ast.Starred) and isinstance(e.value, ast.Name) and e.value.id in var_assigns:
...
def generate_markdown_table(backends: list[dict[str, Any]], title: str, is_mla_table: bool = False) -> str:
...
```bash
aphrodite serve <model> --attention-backend FLASH_ATTN
```
...
```bash
# Dot notation
aphrodite serve <model> --attention-config.backend FLASH_ATTN
aphrodite serve <model> -ac.backend FLASH_ATTN
...
from aphrodite.config import AttentionConfig
...
1. Aphrodite iterates through backends in **priority order** (see tables below)
...
def generate_priority_section(priorities: dict[str, list[str]]) -> str:
    ...
    lines = [
        ...
        "",
        "When no backend is explicitly selected, Aphrodite chooses the first",
    ...
    if "mla_sm100" in priorities:
        lines.extend(_priority_table(sm100, priorities["mla_sm100"], mla_sm100_annotations))
...
def generate_mla_section(prefill_backends: list[dict[str, Any]], decode_backends: list[dict[str, Any]]) -> str:
...
def generate_docs() -> str:
    ...
    doc_lines.append(generate_markdown_table(non_mla_backends, standard_title, is_mla_table=False))
...
def main():
    parser = argparse.ArgumentParser(description="Generate attention backend documentation table")
    parser.add_argument(
        "--output",
        "-o",
        type=str,
    ...
    if args.check:
        needs_update = not output_path.exists() or output_path.read_text() != new_content
        if needs_update:
