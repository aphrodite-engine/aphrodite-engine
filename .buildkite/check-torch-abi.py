...
"""Audit Aphrodite compiled libraries for PyTorch stable ABI compliance."""
...
def _collect_unstable(report: PackageReport) -> list[str]:
    return sorted(_relative_path(lib, report.root) for lib in _iter_libs(report) if _is_torch_unstable(lib))


def _find_stale_allowlist_entries(report: PackageReport, patterns: tuple[str, ...]) -> list[str]:
...
def check_torch_abi(
    ...
    unexpected = [rel_path for rel_path in unstable if not _matches_allowlist(rel_path, patterns)]
    stale = _find_stale_allowlist_entries(report, patterns)

    if unexpected or stale:
        if unexpected:
            print(
                f"Not allowed: torch-unstable libraries outside ALLOWED_UNSTABLE_LIBRARIES: {', '.join(unexpected)}",
                file=sys.stderr,
            )
        if stale:
            print(
                f"Not allowed: stale ALLOWED_UNSTABLE_LIBRARIES entries: {', '.join(stale)}",
                file=sys.stderr,
            )
        return 1
...
if __name__ == "__main__":
    print(">>> Auditing Aphrodite extension modules for PyTorch stable ABI compliance")
