...
interface ConfigData {
  ...
  aphrodite_endpoints?: AphroditeEndpoint[]
  classifier?: {
    ...
    <div className={styles.section}>
      <div className={styles.sectionHeader}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <span className={styles.badge}>{config?.aphrodite_endpoints?.length || 0} Aphrodite endpoints</span>
          <button
            ...
            onClick={() => {
              openEditModal(
                ...
                [
                  { name: 'name', label: 'Endpoint Name', type: 'text', required: true, placeholder: 'e.g., aphrodite-endpoint-1' },
                ...
                async (data) => {
                  ...
                  if (!newConfig.aphrodite_endpoints) newConfig.aphrodite_endpoints = []
                  newConfig.aphrodite_endpoints.push(data)
      ...
      <div className={styles.sectionContent}>
        {config?.aphrodite_endpoints && config.aphrodite_endpoints.length > 0 ? (
          config.aphrodite_endpoints.map((endpoint, index) => (
                  ...
                  <button
                    ...
                    onClick={() => {
                      openEditModal(
                        ...
                        async (data) => {
                          const newConfig = { ...config }
                          if (newConfig.aphrodite_endpoints) {
                            newConfig.aphrodite_endpoints[index] = data
                          }
                          await saveConfig(newConfig)
                        },
                        'edit'
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (confirm(`Are you sure you want to delete endpoint "${endpoint.name}"?`)) {
                        const newConfig = { ...config }
                        if (newConfig.aphrodite_endpoints) {
                          newConfig.aphrodite_endpoints.splice(index, 1)
                        }
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <button
            ...
            onClick={() => {
              ...
              const endpoints = config?.aphrodite_endpoints
                ? config.aphrodite_endpoints.map(ep => ep.name)
                : []
              ...
              openEditModal(
                ...
                [
                  ...
                  },
                  {
                    ...
                    type: 'json',
                    placeholder: '{"service_name": "aphrodite-router", "service_version": "1.0.0", "deployment_environment": "production"}',
