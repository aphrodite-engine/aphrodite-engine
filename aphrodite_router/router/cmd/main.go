...
import (
	...
	"github.com/aphrodite-engine/aphrodite-engine/aphrodite_router/router/pkg/apiserver"
	"github.com/aphrodite-engine/aphrodite-engine/aphrodite_router/router/pkg/config"
	"github.com/aphrodite-engine/aphrodite-engine/aphrodite_router/router/pkg/extproc"
...
func main() {
	...
	logging.Infof("Starting Aphrodite Router ExtProc with config: %s", *configPath)
	...
	if err != nil {
		...
		if err := candle_binding.InitEmbeddingModels(
			...
			logging.Warnf("Failed to initialize embedding models: %v", err)
