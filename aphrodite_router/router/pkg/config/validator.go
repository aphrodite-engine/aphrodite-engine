...
// validateAPHRODITEEndpoints validates the address format of all Aphrodite endpoints
func validateAPHRODITEEndpoints(endpoints []APHRODITEEndpoint) error {
	for _, endpoint := range endpoints {
		if err := validateIPAddress(endpoint.Address); err != nil {
			return fmt.Errorf("Aphrodite endpoint '%s' address validation failed: %w\n\nSupported formats:\n- IPv4: 192.168.1.1, 127.0.0.1\n- IPv6: ::1, 2001:db8::1\n\nUnsupported formats:\n- Domain names: example.com, localhost\n- Protocol prefixes: http://, https://\n- Paths: /api/v1, /health\n- Ports in address: use 'port' field instead", endpoint.Name, err)
		}
	}
	return nil
}
...
func validateConfigStructure(cfg *RouterConfig) error {
	...
	// Validate Aphrodite endpoints address formats
	if err := validateAPHRODITEEndpoints(cfg.APHRODITEEndpoints); err != nil {
		return err
	}
