...
`
			...
			It("should load configuration successfully", func() {
				...
				// Verify Aphrodite endpoints config
				Expect(cfg.APHRODITEEndpoints).To(HaveLen(2))
				Expect(cfg.APHRODITEEndpoints[0].Name).To(Equal("endpoint1"))
				Expect(cfg.APHRODITEEndpoints[0].Address).To(Equal("127.0.0.1"))
				Expect(cfg.APHRODITEEndpoints[0].Port).To(Equal(8000))
				Expect(cfg.APHRODITEEndpoints[0].Weight).To(Equal(1))

				Expect(cfg.APHRODITEEndpoints[1].Name).To(Equal("endpoint2"))
				Expect(cfg.APHRODITEEndpoints[1].Address).To(Equal("127.0.0.1"))
				Expect(cfg.APHRODITEEndpoints[1].Weight).To(Equal(2))
...
`
	...
	Describe("Aphrodite Endpoints Functions", func() {
		BeforeEach(func() {
			configContent := `
aphrodite_endpoints:
  - name: "endpoint1"
...
`
		...
		Describe("Aphrodite Endpoint Address Validation", func() {
...
`
					...
					cfg, err := Load(configFile)
					Expect(err).NotTo(HaveOccurred())
					Expect(cfg.APHRODITEEndpoints[0].Address).To(Equal("::1"))
				})
			})
	...
	Describe("validateAPHRODITEEndpoints", func() {
		Context("with valid endpoints", func() {
			It("should accept endpoints with valid IP addresses", func() {
				...
				err := validateAPHRODITEEndpoints(endpoints)
				Expect(err).NotTo(HaveOccurred())
			})
		})
