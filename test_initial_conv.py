...
print(
    f"  min={diff_conv_out.min().item():.4f}, max={diff_conv_out.max().item():.4f}, std={diff_conv_out.std().item():.4f}"
)

print("\nOur input_blocks.0 output:")
print(f"  Shape: {our_conv_out.shape}")
print(
    f"  min={our_conv_out.min().item():.4f}, max={our_conv_out.max().item():.4f}, std={our_conv_out.std().item():.4f}"
)
...
print(
    f"  cosine_sim={torch.nn.functional.cosine_similarity(diff_conv_out.flatten(), our_conv_out.flatten(), dim=0).item():.6f}"
)
