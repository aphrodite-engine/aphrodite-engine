...
mod tests {
    ...
    use aphrodite_text::Prompt;
    use aphrodite_text::backend::hf::{HfSpecialTokens, NamedSpecialToken};
    ...
    fn continue_final_message_appends_to_last_text_part() {
        ...
        let rendered = render_mm(
            "{% for item in messages[0].content %}\
             {% if item.type == 'image' %}<image>{% else %}{{ item.text }}{% endif %}\
             {% endfor %}<|im_end|>",
            &request,
            ChatTemplateContentFormatOption::OpenAi,
        )
    ...
    fn continue_final_message_composes_with_aware_templates() {
        ...
        let aware = "{% for m in messages %}<|im_start|>{{ m.role }}\n{{ m.content }}\
                     {% if not (loop.last and continue_final_message) %}<|im_end|>\n\
                     {% endif %}{% endfor %}";
