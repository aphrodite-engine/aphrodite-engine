...
fn server_dev_mode_enabled() -> bool {
    std::env::var("APHRODITE_SERVER_DEV_MODE")
        .ok()
...
fn runtime_lora_updating_enabled() -> bool {
    std::env::var("APHRODITE_ALLOW_RUNTIME_LORA_UPDATING")
        .ok()
...
fn parse_scale_out_endpoints_flag(value: Option<&str>) -> Result<bool, String> {
    ...
    match value.trim() {
        "0" => Ok(false),
        "1" => Ok(true),
        _ => Err("APHRODITE_ENABLE_SCALE_OUT_ENDPOINTS must be 0 or 1".to_string()),
    }
}

fn scale_out_endpoints_enabled() -> bool {
    let value = std::env::var("APHRODITE_ENABLE_SCALE_OUT_ENDPOINTS").ok();
    ...
    if scale_out_endpoints_enabled {
        ...
        info!(
            "scale-out endpoints are disabled; set \
             APHRODITE_ENABLE_SCALE_OUT_ENDPOINTS=1 to enable them"
        );
    }
