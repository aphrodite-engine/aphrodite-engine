...
use uuid::Uuid;
use aphrodite_llm::current_unix_timestamp_secs;
use aphrodite_text::{DecodedTextEvent, Prompt, SampledDelta, TextOutputStreamExt as _, TextRequest};
...
fn apply_encoder_cache_placeholders(text_request: &mut TextRequest) {
    ...
    if is_decode_kv_consumer && let Some(args) = text_request.sampling_params.aphrodite_xargs.as_mut() {
...
impl pb::inference_server::Inference for InferenceServiceImpl {
    ...
    async fn generate(
        ...
        let finish_info = aphrodite_text::Finished {
...
fn text_error_to_status(error: aphrodite_text::Error) -> Status {
