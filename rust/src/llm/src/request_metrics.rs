...
impl RequestMetricsTracker {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            log_request_level: request_level_metrics_enabled(),
        }
    }
    ...
    pub(crate) fn record_finished(&self, received_at: f64, finish_reason: FinishReason) {
        ...
        // Aphrodite-specific: emit a per-request completion log line. The
        // aggregate periodic log in `log_stats` is suppressed while this is on.
    ...
    /// Log one line per finished request, mirroring Python's
    /// `LoggingStatLogger._log_finished_requests` output format.
    fn log_finished_request(
        ...
        info!(
            "Request completed - E2E time: {e2e_latency_seconds:.2}s, \
             ...
             Decode: {} tokens ({decode_throughput:.1} tokens/s){cache_hit_info}",
...
mod tests {
    ...
    use super::{diff_or_zero, RequestMetricsTracker};
