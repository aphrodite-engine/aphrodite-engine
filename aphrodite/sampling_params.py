...
from aphrodite.config import ModelConfig, SpeculativeConfig, StructuredOutputsConfig
...
from aphrodite.v1.serial_utils import PydanticMsgspecMixin
...
def _verify_num_sequences(value: int, parameter_name: str) -> None:
    if not isinstance(value, int):
        raise APHRODITEValidationError(f"{parameter_name} must be an int, but is of type {type(value)}")
    if value < 1:
        raise APHRODITEValidationError(f"{parameter_name} must be at least 1, got {value}.")
...
class SamplerID(IntEnum):
    # Mirror these in the sampler execution order.
    # Values stay aligned with legacy Aphrodite/Kobold ordering.
    DRY = 7
    PENALTIES = 6
    NO_REPEAT_NGRAM = 8
    TEMPERATURE = 5
...
class SamplingParams(
    ...
    The maximum allowed value is controlled by the ``APHRODITE_MAX_N_SEQUENCES``
    ...
    """Probability of applying XTC removal."""
    nsigma: float = 0.0
    """Top-nsigma cutoff."""
    mirostat_mode: int = 0
    """Mirostat mode. Aphrodite currently supports 0 and 2."""
    ...
    """Number of repeated tokens allowed before DRY penalties kick in."""
    dry_sequence_breakers: list[str] | None = None
    """String-form DRY breakers resolved to token ids once a tokenizer exists."""
    dry_sequence_breaker_ids: list[int] = msgspec.field(default_factory=list)
    """Token ids that split DRY repetition windows."""
    dry_range: int = 0
    """Token window used by DRY."""
    dry_max_ngram: int = 12
    """Maximum n-gram length checked by DRY."""
    dry_max_occurrences: int = 8
    """Maximum occurrences DRY scans."""
    ...
    def from_optional(
        ...
        if logit_bias is not None:
            ...
            try:
                ...
                if invalid_keys:
                    raise APHRODITEValidationError(
                        f"logit_bias contains key(s) that cannot be converted to integer token IDs: {invalid_keys!r}",
    ...
    def _verify_args(self) -> None:
        ...
        if not math.isfinite(self.repetition_penalty):
            raise ValueError(f"repetition_penalty must be a finite number, got {self.repetition_penalty}.")
        ...
        if self.dynatemp_exponent <= 0.0:
            raise ValueError("dynatemp_exponent must be positive.")
        if self.top_a < 0.0:
        ...
        if self.eta_cutoff < 0.0:
            raise ValueError("eta_cutoff must be non-negative.")
        if self.epsilon_cutoff < 0.0:
            raise ValueError("epsilon_cutoff must be non-negative.")
        ...
        if self.nsigma < 0.0:
            raise ValueError("nsigma must be non-negative.")
        if self.mirostat_mode not in (0, 2):
            raise ValueError("mirostat_mode must be 0 or 2.")
        if self.mirostat_tau < 0.0 or self.mirostat_eta < 0.0:
            raise ValueError("mirostat parameters must be non-negative.")
        if self.dry_multiplier < 0.0:
            raise ValueError("dry_multiplier must be non-negative.")
        if self.dry_base <= 1.0:
            raise ValueError("dry_base must be greater than 1.")
        if self.dry_allowed_length < 0:
            raise ValueError("dry_allowed_length must be non-negative.")
        if self.dry_range < 0:
            raise ValueError("dry_range must be non-negative.")
        if self.dry_max_ngram < 0 or self.dry_max_occurrences < 0:
    ...
    def update_from_tokenizer(self, tokenizer: TokenizerLike) -> None:
        ...
        max_num_bad_words = envs.APHRODITE_MAX_NUM_BAD_WORDS
    ...
    def verify(
        ...
        self._validate_structured_outputs(model_config, structured_outputs_config, tokenizer)
    ...
    def _validate_logprobs(self, model_config: ModelConfig) -> None:
        ...
        if self.logprob_token_ids is not None:
            ...
            invalid_token_ids = [
                token_id for token_id in self.logprob_token_ids if token_id < 0 or token_id >= vocab_size
            ]
    ...
    def _validate_trace_replay(
        ...
        if not all(isinstance(t, int) and t >= 0 for t in self.trace_decode_token_ids):
            raise ValueError("trace_decode_token_ids must contain non-negative integers.")

        if self.prompt_logprobs is not None:
            raise ValueError("trace_decode_token_ids is not supported with prompt_logprobs.")
        if speculative_config is not None:
            raise ValueError("trace_decode_token_ids is not supported with speculative decoding.")
        if self.structured_outputs is not None:
            raise ValueError("trace_decode_token_ids is not supported with structured outputs.")
        if self.repetition_detection is not None:
            raise ValueError("trace_decode_token_ids is not supported with repetition_detection.")
        if self.thinking_token_budget is not None:
            raise ValueError("trace_decode_token_ids is not supported with thinking_token_budget.")
        if self.bad_words:
            raise ValueError("trace_decode_token_ids is not supported with bad_words.")

        vocab_size = model_config.get_vocab_size()
        invalid_token_ids = [
            token_id for token_id in self.trace_decode_token_ids if token_id < 0 or token_id >= vocab_size
        ]
        if invalid_token_ids:
            raise APHRODITEValidationError(
    ...
    def _validate_structured_outputs(
        ...
        if isinstance(self.structured_outputs.json, str) and self.structured_outputs.json.strip() == "":
        ...
        if self.structured_outputs.regex and "\x00" in self.structured_outputs.regex:
            raise APHRODITEValidationError("structured_outputs.regex must not contain a NUL character ('\\x00')")
        ...
        if backend.startswith("xgrammar"):
            ...
            except APHRODITEValidationError:
