...
"""FlashInfer ``moe_ep`` helpers for DeepSeek V4 Aphrodite integration."""
...
from aphrodite.config.kernel import FLASHINFER_MOE_EP_BACKENDS
...
def fi_moe_ep_backend_spec(moe_backend: str) -> FiMoeEpBackendSpec:
    try:
        ...
        raise ValueError(
            f"{moe_backend!r} is not a flashinfer moe_ep backend; expected one of {sorted(FI_MOE_EP_BACKEND_SPECS)}"
        ) from None


def validate_fi_moe_ep_config(aphrodite_config: AphroditeConfig) -> None:
    ...
    if capability is not None:
        ...
        if cc not in FI_MOE_EP_SUPPORTED_CAPABILITIES:
            supported = ", ".join(f"{m}.{n}" for m, n in sorted(FI_MOE_EP_SUPPORTED_CAPABILITIES))
            raise ValueError(
    ...
    if aphrodite_config.parallel_config.enable_eplb:
...
def make_fi_moe_ep_bootstrap() -> BootstrapConfig:
    ...
    return BootstrapConfig(
        ...
        # Aphrodite already bound this worker's (possibly remapped) device;
...
def ensure_fi_moe_ep_runtime(aphrodite_config: AphroditeConfig) -> None:
    ...
    spec = fi_moe_ep_backend_spec(aphrodite_config.kernel_config.moe_backend)
...
def _dequant_fp4_ue8m0_gran32(packed: torch.Tensor, sf_ue8m0: torch.Tensor) -> torch.Tensor:
    ...
    vals = torch.empty(raw.shape[0], raw.shape[1] * 2, dtype=torch.float32, device=raw.device)
...
def _dequant_expert_weights_to_bf16(weight: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
    """[E, N, K//2] fp4 + [E, N, K//32] ue8m0 -> [E, N, K] bf16 (expert loop)."""
    num_experts, n, k_half = weight.shape
    out = torch.empty(num_experts, n, k_half * 2, dtype=torch.bfloat16, device=weight.device)
...
def build_fi_mega_layer(
    ...
    *,
    ...
    megakernel = fi_moe_ep_backend_spec(aphrodite_config.kernel_config.moe_backend).megakernel
    ...
    if not isinstance(layer, MoEEpMegaLayer):
        raise TypeError(f"fi_moe_ep expected MoEEpMegaLayer, got {type(layer).__name__}")
    return layer
