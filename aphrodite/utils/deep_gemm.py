...
def is_deep_gemm_e8m0_used() -> bool:
    ...
    if current_platform.is_device_capability(100) and \
        ...
        log_once("INFO", "DeepGEMM E8M0 enabled on Blackwell GPU.")
        return True

    if current_platform.is_device_capability(90) and \
            envs.APHRODITE_USE_DEEP_GEMM_E8M0_HOPPER:
        log_once("INFO", "DeepGEMM E8M0 enabled on Hopper GPU.")
        return True

    log_once("INFO", "DeepGEMM E8M0 disabled on current configuration.")
    return False
...
def _resolve_symbol(module, new: str, old: str) -> Callable[..., Any] | None:
    ...
    if hasattr(module, old):
        ...
        log_once(
            "WARNING",
            "Found legacy DeepGEMM symbol `{}`. Please upgrade the "
            "`deep_gemm` package so that `{}` is available. Support for the "
            "legacy symbol will be removed in a future Aphrodite release.",
            old,
            new,
        )
