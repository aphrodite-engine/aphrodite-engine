...
class DeepGemmQuantScaleFMT(Enum):
    ...
    def init_oracle_cache(cls) -> None:
        ...
        cls._oracle_cache = (  # type: ignore
            cls.UE8M0 if current_platform.is_device_capability_family(100) else cls.FLOAT32_CEIL_UE8M0
        )
...
def is_deep_gemm_e8m0_used() -> bool:
    """Return `True` if Aphrodite is configured to use DeepGEMM "
...
def _import_deep_gemm():
    ...
    try:
        ...
        logger.debug_once("deep_gemm not found in site-packages, trying vendored aphrodite.third_party.deep_gemm")

    # 2. Fall back to the vendored copy bundled in the Aphrodite wheel.
    try:
        module = importlib.import_module("aphrodite.third_party.deep_gemm")
        logger.debug_once("Imported deep_gemm module from aphrodite.third_party.deep_gemm")
        return module
    except ImportError:
...
def fp8_fp4_mqa_logits(
    ...
    """Compute FP8/FP4 MQA logits for a single sequence without KV paging."""
    _lazy_init()
