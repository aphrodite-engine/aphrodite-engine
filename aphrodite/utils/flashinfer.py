...
def supports_trtllm_attention() -> tuple[bool, Optional[str]]:
    ...
    if env_value is not None:
        log_once("INFO", "APHRODITE_USE_TRTLLM_ATTENTION is set to {}",
                 env_value)
        ...
        if use_trtllm:
            log_once("INFO", "Using TRTLLM attention.")
...
def use_trtllm_attention(
    ...
    if q_dtype == current_platform.fp8_dtype():
        log_once("INFO", "Using TRTLLM attention (query is quantized).")
        return True
...
if has_flashinfer():
    ...
    @torch.library.register_fake("aphrodite::flashinfer_mm_fp4", )
    ...
    @torch.library.register_fake("aphrodite::bmm_fp8", )
    def bmm_fp8_fake(
        A: torch.Tensor,
        B: torch.Tensor,
        A_scale: torch.Tensor,
