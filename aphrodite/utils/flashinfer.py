...
def _force_use_trtllm_attention(env_value: bool | None) -> bool | None:
    """Cache the env value for APHRODITE_USE_TRTLLM_ATTENTION"""
...
def force_use_trtllm_attention() -> bool | None:
    """
    Return `None` if APHRODITE_USE_TRTLLM_ATTENTION is not set,
    return `True` if TRTLLM attention is forced to be used,
    return `False` if TRTLLM attention is forced to be not used.
    """
    return _force_use_trtllm_attention(envs.APHRODITE_USE_TRTLLM_ATTENTION)
...
def use_trtllm_attention(
    ...
    if num_qo_heads % num_kv_heads != 0:
        if force_use_trtllm:
            logger.warning_once(
                ...
                "query and key heads, but APHRODITE_USE_TRTLLM_ATTENTION is set to 1"
            )
        return False
    ...
    logger.info_once("Using TRTLLM attention (APHRODITE_USE_TRTLLM_ATTENTION is set to 1)")
    return True
...
def flashinfer_disable_q_quantization() -> bool:
    ...
    return envs.APHRODITE_FLASHINFER_DISABLE_Q_QUANTIZATION
