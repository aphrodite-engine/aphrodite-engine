...
def has_flashinfer_cubin() -> bool:
    ...
    if envs.APHRODITE_HAS_FLASHINFER_CUBIN:
        return True
...
flashinfer_prepare_bf16_fp4_weights = _lazy_import_wrapper("flashinfer.gemm", "prepare_bf16_fp4_weights")
...
def has_flashinfer_sm90_nope_mla() -> bool:
    ...
    try:
        ...
        return False
    return "ckv_scale_arr" in params and params["ckv_scale_arr"].kind is inspect.Parameter.KEYWORD_ONLY
...
def has_flashinfer_sparse_mla_sm120_config(num_q_heads: int, top_k: int) -> bool:
    ...
    valid Aphrodite configuration. Inspect FlashInfer's dispatch table until it
...
def has_flashinfer_bf16_fp4() -> bool:
    ...
    return mod is not None and all(hasattr(mod, name) for name in ("mm_bf16_fp4", "prepare_bf16_fp4_weights"))
...
def has_flashinfer_b12x_moe() -> bool:
    """Return ``True`` if FlashInfer B12x fused MoE is available."""
    if not has_flashinfer_moe():
        return False
...
def force_use_trtllm_attention() -> bool | None:
    """
    This function should only be called during initialization stage when aphrodite config
    is set.
...
def use_trtllm_attention(
    ...
    if dcp_world_size > 1:
        logger.warning_once("TRTLLM prefill does not support DCP, reverting to FlashInfer")
        return False
...
if has_flashinfer():
    ...
    @torch.library.custom_op(
        "aphrodite::flashinfer_mm_bf16_fp4",
        mutates_args=[],
        device_types="cuda",
    )
    ...
    @torch.library.custom_op(
        "aphrodite::flashinfer_mxfp4_quantize",
        mutates_args=[],
        device_types="cuda",
    )
    ...
    @torch.library.register_fake("aphrodite::flashinfer_mxfp4_quantize")
    ...
    @torch.library.custom_op(
        "aphrodite::flashinfer_mxfp8_quantize_8x4",
        mutates_args=[],
        device_types="cuda",
    )
