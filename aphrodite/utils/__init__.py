...
class AsyncMicrobatchTokenizer:
    ...
    Pulls pending encode/decode requests from a queue and batches
    them up to reduce overhead. A single-thread ThreadPoolExecutor
    is used so the event loop stays responsive.
    """
...
def get_ip() -> str:
    host_ip = envs.APHRODITE_HOST_IP
    if "HOST_IP" in os.environ and "APHRODITE_HOST_IP" not in os.environ:
        logger.warning(
            "The environment variable HOST_IP is deprecated and ignored, as"
            " it is often used by Docker and other software to"
            " interact with the container's network stack. Please "
            "use APHRODITE_HOST_IP instead to set the IP address for "
            "Aphrodite processes to communicate with each other.")
    if host_ip:
        return host_ip
...
def get_loopback_ip() -> str:
    loopback_ip = envs.APHRODITE_LOOPBACK_IP
    if loopback_ip:
        return loopback_ip

    # APHRODITE_LOOPBACK_IP is not set, try to get it based on network
    # interface
    ...
    if test_loopback_bind("127.0.0.1", socket.AF_INET):
        ...
        raise RuntimeError(
            ...
            "Set the APHRODITE_LOOPBACK_IP environment variable explicitly.")
...
def get_open_port() -> int:
    """
    Get an open port for the Aphrodite process to listen on.
    ...
    """
    if "APHRODITE_DP_MASTER_PORT" in os.environ:
        dp_master_port = envs.APHRODITE_DP_MASTER_PORT
...
def find_process_using_port(port: int) -> Optional[psutil.Process]:
    ...
    # of Aphrodite. For now, let's not look for processes in this case.
...
def enable_trace_function_call_for_thread(
    ...
    if envs.APHRODITE_TRACE_FUNCTION:
        ...
        log_path = os.path.join(tmp_dir, "aphrodite",
                                f"aphrodite-instance-{aphrodite_config.instance_id}",  # noqa: E501
                                filename)
...
class _PlaceholderBase:
    ...
    [`__getattr__`][aphrodite.utils._PlaceholderBase.__getattr__]
    ...
    def __getattr__(self, key: str):
        ...
        try:
            ...
            for extra, names in get_aphrodite_optional_dependencies().items():
                if name in names:
                    msg = (f"Please install aphrodite-engine[{extra}] for "
                           f"{extra} support")
...
def direct_register_custom_op(
    ...
    if not supports_custom_op():
        ...
        assert not current_platform.is_cuda_alike(), (
            ...
            "use Aphrodite in a fresh new environment and let it install "
...
def memory_profiling(
    ...
    baseline_snapshot: the memory snapshot before the current Aphrodite instance.
    ...
    1. memory used by anything other than the current Aphrodite instance.
    2. memory used by torch in the current Aphrodite instance.
    3. memory used in the current Aphrodite instance, but not by torch.

    A quantitive example:

    Before creating the current Aphrodite instance:
        category 1: 1 GiB
        category 2: 0 GiB
        category 3: 0 GiB

    After creating the current Aphrodite instance and loading the model,
    ...
    The increase of `non_torch_memory` from creating the current Aphrodite instance until after profiling to get (c.).
    """  # noqa
    gc.collect()
...
def import_pynvml():
    ...
    - `nvidia-ml-py` (https://pypi.org/project/nvidia-ml-py/): The official
        wrapper. It is a dependency of Aphrodite, and is installed when users
        install Aphrodite. It provides a Python module named `pynvml`.
    ...
    https://github.com/aphrodite-project/aphrodite/issues/12847 for example.
    After all the troubles, we decide to copy the official `pynvml`
    module to our codebase, and use it directly.
    """
    import aphrodite.third_party.pynvml as pynvml
    return pynvml
...
def set_process_title(name: str,
    ...
    if append:
        ...
        name = f"{envs.APHRODITE_PROCESS_NAME_PREFIX}::{name}"
