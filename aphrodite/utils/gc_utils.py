    ...
    def __init__(self, gc_debug_conf: str | None = None) -> None:
        ...
        if not gc_debug_conf or gc_debug_conf == "0":
            ...
            try:
                ...
                self.enabled = False
                logger.error("Failed to parse APHRODITE_GC_DEBUG(%s)", envs.APHRODITE_GC_DEBUG)
...
class GCDebugger:
    ...
    def handle(self, phase: str, info: dict[str, int]) -> None:
        ...
        if phase == "start":
            ...
            self.gc_top_collected_objects = _compute_top_gc_collected_objects(objects, self.config.top_objects)
        elif phase == "stop":
            ...
            logger.info(
                "GC took %.3fms to complete. Collected %s objects (out of %d) in GC generation %d.%s",
                elpased_ms,
...
def maybe_attach_gc_debug_callback() -> None:
    """
    Attached a callback for GC debug when APHRODITE_GC_DEBUG is enabled.
    """
    config = GCDebugConfig(envs.APHRODITE_GC_DEBUG)
    if config.enabled:
