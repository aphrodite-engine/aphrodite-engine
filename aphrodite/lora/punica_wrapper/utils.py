...
def convert_mapping(
    ...
    Returns:
        A tuple of tensors:
            ...
            sampler_indices_padded: Tensor of shape [batch_size] mapping
                ...
                max_loras.
            embeddings_indices: Tensor of shape [batch_size] mapping
                requests to the row offset of their LoRA.lora_a embeddings.
    ...
    prompt_mapping: list[int] = [lora_id_to_index[x] if x > 0 else -1 for x in mapping.prompt_mapping]
    lora_idx = None
    for i in range(len(index_mapping_indices)):
        lora_idx = lora_id_to_index[index_mapping_indices[i]] if index_mapping_indices[i] > 0 else -1
    ...
    prompt_mapping_tensor = async_tensor_h2d(prompt_mapping, dtype=torch.long, device=device)
    embeddings_indices = indices[2] * vocab_size
