...
from aphrodite.lora.lora_model import LoRAModel, MoEEPLoadSpec
from aphrodite.lora.lora_weights import LoRALayerWeights, PackedLoRALayerWeights
from aphrodite.lora.punica_wrapper import PunicaWrapperBase, get_punica_wrapper
...
class LoRAModelManager:
    ...
    def _init_punica_wrapper(self, max_num_batched_tokens: int, aphrodite_config: AphroditeConfig) -> None:
        ...
        if self.supports_mm:
            self._maybe_init_mm(aphrodite_config, max_num_batched_tokens)
        else:
    ...
    def _maybe_init_mm(
        ...
        self.supports_tower_connector_lora = self.supports_mm and self.model.supports_tower_connector_lora
        ...
        if (
            aphrodite_config.model_config.multimodal_config
            and aphrodite_config.model_config.multimodal_config.language_model_only
        ):
            logger.warning(
        ...
        num_encoder_tokens = max(tower_tokens for tower_tokens, _ in lora_token_counts_by_modality)
    ...
    def _match_target_modules(self, module_name: str, module: nn.Module) -> bool:
        ...
        This method first checks if the module is in Aphrodite's supported LoRA
