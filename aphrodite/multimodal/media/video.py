...
class VideoMediaIO(MediaIO[MediaWithBytes[tuple[npt.NDArray, dict[str, Any]]]]):
            ...
            for key in ("video_backend", "backend"):
                ...
                if requested and VIDEO_LOADER_REGISTRY.backend_requires_gpu(requested):
                    ...
                    if static_val != requested:
                        ...
                        runtime_kwargs = {k: v for k, v in runtime_kwargs.items() if k != key}
    ...
    def load_bytes(self, data: bytes) -> MediaWithBytes[tuple[npt.NDArray, dict[str, Any]]]:
        video = self.video_loader.load_bytes(data, num_frames=self.num_frames, **self.kwargs)
        return MediaWithBytes(video, data)

    def load_base64(self, media_type: str, data: str) -> MediaWithBytes[tuple[npt.NDArray, dict[str, Any]]]:
    ...
    def load_file(self, filepath: Path) -> MediaWithBytes[tuple[npt.NDArray, dict[str, Any]]]:
