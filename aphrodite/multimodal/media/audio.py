...
from aphrodite.multimodal.audio import resample_audio_pyav
...
_TORCHCODEC_UNAVAILABLE_MSG = (
    "torchcodec audio backend is unavailable (requires the torchcodec package and a system ffmpeg installation)"
)
    ...
    with container:
        ...
        if max_duration_s is not None:
            ...
            if metadata_duration_s is not None and metadata_duration_s > max_duration_s:
                raise ValueError(
                    ...
                    f"APHRODITE_MAX_AUDIO_DECODE_DURATION_S to "
        ...
        try:
            for frame in container.decode(stream):
                ...
                if max_decode_bytes is not None and total_decode_bytes > max_decode_bytes:
                    raise ValueError(
                        ...
                        f"APHRODITE_MAX_AUDIO_DECODE_BYTES to increase this "
                        f"limit."
                    )
        except av.error.FFmpegError as e:
            raise ValueError(
        ...
        if max_decode_bytes is not None:
            ...
            if estimated_bytes > max_decode_bytes:
                raise ValueError(
                    ...
                    f"APHRODITE_MAX_AUDIO_DECODE_BYTES to increase this limit."
                )
...
def load_audio_torchcodec(
    ...
    *,
    ...
    if max_decode_bytes is not None:
        ...
        if est_duration_s is not None and est_sample_rate is not None and est_num_channels is not None:
            estimated_bytes = int(est_duration_s * est_sample_rate) * est_num_channels * np.dtype(np.float32).itemsize
            if estimated_bytes > max_decode_bytes:
                raise ValueError(
    ...
    try:
        if max_duration_s is None:
            ...
            samples = decoder.get_samples_played_in_range(0.0, max_duration_s + _DURATION_GUARD_MARGIN_S)
    except RuntimeError as e:
        raise ValueError(
            "Invalid or corrupted audio data. Ensure the input is a valid "
            "audio or video file (e.g. a complete WAV, MP3, or MP4)."
    ...
    if max_decode_bytes is not None and samples.data.nbytes > max_decode_bytes:
        raise ValueError(
            ...
            f"Set APHRODITE_MAX_AUDIO_DECODE_BYTES to increase this limit."
        )
...
def load_audio(
    ...
    *,
    ...
    if backend not in AUDIO_BACKENDS:
        raise ValueError(f"Unknown audio backend {backend!r}. Available backends: {list(AUDIO_BACKENDS)}")
    ...
    try:
        ...
        logger.warning("torchcodec unavailable (%r); falling back to soundfile/PyAV.", exc)
    ...
    # "install aphrodite[audio]" ImportError.
    if isinstance(path, BytesIO):
        path.seek(0)
    try:
...
class AudioMediaIO(MediaIO[tuple[npt.NDArray, float]]):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        if backend is not None and backend not in AUDIO_BACKENDS:
            raise ValueError(f"Unknown audio_backend {backend!r}. Available backends: {list(AUDIO_BACKENDS)}")
    ...
    def get_max_bytes(self) -> int:
        return int(envs.APHRODITE_MAX_AUDIO_CLIP_FILESIZE_MB * MiB_bytes)
    ...
    def load_bytes(self, data: bytes) -> tuple[npt.NDArray, float]:
        ...
        return load_audio(
            ...
            max_decode_bytes=envs.APHRODITE_MAX_AUDIO_DECODE_BYTES,
