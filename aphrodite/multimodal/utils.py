...
from aphrodite.inputs import MultiModalPlaceholders
...
from .media import AudioMediaIO, ImageMediaIO, MediaConnector, MediaWithBytes, VideoMediaIO
...
def encode_video_url(
    ...
    *,
    ...
    if format.lower() == "jpeg":
        ...
        mimetype = mimetypes.types_map.get("." + format.lower(), f"video/{format.lower()}")
...
def _can_batch_mm_items(
    ...
    for key, left_elem in left.items():
        ...
        if (type(left_field) is not type(right_field)) or (
            is_shared_field and not nested_tensors_equal(left_elem.data, right_elem.data, check_dtype=True)
        ):
            return False
...
def strip_covered_mm_data(
    ...
    def maybe_strip(f: MultiModalFeatureSpec) -> MultiModalFeatureSpec:
        ...
        if uses_mrope or uses_xdrope:
            data = MultiModalKwargsItem({k: elem for k, elem in f.data.items() if elem.field.keep_on_cpu})
...
def group_and_batch_mm_items(
    ...
    *,
    ...
    for end_idx in range(1, len(items) + 1):
        if end_idx < len(items) and _can_batch_mm_items(items[end_idx - 1], items[end_idx]):
            continue
...
def copy_mm_embedding_modality(src: "torch.Tensor", dst: "torch.Tensor") -> "torch.Tensor":
