...
def reserve_mm_ipc_gpu_memory(
    ...
    * For GPU video backends, a fixed upper bound for each API process's
    ...
    uses_pynvvideocodec = (
        video_loader_backend == PYNVVIDEOCODEC_VIDEO_BACKEND or codec_backend == PYNVVIDEOCODEC_VIDEO_BACKEND
    )
    hw_decoders = (
        validate_pynvvideocodec_hw_decoders(video_kwargs.get("hw_decoders", PYNVVIDEOCODEC_DEFAULT_HW_DECODERS))
        if uses_pynvvideocodec
        else 1
    )
    per_server_decoder_bytes = PYNVVIDEOCODEC_DECODER_GPU_MEMORY_BYTES * hw_decoders + PYNVVIDEOCODEC_CUDA_CONTEXT_BYTES
    decoder_reserved_bytes = num_api_servers * per_server_decoder_bytes if mm_config.use_gpu_video_backend() else 0
