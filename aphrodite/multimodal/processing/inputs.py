...
class ProcessorInputs:
    ...
    [`aphrodite.multimodal.processing.BaseMultiModalProcessor.apply`][].
    """
    ...
    def get_mm_hashes(
        ...
        for modality, data_items in mm_data_items.items():
            ...
            if modality in _HF_MODALITY_PROCESSOR_KWARGS:
                ...
                media_io_kwargs = {modality: modality_media_io_kwargs} if modality_media_io_kwargs else {}
                scoped_key = _HF_MODALITY_PROCESSOR_KWARGS[modality]
                mm_processor_kwargs = {
                    key: value
                    for key, value in self.hf_processor_mm_kwargs.items()
                    if key not in _HF_MODALITY_PROCESSOR_KWARGS.values() or key == scoped_key
                }
            else:
            ...
            uuid_items = mm_uuid_items[modality] if modality in mm_uuid_items else ([None] * len(data_items))
