...
class PromptIndex:
    ...
    [`BaseMultiModalProcessor._apply_prompt_updates`][aphrodite.multimodal.processing.BaseMultiModalProcessor].
    """
...
[`modality`][aphrodite.multimodal.processing.PromptUpdate.modality],
...
class PromptUpdateDetails:
    ...
    """
    Given [`full`][aphrodite.multimodal.processing.PromptUpdateDetails.full],
    ...
    [`SupportsMultiModal.embed_multimodal`][aphrodite.model_executor.models.interfaces.SupportsMultiModal.embed_multimodal].
    """
...
use [`PromptUpdateDetails`][aphrodite.multimodal.processing.PromptUpdateDetails] to
specify which part.
"""
...
class PromptInsertion(PromptUpdate):
    ...
    [`target`][aphrodite.multimodal.processing.PromptUpdate.target].
...
def full_groupby_modality(values: Iterable[_M]) -> ItemsView[str, list[_M]]:
    ...
    [`full_groupby`][aphrodite.utils.collection_utils.full_groupby]
    based on modality.
    """
...
class ResolvedPromptUpdate:
    """
    A [`PromptUpdate`][aphrodite.multimodal.processing.PromptUpdate] with its
...
def _compile_prompt_update_queues(
    ...
    for modality_updates in mm_prompt_updates.values():
        for updates in modality_updates:
            signature = tuple((update.mode, _target_key(update.target)) for update in updates)
            queues_by_signature.setdefault(signature, deque()).append((priority, updates))
            priority += 1
...
def _plan_prompt_updates(
    ...
    return _plan_prompt_updates_with(
        mm_prompt_updates,
        lambda update, start_idx: update.iter_token_matches(prompt, start_idx=start_idx),
    )
...
def _all_items_found(
    ...
    return all(item_idx >= mm_item_counts[modality] for modality, item_idx in mm_found_counts.items())
...
def _apply_token_matches_with_placeholders(
    ...
    placeholders: dict[str, list[PlaceholderFeaturesInfo]] = {modality: [] for modality in mm_prompt_updates}
...
def _iter_placeholders(
    ...
    while start_idx < prompt_len:
        ...
        for modality, update, content_tokens_full in candidates:
            ...
            if first_token == content_tokens_full[0] and prompt[start_idx:end_idx_full] == content_tokens_full:
...
[`MultiModalKwargsItems`][aphrodite.multimodal.inputs.MultiModalKwargsItems].
"""
...
class BaseMultiModalProcessor(ABC, Generic[_I]):
    """
    Abstract base class to process multi-modal inputs to be used in Aphrodite.
    ...
    def _get_prompt_updates(
        ...
        [`PlaceholderRange`][aphrodite.multimodal.inputs.PlaceholderRange]
    ...
    def _get_hf_processor_text(self, mm_counts: Mapping[str, int]) -> str | None:
        ...
        [`DummyInputsBuilder`][aphrodite.multimodal.processing.BaseDummyInputsBuilder].
        """
        return None
    ...
    def _apply_hf_processor_main(
        ...
        if processor_data:
            processor_data, hf_processor_mm_kwargs = self._preprocess_hf_mm_data(processor_data, hf_processor_mm_kwargs)
    ...
    def _apply_hf_processor(
        ...
        mm_kwargs = MultiModalKwargsItems.from_hf_inputs(
            mm_processed_data,
            self._get_mm_fields_config(mm_processed_data, inputs.hf_processor_mm_kwargs),
        )
    ...
    def _cached_apply_hf_processor(
        ...
        mm_missing_kwargs = MultiModalKwargsItems.from_hf_inputs(
            ...
            self._get_mm_fields_config(mm_missing_processed_data, inputs.hf_processor_mm_kwargs),
        )
    ...
    def _apply_prompt_updates_via_text(
        ...
        *,
        ...
        matched_updates, result = _plan_prompt_updates_with(
            mm_prompt_updates,
            lambda update, start_idx: self._iter_text_matches(update, prompt_text, tokenizer, start_idx=start_idx),
        )
        ...
        if encode_segments_separately:
            ...
            new_token_ids = flatten_2d_lists([tokenizer.encode(text, add_special_tokens=False) for text in out_texts])
        else:
            new_token_ids = tokenizer.encode("".join(out_texts), add_special_tokens=False)
    ...
    def _apply_prompt_updates(
        ...
        new_token_ids, match_result, placeholders = self._apply_token_matches_with_placeholders(
            token_ids,
            mm_prompt_updates,
        )

        if all(all(update_idx is not None for update_idx in update_idxs) for update_idxs in match_result.values()):
            placeholders = {
...
class EncDecMultiModalProcessor(BaseMultiModalProcessor[_I]):
    ...
    def _get_enc_dec_inputs(
        ...
        if isinstance(decoder_prompt_raw, str):
            ...
            decoder_prompt_ids = tokenizer.encode(decoder_prompt_raw, add_special_tokens=False)
        else:
