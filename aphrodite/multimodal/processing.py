...
from aphrodite.transformers_utils.tokenizer import (AnyTokenizer,
                                                    decode_tokens,
                                                    encode_tokens)
from aphrodite.utils import GiB_bytes, LRUCache, flatten_2d_lists, full_groupby
...
class PromptUpdateDetails(Generic[_S]):
    ...
    [`SupportsMultiModal.get_multimodal_embeddings`][aphrodite.modeling.models.interfaces.SupportsMultiModal.get_multimodal_embeddings].
    """
...
def full_groupby_modality(values: Iterable[_M]) -> ItemsView[str, list[_M]]:
    """Convenience function to apply [`full_groupby`][aphrodite.utils.full_groupby]
    based on modality."""
...
class _BoundPromptSequence:
    """
    A [`_PromptSeq`][aphrodite.multimodal.processing.PromptSeq] bound
...
class BoundPromptUpdate:
    """
    A [`PromptUpdate`][aphrodite.multimodal.processing.PromptUpdate] bound
    to a tokenizer to automatically convert
    [`target`][aphrodite.multimodal.processing.PromptUpdate.target] and the result of
    [`get_content`][aphrodite.multimodal.processing.BoundPromptUpdate.get_content]
...
class ProcessingCache:
    ...
    def put(
        ...
        (see [`get`][aphrodite.multimodal.processing.ProcessingCache.get]).
        """
        cache_key = MultiModalHasher.hash_kwargs(model_id=model_id,
                                                 **{modality: input_item},
                                                 **input_kwargs)
...
class BaseMultiModalProcessor(ABC, Generic[_I]):
    ...
    def _get_data_parser(self) -> MultiModalDataParser:
        ...
        [`_get_hf_mm_data`][aphrodite.multimodal.processing.BaseMultiModalProcessor._get_hf_mm_data].

        You can support additional modalities by creating a subclass
        of [`MultiModalDataParser`][aphrodite.multimodal.parse.MultiModalDataParser]
    ...
    def _to_mm_items(
        ...
        """
        Normalize
        [`MultiModalDataDict`][aphrodite.multimodal.inputs.MultiModalDataDict]
        to [`MultiModalDataItems`][aphrodite.multimodal.parse.MultiModalDataItems]
        before passing them to
        [`_get_hf_mm_data`][aphrodite.multimodal.processing.BaseMultiModalProcessor._get_hf_mm_data].
        """
    ...
    def _apply_hf_processor_tokens_only(
        ...
        with the output of
        [`_apply_hf_processor_text_only`][aphrodite.multimodal.processing.BaseMultiModalProcessor._apply_hf_processor_text_only]
        on the
        corresponding text.
        """
    ...
    def _apply_hf_processor_mm_only(
        ...
        [`DummyInputsBuilder`][aphrodite.multimodal.profiling.BaseDummyInputsBuilder]
