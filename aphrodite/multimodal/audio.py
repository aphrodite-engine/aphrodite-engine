    ...
    def resample(
        ...
        *,
        ...
        if self.method == "pyav":
            ...
            raise ValueError(
                f"Invalid resampling method: {self.method}. Supported methods are 'pyav', 'scipy', and 'soxr'."
            )
...
def split_audio(
    ...
    Raises:
        ValueError: If ``audio_data`` is not 1D.
    ...
    if audio_data.ndim > 1:
        raise ValueError(f"split_audio expects mono audio, got shape {audio_data.shape}")
