...
class MultiModalProcessorSenderCache(BaseMultiModalProcessorCache):
    ...
    def get_and_update_item(
        ...
        self.cache_if_fits(self._cache, mm_hash, MultiModalProcessorCacheItemMetadata(*mm_item))
        return mm_item
...
class ShmObjectStoreReceiverCache(BaseMultiModalReceiverCache):
    ...
    def get_and_update_features(self, mm_features: list["MultiModalFeatureSpec"]) -> list["MultiModalFeatureSpec"]:
        ...
        features_with_data = [feature for feature in mm_features if feature.data is not None]
