...
VideoDecoderBackend = Literal["opencv", "pyav", "torchcodec", "pynvvideocodec", "deepstream"]
...
def _get_backend_option_defaults(backend: str) -> dict[str, Any]:
    try:
        ...
        raise ValueError(f"Unknown video codec backend {backend!r}; valid options: {valid}.") from None
...
def decode_video(
    ...
    *,
    ...
    if frame_recovery and backend != "opencv":
        error = ValueError if backend == PYNVVIDEOCODEC_VIDEO_BACKEND else AssertionError
