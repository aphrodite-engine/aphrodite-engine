    ...
    def configure(self, hw_decoders: int) -> None:
        with self.cond:
            if self.max_slots is None:
                ...
                raise RuntimeError(
                    f"PyNvVideoCodec decoder count is already configured as {self.max_slots}, got {hw_decoders}"
                )
    ...
    def decode_frames_pynvvideocodec(
        ...
        **kwargs,
        ...
        try:
            ...
            frame_idx = loader_cls.compute_frames_index_to_sample(source=source, target=target, **kwargs)
