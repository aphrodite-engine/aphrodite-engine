...
def decode_pyav(loader_cls, data: bytes, target: VideoTargetMetadata, sampling_kwargs: dict):
    with av.open(BytesIO(data)) as container:
        source = loader_cls._prepare_source(PyAVVideoBackendMixin.get_metadata(container))
        stream = container.streams.video[0]
        check_frame_pixel_limit(stream.width, stream.height)
        indices = loader_cls.compute_frames_index_to_sample(source=source, target=target, **sampling_kwargs)
        frames, valid = PyAVVideoBackendMixin.decode_frames(container, indices, source.original_fps, source.duration)
    return frames, source, indices, valid
