...
def decode_torchcodec(
    ...
    *,
    ...
    source = loader_cls._prepare_source(TorchCodecVideoBackendMixin.get_torchcodec_metadata(decoder))
    frame_idx = loader_cls.compute_frames_index_to_sample(source=source, target=target, **sampling_kwargs)
    frames, valid = TorchCodecVideoBackendMixin.decode_torchcodec_frames(decoder, frame_idx)
