...
class TokenIDScanner:
    ...
    def _decode_token(self, token_id: int) -> str:
        if token_id not in self._token_text_cache:
            ...
            self._token_text_cache[token_id] = token_text if isinstance(token_text, str) else ""
    ...
    def scan(
        ...
        if self._deferred_terminals:
            prefix_items, effective_text, deferred_trailing_count = self._resolve_deferred(
                delta_text, len(delta_token_ids)
            )
        ...
        for idx, tid in enumerate(delta_token_ids):
            ...
            if terminal is not None:
                ...
                results.append(PreLexedTerminal(terminal, tid, decoded_token_texts[idx]))
            else:
        ...
        if text_accum:
            joined = "".join(text_accum)
            if joined:
                results.append(TextChunk(joined, tuple(token_text_accum), len(token_text_accum)))
    ...
    def flush_pending(self) -> list[LexerInput]:
        ...
        if self._deferred_post_text:
            prefix_count = self._deferred_prefix_token_counts[0] if self._deferred_prefix_token_counts else 0
            results.append(TextChunk(self._deferred_post_text, token_count=prefix_count))
            self._deferred_post_text = ""
    ...
    def _resolve_deferred(
        ...
        for idx, terminal in enumerate(deferred):
            ...
            if pos > 0:
                results.append(TextChunk(remaining[:pos], token_count=prefix_token_count))
    ...
    def _rebuild_from_anchors(
        ...
        for i, anchor in enumerate(anchors):
            ...
            if pos >= consumed:
                ...
                if not has_later_valid and consumed < len(delta_text) and anchor.terminal != DROP_TERMINAL:
