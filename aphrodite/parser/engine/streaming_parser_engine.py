...
class StreamingParserEngine:
    ...
    def __init__(
        ...
        self._reasoning_markup_terminals: frozenset[str] = self._compute_reasoning_markup_terminals()
    ...
    def _record_reasoning_tokens(self, events: Sequence[SemanticEvent]) -> None:
        self._reasoning_token_count += sum(
            event.token_count for event in events if event.type == EventType.REASONING_CHUNK
        )
        ...
        if (
            ...
            for tid in delta_token_ids:
                if tid in self._resolved_token_ids:
                    ...
                    break
            if not has_special:
                events = self._emit_for_state(delta_text, token_count=len(delta_token_ids))
        ...
        for item in items:
            if isinstance(item, PreLexedTerminal):
                ...
                if not item.text and item.token_count:
                    events.extend(self._emit_for_state("", token_count=item.token_count))
                else:
                    events.extend(
                        self._process_lex_tokens(self._lexer.feed(item.text, item.token_texts, item.token_count))
                    )
        return events
    ...
    def _compute_reasoning_markup_terminals(self) -> frozenset[str]:
        ...
        markers = frozenset(
            ...
            if ParserState.REASONING in (state, tr.next_state) and tr.next_state not in self._TOOL_STATES
        )
        for (state, terminal), tr in self.config.transitions.items():
    ...
    def _on_terminal(self, terminal: str, value: str, token_count: int = 0) -> list[SemanticEvent]:
        ...
        if self.skip_tool_parsing and terminal in self._tool_terminals:
            ...
            used_as_plain_closer = is_exit and not is_opener and not self._in_skipped_tool_span
    ...
    def _apply_transition(
        ...
        for event_type in transition.events:
            ...
            event_value = (
                message_header
                if previous_state == ParserState.MESSAGE_HEADER and event_type == EventType.TEXT_CHUNK
                else value
            )
