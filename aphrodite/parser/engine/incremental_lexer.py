...
class IncrementalLexer:
    ...
    def flush(self) -> list[LexToken]:
        ...
        if self.buffer:
            tokens.append(LexToken(self.content_terminal, self.buffer, sum(self._token_counts)))
            self.buffer = ""
            self._token_counts.clear()
        return tokens
        ...
        while self.buffer:
            if has_only_literals and first_chars:
                ...
                if not has_potential:
                    tokens.append(LexToken(content_terminal, self.buffer, sum(self._token_counts)))
                    self.buffer = ""
                    self._token_counts.clear()
                    break
