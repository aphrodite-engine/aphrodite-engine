...
if TYPE_CHECKING:
    ...
    from aphrodite.parser.engine.events import SemanticEvent
    from aphrodite.parser.engine.parser_engine import ToolCallSlot
...
_DEFAULT_JSON_SCHEMA: dict[str, Any] = {"anyOf": [{"type": "object"}, {"type": "array"}]}
...
class MistralParser(ParserEngine):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.bot_token_id is None:
            raise RuntimeError("Mistral parser could not locate the tool call token in the tokenizer!")
    ...
    def adjust_request(
        ...
        if request.structured_outputs is not None:
            ...
            if request.response_format.type == "json_object":
                ...
                raise ValueError(
                    f"MistralParser only accepts `text`, `json_object` or `json_schema`, got {request.response_format=}"
                )
            request.response_format = None
    ...
    def _build_guided_schema_pre_v11(
        ...
        if tool_choice == "required":
            ...
            if not applicable_tools:
                logger.warning(
                    "Named tool %r not found in tools list; skipping guided schema injection.",
                    chosen_name,
                )
                return None
            extra["maxItems"] = 1
    ...
    def _legacy_extract_tool_calls(
        ...
        elif request.tool_choice == "required" or isinstance(request.tool_choice, ChatCompletionNamedToolChoiceParam):
    ...
    def _extract_tool_calls_streaming_pre_v11_tokenizer(
        ...
        while len(delta_text) > 0:
            ...
            if self.current_tool_name is not None:
                ...
                self.prev_tool_call_arr[self.current_tool_id]["name"] = self.current_tool_name
