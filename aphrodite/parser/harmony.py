...
from aphrodite.tool_parsers.structural_tag_registry import (
    ...
    register_aphrodite_structural_tag,
)
...
class HarmonyParser(DelegatingParser):
    def __init__(self, tokenizer, tools=None, *args, **kwargs):
        ...
        if self.reasoning_parser and not isinstance(self.reasoning_parser, GptOssReasoningParser):
            raise ValueError(f"Harmony requires GptOssReasoningParser, got {self.reasoning_parser.__class__.__name__}.")

        if self.tool_parser and not isinstance(self.tool_parser, GptOssToolParser):
            raise ValueError(f"Harmony requires GptOssToolParser, got {self.tool_parser.__class__.__name__}.")
...
def _assemble_tag(allow_analysis: bool, allow_commentary: bool, content: Format) -> StructuralTag:
    tags = []
    if allow_analysis:
...
@register_aphrodite_structural_tag("harmony")
def get_harmony_structural_tag(
    ...
    if builtin_tools:
        ...
        tags = [
            TagFormat(
                begin=pattern.format(name=tool.function.name, channel=channel, constrain=constrain),
    ...
    return _assemble_tag(allow_analysis=True, allow_commentary=True, content=OrFormat(elements=tags))
...
def _params_to_final_content(params: StructuredOutputsParams) -> Format | None:
    ...
    if params.choice is not None:
        return OrFormat(elements=[ConstStringFormat(value=choice) for choice in params.choice])
...
def _adjust_output_format(
    ...
    if isinstance(final_content, JSONSchemaFormat):
        ...
        content = TagFormat(begin=_FINAL_BEGIN.format(constrain=""), content=final_content, end=_END_TAG)
    structural_tag = _assemble_tag(allow_analysis=True, allow_commentary=False, content=content)
