...
if TYPE_CHECKING:
    ...
    me_quant = LazyLoader("quantization", globals(),
                          "aphrodite.quantization")
    me_models = LazyLoader("modeling", globals(),
...
class ModelConfig:
    ...
    """Number of bits to use for DeepSpeed FP quantization. Supported number
    of bits are: 4, 6, 8, 12."""
    ...
    configure the neuron config that can not be gathered from the aphrodite
    ...
    def __post_init__(self) -> None:
        ...
        # NOTE: In V0, we set the default seed to None because the
        ...
        if envs.APHRODITE_USE_V1 and self.seed is None:
            self.seed = 0
            if not envs.APHRODITE_ENABLE_V1_MULTIPROCESSING:
                logger.warning(
                    "The global random seed is set to {}. Since "
                    "APHRODITE_ENABLE_V1_MULTIPROCESSING is set to False, "
        ...
        logger.info("Resolved architecture: {}", arch)
        ...
        if not self.disable_sliding_window and has_interleaved_attention:
            if not envs.APHRODITE_USE_V1 and (
    ...
    def _get_convert_type(
        ...
        if convert_type != "none":
            logger.info(
                "Resolved `--convert auto` to `--convert {}`. "
                "Pass the value explicitly to silence this message.",
                convert_type)
...
class CacheConfig:
    ...
    def _verify_prefix_caching(self) -> None:
        ...
        if self.sliding_window is not None and not envs.APHRODITE_USE_V1:
            raise NotImplementedError(
...
class ParallelConfig:
    ...
    is provided explicitly to aphrodite serve."""
    data_parallel_hybrid_lb: bool = False
    """Whether to use "hybrid" DP LB mode. Applies only to online serving
    and when data_parallel_size > 0. Enables running an AsyncLLM
    and API server on a "per-node" basis where Aphrodite load balances
    between local data parallel ranks, but an external LB balances
    between Aphrodite nodes/replicas. Set explicitly in conjunction with
    ...
    def compute_hash(self):
        ...
        factors.append(envs.APHRODITE_ALL2ALL_BACKEND)
        return hashlib.sha256(str(factors).encode()).hexdigest()
    ...
    def __post_init__(self) -> None:
        ...
        if self.data_parallel_size > 1 or self.data_parallel_size_local == 0:
            ...
            self.data_parallel_size = envs.APHRODITE_DP_SIZE
            self.data_parallel_rank = envs.APHRODITE_DP_RANK
            self.data_parallel_rank_local = envs.APHRODITE_DP_RANK_LOCAL
            self.data_parallel_master_ip = envs.APHRODITE_DP_MASTER_IP
        ...
        if self.distributed_executor_backend is None and self.world_size > 1:
            ...
            if current_platform.is_neuron():
                ...
                backend = "uni"
            elif current_platform.is_tpu() and envs.APHRODITE_XLA_USE_SPMD:
                backend = "uni"
            ...
            logger.debug("Defaulting to use {} for distributed inference",
                         backend)
...
class DeviceConfig:
    ...
    """Device type for Aphrodite execution.
        ...
        if self.device == "auto":
            ...
            if not self.device_type:
                raise RuntimeError(
                    ...
                    "the environment variable `APHRODITE_LOGGING_LEVEL=DEBUG` "
...
class SpeculativeConfig:
    ...
    enable_chunked_prefill: SkipValidation[bool] = None  # type: ignore
    """Whether Aphrodite is configured to use chunked prefill or not. Used for
            ...
            if self.model is not None:
                ...
                if self.method in ("eagle", "eagle3"):
                    ...
                    from aphrodite.transformers_utils.configs import (
                        SpeculatorsConfig)
                    from aphrodite.transformers_utils.configs.eagle import (
                        EAGLEConfig)
...
class LoRAConfig:
    ...
    def verify_with_cache_config(self, cache_config: CacheConfig):
        if cache_config.cpu_offload_gb > 0 and not envs.APHRODITE_USE_V1:
            raise ValueError(
...
class MultiModalConfig:
    ...
    def get_limit_per_prompt(self, modality: str) -> int:
        ...
        return self.limit_per_prompt.get(
            modality,
            999 if envs.APHRODITE_USE_V1 else 1,
        )
...
class PoolerConfig:
    ...
    :class:`aphrodite.modeling.layers.pooler.PoolingType`.
    """
...
def _resolve_auto_dtype(
    ...
    *,
    ...
    logger.warning(
        ...
        "Falling back to {} for compatibility.",
        device_str,
        config_dtype,
        preferred_dtype
    )
...
class KVTransferConfig:
    ...
    """The KV connector for Aphrodite to transmit KV caches between
    Aphrodite instances.
    """
...
class PassConfig:
    ...
    enable_fusion: bool = field(default_factory=lambda: not envs.APHRODITE_USE_V1)
    """Whether to enable the custom fusion (RMSNorm/SiluMul+quant) pass."""
    enable_attn_fusion: bool = False
    """Whether to enable the custom attention+quant fusion pass."""
    enable_noop: bool = field(default_factory=lambda: not envs.APHRODITE_USE_V1)
...
class CompilationConfig:
    ...
    - Top-level Compilation control:
        - [`level`]
          [aphrodite.common.config.CompilationConfig.level]
        - [`debug_dump_path`]
          [aphrodite.common.config.CompilationConfig.debug_dump_path]
        - [`cache_dir`]
          [aphrodite.common.config.CompilationConfig.cache_dir]
        - [`backend`]
          [aphrodite.common.config.CompilationConfig.backend]
        - [`custom_ops`]
          [aphrodite.common.config.CompilationConfig.custom_ops]
        - [`splitting_ops`]
          [aphrodite.common.config.CompilationConfig.splitting_ops]
    - CudaGraph capture:
        - [`use_cudagraph`]
          [aphrodite.common.config.CompilationConfig.use_cudagraph]
        - [`cudagraph_capture_sizes`]
          [aphrodite.common.config.CompilationConfig.cudagraph_capture_sizes]
        - [`cudagraph_num_of_warmups`]
          [aphrodite.common.config.CompilationConfig.cudagraph_num_of_warmups]
        - [`cudagraph_copy_inputs`]
          [aphrodite.common.config.CompilationConfig.cudagraph_copy_inputs]
        - [`full_cuda_graph`]
          [aphrodite.common.config.CompilationConfig.full_cuda_graph]
    - Inductor compilation:
        - [`use_inductor`]
          [aphrodite.common.config.CompilationConfig.use_inductor]
        - [`compile_sizes`]
          [aphrodite.common.config.CompilationConfig.compile_sizes]
        - [`inductor_compile_config`]
          [aphrodite.common.config.CompilationConfig.inductor_compile_config]
        - [`inductor_passes`]
          [aphrodite.common.config.CompilationConfig.inductor_passes]
    ...
    use_cudagraph: bool = field(default_factory=lambda: envs.APHRODITE_USE_V1)
    """Whether to use cudagraph inside compilation.
    - False: cudagraph inside compilation is not used.
    - True: cudagraph inside compilation is used. It requires
        that all input buffers have fixed addresses, and all
        splitting ops write their outputs to input buffers.
    In the Aphrodite V1 Engine, this flag only applies for
    ...
    def __post_init__(self) -> None:
        ...
        #    https://github.com/aphrodite-project/aphrodite/issues/14703
    ...
    def init_backend(
        ...
        return AphroditeBackend(aphrodite_config)
...
class AphroditeConfig:
    ...
    instance_id: str = ""
    """The instance id for the Aphrodite instance."""
    ...
    def compute_hash(self) -> str:
        ...
        aphrodite_factors.append(envs.APHRODITE_USE_V1)
        ...
        if self.decoding_config:
            aphrodite_factors.append(self.decoding_config.compute_hash())
        else:
            aphrodite_factors.append("None")
    ...
    def __post_init__(self):
        ...
        if envs.APHRODITE_USE_V1 and self.model_config is not None and \
        ...
        if ((not envs.APHRODITE_USE_V1) and self.lora_config is not None
        ...
        if (envs.APHRODITE_USE_V1
            ...
            if self.model_config is not None and \
                ...
                if self.speculative_config is not None and \
                    ...
                    not envs.APHRODITE_ALLOW_CHUNKED_LOCAL_ATTN_WITH_HYBRID_KV_CACHE:
                    logger.warning(
...
_current_aphrodite_config: Optional[AphroditeConfig] = None
...
def get_layers_from_aphrodite_config(
    ...
    if layer_names is None:
        layer_names = list(
            aphrodite_config.compilation_config.static_forward_context.keys())
