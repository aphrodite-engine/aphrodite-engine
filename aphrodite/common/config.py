...
from aphrodite.quantization import (QUANTIZATION_METHODS, QuantizationMethods,
...
class ModelConfig:
    ...
    """The task to use the model for. Each Aphrodite instance only supports one
    ...
    def get_diff_sampling_param(self) -> dict[str, Any]:
        ...
        `generation_config` is `"aphrodite"`, an empty dictionary is returned.
...
class LoRAConfig:
    ...
    def verify_lora_support(self):
        if self.long_lora_scaling_factors is not None and envs.APHRODITE_USE_V1:
            raise ValueError(
...
class AphroditeConfig:
    ...
    def compute_hash(self) -> str:
        ...
        if self.additional_config:
            aphrodite_factors.append(self.additional_config.compute_hash())
        else:
            aphrodite_factors.append("None")
...
def set_current_aphrodite_config(aphrodite_config: AphroditeConfig,
    ...
    """
    global _current_aphrodite_config
...
def get_layers_from_aphrodite_config(aphrodite_config: AphroditeConfig,
    ...
    return {
        ...
        aphrodite_config.compilation_config.static_forward_context.items()
