...
from aphrodite.config.utils import compute_hash_cached
...
class EplbState:
    ...
    def add_model(
        ...
        num_unpadded_tokens_tensors = [
            torch.tensor(0, dtype=torch.int32, device=self.device) for _ in range(num_ubatches)
        ]
    ...
    def step(
        ...
        if (
            ...
            for expert_load_pass, eplb_model_state in zip(expert_load_pass_list, self.model_states.values()):
                ...
                avg_tokens_tensor, max_tokens_tensor = _compute_eplb_load_stats(num_tokens_per_rank)
    ...
    def rearrange(
        ...
        for eplb_model_state in self.model_states.values():
            ...
            logical_expert_load_window.scatter_add_(
                dim=-1,
                index=physical_to_logical.masked_fill(physical_to_logical < 0, invalid_idx)
                .unsqueeze(0)
            ...
            if not self.is_async or is_profile:
                ...
                if (
                    ...
                    def rank_load_imbalance(
                        ...
                        loads_per_rank = loads_per_replica.reshape(logical_loads.shape[0], ep_size, -1).sum(dim=(0, 2))
                        mean_load = loads_per_rank.mean()
                        if mean_load == 0:
                            return 1.0
                        return (loads_per_rank.max() / mean_load).item()

                    current_imbalance = rank_load_imbalance(eplb_model_state.physical_to_logical_map)
                    proposed_imbalance = rank_load_imbalance(new_physical_to_logical_map)
                    relative_improvement = (current_imbalance - proposed_imbalance) / current_imbalance
                    skip_rearrange = relative_improvement < 0.05
                    if skip_rearrange and is_main_rank:
                        logger.info(
                            "[EPLB] Skip rearrange: imbalance %.4f -> %.4f (no material gain)",
                            current_imbalance,
                            proposed_imbalance,
                        )
    ...
    def create_communicator(self, model_config: ModelConfig, group_coordinator: GroupCoordinator) -> EplbCommunicator:
...
def _commit_eplb_maps(
    ...
    # number of GPUs available to Aphrodite changes while Aphrodite is running. Otherwise copy the
