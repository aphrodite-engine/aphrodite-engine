...
Usage:
    ...
    Example:
        aphrodite serve <model> --kv-transfer-config '{
    ...
    Then run your benchmark with desired input/output lengths:
        aphrodite bench serve --base-url http://127.0.0.1:8000 --model <model> \\
...
from aphrodite.v1.kv_cache_interface import CircularBufferSpec, iter_layer_specs
...
class DecodeBenchConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def __init__(
        ...
        if role == KVConnectorRole.SCHEDULER:
            self.connector_scheduler = DecodeBenchConnectorScheduler(aphrodite_config, kv_cache_config)
        elif role == KVConnectorRole.WORKER:
            self.connector_worker = DecodeBenchConnectorWorker(aphrodite_config, kv_cache_config)
    ...
    def update_state_after_alloc(self, request: "Request", blocks: "KVCacheBlocks", num_external_tokens: int):
        ...
        for group_idx, (
            ...
            is_circular_buffer = all(
                isinstance(spec, CircularBufferSpec) for spec in iter_layer_specs(group.kv_cache_spec)
            )
            if not is_circular_buffer:
                ...
                assert 0 <= external_block_start <= num_computed_blocks <= len(group_blocks), (
            ...
            block_ids = [block.block_id for block in selected_blocks if not block.is_null]
            if not block_ids:
                logger.warning(
...
class DecodeBenchConnectorWorker:
    ...
    def __init__(self, aphrodite_config: "AphroditeConfig", kv_cache_config: "KVCacheConfig"):
        ...
        self.group_to_layers = {
            group_idx: list(group.layer_names) for group_idx, group in enumerate(kv_cache_config.kv_cache_groups)
        }
        self._zero_fill_group_ids = {
            group_idx
            for group_idx, group in enumerate(kv_cache_config.kv_cache_groups)
            if any(isinstance(spec, CircularBufferSpec) for spec in iter_layer_specs(group.kv_cache_spec))
        }
    ...
    def _fill_blocks(self, group_idx: int, block_ids: list[int], num_tokens: int):
        ...
        fill_mean, fill_std = (0.0, 0.0) if group_idx in self._zero_fill_group_ids else (self.fill_mean, self.fill_std)
    ...
    def _fill_state_tensor(self, kv_cache: torch.Tensor, fill_mean: float, fill_std: float):
