...
class OffloadingEventsTracker:
    ...
    def _record_partial_tail(
        ...
        chunk_start = ((boundary_tokens - 1) // group_config.tokens_per_chunk) * group_config.tokens_per_chunk
        ...
        block_hashes = tuple(block_hash for block_hash in maybe_block_hashes if block_hash is not None)
        assert block_hashes and len(block_hashes) == len(maybe_block_hashes)
        parent_block_hash = req.block_hashes[first_hash_idx - 1] if first_hash_idx > 0 else None
    ...
    def _build_event_metadata(
        ...
        assert len(chunk_hashes) == (group_config.tokens_per_chunk // group_config.tokens_per_block)
    ...
    def _take_stored_event(self, event: OffloadingEvent) -> Iterable[KVCacheEvent]:
        ...
        for key in event.keys:
            ...
            if meta is None:
                ...
                yield self._placeholder_stored(key, event.medium, locality, event.ownership)
                continue
