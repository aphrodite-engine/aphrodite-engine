...
from aphrodite.v1.core.single_type_kv_cache_manager import SingleTypeKVCacheManager
...
class SchedulerOffloadConfig(NamedTuple):
    ...
    def from_spec(
        ...
        if len(full_attn_tokens_per_chunk) == 1:
            ...
            all_tokens_per_chunk = [tpb * spec.blocks_per_chunk for tpb in spec.tokens_per_block]
            if all_tokens_per_chunk:
                alignment_tokens = max(all_tokens_per_chunk)

        retention_interval = aphrodite_config.cache_config.prefix_cache_retention_interval
        ...
        use_eagle_block_drop = (
            ...
            and aphrodite_config.speculative_config.use_eagle_block_drop()
        )
        ...
        for idx, tokens_per_block in enumerate(spec.tokens_per_block):
            ...
            assert manager_cls is not None, f"No manager found for KV cache spec {type(kv_spec).__name__}"
            kv_group_configs_list.append(
                GroupOffloadConfig(
                    ...
                    requires_cow_source=(isinstance(kv_spec, MambaSpec) and kv_spec.mamba_cache_mode == "align"),
                )
            )
        ...
        has_partial_recurrent_group = any(
            config.requires_cow_source and config.tokens_per_block > spec.tokens_per_hash for config in kv_group_configs
        )
        ...
        supports_partial_tail = (
            ...
            and all(
                config.sliding_window_size_in_chunks is None or config.requires_cow_source
        ...
        if retention_interval is not None:
            if retention_interval < 0:
                raise ValueError(
                    f"APHRODITE_PREFIX_CACHE_RETENTION_INTERVAL ({retention_interval}) must be non-negative."
                )
            for config in kv_group_configs:
                if (
                    ...
                    raise ValueError(
                        f"APHRODITE_PREFIX_CACHE_RETENTION_INTERVAL "
...
class OffloadingConnectorScheduler:
    ...
    def __init__(
        ...
        self._partial_tail_block_size = (
            self.config.kv_group_configs[0].tokens_per_block if self.config.supports_partial_tail else 0
        )
        self._cow_source_groups = frozenset(
            config.group_idx for config in self.config.kv_group_configs if config.requires_cow_source
        )
    ...
    def _make_boundary_key(self, request: Request, group_idx: int, boundary_tokens: int) -> OffloadKey:
    ...
    def _lookup(self, req_status: RequestOffloadState) -> int | None:
        ...
        max_boundary = round_down(min(req_status.req.num_prompt_tokens - 1, block_end - 1), tokens_per_hash)
        ...
        for boundary in range(max_boundary, complete_boundary, -tokens_per_hash):
            ...
            boundary_keys = []
            for group_config in self.config.kv_group_configs:
                key = self._make_boundary_key(req_status.req, group_config.group_idx, boundary)
            ...
            if not boundary_missed and not boundary_pending:
                for group_config, key in zip(self.config.kv_group_configs, boundary_keys):
                    self._events_tracker.record_partial_lookup(req_status.req, group_config, boundary, key)
    ...
    def update_state_after_alloc(self, request: Request, blocks: KVCacheBlocks, num_external_tokens: int):
        ...
        for group_config, group_state, group_blocks in zip(
            ...
            if num_pending_gpu_blocks:
                start_chunk_idx = load_start_gpu_block_idx // self.config.blocks_per_chunk
                ...
                if partial_tail_boundary is not None:
                    keys_to_load.append(self._make_boundary_key(request, group_config.group_idx, partial_tail_boundary))

            dst_block_ids.extend(block.block_id for block in group_blocks[load_start_gpu_block_idx:num_gpu_blocks])
    ...
    def _build_aligned_boundary_store_jobs(
        ...
        for req_id, entries in handoffs.items():
            ...
            for group_idx, block_id, boundary in entries:
                ...
                if block_id == 0 or boundary > max_boundary or boundary % group_config.tokens_per_chunk != 0:
                    continue
    ...
    def _build_partial_tail_store_jobs(self, scheduler_output: SchedulerOutput) -> dict[int, TransferJob]:
        ...
        for req_id, entries in handoffs.items():
            entries = [entry for entry in entries if entry[2] % self._partial_tail_block_size != 0]
            if not entries:
                continue
            req_status = self._req_status.get(req_id)
            ...
            if any(
                ...
                continue
            keys = [self._make_boundary_key(req, group.group_idx, boundary) for group in self.config.kv_group_configs]
            block_ids = [
            ...
            for group_config, key in zip(self.config.kv_group_configs, keys):
                if key in store_output.keys_to_store:
                    self._events_tracker.record_partial_store(req, group_config, boundary, key)
    ...
    def _reachable_store_block_mask(
        ...
        return group_config.manager_cls.reachable_block_mask(
            ...
            final_segment_end_block=(
                final_segment_end_chunk_idx * blocks_per_chunk if final_segment_end_chunk_idx is not None else None
            ),
        )

    def _final_swa_alignment_blocks(self, group_config: GroupOffloadConfig) -> int | None:
    ...
    def _build_store_jobs(
        ...
        for req_id in chain(
            ...
            prompt_offloadable_tokens = self._calc_num_offloadable_tokens(req_status, req.num_prompt_tokens)
            ...
            for group_config, group_state in zip(self.config.kv_group_configs, req_status.group_states):
                ...
                prompt_horizon_chunks = prompt_offloadable_tokens // group_config.tokens_per_chunk
                final_swa_alignment_blocks = self._final_swa_alignment_blocks(group_config)
                ...
                if self.config.retention_interval is not None or group_config.is_eagle_group:
                ...
                if reconsider_final_swa_tail:
                    ...
                    partial_segment_start_block = horizon_blocks - horizon_blocks % final_swa_alignment_blocks
                    partial_segment_start_chunk = partial_segment_start_block // blocks_per_chunk
                ...
                offload_block_ids = group_state.block_ids[
                    start_chunk_idx * blocks_per_chunk + blocks_per_chunk - 1 : num_chunks
                ...
                for key_idx, (offload_key, block_id) in enumerate(zip(offload_keys, offload_block_ids)):
                    ...
                    if (
                        ...
                        and (
                            ...
                            or any(
                                prompt_horizon_block_mask[key_idx * blocks_per_chunk + block_idx]
                    ...
                    if block_mask is not None and not any(
                        block_mask[key_idx * blocks_per_chunk + b] for b in range(blocks_per_chunk)
                    ):
                        continue
            ...
            for group_config, group_state, store_range in zip(
                ...
                start_block_idx, num_blocks = store_range
        ...
        for job_id, count in meta.completed_jobs.items():
            ...
            if self._block_id_to_pending_jobs:
                ...
                if req_status.req.is_finished():
                    self._remove_pending_job(job_id, job_status.deferred_fence_block_ids)
