...
def _coalesce_runs(runs: list[CopyRun]) -> tuple[CopyRun, ...]:
    ...
    for run in runs:
        ...
        if (
            ...
            out[-1] = CopyRun(prev.local_offset, prev.canonical_offset, size, 1, size, size)
        else:
            out.append(run)
    return tuple(out)
        ...
        for chunk_start in range(0, num_tokens, ctx.interleave):
            ...
            runs.append(
                CopyRun(
                    ...
                    region.canonical_offset + canonical_token * region.canonical_token_stride,
                    region.bytes_per_token,
                    ctx.interleave,
                    region.bytes_per_token,
                    region.canonical_token_stride,
    ...
    if head_stride == bs * head_elems and token_stride == head_elems:  # HND
        ...
        return [
            ByteRegion(
                ...
                canonical_offset=(head_shard * heads + head) * canonical_span * head_bytes,
    ...
    if token_stride == heads * head_size and head_stride == head_size:  # NHD
        ...
        return [
            ByteRegion(
                ...
                canonical_offset=half * canonical_half_bytes + head_shard * token_row_bytes,
                bytes_per_token=token_row_bytes,
                canonical_token_stride=num_head_shards * token_row_bytes,
            )
    ...
    if head_stride == bs * head_size and token_stride == head_size:  # HND
        ...
        return [
            ByteRegion(
                ...
                canonical_offset=(half * total_heads + head_shard * heads + head) * canonical_span * head_bytes,
                bytes_per_token=head_bytes,
                canonical_token_stride=head_bytes,
            )
            for half in range(2)
...
def _layer_mapping(
    ...
    if isinstance(spec, MLAAttentionSpec):
        ...
        if spec.compress_ratio != 1 or page % bs or ctx.tp_size % ctx.dcp_size or spec.kv_quant_mode.is_per_token_head:
            return None
        row = page // bs
    ...
    regions = _attention_byte_regions(kv_cache, spec, num_blocks, head_shard, num_head_shards, ctx.cp_size)
    if regions is None:
        return None
    return CanonicalPageMapping(
...
def _opaque_fallback_mapping(page_size_bytes: int, num_ranks: int, rank: int) -> CanonicalPageMapping:
    """Fallback: place the worker's page whole at a worker-exclusive offset."""
    run = CopyRun(0, rank * page_size_bytes, page_size_bytes, 1, page_size_bytes, page_size_bytes)
    return CanonicalPageMapping(
...
def _verify_tiling(layer_name: str, per_rank: list[CanonicalPageMapping]) -> None:
    ...
    for block_id in range(num_writers):
        ...
        assert _is_exact_partition(stored, size), (
            f"writers of block {block_id} do not tile the canonical page of layer {layer_name}"
        )
...
def derive_canonical_mappings(
    ...
    def ctx(rank: int) -> _RankContext:
        return _RankContext(
            ...
            total_kv_heads=aphrodite_config.model_config.get_total_num_kv_heads(),
            rank=rank,
        )
    ...
    for kv_cache_group in kv_cache_config.kv_cache_groups:
        ...
        for layer_name in kv_cache_group.layer_names:
            ...
            for rank in range(group_size):
                mapping = _layer_mapping(spec, kv_caches.get(layer_name), num_blocks, ctx(rank))
                if mapping is None:
                    break
            ...
            if len(per_rank) != group_size:
                ...
                if page is None:
                    continue
                per_rank = [_opaque_fallback_mapping(page, group_size, rank) for rank in range(group_size)]
