...
"""Translate Aphrodite KV cache metadata for native offloading backends."""
...
def build_offloading_config(
    ...
    """Translate Aphrodite configuration into the native offloading boundary."""
    ...
    _, tokens_per_hash = resolve_kv_cache_block_sizes(kv_cache_config, aphrodite_config)
    for group in groups:
    ...
    if blocks_per_chunk_config is not None and tokens_per_chunk is not None:
        raise ValueError("Specify only one of 'block_size' or 'blocks_per_chunk' in kv_connector_extra_config.")
    ...
    single_group_spec = (
        kv_cache_config.kv_cache_groups[0].kv_cache_spec if len(kv_cache_config.kv_cache_groups) == 1 else None
    )
    replicated_layout = (
        aphrodite_config.model_config.use_mla
        ...
        == single_group_spec.page_size_bytes * len(kv_cache_config.kv_cache_groups[0].layer_names)
    ...
    is_parallelism_agnostic = (
        ...
        == aphrodite_config.model_config.get_total_num_kv_heads()
    ...
    if canonical_layout and not is_parallelism_agnostic:
        ...
        def spec_certifiable(spec: KVCacheSpec) -> bool:
            ...
            if type(spec) is MLAAttentionSpec:
                return spec.tokens_per_state == 1 and spec.real_page_size_bytes % spec.block_size == 0
            if isinstance(spec, (SlidingWindowMLASpec, MLAAttentionSpec)):
                return False
            if not isinstance(spec, (FullAttentionSpec, SlidingWindowSpec)):
                return False
            return (total_kv_heads % tp_size == 0 or tp_size % total_kv_heads == 0) and spec.num_kv_heads == max(
                1, total_kv_heads // tp_size
            )
        ...
        layer_specs = [
            spec for group in kv_cache_config.kv_cache_groups for spec in iter_layer_specs(group.kv_cache_spec)
        ]
        is_parallelism_agnostic = (
            len(layer_specs) > 0
    ...
    return OffloadingConfig(
        ...
        kv_cache_layout=aphrodite_config.cache_config.kv_cache_layout,
    )
