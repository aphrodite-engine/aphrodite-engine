...
lease expires (the base worker reports it via ``done_sending``) or the
WRITE completes,
...
from aphrodite.distributed.kv_transfer.kv_connector.utils import BlockIds
...
from aphrodite.distributed.kv_transfer.kv_connector.v1.nixl.tp_mapping import ReadSpec
from aphrodite.distributed.kv_transfer.kv_connector.v1.nixl.utils import get_base_request_id
...
class NixlPushConnectorWorker(NixlBaseConnectorWorker):
    ...
    def __init__(
        ...
        # Handshakes that completed and are ready for a WRITE on the writer.
        self._deferred_push_inbox: queue.Queue[tuple[str, BlockIds, dict[str, Any]]] = queue.Queue()
    ...
    def _push_writer_loop(self) -> None:
        ...
        while not self._push_writer_stop.is_set():
            try:
                ...
                # 2. Deferred P→D pushes whose handshake just completed.
                while True:
                    try:
    ...
    def _do_start_push_kv(
        ...
        If it is not ready yet, defer the WRITE and re-drive this request
        through ``_deferred_push_inbox`` after the handshake resolves.
        """
        ...
        # Both sides remain logical here. The transfer helper converts each
        # with its own physical-blocks-per-logical ratio.
    ...
    def _xfer_blocks(
        ...
        Returns the in-flight transfer handle, or ``None`` if nothing was submitted.
        """
        ...
        if block_size_ratio > 1:
            local_block_ids, remote_block_ids = self._map_block_ids_for_block_size_ratio(
