...
from aphrodite.distributed.kv_transfer.kv_connector.v1.base import CopyBlocksOp
...
from aphrodite.distributed.kv_transfer.kv_connector.v1.nixl.utils import (
    _NIXL_SUPPORTED_DEVICE,
    get_representative_spec_type,
    zmq_ctx,
)
from aphrodite.distributed.kv_transfer.kv_connector.v1.ssm_conv_transfer_utils import (
    MambaConvSplitInfo,
    derive_mamba_conv_split,
)
from aphrodite.distributed.nixl_utils import NixlWrapper, nixl_agent_config
...
from aphrodite.utils.network_utils import make_zmq_path
...
def _tensor_byte_span_end(cache: torch.Tensor) -> int:
    ...
    max_element_offset = sum((size - 1) * stride for size, stride in zip(cache.shape, cache.stride()))
...
class NixlBaseConnectorWorker:
    ...
    def _compute_desc_ids(
        ...
        if self._has_mamba:
            ...
            num_ssm_regions = len(self._ssm_region_indices or self.block_len_per_layer) * ssm_regions_per_layer
        ...
        if num_ssm_regions == 0:
            ...
            block_arr = np.concatenate([np.asarray(g, dtype=np.int32) for g in block_ids])[None, :]
        ...
        for i, group in enumerate(block_ids):
            ...
            if _is_attention_spec(spec_type):
                ...
                all_descs.append((fa_region_ids * num_blocks + group_arr[None, :]).ravel())
                ...
                all_descs.append((ssm_region_ids * logical_blocks + group_arr[None, :] + num_fa_descs).ravel())
            else:
    ...
    def _build_local_splits_from_plan(
        ...
        ssm_num_splits = len(plan.source_ranks_per_group[ssm_idx]) if has_ssm_descs and ssm_idx is not None else 0
        ...
        assert block_size_ratio == 1 or fa_num_splits == 1 or all(fa_desc_replicated), (
            "Head-sharded attention reads with P_TP > D_TP and heterogeneous block sizes are not supported"
        )
    ...
    def __init__(
        ...
        self.block_size: int = cast(int, aphrodite_config.cache_config.block_size)
        ...
        self.nixl_backends = aphrodite_config.kv_transfer_config.get_from_extra_config("backends", ["UCX"])
        kv_lease_duration: int = aphrodite_config.kv_transfer_config.get_from_extra_config("kv_lease_duration", 30)
        # NOTE (NickLucche): For now we use a hardcoded value for a simpler interface.
        self._lease_extension = kv_lease_duration * 2 // 3

        self._bidirectional_kv_xfer_enabled: bool = aphrodite_config.kv_transfer_config.get_from_extra_config(
        ...
        self._is_hma_required = not aphrodite_config.scheduler_config.disable_hybrid_kv_cache_manager and any(
            not isinstance(spec, FullAttentionSpec) for spec in self._layer_specs.values()
        )
        ...
        self._has_mamba = any(isinstance(spec, MambaSpec) for spec in self._layer_specs.values())
        ...
        if self._is_csa_linear:
            ple_groups = [
                ...
                and all(
                    isinstance(spec, MambaSpec) and spec.tp_replicated for spec in iter_layer_specs(group.kv_cache_spec)
                )
            ]
            if len(ple_groups) != 1 or len(ple_groups[0][1].layer_names) != 1:
                raise ValueError("CSA-linear NIXL requires exactly one PLE cache owner.")
        ...
        if self._has_mamba:
            ...
            assert is_conv_state_dim_first(), (
                "3-read Mamba conv transfer requires DS conv state layout. Set APHRODITE_SSM_CONV_STATE_LAYOUT=DS"
            )
            mamba_spec = next(
                spec
                for spec in self._layer_specs.values()
                if isinstance(spec, MambaSpec) and (not self._is_csa_linear or not spec.tp_replicated)
            )
        ...
        num_threads = aphrodite_config.kv_transfer_config.get_from_extra_config("num_threads", 4)
        ...
        self._remote_agents: dict[EngineId, dict[tuple[int, int], str]] = defaultdict(dict)
        ...
        self.pcp_size = aphrodite_config.parallel_config.prefill_context_parallel_size
        self.dcp_size = aphrodite_config.parallel_config.decode_context_parallel_size
        ...
        if self.device_type == "cpu":
            ...
            if rsv_cores_for_kv:
                if not hasattr(os, "sched_setaffinity"):
                    raise NotImplementedError("os.sched_setaffinity is not available on this platform")
        ...
        if self.pp_size > 1 and self._is_hma_required:
            raise NotImplementedError(
                "NixlPushConnector does not support pipeline_parallel_size > 1 with hybrid KV cache layouts (HMA) yet."
            )
        if aphrodite_config.kv_transfer_config.kv_role == "kv_consumer" and self.pp_size > 1:
            raise NotImplementedError(
                "NixlPushConnector consumer (decode) does not support pipeline_parallel_size > 1."
            )
        ...
        self._handshake_futures: dict[EngineId, Future[tuple[dict[tuple[int, int], str], float]]] = {}
        # Protects _handshake_futures and _remote_agents.
        self._handshake_lock = threading.RLock()

        # TTL-based eviction of stale remote engine state.
        self._engine_last_active: dict[EngineId, float] = {}
        self._engine_ttl: float = aphrodite_config.kv_transfer_config.get_from_extra_config("engine_ttl", 3600.0)
        ...
        self.attn_backends = get_current_attn_backends(aphrodite_config)
        self.backend_name = self.attn_backends[0].get_name()

        self.kv_cache_layout = aphrodite_config.cache_config.get_resolved_kv_cache_layout().name
        ...
        self._group_spec_types = tuple(
            get_representative_spec_type(g.kv_cache_spec) for g in self.kv_cache_config.transfer_groups
        )
    ...
    def _validate_remote_parallel_config(self, agent_metadata: NixlAgentMetadata) -> None:
        ...
        if (local_pcp_size > 1 and remote_dcp_size > 1) or (remote_pcp_size > 1 and local_dcp_size > 1):
            raise NotImplementedError(
    ...
    def _nixl_handshake(
        ...
        assert self.transfer_topo is not None
        p_remote_ranks = self.transfer_topo.handshake_target_ranks(remote_tp_size, remote_dcp_size)
        remote_rank_to_agent_name: dict[tuple[int, int], str] = {}
        path = make_zmq_path("tcp", host, port)
        # Clock offset to the peer, estimated from the handshake round-trip.
        # Keep the lowest-RTT sample: hop cost is roughly uniform across ranks,
        # so a higher RTT is just noise that skews the midpoint estimate.
        best_rtt = float("inf")
        best_offset: float | None = None

        with zmq_ctx(zmq.REQ, path) as sock:
            for remote_pp_rank, remote_rank in itertools.product(range(remote_pp_size), p_remote_ranks):
                logger.debug(
                ...
                msg = msgspec.msgpack.encode((GET_META_MSG, remote_pp_rank, remote_rank))
                ...
                if self.enforce_compat_hash and handshake_payload.compatibility_hash != self.compat_hash:
                    raise RuntimeError(
                        ...
                        f"configurations. This may be due to: different Aphrodite versions,"
                ...
                if notif_agents_only:
                    remote_agent_name = self._add_notif_only_remote_agent(metadata, remote_tp_size, metadata.dcp_size)
                else:
                    remote_agent_name = self.add_remote_agent(metadata, remote_rank, remote_tp_size, metadata.dcp_size)
    ...
    def _add_notif_only_remote_agent(
        ...
        self.transfer_topo.register_remote_engine(
            ...
            EngineTransferInfo(
                ...
                remote_physical_blocks_per_logical=(metadata.physical_blocks_per_logical_kv_block),
            ...
            context.update(
                {
                    ...
                    "local_block_ids_sample": meta.local_block_ids[0][:10] if meta.local_block_ids else [],
                }
            )
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        self.transfer_topo = TransferTopology(
            ...
            engine_id=self.engine_id,
            is_mla=self.use_mla,
            total_num_kv_heads=1 if self.use_mla else self.model_config.get_total_num_kv_heads(),
        ...
        if self._is_csa_linear and self.use_host_buffer:
            raise NotImplementedError("NIXL host staging does not preserve CSA-linear shared tensors.")
        ...
        for layer_name, cache in xfer_buffers.items():
            ...
            if _uses_dense_virtual_transfer_pages(layer_spec, cache, physical_page_size, num_blocks):
        ...
        for layer_name, cache in xfer_buffers.items():
            ...
            if isinstance(layer_spec, KpoolTailSpec):
                ...
                if compressed_owner is not None:
                    ...
                    logger.debug(
                        "Skipping layer %s because its compressed indexer region covers the same storage",
                        layer_name,
                    )
                    continue
            ...
            if isinstance(layer_spec, MambaSpec):
                ...
                region_specs = [(cache.data_ptr(), block_len, block_stride // physical_ratio)]
            else:
                if cache.ndim == 1:
                ...
                if virtual_transfer_pages:
                    ...
                    region_specs = [(cache.data_ptr(), physical_page_size, physical_page_size)]
                elif storage_is_block_major and (
                    (packed_storage and is_mla_region) or (not hnc_contiguous and not self._is_csa_linear)
                ):
                    # Packed MLA layouts transfer the complete storage row.
                    storage_block_len = storage.nbytes() // num_blocks
                    region_specs = [(storage_addr, storage_block_len, storage_block_len)]
                elif storage_is_block_major:
                    region_specs = [(cache.data_ptr(), physical_page_size, block_stride)]
                else:
                    ...
                    region_block_len = block_stride if num_segments > 1 else physical_page_size
                    region_specs = [
                        (
            ...
            for base_addr, block_len, block_stride in region_specs:
                ...
                elif isinstance(layer_spec, CircularBufferSpec) and region_index not in self._scratch_region_indices:
                    self._scratch_region_indices.append(region_index)

            # When there's a mismatch between kbs<>bs, we rely on HMA
            # to ensure caches are either [NB, PS] or [NB*r, PS/r]
            # where r is bs/kbs.
        ...
        self.src_xfer_handles_by_block_size[self.block_size], self.src_blocks_data = self.register_local_xfer_handler(
            self.block_size
        )
    ...
    def _build_mamba_local(self, base_addresses: list[int]) -> np.ndarray:
        ...
        if (region_index := self._ple_region_index) is not None:
            ...
            block_stride = self.block_stride_per_layer[region_index] * physical_per_logical
    ...
    def _build_mamba_remote(
        ...
        assert nixl_agent_meta.kv_caches_base_addr, "Remote KV cache base addresses must not be empty."
        assert self._conv_decomp is not None
        ...
        region_indices = self._ssm_region_indices or range(len(nixl_agent_meta.kv_caches_base_addr))
        for i in region_indices:
            base_addr = nixl_agent_meta.kv_caches_base_addr[i]
            block_stride = nixl_agent_meta.block_strides[i] * remote_physical_per_logical
            blk_addrs = base_addr + block_arange * block_stride
            for off, sz in conv_offsets:
                parts.append(self._stack_descs(blk_addrs + off, sz, device_id))
        ...
        if (region_index := self._ple_region_index) is not None:
            local_block_len = self.block_len_per_layer[region_index] * self._physical_blocks_per_logical_kv_block
            remote_block_len = nixl_agent_meta.block_lens[region_index] * remote_physical_per_logical
            if local_block_len != remote_block_len:
                raise ValueError(
                    f"PLE pages require identical P/D geometry: local={local_block_len}, remote={remote_block_len}."
                )
            remote_block_stride = nixl_agent_meta.block_strides[region_index] * remote_physical_per_logical
            block_addrs = nixl_agent_meta.kv_caches_base_addr[region_index] + block_arange * remote_block_stride
    ...
    def _build_fa_remote(
        ...
        assert self.transfer_topo is not None
        assert nixl_agent_meta.kv_caches_base_addr, "Remote KV cache base addresses must not be empty."
        fa_group_idx = next(i for i, t in enumerate(self._group_spec_types) if _is_attention_spec(t))
    ...
    def add_remote_agent(
        ...
        if (0, remote_tp_rank) in self._remote_agents.get(engine_id, {}):
            logger.debug(
                "Remote agent with engine_id %s and rank%s already exchanged metadata, skip handshake.",
                engine_id,
                remote_tp_rank,
            )
        ...
        if self.pp_size > 1 and len(nixl_agent_meta.kv_caches_base_addr) > num_local_regions:
            ...
            nixl_agent_meta.kv_caches_base_addr = nixl_agent_meta.kv_caches_base_addr[start:end]
        ...
        self._validate_remote_agent_handshake(nixl_agent_meta, remote_tp_size, remote_dcp_size)
        ...
        if block_size_ratio > 1:
            if remote_block_size not in self.src_xfer_handles_by_block_size:
                handle, blocks_data = self.register_local_xfer_handler(remote_block_size)
    ...
    def _validate_remote_agent_handshake(
        ...
        assert self.dcp_size % remote_dcp_size == 0 or remote_dcp_size % self.dcp_size == 0, (
        ...
        if (
            ...
            and self.aphrodite_config.cache_config.enable_prefix_caching
        ):
            raise RuntimeError(
        ...
        if block_size_ratio != 1:
            ...
            assert not self.use_host_buffer, "Heterogeneous block sizes are not supported with host buffer"
        kv_cache_layout = self.kv_cache_layout if not self.use_host_buffer else self.host_buffer_kv_cache_layout
        if not self.use_mla and nixl_agent_meta.kv_cache_layout != kv_cache_layout:
            if self.kv_transfer_config.enable_permute_local_kv and nixl_agent_meta.kv_cache_layout == "LBHNC":
                logger.info("Remote is LBHNC and local is LBNHC, enabled additional permute on local device KV.")
        ...
        if nixl_agent_meta.attn_backend_name != self.backend_name and self.backend_name in ["CPU_ATTN"]:
            if self._is_hma_required:
                raise RuntimeError("heterogeneous attn post process is not supported with HMA")
            logger.info("[Experimental] CPU_ATTN backend is used, hint heterogeneous attn post process")
        ...
        if self._has_mamba and self.use_mla:
            ...
            for i, local_len in enumerate(self.block_len_per_layer):
                ...
                if replicated:
                    ...
                    assert remote_len == (local_len * remote_heads // local_heads) // block_size_ratio, (
    ...
    def _attention_kv_caches(self) -> list[torch.Tensor]:
        ...
        assert self.device_kv_caches, "_attention_kv_caches accessed before register_kv_caches"
        mamba_layers = {
            name
    ...
    def post_process_device_kv_on_receive(
        ...
        assert self.transfer_topo is not None
        if not convert:
            logger.debug("Post-processing device kv cache on receive by zeroing untransferred blocks.")
            ...
            logger.debug("Post-processing device kv cache on receive by permuting layoutfrom HND to NHD.")
        else:
            logger.debug(
    ...
    def get_finished(self) -> tuple[set[str], set[str]]:
        ...
        if len(done_sending) > 0 or len(done_recving) > 0:
            logger.debug(
                "Rank %s, get_finished: %s requests done sending and %s requests done recving (%s failed)",
                self.tp_rank,
                len(done_sending),
                len(done_recving),
        ...
        for req_id in done_recving:
            ...
            # Post processing for heteroblocksize/layout and clipped blocks.
            ...
            hetero_ppl = remote_info.remote_physical_blocks_per_logical != self._physical_blocks_per_logical_kv_block
            if block_size_ratio > 1 or self.enable_permute_local_kv or hetero_ppl:
                for g, local_group in enumerate(meta.local_physical_block_ids):
                    ...
                    block_ids_for_blocksize_post_process[block_size_ratio].append((local_group, covered_sub_blocks))
            # post processing for heterogeneous attention
            if self.enable_heterogeneous_attn_post_process:
                block_ids_for_heterogeneous_attn_post_process.append(meta.local_physical_block_ids[0])
        for (
            block_size_ratio,
            block_ids_list,
            ...
            convert = not self.use_mla and (block_size_ratio > 1 or self.enable_permute_local_kv)
            self.post_process_device_kv_on_receive(block_size_ratio, block_ids_list, convert)
    ...
    def _handle_heartbeat(self, payload: str) -> None:
        ...
        for req_id in payload.split(","):
            if req_id in self._reqs_to_send:
                ...
                logger.debug(
                    "Heartbeat extended lease for request %s by %ds (old_expiry=%.1f, new_expiry=%.1f)",
                    req_id,
    ...
    def _map_block_ids_for_block_size_ratio(
        ...
        for i, remote_group in enumerate(remote_block_ids):
            ...
            if _is_ssm_spec(self._group_spec_types[i]):
                ...
                continue
            mapped = self.get_mapped_blocks(np.asarray(local_group), block_size_ratio).tolist()
