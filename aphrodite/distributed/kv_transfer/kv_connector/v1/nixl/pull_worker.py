...
class NixlPullConnectorWorker(NixlBaseConnectorWorker):
    ...
    def _read_blocks_for_req(self, req_id: str, meta: ReqMeta):
        ...
        def group_ids(block_ids: BlockIds, rank: int) -> list[list[int]]:
            return [list(block_ids[g]) if rank in plan.source_ranks_per_group[g] else [] for g in range(num_groups)]
    ...
    def _read_blocks(
        ...
        assert len(remote_block_ids) == len(local_block_ids) == len(self.kv_cache_config.transfer_groups)
