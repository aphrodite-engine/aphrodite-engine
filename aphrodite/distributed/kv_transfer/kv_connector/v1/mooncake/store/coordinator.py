...
from aphrodite.v1.core.kv_cache_coordinator import SpecGroup
...
class ExternalCachedBlockPool:
    ...
    def contains(self, group_id: int, block_hash: BlockHash) -> bool:
        ...
        return self._exists is not None and (group_id, bytes(block_hash)) in self._exists
...
class MooncakeStoreCoordinator:
    ...
    def __init__(
        ...
        self.enable_partial_hash_hits = partial_hash_hits_enabled(kv_cache_groups, hash_block_size, dcp_world_size)
    ...
    def _verify_and_split_kv_cache_groups(self) -> None:
        ...
        self.eagle_group_ids = {gid for g in attention_groups if g.use_eagle for gid in g.group_ids}
    ...
    def _reachable_masks(
        ...
        *,
        ...
        mask_alignment = self.hash_block_size if self.enable_partial_hash_hits else self.lcm_block_size
        assert aligned_token_len % mask_alignment == 0, (
            f"aligned_token_len ({aligned_token_len}) must be a multiple of {mask_alignment}"
        )
    ...
    def _find_hit_blocks(
        ...
        *,
        ...
        """
        alignment_tokens = self.hash_block_size if self.enable_partial_hash_hits else self.lcm_block_size
        ...
        while True:
            ...
            for idx, (spec, group_ids, manager_cls, group_eagle) in enumerate(self.attention_groups):
