...
class RankLocalStoreLayout(StoreLayout):
    ...
    def prepare_values(
        ...
        if not chunks:
            return [], [], []
        assert len(chunks) == len(shard_ids) and all(shard_id == 0 for shard_id in shard_ids)
    ...
    def prepare_value_for_block(self, block_id: int) -> tuple[list[int], list[int]]:
        ...
        return (
            ...
            [self.block_len[index % length] for index in range(len(self.kv_caches_base_addr))],
        )
...
class TPShardedStoreLayout(StoreLayout):
    ...
    def shared_namespace(cls, store_tp_size: int, pp_size: int) -> str:
        return f"@store_tp:{store_tp_size}@store_pp:{pp_size}@store_format:{cls.store_format}"
    ...
    def __init__(
        ...
        self.store_shard_ids = tuple(range(first_shard, first_shard + self.shards_per_rank))
        ...
        self._shard_key_prefixes = {
            shard_id: PoolKey.build_prefix(metadata, tp_rank=shard_id) for shard_id in self.store_shard_ids
        }
    ...
    def key_for(self, shard_id: StoreShardId, chunk_hash: BlockHash) -> str:
        return PoolKey.build_key_string(self._shard_key_prefixes[shard_id], chunk_hash.hex())
    ...
    def _set_segment_templates(self, templates: Sequence[tuple[list[int], list[int], list[int]]]) -> None:
        self._shard_addr_bases = np.asarray([template[0] for template in templates], dtype=np.uint64)
        self._shard_block_strides = np.asarray([template[1] for template in templates], dtype=np.uint64)
        self._shard_sizes = np.asarray([template[2] for template in templates], dtype=np.uint64)

    def _has_packed_strides(self, head_stride: int, token_stride: int, content_bytes: int) -> bool:
        raise NotImplementedError
    ...
    def register_kv_caches(
        ...
        templates: list[tuple[list[int], list[int], list[int]]] = [([], [], []) for _ in range(self.shards_per_rank)]
        for cache in kv_caches:
            ...
            if content_stride != element_size or not self._has_packed_strides(head_stride, token_stride, content_bytes):
                raise ValueError(f"TP-shared Mooncake store requires packed {self.store_format} KV layout")

            for local_shard, (addr_bases, block_strides, sizes) in enumerate(templates):
                head_start = local_shard * self.heads_per_store_shard
                for offset, size in self._shard_segments(head_start, head_stride, token_stride, content_bytes):
    ...
    def prepare_values(
        ...
        count = len(chunks)
        starts = np.fromiter((chunk[0] for chunk in chunks), dtype=np.int64, count=count)
...
class LBHNCStoreLayout(TPShardedStoreLayout):
    ...
    def _has_packed_strides(self, head_stride: int, token_stride: int, content_bytes: int) -> bool:
        return token_stride == content_bytes and head_stride == self.block_size * content_bytes
...
class LBNHCStoreLayout(TPShardedStoreLayout):
    ...
    def _has_packed_strides(self, head_stride: int, token_stride: int, content_bytes: int) -> bool:
        return head_stride == content_bytes and token_stride == self.local_num_kv_heads * content_bytes
...
class ChunkedTokenDatabase:
    ...
    def __init__(
        ...
        self.store_layout = store_layout or RankLocalStoreLayout(metadata, block_size, self.hash_block_size)
...
class MooncakeStoreWorkerMetadata(KVConnectorWorkerMetadata):
    ...
    def aggregate(self, other: "KVConnectorWorkerMetadata") -> "MooncakeStoreWorkerMetadata":
        assert isinstance(other, MooncakeStoreWorkerMetadata)
        for store_job_id, count in other.completed_saves.items():
            self.completed_saves[store_job_id] = self.completed_saves.get(store_job_id, 0) + count
        return self
