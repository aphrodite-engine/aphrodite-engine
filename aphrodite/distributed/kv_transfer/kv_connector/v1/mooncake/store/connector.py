...
class MooncakeStoreKVEvents(KVConnectorKVEvents):
    ...
    def pop_common_events(self) -> list[KVCacheEvent]:
        common_events = [event for event, count in self._event_counter.items() if self._is_common_event(event, count)]
...
class MooncakeStoreConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def _validate_kv_cache_config(aphrodite_config: AphroditeConfig, kv_cache_config: KVCacheConfig) -> None:
        ...
        cache_block_size = aphrodite_config.cache_config.block_size
    ...
    def __init__(
        ...
        self._capacity_only = (
            self.kv_role == "kv_consumer" and not extra_config.get("enable_lookup", True) and not save_decode_cache
        )
        if not self._capacity_only:
            self._validate_kv_cache_config(aphrodite_config, kv_cache_config)
    ...
    def register_finished_partial_tail(
        ...
        assert self.connector_scheduler is not None
        return self.connector_scheduler.register_finished_partial_tail(request, block_ids, partial_tail_offloads)
    ...
    def get_kv_connector_kv_cache_events(
        ...
        assert self.connector_worker is not None
        if not self.connector_worker.enable_kv_events or self.connector_worker.kv_send_thread is None:
            return None
        ...
        kv_events = MooncakeStoreKVEvents(
            num_workers=1,
            group_tp_replication_factors=(self.connector_worker.group_tp_replication_factors),
        )
