...
#
# Adapted from https://github.com/vllm-project/vllm-ascend
...
from aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.store.worker import (  # noqa: E501
    LookupKeyClient,
)
...
from aphrodite.v1.core.sched.output import NewRequestData, SchedulerOutput
...
class MooncakeStoreScheduler:
    ...
    def __init__(
        ...
        kvc_extra_config = aphrodite_config.kv_transfer_config.kv_connector_extra_config
        ...
        self.client = LookupKeyClient(aphrodite_config)
        self.kv_cache_config = kv_cache_config

        # Align with the engine's own scheduler_block_size and hash_block_size.
        self._block_size, self._hash_block_size = resolve_kv_cache_block_sizes(kv_cache_config, aphrodite_config)
        ...
        assert all(spec.mamba_cache_mode == "align" for spec in mamba_groups.values()), (
            "MooncakeStoreScheduler requires mamba_cache_mode='align'"
        )
        self._boundary_state_group_ids = frozenset(mamba_groups)

        self._gpu_block_pool: BlockPool | None = None
        self._num_workers = aphrodite_config.parallel_config.world_size
    ...
    def get_num_new_matched_tokens(
        ...
        align = self._hash_block_size if self.enable_partial_hash_hits else self._block_size
    ...
    def update_state_after_alloc(
        ...
        if num_external_tokens > 0:
            local_block_ids = self.kv_cache_config.select_transfer_block_ids(blocks.get_block_ids())
    ...
    def build_connector_meta(self, scheduler_output: SchedulerOutput) -> KVConnectorMetadata:
        ...
        for request in scheduler_output.scheduled_new_reqs:
            ...
            unfolded_block_ids = tuple(
                blocks.copy() for blocks in self.kv_cache_config.select_transfer_block_ids(request.block_ids)
            )
            ...
            request_tracker = RequestTracker(
                ...
                num_saved_tokens=0,
                token_ids=(prefill_tokens[:num_tokens_to_compute] if self.enable_kv_events else None),
        ...
        if can_process_cached:
            for i, req_id in enumerate(cached_reqs.req_ids):
                ...
                if new_block_ids:
                    new_block_ids = self.kv_cache_config.select_transfer_block_ids(new_block_ids)
        ...
        if block_state is not None and block_state.boundary_state_offloads and not is_consumer:
            self._handle_boundary_state_offloads(block_state.boundary_state_offloads, meta)
    ...
    def _apply_current_save_block_ids(
        ...
        assert block_state is not None, "Current block tables are required for Mooncake store jobs"
        for req_meta in save_metas:
            block_ids = block_state.block_ids.get(req_meta.req_id)
            assert block_ids is not None, f"Missing current block table for store request {req_meta.req_id}"
    ...
    def _reference_save_blocks(self, meta: MooncakeStoreConnectorMetadata) -> None:
        ...
        for req_meta in meta.requests:
            if not req_meta.can_save:
                continue
            assert pool is not None, "GPU block pool must be bound before any store job is emitted"
            ...
            if req_meta.boundary_state_offloads:
                block_ids.extend(block_id for _, block_id, _ in req_meta.boundary_state_offloads)
            assert NULL_BLOCK_ID not in block_ids, "A null block cannot back a boundary-state offload"
    ...
    def register_finished_partial_tail(
        ...
        pool = self._gpu_block_pool
        assert pool is not None, "GPU block pool must be bound before a finish-time handoff"
    ...
    def update_connector_output(self, connector_output: KVConnectorOutput) -> None:
        ...
        for store_job_id, count in meta.completed_saves.items():
            ...
            if remaining > 0:
                ...
                continue
            assert remaining == 0, f"store job {store_job_id} reported by too many ranks"
