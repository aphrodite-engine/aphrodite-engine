...
# KVCacheStoreRecvingThread) is adapted from https://github.com/vllm-project/vllm-ascend
...
from aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake import rdma_utils
from aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.store.coordinator import (  # noqa: E501
    ExternalCachedBlockPool,
    MooncakeStoreCoordinator,
)
from aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.store.data import (  # noqa: E501
    BlobBlockHashes,
...
from aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.store.protocol import (  # noqa: E501
    LOOKUP_MSG,
    RESET_MSG,
    RESP_ERR,
    RESP_OK,
...
def resolve_store_tp_size(extra_config: dict[str, Any]) -> int | None:
    ...
    if extra_config.get("enable_store_tp_lcm") is True:
        ...
        if not isinstance(prefill_tp_sizes, list) or not prefill_tp_sizes:
            return None
        if any(type(tp_size) is not int or tp_size <= 0 for tp_size in prefill_tp_sizes):
            return None
...
def _make_mooncake_group_id(metadata: KeyMetadata, chunk_hash: str) -> str:
    # Mooncake group ids describe the lifecycle unit. For Aphrodite, that unit is
    ...
    return f"aphrodite-mooncake-store:{prefix}{metadata.model_name}{metadata.store_namespace}@{chunk_hash}"
...
def _get_usable_disk_offload_buffer_budget_bytes(raw_budget_bytes: int) -> int:
    return max(1, int(raw_budget_bytes * envs.APHRODITE_MOONCAKE_DISK_STAGING_USABLE_RATIO))
...
class KVCacheStoreSendingThread(KVTransferThread):
    ...
    def __init__(
        ...
        self.group_participates = (
            list(group_participates) if group_participates is not None else [True] * len(token_databases)
        )
    ...
    def add_request(self, request: ReqMeta) -> None:
        ...
        with self.done_task_lock:
            self.stored_requests.setdefault(request.req_id, set()).add(request.store_job_id)
    ...
    def finish_store_job(self, req_meta: ReqMeta) -> None:
        ...
        assert store_job_id is not None, "a queued store job always carries a store_job_id"
        with self.done_task_lock:
            live = self.stored_requests.get(req_meta.req_id)
            if live is not None:
                live.discard(store_job_id)
            self._completed_saves[store_job_id] = self._completed_saves.get(store_job_id, 0) + 1
    ...
    def _boundary_snapshot_puts(
        ...
        for group_id, block_id, boundary in entries:
            ...
            puts.append((db.key_for(req_meta.block_hashes[hash_idx]), addr, size, db.metadata))
        return puts
    ...
    def _sub_block_tail_puts(
        ...
        if len(boundaries) != 1:
            raise ValueError("Sub-block partial-tail offloads for one request must share a boundary")
        boundary = boundaries.pop()
        hash_block_size = self.coord.hash_block_size
        if boundary == 0 or boundary // hash_block_size - 1 >= len(req_meta.block_hashes):
            return []
        ...
        for g_idx, db in enumerate(self.token_databases):
            ...
            for block_idx in range(min(saved // db.block_size, last_block), last_block + 1):
                ...
                if block_id == NULL_BLOCK_ID:
                    logger.debug(
                        "Skipping unavailable partial-tail source block (req=%s, group=%d, block=%d)",
                        req_meta.req_id,
                        g_idx,
                        block_idx,
                    )
    ...
    def _maybe_offload_boundary_states(self, req_meta: ReqMeta) -> bool:
        ...
        group_ids: list[str] | None = (
            [_make_mooncake_group_id(metadata, key.rsplit("@", 1)[-1]) for key, _, _, metadata in puts]
        ...
        if failed:
            ...
            logger.warning(
                "Boundary-state put failed for request %s: %d/%d keys failed (codes=%s)",
                req_meta.req_id,
                len(failed),
                len(keys),
                failed_codes,
        ...
        if self._clear_store_pressure():
            logger.info("Mooncake CPU/disk offloading pressure cleared after a successful boundary-state batch")
        return True
    ...
    def _handle_request(self, req_meta: ReqMeta):
        ...
        try:
            ...
            if req_meta.boundary_state_offloads is not None and not (self._maybe_offload_boundary_states(req_meta)):
                return
            ...
            ends: list[int] = []
            keys: list[str] = []
            event_specs: list[tuple[int, int, int, BlockHash]] | None = [] if self.enable_kv_event else None
            ...
            for g_idx, db in enumerate(self.token_databases):
                ...
                for start, end, block_hash in db.process_tokens(
                    ...
                    group_blocks = block_ids_per_group[g_idx]
                    if block_idx >= len(group_blocks) or (group_blocks[block_idx] == NULL_BLOCK_ID):
                        logger.debug(
                            "Skipping unavailable Mooncake store source block (req=%s, group=%d, block=%d)",
                            req_id,
                            g_idx,
                            block_idx,
                        )
            ...
            addrs: list[list[int]] = []
            sizes: list[list[int]] = []
            chunks_per_group: list[list[tuple[int, int]]] = [[] for _ in self.token_databases]
            shards_per_group: list[list[StoreShardId]] = [[] for _ in self.token_databases]
            ...
            try:
                ...
                if failed_indices:
                    logger.warning(
                        "batch_put failed: %d/%d keys failed (codes=%s, batch_bytes=%d), first_key=%s",
                        len(failed_indices),
                        len(keys),
                        failed_codes,
                        batch_bytes,
            ...
            if self.enable_kv_event and not put_had_exception:
                ...
                for (
                    ...
                    token_ids = (
                        ...
                        if event_token_ids is not None and token_ids_start <= s and end <= token_ids_end
                        else []
                    )
                    stored_events.append(
                        BlockStored(
                            ...
                            parent_block_hash=(
                                maybe_convert_block_hash(req_meta.block_hashes[s // db.hash_block_size - 1])
                                if s > 0
                                else None
                            ),
                            token_ids=token_ids,
...
class KVCacheStoreRecvingThread(KVTransferThread):
    ...
    def _handle_request(self, req_meta: ReqMeta):
        ...
        req_id = req_meta.req_id
        mask_num = (
            req_meta.load_spec.aphrodite_cached_tokens  # type: ignore[union-attr]
            // self.block_size
            * self.block_size
        )
        ...
        for g_idx, db in enumerate(self.token_databases):
            ...
            for start, end, block_hash in db.process_tokens(token_len, req_meta.block_hashes, mask_num):
                ...
                if chunk_idx >= len(mask) or not mask[chunk_idx]:
                    continue
                boundary_tokens = tail_key_boundaries.get(g_idx) if end == token_len else None
                if boundary_tokens is not None:
                    block_hash = req_meta.block_hashes[boundary_tokens // db.hash_block_size - 1]
                for store_shard_id in db.store_layout.local_shard_ids:
        ...
        try:
            for (
                ...
                if envs.APHRODITE_MOONCAKE_STORE_TIER_LOG:
...
class MooncakeStoreWorker:
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                ...
                "en/build.md to run Aphrodite with MooncakeStoreConnector."
            ) from e
        ...
        kv_role = aphrodite_config.kv_transfer_config.kv_role
        assert kv_role is not None
        self.kv_role = kv_role
        extra_config = aphrodite_config.kv_transfer_config.kv_connector_extra_config
        self.can_put = self.kv_role in ("kv_producer", "kv_both") or (extra_config.get("save_decode_cache", False))
        self.load_async = extra_config.get("load_async", True)
        # Mirrors MooncakeStoreConnector._capacity_only.
        self._capacity_only = (
            self.kv_role == "kv_consumer" and not extra_config.get("enable_lookup", True) and not self.can_put
        )
        ...
        self.enable_group_semantics = str(extra_config.get("enable_group_semantics", "False")).strip().lower() == "true"
        self._supports_group_ids = _replicate_config_supports_group_ids(ReplicateConfig, self.store_replicate_config)
        ...
        if aphrodite_config.parallel_config.rank == 0:
            self.lookup_server = LookupKeyServer(self, aphrodite_config)

        kv_event_config = aphrodite_config.kv_events_config
        ...
        self.num_recv_threads = max(1, envs.APHRODITE_MOONCAKE_LOAD_RECV_THREADS)
        ...
        if self._capacity_only:
            logger.info(
                "Mooncake store in capacity-only mode: segment mounted (global_segment_size=%d), KV transfer disabled.",
                store_config.global_segment_size,
            )
            return
        ...
        self._kv_cache_groups = [
            dataclasses.replace(
                group,
                kv_cache_spec=resolve_dcp_kv_cache_spec(group.kv_cache_spec, self.dcp_size),
            )
        ...
        use_eagle_block_drop = bool(
            ...
            if spec_cfg is not None and callable(getattr(spec_cfg, "use_eagle_block_drop", None))
            else False
        )
        ...
        self.store_tp_size, store_namespace, store_layout_cls = self._select_store_layout(extra_config)
        ...
        self._group_tp_replication_factors: tuple[int, ...] = self._compute_group_tp_replication_factors()
    ...
    def _supports_tp_sharded_store_layout(
        ...
        if layout_cls is None or self.pcp_size != 1 or self.dcp_size != 1 or len(self._kv_cache_groups) != 1:
            return False
        ...
        return (
            ...
            and str(extra_config.get("enable_cross_layers_blocks", "False")).lower() != "true"
        )
    ...
    def _select_store_layout(
        ...
        store_tp_requested = lcm_store_tp_enabled or extra_config.get("store_tp_size") is not None
        ...
        if (
            ...
            if self.num_kv_head == 1:
                logger.info(
                    "Mooncake heterogeneous-TP store sharing uses the replicated MQA layout for store_tp_size=%d",
                    requested_store_tp_size,
                )
                return (
                    None,
        ...
        store_namespace = (
            f"@store_pp:{self.pp_size}@store_format:rank_local_tp{self.tp_size}_layout_{cache_layout.name}"
        )
        requested_topology = (
            extra_config.get("prefill_tp_sizes") if lcm_store_tp_enabled else extra_config.get("store_tp_size")
        )
        logger.warning(
    ...
    def _build_token_databases(
        ...
        for group_idx, group in enumerate(self._kv_cache_groups):
            hash_block_size = (
                self.hash_block_size if group.kv_cache_spec.prefix_cacheable else group.kv_cache_spec.block_size
            )
    ...
    def _spec_tp_replication_factor(self, spec: KVCacheSpec) -> int:
        if self.dcp_size > 1:
            return 1
        inner_specs = tuple(spec.kv_cache_specs.values()) if isinstance(spec, UniformTypeKVCacheSpecs) else (spec,)
        if any(isinstance(inner, MambaSpec) for inner in inner_specs):
            return 1
        if all(isinstance(inner, (MLAAttentionSpec, SlidingWindowMLASpec)) for inner in inner_specs):
            return self.tp_size
    ...
    def _compute_group_tp_replication_factors(self) -> tuple[int, ...]:
        ...
        ``tp_size // num_kv_head``.
        """
        return tuple(self._spec_tp_replication_factor(group.kv_cache_spec) for group in self._kv_cache_groups)
    ...
    def _init_lookup_key_prefixes(self) -> None:
        ...
        self._lookup_key_prefixes = tuple(
            db.store_layout.lookup_key_prefixes(rank_namespaces(self._group_tp_replication_factors[g_idx]))
    ...
    def register_kv_caches(
        ...
        if self.can_put:
            ...
            self.kv_send_thread = KVCacheStoreSendingThread(
                ...
                group_participates=[group.kv_cache_spec.prefix_cacheable for group in self._kv_cache_groups],
            )
    ...
    def lookup(self, num_tokens: int, block_hashes: Sequence[BlockHash]) -> MooncakeLookupResult:
        ...
        for g_idx, db in enumerate(self.token_dbs):
            ...
            if fine_grained:
                ...
                assert lookup_masks is not None
                ...
                unit_ids = [chunk_id for chunk_id in range(mask_limit) if lookup_mask is None or lookup_mask[chunk_id]]
    ...
    def _tail_key_boundaries(
        ...
        for group_id, db in enumerate(self.token_dbs):
            ...
            contains_hit_boundary = cached_block_pool.contains(group_id, block_hashes[hit_boundary_hash_idx])
            ...
            if not contains_hit_boundary:
                ...
                for hash_idx in range(hit_boundary_hash_idx + 1, next_chunk_hash_idx):
                    ...
                    raise AssertionError(f"No tail key found for cache group {group_id} at hit length {hit_length}")
...
class LookupKeyServer:
    ...
    def __init__(
        ...
        socket_path = get_zmq_rpc_path_lookup(aphrodite_config)
...
class LookupKeyClient:
    ...
    def _lookup(self, num_tokens: int, block_hashes: list[BlockHash]) -> MooncakeLookupResult:
...
def get_zmq_rpc_path_lookup(aphrodite_config: AphroditeConfig) -> str:
    """Construct IPC path for ZMQ lookup socket."""
    assert aphrodite_config.kv_transfer_config is not None
    dp_rank = aphrodite_config.parallel_config.data_parallel_index
