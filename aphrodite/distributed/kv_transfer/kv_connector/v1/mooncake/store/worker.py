...
class KVCacheStoreSendingThread(KVTransferThread):
    ...
    def _maybe_offload_partial_tail(self, req_meta: ReqMeta) -> bool:
        ...
        if len(boundaries) != 1:
            raise ValueError("Partial-tail offloads for one request must share a boundary")
        boundary = boundaries.pop()
        if boundary == 0:
            return True
        if boundary // hash_block_size - 1 >= len(req_meta.block_hashes):
            return True
        mamba_offloads = {group_id: block_id for group_id, block_id, _ in partial_tail_offloads}
        ...
        for g_idx, db in enumerate(self.token_databases):
            ...
            for block_idx in range(min(saved // db.block_size, last_block), last_block + 1):
                ...
                if g_idx in mamba_offloads and valid_end == boundary and boundary % db.block_size != 0:
        ...
        if self._clear_store_pressure():
            logger.info("Mooncake CPU/disk offloading pressure cleared after a successful partial-tail batch")
        return True
    ...
    def _handle_request(self, req_meta: ReqMeta):
        ...
        try:
            ...
            if req_meta.partial_tail_offloads is not None and not (self._maybe_offload_partial_tail(req_meta)):
                return
            ...
            for g_idx, chunks in enumerate(chunks_per_group):
                ...
                group_addrs, group_sizes, _ = db.prepare_values(chunks, block_ids_per_group[g_idx])
...
class KVCacheStoreRecvingThread(KVTransferThread):
    ...
    def _handle_request(self, req_meta: ReqMeta):
        ...
        for g_idx, db in enumerate(self.token_databases):
            ...
            g_addrs, g_sizes, g_block_ids = db.prepare_values(chunks, req_meta.block_ids[g_idx])
...
class MooncakeStoreWorker:
    ...
    def __init__(
        ...
        self.coord = MooncakeStoreCoordinator(
            ...
            use_eagle=use_eagle,
            retention_interval=envs.APHRODITE_PREFIX_CACHE_RETENTION_INTERVAL,
        )
    ...
    def _init_lookup_key_prefixes(self) -> None:
        ...
        self._lookup_key_prefixes = tuple(
            tuple(
                ...
                for tp_rank, pcp_rank, dcp_rank, pp_rank in rank_namespaces(self._group_tp_replication_factors[g_idx])
            )
...
def get_zmq_rpc_path_lookup(aphrodite_config: AphroditeConfig) -> str:
    ...
    assert aphrodite_config.kv_transfer_config is not None
    dp_rank = get_mooncake_dp_engine_index(aphrodite_config.parallel_config)
