...
from aphrodite.distributed.kv_transfer.kv_connector.v1.mooncake.stats import (
...
from aphrodite.utils.torch_utils import is_non_overlapping_and_dense
...
try:
    ...
    logger.warning(
        ...
        "to run APHRODITE with MooncakeTransferEngine."
    )
    TransferEngine = None
...
def _expand_transfer_regions(
    ...
    assert len(base_addrs) == len(block_lens) == len(kv_block_lens) == len(layer_names) == len(layer_indices), (
...
class MooncakeConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def get_required_kvcache_layout(cls, aphrodite_config: AphroditeConfig):
        ...
        if aphrodite_config.model_config.use_mla:
            return None
        logger.info_once("MooncakeConnector setting KV cache layout to LBHNC for heterogeneous TP-safe KV transfer.")
        return "LBHNC"
...
class MooncakeConnectorScheduler:
    ...
    def __init__(
        ...
        # GDN is represented as a MambaSpec in Aphrodite. This Mooncake MambaSpec
    ...
    def _truncate_mamba_request_for_prefill(self, request: "Request") -> None:
        ...
        if params is not None and not params.get("_p_side_truncated") and request.num_prompt_tokens > 1:
...
class MooncakeConnectorWorker:
    ...
    def __init__(
        ...
        assert (parallel_config := aphrodite_config.parallel_config)
        ...
        if not self.is_kv_consumer:
            ...
            self._sender_executor = ThreadPoolExecutor(
                ...
                thread_name_prefix="aphrodite-mooncake-sender",
            ...
            if should_launch_bootstrap_server(aphrodite_config):
                _, port = get_mooncake_bootstrap_addr(aphrodite_config)
        ...
        for group in kv_cache_config.kv_cache_groups:
            ...
            for layer_name in group.layer_names:
                self._layer_specs[layer_name] = specs_by_layer.get(layer_name, group_spec)
    ...
    async def register_worker_with_bootstrap(self):
        host, port = get_mooncake_bootstrap_addr(self.aphrodite_config)
    ...
    async def send_kv_to_decode(self, identity: bytes, sock: zmq.asyncio.Socket, meta: MooncakeXferMetadata):
        ...
        local_regions, remote_regions, align_err = _align_transfer_regions(local_regions, remote_regions)
        ...
        while wait_tasks:
                ...
                wait_tasks,
                timeout=envs.APHRODITE_MOONCAKE_ABORT_REQUEST_TIMEOUT,
    ...
    def _logical_to_kernel_block_ids(self, block_ids: list[list[int]]) -> list[list[int]]:
        ...
        block_arange = np.arange(self._physical_blocks_per_logical_kv_block).reshape(1, -1)
        ...
        for d_req_id, send_meta in ready_reqs:
            ...
            for group_index, (local_group, remote_group) in enumerate(
                ...
                if is_mamba_group:
                    ...
                    local_group = [block_id for block_id in local_group if block_id != NULL_BLOCK_ID]
                    remote_group = [block_id for block_id in remote_group if block_id != NULL_BLOCK_ID]

                n_local = len(local_group)
                n_remote = len(remote_group)
                if n_local < n_remote:
                    logger.error(
                        "req %s: local blocks(%d) < remote blocks(%d) in a KV cache group (is_mamba_group=%s)",
                        d_req_id,
                        n_local,
                        n_remote,
                        is_mamba_group,
            ...
            remote_block_ids_by_group = self._logical_to_kernel_block_ids(remote_block_ids_by_group)
            ...
            for local_region, remote_region in zip(local_regions, remote_regions):
                ...
                assert group_index < len(local_block_ids_by_group), "Transfer region references a missing KV group."
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        for layer_name, cache in kv_caches.items():
            ...
            if not block_is_contiguous:
                ...
                assert all(is_non_overlapping_and_dense(region[0]) for region in region_caches)
            else:
                region_caches = [cache]
            ...
            for region_cache in region_caches:
                ...
                if isinstance(layer_spec, KpoolTailSpec):
                    ...
                    assert layer_spec.page_size_bytes % self._physical_blocks_per_logical_kv_block == 0
                    kv_block_len = layer_spec.page_size_bytes // self._physical_blocks_per_logical_kv_block
                else:
                ...
                self.registered_group_indices.append(self._layer_group_indices[layer_name])
        ...
        for transfer_id, send_meta in self.reqs_need_send.items():
            if send_meta.p_req_id and send_meta.expire_time < now and send_meta.sending == 0:
                logger.warning(
                    ...
                    send_meta.p_req_id,
                    envs.APHRODITE_MOONCAKE_ABORT_REQUEST_TIMEOUT,
                )
    ...
    def receive_kv(
        ...
        logger.debug(
            "Receiving Mooncake KV for engine %s from producer TP ranks %s and PP ranks %s",
            remote_engine_id,
            remote_tp_ranks,
            selected_remote_pp,
        )
    ...
    def _get_transfer_regions(
        ...
        if not group_indices:
            group_indices = [self._layer_group_indices.get(layer_name, 0) for layer_name in layer_names]
...
def get_mooncake_side_channel_port(aphrodite_config: AphroditeConfig) -> int:
...
def should_launch_bootstrap_server(aphrodite_config: AphroditeConfig) -> bool:
    assert (parallel_config := aphrodite_config.parallel_config)
...
def get_mooncake_bootstrap_addr(aphrodite_config: AphroditeConfig) -> tuple[str, int]:
    ...
    port = envs.APHRODITE_MOONCAKE_BOOTSTRAP_PORT
    return (host, port)
