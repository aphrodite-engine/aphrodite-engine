...
def kv_cache_scatter_kernel(
    ...
    for i in range(0, hidden_size, BLOCK_SIZE):
        ...
        target_offset = (
            block_idx * block_stride + head_idx * head_stride + state_idx * token_stride + content_idx * content_stride
        )
...
def kv_cache_gather_kernel(
    ...
    for i in range(0, hidden_size, BLOCK_SIZE):
        ...
        dst_offset = ((layer_idx * num_heads + head_idx) * num_tokens_in_block + token_pos) * content_size + content_idx
...
def scatter_kv_caches(
    ...
    token_indices_tensor = async_tensor_h2d(token_indices, device=device, dtype=torch.int32)
