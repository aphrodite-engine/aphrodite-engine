...
from aphrodite.distributed.kv_transfer.kv_connector.v1.moriio.moriio_engine import (
    MoRIIOWrapper,
    MoRIIOWriter,
)
from aphrodite.distributed.kv_transfer.kv_connector.v1.moriio.moriio_layout import (
...
class MoRIIOConnector(KVConnectorBase_V1):
    def __init__(
        ...
        if (
            ...
            and aphrodite_config.compilation_config.cudagraph_mode.has_full_cudagraphs()
        ):
            ...
            logger.warning_once(
                ...
                aphrodite_config.compilation_config.cudagraph_mode.name,
            )
...
class MoRIIOConnectorScheduler:
    ...
    def __init__(self, aphrodite_config: AphroditeConfig, engine_id: str):
        ...
        _pc = self.aphrodite_config.parallel_config
        self.dp_rank = fold_local_rank(_pc.data_parallel_rank, _pc.data_parallel_size_local)
        ...
        self._is_kv_master = pod_index(_pc.data_parallel_rank, _pc.data_parallel_size_local) == 0
        # Global DP rank for pinned request ownership check.
        self._global_dp_rank = self.aphrodite_config.parallel_config.data_parallel_rank
    ...
    def unmap_request_id(self, request_id: ReqId, transfer_id: TransferId | None = None):
        ...
        if transfer_id is not None and transfer_id in self.transfer_id_to_request_id:
            ...
            if original_rid != request_id:
                logger.debug("MoRI-IO unmap via transfer_id: %r -> %r", request_id, original_rid)
    ...
    def update_state_after_alloc(
        ...
        request hangs until ``APHRODITE_MORIIO_DEFERRED_TIMEOUT_S``.
        ...
        ways: the ``X-data-parallel-rank`` dispatch header (which Aphrodite engine
        ...
        if params is not None and params.get("do_remote_prefill"):
            if self.mode == MoRIIOMode.READ:
                ...
                try:
                    _dp_local = int(request.kv_transfer_params.get("remote_dp_size_local", 0) or 0)
                    if _dp_local > 0:
                ...
                if _should_notify:
                    ...
                    if peer_zmq is not None:
                        ...
                        try:
                            remote_notify_port = int(params.get("remote_notify_port") or 0)
                        except (TypeError, ValueError):
                    ...
                    block_notify_list = blocks.get_block_ids()[0] if num_external_tokens > 0 else []
                    ...
                    _remote_dp_rank_for_port = fold_local_rank(remote_dp_rank, _dp_local)
                    ...
                    for tp_index in range(self.tp_size):
                        target_port = remote_notify_port + get_port_offset(_remote_dp_rank_for_port, tp_index)
            ...
            for i, req_id in enumerate(scheduler_output.scheduled_cached_reqs.req_ids):
                ...
                if new_block_ids is not None:
                    ...
                    if len(self._reqs_need_pending_save[req_id][1]) * self.block_size >= req.num_prompt_tokens:
                        ...
                        kv_params = self._req_kv_params.pop(req_id, req.kv_transfer_params or {})
                        meta.add_new_req(
                            request_id=req_id,
    ...
    def request_finished(
        ...
        return delay_free_blocks, dict(
            ...
            remote_dp_size=self.aphrodite_config.parallel_config.data_parallel_size,
            remote_dp_size_local=(self.aphrodite_config.parallel_config.data_parallel_size_local),
    ...
    def update_connector_output(self, connector_output: KVConnectorOutput) -> None:
        ...
        * A parked ACK that never matches a deferral before its own
        ...
        expired = [req_id for req_id, (deadline, _) in self._deferred_send_deadlines.items() if now >= deadline]
        if expired:
            safe.update(expired)
            logger.warning(
        ...
        stale = [req_id for req_id, deadline in self._pending_sent_acks.items() if now >= deadline]
        for req_id in stale:
            self._pending_sent_acks.pop(req_id, None)
...
class MoRIIOConnectorWorker:
    ...
    def __init__(
        ...
        self._handshake_initiation_executor = ThreadPoolExecutor(
            ...
            max_workers=1,
            thread_name_prefix="aphrodite-moriio-handshake-initiator",
        )
            ...
            while True:
                try:
                    data = {
                        ...
                        # dp_size/tp_size are not used by the official Aphrodite router
    ...
    def _build_shared_kv_mr(self, kv_caches: dict[str, torch.Tensor]) -> tuple[torch.Tensor, dict[str, int]]:
        page_size = 4096
        ...
        if reg_nbytes > nbytes:
            raise ValueError(f"shared KV backing too small for page-aligned MR: need {reg_nbytes}, have {nbytes}")
        reg = torch.as_strided(base, (reg_nbytes,), (1,), 0 if slack == 0 else -slack)
        mr_ptr = reg.data_ptr()
        return reg, {name: cache.data_ptr() - mr_ptr for name, cache in kv_caches.items()}
    ...
    def _remote_layer_mr_offset(
        ...
        metas = self.layer_name_to_remote_kv_cache_metadata.get(remote_engine_id, {}).get(layer_name)
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        if first_geometry.block_size != self.block_size:
            ...
            # larger) block_size than aphrodite_config.cache_config.block_size.
        ...
        for layer_name, kv_cache in kv_caches.items():
            ...
            moriio_mem_metadata = shared_mr or (self.moriio_wrapper.register_local_tensor(kv_cache))
    ...
    def wait_for_layer_load(self, layer_name: str) -> None:
        ...
        while True:
            ...
            if time.monotonic() > deadline:
                logger.warning(
                    "MoRIIO READ barrier timed out for layer %s; proceeding (request dropped via get_finished).",
                    layer_name,
                )
                return
    ...
    def _pop_done_transfers(self) -> set[str]:
        ...
        _xfer_timeout = int(os.environ.get("APHRODITE_MORIIO_TRANSFER_TIMEOUT_S", "120"))
        with self.moriio_wrapper.lock:
            to_remove = []
            for req_id, status_by_layer in self._recving_transfers.items():
                statuses = list(status_by_layer.values())
                failed_status = next((status for status in statuses if status.Failed()), None)
                if statuses and all(status.Succeeded() for status in statuses):
                    ...
                    if _age > _xfer_timeout:
                        logger.error(
                            "RDMA read TIMED OUT for req %s after %.1fs (APHRODITE_MORIIO_TRANSFER_TIMEOUT_S=%d)",
                            req_id,
                            _age,
                            _xfer_timeout,
                        )
    ...
    def _compute_block_transfer_offsets(
        ...
        validate_moriio_heterogeneous_tp_kv_heads(
            ...
            remote_tp_size=(remote_tp_size if remote_tp_size and remote_tp_size > 0 else self.world_size),
        ...
        if self.kv_layer_mr_offset:
            ...
            remote_off = self._remote_layer_mr_offset(layer_name, remote_moriio_meta, remote_engine_id)
