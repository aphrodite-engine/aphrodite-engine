...
from aphrodite.v1.simple_kv_offload.worker import (
...
class SimpleCPUOffloadConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def __init__(
        ...
        if kv_offload_backend not in VALID_KV_OFFLOAD_BACKENDS:
            raise ValueError(
                f"Unknown kv_offload_backend {kv_offload_backend!r}; expected one of {VALID_KV_OFFLOAD_BACKENDS}"
            )
        disk_mode = kv_offload_backend == "disk"

        disk_path = extra_config.get("disk_path", None) or None
        disk_capacity_bytes = int(extra_config.get("disk_capacity_bytes", 100 * (1024**3)))
        ...
        if disk_mode:
            if disk_path is None:
                raise ValueError('kv_offload_backend="disk" requires disk_path to be set.')
            if disk_capacity_bytes <= 0:
                raise ValueError('kv_offload_backend="disk" requires disk_capacity_bytes > 0.')
        else:
        ...
        logger.info(
            "SimpleCPUOffloadConnector: role=%s, per_rank=%.2f GB, world_size=%d, mode=%s, backend=%s, disk=%s",
            role.name,
            cpu_capacity_per_rank / (1024**3),
            world_size,
