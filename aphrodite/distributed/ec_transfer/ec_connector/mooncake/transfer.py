...
def ensure_mooncake_available() -> None:
    if _MOONCAKE_IMPORT_ERROR is not None:
        raise ImportError("Install mooncake-transfer-engine to use ECMooncakeConnector.") from _MOONCAKE_IMPORT_ERROR
...
class MooncakeTransfer:
    ...
    def register_memory(self, tensor: torch.Tensor) -> int:
        return self._ensure_engine().batch_register_memory([tensor.data_ptr()], [tensor.nbytes])
            ...
            for address, (nbytes, tensor) in ranges.items():
                ...
                if entry is not None:
                    if entry.nbytes != nbytes:
                        raise RuntimeError("Mooncake EC source storage changed size while registered")
                    entry.users += 1
    ...
    def release_sources(self, addresses: list[int]) -> bool:
        ...
        with self._registration_lock:
            ...
            if ret != 0:
                logger.warning(
                    "Keeping %d EC source tensors registered after Mooncake unregistration failure",
                    len(unused),
                )
                return False
