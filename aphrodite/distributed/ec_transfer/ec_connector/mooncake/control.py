    ...
    def drain(self, base_addr: str) -> list[dict[str, Any]]:
        ...
        while True:
            try:
                ...
                logger.warning("Discarding an undecodable EC Mooncake event.", exc_info=True)
        ...
        def loop() -> None:
            ...
            def queue_event(event: dict[str, Any]) -> None:
                ...
                if len(pending_events) >= _MAX_PENDING_EVENTS:
                    ...
                    logger.warning(
                        "EC Mooncake event backlog full on port %d; dropping readiness for transfer_id=%s",
                        self.port,
            ...
            try:
                ...
                self.event_port = int(event_socket.getsockopt(zmq.LAST_ENDPOINT).decode().rsplit(":", 1)[1])
            ...
            try:
                while not self._stop.is_set():
                    ...
                    try:
                        ...
                        logger.exception(
                            "EC Mooncake control channel on port %d received an undecodable request",
                            self.port,
                        )
                        socket.send_json({"ok": False, "error": "malformed control request"})
                        continue
                    try:
                        op = request.get("op")
                        result: Any = None
                        if op in ("reserve", "reserve_batch"):
                            ...
                            for item in items:
                                ...
                                accepted, became_ready = self._complete(transfer_id, str(item["reservation_id"]))
