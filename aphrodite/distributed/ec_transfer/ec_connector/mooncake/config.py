...
class MooncakeECConfig:
    ...
    @classmethod
    def from_aphrodite_config(cls, aphrodite_config: AphroditeConfig) -> MooncakeECConfig:
        ...
        if ec_config.is_ec_producer:
            if parallel_config.tensor_parallel_size > 1:
                raise ValueError("ECMooncakeConnector producers require tensor_parallel_size=1.")
            if parallel_config.pipeline_parallel_size > 1:
                raise ValueError("ECMooncakeConnector producers do not support pipeline parallelism.")
            if parallel_config.data_parallel_size > 1:
                raise ValueError("ECMooncakeConnector producers require data_parallel_size=1.")

        registered_buffer_size = _positive_int("ec_buffer_size", ec_config.ec_buffer_size)
        ...
        return cls(
            ...
            control_timeout_ms=max(
                1,
                math.ceil(_positive_float("control_timeout_s", get("control_timeout_s", 30)) * 1000),
            ),
            push_wait_timeout_s=_positive_float("push_wait_timeout_s", get("push_wait_timeout_s", 60)),
