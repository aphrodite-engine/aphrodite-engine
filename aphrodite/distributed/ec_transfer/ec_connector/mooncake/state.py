...
class SchedulerTransferTable:
    ...
    def records_for_hash(
        ...
        return [
            ...
            if (record := self._records.get(transfer_id)) is not None and record.state in wanted
        ]
    ...
    def observe_ready(self, spec: ECMooncakeLoadSpec, deadline: float) -> tuple[SchedulerTransfer, bool]:
    ...
    def begin_load(
        ...
        if record is not None and (record.mm_hash != mm_hash or record.state is not SchedulerTransferState.AVAILABLE):
            record = None
        if record is None:
    ...
    def take_loads_to_dispatch(self) -> list[SchedulerTransfer]:
        records = [
            ...
            if (record := self._records.get(transfer_id)) is not None and record.state is SchedulerTransferState.LOADING
        ]
    ...
    def cancel(
        ...
        record = self._records.get(transfer_id)
        if record is not None and mm_hash and not self._identity_matches(record, mm_hash):
            return False
        if record is None:
            record = SchedulerTransfer(
                transfer_id=transfer_id,
    ...
    def expire(self, now: float, terminal_limit: int) -> list[SchedulerTransfer]:
        ...
        while self._terminal_ids:
            ...
            record = self._records[transfer_id]
            if record.deadline is not None and record.deadline > now and len(self._terminal_ids) <= terminal_limit:
                break
    ...
    def _insert(self, record: SchedulerTransfer) -> None:
        ...
        if record.mm_hash:
            self._hash_index.setdefault(record.mm_hash, deque()).append(record.transfer_id)

    @staticmethod
    def _identity_matches(record: SchedulerTransfer, mm_hash: str) -> bool:
        return not record.mm_hash or record.mm_hash == mm_hash

    def _refuse_colliding_id(self, record: SchedulerTransfer, mm_hash: str, request_id: str) -> None:
        ...
        """
        logger.warning(
            "EC Mooncake transfer_id=%s already names mm_hash=%s; refusing mm_hash=%s for request %s",
            record.transfer_id,
            record.mm_hash[:16],
            mm_hash[:16],
