...
from aphrodite.distributed.ec_transfer.ec_connector.mooncake.state import (
...
class ECMooncakeScheduler:
    """Coordinate Mooncake transfers from the Aphrodite Scheduler process."""
    ...
    def __init__(self, aphrodite_config: AphroditeConfig) -> None:
        ...
        self._encoder_cache_hidden_dim = _get_encoder_cache_hidden_dim(aphrodite_config) if config.is_producer else None
        self._model_config = aphrodite_config.model_config
        ...
        self._transfers = SchedulerTransferTable(config.pool_size, _RESERVATION_TTL_SECONDS)
    ...
    def _cancel_remote(self, consumer_zmq: str, transfer_id: str) -> bool:
        ...
        for _ in range(_CANCEL_ATTEMPTS):
            ...
            if pending is not None:
                break
        if pending is None:
            raise RuntimeError(f"Could not discover every EC consumer shard at {consumer_zmq}")
    ...
    def _poll_pending_cancels(self) -> None:
        ...
        for transfer_id, future in self._pending_cancels.items():
            ...
            try:
                ...
                logger.warning("EC Mooncake reservation cancellation failed", exc_info=True)
    ...
    def _note_shard_ready(self, data: dict[str, Any]) -> bool:
        ...
        if record is not None and record.state is not SchedulerTransferState.WAITING_EVENT:
            return False
    ...
    def _drain_push_notifications(self) -> None:
        ...
        for data in events:
            try:
                ...
                logger.warning("Discarding a malformed EC readiness event.", exc_info=True)
    ...
    def _warn_unresolved_transfer_id(self, request: Any, index: int, where: str) -> None:
    ...
    def ensure_cache_available(self, request: Any, num_computed_tokens: int) -> bool:
        if self._is_producer:
            for index, feature in enumerate(request.mm_features):
                if feature.mm_position.offset + feature.mm_position.length > num_computed_tokens:
        ...
        for index, feature in enumerate(request.mm_features):
            ...
            if transfer_id is not None:
                self._transfers.touch_available(transfer_id, now + _RESERVATION_TTL_SECONDS)
    ...
    def build_connector_meta(self, scheduler_output: SchedulerOutput) -> ECConnectorMetadata:
        ...
        meta = ECMooncakeConnectorMetadata(freed=scheduler_output.free_encoder_mm_hashes)
