...
from aphrodite.distributed.ec_transfer.ec_connector.mooncake.config import (
    _RESERVATION_TTL_SECONDS,
    MooncakeECConfig,
)
from aphrodite.distributed.ec_transfer.ec_connector.mooncake.control import (
    ConsumerControlServer,
    ControlClient,
    make_cancel_request,
)
from aphrodite.distributed.ec_transfer.ec_connector.mooncake.memory import (
    ConsumerMemoryPool,
    ProducerMemoryPool,
)
from aphrodite.distributed.ec_transfer.ec_connector.mooncake.metadata import (
    ECMooncakeConnectorMetadata,
    ECMooncakePushSpec,
    ECMooncakeWorkerMetadata,
)
from aphrodite.distributed.ec_transfer.ec_connector.mooncake.producer import (
    ProducerPushManager,
    ProducerPushRecord,
)
from aphrodite.distributed.ec_transfer.ec_connector.mooncake.reservation import (
    ConsumerReservationManager,
    ConsumerReservationState,
)
from aphrodite.distributed.ec_transfer.ec_connector.mooncake.transfer import (
    MooncakeTransfer,
...
class ECMooncakeWorker:
    ...
    def __init__(self, aphrodite_config: AphroditeConfig) -> None:
        ensure_mooncake_available()
        config = MooncakeECConfig.from_aphrodite_config(aphrodite_config)
    ...
    def _reserve_push_destination(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        self._reservations.expire()
        reservation, should_write = self._reservations.reserve(transfer_id, mm_hash, nbytes, shape, dtype_name, dtype)
    ...
    def _shard_executor(self) -> ThreadPoolExecutor:
        ...
        with self._shard_pool_lock:
            if self._shard_pool is None:
                self._shard_pool = ThreadPoolExecutor(max_workers=32, thread_name_prefix="ec-mooncake-shard")
    ...
    def _cancel_reservations(
        ...
        *,
        ...
        reservations = [shard for shard in reservations if not shard.get("cancelled", False)]
        if not reservations:
            return
        ...
        def cancel(shard: dict[str, Any]) -> dict[str, Any]:
            ...
            if not isinstance(addr, str) or not addr:
                raise RuntimeError("EC reservation is missing a confirmed shard address")
            result = self._control_client.request(
                addr,
                make_cancel_request(
                    spec.transfer_id,
        ...
        *,
        ...
        for _ in range(_CANCEL_ATTEMPTS):
            try:
                ...
                error = exc
                pending = [shard for index, shard in enumerate(pending) if exc.results[index] is None]
                continue
            return
    ...
    def _refresh_remote_reservations(
        ...
        try:
            ...
            pending = [shard for index, shard in enumerate(stale) if exc.results[index] is None]
            try:
    ...
    def _reserve_remote_many(self, specs: list[ECMooncakePushSpec]) -> list[list[dict[str, Any]] | Exception]:
        shards = None
        for _ in range(_CANCEL_ATTEMPTS):
            ...
            if shards is not None:
                break
        if shards is None:
            raise RuntimeError(f"Could not discover every EC consumer shard at {specs[0].consumer_zmq}")
    ...
    @staticmethod
    def _reservation_batch_done(records: list[ProducerPushRecord], future: Future[None]) -> None:
            ...
            if ready:
                ...
                try:
                    ...
                    def track_write(index: int, future: Future[None]) -> None:
                        ...
                        self._producer_pushes.track_shard_futures(list(session_records[session].values()), [future])
        ...
        try:
            ...
            if resolution_error is not None:
                combined_error = RuntimeError(
                    f"EC reservation resolution failed ({resolution_error}); cleanup also failed ({cleanup_error})"
                )
    ...
    def close(self) -> None:
        ...
        self._producer_pushes.submit_batches(self._io_executor, self._push_batch, wait=True)
        ...
        drained = True
        if self._control_server is not None:
            drained = self._reservations.wait_for_writers(self._shutdown_drain_timeout_s)
            self._control_server.close()
        if drained:
            self._consumer_memory.close()
        else:
            logger.error("Timed out waiting for Mooncake EC writers; keeping the consumer receive pool registered")
