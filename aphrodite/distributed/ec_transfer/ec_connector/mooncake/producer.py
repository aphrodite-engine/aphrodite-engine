...
def _same_destination(existing: ECMooncakePushSpec, incoming: ECMooncakePushSpec) -> bool:
...
class ProducerPushManager:
        ...
        with self._lock:
            ...
            if existing is not None:
                if not _same_destination(existing.spec, spec):
                    raise ValueError(f"Conflicting EC destination for transfer_id={spec.transfer_id}")
            ...
            self._source_waiters.setdefault(spec.mm_hash, OrderedDict())[spec.transfer_id] = None
            record.reservation_future.add_done_callback(lambda future: self._reservation_done(record, future))
            return record, True
    ...
    def bind_source(
        ...
        with self._lock:
            ...
            for transfer_id in waiters:
                ...
                if record.source_tensor is not None or record.state is not ProducerPushState.WAITING_INPUTS:
                    continue
    ...
    def _reservation_done(
        ...
        try:
            ...
            with self._lock:
                ...
                if record.state is ProducerPushState.WAITING_INPUTS and record.source_tensor is None:
    ...
    def begin_writing(self, record: ProducerPushRecord) -> None:
        with self._lock:
            if record.source_tensor is None:
                raise RuntimeError(f"Producer push {record.spec.transfer_id!r} has no source tensor")
    ...
    def _check_sources_releasable(records: list[ProducerPushRecord]) -> None:
        for record in records:
            ...
            if record.source_tensor is not None and not all(future.done() for future in futures):
                raise RuntimeError(f"Producer push {record.spec.transfer_id!r} released its source too early")
    ...
    def _transition(self, record: ProducerPushRecord, state: ProducerPushState) -> None:
        if state not in _ALLOWED_TRANSITIONS[record.state]:
            raise RuntimeError(
                f"Cannot transition producer push {record.spec.transfer_id!r} from {record.state.name} to {state.name}"
            )
        record.state = state
