...
class ConsumerReservationManager:
    ...
    def reserve(
        ...
        with self._condition:
            ...
            if existing is not None and existing.state is ConsumerReservationState.CANCELLED:
                return existing, False
            if existing is not None and existing.state is ConsumerReservationState.EXPIRED:
                self._remove(transfer_id)
                existing = None
            if existing is not None and existing.state is ConsumerReservationState.EXPIRE_PENDING:
                raise RuntimeError(f"Transfer {transfer_id!r} still has an active writer")
            if existing is not None:
                if existing.mm_hash != mm_hash or existing.shape != shape or existing.dtype != dtype_name:
                    raise ValueError("conflicting reservation for transfer_id")
                if existing.state not in _ACTIVE_STATES:
                    raise RuntimeError(f"Cannot reserve transfer {transfer_id!r} in {existing.state.name}")
    ...
    def _publish_followers(self, writer: ConsumerReservation) -> None:
        ...
        for record in [writer, *(self._records[key] for key in followers)]:
            ...
            record.lease = self._memory.acquire_cached(record.mm_hash, record.shape, allocation.tensor.dtype)
    ...
    def wait_for_writers(self, timeout: float) -> bool:
        ...
        def writers_finished() -> bool:
            return not any(self._records[transfer_id].state in _WRITER_OWNED_STATES for transfer_id in self._active_ids)
    ...
    def cancel(
        ...
        with self._memory.lock:
            record = self._records.get(transfer_id)
            if record is not None and reservation_id and record.reservation_id != reservation_id:
                return False
            if record is None:
                record = ConsumerReservation(
                    transfer_id,
    ...
    def take(self, transfer_id: str, mm_hash: str) -> MemoryAllocation:
        with self._memory.lock:
            ...
            if (
                ...
                raise RuntimeError(f"Pushed EC tensor is not ready for mm_hash={mm_hash}")
    ...
    def retire_stale(self, encoder_cache: dict[str, torch.Tensor], freed: list[str] | None = None) -> None:
        with self._memory.lock:
    ...
    def _defer(self, record: ConsumerReservation, state: ConsumerReservationState) -> None:
        """Keep the destination until the writer completes or abandons it."""
        self._transition(record, state)
        record.expires_at = float("inf")

    def _terminate(self, record: ConsumerReservation, state: ConsumerReservationState) -> None:
        self._transition(record, state)
    ...
    def _release(self, record: ConsumerReservation) -> None:
        ...
        for transfer_id in list(self._followers.pop(record.transfer_id, {})):
            self._terminate(self._records[transfer_id], ConsumerReservationState.CANCELLED)
    ...
    def _reap_tombstones(self, now: float) -> int:
        ...
        while self._tombstones:
            ...
            record = self._records[transfer_id]
            if record.expires_at > now and len(self._tombstones) <= self._tombstone_limit:
                break
            self._remove(transfer_id)
            dropped += 1
        return dropped
    ...
    def _transition(self, record: ConsumerReservation, state: ConsumerReservationState) -> None:
        if state not in _ALLOWED_TRANSITIONS[record.state]:
            raise RuntimeError(f"Cannot transition {record.transfer_id!r} from {record.state.name} to {state.name}")
        record.state = state
