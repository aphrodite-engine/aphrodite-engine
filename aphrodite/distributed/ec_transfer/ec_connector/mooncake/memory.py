...
class ContiguousAllocator:
    ...
    def view(self, offset: int, nbytes: int, shape: tuple[int, ...], dtype: torch.dtype) -> torch.Tensor:
...
class ConsumerMemoryPool:
    ...
    def _defer_or_free(self, allocation: MemoryAllocation, event: torch.Event | None) -> None:
    ...
    def try_allocate(self, nbytes: int, shape: tuple[int, ...], dtype: torch.dtype) -> MemoryAllocation | None:
        with self.lock:
            allocator = self._allocator
    ...
    def reclaim_and_allocate(self, nbytes: int, shape: tuple[int, ...], dtype: torch.dtype) -> MemoryAllocation | None:
        with self.lock:
    ...
    def take_resident(self, mm_hash: str, shape: tuple[int, ...], dtype_name: str) -> torch.Tensor | None:
        with self.lock:
            allocation = self._residents.get(mm_hash)
            if allocation is None:
                return None
