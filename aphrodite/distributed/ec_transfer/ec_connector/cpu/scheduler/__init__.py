...
from aphrodite.distributed.ec_transfer.ec_connector.cpu.scheduler.embedding_cache import (
    EmbeddingCache,
)
from aphrodite.distributed.ec_transfer.ec_connector.utils import (
...
class ECCPUScheduler:
    ...
    def __init__(self, aphrodite_config: "AphroditeConfig") -> None:
        ec_config = aphrodite_config.ec_transfer_config
        ...
        self._region = create_ec_shared_region(aphrodite_config)
        # Block allocator + LRU eviction policy for the shared region.
        self._cache = EmbeddingCache(self._region.num_blocks)
        self._metadata_resolver = PlaceholderMetadataResolver(aphrodite_config.model_config)
        ...
        self._expected_load_reports = (
            pc.tensor_parallel_size * pc.prefill_context_parallel_size if pc.distributed_executor_backend == "mp" else 1
        )
        ...
        self._nixl_enabled = (
            raw_nixl if isinstance(raw_nixl, bool) else str(raw_nixl).strip().lower() in ("true", "1", "yes")
        )
        ...
        if self._nixl_enabled:
            self._setup_nixl(aphrodite_config)

    def _setup_nixl(self, aphrodite_config: "AphroditeConfig") -> None:
        # Lazy imports keep nixl/zmq off the gate-off path.
        from aphrodite import envs
        from aphrodite.distributed.ec_transfer.ec_connector.cpu.control.zmq import (
            ZmqClientTransport,
            ZmqServerTransport,
        )
        from aphrodite.distributed.ec_transfer.ec_connector.cpu.data.nixl import (
            NixlDataTransport,
        )
        from aphrodite.distributed.ec_transfer.ec_connector.cpu.protocol import (
            compute_ec_compatibility_hash,
        )
        from aphrodite.distributed.ec_transfer.ec_connector.cpu.session import (
            ProducerSession,
        )
        ...
        self._hidden_dim = _get_encoder_cache_hidden_dim(aphrodite_config)
        ...
        self._ack_timeout_s = float(self._ec_config.get_from_extra_config("consumer_ack_timeout_s", 2.0))
        self._compat_hash = compute_ec_compatibility_hash(
            aphrodite_version=APHRODITE_VERSION,
            model=str(aphrodite_config.model_config.model),
            dtype=str(self._dtype),
            block_size_bytes=self._region.block_size_bytes,
        )
        if self._is_producer:
            self._peer_host = envs.APHRODITE_EC_SIDE_CHANNEL_HOST
            self._peer_port = envs.APHRODITE_EC_SIDE_CHANNEL_PORT
        ...
        try:
            ...
            if self._is_producer:
                ...
                self._producer_session = ProducerSession(
                    transport=ZmqServerTransport(host=self._peer_host, port=self._peer_port),
                    data=self._data,
                    cache=self._cache,
    ...
    def _nixl_consumer_admit(self, request: "Request", num_computed_tokens: int) -> bool:
        ...
        """
        params: dict[str, dict[str, Any]] = getattr(request, "ec_transfer_params", None) or {}
        if not params:
            return True
        pending = False
        for feature in request.mm_features:
            ...
            if int(info.get("size_bytes", -1)) != expected:
                logger.warning("EC consumer: size mismatch mm_hash=%s; local encode", mm_hash)
                continue
            try:
                started = self._start_xfer(mm_hash, info, expected)
            except Exception:
                logger.exception("EC consumer: failed to start NIXL xfer mm_hash=%s", mm_hash)
                continue
            if not started:
                continue
    ...
    def _start_xfer(self, mm_hash: str, info: "dict[str, Any]", size_bytes: int) -> bool:
    ...
    def _process_session_results(self, session: Any) -> None:
        ...
        for mm_hash, _block_indices in r.settled:
            self._cache.discard(mm_hash)
            logger.debug("EC consumer: quarantined NIXL xfer mm_hash=%s settled", mm_hash)
    ...
    def update_connector_output(self, connector_output: "ECConnectorOutput") -> None:
        ...
        for mm_hash in meta.completed_saves:
            ...
            if entry is None:
                logger.debug(
                    "EC producer: worker reported completed save for unknown mm_hash=%s (already discarded/evicted?)",
                    mm_hash,
                )
