...
from aphrodite.distributed.ec_transfer.ec_connector.cpu.data.base import (
    DataTransport,
)
...
from aphrodite.distributed.ec_transfer.ec_connector.cpu.utils import PeerAddr
...
class ConsumerXfer:
    ...
    def handle_ack(self, ack: XferAck, agent_name: str) -> bool:
        ...
        logger.debug(
            "EC consumer: NIXL READ posted mm_hash=%s agent=%s local_blocks=%d remote_blocks=%d",
            self.mm_hash,
            agent_name,
    ...
    def poll(self, now: float) -> XferState:
        ...
        if self.transfer_handle is None:
            return XferState.ACK_TIMEOUT if now > self.deadline else XferState.WAITING_ACK
    ...
    def _poll_reading(self, now: float) -> XferState:
        ...
        try:
            ...
            logger.exception("EC Connector: check_xfer_state failed for mm_hash=%s", self.mm_hash)
        ...
        if state == "PROC":
            if now > self.deadline:
                ...
                logger.warning(
                    "EC Connector: READ for mm_hash=%s from %s:%d timed out; quarantining",
                    self.mm_hash,
                    self.addr[0],
                    self.addr[1],
                )
    ...
    def _poll_quarantined(self, now: float) -> XferState:
        ...
        if now > self.deadline:
            ...
            self.transfer_handle = None
            logger.warning(
                "EC Connector: quarantined mm_hash=%s from %s:%d timed out; evicting blocks",
                self.mm_hash,
                self.addr[0],
                self.addr[1],
            )
...
class ProducerSession:
    ...
    def _grant_or_nack(self, req: XferReq) -> XferAck:
        if req.connector_version != EC_CONNECTOR_VERSION:
            logger.warning("EC Connector: incompatible version for mm_hash=%s", req.mm_hash)
            return XferAck(mm_hash=req.mm_hash, status=XferStatus.NACK_VERSION)
        if req.compatibility_hash != self._compat_hash:
            logger.warning("EC Connector: incompatible compat hash for mm_hash=%s", req.mm_hash)
        ...
        if not req.session_id:
            ...
            logger.warning("EC Connector: XferReq without session_id for mm_hash=%s", req.mm_hash)
            return XferAck(mm_hash=req.mm_hash, status=XferStatus.NACK_INTERNAL)
        entry = self._cache.pin_if_ready(req.mm_hash)
        if entry is None or not entry.ready:
            status = XferStatus.NACK_MISSING if entry is None else XferStatus.NACK_NOT_READY
            logger.log(
...
class ConsumerSession:
    ...
    def __init__(
        ...
        self._data = data
        self._compat_hash = compat_hash
        self._session_id = str(uuid.uuid4())  # stable identity for (session_id:mm_hash) keys
    ...
    def start_xfer(self, mm_hash: str, block_indices: list[int], deadline: float) -> None:
