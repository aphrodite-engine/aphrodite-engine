...
from aphrodite.distributed.ec_transfer.ec_connector.cpu.utils import (
    build_block_descs,
...
class NixlDataTransport(DataTransport):
    ...
    NixlWrapper is not thread-safe, and neither is this class. Every call site
    runs on the scheduler thread, which serializes the transport by
    construction; transfers are async inside NIXL and are drained from that
    same thread via get_new_notifs() / check_xfer_state().
    """
    ...
    def __init__(
        ...
        if NixlWrapper is None or nixl_agent_config is None:
            raise RuntimeError(
                "ECCPUConnector requires NIXL; install the `nixl` package or set a different ec_connector."
            )
        ...
        self._register_local_memory(base_ptr, num_blocks, block_size_bytes, total_size_bytes)
    ...
    def _register_local_memory(
        ...
        self._reg_descs = self._nixl.get_reg_descs([(base_ptr, total_size_bytes, 0, "")], _NIXL_DRAM)
        self._nixl.register_memory(self._reg_descs, backends=["UCX"])
        try:
            xfer_descs = self._nixl.get_xfer_descs(block_descs, _NIXL_DRAM)
            self._local_xfer_handle = self._nixl.prep_xfer_dlist("NIXL_INIT_AGENT", xfer_descs)
    ...
    def add_remote_peer(self, metadata: bytes, mem_descriptor: bytes) -> str:
        ...
        self._peer_handles[agent_name] = self._nixl.prep_xfer_dlist(agent_name, remote_xfer_descs)
        return agent_name
    ...
    def remove_remote_peer(self, agent_name: str) -> None:
        ...
        try:
            ...
            logger.warning("EC: remove_remote_peer failed for %s", agent_name, exc_info=True)
    ...
    def post_read(
        ...
        if len(local_indices) != len(remote_indices):
            raise ValueError(f"EC: local/remote block count mismatch ({len(local_indices)} vs {len(remote_indices)})")
