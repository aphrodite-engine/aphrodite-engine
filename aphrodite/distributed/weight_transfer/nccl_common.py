...
from aphrodite.distributed.weight_transfer.base import WeightTransferInitInfo
...
def decode_nccl_unique_id(
    *,
    ...
    if has_uid and (has_addr or has_port):
        raise ValueError(f"{ctx}: pass nccl_unique_id_b64 OR master_address/master_port, not both")
    if not has_uid:
    ...
    try:
        ...
        raise ValueError(
            f"{ctx}: nccl_unique_id_b64 is not valid standard base64 (URL-safe base64 is not accepted)"
        ) from e
    if len(decoded) != NCCL_UNIQUE_ID_BYTES:
        raise ValueError(f"{ctx}: nccl_unique_id_b64 must decode to {NCCL_UNIQUE_ID_BYTES} bytes, got {len(decoded)}")
    return decoded
