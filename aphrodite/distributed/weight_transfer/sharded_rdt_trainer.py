...
class ShardedRDTTrainerWeightTransferEngine(TrainerWeightTransferEngine[ShardedRDTTrainerInitInfo]):
    ...
    def _resolve_ownership(self, world: int, rank: int) -> None:
        ...
        if held is None:
            ...
            if unknown:
                raise ValueError(
                    f"WeightSource.held_names() lists {len(unknown)} name(s) not in metadata(), e.g. {unknown[:3]}."
                )
            if not held_set:
                raise ValueError(
    ...
    def _spawn_server(self, served_names: list[str]) -> None:
        ...
        ii = self._init_info
        self._server_name = f"aphrodite_rdt_producer_{uuid.uuid4().hex[:12]}_rk{ii.rank}"
        ...
        # rank runs under (library paths etc.) so it imports torch/aphrodite the
