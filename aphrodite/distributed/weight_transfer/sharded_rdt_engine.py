...
Pulls only the slice each Aphrodite worker consumes under tensor/expert parallelism,
...
class ShardedRDTWeightTransferEngine(
    ...
    def __init__(
        ...
        backend = aphrodite_config.parallel_config.distributed_executor_backend
        ...
        if isinstance(backend, str) and backend != "ray":
            raise ValueError(
                ...
                f"APHRODITE_USE_RAY_V2_EXECUTOR_BACKEND=1, the default, so the "
    ...
    def init_transfer_engine(self, init_info: ShardedRDTWeightTransferInitInfo) -> None:
        ...
        """
        # Read the attribute directly: if a Aphrodite bump renames it, this guard
    ...
    def _resolve_producers(self, init_info: ShardedRDTWeightTransferInitInfo) -> None:
        ...
        if name_owner_class and len(name_owner_class) != len(init_info.names):
            raise RuntimeError(
                f"Sharded RDT engine: {len(name_owner_class)} owner-class entries for {len(init_info.names)} names."
            )
        router = RdtRouter(
            len(producer_names),
            self._num_consumers(),
        ...
        self._router = router
        logger.info(
            "Sharded RDT engine (consumer %d) bound to all %d producers (batched method %r, %d owner class(es))",
            consumer_id,
            len(producer_names),
    ...
    def _build_static_plan(self, init_info: ShardedRDTWeightTransferInitInfo) -> None:
        ...
        if sum(init_info.group_lens) != len(init_info.names):
            raise ValueError(
                f"init_info.group_lens sums to {sum(init_info.group_lens)} but {len(init_info.names)} names were given."
            )
    ...
    def _global_worker_index(self) -> int:
        ...
        ``data_parallel_index``, not ``data_parallel_rank``: Aphrodite resets the
    ...
    def receive_weights(
        ...
        if self._cached_plan is None:
            raise RuntimeError(
                "Sharded RDT engine has no call plan: init_info.group_lens must be supplied at init_transfer_engine()."
            )
    ...
    def _pull_targets(self, chunk: "_Chunk", slot: int, buffer: torch.Tensor) -> "list[torch.Tensor]":
        ...
        if cached is not None and cached[0] == buffer.data_ptr():
            return cached[1]
        targets = [
            buffer[off : off + n * dt.itemsize].view(dt).reshape(shape) for off, dt, n, shape in chunk.pack_layout
        ]
    ...
    def _bake(self, init_info: ShardedRDTWeightTransferInitInfo) -> None:
        ...
        from aphrodite.model_executor.model_loader.reload.utils import get_layer_size
    ...
    def _install_recording_stamps(self, model: torch.nn.Module, recorder: "BakeSink") -> None:
        ...
        ``functools.wraps`` keeps the loader's real signature (so Aphrodite's
    ...
    def _chunk_module_scatters(self, modules: "list[list[_Scatter]]") -> "list[tuple[int, list[_Scatter]]]":
    ...
    def _run_chunk_pipeline(self, plan: "_CallPlan") -> None:
        ...
        def drain_one() -> None:
            ...
            self._dispatch_item(_ProcItem(chunk=chunk, results=results, slot=pending.slot))
    ...
    def _process_item(self, item: "_ProcItem") -> None:
        ...
        from aphrodite.model_executor.model_loader.reload.meta import materialize_layer
