...
from aphrodite.model_executor.model_loader.checkpoint_weight_patch import (
...
class SparseNCCLTrainerWeightTransferEngine(TrainerWeightTransferEngine[SparseNCCLTrainerInitInfo]):
    ...
    def _broadcast_chunk(self, patches: list[SparseWeightPatch]) -> None:
        ...
        device = self.model_update_group.device
        device_context = torch.accelerator.device_index(device.index) if device.type != "cpu" else nullcontext()
        with device_context:
    ...
    def _validate_patch(patch: SparseWeightPatch) -> None:
        ...
        if patch.indices.dtype != torch.int32:
            raise ValueError(f"Sparse weight updates require int32 indices: {patch.name}")
