...
from aphrodite.distributed.weight_transfer.sharded_rdt_common import SUPPORTED_OPS
...
class BakeSink:
    ...
    copies_by_layer: "dict[Any, list[_Scatter | None]]" = field(default_factory=lambda: defaultdict(list))
...
class FakeRDTTensor(torch.Tensor):
    ...
    def _intercept(
        ...
        raise _UnsupportedFakeOp(
            f"FakeRDTTensor: {op_name!r} returned a non-tensor ({type(meta_result).__name__}); cannot defer."
        )
