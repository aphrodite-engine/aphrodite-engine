...
class PPLXAll2AllManager(All2AllManagerBase):
    ...
    def __init__(self, cpu_group):
        # TODO: port pplx_kernels to aphrodite
        assert has_pplx(
        ), "pplx_kernels not found. Please follow https://github.com/ppl-ai/pplx-kernels to install pplx_kernels."  # noqa
        super().__init__(cpu_group)
        ...
        if self.internode:
            ...
            logger.debug(
                ...
                "rank={}, world size={}", self.rank, self.world_size)
            uid = nvshmem_get_unique_id(
            ) if self.rank == 0 else nvshmem_alloc_empty_unique_id()
            dist.broadcast(uid,
                           src=dist.get_process_group_ranks(self.cpu_group)[0],
                           group=self.cpu_group)
            logger.debug("PPLX NVSHMEM UID = {}", uid)
...
class DeepEPAll2AllManagerBase(All2AllManagerBase):
    ...
    def __init__(self, cpu_group):
        assert has_deep_ep(
        ), "DeepEP kernels not found. Please follow https://github.com/aphrodite-project/aphrodite/blob/main/tools/ep_kernels/README.md to install DeepEP kernels."  # noqa
        super().__init__(cpu_group)
        self.handle_cache = Cache()
...
class DeepEPHTAll2AllManager(DeepEPAll2AllManagerBase):
    ...
    def get_handle(self, kwargs):
        ...
        logger.debug("DeepEP all2all args {}", buffer_kwargs)
