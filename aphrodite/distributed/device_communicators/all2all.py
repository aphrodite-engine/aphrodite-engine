...
from aphrodite.distributed import get_dp_group, get_ep_group, get_pcp_group
...
from aphrodite.utils.import_utils import has_deep_ep, has_deep_ep_v2, has_mori
...
class DeepEPLLAll2AllManager(DeepEPAll2AllManagerBase):
    ...
    def __init__(self, cpu_group, tcp_store_group=None):
        super().__init__(cpu_group, tcp_store_group)
        self.support_fault_tolerance = get_current_aphrodite_config().parallel_config.enable_fault_tolerance
...
class NixlEPAll2AllManager(All2AllManagerBase):
    ...
    def __init__(self, cpu_group, tcp_store_group=None):
        ...
        self.max_num_ep_ranks = envs.APHRODITE_NIXL_EP_MAX_NUM_RANKS
    ...
    def clean_buffers(self) -> None:
        if NixlEPAll2AllManager._buffer is None:
            return
        state = NixlEPAll2AllManager._buffer
        state.buffer.get_local_buffer_tensor(dtype=torch.int8, use_rdma_buffer=True).zero_()
        torch.accelerator.synchronize()
...
class FlashInferNVLinkOneSidedManager(All2AllManagerBase):
    ...
    def initialize(
        ...
        try:
            self._combine_supports_output = supports_kw(self.moe_alltoall.combine, "output", allow_var_kwargs=False)
...
class DeepEPV2All2AllManager(All2AllManagerBase):
    ...
    def _make_all2all_kwargs(
        ...
        return dict(
            ...
            allow_hybrid_mode=envs.APHRODITE_DEEPEP_V2_ALLOW_HYBRID_MODE,
            prefer_overlap_with_compute=envs.APHRODITE_DEEPEP_V2_PREFER_OVERLAP,
            allow_multiple_reduction=(envs.APHRODITE_DEEPEP_V2_ALLOW_MULTIPLE_REDUCTION),
            explicitly_destroy=True,
        )
    ...
    def _check_gin_support(self, group) -> None:
        ...
        if gin_type == 0:
            raise RuntimeError(
                ...
                "are not available. See tools/ep_kernels/README.md for requirements."
            )
