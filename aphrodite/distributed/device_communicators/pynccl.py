...
class PyNcclCommunicator:
    ...
    def __init__(
        ...
        if self.world_size == 1 or envs.APHRODITE_DISABLE_PYNCCL:
    ...
    def from_unique_id_bytes(
        ...
        so every peer -- including a foreign, non-Aphrodite rank -- must issue a
        matching one-element ``all_reduce`` before any other collective, or all
        ranks deadlock.
        """
        if len(unique_id_bytes) != NCCL_UNIQUE_ID_BYTES:
            raise ValueError(f"expected a {NCCL_UNIQUE_ID_BYTES}-byte NCCL unique id, got {len(unique_id_bytes)} bytes")
        ...
        try:
            ...
            raise RuntimeError("failed to load the NCCL library for unique-id rendezvous") from e
