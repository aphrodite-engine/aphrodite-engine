...
class FlashInferPcieIpcAllReduce:
    """Aphrodite lifecycle wrapper for FlashInfer's PCIe IPC all-reduce."""
    ...
    def __init__(
        ...
        if not current_platform.is_cuda():
            logger.warning_once("FlashInfer PCIe IPC all-reduce requires the CUDA platform.")
            return
        if self.world_size not in _SUPPORTED_WORLD_SIZES:
            logger.warning_once(
                "FlashInfer PCIe IPC all-reduce does not support world_size=%d; supported sizes are %s.",
                self.world_size,
                _SUPPORTED_WORLD_SIZES,
            )
            return
        if not all(in_the_same_node_as(tune_group, source_rank=0)):
            logger.warning_once("FlashInfer PCIe IPC all-reduce requires every rank in the TP group to be on one node.")
            return
    ...
    def setup(
        ...
        *,
        ...
        if dtype not in _SUPPORTED_DTYPES:
            logger.warning_once(
                "FlashInfer PCIe IPC all-reduce does not support dtype=%s; falling back to another all-reduce backend.",
                dtype,
            )
            self.disabled = True
            return
        ...
        workspace.rebind_stream()
        logger.info_once(
            "Initialized FlashInfer PCIe IPC all-reduce for TP%d, hidden_dim=%d, dtype=%s, max_tokens=%d.",
            self.world_size,
            self.hidden_dim,
            dtype,
            batches[-1],
...
def warmup_flashinfer_pcie_ipc_allreduce(worker: "Worker") -> None:
    ...
    if pcie_comm is None or pcie_comm.disabled:
        return
    if worker.aphrodite_config.parallel_config.use_ubatching:
        logger.warning_once(
    ...
    capture_sizes = worker.aphrodite_config.compilation_config.cudagraph_capture_sizes
