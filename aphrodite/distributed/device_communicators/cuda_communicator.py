...
class CudaCommunicator(DeviceCommunicatorBase):
    def __init__(
        ...
        if "tp" not in unique_name:
            ...
            use_flashinfer_allreduce = envs.APHRODITE_ALLREDUCE_USE_FLASHINFER and not envs.APHRODITE_BATCH_INVARIANT
            use_flashinfer_pcie_ipc_allreduce = (
                envs.APHRODITE_ALLREDUCE_USE_FLASHINFER_PCIE_IPC and not envs.APHRODITE_BATCH_INVARIANT
            )
        ...
        from aphrodite.distributed.device_communicators.flashinfer_pcie_ipc_all_reduce import (  # noqa: E501
        ...
        if self.use_flashinfer_pcie_ipc_allreduce and self.world_size > 1 and self.device_group is not None:
        ...
        if self.use_all2all:
            if self.all2all_backend in ("naive", "allgather_reducescatter"):
                ...
                self.all2all_manager = DeepEPHTAll2AllManager(self.cpu_group, tcp_store_group)
            elif self.all2all_backend == "deepep_low_latency":
                from .all2all import DeepEPLLAll2AllManager

                self.all2all_manager = DeepEPLLAll2AllManager(self.cpu_group, tcp_store_group)
                ...
                self.all2all_manager = MoriAll2AllManager(self.cpu_group, self.all2all_backend)
            ...
            elif self.all2all_backend == "flashinfer_all2allv" or self.all2all_backend == "flashinfer_nvlink_two_sided":
                ...
                self.all2all_manager = FlashInferNVLinkTwoSidedManager(self.cpu_group, tcp_store_group)
    ...
    def _log_all_reduce_backend_selection(self) -> None:
        ...
        if self.fi_pcie_ipc_ar_comm is not None and not self.fi_pcie_ipc_ar_comm.disabled:
        ...
        # APHRODITE_BATCH_INVARIANT off, NCCL symm mem enabled, world_size meets
        ...
        nccl_symm_ws_ok = self.world_size >= NCCL_SYMM_MEM_ALL_REDUCE_CONFIG["min_world_size"] and (
            self.world_size in NCCL_SYMM_MEM_ALL_REDUCE_CONFIG["custom_ar_preferred_ranges"]
        ...
        if (
            ...
            and not envs.APHRODITE_BATCH_INVARIANT
            and nccl_symm_ws_ok
        ):
        ...
        logger.info_once(
            "Using %s all-reduce backends (in dispatch order) for group '%s' out of potential backends: %s.",
    ...
    def all_reduce(self, input_):
        ...
        use_fi_ar = fi_ar_comm is not None and not fi_ar_comm.disabled and fi_ar_comm.should_use_fi_ar(input_)
    ...
    def reduce_scatterv(self, input_: torch.Tensor, dim: int = -1, sizes: list[int] | None = None):
        ...
        if use_symm_mem:
            ...
            output = torch.empty(output_shape, dtype=input_tensor.dtype, device=input_tensor.device)
            use_deterministic_rs = envs.APHRODITE_BATCH_INVARIANT and world_size > 2
    ...
    def _reduce_scatter_symm_mem(
        ...
        symm_output = self._get_symm_scratch("rs_out", output_shape, input_tensor.dtype, input_tensor.device)
    ...
    def _all_gather_symm_mem(self, input_: torch.Tensor) -> torch.Tensor:
        ...
        symm_output = self._get_symm_scratch("ag_out", out_size, input_.dtype, input_.device)
        pynccl_comm.all_gather(symm_output, input_)
        return symm_output

    def _all_gather_batched_symm_mem(self, inputs: list[torch.Tensor]) -> list[torch.Tensor]:
        ...
        with nccl_symm_mem_context(pynccl_comm):
            for inp in inputs:
                ...
                symm_outputs.append(torch.empty(out_size, dtype=inp.dtype, device=inp.device))
