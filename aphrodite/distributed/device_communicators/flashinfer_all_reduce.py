...
from aphrodite.distributed.parallel_state import _node_count, get_node_count
...
def _get_tuned_standalone_max_size(
    ...
    if capability is None:
        return None
    max_size_mb = FI_MNNVL_ALLREDUCE_MAX_SIZE_MB.get((capability.to_int(), world_size, _node_count(group)))
...
def _create_workspace(
    ...
    try:
        ...
        if backend == "mnnvl" and not getattr(workspace, "mc_ptr", 0):
            workspace.destroy()
            logger.warning_once("FlashInfer MNNVL multicast is unavailable on the current topology.")
            return None
    ...
    if workspace_group is not None and workspace_group is not group:
        raise RuntimeError(
            "FlashInfer returned an all-reduce workspace already associated with a different process group"
        )
...
def _resolve_fi_ar_backend() -> tuple[str, bool]:
    ...
    """
    backend = envs.APHRODITE_FLASHINFER_ALLREDUCE_BACKEND
    if backend != "auto":
...
def get_fi_ar_workspace(
    ...
    APHRODITE_FLASHINFER_ALLREDUCE_BACKEND env var.
    """
    ...
    if (
        envs.APHRODITE_ALLREDUCE_USE_FLASHINFER
        and (max_size := _get_tuned_standalone_max_size(world_size, backend, group)) is not None
    ):
...
def get_fi_ar_quant_workspace(
    ...
    Backend is controlled by APHRODITE_FLASHINFER_ALLREDUCE_BACKEND env var,
    matching non-quant fusion. With ``auto`` this prefers mnnvl and falls back
    to trtllm only on single-node topologies where mnnvl multicast is
    unavailable.
    """
    ...
    _fi_ar_quant_workspace = _create_workspace(backend, world_size, rank, max_token_num, hidden_dim, dtype, group)
    ...
    if _fi_ar_quant_workspace is not None:
        logger.info_once(f"Initialized FlashInfer Allreduce norm quantization fusion workspace with backend={backend}")
    else:
        logger.warning_once(
            f"Failed to initialize FlashInfer Allreduce norm quantization fusion workspace with backend={backend}"
        )
...
class FlashInferAllReduce:
    def __init__(
        ...
        default_max_size_mb = PassConfig.default_fi_allreduce_fusion_max_size_mb().get(self.world_size)
        ...
        self.max_workspace_size = tuned_max_size if tuned_max_size is not None else int(default_max_size_mb * MiB)
