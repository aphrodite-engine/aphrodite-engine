...
from aphrodite.distributed.utils import is_weak_contiguous  # noqa: E402
...
class CustomAllreduce:
    ...
    def __init__(
        ...
        if same_node:
            physical_device_id = current_platform.visible_device_id_to_physical_device_id(device.index)
        ...
        if same_node and not current_platform.is_rocm() and not _can_p2p(rank, world_size):
            logger.warning(
        ...
        if max_mnnvl_all_gather_size is None:
            max_mnnvl_all_gather_size = self._DEFAULT_MNNVL_ALL_GATHER_MAX_SIZES[world_size]
        ...
        if not same_node and not self.mnnvl_multicast_ptr:
            logger.warning(
                "Custom collectives are disabled because this multi-node group does not support MNNVL multicast."
            )
            self.close()
            self.disabled = True
    ...
    def _init_mnnvl_buffer(self, stage_size: int) -> None:
        ...
        try:
            ...
            buffer = torch_symm_mem.empty(buffer_size, dtype=torch.uint8, device=self.device)
    ...
    def should_custom_all_gather(self, inp: torch.Tensor) -> bool:
        ...
        if inp.dtype not in (
            ...
            return False
        max_size = self.max_mnnvl_all_gather_size if self.mnnvl_multicast_ptr else self.max_all_gather_size
        return (
            0 < inp_size <= max_size
            and inp_size % 16 == 0
    ...
    def should_custom_reduce_scatter(self, inp: torch.Tensor) -> bool:
        ...
        max_size = self.max_mnnvl_reduce_scatter_size if self.mnnvl_multicast_ptr else self.max_reduce_scatter_size
        return (
            0 < inp_size <= max_size
