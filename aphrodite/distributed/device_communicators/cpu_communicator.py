...
from aphrodite.distributed.utils import pickle
...
class CpuCommunicator(DeviceCommunicatorBase):
    def __init__(
        ...
        super().__init__(cpu_group, device, device_group, unique_name, use_all2all=use_all2all)
        ...
        if self.use_all2all:
            if self.all2all_backend not in (
                ...
                logger.warning(
                    "`%s` all2all manager is not supported on CPU. Falling back to `allgather_reducescatter` manager.",
    ...
    def _all_group_ranks_share_shm_group_name(self) -> bool:
        ...
        This is a lightweight consistency check for APHRODITE_DIST_IDENT/name inputs.
        """
    ...
    def send_tensor_dict(
        ...
        if not self.supports_tensor_dict:
            raise NotImplementedError(
                "CpuCommunicator does not support tensor dict fastpath with torch.distributed backend."
            )
...
class _CPUSHMDistributed:
    ...
    def make_group_name(communicator: CpuCommunicator) -> str:
        instance_identifier = os.environ["APHRODITE_DIST_IDENT"]
