...
class ElasticEPScalingState:
    def __init__(
        ...
        if scale_type == "scale_up":
            self.state = (
                ScaleUpNewEngineState.PRE_KV_INIT if worker_type == "new" else ScaleUpExistingEngineState.PREPARE
            )
        else:
            self.state = (
    ...
    def _execute_async(self, execute_method: str, *args) -> bool:
        if self._prepare_future is None:
            ...
            self._prepare_future = self._prepare_executor.submit(coord_store.wait, done_keys)
    ...
    def _ensure_new_dp_group(self) -> bool:
        ...
        if self._prepare_future is None:
            self._prepare_future = self._prepare_executor.submit(self._init_new_dp_group)
        if not self._prepare_future.done():
            return False
