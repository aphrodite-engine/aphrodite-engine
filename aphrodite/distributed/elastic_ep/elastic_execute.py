...
from aphrodite.compilation.wrapper import reset_compile_wrapper
...
from aphrodite.distributed.elastic_ep.standby_state import (
...
from aphrodite.model_executor.layers.fused_moe.eep_reconfigure import (
...
from aphrodite.v1.worker.workspace import lock_workspace, unlock_workspace
...
class ElasticEPScalingExecutor:
    def __init__(self, worker):
        ...
        self._async_executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="ElasticEPAsync")
    ...
    def start_async(self, execute_method: str, *args, **kwargs) -> str:
        ...
        self._async_future = self._async_executor.submit(self._run_async, execute_method, *args, **kwargs)
    ...
    def _run_async(self, execute_method: str, *args, **kwargs) -> None:
        ...
        self.worker.aphrodite_config.enable_trace_function_call_for_thread()
        assert hasattr(self.worker, "device")
        current_platform.set_device(self.worker.device)
        with set_current_aphrodite_config(self.worker.aphrodite_config):
    ...
    def _start_group_cleanup(self, groups: tuple[GroupCoordinator | None, ...]) -> None:
        ...
        self._group_cleanup_future = self._async_executor.submit(self._destroy_retired_groups, groups)
    ...
    def prepare_reconfiguration(self, reconfig_request: ReconfigureDistributedRequest, use_all2all: bool) -> None:
        ...
        parallel_config = self.worker.aphrodite_config.parallel_config
    ...
    def _prepare_eplb_communicator(self, eplb_group) -> None:
        ...
        self._prepared_eplb_communicator = eplb_state.create_communicator(model_runner.model_config, eplb_group)
    ...
    def _make_eep_moe_config(self, module, dp_group, ep_group):
        ...
        moe_parallel_config = FusedMoEParallelConfig.make(
            ...
            sp_size_=sp_size,
            aphrodite_parallel_config=parallel_config,
        )
        return replace(
            module.moe_config,
    ...
    def switch_and_remove(self) -> None:
        ...
        retired_groups = _replace_active_groups(world=None, dp=None, ep=None, eplb=None, node_count=None)
    ...
    def receive_expert_mapping(self) -> torch.Tensor:
        ...
        tp_size = self.worker.aphrodite_config.parallel_config.tensor_parallel_size
        new_ep_size = new_dp_size * tp_size
        expanded_physical_to_logical = torch.full(
        ...
        expanded_physical_to_logical[:, : physical_to_logical.shape[1]] = physical_to_logical
