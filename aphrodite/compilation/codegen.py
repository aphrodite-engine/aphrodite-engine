...
def generate_execution_code_with_name(
    ...
    for i, node in enumerate(nodes):
        if node.op == "placeholder":
            ...
            if isinstance(submod, torch.fx.GraphModule):
                callable_name = f"__aphrodite_inlined_submods__{idx}"
                inlined_code, _ = generate_execution_code_with_name(submod, callable_name, with_submod=False)
                inlined_submods.append(inlined_code)
            else:
                callable_name = f"__aphrodite_submods__[{idx}]"
    ...
    header = f"\ndef {fn_name}({params}{', *, __aphrodite_submods__' if with_submod else ''}):"
    return "".join(inlined_submods) + "\n".join([header] + lines) + "\n", submod_names


@dynamo_timed("aphrodite.generate_execution_code")
def generate_execution_code(
    ...
    submodules via a __aphrodite_submods__ list, avoiding FX GraphModule overhead
    ...
    code, submod_names = generate_execution_code_with_name(split_gm, "execution_fn", with_submod=True)
    return "import torch\nimport operator\n" + code, submod_names


@dynamo_timed("aphrodite.compile_execution_fn")
def compile_execution_fn(
    ...
    return partial(fn, __aphrodite_submods__=submods_list)
