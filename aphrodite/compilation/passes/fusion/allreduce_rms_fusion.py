...
def _fused_ar_workspace_hidden_dim(config: AphroditeConfig) -> int:
    ...
    target-sized buffer (Aphrodite #52023).
    """
...
class AiterAllreduceFusedAddRMSNormOutputOnlyPattern(AiterAllreduceFusedAddRMSNormPattern):
    ...
    def pattern(self):
        ...
        def _pattern(residual: torch.Tensor, input: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
    ...
    def replacement(self):
        ...
        def _replacement(residual: torch.Tensor, input: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
...
class AiterAllreduceFusedAddRMSNormGroupQuantWithIndexerPattern(BasePattern, AphroditePatternReplacement):
    ...
    *and* a bf16 ``rocm_unquantized_gemm`` for the indexer ``wk_weights_proj``.
    ...
    lowers to (``aphrodite.rocm_aiter_group_fp8_quant``).
    """
