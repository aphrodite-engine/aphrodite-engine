...
_IR_RMS_NORM_OP = torch.ops.aphrodite_ir.rms_norm.default
...
def _select_flashinfer_allreduce_use_oneshot(
    ...
    if workspace_backend == "mnnvl":
        ...
        # Forcing Aphrodite's per-rank threshold can request one-shot for tensors
...
if flashinfer_comm is not None:
    from aphrodite.distributed.device_communicators.flashinfer_all_reduce import (
    ...
    def call_trtllm_fused_allreduce_norm(
        ...
        get_workspace_fn = get_fi_ar_quant_workspace if is_quant_pattern else get_fi_ar_workspace
        ...
        def unfused_fallback() -> None:
            ...
            gamma = rms_gamma if weight_bias == 0.0 else (rms_gamma.float() + weight_bias).to(rms_gamma.dtype)
        ...
        # FlashInfer sizes MNNVL's one-shot region using its own threshold.
        # Let its automatic strategy match that workspace sizing.
        ...
        try:
            flashinfer_comm.allreduce_fusion(
                ...
                trigger_completion_at_end=(
                    True if mnnvl_auto else (use_oneshot or num_tokens > PDL_ADVANCE_LAUNCH_TOKENS)
                ),
            )
        except ValueError as e:
            # FlashInfer raises before launching when the cached workspace is
            # too small, so the unfused path remains safe during capture.
            assert not is_quant_pattern, (
                f"flashinfer fused allreduce failed and the unfused fallback does not cover quant patterns: {e}"
            )
            logger.warning_once(
    ...
    flashinfer_trtllm_fused_allreduce_norm = torch.ops.aphrodite.flashinfer_trtllm_fused_allreduce_norm.default
...
class AllReduceFusedAddRMSNormPattern(BasePattern):
    ...
    def register(self, pm_pass: PatternMatcherPass) -> None:
        def pattern(
            ...
            allreduce_output = tensor_model_parallel_all_reduce(input)
            rms, residual = aphrodite.ir.ops.fused_add_rms_norm(allreduce_output, residual, weight, self.epsilon)
            return rms, residual
...
class AllReduceGemmaRMSNormPattern(BasePattern):
    ...
    def register(self, pm_pass: PatternMatcherPass) -> None:
        def pattern(input: torch.Tensor, weight: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
            allreduce_output = tensor_model_parallel_all_reduce(input)
            rms = aphrodite.ir.ops.rms_norm(allreduce_output, weight.float() + 1.0, self.epsilon)
            return rms, allreduce_output
...
class AllReduceFusedAddGemmaRMSNormPattern(BasePattern):
    ...
    def register(self, pm_pass: PatternMatcherPass) -> None:
        def pattern(
            ...
            allreduce_output = tensor_model_parallel_all_reduce(input)
            rms, residual = aphrodite.ir.ops.fused_add_rms_norm(
...
class AllReduceFusedRMSNormStaticQuantFP8Pattern(BasePattern):
    ...
    def register(self, pm_pass: PatternMatcherPass) -> None:
        def pattern(
            ...
            rms = aphrodite.ir.ops.rms_norm(all_reduce, weight, self.epsilon)
...
class AllReduceFusedAddRMSNormStaticQuantFP8Pattern(BasePattern):
    ...
    def register(self, pm_pass: PatternMatcherPass) -> None:
        def pattern(
            ...
            allreduce_output = tensor_model_parallel_all_reduce(input)
            rms, res = aphrodite.ir.ops.fused_add_rms_norm(allreduce_output, residual, weight, self.epsilon)
            quant, _ = self.quant_matcher(rms, scale)
...
class AllReduceFusedRMSNormStaticQuantNVFP4Pattern(BasePattern):
    ...
    def get_inputs(self) -> list[torch.Tensor]:
        ...
        input_global_scale = torch.empty([1, 1], device=self.device, dtype=torch.float32)
...
class AllReduceFusionPass(AphroditePatternMatcherPass):
    def __init__(self, config: AphroditeConfig) -> None:
        ...
        if self.tp_size <= 1:
            ...
            return
        self.patterns: PatternMatcherPass = PatternMatcherPass(pass_name="all_reduce_fusion_pass")
        ...
        if max_size is None:
            ...
            logger.warning(
                "Flashinfer allreduce fusion is not supported for world size %s or max size is not provided",
                self.tp_size,
            )
            return
        ...
        self.max_token_num = min(self.max_token_num, config.scheduler_config.max_num_batched_tokens)
        logger.debug_once(
        ...
        self.supports_quant_fusion = get_fi_ar_quant_workspace(**workspace_kwargs) is not None
...
class AiterAllreduceFusedRMSNormGroupQuantFP8Pattern(BasePattern, AphroditePatternReplacement):
    ...
    def __init__(
        ...
        self.FUSED_AR_RMS_QUANT_OP = rocm_aiter_ops.get_fused_allreduce_rmsnorm_quant_per_group_op()
...
class AiterAllreduceFusedAddRMSNormGroupQuantFP8Pattern(BasePattern, AphroditePatternReplacement):
    ...
    def pattern(self):
        def _pattern(
            ...
            allreduce_output = tensor_model_parallel_all_reduce(input)
            rms, residual_out = aphrodite.ir.ops.fused_add_rms_norm(allreduce_output, residual, weight, self.epsilon)
            quant, scale = self.quant_matcher(rms)
...
class AiterAllreduceFusedAddRMSNormGroupQuantWithIndexerPattern(BasePattern, AphroditePatternReplacement):
    ...
    *and* a bf16 ``rocm_unquantized_gemm`` for the indexer ``wk_weights_proj``.
    ...
    lowers to (``aphrodite.triton_per_token_group_quant_fp8`` or
    ``aphrodite.rocm_aiter_group_fp8_quant``).
    """
    ...
    def pattern(self):
        ...
        def _pattern(
            ...
            rms, res_out = aphrodite.ir.ops.fused_add_rms_norm(ar_out, residual, norm_weight, eps)
            q, s = self.quant_matcher(rms)
            idx = torch.ops.aphrodite.rocm_unquantized_gemm(rms, indexer_weight)
    ...
    def replacement(self):
        ...
        def _replacement(
            ...
            idx = torch.ops.aphrodite.rocm_unquantized_gemm(bf16_norm, indexer_weight)
...
class RocmAiterAllReduceFusionPass(AphroditeFusionPatternMatcherPass):
    def __init__(self, config: AphroditeConfig) -> None:
        ...
        if ca_comm is None:
            logger.warning_once(
                "AITER allreduce fusions are disabled because AITER Custom "
                "All Reduce is not enabled. Set "
                "APHRODITE_ROCM_USE_AITER_CUSTOM_AR=1 to enable it."
            )
            return
        ...
        if not ca_comm.supports_dynamic_hidden_dim and hidden_dim not in _AITER_OLD_FUSED_AR_RMS_HIDDEN:
            logger.warning_once(
