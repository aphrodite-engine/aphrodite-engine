...
class _SequenceParallelPatternHelper:
    ...
    def _all_gather(self, x: torch.Tensor) -> torch.Tensor:
        return torch.ops.aphrodite.all_gather.default(
            x, dim=0, world_size=self.tp_size, group_name=self.tp_group.unique_name
        )
...
class FirstAllReduceRMSNormPattern(_SequenceParallelPatternHelper):
    ...
    def register(self, pm_pass: PatternMatcherPass) -> None:
        def pattern(
            ...
            rmsnorm = aphrodite.ir.ops.rms_norm(all_reduce, weight, self.epsilon)
        ...
        def replacement(
            ...
            rmsnorm = aphrodite.ir.ops.rms_norm(reduce_scatter, weight, self.epsilon)
...
class FirstAllReduceRMSNormStaticFP8Pattern(_SequenceParallelPatternHelper):
    ...
    def register(self, pm_pass: PatternMatcherPass) -> None:
        ...
        def replacement(
            ...
            reduce_scatter = self._reduce_scatter(input)
            rms = aphrodite.ir.ops.rms_norm(reduce_scatter, weight, self.epsilon)
            quant, _ = self.quant_matcher(rms, scale)
