...
def make_compiler(compilation_config: CompilationConfig) -> CompilerInterface:
    if compilation_config.use_inductor:
        if envs.APHRODITE_USE_STANDALONE_COMPILE and is_torch_equal_or_newer(
                "2.8.0.dev"):
...
class CompilerManager:
    ...
    def load(self,
        ...
        if runtime_shape is None:
            logger.debug(
                "Directly load the {}-th graph for dynamic shape from {} via "
                "handle {}", graph_index, self.compiler.name, handle)
        else:
            logger.debug(
                "Directly load the {}-th graph for shape {} from {} via "
                "handle {}", graph_index, str(runtime_shape),
    ...
    def compile(self,
            ...
            if graph_index == num_graphs - 1:
                ...
                if runtime_shape is None:
                    ...
                    logger.debug(
                        ...
                        "from the cache, took {:.3f} s", runtime_shape,
                        elapsed)
            return compiled_graph
        ...
        if not envs.APHRODITE_DISABLE_COMPILE_CACHE and handle is not None:
            ...
            if runtime_shape is None:
                logger.debug(
                    "Store the {}-th graph for dynamic shape from {} via "
                    "handle {}", graph_index, self.compiler.name, handle)
            else:
                logger.debug(
                    "Store the {}-th graph for shape {} from {} via "
                    "handle {}", graph_index, str(runtime_shape),
                    self.compiler.name, handle)
        ...
        if graph_index == num_graphs - 1:
            ...
            if runtime_shape is None:
                logger.debug("Compiling a graph for dynamic shape takes {:.2f} s",
                             elapsed)
            else:
...
class AphroditeBackend:
    ...
    def __call__(self, graph: fx.GraphModule, example_inputs) -> Callable:
        ...
        if not self.compilation_config.cache_dir:
            ...
            # APHRODITE_PP_LAYER_PARTITION will affects the computation graph.
