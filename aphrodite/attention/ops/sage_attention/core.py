...
from .quant_per_block import per_block_int8 as per_block_int8_triton
from .quant_per_block_varlen import (
    ...
    **kwargs: Any,
    ...
    if arch == "8.0":
        ...
        return sageattn_qk_int8_pv_fp8_cuda(
            ...
            return_lse=return_lse,
            pv_accum_dtype="fp32+fp16",
        )  # sm120 has accurate fp32 accumulator for fp8 mma and triton kernel
        # is currently not usable on sm120.
    else:
...
def sageattn_qk_int8_pv_fp16_triton(
    ...
    **kwargs: Any,
    ...
    if attn_mask is not None:
        assert attn_mask.dtype == torch.bool or attn_mask.dtype == q.dtype, (
    ...
    if smooth_k:
        ...
        if return_lse:
            if tensor_layout == "NHD":
                lse_correction = (
                    torch.matmul(
                        q.transpose(1, 2), km.transpose(1, 2).transpose(2, 3)
                    )
                    .squeeze(-1)
                    .to(torch.float32)
                )
                ...
                lse_correction = (
                    torch.matmul(q, km.transpose(2, 3))
                    .squeeze(-1)
                    .to(torch.float32)
                )
    else:
    ...
    if quantization_backend == "triton":
        ...
        raise ValueError(
            f"Unsupported quantization backend: {quantization_backend}"
        )
    if is_causal:
        ...
        if attn_mask is not None:
            if tensor_layout == "HND":
                ...
                raise ValueError(
                    f"tensor_layout {tensor_layout} not supported")
            try:
                attn_mask = attn_mask.expand(target_shape)
            except Exception as e:
                raise AssertionError(
                    f"attn_mask shape {attn_mask.shape} cannot be broadcast "
                    f"to {target_shape}"
                ) from e
        o, lse = attn_false(
            q_int8,
...
def sageattn_varlen(
    ...
    **kwargs: Any,
    ...
    if smooth_k:
        km = k.mean(
            dim=0, keepdim=True
        )  # ! km is calculated on the all the batches. Calculate over each
        # individual sequence requires dedicated kernel.
        k = k - km

    if sm_scale is None:
        sm_scale = 1.0 / (head_dim_og**0.5)

    q_int8, q_scale, k_int8, k_scale, cu_seqlens_q_scale, cu_seqlens_k_scale = (  # noqa: E501
...
def sageattn_qk_int8_pv_fp16_cuda(
    ...
    **kwargs: Any,
    ...
    dtype = q.dtype
    assert current_platform.get_device_capability().as_version_str() == "8.0", (  # noqa: E501
        "SM80 kernel is not available. make sure you GPUs with compute"
    ...
    if qk_quant_gran == "per_warp":
        q_int8, q_scale, k_int8, k_scale = per_warp_int8_cuda(
            ...
            WARPQ=(
                16
                if (q.size(-1) == 128 and pv_accum_dtype == "fp16+fp32")
                else 32
            ),
            BLKK=64,
        )
    ...
    if pv_accum_dtype in ["fp32", "fp16+fp32"] and smooth_v:
        logger.warning(
            f"pv_accum_dtype is {pv_accum_dtype}, smooth_v will be ignored."
        )
        smooth_v = False
...
def sageattn_qk_int8_pv_fp8_cuda(
    ...
    **kwargs: Any,
    ...
    dtype = q.dtype
    assert current_platform.get_device_capability().as_version_str() == "8.9", (  # noqa: E501
        "SM89 kernel is not available. Make sure you GPUs with compute"
        " capability 8.9."
    )
    assert q.is_cuda, "Input tensors must be on cuda."
    assert dtype in [torch.float16, torch.bfloat16], (
    ...
    if pv_accum_dtype == "fp32+fp32" and smooth_v:
        logger.warning(
            "pv_accum_dtype is 'fp32+fp32', smooth_v will be ignored."
        )
        smooth_v = False

    if pv_accum_dtype == "fp32+fp16" and smooth_v:
        logger.warning(
            "pv_accum_dtype is 'fp32+fp16', smooth_v will be ignored."
        )
        smooth_v = False
...
def sageattn_qk_int8_pv_fp8_cuda_sm90(
    ...
    **kwargs: Any,
    ...
    dtype = q.dtype
    assert current_platform.get_device_capability().as_version_str() == "9.0", (  # noqa: E501
        "SM90 kernel is not available. Make sure you GPUs with compute"
        " capability 9.0."
    )
    assert q.is_cuda, "Input tensors must be on cuda."
    assert dtype in [torch.float16, torch.bfloat16], (
    ...
    if pv_accum_dtype == "fp32":
        raise NotImplementedError(
            "Please use pv_accum_dtype='fp32+fp32' for sm90."
        )
        lse = ops.qk_int8_sv_f8_accum_f32_fuse_v_scale_attn(
            q_int8,
            k_int8,
