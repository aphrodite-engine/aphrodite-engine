...
def per_block_int8(
    ...
    """
    Quantize the query tensor `q` and the key tensor `k` with per block
    quantization.
...
def per_channel_fp8(
    ...
    ops.transpose_pad_permute(v, v_transposed_permutted, _tensor_layout)

    v_fp8 = torch.empty(
        v_transposed_permutted.shape, dtype=torch.float8_e4m3fn,
        device=v.device
    )
