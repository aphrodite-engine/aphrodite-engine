...
def _attn_fwd(Q, K, V,
    ...
    k_scale_offset = cu_seq_lens_k_scale_start * (
    ...
    Q_ptrs = Q + ((cu_seqlens_q_start * stride_qn + off_h * stride_qh) +
                  offs_m[:, None] * stride_qn + offs_k[None, :])
    Q_scale_ptr = Q_scale + q_scale_offset
    K_ptrs = K + (cu_seqlens_k_start * stride_kn + (off_h // num_kv_groups) *
                  stride_kh) + offs_n[None, :] * stride_kn + offs_k[:, None]
    K_scale_ptr = K_scale + k_scale_offset
    V_ptrs = V + (cu_seqlens_k_start * stride_vn + (off_h // num_kv_groups) *
                  stride_vh) + offs_n[:, None] * stride_vn + offs_k[None, :]
    O_block_ptr = Out + ((cu_seqlens_q_start * stride_on + off_h * stride_oh) +
                         offs_m[:, None] * stride_on + offs_k[None, :])
    ...
    acc, l_i, m_i = _attn_fwd_inner(
        acc, l_i, m_i, q, q_scale, kv_len, K_ptrs,
        K_scale_ptr, V_ptrs, stride_kn, stride_vn,
