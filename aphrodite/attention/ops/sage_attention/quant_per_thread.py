...
def quant_query_per_thread_int8_kernel(
    ...
    scale_ptrs = (
        Scale + off_b * stride_sz + off_h * stride_sh + off_blk * 8 + off_tld
    )
...
def quant_key_per_thread_int8_kernel(
    ...
    output_ptrs1 = (
        ...
        + off_b * stride_oz
        + off_h * stride_oh
        + offs_n1[:, None] * stride_on
        + offs_k[None, :]
    )
    scale_ptrs = (
        Scale + off_b * stride_sz + off_h * stride_sh + off_blk * 4 + off_tld
    )
...
@triton.jit
def quant_query_per_thread_int4_kernel(
    Input,
    Output,
    Scale,
    L,
    ...
    x = tl.load(input_ptrs, mask=offs_n[:, None] < L)
    x = x.to(tl.float32)
    scale = tl.max(tl.abs(x)) / 7.0 + 0.0000001
    x_int8 = x / scale
    x_int8 += 0.5 * tl.where(x_int8 >= 0, 1, -1)
    x_int8 = x_int8.to(tl.int8)
    tl.store(output_ptrs, x_int8, mask=offs_n[:, None] < L)
...
@triton.jit
def quant_key_per_thread_int4_kernel(
    Input,
    Output,
    Scale,
    L,
    ...
    offs_n = (
        off_blk * BLK
        + tl.cat(
            tl.arange(0, BLK // 8) * 8, tl.arange(0, BLK // 8) * 8 + 1, True
        )
        + off_tld * 2
    )
    offs_k = tl.arange(0, C)
