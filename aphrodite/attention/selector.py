...
def _cached_get_attn_backend(
    ...
    if backend_by_global_setting is not None:
        ...
        backend_by_env_var: str | None = envs.APHRODITE_ATTENTION_BACKEND
        if backend_by_env_var is not None:
            if backend_by_env_var.endswith("_APHRODITE_V1"):
                logger.warning(
                    "The suffix '_APHRODITE_V1' in the environment variable "
                    "%s is no longer necessary as V0 backends have been "
                    "deprecated. Please remove this suffix from your "
                    "environment variable setting.",
                    STR_BACKEND_ENV_VAR,
                )
                backend_by_env_var = backend_by_env_var.removesuffix("_APHRODITE_V1")
