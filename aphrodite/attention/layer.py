...
class Attention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        self.kv_cache = [
            torch.tensor([]) for _ in range(get_current_aphrodite_config(
    ...
    def process_weights_after_loading(self, act_dtype: torch.dtype):
        ...
        if (self.backend == _Backend.FLASHINFER_APHRODITE_V1
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        if current_platform.is_rocm():
            ...
            if backend in (_Backend.FLASH_ATTN, _Backend.FLASH_ATTN_APHRODITE_V1,
                           _Backend.FLEX_ATTENTION):
                backend = _Backend.XFORMERS

            self.attn_backend = backend if backend in {
                _Backend.TORCH_SDPA, _Backend.XFORMERS, _Backend.PALLAS_APHRODITE_V1
    ...
    def forward(
        ...
        elif self.attn_backend == _Backend.PALLAS_APHRODITE_V1:
            query, key, value = (x.transpose(1, 2)
                                 for x in (query, key, value))
