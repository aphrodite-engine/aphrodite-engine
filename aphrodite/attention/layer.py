...
from aphrodite.utils import GiB_bytes, direct_register_custom_op
...
class Attention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        try:
            ...
            logger.error("Failed to initialize attention q/k/v range constants: {}", e)
            if torch.cuda.is_available():
                logger.debug("CUDA device: {}", torch.cuda.current_device())
                logger.debug(
    ...
    def forward(
        ...
        if self.calculate_kv_scales:
            torch.ops.aphrodite.maybe_calc_kv_scales(query, key, value, self.layer_name)
            ...
            if self.attn_backend.accept_output_buffer:
                ...
                if self.calculate_kv_scales:
                    ...
                    if getattr(attn_metadata, "enable_kv_scales_calculation", False):
                        self.calc_kv_scales(q, kv_c_normed, k_pe)
                return torch.ops.aphrodite.unified_mla_attention(
                    q,
                    kv_c_normed,
                    k_pe,
                    self.layer_name,
