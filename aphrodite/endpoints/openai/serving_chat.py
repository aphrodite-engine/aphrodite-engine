...
from aphrodite.endpoints.openai.protocol import (
    ...
    ToolCall, UsageInfo)
from aphrodite.endpoints.openai.reasoning_parsers import (
...
class OpenAIServingChat(OpenAIServing):
    ...
    async def create_chat_completion(
        ...
        if error_check_ret is not None:
            logger.error(f"Error with model {error_check_ret}")
    ...
    async def chat_completion_stream_generator(
        ...
        try:
            ...
            # TODO: Use an aphrodite-specific Validation Error
            logger.error(f"error in chat completion stream generator: {e}")
            data = self.create_streaming_error_response(str(e))
            yield f"data: {data}\n\n"
