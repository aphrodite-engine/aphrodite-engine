...
class DeepSeekV31ToolParser(ToolParser):
    def __init__(self, tokenizer: AnyTokenizer):
        ...
        self.stream_tool_call_name_regex = re.compile(r"(?P<function_name>.*)<｜tool▁sep｜>")
        ...
        if self.tool_calls_start_token_id is None or self.tool_calls_end_token_id is None:
            raise RuntimeError(
                "DeepSeek-V3.1 Tool parser could not locate tool call start/end tokens in the tokenizer!"
            )
