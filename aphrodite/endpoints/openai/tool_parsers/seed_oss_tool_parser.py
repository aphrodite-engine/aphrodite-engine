    ...
    def _parse_xml_function_call(
        ...
        def convert_param_value(param_value: str, param_name: str,
            ...
            if param_name not in param_config:
                if param_config != {}:
                    logger.warning(
                        "Parsed parameter '{}' is not defined in "
                        "the tool parameters for tool '{}', "
            ...
            if param_type in [
                ...
                try:
                    ...
                    logger.warning(
                        "Parsed value '{}' of parameter '{}' is not an integer in tool "
                        "'{}', degenerating to string.", param_value,
                ...
                try:
                    ...
                    logger.warning(
                        "Parsed value '{}' of parameter '{}' is not a float in tool "
                        "'{}', degenerating to string.", param_value,
                        param_name, func_name)
                return param_value
                ...
                if param_value not in ["true", "false"]:
                    logger.warning(
                        "Parsed value '{}' of parameter '{}' is not a boolean "
                        "(`true` of `false`) in tool '{}', degenerating to false.",
                ...
                if param_type == "object" or param_type.startswith("dict"):
                    try:
                        ...
                        logger.warning(
                            "Parsed value '{}' of parameter '{}' is not a valid JSON "
                            "object in tool '{}', will try other methods to parse it.",
                            param_value, param_name, func_name)
                try:
                    param_value = ast.literal_eval(param_value)
                except (ValueError, SyntaxError):
                    logger.warning(
                        "Parsed value '{}' of parameter '{}' cannot be converted via "
                        "Python `ast.literal_eval()` in tool '{}', degenerating to string.",
                        param_value, param_name, func_name)
                return param_value
