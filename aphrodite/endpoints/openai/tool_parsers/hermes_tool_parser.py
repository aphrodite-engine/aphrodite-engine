...
class Hermes2ProToolParser(ToolParser):
    ...
    def extract_tool_calls_streaming(
        ...
        logger.debug("delta_text: {}", delta_text)
        logger.debug("delta_token_ids: {}", delta_token_ids)
        ...
        try:
            ...
            if (cur_tool_start_count > cur_tool_end_count
                ...
                logger.debug("Starting on a new tool {}", self.current_tool_id)
            ...
            try:
                ...
                logger.debug("Parsed tool call {}", current_tool_call)
            ...
            logger.debug("diffing old arguments: {}", prev_arguments)
            logger.debug("against new ones: {}", cur_arguments)

            # case -- no arguments have been created yet. skip sending a delta.
            if not cur_arguments and not prev_arguments:
                logger.debug("Skipping text {} - no arguments", delta_text)
                delta = None
            ...
            #   autocompleting the JSON
                ...
                cur_arguments_json = json.dumps(cur_arguments,
                                                ensure_ascii=False)
                logger.debug("finding {} in {}", delta_text,
                             cur_arguments_json)
            ...
            # last case -- we have an update to existing arguments.
                ...
                logger.debug("got diff {}", delta_text)
