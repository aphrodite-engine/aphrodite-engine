...
from aphrodite.endpoints.context import ConversationContext, SimpleContext
...
class OpenAIServingResponses(OpenAIServing):
    ...
    def __init__(
        ...
        *,
        ...
        # NOTE: This may not be intuitive for users, as the default
        ...
        if self.enable_store:
            log_once(
                ...
                "the store.")
        # HACK: This is a hack. We should use a better store.
        ...
        if request.store and not self.enable_store:
            if request.background:
                return self.create_error_response(
                    ...
                    message=(
                        "This Aphrodite engine does not support `store=True` "
                        "and therefore does not support the background mode. "
                        "To enable these features, set the environment "
                        "variable `APHRODITE_ENABLE_RESPONSES_API_STORE=1` "
                        "when launching the Aphrodite server."),
                    status_code=HTTPStatus.BAD_REQUEST,
                )
            # Disable the store option.
            # NOTE: Although returning an error is possible, we opted
    ...
    def _make_store_not_supported_error(self) -> ErrorResponse:
        return self.create_error_response(
            ...
            message=("`store=True` (default) is not supported. Please set "
                     ...
                     "`APHRODITE_ENABLE_RESPONSES_API_STORE=1` in the env "
                     "var when starting the Aphrodite server."),
            status_code=HTTPStatus.BAD_REQUEST,
        )
