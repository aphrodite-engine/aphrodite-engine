...
class StreamingXMLToolCallParser:
    ...
    def parse_single_streaming_chunks(self, xml_chunk: str) -> DeltaMessage:
        ...
        if entry_call_id is not None:
            if self.current_call_id == entry_call_id and self.tool_call_index == entry_tool_call_index:
                fallback_call_id = entry_call_id
        elif self.current_call_id is not None and self.tool_call_index == entry_tool_call_index + 1:
        ...
        if found_elements:
            ...
            try:
                ...
                if fallback_call_id is not None and self.function_end_token in xml_chunk:
                ...
                if fallback_call_id is not None and self.tool_call_end_token in xml_chunk:
            ...
            result_delta = self._merge_new_deltas_to_single_response(initial_delta_count)
            return result_delta
        else:
            ...
            if fallback_call_id is not None and (
                self.function_end_token in xml_chunk or self.tool_call_end_token in xml_chunk
            ):
                # Close potentially unclosed element
                if self.current_param_name:
                    self._end_element("parameter")
    ...
    def _fix_incomplete_tag_in_chunk(self, chunk: str) -> str:
        ...
        for tag_type in ["parameter", "function"]:
            ...
            if gt_pos != -1 and (lt_pos == -1 or gt_pos < lt_pos) and pattern in after_tag[:gt_pos]:
                continue
            ...
            while tag_name and not (tag_name[-1].isalnum() or tag_name[-1] in ("-", "_")):
                tag_name = tag_name[:-1]
            ...
            # Apply fix
            chunk = chunk.replace(f"<{tag_type}={content[:end_pos]}", f"<{tag_type}={tag_name}>", 1)
    ...
    def _find_next_complete_element(self, start_pos: int) -> tuple[str | None, int]:
        ...
        if buffer.startswith("<"):
            ...
            is_incomplete_param = buffer.startswith("<parameter=") and ">" not in buffer.split("\n")[0]
            is_incomplete_func = buffer.startswith("<function=") and ">" not in buffer.split("\n")[0]
            ...
            if tag_end != -1 and tag_end2 != -1:
                ...
                if self.current_call_id is None:
                    ...
                    if buffer == "<tool_call>"[: len(buffer)]:
                        ...
                        return None, start_pos
                    elif buffer.startswith("<function=") or buffer == "<function="[: len(buffer)]:
    ...
    def _preprocess_xml_chunk(self, chunk: str) -> str:
        ...
        is_tool_call = False
        if chunk.startswith(self.tool_call_start_token) or chunk.startswith(self.tool_call_end_token):
            is_tool_call = True
        ...
        if (
            ...
            is_tool_call = True
        if chunk.startswith(self.parameter_start_token) or chunk.startswith(self.parameter_end_token):
            is_tool_call = True
        ...
        # parameter tag
        if self.current_call_id is not None or chunk.startswith("<function") or chunk.startswith("<parameter"):
        ...
        if self._pre_inside_parameter:
            ...
            if processed.startswith("</parameter>"):
                ...
                if self._pre_param_buffer == "":
                    ...
                    param_type = (
                        self._get_param_type(self._pre_current_param_name) if self._pre_current_param_name else "string"
                    )
                    ...
                    has_container_hint = ("[" in original_chunk) or ("{" in original_chunk) or ("(" in original_chunk)
                    ...
                    if is_complex_type:
                        ...
                        need_defer = True
                    elif is_object_type and has_container_hint and ("'" in original_chunk):
                    ...
                    delta = DeltaMessage(
                        tool_calls=[
                            DeltaToolCall(
                                ...
                                id=self.current_call_id,
                                type="function",
                                function=DeltaFunctionCall(name=None, arguments=json_start),
                            )
                        ]
                    )
                    self._emit_delta(delta)
                    ...
                    delta = DeltaMessage(
                        tool_calls=[
                            DeltaToolCall(
                                ...
                                id=self.current_call_id,
                                type="function",
                                function=DeltaFunctionCall(name=None, arguments=json_continue),
                            )
                        ]
                    )
                    self._emit_delta(delta)
    ...
    def _char_data(self, data: str):
        ...
        if data and self.current_param_name:
            ...
            if param_type in ["string", "str", "text", "varchar", "char", "enum"] and not self.start_quote_emitted:
            ...
            converted_value = self._convert_param_value(self.current_param_value, param_type)
    ...
    def _end_element(self, name: str):
        ...
        if (name.startswith("function") or name == "function" or name == "tool_call") and self.current_param_name:
            self._auto_close_open_parameter_if_needed()

        if (name.startswith("parameter") or name == "parameter") and self.current_param_name:
            ...
            if self.defer_current_parameter:
                raw_text = self.deferred_param_raw_value if self.deferred_param_raw_value else param_value
                parsed_value = None
                ...
                delta = DeltaMessage(
                    tool_calls=[
                        DeltaToolCall(
                            ...
                            id=self.current_call_id,
                            type="function",
                            function=DeltaFunctionCall(name=None, arguments=output_arguments),
                        )
                    ]
                )
                self._emit_delta(delta)
    ...
    def _get_param_type(self, param_name: str) -> str:
        ...
        for tool in self.tools:
            if not hasattr(tool, "type") or not (hasattr(tool, "function") and hasattr(tool.function, "name")):
                continue
            if tool.type == "function" and tool.function.name == self.current_function_name:
                if not hasattr(tool.function, "parameters"):
                    return "string"
                params = tool.function.parameters
                if isinstance(params, dict) and "properties" in params:
                    ...
                    if param_name in properties and isinstance(properties[param_name], dict):
                        return self.repair_param_type(str(properties[param_name].get("type", "string")))
...
class Step3p5ToolParser(ToolParser):
        ...
        if not result.tool_calls:
            ...
            for tool_call in result.tool_calls:
                if tool_call.function and tool_call.function.name:
                    ...
                    tool_index = tool_call.index if tool_call.index is not None else len(self.prev_tool_call_arr) - 1
                    ...
                    self.prev_tool_call_arr[tool_index]["name"] = tool_call.function.name
                    self.prev_tool_call_arr[tool_index]["arguments"] = tool_call.function.arguments

                    # Update streamed arguments
                    if tool_call.function.arguments:
                        self.streamed_args_for_tool[tool_index] = tool_call.function.arguments
    ...
    def extract_tool_calls_streaming(
        ...
        if not delta_text and delta_token_ids:
            open_calls = current_text.count(self.parser.tool_call_start_token) - current_text.count(
                self.parser.tool_call_end_token
            )
            if open_calls == 0 and self.parser.tool_call_index > 0 or not self.parser.tool_call_index and current_text:
