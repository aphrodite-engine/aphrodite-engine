...
from aphrodite.tool_parsers.utils import partial_tag_overlap
...
# =============================================================================
# Plain-dict contract with the Aphrodite-side wrapper
# =============================================================================
...
# NOTE: mirrored in ``aphrodite.reasoning.hy_v4_reasoning_parser`` (same pattern as
...
def detect_token_suffix(tokenizer: TokenizerLike) -> str:
    ...
    if int(transformers.__version__.split(".")[0]) >= 5:
        ...
        think_begin_as_special = init_kwargs.get("model_specific_special_tokens", {}).get("think_begin_token", "")
    ...
    structural_token_re = re.compile(r"<(?:think|tool_calls|tool_call|arg_key|arg_value)(:[^\s>]+)?>")
...
class HYV4ToolExtractor:
    """Pure tool-call parsing logic for HYV4. Returns plain dicts; no Aphrodite types.
    ...
    def __init__(self, vocab: dict[str, int], token_suffix: str, strict: bool):
        ...
        # Running state read by the Aphrodite serving layer (mirrored by the wrapper).
        ...
        if self.tool_calls_start_token_id is None or self.tool_calls_end_token_id is None:
            raise RuntimeError("HYV4 tool extractor could not locate tool call start/end tokens in the tokenizer!")
    ...
    def _extract_tool_calls_strict(
        ...
        tool_block = model_output[start_idx + len(self.tool_calls_start_token) : end_idx]
        if tool_block.count(self.tool_call_start_token) != tool_block.count(self.tool_call_end_token):
        ...
        for tool_call_body in function_calls:
            ...
            ) or function_args.count(self.arg_value_start_token) != function_args.count(self.arg_value_end_token):
    ...
    def extract_tool_calls(
        ...
        if self.tool_calls_start_token not in model_output:
            return ExtractResult(tools_called=False, content=model_output, tool_calls=[])
        else:
            try:
                if self._strict:
                ...
                logger.warning("HYV4ToolExtractor detected malformed tool output; falling back to raw content.")
                return ExtractResult(
                    tools_called=False,
    ...
    def extract_tool_calls_streaming(
        ...
        *,
        ...
        while True:
            ...
            if self._streaming_tool_name is None and self._buffer.startswith(self.tool_call_start_token):
                continue
            break
    ...
    def _extract_streaming_incremental(
        ...
        for key, value in arg_pairs:
            ...
            if key not in self._completed_args:
                self._completed_args[key] = _parse_value(value, self._streaming_tool_name or "", key, tools)
            ...
            if ak_end != -1:
                ...
                if av_start != -1:
                    ...
                    if self._current_arg_is_string:
                        ...
                        keep = len(raw_value) - partial_tag_overlap(raw_value, self.arg_value_end_token)
...
def build_tool_extractor(tokenizer: TokenizerLike, *, strict: bool) -> HYV4ToolExtractor:
    return HYV4ToolExtractor(tokenizer.get_vocab(), detect_token_suffix(tokenizer), strict)
...
def _tools_to_plain(tools) -> list[ToolSchema] | None:
    ...
    if not tools:
        return None
    return [ToolSchema(name=t.function.name, parameters=t.function.parameters) for t in tools]


class HYV4ToolParser(ToolParser):
    """Aphrodite adapter around :class:`HYV4ToolExtractor`.

    Converts the extractor's plain dicts to Aphrodite protocol objects, mints
    ...
    def get_structural_tag(
        ...
        *,
        ...
        if tool_choice is None or tool_choice == "auto" or getattr(tool_choice, "mode", None) == "auto":
            return None
        ...
        try:
            ...
            logger.warning(
                "HYV4ToolParser failed to build a structural tag; falling back to the default guided-decoding path.",
                exc_info=True,
            )
            return None
    ...
    def _is_guided(request: ChatCompletionRequest) -> bool:
        ...
        return structured_outputs is not None and structured_outputs.structural_tag is not None
