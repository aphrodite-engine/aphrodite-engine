...
class DotsToolParser(ToolParser):
    ...
    def _resolve_param_type(
        ...
        if isinstance(ref, str) and ref.startswith("#/$defs/"):
            return self._resolve_param_type(defs.get(ref.rsplit("/", 1)[-1]), defs, depth + 1)
    ...
    def _parse_xml_invoke(
        ...
        for parameter in self._parameter_regex.finditer(match.group("body")):
            ...
            if param_name in properties:
                param_type = self._resolve_param_type(properties[param_name], defs) or "string"
    ...
    def _parse_block(
        ...
        if content.startswith("<invoke"):
            return [self._parse_xml_invoke(match, tools) for match in self._invoke_regex.finditer(content)]
    ...
    def extract_tool_calls_streaming(
        ...
        while self._buffer:
            ...
            if marker_index == -1:
                partial_len = partial_tag_overlap(self._buffer, self.tool_call_start_token)
                if partial_len:
                    ...
                    self._buffer = ""
                normal_parts = [part.replace(self.tool_call_end_token, "") for part in normal_parts]
                break
            ...
            end_index = self._buffer.find(self.tool_call_end_token, len(self.tool_call_start_token))
            if end_index == -1:
            ...
            try:
                ...
                valid_calls = [
                    ...
                    if (validated := self._validated_call(parsed, request.tools)) is not None
                ]
