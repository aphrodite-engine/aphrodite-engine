...
class KimiK3ToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: TokenizerLike, tools: list[Tool] | None = None):
        ...
        # path leaves Aphrodite's added-token spacing on -- this `\s*` is a no-op on
    ...
    def adjust_request(
        ...
        # that, and Aphrodite's detokenizer couples them:
    ...
    def _attrs(self, s: str) -> dict[str, str]:
        return {m["k"]: m["v"].replace("&quot;", '"').replace("&amp;", "&") for m in self._attr_re.finditer(s)}
    ...
    def extract_tool_calls_streaming(
        ...
        calls = [
            tc for m in self._call_re.finditer(section) if (tc := self._decode_call(m["attrs"], m["body"])) is not None
        ]
