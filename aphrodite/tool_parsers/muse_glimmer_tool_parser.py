...
#                                                      / untagged content)
_MSG_HEADER_RE = re.compile(r"(?:<\|start\|>\s*assistant)?[^\S\n]*(?:to=(?P<rcpt>[A-Za-z0-9_.\-]+))?<\|message\|>")
...
class MuseGlimmerToolParser(ToolParser):
    ...
    # named/required tool_choice path in aphrodite/parser/abstract_parser.py assigns
    ...
    def __init__(
        ...
        # Streaming cursors. Aphrodite constructs one ToolParser per request, so
    ...
    def adjust_request(
        ...
        if request.tools and (
            ...
            or isinstance(tool_choice, (ChatCompletionNamedToolChoiceParam, ToolChoiceFunction))
        ):
            return request
    ...
    def _tool_channel_text(cls, text: str) -> str:
        ...
        bodies = [
            ...
            if rcpt is not None and rcpt != _REASONING_RECIPIENT and rcpt != _USER_RECIPIENT
        ]
        if bodies:
            return "\n".join(bodies)
        if _MSG_HEADER_RE.search(text) is None and (_FUNCTION_CALLS_OPEN in text or "<atem:invoke" in text):
            logger.warning("MuseGlimmer: ATEM markup with no channel framing; is skip_special_tokens enabled upstream?")
            return text
        return ""
    ...
    def _normalize_name(emitted: str, registered: set[str]) -> str:
        ...
        if sep and head == tail and head in registered:
            return head
        logger.warning(
            "MuseGlimmer: emitted tool name %r does not match any registered tool; passing through unchanged.",
            emitted,
        )
        return emitted

    @classmethod
    def _parse_tool_calls(cls, text: str, registered: set[str] | None = None) -> list[ToolCall]:
    ...
    def extract_tool_calls(self, model_output: str, request: ChatCompletionRequest) -> ExtractedToolCallInformation:
        if _FUNCTION_CALLS_OPEN not in model_output and "<atem:invoke" not in model_output:
    ...
    def extract_tool_calls_streaming(
        ...
        aphrodite/parser/abstract_parser.py::parse_delta the "pass through as
