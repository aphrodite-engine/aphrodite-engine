...
class K2HorizonToolParser(ToolParser):
    ...
    def _request_tool_format(cls, request: ToolParserRequest) -> str:
        ...
        if not isinstance(tool_format, str) or tool_format not in cls._SUPPORTED_TOOL_FORMATS:
            supported = ", ".join(sorted(cls._SUPPORTED_TOOL_FORMATS))
            raise ValueError(f"Unsupported tool_call_format {tool_format!r}. Supported values: {supported}.")
        return tool_format
    ...
    def _coerce_json_value(value: Any, schema: Any) -> Any:
        if not isinstance(schema, dict):
            return value
        raw_value = value if isinstance(value, str) else json.dumps(value, ensure_ascii=False, allow_nan=False)
    ...
    def extract_tool_calls(
        ...
        try:
            tool_calls = [
                self._make_tool_call(*self._parse_call(match.group(1), request, tool_format)) for match in matches
            ]
    ...
    def _streaming_tool_calls(
        ...
        for index, tool_call in enumerate(tool_calls):
            ...
            self.prev_tool_call_arr.append({"name": tool_call.function.name, "arguments": arguments})
    ...
    def extract_tool_calls_streaming(
        ...
        if request.tool_choice == "none":
            ...
            return DeltaMessage(content=emitted_content) if emitted_content is not None else None
        ...
        if self._stream_group_start is None:
            group_start = self._stream_text.find(self._TOOL_CALLS_START, self._stream_cursor)
            if group_start == -1:
                ...
                content = self._emit_content(self._stream_text[self._stream_cursor : sendable_end])
            ...
            content = self._emit_content(self._stream_text[self._stream_cursor : group_start])
        ...
        if not parsed.tools_called:
            failed_content = self._emit_content(self._stream_text[self._stream_cursor :])
    ...
    def finish_streaming(self) -> DeltaMessage | None:
        if not self._stream_resolved:
            content = self._emit_content(self._stream_text[self._stream_cursor :], final=True)
            self._stream_cursor = len(self._stream_text)
            self._stream_resolved = True
        else:
