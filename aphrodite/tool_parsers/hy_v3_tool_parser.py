    ...
    def _drain_calls(self, request: ChatCompletionRequest) -> list[DeltaToolCall]:
        ...
        while (start_idx := self._buffer.find(self.tool_call_start_token)) != -1:
            ...
            for call in self._extract_tool_calls(complete_call, request):
                ...
                self.prev_tool_call_arr.append({"name": call.function.name, "arguments": json.loads(arguments)})
                ...
                deltas.append(
                    DeltaToolCall(
                        ...
                        type="function",
                        function=DeltaFunctionCall(name=call.function.name, arguments=arguments),
                    )
                )
        return deltas
