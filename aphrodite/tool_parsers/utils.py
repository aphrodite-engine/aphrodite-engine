...
def get_parameter_value(val: ast.expr) -> Any:
    ...
    if isinstance(val, ast.Constant):
        ...
        return [get_parameter_value(v) for v in val.elts]
    elif isinstance(val, ast.JoinedStr) and all(isinstance(part, ast.Constant) for part in val.values):
...
def escape_nested_quotes_in_strings(text: str) -> tuple[str, bool]:
    ...
    while index < len(text):
        ...
        for close in (j for j in quotes if is_closer(j)):
            ...
            candidate = "".join(["".join(prefix), char, "".join(interior), char, text[close + 1 :]])
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", SyntaxWarning)
                try:
