...
# Aphrodite-side validation and for documenting the xgrammar builtin surface that
...
APHRODITE_BUILTIN_STRUCTURAL_TAG_MODELS = frozenset({"hermes", "hy_v4", "kimi_k3"})
...
def register_aphrodite_structural_tag(
    model: str,
) -> Callable[[StructuralTagBuilder], StructuralTagBuilder]:
    """Register a Aphrodite-owned structural tag builder."""

    def decorator(func: StructuralTagBuilder) -> StructuralTagBuilder:
        _APHRODITE_STRUCTURAL_TAG_REGISTRY[model] = func
        return func
...
def get_model_structural_tag(
    ...
    if model in _APHRODITE_STRUCTURAL_TAG_REGISTRY:
        function_tools, builtin_tools, simplified_tool_choice = normalize_tool_choice(
            dumped_tools,
            dumped_tool_choice,
        )
        return _APHRODITE_STRUCTURAL_TAG_REGISTRY[model](
            function_tools,
            builtin_tools,
...
def _hermes_tool_tags(tools: list[FunctionToolParam]) -> list[TagFormat]:
    ...
    return [
        TagFormat(
            ...
            content=JSONSchemaFormat(json_schema=get_function_parameters(tool.function)),
            end=end,
        )
        for tool in tools
...
@register_aphrodite_structural_tag("hermes")
...
@register_aphrodite_structural_tag("minimax")
...
def _k3_argument_tag(
    ...
    xtml_type = _K3_JSON_TO_XTML_TYPE.get(json_type) if isinstance(json_type, str) else None
...
@register_aphrodite_structural_tag("kimi_k3")
def get_kimi_k3_structural_tag(
    ...
    if not tools:
        return StructuralTag(format=SequenceFormat(elements=[*_k3_response_prefix(), trailer]))
    ...
    return StructuralTag(format=SequenceFormat(elements=[*_k3_response_prefix(), tools_part, trailer]))
...
@register_aphrodite_structural_tag("hy_v4")
def get_hy_v4_structural_tag(
    ...
    def single_tool_call(tool: FunctionToolParam) -> TagFormat:
        ...
        if optional_keys:
            ...
            elements.append(
                StarFormat(content=optional_pairs[0] if len(optional_pairs) == 1 else OrFormat(elements=optional_pairs))
            )
        content: Any = SequenceFormat(elements=elements) if elements else AnyTextFormat()
        return TagFormat(
