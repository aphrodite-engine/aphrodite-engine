...
class Lfm2ToolParser(ToolParser):
    ...
    def extract_tool_calls(self, model_output: str, request: ChatCompletionRequest) -> ExtractedToolCallInformation:
        ...
        try:
            is_tool_call_pattern = (
                self.TOOL_CALL_REGEX.match(
                    tool_text,
                    timeout=envs.APHRODITE_TOOL_PARSE_REGEX_TIMEOUT_SECONDS,
                )
                is not None
            )
        except TimeoutError:
        ...
        try:
            ...
            try:
                ...
                escaped = escape_ctrl_chars_in_strings(normalize_leading_zero_ints(tool_text))
                candidates = [escaped]
            ...
            if isinstance(parsed, ast.List) and parsed.elts and all(isinstance(e, ast.Call) for e in parsed.elts):
                tool_calls = [
    ...
    def extract_tool_calls_streaming(
        ...
        try:
            ...
            tool_text = escape_ctrl_chars_in_strings(normalize_leading_zero_ints(tool_text))
            if has_end_in_current:
                ...
                try:
                    ...
                    requoted_text, requote_changed = escape_nested_quotes_in_strings(tool_text)
                    if requote_changed:
