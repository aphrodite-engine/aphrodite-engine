...
def try_create_mm_pooling_model_cls(orig_cls: _T) -> _T:
    ...
    if "init_aphrodite_registered_model" not in visitor.calls:
        return None

    class ModelForPooling(orig_cls, AphroditeModelForPooling):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            if not getattr(self, "pooler", None):
                self._init_pooler(aphrodite_config, prefix=prefix)
...
def as_seq_cls_model(cls: _T) -> _T:
    ...
    from aphrodite.modeling.layers.pooler import DispatchPooler, Pooler
    from aphrodite.modeling.models.interfaces import SupportsCrossEncoding
    ...
    class ModelForSequenceClassification(
        ...
        def _init_pooler(self, aphrodite_config: "AphroditeConfig", prefix: str = ""):
            ...
            self.score = ReplicatedLinear(
                ...
                bias=False,
                params_dtype=aphrodite_config.model_config.head_dtype,
...
def as_reward_model(cls: _T) -> _T:
    """
    Subclass an existing Aphrodite model to support reward modeling.
...
def seq_cls_model_loader(model, weights: Iterable[tuple[str, torch.Tensor]]):
    ...
    text_config = model.aphrodite_config.model_config.hf_config.get_text_config()
    method = getattr(text_config, "method", None)
    assert method in SEQ_CLS_LOAD_METHODS, f"method {method} not supported"
