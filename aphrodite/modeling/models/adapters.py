...
def _load_dense_weights(linear: nn.Linear, folder: str,
    ...
    for filename in ["model.safetensors", "pytorch_model.bin"]:
        ...
        try:
            ...
            logger.exception("Failed to load {}", filename)
            continue
...
def as_seq_cls_model(cls: _T) -> _T:
    ...
    from aphrodite.modeling.layers.pooler import (ClassifierPooler,
...
def seq_cls_model_loader(model, weights: Iterable[tuple[str, torch.Tensor]]):
    ...
    config = model.aphrodite_config.model_config.hf_config
    method = getattr(config, "method", None)
    assert method in SEQ_CLS_LOAD_METHODS, f"method {method} not supported"
