...
from transformers.generation.utils import ALL_CACHE_NAMES, GenerationConfig, GenerationMixin
...
class HunyuanTopKGate(nn.Module):
    def __init__(self, config: HunyuanImage3Config, layer_idx: int | None = None):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        ...
        **kwargs,
        ...
        with nvtx.range("attention"):
            if mode == "gen_text":
                ...
                gen_timestep_scatter_index: torch.Tensor | None = kwargs.get("gen_timestep_scatter_index")
...
class HunyuanImage3DecoderLayer(nn.Module):
    def __init__(self, config: HunyuanImage3Config, layer_idx: int):
        ...
        if config.norm_type == "hf_rms" or config.norm_type == "rms":
            ...
            raise AssertionError("other norm_type are not supported")
...
class HunyuanImage3ForCausalMM(HunyuanImage3PreTrainedModel, GenerationMixin):
    def __init__(self, config: HunyuanImage3Config):
        super().__init__(config)
        self.config = config
        self._tkwrapper: TokenizerWrapper | None = None
    ...
    def check_inputs(prompt=None, message_list=None):
        ...
        if message_list is not None:
            ...
            for message in message_list:
                assert isinstance(message, (list, dict)), (
