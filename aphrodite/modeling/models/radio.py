...
from aphrodite.modeling.models.intern_vit import InternVisionEncoder
...
class RadioModel(nn.Module):
    ...
    def load_weights(self, weights) -> set[str]:
        ...
        for name, weight in weights_list:
            ...
            # Skip buffers not used in Aphrodite
            ...
            if sub.startswith("model.patch_generator."):
                aphrodite_key = f"model.patch_generator.{sub.split('.', 2)[-1]}"
            elif sub.startswith("input_conditioner."):
                aphrodite_key = f"input_conditioner.{sub.split('.', 1)[-1]}"
            elif sub.startswith("model.blocks."):
                # Encoder blocks: HF 'model.blocks.{i}.' ->
                # Aphrodite 'model.encoder.layers.{i}.'
                parts = sub.split(".")
                if len(parts) >= 4:
                    layer_idx = parts[2]
                    suffix = ".".join(parts[3:])
                    # Skip layer-scale entries that Aphrodite doesn't use
                    if suffix in {"ls1", "ls2"} or suffix.startswith(("ls1.", "ls2.")):
                        continue
                    aphrodite_key = f"model.encoder.layers.{layer_idx}.{suffix}"

            if aphrodite_key and aphrodite_key in params_dict:
                param = params_dict[aphrodite_key]
                weight_loader = getattr(param, "weight_loader", default_weight_loader)
                weight_loader(param, weight)
                loaded_params.add(aphrodite_key)
