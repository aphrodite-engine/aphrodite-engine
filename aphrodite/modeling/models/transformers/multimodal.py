...
class MultiModalMixin(SupportsMultiModal, SupportsMRoPE):
    ...
    def get_language_model(self) -> torch.nn.Module:
        """Transformers backend multimodal classes do not contain a separate Aphrodite
        language model class. Therefore, in order to return a language model Aphrodite class,
        we use a wrapper to give `self` the same interface as a text model."""

        # Exclude self and object
        bases = self.__class__.mro()[1:-1]
        # Keep only classes defined in `aphrodite.modeling.models.transformers`
