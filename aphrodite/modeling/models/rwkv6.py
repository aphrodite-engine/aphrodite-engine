...
from aphrodite.modeling.layers.fla.rwkv6 import fused_recurrent_rwkv6
...
class RWKV6MLP(torch.nn.Module):
    ...
    def forward(self, hidden_states, positions,
                kv_cache: Optional[MambaCacheParams], input_ids):
...
class RWKV6Attention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.time_faaaa = RowAphroditeParameter(
            data=torch.zeros(
    ...
    def forward(
        self,
        hidden_states,
        kv_cache: Optional[MambaCacheParams],
...
class RWKV6DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        rwkv6_cache_params: Optional[MambaCacheParams],
...
class Rwkv6ForCausalLM(nn.Module, HasInnerState, IsAttentionFree):
    ...
    def __init__(self,
        ...
        setattr(
            ...
            rwkv6Model(aphrodite_config=aphrodite_config,
        ...
        if self.scheduler_config is not None and \
            ...
            if self.scheduler_config.max_num_seqs > \
                    aphrodite_config.compilation_config.max_capture_size:
                self.max_batch_size = \
                    aphrodite_config.compilation_config.max_capture_size
            else:
                self.max_batch_size = aphrodite_config.pad_for_cudagraph(
    ...
    def forward(self,
                ...
                **kwargs):
        # Extract kv_caches and attn_metadata from kwargs
        # kv_caches is extracted for API consistency but not directly used
        # in this method
        kv_caches = kwargs.get('kv_caches')  # noqa: F841
        attn_metadata = kwargs.get('attn_metadata')

        # Provide default values for cache metadata if missing
        if "request_ids_to_seq_ids" not in kwargs:
            kwargs["request_ids_to_seq_ids"] = {"dummy_req": [0]}
        if "finished_requests_ids" not in kwargs:
            kwargs["finished_requests_ids"] = []

        if self.rwkv6_cache is None:
            conv_state_shape, temporal_state_shape = \
                self._get_rwkv6_cache_shape()
            # Use bfloat16 for both conv and temporal states
            conv_state_dtype = torch.bfloat16
            temporal_state_dtype = torch.bfloat16
            self.rwkv6_cache = MambaCacheManager(
                self.aphrodite_config, self.config.num_hidden_layers,
                conv_state_shape, temporal_state_shape,
                conv_state_dtype, temporal_state_dtype)

        rwkv6_cache_params = self.rwkv6_cache.current_run_tensors(**kwargs)
