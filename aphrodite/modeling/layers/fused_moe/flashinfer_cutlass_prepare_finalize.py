...
from aphrodite.distributed.device_communicators.base_device_communicator import All2AllManagerBase
...
from aphrodite.utils.flashinfer import nvfp4_block_scale_interleave
...
class FlashInferAllGatherMoEPrepareAndFinalize(FlashInferCutlassMoEPrepareAndFinalize):
    ...
    def finalize(
        ...
        if self.use_dp:
            fused_expert_output = get_dp_group().reduce_scatterv(fused_expert_output, dim=0, sizes=get_local_sizes())
        output.copy_(fused_expert_output)
