...
_FlashInferBf16RuntimeCheck = Callable[[torch.Tensor, torch.Tensor, torch.Tensor | None], bool]
...
def _get_flashinfer_bf16_backend(aphrodite_backend: str) -> _FlashInferBf16Backend:
    backend_spec = _FLASHINFER_BF16_BACKENDS.get(aphrodite_backend)
    if backend_spec is None:
        supported = ", ".join(sorted(_FLASHINFER_BF16_BACKENDS))
        raise ValueError(
            f"Unsupported Aphrodite FlashInfer BF16 backend {aphrodite_backend!r}; supported backends: {supported}"
        )
    return backend_spec
...
def cuda_flashinfer_bf16_gemm_impl(
    ...
    bias: torch.Tensor | None,
    pdl: bool,
    aphrodite_backend: str,
) -> torch.Tensor:
    backend_spec = _get_flashinfer_bf16_backend(aphrodite_backend)
...
def cuda_flashinfer_bf16_gemm(
    ...
    *,
    ...
    return torch.ops.aphrodite.cuda_flashinfer_bf16_gemm(
        x,
        weight,
        bias,
        pdl,
...
def rocm_unquantized_gemm_impl(x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor | None = None) -> torch.Tensor:
    from aphrodite.platforms.rocm import on_gfx1x, on_gfx9, on_gfx950, on_gfx1250
    ...
    skinny_operands_compatible = weight.is_contiguous() and (bias is None or bias.is_contiguous())
    ...
    use_skinny_reduce_counting = (
        ...
        and x.dim() == 2
        and (
            10 <= n <= 128 and k % 8 == 0 and k > 512 and m % 16 == 0 and fits_wvsplitkrc and skinny_operands_compatible
        )
    )
    ...
    if rocm_aiter_ops.is_tgemm_enabled() and not (on_gfx1250() and k % 256 != 0):
...
def rocm_unquantized_gemm(
    ...
    return torch.ops.aphrodite.rocm_unquantized_gemm(x, weight, bias)
...
def dispatch_cpu_unquantized_gemm(
    ...
    if layer.weight.ndim != 2:
        ...
        # (e.g. AMD Zen5/Turin).
        if torch.cpu._is_avx512_bf16_supported() and hasattr(ops, "causal_conv1d_weight_pack"):
            # prepack conv weight
            unpacked = (
                layer.weight.view(
                    layer.weight.size(0),
    ...
    if ops._supports_onednn and current_platform.get_cpu_architecture() != CpuArchEnum.POWERPC:
        try:
    ...
    logger.debug_once("CPU unquantized GEMM dispatch: using torch.nn.functional.linear (fallback)")
...
def dispatch_unquantized_gemm(
    ...
    return functools.partial(
        ...
        aphrodite_backend=linear_backend,
