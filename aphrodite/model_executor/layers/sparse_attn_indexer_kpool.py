...
if TYPE_CHECKING:
    from aphrodite.models.glm5next.nvidia.ops import kpool_compress as kpool_ops
elif current_platform.is_rocm():
    from aphrodite.models.glm5next.amd.ops import kpool_compress as kpool_ops
else:
    from aphrodite.models.glm5next.nvidia.ops import kpool_compress as kpool_ops

from aphrodite.utils.deep_gemm import has_deep_gemm
...
def _decode_topk_seq_lens(
    ...
    scatter_idx = _build_decode_scatter_indices(decode_lens, batch_size, num_decode_tokens)
...
def _fill_short_decode_causal_indices(
    ...
    if positions is None or positions.numel() == 0 or max_seq_len > topk_tokens:
        return False
    _fill_causal_indices(topk_indices_buffer[:num_decode_tokens], positions[:num_decode_tokens])
    return True
...
def sparse_attn_indexer_kpool(
    ...
    if not isinstance(attn_metadata, dict):
        ...
        cfg = get_current_aphrodite_config_or_none()
        worst_decode_tokens = 0
        if cfg is not None:
            sched = cfg.scheduler_config
            num_spec = cfg.speculative_config.num_speculative_tokens if cfg.speculative_config is not None else 0
        ...
        prefill_cap_elems = envs.APHRODITE_SPARSE_INDEXER_MAX_LOGITS_MB * 1024 * 1024
    ...
    if has_prefill:
        ...
        if prefill_metadata.max_prefill_seq_len >= 0:
            short_prefill = (
                n_prefill_sf > 0 and positions is not None and prefill_metadata.max_prefill_seq_len <= topk_tokens
            )
        else:
            short_prefill = (
                n_prefill_sf > 0
                and positions is not None
                and int(positions[num_decode_tokens:num_tokens].max().item()) + 1 <= topk_tokens
            )
        if short_prefill:
        ...
        for chunk in prefill_metadata.chunks if not short_prefill else ():
            ...
            if current_platform.is_rocm():
                ...
                from aphrodite.utils.deep_gemm import fp8_fp4_mqa_logits
            ...
            if index_kpool > 1:
                pool_topk = torch.empty((num_rows, select_k), dtype=torch.int32, device=logits.device)
                topk_dst = pool_topk
            else:
                topk_dst = topk_indices_buffer[chunk.token_start : chunk.token_end, :topk_tokens]
            ...
            if index_kpool > 1:
                ...
                if positions is not None:
                    ...
                    q_seq = positions[chunk.token_start : chunk.token_end].to(torch.int32) + 1
                    expanded = kpool_ops.expand_pools_and_append_tail(pool_ids, q_seq, index_kpool)
                else:
                    valid = pool_ids >= 0
                    expanded = kpool_ops.expand_pools_to_tokens(pool_ids, valid, topk_tokens, index_kpool)
                topk_indices_buffer[chunk.token_start : chunk.token_end, : expanded.shape[-1]] = expanded
    ...
    if has_decode:
        ...
        if (
            ...
            if per_req_lens is not None:
                use_uniform = (
                    ...
                    and num_decode_tokens == num_requests * decode_metadata.write_max_decode_len
                )
            ...
            if not use_uniform:
                ...
                scatter_idx = _build_decode_scatter_indices(group_lens, num_requests, num_decode_tokens)
                dec_k = _scatter_decode_tokens_by_request(k[:num_decode_tokens], 0, num_requests, lmax, scatter_idx)
            ...
            tail_meta = attn_metadata.get(_resolve_layer_name(tail_prefix)) if tail_prefix is not None else None
        ...
        if decode_metadata.requires_padding:
            ...
            if q_scale is not None:
                ...
                padded_q_scale = None
            padded_weights = pack_seq_triton(weights[:num_decode_tokens], decode_lens, pad_value=0).reshape(
                -1, *weights.shape[1:]
            )
        else:
        ...
        if current_platform.is_rocm():
            ...
            from aphrodite.utils.deep_gemm import fp8_fp4_paged_mqa_logits
        ...
        if decode_metadata.requires_padding:
            ...
            out = unpack_seq_triton(out.reshape(batch_size, -1, out.shape[-1]), decode_lens)
    ...
    def forward_hip(
        ...
        *,
        ...
        raise RuntimeError(
            ...
            "Please enable aiter with APHRODITE_ROCM_USE_AITER=1"
        )
