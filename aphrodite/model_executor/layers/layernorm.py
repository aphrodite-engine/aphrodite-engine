...
from aphrodite import _oink_ops, envs, ir
...
class RMSNorm(CustomOp):
    ...
    def __init__(
        ...
        if (
            ...
            and bool(getattr(envs, "APHRODITE_USE_OINK_OPS", False))
        ):
            ...
            if getattr(self._forward_method, "__func__", None) is getattr(
                ...
                try:
                    ...
                    custom_ops = ["<unknown>"]
                logger.warning_once(
                    "APHRODITE_USE_OINK_OPS=1 but the `rms_norm` custom op is "
                    "disabled (CompilationConfig.custom_ops=%s). Enable it via "
                    "`compilation_config={'custom_ops': ['none', '+rms_norm']}` "
                    "(or `['all']`) to let Aphrodite call into torch.ops.oink.*.",
                    custom_ops,
                )
                ...
                try:
                    ...
                    logger.warning_once(
                        "APHRODITE_USE_OINK_OPS=1 but failed to initialize Oink "
                        "RMSNorm; falling back to Aphrodite RMSNorm. Error: %s",
                        e,
                    )
                    self._use_oink_fused_add_rmsnorm = False
    ...
    def forward_cuda(
        ...
        # This mirrors Aphrodite's fused_add_rms_norm semantics by mutating both
        ...
        if (
            ...
            if _can_view_as_2d(x) and _can_view_as_2d(residual):
                ...
                # The Oink in-place pointer path supports the common Aphrodite
                # layout where:
                # - `x` may be strided/padded row-major (stride(1) == 1), and
                # - `residual` is contiguous row-major ([M, N] with stride(0) == N).
                # If these conditions are not met, fall back to Aphrodite's built-in
                # fused kernel.
                if (
