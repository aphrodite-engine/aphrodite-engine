...
from aphrodite.config import get_current_aphrodite_config, get_current_aphrodite_config_or_none
...
from aphrodite.model_executor.layers.quantization import resolve_quant_method
...
class UnquantizedLinearMethod(LinearMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        if current_platform.is_cpu():
            ...
            if envs.APHRODITE_XPU_FORCE_N_CONTIG_WEIGHT and weight.ndim == 2 and weight.stride(0) != 1:
    ...
    def apply(
        ...
        if envs.APHRODITE_BATCH_INVARIANT and (current_platform.is_cuda_alike() or current_platform.is_xpu()):
...
class LinearBase(PluggableLayer):
    ...
    def update_param_tp_status(self):
        # Single source of truth for a parameter's TP state.
        # BaseAphroditeParameter stamps self.tp_rank with the global rank in
        # __init__; this reconciles every child parameter to the layer's
        # tp_rank/tp_size, which correctly accounts for disable_tp.
        #
        # Must be re-run whenever parameters are (re-)created after
        # construction, e.g. after quant_method.process_weights_after_loading()
        # swaps in fresh Parameters. Otherwise a later load_weights()/weight
        # refit would narrow a replicated weight at global_rank * shard_size.
        for param in self.parameters():
            if isinstance(param, BaseAphroditeParameter):
...
class DCPGroupColumnParallelLinear(ColumnParallelLinear):
    ...
    def __init__(self, *args, **kwargs):
        dcp_world_size = get_current_aphrodite_config().parallel_config.decode_context_parallel_size
...
class MergedColumnParallelLinear(ColumnParallelLinear):
    ...
    def validate_shard_id(self, shard_id: Any) -> TypeIs[int | tuple[int, ...] | None]:
        if isinstance(shard_id, int):
            if shard_id < 0 or shard_id >= len(self.output_sizes):
                raise ValueError(
                    f"Shard id should be between 0 and {len(self.output_sizes) - 1}. Got shard id {shard_id}."
                )
            return True
        if shard_id is None:
            return True
        if isinstance(shard_id, tuple):
            ...
            if len(shard_id) > 1 and any(b - a != 1 for a, b in zip(shard_id[:-1], shard_id[1:])):
                raise ValueError(f"Shard id with multiple indices should be consecutive. Got shard id {shard_id}.")
            return True
    ...
    def weight_loader(
        ...
        if loaded_shard_id is None or isinstance(loaded_shard_id, tuple):
            ...
            use_bitsandbytes_4bit = getattr(param, "use_bitsandbytes_4bit", False)
            if use_bitsandbytes_4bit and isinstance(loaded_shard_id, tuple) and self.tp_size > 1:
                raise NotImplementedError(
                    "Shard id with multiple indices is not supported for BNB quantization with TP yet."
                )
        ...
        if output_dim is not None:
            ...
            if use_bitsandbytes_4bit:
                ...
                orig_offsets = {str(i): (index[i], size) for i, size in enumerate(self.output_sizes)}
                orig_offsets["total"] = (self.output_size, 0)
                shard_size, shard_offset = adjust_bitsandbytes_4bit_shard(param, orig_offsets, str(loaded_shard_id))
    ...
    def weight_loader_v2(
        ...
        if loaded_shard_id is None or isinstance(loaded_shard_id, tuple):
            if isinstance(param, PerTensorScaleParameter):
                if isinstance(loaded_shard_id, tuple):
                    for idx in loaded_shard_id:
                        param.load_merged_column_weight(loaded_weight=loaded_weight, shard_id=idx)
                else:
                ...
                return
            output_sizes = [self.output_sizes[idx] for idx in loaded_shard_id] if loaded_shard_id else None
            if isinstance(param, BlockQuantScaleParameter):
                weight_block_size = getattr(self, "weight_block_size", None)
                output_sizes = [
            ...
            self._load_fused_module_from_checkpoint(param, loaded_weight, output_sizes=output_sizes)
            return
...
class QKVParallelLinear(ColumnParallelLinear):
    ...
    def validate_shard_id(self, shard_id: Any) -> TypeIs[str | None]:
        if shard_id in {"q", "k", "v"} or shard_id is None:
            return True
        raise ValueError(f"Shard id for QKVParallelLinear should be 'q', 'k', or 'v', got shard id {shard_id}.")
    ...
    def _get_shard_offset_mapping(self, loaded_shard_id: str):
        shard_offset_mapping = {
            ...
            "total": (self.num_heads + self.num_kv_heads) * self.head_size + self.num_kv_heads * self.v_head_size,
        }
        ...
        if output_dim is not None:
            ...
            if use_bitsandbytes_4bit:
                orig_qkv_offsets = {
                    ...
                    "total": (
                        (self.num_heads + self.num_kv_heads) * self.head_size + self.num_kv_heads * self.v_head_size,
                        0,
                    ),
                }
    ...
    *replication* path when ``tp_size > total_num_kv_heads`` (this is what makes
    ...
    def __init__(
        ...
        assert total_num_index_heads == total_num_kv_heads, (
            "MinimaxM3QKVParallelLinearWithIndexer requires total_num_index_heads == total_num_kv_heads"
        )
        self.hidden_size = hidden_size
        self.head_size = head_size
...
class KimiK3MergedQKVGateLinear(MergedColumnParallelLinear):
    ...
    def weight_loader(
        ...
        self._load_with_replicated_latents(super().weight_loader, param, loaded_weight, loaded_shard_id)
    ...
    def weight_loader_v2(
        ...
        self._load_with_replicated_latents(super().weight_loader_v2, param, loaded_weight, loaded_shard_id)
