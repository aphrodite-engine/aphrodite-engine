...
def _triton_mrope_forward(
    ...
    # This version supports flatten input tensors from aphrodite
    ...
    if is_neox_style:
        ...
        first_q_mask = (tl.arange(0, pad_n_qh)[:, None] < n_qh) & (rotary_offsets[None, :] < rd // 2)
        first_k_mask = (tl.arange(0, pad_n_kh)[:, None] < n_kh) & (rotary_offsets[None, :] < rd // 2)

        q_tile_1 = tl.load(q_ptr + first_q_offsets, mask=first_q_mask, other=0).to(sin_row.dtype)
        k_tile_1 = tl.load(k_ptr + first_k_offsets, mask=first_k_mask, other=0).to(sin_row.dtype)

        second_q_offsets = first_q_offsets + (rd // 2)
        second_k_offsets = first_k_offsets + (rd // 2)
        q_tile_2 = tl.load(q_ptr + second_q_offsets, mask=first_q_mask, other=0).to(sin_row.dtype)
        k_tile_2 = tl.load(k_ptr + second_k_offsets, mask=first_k_mask, other=0).to(sin_row.dtype)
        ...
        q_mask = (tl.arange(0, pad_n_qh)[:, None] < n_qh) & (rotary_offsets[None, :] < rd)
        k_mask = (tl.arange(0, pad_n_kh)[:, None] < n_kh) & (rotary_offsets[None, :] < rd)
