...
class MLAAttention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        if self.attn_backend.get_name() == "FLASHINFER_MLA_SPARSE" and is_quantized_kv_cache(kv_cache_dtype):
            logger.info_once(
    ...
    def get_kv_cache_spec(self, aphrodite_config: AphroditeConfig) -> KVCacheSpec:
        ...
        return MLAAttentionSpec(
            ...
            cache_dtype_str=aphrodite_config.cache_config.cache_dtype,
        )
...
try:
    ...
    # On ROCm, aphrodite_flash_attn is not available, try upstream flash_attn instead.
    # On CUDA, aphrodite_flash_attn should always be available (built with Aphrodite),
...
class MLACommonMetadataBuilder(AttentionMetadataBuilder[M]):
    ...
    def determine_prefill_query_data_type(
        ...
        if use_fp8:
            ...
            logger.info_once("FP8 prefill attention enabled: query data type is FP8", scope="local")
            return fp8_dtype
    ...
    def build(
        ...
        if num_prefills > 0:
            ...
            if max_context_len_cpu > 0:
                ...
                token_to_seq_tensor_cpu = torch.zeros([num_chunks, max_token_num_over_chunk], dtype=torch.int32)
...
class MLACommonImpl(MLAAttentionImpl[M], Generic[M]):
    ...
    def __init__(
        ...
        if use_trtllm_ragged_deepseek_prefill():
            logger.info_once("Using TRT-LLM ragged DeepSeek prefill for MLA", scope="local")
            ...
            # flash_attn and the one from aphrodite_flash_attn. The former is used on
            # RoCM and the latter has an additional parameter to control
            # FA2 vs FA3
            self.flash_attn_varlen_func = flash_attn_varlen_func
            self.aphrodite_flash_attn_version = get_flash_attn_version(head_size=self.qk_head_dim)
            ...
            self._pad_v = self.aphrodite_flash_attn_version is None or not (
