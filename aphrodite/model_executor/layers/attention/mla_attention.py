...
from aphrodite.model_executor.layers.attention.pcp import (
...
from aphrodite.v1.attention.ops.common import cp_lse_ag_out_ar, cp_lse_ag_out_rs
...
from aphrodite.v1.attention.ops.triton_merge_attn_states import mask_empty_context
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        if self.impl.is_sparse and not self.impl.supports_dense_mha_prefill:
            logger.warning_once("Sparse MLA impl has no dense-MHA prefill path; using the top-k MQA path only.")
        ...
        _aphrodite_config = get_current_aphrodite_config_or_none()
        self.dcp_a2a = (
            _aphrodite_config is not None
            and _aphrodite_config.parallel_config.decode_context_parallel_size > 1
            and _aphrodite_config.parallel_config.dcp_comm_backend == "a2a"
        )
        ...
        if num_mqa_tokens > 0:
            ...
            if sparse_dcp_local_prefill:
                ...
                if num_dcp_decode_tokens > 0:
                    ...
                    self._v_up_proj(attn_out, out=mqa_output_slice[:num_dcp_decode_tokens])
...
def _use_masked_mha(
    *,
    ...
    thresholds = _DSV32_MASKED_MHA_THRESHOLDS.get(backend_name, {}).get(tensor_parallel_size)
...
def build_mla_chunked_context_metadata(
    *,
    ...
    chunk_starts = torch.empty(num_chunks, num_prefills, dtype=torch.int32, pin_memory=True).copy_(
    ...
    token_to_seq_cpu = torch.zeros((num_chunks, max_tokens_over_chunk), dtype=torch.int32, pin_memory=True)
    ...
    prefill_tokens_with_context = prefill_query_start_loc_cpu[num_prefills_with_context].item()
    ...
    if dcp_world_size > 1:
        ...
        padded_local_max_context_chunk = cdiv(max_context_chunk, dcp_virtual_block_size) * dcp_local_block_size
        local_chunk_starts = torch.empty(num_chunks, num_prefills, dtype=torch.int32, pin_memory=True).copy_(
            torch.arange(num_chunks, dtype=torch.int32).multiply_(padded_local_max_context_chunk).unsqueeze(1)
        )
        ...
        padded_local_token_to_seq_cpu = torch.zeros((num_chunks, max_padded_local_tokens), dtype=torch.int32)
        for i in range(num_chunks):
            token_to_seq = torch.repeat_interleave(req_indices, padded_local_chunk_seq_lens[i])
            padded_local_token_to_seq_cpu[i, : token_to_seq.shape[0]] = token_to_seq
        ...
        chunked_context_metadata = metadata_cls(
            ...
            padded_local_cu_seq_lens=padded_local_cu_seq_lens_cpu.to(device, non_blocking=True),
            padded_local_token_to_seq=padded_local_token_to_seq_cpu.to(device, non_blocking=True),
...
class MLACommonMetadataBuilder(AttentionMetadataBuilder[M]):
    ...
    def __init__(
        ...
        self._prefill_backend = self.compilation_config.static_forward_context[layer_names[0]].prefill_backend.clone()
    ...
    def build(
        ...
        if num_prefills > 0:
            ...
            context_lens_cpu = seq_lens_cpu[reqs_start:num_reqs] - prefill_query_lens_cpu
            ...
            if max_context_len_cpu > 0:
                ...
                for i in range(num_chunks):
                    chunk_token_to_seq_tensor = torch.repeat_interleave(range_idx, chunk_seq_lens[i])
                ...
                if self.dcp_world_size > 1:
                    ...
                    max_padded_local_tokens_over_chunk = padded_local_cu_chunk_seq_lens_cpu[:, -1].max().item()
                    padded_local_token_to_seq_tensor_cpu = torch.zeros(
                        [num_chunks, max_padded_local_tokens_over_chunk],
                        dtype=torch.int32,
                    )
                    for i in range(num_chunks):
                        chunk_token_to_seq_tensor = torch.repeat_interleave(range_idx, padded_local_chunk_seq_lens[i])
                        chunk_len = chunk_token_to_seq_tensor.shape[0]
                        padded_local_token_to_seq_tensor_cpu[i, :chunk_len] = chunk_token_to_seq_tensor
                ...
                if self.dcp_world_size > 1:
                    chunked_context_metadata = _ChunkedMetadata(
                        ...
                        seq_lens=chunk_seq_lens,
                        context_lens=context_lens_cpu.to(device, non_blocking=True),
                        token_to_seq=token_to_seq_tensor_cpu.to(device, non_blocking=True),
                        chunk_total_token=chunk_total_token.tolist(),
                        workspace=self.chunked_prefill_workspace,
                        has_empty_context=has_empty_context,
                        padded_local_chunk_seq_lens=padded_local_chunk_seq_lens.tolist(),
                        local_context_lens_allranks=local_context_lens_allranks.tolist(),
                        padded_local_cu_seq_lens=padded_local_cu_chunk_seq_lens_cpu.to(device, non_blocking=True),
                        padded_local_token_to_seq=padded_local_token_to_seq_tensor_cpu.to(device, non_blocking=True),
                        cu_seq_lens_lst=cu_seq_lens_cpu.tolist(),
...
class MLACommonImpl(MLAAttentionImpl[M], Generic[M]):
    ...
    def _compute_prefill_context(
        ...
        for i in range(iters):
            ...
            _kv_b_proj_w_dtype = (
                self.kv_b_proj.weight.dtype if hasattr(self.kv_b_proj, "weight") else self.kv_b_proj.params_dtype
            )
    ...
    def _context_parallel_compute_prefill_context(
        ...
        for i in range(iters):
            ...
            if toks == 0:
                continue
            padded_local_cu_seq_lens = prefill_metadata.chunked_context.padded_local_cu_seq_lens[i]
            if self.kv_cache_dtype == "fp8_ds_mla":
                ops.cp_gather_and_upconvert_fp8_kv_cache(
                    src_cache=kv_c_and_k_pe_cache,
                    dst=workspace[:toks],
                ...
                ops.gather_and_maybe_dequant_cache(
                    ...
                    token_to_seq=prefill_metadata.chunked_context.padded_local_token_to_seq[i],
                    num_tokens=toks,
                    kv_cache_dtype=self.kv_cache_dtype,
                    scale=k_scale,
                    seq_starts=prefill_metadata.chunked_context.starts[i],
            ...
            cur_allgather_kvcache.copy_(get_dcp_group().all_gather(local_gathered_kvcache, dim=0))
            ...
            kv_c_normed, k_pe = reorg_kvcache(
                ...
                allgatered_k_pe,
                padded_local_chunk_seq_lens_lst=prefill_metadata.chunked_context.padded_local_chunk_seq_lens[i],
