...
from aphrodite.model_executor.layers.attention.kv_transfer_utils import (
...
from aphrodite.utils.math_utils import cdiv, round_down, round_up
...
from aphrodite.v1.attention.backends.mla.prefill import (
    MLAPrefillBackend,
...
from aphrodite.v1.attention.ops.dcp import MLADCPManager
from aphrodite.v1.attention.ops.merge_attn_states import merge_attn_states
from aphrodite.v1.attention.ops.pcp import (
...
def _get_kv_b_proj_input_dtype(kv_b_proj: ColumnParallelLinear, use_fp8_prefill: bool) -> torch.dtype | None:
    ...
    if weight_dtype == current_platform.fp8_dtype():
        ...
        if isinstance(quant_method, ModelOptLinearMethod) and quant_method.spec.weight is kFp8Static128BlockSym:
...
class MLAAttention(nn.Module, AttentionLayerBase):
        ...
        **extra_impl_args,
        ...
        if attn_backend is not None:
            assert attn_backend.is_mla(), (
                f"MLAAttention: attn_backend must be an MLA backend, got {attn_backend.get_name()} instead"
            )
        ...
        normalized_kv_cache_dtype = _canonicalize_sparse_mla_kv_cache_dtype(self.attn_backend, kv_cache_dtype)
        ...
        if self.impl.is_sparse and not (self.impl.supports_dense_mha_prefill and self.supports_dense_mha_prefill):
            logger.warning_once("Sparse MLA layer has no dense-MHA prefill path; using the top-k MQA path only.")
            self.prefill_backend = None
        else:
            try:
                prefill_backend_cls = prefill_backend_cls or get_mla_prefill_backend(aphrodite_config)
            except ValueError:
                if not self.impl.is_sparse or aphrodite_config.attention_config.mla_prefill_backend is not None:
                    raise
                logger.warning_once(
        ...
        if aphrodite_config.kernel_config.enable_jit_warmup:
            ...
            if backend_name in (
                ...
                from aphrodite.v1.attention.backends.mla.compressor_utils import (
        ...
        self._aphrodite_config = get_current_aphrodite_config()
    ...
    def bind_kv_cache(self, kv_cache: torch.Tensor) -> None:
        ...
        if self._aphrodite_config.kernel_config.enable_jit_warmup and self.attn_backend.get_name() in (
            "FLASHMLA_SPARSE",
            "FLASHINFER_MLA_SPARSE",
            "FLASHINFER_MLA_SPARSE_SM120",
            "DEEPSEEK_V32_INDEXER",
    ...
    def chunked_prefill_workspace_size(self) -> int:
        if self._chunked_prefill_workspace_size is None:
            self._chunked_prefill_workspace_size = MLACommonMetadataBuilder.determine_chunked_prefill_workspace_size(
                self._aphrodite_config
            )
    ...
    def forward(
        ...
        if self.use_direct_call:
            ...
            kv_for_cache, kpe_for_cache, layer_slot_mapping = maybe_gather_mla_latent_cache_inputs(
                kv_c_normed,
                k_pe,
                layer_slot_mapping,
            ...
            kv_cache_dummy_dep = torch.ops.aphrodite.unified_mla_kv_cache_update(
                kv_c_normed,
                k_pe,
                encoded,
                self.kv_cache_dtype,
            ...
            output = torch.empty(output_shape, dtype=q.dtype, device=q.device)
            torch.ops.aphrodite.unified_mla_attention_with_output(
                q,
                kv_c_normed,
                k_pe,
                output,
    ...
    def forward_impl(
        ...
        quant_key = _detect_output_quant_key(output, output_scale, output_block_scale, self.num_heads * self.v_head_dim)
        ...
        if self.use_pcp and self.impl.dcp_world_size > 1 and quant_key is not None:
            raise NotImplementedError("MRV2 MLA PCP+DCP does not support fused output quantization yet.")
        ...
        if num_mqa_tokens > 0:
            if q_dcp_replicated is not None and not sparse_dcp_local_prefill:
            ...
            mqa_q_nope, mqa_q_pe = mqa_q.split([self.qk_nope_head_dim, self.qk_rope_head_dim], dim=-1)
            ...
            if fp8_attention and self.impl.supports_quant_query_input:
                ...
                mqa_q = self._decode_concat_quant_fp8_op(mqa_ql_nope, mqa_q_pe, self._q_scale)
            else:
            ...
            if sparse_dcp_local_prefill:
                ...
                if mqa_prefill_q.shape[0] > 0:
                    prefill_out = self.impl.forward_mqa_local_prefill(mqa_prefill_q, attn_metadata)  # type: ignore[attr-defined]
                    self._v_up_proj(prefill_out, out=mqa_output_slice[num_dcp_decode_tokens:])
                # The local prefill is complete; only the decode prefix needs
                # attention over the sharded cache and a collective reduction.
                mqa_output_slice = mqa_output_slice[:num_dcp_decode_tokens]

            if mqa_q.shape[0] > 0:
                attn_out, lse = cast(MLAAttentionImpl, self.impl).forward_mqa(mqa_q, kv_cache, attn_metadata, self)
            else:
                attn_out, lse = None, None

            # correct dcp attn_out with lse.
            if self.impl.dcp_world_size > 1 and attn_out is not None:
                assert lse is not None
                assert self.dcp_manager is not None
                decode_metadata = getattr(attn_metadata, "decode", None)
                if not use_mha and not sparse_dcp_local_prefill:
        ...
        if quant_key is not None:
            ...
            if quant_key == kNvfp4Dynamic:
                ...
                assert quant_group_size is not None, (
                    "Group FP8 output quant requested but quant_group_size not passed through custom op"
                )
    ...
    def _use_sparse_mha(self, attn_metadata: "MLACommonMetadata") -> bool:
        if getattr(type(self.impl), "forward_mha", None) is MLAAttentionImpl.forward_mha:
            return False
        ...
        use_masked_mha = (
            ...
            and _use_masked_mha(
                backend_name=self.attn_backend.get_name(),
                tensor_parallel_size=(self._aphrodite_config.parallel_config.tensor_parallel_size),
                qk_head_dim=self.qk_nope_head_dim + self.qk_rope_head_dim,
                v_head_dim=self.v_head_dim,
        ...
        return (prefill.use_dense_mha or use_masked_mha) and not (
            self._aphrodite_config.attention_config.sparse_mla_force_mqa
        )
    ...
    def process_weights_after_loading(self, act_dtype: torch.dtype):
        ...
        if self.is_amx_bmm_enabled:
            ...
            self.kv_b_proj.weight = torch.nn.Parameter(torch.empty(0), requires_grad=False)
            return

        # we currently do not have quantized bmm's which are needed for
        # `W_UV` and `W_UK_T`, we just store fp16/bf16 copies and perform
        # the bmm's in 16-bit, the extra memory overhead of this is fairly low
        kv_b_proj_weight = get_and_maybe_dequant_weights(self.kv_b_proj, out_dtype=act_dtype).T

        if self.dcp_q_replicate:
            # qrep wired here: validate unsupported decode backends once.
            assert self.q_pad_num_heads in (None, self.num_heads), (
                "DCP query replication is unsupported on head-padding MLA backends (q_pad_num_heads)."
            )
            if self.is_aiter_triton_fp4_bmm_enabled or self.is_aiter_triton_fp8_bmm_enabled:
                raise NotImplementedError(
                    "DCP query replication is not implemented for the aiter FP4/FP8 MLA BMM paths."
                )
        ...
        # If kv_b_proj_weight is unquantized, quantize it to mxfp4 if supported
        if self.is_aiter_triton_fp4_bmm_enabled:
            from aphrodite.model_executor.layers.quantization.quark.utils import (
            ...
            self.W_V, self.W_V_scale = quark_quantize_weight_to_mxfp4(W_UV.permute(1, 2, 0))
        elif self.is_aiter_triton_fp8_bmm_enabled:
            ...
            for m in pre_compilation_list:
                ...
                rocm_aiter_ops.triton_fp8_bmm(x, self.W_K, self.W_K_scale, group_size=128, transpose_bm=True)
                ...
                rocm_aiter_ops.triton_fp8_bmm(x, self.W_V, self.W_V_scale, group_size=128, transpose_bm=True)
        else:
            ...
            if self.dcp_q_replicate:
                self.W_UK_T_dcp_qrep = get_dcp_group().all_gather(self.W_UK_T.contiguous(), dim=0)

        # If we should not load quant weights, we initialize the scales to 1.0
        # as the default value. See [Note: Register q/k/v/prob scales in state dict]
        # for more details.
        quant_method = (
            resolve_quant_method(self.quant_config, self, prefix=self.layer_name) if self.quant_config else None
        )
    ...
    def get_kv_cache_spec(self, aphrodite_config: AphroditeConfig) -> KVCacheSpec:
        ...
        common_kwargs = dict(
            ...
            state_content_bytes={"fp8_ds_mla": 656, "nvfp4_ds_mla": 352}.get(self.kv_cache_dtype),
        )
    ...
    def _v_up_proj(self, x: torch.Tensor, out: torch.Tensor):
        ...
        if self.is_aiter_triton_fp4_bmm_enabled:
            ...
            x = rocm_aiter_ops.triton_fp8_bmm(x, self.W_V, self.W_V_scale, group_size=128, transpose_bm=True, YQ=out)
        elif self.is_amx_bmm_enabled:
            # bmm_cpu computes out[n] = mat1[n] @ mat2[n]^T against
...
def _use_masked_mha(
    *,
    ...
    thresholds = (
        _MASKED_MHA_THRESHOLDS.get((qk_head_dim, v_head_dim), {}).get(backend_name, {}).get(tensor_parallel_size)
    )
...
def align_mla_chunked_context_workspace_size(
    ...
    parallel_config = aphrodite_config.parallel_config
    alignment = aphrodite_config.cache_config.block_size
...
def build_mla_chunked_context_metadata(
    *,
    ...
    local_context_lens_allranks = (
        get_dcp_local_seq_lens(context_lens_cpu, dcp_world_size, None, dcp_local_block_size).tolist()
        if use_dcp
        else None
    )
    ...
    for plan in plans:
        ...
        cu_seqlens_q_flat.extend(
            query_start_loc[request] - query_base for request in range(plan.request_start, plan.request_end + 1)
        )
        token_to_seq_parts.append(np.repeat(np.arange(num_requests, dtype=np.int32), plan.seq_lens))
        ...
        if use_dcp:
            ...
            local_seq_lens = [
                ...
                for start, length, local_start in zip(plan.starts, plan.seq_lens, local_starts)
            ]
            local_starts_per_chunk.append(local_starts)
            local_seq_lens_per_chunk.append(local_seq_lens)
            padded_local_cu_seq_lens_flat.extend(itertools.accumulate(local_seq_lens, initial=0))
            padded_local_token_to_seq_parts.append(np.repeat(np.arange(num_requests, dtype=np.int32), local_seq_lens))
    ...
    token_to_seq = _flat_int32(np.concatenate(token_to_seq_parts)).to(device, non_blocking=True)
    if use_dcp:
        padded_local_cu_seq_lens = _flat_int32(padded_local_cu_seq_lens_flat).to(device, non_blocking=True)
        padded_local_token_to_seq = _flat_int32(np.concatenate(padded_local_token_to_seq_parts)).to(
    ...
    for index, (plan, layout) in enumerate(zip(plans, layouts)):
        ...
        chunk = MLACommonPrefillMetadata.ContextChunk(
            ...
            token_slice=slice(query_start_loc[plan.request_start], query_start_loc[plan.request_end]),
        ...
        if use_dcp:
            ...
            chunk.local_context_lens_allranks = local_context_lens_allranks[plan.request_start : plan.request_end]
            chunk.padded_local_cu_seq_lens = padded_local_cu_seq_lens[boundary_slice]
            chunk.padded_local_token_to_seq = padded_local_token_to_seq[local_token_slice]
...
class MLACommonMetadataBuilder(AttentionMetadataBuilder[M]):
    ...
    def _validate_dspark_dcp_support(self, supports_dcp_with_varlen: bool) -> None:
        speculative_config = getattr(self.aphrodite_config, "speculative_config", None)
    ...
    def determine_prefill_query_data_type(
        ...
        """
        use_fp8 = (
            is_quantized_kv_cache(aphrodite_config.cache_config.cache_dtype)
            and aphrodite_config.attention_config.use_prefill_query_quantization
        ...
        if use_fp8:
            ...
            return fp8_dtype
        elif aphrodite_config.attention_config.use_prefill_query_quantization:
            logger.info_once(
    ...
    def __init__(
        ...
        self.device = device
        self.use_pcp = parallel_config.prefill_context_parallel_size > 1
        self.non_causal_multi_token_decode = getattr(kv_cache_spec, "non_causal_multi_token_decode", False)
        ...
        self.kv_cache_spec = kv_cache_spec
        self.q_data_type = self.determine_prefill_query_data_type(aphrodite_config, self.model_config.dtype)
        ...
        use_packed_fp8_cache = aphrodite_config.cache_config.cache_dtype == "fp8_ds_mla"
        self.dcp_manager: MLADCPManager | None = None
        if self.dcp_world_size > 1:
            ...
            self.chunked_prefill_workspace = torch.empty(
                (
                    self.chunked_prefill_workspace_size + self.chunked_prefill_workspace_size // self.dcp_world_size,
                    self.mla_dims.kv_lora_rank + self.mla_dims.qk_rope_head_dim,
                ),
                dtype=torch.bfloat16 if use_packed_fp8_cache else self.model_config.dtype,
                device=device,
            )
    ...
    def build(
        ...
        if non_causal_decode:
            if not (self.supports_non_causal_multi_token_decode and self.non_causal_multi_token_decode):
                raise ValueError("Non-causal multi-token MLA requires an explicitly supported attention group.")
            ...
            if not (uniform_active_queries and trailing_graph_padding):
                raise ValueError(
                    f"Non-causal MLA requires a uniform query block; got query lengths {query_lens.tolist()}."
                )
        ...
        if num_prefills > 0:
            ...
            prefill_query_lens_cpu = (
                query_start_loc_cpu[reqs_start + 1 : num_reqs + 1] - query_start_loc_cpu[reqs_start:num_reqs]
            )
            context_lens_cpu = torch.empty(num_prefills, dtype=seq_lens_cpu.dtype, pin_memory=PIN_MEMORY)
            torch.subtract(
...
def accumulate_mla_context_chunk(
    ...
    if chunk.is_continuation:
        assert not output_written, "a continuation chunk must not write over the partial it merges with"
...
class MLACommonBaseImpl(MLAAttentionImpl[M], Generic[M]):
    """
    ...
    def _concat_k_nope_k_pe(self, k_nope: torch.Tensor, k_pe: torch.Tensor) -> torch.Tensor:
        ...
        if self._use_flashinfer_concat_mla_k:
            torch.ops.aphrodite.flashinfer_concat_mla_k(k, k_nope, k_pe)
        else:
    ...
    def _compute_prefill_context(
        ...
        kv_b_proj_input_dtype = _get_kv_b_proj_input_dtype(self.kv_b_proj, use_fp8_prefill)
        ...
        for chunk in chunked_context.chunks:
            ...
            if self.kv_cache_dtype == "fp8_ds_mla":
                ...
                assert not is_quantized_kv_cache(self.kv_cache_dtype), (
                    f"CPU MLA context gather fallback only supports non-quantized KV cache, got {self.kv_cache_dtype}"
                )
            ...
            if output is None:
                if len(chunked_context.chunks) == 1 and not chunked_context.empty_token_slices:
            ...
            accumulate_mla_context_chunk(chunk, attn_output, attn_softmax_lse, output, output_lse)
    ...
    def _context_parallel_compute_prefill_context(
        ...
        for chunk in chunked_context.chunks:
            ...
            cur_allgather_workspace = workspace[allgather_offset : allgather_offset * (1 + dcp_world_size)]
            ...
            assert cur_allgather_kvcache.shape[-1] == self.kv_lora_rank + self.qk_rope_head_dim
            allgatered_kv_c_normed, allgatered_k_pe = cur_allgather_kvcache.unsqueeze(1).split(
    ...
    def forward_mha(
        ...
        output_prefill = prefill_metadata.prefill_backend.run_prefill_new_tokens(
            ...
            out=(output.view(-1, self.num_heads, self.v_head_dim) if output_scale is not None else None),
            output_scale=output_scale,
        )
