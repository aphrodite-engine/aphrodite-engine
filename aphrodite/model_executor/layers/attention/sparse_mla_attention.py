...
from aphrodite.utils.torch_utils import is_quantized_kv_cache, np_to_pinned_tensor
from aphrodite.v1.attention.backend import AttentionMetadata, AttentionMetadataBuilder
...
class SparseMLACommonMetadataBuilder(AttentionMetadataBuilder[T]):
    ...
    def __init__(
        ...
        attention_layer = aphrodite_config.compilation_config.static_forward_context[layer_names[0]]
    ...
    def determine_chunked_prefill_workspace_size(aphrodite_config: "AphroditeConfig") -> int:
        ...
        if aphrodite_config.parallel_config.decode_context_parallel_size > 1:
            return align_mla_chunked_context_workspace_size(aphrodite_config, workspace_size)
        ...
        if num_prefills > 0 and self._prefill_backend is not None:
            ...
            prefill = MLACommonPrefillMetadata(
                ...
                use_dense_mha=(
                    ...
                    and not self.aphrodite_config.attention_config.sparse_mla_force_mqa
                ),
...
def _build_topk_mask(
    ...
    num_topk = topk_packed.shape[1]
    q_lens_tensor = np_to_pinned_tensor(np.asarray(q_lens, dtype=np.int32)).to(out.device, non_blocking=True)
...
class SparseMLACommonImpl(MLACommonImpl[Any], Generic[T]):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.masked_mha_available = _is_masked_mha_available(
            num_heads_total=self.num_heads * get_tensor_model_parallel_world_size(),
    ...
    def masked_mha_workspace_fits(prefill: MLACommonPrefillMetadata) -> bool:
        ...
        if prefill.chunked_context is not None:
            max_context_chunk_seq_len = max(chunk.max_seq_len for chunk in prefill.chunked_context.chunks)
    ...
    def _project_kv(self, kv_c_normed: torch.Tensor, k_pe: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def _try_build_global_mask(
        ...
        mask = topk_mask_workspace[:needed].view(batch_size, padded_q_len, num_words_padded)
        _build_topk_mask(
            topk_per_req,
            q_lens,
            padded_q_len,
    ...
    def _run_masked_mha(
        ...
        from aphrodite.model_executor.layers.attention.sparse_mla_mask import (
            dense_mask_mod,
        ...
        if dense_mask is None:
            ...
            batch_size, padded_q_len, num_words = _topk_mask_shape(len(q_lens), max_seqlen_q, max_seqlen_k)
            ...
            workspace_3d = topk_mask_workspace[:words_needed].view(batch_size, padded_q_len, num_words)
    ...
    def _compute_context_mha(
        ...
        if self.dcp_world_size > 1:
            raise NotImplementedError("Masked MHA with context does not yet support decode context parallelism")
    ...
    def forward_mha(  # type: ignore[override]
        ...
        topk_all = self.topk_indices_buffer[num_decode_tokens : num_decode_tokens + q.shape[0]]
        ...
        suffix_output, suffix_lse = self._run_masked_mha(
            ...
            key_starts=(chunked_context.context_lens if dense_mask is not None else None),
            topk_mask_workspace=prefill_metadata.topk_mask_workspace,
        )
