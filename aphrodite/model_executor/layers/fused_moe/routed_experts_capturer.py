...
from aphrodite.v1.kv_cache_interface import KVCacheConfig, is_full_attention_spec
from aphrodite.v1.outputs import RoutedExpertsTensors
...
def _get_routed_experts_shape(aphrodite_config: AphroditeConfig) -> tuple[int, int, int]:
...
class RoutedExpertsCapturer:
    ...
    def __init__(
        ...
        num_layers, _, num_experts_per_tok = _get_routed_experts_shape(aphrodite_config)
        logger.info(
            ...
            num_layers,
            num_experts_per_tok,
            aphrodite_config.model_config.hf_text_config.model_type,
        )
    ...
    def capture(self, layer_id: int, topk_ids: torch.Tensor) -> None:
        ...
        if layer_id >= self.device_buffer.shape[1]:
            raise IndexError(
                f"routed-experts layer {layer_id} exceeds capture buffer layer count {self.device_buffer.shape[1]}"
            )
    ...
    def get_routed_experts(self, slot_mappings: torch.Tensor, num_tokens: int) -> RoutedExpertsTensors:
...
def bind_routed_experts_capturer(
    ...
    for module in model.modules():
        ...
        if quant_method.is_monolithic:
            if not (
                isinstance(fused_experts, FusedMoEExpertsMonolithic) and fused_experts.supports_routing_replay_capture()
            ):
                raise ValueError(
            ...
            raise ValueError(f"Routed-experts capture is not supported with router {type(module.router).__name__}.")
...
class RoutedExpertsManager:
    ...
    def __init__(
        ...
        num_layers, num_experts, num_experts_per_tok = _get_routed_experts_shape(aphrodite_config)
