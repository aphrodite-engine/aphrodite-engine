...
def _is_supported_wna16_weight_key(weight_key: QuantKey | None) -> bool:
    ...
    if not (group_shape == GroupShape.PER_CHANNEL or (group_shape.row == 1 and group_shape.col > 0)):
        return False
    ...
    return isinstance(dtype, ScalarType) and dtype.is_integer() and not dtype.is_signed() and 2 <= dtype.size_bits <= 8


def get_humming_moe_gemm_type(moe_config: FusedMoEConfig | None = None) -> str:
    env_gemm_type: str | None = envs.APHRODITE_HUMMING_MOE_GEMM_TYPE
...
class HummingExpertsBase(mk.FusedMoEExpertsModular):
    ...
    def init_humming_moe(self):
        from aphrodite.utils.humming import get_heuristics_config

        self.compute_config = {
            "use_batch_invariant": envs.APHRODITE_BATCH_INVARIANT,
            "use_f16_accum": envs.APHRODITE_HUMMING_USE_F16_ACCUM,
    ...
    def quantize_input(
        ...
        from aphrodite.utils.humming import may_quant_input
    ...
    def humming_forward(
        ...
        **kwargs: Any,
        ...
        from aphrodite.utils.humming import humming_forward
        ...
        return humming_forward(
            ...
            inputs=inputs,
            weight=weight,
            weight_scale=(self.quant_config.w1_scale if is_w13 else self.quant_config.w2_scale),
            ...
            weight_scale_2=(self.quant_config.g1_alphas if is_w13 else self.quant_config.g2_alphas),
            input_scale=input_scale,
            outputs=outputs,
            locks=self.locks,
            **kwargs,
    ...
    def _get_permute_scratch(self, topk: int) -> MoEPermuteScratch | None:
        ...
        if scratch is None:
            max_expanded_rows = (
                self.moe_config.max_num_tokens * self.moe_config.dp_size * self.moe_config.experts_per_token
            )
    ...
    def _supports_quant_scheme(
        ...
        return (weight_key, activation_key) in SUPPORTED_W_A or (
            activation_key in (None, kFp8DynamicTokenSym) and _is_supported_wna16_weight_key(weight_key)
        )
    ...
    def _prequantizes_dispatch_activation(self) -> bool:
        ...
        produced by Aphrodite's block-FP8 quantization ([M, K // 128] float32,
        row-major) matches what the Humming WGMMA grouped GEMM expects.
        """
        quant_config = self.quant_config
        return quant_config.is_block_quantized and quant_config.quant_dtype == current_platform.fp8_dtype()
    ...
    def get_buffer_metas(self, M: int, topk: int, activation: MoEActivation):
        from aphrodite.utils.humming import GemmType as HummingGemmType
        from aphrodite.utils.humming import dtypes
    ...
    def fused_situ_quant_enabled(self, activation: MoEActivation) -> bool:
        ...
        if not (w2cfg.a_dtype.num_bits == 8 and str(w2cfg.a_dtype) == "float8e4m3"):
            ...
            reason = f"w2 input_scale_group_size is {w2cfg.input_scale_group_size} (need 128)"
        ...
        if reason is not None:
            logger.warning_once(
                "Humming fused SITU+FP8 quant disabled, using unfused situ_and_mul + quant: %s",
                reason,
            )
            return False
        return True
    ...
    def fused_situ_quant(
        ...
        assert cfg.activation_situ_beta is not None, "SITU requires activation_situ_beta from FusedMoEConfig"
...
class HummingIndexedExperts(HummingExpertsBase):
    ...
    def apply(
        ...
        if expert_tokens_meta is not None and expert_tokens_meta.psum_recv_per_rank is not None:
        ...
        if self.fused_situ_quant_enabled(activation):
            ...
            valid_token_counts = valid_tokens * topk if valid_tokens is not None else None
...
class HummingGroupedExperts(HummingExpertsBase):
    ...
    def apply(
        ...
        self.apply_activation(
            ...
            input=buffers["gate_up_output"],
            output=buffers["activation_output"],
            valid_token_counts=(expert_first_token_offset[-1:].to(torch.int32) if expert_tokens_meta is None else None),
        )
