...
def _aiter_raw(t):
    ...
    if t is None or isinstance(t, torch.Tensor):
        return t
    assert hasattr(t, "storage"), f"expected a triton_kernels wrapped tensor with a .storage attribute, got {type(t)}"
...
def aiter_triton_kernel_w4a16_moe_forward(
    ...
    # AITER released, and AITER pin in Aphrodite increased from 0.1.19.
    ...
    swiglu_alpha = quant_config.gemm1_alpha if quant_config.gemm1_alpha is not None else 1.0
...
class AiterW4A16ExpertsMonolithic(mk.FusedMoEExpertsMonolithic):
    ...
    def _supports_current_device() -> bool:
        if not rocm_aiter_ops.is_enabled():
            return False
        from aphrodite.platforms.rocm import on_gfx942, on_gfx950, on_gfx1250
    ...
    def apply(
        ...
        score_mode = "sqrtsoftplus" if self.moe_config.routing_method == RoutingMethodType.DeepseekV4 else None
