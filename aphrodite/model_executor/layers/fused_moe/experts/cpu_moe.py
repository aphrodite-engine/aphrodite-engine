...
from aphrodite.model_executor.kernels.linear.zentorch_utils import (
...
class CPUUnquantizedExperts(mk.FusedMoEExpertsMonolithic):
    ...
    def _supports_grouped_gemm(
        ...
        if moe_config.hidden_dim % cls.output_alignment != 0 or intermediate_size % cls.output_alignment != 0:
            return False, (f"kernel requires hidden and intermediate dimensions divisible by {cls.output_alignment}")
        if moe_config.hidden_dim % cls.reduction_alignment != 0 or intermediate_size % cls.reduction_alignment != 0:
            return False, (f"kernel requires reduction dimensions divisible by {cls.reduction_alignment}")
        return True, None
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        replace_parameter(layer, "w13_weight", cpu_prepack_moe_weight(layer.w13_weight, self.isa))
        replace_parameter(layer, "w2_weight", cpu_prepack_moe_weight(layer.w2_weight, self.isa))
...
class PowerCPUUnquantizedExperts(CPUUnquantizedExperts):
    ...
    @staticmethod
    def _supports_current_device() -> bool:
        return current_platform.is_cpu() and current_platform.get_cpu_architecture() == CpuArchEnum.POWERPC
...
class ZenCPUExpertsInt8(mk.FusedMoEExpertsMonolithic):
    ...
    def __init__(
        ...
        self._w1_scale_bf16 = self.w1_scale.detach().to(torch.bfloat16).reshape(num_experts, -1).contiguous()
        self._w2_scale_bf16 = self.w2_scale.detach().to(torch.bfloat16).reshape(num_experts, -1).contiguous()
        self._w1_bias_bf16 = None if self.w1_bias is None else self.w1_bias.detach().to(torch.bfloat16).contiguous()
        self._w2_bias_bf16 = None if self.w2_bias is None else self.w2_bias.detach().to(torch.bfloat16).contiguous()
        logger.info_once("[zen_cpu] Using zentorch_fused_moe for W8A8 INT8 MoE (monolithic experts)")
