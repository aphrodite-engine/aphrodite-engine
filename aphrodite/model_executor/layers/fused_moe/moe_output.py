...
def convert_flashinfer_moe_output(
    ...
    *,
    ...
    if do_finalize:
        ...
        elif len(flashinfer_output) == 1 and isinstance(flashinfer_output[0], torch.Tensor):
            returned_output = flashinfer_output[0]
        else:
            raise ValueError("Finalized FlashInfer MoE output must contain exactly one tensor.")
        if finalized_output is not None and (
            ...
            raise ValueError("Finalized FlashInfer MoE output must alias the provided destination.")
    ...
    if isinstance(flashinfer_output, torch.Tensor) or len(flashinfer_output) != 3:
        raise ValueError(
            "Deferred FlashInfer MoE output must contain GEMM2 output, routing weights, and a permutation map."
        )
    ...
    if expert_weights.numel() != expected_routes:
        raise ValueError(
            f"Deferred FlashInfer routing weights must contain num_tokens * top_k ({expected_routes}) elements."
        )
    if expanded_idx.numel() != expected_routes:
        raise ValueError(
            f"Deferred FlashInfer permutation map must contain num_tokens * top_k ({expected_routes}) elements."
        )
    ...
    if not all(output.is_contiguous() for output in (gemm2_permuted, expert_weights, expanded_idx)):
