...
from aphrodite.utils.torch_utils import aux_stream, current_stream
...
class LatentMoERunner(MoERunner):
    ...
    def _get_zero_residual(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if buf is None or buf.dtype != hidden_states.dtype or buf.device != hidden_states.device or buf.numel() < numel:
            buf = torch.zeros(numel, dtype=hidden_states.dtype, device=hidden_states.device)
    ...
    def _fused_forward(
        ...
        if self.enable_k3_latent_moe_tail_fusion:
            ...
            if 0 < fused_output.shape[0] <= op.contract.max_num_tokens:
                ...
                result = self._maybe_reduce_final_output(result, og_hidden_dim_post_xform, output_is_reduced=True)
        ...
        shared_expert_stream = (
            aux_stream() if shared_output.size(0) <= envs.APHRODITE_SHARED_EXPERTS_STREAM_TOKEN_THRESHOLD else None
        )
