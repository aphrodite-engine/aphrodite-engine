...
class SharedExperts(torch.nn.Module):
    ...
    def _disable_shared_experts_overlap(self) -> bool:
        ...
        if getattr(self._layer, "shard_sequence_parallel", False):
            ...
            return True
        return (
            parallel_config.enable_eplb and parallel_config.all2all_backend not in _EPLB_OVERLAP_SAFE_BACKENDS
    ...
    def _determine_shared_experts_order(
        ...
        overlap_is_beneficial = not current_platform.is_rocm() or (
            self._moe_config.moe_parallel_config.dp_size > 1 and self._moe_config.moe_parallel_config.tp_size == 1
        )
    ...
    def maybe_forward_async(self, shared_experts_input: torch.Tensor) -> bool:
        """Enqueue shared experts on the auxiliary stream.

        Return whether work was enqueued. Call :meth:`wait` before reading
        ``output`` when this returns true.
        """
        if self._determine_shared_experts_order(shared_experts_input) != SharedExpertsOrder.MULTI_STREAM_OVERLAPPED:
            return False
    ...
    def wait(self) -> None:
        """Make the current stream wait for the asynchronous output."""
        assert self._stream is not None
