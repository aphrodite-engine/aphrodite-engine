...
from aphrodite.model_executor.layers.fused_moe.runner.moe_runner_base import MoERunnerBase
...
class ChunkingMoERunner(MoERunnerBase):
    ...
    def _forward_impl(
        ...
        final_shared_hidden_states, final_fused_hidden_states = self._allocate_dp_chunking_outputs(
        ...
        if self.moe_config.is_sequence_parallel:
            max_tokens_across_dispatchers = cdiv(max_tokens_across_dispatchers, self.moe_config.sp_size)
        ...
        for chunk_idx, chunk_start_ in enumerate(range(0, max_tokens_across_dispatchers, moe_dp_chunk_size_per_rank)):
            ...
            chunk_sizes = ctx.dp_metadata.chunked_sizes(self.moe_config.sp_size, moe_dp_chunk_size_per_rank, chunk_idx)
            with chunk_sizes:
                ...
                shared_experts_input_chunk = (
                    shared_experts_input[chunk_start:chunk_end, :] if shared_experts_input is not None else None
                )
                ...
                if chunk_start < num_tokens:
                    if self.shared_experts is not None:
                        ...
                        final_fused_hidden_states[chunk_start:chunk_end, :].copy_(chunk_result, non_blocking=True)
