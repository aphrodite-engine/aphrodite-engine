    ...
    if not _USE_LAYERNAME and layer_name == "from_forward_context":
        ...
        if moe_layer_index >= len(all_moe_layers):
            raise AssertionError(
                ...
                "{aphrodite.moe_forward, aphrodite.moe_forward_shared} calls."
            )
...
def _unpack(
    result: torch.Tensor | UnfinalizedMoEOutput | tuple[torch.Tensor, torch.Tensor | UnfinalizedMoEOutput],
...
class MoERunner(MoERunnerInterface):
    ...
    integration with Aphrodite's distributed execution framework.
    ...
    def __init__(
        ...
        if shared_experts is not None:
            ...
            routed_input_is_quantized = lambda: (
                ...
                and self.routed_experts.quant_method.moe_quant_config.quant_dtype is not None
            )
            is_multistream_safe = lambda: (not current_platform.is_rocm() or routed_input_is_quantized())
    ...
    def _maybe_dispatch(
        ...
        if self.moe_config.pcp_size > 1 and not self.moe_config.moe_parallel_config.use_all2all_kernels:
    ...
    def _maybe_combine(
    ...
    ) -> torch.Tensor | UnfinalizedMoEOutput | tuple[torch.Tensor | None, torch.Tensor | UnfinalizedMoEOutput]:
        if self.do_naive_dispatch_combine:
            if isinstance(hidden_states, UnfinalizedMoEOutput):
                raise RuntimeError("Naive expert-parallel combine cannot consume a deferred MoE output.")
        ...
        if self.moe_config.pcp_size > 1 and not self.moe_config.moe_parallel_config.use_all2all_kernels:
            if isinstance(hidden_states, UnfinalizedMoEOutput):
                raise RuntimeError("PCP reduce-scatter cannot consume a deferred MoE output.")
    ...
    def _forward_impl(
        ...
        shared_experts_input: torch.Tensor | None,
        input_ids: torch.Tensor | None = None,
    ) -> torch.Tensor | UnfinalizedMoEOutput | tuple[torch.Tensor, torch.Tensor | UnfinalizedMoEOutput]:
