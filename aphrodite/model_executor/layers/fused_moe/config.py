...
class FusedMoEParallelConfig:
    ...
    def use_all2all_kernels(self):
        return self.use_ep and (self.dp_size > 1 or self.pcp_size > 1 or self.is_sequence_parallel)
    ...
    def make(
        ...
        dp_size_: int,
        sp_size_: int,
        aphrodite_parallel_config: ParallelConfig,
        ...
        if not use_ep:
            return FusedMoEParallelConfig(
                ...
                sp_size=sp_size_,
                use_ep=False,
                all2all_backend=aphrodite_parallel_config.all2all_backend,
                enable_eplb=aphrodite_parallel_config.enable_eplb,
            )
...
class FusedMoEConfig:
    ...
    def should_defer_moe_finalize(self, num_tokens: int) -> bool:
        ...
        return (
            self.use_deferred_moe_finalize and num_tokens > 0 and (max_num_tokens < 0 or num_tokens <= max_num_tokens)
        )
