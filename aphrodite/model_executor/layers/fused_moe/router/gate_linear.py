...
class GateLinear(ReplicatedLinear):
    ...
    def __init__(
        ...
        if is_gfx950:
            from aphrodite.model_executor.layers.fused_moe.router.rocm_fp32_router_gemm import (  # noqa: E501
        ...
        self.allow_cublas_router_gemm = (
            self._router_gemm_cublas_capable and self.weight.dtype == torch.bfloat16 and self.out_dtype == torch.float32
        )
        ...
        if can_use_specialized_kernels:
            ...
            self.allow_ll_bf16_gemm = (
                self.weight.dtype == torch.bfloat16 and self.out_dtype == torch.float32 and is_available()
            )
    ...
    def set_out_dtype(self, out_dtype: torch.dtype) -> None:
        ...
        if not self.allow_cublas_router_gemm and self._router_gemm_cublas_capable and out_dtype == torch.float32:
        ...
        if self.allow_specialized_router_gemm:
            ...
            self.allow_ll_bf16_gemm = (
                self.weight.dtype == torch.bfloat16 and out_dtype == torch.float32 and is_available()
            )
