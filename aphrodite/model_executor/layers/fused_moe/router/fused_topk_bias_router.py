...
def aphrodite_topk_softplus_sqrt(
    topk_weights: torch.Tensor,
    topk_indices: torch.Tensor,
    token_expert_indices: torch.Tensor,
    gating_output: torch.Tensor,
...
def fused_topk_bias(
    ...
    if input_tokens is not None and hash_indices_table is not None and input_tokens.dtype != hash_indices_table.dtype:
    ...
    if not rocm_aiter_ops.is_fused_moe_enabled():
        ...
        if scoring_func == "softmax":
            ...
            topk_weights, topk_ids = aphrodite_topk_sigmoid(
                topk_weights,
                topk_ids,
                token_expert_indices,
                gating_output,
    ...
    if bias_vl is not None and image_sentinel_lo > 0:
        ...
        assert input_tokens is not None, "bias_vl routing requires input_tokens"
        image_mask = ((input_tokens >= image_sentinel_lo) & (input_tokens < image_sentinel_lo + 5)).unsqueeze(-1)
        text_bias = e_score_correction_bias if e_score_correction_bias is not None else torch.zeros_like(bias_vl)
    ...
    if hash_indices_table is not None:
        ...
        if image_mask is None:
            ...
            # lookup well-defined.
            safe_ids = torch.where(image_mask.squeeze(-1), 0, input_tokens).to(hash_indices_table.dtype)
            topk_indices = hash_indices_table[safe_ids]
            vl_indices = torch.topk(scores_for_choice, k=topk, dim=-1)[1]
            topk_indices = torch.where(image_mask, vl_indices.to(topk_indices.dtype), topk_indices)
    else:
