...
def fused_flydsl_moe_impl(
    ...
    tile_m, tile_n, tile_k, tile_n2, tile_k2 = _snap_flydsl_int4_tiles(tile_m, tile_n, tile_k, tile_n2, tile_k2)
    ...
    inter_sorted = torch.empty((sorted_size, inter_dim), device=device, dtype=torch.bfloat16)
    w1_scale_u8 = w1_scale.view(torch.uint8).contiguous().view(-1) if w1_scale is not None else empty_scale
    flydsl_a16w4_gemm1(
    ...
    w2_scale_u8 = w2_scale.view(torch.uint8).contiguous().view(-1) if w2_scale is not None else empty_scale
    flydsl_a16w4_gemm2(
        inter_sorted_bf16=inter_sorted,
...
def fused_flydsl_moe(
    ...
    return torch.ops.aphrodite.fused_flydsl_moe_impl(
