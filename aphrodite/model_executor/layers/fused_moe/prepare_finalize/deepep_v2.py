...
def _quantize_before_dispatch(quant_config: FusedMoEQuantConfig, defer_input_quant: bool) -> bool:
    """
...
def _pack_mxfp8_scale(scale: torch.Tensor) -> torch.Tensor:
    ...
    assert scale.size(1) % 4 == 0, (
        f"mxfp8 dispatch needs hidden_size % {MXFP8_BLOCK_SIZE * 4} == 0, got {scale.size(1)} scale columns"
    )
    return scale.contiguous().view(torch.int32)


def _unpack_mxfp8_scale(scale: torch.Tensor, hidden_size: int, is_scale_swizzled: bool) -> torch.Tensor:
    ...
    **Prefill mode (use_cudagraph=False):**
    ...
    def _global_expert_ids(self, num_local: int, device: torch.device) -> torch.Tensor:
        ...
        if ids is None or ids.numel() != num_local or ids.device != device:
            ids = torch.arange(num_local, dtype=torch.int64, device=device) + self.rank_expert_offset
        ...
        if recv_topk_idx is None:
            ...
            if total_tokens > 0:
                recv_topk_idx = torch.repeat_interleave(
                    self._global_expert_ids(len(recv_expert_num_tokens), expert_x.device),
