...
class RoutedExperts(PluggableLayer):
    ...
    def expert_map(self) -> torch.Tensor | None:
        ...
        # rather than keying on the global APHRODITE_ROCM_USE_AITER switch, which
    ...
    def update_expert_map_info(self):
        ...
        self.register_buffer("_expert_map", self.expert_map_manager.expert_map, persistent=False)
        self.register_buffer("expert_mask", self.expert_map_manager.expert_mask, persistent=False)
        ...
        if routing_tables is not None:
            ...
            self.register_buffer("expert_global_to_physical", global_to_physical, persistent=False)
            self.register_buffer("expert_physical_to_global", physical_to_global, persistent=False)
            self.register_buffer("expert_local_to_global", local_global, persistent=False)
    ...
    def _orient_fused_weight(
        ...
        """Normalise a fused expert tensor to the Aphrodite weight layout."""
    ...
    def weight_loader(
        ...
        if "scale" in weight_name or "zero" in weight_name or "offset" in weight_name:
            ...
            if quant_method == FusedMoeWeightScaleSupported.CHANNEL.value:
                ...
                if quant_method == FusedMoeWeightScaleSupported.BLOCK.value and scale_refine is not None:
                    ...
                    loaded_weight = loaded_weight.repeat_interleave(scale_refine[0], dim=-2).repeat_interleave(
                        scale_refine[1], dim=-1
                    )
                self._load_model_weight_or_group_weight_scale(
                    shard_id=shard_id,
                    shard_dim=shard_dim,
        ...
        for expert_name, loaded_weight in weights:
            ...
            for param_name, weight_name, expert_id, shard_id in expert_mapping:
                ...
                is_per_expert_fused_w13 = (
                    ...
                    and any(f".{fused_name}." in qual_name for fused_name in ("gate_up_proj", "w13"))
                )
                ...
                if param is None:
                    ...
                    raise AttributeError(
                        f"Layer {self.layer_name} has no parameter {param_name!r} for checkpoint weight {qual_name!r}"
                    )
                if is_fused:
                    quant_method = getattr(param, "quant_method", None)
                    ...
                    uses_weight_layout = (
                        "scale" not in weight_name or quant_method == FusedMoeWeightScaleSupported.BLOCK.value
                    )
                    ...
                    experts_shard = loaded_weight.chunk(2, dim=0)[shard_index].unsqueeze(0)
                    start = expert_id
                else:
    ...
    def build_expert_params_mapping(
        ...
        w13 = f"experts.{lora_base_layer_prefix_on_param_name}{routed_experts_prefix}w13_"
        ...
        if include_fused:
            ...
            if gate_up is not None:
                ...
                fused_mapping.extend(
                    (
                        w13,
                        (f"experts.{physical_to_logical_map[expert_id]}.{gate_up}.{lora_base_layer_prefix}"),
                        expert_id,
                        shard_id,
                    )
