...
class MoEActivation(Enum):
    ...
    def custom_op_name(self) -> str:
        ...
        in aphrodite/model_executor/layers/activation.py."""
...
def _apply_moe_activation_masked(
    ...
    if masked_activation is None:
        raise NotImplementedError(f"Masked MoE activation is not implemented for {activation.value}")
    ...
    assert valid_token_counts.dtype == torch.int32, "valid_token_counts must use torch.int32"
    assert valid_token_counts.dim() == 1, "valid_token_counts must be 1D"
    assert valid_token_counts.device == input.device, "valid_token_counts must be on the input device"
    ...
    assert valid_token_counts.size(0) == expected_counts, (
        f"valid_token_counts must have {expected_counts} element(s) for {input.dim()}D input"
    )
    ...
    if activation == MoEActivation.SILU and config.clamp_limit is not None:
        ...
        assert config.activation_situ_beta is not None, "SITU requires activation_situ_beta from FusedMoEConfig"
    elif activation == MoEActivation.SWIGLUOAI:
        masked_clamp_limit = 7.0
        masked_alpha = 1.702
    elif activation == MoEActivation.SWIGLUOAI_UNINTERLEAVE:
        assert config.clamp_limit is not None, "SWIGLUOAI_UNINTERLEAVE requires clamp_limit"
    ...
    torch.ops._C.masked_moe_activation(
        ...
        -1.0 if config.activation_situ_linear_beta is None else config.activation_situ_linear_beta,
    )
    return output
...
def silu_and_mul_with_clamp(
    ...
    if current_platform.is_xpu():
        ...
        from aphrodite.model_executor.layers.fused_moe.utils import swiglu_limit_func
...
def apply_moe_activation(
    ...
    *,
    ...
    buffer.
    """
    config = _DEFAULT_APPLY_MOE_ACTIVATION_CONFIG if activation_config is None else activation_config
    ...
    if activation == MoEActivation.SILU:
        ...
        assert config.clamp_limit is not None, "SWIGLUOAI_UNINTERLEAVE requires clamp_limit"
        torch.ops._C.silu_and_mul_with_clamp(output, input, config.clamp_limit, config.alpha, config.beta)
    elif activation == MoEActivation.SWIGLUSTEP:
        from aphrodite.model_executor.layers.activation import swiglustep_and_mul_triton
