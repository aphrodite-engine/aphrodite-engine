...
def map_wna16_backend(runner_backend: MoEBackend) -> WNA16MoEBackend:
    ...
    if backend := mapping.get(runner_backend):
        return backend
    raise ValueError(
        f"moe_backend='{runner_backend}' is not supported for WNA16 MoE. Expected one of {list(mapping.keys())}."
    )
...
def select_wna16_moe_backend(
    ...
    if runner_backend != "auto":
        ...
        reason = _backend_incompatibility_reason(requested_backend, quant_config, may_have_zp, may_have_bias)
        if reason is not None:
            raise ValueError(_make_log_unsupported(requested_backend, reason))
        return _return_or_raise(requested_backend, config, weight_key, None, activation_format)
        return _return_or_raise(requested_backend, config, weight_key, None, activation_format)
    ...
    for backend in AVAILABLE_BACKENDS:
        reason = _backend_incompatibility_reason(backend, quant_config, may_have_zp, may_have_bias)
        if reason is not None:
...
def _process_weights_cpu(
    ...
    from aphrodite.model_executor.layers.quantization.auto_awq import (
        AutoAWQConfig,
    )
...
def _process_weights_xpu(
    ...
    `aphrodite_xpu_kernels.fused_moe_interface.xpu_fused_moe(is_int4=True)` expects:
...
def _humming_wna16_weight_schema(
    ...
    raise TypeError(
        f"Humming WNA16 checkpoint schema requires AutoAWQConfig or AutoGPTQConfig, got {type(quant_config).__name__}."
    )
...
def _convert_moe_wna16_humming_tensors(
    ...
    if sys.byteorder != "little":
        raise NotImplementedError("MoeWNA16 to Humming conversion requires a little-endian host.")
    ...
    if has_zero_point:
        ...
        output["zero_point"] = qzeros.transpose(-1, -2).contiguous().view(torch.int32).transpose(-1, -2).contiguous()
    return output
...
class _MoeWNA16HummingWeightSchema:
    ...
    def convert_humming(
        ...
        from aphrodite.utils.humming import HummingWeightSchema, dtypes

        output = _convert_moe_wna16_humming_tensors(tensors, has_zero_point=self.has_zero_point)
...
def convert_to_wna16_moe_kernel_format(
    ...
    if backend == WNA16MoEBackend.HUMMING:
        ...
        if isinstance(quant_config, MoeWNA16Config):
            from aphrodite.utils.humming import HummingInputSchema
