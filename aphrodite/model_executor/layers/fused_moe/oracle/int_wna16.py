...
def _process_weights_marlin(
    ...
    marlin_w2_scales = marlin_moe_permute_scales(
        ...
        * (quant_config.group_size if quant_config.group_size != -1 else quant_config.pack_factor),
...
def convert_to_wna16_moe_kernel_format(
    ...
    if backend in (
        ...
        if not isinstance(quant_config, GPTQMarlinConfig):
            raise TypeError(f"Marlin WNA16 MoE backend requires GPTQMarlinConfig, got {type(quant_config).__name__}.")
