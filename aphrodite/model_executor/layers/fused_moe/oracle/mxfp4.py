...
def backend_to_kernel_cls(
    ...
    if backend == Mxfp4MoeBackend.DEEPGEMM_MXFP4:
        ...
        from aphrodite.model_executor.layers.fused_moe.flashinfer_cutlass_moe import (
            FlashInferExperts,
        )
...
def select_gpt_oss_mxfp4_moe_backend(
    ...
    triton_kernels_supported = (
        has_triton_kernels() and device_capability is not None and (9, 0) <= device_capability < (11, 0)
    )
    ...
    if envs.is_set("APHRODITE_USE_FLASHINFER_MOE_MXFP4_BF16"):
        if not envs.APHRODITE_USE_FLASHINFER_MOE_MXFP4_BF16:
            ...
            raise ValueError(
                "APHRODITE_USE_FLASHINFER_MOE_MXFP4_BF16=1 is set but the "
    ...
    if (
        envs.is_set("APHRODITE_USE_FLASHINFER_MOE_MXFP4_MXFP8_CUTLASS")
        and envs.APHRODITE_USE_FLASHINFER_MOE_MXFP4_MXFP8_CUTLASS
    ):
        return _return_or_raise(
            Mxfp4MoeBackend.FLASHINFER_CUTLASS_MXFP4_MXFP8,
            config,
    ...
    if envs.is_set("APHRODITE_MXFP4_USE_MARLIN") and envs.APHRODITE_MXFP4_USE_MARLIN:
        return _return_or_raise(
            Mxfp4MoeBackend.MARLIN,
            config,
            kMxfp4Static,
...
def convert_weight_to_mxfp4_moe_kernel_format(
    ...
    if mxfp4_backend in TRTLLM_BACKENDS:
        ...
        raise ValueError(
            f"Unsupported mxfp4_backend for Mxfp4MoEMethod: {mxfp4_backend}. Expected TRTLLM or Triton backend."
        )
...
def make_mxfp4_moe_kernel(
    ...
    kernel = mk.FusedMoEKernel(
        prepare_finalize,
        experts,
        shared_experts=(shared_experts if moe_config.moe_parallel_config.use_batched_activation_format else None),
