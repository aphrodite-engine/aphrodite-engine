...
from aphrodite.model_executor.layers.quantization.utils.w8a8_utils import all_close_1d
...
def backend_to_kernel_cls(
    ...
    if backend in B12X_BACKENDS:
        from aphrodite.model_executor.layers.fused_moe.b12x import B12xExperts
        ...
        from aphrodite.model_executor.layers.fused_moe.experts.trtllm_mxfp4_moe import (
        ...
        from aphrodite.model_executor.layers.fused_moe.experts.flashinfer_cutlass_moe import (  # noqa: E501
            FlashInferExperts,
        )
        ...
        from aphrodite.model_executor.layers.fused_moe.experts.fused_humming_moe import (
        ...
        from aphrodite.model_executor.layers.fused_moe.experts.aiter_mxfp4_w4a16_moe import (
        ...
        from aphrodite.model_executor.layers.fused_moe.experts.cpu_moe import CPUExpertsMxfp4

        return [CPUExpertsMxfp4]

    elif backend == Mxfp4MoeBackend.EMULATION:
        from aphrodite.model_executor.layers.fused_moe.experts.ocp_mx_emulation_moe import (
...
def _user_moe_activation_override() -> QuantKey | None:
    ...
    args = get_current_aphrodite_config().model_config.quantization_config
...
def _make_log_unsupported(backend: Mxfp4MoeBackend, reason: str | None) -> str:
    base = f"Mxfp4 MoE backend '{backend.value}' does not support the deployment configuration"
...
def _filter_by_activation(
    ...
    if requested_activation_key is not None:
        return [
            b
            for b in backends
            if _backend_activation_key(b) == requested_activation_key or b == Mxfp4MoeBackend.EMULATION
        ]
...
def _get_requested_backends(
    ...
    if runner_backend == "b12x":
        if envs.APHRODITE_B12X_MOE_FP4_FORCE_A16:
...
def select_mxfp4_moe_backend(
    ...
    if runner_backend != "auto":
        requested_backends = _get_requested_backends(runner_backend, requested_activation_key)
        if activation_format == mk.FusedMoEActivationFormat.BatchedExperts:
            requested_backends = [
                Mxfp4MoeBackend.BATCHED_MARLIN if b == Mxfp4MoeBackend.MARLIN else b for b in requested_backends
            ]
        if not requested_backends:
            raise ValueError(f"moe_backend={runner_backend!r} does not support activation={requested_activation_key}")
    ...
    AVAILABLE_BACKENDS = _filter_by_activation(_get_priority_backends_for_gpt_oss(), requested_activation_key)

    unsupported_reasons = []
    for backend in AVAILABLE_BACKENDS:
        # Use requested_activation_key if provided, otherwise use backend default
        act_key = requested_activation_key if requested_activation_key is not None else _backend_activation_key(backend)
        for k_cls in backend_to_kernel_cls(backend):
            supported, reason = k_cls.is_supported_config(k_cls, config, kMxfp4Static, act_key, activation_format)
            if supported:
    ...
    unsupported_log = "; ".join(
        [f"backend: {backend.value}, reason: {reason}" for backend, reason in unsupported_reasons]
    )
...
def select_deepseek_v4_mxfp4_moe_backend(
    ...
    if current_platform.is_rocm() and config.routing_method == RoutingMethodType.DeepseekV4:
        priority_backends = [
            Mxfp4MoeBackend.AITER_MXFP4_BF16,
            Mxfp4MoeBackend.TRITON_UNFUSED,
        ]
...
def mxfp4_round_up_hidden_size_and_intermediate_size(
    ...
    if backend == Mxfp4MoeBackend.EMULATION:
        # Emulation has no kernel tile; it only needs OCP MX block alignment so
        # scale buffers don't floor-truncate a non-block-aligned TP/DP shard.
...
def convert_gpt_oss_weight_to_mxfp4_moe_kernel_format(
    ...
    if mxfp4_backend == Mxfp4MoeBackend.HUMMING:
        ...
        if mxfp4_backend == Mxfp4MoeBackend.FLASHINFER_CUTLASS_MXFP4_MXFP8:
            ...
            w13_weight_interleaved = interleave_moe_weights_for_sm90_mixed_gemm(w13_weight_swapped.contiguous(), "fp4")
            w2_weight_interleaved = interleave_moe_weights_for_sm90_mixed_gemm(w2_weight.contiguous(), "fp4")
            w31_scales_interleaved = interleave_moe_scales_for_sm90_mixed_gemm(w13_scale_swapped.to(torch.uint8))
            w2_scale_interleaved = interleave_moe_scales_for_sm90_mixed_gemm(w2_weight_scale.data.to(torch.uint8))
        ...
        w13_weight_scale.data = e8m0_shuffle(w13_weight_scale.view(s0 * s1, -1)).view(s0, s1, -1)

        s0, s1, _ = w2_weight_scale.shape
        w2_weight_scale.data = e8m0_shuffle(w2_weight_scale.view(s0 * s1, -1)).view(s0, s1, -1)
        ...
        shuffled_w13, shuffled_w2 = rocm_aiter_ops.shuffle_weights(w13_weight, w2_weight)
        ...
        packed_w13, packed_w2, packed_w13_scale, packed_w2_scale = prepare_mxfp4_moe_layer_for_cpu(
            w13_weight.data,
            w2_weight.data,
            w13_weight_scale.data,
        ...
        w13_has_per_expert_scale = (
            w13_input_scale is not None and w13_input_scale.ndim == 1 and not all_close_1d(w13_input_scale)
        )
        w2_has_per_expert_scale = (
            w2_input_scale is not None and w2_input_scale.ndim == 1 and not all_close_1d(w2_input_scale)
        )
        ...
        if w13_input_scale is not None:
            layer.w13_input_scale = torch.nn.Parameter(w13_input_scale.max().to(torch.float32), requires_grad=False)
        if w2_input_scale is not None:
            layer.w2_input_scale = torch.nn.Parameter(w2_input_scale.max().to(torch.float32), requires_grad=False)
        return (
            w13_weight,
            w2_weight,
            w13_weight_scale,
...
def convert_weight_to_mxfp4_moe_kernel_format(
    ...
    if mxfp4_backend == Mxfp4MoeBackend.HUMMING:
        ...
        convert_to_humming_moe_kernel_format(layer, quant_config={"quant_method": "mxfp4"})
        return (
            layer.w13_weight,
            layer.w2_weight,
            layer.w13_weight_scale,
    ...
    if mxfp4_backend in TRTLLM_BACKENDS:
        ...
        w13_weight = torch.stack([w3_weight, w1_weight], dim=2).reshape(w13_weight.shape)

        w1_scale = w13_weight_scale[:, :intermediate_size, :]
        w3_scale = w13_weight_scale[:, intermediate_size:, :]
        w13_weight_scale = torch.stack([w3_scale, w1_scale], dim=2).reshape(w13_weight_scale.shape)
        ...
        if w13_bias is not None:
            ...
            w13_bias = w13_bias.reshape(num_experts, -1, 1)[:, w13_b_perm].reshape(num_experts, -1)
        ...
        if w2_bias is not None:
            ...
            w2_bias = w2_bias.reshape(num_experts, -1, 1)[:, w2_b_perm].reshape(num_experts, -1)
        ...
        if activation == MoEActivation.SITU:
            ...
            w13 = rocm_aiter_ops.shuffle_weight_a16w4(w13_weight.data.view(fp4_dtype), 16, guinterleave)
            w2 = rocm_aiter_ops.shuffle_weight_a16w4(w2_weight.data.view(fp4_dtype), 16, False)
    ...
    elif mxfp4_backend in TRITON_BACKENDS or (mxfp4_backend == Mxfp4MoeBackend.AITER_MXFP4_BF16 and is_gfx1250):
        from triton_kernels.matmul_ogs import FlexCtx, PrecisionConfig
        ...
        if mxfp4_backend == Mxfp4MoeBackend.FLASHINFER_CUTLASS_MXFP4_MXFP8:
            ...
            w13_weight_scale = block_scale_interleave(w13_weight_scale.view(torch.uint8)).reshape(w13_scale_shape)

            w2_scale_shape = w2_weight_scale.shape
            w2_weight_scale = block_scale_interleave(w2_weight_scale.data.view(torch.uint8)).reshape(w2_scale_shape)
        else:
            from flashinfer.fused_moe import (
                interleave_moe_scales_for_sm90_mixed_gemm,
                interleave_moe_weights_for_sm90_mixed_gemm,
            ...
            w13_weight = interleave_moe_weights_for_sm90_mixed_gemm(w13_weight.contiguous(), "fp4")
            w2_weight = interleave_moe_weights_for_sm90_mixed_gemm(w2_weight.data.contiguous(), "fp4")
            w13_weight_scale = interleave_moe_scales_for_sm90_mixed_gemm(w13_weight_scale.to(torch.uint8))
            w2_weight_scale = interleave_moe_scales_for_sm90_mixed_gemm(w2_weight_scale.data.to(torch.uint8))
