...
def _flashinfer_topk() -> Callable[..., tuple[torch.Tensor, torch.Tensor]] | None:
    ...
    if not has_flashinfer():
        logger.info_once("flashinfer is unavailable; vocab-parallel top-k uses torch.topk, at roughly half the speed.")
        return None
...
class LogitsProcessor(PluggableLayer):
    ...
    def _apply_head(
        ...
        if not isinstance(lm_head.quant_method, (UnquantizedEmbeddingMethod, UnquantizedLinearMethod)):
            raise ValueError(
                "A head_dtype different from the model dtype is only supported for an unquantized lm_head."
            )
        if (
    ...
    def get_top_k_tokens(
        ...
        if self.scale <= 0.0 and self.scale != 1.0:
            raise ValueError(
                "The local top-k reduction optimization is not supported for non-positive logit scaling factors."
            )
