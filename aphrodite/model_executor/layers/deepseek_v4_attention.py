...
from aphrodite.utils.deep_gemm import fp8_einsum
from aphrodite.utils.torch_utils import direct_register_custom_op
from aphrodite.v1.attention.ops.deepseek_v4_ops import (
...
from aphrodite.model_executor.layers.deepseek_compressor import DeepseekCompressor
...
class DeepseekV4MultiHeadLatentAttentionWrapper(PluggableLayer):
    ...
    def __init__(
        ...
        if num_heads <= 64:
            ...
            raise ValueError(f"DeepseekV4 attention does not support {num_heads} heads (must be <= 128).")
        ...
        # Extract config from aphrodite_config
        config = mla_modules.aphrodite_config.model_config.hf_config
        ...
        compilation_config = mla_modules.aphrodite_config.compilation_config
        # HACK
        ...
        if self.compress_ratio > 1:
            self.compressor = DeepseekCompressor(
                aphrodite_config=mla_modules.aphrodite_config,
                compress_ratio=self.compress_ratio,
    ...
    def forward(
        ...
        torch.ops.aphrodite.deepseek_v4_fp8_einsum(
            o_fp8,
            o_scale,
            wo_a_fp8,
            wo_a_scale,
    ...
    def attention_impl(
        ...
        if not isinstance(attn_metadata, dict):
            ...
            N = 0 if swa_only else (sub.max_model_len + sub.compress_ratio - 1) // sub.compress_ratio
...
class DeepseekV4MLAAttention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        # Get Aphrodite config for cache setup
        ...
        assert kv_cache_dtype.startswith("fp8"), (
            f"DeepseekV4 only supports fp8 kv-cache format for now, got {kv_cache_dtype}"
        )
    ...
    def forward(
        ...
        flashmla_metadata = cast(FlashMLASparseMetadata | None, attn_metadata.get(self.prefix))
        swa_metadata = cast("DeepseekSparseSWAMetadata | None", attn_metadata.get(self.swa_cache_layer.prefix))
        assert swa_metadata is not None
