...
class QuarkOCP_MX(QuarkScheme):
    ...
    def __init__(
        ...
        self.input_dtype = (
            next(dtype for dtype, quant_key in _ACTIVATION_QUANT_KEY_MAP.items() if quant_key == activation_quant_key)
        ...
        if current_platform.supports_mx() and (self.input_dtype != "mxfp4" or self.weight_dtype != "mxfp4"):
            logger.warning_once(
                ...
                "in Aphrodite. Simulated weight dequantization and activation "
                "QDQ (quantize and dequantize) will be used, with the linear "
                "layers computed in high precision."
            )
