...
from aphrodite.model_executor.layers.quantization.base_config import (  # noqa: E501
    QuantizationConfig,
    QuantizeMethodBase,
)
from aphrodite.model_executor.layers.quantization.kv_cache import BaseKVCacheMethod
from aphrodite.model_executor.layers.quantization.quark.quark_moe import (  # noqa: E501
    QuarkMoEMethod,
)
from aphrodite.model_executor.layers.quantization.quark.schemes import (
    QuarkNVFP4,
    QuarkOCP_MX,
    QuarkScheme,
    QuarkW4A8_MXFP4_FP8,
...
class QuarkConfig(QuantizationConfig):
    def __init__(
        ...
        self.packed_modules_mapping = dict(getattr(type(self), "packed_modules_mapping", {}))
    ...
    def apply_aphrodite_mapper(  # noqa: B027
        self, hf_to_aphrodite_mapper: "WeightsMapper"
    ):
        """
        Interface for models to update module names referenced in
        quantization configs in order to reflect the aphrodite model structure

        Args:
            hf_to_aphrodite_mapper: maps from hf model structure (the assumed
                structure of the qconfig) to aphrodite model structure
        """
        quant_config_with_hf_to_aphrodite_mapper: dict[str, Any] = {}

        for k, v in self.quant_config.items():
            if isinstance(v, list):
                quant_config_with_hf_to_aphrodite_mapper[k] = hf_to_aphrodite_mapper.apply_list(v)
            elif isinstance(v, dict):
                quant_config_with_hf_to_aphrodite_mapper[k] = hf_to_aphrodite_mapper.apply_dict(v)
            else:
                if isinstance(v, str):
                    mapped_v_list = hf_to_aphrodite_mapper.apply_list([v])
                    if mapped_v_list:
                        quant_config_with_hf_to_aphrodite_mapper[k] = mapped_v_list[0]
                else:
                    quant_config_with_hf_to_aphrodite_mapper[k] = v

        self.quant_config = quant_config_with_hf_to_aphrodite_mapper

    def get_quant_method(self, layer: torch.nn.Module, prefix: str) -> "QuantizeMethodBase | None":
        weight_quant_key, activation_quant_key, method_cls = self.get_quant_method_target(prefix, type(layer))
        ...
        dynamic_mxfp4_quant = is_ignored and "self_attn" in prefix and self.dynamic_mxfp4_quant
    ...
    def get_quant_method_target(
        ...
        QuantKey | None,
        QuantKey | None,
        type[QuantizeMethodBase] | type[UnquantizedLinearMethod] | type[UnquantizedFusedMoEMethod] | None,
    ]:
        ...
        if should_ignore_layer(
            ...
            if issubclass(layer_type, LinearBase):
                if "self_attn" in prefix and self.dynamic_mxfp4_quant:
                    weight_quant_key, activation_key, _ = self.get_scheme_cls(layer_type, prefix)
        ...
        if is_routed_experts:
            weight_quant_key, activation_quant_key, quant_method_cls = QuarkMoEMethod.get_moe_method_target(
                self, layer_type, prefix
            )
    ...
    def from_config(cls, config: dict[str, Any]) -> "QuarkConfig":
        ...
        if export_config is None:
            raise ValueError("The export key should be included in the configurations of Quark quantized model")
        ...
        if len(kv_cache_group) == 0:
            ...
            if not all(
                deep_compare(q_config["output_tensors"], q_configs[0]["output_tensors"]) for q_config in q_configs
            ):
                raise ValueError(
    ...
    def _is_fp8_w4a8(
        ...
        if not isinstance(weight_quant, list) or len(weight_quant) != 2 or not isinstance(input_quant, dict):
            return QuantKeyMatch(False, None, None)
    ...
    def _is_fp8_w8a8(
        ...
        # Confirm weight scheme is supported
        is_fp8_dtype = weight_quant.get("dtype") == "fp8_e4m3" and input_quant.get("dtype") == "fp8_e4m3"
        ...
        if input_quant.get("is_dynamic"):
            activation_quant_key = (
                kFp8DynamicTokenSym if input_quant.get("qscheme") == "per_channel" else kFp8DynamicTensorSym
            )
            weight_quant_key = (
                kFp8StaticChannelSym if weight_quant.get("qscheme") == "per_channel" else kFp8StaticTensorSym
            )
        ...
        if not is_per_tensor_activation:
            return QuantKeyMatch(False, None, None)
        weight_quant_key = kFp8StaticChannelSym if weight_quant.get("qscheme") == "per_channel" else kFp8StaticTensorSym
    ...
    def _is_w8a8_int8(
        ...
        is_int8_dtype = weight_quant.get("dtype") == "int8" and input_quant.get("dtype") == "int8"

        is_valid_weight_scheme = weight_quant.get("qscheme") in [
            "per_tensor",
            "per_channel",
        ]
        is_static_input = input_quant.get("qscheme") == "per_tensor" and not input_quant.get("is_dynamic")
        is_dynamic_input = input_quant.get("qscheme") == "per_channel" and input_quant.get("is_dynamic") is True
        ...
        matches = (
            ...
            and ((is_static_input and not weight_quant.get("is_dynamic")) or is_dynamic_input)
        )
        if not matches:
            return QuantKeyMatch(False, None, None)
        weight_quant_key = (
            kInt8StaticChannelSym if weight_quant.get("qscheme") == "per_channel" else kInt8StaticTensorSym
        )
        if is_static_input:
            activation_quant_key = (
                kInt8StaticTensorSym if input_quant.get("symmetric") is True else kInt8StaticTensorAsym
            )
        elif weight_quant.get("qscheme") == "per_channel":
            activation_quant_key = (
                kInt8DynamicTokenSym if input_quant.get("symmetric") is True else kInt8DynamicTokenAsym
            )
        else:
            activation_quant_key = (
                kInt8DynamicTensorSym if input_quant.get("symmetric") is True else kInt8DynamicTensorAsym
            )
        return QuantKeyMatch(True, activation_quant_key, weight_quant_key)
    ...
    def _is_w_ocp_mx_a_x(
        ...
        if weight_quant.get("qscheme") != "per_group":
            logger.debug(
                "Quark model's weight quantization is incompatible with OCP MX format: weight is not per_group."
            )
            return QuantKeyMatch(False, None, None)
        ...
        if weight_quant.get("group_size") != 32:
            logger.debug(
                "Quark model's weight quantization is incompatible with OCP MX format: group_size of weight is not 32."
            )
            return QuantKeyMatch(False, None, None)
        ...
        if input_quant is None:
            ...
            if (
                ...
                logger.debug("Quark model's OCP MX quantization is incompatible with static input scales.")
                return QuantKeyMatch(False, None, None)
            ...
            if input_dtype not in _ACTIVATION_QUANT_KEY_MAP:
                raise ValueError(
                    f"Unsupported input_dtype={input_dtype} in Quark's Aphrodite "
    ...
    def get_layer_quant_config_from_name(self, layer_name: str) -> dict[str, Any] | None:
        ...
        if proj_name in self.packed_modules_mapping:
            ...
            if matched_configs and not all(deep_compare(config, matched_configs[0]) for config in matched_configs):
                raise ValueError(
            ...
            layer_quant_config = cast(dict[str, Any], self.quant_config.get("layer_quant_config") or {})
    ...
    def _find_matched_config(self, layer_name: str, module: torch.nn.Module | type[torch.nn.Module]) -> dict[str, Any]:
        # Priority order:
        ...
        layer_type_quant_config = cast(dict[str, Any], self.quant_config.get("layer_type_quant_config"))
        global_quant_config = cast(dict[str, Any], self.quant_config.get("global_quant_config"))
        ...
        if proj_name in self.packed_modules_mapping:
            ...
            if not all(deep_compare(config, shard_configs[0]) for config in shard_configs):
                raise ValueError(
                    f"Found a different quantization configuration for "
                    f"{shard_proj_names} in {layer_name}. Aphrodite requires all "
    ...
    def _get_scheme_cls_from_config(
        ...
        if config.get("output_tensors") or config.get("bias"):
            raise NotImplementedError(
                "Currently, Quark models with output_tensors and bias quantized are not supported"
            )
        ...
        if (
            ...
            raise ValueError(
                "Quark W8A8 FP8 per-block weight quantization requires `block_size` in the weight quantization config."
            )
        ...
        raise NotImplementedError(
            f"No quark compatible scheme was found. Weight config: {weight_config}, Input config: {input_config}"
        )
    ...
    def get_scheme_cls(
        ...
        return self._get_scheme_cls_from_config(self._find_matched_config(layer_name, layer_type))
    ...
    def get_cache_scale_mapper() -> "WeightsMapper":
        """Map Quark KV-cache scale names to Aphrodite names."""
        orig_to_new_suffix = {
...
class QuarkKVCacheMethod(BaseKVCacheMethod):
    ...
    def validate_kv_cache_config(kv_cache_config: dict[str, Any] | None):
        ...
        if dtype != "fp8_e4m3":
            raise NotImplementedError(
                f"Currently supported kv cache quantization is dtype=fp8_e4m3, however received {dtype}"
            )
