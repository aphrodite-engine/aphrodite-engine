...
class Fp8PtpcOnlineLinearMethod(OnlineLinearBase):
    ...
    def process_weights_after_loading(self, layer: Module) -> None:
        ...
        amax = amax_for_tp_weight_quant(amax, _is_tp_sharded(layer, reduces_output_dim=False))
    ...
    def apply(
        ...
        if envs.APHRODITE_BATCH_INVARIANT and not isinstance(self.fp8_linear, CutlassFP8ScaledMMLinearKernel):
            weight_dequant = layer.weight.to(x.dtype) * layer.weight_scale.to(x.dtype).t()
    ...
    def __init__(
        ...
        *,
        ...
        self.fp8_backend, self.experts_cls = select_fp8_moe_backend(
            ...
            allow_aphrodite_cutlass=allow_aphrodite_cutlass,
        )
...
class Fp8PerTensorOnlineMoEMethod(_Fp8OnlineMoEBase):
    ...
    def process_weights_after_loading(self, layer: Module) -> None:
        ...
        for expert in range(layer.local_num_experts):
            w13[expert, :, :], _ = ops.scaled_fp8_quant(layer.w13_weight[expert, :, :], scale=w13_scale[expert])
            w2[expert, :, :], _ = ops.scaled_fp8_quant(layer.w2_weight[expert, :, :], scale=w2_scale[expert])
...
class Fp8PtpcOnlineMoEMethod(_Fp8OnlineMoEBase):
    ...
    def process_weights_after_loading(self, layer: Module) -> None:
        ...
        w13_scale = torch.ones(w13.shape[0], n_w13, 1, device=w13.device, dtype=torch.float32)
        layer.w13_input_scale = None
        layer.w2_input_scale = None
        ...
        for expert in range(layer.local_num_experts):
            ...
            w2[expert] = _fp8_quant_per_channel(layer.w2_weight[expert], w2_scale[expert])
