...
from aphrodite.model_executor.layers.fused_moe.oracle.mxfp8 import (
...
class ModelOptQuantConfigBase(QuantizationConfig):
    ...
    def apply_aphrodite_mapper(self, hf_to_aphrodite_mapper: "WeightsMapper"):
        if len(self.exclude_modules) > 0:
            ...
            self.exclude_modules = hf_to_aphrodite_mapper.apply_list(new_exclude_modules)
...
class ModelOptFp8Config(ModelOptQuantConfigBase):
    ...
    def __init__(
        ...
        if self.quant_method not in supported:
            raise ValueError(
                "Unsupported ModelOpt FP8 quant_algo for Aphrodite: "
                f"{self.quant_method}. Supported: {' / '.join(supported)}."
            )

    def has_blocked_weights(self) -> bool:
        # Gates "+quant_fp8" in AphroditeConfig. FP8_PB_WO runs the block-scaled
...
class ModelOptNvFp4Config(ModelOptQuantConfigBase):
    ...
    def __init__(
        ...
        if quant_method not in supported:
            raise ValueError(
                f"Unsupported ModelOpt NVFP4 quant_algo: {quant_method}. Supported: {' / '.join(supported)}."
            )
...
class ModelOptMxFp8FusedMoE(FusedMoEMethodBase):
    ...
    def _dequant_mxfp8_weights_to_bf16(self, layer: RoutedExperts) -> None:
        ...
        w13_bf16 = dequant_mxfp8_to_bf16(layer.w13_weight, layer.w13_weight_scale).to(target_dtype)
        w2_bf16 = dequant_mxfp8_to_bf16(layer.w2_weight, layer.w2_weight_scale).to(target_dtype)

        replace_parameter(layer, "w13_weight", w13_bf16)
        replace_parameter(layer, "w2_weight", w2_bf16)

        logger.info_once(
            "MXFP8->BF16 load-time dequant complete (%d experts/layer); MoE now runs in BF16 with no per-step dequant.",
            num_experts,
        )
    ...
    def process_weights_after_loading(self, layer: RoutedExperts) -> None:
        ...
        # Opt out (APHRODITE_MXFP8_EMULATION_DEQUANT_AT_LOAD=0) to keep the 1-byte
        # MXFP8 weights and dequant per-step (~half the memory, much slower).
        if self.mxfp8_backend == Fp8MoeBackend.EMULATION and envs.APHRODITE_MXFP8_EMULATION_DEQUANT_AT_LOAD:
...
class ModelOptMixedPrecisionConfig(ModelOptQuantConfigBase):
    ...
    def has_blocked_weights(self) -> bool:
        ...
        return any(info.get("quant_algo", "").upper() == "FP8_PB_WO" for info in self.quantized_layers.values())
    ...
    def _resolve_quant_algo(self, prefix: str) -> str | None:
        """Look up the quant_algo for a Aphrodite-side layer prefix.
        ...
        # shard names instead of Aphrodite's packed module name.
    ...
    def _quantized_layer_prefix_candidates(prefix: str) -> tuple[str, ...]:
        ...
        if prefix.startswith("language_model.model."):
            candidates.append("model.language_model." + prefix[len("language_model.model.") :])
        elif prefix.startswith("model.language_model."):
            candidates.append("language_model.model." + prefix[len("model.language_model.") :])
    ...
    def apply_aphrodite_mapper(self, hf_to_aphrodite_mapper: "WeightsMapper"):
        super().apply_aphrodite_mapper(hf_to_aphrodite_mapper)
        if self.quantized_layers:
            self.quantized_layers = hf_to_aphrodite_mapper.apply_dict(self.quantized_layers)
...
class QuantKeyScheme:
    ...
    def reject(role) -> None:
        raise NotImplementedError(f"role {role!r} not validated for this QuantKey scheme")
...
class KFp8Block128(QuantKeyScheme):
    ...
    def process(self, layer, role) -> None:
        ...
        if s.dim() == 4:
            ...
            raise ValueError(f"Unexpected FP8_PB_WO weight_scale shape {tuple(s.shape)}")
...
class KMxfp8Static(QuantKeyScheme):
    ...
    def create_weights(self, layer, role, ctx, shapes, wl) -> None:
        ...
        if shapes.input_size_per_partition % MXFP8_BLOCK_SIZE != 0:
            raise ValueError(
                f"MXFP8 requires input size divisible by {MXFP8_BLOCK_SIZE}, got {shapes.input_size_per_partition}"
            )
        self.register_params(
            layer,
            "weight",
    ...
    def process(self, layer, role) -> None:
        ...
        # MXFP8 weight with BF16 (APHRODITE_MXFP8_EMULATION_DEQUANT_AT_LOAD, default
...
class _DropInputScale(FormatScheme):
    ...
    def extra_weights(self, layer, shapes, ctx, wl) -> None:
        ...
        layer.register_parameter("input_scale", PerTensorScaleParameter(data=data, weight_loader=wl))
    ...
    def post_process(self, layer) -> None:
        ...
        if not torch.isnan(scale).all():
            logger.warning_once("Dropping input_scale from a weight-only W4A16 NVFP4 checkpoint (deprecated).")
        del layer.input_scale
...
class ModelOptLinearMethod(LinearMethodBase):
    ...
    ``BaseAphroditeParameter`` params route through the v2 fused loader, not the
    ...
    def __init__(
        ...
        model_config = getattr(get_current_aphrodite_config_or_none(), "model_config", None)
        dtype = model_config.dtype if model_config is not None else torch.get_default_dtype()
...
# @register_weight_loader_v2_supported_method if it uses BaseAphroditeParameter
