...
class ModelOptFp8PbWoLinearMethod(LinearMethodBase):
    ...
    Aphrodite executes it as FP8 GEMM with *dynamic per-token* activation quant.
    """
...
class ModelOptNvFp4Config(ModelOptQuantConfigBase):
    ...
    def __init__(
        ...
        if quant_method == "NVFP4":
            ...
            raise ValueError(f"Unsupported ModelOpt NVFP4 quant_algo: {quant_method}. Supported: NVFP4 / W4A16_NVFP4.")
...
class ModelOptNvFp4W4A16LinearMethod(LinearMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        del input_size, output_size
        if not self.quant_config.is_checkpoint_nvfp4_serialized:
            raise ValueError("W4A16_NVFP4 quantization was selected; dynamic quantization is not supported.")
        output_size_per_partition = sum(output_partition_sizes)
        weight_loader = extra_weight_attrs.get("weight_loader")
        layer.logical_widths = output_partition_sizes
        layer.input_size_per_partition = input_size_per_partition
