...
try:
    ...
    dequant_mxfp4 = torch.ops.aphrodite.dequant_mxfp4
...
try:
    ...
    quant_dequant_mxfp4 = torch.ops.aphrodite.quant_dequant_mxfp4
except AttributeError as error:
    raise error


def xpu_mxfp4_quantize(x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    return torch.ops.aphrodite.xpu_mxfp4_quantize(x)
...
def _compute_quant_and_scale(src_tensor, valid_src_mask):
    ...
    denormal_mantissas |= ((denormal_mantissas << adjusted_exponents) != folded_mantissas).to(tl.uint32)
...
def downcast_to_mxfp(
    ...
    prmted_scale_shape = permute_shape(src_tensor.shape, permute_order)[:-1] + (out_scale_dim,)
