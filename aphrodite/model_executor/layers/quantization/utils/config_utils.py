...
def get_quark_ocp_mx_group_size(
    ...
    from aphrodite.model_executor.layers.quantization.quark.utils import should_ignore_layer
...
def is_shared_expert_quant_fse_compatible(
    ...
    if isinstance(quant_config, DeepseekV4FP8Config):
        ...
        if layer_idx >= hf_config.num_hidden_layers:
            shared_expert_prefix = f"mtp.{layer_idx - hf_config.num_hidden_layers}.ffn.shared_experts"
        else:
        ...
        if layer_config is None:
            layer_config = next(
                (
                    ...
                    and re.fullmatch(pattern.removeprefix("re:"), shared_expert_weight_name)
                ),
                None,
            )
        shared_weight_config = (layer_config or quantization_config.get("global_quant_config") or {}).get(
            "weight"
        ) or {}
    ...
    if isinstance(quant_config, QuarkConfig):
        ...
        def get_projection_quant_configs(layer_name: str) -> list[object]:
            ...
            packed_projection_names = quant_config.packed_modules_mapping.get(projection_name, [projection_name])
            return [
                quant_config.get_layer_quant_config_from_name(f"{module_prefix}.{packed_projection_name}")
                or global_quant_config
                for packed_projection_name in packed_projection_names
            ]

        expert_quant_config = quant_config.get_layer_quant_config_from_name(expert_prefix) or global_quant_config
        shared_expert_quant_configs = [
            config
            for projection_name in projection_names
            for config in get_projection_quant_configs(f"{shared_expert_prefix}.{projection_name}")
        ]
        if all(config == expert_quant_config for config in shared_expert_quant_configs):
            return True, None
        return (
            False,
            f"Quark uses different quantization configurations for routed and shared experts at {shared_expert_prefix}",
        )
    ...
    if isinstance(quant_config, Fp8Config):
        ...
        if any(
            ...
            return (
                False,
                f"FP8 ignores routed and shared experts inconsistently at {shared_expert_prefix}",
            )
    ...
    return (
        False,
        f"shared-expert FSE quantization compatibility is not implemented for {type(quant_config).__name__}",
    )
...
def find_matching_patterns(
    ...
    matches = [pattern for pattern in patterns if is_equal_or_regex_match(layer_name, pattern, use_fnmatch=use_fnmatch)]
    if matches:
        return [set(matches)]
    ...
    shard_names = [layer_name.replace(proj_name, shard_proj_name) for shard_proj_name in fused_mapping[proj_name]]
    per_shard_matches = [
        {pattern for pattern in patterns if is_equal_or_regex_match(shard_name, pattern, use_fnmatch=use_fnmatch)}
