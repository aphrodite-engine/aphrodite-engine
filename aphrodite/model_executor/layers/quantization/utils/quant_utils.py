...
def weight_amax(weight: torch.Tensor, *, dim: int | None = None, keepdim: bool = False) -> torch.Tensor:
...
def amax_for_moe_activation_quant(a_scale: torch.Tensor, enable_eplb: bool) -> torch.Tensor:
...
class ScaleDesc:
    ...
    def __str__(self):
        ...
        return f"{_dtype_abbr(self.dtype)},{'static' if self.static else 'dynamic'},{group_shape}"
...
kMxfp6E3M2Static = QuantKey(scalar_types.float6_e3m2f, scale=kMxfp6E3M2StaticGroupScale, symmetric=True)

kMxfp6E3M2DynamicGroupScale = ScaleDesc(MXFP_SCALE_DTYPE, False, GroupShape(1, 32))
kMxfp6E3M2Dynamic = QuantKey(scalar_types.float6_e3m2f, scale=kMxfp6E3M2DynamicGroupScale, symmetric=True)

kMxfp6E2M3StaticGroupScale = ScaleDesc(MXFP_SCALE_DTYPE, True, GroupShape(1, 32))
kMxfp6E2M3Static = QuantKey(scalar_types.float6_e2m3f, scale=kMxfp6E2M3StaticGroupScale, symmetric=True)

kMxfp6E2M3DynamicGroupScale = ScaleDesc(MXFP_SCALE_DTYPE, False, GroupShape(1, 32))
kMxfp6E2M3Dynamic = QuantKey(scalar_types.float6_e2m3f, scale=kMxfp6E2M3DynamicGroupScale, symmetric=True)
...
def is_layer_skipped(
    ...
    *,
    ...
    def suffix_match(prefix: str, ignored_layers: list[str]) -> bool:
        return any(prefix == layer or prefix.endswith(f".{layer}") for layer in ignored_layers)
