...
def align_fp4_moe_weights_for_fi(
    ...
    if is_act_and_mul:
        ...
        padded_w13[:, padded_intermediate : padded_intermediate + intermediate, :] = w13[:, intermediate:, :]
    else:
    ...
    if is_act_and_mul:
        ...
        padded_w13_scale[:, padded_intermediate : padded_intermediate + intermediate, :] = w13_scale[
            :, intermediate:, :
        ]
    else:
...
def _shuffle_deepseek_fp8_moe_weights(
    ...
    def shuffle_to_block_major_k(w: torch.Tensor) -> torch.Tensor:
        ...
        rows = get_shuffle_matrix_a_row_indices(w[0].view(torch.uint8), epilogue_tile_m).to(w.device)
...
def _shuffle_mxfp8_moe_weights(
    ...
    if is_gated:
        gate_idx = get_reorder_rows_for_gated_act_gemm_row_indices(w13_u[0].reshape(w13_u.shape[1], -1))
    ...
    def shuffle_scales(s: torch.Tensor) -> torch.Tensor:
        ...
        idx = get_shuffle_matrix_a_row_indices(s[0].view(torch.uint8).reshape(s.shape[1], -1), epilogue_tile_m).to(
            s.device
        )
