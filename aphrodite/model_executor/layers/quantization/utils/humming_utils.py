...
if TYPE_CHECKING:
    ...
    from aphrodite.utils.humming import dtypes as humming_dtypes
...
def _humming_weight_schema_to_quant_key(
    ...
    from aphrodite.utils.humming import WeightScaleType
...
def make_humming_moe_quant_config(
    ...
    if quant_dtype is None:
        ...
        a_quant_desc = FusedMoEQuantDesc(dtype=quant_dtype, shape=activation_group_shape)
    else:
...
def get_humming_moe_quant_config(
    ...
    if q_dtype is not None and q_dtype.startswith("float8") and input_scale_group_size == 128:
...
def convert_to_humming_moe_kernel_format(
    ...
    if weight_schema is None or input_schema is None:
        ...
        if input_schema is None:
            input_quant_config = envs.APHRODITE_HUMMING_INPUT_QUANT_CONFIG or {}
    ...
    for sublayer_name, configs in sublayer_configs.items():
        final_weight_schema, final_input_schema, humming_config = _process_single_sublayer(
            layer=layer,
            sublayer_name=sublayer_name,
