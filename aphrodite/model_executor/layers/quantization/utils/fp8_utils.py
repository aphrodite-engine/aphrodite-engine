...
def get_fp8_block_weight_scale(layer: torch.nn.Module) -> torch.Tensor | None:
    ...
    from aphrodite.model_executor.layers.quantization.quark.schemes.quark_w8a8_fp8 import (
...
def _silu_mul_quant_fp8_packed_kernel(
    ...
    while row_start < M:
        ...
        up_flat = tl.load(
            input_ptr + input_row_start + hidden_size + col_start + col_offsets[None, :],
            mask=row_mask[:, None] & col_mask[None, :],
            other=0.0,
        )

        gate = tl.reshape(gate_flat, (BLOCK_M, groups_per_cta, GROUP_SIZE)).to(tl.float32)
        ...
        scale_ptrs = output_scale_ptr + scale_pack[None, :] * output_scale_stride_k + rows[:, None]
        tl.store(
            scale_ptrs,
            packed_scale,
...
def per_token_group_quant_fp8(
    ...
    if (current_platform.is_cuda_alike() or current_platform.is_xpu()) and x.is_contiguous():
...
def w8a8_triton_block_scaled_mm(
    ...
    if _on_gfx1250:
        ...
        _As = _upcast_e8m0_to_fp32(As) if As.dtype == torch.float8_e8m0fnu else As.to(torch.float32)
        _Bs = _upcast_e8m0_to_fp32(Bs) if Bs.dtype == torch.float8_e8m0fnu else Bs.to(torch.float32)
        _K = A.shape[-1]
        _N = B.shape[0]
        _Af = A.to(torch.float32).reshape(-1, _K)
        _Asf = _As.to(torch.float32).reshape(-1, _As.shape[-1]).repeat_interleave(_bk, dim=1)[:, :_K]
    ...
    # may store block scales in exponent-only E8M0 format on any platform, so
...
def _maybe_pad_fp8_weight(weight: torch.Tensor) -> torch.Tensor:
    ...
    if (
        envs.APHRODITE_ROCM_FP8_PADDING
...
def process_fp8_weight_block_strategy(
    ...
    if current_platform.is_fp8_fnuz() and weight.dtype == torch.float8_e4m3fn:
        if weight_scale.dtype == torch.float8_e8m0fnu:
            ...
            weight_scale = (exp_bytes.to(torch.int16) + 1).clamp(max=254).to(torch.uint8).view(torch.float8_e8m0fnu)
        else:
