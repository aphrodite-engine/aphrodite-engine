...
def prepare_nvfp4_moe_layer_for_flashinfer_cutedsl(
    ...
    a13_scale = amax_for_moe_activation_quant(a13_scale, enable_eplb).repeat(num_experts)
    a2_scale = amax_for_moe_activation_quant(a2_scale, enable_eplb).repeat(num_experts)

    if layer.activation.is_gated:
        w13, w13_scale = reorder_w13_to_w31_for_flashinfer_cutedsl(layer.activation, w13, w13_scale)
...
def prepare_static_weights_for_trtllm_fp4_moe(
    ...
    gemm1_scale_scratch = torch.empty_like(gemm1_scales_linear_fp4[0], dtype=torch.uint8)
    gemm2_scale_scratch = torch.empty_like(gemm2_scales_linear_fp4[0], dtype=torch.uint8)
...
def prepare_nvfp4_moe_layer_for_fi_or_cutlass(
    ...
    if backend == NvFp4MoeBackend.FLASHINFER_TRTLLM:
        ...
        # FlashInfer's TRT-LLM block-scale shuffle asserts the gate/up
        # row dim (= up_mult * padded_intermediate, up_mult=2 when gated)
        # is a multiple of 128. So gated needs padded_intermediate % 64
        # (2*64=128); the old value 16 left 2*intermediate a multiple of
        # only 32, so an NVFP4 MoE whose rank-local intermediate is not
        # 128-aligned at TP>1 hit `assert M % 128 == 0`.
        # Padded rows are zero -> outputs unchanged.
