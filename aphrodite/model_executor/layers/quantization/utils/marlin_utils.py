...
def check_moe_marlin_supports_layer(
    ...
    return check_moe_marlin_supports_config(layer.moe_config, group_size, allow_tile_padding)
...
def marlin_make_workspace_new(
    ...
    if existing is not None:
        if existing.device != device or existing.dtype != torch.int or existing.numel() != size:
            raise ValueError(
...
def should_use_atomic_add_reduce(m: int, n: int, k: int, device: torch.device, dtype: torch.dtype) -> bool:
    ...
    if not envs.APHRODITE_MARLIN_USE_ATOMIC_ADD:
...
def get_marlin_input_dtype(prefix: str | None = None):
    if envs.APHRODITE_MARLIN_INPUT_DTYPE is None:
        return
        ...
        if not current_platform.is_device_capability(89) and not current_platform.is_device_capability_family(120):
            raise ValueError(
                ...
                "(set APHRODITE_MARLIN_INPUT_DTYPE=int8)."
            )
