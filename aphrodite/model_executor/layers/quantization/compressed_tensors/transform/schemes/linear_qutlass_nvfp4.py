...
def is_qutlass_fp4_scheme(
    ...
    return (
        ...
        and all(input_tfm.scheme.head_dim in (16, 32, 64, 128) for input_tfm in input_tfms.values())
    )
...
class QutlassNvFP4LinearMethod(CompressedTensorsLinearTransformMethod):
    ...
    def _get_flashinfer_gemm_backend(kernel: NvFp4LinearKernel) -> str:
        ...
        aphrodite.model_executor.kernels.linear._LINEAR_BACKEND_KERNEL_MAP as source of truth
        """
        ...
        raise ValueError(f"QutlassNvFP4 transform requires a FlashInfer kernel, got {kernel_type.__name__}")
    ...
    def process_weights_after_loading(self, layer):
        ...
        layer.fused_alpha = Parameter(layer.weight_global_scale / NVFP4_MAX, requires_grad=False)
        ...
        layer.flashinfer_gemm_backend = self._get_flashinfer_gemm_backend(layer.scheme.kernel)
    ...
    def apply(
        ...
        x_fp4, x_scales = fusedQuantizeNv(x_flat, layer.hadamard_matrix, layer.fused_global_scale)
