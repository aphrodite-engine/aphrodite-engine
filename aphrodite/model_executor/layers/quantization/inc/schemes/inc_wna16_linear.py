...
class INCWNA16LinearScheme(INCLinearScheme):
    ...
    def _build_gptq_method(self):
        assert isinstance(self.layer_config.group_size, int), "WNA16 only supports integer group_size."
...
class INCXPULinearBase(INCLinearScheme):
    ...
    def __init__(self, layer_config: "INCLayerConfig") -> None:
        ...
        assert isinstance(layer_config.group_size, int), "INCXPULinearBase requires integer group_size."
...
class INCARKLinearMethod(INCXPULinearBase):
    ...
    def apply_weights(
        ...
        return torch.ops.aphrodite.inc_ark_woq_linear.default(
            x,
            layer.qweight,
            layer.ark_bias,
            layer.out_features,
