...
from aphrodite.model_executor.layers.linear import LinearMethodBase, set_weight_attrs
...
from aphrodite.model_executor.layers.quantization.utils.gptq_utils import (
...
class AutoGPTQConfig(QuantizationConfig):
    ...
    def get_quant_method(self, layer: torch.nn.Module, prefix: str) -> "QuantizeMethodBase | None":
        if isinstance(layer, RoutedExperts):
            ...
            if not check_moe_marlin_supports_layer(layer, self.group_size, allow_tile_padding=not self.desc_act):
                logger.warning_once(
        ...
        quant_method = get_linear_quant_method(self, layer, prefix, AutoGPTQLinearMethod)
...
class AutoGPTQMoEMethod(FusedMoEMethodBase):
        ...
        **extra_weight_attrs,
        ...
        w13_qzeros = torch.nn.Parameter(
            torch.empty(
                num_experts,
                scales_size13,
                self.moe.w13_num_shards * intermediate_size_per_partition // self.quant_config.pack_factor,
                dtype=torch.int32,
            ),
            requires_grad=False,
        )
    ...
    def process_weights_after_loading(self, layer: RoutedExperts) -> None:
        def replace_or_register(name: str, val: torch.Tensor | None):
            ...
            if hasattr(layer, name):
                ...
                layer.register_parameter(name, torch.nn.Parameter(val, requires_grad=False))
    ...
    def get_fused_moe_quant_config(self, layer: RoutedExperts) -> FusedMoEQuantConfig:
        ...
        use_zp = not self.quant_config.is_sym or self.wna16_moe_backend == WNA16MoEBackend.CPU
