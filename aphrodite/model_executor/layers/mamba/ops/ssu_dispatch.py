...
from aphrodite.config.mamba import MambaBackendEnum, MambaConfig, MambaSSUAlgorithm
...
class TritonSSUBackend(MambaSSUBackend):
    """Triton-based SSU backend (Aphrodite's default)."""
...
def selective_state_update_replayssm_flashinfer(
    ...
    rand_seed = (
        torch.randint(0, 2**32, (1,), device=state.device, dtype=torch.int64) if enable_stochastic_rounding else None
    )
    ...
    *,
    ...
    if use_replayssm and backend == MambaBackendEnum.FLASHINFER:
        try:
            ...
            raise ImportError("FlashInfer ReplaySSM requires a compatible flashinfer-python package") from e
