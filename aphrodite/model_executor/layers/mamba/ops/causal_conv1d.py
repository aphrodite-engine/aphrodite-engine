...
def _causal_conv1d_fwd_kernel(  # continuous batching
    ...
    num_cache_lines: tl.constexpr,  # added to support Aphrodite larger cache lines
    # Strides
    ...
    if chunk_offset == 0:
        ...
        if (chunk_offset - 1) < n_block_to_fill:
            ...
            idx_tokens_last = (
                last_full_block_token_index - (n_block_to_fill - chunk_offset) * B_size - state_len
            ) + tl.arange(0, NP2_STATELEN)  # [BLOCK_M]
            x_ptrs = (
                x_ptr + (idx_tokens_last * stride_x_token)[:, None] + (idx_feats * stride_x_dim)[None, :]
            )  # [BLOCK_M,BLOCK_N,]
...
def causal_conv1d_fn(
    ...
    if conv_states is not None:
        # extensions to support Aphrodite:
...
def causal_conv1d_update(
    ...
    # adopt the strategy in Aphrodite that overwrite on 'x' directly, rather than creating a new tensor 'o'
    out = x
