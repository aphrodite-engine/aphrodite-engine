...
from aphrodite.model_executor.layers.mamba.ops.mamba_ssm import convert_rs_fp16x2, softplus
from aphrodite.model_executor.layers.mamba.ops.replayssm_config import get_replayssm_config
...
def _replayssm_output_only_precompute_kernel(
    ...
    B_cache_ptrs = B_cache_ptr + offs_k[:, None] * stride_B_cache_pos + offs_n[None, :] * stride_B_cache_dstate
    B_cache = tl.load(
        B_cache_ptrs,
...
def _replayssm_output_only_kernel(
    ...
    if not is_flush:
        ...
        x_all_cache_ptrs = (
            x_cache_ptr + offs_m[:, None] * stride_x_cache_dim + offs_k_cache[None, :] * stride_x_cache_pos
        )
        x_all_cache = tl.load(
            x_all_cache_ptrs,
        ...
        if pid_m == 0:
            B_cur = tl.load(
                B_ptr + offs_n_full * stride_B_dstate,
                mask=offs_n_full < dstate,
                other=0.0,
            )
            tl.store(dt_cache_ptr + write_pos * stride_dt_cache_pos, dt_cur)
            tl.store(
                B_cache_ptr + write_pos * stride_B_cache_pos + offs_n_full * stride_B_cache_dstate,
                B_cur,
                mask=offs_n_full < dstate,
            )
    else:
        ...
        for i in tl.static_range(FL_NDS):
            ...
            B_all_dot = tl.load(
                ...
                mask=(offs_k_dot[:, None] < write_pos) & nmask_f[None, :],
                other=0.0,
            )
            ...
            delta_state = tl.dot(x_all_ty, B_scaled, input_precision=DOT_INPUT_PRECISION)
...
def selective_state_update_replayssm_output_only(
    ...
    assert A.stride(-1) == 0 and A.stride(-2) == 0, "Cached kernel requires TIE_HDIM (A scalar per head)"
    assert dt.stride(-1) == 0, "Cached kernel requires TIE_HDIM (dt scalar per head)"
    if dt_bias is not None:
        assert dt_bias.stride(-1) == 0, "Cached kernel requires TIE_HDIM (dt_bias scalar per head)"
