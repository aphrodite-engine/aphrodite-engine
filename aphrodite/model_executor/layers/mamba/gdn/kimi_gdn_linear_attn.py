...
class _KimiGDNMergedColumnParallelLinear(MergedColumnParallelLinear):
    ...
    def weight_loader(
        ...
        tp_rank = int(self.tp_rank)  # type: ignore[has-type]
...
class KimiGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def __init__(
        ...
        assert backend == "triton", (
            f"The shared Kimi GDN layer only supports the Triton KDA prefill backend, got {backend!r}."
        )
    ...
    def _forward(
        ...
        if TYPE_CHECKING:
            ...
            from aphrodite.models.kimi_k3.amd.ops.third_party.kda import (  # type: ignore[assignment]
                chunk_kda_with_fused_gate,
                fused_recurrent_kda,
                fused_recurrent_kda_packed_decode,
            )
