...
from aphrodite.model_executor.layers.mamba.linear.base import LinearAttention
from aphrodite.model_executor.layers.mamba.linear.minimax_linear_attn import (
...
def linear_attention_prefill_and_mix(
    ...
    for prefill_idx in range(getattr(attn_metadata, "num_prefills", 0)):
        if req_offset + prefill_idx + 1 >= len(attn_metadata.query_start_loc):
            break
        if prefill_state_indices is not None and prefill_idx >= len(prefill_state_indices):
            break
        if prefill_state_indices is None and prefill_idx >= len(state_indices_tensor):
            break
        start = attn_metadata.query_start_loc[req_offset + prefill_idx]
        end = attn_metadata.query_start_loc[req_offset + prefill_idx + 1]
        if prefill_state_indices is not None:
            slot_id = prefill_state_indices[prefill_idx]
        else:
            slot_id = state_indices_tensor[req_offset + prefill_idx]
        qs = q[start:end].transpose(0, 1).contiguous()
        ks = k[start:end].transpose(0, 1).contiguous()
        vs = v[start:end].transpose(0, 1).contiguous()
...
def _bailing_linear_attn_decode_spec_step_kernel(
    ...
    dst_slot = tl.load(state_indices_ptr + req_id * state_indices_b_stride + DRAFT_IDX * state_indices_t_stride).to(
        tl.int64
    )
    if dst_slot == -1:
        return
    ...
    if DRAFT_IDX == 0:
        ...
        src_slot = tl.load(
            state_indices_ptr + req_id * state_indices_b_stride + accepted_offset * state_indices_t_stride
        ).to(tl.int64)
    else:
        src_slot = tl.load(
            state_indices_ptr + req_id * state_indices_b_stride + (DRAFT_IDX - 1) * state_indices_t_stride
        ).to(tl.int64)
    if src_slot == -1:
        return
    ...
    cache_offsets = qk_offsets[:, None] * cache_d0_stride + v_offsets[None, :] * cache_d1_stride
    src_cache_ptr = kv_cache_ptr + src_slot * cache_b_stride + head_id * cache_h_stride + cache_offsets
    dst_cache_ptr = kv_cache_ptr + dst_slot * cache_b_stride + head_id * cache_h_stride + cache_offsets
    ...
    tl.store(
        output_ptr + token_idx * output_b_stride + (head_id * D + v_offsets) * output_d_stride,
        output,
        mask=v_mask,
    )
...
class BailingMoELinearAttention(LinearAttention):
    ...
    Aphrodite's v1 engine with MambaBase interface support.
    """
    ...
    def _decode_infer(self, q, k, v, kv_cache, state_indices_tensor, attn_metadata):
        ...
        decode_state_indices = state_indices_tensor if decode_state_indices is None else decode_state_indices
