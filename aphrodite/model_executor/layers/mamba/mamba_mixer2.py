...
from aphrodite.model_executor.layers.mamba.ops.layernorm_gated import rms_norm_gated
from aphrodite.model_executor.layers.mamba.ops.selective_state_update_replayssm_output_only import (  # noqa: E501
    ...
    **selective** state spaces)
    ...
    def __init__(
        ...
        if n_groups % self.tp_size == 0:
            ...
            # (ModelWeightParameter which extends BaseAphroditeParameter)
            if isinstance(self.in_proj.weight, BaseAphroditeParameter):
                # For BaseAphroditeParameter subclasses (quantized layers like FP8)
        ...
        self.prefix = prefix
        self.use_replayssm = cache_config.use_replayssm if cache_config is not None else False
        self.replayssm_buffer_len = (
            cache_config.replayssm_buffer_len if cache_config is not None and cache_config.use_replayssm else None
        )
        self.mamba_config = aphrodite_config.mamba_config
        if self.use_replayssm and self.num_heads % self.tp_size != 0:
            raise ValueError("--use-replayssm requires tensor-parallel heads to divide evenly")
        ...
        self.num_spec = aphrodite_config.num_speculative_tokens
        if self.num_spec > 0:
    ...
    def forward(
        ...
        torch.ops.aphrodite.mamba_mixer2(
            projected_states,
            ssm_output,
    ...
    def conv_ssm_forward(
        ...
        if attn_metadata_raw is not None:
            ...
            assert layer_metadata is None or isinstance(layer_metadata, Mamba2AttentionMetadata)
        ...
        if has_prefill:
            ...
            if is_mamba_cache_all:
                ...
                for seq_idx in range(num_prefills):
                    ...
                    # already cached
                    num_unaligned_computed_tokens = num_computed_tokens_p_cpu[seq_idx] % mamba_block_size
        ...
        if has_decode:
            ...
            # NOTE: final output is an in-place update of out tensor
            preallocated_ssm_out_d = preallocated_ssm_out_d.view(num_decode_tokens, -1, self.head_dim)
            if self.use_replayssm:
                ...
                if self.mamba_config.backend == MambaBackendEnum.FLASHINFER:
                    ...
                    selective_state_update_replayssm_flashinfer(
                        ...
                        stochastic_rounding_philox_rounds=(self.mamba_config.stochastic_rounding_philox_rounds),
                    )
                else:
                    selective_state_update_replayssm_output_only(
                        ssm_state,
                        ...
                        enable_stochastic_rounding=(self.mamba_config.enable_stochastic_rounding),
                        cache_philox_rounds=(self.mamba_config.stochastic_rounding_philox_rounds),
                    )
            else:
                selective_state_update(
                    ssm_state,
    ...
    def get_state_dtype(self) -> tuple[torch.dtype, ...]:
        ...
        if self.use_replayssm:
            return MambaStateDtypeCalculator.append_replayssm_ring(base_dtype, self.model_config.dtype)
        return base_dtype
    ...
    for group_layer_names in groups_by_namespace.values():
        ...
        for layer_name in group_layer_names:
            ...
            if (state.shape[0], state.device) != (num_blocks, device):
                raise ValueError("ReplaySSM layers in one cache group must share cache capacity")
