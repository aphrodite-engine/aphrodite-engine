...
class ShortConv(MambaBase, PluggableLayer):
    ...
    def forward_native(
        ...
        if attn_metadata_raw is not None:
            ...
            assert attn_metadata is None or isinstance(attn_metadata, ShortConvAttentionMetadata)
        ...
        B_d, B_p = torch.split(B[:num_actual_tokens], [num_decodes, num_prefill_tokens], dim=0)
        C_d, C_p = torch.split(C[:num_actual_tokens], [num_decodes, num_prefill_tokens], dim=0)
        x_d, x_p = torch.split(x[:num_actual_tokens], [num_decodes, num_prefill_tokens], dim=0)
