...
from aphrodite.v1.attention.backends.hpc_attn import HpcAttnMetadata
...
def hpc_rope_norm_forward(
    ...
    assert kv_cache.dim() == 4, f"Expected kv_cache to have 4 dims, got {tuple(kv_cache.shape)}"
...
class HpcRopeNorm(CustomOp, HpcModule):
    ...
    - In compiled mode: forward_cuda() calls torch.ops.aphrodite.hpc_rope_norm_forward
    ...
    def __init__(
        ...
        if self.use_fp8:
            self._quant_type = hpc.QuantType.QPERTOKEN_PERHEAD_KPERTENSOR_VPERTENSOR.value
        else:
    ...
    def support(
        ...
        if kv_cache_dtype not in ("fp8_e4m3", "auto", "bfloat16"):
            logger.warning_once(
                f"hpc rope_norm not support kv_cache_dtype:{kv_cache_dtype}, only support fp8_e4m3, auto, bfloat16"
            )
            return False
    ...
    def forward_cuda(
        ...
        torch.ops.aphrodite.hpc_rope_norm_forward(qkv, output, layer_name)
        return output
    ...
    def _zero_pad_last_blocks(
        ...
        is_tail = (offsets.unsqueeze(0) >= remainder.unsqueeze(1)) & (seq_lens > 0).unsqueeze(1)
    ...
    def _forward_impl(
        ...
        if num_prefill_reqs > 0:
            ...
            if out_k is not None:
                self._zero_pad_last_blocks(key_cache, value_cache, seq_lens_prefill, block_table_prefill)
        ...
        if num_decode_reqs > 0:
            ...
            if out_k is not None:
                self._zero_pad_last_blocks(key_cache, value_cache, num_seq_kvcache, block_table_decode)
