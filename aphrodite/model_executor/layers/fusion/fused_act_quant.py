...
from aphrodite.model_executor.layers.fusion.quant_activation import QuantizedActivation
...
def _silu_and_mul_fp8_static(x: torch.Tensor, linear: LinearBase) -> QuantizedActivation:
...
def _silu_and_mul_fp8_dynamic_128(x: torch.Tensor, linear: LinearBase) -> QuantizedActivation:
...
def _silu_and_mul_nvfp4_dynamic(x: torch.Tensor, linear: LinearBase) -> QuantizedActivation:
    ...
    num_k_tiles = (d + 63) // 64
    block_scale = torch.empty((num_tokens, num_k_tiles * 4), dtype=FP8_DTYPE, device=x.device)

    input_global_scale = getattr(linear, "input_global_scale", None)
    assert input_global_scale is not None, "input_global_scale is required for NVFP4 quantization"
