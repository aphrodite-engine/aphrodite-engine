...
from aphrodite.model_executor.layers.utils import dispatch_unquantized_gemm
...
class VocabParallelEmbedding(PluggableLayer):
    ...
    def __init__(
        ...
        *,
        ...
        self.use_fused_embedding = (
            self.tp_size > 1 and current_platform.is_cuda() and isinstance(quant_method, UnquantizedEmbeddingMethod)
        )
    ...
    def weight_loader(self, param: Parameter, loaded_weight: torch.Tensor):
        ...
        if output_dim is None:
            if loaded_weight.ndim == 0 and param.data.ndim == 1 and param.data.numel() == 1:
        ...
        if packed_dim is not None and packed_dim == output_dim:
            packed_factor = param.packed_factor if isinstance(param, BaseAphroditeParameter) else param.pack_factor
    ...
    def forward(self, input_):
        ...
        if self.use_fused_embedding:
            ...
            if input_.ndim != 1:
                output_parallel = output_parallel.view(*input_.shape, self.embedding_dim)
        else:
