...
def fused_qk_rmsnorm_rope_gate(
    ...
    if q_gate.dtype not in (torch.float16, torch.bfloat16) or k.dtype != q_gate.dtype:
        raise ValueError(f"q_gate and k must have the same FP16 or BF16 dtype, got {q_gate.dtype} and {k.dtype}")
    for name, tensor in (
        ("q_gate", q_gate),
        ("k", k),
        ("q_weight", q_weight),
    ...
    if positions.shape[-1] != q_gate.shape[0]:
        raise ValueError(
            f"positions token dimension must match q_gate, got {positions.shape[-1]} and {q_gate.shape[0]}"
        )

    has_mrope = positions.ndim == 2
    if has_mrope:
        if positions.shape[0] != 3:
            raise ValueError(f"MRoPE positions must have shape (3, n_tokens), got {positions.shape}")
        ...
        if sum(mrope_section) != rotary_dim // 2:
            raise ValueError(
                f"mrope_section must sum to rotary_dim // 2, got {mrope_section} and rotary_dim={rotary_dim}"
            )
