...
def _load_nan_masked_weights(
    ...
    def copy_non_nan_(
        ...
        if not (destination.is_floating_point() and source.is_floating_point() and destination.shape == source.shape):
            raise NotImplementedError("Sparse checkpoint patches require a same-shaped floating-point final copy")
...
def _validate_patch_structure(patch: CheckpointWeightPatch) -> int:
    ...
    if patch.indices is None:
        if patch.values.numel() != numel:
            raise ValueError(
                f"{patch.name}: dense patch has {patch.values.numel()} values for a {numel}-element tensor"
            )
        return numel
    ...
    if patch.indices.numel() != patch.values.numel():
        raise ValueError(f"{patch.name}: sparse indices and values must have matching lengths")
...
def load_checkpoint_weight_patches(
    ...
    *,
    ...
    Online checkpoint-format dense updates require the caller to manage Aphrodite's
    ...
    if sparse:
        for patch, numel in zip(patch_list, patch_numels, strict=True):
            ...
            if indices.numel():
                ...
                patch_checks.append(torch.logical_or(indices < 0, indices >= numel).any())
                if validate_unique_indices:
                    ...
                    patch_checks.append((sorted_indices[1:] == sorted_indices[:-1]).any())
    ...
    for patch, numel in zip(patch_list, patch_numels, strict=True):
        ...
        checkpoint_tensor_bytes = numel * torch.empty((), dtype=patch.dtype).element_size()
        ...
        if weight_chunk and (chunk_bytes + checkpoint_tensor_bytes > max_chunk_bytes or patch.name in chunk_names):
