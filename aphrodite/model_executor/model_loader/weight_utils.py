...
from aphrodite.config.load import (
    DEFAULT_SAFETENSORS_PREFETCH_BLOCK_SIZE,
    DEFAULT_SAFETENSORS_PREFETCH_NUM_THREADS,
    LoadConfig,
)
from aphrodite.distributed import get_tensor_model_parallel_rank, get_world_group
...
from aphrodite.transformers_utils.repo_utils import hf_api, hf_fs
...
def get_quant_config(model_config: ModelConfig, load_config: LoadConfig) -> QuantizationConfig:
    ...
    from aphrodite.config.quantization import _ONLINE_SHORTHANDS, QuantizationConfigArgs
    ...
    def maybe_compose_online_quantization(
        ...
        checkpoint_config.online_quantization_config = OnlineQuantizationConfig(online_args)
        return checkpoint_config
    ...
    if hf_quant_config is not None:
        ...
        if model_config.quantization == "modelopt_mixed" and "quantized_layers" not in hf_quant_config:
            ...
            return maybe_compose_online_quantization(quant_cls.from_config(hf_quant_config))
    ...
    if quantization_config_file is not None:
        if hasattr(quant_cls, "from_config_file"):
            return maybe_compose_online_quantization(quant_cls.from_config_file(quantization_config_file))
        else:
    ...
    if quantization_config_json is not None:
        if hasattr(quant_cls, "from_config_dict_json"):
            return maybe_compose_online_quantization(quant_cls.from_config_dict_json(quantization_config_json))
        else:
            raise NotImplementedError(
    ...
    # Inflight BitsAndBytes quantization has no checkpoint quantization file.
...
def _get_available_ram_bytes() -> int:
    ...
    from aphrodite.utils.cpu_resource_utils import get_cgroup_memory_limit

    cgroup_limit, cgroup_usage = get_cgroup_memory_limit()
    if cgroup_limit is None:
        return host_available
    cgroup_available = cgroup_limit if cgroup_usage is None else max(0, cgroup_limit - cgroup_usage)
    ...
    if safetensors_load_strategy is None:
        if is_net_fs and fits_in_ram:
            ...
            logger.debug_once(
                ...
                "prefetching, start Aphrodite with --safetensors-load-strategy=prefetch.",
                fs_name,
            )
...
def fastsafetensors_weights_iterator(
    ...
    queue_size = envs.APHRODITE_FASTSAFETENSORS_QUEUE_SIZE
...
def instanttensor_weights_iterator(
    ...
    try:
        ...
        raise ImportError("Please install instanttensor via `pip install aphrodite-engine[instanttensor]`") from e
...
def maybe_remap_kv_scale_name(name: str, params_dict: dict) -> str | None:
    ...
    """
    # Already in Aphrodite's expected form (e.g. weights pre-renamed by a
