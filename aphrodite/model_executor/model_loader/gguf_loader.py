...
from aphrodite.transformers_utils.gguf_utils import detect_gguf_multimodal
from aphrodite.utils.torch_utils import set_default_torch_dtype

if TYPE_CHECKING:
    from aphrodite.model_executor.layers.quantization.gguf import GGUFConfig
...
class GGUFModelLoader(BaseModelLoader):
    ...
    def load_model(
        ...
        if TYPE_CHECKING:
            aphrodite_config.quant_config = cast(GGUFConfig, aphrodite_config.quant_config)
        aphrodite_config.quant_config.unquantized_modules.extend(unquant_names)

        target_device = torch.device(device_config.device)
        with set_default_torch_dtype(model_config.dtype):
            with target_device:
                model = initialize_model(aphrodite_config=aphrodite_config, prefix=prefix)
