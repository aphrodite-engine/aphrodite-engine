...
SOCKET_NAME_TEMPLATE = "aphrodite_weight_cache_gpu{gpu_id}.sock"
SOCKET_DIR_TEMPLATE = "aphrodite_weight_cache_{uid}"
...
def check_ipc_quant_support(model_config: ModelConfig, *, where: str) -> None:
    ...
    quant_config = getattr(getattr(model_config, "model_arch_config", None), "quantization_config", None)
    if quant_config is None:
        quant_config = getattr(model_config.hf_config, "quantization_config", None)
    if is_ipc_quant_supported(quantization, quant_config):
        return
    verified = ", ".join("unquantized" if name is None else repr(name) for name in IPC_QUANT_ALLOWLIST)
...
def get_socket_dir(socket_dir: str | None = None) -> str:
    ...
    if socket_dir is not None:
        return socket_dir
    return os.path.join(tempfile.gettempdir(), SOCKET_DIR_TEMPLATE.format(uid=_current_uid()))
...
def verify_private_dir(directory: str, strict_perms: bool = True) -> None:
    ...
    if stat.S_ISLNK(info.st_mode):
        raise WeightCacheUnavailableError(f"Refusing to use symlinked socket directory {directory}")
    if not stat.S_ISDIR(info.st_mode):
        raise WeightCacheUnavailableError(f"{directory} is not a directory")
    uid = _current_uid()
    if uid != -1 and info.st_uid != uid:
        raise WeightCacheUnavailableError(f"Socket directory {directory} is not owned by the current user")
    if strict_perms and info.st_mode & 0o077:
        raise WeightCacheUnavailableError(f"Socket directory {directory} is group/world accessible")
...
def verify_socket_owner(socket_path: str, strict_perms: bool = True) -> None:
    ...
    if stat.S_ISLNK(info.st_mode):
        raise WeightCacheUnavailableError(f"Refusing to connect to symlinked socket {socket_path}")
    uid = _current_uid()
    if uid != -1 and info.st_uid != uid:
        raise WeightCacheUnavailableError(f"Socket {socket_path} is not owned by the current user")
...
def hash_checkpoint(model: str) -> str | None:
    ...
    if os.path.isfile(os.path.join(model, SAFE_WEIGHTS_INDEX_NAME)):
        files = filter_duplicate_safetensors_files(files, model, SAFE_WEIGHTS_INDEX_NAME)
    if not files:
        return None
...
class WeightCacheKey:
    ...
    @classmethod
    def from_model_config(cls, model_config: ModelConfig, tp_size: int, tp_rank: int) -> "WeightCacheKey":
        ...
        return cls(
            ...
            aphrodite_version=aphrodite.version.__version__,
        )
