...
rank in GPU memory and serves CUDA IPC handles to Aphrodite engines over a Unix
...
Launch one daemon per TP rank with a single command:
    ...
    python -m aphrodite.model_executor.model_loader.weight_cache.daemon \\
        --model /path/to/model --tensor-parallel-size 4

Engines then load from the daemons with:

    aphrodite serve /path/to/model --tensor-parallel-size 4 \\
...
def _report_ready(message: str) -> None:
    ...
    streams and can leave them (and Aphrodite's logging handler, which caches the
...
class WeightCacheDaemon:
    ...
    def serve_forever(self, ready_callback: Callable[[], None] | None = None) -> None:
        ...
        ensure_private_socket_dir(os.path.dirname(socket_path), strict_perms=self.socket_dir is None)
        ...
        server.listen()
        logger.info("Weight cache daemon rank %d serving on %s", self.tp_rank, socket_path)
        print(f"Weight cache daemon rank {self.tp_rank} ready: serving on {socket_path}")
            ...
            while True:
                ...
                with conn:
                    try:
                        ...
                        logger.exception("Error handling weight cache client; continuing")
        finally:
            server.close()
            if os.path.exists(socket_path):
                os.unlink(socket_path)
    ...
    def _acquire_gpu_lock(self, socket_path: str) -> int:
        ...
        try:
            ...
            os.close(lock_fd)
            raise WeightCacheUnavailableError(f"Another weight cache daemon already owns {socket_path}") from e
        return lock_fd
...
def _run_daemon(
    ...
    daemon = WeightCacheDaemon(aphrodite_config, tp_rank, distributed_init_method, socket_dir)
    daemon.load_model()
...
def _reject_unsupported_parallelism(parallel_config: ParallelConfig) -> None:
    ...
    for name, enabled in unsupported.items():
        if enabled:
            raise ValueError(f"The weight cache daemon only supports tensor parallelism; {name} is not supported")


def main() -> None:
    from aphrodite.engine.arg_utils import EngineArgs
    from aphrodite.utils.argparse_utils import FlexibleArgumentParser
    from aphrodite.utils.network_utils import get_distributed_init_method, get_open_port

    parser = FlexibleArgumentParser(description="Launch weight cache daemons (one per TP rank).")
    ...
    if aphrodite_config.load_config.load_format == "ipc_cache":
        raise ValueError("The weight cache daemon itself must load from disk; use the default --load-format")
    ...
    check_ipc_quant_support(aphrodite_config.model_config, where="daemon")
    ...
    procs = [
        ctx.Process(
            ...
            name=f"aphrodite-weight-cache-daemon-{rank}",
        )
    ...
    while len(ready_ranks) < tp_size:
        try:
            ...
            if dead:
                logger.error(
                    "Weight cache daemon rank(s) exited during startup (exitcodes=%s); shutting down.",
