...
from aphrodite.model_executor.model_loader.utils import (
    initialize_model,
    process_weights_after_loading,
)
from aphrodite.model_executor.model_loader.weight_cache.protocol import (
...
from aphrodite.model_executor.utils import weights_already_processed
...
class IpcModelLoader(BaseModelLoader):
    ...
    def __init__(self, load_config: LoadConfig):
        ...
        self.connect_timeout_s: float = float(extra_config.pop("connect_timeout_s", _CONNECT_TIMEOUT_S))
        self.state_timeout_s: float = float(extra_config.pop("state_timeout_s", _STATE_TIMEOUT_S))
        if self.mode not in ("zero_copy", "copy"):
            raise ValueError(f"Invalid weight cache mode {self.mode!r}, expected 'zero_copy' or 'copy'")
        if extra_config:
            raise ValueError(
                f"Unexpected extra config keys for load format {load_config.load_format}: {sorted(extra_config)}"
            )
    ...
    def load_model(self, aphrodite_config: AphroditeConfig, model_config: ModelConfig, prefix: str = "") -> nn.Module:
        ...
        self._check_supported(aphrodite_config, model_config)
        state_fetched = False
        try:
            entries, aliases = self._fetch_entries(model_config)
            state_fetched = True
            return self._build_model(aphrodite_config, model_config, prefix, entries, aliases)
        except (WeightCacheUnavailableError, CacheConfigMismatchError) as e:
            if not self.fallback:
                raise
            logger.warning("Weight cache unusable (%s); falling back to disk loading", e)
        except Exception:
            if not self.fallback:
                raise
            logger.exception("Weight cache IPC loading failed; falling back to disk loading")
        ...
        return self._fallback_load(aphrodite_config, model_config, prefix)
    ...
    def _build_model(
        ...
        load_device = device_config.device if self.load_config.device is None else self.load_config.device
        target_device = torch.device(load_device)
        device_index = (
            target_device.index if target_device.index is not None else torch.accelerator.current_device_index()
        )
        with set_default_torch_dtype(model_config.dtype):
            ...
            _materialize_remaining_meta_tensors(model, torch.device(target_device.type, device_index))
        if self.mode == "copy":
            self._send_release()
        logger.info(
    ...
    @staticmethod
    def _check_supported(aphrodite_config: AphroditeConfig, model_config: ModelConfig) -> None:
    ...
    def _apply_entries(
        ...
        def _register(name: str, tensor: torch.Tensor, is_param: bool) -> None:
            ...
            if is_param:
                obj: torch.Tensor = (
                    tensor if isinstance(tensor, nn.Parameter) else nn.Parameter(tensor, requires_grad=False)
                )
    ...
    def _fetch_entries(self, model_config: ModelConfig) -> tuple[dict[str, TensorEntry], dict[str, str]]:
    ...
    def _request_state(self, cache_config: WeightCacheKey) -> tuple[dict[str, TensorEntry], dict[str, str]]:
        ...
        if status == "mismatch":
            raise CacheConfigMismatchError(f"WeightCacheKey mismatch on fields: {response.get('fields')}")
        if status != "ok":
            raise WeightCacheUnavailableError(f"Weight cache daemon error: {response.get('message')}")
    ...
    def _connect(self, timeout: float) -> socket.socket:
        ...
        try:
            ...
            raise WeightCacheUnavailableError(f"Weight cache socket {socket_path} is unavailable: {e}") from e
        ...
        try:
            ...
            sock.close()
            raise WeightCacheUnavailableError(f"Cannot connect to weight cache daemon at {socket_path}: {e}") from e
        return sock
    ...
    def _check_gpu_uuid(self, daemon_uuid: str | None) -> None:
        ...
        if daemon_uuid != local_uuid:
            raise CacheConfigMismatchError(
                f"Daemon GPU {daemon_uuid} != engine GPU {local_uuid}; check the socket path / GPU mapping"
            )
    ...
    def _fallback_load_config(self) -> LoadConfig:
        ...
        # extra config keys.
        return dataclasses.replace(self.load_config, load_format="auto", model_loader_extra_config={})

    def _fallback_load(self, aphrodite_config: AphroditeConfig, model_config: ModelConfig, prefix: str) -> nn.Module:
    ...
    for module_name, module in model.named_modules():
        for leaf, param in list(module._parameters.items()):
            if param is not None and param.device.type == "meta":
                ...
                module._parameters[leaf] = nn.Parameter(torch.empty_like(param, device=device), requires_grad=False)
