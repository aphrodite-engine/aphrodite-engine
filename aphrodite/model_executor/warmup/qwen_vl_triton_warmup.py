...
def _warm_mrope(runner: "GPUModelRunner", model: torch.nn.Module) -> None:
    ...
    from aphrodite.model_executor.models.interfaces import supports_mrope
    ...
    for num_tokens in _MROPE_TOKEN_COUNTS:
        positions = torch.arange(num_tokens, dtype=torch.long, device=runner.device).expand(3, -1)
        query = torch.empty(
