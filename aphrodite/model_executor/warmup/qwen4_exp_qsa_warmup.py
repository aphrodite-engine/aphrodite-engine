...
if TYPE_CHECKING:
    from aphrodite.v1.worker.gpu.model_runner import GPUModelRunner as GPUModelRunnerV2
...
def qwen4_exp_qsa_triton_warmup(worker: "Worker") -> None:
    ...
    qsa_module = sys.modules.get("aphrodite.models.qwen4_exp.nvidia.indexer_qsa")
    attn_module = sys.modules.get("aphrodite.models.qwen4_exp.nvidia.qsa")
    ...
    def block_table_for(prefix: str) -> torch.Tensor:
        group_id = next(
            i for i, group in enumerate(runner.kv_cache_config.kv_cache_groups) if prefix in group.layer_names
        )
        ...
        return runner.input_batch.block_table[group_id].get_device_tensor(runner.max_num_reqs)
    ...
    from aphrodite.models.qwen4_exp.nvidia.ops.qsa_indexer import (
