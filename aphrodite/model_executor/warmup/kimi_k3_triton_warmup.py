...
def _get_kda_layer(worker: Worker) -> KimiK3DeltaAttention | None:
    ...
    if not isinstance(static_context, dict):
        return None
    return next(
        (layer for layer in static_context.values() if isinstance(layer, KimiK3DeltaAttention)),
        None,
    )


def _warm_attn_res(worker: Worker) -> None:
    from aphrodite.models.kimi_k3.nvidia.ops.attn_res import (
        attn_res,
...
def _warm_recurrent_kda(
    ...
    from aphrodite.models.kimi_k3.nvidia.ops.third_party.kda.fused_recurrent import (
        fused_recurrent_kda,
    ...
    for num_sequences in get_fused_recurrent_kda_fwd_warmup_profiles(h):
        ...
        q, k, v = (tensor.view(1, num_tokens, h, d) for tensor in packed_qkv.split(h * d, dim=-1))
        fused_recurrent_kda(
            q=q,
            k=k,
            v=v,
