...
from aphrodite.model_executor.layers.quantization.online.mxfp8 import Mxfp8OnlineLinearMethod
...
def _is_deep_gemm_backed_kernel(fp8_linear: object) -> bool:
    ...
    if isinstance(fp8_linear, DeepGemmFp8BlockScaledMMKernel):
        return True
    return isinstance(getattr(fp8_linear, "fallback", None), DeepGemmFp8BlockScaledMMKernel)
...
def _get_fp8_gemm_nt_m_values(w: torch.Tensor, max_tokens: int) -> list[int]:
    ...
    # Use optimal M values only if APHRODITE_DEEP_GEMM_WARMUP is set to "relax".
    # Otherwise warmup all token sizes to avoid JIT compilation in hotpath
    if envs.APHRODITE_DEEP_GEMM_WARMUP == "relax":
...
def _deepgemm_fp8_gemm_nt_warmup(
    ...
    a1q_scales = torch.zeros((max_tokens, k // block_m), device=device, dtype=torch.float32)
...
def _get_grouped_gemm_params(
    ...
    for request_m in request_m_values:
        ...
        cases_by_shape[(M_sum, align_used)] = torch.repeat_interleave(expert_ids_block, align_used, dim=0)

    max_m = max(M_sum for M_sum, _ in cases_by_shape)
    warmup_cases = [
        (M_sum, align_used, expert_ids) for (M_sum, align_used), expert_ids in sorted(cases_by_shape.items())
    ]
...
def _deepgemm_grouped_fp8_gemm_nt_contiguous_warmup(
    ...
    MAX_M, block_m, warmup_cases = _get_grouped_gemm_params(w1, w2, num_topk, max_tokens)
    if not warmup_cases:
        return
    device = w1.device
    ...
    def _warmup(w: torch.Tensor, w_scale: torch.Tensor):
        ...
        a1q_scales = torch.zeros((MAX_M, k // block_m), device=device, dtype=torch.float32)
