...
class AphroditeTritonJitKernel(AphroditeJitKernel[CompileKeyT], Generic[CompileKeyT]):
    ...
    def _kernel_arg_names(self) -> tuple[str, ...]:
        ...
        raise TypeError(f"Cannot inspect kernel parameters for {type(self.kernel).__name__}")
    ...
    def launch(
        ...
        **kwargs: Any,
        ...
        if runtime_launcher is not None:
            regular_args = [kwargs.pop(name) for name in self._kernel_arg_names[:runtime_launcher_arg_count]]
...
def kernel_launcher(
    ...
    @wraps(call_fn)
    def wrapper(
        self: AphroditeTritonJitKernel[Any],
        *args: Any,
        **kwargs: Any,
    ) -> None:
        ...
        inputs = {name: value for name, value in bound.arguments.items() if name != "self"}
    ...
    if isinstance(node, ast.List | ast.Tuple):
        ...
        for elt in node.elts:
            if isinstance(elt, ast.Constant) and isinstance(elt.value, str | int):
                ...
                raise ValueError(f"Unsupported Triton specialization ref: {ast.dump(elt)}")
        return tuple(refs)
...
def trace_triton_kernel_specialization_args(
    ...
    do_not_specialize_args = _triton_do_not_specialize_args(kernel, function_def, arg_names)
    pointer_args = _pointer_arg_names(function_def, arg_names)

    return tuple(
        name
        for name in arg_names
        if name in constexpr_args or (name not in pointer_args and name not in do_not_specialize_args)
    )
