...
def spec_decode_rejection_warmup(worker: Worker) -> None:
    ...
    from aphrodite.v1.worker.gpu.spec_decode.rejection_sampler_utils import (
        rejection_sample,
    )

    model_config = worker.aphrodite_config.model_config
    ...
    logger.info(
        "Warming up spec-decode rejection sampler kernels (vocab=%d, num_spec=%d, dtype_pairs=%s, block_verify=%s).",
        vocab_size,
        num_spec,
    ...
    for tgt_dtype, draft_dtype in warmup_dtype_pairs:
        target_logits = torch.zeros((num_logits, vocab_size), dtype=tgt_dtype, device=device)
        draft_logits = torch.zeros((num_reqs, num_spec, vocab_size), dtype=draft_dtype, device=device)
        synthetic_rates = torch.full((num_spec,), 0.5, dtype=torch.float32, device=device) if use_synthetic else None
        try:
            rejection_sample(
            ...
            logger.warning("Skipping spec-decode rejection sampler warmup.", exc_info=True)
            return
