...
class SiglipVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.head is None:
            drops["head."] = None
        if drops:
            self.hf_to_aphrodite_mapper = self.hf_to_aphrodite_mapper | WeightsMapper(orig_to_new_prefix=drops)
    ...
    def maybe_layer_norm_and_apply_head(self, encoder_outputs: torch.Tensor) -> torch.Tensor:
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        def _filter(ws):
            for name, w in ws:
                ...
                if name.startswith("encoder.layers.") and (int(name.split(".")[2]) >= layer_count):
                    continue
                yield name, w
...
class SiglipEmbeddingModel(nn.Module, SupportsMultiModal, SupportsQuant):
    ...
    hf_to_aphrodite_mapper = WeightsMapper(orig_to_new_substr={".position_ids": None})
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        config: SiglipConfig = aphrodite_config.model_config.hf_config
    ...
    def get_text_features(
        ...
        text_features = self._flip_sequences_by_position_ids(text_features, position_ids)
    ...
    def _flip_sequences_by_position_ids(
        ...
        sequence_ids_cpu = torch.arange(len(lengths_cpu), dtype=boundary_mid_cpu.dtype).repeat_interleave(lengths_cpu)

        # Calculate flipped indices for all positions at once
        current_positions_cpu = torch.arange(len(features), dtype=boundary_mid_cpu.dtype)
        flip_indices_cpu = (starts_cpu[sequence_ids_cpu] + ends_cpu[sequence_ids_cpu]) - (1 + current_positions_cpu)
    ...
    def embed_input_ids(
        ...
        *,
        ...
        has_mm_embeddings = multimodal_embeddings is not None and len(multimodal_embeddings) > 0
        self._mm_token_mask = is_multimodal
        self._has_text_tokens = dual_encoder_has_text_tokens(has_mm_embeddings, is_multimodal)
    ...
    def forward(
        ...
        **kwargs: object,
        ...
        return merge_dual_encoder_text_and_vision(text_features, vision_embeds, self._mm_token_mask)
