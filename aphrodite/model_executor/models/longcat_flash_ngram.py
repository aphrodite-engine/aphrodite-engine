...
class NgramEmbedding(nn.Module):
    ...
    def _init_ngram_embeddings(self) -> None:
        ...
        self.oe_embedder = VocabParallelEmbedding(offsets[-1], oe_dim, params_dtype=self._dtype)
        # Stacked projections: oe_projection[i] = post_projs[i].weight.T
        self.oe_projection = nn.Parameter(
            torch.empty(self.num_embedders, oe_dim, self.config.hidden_size, dtype=self._dtype),
            requires_grad=False,
        )
    ...
    def load_weight(self, weight_name: str, loaded_weight: torch.Tensor) -> str:
        ...
        if "ngram_embeddings.embedders." in weight_name:
            index = int(weight_name.split("ngram_embeddings.embedders.")[1].split(".")[0])
            lo, hi = self._offsets[index], self._offsets[index + 1]
            assert hi - lo == loaded_weight.shape[0], f"{hi - lo=} {loaded_weight.shape[0]=}"
            ...
            if load_start < load_end:
                self.oe_embedder.weight.data[load_start - tp_start : load_end - tp_start] = loaded_weight[
                    load_start - lo : load_end - lo
                ]
            return "ngram_embeddings.oe_embedder.weight"
        elif "ngram_embeddings.post_projs." in weight_name:
            index = int(weight_name.split("ngram_embeddings.post_projs.")[1].split(".")[0])
...
class FlashNgramModel(FlashModel):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        hf = aphrodite_config.model_config.hf_config
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, w in weights:
            if self.ngram_embeddings is not None and (
                "ngram_embeddings.embedders." in name or "ngram_embeddings.post_projs." in name
            ):
...
class LongcatFlashNgramForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    hf_to_aphrodite_mapper = WeightsMapper(orig_to_new_prefix={"model.mtp.": None})
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        if not aphrodite_config.use_v2_model_runner:
            raise NotImplementedError(
                ...
                "automatically unless APHRODITE_USE_V2_MODEL_RUNNER=0 is set."
            )
        config = FlashConfig(**aphrodite_config.model_config.hf_config.__dict__)
        config.intermediate_size = getattr(config, "ffn_hidden_size", config.intermediate_size)
        self.config = config
        self.quant_config = aphrodite_config.quant_config

        self.model = FlashNgramModel(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, "model"))
...
class LongcatNgramModelState(DefaultModelState):
    ...
    def __init__(self, aphrodite_config, model, encoder_cache, device) -> None:
        ...
        self.token_context = torch.full((self.max_num_reqs, self.ctx_len), -1, dtype=torch.int32, device=device)
    ...
    def add_request(self, req_index: int, new_req_data: NewRequestData) -> None:
        ...
        self.token_context[req_index] = torch.tensor(ctx, dtype=torch.int32, device=self.token_context.device)
    ...
    def _compute_oe_ids(self, input_batch: InputBatch) -> torch.Tensor:
        ...
        tok_req = torch.repeat_interleave(torch.arange(num_reqs, device=device), req_lens.long())
        col = ctx_len + (torch.arange(num_tokens, device=device) - qsl[:-1].long()[tok_req])
        table[tok_req, col] = cur_neg

        column_starts = torch.full((num_reqs,), ctx_len, dtype=torch.int32, device=device)
        row_indices = torch.arange(num_reqs, dtype=torch.int64, device=device)
        n_gram_ids = torch.empty(num_tokens, self.ngram.num_embedders, dtype=torch.int32, device=device)
        ...
        gather = req_lens.long().unsqueeze(1) + torch.arange(ctx_len, device=device).unsqueeze(0)
