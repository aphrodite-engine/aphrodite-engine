...
"""Aphrodite implementation for BailingMoeV3ForCausalLM.
...
Aphrodite's parallel linear layers, MLA kernel, KDA kernel and fused MoE loader.
"""
...
from aphrodite.model_executor.layers.activation import SiluAndMul, SwigluStepAndMul
...
from aphrodite.model_executor.layers.mamba.ops.causal_conv1d import (
    causal_conv1d_fn,
...
def _is_kda_layer(layer_idx: int, layer_group_size: int, num_hidden_layers: int) -> bool:
    return not (
        (layer_idx + 1) % layer_group_size == 0 or layer_idx >= num_hidden_layers // layer_group_size * layer_group_size
    )
...
def _configure_ling_fp8_quant_config(
    ...
    if routed_quant_method != "mxfp4":
        raise ValueError(f"Unsupported routed experts quantization: {routed_quant_method!r}")
...
def _is_fp8_module_excluded(
    ...
    """Match Ling's abbreviated FP8 exclusions against mapped Aphrodite prefixes."""
    if not _is_block_fp8_config(quant_config):
        return False
...
def _pad_block_fp8_mlp_checkpoint_tensor(
    ...
    if ".down_proj." in name:
        ...
        logical_shards = 1
    elif any(projection in name for projection in (".gate_proj.", ".up_proj.", ".gate_up_proj.")):
        dim = 0
    ...
    if current_size % logical_shards != 0:
        raise ValueError(f"Cannot split {name} dimension {current_size} into {logical_shards} logical shards.")
...
def _maybe_pad_block_fp8_shared_expert_checkpoint_tensor(
    ...
    shared_intermediate = config.moe_shared_expert_intermediate_size * config.num_shared_experts
...
class BailingMoeV3MLP(nn.Module):
    def __init__(
        ...
        self.act_fn = SwigluStepAndMul(limit=swiglu_limit) if swiglu_limit else SiluAndMul()
...
class BailingMoeV3MLAAttention(nn.Module):
    def __init__(
        ...
        self.gated_attention_proj_granularity_type = getattr(config, "gated_attention_proj_granularity_type", None)
    ...
    def forward(self, hidden_states: torch.Tensor, positions: torch.Tensor):
        ...
        if self.g_proj is not None:
            gate = torch.sigmoid(self.g_proj(hidden_states)[0].float()).to(hidden_states.dtype)
            if self.gated_attention_proj_granularity_type == "head_wise":
...
class BailingMoeV3KimiDeltaAttention(PluggableLayer, MambaBase):
    ...
    def __init__(
        ...
        self.dt_bias = nn.Parameter(torch.empty(projection_size // self.tp_size, dtype=torch.float32))
        ...
        splitting_op = "aphrodite::bailing_v3_kda_attention"
        if compilation_config.splitting_ops is not None and splitting_op not in compilation_config.splitting_ops:
    ...
    def forward(
        ...
        torch.ops.aphrodite.bailing_v3_kda_attention(q, k, v, g1, beta, core_attn_out, self.prefix)
    ...
    def _forward(
        ...
        spec_only = (
            spec_sequence_masks is not None and attn_metadata.num_prefills == 0 and attn_metadata.num_decodes == 0
        )
...
class BailingMoeV3Gate(nn.Module):
    def __init__(
        ...
        if params_dtype is None:
            params_dtype = torch.float32
        self.weight = nn.Parameter(torch.empty((config.num_experts, config.hidden_size), dtype=params_dtype))
        self.expert_bias = nn.Parameter(torch.empty((config.num_experts,), dtype=torch.float32))

    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return F.linear(hidden_states.to(self.weight.dtype), self.weight).to(hidden_states.dtype)
...
class BailingMoeV3MoE(nn.Module):
    def __init__(
        ...
        self.expert_swiglu_limit = _get_layer_swiglu_limit(getattr(config, "expert_swiglu_limit_list", None), layer_id)
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = self.experts(hidden_states=hidden_states, router_logits=router_logits)
...
class BailingMoeV3DecoderLayer(nn.Module):
    def __init__(
        ...
        self.layer_id = layer_id
        self.hidden_size = config.hidden_size
        self.is_kda_layer = _is_kda_layer(layer_id, config.layer_group_size, config.num_hidden_layers)
...
class BailingMoeV3Model(nn.Module):
    ...
    def forward(
        ...
        if get_pp_group().is_first_rank:
            hidden_states = self.word_embeddings(input_ids) if inputs_embeds is None else inputs_embeds
            residual = None
        else:
...
class BailingMoeV3ForCausalLM(nn.Module, HasInnerState, IsHybrid, SupportsPP):
    hf_to_aphrodite_mapper = WeightsMapper(orig_to_new_substr={".attention.": ".self_attn."})
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        self.model = BailingMoeV3Model(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, "model"))
        if get_pp_group().is_last_rank:
    ...
    def get_mamba_state_shape_from_config(
        ...
        return _get_kda_state_shape_for_config(aphrodite_config)

    @classmethod
    def get_mamba_state_dtype_from_config(cls, aphrodite_config: AphroditeConfig) -> tuple[torch.dtype, torch.dtype]:
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        def load_param(name: str, tensor: torch.Tensor, shard_id=None) -> bool:
            ...
            weight_loader: Callable[..., None] = getattr(param, "weight_loader", default_weight_loader)
            if shard_id is None:
