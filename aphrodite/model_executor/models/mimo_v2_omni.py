...
class MiMoVisionAttention(nn.Module):
    ...
    def _forward_window_attn(
        ...
        sinks = self.sinks[head_start : head_start + self.num_heads_per_partition] if self.sinks is not None else None
        context_attention_fwd(
            q,
            k,
            v,
...
class MiMoV2OmniMultiModalProcessor(BaseMultiModalProcessor[MiMoV2OmniProcessingInfo]):
    ...
    def _get_mm_fields_config(
        ...
        if "video_audio_n_segs" in hf_inputs:
            fields["video_audio_n_segs"] = MultiModalFieldConfig.batched("video", keep_on_cpu=True)
        # video_audio_seg_lens: list of per-video 1D tensors, batched("video")
        if "video_audio_seg_lens" in hf_inputs:
            fields["video_audio_seg_lens"] = MultiModalFieldConfig.batched("video", keep_on_cpu=True)
...
class MiMoV2OmniForCausalLM(nn.Module, SupportsMultiModal, SupportsPP, SupportsQuant):
    ...
    def _parse_and_validate_audio_input(self, **kwargs: object) -> MiMoV2OmniAudioInputs | None:
        ...
        assert audio_token_lens is None or isinstance(audio_token_lens, (torch.Tensor, list))
        return {
            "type": "audio",
    ...
    def _parse_and_validate_multimodal_inputs(self, **kwargs: object) -> MiMoV2OmniMultiModalInputs:
    ...
    def _process_audio_input(self, audio_input: MiMoV2OmniAudioInputs) -> tuple[torch.Tensor, ...]:
    ...
    def embed_multimodal(self, **kwargs: object) -> MultiModalEmbeddings:
        ...
        assert video_audio_n_segs is None or isinstance(video_audio_n_segs, torch.Tensor)
        assert video_audio_seg_lens is None or isinstance(video_audio_seg_lens, torch.Tensor)
