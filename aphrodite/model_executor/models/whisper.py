...
from aphrodite.model_executor.models.whisper_utils import (
    ...
    def forward(
        ...
        if cu_seqlens is None:
            ...
            assert self.attn_type == AttentionType.ENCODER, (
                "Variable-length attention metadata is only supported for encoder self-attention."
            )
    ...
    def __init__(
        ...
        *,
        ...
        def create_layer(prefix: str) -> nn.Module:
            return WhisperEncoderLayer(aphrodite_config=aphrodite_config, prefix=f"{prefix}.layers")
        ...
        if enable_pp:
            ...
            self.layers = nn.ModuleList(
                get_offloader().wrap_modules(
                    create_layer(prefix=f"{prefix}.layers.{idx}") for idx in range(config.encoder_layers)
                )
            )
