...
from aphrodite.transformers_utils.configs.kimi_k25 import KimiK25VisionConfig
...
class MoonViT3dEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.final_layernorm = _make_vision_norm(block_cfg.get("norm_type", "layernorm"), hidden_dim)
...
class MoonViT3dPretrainedModel(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = MoonViT3dEncoder(
            ...
            block_cfg={
                ...
                "activation": get_act_fn(getattr(config, "activation_func", "gelu_pytorch_tanh")),
    ...
    def prepare_encoder_cudagraph_metadata(
        ...
        *,
        ...
        metadata["pos_embeds"] = self.patch_embed.pos_emb.get_pos_embeds(grid_thw_list).to(device=device)
        merge_gather_idx = build_image_merge_gather_idx(grid_thw_list, self.merge_kernel_size)
        metadata["merge_gather_idx"] = torch.from_numpy(merge_gather_idx).to(device=device, non_blocking=True)
        return metadata
...
def vision_tower_forward(
    ...
    Uses Aphrodite's standard data parallelism utility to shard the batch
