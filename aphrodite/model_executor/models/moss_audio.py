...
from aphrodite.model_executor.layers.activation import _ACTIVATION_REGISTRY, SiluAndMul
...
def _ensure_moss_audio_placeholder_tokens(tokenizer: object) -> None:
    ...
    missing_tokens = [
        token for token in MOSS_AUDIO_PLACEHOLDER_TOKENS if convert_tokens_to_ids(token) in (None, unk_token_id)
    ]
    if missing_tokens:
