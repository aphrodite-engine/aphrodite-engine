...
"""
Inference-only LLaVA-OneVision-2 (OV2) model for Aphrodite.
...
from aphrodite.compilation.decorators import (
...
from aphrodite.inputs import ModalityData, MultiModalDataDict
...
from aphrodite.model_executor.models.vision import get_vit_attn_backend
...
from aphrodite.multimodal.processing.dummy_inputs import BaseDummyInputsBuilder
...
def _load_ov2_processor(
    ...
    **kwargs: Any,
    ...
    image_processor = Qwen2VLImageProcessor.from_pretrained(path, revision=revision, **kwargs)
    tokenizer = AutoTokenizer.from_pretrained(path, revision=revision, trust_remote_code=trust_remote_code, **kwargs)
...
def _validate_video_source(path: str, model_config) -> str:
    ...
    The codec backend keeps the raw path string alive past Aphrodite's
    ...
    if scheme in ("http", "https", "data"):
        raise ValueError(
            ...
            f"its trust-remote-code decoder fetches them outside Aphrodite's domain "
            f"and redirect controls. Use a local file path, or the frame "
            f"backend for remote videos."
        )
    if scheme not in ("", "file"):
        raise ValueError(
            f"Unsupported codec video URL scheme {scheme!r}; only local file paths or file:// URLs are supported."
        )
    ...
    if not allowed_local:
        raise ValueError(
            "Local video file access is disabled. Set --allowed-local-media-path to enable reading local videos."
        )
    if scheme == "file":
    ...
    if not local.is_absolute():
        raise ValueError(f"Local video path {str(path)!r} must be absolute; relative paths are not supported.")
    allowed_root = Path(allowed_local).resolve()
    resolved = local.resolve()
    if resolved != allowed_root and allowed_root not in resolved.parents:
        raise ValueError(f"Video path {str(path)!r} is outside the allowed local media directory {allowed_local!r}.")
    return str(resolved)
...
#
# * frame backend: a normal Aphrodite VIDEO_LOADER_REGISTRY loader
...
def prepare_codec_video_input(video_path: str) -> tuple:
    """Wrap a video path for Aphrodite's MultiModalDataParser + OV2 codec backend.
...
def _extract_codec_video_paths(videos: Any) -> list[str] | None:
    # Aphrodite's parser yields list-of-(ndarray, metadata-dict) for tuple inputs.
    # We accept either that shape or a single raw tuple (pre-parser cases).
    def _path_from(item):
        if isinstance(item, tuple) and len(item) == 2 and isinstance(item[1], dict) and _CODEC_VIDEO_MARKER in item[1]:
...
def _codec_fps_for(video_path: str, hf_processor) -> float:
    ...
    codec_mod = importlib.import_module(
        f"{pkg}.codec_video_processing_llava_onevision2" if pkg else "codec_video_processing_llava_onevision2"
    )
...
def _create_field_factory(
    ...
    def _field_config(hf_inputs: Mapping[str, torch.Tensor]):
        ...
        # OV2 emits one grid_thw row per frame, so Aphrodite's per-video sharding
        ...
        if video_num_frames.numel() > 0:
            ...
            offsets = torch.cumsum(torch.cat([torch.zeros(1, dtype=torch.long), video_num_frames[:-1]]), 0).tolist()
        ...
        return dict(
            ...
            patch_positions=MultiModalFieldConfig.flat_from_sizes("image", image_pixel_grid_sizes),
            pixel_values_videos=MultiModalFieldConfig.flat_from_sizes("video", video_patch_sizes),
            video_grid_thw=MultiModalFieldConfig.flat_from_sizes("video", video_num_frames),
            patch_positions_videos=MultiModalFieldConfig.flat_from_sizes("video", video_patch_sizes),
...
# ``aphrodite_hf_chat`` adapter, and empirically scores higher on Video-MME than
...
# Token sequence emitted by the OV2 Aphrodite dummy inputs builder for each video
...
def _frame_video_to_pil_and_timestamps(
    ...
    if len(indices) != len(pil_frames):
        if len(indices) > len(pil_frames):
            ...
            indices = list(indices) + [indices[-1] if indices else 0] * (len(pil_frames) - len(indices))
...
def _expand_video_markers_in_prompt(
    ...
    *,
    ...
    blocks -- one per frame -- matching ``aphrodite_hf_chat._build_prompt``.
    ...
    for m in re.finditer(pattern, prompt):
        ...
        if idx >= len(per_video_timestamps):
            raise ValueError(
                f"Prompt has more video markers than supplied timestamp groups ({len(per_video_timestamps)})"
            )
        timestamps = per_video_timestamps[idx]
        expanded = "".join(f"<{t:.{timestamp_decimals}f} seconds>{_IMAGE_MARKER}" for t in timestamps)
    ...
    if idx != len(per_video_timestamps):
        raise ValueError(
            f"Prompt has {idx} video markers but {len(per_video_timestamps)} timestamp groups were supplied"
        )
    return "".join(parts)
...
# ---------------------------------------------------------------------------
# OV2 frame-sampling policy, expressed as a Aphrodite ``VideoBackend`` so videos
# entering through the standard ``video_url`` -> MediaConnector -> VideoMediaIO
# path are decoded + sampled inside Aphrodite (no qwen_vl_utils dependency, and the
...
# whereas Aphrodite uses OpenCV/PyAV, whose ``CAP_PROP_FRAME_COUNT`` (a duration x
...
class LlavaOnevision2VideoBackend(VideoBackend):
    ...
    def compute_frames_index_to_sample(
        ...
        **kwargs,
        ...
        target_max_frames = int(target.num_frames) if target.num_frames > 0 else cls._MAX_FRAMES
...
LlavaOnevision2ImageInputs = LlavaOnevision2ImagePixelInputs | LlavaOnevision2ImageEmbeddingInputs
...
class LlavaOnevision2VisionRotaryEmbedding(nn.Module):
    ...
    def __init__(self, head_dim: int, theta: float = 10000.0) -> None:
        ...
        self.register_buffer(
            "inv_freq_t",
            1.0 / (self.base ** (torch.arange(self.t_size, dtype=torch.float32) / self.t_size)),
            persistent=False,
        )
        self.register_buffer(
            "inv_freq_h",
            1.0 / (self.base ** (torch.arange(self.h_size, dtype=torch.float32) / self.h_size)),
            persistent=False,
        )
        self.register_buffer(
            "inv_freq_w",
            1.0 / (self.base ** (torch.arange(self.w_size, dtype=torch.float32) / self.w_size)),
            persistent=False,
        )
...
class LlavaOnevision2VisionEmbeddings(nn.Module):
    def __init__(self, patch_size: int = 14, in_channels: int = 3, embed_dim: int = 1024) -> None:
        super().__init__()
...
class LlavaOnevision2VisionAttn(nn.Module):
    ...
    def _apply_rotary_pos_embed(self, t: torch.Tensor, freqs: torch.Tensor) -> torch.Tensor:
...
class LlavaOnevision2VisionTower(nn.Module):
    ...
    def __init__(
        ...
        self.rotary_pos_emb = LlavaOnevision2VisionRotaryEmbedding(self.head_dim, theta=rope_theta)
        ...
        self.attn_backend = get_vit_attn_backend(head_size=self.head_dim, dtype=torch.get_default_dtype())
    ...
    def forward(
        ...
        rotary_pos_emb = self.rotary_pos_emb.forward_from_positions(patch_positions.to(self.device))
...
class LlavaOnevision2ProcessingInfo(BaseProcessingInfo):
        ...
        *,
        ...
        if do_resize:
            ...
            if min_pixels is None or max_pixels is None:
                ...
                min_pixels = getattr(size, "shortest_edge", None) or size["shortest_edge"]
    ...
    def get_image_size_with_most_features(self) -> ImageSize:
        sz, _ = self._get_vision_info(image_width=1800, image_height=1800, image_processor=None)
        return sz

    def get_max_image_tokens(self) -> int:
        w, h = self.get_image_size_with_most_features()
        return self.get_num_image_tokens(image_width=w, image_height=h, image_processor=None)
...
class LlavaOnevision2DummyInputsBuilder(BaseDummyInputsBuilder[LlavaOnevision2ProcessingInfo]):
    ...
    def get_dummy_mm_data(
        ...
        if n_vid:
            ...
            out["video"] = self._get_dummy_videos(width=w, height=h, num_frames=4, num_videos=n_vid)
        return out
...
class LlavaOnevision2MultiModalProcessor(BaseMultiModalProcessor[LlavaOnevision2ProcessingInfo]):
    def _get_data_parser(self) -> MultiModalDataParser:
        # Retained for symmetry; Aphrodite actually fetches the parser via
        # info.get_data_parser() (see ProcessingInfo override above).
        return LlavaOnevision2MultiModalDataParser(self.info.get_hf_config().vision_config.spatial_merge_size)
    ...
    def _call_hf_processor(
        ...
        # ProcessorMixin ``_merge_kwargs`` machinery, so Aphrodite's default path
        # fails; overriding this method routes the base class to call us
        # directly.
        hf_processor = self.info.get_hf_processor(**mm_kwargs)
        merged_kwargs = self.info.ctx.get_merged_mm_kwargs(dict(**mm_kwargs, **tok_kwargs))
        ...
        codec_video_paths = _extract_codec_video_paths(mm_data["videos"]) if videos_present else None
        ...
        # lmms-eval's chat/aphrodite.py ov2_path_video=True). Recover the path
        # strings directly from mm_data["videos"] so the codec rename
        # branch still fires and video-modality fields get populated.
        is_codec_kwarg = not is_codec_marker and videos_present and call_kwargs.get("video_backend") == "codec"
        if is_codec_kwarg:
            raw = mm_data["videos"]
        ...
        if is_codec:
            ...
            codec_video_paths = _validate_video_sources(codec_video_paths, self.info.ctx.model_config)
            ...
            mm_data["videos"] = codec_video_paths if len(codec_video_paths) > 1 else codec_video_paths[0]
            ...
            data = dict(output)
            return BatchFeature(self._rename_codec_outputs_to_video(data, codec_video_paths, hf_processor))
        ...
        # as video-series so Aphrodite's ``<|video_pad|>`` placeholder replacement
        ...
        if videos_present:
            timestamp_decimals = int(mm_kwargs.get("timestamp_decimals", _DEFAULT_TIMESTAMP_DECIMALS))
            ...
            marker_pattern = re.compile(
                "(?P<image>" + re.escape(_IMAGE_MARKER) + ")|(?P<video>" + re.escape(_VIDEO_MARKER) + ")"
            )
            ...
            if grid is not None and has_caller_images:
                ...
                data["patch_positions_videos"] = _gather_rows(patch_positions, video_rows)
            ...
            data["video_num_frames"] = torch.tensor([len(ts) for ts in per_video_timestamps], dtype=torch.long)
            data["video_is_codec"] = torch.zeros((len(per_video_timestamps),), dtype=torch.long)
    ...
    def _rename_codec_outputs_to_video(
        ...
        # Codec branch emits image-series keys; Aphrodite expects video-series.
        ...
        for idx, video_path in enumerate(codec_video_paths):
            num_canvases = self._count_canvases_for_video(patch_positions, video_grid_thw, 0, grid_offset)
        ...
        data["video_num_frames"] = torch.tensor(per_video_canvas_counts, dtype=torch.long)
        data["video_is_codec"] = torch.ones((len(per_video_canvas_counts),), dtype=torch.long)
        # Stored as scaled int64 (fps * 1e9 rounded) because Aphrodite casts
        ...
        data["codec_fps"] = torch.tensor([int(round(f * 1_000_000_000)) for f in per_video_fps], dtype=torch.int64)
    ...
    def _get_prompt_updates(
        ...
        def get_video_replacement(item_idx: int):
            ...
            is_codec = bool(int(is_codec_field.data.item())) if is_codec_field is not None else False
            tokens: list[int] = []
            ...
            if is_codec:
                ...
                runs = _codec_timestamp_runs(patch_positions, fps, image_processor.merge_size)
...
class LlavaOnevision2ForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    """Aphrodite-side OV2 top-level model.

    Weight name rewriting (HF checkpoint → Aphrodite module tree):
    ...
    def _validate_and_reshape_mm_tensor(self, mm_input: object, name: str) -> torch.Tensor:
        ...
        if isinstance(mm_input, torch.Tensor):
            if mm_input.ndim == 2:
                return mm_input
            if mm_input.ndim != 3:
                raise ValueError(f"{name} must be 2D or batched-3D, got shape={mm_input.shape}")
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> LlavaOnevision2ImageInputs | None:
        ...
        if pixel_values is not None:
            ...
            patch_positions = self._validate_and_reshape_mm_tensor(patch_positions, "image patch_positions")
    ...
    def _process_image_input(self, image_input: LlavaOnevision2ImageInputs) -> tuple[torch.Tensor, ...]:
    ...
    def _parse_and_validate_video_input(self, **kwargs: object) -> LlavaOnevision2VideoPixelInputs | None:
        ...
        if patch_positions_videos is None:
            raise ValueError("OV2 requires patch_positions_videos alongside pixel_values_videos.")
        patch_positions_videos = self._validate_and_reshape_mm_tensor(patch_positions_videos, "video patch_positions")
        if video_num_frames is None:
            raise ValueError("OV2 requires video_num_frames alongside pixel_values_videos.")
