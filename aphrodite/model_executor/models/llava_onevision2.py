...
def _create_field_factory(
    ...
    def _field_config(hf_inputs: Mapping[str, torch.Tensor]):
        ...
        return dict(
            ...
            video_grid_thw=MultiModalFieldConfig.flat_from_sizes("video", video_num_frames, keep_on_cpu=True),
    ...
    def _apply_hf_processor_main(
        ...
        if is_codec:
            ...
            output = self.info.ctx.call_hf_processor(
                self.info.get_hf_processor(**{**hf_processor_mm_kwargs, "video_backend": "codec"}),
                dict(text=prompt_text, **mm_data),
                {**hf_processor_mm_kwargs, "video_backend": "codec"},
            )
            data = dict(output)
            processed_data = BatchFeature(self._rename_codec_outputs_to_video(data, codec_video_paths, hf_processor))
        ...
        if videos_present:
            timestamp_decimals = hf_processor_mm_kwargs.get("timestamp_decimals", _DEFAULT_TIMESTAMP_DECIMALS)
            if not isinstance(timestamp_decimals, (str, Buffer, SupportsInt, SupportsIndex)):
                raise TypeError(
...
class LlavaOnevision2ForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def _parse_and_validate_video_input(self, **kwargs: object) -> LlavaOnevision2VideoPixelInputs | None:
        ...
        if isinstance(video_num_frames, list):
            ...
            raise TypeError(f"video_num_frames must be a tensor or a list of tensors, got {type(video_num_frames)}")
    ...
    def _parse_and_validate_multimodal_inputs(self, **kwargs: object) -> LlavaOnevision2InputsByModality:
