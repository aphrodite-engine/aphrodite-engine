...
def _rename_and_normalize_weights(
    ...
    """
    Translate HuggingFace Param2MoE weight names to Aphrodite internal names
    and zero-mean the expert-bias tensor so the router stays balanced.

    Mapping table (HF → Aphrodite):
    ...
    for name, w in weights:
        ...
        # Fused QKV projection  (HF: query_key_value → Aphrodite: qkv_proj)
        name = name.replace(".attention.query_key_value.", ".self_attn.qkv_proj.")
        # Output projection  (HF: dense → Aphrodite: o_proj)
...
class Param2MoEAttention(nn.Module):
    ...
    Notable differences from a vanilla GQA layer:
      * The checkpoint fuses Q, K, V into a single ``query_key_value`` weight.
        Aphrodite receives it already renamed to ``qkv_proj`` by the weight-name
    ...
    def __init__(
        ...
        if config.rope_scaling is not None:
            ...
            # Normalise key: some checkpoints use "type", Aphrodite wants "rope_type"
...
class Param2MoEModel(nn.Module):
    ...
    def load_weights(
        ...
        stacked_params_mapping = [
            # (aphrodite_param_name, ckpt_weight_name, shard_id)
