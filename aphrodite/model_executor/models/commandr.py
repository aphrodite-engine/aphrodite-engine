...
from aphrodite.model_executor.model_loader.weight_utils import row_parallel_weight_loader
...
class CohereAttention(nn.Module):
    def __init__(
        ...
        # tokens.  Aphrodite's FlashAttention backend does (value - 1, 0), so we pass
        ...
        if self.use_qk_norm:
            self.q_norm = norm_cls(param_shape=(self.num_heads, self.head_dim), eps=norm_eps)
            self.k_norm = norm_cls(param_shape=(self.num_kv_heads, self.head_dim), eps=norm_eps)
...
class CohereDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.tp_size > 1:
            ...
            parallel_block_output = tensor_model_parallel_all_reduce(parallel_block_output)
