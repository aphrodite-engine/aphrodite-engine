...
from aphrodite.model_executor.layers.activation import QuickGELU
...
class Ernie4_5_VisionAttention(nn.Module):
    """VisionAttention using APHRODITE framework APIs"""
...
class Ernie4_5_VisionRotaryEmbedding(nn.Module):
    def __init__(self, dim: int, theta: float = 10000.0) -> None:
        ...
        self.register_buffer(
            "inv_freq",
            1.0 / theta ** (torch.arange(start=0, end=dim, step=2, dtype=torch.float32) / dim),
            persistent=False,
        )
        ...
        if max_batch_size is not None:
            ...
            if num_seqs < max_batch_size:
                cu_seqlens = torch.cat(
                    [
                        cu_seqlens,
                        cu_seqlens.new_full((max_batch_size - num_seqs,), cu_seqlens[-1]),
                    ]
                )
    ...
    def forward(
        ...
        *,
        ...
        if encoder_metadata is None:
            ...
            encoder_metadata = self.prepare_encoder_metadata(grid_thw, num_pad=num_pad, device=hidden_states.device)
...
class VariableResolutionResamplerModel(nn.Module):
    ...
    def forward(self, x, grid_thw):
        ...
        def fwd_placeholder(x, grid_thw, to_tensor=False):
            ...
            slice_offsets = async_tensor_h2d(np.concatenate(slice_offsets, axis=-1), device=x.device)
            ...
            slice_offsets2 = async_tensor_h2d(np.concatenate(slice_offsets2, axis=-1), device=x.device)
...
class Ernie4_5VLMultiModalProcessor(BaseMultiModalProcessor[Ernie4_5_VLProcessingInfo]):
    ...
    def _apply_hf_processor_main(
        ...
        supports_video_metadata = getattr(hf_processor, "supports_video_metadata", False)
        ...
        processor_output = self.info.ctx.call_hf_processor(
            hf_processor,
            dict(text=[prompt_text], images=mm_data["images"], videos=mm_data["videos"]),
        ...
        if processor_output is not None:
            ...
            if pixel_values is not None:
                processor_output["images"] = self._pixel_values_norm(pixel_values, hf_processor_mm_kwargs)
    ...
    def _get_prompt_updates(
        ...
        def get_replacement_ernie45vl(item_idx: int, modality: str):
            ...
            placeholder_ids = cached_encode(tokenizer, after_placeholder[modality], add_special_tokens=False)
            return placeholder_ids * num_tokens

        return [
            PromptReplacement(
                modality=modality,
                target=cached_encode(tokenizer, before_placeholder[modality], add_special_tokens=False),
...
class Ernie4_5_VLMoeForConditionalGeneration(
    ...
    def __init__(self, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        if getattr(self.config, "im_patch_id", None):
            ...
            visual_token_ids_tensor_cache = torch.tensor(visual_token_ids, dtype=torch.long)
        else:
    ...
    def get_encoder_cudagraph_budget_range(self, aphrodite_config) -> tuple[int, int]:
    ...
    def prepare_encoder_cudagraph_replay_buffers(
        ...
        metadata = self.vision_model.prepare_encoder_metadata(grid_thw_list, max_batch_size=max_batch_size)
        values = metadata | {
            "pixel_values": mm_kwargs["pixel_values"],
        }
    ...
    def encoder_eager_forward(self, mm_kwargs: dict[str, Any], path: str = "default") -> torch.Tensor:
        ...
        grid_thw = async_tensor_h2d(mm_kwargs["image_grid_thw"], self.vision_model.device)
        image_features = self.vision_model(pixel_values, grid_thw)
    ...
    def postprocess_encoder_output(
        ...
        num_valid = int((grid_thw_cpu[:, 0] * grid_thw_cpu[:, 1] * grid_thw_cpu[:, 2]).sum())
    ...
    def _parse_and_validate_multimodal_inputs(self, **kwargs: object) -> dict:
        modalities: dict[str, Ernie4_5_VLImageInputs | Ernie4_5_VLVideoInputs | None] = {}
