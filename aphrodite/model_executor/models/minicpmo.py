...
if TYPE_CHECKING:
    from aphrodite.transformers_utils.processors.minicpmo import MiniCPMOProcessor
...
class MiniCPMOMultiModalProcessor(MiniCPMVMultiModalProcessor[MiniCPMOProcessingInfo]):
    ...
    def _get_prompt_updates(
        ...
        audio_placeholder = cached_encode(tokenizer, self.info.audio_pattern, add_special_tokens=False)
        unk_token_ids = [vocab["<unk>"]]

        def get_audio_replacement(item_idx: int):
            audios = mm_items.get_items("audio", (MiniCPMOAudioEmbeddingItems, AudioProcessorItems))
            assert isinstance(audios, (MiniCPMOAudioEmbeddingItems, AudioProcessorItems))
...
class MiniCPMOBaseModel(_MiniCPMOBaseModelBase):
    ...
    hf_to_aphrodite_mapper = WeightsMapper(orig_to_new_prefix={"tts": None})
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        with self._mark_tower_model(aphrodite_config, "audio"):
            self.apm = self.init_audio_module(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, "apm"))

    def init_audio_module(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
    ...
    def _parse_and_validate_multimodal_inputs(self, **kwargs: object) -> MiniCPMOMultiModalInputs:
        modalities = MiniCPMOMultiModalInputs(**super()._parse_and_validate_multimodal_inputs(**kwargs))
    ...
    def _process_multimodal_inputs(self, modalities: Mapping[str, object]):
        ...
        assert image_input is None or isinstance(image_input, (MiniCPMVImagePixelInputs, MiniCPMVImageEmbeddingInputs))
        assert video_input is None or isinstance(video_input, (MiniCPMVImagePixelInputs, MiniCPMVImageEmbeddingInputs))
...
class MiniCPMO(MiniCPMOBaseModel, MiniCPMV2_6):
    ...
    def __new__(cls, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        if instance_cls is None:
            supported_versions = ", ".join([f"{v[0]}.{v[1]}" for v in sorted(_MINICPMO_SUPPORT_VERSION.keys())])
            raise ValueError(
                f"Currently, MiniCPMO only supports versions {supported_versions}. Got version: {version_values}"
            )
