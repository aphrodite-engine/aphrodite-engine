...
class Idefics3MultiModalProcessor(BaseMultiModalProcessor[Idefics3ProcessingInfo]):
    ...
    def _get_prompt_updates(
        ...
        def get_replacement_idefics3(item_idx: int) -> PromptUpdateDetails:
            ...
            image_repl_ids = cached_encode(tokenizer, image_repl, add_special_tokens=False)
...
class Idefics3Model(nn.Module):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        config: Idefics3Config = aphrodite_config.model_config.hf_config
...
class Idefics3ForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsLoRA, SupportsEncoderCudaGraph):
    ...
    def select_encoder_cudagraph_items(
        ...
        if len(indices) == 0:
            return {
                ...
                "num_patches": torch.empty((0,), dtype=torch.long, device=pixel_values.device),
            }
        ...
        selected_pixel_values = torch.cat([pixel_values[cum_patches[i] : cum_patches[i + 1]] for i in indices])
    ...
    def prepare_encoder_cudagraph_capture_inputs(
        ...
        dummy_pixel_values = torch.randn(num_tiles, 3, image_size, image_size, device=device, dtype=dtype)
        dummy_pixel_attention_mask = torch.ones(num_tiles, image_size, image_size, device=device, dtype=torch.bool)
    ...
    def prepare_encoder_cudagraph_replay_buffers(
        ...
        return EncoderCudaGraphReplayBuffers(
            values={
                "pixel_values": mm_kwargs["pixel_values"],
                "pixel_attention_mask": mm_kwargs["pixel_attention_mask"],
                "position_ids": self.model.get_position_ids(mm_kwargs["pixel_attention_mask"]),
            }
        )
