...
from aphrodite.multimodal.parse import ImageProcessorItems, ImageSize, VideoProcessorItems
...
def _minicpmv4_6_field_config(hf_inputs: Mapping[str, torch.Tensor]):
    ...
    if "use_vit_merger" in hf_inputs:
        fields["use_vit_merger"] = MultiModalFieldConfig.batched("image", keep_on_cpu=True)
    return fields
...
class MiniCPMV4_6MultiModalProcessor(MiniCPMVMultiModalProcessor):
    ...
    def process_images(
        ...
        # (NaViT-style), and `target_sizes` (n_slices, 2). Aphrodite expects each
    ...
    def _recompute_cached_prompt_update(
        ...
        new_update = super()._recompute_cached_prompt_update(cached_update, new_item_idx)
...
class MiniCPMV4_6ProcessingInfo(MiniCPMVProcessingInfo):
    ...
    def _compute_visual_tokens(
        ...
        # Aphrodite ImageSize is (width, height); transformers expects (height, width)
...
class MiniCPMV4_6ForConditionalGeneration(
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        super().__init__()
        config: MiniCPMV4_6Config = aphrodite_config.model_config.hf_config
