...
from aphrodite.model_executor.models.ovis import VisualEmbedding
from aphrodite.model_executor.models.siglip2navit import Siglip2NavitModel
...
class Ovis2_5MultiModalProcessor(BaseMultiModalProcessor[Ovis2_5ProcessingInfo]):
    ...
    def _postprocess_hf_mm_data(
        ...
        if "videos" in mm_data:
            visual_indicators = [
                hf_processor.construct_visual_indicators((1, 1, 1), True) for grid in processed_data["video_grids"]
            ]
        ...
        if "images" in mm_data:
            visual_indicators = [
                hf_processor.construct_visual_indicators((1, 1, 1), False) for grid in processed_data["grids"]
            ]
            indicator_tokens = [self.visual_indicators_to_visual_tokens(indicator) for indicator in visual_indicators]
