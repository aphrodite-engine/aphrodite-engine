...
from aphrodite.model_executor.layers.resampler import (
    BaseResampler,
    Resampler2,
    get_2d_sincos_pos_embed,
)
from aphrodite.model_executor.models.llama import LlamaForCausalLM
from aphrodite.model_executor.models.minicpm import MiniCPMForCausalLM
from aphrodite.model_executor.models.module_mapping import MultiModelKeys
from aphrodite.model_executor.models.qwen2 import Qwen2ForCausalLM
...
from aphrodite.utils.collection_utils import flatten_2d_lists, is_list_of
...
class Resampler2_5(BaseResampler):
    ...
    def forward(self, x: torch.Tensor, tgt_sizes: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(bs):
            ...
            pos_embed.append(
                self.pos_embed[:tgt_h, :tgt_w, :].reshape((tgt_h * tgt_w, -1)).to(device=device, dtype=dtype)
            )  # patches * D
...
class Resampler4_5(Resampler2_5):
    ...
    def get_1d_sincos_pos_embed_from_temporal_size(self, embed_dim: int, pos: np.ndarray):
        """
    ...
    def _set_temporal_pos_cache(self, max_temporal_size: int, device: torch.types.Device = "cpu") -> None:
    ...
    def _adjust_temporal_pos_cache(self, max_temporal_size: int, device: torch.types.Device = "cpu"):
    ...
    def forward(
        ...
        pos_embed_2d = torch.nn.utils.rnn.pad_sequence(pos_embed_2d, batch_first=True, padding_value=0.0).permute(
            1, 0, 2
        )  # BLD => L * B * D
        ...
        if pos_embed_temporal:
            ...
            k = torch.nn.utils.rnn.pad_sequence(merge_k, batch_first=True, padding_value=0.0).permute(
                1, 0, 2
            )  # L*(end-start)
            v = torch.nn.utils.rnn.pad_sequence(merge_v, batch_first=True, padding_value=0.0).permute(
                1, 0, 2
            )  # L*(end-start)
...
def _image_kwargs_from_video(kwargs: Mapping[str, object]) -> dict[str, object]:
    return {_VIDEO_TO_IMAGE_KWARGS[k]: v for k, v in kwargs.items() if k in _VIDEO_TO_IMAGE_KWARGS}
...
class MiniCPMVProcessingInfo(BaseProcessingInfo):
    ...
    def _image_processor_cls(self):
        ...
        if not class_ref:
            raise ValueError(
                f"Missing auto_map['AutoImageProcessor'] in image processor config for {model_config.model!r}"
            )
    ...
    def get_hf_processor(self, **kwargs: object):
        ...
        from aphrodite.transformers_utils.processors.minicpmv import MiniCPMVProcessor
...
class MiniCPMVMultiModalProcessor(BaseMultiModalProcessor[_I]):
    ...
    def _get_prompt_updates(
        ...
        for modality, pattern in placeholders:
            sub_pattern = tokenizer.decode(cached_encode(tokenizer, pattern, add_special_tokens=False))
        ...
        def get_image_replacement(item_idx: int):
            ...
            assert isinstance(images, (MiniCPMVImageEmbeddingItems, ImageProcessorItems))
        ...
        def get_video_replacement(item_idx: int):
            ...
            assert isinstance(videos, (MiniCPMVVideoEmbeddingItems, VideoProcessorItems))
...
class MiniCPMVBaseModel(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        # check `tie_word_embeddings` until Aphrodite integrate MiniCPM-V model
        # and config class
        self.config = config
        ...
        with self._mark_language_model(aphrodite_config):
            self.llm = self.init_llm(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, "llm"))

        with self._mark_tower_model(aphrodite_config, {"image", "video"}):
            self.vpm = self.init_vision_module(config, quant_config, prefix=maybe_prefix(prefix, "vpm"))
            self.vision_dim = self.vpm.embed_dim if self.version == (2, 0) else self.vpm.embeddings.embed_dim
    ...
    def _parse_and_validate_multimodal_inputs(self, **kwargs: object) -> MiniCPMVMultiModalInputs:
...
class MiniCPMV2_0(MiniCPMVBaseModel):
    ...
    def init_llm(
        ...
        return MiniCPMForCausalLM(aphrodite_config=aphrodite_config, prefix=prefix)
    ...
    def init_vision_module(
        ...
        if isinstance(model, timm.models.VisionTransformer) and model.attn_pool is not None:
...
class MiniCPMV2_5(MiniCPMVBaseModel, SupportsLoRA):
    ...
    def init_llm(
        ...
        return LlamaForCausalLM(aphrodite_config=aphrodite_config, prefix=prefix)
...
class MiniCPMV2_6(MiniCPMVBaseModel, SupportsLoRA):
    ...
    def init_llm(
        ...
        return Qwen2ForCausalLM(aphrodite_config=aphrodite_config, prefix=prefix)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        loader = AutoWeightsLoader(self)
        loaded = loader.load_weights(weights, mapper=self.hf_to_aphrodite_mapper)
        self._ensure_resampler_device()
        return loaded


class MiniCPMV4_0(MiniCPMVBaseModel, SupportsLoRA):
    hf_to_aphrodite_mapper = MiniCPMV2_6.hf_to_aphrodite_mapper
    packed_modules_mapping = {
        "qkv_proj": [
            "q_proj",
            "k_proj",
...
class MiniCPMV4_5(MiniCPMVBaseModel, SupportsLoRA):
    ...
    def init_llm(
        ...
        return Qwen3ForCausalLM(aphrodite_config=aphrodite_config, prefix=prefix)
...
class MiniCPMV(MiniCPMVBaseModel, SupportsMultiModal, SupportsLoRA):
    ...
    def __new__(cls, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        config = aphrodite_config.model_config.hf_config
        ...
        if instance_cls is None:
            ...
            raise ValueError(
                f"Currently, MiniCPMV only supports versions {supported_versions}. Got version: {version_values}"
            )
        ...
        return instance_cls(aphrodite_config=aphrodite_config, prefix=prefix)
