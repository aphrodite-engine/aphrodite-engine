...
from aphrodite.model_executor.layers.fused_qk_norm_rope import fused_qk_rmsnorm_rope_gate
...
from aphrodite.model_executor.layers.rotary_embedding import MRotaryEmbedding, get_rope
from aphrodite.model_executor.layers.vocab_parallel_embedding import (
    ParallelLMHead,
    VocabParallelEmbedding,
)
from aphrodite.model_executor.models.qwen2_moe import Qwen2MoeMLP as Qwen3NextMLP
...
def _should_use_sequence_parallel(aphrodite_config: AphroditeConfig) -> bool:
...
def _should_replicate_misaligned_shared_expert(
    ...
    if remainder != 0:
        raise ValueError(
            f"Shared-expert intermediate size {intermediate_size} must be divisible by tensor-parallel size {tp_size}."
        )
...
class Qwen3NextSparseMoeBlock(nn.Module):
    def __init__(self, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        if config.shared_expert_intermediate_size > 0 and not self.replicate_shared_expert:
        ...
        if self.is_fused_shared_expert_enabled or config.shared_expert_intermediate_size <= 0:
        ...
        self.experts = FusedMoEFactory(
            shared_experts=(None if self.replicate_shared_expert else self.shared_expert),
            gate=self.gate,
...
class Qwen3NextAttention(nn.Module):
    ...
    def _project_qkv_gate(
        ...
        if self.use_fused_qk_norm_rope_gate:
            ...
            if positions.ndim == 2 and not getattr(self.rotary_emb, "mrope_section", None):
            ...
            q, k, gate = fused_qk_rmsnorm_rope_gate(
                ...
                norm_beta=1.0,
                mrope_section=(getattr(self.rotary_emb, "mrope_section", None) if positions.ndim == 2 else None),
            )
            return q, k, v, gate
...
class Qwen3NextDecoderLayer(nn.Module):
    def __init__(
        ...
        self.use_attn_reduce_scatter_for_moe = _should_use_sequence_parallel(aphrodite_config)
...
class Qwen3NextModel(nn.Module, EagleModelMixin):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        config: Qwen3NextConfig = aphrodite_config.model_config.hf_text_config
        parallel_config = aphrodite_config.parallel_config
...
class Qwen3NextForCausalLM(
    ...
    hf_to_aphrodite_mapper = WeightsMapper(orig_to_new_prefix={"mtp.": None})
