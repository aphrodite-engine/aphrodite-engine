...
from aphrodite.inputs import ModalityData, MultiModalDataDict, PromptType, TextPrompt
...
from aphrodite.model_executor.models.whisper import (
    WhisperEncoder,
...
from aphrodite.multimodal.processing.processor import ProcessorInputs
...
_MOSS_DIARIZED_HEADER_RE = re.compile(r"\[(?P<start>[0-9.]{1,32})\]\s*\[(?P<speaker>S[0-9]{1,15})\]")
...
MossTranscribeDiarizeInputs: TypeAlias = MossTranscribeDiarizeAudioInputs | MossTranscribeDiarizeEmbeddingInputs
...
def _as_audio_embedding_list(audio_embeds: object) -> list[torch.Tensor]:
    if isinstance(audio_embeds, torch.Tensor):
        ...
        raise ValueError(f"`audio_embeds` must be a 2D or 3D tensor, got {audio_embeds.ndim}D.")
    ...
    raise TypeError(
        f"`audio_embeds` must be a torch.Tensor or a list of torch.Tensor objects, got {type(audio_embeds)!r}."
    )
...
def _get_audios_from_mm_data(mm_data: Mapping[str, object]) -> list[Any]:
    ...
    if isinstance(audio_data, list):
        audios = audio_data
    elif isinstance(audio_data, tuple) and not (len(audio_data) == 2 and isinstance(audio_data[1], (int, float))):
...
def _add_aphrodite_audio_metadata(
    ...
    if audio_chunk_mapping.numel() > 0 and (
        audio_chunk_mapping.min().item() < 0 or audio_chunk_mapping.max().item() >= num_audios
    ):
...
class MossTranscribeDiarizeWhisperEncoder(WhisperEncoder):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            aphrodite_config=aphrodite_config.with_hf_config(aphrodite_config.model_config.hf_config.audio_config),
            prefix=prefix,
        )
        self.audio_merge_size = int(aphrodite_config.model_config.hf_config.audio_merge_size)
    ...
    def forward(
        ...
        for start in range(0, input_features.shape[0], batch_size):
            encoded_parts.append(super().forward([input_features[start : start + batch_size]]))
...
class MossTranscribeDiarizeDummyInputsBuilder(BaseDummyInputsBuilder[MossTranscribeDiarizeProcessingInfo]):
...
class MossTranscribeDiarizeMultiModalProcessor(BaseMultiModalProcessor[MossTranscribeDiarizeProcessingInfo]):
    ...
    def _postprocess_hf_mm_data(
        ...
        audios = _get_audios_from_mm_data(mm_data)
        if audios:
            processed_data = _add_aphrodite_audio_metadata(processed_data, len(audios))
    ...
    def _get_prompt_updates(
        ...
        if audio_token_lengths_tensor is None:
            ...
            if audio_embeds is None:
                ...
                audio_token_lengths = [
                    int(audio_embed.shape[0]) for audio_embed in _as_audio_embedding_list(audio_embeds)
                ]
        else:
            if not isinstance(audio_token_lengths_tensor, torch.Tensor):
                raise TypeError(
                    f"`audio_token_lengths` must be a torch.Tensor, got {type(audio_token_lengths_tensor)!r}."
                )
            audio_token_lengths = [int(length) for length in audio_token_lengths_tensor.tolist()]
        ...
        def get_num_tokens(item_idx: int) -> int:
            if item_idx >= len(audio_token_lengths):
                raise ValueError(f"Cannot determine the number of audio tokens for audio item {item_idx}.")
        ...
        return [
            PromptReplacement(
                modality="audio",
                target=cached_encode(tokenizer, AUDIO_PLACEHOLDER, add_special_tokens=False),
...
class MossTranscribeDiarizeForConditionalGeneration(
    ...
    def parse_diarized_transcript(cls, text: str) -> list[DiarizedTranscriptionSegment]:
        ...
        for index, (header, start, speaker) in enumerate(headers):
            next_header_start = headers[index + 1][0].start() if index + 1 < len(headers) else len(text)
    ...
    def _process_audio_input(self, audio_input: MossTranscribeDiarizeInputs) -> list[torch.Tensor]:
        ...
        if input_features is None or audio_feature_lengths is None:
            raise ValueError(
                "MOSS-Transcribe-Diarize audio inputs require both `input_features` and `audio_feature_lengths`."
            )
        ...
        audio_chunk_offsets = torch.cat([audio_chunk_offsets.new_zeros(1), audio_chunk_offsets])
        tokens_per_item = [
