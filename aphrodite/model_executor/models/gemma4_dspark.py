...
from aphrodite.model_executor.layers.linear import ColumnParallelLinear, ReplicatedLinear
...
class Gemma4DSparkAttention(Gemma4MTPAttention):
    ...
    def __init__(
        ...
        self.use_k_eq_v = layer_type == "full_attention" and getattr(config, "attention_k_eq_v", False)
...
class Gemma4DSparkDecoderLayer(Gemma4MTPDecoderLayer):
    ...
    def __init__(
        ...
        get_current_aphrodite_config().compilation_config.static_forward_context.pop(
...
class Gemma4DSparkModel(DFlashQwen3Model):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        self.target_layer_ids = tuple(getattr(config, "dspark_target_layer_ids", None) or config.target_layer_ids)
        ...
        self.register_buffer(
            "normalizer",
            torch.tensor(config.hidden_size**0.5, dtype=aphrodite_config.model_config.dtype),
            persistent=False,
        )
    ...
    def _project_context_kv(
        ...
        all_k = all_k_flat.view(num_ctx, num_layers, num_kv_heads, head_dim).permute(1, 0, 2, 3).contiguous()
    ...
    def forward(
        ...
        hidden_states = self.embed_input_ids(input_ids) if input_embeds is None else input_embeds
