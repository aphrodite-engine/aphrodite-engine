...
from aphrodite.multimodal.inputs import MultiModalFieldConfig, MultiModalKwargsItem
...
LlavaNextImageInputs: TypeAlias = LlavaNextImagePixelInputs | LlavaNextImageEmbeddingInputs
...
class LlavaNextProcessingInfo(BaseLlavaProcessingInfo):
    ...
    def get_image_size_with_most_features(self) -> ImageSize:
        ...
        for height, width in hf_config.image_grid_pinpoints:
            feat_size = self.get_num_image_tokens(image_width=width, image_height=height)
...
class LlavaNextForConditionalGeneration(nn.Module, SupportsLoRA, SupportsMultiModal, SupportsPP):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        # Used for multimodal granite models to control encoder outputs
            ...
            vision_hidden_size = config.vision_config.hidden_size * len(vision_feature_layer)
            self.select_layers = vision_feature_layer
        else:
            raise TypeError(f"vision_layer_feature type: {type(vision_feature_layer)} is not supported")
    ...
    def _process_image_pixels(
        ...
        return torch.split(self.multi_modal_projector(stacked_image_features), num_patches_per_batch)
    ...
    def _process_image_input(
        ...
        return [
            self._merge_image_patch_embeddings(image_sizes[i], patch_features_batch, strategy="spatial_unpad")
    ...
    def forward(
        ...
        **kwargs: object,
        ...
        is given by [`LlavaNextProcessingInfo.get_num_image_tokens`][aphrodite.\
model_executor.models.llava_next.LlavaNextProcessingInfo.get_num_image_tokens].
        ...
        Info:
            [`LlavaNextImageInputs`][aphrodite.model_executor.models.llava_next.LlavaNextImageInputs]
        """
    ...
    def get_mm_lora_token_counts(
        ...
        *,
        ...
        tokens_per_tile = encoder_info.get_num_image_tokens(image_width=tile_size, image_height=tile_size)
        selected_per_tile = get_num_selected_vision_tokens(tokens_per_tile, self.config.vision_feature_select_strategy)
