...
Cohere_VLImageInputs: TypeAlias = Cohere_VLImagePixelInputs | Cohere_VLImageEmbeddingInputs
    ...
    def _parse_image_data(
        ...
        if isinstance(data, dict):
            return DictEmbeddingItems(
                ...
                fields_factory=_create_cohere_compass_field_factory(self._spatial_merge_size),
            )
...
class CohereCompassDummyInputsBuilder(BaseDummyInputsBuilder[CohereCompassProcessingInfo]):
...
class CohereCompassMultiModalProcessor(BaseMultiModalProcessor[CohereCompassProcessingInfo]):
    def _get_mm_fields_config(
        self,
        hf_inputs: BatchFeature,
        hf_processor_mm_kwargs: Mapping[str, object],
    ) -> Mapping[str, MultiModalFieldConfig]:
        return _create_cohere_compass_field_factory(self.info.get_hf_config().vision_config.spatial_merge_size)(
            hf_inputs
        )
...
class CohereCompassForConditionalGeneration(
    ...
    def prepare_encoder_cudagraph_capture_inputs(
        ...
        grid_config = [[1, spatial_merge_size, per_mm_item_output * spatial_merge_size] for _ in range(max_batch_size)]
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Cohere_VLImageInputs | None:
    ...
    def _process_image_input(self, image_input: Cohere_VLImageInputs) -> tuple[torch.Tensor, ...]:
...
class CohereCompassAttention(nn.Module):
    def __init__(
        ...
        sliding_window = config.sliding_window if layer_type == "sliding_attention" else None
        self.attn = Attention(
            self.num_heads,
            self.head_dim,
            self.scaling,
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(
            ...
            lambda prefix: CohereCompassDecoderLayer(
                ...
                cache_config,
                quant_config=quant_config,
                max_position_embeddings=aphrodite_config.model_config.max_model_len,
                prefix=prefix,
            ),
