...
def _restore_full_token_layout_if_needed(
    ...
    hidden_states, residual = combined_states.split([hidden_states.shape[-1], residual.shape[-1]], dim=-1)
    return hidden_states, residual
...
class DeepSeekMultiTokenPredictor(nn.Module):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.layers = torch.nn.ModuleDict(
            {
                str(idx): DeepSeekMultiTokenPredictorLayer(aphrodite_config, f"{prefix}.layers.{idx}")
                for idx in range(
