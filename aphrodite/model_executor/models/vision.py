...
from aphrodite.config import ModelConfig, MultiModalConfig, get_current_aphrodite_config_or_none
...
from aphrodite.transformers_utils.processor import get_processor, get_processor_config
...
def is_vit_use_data_parallel(num_heads: int | None = None) -> bool:
    ...
    """
    mm_cfg = get_multimodal_config()
    can_split = num_heads % get_tensor_model_parallel_world_size() == 0 if num_heads is not None else None
...
class FusedInputNorm(nn.Module):
    ...
    def __init__(
        ...
        image_mean_cpu = torch.tensor(image_mean, dtype=dtype, device="cpu") * (1.0 / rescale_factor)
        image_std_cpu = torch.tensor(image_std, dtype=dtype, device="cpu") * (1.0 / rescale_factor)
    ...
    @classmethod
    def identity(cls, channel: int = 3, dtype: torch.dtype = torch.float32) -> "FusedInputNorm":
        return cls(
    ...
    def from_model_config(cls, model_config: "ModelConfig") -> nn.Module:
        ...
        return cls(image_mean=image_mean, image_std=image_std, rescale_factor=rescale_factor)
    ...
    def forward(
        ...
        if current_platform.is_xpu() and grid_thw.dtype == torch.uint8 and self.weight.dtype == torch.float32:
            return torch.ops.aphrodite.xpu_fused_input_norm(grid_thw, self.weight, self.bias, visual_dtype)
