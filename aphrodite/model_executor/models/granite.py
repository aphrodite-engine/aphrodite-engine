...
def granite_layer_attn_params(config: PretrainedConfig, layer_idx: int) -> tuple[int | None, float, bool]:
    ...
    rope_theta = layer_rope_theta[layer_idx] if layer_rope_theta is not None else config.rope_parameters["rope_theta"]
...
class GraniteAttention(nn.Module):
    def __init__(
        ...
        sliding_window, rope_theta, has_sink = granite_layer_attn_params(config, extract_layer_index(prefix))
...
class GraniteForCausalLM(nn.Module, SupportsLoRA, SupportsPP, SupportsQuant):
    hf_to_aphrodite_mapper = GraniteModel.hf_to_aphrodite_mapper
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.model = GraniteModel(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, "model"))
        if get_pp_group().is_last_rank:
