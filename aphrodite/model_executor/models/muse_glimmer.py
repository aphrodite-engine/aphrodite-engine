...
"""Inference-only MuseGlimmer multimodal model for Aphrodite.
...
from aphrodite.model_executor.layers.attention import Attention, MMEncoderAttention
...
from aphrodite.transformers_utils.processors.muse_glimmer import MuseGlimmerProcessor
...
class MuseGlimmerProcessingInfo(BaseProcessingInfo):
    ...
    def get_mm_max_tokens_per_item(
        ...
        return {
            ...
            "video": (num_frames // int(video_processor.patch_temporal) * int(video_processor.max_video_frame_tokens)),
        }

    def get_image_size_with_most_features(self) -> ImageSize:
        image_processor = self.get_hf_processor().image_processor
        grid_size = math.isqrt(int(image_processor.max_image_tokens))
        side = int(image_processor.patch_size) * int(image_processor.downsample_factor) * grid_size
        return ImageSize(width=side, height=side)

    def get_num_frames_with_most_features(self, seq_len: int, mm_counts: Mapping[str, int]) -> int:
...
class MuseGlimmerDummyInputsBuilder(BaseDummyInputsBuilder[MuseGlimmerProcessingInfo]):
    def get_dummy_text(self, mm_counts: Mapping[str, int]) -> str:
        return IMAGE_TOKEN * mm_counts.get("image", 0) + VIDEO_TOKEN * mm_counts.get("video", 0)
    ...
    def get_dummy_mm_data(
        ...
        video_size = int(video_processor.patch_size) * int(video_processor.downsample_factor) * video_grid
...
class MuseGlimmerMultiModalProcessor(BaseMultiModalProcessor[MuseGlimmerProcessingInfo]):
    ...
    def _get_mm_fields_config(
        ...
        if "image_pixel_values" in hf_inputs:
            fields.update(
                ...
                image_feature_sizes=MultiModalFieldConfig.batched("image", keep_on_cpu=True),
            )
        if "video_pixel_values" in hf_inputs:
            fields.update(
                video_pixel_values=MultiModalFieldConfig.batched("video"),
                video_feature_sizes=MultiModalFieldConfig.batched("video", keep_on_cpu=True),
            )
        return fields
    ...
    def _get_prompt_updates(
        ...
        def image_replacement(item_idx: int) -> PromptUpdateDetails:
            ...
            replacement = [image_start_id] + [config.image_token_id] * num_tokens + [image_end_id]
            return PromptUpdateDetails.select_token_id(replacement, config.image_token_id)
        ...
        def video_replacement(item_idx: int) -> PromptUpdateDetails:
            ...
            if remainder:
                raise ValueError("MuseGlimmer video feature size must be divisible by frame groups")
            ...
            for group_idx in range(num_groups):
                ...
                replacement.extend(tokenizer.encode(f"Time: {timestamp:.1f}s", add_special_tokens=False))
                replacement.extend([config.video_token_id] * tokens_per_group)
                replacement.append(video_separator_id if group_idx < num_groups - 1 else video_end_id)
            return PromptUpdateDetails.select_token_id(replacement, config.video_token_id)
...
class MuseGlimmerVisionEncoder(nn.Module):
    def __init__(self, config, prefix: str = "") -> None:
        ...
        self.fp8_padded_hidden_size = get_fp8_padded_hidden_size(config.num_attention_heads, self.head_dim)
    ...
    def _make_2d_rope(
        ...
        inv_freq = 1.0 / (
            10000.0 ** (torch.arange(0, spatial_dim, 2, dtype=torch.float32, device=device) / spatial_dim)
        )
    ...
    def _get_pos_emb(self, grid_height: int, grid_width: int, device: torch.device) -> torch.Tensor:
        h_grid = (torch.arange(grid_height, device=device, dtype=torch.float32) + 0.5) * (
            self.pos_emb_height / grid_height
        ) - 0.5
        w_grid = (torch.arange(grid_width, device=device, dtype=torch.float32) + 0.5) * (
            self.pos_emb_width / grid_width
        ) - 0.5
    ...
    def _pixel_shuffle_downsample(self, hidden_states: torch.Tensor, grid_height: int, grid_width: int) -> torch.Tensor:
        factor = self.merge_kernel_size
        output_tokens = (grid_height // factor) * (grid_width // factor)
        permutation = torch.arange(grid_height * grid_width, device=hidden_states.device)
        permutation = permutation.view(grid_height // factor, factor, grid_width // factor, factor)
    ...
    def _get_sparse_permutation(
        ...
        indices = torch.arange(grid_height * grid_width, device=device).view(grid_height, grid_width)
        indices = F.pad(
            indices,
    ...
    def _patchify(self, pixels: torch.Tensor) -> torch.Tensor:
        ...
        if channels == 3:
            patches = pixels.unfold(2, patch_size, patch_size).unfold(3, patch_size, patch_size)
            patches = patches.contiguous().view(1, channels, grid_height, grid_width, patch_size, patch_size)
            ...
            for frame_idx in range(self.patch_temporal):
                ...
                frame = frame.unfold(2, patch_size, patch_size).unfold(3, patch_size, patch_size)
                frame = frame.contiguous().view(1, 3, grid_height, grid_width, patch_size, patch_size)
                frame_patches.append(frame.permute(0, 2, 3, 1, 4, 5).contiguous())
            patches = torch.stack(frame_patches, dim=3)
        else:
            raise ValueError(
                f"MuseGlimmer vision input has {channels} channels; expected 3 or {self.patch_temporal * 3}"
            )
    ...
    def forward(self, pixel_values: Sequence[torch.Tensor]) -> torch.Tensor:
        ...
        has_sparse_layers = any(layer_type != "full_attention" for layer_type in self.layer_types)
        ...
        for pixels in pixel_values:
            ...
            if pixels.shape[-2] % spatial_stride or pixels.shape[-1] % spatial_stride:
                raise ValueError("MuseGlimmer vision input dimensions must divide the merged patch stride")
            ...
            hidden_states = hidden_states + self._get_pos_emb(grid_height, grid_width, device).unsqueeze(0).to(dtype)
            hidden_states = self.ln_pre(hidden_states.view(-1, self.hidden_size)).view(1, -1, self.hidden_size)
            rotary_pos_emb_cos, rotary_pos_emb_sin = self._make_2d_rope(grid_height, grid_width, device)

            permutation = None
            if has_sparse_layers:
                permutation, seq_lens = self._get_sparse_permutation(grid_height, grid_width, device)
        ...
        sparse_attention_metadata = self._get_attention_metadata(sparse_seq_lens, device) if sparse_seq_lens else None
        for layer_type, block in zip(self.layer_types, self.transformer):
            attention_metadata = (
                global_attention_metadata if layer_type == "full_attention" else sparse_attention_metadata
            )
        ...
        for grid_height, grid_width, num_tokens, permutation in metadata:
            ...
            if permutation is not None:
                ...
                item = item[:, inverse]
            item = self.ln_post(item.view(-1, self.hidden_size)).view(1, -1, self.hidden_size)
            features.append(self._pixel_shuffle_downsample(item, grid_height, grid_width).squeeze(0))
...
class MuseGlimmerMLP(nn.Module):
    def __init__(
        ...
        if hidden_activation != "silu":
            raise ValueError(f"MuseGlimmer uses `silu` as the hidden activation; got `{hidden_activation}`.")
...
class MuseGlimmerDecoderLayer(nn.Module):
    def __init__(
        ...
        self.input_layernorm = MuseGlimmerRMSNorm(config.hidden_size, eps=config.rms_norm_eps, weight_offset=1)
...
class MuseGlimmerModel(nn.Module, EagleModelMixin):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        super().__init__()
        config = _text_config(aphrodite_config.model_config.hf_config)
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(
            ...
            lambda prefix: MuseGlimmerDecoderLayer(config, cache_config, quant_config, prefix=prefix),
    ...
    def forward(
        ...
        aux_hidden_states = self._maybe_add_hidden_state([], self.start_layer, hidden_states, None)
        for layer_idx, layer in enumerate(
            islice(self.layers, self.start_layer, self.end_layer),
            start=self.start_layer,
        ):
            hidden_states, residual = layer(positions, hidden_states, residual)
            self._maybe_add_hidden_state(aux_hidden_states, layer_idx + 1, hidden_states, None)
...
class MuseGlimmerForCausalLM(nn.Module, SupportsLoRA, SupportsMultiModal, SupportsPP, SupportsEagle3):
    ...
    hf_to_aphrodite_mapper = WeightsMapper(
        ...
        orig_to_new_prefix={
            ...
            "model.vision_tower.patch_embedder.patch_embedding.": ("model.vision_tower.conv1_linear."),
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        image_token_id = getattr(config, "image_token_id", getattr(config, "patch_token_id", 200092))
        video_token_id = getattr(config, "video_token_id", 200091)
        self.configure_mm_token_handling(text_config.vocab_size, [image_token_id, video_token_id])
    ...
    def _parse_and_validate_image_input(
        ...
        **kwargs: object,
        ...
        if pixel_values is None or feature_sizes is None:
            raise ValueError("MuseGlimmer image_pixel_values and image_feature_sizes must be provided together")
    ...
    def _parse_and_validate_video_input(
        ...
        **kwargs: object,
        ...
        if pixel_values is None and feature_sizes is None:
            return None
        if pixel_values is None or feature_sizes is None:
            raise ValueError("MuseGlimmer video_pixel_values and video_feature_sizes must be provided together")
        if not isinstance(feature_sizes, torch.Tensor):
    ...
    def _encode_pixel_groups(
        ...
        if features.shape[0] != sum(feature_sizes):
            raise ValueError(
                f"MuseGlimmer produced {features.shape[0]} vision features for {sum(feature_sizes)} placeholder tokens"
            )
