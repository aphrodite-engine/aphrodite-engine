...
from aphrodite.model_executor.models.interfaces import EagleModelMixin, SupportsEagle3
...
def _get_dflash_layer_types(config) -> tuple[str, ...]:
    ...
    if len(layer_types) != config.num_hidden_layers:
        raise ValueError(
            f"DFlash layer_types length {len(layer_types)} does not match num_hidden_layers {config.num_hidden_layers}."
        )
    # Laguna DFlash checkpoints use a uniform drafter attention flavor.
    if len(set(layer_types)) > 1:
        raise NotImplementedError(
            f"Laguna DFlash drafter requires a uniform `layer_types` (got {sorted(set(layer_types))})."
        )
...
class DFlashLagunaModel(DFlashQwen3Model, EagleModelMixin):
    def __init__(
        ...
        *,
        ...
        if not target_layer_ids:
            raise ValueError("Laguna DFlash config requires non-empty `dflash_config.target_layer_ids`.")
        ...
        self.layers = nn.ModuleList(
            [
                LagunaDecoderLayer(
                    ...
                    layer_idx=layer_idx,
                    attention_prefix=maybe_prefix(prefix, f"layers.{layer_idx + target_layer_count}"),
                )
        ...
        target_hidden_size = aphrodite_config.model_config.get_hidden_size()
    ...
    def _build_context_kv_buffers(
        ...
        self._kv_weights = torch.stack([a.qkv_proj.weight[a.q_size :] for a in layers_attn], dim=0).contiguous()
        if has_bias:
    ...
    def _project_context_kv(
        ...
        all_kv = all_kv_flat.view(num_layers, num_ctx, 2, num_kv_heads, head_dim).permute(2, 0, 1, 3, 4).contiguous()
        all_k = all_kv[0]
        all_v = all_kv[1]
        return all_k, all_v
...
class DFlashLagunaForCausalLM(nn.Module, SupportsEagle3):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        target_layer_num = aphrodite_config.model_config.get_total_num_hidden_layers()
