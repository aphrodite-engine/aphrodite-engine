...
from aphrodite.model_executor.layers.activation import XIELU
...
class ApertusMLP(nn.Module):
    def __init__(
        ...
        if hidden_act != "xielu":
            raise ValueError(f"Unsupported activation: {hidden_act}. Only xIELU is supported for now.")
        self.act_fn = XIELU()
...
class ApertusDecoderLayer(nn.Module):
    def __init__(
        ...
        self.feedforward_layernorm = RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class ApertusForCausalLM(nn.Module, SupportsLoRA, SupportsPP, SupportsEagle, SupportsEagle3):
    ...
    def _init_model(
        ...
        return ApertusModel(aphrodite_config=aphrodite_config, prefix=prefix, layer_type=layer_type)
