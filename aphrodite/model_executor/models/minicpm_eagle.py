...
class EagleMiniCPMDecoderLayer(nn.Module):
    def __init__(
        ...
        self.mup_denominator = getattr(config, "mup_denominator", config.num_hidden_layers)
        self.prefix = prefix
    ...
    def forward(
        ...
        hidden_states = residual + hidden_states * (self.config.scale_depth / math.sqrt(self.mup_denominator))
