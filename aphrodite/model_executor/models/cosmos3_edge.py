...
class Cosmos3EdgeVisionEncoder(Siglip2VisionTransformer):
    """Adapts Cosmos (T, H, W) metadata to Aphrodite packed SigLIP2."""
...
class Cosmos3EdgeForCausalLM(nn.Module):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        aphrodite_config = aphrodite_config.with_hf_config(text_config)
        ...
        if unsupported:
            raise ValueError(
                f"Cosmos3 Edge only supports attention (`*`) and MLP (`-`) layers, but found: {sorted(unsupported)}"
            )
