...
def calc_router_weights(
    ...
    if score_func == "softmax":
        ...
        assert score_func == "sigmoid", f"Unsupported router score function: {score_func}"
...
def fused_mova_impl(
    ...
    intermediate_cache1 = torch.empty((M, top_k_num, N), device=hidden_states.device, dtype=hidden_states.dtype)
...
class K2HorizonRMSNorm(RMSNorm):
    ...
    def forward(self, x, residual=None):
        ...
        orig_shape = x.shape
        x_grouped = x.reshape(-1, self.n_groups, self.group_size).reshape(-1, self.group_size)
        y_grouped = torch.empty_like(x_grouped)

        # Reuse Aphrodite's RMSNorm CUDA kernel on each group.
        ops.rms_norm(
            y_grouped,
            x_grouped,
...
class K2HorizonSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        assert hidden_states.dim() <= 2, "K2HorizonSparseMoeBlock only supports 1D or 2D inputs"
...
class K2HorizonAttention(nn.Module):
    def __init__(
        ...
        if self.query_key_norm:
            self.q_norm = K2HorizonRMSNorm(hidden_size=self.q_size, n_groups=self.num_heads, eps=rms_norm_eps)
            self.k_norm = K2HorizonRMSNorm(hidden_size=self.kv_size, n_groups=self.num_kv_heads, eps=rms_norm_eps)
    ...
    def forward(
        ...
        if self.rope_head_dim == self.head_dim:
            ...
            q_rope, k_rope = self.rotary_emb(positions, interleaved_to_split(q_rope), interleaved_to_split(k_rope))

            q = interleaved_to_split(torch.cat([split_to_interleaved(q_rope), q_nope], dim=-1)).reshape(
                *q.shape[:-2], -1
            )

            k = interleaved_to_split(torch.cat([split_to_interleaved(k_rope), k_nope], dim=-1)).reshape(
                *k.shape[:-2], -1
            )
...
class K2HorizonMoVAAttention(nn.Module):
    def __init__(
        ...
        self.fused_mova_config = try_get_optimal_moe_config(
            ...
            dtype=config_dtype,
            M=aphrodite_config.scheduler_config.max_num_batched_tokens,
            block_shape=None,
        )
...
class K2HorizonDecoderLayer(nn.Module):
    def __init__(self, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        if is_sparse_layer:
            self.mlp = K2HorizonSparseMoeBlock(aphrodite_config=aphrodite_config, prefix=f"{prefix}.mlp")
        else:
...
class K2HorizonModel(nn.Module, EagleModelMixin):
    def __init__(
        ...
        *,
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(
            ...
            lambda prefix: decoder_layer_type(aphrodite_config=aphrodite_config, prefix=prefix),
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            ...
            # QK norm weights
            if name.endswith(".self_attn.q_norm.weight") or name.endswith(".self_attn.k_norm.weight"):
                ...
                if (
                    ...
                    if weight_name == "k_proj":
                        ...
                        if num_kv_heads < tp_size:
                            num_kv_head_replicas = tp_size // num_kv_heads
                            loaded_weight = (
                                loaded_weight.reshape(num_kv_heads, -1, *loaded_weight.shape[1:])
                    ...
                    weight_loader = getattr(param, "weight_loader", default_weight_loader)
                    weight_loader(param, loaded_weight, 0 if weight_name == "q_proj" else 1)
...
class K2HorizonForCausalLM(nn.Module, SupportsPP, SupportsLoRA, SupportsEagle, SupportsEagle3, MixtureOfExperts):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.model = K2HorizonModel(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, "model"))
