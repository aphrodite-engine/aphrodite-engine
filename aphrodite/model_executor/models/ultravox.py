...
from aphrodite.model_executor.layers.activation import MulAndSilu, get_act_fn
...
class UltravoxAudioEmbeddingInputs(TensorSchema):
    ...
    data: Annotated[torch.Tensor | list[torch.Tensor], TensorShape("b", "na", "afs", "hs")]


UltravoxAudioInputs: TypeAlias = UltravoxAudioFeatureInputs | UltravoxAudioEmbeddingInputs
...
class UltravoxMultiModalProcessor(BaseMultiModalProcessor[UltravoxProcessingInfo]):
    ...
    def _apply_hf_processor_main(
        ...
        if processor_data:
            ...
            def patched_call(**kwargs) -> BatchFeature:
                ...
                # already matches Aphrodite's intended behavior.
    ...
    def _get_mm_fields_config(
        ...
        return dict(
            ...
            audio_lens=MultiModalFieldConfig.flat_from_sizes("audio", num_chunks, keep_on_cpu=True),
    ...
    def _get_prompt_updates(
        ...
        chunks_start_idx: torch.Tensor = torch.cumsum(num_chunks, dim=0, dtype=torch.int32)
        chunks_start_idx = torch.cat([torch.tensor([0], dtype=torch.int32), chunks_start_idx])
...
class StackAudioFrames(nn.Module):
    ...
    def forward(self, audio_embeds: torch.Tensor) -> torch.Tensor:
        ...
        B, T, C = audio_embeds.shape
        audio_embeds = audio_embeds.view(B, T // self.stack_factor, C * self.stack_factor)
        return audio_embeds
...
def _build_chunk_attn_metadata(
    ...
    sequence_lengths = MMEncoderAttention.maybe_compute_seq_lens(attn_backend, cu_seqlens_np, device)
...
# Maps HF whisper encoder layer names to Aphrodite's fused qkv_proj and nested
...
def _load_whisper_layer_weights(module: nn.Module, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
...
class UltravoxFeedForwardProjector(nn.Module):
    def __init__(
        ...
        *,
        ...
        # ReplicatedLinear (a Aphrodite-native linear) so the connector can be
    ...
    def forward(self, audio_features: torch.Tensor, audio_token_len: torch.Tensor) -> torch.Tensor:
...
class UltravoxTransformerProjector(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        config: UltravoxConfig = aphrodite_config.model_config.hf_config
        ...
        audio_aphrodite_config = aphrodite_config.with_hf_config(audio_config, architectures=["UltravoxModel"])
        self.layers = nn.ModuleList(
            [
                WhisperEncoderLayer(
                    aphrodite_config=audio_aphrodite_config,
    ...
    def forward(self, audio_features: torch.Tensor, audio_token_len: torch.Tensor) -> torch.Tensor:
        ...
        positions = self.embed_positions(torch.arange(hidden_states.size(1), device=hidden_states.device))
...
class UltravoxWhisperEncoder(WhisperEncoder):
    """Ultravox's `ModifiedWhisperEncoder` on top of Aphrodite's `WhisperEncoder`.
    ...
    Aphrodite-native so the tower can be wrapped for LoRA.
    """
...
class UltravoxModel(nn.Module, SupportsMultiModal, SupportsPP, SupportsLoRA):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.pad_audio_to_max_context = bool(lora_config is not None and lora_config.enable_tower_connector_lora)
        ...
        with self._mark_tower_model(aphrodite_config, "audio"):
            self.audio_tower = UltravoxWhisperEncoder(
                aphrodite_config=aphrodite_config.with_hf_config(config.audio_config, architectures=["UltravoxModel"]),
    ...
    def _get_max_tokens_per_chunk(self) -> int:
        ...
        return math.ceil(self.config.audio_config.max_source_positions / self.config.stack_factor)
    ...
    def _audio_features_to_embeddings(
        ...
        encoder_batch_size = max(batch_size, 1) if self.pad_audio_to_max_context else _MAX_ENCODER_BATCH_SIZE
        for start in range(0, batch_size, encoder_batch_size):
            end = min(start + encoder_batch_size, batch_size)
            # Process through audio tower
            batch_features = self.audio_tower(audio_features[start:end], audio_lens[start:end])
            batch_features = batch_features.to(self.audio_tower.dtype)

            # Process through projector
            batch_embeddings = self.multi_modal_projector(batch_features, audio_token_len[start:end])
    ...
    def _parse_and_validate_audio_input(self, **kwargs: object) -> UltravoxAudioInputs | None:
    ...
    def _process_audio_input(
        ...
        embeddings = self._audio_features_to_embeddings(audio_features, audio_lens, audio_token_len)
        ...
        indices = torch.arange(max_len, device=embeddings.device).expand(embeddings.shape[0], -1)
        ...
        with gpu_sync_allowed():
            ...
            embed_lens = [
                chunk_lens.sum().item() for chunk_lens in audio_token_len.split(audio_input["num_chunks"].tolist())
            ]
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = language_model.model(input_ids, positions, intermediate_tensors, inputs_embeds=inputs_embeds)
        return hidden_states
