...
from aphrodite.model_executor.models.interfaces import supports_any_eagle
...
class WeightsMapper:
    ...
    def get_rename_mapper(self) -> "WeightsMapper":
        ...
        rather than being rewritten to the stacked Aphrodite name (`qkv_proj`). Mappings to
...
class AutoWeightsLoader:
    ...
    def _load_param(
        ...
        for weight_name, weight_data in weights:
            ...
            if weight_name != "":
                ...
                raise ValueError(
                    f"Attempted to load nested weight {weight_qualname!r} into a single parameter {base_prefix!r}"
                )
    ...
    def _load_module(
        ...
        for child_prefix, child_weights in self._groupby_prefix(weights):
            ...
            if child_prefix in child_modules:
                ...
                desc_param_keys = {maybe_prefix(base_prefix, k) for k, _ in named_parameters}
                msg = (
    ...
    def _filter_skipped(self, weights: Iterable[tuple[str, torch.Tensor]]) -> Iterable[tuple[str, torch.Tensor]]:
...
def init_aphrodite_registered_model(
    ...
    *,
    ...
    """
    from aphrodite.model_executor.model_loader.utils import initialize_model
    ...
    if hf_config is not None:
        aphrodite_config = aphrodite_config.with_hf_config(hf_config, architectures=architectures)
...
def spec_decode_needs_target_embed(aphrodite_config: AphroditeConfig) -> bool:
...
def make_layers(
    ...
    modules = torch.nn.ModuleList(
        ...
        + get_offloader().wrap_modules(layer_fn(prefix=f"{prefix}.{idx}") for idx in range(start_layer, end_layer))
...
def get_draft_quant_config(aphrodite_config: AphroditeConfig) -> "QuantizationConfig | None":
    ...
    Args:
        aphrodite_config: The Aphrodite configuration object.
    ...
    draft_load_config = aphrodite_config.load_config

    return (
        AphroditeConfig.get_quantization_config(draft_model_config, draft_load_config) if draft_model_config else None
    )
...
def sequence_parallel_chunk(x: torch.Tensor) -> torch.Tensor:
    return torch.ops.aphrodite.sequence_parallel_chunk_impl(x)
...
def parse_diarized_timestamp(marker: str) -> float | None:
    if not marker or not marker.isascii() or marker.count(".") > 1 or not marker.replace(".", "").isdigit():
        return None
    return float(marker)


def parse_diarized_speaker(speaker: str) -> str | None:
    if len(speaker) < 2 or speaker[0] != "S" or not speaker[1:].isascii() or not speaker[1:].isdigit():
        return None
    return speaker
