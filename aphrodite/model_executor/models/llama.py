...
from aphrodite.model_executor.layers.fusion.fused_act_quant import maybe_fused_act_quant
    ...
    def __init__(
        ...
        if layer_types := getattr(config, "layer_types", None):
            ...
            assert effective_layer_idx < len(layer_types), (
                f"effective_layer_idx: {effective_layer_idx} is out of bounds for layer_types: {layer_types}"
            )
...
class LlamaDecoderLayer(nn.Module):
    def __init__(
        ...
        config = config or aphrodite_config.model_config.hf_config
        cache_config = aphrodite_config.cache_config
        quant_config = self.get_quant_config(aphrodite_config)
...
class LlamaModel(nn.Module, EagleModelMixin):
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            or spec_decode_needs_target_embed(aphrodite_config)
        ):
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(
            ...
            lambda prefix: layer_type(aphrodite_config=aphrodite_config, prefix=prefix),
    ...
    def forward(
        ...
        **extra_layer_kwargs,
        ...
        if get_pp_group().is_first_rank:
            self._maybe_add_hidden_state(aux_hidden_states, self.start_layer, hidden_states, residual)
...
class LlamaForCausalLM(
    ...
    hf_to_aphrodite_mapper = LlamaModel.hf_to_aphrodite_mapper
    ...
    def _init_model(
        ...
        return LlamaModel(aphrodite_config=aphrodite_config, prefix=prefix, layer_type=layer_type)
...
if TYPE_CHECKING:
    ...
    _LlamaBidirectionalForSequenceClassificationBase = as_seq_cls_model(LlamaForCausalLM)
    _LlamaBidirectionalModelBase = as_embedding_model(LlamaForCausalLM)


class LlamaBidirectionalForSequenceClassification(_LlamaBidirectionalForSequenceClassificationBase):
