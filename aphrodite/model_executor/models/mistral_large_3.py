...
from aphrodite.model_executor.models.deepseek_v2 import DeepseekV3ForCausalLM
...
class MistralLarge3ForCausalLM(DeepseekV3ForCausalLM):
    ...
    hf_to_aphrodite_mapper = WeightsMapper(
        orig_to_new_regex={
            regex.compile(r"\Alayers\.(\d+)\.attention_norm\.weight\Z"): r"model.layers.\1.input_layernorm.weight",
            regex.compile(r"\Alayers\.(\d+)\.attention\.wq_a\.(\w+)\Z"): r"model.layers.\1.self_attn.q_a_proj.\2",
            regex.compile(
                r"\Alayers\.(\d+)\.attention\.q_a_norm\.weight\Z"
            ): r"model.layers.\1.self_attn.q_a_layernorm.weight",
            regex.compile(r"\Alayers\.(\d+)\.attention\.wq_b\.(\w+)\Z"): r"model.layers.\1.self_attn.q_b_proj.\2",
            regex.compile(
            ...
            regex.compile(r"\Alayers\.(\d+)\.attention\.wkv_b\.(\w+)\Z"): r"model.layers.\1.self_attn.kv_b_proj.\2",
            regex.compile(r"\Alayers\.(\d+)\.attention\.wo\.(\w+)\Z"): r"model.layers.\1.self_attn.o_proj.\2",
            regex.compile(r"\Alayers\.(\d+)\.ffn_norm\.weight\Z"): r"model.layers.\1.post_attention_layernorm.weight",
            regex.compile(r"\Alayers\.(\d+)\.feed_forward\.w1\.(\w+)\Z"): r"model.layers.\1.mlp.gate_proj.\2",
            regex.compile(r"\Alayers\.(\d+)\.feed_forward\.w2\.(\w+)\Z"): r"model.layers.\1.mlp.down_proj.\2",
            regex.compile(r"\Alayers\.(\d+)\.feed_forward\.w3\.(\w+)\Z"): r"model.layers.\1.mlp.up_proj.\2",
