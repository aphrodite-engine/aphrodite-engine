...
from aphrodite.model_executor.layers.activation import MulAndSilu, SiluAndMul, get_act_fn
...
from aphrodite.utils.math_utils import round_down
...
def build_flat_image_bool_length(
    ...
    low_res_use_col_tokens = image_use_col_tokens if use_single_crop_col_tokens is None else use_single_crop_col_tokens
    low_res_extra = int(low_res_use_col_tokens)
    high_res_extra = int(image_use_col_tokens)

    lengths = resized_h * (resized_w + low_res_extra) + h * (w + high_res_extra) + 4  # [B]
    ...
    for i in range(B):
        ...
        flat[idx] = low_res_image_start_id if use_single_crop_start_token else image_start_id
        idx += 1
...
class Molmo2ProcessingInfo(BaseProcessingInfo):
    ...
    def _get_video_second_idx(
        ...
        if do_sample_frames:
            ...
            # Time-based sampling is done in aphrodite molmo2 video loader or molmo_utils
...
class Molmo2MultiModalProcessor(BaseMultiModalProcessor[Molmo2ProcessingInfo]):
    ...
    def _apply_hf_processor_main(
        ...
        if (images := mm_data.get("images")) is not None:
            ...
            processed_data["image_num_patches"] = processed_data["image_num_crops"] * n_patches
            (
    ...
    def _get_mm_fields_config(
        ...
        return dict(
            ...
            image_num_pooled_patches=MultiModalFieldConfig.batched("image", keep_on_cpu=True),
            ...
            video_num_pooled_patches=MultiModalFieldConfig.batched("video", keep_on_cpu=True),
...
class Molmo2ForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP, SupportsLoRA, SupportsQuant):
    ...
    def _parse_and_validate_multimodal_inputs(self, **kwargs: object) -> Molmo2MultiModalInputs:
