...
class Qwen3OmniMoeAudioAttention(nn.Module):
    ...
    def __init__(
        ...
        self.num_local_heads = self.num_heads if self.disable_tp else self.num_heads // tp_size
...
class Qwen3OmniMoeAudioEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.embed_dim = config.d_model
        self.self_attn = Qwen3OmniMoeAudioAttention(config, prefix=f"{prefix}.self_attn")
...
class Qwen3OmniMoeAudioEncoder(nn.Module):
    """Aphrodite-native Qwen3-Omni Audio Encoder."""
    ...
    def forward(
        ...
        cu_seqlens = async_tensor_h2d(cu_chunk_lens, dtype=torch.int32, device=aftercnn_lens.device).cumsum(
            -1, dtype=torch.int32
        )
...
class Qwen3Omni_VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if self.apply_vit_abs_pos_embed:
            ...
            self.pos_embed = nn.Parameter(torch.empty([1, self.num_grid_per_side**2, self.hidden_size]))
    ...
    def fast_pos_embed_interpolate(self, grid_thw: list[list[int]]) -> torch.Tensor:
        ...
        for t, h, w in grid_thw:
            ...
            repeated = repeated.view(t, h // m_size, m_size, w // m_size, m_size, hidden_dim)
...
class Qwen3OmniMoeThinkerForConditionalGeneration(
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        thinker_config: Qwen3OmniMoeThinkerConfig = aphrodite_config.model_config.hf_config.thinker_config
        # MoE LoRA uses the architecture to determine its weight layout.
        thinker_config.architectures = aphrodite_config.model_config.hf_config.architectures
        quant_config = aphrodite_config.quant_config
        ...
        self.use_deepstack = hasattr(thinker_config.vision_config, "deepstack_visual_indexes") and not isinstance(
