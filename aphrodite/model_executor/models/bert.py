...
from aphrodite.config import AphroditeConfig, CacheConfig, ModelConfig, PoolerConfig
...
from aphrodite.model_executor.layers.pooler.activations import LambdaPoolerActivation
from aphrodite.model_executor.layers.pooler.seqwise import (
    EmbeddingPoolerHead,
    SequencePooler,
    SequencePoolerOutput,
    get_seq_pooling_method,
)
from aphrodite.model_executor.layers.pooler.tokwise import (
...
class BertModel(nn.Module, SupportsQuant):
    ...
    hf_to_aphrodite_mapper = WeightsMapper(
        ...
        # LayerNorm names; rename to Aphrodite's `weight`/`bias`.
        orig_to_new_substr={
    ...
    def __init__(
        ...
        *,
        ...
        self.encoder = BertEncoder(aphrodite_config=aphrodite_config, prefix=f"{prefix}.encoder")
...
class BertPoolingModel(BertModel):
    ...
    hf_to_aphrodite_mapper = replace(BertModel.hf_to_aphrodite_mapper, orig_to_new_prefix={})
    ...
    def forward(
        ...
        for i in range(B):
            ...
            if self.remove_cls_sep:
                if self.cls_token_id is not None and int(token_ids[0]) == self.cls_token_id:
                    start_idx = 1
                if self.sep_token_id is not None and int(token_ids[L - 1]) == self.sep_token_id:
...
class BertSpladeSparseEmbeddingModel(BertEmbeddingModel):
    ...
    def _build_pooler(self, pooler_config: PoolerConfig) -> Pooler:
        ...
        # None of Aphrodite's built-in sequence pooling types are
...
class BertForSequenceClassification(nn.Module, SupportsCrossEncoding, SupportsQuant):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.classifier = nn.Linear(
            config.hidden_size,
            config.num_labels,
            dtype=aphrodite_config.model_config.head_dtype,
        )
...
class BertForMaskedLM(nn.Module):
    ...
    Produces per-token logits over the vocabulary. In Aphrodite terms this is a
