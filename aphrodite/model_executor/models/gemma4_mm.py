...
from aphrodite.config.model import get_served_model_name
...
from aphrodite.model_executor.models.gemma3n_mm import batch_audio_features
...
class Gemma4ProcessingInfo(BaseProcessingInfo):
    ...
    def validate_num_items(self, modality: str, num_items: int) -> None:
        if modality == "audio" and num_items > 0 and self.get_hf_config().audio_config is None:
            model_config = self.ctx.model_config
            model = get_served_model_name(model_config.model, model_config.served_model_name)
            raise ValueError(
    ...
    def get_mm_max_tokens_per_item(self, seq_len: int, mm_counts: Mapping[str, int]) -> Mapping[str, int] | None:
        ...
        # Aphrodite needs the max for memory planning.
    ...
    def get_image_repl(
        ...
        *,
        ...
        config = self.get_hf_config()
        token_ids = [config.boi_token_id] + [processor.image_token_id] * num_soft + [config.eoi_token_id]
        return PromptUpdateDetails.select_token_id(token_ids, processor.image_token_id)

    @staticmethod
    def _compute_audio_num_tokens(num_samples: int, sampling_rate: int, audio_seq_length: int) -> int:
    ...
    def get_audio_repl(
        ...
        *,
        ...
        num_tokens = self._compute_audio_num_tokens(audio_len, sampling_rate, processor.audio_seq_length)
        config = self.get_hf_config()
        token_ids = (
            [config.boa_token_id]
...
class Gemma4MultiModalProcessor(BaseMultiModalProcessor[Gemma4ProcessingInfo]):
    def _apply_hf_processor_main(
        ...
        if val is not None and val not in _SUPPORTED_SOFT_TOKENS:
            raise ValueError(f"Unsupported max_soft_tokens value: {val}. Valid values are {_SUPPORTED_SOFT_TOKENS}.")
        ...
        if videos := mm_data.pop("videos", []):
            ...
            for item in videos:
                ...
                if isinstance(video_array, np.ndarray):
                    frames = [PILImage.fromarray(video_array[i]) for i in range(video_array.shape[0])]
                else:
                ...
                if "image_position_ids" in frame_outputs:
                    frame_outputs["pixel_position_ids"] = frame_outputs.pop("image_position_ids")
                ...
                for img in frames:
                    w, h = img.size
                    n = self.info._compute_num_soft_tokens(w, h, max_soft_tokens=_VIDEO_MAX_SOFT_TOKENS)
                ...
                replacement = " ".join(
                    f"{t} {processor.boi_token}{processor.video_token * n}{processor.eoi_token}"
        ...
        # Aphrodite's call_hf_processor (context.py) re-merges
        ...
        # HF uses 'image_position_ids'; Aphrodite uses 'pixel_position_ids'.
        # Remap here to keep a single translation point.
        if "image_position_ids" in processed_data:
            processed_data["pixel_position_ids"] = processed_data.pop("image_position_ids")
    ...
    def _get_mm_fields_config(
        ...
        if video_frame_counts is not None:
            ...
            fields.update(
                pixel_values_videos=(MultiModalFieldConfig.flat_from_sizes("video", vfc)),
                pixel_position_ids_videos=(MultiModalFieldConfig.flat_from_sizes("video", vfc)),
                video_frame_counts=MultiModalFieldConfig.batched("video", keep_on_cpu=True),
                video_num_soft_tokens=MultiModalFieldConfig.batched("video", keep_on_cpu=True),
    ...
    def _get_prompt_updates(
        ...
        if "image" in mm_items:
            ...
            # Aphrodite tokenizes the prompt without token expansion, so only
            ...
            def get_replacement_image(item_idx: int):
                ...
                val, _ = _get_max_soft_tokens(merged_kwargs)
                max_soft_tokens = val if isinstance(val, int) and val in _SUPPORTED_SOFT_TOKENS else None
            ...
            def get_replacement_video(item_idx: int):
                ...
                for count in num_soft_data:
                    if not isinstance(count, int):
                        raise TypeError("each video soft-token count must be an integer")
...
class Gemma4MultimodalEmbedder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        embedding_dim = getattr(multimodal_config, "output_proj_dims", None) or multimodal_config.hidden_size
...
class Gemma4ForConditionalGeneration(
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.model_dtype = aphrodite_config.model_config.dtype
        ...
        if quant_config and quant_config.get_name() in [
            ...
            quantizable = vision_cfg.hidden_size % 64 == 0 and vision_cfg.intermediate_size % 64 == 0
        ...
        # ---- Language model (Aphrodite optimised) ----
        with self._mark_language_model(aphrodite_config):
            self.language_model: Gemma4ForCausalLM = init_aphrodite_registered_model(
        ...
        gen_cfg = aphrodite_config.model_config.try_get_generation_config()
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Gemma4ImageInputs | None:
    ...
    def _parse_and_validate_audio_input(self, **kwargs: object) -> Gemma4AudioInputs | None:
    ...
    def _parse_and_validate_video_input(self, **kwargs: object) -> Gemma4VideoInputs | None:
    ...
    def _parse_and_validate_multimodal_inputs(
        ...
        mm_input_by_modality: dict[str, Gemma4ImageInputs | Gemma4AudioInputs | Gemma4VideoInputs | None] = {}
    ...
    def _process_image_input(
        ...
        total_images = len(pixel_values) if isinstance(pixel_values, list) else pixel_values.shape[0]
        ...
        for patches, items in buckets.items():
            if self._enable_mm_lora:
                ...
                max_batch_size = min(
                    len(items),
                    self._encoder_chunk(patches, free, total, vision_cfg.position_embedding_size),
                )

            for chunk_idx in range(0, len(items), max_batch_size):
                chunk_items = items[chunk_idx : chunk_idx + max_batch_size]

                pv_tensor = torch.cat([item[1].unsqueeze(0) for item in chunk_items], dim=0)
                pp_tensor = torch.cat([item[2].unsqueeze(0) for item in chunk_items], dim=0)
        ...
        flat_proj_embs = self.embed_vision(inputs_embeds=flat_valid_states.unsqueeze(0)).squeeze(0)
    ...
    def _process_video_input(
        ...
        frame), matching the flat_from_sizes grouping that Aphrodite expects
        ...
        flat_valid_states = torch.cat(all_frame_valid_states, dim=0).to(self.model_dtype)
        flat_proj_embs = self.embed_vision(inputs_embeds=flat_valid_states.unsqueeze(0)).squeeze(0)
    ...
    def get_encoder_cudagraph_config(self) -> "EncoderCudaGraphConfig":
        from aphrodite.v1.worker.encoder_cudagraph_defs import EncoderCudaGraphConfig
        ...
        if modality == "image":
            ...
            if len(indices) == 0:
                ...
                return {
                    ...
                    "pixel_position_ids": ([] if is_pp_list else pixel_position_ids[:0]),
                }
            if isinstance(pixel_values, list):
                return {
            ...
            if len(indices) == 0:
                ...
                return {
                    ...
                    "video_frame_counts": (video_frame_counts[:0] if is_fc_tensor else []),
                }
            ...
            selected_pv = torch.cat(
                [pixel_values_videos[cum_frames[i] : cum_frames[i + 1]] for i in indices],
                dim=0,
            )
            selected_pp = torch.cat(
                [pixel_position_ids_videos[cum_frames[i] : cum_frames[i + 1]] for i in indices],
                dim=0,
            )
            selected_fc = (
        ...
        if modality == "image":
            ...
            for fc in fc_list:
                for f in range(fc):
                    ...
                    gather_indices[dst_offset : dst_offset + safe_frame_output_tokens] = torch.arange(
                        src_start, src_end, dtype=torch.long, device=device
                    )
    ...
    def encoder_cudagraph_forward(
        ...
        **kwargs: Any,
        ...
        flat_proj_embs = self.embed_vision(inputs_embeds=gathered_states.unsqueeze(0)).squeeze(0)
    ...
    def embed_input_ids(
        ...
        *,
        ...
        if self.per_layer_embeddings is not None:
            ...
            per_layer_inputs = self.language_model.model.get_per_layer_inputs(ple_input_ids)
    ...
    def compute_logits(
        ...
        if logits is not None and self._suppress_token_ids:
            ...
            if suppress_idx is None:
                suppress_idx = async_tensor_h2d(self._suppress_token_ids, dtype=torch.long, device=logits.device)
        ...
        *,
        ...
        if modality in ("image", "video"):
            ...
            if tower_tokens is None:
                ...
                tower_tokens = math.ceil(num_mm_embeds / min_soft_tokens) * max_soft_tokens * pooling_k2
