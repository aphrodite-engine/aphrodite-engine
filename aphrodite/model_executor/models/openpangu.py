...
class OpenPanguModel(nn.Module):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(
            ...
            lambda prefix: OpenPanguDecoderLayer(config, prefix, aphrodite_config),
    ...
    def _filter_spec_layers(self, weights: Iterable[tuple[str, torch.Tensor]]) -> Iterable[tuple[str, torch.Tensor]]:
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        if any(".fused_qkv_a_proj." in name for name in param_names):
            stacked[".q_a_proj"] = (".fused_qkv_a_proj", 0)
            stacked[".kv_a_proj_with_mqa"] = (".fused_qkv_a_proj", 1)
        if any(".qkv_proj." in name for name in param_names):
