...
from aphrodite.model_executor.models.transformers.fuser import BaseFuser, Fusers
...
class Base(
    ...
    def __init__(self, *, aphrodite_config: "AphroditeConfig", prefix: str = ""):
        ...
        self._create_hf_to_aphrodite_mapper()
        # Remove layers not on this pipeline parallel rank
        self.pipeline_parallel()
        # Substitute remaining layers with Aphrodite's layers as needed
    ...
    def _patch_config(self):
        ...
        - Sets the attention implementation to "aphrodite" so the attention instances from
    ...
    def _decorate_cls_for_torch_compile(
        ...
        enable_if: Callable[["AphroditeConfig"], bool],
        is_encoder: bool,
    ):
        """
        Decorate `cls` to indicate to Aphrodite that it supports torch compile.
        ...
        Args:
            ...
            dynamic_arg_dims: A mapping from argument name to the dynamic dimensions
                ...
                [`support_torch_compile`][aphrodite.compilation.decorators.support_torch_compile]
                for more details.
            enable_if: A function which takes in the Aphrodite config and returns whether
    ...
    def _decorate_for_torch_compile(self, **kwargs: dict):
        """
        Decorate the model's decoder class to indicate to Aphrodite that it supports torch
    ...
    def _create_hf_to_aphrodite_mapper(self):
        """
        ...
        orig_to_new_renaming = self.hf_to_aphrodite_mapper.orig_to_new_renaming
    ...
    def recursive_replace(self):
        ...
        - `nn.Linear` with Aphrodite's tensor parallel linear classes
        - `nn.Conv2d` / `nn.Conv3d` with Aphrodite's `Conv2d` / `Conv3d`
        - RMSNorm (detected from their dataflow) with Aphrodite's `RMSNorm` or
          `GemmaRMSNorm`
        """
        ...
        fusers = Fusers(self.model, self.aphrodite_config)
        ...
        def register_fusion(fuser: BaseFuser, prefix: str):
            ...
            self.hf_to_aphrodite_mapper.orig_to_new_stacked.update(orig_to_new_stacked)
        ...
        def _recursive_replace(module: nn.Module, prefix: str):
            for child_name, child_module in module.named_children():
                ...
                if isinstance(child_module, nn.Linear):
                    ...
                    new_module = fuser.fuse(child_module, qual_name, self.aphrodite_config)
    ...
    def create_attention_instances(self) -> dict[int, Attention]:
        ...
        for i in range(start, end):
            ...
            attention_instances[i] = attn_cls(
                ...
                # Transformers, it's updated in aphrodite_attention_forward
                scale=head_size**-0.5,
