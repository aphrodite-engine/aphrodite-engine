...
from aphrodite.model_executor.models.transformers.fusers import AttentionFuser, MLAFuser
from aphrodite.model_executor.models.transformers.fusers.attention import (
    APHRODITE_ATTN_IMPL,
    APHRODITE_MLA_ATTN_IMPL,
)
...
APHRODITE_ATTN_ATTR = "attn"
"""Attribute in which to store the Aphrodite `Attention` instance that serves a Transformers
attention module. `"attn"` is chosen deliberately to match the mappings generated by
...
class Base(
    ...
    def __init__(self, *, aphrodite_config: "AphroditeConfig", prefix: str = ""):
        ...
        with (
            self._mark_model_components(aphrodite_config),
    ...
    def _patch_config(self):
        ...
        """
        self.text_config._attn_implementation = APHRODITE_ATTN_IMPL
    ...
    def _decorate_for_torch_compile(self):
        """Decorate the model's decoder class to indicate to Aphrodite that it
    ...
    def _vocab_embeddings(self) -> set[nn.Embedding]:
        ...
        embeddings = {
            module for module in self.model.get_input_embeddings().modules() if isinstance(module, nn.Embedding)
        }
        embeddings.update(
            module
    ...
    def recursive_replace(self):
        ...
        - Vocab `nn.Embedding`s with Aphrodite's `VocabParallelEmbedding`
        - RMSNorm (detected from their dataflow) with Aphrodite's `RMSNorm`or `GemmaRMSNorm`
        """
        ...
        def register_fusion(fuser: BaseFuser, prefix: str, module: nn.Module):
            ...
            if isinstance(fuser, AttentionFuser) and (index := fuser.layer_index(module)) is not None:
                if index in self.attention_fusers:
                    other, _ = self.attention_fusers[index]
                    raise ValueError(
                        f"Layer {index} dispatches attention from more than one "
                        f"module ({other} and {prefix}), which one Aphrodite attention "
                        "layer cannot serve."
                    )
        ...
        def _recursive_replace(module: nn.Module, prefix: str):
            for child_name, child_module in module.named_children():
                ...
                if isinstance(child_module, nn.Linear):
                    ...
                    new_module = replace_embedding_class(child_module, self.quant_config, prefix=qual_name)
                elif child_module_fusers := fusers[child_module]:
                    for fuser in child_module_fusers:
                        register_fusion(fuser, qual_name, child_module)
                        new_module = fuser.fuse(new_module, qual_name, self.aphrodite_config)
    ...
    def create_attention_instances(self) -> dict[int, Attention]:
        ...
        if getattr(text_config, "kv_lora_rank", None) is not None:
            if attn_cls is MLAAttention:
                text_config._attn_implementation = APHRODITE_MLA_ATTN_IMPL
            else:
        ...
        for i in range(start, end):
            if i not in self.attention_fusers:
                ...
                raise ValueError(
                    ...
                    "attention interface and Aphrodite has no other way to handle it."
                )
            ...
            num_heads = self.model_config.get_num_attention_heads(self.parallel_config, arch_config)
            ...
            if scale is None:
                scale = head_size**-0.5
            num_kv_heads = self.model_config.get_num_kv_heads(self.parallel_config, arch_config)
            ...
            if attn_cls is MLAAttention:
                mla_fuser = next((f for f in self.fusers[prefix] if isinstance(f, MLAFuser)), None)
                ...
                kwargs.update(
                    ...
                    kv_b_proj=attn_module.get_submodule(mla_fuser.kv_b_proj_name),
                )
            else:
                kwargs.update(
                    head_size=head_size,
            ...
            # Attach the instance to the module it serves, which is the only way it
            # is in the module tree at all: a plain dict is invisible to
            # `nn.Module.__setattr__`. Being in the tree runs its
            # `process_weights_after_loading` and moves its KV-scale buffers with
            # `model.to(device)`, and lets the attention interface read it off the
            # module rather than index by `module.layer_idx`, an `int` that bakes
            # layer identity into the traced graph and so costs one compiled
            # artifact per layer.
            setattr(attn_module, APHRODITE_ATTN_ATTR, attn_instance)
    ...
    def _get_attn_cls(self) -> type[AttentionLayerBase]:
        ...
        if self.model_config.use_mla:
            ...
            if any(isinstance(fuser, MLAFuser) for fusers in self.fusers.values() for fuser in fusers):
                return MLAAttention
            logger.warning_once(
            ...
            os.environ["APHRODITE_MLA_DISABLE"] = "1"
        return Attention
