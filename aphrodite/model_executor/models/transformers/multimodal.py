...
from aphrodite.model_executor.models.interfaces import SupportsMRoPE, SupportsMultiModal
...
class MultiModalProcessor(BaseMultiModalProcessor[MultiModalProcessingInfo]):
    ...
    def _get_mm_fields_config(
        ...
        if self.info._is_audio_model():
            ...
            mm_fields.update(
                {key: MultiModalFieldConfig.flat_from_sizes("audio", num_audio_tokens) for key in hf_inputs}
            )
        ...
        if self.info._is_image_model():
            ...
            mm_fields.update(
                {key: MultiModalFieldConfig.flat_from_sizes("image", num_image_patches) for key in hf_inputs}
            )
            ...
            mm_fields["num_image_patches"] = MultiModalFieldConfig.batched("image", keep_on_cpu=True)
        return mm_fields
...
class MultiModalMixin(SupportsMultiModal, SupportsMRoPE):
    ...
    def _embed_audio(self, **kwargs) -> list[torch.Tensor] | None:
        ...
        if current_platform.is_rocm():
            context = torch.nn.attention.sdpa_kernel(backends=[torch.nn.attention.SDPBackend.MATH])
        with context:
    ...
    def embed_multimodal(self, **kwargs):
        ...
        if "pixel_values" in kwargs or "image_embeds" in kwargs or "image_patches" in kwargs:
