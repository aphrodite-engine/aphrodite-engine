...
from aphrodite.compilation.decorators import should_torch_compile_mm_encoder
...
class MultiModalProcessingInfo(BaseProcessingInfo):
    def _get_audio_processor(self) -> Any:
        ...
        return getattr_iter(self.get_hf_processor(), ("audio_processor", "feature_extractor"))
...
class _MultiModalProcessorBase(BaseMultiModalProcessor[MultiModalProcessingInfo]):
    ...
    def _get_mm_fields_config(
        ...
        sizes = {modality: hf_inputs.get(key) for modality, key in _MODALITY_SIZE_KEYS.items()}
        ...
        own_keys = set(_MODALITY_SIZE_KEYS.values()) | {
            f"{modality}_replacement_{suffix}" for modality in modalities for suffix in ("ids", "sizes")
        }
        ...
        for modality in modalities:
            ...
            mm_fields[_MODALITY_SIZE_KEYS[modality]] = MultiModalFieldConfig.batched(modality, keep_on_cpu=True)
            replacement_sizes = hf_inputs.get(f"{modality}_replacement_sizes")
            if replacement_sizes is not None:
                mm_fields[f"{modality}_replacement_ids"] = MultiModalFieldConfig.flat_from_sizes(
                    modality, replacement_sizes, keep_on_cpu=True
                )

        if "image" in modalities:
            # Always one row per item, whatever they describe
            mm_fields["image_grid_thw"] = MultiModalFieldConfig.batched("image", keep_on_cpu=True)
            # TODO: route to "video" once the video modality is supported
            mm_fields["video_grid_thw"] = MultiModalFieldConfig.batched("image", keep_on_cpu=True)
    ...
    def _unpad_audios(
        ...
        alone = [
            self.info.ctx.call_hf_processor(
                ...
                dict(text=self.dummy_inputs.get_dummy_text({"audio": 1}), audio=[audio]),
                mm_kwargs,
            )
            for audio in audios
        ]
    ...
    def _unpad_images(self, hf_inputs: "BatchFeature") -> None:
        ...
        hf_inputs["pixel_values"] = [
            image[..., :height, :width] for image, (height, width) in zip(pixel_values, image_sizes.tolist())
        ]
...
class LegacyMultiModalProcessor(_MultiModalProcessorBase):
    ...
    def _get_mm_token_ids(self, modality: str) -> list[int]:
        ...
        if token_id is None:
            raise ValueError(f"Cannot find {modality}_token_id on processor or model config")
        return [token_id]
    ...
    def _apply_audio(
        ...
        ranges = [
            PlaceholderRange(offset=offset.item(), length=length.item()) for offset, length in zip(offsets, lengths)
        ]
    ...
    def _get_num_multimodal_tokens(
        ...
        image_sizes = [(size.height, size.width) for size in map(images.get_image_size, range(len(images)))]
    ...
    def _apply_vision(
        ...
        mm_tokens_per_modality = self._get_num_multimodal_tokens(mm_items, hf_processor_mm_kwargs)
    ...
    def apply(
        ...
        with timing_ctx.record("apply_hf_processor"):
            ...
            try:
                ...
                processed_data = transformers.BatchFeature(dict(input_ids=[prompt_ids]), tensor_type="pt")
        ...
        if num_audios := mm_items.get_count("audio", strict=False):
            mm_placeholders.update(self._apply_audio(prompt_ids, processed_data, num_audios))
...
class OffsetsMultiModalProcessor(_MultiModalProcessorBase):
    ...
    def _get_num_patches_per_image(self, mm_data: Mapping[str, object]) -> list[int] | None:
    ...
    def _apply_hf_processor_main(
        ...
        try:
            ...
            # no data to go with them, so tokenize it without them
            tokenizer = self.info.get_tokenizer()
            hf_inputs = transformers.BatchFeature(dict(input_ids=[tokenizer.encode(prompt_text)]), tensor_type="pt")
        ...
        for entry in offsets[0]:
            replacements[entry["type"]].append(cached_encode(tokenizer, entry["replacement"], add_special_tokens=False))

        for modality, seqs in replacements.items():
            hf_inputs[f"{modality}_replacement_ids"] = torch.tensor([token_id for seq in seqs for token_id in seq])
            hf_inputs[f"{modality}_replacement_sizes"] = torch.tensor([len(seq) for seq in seqs])
            if modality == "image":
                hf_inputs["num_image_patches"] = self._get_num_image_patches(hf_inputs, mm_data, len(seqs))
            elif modality == "audio":
                counts = []
                for seq in seqs:
...
MultiModalProcessor = (
    LegacyMultiModalProcessor if Version(transformers.__version__) < Version("5.15.0") else OffsetsMultiModalProcessor
)
...
class MultiModalMixin(SupportsMultiModal, SupportsMRoPE):
    ...
    def _find_encoder_classes(self, model: "PreTrainedModel") -> dict[str, type["PreTrainedModel"]]:
    ...
    @contextmanager
    def _mark_model_components(self, aphrodite_config: "AphroditeConfig"):
        ...
        if model_config.skip_tokenizer_init:
            ...
            if mm_config.mm_encoder_only or any(
                mm_config.get_limit_per_prompt(modality) == 0 for modality in encoder_classes
            ):
                logger.warning_once(
        ...
        supported_modalities = MULTIMODAL_REGISTRY.get_processing_info(model_config).supported_mm_limits
        ...
        with ExitStack() as stack:
            stack.enter_context(
                self._mark_language_model(aphrodite_config, targets=self._pre_trained_model_classes.decoder)
            )
            for encoder_cls, modalities in modalities_by_encoder.items():
                stack.enter_context(self._mark_tower_model(aphrodite_config, modalities, targets=encoder_cls))
            yield

    def _decorate_for_torch_compile(self):
        """
        Decorate the model's decoder and encoder classes to indicate to Aphrodite that they
    ...
    def get_language_model(self) -> torch.nn.Module:
        ...
        Aphrodite language model class. Therefore, in order to return a language model Aphrodite
        ...
        # Keep only classes defined in `aphrodite.model_executor.models.transformers`
    ...
    def get_mm_mapping(self) -> MultiModelKeys:
        ...
        raise ValueError("Could not locate the language model submodule for LoRA support")

    def _split_embeddings(self, embeddings: torch.Tensor, split_sizes: list[int]) -> list[torch.Tensor]:
    ...
    def _process_audio_input(self, **kwargs) -> list[torch.Tensor] | None:
        ...
        if isinstance(input_features, torch.Tensor):
            ...
            with gpu_sync_allowed():
                audio_output = self.model.get_audio_features(input_features, return_dict=True, **kwargs)
        ...
        for index, features in enumerate(input_features):
            ...
            embeddings.extend(self._split_embeddings(audio_output.pooler_output, [split_sizes[index]]))
        return embeddings
    ...
    def _select_item_kwargs(self, kwargs: dict[str, Any], index: int, num_items: int) -> dict[str, Any]:
