...
"""The unpatched `nn.Module.__call__`, used for meta execution."""
...
def _reference_weight(module: nn.Module) -> torch.Tensor | None:
    """A weight whose trailing dimension is the module's hidden size."""
...
class _MetaAttribute(_MetaProxy, fx.proxy.Attribute):
    """Attribute proxy carrying its inferred meta value."""
...
class _AllLeafTracer(fx.Tracer):
    ...
    def _infer_meta(self, kind: str, target: object, args: tuple, kwargs: dict):
        ...
        if kind == "call_module":
            ...
            state = {
                ...
                for name, tensor in chain(child.named_parameters(), child.named_buffers())
            }
    ...
    def _is_varkw(self, node: object) -> bool:
        return isinstance(node, fx.Node) and node.op == "placeholder" and str(node.target).lstrip("*") == self.varkw

    def iter(self, obj: fx.Proxy):
        # Assume kwargs is always empty to simplify tracing.
        node = obj.node
        if self._is_varkw(node) or (
            node.op == "call_method" and node.target == "keys" and self._is_varkw(node.args[0])
        ):
            return iter(())
...
def trace(module: nn.Module) -> fx.Graph | None:
    ...
    tracer.varkw = next(
        (p.name for p in parameters.values() if p.kind is inspect.Parameter.VAR_KEYWORD),
        None,
    )
    try:
...
def single_self_call(funcdef: ast.FunctionDef, name: str) -> ast.Call:
    ...
    """
    uses = [node for node in ast.walk(funcdef) if isinstance(node, ast.Attribute) and node.attr == name]
    if len(uses) != 1:
    ...
    if len(calls) != 1 or not isinstance(uses[0].value, ast.Name) or uses[0].value.id != "self":
        raise ValueError(f"{name} is not a single-argument call on self")
    return calls[0]


def innermost_block(block: list[ast.stmt], node: ast.AST) -> tuple[list[ast.stmt], int] | None:
    ...
    for index, stmt in enumerate(block):
        if not any(child is node for child in ast.walk(stmt)):
            continue
        child_blocks = [getattr(stmt, fld, None) for fld in ("body", "orelse", "finalbody")]
...
def is_linear(node: fx.Node, module: nn.Module) -> bool:
    ...
    return node.op == "call_module" and isinstance(module.get_submodule(node.target), nn.Linear)
...
def peel(node: object) -> object:
    ...
    while isinstance(node, fx.Node) and node.op == "call_method" and node.target in _DTYPE_CASTS:
        node = node.args[0]
    return node
...
def is_fn(node: object, target: Callable) -> bool:
    """Is node `<target>()`."""
    return isinstance(node, fx.Node) and node.op == "call_function" and node.target is target


def is_method(node: object, name: str) -> bool:
    """Is node `.<name>()`."""
    return isinstance(node, fx.Node) and node.op == "call_method" and node.target == name
...
def is_op(node: object, name: str) -> bool:
    ...
    """
    return any(is_fn(node, getattr(module, name, None)) for module in (torch, F, operator)) or (
        hasattr(torch.Tensor, name) and is_method(node, name)
    )
