...
from aphrodite.model_executor.models.utils import extract_layer_index, maybe_prefix
from aphrodite.utils.torch_utils import STR_DTYPE_TO_TORCH_DTYPE, direct_register_custom_op
...
class TransformersMoERunner(MoERunner):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        return torch.ops.aphrodite.transformers_moe_forward(
            hidden_states,
...
class TransformersRoutedExperts(RoutedExperts):
    def get_expert_mapping(self, include_fused: bool = False) -> list[tuple[str, str, int, str]]:
        ...
        def _recursive_replace(module: nn.Module, prefix: str):
            for child_name, child_module in module.named_children():
                ...
                if child_name == experts_name and (is_modulelist or is_3d):
                    ...
                    layer_idx = extract_layer_index(qual_name) if any(isinstance(v, list) for v in per_layer) else 0
                    ...
                    if reaches_fp16_trick:
                        logger.warning_once(
                            "%s could be fused but routing it in Aphrodite would apply "
                    ...
                    if fuser is not None and not reaches_fp16_trick:
                        ...
                        kwargs |= dict(
                            ...
                            is_sequence_parallel=(self.parallel_config.use_sequence_parallel_moe),
                            gate=gate,
