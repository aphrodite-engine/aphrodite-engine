...
from aphrodite.model_executor.models.transformers.layers import (
    GemmaRMSNorm,
    RMSNorm,
)
...
class RMSNormFuser(BaseFuser):
    ...
    @classmethod
    def _eps_source(cls, graph: fx.Graph, module: nn.Module) -> tuple[str | None, float | None]:
        ...
        candidates = {name: value for name, value in vars(module).items() if isinstance(value, float) and value == eps}
    ...
    def fuse(self, module: nn.Module, prefix: str, aphrodite_config: "AphroditeConfig") -> nn.Module:
        ...
        if not isinstance(eps, (int, float)):
            ...
            dtype = weight.dtype if has_weight else aphrodite_config.model_config.dtype
