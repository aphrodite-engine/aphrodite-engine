...
"""MLA fuser: adapt a Transformers MLA attention module for Aphrodite's MLA layer."""
...
def _consumes_placeholder(node: fx.Node) -> bool:
    ...
    return len(node.args) == 1 and isinstance(node.args[0], fx.Node) and node.args[0].op == "placeholder"
...
def _single_expand_call(funcdef: ast.FunctionDef, module: nn.Module, kv_b_proj_name: str) -> ast.Call:
    ...
    def is_expansion_method(name: str) -> bool:
        ...
        return bool(returns) and all(isinstance(ret.value, ast.Tuple) and len(ret.value.elts) == 2 for ret in returns)
...
class MLAFuser(StackedFuser):
    ...
    def info(self, name: str) -> str:
        info_str = f"Fused: {name} ({self.source_cls}) -> MLAAttention (attention interface)"
        if self.has_q_lora:
    ...
    def match(cls, graph: fx.Graph, module: nn.Module) -> "MLAFuser | None":
        ...
        if q_a_chains:
            ...
            placeholder_linears = {
                node.target for node in graph.nodes if is_linear(node, module) and _consumes_placeholder(node)
            }
    ...
    def validate(self, module: nn.Module, aphrodite_config: "AphroditeConfig") -> bool:
        return aphrodite_config.model_config.use_mla

    def update_forward(self, module: nn.Module) -> None:
        """Merge `q_a_proj` and `kv_a_proj` into one fused down proj then split.
        Bypass the KV expansion method so the compressed latent reaches the `aphrodite_mla`
        ...
        if self.has_q_lora:
            ...
            placeholder = next(node for node in ast.walk(assign) if isinstance(node, ast.Name) and node.id == "__arg__")
        ...
        replace_expr(funcdef, expand_call, ast.Tuple(elts=list(expand_call.args), ctx=ast.Load()))
        self.fused_forward = compile_forward(funcdef, fn)

    def update_attrs(self, module: nn.Module, prefix: str, aphrodite_config: "AphroditeConfig"):
        quant_config = aphrodite_config.quant_config

        def replace_linear_by_name(name: str, style: str):
            linear = module.get_submodule(name)
            replacement = replace_linear_class(linear, style, quant_config, prefix=maybe_prefix(prefix, name))
        ...
        # MLAAttention calls kv_b_proj and expects Aphrodite's default return_bias=True
