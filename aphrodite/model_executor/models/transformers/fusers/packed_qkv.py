...
class PackedQKVFuser(RewriteFuser):
    ...
    def info(self, name: str) -> str:
        return f"Fused: {self.qkv_name} ({name}: {self.source_cls}) -> QKVParallelLinear"
    ...
    def match(cls, graph: fx.Graph, module: nn.Module) -> "PackedQKVFuser | None":
        for node in graph.nodes:
            ...
            if o_name == qkv_name or (o_name is not None and module.get_submodule(o_name).in_features != q_size):
                o_name = None
            return cls(
    ...
    def _split_call(self, funcdef: ast.FunctionDef) -> ast.Call:
        ...
        calls = [
            ...
            and any(
                isinstance(inner, ast.Attribute) and inner.attr == self.qkv_name for inner in ast.walk(node.func.value)
            )
        ]
        if len(calls) != 1:
    ...
    def update_attrs(self, module: nn.Module, prefix: str, aphrodite_config: "AphroditeConfig") -> None:
        ...
        if self.o_name is not None:
            ...
            new_o = replace_linear_class(o_proj, "rowwise", quant_config, prefix=o_prefix)
