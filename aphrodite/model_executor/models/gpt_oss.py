...
class GptOssRoutedExperts(RoutedExperts):
    ...
    def _load_expert_bias(
        ...
        if shard_id == "gpt_oss_w13":
            loaded_weight = self._narrow_for_rank(loaded_weight, 0, tp_rank, expert_data.shape[0])
        elif tp_rank != 0:
    ...
    def _load_unquantized_expert(
        ...
        tp_rank = self.moe_config.moe_parallel_config.tp_rank
        if shard_id == "gpt_oss_w13":
            loaded_weight = self._narrow_for_rank(loaded_weight, 1, tp_rank, expert_data.shape[0])
            loaded_weight = loaded_weight.t().contiguous()
        else:
            loaded_weight = self._narrow_for_rank(loaded_weight, 0, tp_rank, expert_data.shape[1])
            loaded_weight = loaded_weight.t().contiguous()
        self._copy_to_expert(expert_data, loaded_weight)
    ...
    def _load_packed_expert(
        ...
        if is_w13 and (is_weight or is_partitioned_scale):
            ...
            loaded_weight = self._narrow_for_rank(loaded_weight, -1, tp_rank, expert_data.shape[-1])
...
class MLPBlock(torch.nn.Module):
    def __init__(
        ...
        self.is_sequence_parallel = parallel_config.use_sequence_parallel_moe and aphrodite_config.lora_config is None
...
class GptOssModel(nn.Module, EagleModelMixin):
    ...
    def _load_weights_quark(
        ...
        for name, loaded_weight in weights:
            ...
            if "experts" in name:
                ...
                fused_name = fused_name.replace(".mlp.experts.", ".mlp.experts.routed_experts.")
    ...
    def _get_streamed_expert_info(
        ...
        for base_layer_prefix in ("", "base_layer."):
            fused_name = f"{prefix}.mlp.experts.{base_layer_prefix}routed_experts.{param_suffix}"
