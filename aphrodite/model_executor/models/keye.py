...
class KeyeSiglipVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        split_sizes = [int(np.prod(thw)) for thw in self.encoder.flatten_list(image_grid_thw)]
        return [tensor.squeeze(0) for tensor in last_hidden_state.split(split_sizes, dim=1)]
...
class BaseKeyeModule(nn.Module, SupportsMultiModal):
    ...
    def _process_video_embeds(
        ...
        if video_type == "video_embeds":
            ...
            cu_seqlens = async_tensor_h2d(cu_seqlens, dtype=torch.int32, device=pixel_values_videos.device)
            sample_indices = torch.concat(
                sample_indices,
                dim=0,
                out=torch.empty(
