...
class LagunaDecoderLayer(nn.Module):
    def __init__(
        ...
        self.self_attn = LagunaAttention(
            ...
            layer_idx=layer_idx,
            attention_prefix=(f"{attention_prefix}.self_attn" if attention_prefix is not None else None),
        )
...
class LagunaModel(nn.Module, EagleModelMixin):
    ...
    def _slice_sink(self, weights: Iterable[tuple[str, torch.Tensor]]) -> Iterable[tuple[str, torch.Tensor]]:
        tp_size = get_tensor_model_parallel_world_size()
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        return loader.load_weights(self._slice_sink(weights), mapper=self.hf_to_aphrodite_mapper)
