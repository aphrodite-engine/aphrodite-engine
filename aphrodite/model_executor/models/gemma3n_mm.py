...
from aphrodite.model_executor.models.whisper import ISO639_1_SUPPORTED_LANGS
...
def batch_audio_features(
    ...
    batched_features = input_features[0].new_zeros((len(input_features), max_len, input_features[0].shape[-1]))
    batched_mask = input_features_mask[0].new_zeros((len(input_features_mask), max_len), dtype=torch.bool)
    for i, (features, mask) in enumerate(zip(input_features, input_features_mask, strict=True)):
...
class Gemma3nProcessingInfo(BaseProcessingInfo):
    ...
    def get_image_repl(
        ...
        *,
        ...
        """
        full_token_ids = cached_encode(processor.tokenizer, processor.full_image_sequence, add_special_tokens=False)
        return PromptUpdateDetails.select_token_id(full_token_ids, processor.image_token_id)
    ...
    def get_audio_repl(
        ...
        *,
        ...
        full_token_ids = cached_encode(processor.tokenizer, processor.full_audio_sequence, add_special_tokens=False)
        return PromptUpdateDetails.select_token_id(full_token_ids, processor.audio_token_id)
    ...
    def _postprocess_hf_mm_data(
        ...
        if "input_features" in processed_data:
            ...
            processed_data["input_features_padded"] = [
                features[:n] for features, n in zip(processed_data.pop("input_features"), num_frames)
            ]
            processed_data["input_features_mask"] = [
                mask[:n] for mask, n in zip(processed_data["input_features_mask"], num_frames)
            ]
...
class Gemma3nForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsTranscription):
    ...
    def _parse_and_validate_multimodal_inputs(self, **kwargs: object) -> dict:
        mm_input_by_modality: dict[str, Gemma3nImageInputs | Gemma3nAudioInputs | None] = {}
    ...
    def get_generation_prompt(cls, stt_params: SpeechToTextParams) -> PromptType:
        ...
        full_lang_name = cls.supported_languages.get(language, "") if language is not None else ""
        # Translation only for now
        full_lang_name_to = cls.supported_languages.get(to_language, "") if to_language is not None else ""
