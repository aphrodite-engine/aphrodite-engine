...
from aphrodite.model_executor.models.qwen2_audio import (
...
def merge_interleaved_embeddings(
    ...
    """
    from aphrodite.multimodal.utils import get_mm_embedding_modalities
    ...
    def _merge_embedding_group(
        ...
        if num_actual_tokens != num_expected_tokens:
            raise ValueError(
                f"Attempted to assign {num_actual_tokens} {modality} tokens to {num_expected_tokens} placeholders"
            )
        if embeddings:
    ...
    video_embeds = [
        embedding for embedding, modality in zip(multimodal_embeddings, embedding_modalities) if modality == "video"
    ]
    audio_embeds = [
        embedding for embedding, modality in zip(multimodal_embeddings, embedding_modalities) if modality == "audio"
    ]
    image_embeds = [
        embedding for embedding, modality in zip(multimodal_embeddings, embedding_modalities) if modality == "image"
    ]
...
def create_qwen2_5_omni_thinker_field_factory(
    ...
    def _qwen2_5_omni_thinker_field_config(hf_inputs: Mapping[str, torch.Tensor]):
        ...
        return dict(
            ...
            use_audio_in_video=MultiModalFieldConfig.shared("video", num_videos, keep_on_cpu=True),
        )
    ...
    def _parse_audio_data(
        ...
        if isinstance(data, dict):
            return DictEmbeddingItems(
                ...
                fields_factory=create_qwen2_5_omni_thinker_field_factory(self._spatial_merge_size),
            )

        return super()._parse_audio_data(data)


class Qwen2_5OmniThinkerProcessingInfo(Qwen2AudioProcessingInfo, Qwen2_5_VLProcessingInfo):
...
class Qwen2_5OmniThinkerDummyInputsBuilder(BaseDummyInputsBuilder[Qwen2_5OmniThinkerProcessingInfo]):
...
class Qwen2_5OmniThinkerMultiModalProcessor(BaseMultiModalProcessor[Qwen2_5OmniThinkerProcessingInfo]):
    def _get_mm_fields_config(
        self,
        hf_inputs: BatchFeature,
        hf_processor_mm_kwargs: Mapping[str, object],
    ) -> Mapping[str, MultiModalFieldConfig]:
        return create_qwen2_5_omni_thinker_field_factory(self.info.get_hf_config().vision_config.spatial_merge_size)(
            hf_inputs
        )
    ...
    def _apply_hf_processor_main(
        ...
        if use_audio_in_video and "video" in mm_counts:
            if mm_counts.get("audio", 0) < mm_counts["video"]:
                raise ValueError("Video doesn't have audio track with `audio_in_video=True`")
        ...
        elif hf_processor_mm_kwargs.get("use_audio_in_video") and "audio" not in mm_data and mm_data.get("videos"):
        ...
        if mm_data.get("videos") and (merged.keys() & {"size", "min_pixels", "max_pixels"}):
...
class Qwen2_5OmniConditionalGenerationMixin:
    ...
    def _process_audio_input(
        ...
        audio_feature_lengths = async_tensor_h2d(audio_input["audio_feature_lengths"], input_features.device)
...
class Qwen2_5OmniThinkerForConditionalGeneration(
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        thinker_config: Qwen2_5OmniThinkerConfig = aphrodite_config.model_config.hf_config.thinker_config
