...
from aphrodite.model_executor.determinism.batch_invariant_configs import (
...
def matmul_kernel_descriptor_persistent(
    ...
    for tile_id in tl.range(start_pid, num_tiles, NUM_SMS, flatten=True):
        ...
        if HAS_BIAS:
            ...
            bias_vals = tl.load(bias_ptr + bias_offs, mask=bias_offs < N, other=0.0).to(tl.float32)
...
def matmul_descriptor_persistent(a: torch.Tensor, b: torch.Tensor, bias: torch.Tensor | None = None) -> torch.Tensor:
...
def _rms_norm_kernel(
    ...
    # Step 3: Normalize and optionally apply weight
    for col_offset in range(0, n_cols, BLOCK_SIZE):
        col_idx = col_offset + tl.arange(0, BLOCK_SIZE)
        mask = col_idx < n_cols
        vals = tl.load(row_start_ptr + col_idx, mask=mask, other=0.0)
