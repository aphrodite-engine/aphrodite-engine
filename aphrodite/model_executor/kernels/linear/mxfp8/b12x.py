...
from aphrodite.utils.b12x import B12xWarmupUnit, reuse_packed_weight_storage
...
class B12xMxfp8LinearKernel(Mxfp8LinearKernel):
    ...
    def is_supported(
        ...
        mxfp8 = _import_b12x_mxfp8()
        if mxfp8 is None:
            return False, "Install the B12X backend with `pip install aphrodite[b12x]`"
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        assert weight.dtype == MXFP8_VALUE_DTYPE, f"b12x MXFP8 requires {MXFP8_VALUE_DTYPE}, got {weight.dtype}"
        ...
        assert in_features % MXFP8_BLOCK_SIZE == 0, (
            f"b12x MXFP8 requires input features divisible by {MXFP8_BLOCK_SIZE}, got {in_features}"
        )
        weight_scale = layer.weight_scale.data
        assert weight_scale.dtype == MXFP8_SCALE_DTYPE, (
            f"b12x MXFP8 requires {MXFP8_SCALE_DTYPE} weight_scale, got {weight_scale.dtype}"
        )
        assert weight_scale.ndim == 2, f"b12x MXFP8 weight_scale must be 2D, got {weight_scale.ndim}D"
