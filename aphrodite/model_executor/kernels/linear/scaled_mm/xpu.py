...
class XPUFp8BlockScaledMMKernel(Fp8BlockScaledMMLinearKernel):
    ...
    def _prepare_bmm_params(self, layer: torch.nn.Module, scale_kn: torch.Tensor) -> None:
        ...
        layer.bmm_scale = scale_kn.reshape(k_blocks, batch, n_blocks // batch).permute(1, 0, 2).contiguous()
        w = layer.weight
        N_total, K = w.shape
        layer.bmm_weight = w.reshape(batch, N_total // batch, K).permute(0, 2, 1)  # [G, K, N_per_group]
