...
class AiterInt8ScaledMMLinearKernel(CutlassInt8ScaledMMLinearKernel):
    ...
    def is_supported(cls, compute_capability: int | None = None) -> tuple[bool, str | None]:
        ...
        if not rocm_aiter_ops.is_linear_enabled():
            return (
                False,
                "requires setting `APHRODITE_ROCM_USE_AITER=1` "
                "and `APHRODITE_ROCM_USE_AITER_LINEAR=1`. "
                "`APHRODITE_ROCM_USE_AITER_LINEAR` default is True.",
            )
        return True, None
    ...
    def is_supported(cls, compute_capability: int | None = None) -> tuple[bool, str | None]:
        ...
        if not rocm_aiter_ops.is_linear_hipbmm_enabled():
            return (
                False,
                "requires setting `APHRODITE_ROCM_USE_AITER=1`, "
                "`APHRODITE_ROCM_USE_AITER_LINEAR=1`, "
                "and `APHRODITE_ROCM_USE_AITER_LINEAR_HIPBMM=1`.",
            )
        try:
            import aiter  # noqa: F401
        except Exception:
...
class AiterFp8BlockScaledMMKernel(Fp8BlockScaledMMLinearKernel):
    ...
    def is_supported(cls, compute_capability=None):
        if rocm_aiter_ops.is_linear_enabled() or rocm_aiter_ops.is_rdna_linear_enabled():
            return True, None
        return (
            False,
