    ...
    def get_b12x_warmup_unit(
        ...
        def compile() -> None:
            for tokens in token_counts:
                ...
                _run_b12x_fp8_block_scaled_mm(a, weight, a_scale, weight_scale, output_dtype)
...
class B12xTensorFP8ScaledMMLinearKernel(FP8ScaledMMLinearKernel):
    ...
    def can_implement(
        ...
        if not activation_scale.static or not activation_scale.group_shape.is_per_tensor():
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        assert tensor_fp8 is not None
        output_scale = (
            input_scale.detach().to(torch.float32).reshape(1) * weight_scale.detach().to(torch.float32).reshape(1)
        ).contiguous()
