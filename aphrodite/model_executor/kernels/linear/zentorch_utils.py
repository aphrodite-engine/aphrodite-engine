...
def is_zentorch_moe_supported(layer: torch.nn.Module) -> bool:
    if not has_zentorch_op(["zentorch_fused_moe"]):
        logging.debug("Skipping zentorch fused-MoE: not a Zen CPU or zentorch not loaded; using default MoE.")
        return False
    ...
    if activation is None:
        logging.debug(
            "Skipping zentorch fused-MoE: layer has no 'activation' attribute, so the activation can't be verified."
        )
        return False
