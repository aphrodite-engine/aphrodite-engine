...
class FlashInferCuteDslNvFp4W4A16LinearKernel(NvFp4LinearKernel):
    ...
    def is_supported(cls, compute_capability: int | None = None) -> tuple[bool, str | None]:
        ...
        if compute_capability not in (100, 103) and not (compute_capability >= 120 and compute_capability < 130):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        padded_weight, weights_padding_bytes = pad_nvfp4_weight_for_cutlass(layer.weight.data, alignment=64)
    ...
    def apply_weights(
        ...
        if x.dtype != torch.bfloat16:
            raise ValueError(f"FlashInfer CuTe-DSL W4A16 requires BF16 input, got {x.dtype}")
        ...
        out = torch.ops.aphrodite.flashinfer_mm_bf16_fp4(
            x_2d,
            layer.weight,
            layer.weight_scale,
...
class FlashInferB12xNvFp4LinearKernel(NvFp4LinearKernel):
    ...
    def is_supported(cls, compute_capability: int | None = None) -> tuple[bool, str | None]:
        ...
        return (
            False,
            "FlashInfer b12x requires SM120+ and FlashInfer with Sm120BlockScaledDenseGemmKernel",
        )
