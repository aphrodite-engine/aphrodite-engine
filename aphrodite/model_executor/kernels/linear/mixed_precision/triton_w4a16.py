...
def _triton_w4a16_gemm_impl(
    ...
    if current_platform.is_rocm():
        from aphrodite.platforms.rocm import on_gfx1x
...
def triton_w4a16_gemm(
    ...
    return torch.ops.aphrodite.triton_w4a16_gemm(a, b_q, scales, qzeros, group_size, zp_bias)
...
class TritonW4A16LinearKernel(MPLinearKernel):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        def repack_w_q(x: BaseAphroditeParameter) -> BaseAphroditeParameter:
        ...
        def repack_w_s(x: BaseAphroditeParameter) -> BaseAphroditeParameter:
        ...
        if self.w_zp_name is not None:
            ...
            if zp is not None:
                ...
                if zp_output_dim is not None:
                    ...
                    if tuple(zp.data.shape) == expected_shape and expected_shape != transposed_shape:
                        needs_transpose = False
                    else:
                        needs_transpose = True
                zp_data = zp.data.t().contiguous() if needs_transpose else zp.data.contiguous()
                replace_parameter(
                    layer,
                    self.w_zp_name,
