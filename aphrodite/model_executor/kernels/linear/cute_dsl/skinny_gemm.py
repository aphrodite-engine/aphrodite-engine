...
class ShapeDynamicSkinnyGemm:
    ...
    def get_cutedsl_warmup_compile_units(self):
        from aphrodite.model_executor.warmup.cutedsl_warmup import CuTeDSLCompileUnit

        return tuple(
            CuTeDSLCompileUnit(
                name=("shape-dynamic skinny GEMM with residual" if has_residual else "shape-dynamic skinny GEMM"),
    ...
    def __call__(
        ...
        if a.shape[1] % (config.block_size * config.vector_width) != 0:
            raise ValueError("K must be divisible by block_size * vector_width for this config")
