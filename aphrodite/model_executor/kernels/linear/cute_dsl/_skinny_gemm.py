...
class CuteSkinnyGemm:
    ...
    def __init__(
        ...
        *,
        ...
        if static_k is not None and (static_k % k_tile_size or static_k < _PREFETCH_TILES * k_tile_size):
    ...
    def kernel(
        ...
        acc_layout = cute.make_layout((num_rows, outputs_per_block), stride=(outputs_per_block, 1))
        ...
        b_regs = cute.make_rmem_tensor(
            cute.make_layout((outputs_per_block, vector_width), stride=(vector_width, 1)),
            self.element_type,
        )
        ...
        if const_expr(self.static_k is not None):
            ...
            for k_tile in cutlass.range(num_k_tiles, unroll=self.k_unroll):
                ...
                for vi in cutlass.range_constexpr(vector_width):
                    for mi in cutlass.range_constexpr(num_rows):
                        for ni in cutlass.range_constexpr(outputs_per_block):
                            acc[mi, ni] = acc[mi, ni] + a_regs[mi, vi].to(cutlass.Float32) * b_regs[ni, vi].to(
                                cutlass.Float32
                            )
