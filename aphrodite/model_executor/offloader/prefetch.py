...
def _get_next_prefetch_index(
    ...
    if prefetch_step < module_count and next_index % prefetch_step != index % prefetch_step:
...
class PrefetchOffloader(BaseOffloader):
            ...
            if module_index % self.group_size >= self.group_size - self.num_in_group:
                if self.offload_params:
                    whitelist = [
                        ...
                        if any(f".{p}." in f".{prefix}{name}." for p in self.offload_params)
                    ]
                else:
    ...
    def _hook_module_forward(self, index: int, module: nn.Module):
        ...
        def forward(*args, **kwargs):
            ...
            torch.ops.aphrodite.wait_prefetch(input_tensor, index)
            ...
            if isinstance(output, tuple):
                torch.ops.aphrodite.start_prefetch(output[0], next_index)
            else:
                torch.ops.aphrodite.start_prefetch(output, next_index)
