...
class UVAOffloader(BaseOffloader):
    ...
    def wrap_modules(
        ...
        if prefix:
            prefix = f"{prefix}."
        modules = [self._maybe_offload_to_cpu(module, prefix) for module in modules_generator]
    ...
    def _maybe_offload_to_cpu(self, module: nn.Module, prefix: str = "") -> nn.Module:
        ...
        for name, p in module.named_parameters():
            ...
            if p.device.type == "cpu" or getattr(p, "_aphrodite_is_uva_offloaded", False):
                continue
            ...
            if self.cpu_offload_params:
                ...
                should_offload = any(f".{param}." in f".{prefix}{name}." for param in self.cpu_offload_params)
