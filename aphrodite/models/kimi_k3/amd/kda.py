...
from aphrodite.models.kimi_k3.amd.kda_metadata import KimiK3ROCmKDABackend
from aphrodite.models.kimi_k3.amd.ops.kda_decode import (
...
class KimiK3DeltaAttention(GatedDeltaNetAttention):
    ...
    def __init__(
        ...
        assert backend == "triton", (
            f"The ROCm Kimi-K3 KDA layer only supports the Triton KDA prefill backend, got {backend!r}."
        )
    ...
    def rearrange_mixed_qkv(self, mixed_qkv: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
            ...
            if m.num_prefills > 0:
                ...
                if split_non_spec:
                    ...
                    core_attn_out_decode, _ = fused_recurrent_kda(
                        ...
                        ssm_state_indices=non_spec_state_indices_tensor[: m.num_decodes],
                    )
                    q_ns = q_ns[:, nd_tok:]
                    k_ns = k_ns[:, nd_tok:]
                    v_ns = v_ns[:, nd_tok:]
