...
from aphrodite.third_party.flash_linear_attention.ops.chunk_delta_h import (
    chunk_gated_delta_rule_fwd_h,
)
from aphrodite.third_party.flash_linear_attention.ops.cumsum import chunk_local_cumsum
...
def chunk_kda_with_fused_gate_fwd(
    ...
    chunk_size = FLA_CHUNK_SIZE
    # Building chunk metadata from a device `cu_seqlens` costs a blocking
    # device->host copy. Callers that already hold it, i.e. the attention
    # metadata builder, should pass it in.
    if chunk_indices is None and cu_seqlens is not None:
        chunk_indices = prepare_chunk_indices(cu_seqlens, chunk_size)
