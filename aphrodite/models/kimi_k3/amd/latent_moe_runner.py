...
class ROCmLatentMoERunner(MoERunner):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._tail_shardable:
            ...
            logger.warning_once(
                "K3 latent-MoE tail is not shardable under this config, falling back to the replicated up-projection.",
                scope="global",
            )
    ...
    def _shard_up_proj_tail(
        ...
        if not self._logged_sharded_tail:
            ...
            logger.info_once(
                "Kimi-K3 latent-MoE tail: up-projecting only this rank's hidden shard into the shared output.",
                scope="global",
            )
    ...
    def _fused_forward(
        ...
        result = self._shard_up_proj_tail(fused_output, shared_output, og_hidden_dim_post_xform)
