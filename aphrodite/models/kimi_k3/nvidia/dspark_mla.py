...
from aphrodite.model_executor.models.qwen3_dspark import DSparkMarkovHead
...
from aphrodite.models.kimi_k3.nvidia.model import KimiMLP
...
class K3DSparkModel(nn.Module):
    ...
    def precompute_and_store_context_kv(
        ...
        if self._context_kv_fusion_available:
            self._precompute_fused_context_kv(context_states, context_positions, context_slot_mapping)
            return
    ...
    def _build_fused_context_kv_buffers(self) -> None:
        ...
        if not attentions or any(
            attn.fused_qkv_a_proj is None or not hasattr(attn.fused_qkv_a_proj, "weight") for attn in attentions
        ):
            self._context_kv_fusion_available = False
            return
        ...
        for attn in attentions:
            ...
            assert (
                ...
                and attn.kv_a_layernorm.variance_epsilon == attn0.kv_a_layernorm.variance_epsilon
            ), "All MLA DSpark layers must share their latent KV geometry."
            kv_weights.append(attn.fused_qkv_a_proj.weight.detach().narrow(0, attn.q_lora_rank, kv_width))
    ...
    def _precompute_fused_context_kv(
        ...
        if (
            ...
            and (isinstance(context_slot_mapping, torch.Tensor) or all(s is not None for s in context_slot_mapping))
        ):
            # Grouped context KV insert only supports unquantized (bf16) KV cache
            # and assumes that all layers share the same block layout.

            if isinstance(context_slot_mapping, (list, tuple)):
                per_layer_slot_mappings = [s for s in context_slot_mapping if s is not None]
                if len({s.data_ptr() for s in per_layer_slot_mappings}) == 1:
                    # All rows alias to the same slot mapping.
                    slot_mapping = per_layer_slot_mappings[0].unsqueeze(0).expand(num_layers, -1)
                else:
