...
from aphrodite.models.kimi_k3.nvidia.low_latency_gemm import try_low_latency_gemm
from aphrodite.models.kimi_k3.nvidia.ops.fused_mla_key_concat_kv_cache import (
...
from aphrodite.v1.kv_cache_interface import KVCacheSpec, MLAAttentionSpec, get_kv_quant_mode
...
class MultiHeadLatentAttention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        if use_rope:
            ...
            if rope_parameters["rope_type"] == "deepseek_yarn":
                ...
                mscale = 1.0 if scaling_factor <= 1 else 0.1 * float(mscale_all_dim) * math.log(scaling_factor) + 1.0
            ...
            assert self.rotary_emb.cos_sin_cache.dtype == torch.float32, (
                f"K3 fused MLA RoPE requires an fp32 cos/sin cache; got {self.rotary_emb.cos_sin_cache.dtype}."
            )
        ...
        if self.q_lora_rank is not None:
            ...
            if use_output_gate:
                ...
                # for now.
                if aux_stream is not None and isinstance(self.fused_qkv_a_g_proj.quant_method, UnquantizedLinearMethod):
        ...
        self.register_buffer("_one_scale", torch.ones(1, dtype=torch.float32), persistent=False)
        ...
        assert parallel_config.prefill_context_parallel_size == 1, (
            "Kimi-K3 MultiHeadLatentAttention does not support prefill context parallelism."
        )
        ...
        if self.dcp_world_size > 1:
            query_dtype = (
                torch.float8_e4m3fn
                if is_quantized_kv_cache(self.kv_cache_dtype) and self.kv_cache_dtype != "fp8_ds_mla"
                else dtype
            )
    ...
    def process_weights_after_loading(self, act_dtype: torch.dtype) -> None:
        ...
        self.register_buffer("_q_scale_inv", self._q_scale.reciprocal().reshape(1), persistent=False)
        self.register_buffer("_k_scale_inv", self._k_scale.reciprocal().reshape(1), persistent=False)
    ...
    # Forward
    # ------------------------------------------------------------------
    def _unquantized_gemm(self, hidden_states: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
    ...
    def _apply_q_lora_attention(
        ...
        q_c, kv_c, k_pe = qkv_lora.split([q_lora_rank, self.kv_lora_rank, self.qk_rope_head_dim], dim=-1)
    ...
    def _forward_q_lora(
        ...
        if self._gate_events is not None and hidden_states.shape[0] < _GATE_MULTI_STREAM_TOKEN_THRESHOLD:
    ...
    def _attention(
        ...
        attn_metadata = cast("MLACommonMetadata", attn_metadata_by_layer[self.layer_name])
        ...
        if num_mqa_tokens > 0:
            mqa_q_nope, mqa_q_pe = q[:num_mqa_tokens].split([self.qk_nope_head_dim, self.qk_rope_head_dim], dim=-1)
    ...
    def _compute_prefill_context(
        ...
        def run_chunk(chunk, out: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if writes_out:
            ...
            output, output_lse = init_mla_context_partial(chunked_context, attn_output, attn_lse, num_tokens=q.shape[0])
            accumulate_mla_context_chunk(chunks[0], attn_output, attn_lse, output, output_lse)
            chunks = chunks[1:]
        ...
        for chunk in chunks:
            ...
            out = output[chunk.token_slice] if writes_out and not chunk.is_continuation else None
    ...
    def _forward_prefill_fused(
        ...
        kv_nope = self.kv_b_proj(kv_c_normed)[0].view(-1, self.num_local_heads, self.qk_nope_head_dim + self.v_head_dim)
        k_nope, v = kv_nope.split([self.qk_nope_head_dim, self.v_head_dim], dim=-1)

        if self.kv_cache_dtype == "fp8_ds_mla":
            # fp8_ds_mla cache (656B, per-tile self-scaled); bf16 attention.
            assert not fp8_prefill, (
                "Kimi-K3 fp8_ds_mla uses a bf16 prefill query; fp8 prefill query is not supported with fp8_ds_mla."
            )
        ...
        output_prefill = prefill.prefill_backend.run_prefill_new_tokens(
            ...
            out=(out.view(-1, self.num_local_heads, self.v_head_dim) if writes_out else None),
        )

        if has_context:
            if self.dcp_world_size > 1:
                context_output, context_lse = self.impl._context_parallel_compute_prefill_context(  # type: ignore[attr-defined]
                    q,
                ...
                context_output, context_lse = self._compute_prefill_context(q, attn_metadata)
