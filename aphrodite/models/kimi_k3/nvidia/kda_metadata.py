...
def _get_aligned_state_indices_kernel(
    ...
    tl.store(
        state_indices_ptr + rows[:, None] * state_indices_stride_0 + state_slots[None, :] * state_indices_stride_1,
        state_indices,
        mask=valid_row[:, None] & valid_state_slot[None, :],
    )
...
class KimiK3KDAMetadata(GDNAttentionMetadata, RecoverSSMMetadata):
    ...
    recoverssm_context: "KDARecoverSSMCommitContext | None" = field(default=None, repr=False, compare=False)
    ...
    def commit_recoverssm_state(self, num_accepted_tokens: torch.Tensor) -> RecoverSSMPostprocessMetadata | None:
        ...
        context.commit(
            ...
            num_computed_tokens=(align.num_computed_tokens if align is not None else None),
...
class KimiK3KDAMetadataBuilder(GDNAttentionMetadataBuilder):
    ...
    def __init__(
        ...
        self.use_recoverssm = aphrodite_config.cache_config.use_kda_recoverssm
    ...
    def _get_recoverssm_context(self) -> "KDARecoverSSMCommitContext":
        ...
        context = KDARecoverSSMCommitContext.create(
            layers,
            spec_query_len=1 + self.aphrodite_config.num_speculative_tokens,
            max_num_reqs=self.aphrodite_config.scheduler_config.max_num_seqs,
        )
    ...
    def build(  # type: ignore[override]
        ...
        if self.aphrodite_config.cache_config.mamba_cache_mode == "align":
            if self.mamba_aligned_state_indices is not None:
                block_table_tensor = self.mamba_aligned_state_indices[: m.num_reqs]
            else:
                assert not self.aphrodite_config.use_v2_model_runner, "Aligned Mamba state indices must be precomputed"
        ...
        if not self.use_spec_decode or num_decode_draft_tokens_cpu is None:
            ...
            if self.use_recoverssm:
                ...
                active_decode_mask_cpu = (~m.is_prefilling) & (query_start_loc_cpu.diff() > 0)
            ...
            if not self.use_recoverssm and num_decode_draft_tokens_cpu[spec_sequence_masks_cpu].sum().item() == 0:
                spec_sequence_masks_cpu = None
                num_spec_decodes = 0
            else:
        ...
        if num_spec_decodes == 0:
            ...
            if self.use_recoverssm and torch.any(query_lens_cpu[spec_sequence_masks_cpu] > self.num_spec + 1).item():
                raise ValueError(
            ...
            use_prefill = torch.any(non_spec_is_prefilling | (non_spec_query_lens_cpu != 1)).item()
            if use_prefill:
            ...
            if num_prefills == 0 and num_decodes == 0:
                ...
                spec_state_indices_tensor = block_table_tensor[:num_spec_decodes, : self.spec_state_slots]
                ...
                spec_sequence_masks_gpu = async_tensor_h2d(spec_sequence_masks_cpu, device=query_start_loc.device)
                if self.use_recoverssm:
                ...
                spec_state_indices_tensor = block_table_tensor[spec_sequence_masks_cpu, : self.spec_state_slots]
                non_spec_state_indices_tensor = block_table_tensor[active_non_spec_mask_cpu, 0]
            ...
            if self.use_recoverssm:
                ...
                num_accepted_tokens = self.recoverssm_num_accepted_tokens[:num_spec_decodes]
        ...
        if (
            ...
            and self.aphrodite_config.cache_config.mamba_cache_mode == "align"
        ):
            ...
            for row, query_len in zip(request_rows, query_lens):
                ...
                valid = seq_len % block_size != 0 and 0 < offset < query_len and offset % FLASHKDA_CHUNK_SIZE == 0
            ...
            if any(tail for _, tail in checkpoint_splits):
                ...
                request_rows_tensor = async_tensor_h2d(request_rows, dtype=torch.int64, device=query_start_loc.device)
                checkpoint_cols_tensor = async_tensor_h2d(
                    checkpoint_cols, dtype=torch.int64, device=query_start_loc.device
                )
                checkpoint_state_indices = m.block_table_tensor[request_rows_tensor, checkpoint_cols_tensor]
        ...
        if (
            ...
            self.non_spec_state_indices_tensor[num_decodes:batch_size].fill_(NULL_BLOCK_ID)
        ...
        return KimiK3KDAMetadata(
            ...
            recoverssm_context=(self._get_recoverssm_context() if recoverssm_commit is not None else None),
            nums_dict=nums_dict,
            batch_ptr=batch_ptr,
