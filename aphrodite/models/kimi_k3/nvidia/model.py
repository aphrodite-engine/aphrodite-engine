...
from aphrodite.transformers_utils.configs.kimi_k3 import KimiK3Config
...
from aphrodite.utils.torch_utils import aux_stream
...
class KimiK3MegaMoEExperts(DeepseekV4MegaMoEExperts):
    ...
    _transformed_l1_weights: tuple[torch.Tensor, torch.Tensor] | None
    _transformed_l2_weights: tuple[torch.Tensor, torch.Tensor] | None
    w13_weight: nn.Parameter | None
    w13_weight_scale: nn.Parameter | None
    w2_weight: nn.Parameter | None
    w2_weight_scale: nn.Parameter | None
...
class KimiMoE(nn.Module):
    def __init__(
        ...
        self.moe_hidden_size = routed_expert_hidden_size if routed_expert_hidden_size is not None else hidden_size
        ...
        if self.padded_moe_intermediate_size != moe_intermediate_size:
            ...
            self.experts.moe_config.intermediate_size_per_partition_unpadded = moe_intermediate_size // self.tp_size
    ...
    def _maybe_overlap_router_and_down_proj(
        ...
        num_tokens = hidden_states.shape[0]
        (router_output, topk_ids), (routed_hidden_states, _) = maybe_execute_in_parallel(
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        routed_hidden_states, router_output, topk_ids = self._maybe_overlap_router_and_down_proj(hidden_states)
        if self.use_mega_moe:
...
class KimiLinearModel(nn.Module, EagleModelMixin):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.num_attn_res_blocks = (
            cdiv(self.end_layer, self.attn_res_block_size) if self.attn_res_block_size is not None else 0
        )
    ...
    def load_weights(
        ...
        experts_unpacked = not use_mega_moe and not any(n.endswith("w13_weight_packed") for n in params_dict)
...
class KimiK3ForConditionalGeneration(
    ...
    def __init__(
        ...
        with self._mark_tower_model(aphrodite_config, "image"):
            ...
            if vision_attn.is_flash_attn_backend and vision_attn._fa_version == 4:
                from aphrodite.models.kimi_k3.nvidia.ops.vision_fa4_warmup import (
    ...
    def select_encoder_cudagraph_items(self, mm_kwargs: dict[str, Any], indices: list[int]) -> dict[str, Any]:
        ...
        if not indices:
            empty_grid = source_grid[:0] if isinstance(source_grid, torch.Tensor) else []
        ...
        selected_pixel_values = torch.cat([pixel_values[offsets[i] : offsets[i + 1]] for i in indices])
        grid_device = source_grid.device if isinstance(source_grid, torch.Tensor) else None
    ...
    def prepare_encoder_cudagraph_capture_inputs(
        ...
        if output_width > max_output_width:
            raise ValueError(
                f"Encoder CUDA graph budget {token_budget} exceeds K3 RoPE capacity for max_batch_size={max_batch_size}"
            )
        grid_thws = [[1, output_height * kh, output_width * kw] for _ in range(max_batch_size)]
        ...
        return EncoderCudaGraphCaptureInputs(values=metadata | {"pixel_values": pixel_values})
    ...
    def prepare_encoder_cudagraph_replay_buffers(
        ...
        return EncoderCudaGraphReplayBuffers(values=metadata | {"pixel_values": pixel_values})
    ...
    def encoder_eager_forward(
        ...
        image_features = self.vision_tower(
            self._get_pixel_values(mm_kwargs).to(next(self.vision_tower.parameters()).dtype),
            self._get_grid_thws(mm_kwargs),
        )
        return self._project_encoder_features(torch.cat(image_features))

    def _maybe_ignore_quant_config(self, quant_config: QuantizationConfig | None) -> QuantizationConfig | None:
    ...
    def _parse_and_validate_media_input(self, **kwargs: object) -> KimiK25MediaPixelInputs | None:
        ...
        if not isinstance(pixel_values, torch.Tensor):
            raise TypeError(f"pixel_values must be a tensor or a list of tensors, got {type(pixel_values)}")
