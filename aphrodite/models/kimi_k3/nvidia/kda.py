...
from aphrodite.model_executor.layers.mamba.ops.gather_initial_states import (
...
from aphrodite.model_executor.parameter import BaseAphroditeParameter, BlockQuantScaleParameter
...
def a_log_weight_loader(
    ...
    def loader(param: torch.Tensor, loaded_weight: torch.Tensor) -> None:
        ...
        if loaded_weight.dim() == 4:
            ...
            assert loaded_weight.shape[-1] == 1, f"Expected old A_log last dim to be 1, got {loaded_weight.shape}"
...
class _KimiGDNMergedColumnParallelLinear(MergedColumnParallelLinear):
    ...
    def weight_loader(
        ...
        replicate_block_scale = (
            isinstance(param, BlockQuantScaleParameter) and loaded_weight.shape[param.output_dim] < self.tp_size
        )
...
def _flashkda_prefill(
    ...
    import aphrodite._flashkda_C  # noqa: F401
...
def _store_cache_checkpoints_kernel(
    ...
    checkpoint_offset = tl.load(checkpoint_offsets_ptr + seq_idx * checkpoint_offset_stride)
    valid_checkpoint = (state_idx != NULL_STATE_IDX) & (checkpoint_offset > 0)
    valid_conv = (cols < WIDTH * STATE_LEN) & valid_checkpoint & (checkpoint_offset >= STATE_LEN)
    ...
    tl.store(
        conv_state_ptr + state_idx * state_stride_0 + width_idx * state_stride_1 + history_idx * state_stride_2,
        values,
        mask=valid_conv,
    )
...
def resolve_kda_prefill_backend(
    ...
    if backend == "flashkda" and not supported:
        raise RuntimeError("FlashKDA requires CUDA SM90/SM10x/SM12x, bfloat16, head_dim=128, and a bounded KDA gate.")
...
def _make_decode_conv1d_weight_loader(
    ...
    def weight_loader(
        ...
        if decode_conv1d_weight is not None and not param.is_meta:
            decode_conv1d_weight[loaded_shard_id].copy_(loaded_shard.squeeze(1).transpose(0, 1))
...
class KimiK3DeltaAttention(GatedDeltaNetAttention):
    ...
    def get_state_dtype(
        ...
        if self.cache_config.use_kda_recoverssm:
            return MambaStateDtypeCalculator.append_kda_recoverssm_record(base_dtypes, self.model_config.dtype)
        return base_dtypes
    ...
    def __init__(
        ...
        if self.cache_config.use_replayssm and not self.use_recoverssm:
            raise ValueError("Kimi-K3 supports --use-replayssm only with speculative decoding")
        ...
        assert kda_config.get("use_full_rank_gate", False), "KimiK3DeltaAttention requires a full-rank gate"
        self._projection_aux_stream = aux_stream
        self._projection_events = (torch.cuda.Event(), torch.cuda.Event()) if aux_stream is not None else None
        ...
        self._flashkda_buffer_specs: tuple[tuple[tuple[int, ...], torch.dtype], ...] | None = None
        if self.kda_prefill_backend == "flashkda":
            T = aphrodite_config.scheduler_config.max_num_batched_tokens
            N = aphrodite_config.scheduler_config.max_num_seqs
    ...
    def get_kv_cache_spec(self, aphrodite_config: AphroditeConfig) -> MambaSpec:
    ...
    def _forward(
        ...
        from aphrodite.models.kimi_k3.nvidia.ops.third_party.kda import (
        ...
        q_conv_weight, k_conv_weight, v_conv_weight = conv_weights.split(self.local_projection_size, dim=0)
        q_conv_state, k_conv_state, v_conv_state = conv_state.split(self.local_projection_size, dim=-2)
        ...
        if has_spec_decode:
            ...
            spec_max_query_len = self.spec_query_len if self.use_recoverssm else spec_state_indices_tensor.size(-1)
            ...
            spec_out = core_attn_out[:, : q_spec.shape[1]] if m.num_prefills == 0 and m.num_decodes == 0 else None
            if self.use_recoverssm:
                from aphrodite.models.kimi_k3.nvidia.ops.recoverssm import (
                    kda_recoverssm_verify,
                )

                if len(recoverssm_records) != 2:
                    raise ValueError("KDA RecoverSSM requires correction and key/gate buffers")
        ...
        if mixed_qkv_ns is not None:
            ...
            if m.num_prefills > 0:
                ...
                q_ns, k_ns, v_ns = (rearrange(x, "n (h d) -> 1 n h d", d=self.head_dim) for x in (q_ns, k_ns, v_ns))
                ...
                if self.kda_prefill_backend == "flashkda":
                    ...
                    flashkda_out = (workspace_out if has_spec_decode else core_attn_out)[:, : q_ns.shape[1]]
