...
KIMI_K3_PROJECTIONS: dict[tuple[int, int], ProjectionSpec] = {
    ...
    (1536, 7168): ProjectionSpec(1536, 7168, _M1_TO_16, name="shared_gate_up_proj/mla_g_proj"),
    ...
    (7168, 1536): ProjectionSpec(7168, 1536, cute_configs=((1, _cute(1, 96, 4, 2)),), name="o_proj"),
    ...
    (7168, 384): ProjectionSpec(7168, 384, frozenset(range(1, 9)), name="shared_down_proj"),
...
def _sm90_spec(
    ...
    cute_configs = tuple((m, _cute(m, *config)) for m, config in enumerate(configs, start=1))
...
def _backend_for(spec: ProjectionSpec, num_tokens: int, has_residual: bool) -> Backend | None:
    if has_residual:
...
def _run_plan(plan: dict[int, ResolvedCall], x: torch.Tensor, weight: torch.Tensor) -> torch.Tensor | None:
...
def run_kda_projection_overlap(
    ...
    def run_fab_fb() -> tuple[torch.Tensor, torch.Tensor]:
        if num_tokens == 1:
            ...
            g1 = torch.empty((1, 1536), dtype=hidden_states.dtype, device=hidden_states.device)
            ops.dsv3_fused_a_gemm(g1, f_a, f_b_weight.t(), enable_pdl=True)
        else:
            from aphrodite.models.kimi_k3.nvidia.ops.cute_dsl.kda_skinny_gemm import (
                kda_skinny_gemm,
            )
...
def autotune_kda_qkvg(model: nn.Module) -> None:
    ...
    weights_by_shape: dict[tuple[int, int, int, torch.dtype, torch.device], torch.Tensor] = {}
...
def try_low_latency_gemm(
    ...
    use this path.
    """
    if envs.APHRODITE_BATCH_INVARIANT or not _runtime_ok(x, weight):
        return None
...
class _KimiK3LowLatencyApply:
    ...
    def apply(
        ...
        if bias is None and not envs.APHRODITE_BATCH_INVARIANT and _runtime_ok(x, layer.weight):
...
class KimiK3LowLatencyEmbeddingMethod(_KimiK3LowLatencyApply, UnquantizedEmbeddingMethod):
    pass
...
def enable_kimi_k3_low_latency_gemm(
    ...
    for child in module.modules():
        ...
        if spec is None:
            continue
        if is_linear:
            child.quant_method = KimiK3LowLatencyLinearMethod(_build_plan(spec), _build_residual_plan(spec))
        else:
    ...
    if shape_dynamic_skinny_gemm.is_available():
        ...
        if residual_warmup_configs:
            shape_dynamic_skinny_gemm.request_warmup_configs(dtype, residual_warmup_configs, has_residual=True)
