...
def fused_mla_key_concat_kv_cache_insert(
    ...
    k_out = torch.empty((tp, num_heads, qk_head_dim), dtype=k_nope.dtype, device=k_nope.device)
    if tp == 0:
        return k_out
...
def _empty_full_key(k_nope: torch.Tensor, k_pe: torch.Tensor, dtype: torch.dtype) -> torch.Tensor:
...
def fused_mla_decode_q_concat_kv_cache_insert(
    ...
    *,
    ...
    if ds_mla:
        ...
        cache = kv_cache if kv_cache.dtype == torch.float8_e4m3fn else kv_cache.view(torch.float8_e4m3fn)
