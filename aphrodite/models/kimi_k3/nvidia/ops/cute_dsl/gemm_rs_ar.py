...
from aphrodite.cute_utils import _tcgen05, mbarrier, simple_tma_copy, to_cta0_smem
...
def multimem_ld_reduce_16B(x: cute.Tensor, *, loc=None, ip=None) -> cute.Tensor:
    ...
    asm = f"multimem.ld_reduce.relaxed.gpu.global.add.acc::f32{vec_type} {{$0, $1, $2, $3}}, [$4];"
...
class Sm100GemmRsArBF16:
    ...
    def prepare_tma(self, tensor: cute.Tensor, BM: cutlass.Constexpr, BK: cutlass.Constexpr) -> cpasync.TmaInfo:
        tma_group = tcgen05.CtaGroup.TWO if self.cta_group == 2 else tcgen05.CtaGroup.ONE
    ...
    def kernel(
        ...
        local_M = partial_uc.shape[0] // num_ranks if self.all_reduce else output.shape[0]
        ...
        if warp_id == 9:
                ...
                for bid in range(raw_bid, total_tiles, num_bids):
                    ...
                    for iter_k in cutlass.range(cute.ceil_div(K, BK), unroll=1):
                        ...
                        for mma_k in cutlass.range_constexpr(BK // 16):
                            ...
                            _tcgen05.mma_f16(d_tmem, a_desc, b_desc, idesc, enable_d, cta_group)
                            a_desc += 32 >> 4
                            b_desc += 32 >> 4
            ...
            for tile_id in range(raw_bid, total_tiles, num_bids):
                ...
                if global_row_start < M:
                    ...
                    if tid_ == 0:
                        ...
                        arrivals = cute.arch.load(flag_ptr, Int32, sem="relaxed", scope="gpu")
                    ...
                    for vec_iter in cutlass.range_constexpr(vecs_per_tile // 128):
                        ...
                        if local_row < local_row_end and global_row < M:
                            tmp = multimem_ld_reduce_16B(partial_vecs[None, (global_row, col)])
                    ...
                    def release_gemm_tile(
                        ...
                        first_rs_tile = cute.ceil_div((local_start + 1) * grid_m, local_M) - 1
                        ...
                        old_count = cute.arch.atomic_add(flag_ptr, Int32(1), sem="relaxed", scope="gpu")
            ...
            if tid_ == 0:
                ...
                utils.distributed.multimem_red_add1(flags_mc_ptr + exit_flag, order="release", scope=scope)
    ...
    def compile(
        ...
        kernel = Sm100GemmRsArBF16(rank, num_ranks, BN, cta_group, all_reduce=all_reduce)
        return cute.compile(
            kernel,
            A,
            B,
...
class GemmRsAr:
    ...
    Each instance is bound to either RS or AR. A Aphrodite worker has one static
    ...
    def warn_incompatible_projection(self) -> None:
        logger.warning_once(
            "Some projections are incompatible with GEMM-RS/AR; using the unfused path instead.",
            scope="global",
        )
...
def init_gemm_rs_ar(max_M: int, N: int, *, all_reduce: bool = False) -> None:
    ...
    if _gemm_rs_ar is not None:
        if _gemm_rs_ar.all_reduce != all_reduce:
            ...
            raise RuntimeError(
                f"GEMM-RS/AR is already initialized for {current}; a worker cannot reinitialize it for {requested}"
            )
