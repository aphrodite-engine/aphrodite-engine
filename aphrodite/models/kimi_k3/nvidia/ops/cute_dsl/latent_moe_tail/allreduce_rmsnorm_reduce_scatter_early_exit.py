        ...
        if role == 0:
            ...
            if cutlass.const_expr(self.top_k > 0):
                ...
                if cutlass.const_expr(
                    self.top_k == 16 and self.latent_dim == 3584 and expert_weights.element_type == BFloat16
                ):
                    ...
                    route_indices = cute.make_ptr(
                        Int32,
                        (expanded_idx_to_permuted_idx.iterator + Int64(token) * self.top_k).llvm_ptr,
                        cute.AddressSpace.gmem,
                        assumed_align=16,
                    )
                    ...
                    local_values = cute.make_rmem_tensor(cute.make_layout((VEC_BF16,)), Float32)
                    ...
                    for slot in cutlass.range_constexpr(self.top_k):
                        ...
                        if permuted_idx >= Int32(0):
                            permuted_element = Int64(permuted_idx) * self.latent_dim + Int64(packed_idx) * VEC_BF16
                            ...
                            values = packed_u32x4_to_bf16x8(load_global_u32x4(permuted_ptr, volatile=False))
                            weight = expert_weights[token, slot].to(Float32)
                            for element in cutlass.range_constexpr(VEC_BF16):
                                local_values[element] = local_values[element] + values[element].to(Float32) * weight
                    local_packed = sanitize_negative_zero(bf16x8_to_packed_u32x4(local_values.load().to(BFloat16)))
            else:
            ...
            if lane < self.cluster_ctas:
                local_slot = cluster_sums.iterator + parity_offset + cluster_rank * self.warps + warp_idx
            ...
            for peer in cutlass.range_constexpr(self.cluster_ctas):
                ...
                if cutlass.const_expr(self.warps == 4):
                    ...
                    for peer_warp in cutlass.range_constexpr(self.warps):
                        full_sum = full_sum + cluster_sums[parity_offset + peer * self.warps + peer_warp]
            ...
            for destination_round in cutlass.range_constexpr(self.shared_destinations_per_cta):
                round_destination = destination + Int32(destination_round * self.shared_destination_stride)
                if round_destination < Int32(self.tp_size):
                    ...
                    while vector < self.shard_vectors:
                        ...
                        destination_element = current_elements + (
                            (Int64(token) * self.tp_size + self.rank) * self.shard_dim + Int64(vector) * VEC_BF16
                        )
                        store_global_u32x4(
            ...
            for destination_round in cutlass.range_constexpr(self.shared_destinations_per_cta):
                round_destination = destination + Int32(destination_round * self.shared_destination_stride)
                global_tid = (Int64(token) * self.tp_size + Int64(round_destination)) * self.threads + Int64(tidx)
                clear_idx = global_tid
                if round_destination < Int32(self.tp_size) and dirty_num_stages > Uint32(0):
                    while clear_idx < clear_fragments:
                        clear_ptr = cute.make_ptr(
                            BFloat16,
                ...
                if round_destination == self.rank:
                    ...
                    while vector < self.shard_vectors:
                        ...
                        output_element = (
                            Int64(token) * self.hidden_dim + self.rank * self.shard_dim + Int64(vector) * VEC_BF16
                        )
                        store_global_u32x4(
...
def compile_kernel(
    *,
    ...
    expert_weights = torch.empty((max_m, max(top_k, 1)), dtype=torch.bfloat16, device=device)
...
class CollectiveKernel:
    ...
    def __init__(
        ...
        *,
        ...
        self._dummy_expert_weights = torch.empty((max_m, max(top_k, 1)), dtype=torch.bfloat16, device=device)
        self._dummy_expanded_idx = torch.empty((max_m, max(top_k, 1)), dtype=torch.int32, device=device)
    ...
    def __call__(
        ...
        for tensor, shape, name, dtype in expected:
            if tensor.shape != shape or tensor.dtype != dtype or tensor.device != device or not tensor.is_contiguous():
                raise ValueError(f"{name} must be contiguous CUDA {dtype} {list(shape)}")
