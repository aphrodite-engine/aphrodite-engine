...
def _make_top16_bf16_finalize_asm() -> str:
    lines = [
        ...
        "ld.global.v4.u32 {_weight_words0, _weight_words1, _weight_words2, _weight_words3}, [$6];",
        "ld.global.v4.u32 {_weight_words4, _weight_words5, _weight_words6, _weight_words7}, [$6+16];",
    ]
    for pair in range(8):
        lines.append(f"mov.b32 {{_weights{2 * pair}, _weights{2 * pair + 1}}}, _weight_words{pair};")
    ...
    for route in range(16):
        ...
        lines.extend(
            [
                ...
                f"@_valid{route} mad.wide.s32 _address{route}, _index{route}, 7168, $4;",
        ...
        for pair in range(4):
            ...
            lines.extend(
                [
                    ...
                    f"@_valid{route} fma.rn.f32.bf16 _acc{element}, _lo, _weights{route}, _acc{element};",
                    f"@_valid{route} fma.rn.f32.bf16 _acc{element + 1}, _hi, _weights{route}, _acc{element + 1};",
                ]
            )
    for pair in range(4):
...
def load_shared_f32x4(pointer: cute.Pointer, *, loc=None, ip=None):
    ...
    return tuple(Float32(llvm.extractvalue(T.f32(), out, [i], loc=loc, ip=ip)) for i in range(4))
