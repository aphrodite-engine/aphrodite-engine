...
from aphrodite.model_executor.layers.fused_moe.runner.moe_runner import MoERunner, _unpack
...
class LatentTailTier(IntEnum):
    ...
    # APHRODITE_SHARED_EXPERTS_STREAM_TOKEN_THRESHOLD tokens: reduce the latent,
...
class LatentMoERunner(MoERunner):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.enable_k3_latent_moe_tail_fusion and use_fused_path and self.moe_config.tp_size not in (8, 16):
            logger.warning_once(
        ...
        if self.enable_k3_latent_moe_tail_fusion and use_fused_path:
            ...
            self.moe_config.defer_moe_finalize = (
                experts_cls is TrtLlmMxfp4ExpertsMonolithic or experts_cls is TrtLlmNvFp4ExpertsMonolithic
            ) and self.moe_config.hidden_dim == self.moe_config.hidden_dim_unpadded
            from aphrodite.models.kimi_k3.nvidia.ops.latent_moe_tail import (
            ...
            op = KimiK3LatentMoETailOp.initialize(
                ...
                experts_per_token=(
                    self.moe_config.experts_per_token if self.moe_config.use_deferred_moe_finalize else 0
                ),
            ...
            self.moe_config.defer_moe_finalize_max_num_tokens = (
                op.contract.max_num_tokens if self.moe_config.use_deferred_moe_finalize else -1
            )
            if self.moe_config.use_deferred_moe_finalize:
                logger.info_once(
                    "K3 latent-MoE tail fusion with deferred top-k finalization is enabled for up to %d tokens.",
    ...
    def _select_tail_tier(
        ...
        # tier 1
        if (
            num_tokens <= envs.APHRODITE_SHARED_EXPERTS_STREAM_TOKEN_THRESHOLD
            and not envs.APHRODITE_DISABLE_SHARED_EXPERTS_STREAM
        ):
    ...
    def _overlap_allreduce_tail(
        ...
        transform = self.routed_output_transform
        assert transform is not None
        assert shared_output.size(0) <= envs.APHRODITE_SHARED_EXPERTS_STREAM_TOKEN_THRESHOLD
    ...
    def _shard_up_proj_tail(
        ...
        return self._maybe_reduce_final_output(shared_output, trunc_size, output_is_reduced=False)
    ...
    def forward(
        ...
        if self._use_fused_path():
            return self._fused_forward(hidden_states, router_logits, input_ids, shared_experts_input)
        return super().forward(hidden_states, router_logits, input_ids, shared_experts_input)
    ...
    def _fused_forward(
        ...
        if og_hidden_dim_pre_xform is None and self.moe_config.should_defer_moe_finalize(hidden_states.shape[0]):
    ...
    def allreduce_norm_latent_out(
        ...
        if flashinfer_trtllm_fused_allreduce_norm is not None:
            ok, max_token_num = _can_use_flashinfer(hidden_states, self.moe_config.tp_size)
            if ok:
