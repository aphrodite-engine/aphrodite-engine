...
class Dots3NoteImageProcessor:
    ...
    def resized_size(
        ...
        *,
        ...
        min_pixels = int(min_pixels if min_pixels is not None else detail_config.get("min_pixels", self.min_pixels))
        max_pixels = int(max_pixels if max_pixels is not None else detail_config.get("max_pixels", self.max_pixels))
...
class Dots3NoteProcessor:
    """Small HF-like processor used by Aphrodite's multimodal frontend."""
    ...
    def _process_video(
        ...
        *,
        ...
        for part in parts:
            ...
            waveform = torch.from_numpy(np.ascontiguousarray(part.value, dtype=np.float32)).view(torch.int32)
        ...
        return {
            ...
            "video_audio_values": (torch.cat(audio_values) if audio_values else empty_audio),
            ...
            "video_patch_counts": torch.tensor([sum(int(grid.prod()) for grid in image_grids)], dtype=torch.long),
            "video_audio_sample_counts": torch.tensor([sum(value.numel() for value in audio_values)], dtype=torch.long),
        }

    def __call__(self, text: str, **kwargs: object) -> BatchFeature:
        modality_order = [key for key in kwargs if key in ("images", "audios", "videos")]
        raw_images = kwargs.pop("images", None)
        raw_audios = kwargs.pop("audios", None)
        raw_videos = kwargs.pop("videos", None)
        images = list(cast(Sequence[Image.Image], raw_images)) if raw_images is not None else []
        audios = list(cast(Sequence[np.ndarray], raw_audios)) if raw_audios is not None else []
        videos = list(cast(Sequence[object], raw_videos)) if raw_videos is not None else []
        if videos and (images or audios):
            raise ValueError("Dots3Note does not support mixing a native video with separate image/audio inputs")
        if len(videos) > 1:
        ...
        output_reserve = None if reserve_value is None else int(cast(Any, reserve_value))
        ...
        for modality in modality_order:
            if modality == "images" and images:
                ...
                waveforms = [
                    torch.from_numpy(np.ascontiguousarray(audio, dtype=np.float32)).view(torch.int32)
                    for audio in audios
                ]
...
class Dots3NoteProcessingInfo(BaseProcessingInfo):
    ...
    def image_processor(self) -> Dots3NoteImageProcessor | None:
        ...
        if not isinstance(preprocessor_config, dict):
            raise ValueError("NOTE vision checkpoint is missing preprocessor_config.json")
    ...
    def processor(self) -> Dots3NoteProcessor:
        ...
        return Dots3NoteProcessor(
            ...
            video_audio_enabled=(
                ...
                and (mm_config.get_limit_per_prompt("audio") > 0 or mm_config.get_limit_per_prompt("video") > 0)
            ),
...
class Dots3NoteDummyInputsBuilder(BaseDummyInputsBuilder[Dots3NoteProcessingInfo]):
    ...
    def get_dummy_mm_data(
        ...
        if num_audios:
            ...
            chunk_samples = int(
                audio_config.get("chunk_seconds", 60) * audio_config.get("sampling_rate", _DEFAULT_SAMPLE_RATE)
            )
...
class Dots3NoteMultiModalProcessor(BaseMultiModalProcessor[Dots3NoteProcessingInfo]):
    ...
    def _get_mm_fields_config(
        ...
        if "pixel_values" in hf_inputs:
            ...
            fields["pixel_values"] = MultiModalFieldConfig.flat_from_sizes("image", grid_thw.prod(-1))
            fields["image_grid_thw"] = MultiModalFieldConfig.batched("image", keep_on_cpu=True)
        if "audio_values" in hf_inputs:
            fields["audio_values"] = MultiModalFieldConfig.flat_from_sizes("audio", hf_inputs["audio_lengths"])
            fields["audio_lengths"] = MultiModalFieldConfig.batched("audio", keep_on_cpu=True)
        if "video_input_ids" in hf_inputs:
            ...
            fields["video_input_ids"] = MultiModalFieldConfig.flat_from_sizes("video", prompt_lengths, keep_on_cpu=True)
    ...
    def _get_prompt_updates(
        ...
        if "audio" in out_mm_kwargs:
            ...
            stride = (
                _HOP_LENGTH * (8 if config.get("use_conv2d_stem", True) else 2) * int(config.get("merge_factor", 1))
            )
