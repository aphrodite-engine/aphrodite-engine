...
def _decode_frames(
    ...
    if duration <= 0 or orig_h <= 0 or orig_w <= 0:
        raise ValueError(f"Invalid video metadata: duration={duration}, height={orig_h}, width={orig_w}")
    if total_frames <= 0:
    ...
    for index, frame in zip(indices, decoded):
        ...
        frames.append((round(index / orig_fps, 3), _jpeg_roundtrip(image, jpeg_quality)))
...
def _decoded_frames(video) -> tuple[np.ndarray, float]:
    ...
    if frames.ndim != 4 or frames.shape[-1] not in (3, 4):
        raise TypeError("Decoded NOTE video must have shape (frames, height, width, channels)")
    ...
    *,
    ...
    if pcm is None:
        ...
        for timestamp, image in frames:
            parts.append(Dots3NoteVideoPart("text", f"<{_format_timestamp(timestamp)}>"))
    ...
    for group in range(len(bounds) - 1):
        ...
        if sample_end > sample_start:
            segment = np.ascontiguousarray(pcm[sample_start:sample_end].astype(np.float32) / 32768.0)
