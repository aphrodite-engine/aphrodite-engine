...
class VisionSdpaAttention(_VisionAttentionBase):
    def forward(
        ...
        if attention_mask.stride(-1) != 1:
            attention_mask = torch.empty_like(attention_mask, memory_format=torch.contiguous_format).copy_(
                attention_mask
            )
...
def resolve_attn_implementation(
    ...
    *,
    ...
    if attn_implementation == "flash_attention_3" and not _flash_attn3_available:
        logger.warning_once("flash attention 3 not available! fallback to flash attention 2 implementation")
...
def build_vision_attention(
    ...
    *,
    ...
    attn_implementation = resolve_attn_implementation(attn_implementation, eager_fallback=eager_fallback)
...
def prepare_seqlens_for_attention(
    ...
    *,
    ...
    resolved = resolve_attn_implementation(attn_implementation, eager_fallback=eager_fallback)
