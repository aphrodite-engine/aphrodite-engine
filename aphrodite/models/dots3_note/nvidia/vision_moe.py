...
def note_vision_fused_moe_fp8(
    ...
    Unlike Aphrodite's generic functional MoE path, the native NOTE encoder keeps
    ...
    if hidden_states.dtype != torch.bfloat16:
        raise TypeError(f"NOTE vision FP8 MoE requires bfloat16 activations, got {hidden_states.dtype}")
    ...
    first_gemm = cache[: num_tokens * topk * intermediate_size].view(num_tokens, topk, intermediate_size)
    second_gemm = cache[: num_tokens * topk * output_size].view(num_tokens, topk, output_size)
