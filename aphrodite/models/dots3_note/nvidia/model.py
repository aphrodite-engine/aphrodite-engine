...
from aphrodite.model_executor.layers.fused_embed_norm import has_full_vocab_on_rank
...
from aphrodite.model_executor.models.deepseek_v2 import (
...
from aphrodite.models.deepseek_v32.nvidia.model import (
...
def _pad_dense_mlp_weight(
    ...
    if not any(proj_name in name for proj_name in (".gate_proj.", ".up_proj.", ".down_proj.")):
        return loaded_weight
...
class Dots3NoteMoE(DeepseekV2MoE):
    ...
    def forward(
        ...
        output = super().forward(hidden_states, already_sequence_parallel=True) + self.shared_experts(hidden_states)
        if gather_output:
...
def _forward_note_mla(
    ...
    *,
    ...
    kv_c, k_pe = kv_lora.split([attention.kv_lora_rank, attention.qk_rope_head_dim], dim=-1)
...
class Dots3NotePaddedMLAAttention(MLAAttention):
    ...
    def get_kv_cache_spec(self, aphrodite_config: AphroditeConfig) -> MLAAttentionSpec:
...
class Dots3NoteFullAttention(DeepseekV2MLAAttention):
    ...
    def __init__(
        ...
        aphrodite_config.compilation_config.static_forward_context.pop(inner_attention.layer_name)
        prefill_backend_cls = (
            type(inner_attention.prefill_backend) if inner_attention.prefill_backend is not None else None
        )
        ...
        gate_size = (
            config.num_attention_heads if gate_type == "headwise" else config.num_attention_heads * config.v_head_dim
        )
        ...
        self.k_rope_only_layernorm = RMSNorm(config.qk_rope_head_dim, eps=config.rms_norm_eps)
...
class Dots3NoteSlidingAttention(nn.Module):
    ...
    def __init__(
        ...
        self.attention_gate_type = gate_type
        self.q_lora_scale = (config.hidden_size / q_lora_rank) ** 0.5 if apply_rescale else 1.0
        self.kv_lora_scale = (config.hidden_size / kv_lora_rank) ** 0.5 if apply_rescale else 1.0
    ...
    def __init__(
        ...
        if is_moe:
            ...
            self.mlp = DeepseekV2MLP(
                hidden_size=config.hidden_size,
                intermediate_size=_padded_mlp_size(config.intermediate_size, quant_config),
                hidden_act=config.hidden_act,
                quant_config=quant_config,
                prefix=f"{prefix}.mlp",
                reduce_results=False,
...
class Dots3NoteModel(DeepseekV32Model):
    ...
    def _adapt_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> Iterator[tuple[str, torch.Tensor]]:
        ...
        for name, weight in weights:
            ...
            if ".indexer." in name:
                ...
                projection = next(
                    (candidate for candidate in ("wk", "weights_proj") if f".indexer.{candidate}." in name),
                    None,
                )
                is_fp8_weight = name.endswith(".weight") and weight.dtype == torch.float8_e4m3fn
        ...
        if pending_indexer_fp8:
            missing = ", ".join(f"{prefix}.{proj}" for prefix, proj in pending_indexer_fp8)
