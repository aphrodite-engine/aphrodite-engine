...
from aphrodite.third_party.deep_gemm import per_block_cast_to_fp8
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if adapter_type not in ("pixel_shuffle_mlp", "patch_merger"):
            raise ValueError(f"adapter_type must be 'pixel_shuffle_mlp' or 'patch_merger', got {adapter_type!r}")
...
class MoESwiGLUFFN(nn.Module):
    ...
    def __init__(self, config: DotsMoEVitConfig, layer_number: int):
        ...
        self.register_buffer("router_bias", torch.zeros(self.num_routed, dtype=torch.float32))

        self.experts = nn.ModuleList(
            [
                DotsSwiGLUFFN(self.hidden_size, config.moe_intermediate_size, bias=config.use_bias)
                for _ in range(self.num_routed)
            ]
        )

        self.gate_weight = nn.Parameter(torch.empty((self.num_routed, self.hidden_size), dtype=torch.float32))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        aggregated_output = aggregated_output / (aggregated_gate.unsqueeze(-1) + epsilon)
...
class MoEVisionBlock(nn.Module):
    def __init__(self, config: DotsMoEVitConfig, layer_number: int):
        super().__init__()
        attn_impl = resolve_attn_implementation(config.attn_implementation, eager_fallback="eager_v2")
        ...
        if is_moe:
            ...
            self.mlp = DotsSwiGLUFFN(config.embed_dim, config.intermediate_size, bias=config.use_bias)
...
class DotsMoEVitModel(PreTrainedModel):
    ...
    def __init__(self, config: DotsMoEVitConfig) -> None:
        ...
        self.blocks = nn.ModuleList([MoEVisionBlock(config, i) for i in range(config.num_hidden_layers)])

        if config.post_norm:
            self.post_trunk_norm = VisionRMSNorm(config.embed_dim, eps=config.rms_norm_eps)
    ...
    def get_pos_ids_by_grid(self, grid_thw_cpu: torch.Tensor):
        ...
        if self.config.pre_pixel_shuffle:
            rope_merge_size = self.spatial_merge_size if self.spatial_merge_size > 1 else 2
        else:
            rope_merge_size = 1
        pos_ids = []
    ...
    def prepare_meta(self, grid_thw_cpu: torch.Tensor) -> VitForwardMeta:
        ...
        cu_seqlens_cpu = F.pad(frame_lengths.cumsum(dim=0, dtype=torch.int32), (1, 0), value=0)
        resolved_attn = resolve_attn_implementation(self.config.attn_implementation, eager_fallback="eager_v2")
        return VitForwardMeta(
            cu_seqlens=cu_seqlens_cpu.to(self.device, non_blocking=True),
            max_seqlen=int(seq_per_frame.max()),
            rotary_pos_emb=prepare_rotary_pos_emb_vision(self.rot_pos_emb(grid_thw_cpu)),
            grid_thw_cpu=grid_thw_cpu,
            seqlens=(frame_lengths.tolist() if attn_uses_seqlens(resolved_attn) else None),
        )

    def forward(self, hidden_states: torch.Tensor, meta: VitForwardMeta, bf16=True) -> torch.Tensor:
        if bf16:
            hidden_states = hidden_states.to(device=self.device, dtype=self.dtype, non_blocking=True)
