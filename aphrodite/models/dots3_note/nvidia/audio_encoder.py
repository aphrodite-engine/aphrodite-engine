...
from aphrodite.vllm_flash_attn import flash_attn_varlen_func, is_fa_version_supported
...
class RotaryEmbedding(nn.Module):
    def __init__(
        ...
        if self.rotary_dim > 0:
            inv_freq = 1.0 / (
                self.rope_theta ** (torch.arange(0, self.rotary_dim, 2, dtype=torch.float) / max(self.rotary_dim, 1))
            )
        else:
    ...
    def get_cos_sin(
        ...
        if self.inv_freq.dtype != torch.float32:
            inv_freq = 1.0 / (
                self.rope_theta
                ** (torch.arange(0, self.rotary_dim, 2, dtype=torch.float, device=device) / max(self.rotary_dim, 1))
            )
        else:
...
class WhisperAttention(nn.Module):
    ...
    def forward_flash_attn(
        ...
        if cu_seqlens_q is None:
            ...
            attn_output, attn_probs = self._eager_attention(query_states, key_states, value_states, output_attentions)
            ...
            if rotary_cos is not None and rotary_sin is not None:
                query_states, key_states = apply_rotary_pos_emb(query_states, key_states, rotary_cos, rotary_sin)
...
class WhisperEncoderLayer(nn.Module):
    def __init__(self, config: WhisperConfig):
        ...
        self.activation_fn = ACT2FN[config.activation_function] if config.activation_function != "swiglu" else swiglu
...
class DotsSpeechEncoder(DotsSpeechPreTrainedModel):
    ...
    def __init__(self, config: WhisperConfig):
        ...
        if self.use_latent_input and latent_dim is None:
            raise ValueError("DotsSpeechEncoder: use_latent_input=True requires config.latent_dim")
        if self.use_conv2d_stem and self.use_latent_input:
            raise ValueError("DotsSpeechEncoder: use_conv2d_stem and use_latent_input are mutually exclusive")
        self.num_mel_bins = latent_dim if self.use_latent_input and latent_dim is not None else 128
        ...
        if self.use_conv2d_stem:
            ...
            self.conv2d1 = nn.Conv2d(1, dhs, kernel_size=3, stride=2, padding=conv_padding)
            self.conv2d2 = nn.Conv2d(dhs, dhs, kernel_size=3, stride=2, padding=conv_padding)
            self.conv2d3 = nn.Conv2d(dhs, dhs, kernel_size=3, stride=2, padding=conv_padding)
            ...
            self.conv1 = nn.Conv1d(self.num_mel_bins, embed_dim * 2, kernel_size=3, stride=1, padding=1)
    ...
    def _forward_conv2d_stem(self, input_features, input_seq_lens=None, audio_sample_lens=None):
    ...
    def forward(
        ...
        if self.use_conv2d_stem:
            inputs_embeds = self._forward_conv2d_stem(input_features, input_seq_lens, audio_sample_lens)
        elif self.use_latent_input:
        ...
        encoder_states: tuple[torch.Tensor, ...] | None = () if output_hidden_states else None
        ...
        if input_seq_lens is not None:
            ...
            input_seq_lens = input_seq_lens.to(device=hidden_states.device, dtype=torch.long)
            B, S, D = hidden_states.shape
            max_seqlen_kv = max_seqlen_q
            cu_seqlens_q = torch.nn.functional.pad(input_seq_lens.cumsum(0, dtype=torch.int32), (1, 0))
        ...
        if input_seq_lens is not None:
            ...
            recover_positions = torch.arange(max_seqlen_q, device=hidden_states.device)[None, :]
