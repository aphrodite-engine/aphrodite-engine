...
from aphrodite.v1.attention.backends.mla.flashattn_mla_sparse import (
...
from aphrodite.v1.attention.backends.mla.prefill.flash_attn import (
    FlashAttnPrefillBackend,
)
from aphrodite.v1.attention.backends.mla.sparse_utils import (
    triton_convert_req_index_to_global_index,
)
from aphrodite.v1.attention.backends.mla.triton_mla import (
    TritonMLABackend,
    TritonMLAImpl,
    TritonMLAMetadataBuilder,
)
from aphrodite.v1.kv_cache_interface import KVCacheLayout
from aphrodite.v1.worker.workspace import (
    current_workspace_manager,
    is_workspace_manager_initialized,
)
from aphrodite.vllm_flash_attn.flash_attn_interface import flash_attn_varlen_func
...
def _gather_swa_kv_kernel(
    ...
    tl.store(
        valid_out_ptr + req_idx * valid_out_stride_req + token_offsets * valid_out_stride_token,
        physical_valid,
...
def _build_sliding_window_metadata(
    *,
    ...
    query_lens_cpu = (query_start_loc_cpu[1:] - query_start_loc_cpu[:-1]).to(dtype=torch.int32)
    ...
    while req_start < query_lens_cpu.numel():
        ...
        while req_end < query_lens_cpu.numel():
            ...
            if next_len > workspace_size:
                raise ValueError(
                    f"Dots3 NOTE SWA prefill window exceeds the MLA workspace: {next_len} > {workspace_size}"
                )
        ...
        token_to_seq_cpu = torch.repeat_interleave(torch.arange(num_reqs, dtype=torch.int32), kv_lens)
...
class Dots3NoteMLAMetadataBuilder(TritonMLAMetadataBuilder):
    ...
    def _reserve_attn_logits_workspace(self) -> None:
        if not is_workspace_manager_initialized():
            return
        batch = self.aphrodite_config.scheduler_config.max_num_seqs
    ...
    def build(
        ...
        if self.dcp_world_size > 1:
            raise NotImplementedError("Dots3 NOTE SWA prefill does not support decode context parallelism")
        ...
        if metadata.num_decodes > 0 and metadata.num_prefills > 0:
            ...
            for chunk in sliding_metadata.chunks:
                ...
                seq_lens = common_attn_metadata.seq_lens[reqs_start + req_start : reqs_start + req_end]
                query_lens = query_lens_cpu[req_start:req_end].to(self.device, non_blocking=True)
                ...
                chunk.token_to_seq = torch.searchsorted(chunk.cu_seq_lens_k[1:], token_ids, right=True).to(torch.int32)
...
class Dots3NoteTritonMLAImpl(TritonMLAImpl):
    ...
    def _forward_swa_mqa(
        ...
        if is_workspace_manager_initialized():
            kv_latent, kv_valid = current_workspace_manager().get_simultaneous(*workspace_specs)
        else:
            kv_latent = torch.empty(workspace_specs[0][0], dtype=q.dtype, device=q.device)
            kv_valid = torch.empty(workspace_specs[1][0], dtype=torch.bool, device=q.device)
    ...
    def forward_mha(
        ...
        for chunk in sliding.chunks:
            ...
            if (use_fp8_prefill or weight_dtype != current_platform.fp8_dtype()) and weight_dtype != torch.uint8:
                kv_c = kv_c.to(weight_dtype)
            k_pe_chunk = workspace[:toks, self.kv_lora_rank :].unsqueeze(1)
            kv_nope = self.kv_b_proj(kv_c)[0].view(-1, self.num_heads, self.qk_nope_head_dim + self.v_head_dim)
            if use_fp8_prefill:
            ...
            output[chunk.query_start : chunk.query_end].copy_(chunk_output[..., : self.v_head_dim])
...
class Dots3NotePaddedSparseImpl(FlashAttnMLASparseImpl):
    ...
    def forward_mqa(
        ...
        if not isinstance(q, tuple):
            raise NotImplementedError("Dots3NotePaddedSparseImpl expects split (q_nope, q_rope) input.")
        q_nope, q_rope = q
        ...
        if token_stride != physical_head_size or head_stride != 1 or block_stride % physical_head_size != 0:
            raise RuntimeError(
        ...
        num_cache_rows = (kv_c_and_k_pe_cache.shape[0] - 1) * block_row_stride + attn_metadata.block_size
