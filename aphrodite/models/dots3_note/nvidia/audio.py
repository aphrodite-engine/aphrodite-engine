...
class Dots3NoteAudioConfig:
    def __init__(self, **kwargs):
        ...
        self.whisper_adapter_in_dim = kwargs.get("whisper_adapter_in_dim", kwargs.get("adapter_in_dim", 1280))
        self.whisper_adapter_out_dim = kwargs.get("whisper_adapter_out_dim", kwargs.get("adapter_out_dim", 2048))
        ...
        self.conv_stem_gradient_checkpointing = kwargs.get("conv_stem_gradient_checkpointing", False)
...
def compute_audio_token_length(
    ...
    *,
    ...
    while time_step * sample_rate < num_samples:
        chunk_len = min(num_samples - time_step * sample_rate, chunk_seconds * sample_rate)
...
class DotsEncoderWithMask(nn.Module):
    def __init__(self, config: Dots3NoteAudioConfig):
        ...
        whisper_config.conv_stem_gradient_checkpointing = config.conv_stem_gradient_checkpointing
    ...
    def _forward_speech_encoder(
        ...
        chunks = [
            F.pad(chunk, (0, 0, 0, max_seq_len - chunk.shape[1])) if chunk.shape[1] < max_seq_len else chunk
            for chunk in chunks
        ]
    ...
    def encode_waveform(self, audio_waveform: torch.Tensor) -> torch.Tensor:
        ...
        while time_step * SAMPLE_RATE < audio_waveform.shape[0]:
            segments.append(audio_waveform[time_step * SAMPLE_RATE : (time_step + self.chunk_seconds) * SAMPLE_RATE])
        ...
        for audio_segment in segments:
            ...
            token_len = (segment_length - 1) // (HOP_LENGTH * self.conv_temporal_stride * self.merge_factor) + 1
        ...
        audio_embedding = self._forward_speech_encoder(mel_features, input_seq_lens, audio_sample_lens)

        chunk_embeddings = []
        for idx, token_len in enumerate(token_lens):
            chunk_embeddings.append(audio_embedding[idx, : token_len * self.merge_factor, :])
    ...
    def encode_waveforms(self, audio_waveforms) -> list[torch.Tensor]:
        ...
        for audio_waveform in audio_waveforms:
            ...
            time_step = 0
            while time_step * SAMPLE_RATE < audio_waveform.shape[0]:
                segment = audio_waveform[time_step * SAMPLE_RATE : (time_step + self.chunk_seconds) * SAMPLE_RATE]
                segment_length = segment.shape[0]
                padded_segments.append(pad_or_trim(segment.flatten(), length=self.chunk_samples))
        ...
        input_seq_lens = torch.tensor(segment_token_lengths, dtype=torch.long) * self.merge_factor
        audio_embedding = self._forward_speech_encoder(mel_features, input_seq_lens, segment_sample_lengths)
        ...
        for segment_count in audio_segment_counts:
            ...
            for _ in range(segment_count):
                ...
                chunks.append(audio_embedding[segment_idx, : token_length * self.merge_factor, :])
                segment_idx += 1
...
class Dots3NoteAudioModel(nn.Module):
    ...
    def _forward_batched(self, waveforms):
        ...
        merged = [self._merge_embeddings(embedding).squeeze(0) for embedding in raw_embeddings]
