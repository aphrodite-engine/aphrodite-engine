...
def minimax_m3_build_sparse_block_table_decode(
    ...
    assert total_q == expected_q, (
        f"MiniMax-M3 decode top-k rows must equal batch * decode_query_len: {total_q} != {expected_q}"
    )
    sparse_bt, sparse_ctx = minimax_m3_alloc_sparse_block_table(topk_idx)
    topk = topk_idx.shape[-1]
...
def _insert_index_cache_kernel(
    ...
    dst = index_cache_ptr + block_id * stride_c_b + block_off * stride_c_t + offs_d * stride_c_d
...
def minimax_m3_sparse_block_page_stride(k_cache: torch.Tensor, v_cache: torch.Tensor) -> int:
...
def _gluon_scale_arg(
    ...
    *,
    ...
    if scale.dim() != 2 or scale.shape[0] != num_kv_heads:
        raise ValueError("MiniMax-M3 AITER sparse PA supports scalar KV scales or [num_kv_heads, max_kv_tokens] scales")
...
def minimax_m3_sparse_attn_decode_aiter(
    ...
    if (sparse_block_table is None) != (sparse_context_lens is None):
        raise ValueError("MiniMax-M3 prepared sparse block table and context lengths must be provided together")
    if sparse_block_table is None:
        sparse_block_table, sparse_context_lens = minimax_m3_build_sparse_block_table_decode(
            topk_idx,
            block_table,
            seq_lens,
            decode_query_len,
...
def _run_gluon_decode(
    ...
    temporary_output = torch.empty(*intermediate_shape, head_size, dtype=q.dtype, device=q.device)
    ...
    if is_fp8:
        ...
        k_scale_arg = _gluon_scale_arg(k_scale, num_phys_pages=nphys16, num_kv_heads=hkv)
        v_scale_arg = _gluon_scale_arg(v_scale, num_phys_pages=v_nphys16, num_kv_heads=hkv)
    else:
