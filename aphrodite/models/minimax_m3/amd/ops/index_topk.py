...
token. Adapted to Aphrodite's paged KV cache: the KV page size is forced to equal the
sparse block size (128), so one sparse block maps to exactly one page.

Index-K cache layout (Aphrodite): ``(num_blocks, 128, idx_head_dim)`` (one shared
...
def _decode_score_program_budget(
    ...
    *,
    ...
    if not (is_gfx950 and head_dim == 128 and query_dtype == torch.bfloat16 and cache_dtype == torch.bfloat16):
        return None
    if 1 <= num_reqs <= 8:
...
def _decode_score_split_launch_policy(
    ...
    *,
    ...
    use_high_batch_config = (
        is_gfx950
        and MIN_DECODE_SCORE_GFX950_HIGH_BATCH_REQUESTS <= num_reqs <= MAX_DECODE_SCORE_GFX950_HIGH_BATCH_REQUESTS
        and head_dim == 128
        and query_dtype == torch.bfloat16
        and cache_dtype == torch.bfloat16
    )
    target_grid = (
        DECODE_SCORE_GFX950_HIGH_BATCH_TARGET_GRID if use_high_batch_config else DECODE_SCORE_DEFAULT_TARGET_GRID
    )
    target = max(
        1,
        min(
...
def _decode_topk_launch_policy(
    ...
    *,
    ...
    if is_gfx950 and topk == 16 and 0 < max_block <= MAX_DECODE_TOPK_FAST_CHUNKS * DECODE_TOPK_BLOCKS_PER_CHUNK:
...
def _store_decode_topk(
    ...
    tl.store(
        topk_ptr + pid_h * stride_topk_h + pid_b * stride_topk_b + off_t * stride_topk_t,
        topk_idx,
        mask=off_t < topk,
    )
...
def _decode_topk_fused_kernel(
    ...
    if SINGLE_TILE_GUARANTEED:
        ...
        score = tl.load(
            score_ptr + pid_h * stride_s_h + pid_b * stride_s_b + (chunk_start + off_k) * stride_s_k,
            mask=valid,
            other=-1e30,
        ).to(tl.float32)
        ...
        score_ptrs = score_ptr + pid_h * stride_s_h + pid_b * stride_s_b + (chunk_start + off_k) * stride_s_k
        valid = off_k < chunk_blocks
...
def minimax_m3_index_decode(
    ...
    *,
    ...
    if emit_sparse_table and (
        sparse_block_table_out is None or sparse_context_lens_out is None or block_page_stride is None
    ):
        raise ValueError("MiniMax-M3 fused decode sparse-table arguments must be provided together")
    if not emit_sparse_table and (
        sparse_block_table_out is not None or sparse_context_lens_out is not None or block_page_stride is not None
    ):
        raise ValueError("MiniMax-M3 fused decode sparse-table arguments must be provided together")
    if emit_sparse_table:
        ...
        if num_idx_heads != 1:
            raise ValueError(
                f"MiniMax-M3 fused decode sparse-table construction requires one index head, got {num_idx_heads}"
            )
        ...
        if attention_block_table.dim() != 2 or attention_block_table.shape[0] != seq_lens.shape[0]:
            raise ValueError("MiniMax-M3 fused decode attention block table must be [num_requests, max_blocks]")
        int32_tensors = (
        ...
        if any(tensor.device != idx_q.device for tensor in int32_tensors):
            raise ValueError("MiniMax-M3 fused decode sparse tables must share the query device")
        if (
            attention_block_table.stride(1) != 1
            or sparse_block_table_out.stride(1) != 1
            or sparse_context_lens_out.stride(0) != 1
        ):
            raise ValueError("MiniMax-M3 fused decode sparse tables require contiguous rows")
    max_block = triton.cdiv(max_seq_len, SPARSE_BLOCK_SIZE)
    if max_block >= 0xFFFF:
        raise ValueError("MiniMax-M3 decode top-k supports fewer than 65535 sparse blocks")
    if emit_sparse_table:
        assert attention_block_table is not None
        if attention_block_table.shape[1] < max_block:
            raise ValueError(
                f"MiniMax-M3 fused decode attention block table is shorter than the required {max_block} sparse blocks"
            )
    ...
    if score_program_budget is not None:
        ...
        if use_high_batch_config and not (num_idx_heads > 1 and max_decode_query_len > 1):
    ...
    if completion_counter is None:
        ...
        if (
            ...
            raise ValueError(
                "MiniMax-M3 completion counter must be contiguous int32 [num_idx_heads, >=total_q] on the query device"
            )
