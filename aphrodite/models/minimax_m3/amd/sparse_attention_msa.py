...
from aphrodite.v1.attention.backend import AttentionLayer
        ...
        *,
        ...
        if self.num_kv_heads != 1:
            raise NotImplementedError(
                f"MiniMax-M3 AITER sparse PA currently requires per-rank num_kv_heads == 1, got {self.num_kv_heads}"
            )
        ...
        if main_md.num_decodes > 0:
            ...
            minimax_m3_sparse_attn_decode_aiter(
                ...
                sparse_block_table=(decode_sparse_table[0] if decode_sparse_table is not None else None),
                sparse_context_lens=(decode_sparse_table[1] if decode_sparse_table is not None else None),
            )
