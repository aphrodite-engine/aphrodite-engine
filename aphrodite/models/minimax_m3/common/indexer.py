...
if not TYPE_CHECKING and current_platform.is_rocm():
        ...
        *,
        ...
        if index_md.num_decodes > 0:
            ...
            if current_platform.is_rocm():
                decode_backend_kwargs["completion_counter"] = self.topk_completion_counter
...
def select_indexer_impl_cls(
    *,
    ...
    use_msa = is_sm100 and topk_blocks == 16 and indexer_kv_dtype in ("bf16", "fp8", "fp8_e4m3")
    if use_msa:
        ...
        logger.info_once(
            "MiniMax M3 indexer: selected MSA (fmha_sm100 score + top-k) [topk_blocks=%d, indexer_kv_dtype=%s]",
            topk_blocks,
            indexer_kv_dtype,
        )
...
class MiniMaxM3Indexer(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        if not current_platform.is_rocm() or not isinstance(self.impl, MiniMaxM3IndexerTritonImpl):
            raise ValueError("MiniMax-M3 fused decode sparse-table construction is ROCm-only")
        return self.impl(
            index_query,
            attention_block_table=attention_block_table,
            sparse_block_table_out=sparse_block_table_out,
