...
from aphrodite.models.minimax_m3.common.ops.sparse_attn import SPARSE_BLOCK_SIZE
...
if current_platform.is_rocm():
    from aphrodite.models.minimax_m3.amd.ops.sparse_attn import (
...
def minimax_m3_rebase_slots_to_page16(
    ...
    return out.div_(block_size, rounding_mode="floor").mul_(block_size).add_(slot_mapping)
...
def minimax_m3_query_token_positions(
    ...
    query_req_id = torch.searchsorted(cu_seqlens_q[1:].contiguous(), pos, right=True).to(torch.int32)
    query_abs_pos = (prefix_lens[query_req_id] + (pos - cu_seqlens_q[query_req_id])).to(torch.int32)
...
class MiniMaxM3SparseMetadataBuilder(AttentionMetadataBuilder[MiniMaxM3SparseMetadata]):
    ...
    def __init__(
        ...
        self.use_aiter_sparse_pa = minimax_m3_use_aiter_sparse_pa(kv_cache_spec.num_kv_heads)
    ...
    def build(
        ...
        if num_prefills > 0:
            ...
            prefill_cu_seqlens_q = (query_start_loc[num_decodes:] - num_decode_tokens).to(torch.int32)
...
class MiniMaxM3SparseTritonImpl(MiniMaxM3SparseImpl):
    ...
    def forward(
        ...
        *,
        ...
        topk = topk_buffer if current_platform.is_rocm() else topk_buffer[:num_tokens].transpose(0, 1)
...
def select_main_backend_and_impl_cls(
    *,
    ...
    selected = "AITER_SPARSE_PA" if use_aiter_sparse_pa else ("MSA" if use_msa else "Triton")
    logger.info_once(
