...
from aphrodite.config.attention import MiniMaxM3MSADecodeBackend
...
class MSACutlassDecodePlanCache:
    ...
    def _build_plan(
        ...
        *,
        ...
        from aphrodite.third_party.fmha_sm100.api import fmha_sm100_plan
    ...
    def prepare(
        ...
        *,
        ...
        if (
            ...
            raise ValueError("MSA sparse decode requires a contiguous CUDA int32 block table with one row per request")
        if seq_lens.dtype != torch.int32 or not seq_lens.is_contiguous() or seq_lens.device != block_table.device:
            raise ValueError(
                "MSA sparse decode requires contiguous CUDA int32 sequence lengths on the block table device"
            )
        if (
            ...
            raise ValueError(
                "MSA sparse decode requires contiguous CPU int32 sequence lengths matching the device sequence lengths"
            )
...
def msa_cutlass_sparse_decode(
    ...
    *,
    ...
    from aphrodite.third_party.fmha_sm100.api import fmha_sm100
