...
MXFP4 routed experts reuse Sonar's Quark OCP MXFP4 fused-MoE method. Excluded
...
class InklingMoE(nn.Module):
    def __init__(
        ...
        *,
        ...
        if quant_config is not None:
            excluded = set(getattr(quant_config, "quant_config", {}).get("exclude", ()))
            expert_prefix = f"{prefix}.experts"
            routed_weights = {
                f"{expert_prefix}.w13_weight",
                f"{expert_prefix}.w2_weight",
            }
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor | None:
        ...
        out, sink_out = maybe_execute_in_parallel(
            ...
            self._sink_events[0],
            self._sink_events[1],
            self._sink_stream if num_tokens <= envs.APHRODITE_SHARED_EXPERTS_STREAM_TOKEN_THRESHOLD else None,
        )
        self._routed_sel = None
    ...
    def load_expert_weight(self, name: str, weight: torch.Tensor) -> list[str]:
        ...
        # tensor with the expert and projection-row dimensions flattened:
        #   w13: [E * 2I, H / 32], w2: [E * H, I / 32].
        # Restore the expert dimension before applying the EP/TP slicing used
        # for both packed weights and scales.
        if key.endswith("_weight_scale") and weight.ndim == 2:
            ...
            weight = weight.view(
                self.n_routed_experts,
                weight.shape[0] // self.n_routed_experts,
                weight.shape[1],
            )
        ...
        tp_size = experts.moe_config.moe_parallel_config.tp_size
        ...
        if key.endswith("_scale_2"):
            ...
            if logical_half > dst_half:
                raise ValueError(
                    f"checkpoint shard for {name} has {logical_half} rows per "
                    f"gate/up half, but destination only has {dst_half}"
                )
            for gid, lid in slots.items():
                slab = weight[gid].narrow(
                    0,
                    tp_rank * 2 * logical_half,
                    2 * logical_half,
                )
                slab = slab.to(param.device)
                param.data[lid, :logical_half].copy_(slab[0::2])
                param.data[
                    lid,
                    dst_half : dst_half + logical_half,
                ].copy_(slab[1::2])
        else:
            # w2: shard the packed intermediate (last) dim. AITER rounds the
            # destination intermediate width to 256, so derive the logical TP
