...
from aphrodite.models.inkling.common.mm_preprocess import (
...
from aphrodite.models.inkling.common.towers import InklingAudio, InklingVision
...
class InklingDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        mlp_output = self.mlp.forward_partials(mlp_in) if isinstance(self.mlp, InklingMoE) else self.mlp(mlp_in)
        if defer_mlp_add:
        ...
        return _sconv_add_norm(mlp_output, hidden_states, self.mlp_sconv, None, positions)[1]
