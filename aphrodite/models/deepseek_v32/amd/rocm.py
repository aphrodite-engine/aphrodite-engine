...
from aphrodite.models.deepseek_v32.attention import DeepseekV32Attention, DeepseekV32Indexer
...
from aphrodite.v1.attention.backends.mla.indexer import DeepseekV32IndexerBackend
...
class DeepseekV32MLAAttention(DeepseekV32Attention):
    ...
    def __init__(self, aphrodite_config, config, prefix, topk_indices_buffer=None):
        super().__init__(
            aphrodite_config,
            config,
            prefix,
    ...
    def _compute_ql_nope(self, q_c: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.is_aiter_triton_fp4_bmm_enabled:
            ...
            ql_nope = batched_gemm_a16wfp4(q_nope, self.W_K, self.W_K_scale, transpose_bm=True, prequant=True)
        elif self.is_aiter_triton_fp8_bmm_enabled:
            from aphrodite._aiter_ops import rocm_aiter_ops

            ql_nope = rocm_aiter_ops.triton_fp8_bmm(q_nope, self.W_K, self.W_K_scale, group_size=128, transpose_bm=True)
        else:
    ...
    def _compute_uv_out(
        ...
        x = attn_out.view(num_actual, self.num_local_heads, self.kv_lora_rank).transpose(0, 1)  # (N, tokens, L)
        out_view = output[:num_actual].view(num_actual, self.num_local_heads, self.v_head_dim)

        if self.is_aiter_triton_fp4_bmm_enabled:
            from aiter.ops.triton.batched_gemm_a16wfp4 import batched_gemm_a16wfp4

            batched_gemm_a16wfp4(x, self.W_V, self.W_V_scale, out_view, transpose_bm=True, prequant=True)
        elif self.is_aiter_triton_fp8_bmm_enabled:
            from aphrodite._aiter_ops import rocm_aiter_ops
