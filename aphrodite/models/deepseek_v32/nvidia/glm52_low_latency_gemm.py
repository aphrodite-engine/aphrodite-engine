...
class GLM52ProjectionSpec:
    ...
    def build_plan(self) -> dict[int, ResolvedCall]:
        plan: dict[int, ResolvedCall] = {num_tokens: ("cute", config) for num_tokens, config in self.cute_configs}
        plan.update((num_tokens, ("dsv3_fused_a", None)) for num_tokens in self.dsv3_tokens)
        return plan
...
def build_glm52_plan(weight: torch.Tensor | None, dtype: torch.dtype) -> dict[int, ResolvedCall] | None:
...
def enable_glm52_low_latency_gemm(
    ...
    for child in module.modules():
        if not isinstance(child, LinearBase) or type(child.quant_method) is not UnquantizedLinearMethod:
            continue
