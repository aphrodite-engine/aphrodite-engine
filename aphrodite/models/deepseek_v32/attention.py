...
from aphrodite.model_executor.layers.attention.attention import get_attention_context
...
class DeepseekV32Attention(MLAAttention):
    ...
    def __init__(
        ...
        self._dense_mha_metadata_layer_name = self.layer_name if enable_short_prefill_scoring_skip else ""
    ...
    def _sparse_indexer_and_attn(
        ...
        if self.indexer is not None and not self.skip_topk:
            ...
            sparse_attn_indexer(
                ...
                dcp_rank=(self.dcp_manager.group.rank_in_group if self.dcp_manager is not None else 0),
                dcp_world_size=(self._aphrodite_config.parallel_config.decode_context_parallel_size),
                cp_kv_cache_interleave_size=(self._aphrodite_config.parallel_config.cp_kv_cache_interleave_size),
                skip_topk_buffer_clear=True,
            )

        attn_metadata, _, kv_cache, layer_slot_mapping = get_attention_context(self.layer_name)
        ...
        if self.use_pcp:
            ...
            kv_for_cache, kpe_for_cache, cache_slot_mapping = maybe_gather_mla_latent_cache_inputs(
                kv_c,
                k_pe.unsqueeze(1),
                layer_slot_mapping,
