...
def compute_pooled_write_locs(
    ...
    *first* token page of each page-group. ``page_table_64`` maps token pages
    ...
    return packed_page.to(torch.int64) * block_size + torch.remainder(pool_ids, block_size)
...
def build_pooled_page_table(
    ...
    assert block_size % pool_size == 0, f"pool_size ({pool_size}) must divide page columns ({block_size})"
...
def _kpool_softmax_rotate_write_cache_kernel(
    ...
    for slot in tl.static_range(0, POOL_SIZE):
        score = tl.load(
            slot_score_ptr + row * slot_score_stride_0 + slot * slot_score_stride_1 + offs,
            mask=mask,
            other=0.0,
        ).to(tl.float32)
        score += tl.load(ape_ptr + slot * ape_stride_0 + offs, mask=mask, other=0.0).to(tl.float32)
    ...
    if WRITE_CACHE:
        ...
        out_s_offset = (
            loc_page_index * BUF_NUMEL_PER_PAGE // 4 + S_OFFSET_NBYTES_IN_PAGE // 4 + loc_token_offset_in_page
        )
...
def kpool_compress_and_write_cache(
    ...
    if return_compressed:
        ...
        compressed_scale = torch.empty((slot_k.shape[0],), dtype=torch.float32, device=slot_k.device)
    else:
...
def _kpool_tail_seed_kernel(
    ...
    ahead = tl.load(tslot_ptr + i + KPOOL, mask=i + KPOOL < n_tokens, other=-1).to(tl.int64)
...
def append_tail_to_topk(
    ...
    out = torch.empty((rows, out_cols), dtype=topk_result.dtype, device=topk_result.device)
    ...
    if page_table is not None:
        ...
        tail_val = (tail_raw + topk_offsets.to(torch.int64).unsqueeze(1)).to(torch.int32)
