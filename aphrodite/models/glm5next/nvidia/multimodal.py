...
class Glm5NextProcessingInfo(Glm4vProcessingInfo):
    """Wires up the Aphrodite-native processor for the multimodal checkpoint.
    ...
    (``aphrodite/transformers_utils/processors/glm5next.py``), a port of the
    ...
    def _glm5_hf_processor(self):
        from aphrodite.transformers_utils.processors.glm5next import Glm5NextProcessor
    ...
    def _processor_pixel_budget(self, proc) -> tuple[int, int]:
        from aphrodite.transformers_utils.processors.glm5next import _pixel_budget
    ...
    def _get_vision_info(
        ...
        *,
        ...
        from the Aphrodite-native processor so profiling matches runtime geometry.
        """
        from aphrodite.transformers_utils.processors.glm5next import smart_resize
...
class Glm5NextMultiModalProcessor(Glm4vMultiModalProcessor):
    """The Aphrodite-native ``Glm5NextProcessor`` extracts image/video features
    only and passes the prompt text through unchanged, so prompt expansion
    (image token repeat, video frame/timestamp structure) is owned by Aphrodite's
