...
def _pick_compress_num_splits(num_actual: int, compress_ratio: int, head_dim: int) -> int:
...
def _compress_gather_split_sparse_attn(
    ...
    row_base = state_cache_ptr + block_numbers * state_cache_stride0 + block_offsets * state_cache_stride1
    cmask = mask_pos[:, None]
...
def _finalize_norm_rope_quant_store_sparse_attn(
    ...
    block = tl.arange(0, TRITON_BLOCK_SIZE)
    mask = block < HEAD_SIZE
    compressed_kv = tl.load(scratch_ptr + token_idx * scratch_stride + block, mask=mask, other=0.0)
...
def compress_norm_rope_store_two_stage_triton(
    ...
    Run the occupancy-fanned two-stage split for prefill [num_decode_tokens:]
