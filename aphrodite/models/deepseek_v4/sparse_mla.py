...
def build_c128a_topk_metadata(
    ...
    global_decode = global_decode_buffer.view(-1)[: num_decode_tokens * max_compressed_tokens].view(
        num_decode_tokens, max_compressed_tokens
    )
    decode_lens = decode_lens_buffer[:num_decode_tokens]
    prefill_local = prefill_buffer.view(-1)[: num_prefill_tokens * max_compressed_tokens].view(
