...
from aphrodite.model_executor.layers.mhc import HCHeadOp
...
from aphrodite.model_executor.models.qwen3_dspark import (
    DSparkMarkovHead,
)
...
class DSparkDeepseekV4Model(nn.Module):
    ...
    def precompute_and_store_context_kv(
        ...
        for i, layer in enumerate(self.layers):
            slot_mapping = None if context_slot_mappings is None else context_slot_mappings[i]
            attn = layer.attn
    ...
    def forward(
        ...
        if self.layers[-1].use_fused_mhc:
            hidden_states = self.layers[-1].hc_post(hidden_states, residual, post_mix, res_mix)
...
class DSparkDeepseekV4ForCausalLM(nn.Module):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        self.model = DSparkDeepseekV4Model(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, "model"))
    ...
    def precompute_and_store_context_kv(
        ...
        self.model.precompute_and_store_context_kv(context_states, context_positions, context_slot_mappings)
