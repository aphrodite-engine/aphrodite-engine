...
from aphrodite.model_executor.layers.fused_moe.experts.rocm_aiter_moe import rocm_aiter_fused_experts
...
from aphrodite.model_executor.layers.mhc import (
    HAS_AITER_MHC,
    HAS_TILELANG_MHC,
    HCHeadOp,
    MHCFusedPostPreOp,
...
from aphrodite.model_executor.models.interfaces import SupportsPP
...
def _use_heterogeneous_fhmoe(num_tokens: int) -> bool:
    return num_tokens > 0 and (rocm_aiter_ops.fused_moe_supports_heterogeneous_shared_expert(num_tokens) is True)
...
def _validate_heterogeneous_routes(
    ...
    if topk_weights.ndim != 2 or topk_ids.ndim != 2 or topk_weights.shape != topk_ids.shape:
        raise ValueError("Heterogeneous fused MoE routes must have equal two-dimensional shapes.")
    expected_columns = experts_per_token + 1
    if topk_weights.shape[1] != expected_columns:
        raise ValueError(
            f"Heterogeneous fused MoE routes must have exactly {expected_columns} columns, got {topk_weights.shape[1]}."
        )


def _heterogeneous_shared_expert_enabled(aphrodite_config: AphroditeConfig) -> bool:
    ...
    if aphrodite_config.kernel_config.moe_backend != "aiter":
        reasons.append("the MoE backend is not AITER")
    if aphrodite_config.model_config.dtype != torch.bfloat16:
        reasons.append("the model dtype is not BF16")
    if not rocm_aiter_ops.fused_moe_supports_heterogeneous_shared_expert(1):
        reasons.append("AITER lacks CSV-backed DSV4 I384 heterogeneous shared-expert support")
    ...
    offload_config = getattr(aphrodite_config, "offload_config", None)
...
def _prepare_native_fp8_shared_expert(
    ...
    if tuple(w13_scale.shape) != expected_w13_scale:
        raise ValueError(f"Expected shared W13 scale shape {expected_w13_scale}, got {tuple(w13_scale.shape)}.")
    if tuple(w2_scale.shape) != expected_w2_scale:
        raise ValueError(f"Expected shared W2 scale shape {expected_w2_scale}, got {tuple(w2_scale.shape)}.")
...
class DeepseekV4HeterogeneousSharedRoutedExperts(RoutedExperts):
    ...
    def prepare_heterogeneous_shared_expert(self) -> None:
        ...
        def routed_scales(scale: torch.Tensor) -> torch.Tensor:
            scale = scale.reshape(self.local_num_experts, -1, scale.shape[-1])[: self.shared_expert_id]
...
def _shared_experts_are_fp4(config, layer_idx: int | None = None) -> bool:
    ...
    if base and any(e.startswith(base) for e in (quant_cfg.get("exclude") or [])):
        return False
    entry = (quant_cfg.get("layer_quant_config") or {}).get(f"{base}.w1") if base else None
    if entry is None:
...
def _fuse_shared_experts_enabled(config, prefix: str = "") -> bool:
    ...
    if not (
        ...
        and envs.APHRODITE_ROCM_USE_AITER_FUSION_SHARED_EXPERTS
        and not get_current_aphrodite_config().parallel_config.enable_expert_parallel
    ):
        return False
    return _shared_experts_are_fp4(config, extract_layer_index(prefix) if prefix else None)
...
class DeepseekV4MoE(nn.Module):
    def __init__(
        ...
        fse_requested = _fuse_shared_experts_enabled(config) and not self.fuse_heterogeneous_shared_expert
        ...
        fuse_shared_into_routed = self.is_fused_shared_expert_enabled or self.fuse_heterogeneous_shared_expert
        self.experts = FusedMoEFactory(
            shared_experts=(None if fuse_shared_into_routed else self.shared_experts),
            n_shared_experts=(config.n_shared_experts if fuse_shared_into_routed else None),
            ...
            routed_experts_cls=(
                DeepseekV4HeterogeneousSharedRoutedExperts if self.fuse_heterogeneous_shared_expert else None
            ),
            routed_experts_args=(
                {
...
class DeepseekV4Model(nn.Module):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.fuse_heterogeneous_shared_expert = _heterogeneous_shared_expert_enabled(aphrodite_config)
        self.start_layer, self.end_layer, self.layers = make_layers(
            ...
            lambda prefix: DeepseekV4DecoderLayer(
                ...
                aux_stream_list=aux_stream_list,
                fuse_heterogeneous_shared_expert=(self.fuse_heterogeneous_shared_expert),
            ),
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            if ".ffn.shared_experts.w" in name and fuse_by_layer.get(extract_layer_index(name), False):
                name = name.replace(
