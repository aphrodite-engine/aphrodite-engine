...
from aphrodite.models.deepseek_v4.common.ops import dequantize_and_gather_k_cache
...
def _trust_dsv4_extra_cache_nan_free(
    ...
    return _ON_GFX950 and kv_cache_dtype == "fp8_ds_mla" and not has_kv_transfer and has_extra_cache
...
def apply_pre_quantized_block_scaled_mm(
    ...
    """
    from aphrodite.model_executor.kernels.linear.scaled_mm.BlockScaledMMLinearKernel import (
        FP8BlockParams,
    )
    ...
    out = kernel.apply_block_scaled_mm(A=x_fp8, B=params.weight, As=x_scale, Bs=weight_scale)
...
class DeepseekV4ROCMAiterSparseSWAMetadataBuilder(DeepseekSparseSWAMetadataBuilder):
    ...
    def build(
        ...
        if base.num_decode_tokens > 0 and base.decode_swa_indices is not None and base.decode_swa_lens is not None:
            ragged_indices, ragged_indptr = build_ragged_indices_from_dense(
                base.decode_swa_indices.reshape(base.num_decode_tokens, base.decode_swa_width),
...
class DeepseekV4ROCMAiterMLAAttention(DeepseekV4Attention):
    ...
    def __init__(self, *args, **kwargs):
        aphrodite_config = args[0] if args else kwargs["aphrodite_config"]
        super().__init__(*args, **kwargs)
        self._has_kv_transfer = aphrodite_config.kv_transfer_config is not None
    ...
    def prepare_compressor_gemm_fusion(self) -> bool:
        ...
        from aphrodite.model_executor.offloader import NoopOffloader, get_offloader

        if not isinstance(get_offloader(), NoopOffloader):
            logger.warning_once(
                "DeepSeek V4 compressor GEMM fusion is incompatible with weight offloading and will remain disabled."
            )
            return False
    ...
    def _bpre_attn_gemm(
        ...
        out = rocm_aiter_ops.gemm_a8w8_blockscale_bpreshuffle(x_fp8, weight, x_scale, scale, output_dtype=x.dtype)
    ...
    def _split_qkv_and_norm(self, qr_kv: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor | None, torch.Tensor]:
        ...
        if not (
            qr.dim() == 2 and qr.shape[0] > 0 and self.q_lora_rank % 128 == 0 and self._wq_b_uses_aiter_block_scaled
        ):
