...
class DeepseekV4FP8Config(Fp8Config):
    ...
    def _is_quark_mxfp4_ocp(hf_quant_cfg: dict) -> bool:
        ...
        if not isinstance(weight, dict):
            return False
        return weight.get("dtype") == "fp4" and weight.get("qscheme") == "per_group" and weight.get("group_size") == 32
    ...
    def override_quantization_method(cls, hf_quant_cfg, user_quant, hf_config=None) -> QuantizationMethods | None:
        if not (
            ...
            and (
                ...
                or (hf_quant_cfg.get("quant_method") == "quark" and cls._is_quark_mxfp4_ocp(hf_quant_cfg))
            )
        ):
            return None
    ...
    def from_config(cls, config: dict) -> DeepseekV4FP8Config:
        if config.get("quant_method") == "quark":
            ...
            config = {
                ...
                "ignored_layers": [name for name in quark_exclude if isinstance(name, str)],
            }
