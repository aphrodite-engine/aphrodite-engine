...
def _make_deepseek_v4_vl_weights_mapper(expert_dtype: str, image_enabled: bool) -> WeightsMapper:
...
class DeepseekV4ForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP, SupportsEagle3):
    ...
    def __init__(self, *, aphrodite_config, prefix: str = "") -> None:
        ...
        self.language_model.hf_to_aphrodite_mapper = WeightsMapper()
        self.make_empty_intermediate_tensors = (  # type: ignore[method-assign]
            self.language_model.make_empty_intermediate_tensors
        )

        expert_dtype = getattr(config, "expert_dtype", "fp4")
        self.hf_to_aphrodite_mapper = _make_deepseek_v4_vl_weights_mapper(expert_dtype, image_enabled)
    ...
    def _encode_image(
        ...
        image_embeds = self.aligner(self.vision(patches, n_vit_h, n_vit_w), n_vit_h, n_vit_w)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        mapped = sorted(self.hf_to_aphrodite_mapper.apply(weights), key=lambda x: x[0])
