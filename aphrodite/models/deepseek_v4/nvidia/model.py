...
from aphrodite.config.kernel import MEGA_MOE_BACKENDS
...
from aphrodite.models.deepseek_v4.nvidia.flashmla import DeepseekV4FlashMLAAttention
...
from aphrodite.utils.flashinfer_moe_ep import (
...
class DeepseekV4MegaMoEExperts(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self._transformed_shared_l1_weights: tuple[torch.Tensor, torch.Tensor] | None = None
        self._transformed_shared_l2_weights: tuple[torch.Tensor, torch.Tensor] | None = None
    ...
    def _deep_gemm_supports_shared_experts(deep_gemm) -> bool:
        ...
        This also gives users of an older precompiled Aphrodite wheel a safe serial
    ...
    def _finalize_shared_expert_weights(self, deep_gemm, shared_experts: DeepseekV4MLP) -> None:
        ...
        if gate_up_scale.dtype in checkpoint_scale_dtypes and down_scale.dtype in checkpoint_scale_dtypes:
    ...
    def _prepare_shared_expert_scale(
        ...
        if block_size is None or len(block_size) != 2:
            logger.warning(
                "Disabling native MegaMoE shared-expert fusion for %s: shared FP8 weight block size is unavailable.",
                self.prefix,
            )
            return None
    ...
    def finalize_weights(self, shared_experts: DeepseekV4MLP | None = None) -> None:
        from aphrodite.utils.deep_gemm import _import_deep_gemm
        ...
        if not self._deep_gemm_supports_shared_experts(deep_gemm):
            logger.warning_once(
                ...
                "installed DeepGEMM Python API is older than the Aphrodite "
    ...
    def get_symm_buffer(self):
        ...
        if symm_buffer is None:
            symm_buffer = deep_gemm.get_symm_buffer_for_mega_moe(
                ...
                self.hidden_size,
                self.intermediate_size,
                num_shared_experts=(self.num_shared_experts if self.has_fused_shared_experts else 0),
            )
...
class DeepseekV4MoE(nn.Module):
    def __init__(
        ...
        validate_fi_moe_ep_config(aphrodite_config)
        ...
        self.image_sentinel_lo = IMAGE_SENTINEL_BASE_ID if getattr(config, "vision_n_layers", 0) > 0 else 0
        ...
        if self.use_mega_moe:
            self._init_mega_moe_experts(aphrodite_config, config, prefix)
        else:
            self._init_fused_moe_experts(aphrodite_config, config, quant_config, prefix)
    ...
    def _init_mega_moe_experts(
        ...
        fuse_shared_experts = bool(
            ...
            and not envs.APHRODITE_DISABLE_DSV4_MEGAMOE_SHARED_EXPERT_FUSION
        ...
        if self.use_fi_mega_moe:
            ...
            from aphrodite.models.deepseek_v4.nvidia.fi_moe import (
        ...
        self.experts = experts_cls(aphrodite_config, **expert_kwargs)
    ...
    def forward(self, hidden_states: torch.Tensor, input_ids: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if self.shared_experts is not None and not self.experts.has_fused_shared_experts:
...
def _select_dsv4_attn_cls(aphrodite_config: AphroditeConfig) -> type[DeepseekV4Attention]:
...
def _use_sequence_parallel(aphrodite_config: AphroditeConfig) -> bool:
    parallel_config = aphrodite_config.parallel_config
    use_mega_moe = aphrodite_config.kernel_config.moe_backend in MEGA_MOE_BACKENDS
    return (
...
class DeepseekV4DecoderLayer(nn.Module):
    def __init__(
        ...
        config = aphrodite_config.model_config.hf_config
        self.hidden_size = config.hidden_size
        self.use_sequence_parallel = _use_sequence_parallel(aphrodite_config)

        self.rms_norm_eps = config.rms_norm_eps
        self.attn = _select_dsv4_attn_cls(aphrodite_config)(
            aphrodite_config,
...
class DeepseekV4Model(nn.Module, EagleModelMixin):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.use_mega_moe = aphrodite_config.kernel_config.moe_backend in MEGA_MOE_BACKENDS
        self.use_sequence_parallel = _use_sequence_parallel(aphrodite_config)
        if self.use_mega_moe and not aphrodite_config.parallel_config.enable_expert_parallel:
            raise NotImplementedError(
                "DeepSeek V4 MegaMoE currently requires expert parallel. "
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        pad_shared_expert = (
            getattr(self.quant_config, "weight_block_size", None) is not None and not self.use_sequence_parallel
        )

        for name, loaded_weight in weights:
            if pad_shared_expert and ".shared_experts." in name:
                loaded_weight = self._pad_shared_expert_weight(self.quant_config, name, loaded_weight)
    ...
    def finalize_mhc_broadcast_weights(self) -> None:
        ...
        if isinstance(layer, DeepseekV4DecoderLayer):
            broadcast = layer.hc_attn_fn.detach().view(-1, layer.hc_mult, layer.hidden_size).sum(dim=1)
...
def _make_deepseek_v4_weights_mapper(expert_dtype: str) -> WeightsMapper:
    if expert_dtype == "fp4":
        ...
        scale_regex = {
            ...
            re.compile(r"(\.experts\.\d+\.w[123]\.base_layer)\.scale$"): r"\1.weight_scale",
        ...
        scale_regex = {
            ...
            re.compile(r"(\.experts\.\d+\.w[123]\.base_layer)\.scale$"): r"\1.weight_scale_inv",
            re.compile(r"\.scale$"): ".weight_scale_inv",
        }
