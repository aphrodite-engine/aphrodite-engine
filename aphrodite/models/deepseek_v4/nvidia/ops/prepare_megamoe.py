...
def _prepare_megamoe_inputs_kernel(
    ...
    if shared_x_sf is not None:
        ...
        tl.store(
            shared_x_sf + shared_row * shared_x_sf_stride_m + k_block_id * shared_x_sf_stride_k,
            packed_scale,
        )
...
def prepare_megamoe_inputs(
    ...
    if (shared_x_sf is None) != (shared_block_m is None):
        raise ValueError("DeepSeek V4 MegaMoE shared input staging requires both shared_x_sf and shared_block_m.")
    if shared_x_sf is not None:
        ...
        if shared_x_sf.ndim != 2 or shared_x_sf.shape[1] != expected_sf_k:
            raise ValueError(
                f"MegaMoE shared_x_sf must have shape (*, {expected_sf_k}), got {tuple(shared_x_sf.shape)}."
            )
        ...
        if shared_x_sf.shape[0] < required_rows:
            raise ValueError(
                f"MegaMoE shared_x_sf has insufficient rows: requires {required_rows}, got {shared_x_sf.shape[0]}."
            )
