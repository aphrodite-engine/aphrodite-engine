...
from aphrodite.models.deepseek_v4.common.rope import build_deepseek_v4_rope
from aphrodite.models.deepseek_v4.compressor import DeepseekCompressor
...
class DeepseekV4Attention(nn.Module, AttentionLayerBase, ABC):
    ...
    def __init__(
        ...
        self.max_image_tokens = (
            getattr(config, "vision_max_n_token", 0) if getattr(config, "vision_n_layers", 0) > 0 else 0
        )
    ...
    def forward(
        ...
        qr_kv, kv_score, indexer_kv_score, indexer_weights = self._run_parallel_input_projections(hidden_states)
    ...
    def _wq_b_proj(self, qr: torch.Tensor, qr_scale: torch.Tensor | None = None) -> torch.Tensor:
...
class DeepseekV4IndexerCache(torch.nn.Module, AttentionLayerBase):
    ...
    def get_kv_cache_spec(self, aphrodite_config: AphroditeConfig) -> KVCacheSpec:
        ...
        uses_fp8_ds_mla_layout = aphrodite_config.cache_config.cache_dtype == "fp8_ds_mla"
        return MLAAttentionSpec(
...
class DeepseekV4Indexer(nn.Module):
    def __init__(
        ...
        self.compress_ratio = compress_ratio
        self.eager_scratch_pool = eager_scratch_pool
        self.use_fp4_kv = dsa_indexer_uses_fp4(aphrodite_config)
        logger.info_once(
    ...
    def forward(
        ...
        if isinstance(attn_metadata, dict):
            ...
            if (
                ...
                num_tokens = indexer_metadata.num_decode_tokens + indexer_metadata.num_prefill_tokens
                if num_tokens > 0:
