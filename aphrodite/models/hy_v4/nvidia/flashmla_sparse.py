...
vendored FlashMLA kernels already accept an ``attn_sink`` argument, but Aphrodite's
...
`aphrodite.models.deepseek_v4.nvidia.flashinfer_sparse` hands ``sinks`` to the
...
class HYV4FlashMLASparseImpl(FlashMLASparseImpl):
    ...
    `aphrodite.model_executor.layers.attention.MLAAttention` and is consumed by the
    ...
    def _validate_sinks(sinks: torch.Tensor | None, num_heads: int) -> None:
        ...
        if sinks.dtype != torch.float32:
            raise ValueError(
                f"HYV4 FlashMLA sparse attention sinks must have dtype torch.float32, but got {sinks.dtype}."
            )
        if sinks.ndim != 1 or sinks.shape[0] != num_heads:
            raise ValueError(
                f"HYV4 FlashMLA sparse attention sinks must have shape ({num_heads},), but got {tuple(sinks.shape)}."
            )
    ...
    def _bf16_flash_mla_kernel(
        ...
        attn_sink = self._sinks_for_query(q[:, :actual_num_heads], head_dim=1, kernel_heads=padded_num_heads)
