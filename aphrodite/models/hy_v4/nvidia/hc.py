...
when the hpc package is installed, ``APHRODITE_ENABLE_HPC_OPS=1`` and the shape /
...
from aphrodite.model_executor.layers.hpc import HpcIHCHead, HpcIHCPost, HpcIHCPre
...
class HYV4HCPreLayer(nn.Module):
    ...
    def reset_parameters(self, init_std: float, base_noise_std: float = 0.0) -> None:
        ...
        with torch.no_grad():
            self.hc_base[: self.hc_mult].fill_(-torch.log(torch.tensor(self.hc_mult - 1.0, dtype=self.hc_base.dtype)))
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        rsqrt = torch.rsqrt(x_flat.square().mean(-1, keepdim=True) + self.layernorm_epsilon)
        mixes = self.hc_fn(x_flat)[0] * rsqrt  # [num_tokens, 2*hc]

        pre_raw = mixes[..., :hc]
        post_raw = mixes[..., hc : 2 * hc]

        pre = torch.sigmoid(pre_raw * self.hc_scale[0].float() + self.hc_base[:hc].float()) + hc_eps
        post = (
            self.magnitude * torch.sigmoid(post_raw * self.hc_scale[1].float() + self.hc_base[hc : 2 * hc].float())
            + hc_eps
        )
...
class HYV4HCPostLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, residual: torch.Tensor, post: torch.Tensor) -> torch.Tensor:
...
class HYV4HCHeadLayer(nn.Module):
    ...
    def reset_parameters(self, init_std: float = 6e-3, base_noise_std: float = 0.0) -> None:
        ...
        with torch.no_grad():
            self.hc_head_base.fill_(-torch.log(torch.tensor(self.hc_mult - 1.0, dtype=self.hc_head_base.dtype)))
            if base_noise_std > 0.0:
                self.hc_head_base.add_(torch.randn_like(self.hc_head_base) * base_noise_std)
...
class HYV4HCLayer(nn.Module):
    ...
    def _prepare_input_to_3d(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if hidden_states.dim() != 2:
            raise RuntimeError(f"HC expects a 2D/3D tensor, got shape={tuple(hidden_states.shape)}")
