...
from aphrodite.model_executor.layers.fused_moe import fused_moe_make_expert_params_mapping
...
from aphrodite.model_executor.models.deepseek_v2 import _try_load_fp8_indexer_wk
...
def _get_spec_layer_idx_from_weight_name(config: PretrainedConfig, weight_name: str) -> int | None:
...
def _should_skip_missing_mtp_scale_param(
    ...
    return quant_config is None and any(scale_name in name for scale_name in _MTP_SCALE_ONLY_PARAM_SUBSTRINGS)
...
def _prepare_mtp_fp8_expert_scale(
    ...
    if (
        ...
        if getattr(quant_config, "is_scale_e8m0", False) and loaded_weight.dtype == torch.uint8:
...
def _create_mtp_quant_config(
    ...
    if mtp_quant_algo is None or mtp_quant_algo.upper() == "NONE":
        logger.info_once(
            "No explicit MTP quant algo (mtp_quant_algo=%s). Using the backbone's quant config.",
            mtp_quant_algo,
        )
    ...
    if mtp_quant_algo in ("BF16", "FP16"):
        logger.info_once("MTP layers are unquantized (mtp_quant_algo=%s).", mtp_quant_algo)
        return None
    ...
    if mtp_quant_algo == "FP8":
        ...
        ignored_layers = hf_quant_config.get("ignored_layers") or hf_quant_config.get("modules_to_not_convert")
...
def _remap_mtp_quant_exclusions(
    ...
    for offset in range(num_mtp_layers):
        ...
        for pattern in patterns:
            ...
            # matches the draft module prefixes.
                ...
                extra.append(draft_prefix + stripped[len(source_prefix) :] + trailing_stars)
    if not extra:
        return quant_config
    ...
    logger.info_once(
        "HYV4 MTP: mapped %d checkpoint-named quant exclusions (%s) onto the draft module prefixes: %s",
        len(extra),
        exclusion_attr,
        ", ".join(extra),
    )
...
def _extend_layer_types(layer_types: list[str] | None, layer_idx: int, fallback: str) -> list[str] | None:
...
class HYV4MultiTokenPredictor(nn.Module):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        super().__init__()
        target_config = aphrodite_config.model_config.hf_config
        speculative_config = aphrodite_config.speculative_config
        assert speculative_config is not None, "HYV4MTP can only be built as a speculative draft model"
        ...
        self.num_mtp_layers = max(getattr(target_config, "num_nextn_predict_layers", 1), 1)
        self.quant_config = _remap_mtp_quant_exclusions(
            _create_mtp_quant_config(config, aphrodite_config.quant_config),
        ...
        self.requires_topk_indices_buffer = any(layer.mtp_block.self_attn.is_sparse for layer in self.layers.values())
...
class HYV4MTP(nn.Module):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.model = HYV4MultiTokenPredictor(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, "model"))
    ...
    def forward(
        ...
        if self.model.requires_topk_indices_buffer and self.model.topk_indices_buffer is None:
            raise RuntimeError(
    ...
    def _rewrite_spec_layer_name(self, spec_layer: int, name: str) -> str:
        ...
        spec_layer_weight = any(weight_name in name for weight_name in spec_layer_weight_names)
    ...
    def _load_expert_weight(
        ...
        for ckpt_proj, tag in (
            ...
            target = _resolve_fused_expert_param(param_base, name.split(ckpt_proj, 1)[1], params_dict)
            if target is None:
                return False
            if tag == "w13_weight":
                chunks = loaded_weight.chunk(2, dim=-2)
                loaded_w1 = self._load_fused_expert_weights(target, params_dict, chunks[0], "w1", num_experts)
                loaded_w3 = self._load_fused_expert_weights(target, params_dict, chunks[1], "w3", num_experts)
                loaded = loaded_w1 and loaded_w3
            else:
                loaded = self._load_fused_expert_weights(target, params_dict, loaded_weight, "w2", num_experts)
            if loaded:
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            ...
            if name == "__skip__":
                continue
            name, loaded_weight = _prepare_mtp_fp8_expert_scale(self.quant_config, name, loaded_weight)
        ...
        # reaches the warning.
        from aphrodite.model_executor.layers.quantization.kv_cache import (
            KVCacheScaleParameter,
        )

        unassigned = [name for name in unassigned_all if not isinstance(params_dict.get(name), KVCacheScaleParameter)]
        if unassigned:
