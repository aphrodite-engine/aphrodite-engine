...
output gating when the hpc package is installed, ``APHRODITE_ENABLE_HPC_OPS=1``
...
from aphrodite.model_executor.layers.hpc import hpc_gated_mla_gemm, hpc_gated_mla_supported
...
from aphrodite.model_executor.models.deepseek_v2 import DeepseekV32IndexerCache
...
def compute_skip_topk_layers(config: PretrainedConfig) -> set[int]:
    ...
    if indexer_types is not None:
        ...
        if invalid_types:
            raise ValueError(f"indexer_types only supports 'full' and 'shared', got {invalid_types}.")
        return {layer_idx for layer_idx, indexer_type in enumerate(indexer_types) if indexer_type == "shared"}
...
class Indexer(nn.Module):
    ...
    def forward(
        ...
        hidden_states, q_quant, k, weights = self.prepare_inputs(hidden_states, qr, positions, rotary_emb)
...
class HYV4MLAAttention(nn.Module):
    ...
    def __init__(
        ...
        self.skip_topk = requested_sparse and self.layer_id in compute_skip_topk_layers(config)
        ...
        is_mtp_layer = num_hidden_layers is not None and self.layer_id >= num_hidden_layers
        ...
        if self.create_indexer:
            ...
            assert q_lora_rank is not None, "HYV4 sparse attention requires q_lora_rank to be set"
        ...
        if self.gated_mla:
            ...
            self.use_hpc_gated_mla = hpc_gated_mla_supported(config.gating_type, self.linear_gate)
        else:
    ...
    def _resolve_sink_backend(self, kv_cache_dtype: str) -> type[AttentionBackend] | None:
        ...
        if capability is None:
            ...
            return None
        cache_config = get_current_aphrodite_config().cache_config
        block_size = (
            cache_config.block_size if cache_config is not None and cache_config.user_specified_block_size else None
        )
    ...
    def _force_sparse_mqa(self) -> None:
        ...
        """
        attention_config = get_current_aphrodite_config().attention_config
    ...
    def forward(
        ...
        attn_out = torch.empty(output_shape, dtype=hidden_states.dtype, device=hidden_states.device)
        self._indexer_and_attn(hidden_states, q_c, positions, q, kv_c_normed, k_pe, attn_out)
