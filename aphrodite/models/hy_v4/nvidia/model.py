...
class HYV4DecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if config.mlp_layer_types[layer_idx] == "dense":
            ...
            self.mlp = HYV4MoEFused(config=config, quant_config=quant_config, prefix=f"{prefix}.mlp")
            self.block_type = "moe"
        self.post_attention_layernorm = RMSNorm(config.hidden_size, config.rms_norm_eps)
        self.hc_attn_layer = HYV4HCLayer(config, layer_idx, prefix=f"{prefix}.hc_attn_layer")
        self.hc_mlp_layer = HYV4HCLayer(config, layer_idx, prefix=f"{prefix}.hc_mlp_layer")
...
class HYV4Model(nn.Module):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        config = _normalize_hyv4_config(aphrodite_config.model_config.hf_config)
        ...
        if get_pp_group().is_first_rank or (self.config.tie_word_embeddings and get_pp_group().is_last_rank):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        def _is_fused_expert_weight(weight_name: str) -> bool:
            ...
            return ".experts.gate_up_proj" in weight_name or (
                ".experts.down_proj" in weight_name and not _is_split_expert_weight(weight_name)
            )
        ...
        for name, loaded_weight in weights:
            ...
            expert_params_mapping = fused_expert_params_mapping if is_fused_expert else split_expert_params_mapping
...
class HYV4ForCausalLM(nn.Module, SupportsPP, SupportsLoRA):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.model = HYV4Model(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, "model"))
        if get_pp_group().is_last_rank:
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        return loader.load_weights(weights, mapper=self.hf_to_aphrodite_mapper | drop_mapper)
