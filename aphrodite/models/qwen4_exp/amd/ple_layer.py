...
class Qwen4ExpNGramEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if embedding_dim % self.ngram_heads:
            raise ValueError(
                f"ple_embed_dim must be divisible by total ngram heads: {embedding_dim} % {self.ngram_heads} != 0"
            )
    ...
    def forward(
        ...
        if num_tokens > self.positions_buffer.numel():
            raise ValueError(
                f"PLE received {num_tokens} tokens, but its workspace supports at most {self.positions_buffer.numel()}"
            )
        if num_reqs > self.padded_buffer.shape[0]:
            raise ValueError(
                f"PLE received {num_reqs} requests, but its workspace supports at most {self.padded_buffer.shape[0]}"
            )
        ...
        torch.ops.aphrodite.qwen4_exp_amd_ple_ngram_embedding(
            ngram_ids,
            output,
            self.layer_name,
        )
        ...
        for name, loaded_weight in weights:
            ...
            if name in persistent_buffers:
                ...
                if buffer.shape != loaded_weight.shape:
                    raise ValueError(
                        f"Shape mismatch for {name}: expected {tuple(buffer.shape)}, got {tuple(loaded_weight.shape)}"
                    )
...
class Qwen4ExpPLELayer(nn.Module, MambaBase):
    def __init__(
        ...
        self.ple_dense_layer_id = int(ple_dense_layer_id) if ple_dense_layer_id is not None else int(layer_idx)
        self.prefix = prefix
    ...
    def _apply_norm(self, norm: Qwen4ExpPLEGroupedNorm, hidden_states: torch.Tensor) -> torch.Tensor:
    ...
    def _short_conv_dilated_spec_batched(
        ...
        if self.conv_state_len > 0:
            ...
            state_offsets = torch.arange(self.conv_state_len, device=conv_state.device, dtype=torch.int64).view(
                1, 1, self.conv_state_len
            )
        ...
        if self.conv_state_len > 0:
            ...
            update_lengths = (self.conv_state_len + query_lengths - 1).view(num_reqs, 1, 1)
    ...
    def _short_conv_dilated_dispatch(
        ...
        if has_spec:
            ...
            spec_output = self._short_conv_dilated_spec_batched(
                ...
                spec_state_indices_tensor=metadata.spec_state_indices_tensor[: metadata.num_spec_decodes],
    ...
    def forward(
        ...
        torch.ops.aphrodite.qwen4_exp_ple_short_conv(
            normalized,
            conv_output,
            self.prefix,
        )
