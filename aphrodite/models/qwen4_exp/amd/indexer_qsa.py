...
from aphrodite.model_executor.layers.rotary_embedding.mrope import triton_mrope
...
def apply_qsa_rmsnorm(
    ...
    """Use Aphrodite's portable RMSNorm implementation on ROCm."""
...
class QSAIndexer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if aphrodite_config.cache_config is None:
        ...
        self.raw_key_cache = QSAKeyStateCache(
            ...
            cache_rope_positions=aphrodite_config.model_config.uses_mrope,
    ...
    def normalize_compressed_keys(
        ...
        keys = apply_qsa_rmsnorm(self.k_layernorm, keys).reshape(-1, 1, self.index_head_dim)
    ...
    def _metadata(
        ...
        compressed = cast(QSAForwardMetadata, metadata[self.compressed_key_cache.prefix])
        if raw.num_actual_tokens != compressed.num_actual_tokens:
            raise RuntimeError("QSA side-cache metadata token counts disagree")
        if not raw.logical_positions.is_cuda and (not torch.equal(raw.logical_positions, compressed.logical_positions)):
    ...
    def _update_and_compress(
        ...
        if rope_position_cache is None:
            ...
            position_rows = canonical_qsa_rope_positions(positions)[:num_tokens].to(device=raw_key_cache.device)
    ...
    def forward(
        ...
        num_tokens = raw_metadata.num_actual_tokens
        q, token_k = self.project_qk(hidden_states[:num_tokens], positions[..., :num_tokens])
