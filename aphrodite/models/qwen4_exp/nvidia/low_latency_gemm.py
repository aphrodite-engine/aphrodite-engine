...
class _Qwen4ExpLowLatencyApply:
    def apply(
        ...
        if bias is None and not envs.APHRODITE_BATCH_INVARIANT:
            return torch.ops.aphrodite.qwen4_exp_low_latency_gemm(x, layer.weight)
...
class Qwen4ExpLowLatencyEmbeddingMethod(_Qwen4ExpLowLatencyApply, UnquantizedEmbeddingMethod):
    pass
...
def _qwen4_exp_low_latency_gemm(x: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
    ...
    if config is not None and _runtime_ok(x, weight) and shape_dynamic_skinny_gemm.is_available():
        return shape_dynamic_skinny_gemm(x, weight, config)
    return torch.nn.functional.linear(x, weight)


def _qwen4_exp_low_latency_gemm_fake(x: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
