...
def _norm_rope(
    ...
    if IS_MROPE:
        ...
        sin = tl.load(cos_sin_ptr + pos_t[:, None] * cos_sin_stride + QUARTER + pairs[None, :])
    ...
    rotated, passthrough = tl.split(tl.permute(tl.reshape(y, (ROWS, 2, HALF)), (0, 2, 1)))
...
def _qsa_pre_indexer_kernel(
    ...
    if pid >= num_k_work:
        ...
        x = tl.load(
            q_ptr + tokens[:, None, None] * q_stride_token + heads[None, :, None] * D + dims[None, None, :],
            mask=mask,
            other=0.0,
        )
    ...
    if pid < num_k_work:
        ...
        if work_in_request < num_groups:
            ...
            valid_token = (boundary_token >= query_start) & (boundary_token < query_end) & (boundary_token < num_tokens)
            compressed_slot = tl.load(
                compressed_slots_ptr + boundary_token,
                mask=valid_token,
                other=-1,
            )
            valid = valid_token & (compressed_slot >= 0) & (compressed_slot < num_compressed_blocks * COMP_PAGE_SIZE)
            ...
            source_tokens_valid = (
                (source_tokens >= query_start) & (source_tokens < query_end) & (source_tokens < num_tokens)
            )
            current_base = k_ptr + tl.maximum(source_tokens, 0)[:, None] * k_stride_token
            cached_base = (
                state_cache_ptr
            ...
            source_valid = tl.where(source_in_chunk, source_tokens_valid, state_block_valid)
            source = tl.load(
            ...
            pooled = (tl.sum(source, axis=0) / COMPRESS_RATIO).to(tl.bfloat16).to(tl.float32)
            ...
            if CACHE_HAS_ROPE_POS:
                ...
                first_token_valid = (
                    (first_token >= query_start) & (first_token < query_end) & (first_token < num_tokens)
                )
                ...
                if IS_2D_POSITIONS:
                    ...
                    first_pos_w = tl.load(
                        pos_ptr + 2 * pos_stride_axis + safe_first_token * pos_stride_token,
                        mask=load_current_position,
                        other=0,
                    )
                else:
        ...
        if work_in_request == 0:
            ...
            for state_offset in tl.range(0, num_state_rows):
                ...
                valid_token = (token >= query_start) & (token < query_end) & (token < num_tokens)
                slot = tl.load(state_slots_ptr + token, mask=valid_token, other=-1)
                valid_slot = valid_token & (slot >= 0) & (slot < num_state_blocks * STATE_SIZE)
                ...
                if CACHE_HAS_ROPE_POS:
                    ...
                    if IS_2D_POSITIONS:
                        pos_h = tl.load(
                            pos_ptr + pos_stride_axis + tl.maximum(token, 0) * pos_stride_token,
                            mask=valid_slot,
                            other=0,
                        )
                        pos_w = tl.load(
                            pos_ptr + 2 * pos_stride_axis + tl.maximum(token, 0) * pos_stride_token,
                            mask=valid_slot,
                            other=0,
                        )
                    else:
