...
def _qsa_sparse_paged_gqa_splitk_kernel(
    ...
    if NUM_SPLITS == 1:
        ...
        tl.store(
            partial_output_ptr
            + (partial_row * NUM_QUERY_HEADS + first_head + head_offsets[:, None]) * HEAD_DIM
            + dim_offsets[None, :],
            normalized_output,
            mask=output_mask,
        )
...
def _qsa_merge_splitk_kernel(
    ...
    partial_output = tl.load(
        partial_output_ptr + (split_rows[:, None] * NUM_QUERY_HEADS + head) * HEAD_DIM + dim_offsets[None, :],
...
def _select_config(
    ...
    if base_programs > 2048:
        BLOCK_N, target_splits, num_warps = (32, 1, 1) if use_prefill_config else (64, 1, 2)
...
def qsa_sparse_paged_attention(
    ...
    if logical_indices.shape[1] < 2:
        raise ValueError("QSA packed indices need selection columns plus the count column")
...
def warmup_qsa_sparse_paged_attention(
    ...
    *,
    ...
    num_rows = 16
    num_requests = 16
    q_ptr = TritonWarmupTensor(torch.bfloat16, shape=(num_rows, num_query_heads, head_dim))
    ...
    output_ptr = TritonWarmupTensor(torch.bfloat16, shape=(num_rows, num_query_heads, head_dim))
    ...
    for block_n, warps, num_tiles, num_splits in sorted(profiles):
        if num_splits == 1:
            ...
            partial_lse_ptr = TritonWarmupTensor(torch.float32, shape=(num_splits, num_rows, num_query_heads))
