...
def _qsa_mqa_paged_prefill_kernel(
    ...
    request_start = tl.maximum(tl.load(query_start_loc_ptr + request) - query_base, query_offset)
    request_end = tl.minimum(tl.load(query_start_loc_ptr + request + 1) - query_base, query_end)
    ...
    query = tl.load(
        q_ptr + q_rows[None, :] * stride_q_row + heads[None, :] * stride_q_head + dims[:, None],
    ...
    for tile in tl.range(k_tile_start, k_tile_end, num_stages=STAGES):
        ...
        store_mask = valid_rows[None, :] & (columns[:, None] < visible[None, :]) & (columns[:, None] < num_columns)
        tl.store(
            logits_ptr + rows[None, :] * stride_logits_row + columns[:, None],
            score,
            mask=store_mask,
...
def _qsa_decode_warmup_profiles(
    ...
    for dql in range(1, max_dql + 1):
        ...
        profiles.extend((dql, num_requests) for num_requests in requests_by_grouping.values())
...
def _prefill_logits(
    ...
    logits = torch.empty((num_queries, logits_width), dtype=torch.float32, device=q.device)
    # tuned on GB300
...
def qsa_select_paged_prefill(
    ...
    # chunk the inputs to keep temp logits below APHRODITE_SPARSE_INDEXER_MAX_LOGITS_MB
