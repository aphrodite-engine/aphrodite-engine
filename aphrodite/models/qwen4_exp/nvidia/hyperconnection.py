...
standard Aphrodite Linear modules.
...
Typical usage inside a transformer decoder layer::
    ...
    hidden_states, block_input, injection = self.mlp_hc.combine_and_mix(hidden_states, attention_output, injection)
"""
...
class GatedResidual(nn.Module):
    ...
    are Aphrodite Linear modules (merged replicated linear for down+inject), so
    ...
    def __init__(
        ...
        norm_size = self.hyper_hidden_size if config.hc_per_branch_norm else config.hidden_size
        ...
        if use_combine:
            self.input_mix_weight_down_block_inject = MergedColumnParallelLinear(
                ...
                [self.lora_rank, self.hc_count] + ([self.pad_size] if self.pad_size else []),
                bias=False,
