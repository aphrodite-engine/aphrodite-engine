...
class Qwen4ExpPLEFp8EmbeddingMethod(QuantizeMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        weight = create_fp8_weight_parameter(sum(output_partition_sizes), input_size_per_partition, weight_loader)
...
class Qwen4ExpNGramEmbedding(nn.Module):
    ...
    def forward(
        ...
        ngram_ids = input_ids.new_empty((input_ids.numel(), self.ngram_heads), dtype=torch.long)
        torch.ops.aphrodite.qwen4_exp_compute_ple_ngram_ids(
            input_ids,
            query_start_loc,
            ngram_context,
            ngram_ids,
...
class Qwen4ExpPLELayer(nn.Module, MambaBase):
    ...
    def _short_conv_dilated_dispatch(
        ...
        if has_prefill:
            ...
            if non_spec_token_indices is None:
                inputs_d, inputs_p = torch.split(inputs, [num_decode_tokens, num_prefill_tokens], dim=0)
                residual_d, residual_p = torch.split(residual, [num_decode_tokens, num_prefill_tokens], dim=0)
            ...
            num_decode_rows = non_spec_token_indices.numel() if non_spec_token_indices is not None else inputs.size(0)
            ple_conv(
                inputs=inputs,
                residual=residual,
                conv_state=conv_state,
    ...
    def forward(
        ...
        torch.ops.aphrodite.qwen4_exp_ple_short_conv(conv_input, gated_output, self.prefix)
        return gated_output
