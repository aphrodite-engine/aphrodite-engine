...
class Qwen4ExpTextConfig(Qwen3NextConfig):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if rope_parameters is not None:
            ...
            if kwargs.get("partial_rotary_factor") is None and "partial_rotary_factor" in rope_parameters:
        ...
        self.rope_scaling = rope_scaling or rope_parameters or normalized_rope_parameters
    ...
    def _validate_ple_config(self) -> None:
        ...
        if self.ple_embed_dim <= 0:
            raise ValueError(f"ple_embed_dim must be positive, got {self.ple_embed_dim}")
        ngram_heads = (self.ngram_size - 1) * self.heads_per_ngram
        if self.ple_embed_dim % ngram_heads:
            raise ValueError(
                f"ple_embed_dim must be divisible by total ngram heads: {self.ple_embed_dim} % {ngram_heads} != 0"
            )
        if self.ple_conv_kernel_size <= 0:
            raise ValueError(f"ple_conv_kernel_size must be positive, got {self.ple_conv_kernel_size}")
    ...
    def _validate_ple_layer_ids(self) -> None:
        invalid = [layer_id for layer_id in self.ple_layer_ids if not 1 <= int(layer_id) <= self.num_hidden_layers]
        if invalid:
            raise ValueError(
    ...
    def _validate_qsa_config(self) -> None:
        ...
        if block_topk not in (512, 2048):
            raise ValueError(
                f"QSA requires indexer_budget / indexer_compress_ratio to be 512 or 2048, got {block_topk}"
            )
...
class Qwen4ExpConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.rope_parameters = rope_parameters or getattr(self.text_config, "rope_parameters", {})
