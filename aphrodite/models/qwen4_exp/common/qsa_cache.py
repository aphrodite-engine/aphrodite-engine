...
def _build_qsa_metadata_torch(
    ...
    *,
    ...
    if k_work_metadata_buffer is not None:
        ...
        num_work_per_request = common_attn_metadata.seq_lens // compress_ratio - chunk_starts // compress_ratio
...
class QSAMetadataBuilder(AttentionMetadataBuilder[QSAForwardMetadata]):
    """Build QSA metadata from Aphrodite's cache-group-specific common metadata."""
    ...
    def __init__(
        ...
        if isinstance(kv_cache_spec, MLAAttentionSpec):
            ...
            assert isinstance(compress_ratio, int), "QSA compression requires an integer tokens_per_state"
        ...
        self.visible_blocks_buffer = torch.empty(max_tokens, dtype=torch.int32, device=device)
        max_requests = aphrodite_config.scheduler_config.max_num_seqs
    ...
    def build(
        ...
        decode_query_len = 0
        if num_decodes > 0:
            query_lens_cpu = torch.diff(common_attn_metadata.query_start_loc_cpu[: num_decodes + 1])
        ...
        token_to_req, logical_positions, visible_blocks, slot_mapping = build_qsa_metadata(
            common_attn_metadata,
...
class _QSAStateCache(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        *,
        ...
        static_context = aphrodite_config.compilation_config.static_forward_context
...
class QSAKeyStateCache(_QSAStateCache):
    ...
    def get_kv_cache_spec(self, aphrodite_config: AphroditeConfig) -> KVCacheSpec:
        ...
        span = self.compress_ratio + aphrodite_config.num_speculative_tokens
