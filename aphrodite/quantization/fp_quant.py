...
from aphrodite._custom_ops import (cutlass_scaled_fp4_mm, fusedQuantizeMx,
                                   fusedQuantizeNv, matmul_mxf4_bf16_tn)
...
class FPQuantConfig(QuantizationConfig):
    ...
    def __init__(
        ...
        if pseudoquantization:
            raise ValueError("Pseudoquantization is not supported for Aphrodite")
...
def quantized_forward(
    ...
    if forward_dtype == "mxfp4":
        ...
        x_flat_q, x_flat_scales = torch.ops.aphrodite.fused_quantize_nv(
            x_flat, forward_hadamard_matrix, act_global_scale
        )
        y = torch.ops.aphrodite.matmul_nvf4_bf16(
            x_flat_q,
            qweight,
            x_flat_scales,
            weight_scales,
