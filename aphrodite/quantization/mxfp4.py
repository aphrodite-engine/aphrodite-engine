...
from aphrodite.utils import (has_triton_kernels, is_torch_equal_or_newer,
...
def _should_use_flashinfer_mxfp4_bf16():
    ...
    if envs.is_set("APHRODITE_USE_FLASHINFER_MOE_MXFP4_BF16"):
        return envs.APHRODITE_USE_FLASHINFER_MOE_MXFP4_BF16
    ...
    if (current_platform.is_device_capability(100) and has_flashinfer()
            and not envs.is_set("APHRODITE_USE_FLASHINFER_MOE_MXFP4_MXFP8")):
        log_once(
            "INFO",
            "Enabling FlashInfer MXFP4 BF16 backend by default for Blackwell. "
            "For faster performance, consider setting "
            "APHRODITE_USE_FLASHINFER_MOE_MXFP4_MXFP8=1, "
...
def _should_use_flashinfer_mxfp4_mxfp8():
    ...
    return envs.APHRODITE_USE_FLASHINFER_MOE_MXFP4_MXFP8
...
class Mxfp4MoEMethod(FusedMoEMethodBase):
    ...
    def __init__(self, moe: FusedMoEConfig):
        ...
        self.max_capture_size = get_current_aphrodite_config(
        ...
        if current_platform.is_device_capability(100) and not has_flashinfer():
            log_once(
                ...
                "Please `pip install aphrodite[flashinfer]` for best results.")

    def _should_use_marlin(self):
        if envs.APHRODITE_MXFP4_USE_MARLIN is not None:
            return envs.APHRODITE_MXFP4_USE_MARLIN
    ...
    def process_weights_after_loading(self, layer):
        if self.use_marlin:
            ...
            if self.moe.use_ep:
                num_warps = 4 if envs.APHRODITE_MOE_DP_CHUNK_SIZE <= 512 else 8
            else:
                num_warps = 8
