...
from aphrodite.modeling.layers.fused_moe.layer import UnquantizedFusedMoEMethod
from aphrodite.modeling.layers.linear import LinearBase, LinearMethodBase, UnquantizedLinearMethod
from aphrodite.modeling.parameter import BlockQuantScaleParameter, ModelWeightParameter, PerTensorScaleParameter
...
def get_fp8_moe_backend(block_quant: bool) -> Fp8MoeBackend:
    ...
    if (
        ...
        and envs.APHRODITE_USE_FLASHINFER_MOE_FP8
    ...
    use_marlin = not current_platform.has_device_capability(89) or envs.APHRODITE_TEST_FORCE_FP8_MARLIN
    ...
    if current_platform.is_cuda() and current_platform.is_device_capability(100) and block_quant:
...
class Fp8Config(QuantizationConfig):
    ...
    def apply_aphrodite_mapper(self, hf_to_aphrodite_mapper: "WeightsMapper"):
        if self.ignored_layers is not None:
            self.ignored_layers = hf_to_aphrodite_mapper.apply_list(self.ignored_layers)
    ...
    def get_xpu_quant_method(self, layer: torch.nn.Module, prefix: str) -> Optional["QuantizeMethodBase"]:
        from aphrodite.attention.layer import Attention
        from aphrodite.quantization.ipex_quant import XPUFp8LinearMethod, XPUFp8MoEMethod
...
class Fp8LinearMethod(LinearMethodBase):
    ...
    def __init__(self, quant_config: Fp8Config):
        ...
        self.use_marlin = not current_platform.has_device_capability(89) or envs.APHRODITE_TEST_FORCE_FP8_MARLIN
        ...
        if aphrodite_is_batch_invariant():
            ...
            if self.block_quant:
                ...
                if weight_scale.dim() != 2:
                    raise RuntimeError(f"FP8 block scale must be 2D, got {tuple(weight_scale.shape)}")
                ...
                scale_expanded = weight_scale.repeat_interleave(block_n, dim=0).repeat_interleave(block_k, dim=1)
...
class Fp8MoEMethod(FusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: Module) -> None:
        ...
        from aphrodite.modeling.layers.fused_moe.rocm_aiter_fused_moe import is_rocm_aiter_moe_enabled, shuffle_weights
        ...
        if self.block_quant:
            ...
            if current_platform.is_fp8_fnuz():
                w13_weight, w13_weight_scale_inv, w13_input_scale = normalize_e4m3fn_to_e4m3fnuz(
                    layer.w13_weight,
                    layer.w13_weight_scale_inv,
                    layer.w13_input_scale,
                )
                w2_weight, w2_weight_scale_inv, w2_input_scale = normalize_e4m3fn_to_e4m3fnuz(
    ...
    def select_gemm_impl(
        ...
        from aphrodite.modeling.layers.fused_moe import BatchedTritonOrDeepGemmExperts, TritonOrDeepGemmExperts
    ...
    def apply(
        ...
        if self.flashinfer_moe_backend == FlashinferMoeBackend.TENSORRT_LLM and self.fused_experts is None:
            assert activation == "silu", f"Expected 'silu' activation but got {activation}"
            assert scoring_func == "sigmoid", f"Expected 'sigmoid' scoring func but got {scoring_func}"
            if self.block_quant:
                import aphrodite.modeling.layers.fused_moe.flashinfer_trtllm_moe  # noqa: E501, F401

                assert renormalize and use_grouped_topk and custom_routing_function is None
                e_score_correction_bias = (
                    e_score_correction_bias.to(x.dtype) if e_score_correction_bias is not None else None
                )
        ...
        if self.rocm_aiter_moe_enabled:
            from aphrodite.modeling.layers.fused_moe.rocm_aiter_fused_moe import rocm_aiter_fused_experts  # noqa: E501
            ...
            result = fused_experts(
                ...
                quant_config=self.moe_quant_config,
                allow_deep_gemm=self.allow_deep_gemm,
                allow_cutlass_block_scaled_grouped_gemm=(self.allow_cutlass_block_scaled_grouped_gemm),
            )
