...
from aphrodite.modeling.layers.fused_moe.flashinfer_cutlass_moe import is_valid_flashinfer_cutlass_fused_moe
...
from aphrodite.modeling.parameter import ModelWeightParameter, PerTensorScaleParameter
...
from aphrodite.quantization.utils.w8a8_utils import Fp8LinearOp, requantize_with_max_scale
from aphrodite.scalar_type import scalar_types
from aphrodite.utils.flashinfer import flashinfer_scaled_fp4_mm, has_flashinfer, has_flashinfer_moe
...
class ModelOptFp8Config(QuantizationConfig):
    ...
    def __init__(
        ...
        if is_checkpoint_fp8_serialized:
            logger.warning(
                "Detected ModelOpt fp8 checkpoint. Please note that the format is experimental and could change."
            )
    ...
    def apply_aphrodite_mapper(self, hf_to_aphrodite_mapper: "WeightsMapper"):
        if self.exclude_modules is not None:
            self.exclude_modules = hf_to_aphrodite_mapper.apply_list(self.exclude_modules)
    ...
    def from_config(cls, config: dict[str, Any]) -> "ModelOptFp8Config":
        ...
        if quant_method not in QUANT_ALGOS:
            raise ValueError(
                ...
                "quantizations in Aphrodite. Please check the "
    ...
    def is_layer_excluded(self, prefix: str) -> bool:
        ...
        for module in self.exclude_modules:
            ...
            if module != prefix and (
                module in prefix
                or (prefix.startswith("language_model.") and module in prefix.removeprefix("language_model."))
            ):
                return True
        return False
...
class ModelOptFp8LinearMethod(LinearMethodBase):
    ...
    def __init__(self, quant_config: ModelOptFp8Config) -> None:
        ...
        self.fp8_linear = Fp8LinearOp(act_quant_static=True, act_quant_group_shape=GroupShape.PER_TENSOR)
...
class ModelOptFp8MoEMethod(FusedMoEMethodBase):
    ...
    def __init__(
        ...
        if envs.APHRODITE_USE_FLASHINFER_MOE_FP8 and has_flashinfer_moe() and self.moe.is_act_and_mul:
            self.flashinfer_moe_backend = get_flashinfer_moe_backend()
            logger.info_once(f"Using FlashInfer {self.flashinfer_moe_backend.value} kernels")
    ...
    def maybe_make_prepare_finalize(
        ...
        if self.flashinfer_moe_backend == FlashinferMoeBackend.TENSORRT_LLM:
            ...
            prepare_finalize = build_flashinfer_fp8_cutlass_moe_prepare_finalize(self.moe)
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        from aphrodite.quantization.utils.w8a8_utils import per_tensor_dequantize
        ...
        if hasattr(layer, "w13_weight_scale") and layer.w13_weight_scale is not None:
            ...
            if layer.w13_weight_scale.dim() == 2 and layer.w13_weight_scale.shape[1] == 2:
                assert self.moe.is_act_and_mul, "w13_weight_scale should have 2 elements per expert only for gated MoE"
                ...
                for expert_id in range(layer.w13_weight.shape[0]):
                    ...
                    for shard_id in range(2):  # w1 and w3
                        ...
                        dq_weight = per_tensor_dequantize(
                            layer.w13_weight[expert_id][start : start + intermediate_size, :],
        ...
        if hasattr(layer, "w13_input_scale") and layer.w13_input_scale is not None:
            layer.w13_input_scale = Parameter(layer.w13_input_scale.max(), requires_grad=False)
        if hasattr(layer, "w2_input_scale") and layer.w2_input_scale is not None:
            layer.w2_input_scale = Parameter(layer.w2_input_scale.max(), requires_grad=False)
    ...
    def apply(
        ...
        if enable_eplb:
            raise NotImplementedError("EPLB not supported for `ModelOptFp8MoEMethod` yet.")

        if self.flashinfer_moe_backend == FlashinferMoeBackend.TENSORRT_LLM:
            assert self.fused_experts is None
            assert activation == "silu", f"Expected 'silu' activation but got {activation}"
    ...
    def from_config(cls, config: dict[str, Any]) -> "ModelOptNvFp4Config":
        ...
        if "quantization" in config:
            ...
            if kv_cache_quant_algo_raw is None:
                ...
                raise ValueError(f"kv_cache_quant_algo must be a string, got {type(kv_cache_quant_algo_raw)}")
...
class ModelOptNvFp4LinearMethod(LinearMethodBase):
    ...
    def __init__(self, quant_config: ModelOptNvFp4Config) -> None:
        ...
        if envs.APHRODITE_NVFP4_GEMM_BACKEND is None:
            if has_flashinfer() and current_platform.has_device_capability(90):
                ...
                self.backend = "marlin"
        elif envs.APHRODITE_NVFP4_GEMM_BACKEND.startswith("flashinfer-"):
            self.backend = envs.APHRODITE_NVFP4_GEMM_BACKEND
    ...
    def process_weights_after_loading(self, layer: Module) -> None:
        ...
        layer.alpha = Parameter(layer.input_scale * layer.weight_scale_2, requires_grad=False)
...
class ModelOptNvFp4FusedMoE(FusedMoEMethodBase):
    ...
    def __init__(
        ...
        from aphrodite.quantization.utils.nvfp4_moe_support import detect_nvfp4_moe_support  # noqa: E501
        ...
        if self.allow_flashinfer:
            self.flashinfer_moe_backend = get_flashinfer_moe_backend()
            logger.info_once(f"Using FlashInfer {self.flashinfer_moe_backend.value} kernels for ModelOptNvFp4FusedMoE.")

    def maybe_make_prepare_finalize(self) -> mk.FusedMoEPrepareAndFinalize | None:
        if self.use_marlin or (
            self.allow_flashinfer and self.flashinfer_moe_backend == FlashinferMoeBackend.TENSORRT_LLM
        ):
            return None
        elif self.allow_flashinfer and self.flashinfer_moe_backend == FlashinferMoeBackend.CUTLASS:
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if not torch.allclose(layer.w13_weight_scale_2[:, 0], layer.w13_weight_scale_2[:, 1]):
        ...
        layer.w13_input_scale_quant = Parameter((1 / w13_input_scale).to(torch.float32), requires_grad=False)
        ...
        # This is for quantization, so we need to invert it.
        layer.w2_input_scale_quant = Parameter((1 / w2_input_scale).to(torch.float32), requires_grad=False)
    ...
    def apply(
        ...
        if self.allow_flashinfer and self.flashinfer_moe_backend == FlashinferMoeBackend.TENSORRT_LLM:
            ...
            out = flashinfer.fused_moe.trtllm_fp4_block_scale_moe(
                routing_logits=router_logits if use_llama4_routing else router_logits.to(torch.float32),
        ...
        if self.use_marlin:
            ...
            assert is_valid_flashinfer_cutlass_fused_moe(x, layer.w13_weight, layer.w2_weight), (
            ...
            from aphrodite.modeling.layers.fused_moe.flashinfer_cutlass_moe import (  # noqa: E501
