...
def calc_chunks(cu_seqlen, moba_chunk_size):
    ...
    filtered_chunk_indices = torch.arange(num_chunk, device=cu_seqlen.device, dtype=torch.int32)
...
def _select_threshold_head_global(
    ...
    valid_flat = (valid_gate_mask & ~gate_self_chunk_mask).permute(1, 0, 2).reshape(H, CS)  # (H, C*S)
...
def moba_attn_varlen(
    ...
    cu_chunk, filtered_chunk_indices, num_filtered_chunk, chunk_to_batch = calc_chunks(cu_seqlens, moba_chunk_size)
    ...
    gate_seq_idx = (
        torch.arange(seqlen, device=q.device, dtype=torch.int32).unsqueeze(0).expand(num_filtered_chunk, seqlen)
    )
    ...
    gate_self_chunk_mask = (
        ((gate_seq_idx >= chunk_start.unsqueeze(1)) & (gate_seq_idx < chunk_end.unsqueeze(1)))
        .unsqueeze(1)
        .expand(-1, num_head, -1)
    )
    ...
    if select_mode == "topk":
        ...
        if threshold_type == "query_head":
            ...
            raise ValueError(
                f"Invalid threshold_type for topk: {threshold_type}. Choose 'query_head', 'block', or 'overall'."
            )
    ...
    if gate_mask.sum() == 0:
        return flash_attn_varlen_func(q, k, v, cu_seqlens, cu_seqlens, max_seqlen, max_seqlen, causal=False)
    ...
    assert moba_cu_seqlen_kv.shape == moba_cu_seqlen_q.shape, (
        f"Mismatch between moba_cu_seqlen_kv.shape and "
        f"moba_cu_seqlen_q.shape: {moba_cu_seqlen_kv.shape} vs "
        f"{moba_cu_seqlen_q.shape}"
    )
...
def process_moba_input(
    ...
    if isinstance(chunk_size, (float, int)):
        ...
        if len(chunk_size) == 2:
            assert patch_resolution[1] % chunk_size[0] == 0 and patch_resolution[2] % chunk_size[1] == 0, (
...
def test_attn_varlen_moba_speed(
    ...
    print(
        f"batch:{batch} head:{head} seqlen:{seqlen} chunk:{moba_chunk_size} "
        f"topk:{moba_topk} select_mode: {select_mode} "
        f"simsum_threshold:{simsum_threshold}"
    )
