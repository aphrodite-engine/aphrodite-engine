...
from aphrodite.diffusion.configs.models import (
    DiTConfig,
    EncoderConfig,
    ModelConfig,
    VAEConfig,
...
class PipelineConfig:
    ...
    def check_pipeline_config(self) -> None:
        ...
        if len(self.text_encoder_configs) != len(self.text_encoder_precisions):
            raise ValueError(
                f"Length of text encoder configs ({len(self.text_encoder_configs)}) "
                f"must be equal to length of text encoder precisions "
                f"({len(self.text_encoder_precisions)})"
            )

        if len(self.text_encoder_configs) != len(self.preprocess_text_funcs):
            raise ValueError(
                f"Length of text encoder configs ({len(self.text_encoder_configs)}) "
                f"must be equal to length of text preprocessing functions "
                f"({len(self.preprocess_text_funcs)})"
            )

        if len(self.preprocess_text_funcs) != len(self.postprocess_text_funcs):
            raise ValueError(
                f"Length of text postprocess functions "
                f"({len(self.postprocess_text_funcs)}) must be equal to length of "
                f"text preprocessing functions ({len(self.preprocess_text_funcs)})"
            )
