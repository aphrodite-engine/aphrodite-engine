...
from aphrodite.diffusion.configs.models import DiTConfig, EncoderConfig, VAEConfig
...
from aphrodite.diffusion.configs.models.vaes.flux import FluxVAEConfig
from aphrodite.diffusion.configs.pipelines.base import PipelineConfig, preprocess_text
...
class FluxPipelineConfig(PipelineConfig):
    ...
    # Text encoding stage
    text_encoder_configs: tuple[EncoderConfig, ...] = field(default_factory=lambda: (CLIPTextConfig(), T5Config()))
    ...
    def prepare_latent_shape(self, batch, batch_size, num_frames):
        height = 2 * (batch.height // (self.vae_config.arch_config.vae_scale_factor * 2))
    ...
    def prepare_pos_cond_kwargs(self, batch, device, rotary_emb, dtype):
        return {
            "freqs_cis": self.get_freqs_cis(batch.prompt_embeds[1], batch.width, batch.height, device, rotary_emb),
            "pooled_projections": (batch.pooled_embeds[0] if batch.pooled_embeds else None),
        }
    ...
    def prepare_neg_cond_kwargs(self, batch, device, rotary_emb, dtype):
        return {
            ...
            "pooled_projections": (batch.neg_pooled_embeds[0] if batch.neg_pooled_embeds else None),
        }
