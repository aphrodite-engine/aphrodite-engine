...
class SDXLVAEConfig(BaseVAEConfig):
    ...
    def __post_init__(self):
        ...
        if (
            not hasattr(self.arch_config, "scaling_factor")
            or self.arch_config.scaling_factor == 0
        ):
...
class SDXLPipelineConfig(PipelineConfig):
    ...
    text_encoder_configs: tuple[EncoderConfig, ...] = field(
        default_factory=lambda: (SDXLClipConfig(),)
    )
    ...
    preprocess_text_funcs: tuple[Callable[[str], str], ...] = field(
        default_factory=lambda: (sdxl_preprocess_text,)
    )
