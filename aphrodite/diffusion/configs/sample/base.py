...
class SamplingParams:
    ...
    def set_output_file_ext(self):
        ...
        if not any(self.output_file_name.endswith(ext) for ext in [".mp4", ".jpg", ".png", ".webp"]):
            self.output_file_name = f"{self.output_file_name}.{self.data_type.get_default_extension()}"

    def set_output_file_name(self):
        # settle output_file_name
        if self.output_file_name is None and self.prompt and isinstance(self.prompt, str):
    ...
    def from_pretrained(cls, model_path: str, **kwargs) -> "SamplingParams":
        from aphrodite.diffusion.configs.sample.registry import (
            get_sampling_param_cls_for_name,
        )

        sampling_cls = get_sampling_param_cls_for_name(model_path)
        logger.debug("Using pretrained SamplingParam: %s", sampling_cls)
    ...
    def _merge_with_user_params(self, user_params):
        ...
        for field in dataclasses.fields(user_params):
            ...
            if is_user_modified and field_name not in subclass_defined_fields and hasattr(self, field_name):
