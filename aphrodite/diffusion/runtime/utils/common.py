...
def configure_ipv6(dist_init_addr):
    ...
    try:
        ...
        raise ValueError(f"invalid port in IPv6 address: '{port_str}'") from err
    return port, host
...
def get_zmq_socket(context: zmq.Context, socket_type: zmq.SocketType, endpoint: str, bind: bool) -> zmq.Socket:
    ...
    if socket_type == zmq.PUSH:
        ...
        set_recv_opt()
    elif socket_type == zmq.DEALER or socket_type == zmq.REQ or socket_type == zmq.REP:
        set_send_opt()
        set_recv_opt()
    else:
...
def is_host_cpu_x86() -> bool:
    ...
    return machine in ("x86_64", "amd64", "i386", "i686") and hasattr(torch, "cpu") and torch.cpu.is_available()
...
def get_bool_env_var_legacy(name: str, default: str = "false") -> bool:
    ...
    if (value not in truthy_values) and (value not in falsy_values):
        if value not in _warned_bool_env_var_keys:
            logger.warning("get_bool_env_var(%s) see non-understandable value=%s and treat as false", name, value)
