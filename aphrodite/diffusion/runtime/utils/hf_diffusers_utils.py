...
def load_dict(file_path):
    ...
    try:
        ...
        raise RuntimeError(f"Failed to load diffusers config from {file_path}: {e}") from e
...
def verify_model_config_and_directory(model_path: str) -> dict[str, Any]:
    ...
    if "transformer" in model_index:
        ...
        if not os.path.exists(transformer_dir) and not os.path.exists(unet_dir):
            raise ValueError(f"Model directory {model_path} does not contain a transformer/ or unet/ directory.")
...
def maybe_download_model_index(model_name_or_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not os.path.exists(config_path):
            raise ValueError(
                f"Failed to find config.json for {model_name_or_path} after failing "
                f"to find model_index.json. You might be looking for models ending "
                f"with '-Diffusers'"
            ) from None
        with open(config_path) as f:
            config = json.load(f)
