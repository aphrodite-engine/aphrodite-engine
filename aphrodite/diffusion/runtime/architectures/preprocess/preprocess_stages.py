...
import torch
import torchvision
from aphrodite.diffusion.dataset.transform import (
...
from aphrodite.diffusion.configs.configs import VideoLoaderType
from aphrodite.diffusion.runtime.pipelines.pipeline_batch_info import (
    PreprocessBatch,
    Req,
)
...
class VideoTransformStage(PipelineStage):
    ...
    def forward(self, batch: Req, server_args: ServerArgs) -> Req:
        ...
        for i in range(len(batch.video_loader)):
            ...
            if server_args.preprocess_config.video_loader_type == VideoLoaderType.TORCHCODEC:
                video = batch.video_loader[i].get_frames_at(frame_indices).data
            elif server_args.preprocess_config.video_loader_type == VideoLoaderType.TORCHVISION:
                video, _, _ = torchvision.io.read_video(batch.video_loader[i], output_format="TCHW")
                video = video[frame_indices]
            else:
                raise ValueError(f"Invalid video loader type: {server_args.preprocess_config.video_loader_type}")
