...
class SchedulerClient:
    """
    A gateway for Scheduler, forwarding the ForwardBatch from http endpoints
    (or somewhere else) to background scheduler, with TCP socket
    """
    ...
    def initialize(self, server_args: ServerArgs):
        ...
        logger.info("Scheduler client connected to backend scheduler at %s", scheduler_endpoint)
    ...
    async def forward(self, batch: Req) -> Req:
        ...
        if self._closing:
            raise RuntimeError("SchedulerClient is closing; cannot forward new requests")
...
async def run_zeromq_broker(server_args: ServerArgs):
    ...
    logger.info("ZMQ Broker is listening for offline jobs on %s", broker_endpoint)
    ...
    while True:
        try:
            ...
            logger.exception("Error in ZMQ Broker: %s", e)
