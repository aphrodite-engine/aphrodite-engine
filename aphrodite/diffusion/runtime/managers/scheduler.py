...
from aphrodite.diffusion.runtime.managers.gpu_worker import GPUWorker
from aphrodite.diffusion.runtime.pipelines.pipeline_batch_info import OutputBatch
from aphrodite.diffusion.runtime.server_args import (
    PortArgs,
    ServerArgs,
    set_global_server_args,
)
from aphrodite.diffusion.runtime.utils.common import get_zmq_socket
from aphrodite.diffusion.runtime.utils.distributed import broadcast_pyobj
...
class Scheduler:
    ...
    def __init__(
        ...
        logger.info("Scheduler listening at endpoint: %s", endpoint)
        if gpu_id == 0:
    ...
    def event_loop(self) -> None:
        ...
        logger.info("Rank 0 scheduler listening on tcp://*:%s", self.server_args.scheduler_port)
        ...
        while self._running:
            ...
            try:
                ...
                logger.exception("Error receiving requests in scheduler event loop: %s", e)
                continue
            ...
            try:
                ...
                logger.exception("Error executing forward in scheduler event loop: %s", e)
