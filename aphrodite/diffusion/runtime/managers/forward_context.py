...
if TYPE_CHECKING:
    from aphrodite.diffusion.runtime.layers.attention import AttentionMetadata
    from aphrodite.diffusion.runtime.pipelines import Req
...
class ForwardContext:
    ...
    attention_backend_cls: type | None = None
    ...
    def set_attn_backend_cls(self, attention_backend_cls: type):
        if self.attention_backend_cls:
            if self.attention_backend_cls != attention_backend_cls:
                raise RuntimeError(
                    f"Different types of attention backend in a same context detected, "
                    f"previous: {self.attention_backend_cls}, new: {attention_backend_cls}"
                )
        else:
...
@contextmanager
def set_forward_context(current_timestep, attn_metadata, forward_batch: Optional["Req"] = None):
