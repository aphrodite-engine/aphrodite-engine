...
# Copyright 2025 The Aphrodite Authors.
...
from aphrodite.diffusion.runtime.loader.utils import (
...
from aphrodite.diffusion.utils import set_mixed_precision_policy
...
def load_model_from_full_model_state_dict(
    ...
    sharded_sd = {}
    custom_param_sd, reverse_param_names_mapping = hf_to_custom_state_dict(full_sd_iterator, param_names_mapping)  # type: ignore
    for target_param_name, full_tensor in custom_param_sd.items():
        meta_sharded_param = meta_sd.get(target_param_name)
        if meta_sharded_param is None:
            raise ValueError(
                f"Parameter {target_param_name} not found in custom model state dict. "
                "The hf to custom mapping may be incorrect."
            )
