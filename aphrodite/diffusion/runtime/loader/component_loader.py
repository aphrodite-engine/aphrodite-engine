...
from aphrodite.diffusion.configs.models import EncoderConfig
from aphrodite.diffusion.runtime.distributed import get_local_torch_device
from aphrodite.diffusion.runtime.loader.fsdp_load import (
    maybe_load_fsdp_model,
    shard_model,
)
from aphrodite.diffusion.runtime.loader.utils import set_default_torch_dtype
from aphrodite.diffusion.runtime.loader.weight_utils import (
    filter_duplicate_safetensors_files,
    filter_files_not_needed_for_inference,
    pt_weights_iterator,
    safetensors_weights_iterator,
)
from aphrodite.diffusion.runtime.models.registry import ModelRegistry
...
from aphrodite.diffusion.runtime.utils.hf_diffusers_utils import (
    get_config,
...
class TextEncoderLoader(ComponentLoader):
    ...
    def load_model(
        ...
        use_cpu_offload = (
            server_args.text_encoder_cpu_offload and len(getattr(model_config, "_fsdp_shard_conditions", [])) > 0
        )
        ...
        with set_default_torch_dtype(PRECISION_TO_TYPE[dtype]):
            ...
            if is_sdxl and os.path.basename(model_path) == "text_encoder":
                ...
                weights_iter_1, total_bytes_1 = self._get_all_weights_with_size(
                ...
                if os.path.exists(text_encoder_2_path):
                    weights_iter_2_raw, total_bytes_2 = self._get_all_weights_with_size(
                        model, text_encoder_2_path, to_cpu=use_cpu_offload
                    )
...
class VAELoader(ComponentLoader):
    ...
    def load(self, model_path: str, server_args: ServerArgs, *args):
        ...
        with set_default_torch_dtype(PRECISION_TO_TYPE[server_args.pipeline_config.vae_precision]):
        ...
        safetensors_list = filter_duplicate_safetensors_files(safetensors_list, str(model_path), index_file)
...
class TransformerLoader(ComponentLoader):
    ...
    def load(self, model_path: str, server_args: ServerArgs, *args):
        ...
        custom_weights_path = getattr(server_args, "init_weights_from_safetensors", None)
...
class UNetLoader(ComponentLoader):
    ...
    def load(self, model_path: str, server_args: ServerArgs, *args):
        ...
        with set_default_torch_dtype(default_dtype), get_local_torch_device():
            ...
            arch_dict = {k: v for k, v in unet_config.arch_config.__dict__.items() if k in valid_params}
        ...
        total_bytes = sum(os.path.getsize(f) for f in safetensors_list if os.path.exists(f))
        loaded_params = model.load_weights(tensor_progress_bar(weights_iter, total_bytes, "Loading UNet weights"))
...
class GenericComponentLoader(ComponentLoader):
    ...
    def load(self, model_path: str, server_args: ServerArgs, *args):
        ...
        logger.warning("Using generic loader for %s with library %s", model_path, self.library)
        ...
        if self.library == "transformers":
            ...
            logger.info("Loaded generic transformers model: %s", model.__class__.__name__)
            return model
...
class PipelineComponentLoader:
    ...
    def load_module(
        ...
        try:
            ...
            logger.error(
                "Error while loading component: %s, component_model_path=%s",
                module_name,
                component_model_path,
            )
            raise e
