...
from aphrodite.diffusion.runtime.managers.forward_context import (
    ForwardContext,
    get_forward_context,
)
...
class SlidingTileAttentionMetadata(AttentionMetadata):
    ...
    STA_param: list[list[Any]]  # each timestep with one metadata, shape [num_layers, num_heads]
...
class SlidingTileAttentionMetadataBuilder(AttentionMetadataBuilder):
    ...
    def build(  # type: ignore
        ...
        **kwargs: dict[str, Any],
        ...
        param = STA_param
        if param is None:
            return SlidingTileAttentionMetadata(current_timestep=current_timestep, STA_param=[])
        return SlidingTileAttentionMetadata(current_timestep=current_timestep, STA_param=param[current_timestep])
...
class SlidingTileAttentionImpl(AttentionImpl):
    ...
    def preprocess_qkv(
        ...
        self.img_seq_length = self.dit_seq_shape_int[0] * self.dit_seq_shape_int[1] * self.dit_seq_shape_int[2]
    ...
    def forward(
        ...
        if len(STA_param) < head_num * sp_group.world_size:
            ...
            for sparse_attn_hidden_states in sparse_attn_hidden_states_all:
                ...
                attn_L2_loss_ = (
                    torch.mean(
                        (sparse_attn_hidden_states.float() - hidden_states.float()) ** 2,
                        dim=[0, 1, 3],
                    )
                    .cpu()
                    .numpy()
                ...
                attn_L1_loss_ = (
                    torch.mean(
                        torch.abs(sparse_attn_hidden_states.float() - hidden_states.float()),
                        dim=[0, 1, 3],
                    )
                    .cpu()
                    .numpy()
