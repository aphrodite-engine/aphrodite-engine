...
from aphrodite.diffusion.utils import dict_to_3d_list
...
def configure_sta(
    ...
    **kwargs : dict
    ...
    if mode == "STA_searching":
        ...
        mask_selected: list[int] = kwargs.get("mask_selected", list(range(len(mask_candidates))))
        ...
        mask_selected_tuning: list[int] = kwargs.get("mask_selected", list(range(len(mask_candidates_tuning))))
        ...
        if save_dir_tuning is not None:
            ...
            file_path = os.path.join(save_dir_tuning, f"mask_strategy_s{skip_time_steps_tuning}.json")
        ...
        mask_search_files_path_pos: str | None = kwargs.get("mask_search_files_path_pos")
        mask_search_files_path_neg: str | None = kwargs.get("mask_search_files_path_neg")
        ...
        if not mask_search_files_path_pos or not mask_search_files_path_neg or not save_dir_cfg:
            raise ValueError(
                ...
                "save_dir are required for STA_tuning_cfg mode"
            )
        ...
        mask_selected_cfg: list[int] = kwargs.get("mask_selected", list(range(len(mask_candidates_cfg))))
        ...
        file_path = os.path.join(save_dir_cfg, f"mask_strategy_s{skip_time_steps_cfg}.json")
        with open(file_path, "w") as f:
            json.dump(mask_strategy, f, indent=4)
        print(f"Successfully saved mask_strategy to {file_path}")
        ...
        # Get parameters with defaults
        load_path: str | None = kwargs.get("load_path", "mask_candidates/mask_strategy.json")
            ...
            for h in range(head_num):
                if t < skip_time_steps:  # First steps use full attention
                    ...
                    for strategy in selected_masks[:-1]:  # Exclude full attention
                        head_losses.append(averaged_results[loss_type][str(strategy)][t][layer_idx][h])
