...
def get_attn_backend(
    ...
    if supported_attention_backends is not None:
        ...
        be_tuple = tuple(sorted(list(supported_attention_backends), key=lambda b: b.name))
    else:
        be_tuple = None
...
def _cached_get_attn_backend(
    ...
    if backend_by_global_setting is not None:
        ...
        if server_args.attention_backend is not None:
            try:
                selected_backend = AttentionBackendEnum[server_args.attention_backend.upper()]
    ...
    elif not supported_attention_backends or selected_backend not in supported_attention_backends:
        supported_attention_backends_str = [
            supported_attention_backend.__str__() for supported_attention_backend in supported_attention_backends
        ]
        logger.debug(
    ...
    attention_cls = current_platform.get_attn_backend_cls_str(selected_backend, head_size, dtype)
