...
def fuse_scale_shift_kernel(
    ...
    if scale.dim() == 4:
        ...
        if need_scale_scalar and need_shift_scalar and (scale_blc.abs().max() == 0) and (shift_blc.abs().max() == 0):
            output.copy_(x)
            return output
...
def _layer_norm_fwd(
    ...
    x1: Tensor | None = None,
    weight1: Tensor | None = None,
    bias1: Tensor | None = None,
    dropout_p: float = 0.0,
    rowscale: Tensor | None = None,
