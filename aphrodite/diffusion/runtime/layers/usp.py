...
def _usp_all_to_all_single(x: torch.Tensor) -> torch.Tensor:
    ...
    x_shape = x.shape
    x = x.flatten()
    x = ft_c.all_to_all_single(x, output_split_sizes=None, input_split_sizes=None, group=ulysses_pg)
    x = _maybe_wait(x)
    x = x.reshape(x_shape)
    return x
...
def _usp_input_all_to_all(x: torch.Tensor, head_dim: int = 1) -> torch.Tensor:
    ...
    # -> [b, h // world, s * world, d]
    x_c = x_c.reshape(world_size, h // world_size, b, -1, d).permute(2, 1, 0, 3, 4).reshape(b, h // world_size, -1, d)
...
def _usp_output_all_to_all(x: torch.Tensor, head_dim: int = 1) -> torch.Tensor:
    ...
    x_c = _usp_all_to_all_single(x_c)
    # -> [b, h * world, s // world, d]
    x_c = x_c.reshape(world_size, s // world_size, b, -1, d).permute(2, 0, 3, 1, 4).reshape(b, -1, s // world_size, d)
