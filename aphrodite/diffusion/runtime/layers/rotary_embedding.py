...
from aphrodite.diffusion.runtime.distributed.parallel_state import get_sp_group
from aphrodite.diffusion.runtime.layers.custom_op import CustomOp
from aphrodite.diffusion.runtime.layers.triton_ops import apply_rotary_embedding
...
class OneDRotaryEmbedding(torch.nn.Module):
    ...
    def build_freqs(self, device):
        freqs = 1.0 / (
            self.theta
            ** (torch.arange(0, self.dim, 2, dtype=self.dtype, device=device)[: (self.dim // 2)] / self.dim).to(
                device=device
            )
        )
        return freqs
    ...
    @functools.lru_cache(maxsize=16)  # noqa: B019  # noqa: B019
...
class NDRotaryEmbedding(torch.nn.Module):
    ...
    def forward_from_grid(
        ...
        return self._forward_cached_from_grid(grid_size, shard_dim, start_frame, device_str)
...
def get_1d_rotary_pos_embed(
    ...
    freqs = 1.0 / (
        theta ** (torch.arange(0, dim, 2, device=device)[: (dim // 2)].to(dtype) / dim).to(device=device)
    )  # [D/2]
...
def get_nd_rotary_pos_embed(
    ...
    *args,
    ...
    assert shard_dim < ndim, f"shard_dim {shard_dim} must be less than number of dimensions {ndim}"
    ...
    assert len(theta_rescale_factor) == ndim, "len(theta_rescale_factor) should equal to len(rope_dim_list)"

    if isinstance(interpolation_factor, int | float):
        interpolation_factor = [interpolation_factor] * ndim
    elif isinstance(interpolation_factor, list) and len(interpolation_factor) == 1:
        interpolation_factor = [interpolation_factor[0]] * ndim
    assert len(interpolation_factor) == ndim, "len(interpolation_factor) should equal to len(rope_dim_list)"
...
def get_rotary_pos_embed(
    ...
    key = (
        tuple(rope_dim_list),
        float(rope_theta),
        (tuple(theta_rescale_factor) if isinstance(theta_rescale_factor, list) else float(theta_rescale_factor)),
        (tuple(interpolation_factor) if isinstance(interpolation_factor, list) else float(interpolation_factor)),
        dtype,
    )
