...
from aphrodite.diffusion.runtime.layers.triton_ops import (
...
class RMSNorm(CustomOp):
    ...
    def __init__(
        ...
        if get_bool_env_var("APHRODITE_BATCH_INVARIANT"):
            self._forward_method = self.forward_native

    def forward_triton(self, x: torch.Tensor, residual: torch.Tensor | None = None):
        return rms_norm_fn(x, self.weight, bias=None, residual=residual, eps=self.variance_epsilon)
...
class LayerNorm(CustomOp):
    def __init__(
        ...
        if elementwise_affine:
            ...
            self.bias = torch.nn.Parameter(torch.empty(hidden_size, **factory_kwargs)) if bias else None
        else:
    ...
    def _get_weight_fallback(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if wf is None or wf.device != x.device or wf.dtype != x.dtype or wf.numel() != self.hidden_size:
