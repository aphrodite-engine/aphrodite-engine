...
class AutoencoderKL(nn.Module):
    ...
    def set_default_attn_processor(self):
        ...
        if all(proc.__class__ in ADDED_KV_ATTENTION_PROCESSORS for proc in self.attn_processors.values()):
            ...
            raise ValueError(
                f"Cannot call `set_default_attn_processor` when attention processors are "
                f"of type {next(iter(self.attn_processors.values()))}"
            )
    ...
    def decode(self, z: torch.FloatTensor) -> DecoderOutput | torch.FloatTensor:
        """
