...
class ParallelTiledVAE(ABC):
    ...
    def _parallel_data_generator(self, gathered_results, gathered_dim_metadata) -> Iterator[tuple[torch.Tensor, int]]:
        global_idx = 0
    ...
    def parallel_tiled_decode(self, z: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # TODO (PY): use aphrodite distributed methods
        ...
        data: list = [[[[] for _ in range(num_w_tiles)] for _ in range(num_h_tiles)] for _ in range(num_t_tiles)]
    ...
    def _merge_spatial_tiles(self, tiles, blend_height, blend_width, stride_height, stride_width) -> torch.Tensor:
