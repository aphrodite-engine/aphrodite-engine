...
from aphrodite.diffusion.runtime.models.vaes.common import (
...
@contextmanager
def forward_context(first_frame_arg=False, feat_cache_arg=None, feat_idx_arg=None, first_chunk_arg=None):
        ...
        if self.mode == "upsample3d":
            if _feat_cache is not None:
                ...
                if _feat_cache[idx] is None:
                    ...
                    if cache_x.shape[2] < 2 and _feat_cache[idx] is not None and _feat_cache[idx] != "Rep":
...
class AutoencoderKLWan(nn.Module, ParallelTiledVAE):
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        if self.use_feature_cache:
            ...
            with forward_context(feat_cache_arg=self._enc_feat_map, feat_idx_arg=self._enc_conv_idx):
                t = x.shape[2]
                iter_ = 1 + (t - 1) // 4
