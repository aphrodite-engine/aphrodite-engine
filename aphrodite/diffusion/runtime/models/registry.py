...
COMPONENT_DIRS = [
    ...
    if os.path.isdir(os.path.join(MODELS_PATH, d)) and not d.startswith("__") and not d.startswith(".")
]
                    ...
                    if model_cls_name_list:
                        for model_cls_str in model_cls_name_list:
                            if model_cls_str in discovered_models:
                                logger.warning(
                                    "Duplicate architecture found: %s. It will be overwritten.",
                                    model_cls_str,
                                )
...
_APHRODITE_MODELS = _discover_and_register_models()

_SUBPROCESS_COMMAND = [
    sys.executable,
    "-m",
    "aphrodite.diffusion.runtime.models.dits.registry",
]
...
ModelRegistry = _ModelRegistry(
    {
        model_arch: _LazyRegisteredModel(
            module_name=f"aphrodite.diffusion.runtime.models.{component_name}.{mod_relname}",
