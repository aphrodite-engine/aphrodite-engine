...
class UniPCMultistepScheduler(SchedulerMixin, ConfigMixin, BaseScheduler):
    ...
    def set_timesteps(
        ...
        if self.config.timestep_spacing == "linspace":
            ...
            raise ValueError(
                f"{self.config.timestep_spacing} is not supported. Please make sure to "
    ...
    # Copied from
    # diffusers.schedulers.scheduling_dpmsolver_multistep.DPMSolverMultistepScheduler._sigma_to_alpha_sigma_t
        ...
        **kwargs,
        ...
        if prev_timestep is not None:
            deprecate(
                "prev_timestep",
                "1.0.0",
                "Passing `prev_timestep` is deprecated and has no effect as model output "
                "conversion is now handled via an internal counter `self.step_index`",
            )
        ...
        **kwargs,
        ...
        if this_timestep is not None:
            deprecate(
                "this_timestep",
                "1.0.0",
                "Passing `this_timestep` is deprecated and has no effect as model output "
                "conversion is now handled via an internal counter `self.step_index`",
            )
