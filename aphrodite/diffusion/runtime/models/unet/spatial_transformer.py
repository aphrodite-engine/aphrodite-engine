...
class CrossAttention(nn.Module):
    ...
    def forward(
        ...
        if x_max_abs > 1000:
            logger.warning(
                f"Large input x to CrossAttention! x_max_abs={x_max_abs:.2f}, x stats: min={x.min().item():.6f}, max={x.max().item():.6f}"
            )
        ...
        if torch.isnan(x).any() or torch.isinf(x).any():
            logger.warning(
                f"NaN/inf in CrossAttention input x! x stats: min={x.min().item():.6f}, max={x.max().item():.6f}"
            )
        ...
        if context is not None and context is not x:
            ...
            if context_max_abs > 1000:
                logger.warning(
                    f"Large context values in CrossAttention! context_max_abs={context_max_abs:.2f}, "
        ...
        if torch.isnan(q).any():
            raise ValueError("NaN in q after to_q projection!")
        if torch.isnan(k).any():
            raise ValueError(f"NaN in k after to_k projection! context shape: {context.shape}")
        if torch.isnan(v).any():
            raise ValueError("NaN in v after to_v projection!")

        # Log Q/K/V stats to understand magnitude
        if torch.isinf(q).any() or torch.isinf(k).any():
            logger.warning(
                f"Inf in Q/K after projections! q stats: min={q.min().item():.6f}, max={q.max().item():.6f}, "
        ...
        if torch.isnan(q_fp32).any() or torch.isnan(k_fp32).any():
            raise ValueError("NaN in q or k after float conversion!")
        if torch.isinf(q_fp32).any() or torch.isinf(k_fp32).any():
            ...
            logger.warning(
                f"Inf values in q or k before einsum! q stats: min={q_fp32.min().item():.6f}, max={q_fp32.max().item():.6f}, k stats: min={k_fp32.min().item():.6f}, max={k_fp32.max().item():.6f}"
            )
        ...
        if q_max_abs > 1000 or k_max_abs > 1000:
            ...
            logger.warning(
                f"Large Q/K values before einsum! q_max_abs={q_max_abs:.2e}, k_max_abs={k_max_abs:.2e}, "
        ...
        if torch.isnan(sim).any():
            raise ValueError(
                f"NaN in sim after einsum! q stats: min={q.min().item():.6f}, max={q.max().item():.6f}, k stats: min={k.min().item():.6f}, max={k.max().item():.6f}, scale={self.scale}"
            )
        if torch.isinf(sim).any():
            # Clamp sim to prevent inf values (this can happen if q/k are very large)
            logger.warning(
                f"Inf values in sim after einsum! q_max_abs={q_max_abs:.2f}, k_max_abs={k_max_abs:.2f}, "
        ...
        if torch.isnan(sim).any():
            raise ValueError("NaN in sim before softmax!")
        ...
        if torch.isnan(attn).any():
            raise ValueError(
                f"NaN in attn after softmax! sim stats: min={sim.min().item():.6f}, max={sim.max().item():.6f}, sim has inf: {torch.isinf(sim).any().item()}"
            )
        ...
        if torch.isnan(out).any():
            raise ValueError("NaN in out after final einsum!")
...
class BasicTransformerBlock(nn.Module):
    ...
    def _forward(
        ...
        if torch.isinf(x).any():
            ...
            if torch.isnan(x).any():
                raise ValueError("NaN after clamping inf in BasicTransformerBlock input!")
        ...
        if not self.disable_self_attn:
            ...
            if torch.isnan(x_norm).any():
                raise ValueError(
                    f"NaN after BasicTransformerBlock norm1! x stats before norm1: min={x.min().item():.6f}, max={x.max().item():.6f}, has_inf={torch.isinf(x).any().item()}"
                )
            ...
            if torch.isnan(attn1_out).any():
                raise ValueError("NaN after BasicTransformerBlock attn1!")
            ...
            if torch.isnan(x).any() or torch.isinf(x).any():
                ...
                x = torch.clamp(x, min=-1e6, max=1e6)
                if torch.isnan(x).any():
                    raise ValueError("NaN after adding attn1 output!")
        ...
        if torch.isnan(x_norm).any():
            raise ValueError("NaN after BasicTransformerBlock norm2!")
        ...
        if torch.isnan(attn2_out).any():
            raise ValueError("NaN after BasicTransformerBlock attn2!")
        ...
        if torch.isnan(x).any() or torch.isinf(x).any():
            ...
            x = torch.clamp(x, min=-1e6, max=1e6)
            if torch.isnan(x).any():
                raise ValueError("NaN after adding attn2 output!")
        ...
        if torch.isnan(x).any():
            raise ValueError(
                f"NaN in x before BasicTransformerBlock norm3! x stats: min={x.min().item():.6f}, max={x.max().item():.6f}"
            )
        x_norm = self.norm3(x)
        # Check for NaN after norm3
        if torch.isnan(x_norm).any():
            raise ValueError(
                f"NaN after BasicTransformerBlock norm3! x stats: min={x.min().item():.6f}, max={x.max().item():.6f}, x_norm stats: min={x_norm.min().item():.6f}, max={x_norm.max().item():.6f}"
            )
        ff_out, _ = self.ff_0(x_norm)
        # Check for NaN after ff_0
        if torch.isnan(ff_out).any():
            raise ValueError("NaN after BasicTransformerBlock ff_0!")
        if self.ff_gated:
        ...
        if torch.isnan(ff_out).any():
            raise ValueError("NaN after BasicTransformerBlock activation!")
        ...
        if torch.isinf(ff_out_before_ff2).any():
            logger.warning(
                f"Inf values detected before ff_2, clamping to safe range. Stats: min={ff_out_before_ff2.min().item():.6f}, max={ff_out_before_ff2.max().item():.6f}"
            )
        ...
        if torch.isnan(ff_out).any():
            logger.error(
                f"NaN after BasicTransformerBlock ff_2! Input stats: min={ff_out_before_ff2.min().item():.6f}, max={ff_out_before_ff2.max().item():.6f}, "
        ...
        if torch.isnan(x).any():
            raise ValueError("NaN after adding ff_out!")
    ...
    def forward(
        ...
        if not self.proj_in_linear:
            ...
            if torch.isnan(x_in).any():
                raise ValueError("NaN after SpatialTransformer proj_in (conv)!")
        else:
            x_flat = x.flatten(2).transpose(1, 2)  # [B, H*W, C]
            # Check for NaN before proj_in
            if torch.isnan(x_flat).any():
                raise ValueError("NaN in x_flat before proj_in!")
            x_in, _ = self.proj_in(x_flat)
            # Check for NaN after proj_in (linear)
            if torch.isnan(x_in).any():
                raise ValueError(
                    f"NaN after SpatialTransformer proj_in (linear)! x_flat shape: {x_flat.shape}, x_in shape: {x_in.shape}"
                )
            x_in = x_in.transpose(1, 2).view(b, -1, h, w)
            # Check for NaN after reshape
            if torch.isnan(x_in).any():
                raise ValueError(
                    f"NaN after proj_in reshape! Expected shape: [B, {self.n_heads * self.d_head}, H, W], got: {x_in.shape}"
                )
        ...
        if torch.isnan(x_in).any():
            raise ValueError("NaN in x_in before transformer blocks!")
        ...
        if torch.isnan(x_in).any():
            raise ValueError("NaN after reshape back!")
        ...
        if not self.proj_out_linear:
            ...
            if torch.isnan(x_out).any():
                raise ValueError("NaN after SpatialTransformer proj_out (conv)!")
        else:
            x_flat = x_in.flatten(2).transpose(1, 2)  # [B, H*W, inner_dim]
            x_out, _ = self.proj_out(x_flat)
            # Check for NaN after proj_out (linear)
            if torch.isnan(x_out).any():
                raise ValueError("NaN after SpatialTransformer proj_out (linear)!")
        ...
        if torch.isnan(result).any():
            raise ValueError(
                f"NaN in SpatialTransformer final output! x stats: min={x.min().item():.6f}, max={x.max().item():.6f}, x_out stats: min={x_out.min().item():.6f}, max={x_out.max().item():.6f}"
            )
