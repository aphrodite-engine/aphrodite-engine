...
class SDXLUNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.input_blocks = nn.ModuleList(
            [TimestepEmbedSequential(nn.Conv2d(in_channels, model_channels, 3, padding=1, dtype=dtype, device=device))]
        )
            ...
            if should_downsample:
                ...
                self.input_blocks.append(
                    TimestepEmbedSequential(
                        ...
                        if resblock_updown
                        else Downsample(ch, conv_resample, dims=dims, out_channels=out_ch, dtype=dtype, device=device)
                    )
                )
                ch = out_ch
                input_block_chans.append(ch)
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if torch.isnan(t_emb).any():
            raise ValueError(
                f"NaN in timestep embedding input (t_emb)! t_emb shape: {t_emb.shape}, timesteps: {timesteps}, t_emb stats: min={t_emb.min().item():.6f}, max={t_emb.max().item():.6f}"
            )
        ...
        if torch.isnan(emb_out).any():
            raise ValueError(
                f"NaN after time_embed_0! t_emb stats: min={t_emb.min().item():.6f}, max={t_emb.max().item():.6f}"
            )

        emb = self.time_embed_act(emb_out)
        # Check for NaN after activation
        if torch.isnan(emb).any():
            raise ValueError("NaN after time_embed_act!")

        emb_out, _ = self.time_embed_2(emb)
        # Check for NaN after time_embed_2
        if torch.isnan(emb_out).any():
            raise ValueError("NaN after time_embed_2!")
        emb = emb_out
        ...
        if self.label_emb_0 is not None and y is not None:
            ...
            if torch.isnan(y).any():
                raise ValueError(
                    f"NaN in ADM conditioning (y) input! y shape: {y.shape}, y stats: min={y.min().item():.6f}, max={y.max().item():.6f}"
                )
            ...
            if torch.isnan(adm_emb_out).any():
                raise ValueError("NaN after label_emb_2!")

            emb = emb + adm_emb_out
            # Check for NaN after adding ADM to timestep embedding
            if torch.isnan(emb).any():
                raise ValueError("NaN after adding ADM to timestep embedding!")
        ...
        if torch.isnan(emb).any():
            raise ValueError("NaN in timestep embedding before input blocks!")
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
                ...
                if param.size() != loaded_weight.size():
                    ...
                    if "proj_in" in name or "proj_out" in name:
                        if len(param_shape) == 4 and len(loaded_shape) == 2:
                            ...
                            if (
                                param_shape[0] == loaded_shape[0]
                                and param_shape[1] == loaded_shape[1]
                                and param_shape[2] == 1
                                and param_shape[3] == 1
                            ):
                                ...
                                logger.info(
                                    f"Converting Linear to Conv2d weight '{name}': {loaded_shape} -> {param_shape}"
                                )
                            elif (
                                param_shape[0] == loaded_shape[0] * 2
                                and param_shape[1] == loaded_shape[1] * 2
                                and param_shape[2] == 1
                                and param_shape[3] == 1
                            ):
                                ...
                                logger.warning(
                                    f"Resizing and converting Linear to Conv2d weight '{name}': {loaded_shape} -> {param_shape} (doubling channels)"
                                )
                            elif (
                                param_shape[0] == loaded_shape[0] * 2
                                and param_shape[1] == loaded_shape[1]
                                ...
                                transformed_weight = resized.unsqueeze(-1).unsqueeze(-1)
                                logger.warning(
                                    f"Resizing and converting Linear to Conv2d weight '{name}': {loaded_shape} -> {param_shape} (doubling output channels)"
                                )
                            elif (
                                param_shape[0] == loaded_shape[0]
                                and param_shape[1] == loaded_shape[1] * 2
                                ...
                                transformed_weight = resized.unsqueeze(-1).unsqueeze(-1)
                                logger.warning(
                                    f"Resizing and converting Linear to Conv2d weight '{name}': {loaded_shape} -> {param_shape} (doubling input channels)"
                                )
                            ...
                            if (
                                param_shape[0] == loaded_shape[0]
                                and param_shape[1] == loaded_shape[1]
                                and loaded_shape[2] == 1
                                and loaded_shape[3] == 1
                            ):
                                transformed_weight = loaded_weight.squeeze(-1).squeeze(-1)
                                logger.info(
                                    f"Converting Conv2d to Linear weight '{name}': {loaded_shape} -> {param_shape}"
                                )
                    ...
                    if transformed_weight is None and len(param_shape) == len(loaded_shape) and len(param_shape) >= 2:
                        ...
                        if len(param_shape) == 2:
                            ...
                            if param_shape[0] == loaded_shape[0] * 2 and param_shape[1] == loaded_shape[1] * 2:
                                ...
                                logger.warning(
                                    f"Resizing weight '{name}': {loaded_shape} -> {param_shape} (doubling both dimensions)"
                                )
                            # Check if only first dimension needs doubling
                                ...
                                logger.warning(
                                    f"Resizing weight '{name}': {loaded_shape} -> {param_shape} (doubling output channels)"
                                )
                            # Check if only second dimension needs doubling
                                ...
                                logger.warning(
                                    f"Resizing weight '{name}': {loaded_shape} -> {param_shape} (doubling input channels)"
                                )
                            # Check if both dimensions need halving
                                ...
                                logger.warning(
                                    f"Resizing weight '{name}': {loaded_shape} -> {param_shape} (halving both dimensions)"
                                )
                            # Check if only first dimension needs halving
                                ...
                                logger.warning(
                                    f"Resizing weight '{name}': {loaded_shape} -> {param_shape} (halving output channels)"
                                )
                            # Check if only second dimension needs halving
                                ...
                                logger.warning(
                                    f"Resizing weight '{name}': {loaded_shape} -> {param_shape} (halving input channels)"
                                )
                        # For higher-dimensional tensors (e.g., conv weights), handle first dimension
                            ...
                            logger.warning(
                                f"Resizing weight '{name}': {loaded_shape} -> {param_shape} (doubling channels)"
                            )
                        elif param_shape[0] == loaded_shape[0] // 2 and param_shape[1:] == loaded_shape[1:]:
                            # Halve the channels: take first half
                            transformed_weight = loaded_weight[: param_shape[0]]
                            logger.warning(
                                f"Resizing weight '{name}': {loaded_shape} -> {param_shape} (halving channels)"
                            )
                    ...
                    if transformed_weight is not None and transformed_weight.shape == param.shape:
                        ...
                        if max_abs > 1.0:
                            logger.warning(
                                f"Clamping transformed weight '{name}' with max abs value {max_abs:.6e} to [-1.0, 1.0]"
                            )
                        ...
                        if original_name:
                            logger.warning(
                                f"Skipping weight '{name}' (from '{original_name}'): param size {param.size()}, loaded size {loaded_weight.size()} - will remain randomly initialized"
                            )
                        else:
                            logger.warning(
                                f"Skipping weight '{name}': param size {param.size()}, loaded size {loaded_weight.size()} - will remain randomly initialized"
                            )
        ...
        if missing_weights:
            ...
            actual_param_names = [name for name in params_dict if "attn" in name and "weight" in name][:10]
