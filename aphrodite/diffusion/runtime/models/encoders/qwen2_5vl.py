...
from transformers import Cache, DynamicCache, PretrainedConfig, Qwen2_5_VLTextConfig, Qwen2RMSNorm
from transformers.masking_utils import create_causal_mask, create_sliding_window_causal_mask
from transformers.modeling_flash_attention_utils import FlashAttentionKwargs
from transformers.modeling_outputs import BaseModelOutputWithPast
from transformers.utils import TransformersKwargs, is_torchdynamo_compiling

from aphrodite.diffusion.configs.models.encoders.qwen_image import Qwen2_5VLConfig
from aphrodite.diffusion.runtime.layers.attention import LocalAttention
from aphrodite.diffusion.runtime.layers.linear import MergedColumnParallelLinear, RowParallelLinear
...
from aphrodite.diffusion.runtime.utils.common import add_prefix
...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def __init__(self, config: Qwen2_5_VLTextConfig, layer_idx: int | None = None):
        ...
        if layer_idx is None:
            logger.warning(
                "Instantiating %s without passing `layer_idx` is not recommended and will "
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Currently using eager attention
...
class Qwen2_5_VLDecoderLayer(nn.Module):
    def __init__(self, config: Qwen2_5_VLTextConfig, layer_idx: int):
        ...
        if config.use_sliding_window and config._attn_implementation != "flash_attention_2":
            logger.warning(
                "Sliding Window Attention is enabled but not implemented for `%s`; "
...
class Qwen2_5_VLModel(nn.Module):
    ...
    def get_rope_index(
        ...
        Explanation:
            ...
            Examples:
                ...
                tokens_per_second: This is a crucial parameter. It dictates how many
                    "time-steps" or "temporal tokens" are conceptually packed into a
                    one-second interval of the video. In this case, we have 25 tokens per
                    second. So each second of the video will be represented with 25
                    separate time points. It essentially defines the temporal granularity.
                temporal_patch_size: The number of frames that compose one temporal patch.
                    Here, it's 2 frames.
                interval: The step size for the temporal position IDs, calculated as
                    tokens_per_second * temporal_patch_size / fps. In this case,
                    25 * 2 / 1 = 50. This means that each temporal patch will be have
                    a difference of 50 in the temporal position IDs.
                input_ids: [V V V V V V V V V V V V T T T T T],
                    here V is for vision.
                ...
                Here we calculate the text start position_ids as the max vision
                    position_ids plus 1.
    ...
    def get_placeholder_mask(
        ...
        """
        Obtains multimodal placeholder mask from `input_ids` or `inputs_embeds`, and checks that the placeholder token
        count is equal to the length of multimodal features. If the lengths are different,
        an error is raised.
        """
