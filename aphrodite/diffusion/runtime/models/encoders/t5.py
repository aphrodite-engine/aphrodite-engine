...
from aphrodite.diffusion.configs.models.encoders import BaseEncoderOutput, T5Config
from aphrodite.diffusion.runtime.distributed import get_tp_rank, get_tp_world_size
...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            attention_mask = (
                attention_mask.view(bs, 1, 1, -1) if attention_mask.ndim == 2 else attention_mask.unsqueeze(1)
            )
...
class T5Block(nn.Module):
    def __init__(
        ...
        if self.is_decoder:
            self.layer.append(T5LayerCrossAttention(config, quant_config=quant_config, prefix=f"{prefix}.cross_attn"))
