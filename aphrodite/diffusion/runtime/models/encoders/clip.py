...
from aphrodite.diffusion.runtime.distributed import divide, get_tp_world_size
from aphrodite.diffusion.runtime.layers.activation import get_act_fn
from aphrodite.diffusion.runtime.layers.attention import LocalAttention
from aphrodite.diffusion.runtime.layers.linear import (
...
from aphrodite.diffusion.runtime.models.encoders.base import ImageEncoder, TextEncoder
from aphrodite.diffusion.runtime.models.encoders.vision import (
...
class CLIPEncoder(nn.Module):
    ...
    def forward(self, inputs_embeds: torch.Tensor, return_all_hidden_states: bool) -> torch.Tensor | list[torch.Tensor]:
...
class CLIPTextTransformer(nn.Module):
    ...
    def forward(
        ...
        if self.eos_token_id == 2:
            ...
            pooled_output = last_hidden_state[
                torch.arange(last_hidden_state.shape[0], device=last_hidden_state.device),
                # We need to get the first position of `eos_token_id` value
                # (`pad_token_ids` might equal to `eos_token_id`)
                # Note: we assume each sequence (along batch dim.) contains an
                # `eos_token_id` (e.g. prepared by the tokenizer)
...
class CLIPTextModel(TextEncoder):
    def __init__(
        ...
        self.text_model = CLIPTextTransformer(config=config, quant_config=config.quant_config, prefix=config.prefix)
