...
from aphrodite.diffusion.configs.models.dits.flux import FluxConfig
from aphrodite.diffusion.runtime.layers.attention import LocalAttention

# from aphrodite.diffusion.runtime.layers.layernorm import LayerNorm as LayerNorm
from aphrodite.diffusion.runtime.layers.layernorm import RMSNorm
from aphrodite.diffusion.runtime.layers.linear import ReplicatedLinear
from aphrodite.diffusion.runtime.layers.mlp import MLP
from aphrodite.diffusion.runtime.layers.rotary_embedding import (
    NDRotaryEmbedding,
    _apply_rotary_emb,
)
from aphrodite.diffusion.runtime.models.dits.base import CachableDiT
...
class FluxAttention(torch.nn.Module, AttentionModuleMixin):
    def __init__(
        ...
        if added_kv_proj_dim is not None:
            ...
            self.add_q_proj = ReplicatedLinear(added_kv_proj_dim, self.inner_dim, bias=added_proj_bias)
            self.add_k_proj = ReplicatedLinear(added_kv_proj_dim, self.inner_dim, bias=added_proj_bias)
            self.add_v_proj = ReplicatedLinear(added_kv_proj_dim, self.inner_dim, bias=added_proj_bias)
    ...
    def forward(
        ...
        if freqs_cis is not None:
            cos, sin = freqs_cis
            query = _apply_rotary_emb(query, cos, sin, is_neox_style=False, interleaved=False)
            key = _apply_rotary_emb(key, cos, sin, is_neox_style=False, interleaved=False)
...
class FluxTransformerBlock(nn.Module):
    def __init__(
        ...
        self.ff = MLP(input_dim=dim, mlp_hidden_dim=dim * 4, output_dim=dim, act_type="gelu")
        self.ff = FeedForward(dim=dim, dim_out=dim, activation_fn="gelu-approximate")

        self.norm2_context = LayerNorm(dim, eps=1e-6, elementwise_affine=False)
        self.ff_context = MLP(input_dim=dim, mlp_hidden_dim=dim * 4, output_dim=dim, act_type="gelu")
...
class FluxTransformer2DModel(CachableDiT):
    ...
    def __init__(self, config: FluxConfig, hf_config: dict[str, Any]) -> None:
        ...
        self.out_channels = getattr(self.config, "out_channels", None) or self.config.in_channels
        ...
        self.context_embedder = ReplicatedLinear(self.config.joint_attention_dim, self.inner_dim)
