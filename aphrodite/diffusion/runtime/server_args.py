...
from aphrodite.diffusion.configs.configs import PreprocessConfig
from aphrodite.diffusion.configs.pipelines import FluxPipelineConfig
from aphrodite.diffusion.configs.pipelines.base import PipelineConfig, STA_Mode
from aphrodite.diffusion.configs.pipelines.qwen_image import (
    QwenImageEditPipelineConfig,
    QwenImagePipelineConfig,
)
from aphrodite.diffusion.runtime.platforms import (
    AttentionBackendEnum,
    current_platform,
)
from aphrodite.diffusion.runtime.utils.common import (
    is_port_available,
...
class ServerArgs:
    ...
    def is_local_mode(self) -> bool:
        """
        If no server is running when a generation task begins, 'local_mode' will be
        enabled: a dedicated server will be launched
        """
    ...
    def __post_init__(self):
        ...
        try:
            ...
            logger.debug("server_args: %s", json.dumps(safe_args, ensure_ascii=False))
        except Exception:
            # Fallback to default repr if sanitization fails
            logger.debug("server_args: %s", self)
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        parser.add_argument(
            ...
            help=(
                "The attention backend to use. If not specified, the backend is "
                "automatically selected based on hardware and installed packages."
            ),
        )
        ...
        parser.add_argument(
            ...
            action=StoreBoolean,
            help=(
                "Use FSDP for inference by sharding the model weights. Latency is very "
                "low due to prefetch--enable if run out of memory."
            ),
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            action=StoreBoolean,
            help=(
                "Pin memory for CPU offload. Only added as a temp workaround if it "
                'throws "CUDA error: invalid argument". Should be enabled in almost all cases'
            ),
        )
        parser.add_argument(
            "--disable-autocast",
    ...
    @classmethod
    def from_cli_args(cls, args: argparse.Namespace, unknown_args: list[str] | None = None) -> "ServerArgs":
    ...
    def from_dict(cls, kwargs: dict[str, Any]) -> "ServerArgs":
        ...
        for attr in attrs:
            if attr == "pipeline_config":
                ...
                logger.debug("Using PipelineConfig: %s", type(pipeline_config))
    ...
    def load_config_file(config_file: str) -> dict[str, Any]:
        ...
        if config_file.endswith(".json"):
            ...
            try:
                ...
                raise ImportError("Please install PyYAML to use YAML config files. `pip install pyyaml`") from err
            with open(config_file) as f:
    ...
    @staticmethod
    def get_provided_args(args: argparse.Namespace, unknown_args: list[str]) -> dict[str, Any]:
    ...
    def check_server_sp_args(self):
        if self.pipeline_config.is_image_gen:
            if (
                ...
                raise ValueError("SP is not supported for image generation models for now")
        ...
        if self.ulysses_degree is None and self.ring_degree is None and self.sp_degree != 1:
            ...
            logger.info(
                "Automatically set ulysses_degree=sp_degree=%s for best performance",
                self.ulysses_degree,
            )

        if self.ulysses_degree is None:
            self.ulysses_degree = 1
            logger.info("Ulysses degree not set, using default value %s", self.ulysses_degree)

        if self.ring_degree is None:
            self.ring_degree = 1
            logger.info("Ring degree not set, using default value %s", self.ring_degree)

        if self.ring_degree > 1:
            if self.attention_backend is not None and self.attention_backend != "fa3":
                raise ValueError("Ring Attention is only supported for fa3 backend for now")
            else:
                self.attention_backend = "fa3"
                logger.info(
                    "Ring Attention is currently only supported for fa3, "
                    "attention_backend has been automatically set to fa3"
                )

        if self.sp_degree == -1:
            self.sp_degree = self.ring_degree * self.ulysses_degree
            logger.info(
                "sequence_parallel_degree is not provided, using ring_degree * ulysses_degree = %s",
                self.sp_degree,
            )

        if self.sp_degree != self.ring_degree * self.ulysses_degree:
            raise ValueError(
                f"sequence_parallel_degree is not equal to ring_degree * ulysses_degree, "
                f"{self.sp_degree} != {self.ring_degree} * {self.ulysses_degree}"
            )
    ...
    def check_server_dp_args(self):
        ...
        logger.info("Setting dp_degree to: %s", self.dp_degree)
    ...
    def check_server_args(self) -> None:
        ...
        is_flux = isinstance(
            ...
            (FluxPipelineConfig, QwenImagePipelineConfig, QwenImageEditPipelineConfig),
        )
        if is_flux:
        ...
        assert self.sp_degree <= self.num_gpus and self.num_gpus % self.sp_degree == 0, (
        ...
        if self.mode == ExecutionMode.PREPROCESS:
            if self.preprocess_config is None:
                raise ValueError("preprocess_config is not set in ServerArgs when mode is PREPROCESS")
        ...
        if self.enable_cfg_parallel and self.num_gpus == 1:
            raise ValueError("CFG Parallelism is enabled via `--enable-cfg-parallel`, while -num-gpus==1")
...
class PortArgs:
    ...
    @staticmethod
    def from_server_args(server_args: ServerArgs, dp_rank: int | None = None) -> "PortArgs":
