...
from aphrodite.diffusion.runtime.entrypoints.openai.stores import IMAGE_STORE
...
def _choose_ext(output_format: str | None, background: str | None) -> str:
...
async def edits(
    ...
    image_array: list[UploadFile] | None = File(None, alias="image[]"),  # noqa: B008
    prompt: str = Form(...),
    ...
    if (response_format or "b64_json").lower() == "b64_json":
        with open(save_file_path, "rb") as f:
            b64 = base64.b64encode(f.read()).decode("utf-8")
        return ImageResponse(data=[ImageResponseData(b64_json=b64, revised_prompt=prompt)])
    else:
