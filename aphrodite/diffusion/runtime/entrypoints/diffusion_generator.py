...
from aphrodite.diffusion.runtime.entrypoints.utils import prepare_request  # noqa: E402
from aphrodite.diffusion.runtime.launch_server import launch_server  # noqa: E402
from aphrodite.diffusion.runtime.managers.schedulerbase import SchedulerBase  # noqa: E402
from aphrodite.diffusion.runtime.pipelines.pipeline_batch_info import OutputBatch, Req  # noqa: E402
from aphrodite.diffusion.runtime.server_args import PortArgs, ServerArgs  # noqa: E402
from aphrodite.diffusion.runtime.sync_scheduler_client import sync_scheduler_client  # noqa: E402
...
class DiffGenerator:
        ...
        **kwargs,
        ...
        Args:
            **kwargs: Additional arguments to customize model loading,
                set any ServerArgs or PipelineConfig attributes here.
        ...
        if (server_args := kwargs.get("server_args")) is not None:
    ...
    def from_server_args(cls, server_args: ServerArgs) -> "DiffGenerator":
        ...
        Returns:
            The created DiffGenerator
        """
        SchedulerBase.get_class(server_args)
        instance = cls(
            server_args=server_args,
        )
        is_local_mode = server_args.is_local_mode
        logger.info("Local mode: %s", is_local_mode)
        if is_local_mode:
    ...
    def _check_remote_scheduler(self):
        ...
        logger.info(
            "Successfully connected to remote scheduler at %s.",
            self.server_args.scheduler_endpoint(),
        )
    ...
    def generate(
        ...
        **kwargs,
        ...
        pretrained_sampling_params = SamplingParams.from_pretrained(self.server_args.model_path, **kwargs)
        pretrained_sampling_params._merge_with_user_params(sampling_params)
        # TODO: simplify
        data_type = (
            DataType.IMAGE
            if self.server_args.pipeline_config.is_image_gen or pretrained_sampling_params.num_frames == 1
            else DataType.VIDEO
        )
        ...
        for request_idx, req in enumerate(requests):
            ...
            try:
                ...
                for output_idx, sample in enumerate(output_batch.output):
                    ...
                    save_path = os.path.join(req.output_path, output_file_name) if output_file_name else None
                ...
                logger.error("Failed to generate output for prompt %d: %s", request_idx + 1, e)
                continue
    ...
    def shutdown(self):
        ...
        if self.local_scheduler_process:
            ...
            for process in self.local_scheduler_process:
                ...
                if process.is_alive():
                    logger.warning(
                        "Local worker %s did not terminate gracefully, forcing.",
                        process.name,
                    )
                    process.terminate()
    ...
    def __del__(self):
        if self.owns_scheduler_client:
            ...
            logger.warning(
                "Generator was garbage collected without being shut down. Attempting to shut down the local server."
            )
            self.shutdown()
