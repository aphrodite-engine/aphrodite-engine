...
class PipelineStage(ABC):
    ...
    def log_info(self, msg, *args):
        ...
        logger.info("[%s] %s", self.__class__.__name__, msg % args if args else msg)

    def log_warning(self, msg, *args):
        """Logs a warning message with the stage name as a prefix."""
        logger.warning("[%s] %s", self.__class__.__name__, msg % args if args else msg)

    def log_error(self, msg, *args):
        """Logs an error message with the stage name as a prefix."""
        logger.error("[%s] %s", self.__class__.__name__, msg % args if args else msg)

    def log_debug(self, msg, *args):
        """Logs a debug message with the stage name as a prefix."""
        logger.debug("[%s] %s", self.__class__.__name__, msg % args if args else msg)
    ...
    def verify_input(self, batch: Req, server_args: ServerArgs) -> VerificationResult:
        ...
        Example:
            from aphrodite.diffusion.runtime.pipelines.stages.validators import V, VerificationResult
