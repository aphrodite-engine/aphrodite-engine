...
from aphrodite.diffusion.runtime.distributed.communication_op import (
...
from aphrodite.diffusion.runtime.layers.attention.backends.flash_attn import (
    FlashAttentionBackend,
)
from aphrodite.diffusion.runtime.layers.attention.selector import get_attn_backend
from aphrodite.diffusion.runtime.layers.attention.STA_configuration import (
    configure_sta,
    save_mask_search_results,
)
from aphrodite.diffusion.runtime.loader.component_loader import TransformerLoader
...
from aphrodite.diffusion.runtime.pipelines.stages.validators import (
    StageValidators as V,
)
from aphrodite.diffusion.runtime.pipelines.stages.validators import VerificationResult
from aphrodite.diffusion.runtime.platforms.interface import AttentionBackendEnum
from aphrodite.diffusion.runtime.server_args import ServerArgs
from aphrodite.diffusion.utils import dict_to_3d_list, masks_like
from aphrodite.logger import init_logger

try:
    from aphrodite.diffusion.runtime.layers.attention.backends.sliding_tile_attn import (
...
try:
    from aphrodite.diffusion.runtime.layers.attention.backends.vmoba import (
        VMOBAAttentionBackend,
    )
    from aphrodite.diffusion.utils import is_vmoba_available
...
try:
    from aphrodite.diffusion.runtime.layers.attention.backends.video_sparse_attn import (
...
class DenoisingStage(PipelineStage):
    ...
    def __init__(self, transformer, scheduler, pipeline=None, transformer_2=None, vae=None) -> None:
        ...
        if hasattr(self.server_args.pipeline_config, "dit_config") and self.server_args.pipeline_config.dit_config is not None:
            ...
            # UNet models don't use dit_config
            attn_head_size = None

        # torch compile
        if self.server_args.enable_torch_compile:
            full_graph = False
            self.transformer = torch.compile(self.transformer, mode="max-autotune", fullgraph=full_graph)
            self.transformer_2 = (
                torch.compile(self.transformer_2, mode="max-autotune", fullgraph=full_graph)
    ...
    def parallelism_type(self) -> StageParallelismType:
        # return (
        #     StageParallelismType.CFG_PARALLEL
        #     if get_global_server_args().enable_cfg_parallel
        #     else StageParallelismType.REPLICATED
        # )
    ...
    def _prepare_denoising_loop(self, batch: Req, server_args: ServerArgs):
        ...
        if not server_args.model_loaded["transformer"]:
            ...
            self.transformer = loader.load(server_args.model_paths["transformer"], server_args)
            if self.server_args.enable_torch_compile:
        ...
        if server_args.pipeline_config.ti2v_task and batch.pil_image is not None:
            ...
            F = batch.num_frames
            temporal_scale = server_args.pipeline_config.vae_config.arch_config.scale_factor_temporal
    ...
    def _post_denoising_loop(
        ...
        # Gather results if using sequence parallelism
        latents, trajectory_tensor = self._postprocess_sp_latents(batch, latents, trajectory_tensor)
        ...
        batch.latents = self.server_args.pipeline_config.post_denoising_loop(latents, batch)
        ...
        def _shard_tensor(
            ...
            if tensor.dim() == 5:
                ...
                if time_dim > 0 and time_dim % sp_world_size == 0:
                    sharded_tensor = rearrange(tensor, "b c (n t) h w -> b c n t h w", n=sp_world_size).contiguous()
    ...
    def _postprocess_sp_latents(
        ...
        if get_sp_world_size() > 1 and getattr(batch, "did_sp_shard_latents", False):
            ...
            if trajectory_tensor is not None:
                ...
                trajectory_tensor = sequence_model_parallel_all_gather(trajectory_tensor, dim=3)
    ...
    def start_profile(self, batch: Req):
        ...
        prof = torch.profiler.profile(
            ...
            on_trace_ready=lambda _: torch.profiler.tensorboard_trace_handler("./logs"),
            record_shapes=True,
            with_stack=True,
        )
        prof.start()
    ...
    def stop_profile(self, batch: Req):
        try:
            if self.profiler:
                ...
                trace_path = os.path.abspath(os.path.join(log_dir, f"{request_id}.trace.json.gz"))
                logger.info("Saving profiler traces to: %s", trace_path)
    ...
    def _manage_device_placement(
        ...
        if model_to_offload is not None and next(model_to_offload.parameters()).device.type == "cuda":
            model_to_offload.to("cpu")

        # Load the model to use if it's on CPU
        if model_to_use is not None and next(model_to_use.parameters()).device.type == "cpu":
    ...
    def forward(
        ...
        with (
            ...
            for i, t_host in enumerate(timesteps_cpu):
                ...
                if batch.image_latent is not None:
                    assert not server_args.pipeline_config.ti2v_task, (
                ...
                if torch.isnan(latents).any():
                    ...
                    raise ValueError(f"NaN detected in latents after scheduler step {i}")
                if server_args.pipeline_config.ti2v_task and batch.pil_image is not None:
    ...
    def _build_attn_metadata(self, i: int, batch: Req, server_args: ServerArgs) -> Any | None:
        """
        ...
        if (st_attn_available and self.attn_backend == SlidingTileAttentionBackend) or (
            ...
            attn_metadata = self.attn_metadata_builder.build(raw_latent_shape=batch.raw_latent_shape)
        else:
            return None
    ...
    def _predict_noise(
        ...
        **kwargs,
        ...
        # Check if this is a UNet model (SDXL) vs DiT model
        ...
        if 'x' in param_names and 'timesteps' in param_names:
            ...
            output = current_model(
                ...
                t_expand,  # timesteps
                context=prompt_embeds,  # context (text embeddings)
    ...
    def _predict_noise_with_cfg(
        ...
        if not (server_args.enable_cfg_parallel and cfg_rank != 0):
            ...
            with set_forward_context(
                ...
                noise_pred_cond = server_args.pipeline_config.slice_noise_pred(noise_pred_cond, latents)
        ...
        if not server_args.enable_cfg_parallel or cfg_rank != 0:
            ...
            with set_forward_context(
                ...
                noise_pred_uncond = self._predict_noise(
                    ...
                    latent_model_input=latent_model_input,
                    t_expand=t_expand,
                    prompt_embeds=server_args.pipeline_config.get_neg_prompt_embeds(batch),
                    target_dtype=target_dtype,
                    guidance=guidance,
                    **image_kwargs,
                    **neg_cond_kwargs,
                )
                noise_pred_uncond = server_args.pipeline_config.slice_noise_pred(noise_pred_uncond, latents)
        ...
        if server_args.enable_cfg_parallel:
            ...
            if batch.guidance_rescale > 0.0:
                ...
                noise_pred = batch.guidance_rescale * noise_pred_rescaled + (1 - batch.guidance_rescale) * noise_pred
            return noise_pred
        else:
    ...
    def prepare_sta_param(self, batch: Req, server_args: ServerArgs):
        ...
        if STA_mode == STA_Mode.STA_SEARCHING:
            ...
            config_file = envs.APHRODITE_ATTENTION_CONFIG
            if config_file is None:
                raise ValueError("APHRODITE_ATTENTION_CONFIG is not set")
            STA_param = configure_sta(
