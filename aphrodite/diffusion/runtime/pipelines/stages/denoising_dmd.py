...
from aphrodite.diffusion.runtime.models.schedulers.scheduling_flow_match_euler_discrete import (
...
from aphrodite.diffusion.runtime.pipelines.stages import DenoisingStage
from aphrodite.diffusion.runtime.pipelines.stages.denoising import (
    st_attn_available,
    vsa_available,
)
...
class DmdDenoisingStage(DenoisingStage):
    ...
    def forward(
        ...
        if sp_group:
            latents = rearrange(latents, "b (n t) c h w -> b n t c h w", n=sp_world_size).contiguous()
        ...
        with self.progress_bar(total=len(timesteps)) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                guidance_expand = (
                    torch.tensor(
                        [server_args.pipeline_config.embedded_cfg_scale] * latent_model_input.shape[0],
                        dtype=torch.float32,
                        device=get_local_torch_device(),
                    ).to(target_dtype)
                    * 1000.0
