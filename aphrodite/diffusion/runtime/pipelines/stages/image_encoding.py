...
from aphrodite.diffusion.runtime.models.vision_utils import (
    normalize,
    numpy_to_pt,
    pil_to_numpy,
    resize,
...
from aphrodite.diffusion.runtime.server_args import ExecutionMode, ServerArgs
...
class ImageEncodingStage(PipelineStage):
    ...
    def forward(
        ...
        prompt_image = server_args.pipeline_config.preprocess_image(image, self.vae_image_processor)

        if batch.prompt and isinstance(
            server_args.pipeline_config, (QwenImageEditPipelineConfig, QwenImagePipelineConfig)
        ):
        ...
        image_inputs = self.image_processor(images=prompt_image, return_tensors="pt", **image_processor_kwargs).to(
            cuda_device
        )
        if self.image_encoder:
            ...
            batch.prompt_embeds.append(self.encoding_qwen_image_edit(outputs, image_inputs))

            batch.negative_prompt_embeds.append(self.encoding_qwen_image_edit(neg_outputs, neg_image_inputs))
    ...
    def verify_input(self, batch: Req, server_args: ServerArgs) -> VerificationResult:
        ...
        if batch.debug:
            logger.debug("batch.pil_image=%s", batch.pil_image)
            logger.debug("batch.image_embeds=%s", batch.image_embeds)
...
class ImageVAEEncodingStage(PipelineStage):
    ...
    def forward(
        ...
        if server_args.mode == ExecutionMode.PREPROCESS:
            ...
            if isinstance(server_args.pipeline_config, QwenImageEditPipelineConfig):
                ...
                if batch_size > latent_condition.shape[0] and batch_size % latent_condition.shape[0] == 0:
                    # expand init_latents for batch_size
                    additional_image_per_prompt = batch_size // latent_condition.shape[0]
                    image_latents = torch.cat([latent_condition] * additional_image_per_prompt, dim=0)
                elif batch_size > latent_condition.shape[0] and batch_size % latent_condition.shape[0] != 0:
                    raise ValueError(
                        f"Cannot duplicate `image` of batch size {latent_condition.shape[0]} "
                        f"to {batch_size} text prompts."
                    )
                else:
                    image_latents = torch.cat([latent_condition], dim=0)
                image_latent_height, image_latent_width = image_latents.shape[3:]
                num_channels_latents = self.server_args.pipeline_config.dit_config.arch_config.in_channels // 4
    ...
    def preprocess(
        ...
        if isinstance(image, PIL.Image.Image):
            width, height = self.server_args.pipeline_config.vae_config.calculate_dimensions(
