...
from aphrodite.diffusion.runtime.models.utils import pred_noise_to_pred_video
...
class CausalDMDDenoisingStage(DenoisingStage):
    ...
    def forward(
        ...
        # Timesteps for DMD
        timesteps = torch.tensor(server_args.pipeline_config.dmd_denoising_steps, dtype=torch.long).cpu()
        ...
        if self.kv_cache1 is None:
            self._initialize_kv_cache(batch_size=latents.shape[0], dtype=target_dtype, device=latents.device)
            self._initialize_crossattn_cache(
                batch_size=latents.shape[0],
                max_text_len=server_args.pipeline_config.text_encoder_configs[0].arch_config.text_len,
                dtype=target_dtype,
            ...
            for block_index in range(len(self.kv_cache1)):
                self.kv_cache1[block_index]["global_end_index"] = torch.tensor(  # type: ignore
                    [0], dtype=torch.long, device=latents.device
                )
                self.kv_cache1[block_index]["local_end_index"] = torch.tensor(  # type: ignore
                    [0], dtype=torch.long, device=latents.device
                )
        ...
        if getattr(batch, "image_latent", None) is not None:
            ...
            t_zero = torch.zeros([latents.shape[0]], device=latents.device, dtype=torch.long)
            if independent_first_frame and input_frames >= 1:
                # warm-up with the very first frame independently
                image_first_btchw = image_latent[:, :, :1, :, :].to(target_dtype).permute(0, 2, 1, 3, 4)
            ...
            while remaining_frames > 0:
                ...
                ref_btchw = (
                    image_latent[:, :, current_start_frame : current_start_frame + block, :, :]
                    .to(target_dtype)
                    .permute(0, 2, 1, 3, 4)
                )
                with torch.autocast(device_type="cuda", dtype=target_dtype, enabled=autocast_enabled):
    ...
    def _initialize_crossattn_cache(self, batch_size, max_text_len, dtype, device) -> None:
        """
