...
class TextEncodingStage(PipelineStage):
    ...
    def forward(
        ...
        assert len(self.text_encoders) == len(server_args.pipeline_config.text_encoder_configs)
    ...
    def encode_text(
        ...
        for i in indices:
            ...
            text_encoder_extra_arg = (
                text_encoder_extra_args[i] if i < len(text_encoder_extra_args) and text_encoder_extra_args[i] else {}
            )
        ...
        if return_attention_mask:
            ...
            for m in attn_masks_list[1:]:
                if list(m.shape) != base_mask_shape:
                    raise ValueError(
                        "Cannot stack attention masks with differing shapes: %s",
                        [list(m.shape) for m in attn_masks_list],
                    )
    ...
    def verify_output(self, batch: Req, server_args: ServerArgs) -> VerificationResult:
        ...
        if batch.debug:
            logger.debug("batch.prompt_embeds=%s", batch.prompt_embeds)
            logger.debug("batch.negative_prompt_embeds=%s", batch.negative_prompt_embeds)
        return result
