...
from aphrodite.diffusion.runtime.pipelines.composed_pipeline_base import (
    ComposedPipelineBase,
)
from aphrodite.diffusion.runtime.pipelines.lora_pipeline import LoRAPipeline
from aphrodite.diffusion.runtime.server_args import WorkloadType
...
class _PipelineRegistry:
    ...
    pipelines: dict[str, dict[str, type[ComposedPipelineBase] | None]] = dataclasses.field(default_factory=dict)
    ...
    def _try_load_pipeline_cls(
        ...
        try:
            ...
            logger.error(
                "Please check if the ComposedPipeline class has been defined associated with %s.%s",
                pipeline_type.value,
    ...
    def resolve_pipeline_cls(
        ...
        raise ValueError(
            f"Pipeline architecture '{pipeline_name_in_config}' is not supported for "
            f"pipeline type '{pipeline_type.value}' and workload type '{workload_type.value}'. "
...
def import_pipeline_classes(
    ...
    package_name: str = "aphrodite.diffusion.runtime.architectures"
                        ...
                        if not ispkg:
                            ...
                            if hasattr(pipeline_module, "EntryClass"):
                                ...
                                if isinstance(pipeline_module.EntryClass, list):
                                    pipeline_names = [pipeline.__name__ for pipeline in pipeline_module.EntryClass]
                                else:
