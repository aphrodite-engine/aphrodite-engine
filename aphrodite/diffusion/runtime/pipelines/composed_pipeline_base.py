...
class ComposedPipelineBase(ABC):
    ...
    def build_executor(self, server_args: ServerArgs):
        # TODO
        from aphrodite.diffusion.runtime.pipelines.executors.parallel_executor import (
            ParallelExecutor,
        )
    ...
    def load_modules(
        ...
        if "boundary_ratio" in model_index and model_index["boundary_ratio"] is not None:
            ...
            server_args.pipeline_config.dit_config.boundary_ratio = model_index["boundary_ratio"]
        ...
        for module_name in self.required_config_modules:
            if module_name not in model_index and module_name in self._extra_config_module_map:
                ...
                logger.warning(
                    "model_index.json does not contain a %s module, but found "
                    "{%s: %s} in _extra_config_module_map, adding to model_index.",
                    module_name,
                    module_name,
                    extra_module_value,
                )
                if extra_module_value in model_index:
                    ...
                    raise ValueError(
                        ...
                        f"{model_index.get(module_name)} was not found in loaded modules "
                        f"{model_index.keys()}"
                    )
        ...
        for module_name in required_modules:
            if module_name not in components or components[module_name] is None:
                raise ValueError(
                    f"Required module key: {module_name} value: "
                    f"{components.get(module_name)} was not found in loaded modules "
                    f"{components.keys()}"
                )
