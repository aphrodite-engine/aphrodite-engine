...
from aphrodite.diffusion.runtime.utils.performance_logger import PerformanceLogger
...
from aphrodite.diffusion.configs.sample.teacache import (
    TeaCacheParams,
    WanTeaCacheParams,
)
...
class Req:
    ...
    def set_width_and_height(self, server_args: ServerArgs):
        if self.height is None or self.width is None:
            width, height = server_args.pipeline_config.set_width_and_height(self.width, self.height, self.pil_image)
            self.width = width
            self.height = height
        if self.height is None or self.width is None:
            self.width = 1280
