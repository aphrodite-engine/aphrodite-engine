...
from aphrodite.diffusion.runtime.layers.lora.linear import (
    BaseLayerWithLoRA,
    get_lora_layer,
    replace_submodule,
)
from aphrodite.diffusion.runtime.loader.utils import get_param_names_mapping
...
from aphrodite.diffusion.runtime.utils.hf_diffusers_utils import maybe_download_lora
        ...
        if "fake_score_transformer" in self.modules:
            for name, layer in self.modules["fake_score_transformer"].named_modules():
                ...
                if layer is not None:
                    ...
                    replace_submodule(self.modules["fake_score_transformer"], name, layer)
                    converted_count += 1
            logger.info(
    ...
    def set_lora_adapter(self, lora_nickname: str, lora_path: str | None = None):  # type: ignore
        ...
        if lora_path is not None and lora_path != self.cur_adapter_path:
            ...
            for name, weight in lora_state_dict.items():
                ...
                # for (in_dim, r) @ (r, out_dim), we only merge (r, out_dim * n)
                # where n is the number of linear layers to fuse
