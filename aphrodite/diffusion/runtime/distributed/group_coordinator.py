...
from aphrodite.diffusion.runtime.distributed.device_communicators.base_device_communicator import (
    DeviceCommunicatorBase,
)
from aphrodite.diffusion.runtime.distributed.device_communicators.cpu_communicator import (
    CpuCommunicator,
)
...
def get_local_torch_device() -> torch.device:
    ...
    return torch.device(f"cuda:{envs.LOCAL_RANK}") if current_platform.is_cuda_alike() else torch.device("mps")
...
class GroupCoordinator:
    ...
    def __init__(
        ...
        if use_device_communicator and self.world_size > 1:
            ...
            if current_platform.is_cuda_alike():
                from aphrodite.diffusion.runtime.distributed.device_communicators.cuda_communicator import (
                    CudaCommunicator,
                )
    ...
    def all_reduce(
        ...
        if self.world_size == 1:
            ...
            torch.distributed.all_reduce(input_, op=op, group=self.device_group, async_op=async_op)
        return input_
    ...
    def all_gather(
        ...
        if separate_tensors:
            tensor_list = [
                output_tensor.reshape(-1).narrow(0, input_.numel() * i, input_.numel()).view_as(input_)
...
class PipelineGroupCoordinator(GroupCoordinator):
    ...
    def _check_shape_and_buffer(
        ...
        if recv_prev:
            ...
            if shape_list is None or shape_list.get(segment_idx, None) is None:
                recv_flag = True
        ...
        if recv_flag:
            ...
            if self.recv_buffer.get(name, None) is None:
                ...
                if self.recv_buffer[name].get(segment_idx, None) is not None:
                    logger.warning(
                        "Recv buffer [name: %s, segment_idx: %s] already exist. updating...",
                        name,
                        segment_idx,
                    )
...
class SequenceParallelGroupCoordinator(GroupCoordinator):
    def __init__(
        ...
        **kwargs,
        ...
        ulysses_group = kwargs.get("ulysses_group")
        ring_group = kwargs.get("ring_group")
