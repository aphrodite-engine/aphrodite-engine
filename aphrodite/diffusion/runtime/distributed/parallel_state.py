...
#     ), (
#        f"world_size({world_size}) must be greater than or equal to "
...
def maybe_init_distributed_environment_and_model_parallel(
    ...
    if _WORLD is not None and model_parallel_is_initialized():
        ...
        assert get_tp_world_size() == tp_size, (
            f"You are trying to initialize model parallel groups with size {tp_size}, "
            f"but they are already initialized with size {get_tp_world_size()}"
        )
        assert get_sp_world_size() == sp_size, (
            f"You are trying to initialize model parallel groups with size {sp_size}, "
            f"but they are already initialized with size {get_sp_world_size()}"
        )
        return
...
def initialize_tensor_parallel_group(
    ...
    Example usage:
        ...
        tp_group_model1 = initialize_tensor_parallel_group(tensor_model_parallel_size=4, group_name_suffix="model1")
...
def initialize_sequence_parallel_group(
    ...
    Example usage:
        ...
        sp_group_model2 = initialize_sequence_parallel_group(sequence_model_parallel_size=2, group_name_suffix="model2")
