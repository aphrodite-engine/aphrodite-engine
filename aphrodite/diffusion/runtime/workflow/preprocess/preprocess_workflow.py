...
from aphrodite.diffusion.dataset.dataloader.record_schema import (
    basic_t2v_record_creator,
    i2v_record_creator,
)
from aphrodite.diffusion.dataset.dataloader.schema import (
    pyarrow_schema_i2v,
    pyarrow_schema_t2v,
)
...
from aphrodite.diffusion.runtime.distributed.parallel_state import get_world_rank
from aphrodite.diffusion.runtime.pipelines.pipeline_registry import PipelineType
from aphrodite.diffusion.runtime.server_args import ServerArgs, WorkloadType
from aphrodite.diffusion.runtime.workflow.preprocess.components import (
    ParquetDatasetSaver,
...
class PreprocessWorkflow(WorkflowBase):
    def register_pipelines(self) -> None:
        self.add_pipeline_config("preprocess_pipeline", (PipelineType.PREPROCESS, self.server_args))
    ...
    def register_components(self) -> None:
        ...
        video_forward_batch_builder = VideoForwardBatchBuilder(seed=self.server_args.preprocess_config.seed)
    ...
    def get_workflow_cls(cls, server_args: ServerArgs) -> "PreprocessWorkflow":
        if server_args.workload_type == WorkloadType.T2V:
            from aphrodite.diffusion.runtime.workflow.preprocess.preprocess_workflow_t2v import (
                PreprocessWorkflowT2V,
            )

            return cast(PreprocessWorkflow, PreprocessWorkflowT2V)
        elif server_args.workload_type == WorkloadType.I2V:
            from aphrodite.diffusion.runtime.workflow.preprocess.preprocess_workflow_i2v import (
                PreprocessWorkflowI2V,
            )

            return cast(PreprocessWorkflow, PreprocessWorkflowI2V)
        else:
            raise ValueError(f"Workload type: {server_args.workload_type} is not supported in preprocessing workflow.")
