...
from aphrodite.diffusion.runtime.pipelines import ComposedPipelineBase, build_pipeline
...
class WorkflowBase(ABC):
    ...
    def add_pipeline_config(self, pipeline_name: str, pipeline_config: tuple[PipelineType, ServerArgs]) -> None:
        """
    ...
    def get_workflow_cls(cls, server_args: ServerArgs) -> Optional["WorkflowBase"]:
        ...
        if server_args.mode == ExecutionMode.PREPROCESS:
            from aphrodite.diffusion.runtime.workflow.preprocess.preprocess_workflow import (
                PreprocessWorkflow,
            )

            return PreprocessWorkflow.get_workflow_cls(server_args)
        else:
            raise ValueError(f"Execution mode: {server_args.mode} is not supported in workflow.")
