...
class SyncSchedulerClient:
    ...
    def initialize(self, server_args: ServerArgs):
        ...
        self.scheduler_socket.setsockopt(zmq.RCVTIMEO, 6000000)  # 10 minute timeout for generation

        scheduler_endpoint = self.server_args.scheduler_endpoint()
        self.scheduler_socket.connect(scheduler_endpoint)
        logger.debug(
            "SyncSchedulerClient connected to backend scheduler at %s",
            scheduler_endpoint,
        )
    ...
    def forward(self, batch: Req) -> Req:
        ...
        try:
            ...
            raise TimeoutError("Scheduler did not respond in time.") from err
