...
from aphrodite.utils.mem_constants import KiB_bytes, MiB_bytes
...
class HTTPResponseSizeExceededError(APHRODITEValidationError):
...
class MediaDownloadSizeExceededError(ValueError):
    ...
    def __init__(self, max_bytes: int, received_bytes: int | None = None) -> None:
        ...
        message = f"Remote media download exceeds APHRODITE_MAX_MEDIA_DOWNLOAD_SIZE_MB={max_size_mb:g}"
...
def _get_media_download_limit_bytes() -> int | None:
    max_size_mb = envs.APHRODITE_MAX_MEDIA_DOWNLOAD_SIZE_MB
    if max_size_mb <= 0:
        return None
...
def _get_response_size_limit(max_bytes: int | None) -> tuple[int | None, bool]:
    ...
    if media_max_bytes is not None and (max_bytes is None or media_max_bytes <= max_bytes):
        return media_max_bytes, True
    return max_bytes, False


def _raise_size_exceeded(max_bytes: int, received_bytes: int, media_download_limit: bool) -> None:
...
def _read_response_bytes(response: requests.Response, max_bytes: int | None) -> bytes:
    ...
    for chunk in response.iter_content(_RESPONSE_READ_CHUNK_SIZE):
        received_bytes = _append_chunk(chunks, chunk, received_bytes, max_bytes, media_download_limit)
    return b"".join(chunks)


async def _async_read_response_bytes(response: aiohttp.ClientResponse, max_bytes: int | None) -> bytes:
    max_bytes, media_download_limit = _get_response_size_limit(max_bytes)
    if max_bytes is None:
