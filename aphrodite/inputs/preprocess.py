...
from aphrodite.multimodal.inputs import MultiModalDataDict, MultiModalEncDecInputs, MultiModalInputs, MultiModalUUIDDict
from aphrodite.multimodal.processing import BaseMultiModalProcessor
from aphrodite.transformers_utils.tokenizer import AnyTokenizer
from aphrodite.utils.jsontree import json_iter_leaves
...
class InputPreprocessor:
    ...
    def get_decoder_start_token_id(self) -> int | None:
        ...
        if not self.model_config.is_encoder_decoder:
            logger.warning_once("Using None for decoder start token id because this is not an encoder/decoder model.")
            return None

        if self.model_config is None or self.model_config.hf_config is None:
            logger.warning_once("Using None for decoder start token id because model config is not available.")
            return None
    ...
    def _process_multimodal(
        ...
        *,
        ...
        contains_only_strings = all(isinstance(leaf, str) for leaf in json_iter_leaves(mm_hashes))
    ...
    def _process_embeds(
        ...
        return embeds_inputs(prompt_embeds=prompt_embeds, cache_salt=parsed_content.get("cache_salt"))

    def _truncate_inputs(self, inputs: list[int], tokenization_kwargs: dict[str, Any] | None = None) -> list[int]:
        if not tokenization_kwargs or "truncation" not in tokenization_kwargs or self.tokenizer is None:
            return inputs
    ...
    def _prompt_to_llm_inputs(
        ...
        * [`SingletonInputs`][aphrodite.inputs.data.SingletonInputs] instance
        """
    ...
    def _build_enc_dec_llm_inputs(
        ...
        if encoder_inputs["type"] == "embeds" or decoder_inputs and decoder_inputs["type"] == "embeds":
        ...
        if decoder_inputs is None:
            ...
            if "multi_modal_data" in decoder_inputs:
                raise ValueError("Multi-modal decoder inputs of encoder-decoder models are not supported yet")

            dec_token_ids = self._prepare_decoder_input_ids_for_generation(decoder_inputs["prompt_token_ids"])
    ...
    def _split_enc_dec_mm_inputs(
        ...
        """
        if inputs["type"] == "embeds" or decoder_inputs_to_override and decoder_inputs_to_override["type"] == "embeds":
            raise ValueError("Embedding inputs are not supported for encoder-decoder models")
        ...
        if inputs["type"] == "multimodal":  # Multimodal data inputs
            if "encoder_prompt_token_ids" not in inputs:
                raise RuntimeError(
                    "You should register an encoder-decoder multi-modal processor for encoder-decoder models."
                )
    ...
    def _process_encoder_decoder_prompt(
        ...
        *,
        ...
        [`EncoderDecoderInputs`][aphrodite.inputs.data.EncoderDecoderInputs]
        instance.
        ...
        * [`EncoderDecoderInputs`][aphrodite.inputs.data.EncoderDecoderInputs]
        ...
        if is_explicit_encoder_decoder_prompt(prompt):
            ...
            if self.model_config.is_multimodal_model:
                encoder_inputs, decoder_inputs = self._split_enc_dec_mm_inputs(encoder_inputs, decoder_inputs)
        else:
            # `cast` is needed for mypy, but not pyright
    ...
    def _build_decoder_only_llm_inputs(
        ...
        if "prompt_token_ids" in prompt_inputs:
            prompt_inputs = cast(TokenInputs | MultiModalInputs, prompt_inputs)  # Needed for mypy
    ...
    def _process_decoder_only_prompt(
        ...
        *,
        ...
        [`DecoderOnlyInputs`][aphrodite.inputs.data.DecoderOnlyInputs] instance.
