...
- A text prompt ([`str`][] or [`TextPrompt`][aphrodite.inputs.data.TextPrompt])
- A tokenized prompt ([`TokensPrompt`][aphrodite.inputs.data.TokensPrompt])
- An embeddings prompt ([`EmbedsPrompt`][aphrodite.inputs.data.EmbedsPrompt])
...
[`ExplicitEncoderDecoderPrompt`][aphrodite.inputs.data.ExplicitEncoderDecoderPrompt]

A prompt of type [`SingletonPrompt`][aphrodite.inputs.data.SingletonPrompt] may be 
...
class ExplicitEncoderDecoderPrompt(TypedDict, Generic[_T1_co, _T2_co]):
    ...
    [`SingletonPrompt`][aphrodite.inputs.data.SingletonPrompt] schemas,
    ...
    [`SingletonPrompt`][aphrodite.inputs.data.SingletonPrompt] instances.
    """
...
def token_inputs(
    ...
    """Construct [`TokenInputs`][aphrodite.inputs.data.TokenInputs] from optional
    values."""
...
def embeds_inputs(
    ...
    """Construct [`EmbedsInputs`][aphrodite.inputs.data.EmbedsInputs] from optional
    values."""
...
"""
The inputs in [`LLMEngine`][aphrodite.engine.llm_engine.LLMEngine] before they are
...
class EncoderDecoderInputs(TypedDict):
    """
    The inputs in [`LLMEngine`][aphrodite.engine.llm_engine.LLMEngine] before they
...
"""
A processed [`SingletonPrompt`][aphrodite.inputs.data.SingletonPrompt] which can be 
passed to [`aphrodite.common.sequence.Sequence`][].
"""

ProcessorInputs = Union[DecoderOnlyInputs, EncoderDecoderInputs]
"""
The outputs from [`aphrodite.inputs.preprocess.InputPreprocessor`][].
"""
