...
from aphrodite.envs import environment_variables
...
def get_intel_gpu_models():
    if TORCH_AVAILABLE and hasattr(torch, "xpu") and torch.xpu.is_available():
        ...
        return "\n".join("GPU {}: {}".format(i, torch.xpu.get_device_name(i)) for i in range(device_count))
    return None


def get_oneapi_compiler_version(run_lambda):
    """Return Intel oneAPI DPC++/C++ Compiler version via icpx."""
    return run_and_parse_first_match(run_lambda, "icpx --version", r"oneAPI DPC\+\+/C\+\+ Compiler (\S+)")
...
def get_aphrodite_version():
    from aphrodite import __version__, __version_tuple__
...
def pretty_str(envinfo):
    ...
    all_dynamic_xpu_fields_missing = all(mutable_dict[field] is None for field in dynamic_xpu_fields)
    ...
    mutable_dict["intel_gpu_models"] = maybe_start_on_next_line(mutable_dict.get("intel_gpu_models"))
