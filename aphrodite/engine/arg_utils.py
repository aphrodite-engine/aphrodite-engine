...
from aphrodite.config.mamba import MambaBackendEnum, MambaSSUAlgorithm
...
from aphrodite.transformers_utils.config import maybe_override_with_speculators
        ...
        if field.default is not MISSING:
            ...
            if isinstance(default, FieldInfo):
                if default.default_factory is None:
                    ...
                    # AphroditeConfig's Fields have default_factory set to config classes.
...
class EngineArgs:
    ...
    safetensors_load_strategy: SafetensorsLoadStrategy | None = LoadConfig.safetensors_load_strategy
    ...
    cpu_distributed_timeout_seconds: int | None = ParallelConfig.cpu_distributed_timeout_seconds
    ...
    enable_batch_sharded_sampling: bool | None = ParallelConfig.enable_batch_sharded_sampling
    ...
    prefix_cache_retention_interval: int | None = get_field(CacheConfig, "prefix_cache_retention_interval")
    ...
    model_class_overrides: dict[str, str] = get_field(ModelConfig, "model_class_overrides")
    ...
    profiler_config: ProfilerConfig = get_field(AphroditeConfig, "profiler_config")

    kv_transfer_config: KVTransferConfig | None = None
    kv_events_config: KVEventsConfig | None = None

    ec_transfer_config: ECTransferConfig | None = None
    ec_manager_config: EncoderCacheManagerConfig = get_field(AphroditeConfig, "ec_manager_config")
    reasoning_config: ReasoningConfig = get_field(AphroditeConfig, "reasoning_config")
    ...
    def __post_init__(self):
        ...
        self.quantization_config = resolve_quantization_config(self.quantization, self.quantization_config)
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        model_group.add_argument("--quantization-config", **model_kwargs["quantization_config"])
        ...
        model_group.add_argument("--enable-trace-replay", **model_kwargs["enable_trace_replay"])
        ...
        model_group.add_argument("--model-class-overrides", **model_kwargs["model_class_overrides"])
        ...
        model_group.add_argument("--enable-cumem-allocator", **model_kwargs["enable_cumem_allocator"])
        model_group.add_argument(
        ...
        mamba_group.add_argument("--mamba-ssu-algorithm", **mamba_kwargs["ssu_algorithm"])
        ...
        cache_group.add_argument("--prefix-match-unit", **cache_kwargs["prefix_match_unit"])
        cache_group.add_argument("--mamba-cache-mode", **cache_kwargs["mamba_cache_mode"])
        cache_group.add_argument("--replayssm-buffer-len", **cache_kwargs["replayssm_buffer_len"])
        ...
        multimodal_group.add_argument("--video-pruning-method", **multimodal_kwargs["video_pruning_method"])
        multimodal_group.add_argument("--mm-tensor-ipc", **multimodal_kwargs["mm_tensor_ipc"])
        multimodal_group.add_argument(
            "--mm-processor-device",
            choices=["auto", "cpu"]
            + ([current_platform.device_type] if current_platform.device_type not in ("", "cpu") else []),
            default="auto",
        ...
        scheduler_group.add_argument("--max-num-queued-reqs", **scheduler_kwargs["max_num_queued_reqs"])
        scheduler_group.add_argument(
        ...
        aphrodite_group.add_argument("--spec-tokens", **speculative_kwargs["num_speculative_tokens"])
        aphrodite_kwargs["diffusion_config"]["type"] = optional_type(json.loads)
        aphrodite_group.add_argument("--diffusion-config", "-dc", **aphrodite_kwargs["diffusion_config"])
        ...
        aphrodite_group.add_argument("--ec-manager-config", **aphrodite_kwargs["ec_manager_config"])
    ...
    def create_speculative_config(
        ...
        for flag, key, value in (
            ...
            if key in self.speculative_config:
                raise ValueError(f"{flag} and --speculative-config['{key}'] are mutually exclusive")
            self.speculative_config[key] = value

        if self.speculative_config is None:
            return None

        self.speculative_config = {k.replace("-", "_"): v for k, v in self.speculative_config.items()}
    ...
    def _resolve_device_ids(self) -> list[int] | None:
        ...
        if all(isinstance(i, str) for i in ids):
            return [current_platform.device_control_id_to_physical_device_id(i) for i in cast(list[str], ids)]
        ...
        if cvd:
            cvd_ids = [current_platform.device_control_id_to_physical_device_id(x) for x in cvd.split(",")]
            for i in int_ids:
                if i >= len(cvd_ids):
                    raise ValueError(
    ...
    def create_engine_config(
        ...
        NOTE: If AphroditeConfig is incompatible, we raise an error.
        """
        ...
        if resolved_cache_dtype.startswith("turboquant_"):
            ...
            cache_config.kv_cache_dtype_skip_layers = sorted(existing | set(boundary), key=int)
        ...
        # Pre-quantized BitsAndBytes checkpoints require their dedicated loader.
        ...
        if envs.APHRODITE_BATCH_INVARIANT and attention_config.backend is None and current_platform.is_xpu():
            attention_config.backend = AttentionBackendEnum.TRITON_ATTN
            logger.info(
                "APHRODITE_BATCH_INVARIANT is enabled and no attention backend was "
