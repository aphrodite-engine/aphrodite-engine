...
class AphroditeEngine:
    ...
    The :class:`~aphrodite.LLM` class wraps this class for offline batched inference
    and the :class:`AsyncAphrodite` class wraps this class for online serving.

    The config arguments are derived from :class:`~aphrodite.EngineArgs`. (See
    :ref:`engine-args`)
    ...
    def __init__(
        ...
        if envs.APHRODITE_USE_V1:
            raise ValueError(
                "Using V0 AphroditeEngine, but envs.APHRODITE_USE_V1=True. "
                "This should not happen. As a workaround, try using "
                "AphroditeEngine.from_aphrodite_config(...) or explicitly set "
        ...
        self.speculative_config = aphrodite_config.speculative_config  # noqa
        ...
        # Currently used by AsyncAphrodite to ensure quick append
        ...
        if isinstance(self.aphrodite_config.scheduler_config.scheduler_cls, str):
            Scheduler = resolve_obj_by_qualname(
                self.aphrodite_config.scheduler_config.scheduler_cls)
        else:
    ...
    def _initialize_kv_caches(self) -> None:
        ...
        num_gpu_blocks, num_cpu_blocks = (
            self.modeling.determine_num_available_blocks())
    ...
    def _get_executor_cls(cls,
                          engine_config: AphroditeConfig) -> Type[ExecutorBase]:
        ...
        if isinstance(distributed_executor_backend, type):
            ...
            from aphrodite.executor.ray_distributed_executor import (
                RayDistributedExecutor)
            executor_class = RayDistributedExecutor
        elif distributed_executor_backend == "mp":
            from aphrodite.executor.mp_distributed_executor import (
    ...
    def add_request(
        ...
        Details:
            ...
            - Create `n` number of :class:`~aphrodite.common.sequence` objects.
            - Create a :class:`~aphrodite.common.sequenceGroup` object
              from the list of :class:`~aphrodite.common.sequence`.
            - Add the :class:`~aphrodite.common.sequenceGroup` object to the scheduler.
    ...
    def _create_sequence_group_with_sampling(
        ...
        if self.aphrodite_config.speculative_config is not None:
            draft_size = \
                self.aphrodite_config.speculative_config.num_speculative_tokens + 1
    ...
    def get_aphrodite_config(self) -> AphroditeConfig:
        """Gets the aphrodite configuration."""
    ...
    def _process_model_outputs(self,
        ...
        # AphroditeEngine/AsyncAphrodite directly
        if is_async:
            # Log stats.
    ...
    def step(self) -> List[Union[RequestOutput, PoolingRequestOutput]]:
        ...
        if not scheduler_outputs.is_empty():
            ...
            try:
                outputs = self.modeling.execute_model(
    ...
    def sleep(self, level: int = 1) -> None:
        assert self.aphrodite_config.model_config.enable_sleep_mode, (
