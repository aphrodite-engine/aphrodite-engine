...
from aphrodite.config import (AphroditeConfig, BlockSize, CacheConfig,
                              ...
                              LogprobsMode, LoRAConfig, MambaDType,
...
from aphrodite.transformers_utils.config import get_model_path, is_interleaved
...
NEEDS_HELP = (
    "--help" in (argv := sys.argv)  # aphrodite SUBCOMMAND --help
...
class EngineArgs:
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        parallel_group.add_argument(
            "--decode-context-parallel-size", "-dcp", "-cp",
        ...
        parallel_group.add_argument(
            ...
            type=int,
            help=
            "[DEPRECATED] --num-redundant-experts will be removed in a future "
            "release.",
            deprecated=True)
        parallel_group.add_argument(
            "--eplb-window-size",
            type=int,
            help="[DEPRECATED] --eplb-window-size will be removed in a future "
            "release.",
            deprecated=True)
        parallel_group.add_argument(
            "--eplb-step-interval",
            type=int,
            help=
            "[DEPRECATED] --eplb-step-interval will be removed in a future "
            "release.",
            deprecated=True)
        parallel_group.add_argument(
            ...
            help=
            "[DEPRECATED] --eplb-log-balancedness will be removed in a future "
            "release.",
            deprecated=True)
        parallel_group.add_argument(
        ...
        aphrodite_group.add_argument("--speculative-config",
                                     **aphrodite_kwargs["speculative_config"])
        ...
        parser.add_argument('--single-user-mode',
                            action='store_true',
                            help='Enable single user mode. When enabled, '
                            'the engine only allocates enough memory for a '
                            'single sequence, and disables batching.')
    ...
    def create_model_config(self) -> ModelConfig:
        ...
        if self.disable_mm_preprocessor_cache:
            ...
            logger.warning(
                ...
                "and will be removed in a future release. "
                "Please use `--mm-processor-cache-gb {}` instead.",
    ...
    def create_engine_config(
        ...
        scheduler_config = SchedulerConfig(
            ...
            send_delta_data=(envs.APHRODITE_USE_RAY_SPMD_WORKER
    ...
    def _set_default_args_v0(self, model_config: ModelConfig) -> None:
        ...
        if self.enable_prefix_caching:
            # Disable prefix caching for multimodal models for APHRODITE_V0.
            ...
            # APHRODITE_V0 only supports builtin hash algo for prefix caching.
            if self.prefix_caching_hash_algo == "sha256":
                raise ValueError(
                    "sha256 is not supported for prefix caching in V0 engine. "
                    "Please use 'builtin'.")

        # Set max_num_seqs to 256 for APHRODITE_V0.
    ...
    def _set_default_args_v1(self, usage_context: UsageContext,
        ...
        if self.scheduler_cls == EngineArgs.scheduler_cls:
            self.scheduler_cls = "aphrodite.v1.core.sched.scheduler.Scheduler"
...
def _raise_or_fallback(feature_name: str, recommend_to_remove: bool):
    if envs.is_set("APHRODITE_USE_V1") and envs.APHRODITE_USE_V1:
        raise NotImplementedError(
            f"APHRODITE_USE_V1=1 is not supported with {feature_name}.")
