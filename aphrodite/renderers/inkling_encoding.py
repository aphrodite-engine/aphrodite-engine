...
def render_inkling_messages(
    ...
    *,
    ...
    for message in messages:
        ...
        if role == "tool":
            tool_name = message.get("name") or tool_call_id_to_name.get(str(message.get("tool_call_id") or ""), "")
            _append_message(
                input_ids,
                tokenizer,
                "tool",
...
def _thinking_effort_text(reasoning_effort: Any) -> str:
    if isinstance(reasoning_effort, bool) or not isinstance(reasoning_effort, (int, float)):
        raise TypeError(
            f"Inkling reasoning_effort must be a number in [0.0, 0.99], got {type(reasoning_effort).__name__}"
        )
    value = float(reasoning_effort)
    if not 0.0 <= value <= _MAX_REASONING_EFFORT:
        raise ValueError(f"Inkling reasoning_effort must be in [0.0, 0.99], got {value}")
...
def _flatten_text_content(content: Any) -> str:
    ...
    for kind, text in _iter_render_parts(content):
        if kind != "text":
            raise ValueError(f"Inkling tool response content must be text, got a part of kind {kind!r}")
        parts.append(text)
    return "".join(parts)
...
def _expect_role(message: Mapping[str, Any]) -> str:
    ...
    if role not in ROLE_MESSAGE_TOKENS:
        raise ValueError(f"unsupported Inkling message role {role!r}; expected one of {sorted(ROLE_MESSAGE_TOKENS)}")
    return str(role)
