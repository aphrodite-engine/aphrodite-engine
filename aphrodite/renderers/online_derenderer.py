...
class OnlineDerenderer:
    ...
    def _derender_chat(
        ...
        for choice in generate_response.choices:
            ...
            if self.parser is not None and chat_request is not None:
                ...
                is_named_tool_choice = type(chat_request.tool_choice) is ChatCompletionNamedToolChoiceParam
                ...
                skip_special = chat_request.skip_special_tokens if chat_request is not None else True
                decoded_text = tokenizer.decode(choice.token_ids, skip_special_tokens=skip_special)
    ...
    async def derender_chat_stream(
        ...
        if len(generate_chunk.choices) > 1:
            raise ValueError("derender_chat_stream expects at most one choice per chunk")
    ...
    async def derender_completion(
        ...
        return await self._derender_completion_async(generate_responses, prompt_tokens, completion_request)
    ...
    def _derender_completion(
        ...
        skip_special = completion_request.skip_special_tokens if completion_request is not None else True
        tokenizer = self.renderer.get_tokenizer()
    ...
    async def derender_completion_stream(
        ...
        if len(generate_chunk.choices) > 1:
            raise ValueError("derender_completion_stream expects at most one choice per chunk")
