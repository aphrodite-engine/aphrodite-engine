...
from aphrodite.renderers import BaseRenderer, ChatParams, TokenizeParams, merge_kwargs
...
ResponsesPreviousMessages: TypeAlias = list[ChatCompletionMessageParam] | list[OpenAIMessage]
...
class OnlineRenderer:
    ...
    async def render_responses(
        ...
        *,
        ...
        if previous_messages is not None and any(isinstance(message, OpenAIMessage) for message in previous_messages):
            return self.create_error_response(
        ...
        messages = construct_input_messages(
            ...
            prev_response_output=(list(previous_response_outputs) if previous_response_outputs is not None else None),
        )
    ...
    def _get_harmony_builtin_tool_descriptions(
        ...
        for server_name, request_name in BUILTIN_TOOL_TO_MCP_SERVER_LABEL.items():
            description = (
                ...
                if request_name in tool_types and tool_server is not None and tool_server.has_tool(server_name)
                else None
            )
    ...
    async def preprocess_chat(
        ...
        *,
        ...
        if reuse_ids:
            ...
            engine_input = tokens_input(reuse_ids, cache_salt=getattr(request, "cache_salt", None))
        else:
