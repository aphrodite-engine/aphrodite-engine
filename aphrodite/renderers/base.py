...
from aphrodite.multimodal import MULTIMODAL_REGISTRY as mm_registry
from aphrodite.multimodal.cache import BaseMultiModalProcessorCache
from aphrodite.multimodal.gpu_ipc_memory import maybe_init_mm_gpu_ipc_pool
...
from aphrodite.multimodal.processing import ProcessorInputs as MMProcessorInputs
from aphrodite.multimodal.registry import MultiModalTimingRegistry
...
class BaseRenderer(ABC, Generic[_T]):
    def __init__(self, config: "AphroditeConfig", tokenizer: _T | None) -> None:
        super().__init__()
        ...
        # aphrodite/utils/jit_monitor.py, which forces the workqueue layer).
        ...
        self._tokenize_prompt_async = make_async(self._tokenize_prompt, executor=self._executor)
        ...
        self._clear_mm_cache_async = make_async(self.clear_mm_cache, executor=self._mm_executor)
        self._process_multimodal_async = make_async(self._process_multimodal, executor=self._mm_executor)
        self._safe_load_prompt_embeds_async = make_async(safe_load_prompt_embeds, executor=self._executor)
    ...
    def _warmup_mm_processor(
        ...
        *,
        ...
        from aphrodite.multimodal.processing import TimingContext
    ...
    def start_mm_warmup_in_background(self) -> None:
        ...
        # "Fatal Python error: Aborted" (see aphrodite/utils/jit_monitor.py, which
    ...
    def warmup(self, chat_params: ChatParams) -> None:
        ...
        it — already ran it.
        """
        from aphrodite.entrypoints.chat_utils import ChatTemplateResolutionError
    ...
    async def _render_prompt_async(
        ...
        if isinstance(prompt, bytes):
            embeds = await self._safe_load_prompt_embeds_async(self.model_config, prompt)
    ...
    async def render_prompts_async(
        ...
        return await asyncio.gather(*(self._render_prompt_async(prompt) for prompt in prompts))
    ...
    @staticmethod
    def _apply_prompt_char_offset(prompt: TokensPrompt, char_offset: int) -> TokensPrompt:
        ...
        if offsets is not None:
            prompt["prompt_token_offsets"] = [
                ...
                (start, end) if (start, end) == (0, 0) else (start + char_offset, end + char_offset)
    ...
    async def _detokenize_prompt_async(self, prompt: TokensPrompt) -> TokensPrompt:
        prompt["prompt"] = await self._async_tokenizer_decode(prompt["prompt_token_ids"])
    ...
    def _process_tokens(
        ...
        *,
        ...
        if engine_input["type"] == "token" and ((offsets := prompt.get("prompt_token_offsets")) is not None):
