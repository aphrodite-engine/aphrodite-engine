...
from aphrodite.tokenizers.hf import HfTokenizer, maybe_make_thread_pool
    ...
    **kwargs,
    ...
    if return_assistant_tokens_mask and "{% generation %}" in chat_template:
        ...
        try:
            ...
            logger.warning("apply_chat_template failed for assistant_tokens_mask: %s", exc)
        else:
    ...
    try:
        ...
        raise AphroditeValidationError(_template_error_reason(e)) from e
...
class HfRenderer(BaseRenderer[HfTokenizer]):
    def __init__(
        ...
        if (
            ...
            getattr(config.model_config, "enable_prompt_embeds", False) and isinstance(tokenizer, HfTokenizer)
        ):
    ...
    def _apply_prompt_embeds_to_engine_input(
        ...
        for tensor, (start, length) in zip(prompt_embeds_tensors, positions, strict=True):
            ...
            pe_hashes.append(MultiModalHasher.hash_kwargs(mm_config.mm_hasher_algorithm, prompt_embeds=tensor))
