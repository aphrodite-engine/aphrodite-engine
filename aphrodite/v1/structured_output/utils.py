...
class OutlinesDiskCache:
    ...
    def __init__(self, path: str) -> None:
        ...
        self._db.execute(
            "CREATE TABLE IF NOT EXISTS cache (key TEXT PRIMARY KEY, type_tag TEXT NOT NULL, value BLOB NOT NULL)"
        )
        self._db.commit()
    ...
    def __getitem__(self, key: str) -> Any:
        row = self._db.execute("SELECT type_tag, value FROM cache WHERE key=?", (key,)).fetchone()
        if row is None:
            raise KeyError(key)
        type_tag, data = row
