...
from aphrodite.v1.structured_output.utils import maybe_wrap_mistral_common_tokenizer
...
class StructuredOutputManager:
    ...
    def __init__(self, aphrodite_config: AphroditeConfig):
        ...
        if not self.aphrodite_config.model_config.skip_tokenizer_init:
            ...
            self.tokenizer = maybe_wrap_mistral_common_tokenizer(
                cached_tokenizer_from_config(model_config=self.aphrodite_config.model_config)
            )
    ...
    def _create_grammar(self, request: "Request") -> StructuredOutputGrammar:
        ...
        try:
            ...
            stop_token_ids = request.sampling_params.all_stop_token_ids if request.sampling_params is not None else None
            return self.backend.compile_grammar(request_type, grammar_spec, stop_token_ids=stop_token_ids)
        except Exception:
            logger.exception("Failed to compile grammar for request %s", request.request_id)
            raise
    ...
    def grammar_bitmask(
        ...
        if len(structured_output_request_ids) > self.fill_bitmask_parallel_threshold and max_num_spec_tokens == 0:
            ...
            for req_id in structured_output_request_ids:
                ...
                if not (self.aphrodite_config.model_config.is_diffusion and req_tokens):
    ...
    def should_fill_bitmask(self, request: "Request") -> bool:
        ...
        structured_req = request.structured_output_request
        if self.enable_in_reasoning or (structured_req is not None and structured_req.reasoning_ended is True):
            return True
        ...
        if reasoner is not None:
            ...
            if structured_req.reasoning_ended is None:
                ...
                # it can be removed.
                structured_req.reasoning_ended = reasoner.is_reasoning_end(request.prompt_token_ids or [])
    ...
    def trim_reasoning_for_advance(self, request: "Request", new_token_ids: list[int]) -> list[int]:
