...
class ThinkingBudgetStateHolder:
    ...
    def _update_think_state(self, state: dict[str, Any]) -> None:
        ...
        if state["start_thinking"] == -1:
            ...
            start_thinking = self._find_last_sequence_index(output_slice, self.think_start_token_ids)
            if start_thinking >= 0:
        ...
        if state["end_thinking"] == -1:
            scan_offset = state.get("scan_offset", 0)
            output_slice = state.get("output_tok_ids", [])[scan_offset:]
            end_thinking = self._find_last_sequence_index(output_slice, self.think_end_token_ids)
            if end_thinking >= 0:
        ...
        natural_end_with_continue = state.get("continue_thinking", False) and state["end_thinking"] != -1
        if (
            not state.get("in_end", False)
