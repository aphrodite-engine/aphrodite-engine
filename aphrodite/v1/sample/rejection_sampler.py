...
class RejectionSampler(nn.Module):
    ...
    def forward(
        ...
        # [num_tokens, vocab_size]
        # NOTE: `target_logits` can be updated in place inside the
...
def compute_probs(
    ...
    # NOTE: Update `logits` in place to avoid allocating a new tensor.
    ...
    # NOTE: `apply_top_k_top_p` uses sorting to calculate the mask,
...
def sample_recovered_tokens(
    ...
    # NOTE: Create only one distribution for each request.
    batch_size = len(num_draft_tokens)
    vocab_size = target_probs.shape[-1]
    q = torch.empty(
...
# NOTE: Avoid specialization to prevent unnecessary recompilation.
...
def rejection_greedy_sample_kernel(
    ...
    # FIXME: Because is_greedy_ptr is not None at profiling run,
...
def rejection_random_sample_kernel(
    ...
    for pos in range(num_draft_tokens):
        if not rejected:
            ...
            # NOTE: While the draft probability should never be 0,
...
def sample_recovered_tokens_kernel(
    ...
    if NO_DRAFT_PROBS:
        ...
        # NOTE: We don't need `prob = prob / tl.sum(prob)` here because
