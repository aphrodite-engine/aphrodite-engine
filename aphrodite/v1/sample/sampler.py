...
class Sampler(nn.Module):
    ...
    def _apply_sampler_order(
        ...
        for sampler_id in sampler_order:
            if sampler_id == SamplerID.DRY and sampling_metadata.dry_multiplier is not None:
                logger.debug(
                    "Applying DRY with dry_multiplier: %s",
                ...
                logger.debug(
                    "Applying no_repeat_ngram with size: %s",
                    sampling_metadata.no_repeat_ngram_size,
                )
                ...
                logger.debug(
                    "Applying Top-Nsigma with nsigma: %s",
                    sampling_metadata.top_nsigma,
                )
                ...
                logger.debug(
                    "Applying ETA Cutoff with eta_cutoff: %s",
                ...
                logger.debug(
                    "Applying Epsilon Cutoff with epsilon_cutoff: %s",
                ...
                logger.debug(
                    "Applying Typical P with typical_p: %s",
                    sampling_metadata.typical_p,
                )
    ...
    def _subset_sampling_metadata(
        ...
        index_tensor = torch.tensor(
            indices,
            device=sampling_metadata.temperature.device if sampling_metadata.temperature is not None else None,
        )
        ...
        return replace(
            ...
            output_token_ids=(
                [sampling_metadata.output_token_ids[i] for i in indices] if sampling_metadata.output_token_ids else []
            ),
