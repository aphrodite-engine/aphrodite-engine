...
def flashinfer_sampler_supported() -> bool:
    ...
    Returns False (with appropriate logging) when ``APHRODITE_USE_FLASHINFER_SAMPLER``
    ...
    if not current_platform.is_cuda():
        return False
    if not envs.APHRODITE_USE_FLASHINFER_SAMPLER:
        logger.info_once("FlashInfer top-p/top-k sampling disabled via APHRODITE_USE_FLASHINFER_SAMPLER=0.")
        return False
    ...
    if not FlashInferBackend.supports_compute_capability(capability):
        unsupported_reason = f"unsupported compute capability {capability.as_version_str()}"

    if unsupported_reason is None:
        logger.info_once("Using FlashInfer for top-p & top-k sampling.", scope="global")
        return True
    if envs.is_set("APHRODITE_USE_FLASHINFER_SAMPLER"):
        raise RuntimeError(
            f"FlashInfer top-p/top-k sampling unavailable: {unsupported_reason}. "
            "Unset APHRODITE_USE_FLASHINFER_SAMPLER=1."
        )
    logger.warning_once(
        "FlashInfer top-p/top-k sampling unavailable: %s; falling back. "
        "Set APHRODITE_USE_FLASHINFER_SAMPLER=0 to silence.",
        unsupported_reason,
    )
    return False
...
class TopKTopPSampler(nn.Module):
    ...
    def __init__(
        ...
        if current_platform.is_cuda():
            ...
            can_use_flashinfer = (
                logprobs_mode not in ("processed_logits", "processed_logprobs") and flashinfer_sampler_supported()
            )
            ...
            if envs.APHRODITE_XPU_USE_SAMPLER_KERNEL:
    ...
    def forward_native(
        ...
        """
        if k is not None and self.logprobs_mode not in (
            "processed_logits",
            "processed_logprobs",
        ):
    ...
    def forward_top_k_first(
        ...
        probs = topk_logits.softmax(dim=-1, dtype=torch.float32)
        sampled_in_topk = random_sample(probs, generators, self.use_fp64_gumbel)
    ...
    def _init_aiter_ops(self) -> bool:
        ...
        try:
            ...
            self.forward = self.forward_native
            logger.warning_once(
                "aiter.ops.sampling is not available on ROCm. Falling back to PyTorch-native implementation."
            )
            return False
    ...
    def aiter_sample(
        ...
        # Top-k only path
            ...
            probs = logits.softmax(dim=-1, dtype=torch.float32).contiguous()
            renorm_probs = self.aiter_ops.top_k_renorm_probs(probs, *_to_tensor_scalar_tuple(k))
    ...
    def forward_xpu(
        ...
        if generators:
            ...
            return self.forward_native(logits, generators, k, p)
        random_sampled = torch.empty(logits.shape[0], dtype=torch.int64, device=logits.device)
        logits_to_return = None
        ...
        if k is not None:
            k = k.to(torch.int64)
        torch.ops.aphrodite.xpu_topk_topp_sampler(
...
def empty_exponential_noise_like(probs: torch.Tensor, use_fp64_gumbel: bool) -> torch.Tensor:
