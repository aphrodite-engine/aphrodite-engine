...
def _topp_sb_combine(
    ...
    if (done != 0.0) and (nomask == 0.0):
        ...
        numkeep = numdup - tl.cast(tl.maximum(sel_mass - p, 0.0) / dup, tl.uint32).to(tl.float32)
...
def _apply_topp_split(
    ...
    if ws is None:
        ws = {
            ...
            "parts": logits.new_empty((_SPLIT_MAX_BATCH, _SPLIT_ROUNDS, _SPLIT_MAX_SPLITS, _SPLIT_FANOUT, 3)),
        }
...
def apply_top_k_top_p_triton(
    ...
    use_split = topp_enabled and batch_size <= _SPLIT_MAX_BATCH and logits.device.type == "cuda"
