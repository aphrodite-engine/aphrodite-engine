...
class MinTokensLogitsProcessor(LogitsProcessor):
    ...
    def update_state(self, batch_update: BatchUpdate | None):
        ...
        if self.min_toks:
            ...
            to_remove = tuple(
                index for index, (min_toks, out_tok_ids, _, _) in self.min_toks.items() if len(out_tok_ids) >= min_toks
            )
            if to_remove:
                needs_update = True
    ...
    def _mask_stop_token_logits(
        ...
        unique_rows, inverse_indices = torch.unique_consecutive(restore_rows, return_inverse=True)
    ...
    def apply(self, logits: torch.Tensor) -> torch.Tensor:
        if self.min_toks:
            ...
            if self.restore_logits_slice[0].numel() == 0:
                ...
                self._mask_stop_token_logits(logits, self.logits_slice, self.restore_logits_slice)
        return logits
