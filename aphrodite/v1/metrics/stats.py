...
if TYPE_CHECKING:
    from aphrodite.v1.engine import EngineCoreEvent, EngineCoreOutput, FinishReason
...
class PrefillStats:
    ...
    def finalize(self, num_cached_tokens: int) -> None:
        ...
        self.num_cache_creation_tokens = max(
            0,
            min(num_cached_tokens, self.num_prompt_tokens) - self.num_cached_tokens,
        )
...
class RequestSpecDecodeMetrics:
    ...
    def observe(self, num_draft_tokens: int, num_accepted: int, detailed: bool = False) -> None:
...
class IterationStats:
    ...
    def update_from_output(
        ...
        if envs.APHRODITE_COMPUTE_NANS_IN_LOGITS and not req_stats.is_corrupted and output.num_nans_in_logits > 0:
    ...
    def update_from_events(
        ...
        from aphrodite.v1.engine import EngineCoreEventType
    ...
    def update_from_finished_request(
        ...
        mean_time_per_output_token = (
            decode_time / (req_stats.num_generation_tokens - 1) if req_stats.num_generation_tokens - 1 > 0 else 0
        )
