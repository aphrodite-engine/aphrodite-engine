...
from aphrodite.v1.metrics.stats import PerfStats
...
class Breakdown(BaseModel):
    """
    Syntactic sugar so that we can use dot notations
    to construct metric breakdowns.

    e.g.)
        breakdown = Breakdown()
        breakdown.qkv = 1
        breakdown.out = 2
        breakdown.model_dump()  # {"qkv": 1, "out": 2}
    """
...
class Parser(Protocol):
    def parse(self, args: ParsedArgs, aphrodite_config: AphroditeConfig) -> ParsedArgs:
        """
        Parse the aphrodite config and update the current ParsedArgs and pass it on.
        If the parser isn't applicable to the aphrodite_config, it will do nothing.
        """
        ...
...
class ParserChain:
    ...
    def parse(self, aphrodite_config: AphroditeConfig) -> ParsedArgs:
        args = ParsedArgs()
        for parser in self.parsers:
            args = parser.parse(args, aphrodite_config)
        return args
...
class ComponentMetrics(BaseModel, ABC):
    ...
    - parser to parse AphroditeConfig into fields
    ...
    @classmethod
    def from_aphrodite_config(cls, aphrodite_config: AphroditeConfig) -> Self:
        """
        Instantiate this class from AphroditeConfig.
        Raises ValueError if parsing fails.
        """

        parser = cls.get_parser()
        parsed_args = parser.parse(aphrodite_config)
    ...
    def get_num_flops(self, ctx: ExecutionContext, per_gpu: bool = True) -> int:
        return sum(self.get_num_flops_breakdown(ctx, per_gpu).model_dump().values())

    def get_read_bytes(self, ctx: ExecutionContext, per_gpu: bool = True) -> int:
        return sum(self.get_read_bytes_breakdown(ctx, per_gpu).model_dump().values())

    def get_write_bytes(self, ctx: ExecutionContext, per_gpu: bool = True) -> int:
        return sum(self.get_write_bytes_breakdown(ctx, per_gpu).model_dump().values())
...
class BaseConfigParser(Parser):
    def parse(self, args: ParsedArgs, aphrodite_config: AphroditeConfig) -> ParsedArgs:
        ...
        args.dp_size = aphrodite_config.parallel_config.data_parallel_size
        args.tp_size = aphrodite_config.parallel_config.tensor_parallel_size
        args.pp_size = aphrodite_config.parallel_config.pipeline_parallel_size
        args.enable_ep = aphrodite_config.parallel_config.enable_expert_parallel
...
class AttentionQuantizationConfigParser(Parser):
    def parse(self, args: ParsedArgs, aphrodite_config: AphroditeConfig) -> ParsedArgs:
        cfg = aphrodite_config.quant_config
...
class AttentionMetrics(ComponentMetrics):
    ...
    def get_num_flops_breakdown(
        ...
        num_flops.qkv_proj = 2 * T * D * (q + 2 * kv) * d * L
        num_flops.attn_qk = 2 * q * T * C * d * L
        num_flops.attn_av = 2 * q * T * C * d * L
        num_flops.out_proj = 2 * T * D * q * d * L
    ...
    def get_write_bytes_breakdown(
        ...
        write_bytes = Breakdown()

        write_bytes.qkv_output = T * (q + 2 * kv) * d * self.activation_byte_size * L
        write_bytes.kv_cache = 2 * T * kv * d * self.cache_byte_size * L
...
class BaseFfnConfigParser(Parser):
    def parse(self, args: ParsedArgs, aphrodite_config: AphroditeConfig) -> ParsedArgs:
        ...
        args.num_experts_per_tok = getattr_from_list(
            cfg, ["num_experts_per_tok", "moe_topk"], 1
        )
        ...
        is_moe = args.num_experts is not None
...
class UnembedMetrics(ComponentMetrics):
    ...
    def get_read_bytes_breakdown(
        ...
        read_bytes.input = T * D * self.activation_byte_size
        read_bytes.weight = D * V * self.weight_byte_size
    ...
    def get_write_bytes_breakdown(
        ...
        write_bytes.output = T * V * self.activation_byte_size
...
class ModelMetrics:
    def __init__(self, aphrodite_config: AphroditeConfig | None = None) -> None:
        """
        Parse aphrodite_config to instantiate metrics for each component.
        Will raise ValueError if we cannot instantiate any components.
        """
        ...
        for metric_cls in ComponentMetrics.registered_metrics():
            try:
                metric = metric_cls.from_aphrodite_config(aphrodite_config)
                self.metrics.append(metric)
            except ValueError:
                pass

        if not self.metrics:
            raise ValueError("No metrics could be instantiated from aphrodite_config")
    ...
    def get_num_flops_breakdown(
        ...
        for metric in self.metrics:
            ...
            prefixed = {
                f"{component}.{key}": val for key, val in breakdown.model_dump().items()
            }
...
def get_required(obj: object, attr: str):
    """Get an attr from an object, or throw a ValueError if it's not set."""
    if not hasattr(obj, attr):
        raise ValueError(f"Missing required attr {attr} in config")
