...
from aphrodite.v1.engine.utils import EngineHandshakeMetadata, EngineZmqAddresses, get_device_indices
...
class EngineCore:
    ...
    def __init__(
        ...
        if executor_fail_callback is not None:
            self.modeling.register_failure_callback(executor_fail_callback)
        ...
        scheduler_block_size = (
            aphrodite_config.cache_config.block_size * aphrodite_config.parallel_config.decode_context_parallel_size
        )
        ...
        self.scheduler: SchedulerInterface = Scheduler(
            ...
            include_finished_set=aphrodite_config.parallel_config.data_parallel_size > 1,
            log_stats=self.log_stats,
            block_size=scheduler_block_size,
        )
        self.use_spec_decode = aphrodite_config.speculative_config is not None
        if self.scheduler.connector is not None:  # type: ignore
            self.modeling.init_kv_output_aggregator(self.scheduler.connector)  # type: ignore
        ...
        self.batch_queue: queue.Queue[tuple[Future[ModelRunnerOutput], SchedulerOutput]] | None = None
    ...
    def _initialize_kv_caches(self, aphrodite_config: AphroditeConfig) -> tuple[int, int, KVCacheConfig]:
        ...
        if has_kv_cache:
            if os.environ.get("APHRODITE_ELASTIC_EP_SCALE_UP_LAUNCH") == "1":
                ...
                available_gpu_memory = self.modeling.determine_available_memory()
    ...
    def post_step(self, model_executed: bool) -> None:
        if self.use_spec_decode and model_executed:
            ...
            draft_token_ids = self.modeling.take_draft_token_ids()
    ...
    def step_with_batch_queue(
        ...
        if not self.batch_queue.full():
            ...
            if scheduler_output.pending_structured_output_tokens:
                ...
                if model_executed and not self.batch_queue.full() and self.batch_queue.qsize() > 0:
                    # Don't block on next worker response unless the queue is full
                    # or there are no more requests to schedule.
                    # Note: We can't easily peek at the first item in a thread-safe way,
                    # so we just check if queue is not full.
                    return None, True

        elif self.batch_queue.empty():
            # Queue is empty. We should not reach here since this method should
            # only be called when the scheduler contains requests or the queue
            # is non-empty.
            return None, False

        # Block until the next result is available if queue is not empty.
        if not self.batch_queue.empty():
            ...
            if deferred_scheduler_output:
                ...
                future = self.modeling.sample_tokens(grammar_output, non_block=True)
                self.batch_queue.put_nowait((future, deferred_scheduler_output))
    ...
    def execute_dummy_batch(self):
        self.modeling.execute_dummy_batch()
    ...
    def save_sharded_state(
        ...
        self.modeling.save_sharded_state(path=path, pattern=pattern, max_size=max_size)
...
class DPEngineCoreProc(EngineCoreProc):
    ...
    def reinitialize_distributed(self, reconfig_request: ReconfigureDistributedRequest) -> None:
        ...
        if reconfig_request.new_data_parallel_rank == ReconfigureRankType.SHUTDOWN_CURRENT_RANK:
            ...
            logger.info("Distributed environment reinitialized for DP rank %s", self.dp_rank)
