...
from aphrodite.utils import (get_mp_context, get_open_zmq_ipc_path,
                             zmq_socket_ctx)
        ...
        try:
            for proc, local_dp_rank in zip(self.processes, local_dp_ranks):
                with set_device_control_env_var(
                        aphrodite_config, local_dp_rank) if (
...
def set_device_control_env_var(aphrodite_config: AphroditeConfig,
...
class CoreEngineActorManager:
    ...
    def __init__(
        ...
        local_engine_count = \
            aphrodite_config.parallel_config.data_parallel_size_local
        world_size = aphrodite_config.parallel_config.world_size
    ...
    def create_dp_placement_groups(
        ...
        dp_master_ip = \
            aphrodite_config.parallel_config.data_parallel_master_ip
        num_pg_to_create = aphrodite_config.parallel_config.data_parallel_size
        local_engine_count = \
            aphrodite_config.parallel_config.data_parallel_size_local
    ...
    def add_dp_placement_groups(
        ...
        dp_master_ip = \
            old_aphrodite_config.parallel_config.data_parallel_master_ip
    ...
    def scale_up_elastic_ep(self, cur_aphrodite_config: AphroditeConfig,
        ...
        runtime_env = RuntimeEnv(env_vars=self.env_vars_dict
                                 | {"APHRODITE_ELASTIC_EP_SCALE_UP_LAUNCH": "1"})
        for i, (pg,
            ...
            dp_aphrodite_config.parallel_config.data_parallel_size = \
        ...
        cur_aphrodite_config.parallel_config.data_parallel_size = \
            new_data_parallel_size
...
def launch_core_engines(
    ...
    if run_coordinator:
        ...
        logger.info("Started DP Coordinator process (PID: {})",
...
def wait_for_engine_startup(
    ...
    while any(conn_pending) or any(start_pending):
        ...
        if not events:
            if any(conn_pending):
                logger.debug(
                    "Waiting for {} local, {} remote core engine proc(s) "
        ...
        logger.debug("{} from {} core engine process {}.", status,
