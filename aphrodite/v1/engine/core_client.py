...
from aphrodite.v1.engine.tensor_ipc import TensorIpcSender
...
from aphrodite.v1.fault_tolerance.engine_core_sentinel import FT_UTILITY_METHOD
from aphrodite.v1.fault_tolerance.utils import (
    FaultToleranceRequest,
    FaultToleranceResult,
)
from aphrodite.v1.pool.late_interaction import get_late_interaction_engine_index
...
class MPClient(EngineCoreClient):
        ...
        try:
            ...
            while identities:
                if not sync_input_socket.poll(
                    timeout=APHRODITE_ENGINE_READY_TIMEOUT_S * 1000  # convert to ms
                ):
                    raise TimeoutError(
                        ...
                        f"{APHRODITE_ENGINE_READY_TIMEOUT_S}s (configured by "
                        f"APHRODITE_ENGINE_READY_TIMEOUT_S). To increase the "
                        f"timeout, set the environment variable: "
                        f"APHRODITE_ENGINE_READY_TIMEOUT_S=<seconds>"
                    )
    ...
    def _apply_ready_response(self, payload: bytes) -> None:
        ...
        num_gpu_blocks = aphrodite_config.cache_config.num_gpu_blocks or 0
...
class AsyncMPClient(MPClient):
    ...
    def __init__(
        ...
        if self.aphrodite_config.parallel_config.enable_fault_tolerance:
            self._engine_status = {rank: {"id": rank, "status": "healthy"} for rank in self.engine_ranks_managed}
        try:
                    ...
                    if outputs.utility_output:
                        if (
                            ...
                            if outputs.utility_output.result is not None:
                                _self._engine_status[outputs.engine_index] = outputs.utility_output.result.result
                        else:
                            _process_utility_output(outputs.utility_output, utility_results)
                        continue
    ...
    def _send_input_message(self, message: tuple[bytestr, ...], engine: EngineIdentity) -> Awaitable[Any]:
        self.ensure_alive()
        # Any zero-copy tensor/ndarray frames are kept alive by zmq itself
        # until it's finished sending them (there is a ref chain from the underlying
        # memoryview back to the original owning tensor/ndarray).
...
class DPAsyncMPClient(AsyncMPClient):
    ...
    def __init__(
        ...
        self.lb_engines: list[list[int | float]] = [[0, 0, 0.0] for _ in self.core_engines]
                        ...
                        if (
                            ...
                            if len(self.lb_engines) < new_engine_count:
                                self.lb_engines = self.lb_engines + [
                                    [0, 0, 0.0] for _ in range(new_engine_count - len(self.lb_engines))
                                ]
                            else:
...
class DPLBAsyncMPClient(DPAsyncMPClient):
    ...
    async def eep_process_engine_core_notification(self: "DPLBAsyncMPClient", notification_data: tuple[str, int]):
        ...
        if len(cache.pending_notifications[notification_type]) >= abs(cache.num_new_core_engines):
            ...
            self.aphrodite_config.parallel_config.data_parallel_size_local = len(engine_manager.local_engine_actors)
    ...
    async def commit_elastic_ep(self) -> None:
        ...
        self.aphrodite_config.parallel_config.eplb_config.num_redundant_experts = num_redundant_experts
    ...
    async def prepare_elastic_ep(self, new_data_parallel_size: int) -> None:
        ...
        cur_data_parallel_size = len(self.core_engines)
        assert self.aphrodite_config.parallel_config.data_parallel_backend == "ray", (
        ...
        if new_data_parallel_size < cur_data_parallel_size:
            ...
            await self._prepare_scale_up_elastic_ep(new_data_parallel_size, num_redundant_experts)
    ...
    def _wait_for_new_engine_ready(self, new_core_engines: list[bytes]) -> None:
        ...
        while new_engine_identities:
            if not sync_input_socket.poll(timeout=APHRODITE_ENGINE_READY_TIMEOUT_S * 1000):
                raise TimeoutError(
    ...
    async def _prepare_scale_up_elastic_ep(
        ...
        ready_keys.extend(f"eep_ready/{rank}" for rank in range(len(self.core_engines), new_data_parallel_size))
    ...
    async def _commit_scale_up_elastic_ep(self, new_data_parallel_size: int) -> None:
        new_core_engines = [
            rank.to_bytes(2, "little") for rank in range(len(self.core_engines), new_data_parallel_size)
        ]

        await self.pause_scheduler_async(mode="keep", clear_cache=False)
        wait_future = self._eep_wait_for_setup_switch_complete()
        finish_futures = [
            asyncio.create_task(self._call_utility_async("commit_prepared_elastic_ep", engine=engine))
            for engine in self.core_engines
        ]
        try:
        ...
        if isinstance(self.resources.engine_manager, CoreEngineActorManager):
            parallel_config.data_parallel_size_local = len(self.resources.engine_manager.local_engine_actors)
    ...
    async def _commit_scale_down_elastic_ep(self, new_data_parallel_size: int) -> None:
        ...
        for cur_dp_rank, engine in enumerate(old_core_engines):
            if cur_dp_rank < new_data_parallel_size:
                coro = self._call_utility_async("commit_prepared_elastic_ep", engine=engine)
            else:
