...
from aphrodite.v1.utils import BackgroundProcHandle
    ...
    def make_client(
        ...
        if multiprocess_mode and asyncio_mode:
            if aphrodite_config.parallel_config.data_parallel_size > 1:
                return DPAsyncMPClient(aphrodite_config, executor_class, log_stats)

            return AsyncMPClient(aphrodite_config, executor_class, log_stats)
...
class MPClient(EngineCoreClient):
    ...
    def __init__(
        ...
        try:
            ...
            new_core_engine = lambda index, local_dp_rank=None: CoreEngine(
                aphrodite_config, executor_class, log_stats, input_path, self.
                output_path, index, local_dp_rank)

            # Start engine core process(es).
            self._init_core_engines(aphrodite_config, new_core_engine,
    ...
    def _wait_for_engine_startup(self):
        ...
        while identities:
            ...
            # engine core process.
            self.aphrodite_config.cache_config.num_gpu_blocks = message_dict[
                'num_gpu_blocks']
