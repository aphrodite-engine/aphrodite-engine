...
from aphrodite.utils import (close_sockets, get_open_port,
                             get_open_zmq_inproc_path, in_loop,
                             make_zmq_socket)
...
class EngineCoreClient(ABC):
    ...
    * AsyncMPClient: ZMQ + background proc EngineCore w/ asyncio (for AsyncLLM)
    ...
    def make_async_mp_client(
        ...
        client_args = (aphrodite_config, executor_class, log_stats,
    ...
    def __init__(
        ...
        try:
            ...
            if client_addresses is not None:
                ...
                with launch_core_engines(aphrodite_config, executor_class,
    ...
    try:
        ...
        if failure_message is not None:
            logger.error(
                ...
                "with error: {}", failure_message)


class SyncMPClient(MPClient):
    """Synchronous client for multi-proc EngineCore."""

    def __init__(self, aphrodite_config: AphroditeConfig, executor_class: type[Executor],
                 log_stats: bool):
        super().__init__(
            asyncio_mode=False,
            ...
            with (make_zmq_socket(self.ctx,
                ...
                while True:
                    ...
                    if counts is not None:
                        ...
                        logger.debug("Received counts: {} ({})", sliced_counts,
                                     count_slice)
...
class DPLBAsyncMPClient(DPAsyncMPClient):
    ...
    async def scale_elastic_ep(self, new_data_parallel_size: int) -> None:
        ...
        assert self.aphrodite_config.parallel_config.data_parallel_backend == \
    ...
    async def _scale_up_elastic_ep(self, cur_data_parallel_size: int,
        ...
        # for them to be sent
        reconfig_futures = []
        self.aphrodite_config.parallel_config.data_parallel_master_port = \
            get_open_port()
        for engine in self.core_engines:
            reconfig_request = ReconfigureDistributedRequest(
                ...
                new_data_parallel_master_ip=self.aphrodite_config.parallel_config.
                data_parallel_master_ip,
                new_data_parallel_master_port=self.aphrodite_config.parallel_config.
        ...
        self.aphrodite_config.parallel_config.data_parallel_size = \
            new_data_parallel_size
        logger.info(
            "[Elastic EP] Scale up completed, new data parallel size: {}",
            new_data_parallel_size)
    ...
    async def _scale_down_elastic_ep(self, cur_data_parallel_size: int,
        ...
        self.aphrodite_config.parallel_config.data_parallel_size = \
            new_data_parallel_size
        logger.info(
            "[Elastic EP] Scale down completed, new data parallel size: {}",
            new_data_parallel_size)
