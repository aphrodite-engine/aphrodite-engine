...
from aphrodite.inputs import (INPUT_REGISTRY, DecoderOnlyInputs,
                              EncoderDecoderLLMInputs, InputRegistry,
                              PromptType)
...
from aphrodite.v1.core.scheduler import Scheduler
from aphrodite.v1.executor.gpu_executor import GPUExecutor
from aphrodite.v1.request import Request, RequestStatus
from aphrodite.v1.tokenizer.detokenizer import Detokenizer, DetokenizerInputs
...
class AphroditeEngine:
    ...
    def __init__(
        ...
        # NOTE: Now that the detokenizer works asynchronously, we need
    ...
    def _initialize_kv_caches(self) -> None:
        ...
        if self.cache_config.num_gpu_blocks_override is not None:
            ...
            logger.info(
                f"Overriding num_gpu_blocks={num_gpu_blocks} with "
    ...
    def add_request(
        ...
        assert priority == 0, (
            "Aphrodite V1 does not support priority at the moment.")
    ...
    def send_to_detokenizer(self, sampled: List[Tuple[Request, int]]) -> None:
        inputs = DetokenizerInputs(
            ...
            free_req_ids=[],  # TODO: Implement freeing.
        )
