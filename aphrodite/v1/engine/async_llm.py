...
from aphrodite.utils.func_utils import deprecate_kwargs
...
class AsyncLLM(EngineClient):
    def __init__(
        ...
        self.processor = Processor(self.aphrodite_config, tokenizer)
        ...
        if envs.APHRODITE_TORCH_PROFILER_DIR:
            ...
            self.profiler = torch.profiler.profile(
                ...
                on_trace_ready=torch.profiler.tensorboard_trace_handler(
                    envs.APHRODITE_TORCH_PROFILER_DIR, worker_name=worker_name, use_gzip=True
                ),
            )
        else:
            self.profiler = None
        ...
        async def output_handler():
            try:
                while True:
                    ...
                    if num_outputs <= envs.APHRODITE_V1_OUTPUT_PROC_CHUNK_SIZE:
                        slices = (outputs.outputs,)
                    else:
                        slices = np.array_split(
                            outputs.outputs,
                            cdiv(num_outputs, envs.APHRODITE_V1_OUTPUT_PROC_CHUNK_SIZE),
                        )
