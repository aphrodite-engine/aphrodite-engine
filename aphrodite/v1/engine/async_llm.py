...
from aphrodite.outputs import STREAM_FINISHED, PoolingRequestOutput, RequestOutput
from aphrodite.pooling_params import PoolingParams
from aphrodite.profiler.wrapper import TorchProfilerWrapper
...
from aphrodite.v1.engine import EngineCoreRequest, PauseMode
...
class AsyncLLM(EngineClient):
    """An asynchronous wrapper for the Aphrodite engine."""
    ...
    def __init__(
        ...
        Args:
            aphrodite_config: global configuration.
        ...
        if tracing_endpoint is not None:
            init_tracer("aphrodite.llm_engine", tracing_endpoint)
        ...
        self.renderer = renderer = renderer_from_config(self.aphrodite_config)

        # Convert EngineInput --> EngineCoreRequest.
        self.input_processor = InputProcessor(self.aphrodite_config, renderer)

        # Converts EngineCoreOutputs --> RequestOutput.
        self.output_processor = OutputProcessor(
            renderer.tokenizer,
            log_stats=self.log_stats,
            stream_interval=self.aphrodite_config.scheduler_config.stream_interval,
        ...
        if (
            aphrodite_config.profiler_config.profiler == "torch"
            and not aphrodite_config.profiler_config.ignore_frontend
        ):
            profiler_dir = aphrodite_config.profiler_config.torch_profiler_dir
            logger.info(
            ...
            self.profiler = TorchProfilerWrapper(
                aphrodite_config.profiler_config,
    ...
    def check_admission(self, n: int = 1, request_id: str | None = None) -> None:
        ...
        if max_queued_tokens is not None:
            ...
            if current_tokens >= max_queued_tokens:
                logger.info(
                    "Max queued tokens reached - rejecting request %s (current_tokens=%d, max=%d).",
                    request_id,
                    current_tokens,
                    max_queued_tokens,
                )
    ...
    async def _add_request(
        ...
        if parent_req is None and not self.output_processor.has_request(request.request_id):
    ...
    async def generate(
        ...
        *,
        ...
        Note:
            ...
            >>> from aphrodite.inputs import parse_model_prompt
        ...
        except (APHRODITEClientError, GracefulHTTPError) as e:
            if self.log_requests:
    ...
    def _run_output_handler(self):
        ...
        chunk_size = envs.APHRODITE_V1_OUTPUT_PROC_CHUNK_SIZE
    ...
    async def encode(
        ...
        # Request validation error or admission control rejection.
        except (APHRODITEClientError, GracefulHTTPError):
            if self.log_requests:
    ...
    async def _scale_elastic_ep(self, new_data_parallel_size: int, drain_timeout: int) -> None:
    ...
    async def init_weight_transfer_engine(self, request: WeightTransferInitRequest) -> None:
        ...
        """
        await self.collective_rpc("init_weight_transfer_engine", kwargs={"init_info": request.init_info})
