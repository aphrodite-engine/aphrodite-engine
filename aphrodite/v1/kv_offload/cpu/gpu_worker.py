...
from aphrodite.v1.kv_offload.cpu.swap_blocks_triton import (
    THRESHOLD_BYTES,
    swap_blocks_batch,
)
...
def _canonical_page_ids(block_ids: np.ndarray, blocks_per_chunk: int, count: int, skip_count: int) -> np.ndarray:
    ...
    flat = (block_ids[:, np.newaxis] * blocks_per_chunk + np.arange(blocks_per_chunk)).ravel()
    return flat[skip_count : skip_count + count]


def _canonical_block_sizes(layer_refs_per_group: list[list[CanonicalKVCacheRef]], num_tensors: int) -> list[int]:
...
class SingleDirectionOffloadingHandler:
    ...
    def __init__(
        ...
        canonical_bytes_per_block = (
            _canonical_block_sizes(layer_refs_per_group, len(gpu_tensors)) if canonical_layout else None
        )
        ...
        for t_idx, (gpu_tensor, cpu_tensor) in enumerate(zip(gpu_tensors, cpu_tensors)):
            ...
            if canonical_bytes_per_block is not None:
                assert cpu_page_size == canonical_bytes_per_block[t_idx] * blocks_per_chunk
            else:
        ...
        self._swap_blocks_batch = _select_swap_blocks_fn(layer_refs_per_group, gpu_to_cpu)
        ...
        self._canonical_copy_plans: list[list[CopyPlan]] | None = (
            [[_build_copy_plan(ref, gpu_to_cpu) for ref in layer_refs] for layer_refs in layer_refs_per_group]
            if canonical_layout
            else None
        )
        self._fill_group_ops = self._fill_canonical_ops if canonical_layout else self._fill_direct_ops
    ...
    def _estimate_max_copy_ops(self, group_sizes: Sequence[int]) -> int:
        ...
        num_copy_ops = 0
        for g_idx, (group_size, layer_refs) in enumerate(zip(group_sizes, self.layer_refs_per_group)):
            if self._canonical_copy_plans is None:
                num_copy_ops += group_size * len(layer_refs)
            else:
                num_copy_ops += group_size * sum(plan.num_frags for plan in self._canonical_copy_plans[g_idx])
        return num_copy_ops
    ...
    def _fill_canonical_ops(
        ...
        num_bytes = 0
        for plan, data_ref in zip(self._canonical_copy_plans[g_idx], self.layer_refs_per_group[g_idx]):
            if plan.num_frags == 0:
                continue
            t_idx = data_ref.tensor_idx
            ...
            np.add(
                ...
                out=all_src_u64[op_idx:end_idx].reshape(num_active_blocks, plan.num_frags),
            )
            np.add(
                block_bases_dst[:, None],
                plan.frag_offsets_dst[None, :],
                out=all_dst_u64[op_idx:end_idx].reshape(num_active_blocks, plan.num_frags),
            )
            all_sizes[op_idx:end_idx].reshape(num_active_blocks, plan.num_frags)[:] = plan.frag_sizes
    ...
    def shutdown(self) -> None:
        ...
        while self._transfers:
            ...
            try:
                ...
                logger.exception(
                    "Failed to synchronize transfer end event; skipping %d remaining transfers",
...
class CPUOffloadingWorker(OffloadingWorker):
    ...
    def __init__(
        ...
        canonical_bytes_per_block = (
            _canonical_block_sizes(kv_caches.group_data_refs, len(kv_caches.tensors)) if canonical_layout else None
        )
        ...
        for t_idx, kv_cache_tensor in enumerate(kv_caches.tensors):
            ...
            if canonical_bytes_per_block is not None:
                ...
                cpu_tensor = mmap_region.create_next_canonical_view(canonical_bytes_per_block[t_idx] * blocks_per_chunk)
        ...
        if self._mmap_region is not None:
            if handler_failed:
                try:
                    ...
                    logger.warning(
                        "Device sync before mmap cleanup failed; proceeding with cleanup anyway",
                        exc_info=True,
                    )
