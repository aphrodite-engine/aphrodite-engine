...
from aphrodite.v1.kv_offload.tiering.async_lookup import AsyncLookupManager
...
from aphrodite.v1.kv_offload.tiering.fs.io import load_block, probe_o_direct, store_block
...
class FileSystemTierManager(SecondaryTierManager):
    ...
    Cross-process sharing:
        In order to enable KV cache sharing between multiple Aphrodite instances
    ...
    def __init__(
        ...
        """
        Args:
            offloading_spec: contains the aphrodite_config, kv_cache_config
                and blocks_per_chunk.
            ...
            locality: Whether this tier's storage is LOCAL or REMOTE relative
                to the publishing Aphrodite instance.
        """
        ...
        if not self._use_o_direct:
            logger.warning(
                "O_DIRECT is not supported at '%s'; falling back to buffered I/O for the '%s' KV offload tier.",
                root_dir,
                tier_type,
            )
        ...
        self._pool = DualQueueThreadPool(
            n_read_threads,
            n_write_threads,
            thread_name_prefix="aphrodite_kv_py_fs",
        )
