...
try:
    from aphrodite.fs_io_C import (  # pyright: ignore[reportMissingImports]
        batch_load_block as batch_load_block_C,
    )
    from aphrodite.fs_io_C import (
...
def _validate_offsets(view: memoryview, offsets: list[int], block_size: int) -> None:
    ...
    for offset in offsets:
        if offset < 0 or offset + block_size > total_len:
            raise ValueError(
                f"block offset {offset} (block_size {block_size}) is out of bounds for a buffer of size {total_len}"
            )
