...
from aphrodite.v1.kv_offload.tiering.base import TieringOffloadingMetrics
...
class TieringOffloadingSpec(CPUOffloadingSpec):
    ...
    def build_metric_definitions(cls, extra_config: dict[str, Any]) -> dict[str, OffloadingMetricMetadata]:
        ...
        metrics[TieringOffloadingMetrics.LOOKUP_SYNC_DELAY] = OffloadingHistogramMetadata(
            documentation=(
        ...
        metrics[TieringOffloadingMetrics.LOOKUP_ASYNC_DELAY] = OffloadingHistogramMetadata(
            documentation=(
        ...
        metrics[TieringOffloadingMetrics.READ_BYTES] = OffloadingCounterMetadata(
            documentation=("Total bytes read from secondary tiers into the primary tier, labeled by tier."),
            labelnames=("tier",),
        )
        metrics[TieringOffloadingMetrics.READ_TIME] = OffloadingCounterMetadata(
            documentation=(
                "Total time spent reading from secondary tiers into the primary tier, in seconds, labeled by tier."
            ),
            labelnames=("tier",),
        )
        metrics[TieringOffloadingMetrics.WRITE_BYTES] = OffloadingCounterMetadata(
            documentation=("Total bytes written from the primary tier to secondary tiers, labeled by tier."),
            labelnames=("tier",),
        )
        metrics[TieringOffloadingMetrics.WRITE_TIME] = OffloadingCounterMetadata(
            documentation=(
                "Total time spent writing from the primary tier to secondary tiers, in seconds, labeled by tier."
            ),
            labelnames=("tier",),
        )
        metrics[TieringOffloadingMetrics.PROMOTION_JOB_FAILURES] = OffloadingCounterMetadata(
            documentation=("Number of failed secondary-tier promotion jobs, labeled by tier."),
            labelnames=("tier",),
        )
        metrics[TieringOffloadingMetrics.CASCADE_JOB_FAILURES] = OffloadingCounterMetadata(
            documentation=("Number of failed secondary-tier cascade jobs, labeled by tier."),
            labelnames=("tier",),
        )
        metrics[TieringOffloadingMetrics.BLOCK_QUERIES] = OffloadingCounterMetadata(
            documentation=("Number of block lookup queries sent to a tier, labeled by tier."),
            labelnames=("tier",),
        )
        ...
        metrics[TieringOffloadingMetrics.PROMOTION_ALLOCATION_FAILURES] = OffloadingCounterMetadata(
            documentation=(
                "Number of promotion attempts that failed because the primary tier could not allocate space."
            ),
        )
        metrics[TieringOffloadingMetrics.PRIMARY_WRITE_USAGE_PERC] = OffloadingGaugeMetadata(
            documentation=(
                "Current fraction of primary-tier space used by writes from secondary tiers, labeled by tier."
            ),
            labelnames=("tier",),
        )
        metrics[TieringOffloadingMetrics.PRIMARY_READ_USAGE_PERC] = OffloadingGaugeMetadata(
            documentation=("Current fraction of primary-tier space used by reads to secondary tiers, labeled by tier."),
            labelnames=("tier",),
        )
        metrics[TieringOffloadingMetrics.ACTIVE_PROMOTION_JOBS] = OffloadingGaugeMetadata(
            documentation=("Number of active secondary-tier promotion jobs, labeled by tier."),
            labelnames=("tier",),
        )
        metrics[TieringOffloadingMetrics.ACTIVE_CASCADE_JOBS] = OffloadingGaugeMetadata(
            documentation=("Number of active secondary-tier cascade jobs, labeled by tier."),
            labelnames=("tier",),
        )
            ...
            try:
                ...
                for tier in reversed(secondary_tiers):
                    try:
                        ...
                        logger.exception("Failed to shut down secondary tier during initialization cleanup")
                if primary_tier is not None:
                    try:
                        primary_tier.shutdown()
                    except Exception:
                        logger.exception("Failed to shut down primary tier during initialization cleanup")
                elif scheduler_mmap is not None:
                    try:
                        scheduler_mmap.cleanup()
                    except Exception:
                        logger.exception("Failed to clean up scheduler mmap during initialization cleanup")
    ...
    def _validate_canonical_refs(self, kv_caches: CanonicalKVCaches) -> None:
        ...
        all_refs = [ref for group_refs in kv_caches.group_data_refs for ref in group_refs]
        ...
        self.all_layers_portable = all(ref.mapping is not None and ref.mapping.parallelism_agnostic for ref in all_refs)
        if self.config.parallel.is_parallelism_agnostic and not (self.all_layers_portable):
