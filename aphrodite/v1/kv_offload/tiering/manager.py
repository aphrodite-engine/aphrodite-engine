...
from aphrodite.v1.kv_offload.tiering.metrics import TieringMetricsTracker
...
class _SecondaryTierFacingParent(ParentManager):
    ...
    def create_store_job(self, keys: Collection[OffloadKey], req_context: ReqContext) -> TransferJob:
        return self._m.create_store_job(keys, req_context, self._origin_idx)

    def on_request_finished(self, req_context: ReqContext) -> None:
        return self._m.on_request_finished(req_context, exclude_tier_idx=self._origin_idx)
...
class TieringOffloadingManager(OffloadingManager):
    ...
    def __init__(
        ...
        self._tier_parents: dict[SecondaryTierManager, _SecondaryTierFacingParent] = {
            tier: _SecondaryTierFacingParent(self, tier_idx) for tier_idx, tier in enumerate(self.secondary_tiers)
        }
    ...
    def _complete_promotion(self, job_metadata: JobMetadata, completed_job: JobResult) -> None:
        ...
        if completed_job.success:
            ...
            assert failed_keys_set.issuperset(successful_keys), (
                f"Finished promotion job_id {completed_job.job_id} reported unknown successful keys"
            )
        ...
        for i, tier in enumerate(self.secondary_tiers):
            for completed_job in tier.get_finished_jobs():
                ...
                assert job_metadata is not None, (
                    f"Finished job_id {job_id} from tier #{i} ({tier.tier_type}) not in _jobs"
                )
                assert job_metadata.tier_idx == i, (
                    f"Finished job_id {job_id} reported by tier #{i} but belongs to tier #{job_metadata.tier_idx}"
                )
                transfer_job = job_metadata.transfer_job
                ...
                if transfer_job.is_promotion:
                    ...
                    self.primary_tier.complete_read(transfer_job.keys, transfer_job.req_context)
    ...
    def _cascade_existing_blocks_to_request_level_tiers(
        ...
        for key in keys:
            ...
            if result is LookupResult.HIT:
                ...
                assert result is LookupResult.MISS, f"primary tier returned {result} for a cascade key"
        if not ready_keys:
            return
    ...
    def _flush_pending_cascades(self) -> None:
        ...
        for req_id, state in list(self._req_state.items()):
            ...
            self._cascade_existing_blocks_to_request_level_tiers(keys, state.req_context, state.request_level_tiers)
    ...
    def shutdown(self) -> None:
        ...
        for tier_idx, tier in enumerate(self.secondary_tiers):
            try:
                ...
                shutdown_error = exc
                logger.exception(
                    "Failed to shut down secondary tier #%d (tier_type=%s, impl_class=%s)",
                    tier_idx,
                    tier.tier_type,
                    type(tier).__name__,
                )
