...
if TYPE_CHECKING:
    from aphrodite.v1.kv_offload.tiering.base import JobId
...
class ClientRole:
    ...
    def request_blocks(
        ...
        if st.probed and keys:
            assert st.probes, f"symmetric fetch for {kv_request_id} has keys but no probes"
    ...
    def on_transfer_done(self, kv_request_id: str, success: bool, round_seq: int) -> None:
    ...
    def on_abort_ack(self, kv_request_id: str, round_seq: int) -> None:
        ...
        if st is not None and load is not None:
            logger.warning(
                "P2PSession %s: load request %s (job_id=%d) completed with failure after abort",
                self._peer_id,
                kv_request_id,
                load.job_id,
            )
    ...
    def on_lookup_resp(
        ...
        logger.debug(
            "P2P LOOKUP client %s: RECV LookupRespMsg kv_request_id=%s keys=%d hits=%d misses=%d",
            self._peer_id,
            kv_request_id,
            len(keys),
            n_hit,
            ...
            for round_seq, load in st.loads.items():
                if load.aborted_at is None:
                    ...
                    if now - load.aborted_at >= _ABORT_ACK_TIMEOUT_S:
                        ...
                        logger.warning(
                            "P2PSession %s: abort_ack timed out for kv_request_id=%s round=%s",
                            self._peer_id,
                            req_id,
                            round_seq,
                        )
    ...
    def close(self) -> ClientCloseResult:
        ...
        """
        failed_jobs = [load.job_id for st in self._requests.values() for load in st.loads.values()]
        failed_req_ids = [
            req_id
