...
from aphrodite.v1.kv_offload.base import LookupResult, OffloadKey, ReqContext
from aphrodite.v1.kv_offload.tiering.p2p.session.protocol import (
    TYPE_KEY,
    AbortAckMsg,
    LookupRespMsg,
    TransferDoneMsg,
...
if TYPE_CHECKING:
    from aphrodite.v1.kv_offload.tiering.base import JobId, ParentManager
    from aphrodite.v1.kv_offload.tiering.p2p.data import DataTransport
...
class ServerRole:
    ...
    def on_fetch(
        ...
        if existing is not None and existing.demand_received:
            raise ValueError(f"duplicate fetch for kv_request_id={kv_request_id} round={round_seq}")
        st = self._get_or_create_request(kv_request_id)
    ...
    def on_abort_fetch(self, kv_request_id: str, round_seq: int = 0) -> None:
        ...
        st = self._requests.get(kv_request_id)
        if (st is None or not st.outbound) and not self._has_inflight_for(kv_request_id):
            logger.warning(
    ...
    def serve_external_requests(self, parent: ParentManager) -> None:
        ...
        for kv_request_id in list(self._serve_pending):
            ...
            if st.pending_lookups:
                ...
                st.pending_lookups = []
                for pl in pending:
                    self._process_inbound_lookup(kv_request_id, pl.keys, pl.enqueued_at, pl.round_seq, parent)
    ...
    def _process_inbound_lookup(
        ...
        logger.debug(
            "P2P LOOKUP server %s: RESOLVED kv_request_id=%s hits=%d misses=%d pending=%d",
            self._peer_id,
            kv_request_id,
            len(hit_keys),
    ...
    def _resolve_pending_lookups(self, kv_request_id: str, parent: ParentManager) -> None:
    ...
    def _finalize_lookup(self, lookup: _ActiveLookup, parent: ParentManager) -> None:
        ...
        if lookup.keys:
            ...
            logger.debug(
                "P2P LOOKUP server %s: SEND LookupRespMsg kv_request_id=%s keys=%d hits=%d misses=%d",
                self._peer_id,
                lookup.kv_request_id,
                len(lookup.keys),
                n_hit,
    ...
    def collect_results(self) -> list[StoreResult]:
        ...
        for tid in poll_result.done:
            ...
            if st is not None and st.outbound.get(xfer.round_key) is rnd:
                ...
                assert rnd.remaining >= 0, f"remaining went negative for kv_request_id={xfer.kv_request_id}"
                if rnd.remaining == 0:
                    self._finalize_outbound(xfer.kv_request_id, xfer.round_key, success=True)
                elif rnd.finishing and rnd.inflight == 0:
                    self._finalize_outbound(xfer.kv_request_id, xfer.round_key, success=False)
        ...
        if failed_rounds:
            for kv_request_id, rnd in failed_rounds:
                ids_to_cancel = [tid for tid, x in self._inflight.items() if x.round is rnd]
    ...
    def close(self) -> tuple[list[int], list[ReqContext]]:
        ...
        failed_serves = [lu.ctx for st in self._requests.values() for lu in st.lookups.values()]
    ...
    def _settle_xfer_jobs(self, xfer: _InflightXfer, success: bool) -> list[StoreResult]:
    ...
    def _finalize_outbound(
        ...
        if settled is not None:
            ...
            for job_id in req.pending_job_ids:
                if self._store_jobs.pop(job_id, None) is None:
                    continue
                self._pending_store_results.append(StoreResult(job_id=job_id, success=True))
    ...
    def _drain_abort(self, kv_request_id: str, round_seq: int) -> None:
        ...
        ids = [
            tid for tid, x in self._inflight.items() if x.kv_request_id == kv_request_id and x.round_key == round_seq
        ]
        if not ids:
    ...
    def _submit_transfer(
        ...
        if transfer_id is not None:
            ...
            rnd.finishing = True
            st = self._requests.get(kv_request_id)
            if st is not None and st.outbound.get(round_key) is rnd and rnd.inflight == 0:
