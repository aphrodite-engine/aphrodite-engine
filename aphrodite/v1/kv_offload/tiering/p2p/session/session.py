...
from aphrodite.v1.kv_offload.tiering.p2p.session.client import ClientRole, LoadResult
...
if TYPE_CHECKING:
    from aphrodite.v1.kv_offload.base import ReqContext
...
class P2PSession:
    ...
    def _dispatch_message(self, msg: dict) -> None:
        ...
        if msg_type == ConnectMsg.TYPE:
            ...
            keys = [OffloadKey(bh if isinstance(bh, bytes) else bytes(bh)) for bh in msg[FetchMsg.KEYS]]
            ...
            keys = [OffloadKey(bh if isinstance(bh, bytes) else bytes(bh)) for bh in msg[LookupMsg.KEYS]]
            ...
            keys = [OffloadKey(bh if isinstance(bh, bytes) else bytes(bh)) for bh in msg[LookupRespMsg.KEYS]]
