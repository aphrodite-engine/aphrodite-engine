...
class AsyncLookupManager(ABC):
    ...
    def __init__(
        ...
        self._lookup_queue: queue.SimpleQueue[list[tuple[OffloadKey, ReqContext, int]] | None] = queue.SimpleQueue()
        ...
        self._pending_results: queue.SimpleQueue[list[tuple[OffloadKey, int, bool]]] = queue.SimpleQueue()
        self._need_to_drain: bool = False

        self._thread = threading.Thread(
            target=self._worker,
            name=f"aphrodite_offloading_lookup_{tier_type}",
            daemon=True,
        )
        self._thread.start()
