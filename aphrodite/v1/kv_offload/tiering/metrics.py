...
class _RequestMetricsState:
    observed_lookups: dict[TierLabel, dict[OffloadKey, float | None]] | None = field(default_factory=dict)
...
class TieringMetricsTracker:
    ...
    def on_job_finished(self, job_metadata: _JobMetadataLike, result: JobResult) -> None:
    ...
    def on_promotion_allocation_failure(self) -> None:
        self._stats.increase_counter(TieringOffloadingMetrics.PROMOTION_ALLOCATION_FAILURES)
    ...
    def _observe_finished_job_stats(
        ...
        bytes_metric = (
            TieringOffloadingMetrics.READ_BYTES if transfer_job.is_promotion else TieringOffloadingMetrics.WRITE_BYTES
        )
        time_metric = (
            TieringOffloadingMetrics.READ_TIME if transfer_job.is_promotion else TieringOffloadingMetrics.WRITE_TIME
        )
        ...
        if completed_job.transfer_time is not None:
            self._stats.increase_counter(time_metric, completed_job.transfer_time, labelvalues)
    ...
    def _observe_active_transfer_stats(self, stats: OffloadingConnectorStats) -> None:
        for tier_idx, state in enumerate(self._tier_states):
            ...
            write_usage = (
                state.primary_write_block_count / self._num_primary_blocks if self._num_primary_blocks > 0 else 0.0
            )
            read_usage = (
                state.primary_read_block_count / self._num_primary_blocks if self._num_primary_blocks > 0 else 0.0
            )
            stats.set_gauge(
