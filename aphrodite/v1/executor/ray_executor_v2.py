...
class RayWorkerProc(WorkerProc):
    ...
    def __init__(
        ...
        self._parallel_config = aphrodite_config.parallel_config
    ...
    def get_node_and_physical_gpu_ids(self) -> tuple[str, list[int]]:
        ...
        return node_id, [current_platform.device_control_id_to_physical_device_id(str(x)) for x in physical_gpu_ids]
    ...
    def initialize_worker(
        ...
        *assigned_physical_gpu_ids* maps local_rank to physical CUDA device ID.
        ...
        if assigned_physical_gpu_ids is not None:
            ...
            aphrodite_config.parallel_config.assigned_physical_gpu_ids = assigned_physical_gpu_ids
...
class RayExecutorV2(MultiprocExecutor):
    ...
    def _get_actor_resource_kwargs() -> dict[str, Any]:
        ...
        num_devices = envs.APHRODITE_RAY_PER_WORKER_GPUS
        device_key = current_platform.ray_device_key
        if device_key == "GPU":
    ...
    def _init_executor(self) -> None:
        ...
        # while respecting APHRODITE_RAY_BUNDLE_INDICES.
        if envs.APHRODITE_RAY_BUNDLE_INDICES:
            bundle_to_node_id = get_bundles_for_indices(
                placement_group,
                list(map(int, envs.APHRODITE_RAY_BUNDLE_INDICES.split(","))),
                self.world_size,
            )
        else:
        ...
        # *calling* actor (the Aphrodite worker) to opt in via enable_tensor_transport.
        wt_cfg = self.aphrodite_config.weight_transfer_config
        ...
        worker_node_and_physical_gpu_ids = ray.get(
            [h.actor.get_node_and_physical_gpu_ids.remote() for h in self.ray_worker_handles]
        )

        node_workers: dict[str, list[int]] = defaultdict(list)
        node_physical_gpu_ids: dict[str, list[int]] = defaultdict(list)
        for i, (node_id, physical_gpu_ids) in enumerate(worker_node_and_physical_gpu_ids):
        ...
        distributed_init_method = ray.get(self.ray_worker_handles[0].actor.create_dist_init_method.remote())
        init_worker_refs = []
        ...
        if len(node_physical_gpu_ids) == 1:
            ...
            self.aphrodite_config.parallel_config.assigned_physical_gpu_ids = sorted(node_physical_gpu_ids[node_id_0])
