...
class RayDistributedExecutor(Executor):
    ...
    def _init_workers_ray(self, placement_group: "PlacementGroup", **ray_remote_kwargs):
        ...
        # The way ray actors are setup in aphrodite is that the visible devices are
        ...
        for i, (node_id, physical_gpu_ids) in enumerate(worker_node_and_physical_gpu_ids):
            ...
            physical_gpu_ids = [
                current_platform.device_control_id_to_physical_device_id(str(x)) for x in physical_gpu_ids
            ]
        ...
        all_args_to_update_environment_variables: list[dict[str, str]] = [{} for _ in worker_node_and_physical_gpu_ids]
        ...
        def _update_block_size(worker):
            current_platform.update_block_size_for_backend(worker.aphrodite_config)
    ...
    def _compiled_ray_dag(self, enable_asyncio: bool):
        ...
        logger.info(
            "APHRODITE_USE_RAY_COMPILED_DAG_CHANNEL_TYPE = %s",
            envs.APHRODITE_USE_RAY_COMPILED_DAG_CHANNEL_TYPE,
        )
        logger.info(
            "APHRODITE_USE_RAY_COMPILED_DAG_OVERLAP_COMM = %s",
        ...
        return forward_dag.experimental_compile(
            ...
            _overlap_gpu_communication=envs.APHRODITE_USE_RAY_COMPILED_DAG_OVERLAP_COMM,
        )
