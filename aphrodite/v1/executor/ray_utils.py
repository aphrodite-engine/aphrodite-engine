...
def detach_zero_copy_from_model_runner_output(output: "ModelRunnerOutput") -> None:
    ...
    Aphrodite can return numpy-backed logprobs and routed experts in
    ...
    if output.logprobs is not None:
        ...
        if token_ids_c is not token_ids or logprobs_c is not logprobs or ranks_c is not ranks:
    ...
    if output.routed_experts is not None:
        ...
        if routing_data_c is not routing_data or slot_mapping_c is not slot_mapping:
            output.routed_experts = type(output.routed_experts)(routing_data_c, slot_mapping_c)
...
def _verify_bundles(
    ...
    if require_gpu_on_driver and driver_node_id not in node_id_to_bundle:
        raise RuntimeError(
            ...
            "GPUs in a node `{driver_node_id}` before starting an Aphrodite engine."
        )
...
def build_actor_name(
    ...
    name = f"aphrodite_Worker_{instance_id}"
    if tp_size > 1:
...
def get_bundles_for_indices(
    ...
    explicit bundle indices specified via APHRODITE_RAY_BUNDLE_INDICES.
    """
