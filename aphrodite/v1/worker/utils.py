...
from aphrodite.utils.math_utils import largest_power_of_2_divisor
...
class KVBlockZeroer:
    ...
    def _allocate_id_buffers(self) -> None:
        ...
        self._ids_gpu = [
            torch.empty(self._id_cap, dtype=torch.int64, device=self.device) for _ in range(self.max_concurrency)
        ]
        self._id_buffer_index = 0
    ...
    def zero_block_ids(self, block_ids: list[int]) -> None:
        ...
        # while this batch is in flight. Rotate through as many buffers as
        # concurrent in-flight batches, to avoid collisions.
