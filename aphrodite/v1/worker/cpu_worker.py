...
from aphrodite.v1.worker.worker_base import CompilationTimes
...
class CPUWorker(Worker):
    def __init__(
        ...
        if cpu_core.numa_node in allowed_memory_nodes:
            ...
            logger.warning(
                "CPU group key %s is not a valid memory node. Falling back to memory node %s.",
                cpu_core.numa_node,
        ...
        profiler_config = aphrodite_config.profiler_config
    ...
    def init_device(self):
        ...
        os.environ["APHRODITE_DIST_IDENT"] = (
        ...
        if explicit_kv_cache_size is not None:
            if explicit_kv_cache_size > available_memory:
                raise ValueError(
                    ...
                    "Decrease --kv-cache-memory-bytes, APHRODITE_CPU_KVCACHE_SPACE, "
                    "or reduce CPU memory used by other processes."
                )
