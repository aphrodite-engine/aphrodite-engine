...
class CachedRequestState:
    ...
    # Persistent metadata used by custom samplers such as mirostat.
...
class InputBatch:
    def __init__(
        ...
        self.epsilon_cutoff = torch.empty(
            (max_num_reqs,), dtype=torch.float32, device=device
        )
        ...
        self.quadratic_smoothing_factor = torch.empty(
            (max_num_reqs,), dtype=torch.float32, device=device
        )
        ...
        self.quadratic_smoothing_curve = torch.empty(
            (max_num_reqs,), dtype=torch.float32, device=device
        )
        ...
        self.xtc_threshold = torch.empty(
            (max_num_reqs,), dtype=torch.float32, device=device
        )
        ...
        self.xtc_probability = torch.empty(
            (max_num_reqs,), dtype=torch.float32, device=device
        )
        ...
        self.mirostat_mode = torch.empty(
        ...
        self.dry_allowed_length = torch.empty(
            (max_num_reqs,), dtype=torch.int32, device=device
        )
        ...
        self.dry_max_ngram = torch.empty(
            (max_num_reqs,), dtype=torch.int32, device=device
        )
        ...
        self.dry_max_occurrences = torch.empty(
            (max_num_reqs,), dtype=torch.int32, device=device
        )
        ...
        self.dry_early_exit_match_len = torch.empty(
            (max_num_reqs,), dtype=torch.int32, device=device
        )
        ...
        self.no_repeat_ngram_size = torch.empty(
            (max_num_reqs,), dtype=torch.int32, device=device
        )
    ...
    def add_request(
        ...
        if sampling_params := request.sampling_params:
            ...
            if (
                sampling_params.dynatemp_min > _SAMPLING_EPS
                or sampling_params.dynatemp_max > _SAMPLING_EPS
            ):
            ...
            if sampling_params.mirostat_mode != 0:
    ...
    def swap_states(self, i1: int, i2: int) -> None:
        ...
        self.dynatemp_min_cpu[i1], self.dynatemp_min_cpu[i2] = (
            self.dynatemp_min_cpu[i2],
            self.dynatemp_min_cpu[i1],
        )
        self.dynatemp_max_cpu[i1], self.dynatemp_max_cpu[i2] = (
            self.dynatemp_max_cpu[i2],
            self.dynatemp_max_cpu[i1],
        )
        self.dynatemp_exp_cpu[i1], self.dynatemp_exp_cpu[i2] = (
            self.dynatemp_exp_cpu[i2],
            self.dynatemp_exp_cpu[i1],
        )
        ...
        self.eta_cutoff_cpu[i1], self.eta_cutoff_cpu[i2] = (
            self.eta_cutoff_cpu[i2],
            self.eta_cutoff_cpu[i1],
        )
        ...
        self.typical_p_cpu[i1], self.typical_p_cpu[i2] = (
            self.typical_p_cpu[i2],
            self.typical_p_cpu[i1],
        )
        ...
        self.xtc_threshold_cpu[i1], self.xtc_threshold_cpu[i2] = (
            self.xtc_threshold_cpu[i2],
            self.xtc_threshold_cpu[i1],
        )
        ...
        self.top_nsigma_cpu[i1], self.top_nsigma_cpu[i2] = (
            self.top_nsigma_cpu[i2],
            self.top_nsigma_cpu[i1],
        )
        self.mirostat_mode_cpu[i1], self.mirostat_mode_cpu[i2] = (
            self.mirostat_mode_cpu[i2],
            self.mirostat_mode_cpu[i1],
        )
        self.mirostat_tau_cpu[i1], self.mirostat_tau_cpu[i2] = (
            self.mirostat_tau_cpu[i2],
            self.mirostat_tau_cpu[i1],
        )
        self.mirostat_eta_cpu[i1], self.mirostat_eta_cpu[i2] = (
            self.mirostat_eta_cpu[i2],
            self.mirostat_eta_cpu[i1],
        )
        ...
        self.dry_base_cpu[i1], self.dry_base_cpu[i2] = (
            self.dry_base_cpu[i2],
            self.dry_base_cpu[i1],
        )
        ...
        self.dry_ranges_cpu[i1], self.dry_ranges_cpu[i2] = (
            self.dry_ranges_cpu[i2],
            self.dry_ranges_cpu[i1],
        )
        self.dry_max_ngram_cpu[i1], self.dry_max_ngram_cpu[i2] = (
            self.dry_max_ngram_cpu[i2],
            self.dry_max_ngram_cpu[i1],
        )
        ...
        swap_dict_values(self.dry_sequence_breaker_ids, i1, i2)
        swap_dict_values(self.persistent_data, i1, i2)
    ...
    def condense(self) -> None:
        ...
        while empty_req_indices:
            ...
            self.epsilon_cutoff_cpu[empty_index] = self.epsilon_cutoff_cpu[
                last_req_index
            ]
            ...
            self.xtc_threshold_cpu[empty_index] = self.xtc_threshold_cpu[
                last_req_index
            ]
            self.xtc_probability_cpu[empty_index] = self.xtc_probability_cpu[
                last_req_index
            ]
            ...
            self.dry_multiplier_cpu[empty_index] = self.dry_multiplier_cpu[
                last_req_index
            ]
            ...
            self.dry_max_ngram_cpu[empty_index] = self.dry_max_ngram_cpu[
                last_req_index
            ]
            ...
            dry_sequence_breaker_ids = self.dry_sequence_breaker_ids.pop(
                last_req_index, None
            )
            if dry_sequence_breaker_ids is not None:
                self.dry_sequence_breaker_ids[empty_index] = (
                    dry_sequence_breaker_ids
                )
            persistent_data = self.persistent_data.pop(last_req_index, None)
            if persistent_data is not None:
                self.persistent_data[empty_index] = persistent_data
    ...
    def _make_sampling_metadata(self) -> SamplingMetadata:
        ...
        if not self.no_epsilon_cutoff:
            copy_slice(
                self.epsilon_cutoff_cpu_tensor, self.epsilon_cutoff, num_reqs
            )
        if not self.no_typical_p:
            copy_slice(self.typical_p_cpu_tensor, self.typical_p, num_reqs)
        if not self.no_quadratic:
            copy_slice(
                self.quadratic_smoothing_factor_cpu_tensor,
                self.quadratic_smoothing_factor,
                num_reqs,
            )
            copy_slice(
                self.quadratic_smoothing_curve_cpu_tensor,
                self.quadratic_smoothing_curve,
                num_reqs,
            )
        if not self.no_xtc:
            copy_slice(self.xtc_threshold_cpu_tensor, self.xtc_threshold, num_reqs)
            copy_slice(
                self.xtc_probability_cpu_tensor, self.xtc_probability, num_reqs
            )
        ...
        if not self.no_dry:
            ...
            copy_slice(
                self.dry_allowed_length_cpu_tensor, self.dry_allowed_length, num_reqs
            )
            ...
            copy_slice(
                self.dry_max_occurrences_cpu_tensor,
                self.dry_max_occurrences,
                num_reqs,
            )
            copy_slice(
                self.dry_early_exit_match_len_cpu_tensor,
                self.dry_early_exit_match_len,
                num_reqs,
            )
        if not self.no_no_repeat_ngram:
            copy_slice(
                self.no_repeat_ngram_size_cpu_tensor,
        ...
        return SamplingMetadata(
            ...
            dry_sequence_breaker_ids=(
                None
                if self.no_dry
                else self._make_dry_sequence_breaker_ids_tensor(num_reqs)
            ),
            ...
            dry_early_exit_match_len=(
                None if self.no_dry else self.dry_early_exit_match_len[:num_reqs]
            ),
            no_repeat_ngram_size=(
                None
                if self.no_no_repeat_ngram
                else self.no_repeat_ngram_size[:num_reqs]
            ),
            tfs=None if self.no_tfs else self.tfs[:num_reqs],
            eta_cutoff=(None if self.no_eta_cutoff else self.eta_cutoff[:num_reqs]),
            epsilon_cutoff=(
                None if self.no_epsilon_cutoff else self.epsilon_cutoff[:num_reqs]
            ),
            typical_p=(None if self.no_typical_p else self.typical_p[:num_reqs]),
            quadratic_smoothing_factor=(
                None
                if self.no_quadratic
                else self.quadratic_smoothing_factor[:num_reqs]
            ),
            quadratic_smoothing_curve=(
                None
                if self.no_quadratic
                else self.quadratic_smoothing_curve[:num_reqs]
            ),
