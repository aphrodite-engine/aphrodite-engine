...
from aphrodite.v1.attention.backends.utils import split_decodes_prefills_and_extends
...
def prepare_dcp_dummy_context_metadata(
    *,
    ...
    for blk_table in input_batch.block_table.block_tables:
        max_row_blocks = blk_table.max_num_blocks_per_req // blk_table.blocks_per_kv_block
        block_ids = [(block_idx % max_valid_block_id) + 1 for block_idx in range(max_row_blocks)]
        for req_idx in range(num_reqs):
            blk_table.add_row(block_ids, req_idx)
        blk_table.commit_block_table(num_reqs)

    query_pos.copy_to_gpu(num_tokens_unpadded)
    positions[:num_tokens_unpadded] = query_pos.gpu[:num_tokens_unpadded] + dcp_dummy_context_len
...
def run_split_fa2_dcp_context_attention(
    ...
    if num_context_prefill_tokens > 0:
        ...
        prefill_query_start_loc = (
            cu_seqlens_q[num_decode_reqs : num_decode_reqs + num_context_prefill_reqs + 1] - num_decode_tokens
        )
        prefill_req_slice = slice(num_decode_reqs, num_decode_reqs + num_context_prefill_reqs)
