...
class EncoderRunner:
        ...
        for req_id, encoder_input_ids in scheduled_encoder_inputs.items():
            ...
            for mm_input_id in encoder_input_ids:
                ...
                if mm_feature.modality == "prompt_embeds":
                    ...
                    self.encoder_cache.encoder_outputs[mm_feature.identifier] = async_tensor_h2d(
                        embeds, device=self.device
                    )
                    continue
    ...
    def execute_mm_encoder(self, mm_kwargs: list[tuple[str, MultiModalKwargsItem]]) -> list[torch.Tensor]:
        ...
        for modality, num_items, mm_kwargs_batch in group_and_batch_mm_kwargs(
            ...
            cudagraph_output = (
                ...
                if cg_manager is not None and cg_manager.is_captured() and cg_manager.supports_modality(modality)
                else None
            )
            batch_outputs = (
                cudagraph_output if cudagraph_output is not None else self.model.embed_multimodal(**mm_kwargs_batch)
            )
        ...
        try:
            ...
            with self._timing_lock:
                for req_id in request_ids:
                    stats = self.encoder_timing_registry.setdefault(req_id, EncoderTimingStats())
    ...
    def gather_mm_embeddings(
        ...
        for i, req_id in enumerate(req_ids):
            ...
            mm_features = self.encoder_cache.mm_features[req_id]
            lo, hi = get_mm_features_in_window(mm_features, start=cur_query_start, end=cur_query_end)
