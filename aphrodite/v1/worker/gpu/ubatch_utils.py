...
from aphrodite.v1.worker.ubatching import make_ubatch_contexts
...
def _slice_input_batch(
    ...
    seq_lens_cpu_upper_bound = input_batch.seq_lens_cpu_upper_bound[req_start:req_stop].clone()
    ...
    return replace(
        ...
        num_computed_prefill_tokens_np=input_batch.num_computed_prefill_tokens_np[req_start:req_stop],
        ...
        prompt_lens=(None if input_batch.prompt_lens is None else input_batch.prompt_lens[req_start:req_stop]),
    )
...
def slice_model_inputs(model_inputs: dict[str, Any], token_slice: slice) -> dict[str, Any]:
    ...
    sliced["positions"] = positions[:, token_slice] if positions.ndim == 2 else positions[token_slice]
...
def merge_ubatch_outputs(outputs: list[Any]) -> Any:
    ...
    if isinstance(first, IntermediateTensors):
        return IntermediateTensors(
            {key: torch.cat([out.tensors[key] for out in outputs], dim=0) for key in first.tensors}
        )
...
class UBatchRunner:
    ...
    points inside the model (see `aphrodite.v1.worker.ubatching`), so one
    ...
    def __init__(
        ...
        self.ubatch_query_start_loc = [
            torch.zeros(max_num_reqs + 1, dtype=torch.int32, device=device) for _ in range(self.num_ubatches)
        ]
        self.ubatch_seq_lens = [
            torch.zeros(max_num_reqs, dtype=torch.int32, device=device) for _ in range(self.num_ubatches)
        ]
    ...
    def prepare(
        ...
        for i, ubatch_slice in enumerate(ubatch_slices):
            ...
            ubatch_block_tables = tuple(block_table[ubatch_slice.request_slice] for block_table in block_tables)
            ...
            slot_mappings_by_layer.append(build_slot_mappings_by_layer(ubatch_slot_mappings, self.kv_cache_config))
    ...
    def _make_forward_contexts(
        ...
        for i, ubatch_slice in enumerate(ubatch_slices):
            ...
            num_tokens_across_dp = torch.full((dp_size,), num_tokens, dtype=torch.int32, device="cpu")
            forward_contexts.append(
                create_forward_context(
                    attn_metadata[i],
                    self.aphrodite_config,
                    dp_metadata=DPMetadata.make(self.parallel_config, num_tokens, num_tokens_across_dp),
    ...
    def run(
        ...
        if errors:
            failed = min(errors)
            raise RuntimeError(f"Microbatch {failed} of {self.num_ubatches} failed") from errors[failed]
...
def maybe_build_ubatch_runner(
    ...
    logger.info_once(
        "Dual batch overlap is enabled. Microbatched steps run without CUDA graphs on the V2 model runner."
    )
    return UBatchRunner(aphrodite_config, device, model_state, attn_groups, kv_cache_config, max_num_reqs)
