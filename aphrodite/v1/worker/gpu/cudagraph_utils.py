...
from aphrodite.v1.worker.utils import AttentionGroup, clear_layer_kv_caches

if TYPE_CHECKING:
    from aphrodite.v1.worker.gpu.model_runner import GPUModelRunner
    from aphrodite.v1.worker.gpu.pcp_manager import PCPManager
...
def make_cudagraph_stats(batch_desc: BatchExecutionDescriptor, num_tokens: int) -> CUDAGraphStat:
...
def _is_compatible(
    ...
    return (
        ...
        and (desc.max_query_len is None or (max_query_len is not None and desc.max_query_len >= max_query_len))
...
def has_compiled_submodule(model: nn.Module) -> bool:
    ...
    return any(
        isinstance(m, TorchCompileWithNoGuardsWrapper) and not getattr(m, "do_not_compile", True)
...
class CudaGraphManager:
    ...
    def _init_candidates(self) -> None:
        ...
        if speculative_config and speculative_config.uses_dynamic_speculative_decoding():
            ...
            dense_schedule = build_dynamic_sd_schedule_lookup(
                ...
                aphrodite_max_batch_size=self.max_num_reqs,
                aphrodite_num_speculative_tokens=self.aphrodite_config.num_speculative_tokens,
            )
            decode_query_lens = sorted({num_spec + num_new_sampled_tokens_per_step for num_spec in dense_schedule[1:]})
        else:
            decode_query_lens = [self.decode_query_len]

        capture_varlen_decode = separate_decode_routine and bool(decode_mode) and self.varlen_decode
        ...
        for mode in (CUDAGraphMode.FULL, CUDAGraphMode.PIECEWISE):
            ...
            for num_active_loras in self.lora_capture_cases:
                lora_descs = [d for d in mode_descs if d.num_active_loras == num_active_loras]
            ...
            for mode in [CUDAGraphMode.PIECEWISE, CUDAGraphMode.FULL]:
                ...
                for desc in descs:
                    ...
                    if desc.cg_mode == CUDAGraphMode.PIECEWISE and not self.use_breakable_cg:
                        ...
                        with torch.cuda.graph(graph, self.pool, stream=current_stream()):
                            forward_fn(CUDAGraphMode.NONE)
    ...
    def captured_token_counts(self) -> list[int]:
        ...
        return sorted({desc.num_tokens for desc in self.graphs if desc.num_active_loras == 0})
    ...
    def capture(
        ...
        if self.cudagraph_mode.has_piecewise_cudagraphs() and not (
            ...
            raise RuntimeError(
                ...
                "Set APHRODITE_USE_BREAKABLE_CUDAGRAPH=1 or cudagraph_mode=NONE/FULL."
            )
...
def prepare_inputs_to_capture(
    ...
    input_batch = InputBatch.make_dummy(num_reqs, num_tokens, input_buffers, max_query_len=max_query_len)
...
def profile_cudagraph_memory(runner: "GPUModelRunner") -> int:
    ...
    try:
        with set_current_aphrodite_config(runner.aphrodite_config):
        ...
        try:
            ...
            if speculator is not None:
                spec_manager_names = [
                    name for name, value in vars(speculator).items() if isinstance(value, CudaGraphManager)
                ]
...
def _init_minimal_kv_cache_for_profiling(runner: "GPUModelRunner") -> None:
    ...
    kv_cache_groups = get_kv_cache_groups(runner.aphrodite_config, kv_cache_spec)
    # At least one block per sequence is required to capture the graphs.
    min_blocks = min(runner.max_num_reqs, runner.compilation_config.max_cudagraph_capture_size) or 1
    saved_override = runner.cache_config.num_gpu_blocks_override
    runner.cache_config.num_gpu_blocks_override = min_blocks
    try:
        minimal_config = get_kv_cache_config_from_groups(runner.aphrodite_config, kv_cache_groups, available_memory=0)
    finally:
