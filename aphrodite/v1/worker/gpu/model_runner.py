...
from aphrodite.v1.worker.gpu.async_utils import AsyncOutput, AsyncPoolingOutput
...
from aphrodite.v1.worker.gpu.spec_decode.rejection_sampler import RejectionSampler
...
class GPUModelRunner(LoRAModelRunnerMixin):
    def __init__(self, aphrodite_config: AphroditeConfig, device: torch.device):
        ...
        if self.speculative_config is not None:
            ...
            if self.speculative_config.method in ("eagle3", "dflash", "dspark"):
                ...
                if self.use_pp:
                    raise ValueError(f"{self.speculative_config.method} with pipeline parallel is not supported.")
    ...
    def get_supported_tasks(self) -> tuple[SupportedTask, ...]:
        ...
        if self.is_pooling_model:
            ...
            # on the last PP rank.
            tasks.extend(PoolingRunner.get_supported_tasks(self.model, self.model_config))
        return tuple(tasks)
    ...
    def initialize_kv_cache(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        self.attn_groups, attn_cg_support, self.kernel_block_sizes = init_attn_backend(
            self.kv_cache_config, self.aphrodite_config, self.device
        )
        attn_cg_support = attn_cg_support.narrow(*self.model_state.get_additional_cg_support())
    ...
    def prepare_dummy_attn(self, input_batch: InputBatch) -> tuple[tuple[torch.Tensor, ...], torch.Tensor]:
...
def sort_batch_req_ids(num_tokens_per_req: dict[str, int], decode_query_len: int) -> list[str]:
