...
from aphrodite.model_executor.models.interfaces import requires_raw_input_tokens
...
from aphrodite.v1.worker.cp_utils import check_attention_cp_compatibility
from aphrodite.v1.worker.gpu import pcp_manager as pcp
from aphrodite.v1.worker.gpu.async_utils import (
    AsyncOutput,
    AsyncPoolingOutput,
    StepTimingCollector,
)
...
from aphrodite.v1.worker.gpu.buffer_utils import (
    async_copy_to_gpu,
    set_default_max_concurrency,
)
from aphrodite.v1.worker.gpu.cp_utils import prepare_dcp_local_seq_lens
...
from aphrodite.v1.worker.gpu.ec_connector import get_ec_connector
from aphrodite.v1.worker.gpu.eplb_utils import EPLBController, step_eplb_after
from aphrodite.v1.worker.gpu.input_batch import (
    InputBatch,
    InputBuffers,
...
from aphrodite.v1.worker.gpu.kv_connector import (
    NO_OP_KV_CONNECTOR,
    KVConnector,
    get_kv_connector,
)
from aphrodite.v1.worker.gpu.lora_utils import (
    LoraState,
...
from aphrodite.v1.worker.gpu.model_states import init_model_state
from aphrodite.v1.worker.gpu.pool.pooling_runner import PoolingRunner
from aphrodite.v1.worker.gpu.pp_utils import PPHandler
from aphrodite.v1.worker.gpu.sample.batch_shard import (
    BatchSharder,
    all_to_all_logits,
    gather_sampler_output,
)
from aphrodite.v1.worker.gpu.sample.output import SamplerOutput
from aphrodite.v1.worker.gpu.sample.prompt_logprob import PromptLogprobsWorker
from aphrodite.v1.worker.gpu.sample.sampler import Sampler
from aphrodite.v1.worker.gpu.shutdown import free_before_shutdown
from aphrodite.v1.worker.gpu.spec_decode import init_speculator
from aphrodite.v1.worker.gpu.spec_decode.adaptive_verification import (
...
from aphrodite.v1.worker.gpu.spec_decode.rejection_sampler import (
    RejectionSampler,
    get_max_chunk_logits,
)
from aphrodite.v1.worker.gpu.spec_decode.speculator import DraftModelSpeculator
from aphrodite.v1.worker.gpu.spec_decode.utils import DraftTokensHandler
...
from aphrodite.v1.worker.gpu.ubatch_utils import (
    UBatchRunner,
    UBatchState,
...
from aphrodite.v1.worker.workspace import lock_workspace, use_workspace_lane
...
class GPUModelRunner(LoRAModelRunnerMixin):
    def __init__(self, aphrodite_config: AphroditeConfig, device: torch.device):
        ...
        self._draft_workspace_lane = int(self.speculative_config is not None and self.speculative_config.use_dspark())
        ...
        set_default_max_concurrency(aphrodite_config.max_concurrent_batches)
        ...
        self.uses_inputs_embeds = self.supports_mm_inputs or self.model_config.enable_prompt_embeds
        self.encoder_cache = None
        if self.supports_mm_inputs and self.is_first_pp_rank:
            self.encoder_cache = EncoderCache()
        self.ec_connector = get_ec_connector(aphrodite_config, self.encoder_cache)

        # Speculative decoding.
        self.speculator = None
        self.use_aux_hidden_state_outputs = False
        self.num_speculative_steps = aphrodite_config.num_speculative_tokens
        if self.speculative_config is not None:
            if self.is_last_pp_rank:
                self.speculator = init_speculator(self.aphrodite_config, self.device)
        ...
        num_prefill_lookahead = (
            ...
            if self.speculative_config is not None and self.speculative_config.use_multi_module_mtp()
            else 1
        )
        ...
        if self.lora_config:
            self.lora_capture_cases = get_lora_capture_cases(self.lora_config, self.compilation_config)
        ...
        set_offloader(create_offloader(self.aphrodite_config.offload_config))
    ...
    def load_model(self, load_dummy_weights: bool = False, *args, **kwargs) -> None:
        ...
        with DeviceMemoryProfiler() as m:
            model_loader = get_model_loader(self.aphrodite_config.load_config)
            ...
            if self.use_aux_hidden_state_outputs:
                ...
                if self.use_pp:
                    ...
                    verify_supports_aux_hidden_states_over_pp(self.model, self.speculative_config.method)
        ...
        self.model_state = init_model_state(self.aphrodite_config, self.model, self.encoder_cache, self.device)

        self.decode_query_len = self.num_speculative_steps + self.model_state.num_new_sampled_tokens_per_step
        ...
        if self.parallel_config.enable_batch_sharded_sampling:
            if hasattr(self.model, "compute_logits_local"):
                ...
                logger.warning_once(
                    "Disabling batch-sharded sampling: %s does not implement compute_logits_local",
        ...
        if self.is_pooling_model and self.is_last_pp_rank:
            self.pooling_runner = PoolingRunner(self.model, self.aphrodite_config)
    ...
    def reload_weights(self, *args, **kwargs) -> None:
        ...
        from aphrodite.v1.worker.gpu_model_runner import GPUModelRunner as GPUModelRunnerV1
    ...
    def update_config(self, *args, **kwargs) -> None:
        ...
        # v2 reads config via self.aphrodite_config (e.g. in load_model), so keep it
        # in sync with the attributes the v1 helper just replaced.
        self.aphrodite_config.model_config = self.model_config
        self.aphrodite_config.load_config = self.load_config
    ...
    def get_kv_cache_spec(self):
        return get_kv_cache_spec(self.aphrodite_config)
    ...
    def initialize_kv_cache(
        ...
        for kv_cache_group in kv_cache_config.kv_cache_groups:
            ...
            max_num_blocks = spec.max_num_blocks_per_req(self.aphrodite_config, block_table_max_model_len)
            ...
            if isinstance(layer_spec, (MambaSpec, CircularBufferSpec)):
                max_num_blocks = get_block_table_width(max_num_blocks, spec.block_size, token_alignment=None)
            else:
        ...
        if is_profiling:
            ...
            self.kv_connector = get_kv_connector(self.aphrodite_config, kv_caches_dict)
    ...
    def _dummy_run(
        ...
        **kwargs,
        ...
        num_tokens_per_request = [
            num_tokens // num_reqs + (i >= num_reqs - num_tokens % num_reqs) for i in range(num_reqs)
        ]

        assert sum(num_tokens_per_request) == num_tokens
        num_scheduled_tokens = {f"_dummy_req_{i}": n for i, n in enumerate(num_tokens_per_request)}
        ...
        if self.speculator is not None:
            ...
            with use_workspace_lane(self._draft_workspace_lane):
                self.speculator.propose(
                    ...
                    num_sampled=torch.ones(input_batch.num_reqs, dtype=torch.int32, device=self.device),
                    num_rejected=torch.zeros(input_batch.num_reqs, dtype=torch.int32, device=self.device),
    ...
    def _dummy_sampler_run(self, hidden_states: torch.Tensor) -> None:
        ...
        dummy_input_batch = InputBatch.make_dummy(num_reqs, num_reqs, self.input_buffers)
    ...
    def capture_model(self, *, profile_only: bool = False) -> int:
        ...
        capture_encoder = self.model_state.supports_mm_inputs and self.model_state.encoder_runner.has_cudagraph()
    ...
    def update_pp_decode_requests(self):
        ...
        if self.pp_handler is not None:
            outputs = self.pp_handler.get_prev_sampled_outputs(self.req_states.draft_tokens)
    ...
    def add_requests(self, scheduler_output: SchedulerOutput) -> None:
        for new_req_data in scheduler_output.scheduled_new_reqs:
            ...
            self.block_tables.append_block_ids(req_index, new_req_data.block_ids, overwrite=True)
    ...
    def gather_batch_req_state(
        ...
        if dummy_run:
            ...
            return None, get_uniform_decode_token_count(num_reqs, num_toks, max_query_len, has_prefill=False)
        ...
        if self.adaptive_verification is not None and draft_tokens:
            num_toks = self.adaptive_verification.get_num_tokens(num_tokens_per_req, draft_tokens)
        ...
        return batch_state, get_uniform_decode_token_count(num_reqs, num_toks, max_query_len, batch_state.has_prefill)
    ...
    def prepare_inputs(
        ...
        assert num_tokens > 0
        if envs.APHRODITE_MOE_SKIP_PADDING:
        ...
        input_batch = InputBatch(
            ...
            max_query_len=(int(num_scheduled_tokens_upper_bound.max()) if adaptive_verification is not None else None),
        )
    ...
    def prepare_attn(self, input_batch: InputBatch) -> tuple[tuple[torch.Tensor, ...], torch.Tensor]:
        if self.pcp_manager is not None:
        ...
        if shard_metadata is not None:
            ...
            sampler_output = gather_sampler_output(
                ...
                logprobs_dims=self.sampler.get_logprobs_dims(
                    ...
                    include_token_ids=(global_input_batch.num_draft_tokens == 0 or self.rejection_sampler is None),
                ),
            )
    ...
    def postprocess_sampled(
        ...
        self.model_state.postprocess_state(idx_mapping, num_sampled, self.req_states.num_computed_tokens.gpu)
    ...
    def execute_model(
        ...
        if self.lora_config:
            ...
            num_active_loras = get_num_active_loras_for_dispatch(self.lora_config, self.lora_state, req_ids, dummy_run)
        ...
        batch_desc, dp_sync = dispatch_cg_and_sync_dp(
            ...
            allow_ubatching=(self.ubatch_runner is not None and not skip_attn_for_dummy_run),
        ...
        if batch_desc.num_ubatches > 1:
            ...
            ubatch_state = self.ubatch_runner.prepare(input_batch, block_tables, slot_mappings)
            ...
            if dummy_run and is_profile:
                ...
                attn_groups = [
                    [g for g in groups if not isinstance(g.kv_cache_spec, MambaSpec)] for groups in attn_groups
                ]
        ...
        if not self.is_first_pp_rank:
            ...
            new_tensors = {
                k: v[:n] if dummy_run else v[:n].copy_(intermediate_tensors.tensors[k][:n])
        ...
        self.eplb.prepare_forward(self.model_config, input_batch.num_tokens, ubatch_slices)

        self.step_timing.record_batch(input_batch, batch_desc.cg_mode == CUDAGraphMode.FULL)
        ...
        if batch_desc.cg_mode == CUDAGraphMode.FULL:
            ...
            with set_forward_context(
                ...
                if ubatch_state is not None:
                    assert self.ubatch_runner is not None
                    model_output = self.ubatch_runner.run(self.model, model_inputs, ubatch_state)
    ...
    @torch.inference_mode()
    @step_eplb_after()
    def sample_tokens(self, grammar_output: GrammarOutput | None) -> AsyncOutput | ModelRunnerOutput | None:
        ...
        hidden_states, input_batch = pcp.maybe_restore_pcp_for_sampling(self.pcp_manager, hidden_states, input_batch)
        ...
        if self.speculator is not None and self.speculator.supports_mm_inputs:
            ...
            mm_inputs = self.model_state.gather_mm_embeddings(input_batch, draft_lookahead=1)
        ...
        if self.speculator is not None:
            ...
            if self.adaptive_verification is not None:
                self.adaptive_verification.record_confidences(self.speculator.draft_token_confidence_probs, input_batch)
    ...
    def pool(self) -> AsyncPoolingOutput | ModelRunnerOutput | None:
        ...
        assert self.pooling_runner is not None
        pooler_output, finished_mask = self.pooling_runner.pool(hidden_states, input_batch, self.req_states)
    ...
    def shutdown(self) -> None:
        ...
        free_before_shutdown(self.aphrodite_config)
