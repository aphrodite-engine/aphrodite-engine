...
def _trace_replay_kernel(
    ...
    step = tl.load(total_len_ptr + req_state_idx) - tl.load(prompt_len_ptr + req_state_idx)
    if step < 0 or step >= trace_len:
        return

    token_id = tl.load(trace_token_ids_ptr + req_state_idx * trace_token_ids_stride + step)
