...
from aphrodite.v1.worker.gpu.buffer_utils import UvaBackedTensor
...
class ThinkingBudgetState:
    ...
    def __init__(
        ...
        start_ids = [] if reasoning_config is None else reasoning_config.reasoning_start_token_ids or []
        end_ids = [] if reasoning_config is None else reasoning_config.reasoning_end_token_ids or []
        natural_end_ids = [] if reasoning_config is None else reasoning_config.natural_reasoning_end_token_ids or []
        self.enabled = bool(start_ids and end_ids and natural_end_ids)
        if not self.enabled:
            return

        self.thinking_token_budget = UvaBackedTensor(self.max_num_reqs, dtype=torch.int32)
        ...
        self.cached_last_start = torch.full((self.max_num_reqs,), -1, dtype=torch.int32, device=self.device)
        self.cached_last_end = torch.full((self.max_num_reqs,), -1, dtype=torch.int32, device=self.device)
        self.cached_scan_pos = torch.zeros(self.max_num_reqs, dtype=torch.int32, device=self.device)
        self._reset_reqs: list[int] = []
        self._budget_dirty = False

        self.reasoning_start_token_ids = torch.tensor(start_ids, dtype=torch.int32, device=self.device)
        self.natural_reasoning_end_token_ids = torch.tensor(natural_end_ids, dtype=torch.int32, device=self.device)
        self.reasoning_end_token_ids = torch.tensor(end_ids, dtype=torch.int32, device=self.device)
    ...
    def apply_staged_writes(self) -> None:
        if not self.enabled:
            return
        if self._reset_reqs:
            idx = async_tensor_h2d(self._reset_reqs, dtype=torch.int64, device=self.device)
