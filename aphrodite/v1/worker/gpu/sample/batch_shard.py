...
@triton.jit(do_not_specialize=["num_reqs", "local_logits_start", "max_num_reqs_per_rank"])
def _build_shard_plan_kernel(
    ...
    num_logits = tl.load(cu_num_logits_ptr + req_idx + 1) - tl.load(cu_num_logits_ptr + req_idx)
    ...
    num_logits_per_req = tl.load(cu_num_logits_ptr + req_block + 1, mask=req_mask, other=0) - tl.load(
        cu_num_logits_ptr + req_block, mask=req_mask, other=0
    )
    ...
    # batch.
    req_logits_start = tl.sum(tl.where(lower, num_logits_per_req, 0)) + tl.sum(tl.where(earlier, num_logits_per_req, 0))
...
class BatchSharder:
    ...
    def shard_sampler_inputs(
        ...
        num_logits_per_rank_np = np.bincount(req_owner_np, weights=num_logits_per_req_np, minlength=tp_size).astype(
            np.int64
        )
        ...
        np.cumsum(num_logits_per_req_np[local_req_indices_np], out=local_cu_num_logits_np[1:])
        max_num_logits_per_req = int(num_logits_per_req_np.max()) if num_reqs else 1

        # Shard the input batch GPU tensors.
        sorted_logits_indices = torch.empty(num_logits, dtype=torch.int64, device=self.device)
        gathered_src_indices = torch.empty(num_reqs, dtype=torch.int64, device=self.device)
        local_logits_indices = torch.empty(num_local_logits, dtype=torch.int64, device=self.device)
        local_idx_mapping = torch.empty(num_local_reqs, dtype=torch.int32, device=self.device)
        local_cu_num_logits = torch.empty(num_local_reqs + 1, dtype=torch.int32, device=self.device)
        local_expanded_idx_mapping = torch.empty(num_local_logits, dtype=torch.int32, device=self.device)
        local_expanded_local_pos = torch.empty(num_local_logits, dtype=torch.int32, device=self.device)
        local_seq_lens = torch.empty(num_local_reqs, dtype=torch.int32, device=self.device)
        if num_reqs > 0:
        ...
        if input_batch.num_draft_tokens_per_req is not None:
            num_draft_tokens_per_req = input_batch.num_draft_tokens_per_req[local_req_indices_np]
        ...
        if grammar_output is not None:
            local_grammar_output = _shard_grammar_output(grammar_output, input_batch, local_req_ids)
...
def all_to_all_logits(
    ...
    recv = logits_shard.new_empty(metadata.tp_size * metadata.num_local_logits, logits_shard.shape[-1])
...
@triton.jit(do_not_specialize=["max_num_logits_per_req", "num_src_cols", "packed_stride"])
def _pack_sampler_output_kernel(
    ...
    if num_nans_ptr is not None:
        ...
        tl.store(row_ptr + max_num_logits_per_req + 2, tl.sum(nans.to(tl.int64), axis=0))
...
def _unpack_logprobs_kernel(
    ...
    if pos < tl.load(cu_num_logits_ptr + req_idx + 1) - start:
        ...
        logprobs = tl.load(gathered_logprobs_ptr + src * gathered_logprobs_stride + cols, mask=mask)
...
def _gather_logprobs_tensors(
    ...
    send_ids_ranks = torch.empty(num_send_rows, num_logprob_cols + 1, dtype=torch.int64, device=device)
    send_logprobs = torch.empty(num_send_rows, num_logprob_cols, dtype=torch.float32, device=device)
    ...
    logprob_token_ids = torch.empty(num_logits, num_logprob_cols, dtype=torch.int64, device=device)
    logprobs = torch.empty(num_logits, num_logprob_cols, dtype=torch.float32, device=device)
    ...
    return LogprobsTensors(
        ...
        cu_num_generated_tokens=(global_batch.cu_num_logits_np.tolist() if num_logits != num_reqs else None),
    )
...
def gather_sampler_output(
    ...
    if local_output is not None:
        ...
        num_src_cols = min(local_output.sampled_token_ids.shape[1], max_num_logits_per_req)
    ...
    sampled_token_ids = torch.empty(num_reqs, max_num_logits_per_req, dtype=torch.int64, device=device)
    ...
    num_nans = torch.empty(num_reqs, dtype=torch.int32, device=device) if gather_num_nans else None
