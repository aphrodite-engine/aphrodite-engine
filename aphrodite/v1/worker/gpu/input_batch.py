...
def set_dummy_context(
    ...
    for block_table, block_size, bpk in zip(
        ...
        block_ids = torch.arange(num_reqs * num_blocks, dtype=block_table.dtype, device=block_table.device) % (
            num_kv_blocks * bpk
        )
...
def _prepare_prefill_inputs_kernel(
    ...
    tokens = tl.load(request_ptr + pos, mask=in_lookahead & (pos < prefill_len), other=0)
    tl.store(
        next_prefill_tokens_ptr + lookahead * next_prefill_tokens_stride + req_state_idx,
        tokens,
        mask=in_lookahead,
    )
...
def combine_sampled_and_draft_tokens(
    ...
    assert num_new_sampled_tokens in (0, 1), f"num_new_sampled_tokens must be 0 or 1, got {num_new_sampled_tokens}"
    ...
    _combine_sampled_and_draft_tokens_kernel[(num_reqs,)](
        ...
        BLOCK_SIZE=triton.next_power_of_2(num_speculative_steps + num_new_sampled_tokens),
    )
