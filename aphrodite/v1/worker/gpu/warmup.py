...
def _warmup_block_counter(
    ...
    num_lookahead_tokens = model_runner.aphrodite_config.num_lookahead_tokens
...
def run_mixed_prefill_decode_warmup(
    ...
    *,
    ...
    decode_prefill_block_counts = [block_count(decode_prompt_len, s) for s in kv_cache_specs]
    decode_block_counts = [block_count(decode_prompt_len + decode_scheduled_tokens, s) for s in kv_cache_specs]
    decode_block_deltas = [
...
def warmup_kernels(
    ...
    coordination.
    """
    if model_runner.aphrodite_config.is_mm_encoder_only:
        return
    ...
    if model_runner.is_pooling_model:
        ...
        pooling_task = model_runner.model_config.get_pooling_task(model_runner.get_supported_tasks())
    ...
    if not model_runner.is_pooling_model:
        ...
        def _run_decode_step(indices: list[int], spec_flags: list[bool]) -> None:
            ...
            for i, use_spec in zip(indices, spec_flags):
                ...
                deltas = [block_count(after, spec) - held for spec, held in zip(kv_cache_specs, req_blocks[i])]
                cached_req_data.new_block_ids.append(tuple(_alloc_blocks(n) for n in deltas) if any(deltas) else None)
                req_blocks[i] = [held + delta for held, delta in zip(req_blocks[i], deltas)]
            ...
            decode_output.total_num_scheduled_tokens = sum(step_num_scheduled_tokens.values())
