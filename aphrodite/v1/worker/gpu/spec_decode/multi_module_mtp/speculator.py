...
from aphrodite.v1.worker.gpu.spec_decode.autoregressive.cudagraph_utils import (
...
class MultiModuleMTPSpeculator(DraftModelSpeculator):
    def __init__(self, aphrodite_config: AphroditeConfig, device: torch.device):
        ...
        # target is.
        if not self.supports_mm_inputs and self.draft_model_config.hf_config.model_type == "inkling_mtp":
            self.supports_mm_inputs = MULTIMODAL_REGISTRY.supports_multimodal_inputs(aphrodite_config.model_config)
    ...
    def load_draft_model(
        ...
        return load_eagle_model(target_model, self.aphrodite_config)
    ...
    def init_cudagraph_manager(self, cudagraph_mode: CUDAGraphMode) -> None:
        ...
        if cudagraph_mode.has_piecewise_cudagraphs():
            cudagraph_mode = (
                CUDAGraphMode.FULL_DECODE_ONLY if cudagraph_mode.has_full_cudagraphs() else CUDAGraphMode.NONE
            )
    ...
    def propose(
        ...
        self.input_buffers.query_start_loc[: num_reqs + 1].copy_(input_batch.query_start_loc[: num_reqs + 1])
    ...
    def _run_model(
        ...
        with set_forward_context(
            ...
            model_inputs = dict(
                ...
                inputs_embeds=(self.inputs_embeds[:num_tokens] if self.inputs_embeds is not None else None),
            ...
            if cudagraph_runtime_mode == CUDAGraphMode.PIECEWISE:
                ...
                # run_pw_graph.
                assert self.cudagraph_manager is not None
                ret_hidden_states = self.cudagraph_manager.run_pw_graph(self.model, model_inputs)
            else:
    ...
    def _generate_drafts(
        ...
        for step in range(self.num_speculative_steps):
            ...
            if step < self.num_speculative_steps - 1:
                ...
                draft_embeds = self.model.embed_input_ids(next_input_tokens) if self.inputs_embeds is not None else None
                update_draft_inputs(
                    next_input_tokens,
                    draft_embeds,
                    self.input_buffers,
...
def _prepare_input_buffers_kernel(
    ...
    for i in range(1, num_speculative_steps):
        future_token = tl.load(next_prefill_tokens_ptr + i * next_prefill_tokens_stride0 + req_state_idx)
        override = tl.where(num_sampled > 0, -1, future_token).to(tl.int64)
        tl.store(
            draft_input_id_overrides_ptr + req_idx * draft_input_id_overrides_stride0 + i - 1,
            override,
        )
    ...
    for i in range(num_reprefill_tokens):
        ...
        cached_token_id = tl.load(
            cached_draft_input_ids_ptr + req_state_idx * cached_draft_input_ids_stride0 + cache_read_slot
        )
...
def _prepare_input_hidden_states_and_embeddings_kernel(
    ...
    tl.store(
        draft_input_hidden_states_ptr
        + (query_start + num_reprefill_hidden_states + query_block)[:, None] * draft_input_hidden_states_stride0
        + dim_block[None, :],
        hidden_state,
        mask=query_mask,
    )
    ...
    if query_block_idx == 0:
        ...
        for i in range(num_reprefill_hidden_states):
            cache_read_slot = num_speculative_steps - 1 - num_reprefill_hidden_states + i
            ...
            tl.store(
                draft_input_hidden_states_ptr + (query_start + i) * draft_input_hidden_states_stride0 + dim_block,
                cached_hidden_state,
                mask=dim_mask,
            )
            if USE_INPUT_EMBEDS:
                ...
                tl.store(
                    input_embeds_ptr + (query_start + i) * input_embeds_stride0 + dim_block,
                    cached_embed,
                    mask=dim_mask,
                )
...
def prepare_input_hidden_states_and_embeddings(
    ...
    _prepare_input_hidden_states_and_embeddings_kernel[grid](
        ...
        cached_target_hidden_states.stride(0) if cached_target_hidden_states is not None else 0,
        cached_target_hidden_states.stride(1) if cached_target_hidden_states is not None else 0,
        input_embeds,
        input_embeds.stride(0) if input_embeds is not None else 0,
        cached_draft_input_embeds,
        cached_draft_input_embeds.stride(0) if cached_draft_input_embeds is not None else 0,
        cached_draft_input_embeds.stride(1) if cached_draft_input_embeds is not None else 0,
        input_batch.idx_mapping,
        num_rejected,
        input_buffers.query_start_loc,
        num_speculative_steps,
...
def _cache_inputs_kernel(
    ...
    for i in range(max(window_start, query_start), last_token_index + 1):
        ...
        if block_idx == 0:
            ...
            tl.store(
                cached_draft_input_ids_ptr + req_state_idx * cached_draft_input_ids_stride0 + cache_write_slot,
                input_id,
            )
        if USE_INPUT_EMBEDS:
        ...
        hidden_state = tl.load(
            draft_input_hidden_states_ptr + i * draft_input_hidden_states_stride0 + block,
            mask=mask,
        )
        tl.store(
            cached_target_hidden_states_ptr
...
def _shift_input_embeds_kernel(
    ...
    for i in range(1, query_len, BLOCK_SIZE_Q):
        ...
        input_embed = tl.load(
            input_embeds_ptr + (query_start + query_block)[:, None] * input_embeds_stride0 + dim_block[None, :],
            mask=mask,
        )
        tl.store(
            input_embeds_ptr + (query_start + query_block - 1)[:, None] * input_embeds_stride0 + dim_block[None, :],
            input_embed,
            mask=mask,
        )
    draft_embed = tl.load(
...
def update_draft_inputs(
    ...
    if input_embeds is not None:
        ...
        hidden_block_size = 256
        _shift_input_embeds_kernel[(num_reqs, triton.cdiv(hidden_size, hidden_block_size))](
            input_embeds,
