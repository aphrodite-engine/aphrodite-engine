...
from aphrodite.model_executor.models.interfaces import SupportsEagle3, supports_eagle3
...
def verify_supports_aux_hidden_states_over_pp(model: nn.Module, method: str) -> None:
    ...
    if not getattr(inner, "supports_aux_hidden_states_over_pp", False):
        raise ValueError(f"{type(model).__name__} does not support {method} with pipeline parallelism")
...
def aux_hidden_state_relay_keys(model: nn.Module) -> tuple[str, ...]:
    ...
    return tuple(f"{inner.AUX_HIDDEN_STATE_KEY}{i}" for i in range(inner._aux_slot_base_cached))
...
def reserve_aux_intermediate_tensor_slots(model: nn.Module) -> None:
    ...
    def make_empty_with_aux(batch_size, dtype, device):
        ...
        for i in range(num_aux_states):
            tensors[f"{key}{i}"] = torch.zeros((batch_size, hidden_size), dtype=dtype, device=device)
        return tensors
