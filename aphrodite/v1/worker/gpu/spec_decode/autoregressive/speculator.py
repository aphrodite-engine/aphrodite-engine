...
from aphrodite.v1.worker.utils import AttentionGroup, get_uniform_decode_token_count
...
class AutoRegressiveSpeculator(DraftModelSpeculator):
    def __init__(self, aphrodite_config: AphroditeConfig, device: torch.device):
        ...
        self.sample_src_positions = torch.zeros(self.max_num_reqs, dtype=torch.int64, device=device)
    ...
    def capture(self) -> None:
        ...
        decode_fn = self._generate_fused_drafts if self.use_fused_multi_step_decode else self._generate_draft
    ...
    def propose(
        ...
        num_tokens_across_dp = prefill_batch_sync.num_tokens_across_dp if prefill_batch_sync is not None else None
        ...
        num_tokens_across_dp = decode_batch_sync.num_tokens_across_dp if decode_batch_sync is not None else None

        self.on_multi_step_decode_begin(num_reqs)
        # Generate the remaining num_speculative_steps - 1 draft tokens.
        decode_fn = self._fused_multi_step_decode if self.use_fused_multi_step_decode else self._multi_step_decode
        decode_fn(
            num_reqs,
    ...
    def _generate_fused_drafts(
        ...
        positions = self.input_buffers.positions[:num_reqs]
        query_start_loc = self.input_buffers.query_start_loc[: num_reqs + 1]
        attn_groups = [group for groups in self.attn_groups for group in groups] if attn_metadata is not None else []
        ...
        for step in range(1, self.num_speculative_steps):
            ...
            if step < self.num_speculative_steps - 1 and attn_metadata is not None and self.advance_draft_positions:
