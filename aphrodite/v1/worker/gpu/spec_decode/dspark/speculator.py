...
from aphrodite.v1.worker.gpu.spec_decode.dspark.utils import load_dspark_model
...
class DSparkSpeculator(DFlashSpeculator):
    ...
    def __init__(self, aphrodite_config: AphroditeConfig, device: torch.device):
        ...
        self._draft_topk: int | None = getattr(self.draft_model_config.hf_config, "dspark_draft_topk", None)

        self.draft_token_confidence_probs = torch.empty_like(self.draft_tokens, dtype=torch.float32)
        self.enable_adaptive_verification = self.speculative_config.enable_adaptive_verification
    ...
    def load_draft_model(
        ...
        model = load_dspark_model(target_model, self.aphrodite_config)
