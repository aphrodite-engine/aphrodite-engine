...
from aphrodite.v1.worker.gpu.async_utils import StepTimingSample, stream
...
if TYPE_CHECKING:
    ...
    from aphrodite.v1.worker.gpu.attn_utils import AttentionCGSupportInfo
...
_assign_draft_token_budget_compiled = torch.compile(_assign_draft_token_budget, dynamic=True)
...
class AdaptiveVerificationManager:
        ...
        for num_tokens in capture_sizes + sorted(tail_sizes):
            for _ in range(_PROFILE_REPLAYS):
                yield {
                    ...
                    "context_len": envs.APHRODITE_ADAPTIVE_VERIFICATION_PROFILE_CONTEXT_LEN,
                }
    ...
    def set_initial_cost_curves(self, samples: list[StepTimingSample]) -> None:
        ...
        draft_curve = median_curve((s.num_reqs, s.drafter_ms) for s in samples if s.full_cudagraph)
    ...
    def set_cost_curves(
        ...
        draft_curve, verify_curve = get_tp_group().broadcast_object((draft_curve, verify_curve), src=0)
    ...
    def get_num_tokens(
        ...
        scheduled_tokens = np.fromiter(num_tokens_per_req.values(), dtype=np.int32, count=num_reqs)
        ...
        num_drafts_per_req = {
            req_id: int(num_drafts) for req_id, num_drafts in zip(req_ids, scheduled_drafts, strict=True)
        }
        num_non_draft_tokens_per_req = {
            req_id: int(num_tokens) for req_id, num_tokens in zip(req_ids, num_non_draft_tokens, strict=True)
        }
    ...
    def compact_batch(
        ...
        if draft_budget == 0:
            ...
            cu_num_logits_np = np.arange(num_reqs + 1, dtype=cu_num_logits_np.dtype) * self.num_bonus_tokens
    ...
    *,
    ...
    if target_layer_names is not None:
        ...
        if additional_attn_cg_support is not None:
            target_attn_cg_support = target_attn_cg_support.narrow(*additional_attn_cg_support)
