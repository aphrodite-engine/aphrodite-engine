...
def _build_grammar_mapping(
    ...
    for grammar_req_id in grammar_req_ids:
        ...
        if num_draft_tokens_per_req is None:
            num_positions = int(cu_num_logits_np[req_idx + 1] - cu_num_logits_np[req_idx])
        else:
        ...
        mapping.extend(req_idx * mask_stride + position for position in range(num_positions))
    return mapping
...
class StructuredOutputsWorker:
    def __init__(
        ...
        self.grammar_bitmask = torch.zeros((max_num_logits, cdiv(vocab_size, 32)), dtype=torch.int32, device=device)
        self.device = device
    ...
    def apply_grammar_bitmask(
        ...
        with torch.cuda.stream(self.copy_stream):
            bitmask = async_copy_to_gpu(grammar_bitmask, out=self.grammar_bitmask[: grammar_bitmask.shape[0]])
        ...
        with torch.cuda.stream(self.copy_stream):
            logits_indices = torch.tensor(mapping, dtype=torch.int32, device="cpu", pin_memory=PIN_MEMORY)
            logits_indices = self.logits_indices[: len(mapping)].copy_(logits_indices, non_blocking=True)
