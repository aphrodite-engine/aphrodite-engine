...
class BlockTables:
    ...
    def init_block_table_layout_tensors(self) -> None:
        ...
        self.kernel_block_sizes_tensor = torch.tensor(self.kernel_block_sizes, dtype=torch.int32, device=self.device)
        self.slot_mapping_enabled = torch.tensor(self._slot_mapping_enabled, dtype=torch.bool, device=self.device)
...
def _compute_slot_mappings_kernel(
    ...
    for i in range(start_idx, end_idx, TRITON_BLOCK_SIZE):
        ...
        block_indices = tl.where(mapping_enabled, local_positions // kernel_block_size, 0)
