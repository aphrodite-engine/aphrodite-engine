...
from aphrodite.v1.worker.ubatch_utils import check_ubatch_thresholds, get_num_ubatches
...
def sync_cudagraph_and_dp_padding(
    ...
    if torch.all(allow_ubatching_across_dp == 1):
        ...
        if check_ubatch_thresholds(
            ...
            return BatchExecutionDescriptor(
                ...
                num_tokens_across_dp=torch.full_like(num_tokens_across_dp, ubatch_num_tokens),
    ...
    return synced_desc, DPSyncState(
        ...
        num_reqs=(
            ...
            if synced_desc.cg_mode == CUDAGraphMode.FULL and synced_desc.num_reqs is not None
...
def dispatch_cg_and_sync_dp(
    ...
    if dp_sync is not None:
        assert dp_sync.num_tokens_across_dp[dp_rank] == num_tokens, "reusing a DP sync taken over a different batch"
        assert dp_sync.uniform_token_count is None or uniform_token_count == dp_sync.uniform_token_count, (
        ...
        if not dp_sync.eager and batch_desc.num_tokens != num_tokens:
            ...
            dp_sync = replace(
                dp_sync,
                num_tokens_across_dp=torch.full_like(dp_sync.num_tokens_across_dp, batch_desc.num_tokens),
            )
