...
from aphrodite.distributed.parallel_state import get_dcp_group, get_pcp_group
...
class PCPManager:
    ...
    def __init__(
        ...
        if block_tables is not None and max_num_local_reqs is not None:
            self._local_block_tables = tuple(
                table.new_zeros((max_num_local_reqs, table.shape[1])) for table in block_tables.input_block_tables
            )
        ...
        num_kv_cache_groups = block_tables.num_kv_cache_groups if block_tables is not None else 0
    ...
    def _reorder_segments(
        ...
        def is_pure_prefill(segment: RankSegment) -> bool:
            ...
            start_pos = num_computed_tokens[req_idx] + segment.global_batch_slice.start - query_start_loc_np[req_idx]
        ...
        for index, segment in enumerate(segments):
            segments[index] = replace(
                segment,
                rank_local_batch_slice=slice(rank_offset, rank_offset + segment.num_tokens),
            )
    ...
    def get_num_tokens_for_dispatch(
        ...
        return max(
            sum(chunk_len for _, _, chunk_len in self._iter_rank_chunks(rank, num_scheduled_tokens, is_prefilling))
            for rank in range(self.pcp_world_size)
        )
    ...
    def partition_batch(
        ...
        num_local_tokens_padded = max(per_rank_num_tokens) if padded_num_tokens is None else padded_num_tokens
        ...
        if total_num_logits > 0:
            ...
            cu_num_logits = torch.arange(num_local_reqs + 1, device=self.device, dtype=torch.int32)
        else:
            cu_num_logits_np = np.zeros(num_local_reqs + 1, dtype=np.int32)
            cu_num_logits = torch.zeros(num_local_reqs + 1, device=self.device, dtype=torch.int32)
        ...
        local_num_computed_prefill_tokens_np = np.minimum(local_start_pos_np, local_prefill_len_np)
        ...
        return replace(
            ...
            expanded_local_pos=torch.zeros(num_local_reqs, dtype=torch.int32, device=self.device),
    ...
    def _convert_to_gathered_slot_mappings(
        ...
        gathered_kv_slot_mappings = self._gathered_kv_slot_mappings[:, :num_expanded_tokens]
        torch.index_select(
...
def maybe_build_pcp_manager(
    ...
    cls.validate_config(aphrodite_config, supports_mm_inputs)
