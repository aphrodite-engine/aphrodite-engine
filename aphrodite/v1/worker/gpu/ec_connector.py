...
class ECConnector:
    ...
    @contextmanager
    def maybe_get_output(self, scheduler_output: "SchedulerOutput") -> Generator[ECConnectorOutput | None, None, None]:
        yield None
        ...
        try:
            ...
            if cached_hashes is not None:
                for mm_hash in self.encoder_cache.keys() - cached_hashes:
                    ec_connector.save_caches(encoder_cache=self.encoder_cache, mm_hash=mm_hash)
        finally:
...
def get_ec_connector(
    ...
    if not has_ec_transfer() or aphrodite_config.model_config.is_encoder_decoder or encoder_cache is None:
        return NO_OP_EC_CONNECTOR

    return ActiveECConnector(aphrodite_config, encoder_cache.encoder_outputs)
