...
class PPHandler:
    ...
    def relay_aux_hidden_states(
        ...
        return IntermediateTensors(
            ...
            | {key: intermediate_tensors[key] for key in self.aux_hidden_state_relay_keys}
        )
    ...
    def get_prev_sampled_outputs(
        ...
        if slot.draft_tokens is not None and draft_tokens_to_update is not None:
            ...
            if exclude_mask.any():
                ...
                draft_idx_mapping = async_copy_to_gpu(slot.idx_mapping_np[keep], device=self.device)
    ...
    def broadcast_drafts(self, draft_tokens: torch.Tensor, input_batch: InputBatch) -> None:
    ...
    def receive(self, input_batch: InputBatch) -> bool:
        ...
        with torch.cuda.stream(self.broadcast_stream):
            ...
            if self.num_speculative_steps > 0:
                ...
                torch.distributed.broadcast(draft_tokens, src=self.last_rank, group=self.broadcast_group)
