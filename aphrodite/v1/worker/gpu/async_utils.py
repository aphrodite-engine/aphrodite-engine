...
from aphrodite.v1.worker.utils import raise_if_nan_logits
    ...
    def __init__(
        ...
        with stream(copy_stream, main_stream):
            ...
            if isinstance(self.pooler_output, torch.Tensor) and all(finished_mask):
                self.pooler_output_cpu: PoolerOutput = self.pooler_output.to("cpu", non_blocking=True)
            else:
                outputs = (
                    self.pooler_output.unbind() if isinstance(self.pooler_output, torch.Tensor) else self.pooler_output
                )
                self.pooler_output_cpu = [
                    None if output is None or not is_finished else output.to("cpu", non_blocking=True)
