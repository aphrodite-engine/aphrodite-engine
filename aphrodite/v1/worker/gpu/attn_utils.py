...
from aphrodite.v1.worker.gpu.model_states.interface import ModelSpecificAttnMetadata
from aphrodite.v1.worker.ubatch_utils import get_num_ubatches
...
class AttentionCGSupportInfo:
    ...
    def narrow(self, support: AttentionCGSupport, backend: str | None) -> "AttentionCGSupportInfo":
...
def get_kv_cache_spec(aphrodite_config: AphroditeConfig) -> dict[str, KVCacheSpec]:
    kv_cache_spec: dict[str, KVCacheSpec] = {}
    layer_type = cast(type[Any], AttentionLayerBase)
    attn_layers = get_layers_from_aphrodite_config(aphrodite_config, layer_type)
    for layer_name, attn_module in attn_layers.items():
        ...
        if spec := attn_module.get_kv_cache_spec(aphrodite_config):
...
def get_shared_kv_cache_layers(aphrodite_config: AphroditeConfig):
    attn_layers = get_layers_from_aphrodite_config(aphrodite_config, Attention)
    return {
...
def init_attn_backend(
    ...
    add_kv_sharing_layers_to_kv_cache_groups(
        get_shared_kv_cache_layers(aphrodite_config), kv_cache_config.kv_cache_groups
    )
    ...
    for kv_cache_group_id, groups in enumerate(attn_groups):
        ...
        for group in groups:
            group.create_metadata_builders(
                ...
                num_metadata_builders=get_num_ubatches(aphrodite_config.parallel_config),
            )
    ...
    attn_cg_support_info = get_attn_cg_support(attn_groups, aphrodite_config)
...
def get_attn_cg_support(
    ...
    for groups in attn_groups:
        for group in groups:
            if checked_layer_names is not None and checked_layer_names.isdisjoint(group.layer_names):
                continue
...
def init_kv_cache(
    ...
    with allocation_context:
        kv_caches = allocate_kv_cache(
            kv_cache_config,
            device,
            aphrodite_config.cache_config.get_resolved_kv_cache_layout(),
            kernel_block_sizes,
        )
    for layer_name, target in get_shared_kv_cache_layers(aphrodite_config).items():
    ...
    num_attn_module = (
        2 if aphrodite_config.model_config.hf_config.model_type in ("longcat_flash", "longcat_flash_ngram") else 1
    )
    bind_kv_cache(
        kv_caches,
        forward_context,
...
def compute_mm_prefix_ranges(
    ...
    clamp_sliding_window_in_kernel: bool = False,
) -> dict[int, list[tuple[int, int]]]:
    """Compute PrefixLM bidirectional ranges for multimodal tokens.

    Ranges exceeding sliding_window are skipped to prevent early tokens from
    attending across the entire image span unless the model clamps the prefix
    range to the sliding window in-kernel.
    """
    ...
    for req_idx, req_id in enumerate(req_ids):
        ...
        for mm_feature in mm_features.get(req_id, ()):
            ...
            for r in mm_feature.mm_position.extract_embeds_range():
                if (
                    not clamp_sliding_window_in_kernel
