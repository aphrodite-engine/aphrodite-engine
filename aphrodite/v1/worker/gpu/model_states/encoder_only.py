...
class EncoderOnlyModelState(DefaultModelState):
    ...
    def __init__(
        ...
        for group in self.encoder_attn_groups:
            group.create_metadata_builders(aphrodite_config, device)
        ...
        self._dummy_block_table = torch.zeros(self.max_num_reqs, 1, dtype=torch.int32, device=device)
        self._dummy_slot_mapping = torch.zeros(self.max_num_tokens, dtype=torch.int64, device=device)
    ...
    def add_request(self, req_index: int, new_req_data: NewRequestData) -> None:
        ...
        if token_type_start is not None:
            ...
            self.token_type_ids[new_req_data.req_id] = (
                torch.arange(len(new_req_data.prompt_token_ids), dtype=torch.int32) >= token_type_start
            ).to(torch.int32)
    ...
    def prepare_inputs(self, input_batch: InputBatch, req_states: RequestState) -> dict[str, torch.Tensor | None]:
        ...
        for i, req_id in enumerate(input_batch.req_ids):
            ...
            offset += num_tokens
        model_inputs["token_type_ids"] = token_type_ids_cpu.to(self.device, non_blocking=True)
        return model_inputs
    ...
    def prepare_attn(
        ...
        attn_metadata.update(self._build_encoder_attn_metadata(input_batch, cudagraph_mode, for_capture))
