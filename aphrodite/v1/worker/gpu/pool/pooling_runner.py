...
from aphrodite.v1.outputs import PoolerOutput
...
class PoolingRunner:
    def __init__(self, model: nn.Module, aphrodite_config: AphroditeConfig):
        self.model = cast(AphroditeModelForPooling, model)
    ...
    def add_request(
        ...
        if task not in self.supported_tasks:
            raise ValueError(f"Unsupported task: {task!r}. Supported tasks: {sorted(self.supported_tasks)}")
        ...
        if pooling_params.requires_token_ids:
            self.prompt_token_ids[req_index] = torch.tensor(prompt_token_ids, dtype=torch.int64)
    ...
    def _get_pooling_metadata(
        ...
        prompt_lens = torch.from_numpy(req_states.prompt_len.np[input_batch.idx_mapping_np].copy())
    ...
    def pool(
        ...
        pooling_metadata = self._get_pooling_metadata(input_batch, req_states, hidden_states.device)
    ...
    def _dummy_pooler_run_task(self, hidden_states: torch.Tensor, task: PoolingTask) -> PoolerOutput:
        ...
        pooling_metadata = PoolingMetadata(
            prompt_lens=prompt_lens,
            prompt_token_ids=prompt_token_ids,
            prompt_token_ids_cpu=None if prompt_token_ids is None else prompt_token_ids.cpu(),
