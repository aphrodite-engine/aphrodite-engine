...
from aphrodite.config.ec_manager_config import EncoderCacheManagerMetadata
...
from aphrodite.lora.layers import BaseLayerWithLoRA, LoRAMapping, LoRAMappingType
...
from aphrodite.model_executor.layers.mamba.mamba_utils import MambaStateCopyFuncsByType
...
from aphrodite.model_executor.warmup.jit_warmup import JitWarmupRegistry
...
from aphrodite.utils.gc_utils import freeze_gc_for_cudagraph_capture
...
from aphrodite.v1.attention.backends.short_conv_attn import ShortConvAttentionMetadataBuilder
...
from aphrodite.v1.worker.block_table import SlotMappingMode
...
from aphrodite.v1.worker.utils import (
    EncoderTimingStats,
...
def nans_to_dict(counts: list[int], req_id_to_index: dict[str, int]) -> dict[str, int]:
    ...
    """
    return {req_id: int(counts[i]) if i < len(counts) else 0 for req_id, i in req_id_to_index.items()}
...
class AsyncGPUModelRunnerOutput(AsyncModelRunnerOutput):
    ...
    def get_output(self) -> ModelRunnerOutput:
        ...
        if self._num_nans_cpu is not None:
            output.num_nans_in_logits = nans_to_dict(self._num_nans_cpu.tolist(), output.req_id_to_index)
            if envs.APHRODITE_RAISE_ON_LOGIT_NANS:
...
class GPUModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin, ECConnectorModelRunnerMixin):
    def __init__(
        ...
        self.jit_warmup_registry = JitWarmupRegistry(aphrodite_config)
        ...
        placeholder_max_num_blocks = cdiv(max(self.max_model_len, self.max_encoder_len), placeholder_block_size)
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> Callable | None:
        ...
        for new_req_data in scheduler_output.scheduled_new_reqs:
            ...
            if self.is_pooling_model:
                ...
                model = cast(AphroditeModelForPooling, self.get_model())
                ...
                if not req_data.new_token_ids:
                    ...
                    if num_new_tokens == 1:
                        ...
                        if req_index is not None:
                            update_dry_state(
                                self.input_batch.persistent_data.setdefault(req_index, {}),
                                [new_token_ids[-1]],
                            )
            ...
            if not is_last_rank:
                ...
                if end_token_index > start_token_index:
                    if new_token_ids:
                        ...
                        self.input_batch.token_ids_cpu[req_index, start_token_index:end_token_index] = tokens_to_append
                    self.input_batch.is_token_ids[req_index, start_token_index:end_token_index] = True
    ...
    def _update_states_after_model_execute(
        ...
        if self.cache_config.mamba_cache_mode == "align":
            ...
            mamba_utils.postprocess_mamba_align_gpu(
                ...
                num_accepted_tokens_cpu_tensor=(self.input_batch.num_accepted_tokens_cpu_tensor),
    ...
    def _update_streaming_request(self, req_id: str, new_req_data: NewRequestData) -> CachedRequestState:
    ...
    def _init_mrope_positions(self, req_state: CachedRequestState):
        ...
        if req_state.prompt_token_ids is not None:
            ...
            raise ValueError("M-RoPE requires either prompt_token_ids or prompt_embeds.")
    ...
    def _init_xdrope_positions(self, req_state: CachedRequestState):
        ...
        assert req_state.prompt_token_ids is not None, "XD-RoPE requires prompt_token_ids to be available."
    ...
    def _prepare_input_ids(
        ...
        prev_common_req_indices_tensor = torch.tensor(prev_indices, dtype=torch.int64, pin_memory=PIN_MEMORY).to(
        ...
        draft_tokens_index_tensor = torch.tensor(spec_flattened_indices, dtype=torch.int64, pin_memory=PIN_MEMORY).to(
            self.device, non_blocking=True
        )
    ...
    def _get_encoder_seq_lens(
        ...
        for req_id in num_scheduled_tokens:
            ...
            encoder_input_tokens = sum(feature.mm_position.length for feature in req_state.mm_features)
    ...
    def _prepare_inputs(
        ...
        self.discard_request_mask.np[:num_reqs] = self.optimistic_seq_lens_cpu[:num_reqs].numpy() < num_tokens_np
        ...
        if self.use_async_spec_decode and self.valid_sampled_token_count_gpu is not None and prev_req_id_to_index:
        ...
        self.positions[:total_num_scheduled_tokens] = (
            self.num_computed_tokens[req_indices_gpu].to(torch.int64) + self.query_pos.gpu[:total_num_scheduled_tokens]
        )
        ...
        if self.use_async_spec_decode and (self.uses_mrope or self.uses_xdrope_dim > 0):
            drift = self.num_computed_tokens[req_indices_gpu].to(torch.int64) - async_tensor_h2d(
        ...
        # Hot-Swap lora model
        if self.lora_config:
            assert np.sum(num_sampled_tokens) <= self.aphrodite_config.scheduler_config.max_num_batched_tokens
    ...
    def _build_attention_metadata(
        ...
        if self.routed_experts_initialized:
            ...
            self.routed_experts_slot_mapping_device[:num_tokens].copy_(slot_mapping_attn[:num_tokens])
        ...
        if self.is_mm_prefix_lm:
            ...
            _clamps_in_kernel = getattr(self.model, "mm_prefix_clamp_sliding_window", False) or getattr(
                hf_text_config, "mm_prefix_clamp_sliding_window", False
            )
            ...
            _span_pad_modulus = getattr(hf_text_config, "mm_prefix_span_leading_pad_modulus", 0)
            for req_id in self.input_batch.req_ids:
                ...
                for mm_feature in req_state.mm_features:
                    ...
                    if _span_pad_modulus:
                        pad = _span_pad_modulus - 1 - pos_info.offset % _span_pad_modulus
                        img_doc_range = [
                            (
                                pos_info.offset + pad,
                    ...
                    for r in img_doc_range:
                        if not _clamps_in_kernel and _bidi_sw is not None and (r[1] - r[0] + 1) > _bidi_sw:
                            continue
        ...
        if self.cache_config.use_replayssm:
            replayssm_decode_base_cpu = self.input_batch.replayssm_decode_base_cpu_tensor[:num_reqs_padded]
        ...
        def _build_attn_group_metadata(
            ...
            if use_spec_decode and isinstance(
                ...
                if (
                    ...
                    extra_attn_metadata_args["prev_last_scheduled_idx"] = self.mamba_prev_last_scheduled_idx.gpu[
                        :num_reqs_padded
                    ]
    ...
    def _compute_cascade_attn_prefix_lens(
        ...
        cascade_attn_prefix_lens: list[list[int]] = [[] for _ in range(num_kv_cache_groups)]
    ...
    def _compute_cascade_attn_prefix_len(
        ...
        use_sliding_window = isinstance(kv_cache_spec, SlidingWindowSpec) or (
            isinstance(kv_cache_spec, FullAttentionSpec) and kv_cache_spec.sliding_window is not None
        )
        use_local_attention = isinstance(kv_cache_spec, ChunkedLocalAttentionSpec) or (
            isinstance(kv_cache_spec, FullAttentionSpec) and kv_cache_spec.attention_chunk_size is not None
        )
    ...
    def _calc_xdrope_positions(self, scheduler_output: "SchedulerOutput"):
        ...
        for index, req_id in enumerate(self.input_batch.req_ids):
            ...
            if prompt_part_len > 0:
                ...
                self.xdrope_positions.cpu[:, dst_start:dst_end] = req.xdrope_positions[:, src_start:src_end]
    ...
    def _calc_spec_decode_metadata(
        ...
        cu_num_draft_tokens = self._get_cumsum_and_arange(num_draft_tokens, self._arange_scratch, cumsum_dtype=np.int32)
        # [0, 0, 0, 5, 5, 9]
        ...
        cu_num_sampled_tokens = async_tensor_h2d(cu_num_sampled_tokens, device=self.device)
        logits_indices = async_tensor_h2d(logits_indices, device=self.device)
        target_logits_indices = async_tensor_h2d(target_logits_indices, device=self.device)
        bonus_logits_indices = async_tensor_h2d(bonus_logits_indices, device=self.device)
    ...
    def _prepare_kv_sharing_fast_prefill(
        ...
        _, batch_desc = self.cudagraph_dispatcher.dispatch(num_logits, invalid_modes={CUDAGraphMode.FULL})
        num_logits_padded = batch_desc.num_tokens
        logits_indices_padded = self.kv_sharing_fast_prefill_logits_indices[:num_logits_padded]
        ...
        for modality, num_items, mm_kwargs_batch in group_and_batch_mm_kwargs(
            ...
            if (
                ...
                for video_idx in range(num_items):
                    ...
                    with self.timed_encoder_operation(should_time, mm_lora_refs, current_item_idx + video_idx, 1):
                        ...
                        micro_batch_outputs = model.embed_multimodal(**micro_batch_mm_inputs)
                ...
                with self.timed_encoder_operation(should_time, mm_lora_refs, current_item_idx, num_items):
                    cudagraph_output = None
                    if self.encoder_cudagraph_manager is not None and self.encoder_cudagraph_manager.supports_modality(
                        modality
                    ):
    ...
    def get_model(self) -> nn.Module:
        ...
        if isinstance(self.model, (CUDAGraphWrapper, UBatchWrapper, BreakableCUDAGraphWrapper)):
    ...
    def sync_and_gather_intermediate_tensors(
        ...
        tp = self.aphrodite_config.parallel_config.tensor_parallel_size
        is_rs = is_residual_scattered_for_sp(self.aphrodite_config, num_tokens)
        ...
        if sync_self:
            ...
            for k, v in intermediate_tensors.items():
                ...
                self.intermediate_tensors[k][:num_tokens].copy_(v[:num_tokens], non_blocking=True)

        return IntermediateTensors({k: v[:num_tokens] for k, v in self.intermediate_tensors.items()})
    ...
    def _pool(
        ...
        raw_pooler_output: PoolerOutput = model.pooler(hidden_states=hidden_states, pooling_metadata=pooling_metadata)
    ...
    def _bookkeeping_sync(
        ...
        if envs.APHRODITE_COMPUTE_NANS_IN_LOGITS:
            if self.use_async_scheduling:
        ...
        num_reqs = self.input_batch.num_reqs
        discard_sampled_tokens_req_indices = np.nonzero(self.discard_request_mask.np[:num_reqs])[0]
    ...
    def _allow_microbatching(self, num_reqs: int, num_scheduled_tokens_np: np.ndarray) -> bool:
        ...
        readers = np.flatnonzero((query_lens > (self.reorder_batch_threshold or 1)) & (computed > 0))
        if readers.size == 0:
            return True
    ...
    def _determine_batch_execution_and_padding(
        ...
        if self.compilation_config.pass_config.enable_sp:
            assert batch_descriptor.num_tokens % self.aphrodite_config.parallel_config.tensor_parallel_size == 0, (
        ...
        if self.aphrodite_config.observability_config.cudagraph_metrics:
    ...
    def _register_layerwise_nvtx_hooks(self) -> None:
        ...
        if (
            self.aphrodite_config.observability_config.enable_layerwise_nvtx_tracing
    ...
    def _get_slot_mappings(
        ...
        slot_mappings_by_gid = {
            gid: _get_slot_mapping(gid) for gid, _ in enumerate(self.kv_cache_config.kv_cache_groups)
        }
    ...
    def execute_model(
        ...
        with (
            ...
            if not num_scheduled_tokens:
                ...
                return self.kv_connector_no_forward(scheduler_output, self.aphrodite_config)
            ...
            (
                ...
                allow_microbatching=self._allow_microbatching(num_reqs, num_scheduled_tokens_np),
            )
        ...
        with record_function_or_nullcontext("gpu_model_runner: postprocess"):
            ...
            if not self.broadcast_pp_output:
                ...
                if not get_pp_group().is_last_rank:
                    all_gather_tensors = {
                        "residual": not is_residual_scattered_for_sp(self.aphrodite_config, num_tokens_padded)
                    }
    ...
    def _input_fits_in_drafter(self, common_attn_metadata: CommonAttentionMetadata | None) -> bool:
        if common_attn_metadata is None:
            return False
        assert self.speculative_config is not None
        # DFlash queries one extra token (the bonus token) beyond num_spec_tokens
        num_drafter_query_tokens = self.num_spec_tokens + (1 if self.speculative_config.use_dflash() else 0)
        return common_attn_metadata.max_seq_len + num_drafter_query_tokens <= self.effective_drafter_max_model_len
    ...
    def sample_tokens(
        ...
        if spec_config is not None:
            ...
            if use_gpu_toks:
                ...
                if input_fits_in_drafter:
                    ...
                    assert spec_decode_common_attn_metadata is not None
                    next_token_ids, valid_sampled_tokens_count, _ = self.drafter.update_token_ids_ngram(
                        sampled_token_ids,
                        self.input_batch,
                        self.token_ids_gpu_tensor,
                        self.num_tokens_no_spec_gpu,
        ...
        if draft_after_bookkeeping:
            ...
            elif drafter_runs_model_forward and self.parallel_config.data_parallel_size > 1:
                # Prevent hang when DP ranks disagree on input_fits_in_drafter
                assert isinstance(
                    self.drafter,
                    EagleProposer | DFlashProposer | DraftModelProposer | ExtractHiddenStatesProposer | Gemma4Proposer,
        ...
        with record_function_or_nullcontext("gpu_model_runner: AsyncGPUModelRunnerOutput"):
            ...
            routed_experts_snapshot = self.get_routed_experts(scheduler_output.total_num_scheduled_tokens)
    ...
    def _copy_draft_token_ids_to_cpu(self, scheduler_output: "SchedulerOutput", zeros_only: bool = False) -> None:
        ...
        with torch.cuda.stream(self.draft_token_ids_copy_stream):
            if not zeros_only:
                ...
                self.draft_token_ids_cpu[:num_reqs, :num_spec_tokens].copy_(draft_token_ids, non_blocking=True)
            else:
    ...
    def _get_draft_token_ids_cpu(self) -> tuple[list[list[int]], list[str]]:
        ...
        return self.draft_token_ids_cpu[: len(req_ids), :num_spec_tokens].tolist(), req_ids
    ...
    def _get_spec_decode_draft_probs(self, spec_decode_metadata: SpecDecodeMetadata) -> torch.Tensor | None:
    ...
    def propose_draft_token_ids(
        ...
        if spec_config.method == "ngram":
            ...
            if sample_hidden_states.shape[0] == len(sampled_token_ids):
                ...
                indices = []
                offset = 0
                assert spec_decode_metadata is not None, "No spec decode metadata for medusa"
            ...
            assert isinstance(sampled_token_ids, torch.Tensor), (
                "sampled_token_ids should be a torch.Tensor for extract_hidden_states method."
            )
            ...
            alt = getattr(self.get_model(), "get_mtp_target_hidden_states", lambda: None)()
            if alt is not None:
                hidden_states = alt
            ...
            if spec_decode_metadata is None:
                ...
                if spec_config.disable_padded_drafter_batch:
                    ...
                    if self.use_aux_hidden_state_outputs:
                        assert aux_hidden_states is not None
                        target_hidden_states = torch.cat([h[:total_num_tokens] for h in aux_hidden_states], dim=-1)
                    else:
    ...
    def update_config(self, overrides: dict[str, Any]) -> None:
        ...
        for config_name, config_overrides in overrides.items():
            if config_name not in allowed_config_names:
                ...
                raise ValueError(f"Config override '{config_name}' is not supported. Supported configs: {allowed}")
    ...
    def load_model(self, load_dummy_weights: bool = False) -> None:
            ...
            with DeviceMemoryProfiler() as m:
                ...
                if hasattr(self, "drafter"):
                    ...
                    if (
                        ...
                        and (drafter_moe_model := get_mixture_of_experts_model(self.drafter.model)) is not None
                    ):
        ...
        if self.aphrodite_config.compilation_config.mode == CompilationMode.STOCK_TORCH_COMPILE:
            from aphrodite.env_override import _apply_constrain_to_fx_strides_patch

            _apply_constrain_to_fx_strides_patch()
            backend = self.aphrodite_config.compilation_config.init_backend(self.aphrodite_config)
            compilation_counter.stock_torch_compile_count += 1
            self.model.compile(fullgraph=True, backend=backend)
            return
        # for other compilation modes, cudagraph behavior is controlled by
        # CudagraphWrapper and CudagraphDispatcher of aphrodite.
        ...
        if (
            ...
            self.model = BreakableCUDAGraphWrapper(self.model, self.aphrodite_config)
            drafter = getattr(self, "drafter", None)
            if drafter is not None and hasattr(drafter, "model"):
                drafter.model = BreakableCUDAGraphWrapper(drafter.model, self.aphrodite_config)
        elif cudagraph_mode.has_full_cudagraphs() and not self.parallel_config.use_ubatching:
            self.model = CUDAGraphWrapper(self.model, self.aphrodite_config, runtime_mode=CUDAGraphMode.FULL)
        elif self.parallel_config.use_ubatching:
            if cudagraph_mode.has_full_cudagraphs():
                self.model = UBatchWrapper(self.model, self.aphrodite_config, CUDAGraphMode.FULL, self.device)
            else:
                self.model = UBatchWrapper(self.model, self.aphrodite_config, CUDAGraphMode.NONE, self.device)
    ...
    def _setup_eagle3_aux_hidden_state_outputs(self) -> None:
        ...
        if not supports_eagle3(self.get_model()):
            raise RuntimeError("Model does not support EAGLE3 interface but aux_hidden_state_outputs was requested")
        # Try to get auxiliary layers from speculative config,
        # otherwise use model's default layers
        aux_layers = self._get_eagle3_aux_layers_from_config()
        if aux_layers:
            logger.info("Using auxiliary layers from speculative config: %s", aux_layers)
        else:
    ...
    def reload_weights(
        ...
        model = self.get_model()
        weights_to_load = {
            name.replace(".base_layer.", ".") if self.lora_config else name for name, _ in model.named_parameters()
        }
        ...
        if weights_iterator is None:
            ...
            if not hasattr(model_loader, "get_all_weights"):
                raise NotImplementedError(f"Model reloading with `{self.load_config.load_format}` format")
    ...
    def _get_prompt_logprobs_dict(
        ...
        for req_id, num_prompt_logprobs in num_prompt_logprobs_dict.items():
            ...
            prompt_token_ids = async_tensor_h2d(request.prompt_token_ids, device=self.device)
    ...
    def maybe_randomize_inputs(
        ...
        """
        Randomize input_ids if APHRODITE_RANDOMIZE_DP_DUMMY_INPUTS is set.
        ...
        randomize_inputs = randomize_inputs or (envs.APHRODITE_RANDOMIZE_DP_DUMMY_INPUTS and dp_size > 1)
        if not randomize_inputs:
            ...
            inputs_embeds.copy_(rand_inputs_embeds()[: inputs_embeds.size(0)], non_blocking=True)
            yield
    ...
    def _dummy_run(
        ...
        ubatch_slices, ubatch_slices_padded = maybe_create_ubatch_slices(
            ...
            num_tokens_padded,
            num_reqs_padded,
            self.aphrodite_config.parallel_config.num_ubatches,
        )
        logger.debug(
            "ubatch_slices: %s, ubatch_slices_padded: %s",
            ubatch_slices,
        ...
        with self.synchronize_input_prep():
            ...
            if force_attention or cudagraph_runtime_mode == CUDAGraphMode.FULL:
                ...
                attn_metadata, _ = self._build_attention_metadata(
                    ...
                    for_cudagraph_capture=(is_graph_capturing or cudagraph_runtime_mode == CUDAGraphMode.FULL),
                    slot_mappings=slot_mappings_by_group,
        ...
        # Note that for DYNAMO_ONCE and APHRODITE_COMPILE mode,
    ...
    def _dummy_sampler_run(
        ...
        dummy_metadata = SamplingMetadata(
            ...
            # Aphrodite extra-sampler fields (required, no dataclass defaults).
            dynatemp_min=None,
            dynatemp_max=None,
            dynatemp_exp=None,
            top_a=None,
    ...
    def _dummy_pooler_run_task(
        ...
        dummy_token_ids = torch.zeros((num_reqs, req_num_tokens), dtype=torch.int32, device=self.device)
    ...
    def _init_minimal_kv_cache_for_profiling(self) -> None:
        ...
        kv_cache_groups = get_kv_cache_groups(self.aphrodite_config, kv_cache_spec)
        # The minimum number of blocks required is 1 block *per sequence*
        min_blocks = min(self.max_num_reqs, self.compilation_config.max_cudagraph_capture_size) or 1

        # Temporarily change num_gpu_blocks_override to allocate a minimal KV cache
        saved_override = self.cache_config.num_gpu_blocks_override
        self.cache_config.num_gpu_blocks_override = min_blocks
        minimal_config = get_kv_cache_config_from_groups(self.aphrodite_config, kv_cache_groups, available_memory=0)
    ...
    def _create_encoder_cudagraph_manager(self) -> "EncoderCudaGraphManager | None":
        ...
        from aphrodite.v1.worker.encoder_cudagraph import (
    ...
    def profile_cudagraph_memory(self) -> int:
        ...
        encoder_graphs = (
            encoder_cudagraph_manager.get_num_graphs_to_capture() if encoder_cudagraph_manager is not None else 0
        )
        ...
        graph_groups = [
            *(f"{mode.name}={len(descs)} (largest={descs[0].num_tokens})" for mode, descs in capture_descs if descs),
        ]
        if encoder_graphs > 0:
            graph_groups.append(f"ENCODER={encoder_graphs} (largest={encoder_cudagraph_manager.token_budgets[-1]})")

        logger.info_once("Profiling CUDA graph memory: %s", ", ".join(graph_groups))
        ...
        all_wrappers = list(CUDAGraphWrapper._all_instances) + list(BreakableCUDAGraphWrapper._all_instances)
        ...
        # On ROCm, capture these throwaway profiling graphs on Aphrodite's
        # dedicated compute stream instead of the fresh side stream
        # graph_capture() allocates by default. torch's allocator pools free
        # blocks per stream, so a side-stream forward strands a persistent aiter
        # scratch buffer in a separate pool, shifting the physical placement of
        # the real KV cache allocated afterward and slowing bandwidth-bound
        # decode ~20%. The graphs are discarded, so a side stream is unnecessary
        # here.
        # Use current_stream(), not torch.cuda.current_stream(): before
        # Aphrodite initializes its dedicated stream, torch returns the
        # per-thread default stream (cuda_stream=0), which cannot be used for
        # cudagraph capture.
        # cap_ctx=None keeps the side-stream path on CUDA.
        cap_ctx = GraphCaptureContext(current_stream()) if current_platform.is_rocm() else None
        ...
        try:
            ...
            with (
                ...
                for mode, descs in capture_descs:
                    ...
                    logger.debug(
                        "Estimated %s CUDA graph memory: %.2f MiB first-capture + (%d-1) × %.2f MiB per-graph",
                        mode.name,
                        first_capture / (1 << 20),
                        len(descs),
                        per_graph / (1 << 20),
        ...
        decoder_estimate = max(shared_memory_estimate.values(), default=0) + sum(per_graph_estimate.values())
    ...
    def _capture_cudagraphs(
        ...
        if show_progress:
            ...
            task = progress.add_task(
                "Capturing CUDA graphs ({}, {})".format(
                    "decode" if uniform_decode else "mixed prefill-decode",
                    cudagraph_runtime_mode.name,
                ),
                total=len(batch_descriptors),
            )
        ...
        for batch_desc in batch_descriptors:
            ...
            allow_microbatching = (
                ...
                and check_ubatch_thresholds(
                    config=self.aphrodite_config.parallel_config,
    ...
    def initialize_attn_backend(
        ...
        def get_attn_backends_for_group(
            ...
            layers = get_layers_from_aphrodite_config(
                self.aphrodite_config, layer_type, kv_cache_group_spec.layer_names
            )
            attn_backends = {}
            ...
            for layer_name in kv_cache_group_spec.layer_names:
                ...
                attn_backends[key] = AttentionGroupKey(attn_backend, layer_kv_cache_spec, num_heads_q)
        ...
        check_attention_cp_compatibility(self.aphrodite_config)

        for i, attn_backend_map in enumerate(attention_backend_maps):
            self.attn_groups.append(create_attn_groups(attn_backend_map, i))

    def initialize_metadata_builders(self, kv_cache_config: KVCacheConfig, kernel_block_sizes: list[int]) -> None:
        """
        ...
        for kv_cache_group_id in range(len(kv_cache_config.kv_cache_groups)):
            for attn_group in self.attn_groups[kv_cache_group_id]:
                attn_group.create_metadata_builders(
                    self.aphrodite_config,
                    self.device,
                    kernel_block_sizes[kv_cache_group_id] if kv_cache_group_id < len(kernel_block_sizes) else None,
    ...
    def _check_and_update_cudagraph_mode(
        ...
        for attn_backend_set, kv_cache_group in zip(attention_backends, kv_cache_groups):
            for attn_backend in attn_backend_set:
                builder_cls = attn_backend.get_builder_cls()

                cg_support = builder_cls.get_cudagraph_support(self.aphrodite_config, kv_cache_group.kv_cache_spec)
    ...
    def calculate_reorder_batch_threshold(self) -> None:
        ...
        reorder_batch_thresholds: list[int | None] = [
            group.get_metadata_builder().reorder_batch_threshold for group in self._attn_group_iterator()
        ]
    ...
    def may_reinitialize_input_batch(self, kv_cache_config: KVCacheConfig, kernel_block_sizes: list[int]) -> None:
        ...
        for kv_cache_group in kv_cache_config.kv_cache_groups:
            ...
            max_num_blocks_per_req = kv_cache_spec.max_num_blocks_per_req(self.aphrodite_config, max_model_len)
        ...
        if (
            ...
            or self.cp_kv_cache_interleave_size != self.parallel_config.cp_kv_cache_interleave_size
        ):
        ...
        assert self._init_block_sizes == block_sizes, (
            f"InputBatch block_sizes {self._init_block_sizes} != kv_cache block_sizes {block_sizes}"
        )
    ...
    def maybe_add_kv_sharing_layers_to_kv_cache_groups(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        if self.cache_config.kv_sharing_fast_prefill:
            ...
            attn_layers = get_layers_from_aphrodite_config(self.aphrodite_config, Attention)
    ...
    def initialize_kv_cache(
        ...
        initialize_mamba_ssu_backend(
            self.aphrodite_config.mamba_config,
            self.kv_cache_config,
            use_replayssm=self.aphrodite_config.cache_config.use_replayssm,
        )
        ...
        if self.speculative_config and self.speculative_config.uses_extract_hidden_states():
            assert isinstance(self.drafter, ExtractHiddenStatesProposer)
    ...
    def get_kv_cache_spec(self) -> dict[str, KVCacheSpec]:
        ...
        layer_type = cast(type[Any], AttentionLayerBase)
        attn_layers = get_layers_from_aphrodite_config(self.aphrodite_config, layer_type)
        for layer_name, attn_module in attn_layers.items():
            ...
            if spec := attn_module.get_kv_cache_spec(self.aphrodite_config):
    ...
    def get_encoder_timing_stats(self) -> dict[str, dict[str, float | int]]:
        ...
        with self._encoder_timing_lock:
            stats = {req_id: stats_obj.to_dict() for req_id, stats_obj in self.encoder_timing_registry.items()}
