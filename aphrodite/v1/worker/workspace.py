...
_workspace_lane: ContextVar[int] = ContextVar("aphrodite_workspace_lane", default=0)
...
class WorkspaceManager:
    ...
    def __init__(
        ...
        self._current_workspaces: list[torch.Tensor | None] = [None] * (self._num_ubatches * self._num_lanes)
    ...
    def _ensure_workspace_size(self, required_bytes: int) -> torch.Tensor:
        ...
        if lane >= self._num_lanes:
            raise RuntimeError(f"Workspace lane {lane} is not configured; manager has {self._num_lanes} lane(s).")
        ...
        if current_size < required_bytes:
            ...
            if envs.APHRODITE_DEBUG_WORKSPACE:
                logger.info(
                    "[WORKSPACE DEBUG] Resized workspace from '%s': %.2f MB -> %.2f MB (ubatch %d, lane %d)",
                    get_caller_info(),
                    current_size / _MB,
                    required_bytes / _MB,
                    ubatch_id,
