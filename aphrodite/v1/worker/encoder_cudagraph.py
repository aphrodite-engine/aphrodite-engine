...
from aphrodite.model_executor.models.vision import get_load_balance_assignment
...
class EncoderCudaGraphManager:
    ...
    def __init__(
        ...
        comp_config = aphrodite_config.compilation_config
        ...
        self.use_dp = (
            ...
            and aphrodite_config.parallel_config.tensor_parallel_size > 1
        )
        ...
        logger.info(
            "EncoderCudaGraphManager initialized with paths=%s, max_batch_size=%d, max_frames_per_batch=%s, use_dp=%s",
    ...
    def get_num_graphs_to_capture(self) -> int:
        return sum(token_budget > 0 for budgets in self.path_token_budgets.values() for token_budget in budgets)
    ...
    def _select_items(self, mm_kwargs: dict[str, Any], indices: list[int]) -> dict[str, Any]:
    ...
    def _execute_local(
        ...
        per_item_path_tokens = {path: [spec.get_path_output_tokens(path) for spec in item_specs] for path in paths}
        ...
        for orig_idx in sorted_indices:
            ...
            fits = len(current_batch) < self.max_batch_size and all(
                current_tokens[path] + item_tokens[path] <= max_path_budgets[path] for path in paths
            )
        ...
        for batch_indices, path_budgets in batches:
            ...
            for path in paths:
                ...
                if token_budget is None:
                    with torch.inference_mode():
                        output = self.model.encoder_eager_forward(batch_mm_kwargs, path=path)
                else:
                    all_eager = False
                    output = self._run_budget_graph(batch_mm_kwargs, token_budget, path=path)
