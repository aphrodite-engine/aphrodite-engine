...
from aphrodite.v1.simple_kv_offload.metadata import (
...
class SimpleCPUOffloadScheduler:
    ...
    def __init__(
        ...
        offload_capacity = disk_capacity_bytes if disk_capacity_bytes > 0 else cpu_capacity_bytes
        self.enable_kv_cache_events = (
            aphrodite_config.kv_events_config is not None and aphrodite_config.kv_events_config.enable_kv_cache_events
        )
        ...
        self.cpu_kv_cache_config = self._derive_cpu_config(kv_cache_config, offload_capacity)
        ...
        self.fa_block_size: int = (
            self.cpu_kv_cache_config.kv_cache_groups[self.fa_gidx].kv_cache_spec.block_size * self.cp_world_size
        )
        assert self.block_size % self.fa_block_size == 0

        logger.info(
            "SimpleCPUOffloadScheduler: Allocating %d offload blocks (%.2f GB, mode=%s, backend=%s)",
            self.num_cpu_blocks,
        ...
        use_eagle_block_drop = spec_config is not None and spec_config.use_eagle_block_drop()
        ...
        self._pending_cpu_hits: dict[str, tuple[tuple[list[KVCacheBlock], ...], int]] = {}
        ...
        self._expected_worker_count = aphrodite_config.parallel_config.world_size
    ...
    def update_state_after_alloc(
        ...
        num_cached_fa_blocks = sum(blk.block_hash is not None for blk in blocks.blocks[self.fa_gidx])
        ...
        for g in range(num_groups):
            g_block_size = kv_cache_groups[g].kv_cache_spec.block_size * self.cp_world_size
            assert num_external_tokens % g_block_size == 0, (
                f"num_external_tokens={num_external_tokens} not aligned to group {g} block_size={g_block_size}"
            )
    ...
    def build_connector_meta(
        ...
        # --- Stores ---
        store_event = -1
        store_gpu, store_cpu, store_req_ids, store_meta = self.prepare_store_specs(scheduler_output)
        ...
        if store_gpu:
            ...
            self._store_event_to_blocks[store_event] = TransferMeta(store_gpu, store_cpu, store_meta)
        ...
        result = SimpleCPUOffloadMetadata(
            ...
            load_event_to_reqs={event_idx: list(req_ids) for event_idx, req_ids in self._load_event_to_reqs.items()},
    ...
    def _prepare_eager_store_specs(
        ...
        for req_id, new_block_id_groups, preempted in yield_req_data(scheduler_output):
            ...
            gpu_block_ids, advanced_per_group, block_meta = self._select_eager_blocks_to_store(
    ...
    def _select_eager_blocks_to_store(
        ...
        for g, group_gpu_ids in enumerate(block_ids_by_group):
            ...
            group_size = self.cpu_kv_cache_config.kv_cache_groups[g].kv_cache_spec.block_size * self.cp_world_size
            ready = min(len(group_gpu_ids), aligned_tokens // group_size)
            resolved_hashes = (
                resolve_block_hashes(request.block_hashes, self.hash_block_size, group_size)
            ...
            for i, gpu_block_id in enumerate(group_gpu_ids[start:ready], start=start):
                ...
                if (
                    ...
                    or self.cpu_block_pool.cached_block_hash_to_block.get_one_block(gpu_block.block_hash) is not None
                ):
                    advanced_per_group[g] += 1
                    continue
                if len(gpu_block_ids) >= num_free:
                ...
                if block_meta is not None:
                    ...
                    parent_hash = None if i == 0 else maybe_convert_block_hash(resolved_hashes[i - 1])
                    ...
                    for hash_idx in range(first_hash_idx, last_hash_idx):
                        block_hash = make_block_hash_with_group_id(request.block_hashes[hash_idx], g)
                        if block_hash == gpu_block.block_hash:
                            continue
                        hash_start = hash_idx * self.hash_block_size
                        hash_end = hash_start + self.hash_block_size
                        secondary_extra_keys, secondary_mm_idx = generate_block_hash_extra_keys(
                            request,
                            hash_start,
                            hash_end,
                            secondary_mm_idx,
                        ...
                        meta_by_hash[block_hash] = BlockStoreMeta(
                            ...
                            parent_block_hash=(
                                None if hash_idx == 0 else maybe_convert_block_hash(request.block_hashes[hash_idx - 1])
                            ),
                            lora_id=lora_req.adapter_id if lora_req else None,
                            lora_name=lora_req.name if lora_req else None,
    ...
    def _process_store_completion(
        ...
        for i, (gpu_block_id, cpu_block) in enumerate(zip(gpu_block_ids, cpu_blocks)):
            ...
            secondary_hashes = self._gpu_block_pool.cached_block_hashes_by_block.get(gpu_block_id, ())
            for block_hash in secondary_hashes:
                self.cpu_block_pool._insert_block_hash(block_hash, cpu_block, num_tokens=None)
            if self.enable_kv_cache_events:
                meta_by_hash = block_meta[i] if block_meta is not None else {}
                primary_group_idx = get_group_id(primary_hash)
                primary_spec = self.cpu_kv_cache_config.kv_cache_groups[primary_group_idx].kv_cache_spec
                primary_block_size = primary_spec.block_size * (
                    self.cp_world_size if not isinstance(primary_spec, MambaSpec) else 1
                )
                events_to_emit: list[tuple[BlockHashWithGroupId, BlockStoreMeta | None, int]] = [
                    (
                        primary_hash,
                ...
                for secondary_hash in secondary_hashes:
                    ...
                    events_to_emit.append(
                        (
                            ...
                            if secondary_meta is not None and secondary_meta.block_size is not None
                            else 0,
                        )
                    )
                ...
                for block_hash, meta, event_block_size in events_to_emit:
                    ...
                    spec = self.cpu_kv_cache_config.kv_cache_groups[group_idx].kv_cache_spec
                    ...
                    self.cpu_block_pool.kv_event_queue.append(
                        BlockStored(
                            ...
                            extra_keys=([extra_keys] if extra_keys is not None else None),
                            group_idx=group_idx,
                            kv_cache_spec_kind=get_kv_cache_spec_kind(spec).value,
                            kv_cache_spec_sliding_window=(get_kv_cache_spec_sliding_window(spec)),
                            locality="LOCAL",
                        )
                    )
        ...
        self._gpu_block_pool.free_blocks(self._gpu_block_pool.blocks[bid] for bid in gpu_block_ids)
    ...
    def _release_transfer_refs(self, transfer: TransferMeta) -> None:
        ...
        self.cpu_block_pool.free_blocks(cpu_blocks)
        assert self._gpu_block_pool is not None
        self._gpu_block_pool.free_blocks(self._gpu_block_pool.blocks[bid] for bid in transfer.gpu_block_ids)

    def has_pending_stores(self) -> bool:
        """Return True if a store transfer is queued or in flight."""
        return bool(
            self._pending_finished_stores or self._store_event_to_blocks or self._abandoned_store_event_to_blocks
        )
    ...
    def _queue_finished_eager_store(self, request: "Request", block_ids: tuple[list[int], ...]) -> None:
        ...
        self._pending_finished_stores.append(TransferMeta(gpu_ids, [b.block_id for b in cpu_blocks], block_meta))
    ...
    def _free_pending_cpu_hit(self, pending: tuple) -> None:
        ...
        blocks_to_free = [blk for grp in cpu_hit_blocks for blk in grp if not blk.is_null]
        if blocks_to_free:
    ...
    def _cleanup_load_request(self, req_id: str) -> None:
        ...
        if state.transfer_meta is not None:
            ...
            self.cpu_block_pool.free_blocks(
                self.cpu_block_pool.blocks[bid] for bid in state.transfer_meta.cpu_block_ids
            )
            # Free GPU touch refs
            assert self._gpu_block_pool is not None
            self._gpu_block_pool.free_blocks(
                self._gpu_block_pool.blocks[bid] for bid in state.transfer_meta.gpu_block_ids
            )
