...
class DiskBackend:
    ...
    def init(
        ...
        self._per_tensor_bpb = [t.stride(0) * t.element_size() for t in gpu_caches.values()]

        assert total_block_bytes % _ALIGNMENT == 0, f"total_block_bytes={total_block_bytes} not aligned to {_ALIGNMENT}"
        ...
        self._store_slot_views = [
            [memoryview(self._store_buffer_caches[name][slot].numpy()) for name in self._tensor_names]
            for slot in range(num_buffer_slots)
        ]
        self._load_slot_views = [
            [memoryview(self._load_buffer_caches[name][slot].numpy()) for name in self._tensor_names]
            for slot in range(num_buffer_slots)
        ]
        ...
        logger.info(
            "DiskBackend: path=%s, slots=%d, total=%.2f GB, buf=%dx%d bytes (page_cache=%s)",
            disk_path,
            num_disk_slots,
    ...
    def shutdown(self) -> None:
        ...
        # the cheaper failure.
        if any(t is not None and t.is_alive() for t in (self._store_thread, self._load_thread)):
            logger.warning(
    ...
    def _writev_slot(self, buf_slot: int, file_offset: int) -> None:
        ...
        if written < self._total_block_bytes:
            raise OSError(f"Short write: expected {self._total_block_bytes} bytes, wrote {written}")
    ...
    def _readv_slot(self, buf_slot: int, file_offset: int) -> None:
        ...
        if bytes_read < self._total_block_bytes:
            raise OSError(f"Short read: expected {self._total_block_bytes} bytes, read {bytes_read}")
