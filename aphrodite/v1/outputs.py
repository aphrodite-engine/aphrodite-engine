...
if TYPE_CHECKING:
    from aphrodite.distributed.ec_transfer.ec_connector.base import ECConnectorWorkerMetadata
    from aphrodite.distributed.kv_events import KVConnectorKVEvents
...
class SamplingMaskLists(NamedTuple):
    ...
    def slice_request(self, req_idx: int, num_positions: int) -> "SamplingMaskLists":
        ...
        return SamplingMaskLists(self.token_ids[self.offsets[req_idx] : self.offsets[req_idx + 1]])
...
class LogprobsTensors(NamedTuple):
    ...
    def filter(self, mask: torch.Tensor) -> "LogprobsTensors":
        ...
        assert self.cu_num_generated_tokens_tensor is None, "filter can't be used with cu_num_generated_tokens_tensor"
        return LogprobsTensors(
    ...
    def cat(
        ...
        assert tensors
        assert cu_num_generated_tokens is not None or all(tensor.cu_num_generated_tokens is None for tensor in tensors)
        if len(tensors) == 1:
            tensor = tensors[0]
            if cu_num_generated_tokens is None:
                return tensor
        ...
        return LogprobsTensors(
            logprob_token_ids=torch.cat([tensor.logprob_token_ids for tensor in tensors]),
            logprobs=torch.cat([tensor.logprobs for tensor in tensors]),
            selected_token_ranks=torch.cat([tensor.selected_token_ranks for tensor in tensors]),
    ...
    def empty_cpu(num_positions: int, num_tokens_per_position: int) -> "LogprobsTensors":
        ...
        selected_token_ranks = logprob_token_ids.new_empty(num_positions, pin_memory=PIN_MEMORY)
        return LogprobsTensors(
...
class ECConnectorOutput:
    ...
    def is_empty(self):
        return not self.finished_sending and not self.finished_recving and not self.ec_connector_worker_meta
...
class ModelRunnerOutput:
    ...
    def with_ec_conn_output_only(
        ...
        return ModelRunnerOutput.with_ec_conn_output(EMPTY_MODEL_RUNNER_OUTPUT, ec_connector_output)
