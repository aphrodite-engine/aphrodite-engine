...
def estimate_max_model_len(aphrodite_config: AphroditeConfig,
    ...
    def fits_in_memory(model_len: int) -> bool:
        ...
        memory_needed = sum(
            (layer_spec.max_memory_usage_bytes(aphrodite_config)
...
def check_enough_kv_cache_memory(aphrodite_config: AphroditeConfig,
    ...
    needed_memory = 0
    for layer_spec in kv_cache_spec.values():
        needed_memory += layer_spec.max_memory_usage_bytes(aphrodite_config)
...
def _get_kv_cache_config_uniform_type(aphrodite_config: AphroditeConfig,
    ...
    # Calculate total KV cache memory in GiB
...
def get_kv_cache_config(aphrodite_config: AphroditeConfig,
                        kv_cache_spec: dict[str, KVCacheSpec],
                        available_memory: int) -> KVCacheConfig:
    """
