...
def resolve_kv_cache_block_sizes(
    ...
    group_block_sizes = [
        g.kv_cache_spec.block_size * dcp if isinstance(g.kv_cache_spec, AttentionSpec) else g.kv_cache_spec.block_size
        for g in groups
    ]
...
def _pool_bytes_per_block(aphrodite_config: AphroditeConfig, kv_cache_groups: list[KVCacheGroupSpec]) -> int:
    """
...
def _use_packed_kv_cache_config(
    ...
    is_dsv4 = all(isinstance(group.kv_cache_spec, UniformTypeKVCacheSpecs) for group in kv_cache_groups)
    kv_transfer_config = aphrodite_config.kv_transfer_config
    extra_config = kv_transfer_config.kv_connector_extra_config if kv_transfer_config is not None else {}
    ...
    enable_cross_layers = str(extra_config.get("enable_cross_layers_blocks", "False")).lower() == "true"
...
def get_kv_cache_config_from_groups(
    ...
    if len(kv_cache_groups) == 1 and isinstance(kv_cache_groups[0].kv_cache_spec, UniformTypeKVCacheSpecs):
        ...
        num_blocks, kv_cache_tensors = _get_kv_cache_config_packed(aphrodite_config, kv_cache_groups, available_memory)
    else:
        # General case:
...
def get_kv_cache_groups(
    aphrodite_config: AphroditeConfig, kv_cache_spec: dict[str, KVCacheSpec]
) -> list[KVCacheGroupSpec]:
    """
