...
def max_memory_usage_bytes(aphrodite_config: AphroditeConfig,
                           kv_cache_specs: Iterable[KVCacheSpec]) -> int:
    """
    Get the maximum memory usage in bytes for the given KV cache specs.
    """
    return sum(
        spec.max_memory_usage_bytes(aphrodite_config) for spec in kv_cache_specs)


def estimate_max_model_len(aphrodite_config: AphroditeConfig,
    ...
    def fits_in_memory(model_len: int) -> bool:
        ...
        memory_needed = max_memory_usage_bytes(aphrodite_config,
...
def check_enough_kv_cache_memory(aphrodite_config: AphroditeConfig,
                                 kv_cache_spec: dict[str, KVCacheSpec],
...
def get_max_concurrency_for_kv_cache_config(
        aphrodite_config: AphroditeConfig, kv_cache_config: KVCacheConfig) -> float:
    """
...
def get_num_blocks(aphrodite_config: AphroditeConfig, num_layers: int,
...
def _get_kv_cache_config_uniform_type(aphrodite_config: AphroditeConfig,
                                      kv_cache_spec: dict[str, KVCacheSpec],
    ...
    num_blocks = get_num_blocks(aphrodite_config, len(kv_cache_spec),
    ...
    if rank == 0:
        logger.info(
            "GPU KV cache size: {} tokens ({:.2f} GiB, {:.1f}x concurrency)",
            num_tokens_str, total_kv_cache_memory_gib, max_concurrency)
...
def _get_kv_cache_config_uniform_page_size(
    ...
    for layers in same_type_layers.values():
        ...
        if num_padding_layers != group_size:
            logger.warning(
                "Add {} padding layers, may waste at most {:.2f}% KV cache memory",  # noqa
                num_padding_layers,
    ...
    page_size = get_uniform_page_size(kv_cache_spec)
    num_blocks = get_num_blocks(aphrodite_config, group_size, available_memory,
                                page_size)
...
def get_kv_cache_config(
    ...
    if is_kv_cache_type_attention_free(kv_cache_spec):
        ...
        # size.
        return _get_kv_cache_config_uniform_page_size(aphrodite_config,
                                                      kv_cache_spec,
                                                      available_memory)
