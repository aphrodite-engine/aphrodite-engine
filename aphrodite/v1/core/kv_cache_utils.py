...
from aphrodite.utils.hashing import xxhash, xxhash_cbor
...
def maybe_convert_block_hash(hash_bytes: BlockHash) -> ExternalBlockHash:
    if not envs.APHRODITE_KV_EVENTS_USE_INT_BLOCK_HASHES:
        return hash_bytes
...
# default seed, so independent Aphrodite processes compute identical block hashes for
...
DEFAULT_NONE_HASH_SEED = "aphrodite-none-hash"
...
def resolve_dcp_kv_block_size(spec: KVCacheSpec, dcp_world_size: int) -> int:
    ...
    if len(layer_specs) > 0 and all(isinstance(layer_spec, AttentionSpec) for layer_spec in layer_specs):
...
def resolve_kv_cache_block_sizes(
    ...
    group_block_sizes = [resolve_dcp_kv_block_size(g.kv_cache_spec, dcp) for g in groups]
    ...
    hashing_sizes = [
        block_size for group, block_size in zip(groups, group_block_sizes) if group.kv_cache_spec.prefix_cacheable
    ...
    prefix_alignments = {
        ...
        for group in groups
        for spec in iter_layer_specs(group.kv_cache_spec)
        if spec.prefix_cacheable and isinstance(spec.tokens_per_state, int) and spec.tokens_per_state > 1
    }
...
def estimate_max_model_len(
    ...
    original_max_model_len = aphrodite_config.model_config.max_model_len
    ...
    try:
        ...
        aphrodite_config.model_config.max_model_len = original_max_model_len
...
def check_enough_kv_cache_memory(
    ...
    if kv_cache_spec:
        ...
        groups = get_kv_cache_groups(aphrodite_config, dict(kv_cache_spec))
        check_memory = available_memory - _pool_bytes_per_block(groups) if groups else available_memory
        _check_enough_kv_cache_memory(
            check_memory,
            lambda: max_memory_usage_bytes(aphrodite_config, kv_cache_spec.values()),
            aphrodite_config.model_config.max_model_len,
            lambda am: estimate_max_model_len(aphrodite_config, kv_cache_spec, am),
        )
...
def _pp_balanced_mamba_group_count(
    ...
    total_layers = aphrodite_config.model_config.get_total_num_hidden_layers()
...
def _get_kv_cache_groups_glm5_next(
    ...
    tail_specs = {name: spec for name, spec in kv_cache_spec.items() if isinstance(spec, KpoolTailSpec)}
    sliding_specs = {name: spec for name, spec in kv_cache_spec.items() if type(spec) is SlidingWindowSpec}
    attn_specs = {
        name: spec
        for name, spec in kv_cache_spec.items()
        if not isinstance(spec, (MambaSpec, KpoolTailSpec)) and name not in sliding_specs
    }
    if not mamba_specs or not all(type(spec) is MLAAttentionSpec for spec in attn_specs.values()):
        return None

    mla_specs = cast(dict[str, MLAAttentionSpec], attn_specs)
    idx_pages = {spec.page_size_bytes for spec in mla_specs.values() if spec.tokens_per_state > 1}
    if not idx_pages:
        return None
    ...
    if tail_specs:
        ...
        padded_tail_specs: dict[str, KVCacheSpec] = {
            name: replace(spec, page_size_padded=idx_page) for name, spec in tail_specs.items()
        }
    ...
    padded_specs: dict[str, KVCacheSpec] = {name: replace(any_mamba, page_size_padded=mla_page) for name in mamba_specs}
    num_groups = _pp_balanced_mamba_group_count(aphrodite_config, list(mamba_specs), mla_names)
    if num_groups is None:
        raise ValueError(
            "a pipeline stage has mamba layers but no MLA layer to share "
            "slots with; realign the stage boundaries (APHRODITE_PP_LAYER_PARTITION)"
        )
    ...
    # Draft groups share the target's slot shape, but use independent block tables.
    sliding_buckets: dict[SlidingWindowSpec, list[str]] = defaultdict(list)
    for name, spec in sliding_specs.items():
        # Align the logical span before padding so tiny draft blocks do not waste the pool.
    ...
    for spec, names in sliding_buckets.items():
        ...
        sliding_groups.extend(
            KVCacheGroupSpec(names[i::count], spec, is_eagle_group=drop_draft_block) for i in range(count)
        )
...
def _glm5_next_tensor_layout(
    ...
    uniform_groups = [group for group in kv_cache_groups if isinstance(group.kv_cache_spec, UniformTypeKVCacheSpecs)]
    slot_groups = [
        group
        for group in kv_cache_groups
        if isinstance(group.kv_cache_spec, MambaSpec) or type(group.kv_cache_spec) is SlidingWindowSpec
    ]
    ...
    for group in uniform_groups:
        ...
        if all(type(spec) is MLAAttentionSpec for spec in inner.values()):
            ...
            tail_group = group
    if attn_group is None or not any(isinstance(group.kv_cache_spec, MambaSpec) for group in slot_groups):
        return None
    ...
    if not all(type(spec) is MLAAttentionSpec and spec.page_size_padded is None for spec in mla_inner.values()):
        return None
    mla_names = [name for name in attn_group.layer_names if mla_inner[name].tokens_per_state == 1]
    idx_names = [name for name in attn_group.layer_names if mla_inner[name].tokens_per_state > 1]
    ...
    if any(
        group.kv_cache_spec.page_size_bytes != mla_page or len(group.layer_names) > len(mla_names)
    ...
    if tail_group is not None:
        ...
        tail_inner = cast(UniformTypeKVCacheSpecs, tail_group.kv_cache_spec).kv_cache_specs
        tail_pages = {cast(KpoolTailSpec, spec).unpadded_page_size_bytes for spec in tail_inner.values()}
...
def unify_kv_cache_spec_page_size(
    ...
    for layer_name, layer_spec in kv_cache_spec.items():
        if layer_spec.page_size_bytes == max_page_size:
            ...
            elif isinstance(layer_spec, AttentionSpec) and not isinstance(layer_spec, MLAAttentionSpec):
                new_spec = replace(layer_spec, page_size_padded=max_page_size)
            else:
...
def _get_kv_cache_bytes_per_block(
    ...
    bytes_per_block = max(
        sum(_get_per_layer_spec(group, layer_name).page_size_bytes for layer_name in group.layer_names)
        for group in kv_cache_groups
    )
...
def validate_kv_cache_layout(
    ...
    if not layout.is_block_compact or (len(kv_cache_groups) > 1 and layout.is_layer_compact):
        raise ValueError(
            ...
            "set APHRODITE_KV_CACHE_LAYOUT=BLHNC."
        )
...
def get_kv_cache_config_from_groups(
    ...
    if len(kv_cache_groups) == 0:
        ...
        return KVCacheConfig(
            ...
            prefix_cache_retention_interval=(aphrodite_config.cache_config.prefix_cache_retention_interval),
        )
    ...
    if (glm5_layout := _glm5_next_tensor_layout(kv_cache_groups)) is not None:
        ...
        num_blocks = may_override_num_blocks(aphrodite_config, available_memory // bytes_per_block)
        ...
        for index, idx_name in enumerate(idx_names):
            ...
            if tail_names:
                ...
                tail_group = next(group for group in kv_cache_groups if tail_name in group.layer_names)
                tail_specs = cast(UniformTypeKVCacheSpecs, tail_group.kv_cache_spec).kv_cache_specs
    ...
    layout = aphrodite_config.cache_config.get_resolved_kv_cache_layout()
    ...
    for group in kv_cache_groups:
        ...
        for spec, layer_names in layers_by_spec.items():
            ...
            offset = byte_offset * max(layer_stride, spec.page_size_bytes) // spec.page_size_bytes
            kv_cache_tensors.append(
                KVCacheTensor(
                    size=size,
                    layers=layer_names,
...
def _promote_local_kv_cache_specs(
    ...
    if is_kv_cache_spec_uniform(promoted_specs) or UniformTypeKVCacheSpecs.is_uniform_type(promoted_specs):
        return promoted_specs

    has_full_attention = any(isinstance(spec, FullAttentionSpec) for spec in promoted_specs.values())
    has_sliding_window = any(isinstance(spec, SlidingWindowSpec) for spec in promoted_specs.values())
    has_chunked_local_attention = any(isinstance(spec, ChunkedLocalAttentionSpec) for spec in promoted_specs.values())
    full_block_sizes = {spec.block_size for spec in promoted_specs.values() if isinstance(spec, FullAttentionSpec)}
    ...
    if has_full_attention and (has_sliding_window or has_chunked_local_attention):
        for layer_name, spec in kv_cache_spec.items():
            target_cls = next((promotions[c] for c in type(spec).__mro__ if c in promotions), None)
    ...
    if not (is_kv_cache_spec_uniform(promoted_specs) or UniformTypeKVCacheSpecs.is_uniform_type(promoted_specs)):
...
def _try_get_full_allocation_fallback_groups(
    ...
    if any(isinstance(spec, HiddenStateCacheSpec) for spec in kv_cache_spec.values()):
        return None
    if any(isinstance(spec, (SlidingWindowMLASpec, ChunkedLocalAttentionSpec)) for spec in kv_cache_spec.values()):
        return None
...
def _get_packed_kv_cache_groups(
    ...
    repeats_per_group = (
        _approximate_gcd(
            [spec.get_max_layers_per_page_size() for spec, _, balanced in bucketed if balanced],
    ...
    def widest_group_bytes(page_size_layers: dict[int, list[str]], n: int) -> int:
        ...
        return sum(cdiv(len(names), n) * page for page, names in page_size_layers.items())
    ...
    anchor_bytes = max(
        (
            widest_group_bytes(
                page_size_layers,
                len(spec.kv_cache_specs) if isinstance(spec.first_spec, MambaSpec) else num_groups_for(spec, balanced),
            )
            for spec, page_size_layers, balanced in bucketed
        ),
        default=0,
    ...
    for spec, page_size_layers, balanced in bucketed:
        ...
        if anchor_bytes and isinstance(spec.first_spec, MambaSpec):
            ...
            num_groups = max(num_groups, cdiv(len(spec.kv_cache_specs), states_per_block))
        if num_groups == 1:
        ...
        for i in range(num_groups):
            group_layer_names = [name for repeat in pattern_repeats[i::num_groups] for name in repeat]
            group_layer_specs = {name: spec.kv_cache_specs[name] for name in group_layer_names}
    ...
    _annotate_eagle_groups(
        ...
        kv_cache_spec,
        groups,
        use_deepseek_v4_fallback=_is_deepseek_v4_eagle(aphrodite_config),
    )
    _warn_if_unannotated_eagle_mamba(aphrodite_config, groups)
    return groups


def _is_deepseek_v4_eagle(aphrodite_config: AphroditeConfig) -> bool:
    spec_config = aphrodite_config.speculative_config
    if spec_config is None or not spec_config.use_eagle():
        return False
    model_config = aphrodite_config.model_config
    return model_config is not None and model_config.hf_config.model_type == "deepseek_v4"
...
def _annotate_eagle_groups(
    ...
    Args:
        aphrodite_config: Config supplying the speculative method, if any.
...
def _warn_if_unannotated_eagle_mamba(
    ...
    mamba_groups = [
        ...
        if any(isinstance(spec, MambaSpec) for spec in iter_layer_specs(group.kv_cache_spec))
    ]
    if not mamba_groups:
        return
    logger.warning(
...
def get_kv_cache_groups(
    ...
    if is_kv_cache_spec_uniform(kv_cache_spec):
        ...
        return _get_kv_cache_groups_uniform_type(uniform_spec)
    elif glm5_groups := _get_kv_cache_groups_glm5_next(aphrodite_config, kv_cache_spec):
        return glm5_groups

    # Hidden-state layers use their own block table and must not be absorbed
    # into a compatible attention bucket.
    hidden_specs = {k: v for k, v in kv_cache_spec.items() if isinstance(v, HiddenStateCacheSpec)}
    filtered_spec = {k: v for k, v in kv_cache_spec.items() if not isinstance(v, HiddenStateCacheSpec)}

    if packed_groups := _get_packed_kv_cache_groups(aphrodite_config, filtered_spec):
        # Block-outermost blocks are strided by the widest group, so hidden
        # groups need no page alignment.
        packed_groups += [KVCacheGroupSpec([name], spec) for name, spec in hidden_specs.items()]
    ...
    _annotate_eagle_groups(aphrodite_config, kv_cache_spec, groups)
    _warn_if_unannotated_eagle_mamba(aphrodite_config, groups)
    return groups
...
def get_kv_cache_capacity(aphrodite_config: AphroditeConfig, kv_cache_config: KVCacheConfig) -> tuple[int, float]:
    """
...
def _max_memory_usage_bytes_from_groups(
    ...
    if (glm5_layout := _glm5_next_tensor_layout(kv_cache_groups)) is not None:
        ...
        total_blocks = uniform_spec.max_memory_usage_pages(aphrodite_config)
        total_blocks += sum(
            cdiv(
                group.kv_cache_spec.max_memory_usage_bytes(aphrodite_config),
                group.kv_cache_spec.page_size_bytes,
    ...
    for group in kv_cache_groups:
        spec = group.kv_cache_spec
        if isinstance(spec, UniformTypeKVCacheSpecs):
            total_blocks += spec.max_memory_usage_pages(aphrodite_config)
        else:
            total_blocks += cdiv(
...
def _estimate_max_model_len_from_groups(
    ...
    """
    original_max = aphrodite_config.model_config.max_model_len
    ...
    try:
        ...
        aphrodite_config.model_config.max_model_len = original_max
...
def _auto_fit_max_model_len(
    ...
    Args:
        aphrodite_config: The global AphroditeConfig (will be modified in-place)
    ...
    for groups, avail_mem in zip(projected_groups_per_worker, available_memory):
        if not groups:
            continue
        worker_max = _estimate_max_model_len_from_groups(aphrodite_config, groups, avail_mem)
    ...
    if auto_fit_max >= original_max:
        ...
        aphrodite_config.model_config.max_model_len = auto_fit_max
        logger.info_once(
...
def get_kv_cache_configs(
    ...
    extra_retained_tokens = (
        aphrodite_config.speculative_config.num_speculative_tokens - 1
        if aphrodite_config.speculative_config is not None
        and aphrodite_config.speculative_config.use_multi_module_mtp()
        else 0
    )
    ...
    global_kv_cache_groups = get_kv_cache_groups(aphrodite_config, merged_kv_cache_specs)
    ...
    if aphrodite_config.model_config.original_max_model_len == -1:
        _auto_fit_max_model_len(aphrodite_config, projected_groups_per_worker, check_memory)
    ...
    for groups, avail_mem in zip(projected_groups_per_worker, check_memory):
        ...
        _check_enough_kv_cache_memory(
            avail_mem,
            partial(_max_memory_usage_bytes_from_groups, aphrodite_config, groups),
            aphrodite_config.model_config.max_model_len,
            partial(_estimate_max_model_len_from_groups, aphrodite_config, groups),
        )
    ...
    for i, kv_cache_config in enumerate(kv_cache_configs):
        ...
        kv_cache_configs[i] = get_kv_cache_config_from_groups(
            aphrodite_config, groups, min_num_blocks * _pool_bytes_per_block(groups)
        )
