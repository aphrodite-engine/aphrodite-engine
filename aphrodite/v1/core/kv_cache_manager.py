...
class KVCacheManager:
    ...
    def get_computed_blocks(self, request: Request) -> tuple[KVCacheBlocks, int, int]:
        ...
        Returns:
            A tuple containing:
                ...
                - A block-aligned shared-prefix boundary that sparse retention
                  should preserve, or 0 if no such boundary was detected.
        """
        ...
        computed_blocks, num_new_computed_tokens, num_uncached = self.coordinator.find_longest_cache_hit(
    ...
    def allocate_slots(
        ...
        Args:
            ...
            num_external_computed_tokens: The number of tokens that their
                KV caches are not cached by Aphrodite but cached by the connector.
        ...
        ----------------------------------------------------------------------
                                  ...
                                  | details below)>          |
        ----------------------------------------------------------------------
        | Prefix-cached tokens from either Aphrodite   |
        | or connector. Can be safely removed if  |
        | they are outside sliding window.        |
        ----------------------------------------------------------------------
        |   < cached by Aphrodite >    | not cached by |
                                  | Aphrodite, but     |
    ...
    def remove_skipped_blocks(
        ...
        """
        self.coordinator.remove_skipped_blocks(request_id, processed_computed_tokens, num_prompt_tokens)
    ...
    def truncate_computed_blocks(self, blocks: KVCacheBlocks, num_computed_tokens: int) -> KVCacheBlocks:
    ...
    def get_zeroing_block_ids_in_range(self, request_id: str, start_token: int, end_token: int) -> list[int]:
    ...
    def take_partial_tail_offloads(self) -> dict[str, list[tuple[int, int, int]]]:
        ...
        for mgr in self.coordinator.single_type_managers:
            for (
                ...
                offloads.setdefault(req_id, []).append((group_id, block.block_id, boundary_tokens))
        return offloads
