...
def _validate_prefix_cache_retention_interval(
    ...
    if not any(isinstance(g.kv_cache_spec, (SlidingWindowSpec, MambaSpec)) for g in kv_cache_config.kv_cache_groups):
...
class KVCacheCoordinator(ABC):
    ...
    def cache_blocks(self, request: Request, num_computed_tokens: int) -> None:
        ...
        for manager in self.single_type_managers:
            ...
            num_tokens_to_cache = max(0, num_computed_tokens - self.num_reprefillable_tokens)
    ...
    def remove_skipped_blocks(
        ...
        """
        for manager in self.single_type_managers:
            manager.remove_skipped_blocks(request_id, processed_computed_tokens, num_prompt_tokens)
    ...
    def find_longest_cache_hit(
        ...
        """Returns per-group hit blocks, hit length, and uncached common prefix.

        The third value is a shared prefix that a sparse-retention group has not
        cached yet; it is 0 unless hybrid cache reconciliation detects one.
        """
        pass
...
class HybridKVCacheCoordinator(KVCacheCoordinator):
    ...
    def __init__(
        ...
        assert all(block_size % hash_block_size == 0 for block_size in group_block_sizes), (
        ...
        has_partial_mamba_group = any(
            ...
            and (
                ...
                or (dcp_world_size > 1 and g.kv_cache_spec.block_size >= hash_block_size)
            )
            for g in kv_cache_config.kv_cache_groups
        )
    ...
    def _cache_hit_alignment_tokens(self) -> int:
        return self.hash_block_size if self.enable_partial_hash_hits else self.scheduler_block_size
    ...
    def verify_and_split_kv_cache_groups(self) -> None:
        ...
        assert self.attention_groups, "HybridKVCacheCoordinator requires at least one cacheable group."
    ...
    def cache_blocks(self, request: Request, num_computed_tokens: int) -> None:
        ...
        for manager in self.single_type_managers:
            ...
            if manager.use_eagle and cached_num_computed_tokens > 0:
                ...
                # multi-module MTP.
                num_finalized_computed_tokens = max(0, num_computed_tokens - self.num_reprefillable_tokens)
                cached_num_finalized_computed_tokens = self._align_cacheable(num_finalized_computed_tokens)
    ...
    def find_longest_cache_hit(
        ...
        while True:
            ...
            for idx, (spec, group_ids, manager_cls, use_eagle) in enumerate(self.attention_groups):
                ...
                hit_blocks, _new_hit_length = manager_cls.find_longest_cache_hit(
                    ...
                    dcp_world_size=self.single_type_managers[first_group_id].dcp_world_size,
                    pcp_world_size=self.single_type_managers[first_group_id].pcp_world_size,
                )
                if drop_eagle_block:
        ...
        # Uncached shared prefix detection: if any attention group cached a
        # longer prefix than the reconciled hit, it is a common prefix that a
