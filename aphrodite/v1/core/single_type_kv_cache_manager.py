...
class MambaManager(SingleTypeKVCacheManager):
        ...
        if self.mamba_cache_mode != "align":
            ...
            if num_required_blocks <= len(req_blocks) and not has_partial_hit:
                ...
                if prev_block_len < num_skipped_blocks:
                    req_blocks.extend([self._null_block for _ in range(prev_block_len, num_skipped_blocks)])
                ...
                if blocks_allocated:
                    ...
                    assert num_new_blocks <= self.num_speculative_blocks + 1 + int(has_partial_hit)
                new_blocks = self.block_pool.get_new_blocks(num_new_blocks)
    ...
    def pop_blocks_for_free(self, request_id: str) -> list[KVCacheBlock]:
        if self.mamba_cache_mode == "align":
            ...
            self._pending_partial_tail_offloads = [
                entry for entry in self._pending_partial_tail_offloads if entry[0] != request_id
            ]
