...
class SingleTypeKVCacheManager(ABC):
    ...
    def __init__(
        ...
        self._pending_boundary_state_offloads: list[tuple[str, int, KVCacheBlock, int]] = []
    ...
    def _has_partial_local_hit(
        ...
        return len(new_computed_blocks) > 0 and num_local_computed_tokens % self.block_size != 0
    ...
    def allocate_external_computed_blocks(
        ...
        req_blocks = self.req_to_blocks[request_id]
        num_new_blocks = max(0, cdiv(num_total_computed_tokens, self.block_size) - len(req_blocks))
    ...
    def reachable_block_mask(
        ...
        length. ``reachable_boundaries`` are token positions whose reachable
        tail must be retained; the base dense policy ignores them.
        """
        return None
...
class FullAttentionManager(SingleTypeKVCacheManager):
    ...
    def find_longest_cache_hit(
        ...
        if fine_grained:
            ...
            full_block_hashes: BlockHashList = BlockHashListWithBlockSize(block_hashes, alignment_tokens, block_size)
        else:
    ...
    def _cache_partial_tail_block(self, request: Request, num_tokens: int) -> None:
        hash_block_size = self.block_pool.hash_block_size
...
class SlidingWindowManager(SingleTypeKVCacheManager):
    ...
    @classmethod
    def _contiguous_blocks_for_hit(cls, window_size: int, block_size: int, use_eagle: bool) -> int:
    ...
    def reachable_block_mask(
        ...
        # (2) Reachable-boundary tails: the replay boundary and any
        # shared-prefix junction. Sparse retention would otherwise skip them, so
        # keep the tail ending on each boundary explicitly.
        if retention_interval is not None:
            for boundary_tokens in reachable_boundaries:
                ...
                for i in range(max(start_block, end - need), min(end_block, end)):
                    mask[i - start_block] = True
...
class ChunkedLocalAttentionManager(SingleTypeKVCacheManager):
    ...
    def find_longest_cache_hit(
        ...
        assert drop_eagle_block is False, "Hybrid KV cache is not supported for " + "eagle + chunked local attention."
        assert dcp_world_size == 1, "DCP not support chunked local attn now."
        assert pcp_world_size == 1, "PCP not support chunked local attn now."
        assert kv_cache_spec.block_size == alignment_tokens, (
            "KV cache groups with different block sizes are not compatible with chunked local attention now"
        )
        block_hashes = resolve_block_hashes(
            block_hashes,
            block_pool.hash_block_size,
    ...
    def get_num_skipped_tokens(self, num_computed_tokens: int) -> int:
        ...
        Returns:
            The number of tokens that will be skipped for attention computation.
        """
        num_skipped_tokens = (num_computed_tokens // self.attention_chunk_size) * self.attention_chunk_size
...
class MambaManager(SingleTypeKVCacheManager):
    ...
    def __init__(self, kv_cache_spec: MambaSpec, block_pool: BlockPool, **kwargs) -> None:
        ...
        # Mamba align mode uses the spec block size for its recurrent state.
        self.block_size = kv_cache_spec.block_size
        self.mamba_cache_mode = kv_cache_spec.mamba_cache_mode
        self.num_speculative_blocks: int = kv_cache_spec.num_speculative_blocks
        self.has_prefill_checkpoint_blocks = (
            self.mamba_cache_mode == "align" and kv_cache_spec.num_prefill_checkpoint_blocks > 0
        )
    ...
    def find_longest_cache_hit(
        ...
        if alignment_tokens < block_size and block_size % alignment_tokens == 0:
            ...
            max_num_partial_units = min(max_length // hash_block_size, len(block_hashes))
    ...
    def reachable_block_mask(
        ...
        ``retention_interval``:
          ...
          ``0``    -> keep only the reachable-boundary states
          ``> 0``  -> keep one state per ``retention_interval``-sized segment

        ``reachable_boundaries`` are proven reuse points (the replay boundary
        and any cross-request shared-prefix junction); their boundary state is
        always kept so sparse retention does not defeat reuse.
        """
        ...
        if segment_tokens is not None:
            ...
            if per_segment <= 1:
                ...
                return None
            first_boundary = (start_block + per_segment) // per_segment * per_segment - 1
            for i in range(first_boundary - start_block, len(mask), per_segment):
                mask[i] = True

        # (2) Reachable-boundary states: the replay boundary and any
    ...
    def get_num_blocks_to_allocate(
        ...
        if self.mamba_cache_mode != "align":
            ...
            num_evictable_computed_blocks = self._get_num_evictable_blocks(new_computed_blocks)
    ...
    def allocate_new_blocks(self, request_id: str, num_tokens: int, num_tokens_main_model: int) -> list[KVCacheBlock]:
        ...
        if self.mamba_cache_mode != "align":
            ...
            if num_required_blocks <= len(req_blocks) and not has_partial_hit and not checkpoint_block:
                ...
                if prev_block_len < num_skipped_blocks:
                    ...
                    req_blocks.extend([self._null_block for _ in range(prev_block_len, null_end)])
    ...
    def _relocate_speculative_block(self, req_blocks: list[KVCacheBlock], block_idx: int) -> None:
        block = req_blocks[block_idx]
        assert self.block_pool.is_block_writable(block), (
            "Speculative Mamba blocks must be exclusively owned and unhashed before relocation"
        )
    ...
    def pop_blocks_for_free(self, request_id: str) -> list[KVCacheBlock]:
        if self.mamba_cache_mode == "align":
            ...
            # been handed.
            self._pending_boundary_state_offloads = [
                entry for entry in self._pending_boundary_state_offloads if entry[0] != request_id
            ]
...
def register_all_kvcache_specs(aphrodite_config):
    ...
    KVCacheSpecRegistry.register(MambaSpec, MambaManager, uniform_type_base_spec=MambaSpec)
    KVCacheSpecRegistry.register(
        ChunkedLocalAttentionSpec,
    ...
    KVCacheSpecRegistry.register(MLAAttentionSpec, FullAttentionManager, uniform_type_base_spec=FullAttentionSpec)
    KVCacheSpecRegistry.register(RSWASpec, RSWAManager, uniform_type_base_spec=FullAttentionSpec)
    ...
    current_platform.register_custom_kv_cache_specs(aphrodite_config)
