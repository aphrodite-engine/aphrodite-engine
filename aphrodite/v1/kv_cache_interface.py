...
from aphrodite.v1.kv_cache_layout import _DIM_B, _DIM_L, KVCacheLayout
...
def replace_as(
    ...
    **changes,
    ...
    """
    kwargs = {f.name: getattr(spec, f.name) for f in fields(spec) if f.init and f.name not in drop}
...
class KVCacheSpec:
    ...
    def merge(cls, specs: list[Self]) -> Self:
        ...
        if not all(spec == specs[0] for spec in specs[1:]):
            raise AssertionError("All layers in the same KV cache group must be the same.")
    ...
    def max_num_blocks_per_req(self, aphrodite_config: AphroditeConfig, max_len: int) -> int:
        """
...
def compute_layer_kv_cache_shape_bytes(
    ...
    assert spec.block_size % bs == 0, f"Kernel block size {bs} must divide KV cache block size {spec.block_size}."
    ...
    if ratio > 1:
        ...
        if block_stride != dense_page_size:
            raise ValueError(
                ...
                f"{kernel_block_size} or set APHRODITE_KV_CACHE_LAYOUT to a "
                "layer-compact layout (e.g. LBNHC)."
            )
        assert block_stride % ratio == 0, f"Block stride {block_stride} must divide into {ratio} equal kernel blocks."
        block_stride //= ratio
...
class CircularBufferSpec(AttentionSpec):
    ...
    def max_num_blocks_per_req(self, aphrodite_config: AphroditeConfig, max_len: int) -> int:
        del aphrodite_config, max_len
        return 1

    def is_uniform_with_collection(self, kv_cache_specs: dict[str, KVCacheSpec]) -> bool:
        return all(isinstance(spec, CircularBufferSpec) for spec in kv_cache_specs.values())
...
class MambaSpec(KVCacheSpec):
    ...
    @property
    def state_content_size_bytes(self) -> int:
        return sum(prod(shape) * get_dtype_size(dtype) for (shape, dtype) in zip(self.shapes, self.dtypes))
    ...
    def max_memory_usage_bytes(self, aphrodite_config: AphroditeConfig) -> int:
        if aphrodite_config.cache_config.mamba_cache_mode == "all":
            ...
            return self.page_size_bytes * (2 + self.num_speculative_blocks + self.num_prefill_checkpoint_blocks)
        else:
    ...
    def max_num_blocks_per_req(self, aphrodite_config: AphroditeConfig, max_len: int) -> int:
        ...
        if aphrodite_config.cache_config.mamba_cache_mode == "align":
        ...
        return cdiv(self.max_memory_usage_bytes(aphrodite_config), self.page_size_bytes)
...
class CrossAttentionSpec(AttentionSpec):
    ...
    def max_memory_usage_bytes(self, aphrodite_config: AphroditeConfig) -> int:
        ...
        max_encoder_len = aphrodite_config.scheduler_config.max_num_encoder_input_tokens
...
class UniformTypeKVCacheSpecs(KVCacheSpec):
    ...
    def max_memory_usage_bytes(self, aphrodite_config: AphroditeConfig) -> int:
        max_num_pages = max(
            cdiv(spec.max_memory_usage_bytes(aphrodite_config), spec.page_size_bytes)
    ...
    def max_num_blocks_per_req(self, aphrodite_config: AphroditeConfig, max_len: int) -> int:
        ...
        widths = {spec.max_num_blocks_per_req(aphrodite_config, max_len) for spec in self.kv_cache_specs.values()}
        assert len(widths) == 1, (
...
def is_full_attention_spec(kv_cache_spec: KVCacheSpec) -> bool:
    ...
    return len(layer_specs) > 0 and all(isinstance(spec, FullAttentionSpec) for spec in layer_specs)
...
def get_kv_cache_spec_kind(kv_cache_spec: KVCacheSpec) -> KVCacheSpecKind:
    if isinstance(kv_cache_spec, UniformTypeKVCacheSpecs):
        ...
        # already established.
        base_specs = {
            KVCacheSpecRegistry.get_uniform_type_base_spec(spec) for spec in kv_cache_spec.kv_cache_specs.values()
        }
...
class KVCacheConfig:
    ...
    def transfer_group_ids(self) -> tuple[int, ...]:
        ...
        return tuple(group_id for group_id, group in enumerate(self.kv_cache_groups) if group.enable_kv_transfer)

    @cached_property
    def transfer_groups(self) -> tuple[KVCacheGroupSpec, ...]:
        """Cache groups that participate in external KV transfer."""
        return tuple(self.kv_cache_groups[group_id] for group_id in self.transfer_group_ids)
    ...
    def select_transfer_block_ids(self, block_ids: Sequence[list[int]]) -> tuple[list[int], ...]:
        """Select block IDs for externally transferable cache groups."""
        if len(block_ids) != len(self.kv_cache_groups):
            raise ValueError(f"Expected {len(self.kv_cache_groups)} KV cache groups, got {len(block_ids)}.")
