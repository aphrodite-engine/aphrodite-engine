...
from aphrodite.v1.kv_cache_interface import KVCacheLayout, get_kv_quant_mode
...
class AttentionBackend(ABC):
    ...
    def supports_dcp(cls) -> bool:
        try:
            return getattr(cls.get_impl_cls(), "supports_dcp", True)
        except NotImplementedError:
            return False
    ...
    def validate_configuration(
        ...
        if use_adaptive_verification and not cls.supports_device_cpu_query_lens_mismatch():
            invalid_reasons.append(
                "device-cpu query lens mismatch not supported, this is needed for adaptive verification"
            )
...
class CommonAttentionMetadata:
    ...
    def unpadded(self, num_actual_tokens: int, num_actual_reqs: int) -> "CommonAttentionMetadata":
        ...
        return CommonAttentionMetadata(
            ...
            causal=self.causal[:num_actual_reqs] if isinstance(self.causal, torch.Tensor) else self.causal,
...
class AttentionImplBase(ABC, Generic[T]):
    ...
    # aphrodite/v1/attention/ops/dcp.py) branches on this via its IS_BASE_E
    ...
    def __new__(cls, *args, **kwargs):
        ...
        try:
            ...
            self.dcp_rank = 0
        try:
            from aphrodite.distributed.parallel_state import get_pcp_group
...
class AttentionImpl(AttentionImplBase[T], Generic[T]):
    ...
    def do_rope_and_kv_cache_update(
        ...
        by torch.ops.aphrodite.fused_rope_and_unified_kv_cache_update
