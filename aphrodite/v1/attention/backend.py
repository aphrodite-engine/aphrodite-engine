...
class AttentionBackend(ABC):
    ...
    def indexes_kv_by_block_stride(cls) -> bool:
        ...
        """
        try:
            kv_cache_stride_order = cls.get_kv_cache_stride_order(include_num_layers_dimension=False)
            layered_kv_cache_stride_order = cls.get_kv_cache_stride_order(include_num_layers_dimension=True)
...
class CommonAttentionMetadata:
    ...
    def token_to_req_indices(self, buffer: torch.Tensor) -> torch.Tensor:
        ...
        token_to_req_indices = np.repeat(np.arange(query_lens.shape[0], dtype=np.int32), query_lens)
        num_mapped_tokens = token_to_req_indices.shape[0]
        assert buffer.shape[0] >= max(num_mapped_tokens, num_tokens)
        buffer[:num_mapped_tokens].copy_(np_to_pinned_tensor(token_to_req_indices), non_blocking=True)
