...
def mask_empty_context_kernel(
    ...
    while (chunk_start < num_reqs) & (not req_idx_found):
        ...
        matched_idx = tl.sum((req_mask & (req_block_starts <= query_block_idx)).to(tl.int32))
    ...
    for head_start in range(0, NUM_HEADS, BLOCK_HEADS):
        ...
        lse_ptrs = lse + head_indices[:, None] * lse_head_stride + token_lse_offsets[None, :]
...
def merge_attn_states_kernel(
    ...
    if not prefix_mask:
        s_lse = tl.load(suffix_lse + head_idx * suffix_lse_head_stride + token_idx * suffix_lse_token_stride)
        if OUTPUT_LSE:
            tl.store(
                output_lse + head_idx * output_lse_head_stride + token_idx * output_lse_token_stride,
                s_lse,
            )
    ...
    p_lse = tl.load(prefix_lse + head_idx * prefix_lse_head_stride + token_idx * prefix_lse_token_stride)
    s_lse = tl.load(suffix_lse + head_idx * suffix_lse_head_stride + token_idx * suffix_lse_token_stride)
