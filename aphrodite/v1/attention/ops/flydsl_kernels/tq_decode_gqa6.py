...
# kernel is functionally a direct copy of tq_decode.py with the QG knob
...
HEAD_SIZE = 128
# default; overridable via build_tq_decode_gqa6_module(kv_block_size=...)
KV_BLOCK_SIZE = 16
...
def build_tq_decode_gqa6_module(
    num_seqs: int,
    num_kv_heads: int,
    num_partitions: int,
    ...
    ``build_tq_decode_module`` from ``tq_decode.py`` instead.

    ``kv_block_size`` is the number of tokens per Aphrodite cache block. Must
    ...
    assert query_group_size == 6, (
        f"build_tq_decode_gqa6_module is GQA-6 only; got "
        f"query_group_size={query_group_size}. Use build_tq_decode_module "
        f"from tq_decode.py for GQA-8 or GQA-16."
    )
    assert kv_block_size in (16, 32), f"kv_block_size must be 16 or 32; got {kv_block_size}"
    ...
    # --- LDS layout ---
    allocator = SmemAllocator(None, arch=arch, global_sym_name="tq_gqa6_smem")
    centroid_off = 0
    ...
    def tq_decode_gqa6_kernel(
        ...
        for chk in range_constexpr(QK_K_CHUNKS):
            q_idx_i64 = mfma_row * fx.Int32(HEAD_SIZE * 2 // 8) + fx.Int32(chk * 8) + mfma_col_grp * fx.Int32(2)
            qv = vector.load_op(
                T.vec(2, T.i64),
                q_lds_i64,
        ...
        trip_idx = arith.index_cast(
            T.index,
            trip_or_zero.ir_value() if hasattr(trip_or_zero, "ir_value") else trip_or_zero,
        )
        ...
        try:
            ...
            for n_tile in range_constexpr(16):
                ...
                data_bases_byte = data_region + slot * (c_kv_heads * c_data_per_slot) + kv_h * c_data_per_slot
                ...
                vscale_u16 = meta_region // fx.Int32(2) + kv_h * c_meta_u16_per_kvh + c_vscale_off_u16 + slot
                vzero_u16 = meta_region // fx.Int32(2) + kv_h * c_meta_u16_per_kvh + c_vzero_off_u16 + slot
                ...
                knorm_u16 = meta_region // fx.Int32(2) + kv_h * c_meta_u16_per_kvh + c_knorm_off_u16 + slot
                ...
                qk_acc = zero_v4
                for chk in range_constexpr(QK_K_CHUNKS):
                    k_idx_i64 = mfma_row * fx.Int32(HEAD_SIZE * 2 // 8) + fx.Int32(chk * 8) + mfma_col_grp * fx.Int32(2)
                    ...
                    qk_acc = rocdl.mfma_f32_16x16x32_bf16(T.f32x4, [k_op, q_chunks[chk], qk_acc, 0, 0, 0])
                ...
                for elem in range_constexpr(4):
                    kv_tok = (
                        partition_start + fx.Int32(n_tile * TILE_SIZE) + mfma_col_grp * fx.Int32(4) + fx.Int32(elem)
                    )
                    in_b = kv_tok < seq_len
                ...
                for elem in range_constexpr(4):
                    ...
                    tile_sum = tile_sum + p
                    qk_acc = vector.insert(p, qk_acc, static_position=[elem], dynamic_position=[])
                ...
                if USE_HW_TR:
                    ...
                    v_lds_elem_base = tok_in_tile * fx.Int32(HEAD_SIZE) + chunk_in_tok * fx.Int32(32)
                    for w in range_constexpr(4):
                        ...
                        v_bf16 = vector.from_elements(T.vec(8, T.bf16), bf16_elems)
                        v_i64 = vector.bitcast(T.vec(2, T.i64), v_bf16)
                        v_lds_i64_idx = (v_lds_elem_base + fx.Int32(w * 8)) // fx.Int32(4)
                        vector.store(
                            v_i64,
                            kv_lds_i64,
                ...
                if USE_HW_TR:
                    ...
                    v_lane_byte = fx.Int32(kv_off) + token_idx * fx.Int32(HEAD_SIZE * 2) + hd_sub * fx.Int32(2)
                    for h in range_constexpr(PV_N_CHUNKS):
                        ...
                        acc_pv[h] = rocdl.mfma_f32_16x16x16bf16_1k(T.f32x4, [v_op_raw, p_op, acc_pv[h], 0, 0, 0])
                else:
                    ...
                    for h in range_constexpr(PV_N_CHUNKS):
                        v_idx_i64 = (mfma_row + fx.Int32(h * 16)) * fx.Int32(4) + mfma_col_grp
                        kv_load = vector.load_op(
                            T.vec(1, T.i64),
                            kv_lds_i64,
                        ...
                        acc_pv[h] = rocdl.mfma_f32_16x16x16bf16_1k(T.f32x4, [v_op, p_op, acc_pv[h], 0, 0, 0])
        ...
        with ir.InsertionPoint(_if.then_block):
            for h in range_constexpr(PV_N_CHUNKS):
                ...
                out_off_elem = out_base + mfma_row * fx.Int32(HEAD_SIZE) + head_dim_start
            ...
            ml_off = seq * fx.Int32(_stride_ml_seq) + kv_h * c_npq + part * c_qg + mfma_row
            es_off = seq * fx.Int32(_stride_es_seq) + kv_h * c_npq + part * c_qg + mfma_row
            buffer_ops.buffer_store(running_max, ml_rsrc, ml_off)
            buffer_ops.buffer_store(running_sum, es_rsrc, es_off)
            _scf.YieldOp([])

    return tq_decode_gqa6_kernel
