...
from aphrodite.v1.attention.ops.cp_common import (
    DirectCPWorkspace,
    direct_cp_enabled,
...
def mask_dcp_empty_shards_(
    ...
    if seq_lens.ndim != 1 or query_start_loc.ndim != 1 or query_start_loc.shape[0] != seq_lens.shape[0] + 1:
        raise ValueError("query_start_loc must contain one boundary per sequence")

    row_indices = torch.arange(lse.shape[0], device=lse.device, dtype=query_start_loc.dtype)
    sequence_indices = torch.searchsorted(query_start_loc[1:], row_indices, right=True).clamp_max(seq_lens.shape[0] - 1)
...
class CorrectAttnCPOutKernel(AphroditeTritonJitKernel["CorrectAttnCPOutKernel.CompileKey"]):
    ...
    def get_warmup_keys(
        ...
        *,
        ...
        if num_heads is None:
            num_heads = hf_config.num_attention_heads // aphrodite_config.parallel_config.tensor_parallel_size
        if head_dim is None:
            head_dim = get_mla_dims(aphrodite_config.model_config).v_head_dim
        if output_dtype is None:
            output_dtype = aphrodite_config.model_config.dtype
    ...
    def __call__(
        ...
        *,
        ...
        return grid, dict(
            ...
            _runtime_launcher_arg_count=len(self.kernel.arg_names) - len(self.kernel.constexprs),
        )
...
def _dcp_a2a_pack_send_kernel(
    ...
    for rank_idx in tl.static_range(N):
        ...
        lse_val = tl.load(lse_ptr + batch_idx * lse_stride_B + src_head_idx * lse_stride_H).to(tl.float32)
        if LSE_PACK_DIM == 1:
            tl.store(
...
def _dcp_a2a_unpack_combine_kernel(
    ...
    for rank_idx in tl.static_range(N):
        ...
        partial = tl.load(recv_ptr + recv_base + d_offsets * recv_stride_D).to(tl.float32)
...
def get_dcp_workspace_max_num_tokens(aphrodite_config: AphroditeConfig) -> int:
    scheduler_config = aphrodite_config.scheduler_config
    speculative_config = aphrodite_config.speculative_config
    speculative_tokens = aphrodite_config.num_speculative_tokens
    tokens_per_seq = (
        1 + (2 if speculative_config is not None and speculative_config.parallel_drafting else 1) * speculative_tokens
    )
    return min(
        scheduler_config.max_num_batched_tokens,
        max(
            scheduler_config.max_num_seqs * tokens_per_seq,
            aphrodite_config.compilation_config.max_cudagraph_capture_size or 0,
        ),
    )


def reserve_query_head_storage(query: torch.Tensor, padded_num_heads: int) -> torch.Tensor:
...
class DirectDCPA2AWorkspace(DirectCPWorkspace):
    ...
    def __init__(
        ...
        if num_ubatches < 1:
            raise ValueError(f"Direct DCP A2A requires at least one ubatch slot, got {num_ubatches}")
        ...
        self.received_output, self.peer_output_ptrs = self._allocate(output_shape, dtype)
        self.received_lse, self.peer_lse_ptrs = self._allocate(lse_shape, torch.float32)
        self.received_signal, self.peer_signal_ptrs = self._allocate(signal_shape, torch.int32)
    ...
    def lse_reduce(
        ...
        output = partial_output.new_empty((num_tokens, self.heads_per_rank, self.head_dim))
...
def get_direct_dcp_a2a_workspace(
    ...
    if not direct_cp_enabled(group, dtype, envs.APHRODITE_USE_DIRECT_DCP_A2A, _A2A_SUPPORTED_DTYPES):
        return None
    try:
        ...
        if envs.APHRODITE_USE_DIRECT_DCP_A2A:
            raise
        logger.warning_once(
            "Direct symmetric-memory DCP A2A initialization failed; falling back to the configured collective: %s",
            error,
        )
        return None
...
def _q_gather_layout_supported(
    ...
    storage_num_heads = gathered_num_heads if padded_num_heads is None else padded_num_heads
    return heads_per_rank * head_dim * element_size % 16 == 0 and storage_num_heads * head_dim * element_size % 16 == 0
...
class DirectDCPQGatherWorkspace(DirectCPWorkspace):
    ...
    def __init__(
        ...
        if num_ubatches < 1:
            raise ValueError(f"Direct DCP q-gather requires at least one ubatch slot, got {num_ubatches}")
        ...
        if not _q_gather_layout_supported(group.size(), heads_per_rank, head_dim, dtype, padded_num_heads):
        ...
        self.padded_num_heads = self.gathered_num_heads if padded_num_heads is None else padded_num_heads
        ...
        self.multicast_ptrs = list(zip(query_multicast_ptrs, signal_multicast_ptrs, strict=True))
        if not all(query_ptr and signal_ptr for query_ptr, signal_ptr in self.multicast_ptrs):
            raise RuntimeError("Direct DCP q-gather requires NVLS symmetric-memory multicast.")
    ...
    def gather(self, local_query: torch.Tensor) -> torch.Tensor:
        ...
        if not 0 <= ubatch < self.num_ubatches:
            raise ValueError(f"DCP q-gather ubatch {ubatch} exceeds {self.num_ubatches} slots")
        if local_query.ndim == 3 and local_query.shape[1] != self.heads_per_rank:
            raise ValueError(
                f"DCP q-gather expected {self.heads_per_rank} local query heads, got {local_query.shape[1]}"
            )
...
def get_direct_dcp_q_gather_workspace(
    ...
    if not direct_cp_multicast_enabled(group, dtype, envs.APHRODITE_USE_DIRECT_DCP_Q_GATHER):
        return None
    if not _q_gather_layout_supported(group.world_size, heads_per_rank, head_dim, dtype, padded_num_heads):
        return None
    try:
        ...
        if envs.APHRODITE_USE_DIRECT_DCP_Q_GATHER:
            raise
        logger.warning_once(
            "Direct symmetric-memory DCP query-gather initialization failed; "
            "falling back to the configured collective: %s",
            error,
        )
        return None
...
class DirectDCPKVGatherWorkspace(DirectCPWorkspace):
    ...
    def __init__(
        ...
        if num_ubatches < 1:
            raise ValueError(f"Direct DCP kv-gather requires at least one ubatch slot, got {num_ubatches}")
        if max_gathered_tokens < 1 or token_dim < 1:
            raise ValueError(
                f"Direct DCP kv-gather dimensions must be positive, got T={max_gathered_tokens}, D={token_dim}"
            )
        ...
        signal_multicast_ptrs = self._multicast_ptrs(self.received_signal)
        self.multicast_ptrs = list(zip(kv_multicast_ptrs, signal_multicast_ptrs, strict=True))
        if not all(kv_ptr and signal_ptr for kv_ptr, signal_ptr in self.multicast_ptrs):
            raise RuntimeError("Direct DCP kv-gather requires NVLS symmetric-memory multicast.")
    ...
    def gather(self, gathered_kv: torch.Tensor, local_kv: torch.Tensor) -> None:
        ubatch = dbo_current_ubatch_id()
        if not 0 <= ubatch < self.num_ubatches:
            raise ValueError(f"DCP kv-gather ubatch {ubatch} exceeds {self.num_ubatches} slots")
...
def get_direct_dcp_kv_gather_workspace(
    ...
    if not direct_cp_multicast_enabled(
        group,
        dtype,
        envs.APHRODITE_USE_DIRECT_DCP_KV_GATHER,
        _KV_GATHER_SUPPORTED_DTYPES,
    ):
        return None
    if not _kv_gather_layout_supported(token_dim, dtype):
    ...
    try:
        ...
        if envs.APHRODITE_USE_DIRECT_DCP_KV_GATHER:
            raise
        logger.warning_once(
            "Direct symmetric-memory DCP KV-gather initialization failed; "
            "falling back to the configured collective: %s",
            error,
        )
        return None
...
class MLADCPManager:
    ...
    def __init__(
        ...
        self.max_num_tokens = get_dcp_workspace_max_num_tokens(aphrodite_config)
        ...
        if aphrodite_config.kernel_config.enable_jit_warmup and not self.use_a2a:
    ...
    def _init_combine(
        ...
        combine_fn = dcp_a2a_lse_reduce if self.use_a2a else cp_lse_ag_out_ar if use_pcp else cp_lse_ag_out_rs
        return functools.partial(
            combine_fn,
            cp_group=self.group,
            is_lse_base_on_e=is_lse_base_on_e,
    ...
    def init_kv_gather(
        ...
        assert workspace.shape[0] == (max_gathered_tokens + max_gathered_tokens // world_size)
        ...
        if direct_workspace is not None:
            logger.info_once("Using direct symmetric-memory DCP chunked-context KV gather for MLA.")
