...
"""FlyDSL TurboQuant decode launcher (Aphrodite-side).
...
# decode, so Aphrodite startup is unaffected on hosts without FlyDSL.
...
def is_flydsl_available() -> bool:
    ...
    try:
        ...
        # Kernel is vendored in-tree (ships with Aphrodite); FlyDSL provides only
        # the compiler/runtime framework (imported above).
        from aphrodite.v1.attention.ops.flydsl_kernels import tq_decode as tq_mod
        ...
        _FLYDSL_AVAILABLE = False
        logger.warning_once(
            "FlyDSL TQ decode launcher: unavailable (%s). Falling back to SoA Triton decode.",
            ex,
        )
        return _FLYDSL_AVAILABLE
    # Best-effort GQA-6 sibling import (does NOT gate Qwen availability).
    try:
        from aphrodite.v1.attention.ops.flydsl_kernels import (
        ...
        _TQ_MOD_GQA6 = None
        logger.info_once(
            "FlyDSL TQ decode GQA-6 sibling: not available (%s); GQA-6 models will fall back to SoA Triton decode.",
            ex,
        )
    return _FLYDSL_AVAILABLE
...
# -- One-shot log guards ------------------------------------------------------
# Replaces aphrodite `logger.{info,warning}_once` (which hashes the format string
...
def _detect_max_capture_B() -> int:
    """Probe Aphrodite compilation config for the largest cudagraph capture size.

    Returns max(cudagraph_capture_sizes) if available, else falls back to
    512 (Aphrodite's default cap). The segm-pool auto-grows if a larger batch is
    ...
    try:
        ...
        cfg = get_current_aphrodite_config()
...
class _SegmBufPool:
    ...
    def stats(self) -> dict[str, int]:
        return {
            ...
            "bytes": sum(sum(t.numel() * t.element_size() for t in d.values()) for d in self._bufs.values()),
        }
    ...
    if qg == 6:
        if _TQ_MOD_GQA6 is None:
            raise RuntimeError(
                ...
                "(aphrodite.v1.attention.ops.flydsl_kernels.tq_decode_gqa6) "
...
def _reduce_partitions(
    ...
    so_base = n * (NUM_KV_HEADS * NUM_PARTS * QG * HEAD_SIZE) + kv_h * (NUM_PARTS * QG * HEAD_SIZE) + qg * HEAD_SIZE
...
def flydsl_turboquant_decode_attention(
    ...
    assert value_quant_bits == 4, f"FlyDSL expects value_quant_bits=4, got {value_quant_bits}"
    ...
    assert block_size in (16, 32), f"FlyDSL supports kv_block_size 16 or 32, got {block_size}"
    ...
    assert centroids.numel() == _TQ_MOD.N_CENTROIDS, f"centroids.numel={centroids.numel()} != {_TQ_MOD.N_CENTROIDS}"
    ...
    if buf_holder is not None:
        ...
        if PiT_f32 is None:
            ...
            PiT_f32 = _PiT_src if _PiT_src.dtype == torch.float32 else _PiT_src.to(torch.float32)
    ...
    # bounded by `max_model_len` (Aphrodite allocates the block table for the
    ...
    MAX_PARTITIONS = 32
    required_num_partitions = (sizing_max_seq_len + kv_compute_block - 1) // kv_compute_block
    ...
    if sinks is not None and not _LOG_SINKS_WARNED:
        ...
        logger.warning("FlyDSL launcher: sinks ignored (NYI). Disable sinks if they are required for correctness.")
