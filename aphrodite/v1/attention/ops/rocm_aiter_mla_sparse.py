...
from aphrodite.utils.torch_utils import LayerNameType
...
def _get_aiter_top_k_kernel(
    *,
    ...
    if not is_prefill:
        ...
        if num_rows <= _GFX950_C4A_NATIVE_MAX_ROWS and max_valid_seq_len > _GFX950_C4A_AITER_MAX_COMPRESSED_SEQ_LEN:
            return None
...
def fp8_mqa_logits_torch(
    ...
    # Aphrodite callers hand us as ``[N, 1]`` (a ``[N, 4]`` uint8 buffer cast
...
def rocm_aiter_sparse_attn_indexer(
    ...
    fp8_dtype = current_platform.fp8_dtype()
    from aphrodite.utils.torch_utils import _resolve_layer_name
...
def _validate_sparse_dims(
    ...
    assert nope_head_dim > 0, f"{op_name} expected a positive NoPE dimension, got {nope_head_dim}"
    assert rope_head_dim >= 0, f"{op_name} expected a non-negative RoPE dimension, got {rope_head_dim}"
...
def _sparse_attn_prefill_ragged_kernel(
    ...
    for k_start in tl.range(0, kv_len, BLOCK_K):
        ...
        kv = tl.load(
            kv_ptr + _sparse_kv_row_offset(safe_slot[:, None], kv_stride_n) + dim_offsets[None, :] * kv_stride_d,
...
def _sparse_attn_decode_gfx950_partial_kernel(
    ...
    if HAS_EXTRA:
        ...
        extra_hi_full = extra_lo + ((extra_hi - extra_lo) // outer_block_k) * outer_block_k
        for k_start in tl.range(
            extra_lo,
            extra_hi_full,
            outer_block_k,
    ...
    tl.store(part_l_ptr + pm_base, l_i, mask=head_mask)
    acc_base = part_acc_ptr + ((query_idx * NUM_SPLITS + split_id) * num_heads + head_offsets[:, None]) * (
        NOPE_DIM + ROPE_DIM
    )
    tl.store(
...
def _decode_gfx950_num_splits(
    ...
    if base >= 16 and num_splits > 4 and _decode_partial_iters(avg_main_len, avg_extra_len, 4, block_k) <= 3:
        return 4
    if 16 <= base < 64:
        one_wave_splits = max(1, cu // base)
        one_wave_iters = _decode_partial_iters(avg_main_len, avg_extra_len, one_wave_splits, block_k)
    ...
    if base >= 16 and num_splits > 1:
        ...
        target_iters = _decode_partial_iters(avg_main_len, avg_extra_len, num_splits, block_k)
...
def _rocm_sparse_attn_decode_ragged_triton(
    ...
    if out is None:
        ...
        assert out.shape == q.shape, f"expected out shape {q.shape}, got {out.shape}"
        assert out.device == q.device, f"expected out on device {q.device}, got {out.device}"
        assert out.dtype == torch.bfloat16, f"expected out dtype {torch.bfloat16}, got {out.dtype}"
    ...
    adaptive_splits = _ON_GFX950 and adaptive_splits and base_workgroups >= 16 and num_splits > 4
    one_wave_splits = (
        max(1, _decode_cu_count() // base_workgroups) if adaptive_splits and 16 <= base_workgroups < 64 else num_splits
    )
    ...
    if _ON_GFX950:
        _sparse_attn_decode_gfx950_partial_kernel[(num_queries, num_splits, heads_blocks)](
            q,
            main_cache,
            main_indices,
            main_indptr,
