...
def direct_cp_enabled(
    ...
    return (
        ...
        and (all(in_the_same_node_as(group.cpu_group, source_rank=0)) or _symm_mem_spans_group(group))
    )
...
def direct_cp_multicast_enabled(
    ...
    return direct_cp_enabled(group, dtype, use_direct, supported_dtypes) and _symm_mem_spans_group(group)
...
class DirectCPWorkspace:
    ...
    def _allocate(self, shape: tuple[int, ...], dtype: torch.dtype) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        handle.barrier()
        views = [handle.get_buffer(peer, list(shape), dtype, 0) for peer in range(self.world_size)]
        self.device = storage.device
        peer_ptrs = torch.tensor(
            [[view[ubatch].data_ptr() for view in views] for ubatch in range(self.num_ubatches)],
            dtype=torch.int64,
            device=self.device,
        )
    ...
    def _multicast_ptrs(self, storage: torch.Tensor) -> list[int]:
        ...
        try:
            ...
            if not _SymmetricMemory.has_multicast_support(DeviceType.CUDA, storage.device.index):
                return disabled
        ...
        return [multicast_base + (storage[ubatch].data_ptr() - storage_base) for ubatch in range(self.num_ubatches)]
