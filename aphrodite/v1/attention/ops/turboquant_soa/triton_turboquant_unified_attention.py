...
Structure is ported from ``aphrodite/v1/attention/ops/triton_unified_attention.py``
(the AITER unified attention kernel already upstreamed into Aphrodite). Only the K
...
This is an opt-in v3 path behind ``APHRODITE_TQ_DECODE_V3``.
"""
...
from aphrodite.v1.attention.ops.triton_unified_attention import reduce_segments
...
def kernel_tq_unified_attention_2d(
    ...
    if USE_SINKS:
        M = tl.load(sinks_ptr + query_offset_1, mask=query_mask_1, other=float("-inf")).to(tl.float32)
    else:
    ...
    for j in range(0, num_tiles):
        ...
        head_meta_u16_base = (block_base + META_REGION_OFFSET) // 2 + tl.cast(kv_head_idx, tl.int64) * (
...
def triton_turboquant_unified_attention(
    ...
    if sinks is not None:
        ...
        assert sinks_f32.numel() == Hq, f"sinks must have shape [Hq={Hq}], got numel={sinks_f32.numel()}"
        use_sinks = True
    else:
    ...
    segm_max = torch.empty((num_tokens, Hq, num_segments), dtype=torch.float32, device=device)
    segm_expsum = torch.empty((num_tokens, Hq, num_segments), dtype=torch.float32, device=device)
