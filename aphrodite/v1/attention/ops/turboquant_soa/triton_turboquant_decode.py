...
from aphrodite.v1.attention.ops.triton_decode_attention import (
    _fwd_kernel_stage2,
)
...
def _tq_decode_stage1(
    ...
    for start_n in range(split_start, split_end, BLOCK_KV):
        ...
        head_meta_u16_base = (block_base + META_REGION_OFFSET) // 2 + tl.cast(kv_head, tl.int64) * (
        ...
        if KEY_FP8:
            ...
            norm_u16 = tl.load(KV_cache_u16_ptr + knorm_u16_addrs, mask=kv_mask, other=0)
...
def _tq_full_dequant_kv(
    ...
    head_meta_u16_base = (block_base + META_REGION_OFFSET) // 2 + tl.cast(hid, tl.int64) * (NUM_SOA_FIELDS * BLOCK_SIZE)
    ...
    if KEY_FP8:
        ...
        mse_raw0 = tl.load(KV_cache_ptr + data_base + mse_byte_idx, mask=d_mask, other=0).to(tl.int32)
        mse_raw1 = tl.load(KV_cache_ptr + data_base + mse_byte_idx + 1, mask=d_mask, other=0).to(tl.int32)
