...
from aphrodite.v1.attention.ops.triton_decode_attention import _fwd_kernel_stage2
...
def _tq_decode_stage1_v2(
    ...
    for start_n in range(split_start, split_end, TILE_SIZE):
        ...
        head_meta_u16_base = (block_base + META_REGION_OFFSET) // 2 + tl.cast(kv_hid, tl.int64) * (
        ...
        if KEY_FP8:
            ...
            norm_u16 = tl.load(KV_cache_u16_ptr + knorm_u16_addrs, mask=kv_mask_1d, other=0)
        ...
        scale_u16 = tl.load(KV_cache_u16_ptr + vscale_u16_addrs, mask=kv_mask_1d, other=0)
    ...
    out_addrs = bid * stride_mid_b + q_head_offs[:, None] * stride_mid_h + sid * stride_mid_s + d_offs[None, :]
    tl.store(
        Mid_o_ptr + out_addrs,
        acc,
        mask=q_mask[:, None] & d_mask[None, :],
    ...
    lse_addrs = bid * stride_mid_b + q_head_offs * stride_mid_h + sid * stride_mid_s + HEAD_DIM
