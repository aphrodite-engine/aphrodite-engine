...
def _tq_fused_store_fp8(
    ...
    data_base = block_base + off * (H * DATA_BYTES_PER_SLOT) + head_idx_i64 * DATA_BYTES_PER_SLOT

    # SoA metadata region (u16 element indexing): head strip = NUM_SOA_FIELDS
    # consecutive bs-long fp16 arrays.
    head_meta_u16_base = (block_base + META_REGION_OFFSET) // 2 + head_idx_i64 * (NUM_SOA_FIELDS * BLOCK_SIZE)
...
def triton_turboquant_store(
    ...
    key_fp8: bool = False,
    centroids: torch.Tensor | None = None,  # [n_centroids] float32 — required when norm_correction=True
