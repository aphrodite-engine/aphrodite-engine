...
class PleShortConvAttentionMetadataBuilder(ShortConvAttentionMetadataBuilder):
    ...
    def __init__(
        ...
        if max_capture_size is not None:
            self.decode_cudagraph_max_bs = min(self.decode_cudagraph_max_bs, max_capture_size)
            self.decode_cudagraph_max_tokens = min(self.decode_cudagraph_max_tokens, max_capture_size)

        # Persistent buffers reused during full CUDA graph capture and replay.
        self.spec_state_indices_tensor = torch.empty((self.decode_cudagraph_max_bs,), dtype=torch.int32, device=device)
        self.spec_sequence_masks = torch.empty((self.decode_cudagraph_max_bs,), dtype=torch.bool, device=device)
        self.spec_query_start_loc = torch.empty((self.decode_cudagraph_max_bs + 1,), dtype=torch.int32, device=device)
        self.num_accepted_tokens = torch.empty((self.decode_cudagraph_max_bs,), dtype=torch.int32, device=device)
        self.has_initial_states_d = torch.empty((self.decode_cudagraph_max_bs,), dtype=torch.bool, device=device)
    ...
    def _build_non_spec_metadata(
        ...
        **kwargs: Any,
        ...
        if metadata.num_prefills == 0:
            ...
            assert state_indices_d is not None
            assert state_indices_p is not None
            state_indices_tensor = torch.cat((state_indices_d, state_indices_p[: metadata.num_prefills]))
        ...
        if metadata.num_decodes > 0:
            ...
            if (
                ...
                self.has_initial_states_d[: metadata.num_decodes].copy_(has_initial_states_d, non_blocking=True)
                self.has_initial_states_d[metadata.num_decodes : num_decode_rows].fill_(False)
        ...
        if metadata.num_prefills > 0:
            ...
            max_prefill_query_len = int(
                query_lens_cpu[metadata.num_decodes : (metadata.num_decodes + metadata.num_prefills)].max().item()
            )
    ...
    def build(  # type: ignore[override]
        ...
        **kwargs: Any,
        ...
        if query_start_loc.device.type == "cpu":
            ...
            spec_sequence_masks = async_tensor_h2d(spec_sequence_masks_cpu, device=query_start_loc.device)
        ...
        num_spec_decode_tokens = int(query_lens_cpu[spec_sequence_masks_cpu].sum().item())
        # Max prefill query length, CPU-side (no device-to-host sync) for the
        # PLE dilated short-conv packing buffer.
        max_prefill_query_len = int(query_lens_cpu[prefill_mask_cpu].max().item()) if num_prefills > 0 else 0
        ...
        if num_decodes == 0 and num_prefills == 0:
            ...
            non_spec_req_idx = async_tensor_h2d(non_spec_req_idx_cpu, device=query_start_loc.device)
            decode_req_idx = async_tensor_h2d(decode_req_idx_cpu, device=query_start_loc.device)
            ...
            non_spec_query_start_loc_cpu = torch.zeros(num_decodes + num_prefills + 1, dtype=torch.int32)
            torch.cumsum(
        ...
        if num_decodes > 0 or num_prefills > 0:
            ...
            if num_prefills > 0:
                num_computed_tokens_p = num_computed_tokens[num_decodes : num_decodes + num_prefills]
                has_initial_states_p = num_computed_tokens_p > 0
                assert non_spec_query_start_loc is not None
                assert non_spec_query_start_loc_cpu is not None
                query_start_loc_p = non_spec_query_start_loc[num_decodes:] - num_decode_tokens
                query_start_loc_p_cpu = non_spec_query_start_loc_cpu[num_decodes:] - num_decode_tokens
                if query_start_loc.device.type != "cpu":
        ...
        if (
            ...
            self.spec_sequence_masks[:batch_size].copy_(spec_sequence_masks[:batch_size], non_blocking=True)
    ...
    def build_for_cudagraph_capture(
        ...
        """
        m = common_attn_metadata
        assert m.num_reqs <= self.decode_cudagraph_max_bs and m.num_actual_tokens <= self.decode_cudagraph_max_tokens, (
