...
from aphrodite.v1.attention.ops.flydsl_turboquant_decode import (
    flydsl_turboquant_decode_attention,
    is_flydsl_available,
    is_flydsl_gqa6_available,
)
from aphrodite.v1.attention.ops.triton_turboquant_decode import (
    _tq_full_dequant_kv,
    _use_fp8_e4b15,
...
def _soa_imports():
    ...
    """
    from aphrodite.v1.attention.ops.turboquant_soa.triton_turboquant_decode import (
        _tq_full_dequant_kv as soa_dequant,
    )
    from aphrodite.v1.attention.ops.turboquant_soa.triton_turboquant_store import (
        triton_turboquant_store as soa_store,
    )
    from aphrodite.v1.attention.ops.turboquant_soa.triton_turboquant_unified_attention import (  # noqa: E501
...
class TurboQuantAttentionBackend(AttentionBackend):
    ...
    def customize_spec(cls, spec: AttentionSpec) -> AttentionSpec:
        ...
        tq = TurboQuantConfig.from_cache_dtype(spec.kv_quant_mode.name.lower(), spec.head_size)
...
class TurboQuantAttentionImpl(AttentionImpl["TurboQuantMetadata"]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._max_model_len = aphrodite_config.model_config.max_model_len
    ...
    def _ensure_on_device(self, layer, device):
        ...
        if self._soa_store:
            ...
            if not _already_ok:
                self._arange_cache = torch.arange(0, _max_len + 2, device=device, dtype=torch.int32)
            ...
            if is_workspace_manager_initialized() and not current_workspace_manager().is_locked():
                ...
                with contextlib.suppress(AssertionError):
                    current_workspace_manager().get_simultaneous(((_pre_warm_bytes,), torch.uint8))
    ...
    def _decode_attention(
        ...
        if self._use_flydsl:
            ...
            flydsl_gqa_ok = (_gqa in (8, 16)) or (_gqa == 6 and is_flydsl_gqa6_available())
            flydsl_eligible = (
    ...
    def _dispatch_decode_soa(self, **kwargs):
        ...
        if dropped:
            raise NotImplementedError(f"SoA decode does not support kwargs {sorted(dropped)}")
