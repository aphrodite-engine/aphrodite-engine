...
from aphrodite.config import AphroditeConfig, CacheConfig, get_layers_from_aphrodite_config
...
from aphrodite.utils.torch_utils import PIN_MEMORY, async_tensor_h2d, np_to_pinned_tensor
from aphrodite.v1.kv_cache_interface import KVCacheLayout, KVCacheSpec, MambaSpec
...
def get_supported_kv_cache_layouts(
    ...
    supported_layouts_lists: list[Sequence[KVCacheLayout]] = [
        layouts for backend in backends if (layouts := backend.supported_kv_cache_layouts()) is not None
    ...
    if not candidates:
        raise ValueError(
            f"No KV cache layout satisfies every supported set: {list(map(_layout_names, supported_layouts_lists))}."
        )
    return candidates
...
def record_kv_cache_layout(cache_config: CacheConfig, layout_name: str) -> None:
    ...
    if existing is not None and existing != layout.name:
        raise ValueError(f"KV cache layout is already resolved to {existing}; cannot change it to {layout.name}.")
...
def resolve_kv_cache_layout(
    ...
    ``APHRODITE_KV_CACHE_LAYOUT`` must be one of the candidates or resolution fails,
    ...
    assert supported_layouts and all(supported_layouts), "No worker reported supported KV cache layouts."
    ...
    hnc_shapes = {(spec.num_heads, spec.num_states, spec.page_size_bytes) for spec in kv_cache_specs or ()}
    if len(hnc_shapes) > 1:
    ...
    if (requested := envs.APHRODITE_KV_CACHE_LAYOUT) is not None:
        layout = _layout_from_name(requested)
        if layout not in candidates:
            raise ValueError(
                f"APHRODITE_KV_CACHE_LAYOUT={requested} does not satisfy every "
                f"supported set; valid layouts: {_layout_names(candidates)}."
            )
    elif (connector := get_kv_connector_cache_layout(aphrodite_config)) is not None:
...
def get_per_layer_parameters(
    aphrodite_config: AphroditeConfig, layer_names: list[str], cls_: type["AttentionImpl"]
...
def get_num_attention_heads_from_layers(aphrodite_config: AphroditeConfig, layer_names: list[str]) -> int | None:
    ...
    if not attn_layers:
        return None
    heads = {cast(Any, getattr(layer, "impl", layer)).num_heads for layer in attn_layers.values()}
    assert len(heads) == 1, f"All layers in one attention group must share num_heads; got {heads} for {layer_names}."
    return heads.pop()
...
def make_kv_sharing_fast_prefill_common_attn_metadata(
    ...
    num_decode_tokens = torch.zeros(num_reqs, dtype=request_ids.dtype, device=request_ids.device)
    num_decode_tokens.scatter_add_(0, request_ids.to(num_decode_tokens.dtype), torch.ones_like(request_ids))
...
def split_decodes_and_prefills(
    ...
    if require_uniform:
        ...
        if treat_short_extends_as_decodes and torch.all((query_lens == query_lens[0]) | (query_lens == 0)):
