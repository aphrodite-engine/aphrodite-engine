...
from aphrodite.utils.b12x import get_b12x_paged_attention
...
from aphrodite.v1.kv_cache_interface import AttentionSpec, KVCacheLayout, KVCacheSpec
...
def _kv_page_size(key_cache: torch.Tensor, value_cache: torch.Tensor) -> int:
    """Return the static kernel page geometry negotiated by Aphrodite.
    ...
    if key_page_size != value_page_size:
        raise ValueError(f"b12x requires matching K/V page sizes, got {key_page_size} and {value_page_size}.")
    if key_page_size not in _B12X_SUPPORTED_PAGE_SIZES:
        raise ValueError(f"b12x requires runtime page size in {_B12X_SUPPORTED_PAGE_SIZES}, got {key_page_size}.")
...
def _ensure_i32_contiguous(tensor: torch.Tensor, name: str) -> torch.Tensor:
    if tensor.dtype != torch.int32:
        if _capture_alloc_forbidden():
            raise RuntimeError(
                f"b12x would convert {name} to int32 during CUDA graph capture. Prepare int32 metadata before capture."
            )
...
def _dtype_from_cache_config(
    ...
    if kv_cache_dtype != "auto":
        raise NotImplementedError(
            f"b12x currently supports only auto, bfloat16, fp8, and fp8_e4m3 KV cache dtypes; got {kv_cache_dtype!r}."
        )
    return aphrodite_config.model_config.dtype
...
class B12xPagedAttentionBackend(AttentionBackend):
    ...
    def customize_spec(cls, spec: AttentionSpec) -> AttentionSpec:
        if spec.state_content_bytes is not None:
            return spec
        assert spec.head_size == spec.head_size_v, "Separate K/V planes require symmetric K/V head sizes."
        return replace(
            spec,
            num_head_slots=2,
    ...
    supported_kv_cache_dtypes: ClassVar[list[CacheDType]] = list(_B12X_SUPPORTED_KV_CACHE_DTYPES)
    ...
    def supports_compute_capability(cls, capability: DeviceCapability) -> bool:
        ...
        # internally, but keep Aphrodite selection fail-fast and explicit.
    ...
    def supports_combination(
        ...
        if paged_attention is None:
            return "Install the b12x backend with `pip install aphrodite[b12x]`"
...
class B12xPagedMetadataBuilder(AttentionMetadataBuilder[B12xPagedMetadata]):
    ...
    def get_cudagraph_support(
        ...
        del aphrodite_config, kv_cache_spec
...
class B12xPagedAttentionImpl(AttentionImpl[B12xPagedMetadata]):
    ...
    def __init__(
        ...
        if attn_type != AttentionType.DECODER:
            raise NotImplementedError("b12x currently supports decoder self-attention only.")
        if is_quantized_kv_cache(kv_cache_dtype) and not _is_b12x_fp8_kv_cache(kv_cache_dtype):
            raise NotImplementedError("b12x currently supports only fp8/fp8_e4m3 quantized KV cache dtypes.")
        ...
        if not math.isclose(float(scale), expected_scale, rel_tol=1e-5, abs_tol=1e-7):
            raise NotImplementedError(
                f"b12x currently requires canonical softmax scale head_dim**-0.5={expected_scale}, got {scale}."
            )
        if self.total_cp_world_size > 1:
            raise NotImplementedError("b12x does not yet support decode/prefill context parallelism.")
        ...
        if sinks is not None and (sinks.ndim != 1 or int(sinks.shape[0]) != self.num_heads):
            raise ValueError(f"b12x sinks must have shape [{self.num_heads}], got {tuple(sinks.shape)}.")
        ...
        if default_block_size not in _B12X_SUPPORTED_PAGE_SIZES:
            raise ValueError(
                f"b12x requires --block-size in {_B12X_SUPPORTED_PAGE_SIZES}, got {cache_config.block_size}."
            )

        self.device = torch.device("cuda", torch.accelerator.current_device_index())
        self.dtype = model_config.dtype
        self.kv_torch_dtype = _dtype_from_cache_config(kv_cache_dtype, aphrodite_config)
        ...
        capture_sizes = aphrodite_config.compilation_config.cudagraph_capture_sizes or []
        decode_plan_sizes = {int(size) for size in capture_sizes if 0 < int(size) <= max_num_seqs}
        ...
        if spec_config is not None:
            self._verify_q_per_req = 1 + int(getattr(spec_config, "num_speculative_tokens", None) or 0)
        ...
        for page_size in _B12X_SUPPORTED_PAGE_SIZES:
            for batch_size in sorted(decode_plan_sizes):
                self._decode_plans[page_size, batch_size] = self._create_decode_plan(page_size, batch_size)
            if self._verify_q_per_req > 1:
                for batch_size in range(1, max_num_seqs + 1):
                    self._verify_plans[page_size, batch_size] = self._create_verify_plan(page_size, batch_size)
            for batch_size in range(1, max_num_seqs + 1):
                for q_capacity in self._extend_q_capacities:
                    ...
                    if batch_size >= q_capacity:
                        continue
                    self._extend_plans[page_size, batch_size, q_capacity] = self._create_extend_plan(
                        page_size,
                        batch_size,
                        q_capacity,
                    )
    ...
    def _compile_paged_extend_entry(self, page_size: int) -> None:
        ...
        for batch_size in range(1, self._max_num_seqs + 1):
            candidates = sorted(
                (q_capacity, plan)
                for (plan_page_size, plan_batch, q_capacity), plan in (self._extend_plans.items())
        ...
        max_q_rows = max(min(q_capacity, max(64, batch_size + 1)) for batch_size, q_capacity, _, _ in warmup_plans)
        q = torch.zeros(
        ...
        for batch_size, q_capacity, plan, execute in warmup_plans:
            ...
            cache_seqlens = torch.full((batch_size,), page_size, dtype=torch.int32, device=self.device)
            ...
            if _is_b12x_fp8_kv_cache(self.kv_cache_dtype):
                k_descale = torch.ones((), dtype=torch.float32, device=self.device).expand(batch_size)
                v_descale = torch.ones((), dtype=torch.float32, device=self.device).expand(batch_size)
            binding = plan.bind(
    ...
    def _prepare_fp8_descales(
        ...
        def _prepare(scale: torch.Tensor, name: str) -> torch.Tensor:
            ...
            raise ValueError(
                f"b12x {name} must be scalar or rank-1 with at least {num_reqs} values; got shape {tuple(scale.shape)}."
            )
    ...
    def _kv_cache_views(
        ...
        if key_cache.dtype != self.kv_torch_dtype or value_cache.dtype != self.kv_torch_dtype:
            raise TypeError(
                f"b12x plan expects KV dtype {self.kv_torch_dtype}, got {key_cache.dtype}/{value_cache.dtype}."
            )
    ...
    def _select_plan(
        ...
        extend_q_capacity = next(
            (capacity for capacity in self._extend_q_capacities if q_capacity <= capacity),
            None,
        )
        if extend_q_capacity is None:
            raise ValueError(
                f"b12x extend Q capacity {q_capacity} exceeds prepared maximum {self._extend_q_capacities[-1]}."
            )
    ...
    def forward(
        ...
        if output_scale is not None or output_block_scale is not None:
            raise NotImplementedError("b12x does not support fused output quantization.")
        if attn_metadata is None:
            return output.fill_(0)
        if output.shape[-1] != self.output_head_size:
            raise ValueError(f"b12x expected output head dim {self.output_head_size}, got {output.shape[-1]}.")
        if kv_cache.numel() == 0:
            return output.fill_(0)

        # In FULL cudagraph mode Aphrodite may pad attention metadata to the graph
        ...
        if q.dtype != self.dtype or out.dtype != self.dtype:
            raise TypeError(f"b12x plan expects dtype {self.dtype}, got q={q.dtype}, output={out.dtype}.")
