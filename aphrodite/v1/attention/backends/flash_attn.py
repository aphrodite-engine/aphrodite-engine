...
class FlashAttentionMetadataBuilder(AttentionMetadataBuilder[FlashAttentionMetadata]):
    ...
    def _store_scheduler_metadata(self, scheduler_metadata: torch.Tensor | None) -> torch.Tensor | None:
    ...
    def __init__(
        ...
        if self.rswa_window is not None:
            max_num_reqs = aphrodite_config.scheduler_config.max_num_seqs
            self.persistent_rswa_prefix_lens = torch.zeros(max_num_reqs, dtype=torch.int32, device=self.device)
            self.persistent_rswa_window_tensor = torch.tensor([self.rswa_window], dtype=torch.int32, device=self.device)
        ...
        if self.model_config.is_mm_prefix_lm:
            ...
            self.mm_prefix_query_ranges_cpu = torch.empty((max_num_tokens, 2), dtype=torch.int32, pin_memory=PIN_MEMORY)
            self.mm_prefix_query_ranges_np = self.mm_prefix_query_ranges_cpu.numpy()
            self.mm_prefix_query_ranges_gpu = torch.empty((max_num_tokens, 2), dtype=torch.int32, device=self.device)
    ...
    def build(
        ...
        if self.aot_sliding_window is None:
            ...
            # in __init__.
            if aot_schedule:
                sliding_window_configs = _get_sliding_window_configs(self.aphrodite_config)
        ...
        if self.dcp_world_size > 1:
            ...
            if common_attn_metadata.seq_lens_cpu_upper_bound is not None:
                ...
                context_kv_lens_cpu = common_attn_metadata.seq_lens_cpu_upper_bound[:num_reqs] - query_lens_cpu
                skip_dcp_context_attention = should_skip_dcp_context_attention(context_kv_lens_cpu)
        ...
        if self.rswa_window is not None and common_attn_metadata.rswa_prefix_lens is not None:
...
class FlashAttentionImpl(AttentionImpl):
    ...
    def __init__(
        ...
        self.fa4_hd256 = self.flash_attn_version == 4 and uses_fa4_hd256_kernel(head_size)
        ...
        if is_quantized_kv_cache(self.kv_cache_dtype) and not flash_attn_supports_kv_cache_dtype(
            self.kv_cache_dtype,
            requires_alibi=alibi_slopes is not None,
            head_size=head_size,
            head_size_v=head_size,
            ...
            raise NotImplementedError(f"FlashAttention does not support {self.kv_cache_dtype} kv-cache on this device.")
        ...
        if aphrodite_config is not None and self.dcp_world_size > 1:
            self._dcp_dtype = aphrodite_config.model_config.dtype
            self._dcp_max_num_tokens = aphrodite_config.scheduler_config.max_num_batched_tokens
    ...
    def forward(
        ...
        # See aphrodite.utils.torch_utils.canonicalize_singleton_dim_strides.
        ...
        if not attn_metadata.use_cascade:
            ...
            if self.dcp_world_size > 1:
                ...
                sliding_window_size: list[int] | None = list(window) if window is not None else None
                ...
                if (
                    ...
                    sw_val = 1 + layer_window[0] if layer_window is not None and layer_window[0] >= 0 else None
                ...
                if isinstance(causal, torch.Tensor):
                    if self.flash_attn_version != 4:
                        raise NotImplementedError(
                            f"Per-sequence causal requires FA4. Current version: FA{self.flash_attn_version}"
                        )
                    dynamic_causal = causal
                    has_window = sliding_window_size is not None and sliding_window_size[1] >= 0
    ...
    def _forward_with_dcp(
        ...
        if split_dcp_context:
            ...
            assert self.flash_attn_version is not None
...
def _make_mm_prefix_mask_mod(
    ...
    from aphrodite.vllm_flash_attn.cute.utils import (  # type: ignore[import-untyped]
        scalar_to_ssa,
        ssa_to_scalar,
    )
...
def cascade_attention(
    ...
    prefix_output, prefix_lse = flash_attn_varlen_func(
        ...
        s_aux=s_aux,
        num_splits=1 if envs.APHRODITE_BATCH_INVARIANT else max_num_splits,
    )
