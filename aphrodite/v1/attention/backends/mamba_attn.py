...
class BaseMambaAttentionMetadataBuilder(AttentionMetadataBuilder[M], abc.ABC):
    ...
    def __init__(
        ...
        self.num_spec_tokens: int = aphrodite_config.num_speculative_tokens
        ...
        self.use_flashinfer_replayssm = (
            self.use_replayssm and aphrodite_config.mamba_config.backend == MambaBackendEnum.FLASHINFER
        )
        ...
        if self.aphrodite_config.cache_config.mamba_cache_mode == "all":
            ...
            if self.use_spec_decode:
                self.block_idx_last_scheduled_token_prev_step: torch.Tensor = torch.empty(
                    (self.decode_cudagraph_max_bs,),
                    dtype=torch.int32,
                    device=device,
                )
        ...
        self.decode_replayssm_scratch: tuple[torch.Tensor, torch.Tensor, torch.Tensor] | None = None
        ...
        if self.use_replayssm and not self.use_flashinfer_replayssm:
            ...
            bc_scratch_bs = max(self.decode_cudagraph_max_bs, scheduler_config.max_num_seqs)
    ...
    def build_for_cudagraph_capture(self, common_attn_metadata: CommonAttentionMetadata) -> M:
        ...
        if self.use_spec_decode and self.aphrodite_config.cache_config.mamba_cache_mode == "all":
    ...
    def _prefill_cpu_metadata(
        ...
        query_start_loc_p_cpu = common_attn_metadata.query_start_loc_cpu[-num_prefills - 1 :] - common.num_decode_tokens
        prefill_query_lens_cpu = query_start_loc_p_cpu[1:] - query_start_loc_p_cpu[:-1]
        num_computed_tokens_p_cpu = seq_lens_cpu[num_reqs - num_prefills : num_reqs] - prefill_query_lens_cpu
    ...
    def _build_chunk_metadata_tensors(
        ...
        num_computed_tokens_p_cpu, query_start_loc_p_cpu = self._prefill_cpu_metadata(common, common_attn_metadata)
        ...
        cu_chunk_seqlen_p = async_tensor_h2d(cu_chunk_seqlen, dtype=torch.int32, device=device)
        seq_idx_p = async_tensor_h2d(seq_idx, dtype=torch.int32, device=device)
        last_chunk_indices_p = async_tensor_h2d(last_chunk_indices, dtype=torch.int32, device=device)
    ...
    def _compute_common_metadata(
        ...
        *,
        ...
        if torch.any(prefill_to_decode).item():
            ...
            common_attn_metadata = common_attn_metadata.replace(is_prefilling=is_prefilling)
        ...
        if self.use_replayssm and not self.use_flashinfer_replayssm and num_decodes > 0:
            ...
            align_mode = self.aphrodite_config.cache_config.mamba_cache_mode == "align"
            block_size = self.kv_cache_spec.block_size
            effective_base = (
                torch.maximum(decode_base_d, (num_computed_d // block_size) * block_size)
                if align_mode
                else decode_base_d
            )
            ...
            is_flush_cpu = (write_pos_cpu == self.replayssm_buffer_len - 1) | leftover_prompt
            if align_mode:
                is_flush_cpu |= valid_decode_rows & ((num_computed_d + query_lens_cpu) % block_size == 0)
        ...
        bc_pre_scratch = None
        if self.use_replayssm and self.decode_bc_pre_scratch is not None and num_decodes > 0:
    ...
    def _update_metadata_for_cudagraph_capture(
        ...
        if (
            ...
            if self.aphrodite_config.cache_config.mamba_cache_mode == "all":
                ...
                block_idx_last_computed_token = self.block_idx_last_computed_token[:padded_bs]
                block_idx_last_computed_token[metadata.num_decodes :] = 0

                if self.use_spec_decode and block_idx_last_scheduled_token_prev_step is not None:
                    self.block_idx_last_scheduled_token_prev_step[: metadata.num_decodes].copy_(
                        block_idx_last_scheduled_token_prev_step[: metadata.num_decodes],
                        non_blocking=True,
                    )
                    block_idx_last_scheduled_token_prev_step = self.block_idx_last_scheduled_token_prev_step[:padded_bs]
            ...
            if self.use_replayssm and not self.use_flashinfer_replayssm:
                ...
                self.decode_write_pos_d[: metadata.num_decodes].copy_(
                    write_pos_d[: metadata.num_decodes], non_blocking=True
                )
                ...
                self.decode_is_flush_d[: metadata.num_decodes].copy_(
                    is_flush_d[: metadata.num_decodes], non_blocking=True
                )
