...
from aphrodite.v1.attention.ops.flashmla import FlashMLASchedMeta, get_mla_metadata
...
def _layer_type_for(compress_ratio: int) -> str:
    ...
    raise ValueError(f"Unsupported DeepseekV4 compress_ratio={compress_ratio}; expected 1, 4, or 128.")
...
class DeepseekSparseSWABackend(AttentionBackend):
    ...
    def get_builder_cls() -> type["DeepseekSparseSWAMetadataBuilder"]:
        if current_platform.is_rocm():
            from aphrodite.models.deepseek_v4.amd.rocm import (
...
class DeepseekSparseSWAMetadata:
    ...
    prefill_swa_indices: torch.Tensor | None = None  # [num_prefill_tokens, 1, prefill_index_width]
    ...
    flashinfer_sparse_index_cache: dict[str, tuple[torch.Tensor, torch.Tensor]] = field(default_factory=dict)

    def get_prefill_chunk_plan(self, compress_ratio: int, prefill_chunk_size: int) -> list[tuple[int, int, int, int]]:
        ...
        max_workspace_area = prefill_chunk_size * (
            (0 if compress_ratio <= 1 else cdiv(self.prefill_max_model_len, compress_ratio))
...
class ComputePrefillMetadataKernel(AphroditeTritonJitKernel["ComputePrefillMetadataKernel.CompileKey"]):
...
class DeepseekSparseSWAMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.max_model_len = self.aphrodite_config.model_config.max_model_len
        self.max_num_batched_tokens = self.aphrodite_config.scheduler_config.max_num_batched_tokens
        ...
        hf_config = self.aphrodite_config.model_config.hf_config
        ...
        max_tokens = self.aphrodite_config.scheduler_config.max_num_batched_tokens
        ...
        if self.max_image_tokens > 0:
            self.left_visible = torch.zeros(max_tokens, dtype=torch.int32, device=self.device)
            self.right_visible = torch.zeros(max_tokens, dtype=torch.int32, device=self.device)
            self.span_indptr = torch.zeros(
                self.aphrodite_config.scheduler_config.max_num_seqs + 1,
                dtype=torch.int32,
                device=self.device,
            )
            self.span_starts = torch.zeros(max_tokens, dtype=torch.int32, device=self.device)
            self.span_ends = torch.zeros(max_tokens, dtype=torch.int32, device=self.device)
    ...
    def build(
        ...
        The batch is assumed to be reordered with decodes first (by Aphrodite scheduler).
        ...
        token_to_req_indices = common_attn_metadata.token_to_req_indices(self.token_to_req_indices)
        ...
        if self.max_image_tokens > 0 and num_prefill_tokens > 0 and mm_ranges and any(mm_ranges.values()):
        ...
        return DeepseekSparseSWAMetadata(
            ...
            prefill_swa_indices=(self.prefill_swa_indices[:num_prefill_tokens] if num_prefill_tokens > 0 else None),
            prefill_swa_lens=(self.prefill_swa_lens[:num_prefill_tokens] if num_prefill_tokens > 0 else None),
            ...
            max_decode_query_len=min(common_attn_metadata.max_query_len, self.decode_threshold),
    ...
    def build_tile_scheduler(self, num_decode_tokens: int) -> dict[str, FlashMLASchedMeta | None]:
        ...
        for layer_type in self._layer_types:
            ...
            # call site aligned with the rest of the Aphrodite FlashMLA backends
    ...
    def _build_deepseek_v4_metadata(
        ...
        if num_prefills > 0:
            ...
            pfx_gather_lens = torch.empty(num_prefills, dtype=torch.int32, device=seq_lens.device)
...
def _compute_image_visibility_kernel(
    ...
    for i in range(span_lo, span_hi):
        ...
        left = tl.where(in_span, tl.minimum(pos - span_start, max_image_tokens - 1), left)
...
class ComputeSWAIndicesAndLensKernel(AphroditeTritonJitKernel["ComputeSWAIndicesAndLensKernel.CompileKey"]):
    @dataclass(frozen=True)
    class CompileKey:
        window_size: int
        index_width: int
    ...
    def get_warmup_keys(
        ...
        *,
        ...
        return self._trace_dispatch(self.dispatch)(
            zip_inputs(
                *(dict(index_width=index_width, has_image=has_image) for index_width, has_image in width_has_image)
            ),
...
class ComputeDSparkNoncausalSWAIndicesKernel(
    AphroditeTritonJitKernel["ComputeDSparkNoncausalSWAIndicesKernel.CompileKey"]
):
    @dataclass(frozen=True)
    class CompileKey:
        window_size: int
