...
def fit_kpool_indices_to_aiter(token_indices: torch.Tensor, topk_tokens: int) -> torch.Tensor:
    ...
    tail_values = torch.gather(tail, 1, tail_offsets.clamp(min=0, max=tail.shape[1] - 1))
...
class ROCMAiterMLASparseMetadataBuilder(AttentionMetadataBuilder[ROCMAiterMLASparseMetadata]):
    ...
    def __init__(
        ...
        attention_context = aphrodite_config.compilation_config.static_forward_context
        ...
        self._use_persistent_metadata = all(
            getattr(attention_context[name].impl, "sinks", None) is None for name in layer_names
        )
        ...
        self._num_attention_heads = AiterMLAHelper.get_actual_mla_num_heads(self.num_heads)
    ...
    def _sparse_decode_max_split(self, max_seq_len: int) -> int:
        ...
        ideal_splits = triton.next_power_of_2(max(1, effective_len // min_work_per_split))
    ...
    def build(
        ...
        self.req_id_per_token_buffer[:new_req_extent].copy_(np_to_pinned_tensor(req_id_per_token), non_blocking=True)
        ...
        if self._use_persistent_metadata and not use_triton_sparse:
            ...
            if metadata_key != self._prev_metadata_key:
                ...
                max_split_per_batch = self._sparse_decode_max_split(int(common_attn_metadata.max_seq_len))
                get_mla_metadata_v1(
                    qo_indptr,
                    paged_kv_indptr,
...
class ROCMAiterMLASparseImpl(MLAAttentionImpl[ROCMAiterMLASparseMetadata]):
        ...
        **mla_args,
        ...
        if sinks is not None:
            if sinks.dtype != torch.float32:
                raise ValueError(f"ROCm AITER MLA sinks must be float32, got {sinks.dtype}")
            if sinks.ndim != 1 or sinks.numel() != num_heads:
                raise ValueError(f"ROCm AITER MLA sinks must have shape ({num_heads},), got {tuple(sinks.shape)}")
        ...
        (self.q_concat_buffer,) = current_workspace_manager().get_simultaneous(
            (q_concat_shape, aphrodite_config.model_config.dtype),
        )
    ...
    def _forward_mla(
        ...
        from aphrodite.platforms.rocm import on_gfx942
        ...
        triton_sink_fallback = (
            ...
            and (
                ...
                or (q.dtype == torch.bfloat16 and (mla_num_heads > 128 or (on_gfx942() and 32 < mla_num_heads <= 64)))
            )
        )
        ...
        head_dtype_name = ""
        if need_lse:
            if q.dtype == torch.bfloat16 and kv_c_and_k_pe_cache.dtype == torch.bfloat16:
                supported_head_buckets = (16, 32, 64, 128)
                head_dtype_name = "BF16"
            elif q.dtype == current_platform.fp8_dtype() and kv_c_and_k_pe_cache.dtype == current_platform.fp8_dtype():
        ...
        if supported_head_buckets is not None:
            ...
            if on_mi3xx():
                supported_heads = next(
                    (heads for heads in supported_head_buckets if heads >= mla_num_heads),
                    None,
                )
        ...
        output = AiterMLAHelper.get_mla_unpadded_o(self.num_heads, output)
        if lse is not None:
            lse = AiterMLAHelper.get_mla_unpadded_o(self.num_heads, lse.unsqueeze(-1)).squeeze(-1)
        ...
        if self.sinks is not None:
            ...
            has_keys = (attn_metadata.paged_kv_indptr[1:] > attn_metadata.paged_kv_indptr[:-1]).unsqueeze(-1)
    ...
    def forward_mqa(
        ...
        topk_indices = fit_kpool_indices_to_aiter(self.topk_indices_buffer[:num_actual_toks], attn_metadata.topk_tokens)
        ...
        return self._forward_mla(layer, mla_padded_q, kv_c_and_k_pe_cache, attn_metadata)
