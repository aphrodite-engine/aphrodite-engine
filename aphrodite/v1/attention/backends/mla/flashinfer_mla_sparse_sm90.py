...
from aphrodite.utils.flashinfer import has_flashinfer_sm90_nope_mla
...
class FlashInferMLASparseSM90Backend(AttentionBackend):
    ...
    def supports_combination(
        ...
        if aphrodite_config.model_config is not None:
            hf = aphrodite_config.model_config.hf_text_config
...
class _SM90State:
    ...
    def __init__(
        ...
        self.kv_indices = torch.zeros(max_tokens * topk_width, dtype=torch.int32, device=device)
        self.kv_len_arr = torch.full((max_tokens,), topk_width, dtype=torch.int32, device=device)
        ...
        self._lens_cpu = torch.full((self.max_tokens,), self.topk_width, dtype=torch.int32)
...
class FlashInferMLASparseSM90Builder(FlashInferMLASparseMetadataBuilder):
    ...
    def __init__(
        ...
        if not isinstance(impl, FlashInferMLASparseSM90Impl):
            raise TypeError(
                f"FlashInferMLASparseSM90Builder requires an SM90 FlashInfer implementation, got {type(impl).__name__}."
            )
        ...
        self._async_scheduling = bool(aphrodite_config.scheduler_config.async_scheduling)
    ...
    def _kv_lens_host(self, cam: CommonAttentionMetadata) -> tuple[int, torch.Tensor]:
        ...
        if not self._async_scheduling and sl_host is not None:
            ...
            req_of_row = torch.repeat_interleave(torch.arange(num_reqs, dtype=torch.int64), q_lens.to(torch.int64))
            rows = torch.arange(num_rows, dtype=torch.int32)
            ctx = first_pos.to(torch.int64)[req_of_row] + rows.to(torch.int64) - qsl.to(torch.int64)[req_of_row] + 1
...
class FlashInferMLASparseSM90Impl(SparseMLACommonImpl[FlashInferMLASparseSM90Metadata]):
        ...
        **mla_args: Any,
        ...
        if any([alibi_slopes, sliding_window, logits_soft_cap]):
            raise NotImplementedError(
                "FlashInferMLASparseSM90Impl does not support alibi, sliding window, or logits soft cap."
            )
        super().__init__(
            num_heads,
            head_size,
    ...
    def forward_mqa(
        ...
        if not isinstance(q, tuple):
            raise NotImplementedError("FlashInferMLASparseSM90Impl expects split (q_nope, q_rope).")
        q_nope, q_rope = q
        ...
        state.kv_indices[: num_tokens * width].copy_(topk_slots.reshape(-1).clamp_(min=0).to(torch.int32))

        flat = (
            kv_c_and_k_pe_cache.view(torch.float8_e4m3fn) if self.use_fp8_kv_cache else kv_c_and_k_pe_cache
        ).reshape(-1, 1, self.head_size)
        ckv = flat[..., : self.kv_lora_rank]
        kpe = flat[..., self.kv_lora_rank :]

        scale_kwargs = (
            {"ckv_scale": float(layer._k_scale_float or 1.0), "kpe_scale": 1.0} if self.use_fp8_kv_cache else {}
        )
