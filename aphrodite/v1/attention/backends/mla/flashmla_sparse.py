...
from aphrodite.model_executor.layers.attention.mla_attention import MLACommonPrefillMetadata
...
QUANTIZED_DS_MLA_CACHE_FORMATS: frozenset[str] = frozenset({"fp8_ds_mla", "nvfp4_ds_mla"})
...
class FlashMLASparseBackend(AttentionBackend):
    ...
    def supports_combination(
        ...
        if kv_cache_dtype == "nvfp4_ds_mla" and device_capability.major != 10:
            return f"FLASHMLA_SPARSE only supports the {kv_cache_dtype} kv-cache dtype on SM100 (Blackwell)"
        return None
...
class FlashMLASparseMetadataBuilder(SparseMLACommonMetadataBuilder[FlashMLASparseMetadata]):
    ...
    def __init__(
        ...
        if current_platform.is_device_capability_family(100):
            threshold = {8: 128, 16: 128, 32: 128, 64: 256, 128: 1024}.get(num_q_heads, 1024)
        else:
        ...
        self.use_fp8_kv_cache = cache_config.cache_dtype in QUANTIZED_DS_MLA_CACHE_FORMATS
        ...
        if parallel_config.decode_context_parallel_size > 1:
            ...
            # DCP-gathered heads.
            gathered_num_heads = self.num_heads * parallel_config.decode_context_parallel_size
            gathered_padded_heads = FlashMLASparseImpl._compute_fp8_decode_padded_heads(gathered_num_heads)
    ...
    def build(
        ...
        if self.use_fp8_kv_cache:
            if self.fp8_use_mixed_batch:
                metadata.fp8_extra_metadata = self._build_fp8_mixed_decode_prefill(common_attn_metadata)
            else:
                metadata.fp8_extra_metadata = self._build_fp8_separate_prefill_decode(common_attn_metadata, metadata)
...
class FlashMLASparseImpl(SparseMLACommonImpl[FlashMLASparseMetadata]):
    ...
    def __init__(
        ...
        **mla_args,
        ...
        if not is_quantized_kv_cache(kv_cache_dtype):
            q_concat_heads = (num_heads + self.prefill_padding - 1) // self.prefill_padding * self.prefill_padding
        ...
        if self.need_to_return_lse_for_decode and not is_quantized_kv_cache(kv_cache_dtype):
            raise NotImplementedError(
    ...
    def _bf16_flash_mla_kernel(
        ...
        padded_num_heads = (actual_num_heads + self.prefill_padding - 1) // self.prefill_padding * self.prefill_padding
        if q.shape[1] < padded_num_heads:
            logger.warning_once(
                f"Padding num_heads from {actual_num_heads} to {padded_num_heads} for BF16 sparse prefill kernel"
            )
    ...
    def forward_mqa(
        ...
        if not use_fp8_cache:
            ...
            attn_out, lse = self._forward_fp8_kv_mixed_batch(q, kv_c_and_k_pe_cache, topk_indices, attn_metadata)
        else:
