...
def flat_kv_row_view(
    ...
    assert kv_cache.stride(0) % head_dim == 0, (
        "block stride is not a whole number of rows; flat row indexing would silently misaddress"
    )
...
class ConvertReqIndexToGlobalIndexKernel(AphroditeTritonJitKernel["ConvertReqIndexToGlobalIndexKernel.CompileKey"]):
    ...
    def kernel(
        ...
        # Prefill rows index the gathered workspace, which spans the global
        # prompt even when the block table contains only this DCP rank's shard.
    ...
    def get_warmup_keys(
        ...
        *,
        ...
        dcp_interleave = aphrodite_config.parallel_config.cp_kv_cache_interleave_size
        dcp_rank = get_dcp_group().rank_in_group if dcp_size > 1 else 0
        num_topk_tokens = aphrodite_config.model_config.hf_config.index_topk
        max_num_blocks = cdiv(
    ...
    def warmup_inputs(self, compile_key: CompileKey) -> dict[str, Any]:
        ...
        return dict(
            ...
            prefill_workspace_request_ids=(int32_ptr if compile_key.has_prefill_workspace else None),
            prefill_workspace_starts=(int32_ptr if compile_key.has_prefill_workspace else None),
    ...
    def __call__(
        ...
        *,
        ...
        single_tile, block_n, tiles_per_row, num_warps = _remap_tiling(NUM_TOPK_TOKENS, BLOCK_N, COUNT_VALID)
...
def _remap_tiling(NUM_TOPK_TOKENS: int, BLOCK_N: int, count_valid: bool) -> tuple[bool, int, int, int]:
    ...
    """
    single_tile = count_valid and triton.next_power_of_2(NUM_TOPK_TOKENS) == NUM_TOPK_TOKENS
    if single_tile:
...
def triton_convert_req_index_to_global_index(
    ...
    _CONVERT_REQ_INDEX_TO_GLOBAL_INDEX_KERNEL(
        ...
        max_num_blocks_per_req=max_num_blocks_per_req,
        BLOCK_SIZE=BLOCK_SIZE,
        BLOCK_STRIDE_ROWS=(BLOCK_STRIDE_ROWS if BLOCK_STRIDE_ROWS is not None else BLOCK_SIZE),
        BLOCK_N=BLOCK_N,
        NUM_TOPK_TOKENS=NUM_TOPK_TOKENS,
        HAS_PREFILL_WORKSPACE=HAS_PREFILL_WORKSPACE,
