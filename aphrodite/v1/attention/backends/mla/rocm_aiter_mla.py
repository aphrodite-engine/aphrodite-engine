...
from aphrodite.utils.math_utils import cdiv, largest_power_of_2_divisor
...
from aphrodite.v1.attention.ops.rocm_aiter_mla_merge import (
    merge_mla_segments_triton,
)
from aphrodite.v1.kv_cache_interface import AttentionSpec, is_quantized_kv_cache
...
def _aiter_mla_native_h24_metadata_supported() -> bool:
    ...
    try:
        ...
        metadata_source = Path(AITER_CSRC_DIR) / "kernels" / "mla" / "metadata" / "v1_2_device.cuh"
...
def _aiter_mla_native_h24_supported() -> bool:
    ...
    return _aiter_mla_native_h24_reducer_supported() and _aiter_mla_native_h24_metadata_supported()
...
def _segmented_dcp_verify_supported(dcp_world_size: int, cp_interleave: int) -> bool:
    ...
    """
    return dcp_world_size > 1 and cp_interleave == 1 and _segmented_mla_decode_supported()


def _aiter_mla_small_head_mode() -> str:
    """Small-head (<16) MLA decode kernel selection.

    Controlled by ``APHRODITE_ROCM_AITER_MLA_ASM_PADDING``:
    ...
    mode = (envs.APHRODITE_ROCM_AITER_MLA_ASM_PADDING or "auto").lower()
    if mode == "gluon" and not _gluon_mla_decode_supported():
        logger.warning_once(
            "APHRODITE_ROCM_AITER_MLA_ASM_PADDING=gluon requested, but this device "
...
class AiterMLAMetadataBuilder(MLACommonMetadataBuilder[AiterMLAMetadata]):
    ...
    def __init__(
        ...
        # kernel_block_size.
        max_num_pages_per_req = aphrodite_config.model_config.max_model_len
        ...
        self._num_attention_heads = AiterMLAHelper.get_actual_mla_num_heads(self._decode_num_heads)
        kv_cache_dtype_str = getattr(aphrodite_config.cache_config, "cache_dtype", "auto")
        ...
        q_dtype = dtypes.fp8 if kv_cache_dtype_str == "fp8" else self.decode_attn_out_dtype
        ...
        self._fp8_prefill_enabled = _fp8_mla_prefill_supported() and (
            kv_cache_dtype_str == "fp8"
            and aphrodite_config.model_config.dtype == torch.bfloat16
        ...
        if self.compilation_config.cudagraph_mode.has_full_cudagraphs():
            ...
            self._graph_seq_lens = torch.zeros(max_num_reqs, dtype=torch.int32, device=device)
            ...
            if self._supports_segmented_dcp_verify and self._mtp_decode_qlen > 1:
                ...
                graph_max_kv_seq_len = (
                    cdiv(aphrodite_config.model_config.max_model_len, num_dcp_partitions)
                ...
                self._dcp_verify_buffers = AiterMLADCPVerifyMetadata(
                    row_lens=torch.zeros(max_verify_rows, dtype=torch.int32, device=device),
                    block_table=torch.zeros(
                        (max_verify_rows, max_local_pages),
                        dtype=torch.int32,
                        device=device,
                    ),
                    qo_indptr=torch.arange(max_verify_rows + 1, dtype=torch.int32, device=device),
    ...
    def _init_fp8_prefill_ps_buffers(
        ...
        self.fp8_ps_work_metadata = torch.empty(work_metadata_size, dtype=work_metadata_dtype, device=device)
        self.fp8_ps_work_indptr = torch.empty(work_indptr_size, dtype=work_indptr_dtype, device=device)
        self.fp8_ps_work_info = torch.empty(*work_info_size, dtype=work_info_dtype, device=device)
        self.fp8_ps_reduce_indptr = torch.empty(reduce_indptr_size, dtype=reduce_indptr_dtype, device=device)
        self.fp8_ps_reduce_final_map = torch.empty(*reduce_final_map_size, dtype=reduce_final_map_dtype, device=device)
        ...
        if self.num_heads < num_head_k:
            ...
            reservations.append(((max_num_batched_tokens, num_head_k, v_head_dim), attn_out_dtype))
        current_workspace_manager().get_simultaneous(*reservations)

        logger.info(
            "FP8 MLA prefill PS buffers allocated (max_batch=%d, max_qlen=%d, num_head_k=%d)",
            max_num_reqs,
            max_prefill_qlen,
            num_head_k,
        )
    ...
    def _build_fp8_prefill_ps_metadata(
        ...
        kv_indices = torch.arange(total_prefill_tokens, device=qo_indptr.device, dtype=torch.int32)
    ...
    def _build_dcp_verify_row_view(
        ...
        max_kv_seq_len = buffers.max_kv_seq_len if buffers is not None else max(1, int(row_lens.max().item()))
        page_size = self._segmented_page_size
    ...
    def _fill_dcp_verify_page_table(
        ...
        per_req_page_table = (
            block_table[:num_reqs, :max_local_blocks, None] * pages_per_block + subpage_offsets
        ...
        row_block_table.unflatten(0, (num_reqs, qlen)).copy_(per_req_page_table.unsqueeze(1))
    ...
    def _build_decode(
        ...
        padded_mtp_qo_len = self._uniform_padded_mtp_qo_len(qo_len, max_qo_len, num_decode_tokens)
        ...
        if pad_uniform_mtp:
            qo_lens_device = (query_start_loc_device[1 : num_reqs + 1] - query_start_loc_device[:num_reqs]).to(
                torch.int32
            )
        ...
        if self._graph_seq_lens is not None:
            self._graph_seq_lens[:num_kernel_reqs].copy_(seq_lens_for_kernel, non_blocking=True)
        ...
        use_segmented_dcp_verify = self._supports_segmented_dcp_verify and max_qo_len > 1
        ...
        if self.compilation_config.cudagraph_mode.has_full_cudagraphs():
            self.paged_kv_indptr[: 1 + num_kernel_reqs].copy_(paged_kv_indptr, non_blocking=True)
            ...
            if pad_uniform_mtp:
                qo_indptr_src = AiterMLAHelper.qo_indptr_for_uniform_qlen(num_kernel_reqs, int(max_qo_len), device)
            else:
                qo_indptr_src = query_start_loc_device[: 1 + num_kernel_reqs]
            self.qo_indptr[: 1 + num_kernel_reqs].copy_(qo_indptr_src, non_blocking=True)
            self.qo_indptr[1 + num_kernel_reqs :] = qo_indptr_src[-1]
            qo_indptr = self.qo_indptr[: 1 + num_kernel_reqs]

        else:
            if max_qo_len == 1:
                qo_indptr = AiterMLAHelper.qo_indptr_for_uniform_qlen(num_kernel_reqs, 1, device)
            else:
                if pad_uniform_mtp:
                    qo_indptr = AiterMLAHelper.qo_indptr_for_uniform_qlen(num_kernel_reqs, int(max_qo_len), device)
                else:
        ...
        if use_persistent_metadata:
            ...
            uni_qo_len = max_qo_len if pad_uniform_mtp or torch.all(qo_len == max_qo_len) else -1
            get_mla_metadata_v1(
                qo_indptr,
                paged_kv_indptr,
...
class AiterMLAHelper:
    ...
    @staticmethod
    def get_mla_padded_q(num_heads: int, q: torch.Tensor, target_heads: int | None = None) -> torch.Tensor:
        m = target_heads if target_heads is not None else AiterMLAHelper.get_actual_mla_num_heads(num_heads)
        if num_heads == m:
            return q
        if m % num_heads == 0:
    ...
    def use_gluon_decode(num_heads: int, max_qo_len: int, kv_cache_dtype: str) -> bool:
        ...
        # APHRODITE_ROCM_AITER_MLA_ASM_PADDING and the arch (Gluon is gfx950 only).
        m = AiterMLAHelper._AITER_MIN_MLA_HEADS
...
class AiterMLAImpl(MLACommonImpl[AiterMLAMetadata]):
    ...
    def __init__(
        ...
        **mla_args,
        ...
        self._fp8_prefill_enabled = _fp8_mla_prefill_supported() and (
            is_quantized_kv_cache(kv_cache_dtype) and AiterMLAHelper.is_valid_num_heads(self.num_heads)
        )
        if self._fp8_prefill_enabled:
    ...
    def forward_mha(
        ...
        assert output_scale is None, "fused FP8 output not supported by the AITER FP8 MLA prefill path"
    ...
    def _forward_segmented_dcp_verify(
        ...
        assert isinstance(segment_partials, tuple) and len(segment_partials) == 3, (
            "AITER segmented MLA verify must return segment partials when skip_reduce=True."
        )
    ...
    def forward_mqa(
        ...
        if decode.use_gluon_verify:
            ...
            kv_buffer = kv_c_and_k_pe_cache.reshape(-1, kv_c_and_k_pe_cache.shape[-1])
            assert attn_metadata.causal, "AITER MLA small-head verify MTP is causal-only"
            ...
            num_reqs = B // qlen
            if num_reqs * qlen != B:
                raise ValueError(f"verify block {B} rows is not a multiple of qlen {qlen}")
            assert decode.paged_kv_indptr is not None
            assert decode.paged_kv_indices is not None
            mla_gluon = _get_mla_gluon()
            mla_gluon(
        ...
        if verify is not None:
            if type(q) is tuple:
                ...
                q_nope, q_pe = torch.split(q, [self.kv_lora_rank, self.qk_rope_head_dim], dim=-1)
            if is_quantized_kv_cache(self.kv_cache_dtype) and q_nope.dtype != torch.bfloat16:
        ...
        if self.dcp_world_size > 1 and int(decode.max_qo_len) > 1:
            raise RuntimeError("ROCM_AITER_MLA DCP multi-token verify requires segmented MLA.")
        ...
        if self.dcp_world_size > 1:
            # The Aphrodite custom-op wrapper exposes only the in-place output and
