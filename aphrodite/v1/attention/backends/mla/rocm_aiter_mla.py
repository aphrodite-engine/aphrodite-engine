...
class AiterMLAMetadataBuilder(MLACommonMetadataBuilder[AiterMLAMetadata]):
    ...
    def __init__(
        ...
        self._fp8_prefill_enabled = _fp8_mla_prefill_supported() and self.num_heads % 16 == 0
    ...
    def _build_decode(
        ...
        use_gluon_decode = AiterMLAHelper.use_gluon_decode(self.num_heads, int(max_qo_len))
        ...
        _expand_page_indices_kernel[(num_kernel_reqs,)](
        ...
        if self.compilation_config.cudagraph_mode.has_full_cudagraphs():
            ...
            if max_qo_len == 1:
                qo_indptr = torch.arange(0, num_kernel_reqs + 1, step=1, dtype=torch.int32, device=device)
            elif pad_uniform_mtp:
...
class AiterMLAHelper:
    ...
    _AITER_UNSUPPORTED_HEADS: ClassVar[tuple[int, ...]] = (32,)
    ...
    def is_valid_num_heads(num_heads: int) -> bool:
        return num_heads > 0 and (
            num_heads < AiterMLAHelper._AITER_MIN_MLA_HEADS or num_heads % AiterMLAHelper._AITER_MIN_MLA_HEADS == 0
        )
...
class AiterMLAImpl(MLACommonImpl[AiterMLAMetadata]):
    ...
    def forward_mqa(
        ...
        if self.num_heads < AiterMLAHelper._AITER_MIN_MLA_HEADS and int(decode.max_qo_len) > 1:
            ...
            dev = q_nope.device
            row_req = torch.arange(per_req_len.shape[0], device=dev).repeat_interleave(qlen)
            row_len = per_req_len[row_req]
            new_indptr = torch.cat([old_indptr.new_zeros(1), row_len.cumsum(0)]).to(torch.int32)
            total = int(new_indptr[-1].item())
            within = torch.arange(total, device=dev, dtype=torch.int64) - new_indptr[:-1].to(
                torch.int64
            ).repeat_interleave(row_len)
