...
``csrc/cpu/sgl-kernels/``, plugged into Aphrodite's ``MLACommonBackend``/
...
(``aphrodite/v1/attention/backends/mla/cpu_mla.py``): that one targets every CPU
...
from aphrodite.v1.attention.backend import AttentionLayer, AttentionType, MultipleOf
...
def _expand_block_table(block_table: torch.Tensor, block_size: int) -> torch.Tensor:
    """Adapter: Aphrodite's block-table paging -> the flat per-(request, position)
    physical row index the decode/extend kernels expect. Called once per step
    from ``AMXMLAMetadataBuilder.build``, not per layer.
    """
    offsets = torch.arange(block_size, device=block_table.device, dtype=block_table.dtype)
...
class AMXMLAImpl(MLACommonImpl[MLACommonMetadata]):
        ...
        **mla_args,
        ...
        if any(unsupported_features):
            raise NotImplementedError(
                "AMXMLAImpl does not support one of the following: alibi_slopes, sliding_window, logits_soft_cap"
            )
        if attn_type != AttentionType.DECODER:
            raise NotImplementedError(
                "Encoder self-attention and encoder/decoder cross-attention are not implemented for AMXMLAImpl"
            )
    ...
    def do_kv_cache_update(
        ...
        assert kv_cache_dtype == "auto", (
            "AMXMLAImpl only supports an unquantized (bf16) KV cache; fp8 KV cache for CPU MLA is not yet implemented."
        )
        ops.amx_mla_concat_and_cache(kv_c_normed, k_pe.squeeze(1), kv_cache, slot_mapping.flatten())
    ...
    def forward_mha(
        ...
        assert output_scale is None, "AMXMLAImpl.forward_mha does not support fused output quantization"
        prefill = attn_metadata.prefill
        ...
        ql_nope = torch.empty(num_tokens, num_heads, self.kv_lora_rank, dtype=q.dtype, device=q.device)
        ...
        k_extend = torch.cat([kv_c_normed, k_pe.reshape(num_tokens, self.qk_rope_head_dim)], dim=-1).unsqueeze(1)
        v_extend = k_extend[..., : self.kv_lora_rank]

        attn_out = torch.empty(num_tokens, num_heads, self.kv_lora_rank, dtype=q.dtype, device=q.device)
        ops.cpu_mla_extend(
            mqa_q,
            k_extend,
            v_extend,
        ...
        output_view = output.view(num_tokens, num_heads, self.v_head_dim).transpose(0, 1)
