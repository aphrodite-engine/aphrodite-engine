...
def _get_multi_ctas_kv_counter_buffer(min_bytes: int, device: torch.device) -> torch.Tensor:
    ...
    if _fi_multi_ctas_kv_counter is None or _fi_multi_ctas_kv_counter.numel() < min_bytes:
        _fi_multi_ctas_kv_counter = torch.zeros(min_bytes, dtype=torch.uint8, device=device)
...
class FlashInferMLAMetadataBuilder(MLACommonMetadataBuilder[MLACommonMetadata]):
    ...
    def __init__(
        ...
        if dcp_size > 1 and interleave_size != 1:
            raise ValueError(
                f"FlashInfer MLA native DCP requires cp_kv_cache_interleave_size=1; got {interleave_size}."
            )
        super().__init__(
            kv_cache_spec,
            layer_names,
    ...
    def forward_mqa(
        ...
        if self.dcp_world_size > 1:
            ...
            if not attn_metadata.causal and query_len > 1:
                causal_seqlens_kv_global = causal_seqlens_kv_global.repeat_interleave(query_len)
            extra_kwargs.update(
                enable_dcp=True,
        ...
        if decode_backend:
            ...
            extra_kwargs["multi_ctas_kv_counter_buffer"] = _get_multi_ctas_kv_counter_buffer(
