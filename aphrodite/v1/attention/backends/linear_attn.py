...
class BailingLinearAttentionMetadataBuilder(LinearAttentionMetadataBuilder):
    ...
    def __init__(
        ...
        super().__init__(kv_cache_spec, layer_names, aphrodite_config, device)
        self.compilation_config: CompilationConfig = aphrodite_config.compilation_config
        self.num_spec_tokens: int = aphrodite_config.num_speculative_tokens
        self.use_spec_decode: bool = self.num_spec_tokens > 0
        self.decode_cudagraph_max_bs: int = aphrodite_config.scheduler_config.max_num_seqs
    ...
    def build(  # type: ignore[override]
        ...
        *,
        ...
        use_full_cudagraph = cudagraph_mode is not None and cudagraph_mode.has_full_cudagraphs()
        if num_prefills == 0 and num_decodes <= self.decode_cudagraph_max_bs and use_full_cudagraph:
            padded_bs = num_reqs
            ...
            self.decode_legacy_state_indices_tensor[:num_decodes].copy_(
                torch.where(
                    is_padded_decode,
                    torch.full_like(legacy_state_indices_tensor[:num_decodes], PAD_SLOT_ID),
                    legacy_state_indices_tensor[:num_decodes],
                ),
                non_blocking=True,
            )
            legacy_state_indices_tensor = self.decode_legacy_state_indices_tensor[:padded_bs]
