...
from aphrodite.utils import cdiv, is_pin_memory_available
from aphrodite.utils.flashinfer import use_trtllm_attention
from aphrodite.v1.attention.backends.flash_attn import use_cascade_attention
...
class FlashInferBackend(AttentionBackend):
    ...
    @staticmethod
    def get_name() -> str:
        return "FLASHINFER_APHRODITE_V1"
...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def __init__(self, kv_cache_spec: AttentionSpec, layer_names: list[str],
        ...
        max_num_pages_per_req = cdiv(aphrodite_config.model_config.max_model_len,
    ...
    def _get_decode_wrapper(self,
        ...
        if decode_wrapper is None:
            num_qo_heads = (
                self.aphrodite_config.model_config.get_num_attention_heads(
                    self.aphrodite_config.parallel_config))
            num_kv_heads = self.aphrodite_config.model_config.get_num_kv_heads(
                self.aphrodite_config.parallel_config)
    ...
    def _plan(self, attn_metadata: FlashInferMetadata):
        if attn_metadata.use_cascade:
            ...
            if num_decodes > 0:
                ...
                if use_cudagraph:
                    num_input_tokens = (
                        self.aphrodite_config.pad_for_cudagraph(num_decodes))
    ...
    def build(self,
        ...
        num_qo_heads = self.aphrodite_config.model_config.get_num_attention_heads(
            self.aphrodite_config.parallel_config)
        ...
        attn_metadata = FlashInferMetadata(
            ...
            q_data_type=self.aphrodite_config.model_config.dtype,
...
def fast_plan_decode(
    ...
    if not self.is_cuda_graph_enabled or \
        getattr(self, "aphrodite_first_call", True):
        self.plan(
            indptr_cpu,
            indices,
            last_page_len_cpu,
