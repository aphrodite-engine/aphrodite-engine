...
from aphrodite.v1.attention.ops.merge_attn_states import merge_attn_states
...
def _get_trtllm_workspace_buffer():
    ...
    if trtllm_workspace_buffer is None:
        trtllm_workspace_buffer = torch.zeros(
            envs.APHRODITE_FLASHINFER_WORKSPACE_BUFFER_SIZE, dtype=torch.uint8, device="cuda"
        )
    return trtllm_workspace_buffer


def _pack_draft_block_bool_mask(bool_mask: torch.Tensor, num_packed: int) -> torch.Tensor:
    ...
    mask_u32 = (bool_mask.view(num_rows, num_packed, 32).to(torch.int64) * bits).sum(dim=-1).to(torch.uint32)
    return mask_u32.view(torch.uint16).reshape(num_rows, num_packed * 2)


def _make_xqa_draft_block_mask(q_len: int, causal: bool, device: torch.device) -> torch.Tensor:
...
def _trtllm_prefill_attn_kvfp8_dequant(
    ...
    orig_page_num = tl.load(block_tables_prefill_ptr + batch_idx * block_table_stride + mock_block_table_idx).to(
        tl.int64
    )
    if orig_page_num <= 0:
        return
    ...
    for h in range(NUM_KV_HEADS):
        ...
        dst_v = mock_page_idx * DST_KV_CACHE_STRIDE + DST_K_CACHE_STRIDE + h * HEAD_STRIDE + logical_offsets
...
class BatchDCPPrefillWrapper:
    ...
    def run(
        ...
        prefill_query_across_dcp = get_dcp_group().all_gather(prefill_query.contiguous(), dim=1)
...
class FlashInferBackend(AttentionBackend):
    ...
    def get_supported_kernel_block_sizes() -> list[int | MultipleOf]:
        ...
        if aphrodite_config is not None and aphrodite_config.model_config is not None:
            pc = aphrodite_config.parallel_config
            mc = aphrodite_config.model_config
    ...
    def supports_compute_capability(cls, capability: DeviceCapability) -> bool:
        ...
        return capability >= DeviceCapability(8, 0) and capability <= DeviceCapability(12, 1)
...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def __init__(
        ...
        self._prefill_wrapper: BatchPrefillWithPagedKVCacheWrapper | BatchDCPPrefillWrapper | None = (
        ...
        max_num_pages_per_req = cdiv(self.model_config.max_model_len, self.kv_cache_spec.block_size)
        ...
        num_spec_tokens = speculative_config.num_speculative_tokens if speculative_config is not None else 0
        self.enable_cuda_graph = self.compilation_config.cudagraph_mode.decode_mode() == CUDAGraphMode.FULL
        ...
        try:
            ...
            self.dcp_kv_cache_interleave_size = aphrodite_config.parallel_config.dcp_kv_cache_interleave_size
        ...
        self.dcp_a2a = self.use_dcp and aphrodite_config.parallel_config.dcp_comm_backend == "a2a"
        ...
        ) or self.model_config.get_num_attention_heads(self.aphrodite_config.parallel_config)
        ...
        if self.kv_cache_spec.kv_quant_mode != KVQuantMode.NONE:
            ...
            if self.is_kvcache_nvfp4:
                if (
                    ...
                    raise ValueError(
                        f"--kv-cache-dtype {self.cache_dtype} requires the SM100 trtllm-gen FlashInfer path."
                    )
        ...
        self.flashinfer_trtllm_api_decode_kernel: FlashInferDecodeKernel | None = (
            self._get_flashinfer_trtllm_api_decode_kernel() if can_use_xqa_or_trtllm_gen_decode else None
        )
        ...
        if self.flashinfer_trtllm_api_decode_kernel == FlashInferDecodeKernel.XQA and not (
            16 <= self.head_dim <= 256 and self.head_dim % 16 == 0
        ):
            logger.warning_once(
        ...
        if self.use_dcp and self.flashinfer_trtllm_api_decode_kernel == FlashInferDecodeKernel.XQA:
            logger.warning_once(
        ...
        self.use_trtllm_gen_varlen_decode = (
            ...
            and self.flashinfer_trtllm_api_decode_kernel == FlashInferDecodeKernel.TRTLLM_GEN
            and not self.use_dcp
        )
        supports_spec_as_decode = (
            self.flashinfer_trtllm_api_decode_kernel == FlashInferDecodeKernel.TRTLLM_GEN or self.use_dedicated_xqa
        )
        ...
        per_layer_parameters = get_per_layer_parameters(aphrodite_config, layer_names, FlashInferImpl)
        ...
        logger.info_once(
            "FlashInfer resolved query dtypes: prefill=%s, decode=%s, decode_backend=%s, kv_cache_dtype=%s, arch=%s",
        ...
        self.paged_kv_indptr = CpuGpuBuffer(max_num_reqs + 1, dtype=torch.int32, device=self.device, pin_memory=False)
        self.paged_kv_indices = torch.zeros(max_num_pages, dtype=torch.int32, device=self.device)
    ...
    def get_q_data_type(self, is_prefill: bool) -> torch.dtype:
        ...
        if (
            (current_platform.is_device_capability(90) or current_platform.is_device_capability_family(120))
            and not is_prefill
        ...
        if cache_dtype.startswith("fp8"):
            ...
            if current_platform.is_device_capability(90) or current_platform.is_device_capability_family(100):
    ...
    def get_cudagraph_support(
        ...
        if is_sm12x and aphrodite_config.parallel_config.decode_context_parallel_size > 1:
            return AttentionCGSupport.UNIFORM_SINGLE_TOKEN_DECODE

        kv_specs = iter_layer_specs(kv_cache_spec)
        num_qo_heads = aphrodite_config.model_config.get_num_attention_heads(aphrodite_config.parallel_config)
        ...
        if has_trtllm_support and (is_sm12x or not aphrodite_config.attention_config.use_non_causal):
    ...
    def _get_workspace_buffer(self):
        if self._workspace_buffer is None:
            buffer_size = envs.APHRODITE_FLASHINFER_WORKSPACE_BUFFER_SIZE
            if envs.APHRODITE_BATCH_INVARIANT:
            ...
            self._workspace_buffer = torch.zeros(buffer_size, dtype=torch.uint8, device=self.device)
    ...
    def _get_flashinfer_trtllm_api_decode_kernel() -> FlashInferDecodeKernel:
        if current_platform.is_device_capability(90) or current_platform.is_device_capability_family(120):
    ...
    def _get_decode_mask(
        ...
        if ragged_q_lens is None:
            ...
            if buf is None:
                ...
                buf = per_req.unsqueeze(0).expand(self.max_num_reqs, -1, -1).contiguous()
                self._decode_mask_cache[key] = buf
            return buf[:num_decodes]

        return _make_xqa_ragged_draft_block_mask(ragged_q_lens, q_len_per_req, causal, self.device)
    ...
    def _get_prefill_wrapper(
        ...
        if not causal:
            if self.use_dcp:
                raise NotImplementedError("FlashInfer non-causal prefill is not supported with DCP yet.")
            if self.is_kvcache_nvfp4:
                raise NotImplementedError("FlashInfer non-causal attention is not supported with NVFP4 KV cache.")
            if self._noncausal_prefill_wrapper is None:
                if self.has_sinks and current_platform.is_device_capability_family(120):
                    self._noncausal_prefill_wrapper = BatchAttentionWithAttentionSinkWrapper(
    ...
    def _compute_flashinfer_kv_metadata(
        ...
        self.paged_kv_last_page_len.gpu[:num_reqs].copy_(paged_kv_last_page_len_cpu, non_blocking=True)
    ...
    def build(
        ...
        prefill_force_trtllm = True if page_size >= 128 else self.attention_config.use_trtllm_attention
        ...
        decode_with_flashinfer_trtllm_api = self.use_trtllm_decode_attention and (causal or self.use_dedicated_xqa)
        ...
        all_uses_trtllm = causal and (
            (num_prefills == 0 or prefill_use_trtllm) and (num_decodes == 0 or decode_with_flashinfer_trtllm_api)
        )

        if not all_uses_trtllm:
            if self.has_sinks and (not self.use_dedicated_xqa or self.use_dcp or use_cascade):
                raise NotImplementedError(
                    "FlashInfer backend currently does not support attention "
        ...
        if self.use_dcp:
            ...
            if num_prefills > 0:
                qo_indptr_prefill_cpu = qo_indptr_cpu[num_decodes:] - qo_indptr_cpu[num_decodes]
        ...
        needs_native_paged_decode = num_decodes > 0 and not decode_with_flashinfer_trtllm_api
        needs_paged_kv_indices = use_cascade or needs_native_paged_prefill or needs_native_paged_decode
        ...
        if use_cascade:
            ...
            shared_qo_indptr_cpu = torch.tensor([0, num_actual_tokens], dtype=torch.int32, device="cpu")
            shared_kv_page_indptr_cpu = torch.tensor([0, num_common_kv_blocks], dtype=torch.int32, device="cpu")
            shared_kv_page_indices_cpu = block_table_tensor[0, :num_common_kv_blocks]
            shared_kv_last_page_len_cpu = torch.tensor([page_size], dtype=torch.int32, device="cpu")
        ...
        if num_prefills > 0:
            ...
            if prefill_use_trtllm:
                ...
                paged_kv_indptr_prefill_gpu = self.paged_kv_indptr.gpu[prefill_start : num_reqs + 1]
                ...
                paged_kv_last_page_len_prefill_cpu = self.paged_kv_last_page_len.cpu[prefill_start:num_reqs]
                assert paged_kv_last_page_len_prefill_cpu.shape[0] == num_prefills
                paged_kv_indptr_prefill_cpu = self.paged_kv_indptr.cpu[prefill_start : num_reqs + 1]
                ...
                if PIN_MEMORY:
                    ...
                    paged_kv_indptr_prefill_cpu = paged_kv_indptr_prefill_cpu.pin_memory()
                    paged_kv_last_page_len_prefill_cpu = paged_kv_last_page_len_prefill_cpu.pin_memory()
        ...
        if num_decodes > 0:
            if decode_with_flashinfer_trtllm_api:
                ...
                if self.use_dcp:
                    ...
                    seq_lens_decode = common_attn_metadata.dcp_local_seq_lens[:num_decodes]
                q_len_per_req = 1
                q_cu_seq_lens = None
                decode_mask = None
                if self.use_dedicated_xqa:
                    q_len_per_req, q_cu_seq_lens, ragged_q_lens = self._compute_decode_query_lens(
                        qo_indptr,
                        qo_indptr_cpu,
                        num_decodes,
                        num_decode_tokens,
                ...
                use_cudagraph = (
                    self.enable_cuda_graph and pure_decode and num_decode_tokens <= self._decode_cudagraph_max_bs
                )
                ...
                paged_kv_last_page_len_cpu = self.paged_kv_last_page_len.cpu[:num_input_tokens]
                # plan() copies these to the GPU with non_blocking=True;
                # stage them in pinned memory so the copies stay async
                # (the reused buffers themselves are intentionally not pinned).
                if PIN_MEMORY:
                ...
                kv_lens_decode_cpu = get_seq_lens(paged_kv_indptr_cpu, paged_kv_last_page_len_cpu, page_size)
                if PIN_MEMORY:
    ...
    def use_cascade_attention(self, *args, **kwargs) -> bool:
        if self.kv_cache_spec.dtype != self.aphrodite_config.model_config.dtype:
...
class FlashInferImpl(AttentionImpl):
    ...
    def __init__(
        ...
        if attn_type != AttentionType.DECODER:
            raise NotImplementedError(
                "Encoder self-attention and encoder/decoder cross-attention are not implemented for FlashInferImpl"
            )
        ...
        self.supports_xqa_or_trtllm_gen_decode = can_use_trtllm_attention(num_heads, num_kv_heads, is_prefill=False)
        aphrodite_config = get_current_aphrodite_config_or_none()
        # The layout is resolved after model construction, so read it lazily.
        self.cache_config = aphrodite_config.cache_config if aphrodite_config else None
        ...
        self.supports_quant_query_input = (
            ...
            and current_platform.is_device_capability_family(100)
            and aphrodite_config is not None
            and not aphrodite_config.attention_config.disable_flashinfer_q_quantization
        )
        ...
        dcp_a2a = (
            ...
            and aphrodite_config.parallel_config.dcp_comm_backend == "a2a"
        )
        if dcp_a2a:
    ...
    def maybe_quant_query(
        ...
        if query.dtype != q_data_type:
            ...
            query_quantized, _ = custom_ops.scaled_fp8_quant(query.view(num_tokens, num_heads * head_size), scale=scale)
    ...
    def forward(
        ...
        decode_kernel = (
            attn_metadata.decode.kernel if isinstance(attn_metadata.decode, FlashInferTrtllmAPIDecode) else None
        )
        ...
        if output_scale is None:
            assert output_block_scale is None, "output_block_scale is not supported when fusion has not happened"
        else:
            ...
            if output.dtype == FP8_DTYPE:
                assert output_block_scale is None, "output_block_scale should not be provided for fp8 output"
            elif output.dtype == FP4_DTYPE:
                assert output_block_scale is not None, "output_block_scale is required for nvfp4 output"
            else:
        ...
        # FlashInfer treats uint8 KV cache as NVFP4. Aphrodite stores FP8 KV cache
        ...
        if attn_metadata.use_cascade:
            ...
            if self.is_kvcache_nvfp4:
                kv_cache_views = tuple(
                    cache.permute(*stride_order) for cache in kv_cache.split(self.num_kv_heads, dim=1)
                )
            else:
                kv_perm = kv_cache.permute(*stride_order)
                kv_cache_views = kv_perm.split(self.head_size, dim=-1)
            kv_tuple = tuple(canonicalize_singleton_dim_strides(cache) for cache in kv_cache_views)
        ...
        # zero-copy.  See aphrodite.utils.torch_utils.
        fixed = canonicalize_singleton_dim_strides(kv_cache_permute)
        if fixed is not kv_cache_permute:
            logger.debug(
                "Canonicalized degenerate KV cache strides (FlashInfer): shape=%s, strides before=%s, strides after=%s",
        ...
        # Split K/V as zero-copy views. NVFP4 stores K/V as separate head
        ...
        decode_with_dedicated_xqa = decode_with_xqa and current_platform.is_device_capability_family(120)
        ...
        if num_prefill_tokens > 0:
            ...
            if not prefill_use_trtllm:
                ...
                if use_dcp:
                    ...
                    if isinstance(prefill_wrapper, BatchAttentionWithAttentionSinkWrapper):
                ...
                # This path needs to be enabled with APHRODITE_KV_CACHE_LAYOUT = HND
                ...
                if self.is_kvcache_nvfp4:
                ...
                elif attn_metadata.q_data_type_prefill != FP8_DTYPE and self.kv_cache_dtype.startswith("fp8"):
                    ...
                    assert kv_strides[-1] == 1 and kv_strides[-2] == kv_cache_permute.shape[-1], (
                        f"KV cache inner dims (block_size, head_size) must be contiguous, got strides {kv_strides}"
                    )
                    # fp8 uses (B, H, N, 2*hs); reshape to (B, 2, H, N, hs)
                    # for the dequant kernel. The dequant kernel handles the
                    # interleaved K/V block stride.
        ...
        if num_decode_tokens > 0:
            ...
            if not decode_with_flashinfer_trtllm_api:
                ...
                if use_dcp:
                    ...
                    if output.dtype == FP4_DTYPE:
                        raise NotImplementedError(
                            "DCP decode with FlashInfer trtllm-gen does not support FP4 attention output yet."
                        )
                    decode_query = get_dcp_group().all_gather(decode_query.contiguous(), dim=-2)
                    decode_query = canonicalize_singleton_dim_strides(decode_query)

                if decode_with_dedicated_xqa:
                    bmm1_scale = self.get_xqa_bmm1_scale(layer, attn_metadata.q_data_type_decode)
                ...
                if decode_with_xqa and q_len_per_req is not None and q_len_per_req > 1:
                    raise NotImplementedError("FlashInfer XQA speculative decode is not wired in Aphrodite yet.")
    ...
    def do_kv_cache_update(
        ...
        if self.kv_sharing_target_layer_name is None:
            ...
            if self.is_kvcache_nvfp4:
                ...
                k_cache, v_cache = kv_cache.transpose(1, 2).split(self.num_kv_heads, dim=-2)
            else:
...
def fast_plan_decode(
    ...
    if not self.is_cuda_graph_enabled or getattr(self, "aphrodite_first_call", True):
        ...
        self.aphrodite_first_call = False
        return
