...
from aphrodite.usage.usage_lib import (UsageContext, is_usage_stats_enabled,
                                       usage_message)
from aphrodite.utils import (get_open_port, get_open_zmq_ipc_path, get_tcp_uri,
                             kill_process_tree)
...
def wait_for_completion_or_failure(
    ...
    try:
        ...
        logger.exception("Exception occurred while running API servers: {}",
                         str(e))
        raise
    finally:
...
def report_usage_stats(
    ...
    usage_message.report_usage(
        ...
        extra_kvs={
            ...
            "dtype":
            str(aphrodite_config.model_config.dtype),
            "tensor_parallel_size":
            aphrodite_config.parallel_config.tensor_parallel_size,
            "block_size":
            aphrodite_config.cache_config.block_size,
            "gpu_memory_utilization":
            aphrodite_config.cache_config.gpu_memory_utilization,

            # Quantization
            "quantization":
            aphrodite_config.model_config.quantization,
            "kv_cache_dtype":
            str(aphrodite_config.cache_config.cache_dtype),

            # Feature flags
            "enable_lora":
            bool(aphrodite_config.lora_config),
            "enable_prefix_caching":
            aphrodite_config.cache_config.enable_prefix_caching,
            "enforce_eager":
            aphrodite_config.model_config.enforce_eager,
            "disable_custom_all_reduce":
            aphrodite_config.parallel_config.disable_custom_all_reduce,
        })
