...
def _melody_sources_to_aphrodite(
    raw_sources: Any,
    ...
    :meth:`aphrodite.entrypoints.cohere.serving.CohereServingChatV2._build_position_to_source`
...
def _melody_citations_to_aphrodite(
    raw_citations: Any,
    position_to_source: Mapping[tuple[int, int], CitationSource] | None,
) -> list[Citation] | None:
    ...
    :func:`_melody_sources_to_aphrodite`). Citations whose sources all fail
    ...
    for c in raw_citations:
        out.append(
            Citation(
                ...
                sources=_melody_sources_to_aphrodite(getattr(c, "sources", None), position_to_source),
                type=("THINKING_CONTENT" if getattr(c, "is_thinking", False) else "TEXT_CONTENT"),
            )
        )
    return out
...
class BaseCohereCommandReasoningParser(ReasoningParser):
    def __init__(
        ...
        **kwargs,
        ...
        # ``chat_template_kwargs``. See :func:`_melody_sources_to_aphrodite`
        ...
        from aphrodite.renderers.cohere import POSITION_TO_SOURCE_KEY
    ...
    def extract_reasoning_streaming(
        ...
        citations = _melody_citations_to_aphrodite(getattr(r, "citations", None), self._position_to_source)
        if r.content is None and r.reasoning is None and not r.tool_calls and not citations:
            return None
    ...
    def extract_reasoning(
        ...
        self.last_unary_citations = _melody_citations_to_aphrodite(
