...
class K2HorizonReasoningParser(DeepSeekR1ReasoningParser):
    ...
    def __init__(self, tokenizer: TokenizerLike, *args, **kwargs) -> None:
        ...
        if not isinstance(effort, str) or effort not in self._EFFORT_TOKENS:
            ...
            raise ValueError(f"Unsupported reasoning_effort {effort!r}. Supported values: {supported}.")
    ...
    def is_reasoning_end_streaming(
        ...
        del input_ids
        return self._stream_finished or super().is_reasoning_end_streaming((), delta_ids)

    def extract_content_ids(self, input_ids: list[int]) -> list[int]:
        if self.end_token_id in input_ids:
            return input_ids[input_ids.index(self.end_token_id) + 1 :]
        if self._tool_calls_start_id is not None and self._tool_calls_start_id in input_ids:
