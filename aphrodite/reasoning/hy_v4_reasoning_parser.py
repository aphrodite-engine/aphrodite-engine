...
# NOTE: mirrored in ``aphrodite.tool_parsers.hy_v4_tool_parser`` (same pattern as
...
class HYV4ReasoningExtractor:
    ...
    def __init__(self, vocab: dict[str, int], token_suffix: str, thinking: bool = True):
        ...
        if thinking and (start_token_id is None or end_token_id is None):
            raise RuntimeError("HYV4 reasoning extractor could not locate think start/end tokens in the tokenizer!")
    ...
    def reasoning_ended_in_delta(self, token_ids: Sequence[int], delta_ids: Iterable[int]) -> bool:
    ...
    def extract_reasoning_streaming(
        ...
        In no_think mode every delta is content. Otherwise: same logic as Aphrodite's
...
def build_reasoning_extractor(tokenizer: TokenizerLike, *, thinking: bool = True) -> HYV4ReasoningExtractor:
    return HYV4ReasoningExtractor(tokenizer.get_vocab(), detect_token_suffix(tokenizer), thinking)


class HYV4ReasoningParser(ReasoningParser):
    """Aphrodite adapter around :class:`HYV4ReasoningExtractor`.
