...
_COLLAPSE_RE = re.compile(r"<\|eom\|>(?:(?!to=self<\|message\|>).)*?to=self<\|message\|>", re.DOTALL)
...
_STRIP_REASONING_RE = re.compile(r"(?:<\|start\|>assistant\s*)?to=self<\|message\|>.*?<\|eom\|>", re.DOTALL)
...
class MuseGlimmerReasoningParser(ReasoningParser):
    ...
    def adjust_request(
        ...
        Aphrodite's serving default is ``skip_special_tokens=True``, which strips
    ...
    def _classify_bodies(text: str) -> tuple[str, str]:
        ...
        while pos < n:
            ...
            if recipient == "self":
                ...
                recipient == "user" and _FUNCTION_CALLS_OPEN not in body and "<atem:invoke" not in body
            ):
    ...
    def extract_reasoning_streaming(
        ...
        reasoning_delta = ""
        if curr_reason.startswith(self._emitted_reasoning) and len(curr_reason) > len(self._emitted_reasoning):
            reasoning_delta = curr_reason[len(self._emitted_reasoning) :]
            self._emitted_reasoning = curr_reason
        content_delta = ""
        if curr_content.startswith(self._emitted_content) and len(curr_content) > len(self._emitted_content):
