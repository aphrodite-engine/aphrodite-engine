...
"""Aphrodite Kernel does not support variance_size parameter and requires
...
def fused_add_rms_norm(
    ...
    if IS_ROCM and (not x.is_contiguous() or not x_residual.is_contiguous()):
        output, residual = ir.ops.fused_add_rms_norm.impls["native"].impl_fn(x, x_residual, weight, epsilon)
        x.copy_(output)
