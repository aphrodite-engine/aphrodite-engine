...
from aphrodite.kernels.helion.utils import (
    get_fp8_dtype,
    get_int8_min_max,
...
from aphrodite.kernels.helion.register import register_kernel
...
def generate_inputs() -> dict[CaseKey, tuple[Any, ...]]:
    ...
    group_size_list = [128]
    inputs = {}
    for intermediate_size, group_size, num_tokens in product(intermediate_size_list, group_size_list, num_tokens_list):
...
def pick_config(args: tuple[Any, ...], config_keys: list[CaseKey]) -> CaseKey | None:
    ...
    for key in config_keys:
        if key.is_default():
            continue
        configs.setdefault(key["intermediate_size"], {}).setdefault(key["group_size"], []).append(key["num_tokens"])

    if not configs:
        return None

    best_intermediate_size = min(configs, key=lambda s: abs(s - intermediate_size))
    best_group_size = min(configs[best_intermediate_size], key=lambda s: abs(s - group_size))
...
def baseline(
    ...
    scales.copy_(s)
    out.copy_(y.clamp(qtype_min, qtype_max).view(num_tokens, intermediate_size).to(out.dtype))
