...
_METHODS_TO_PATCH = {
    "debug_once": _AphroditeLogger.debug_once,
    "info_once": _AphroditeLogger.info_once,
    "warning_once": _AphroditeLogger.warning_once,
}
...
def _configure_aphrodite_root_logger() -> None:
    ...
    if not APHRODITE_CONFIGURE_LOGGING and APHRODITE_LOGGING_CONFIG_PATH:
        raise RuntimeError(
            ...
            "implies APHRODITE_CONFIGURE_LOGGING. Please enable "
    ...
    for formatter in logging_config.get("formatters", {}).values():
        ...
        if formatter.get("class") == "aphrodite.logging.NewLineFormatter":
            formatter["class"] = "aphrodite.logging_utils.NewLineFormatter"

    if logging_config:
        dictConfig(logging_config)


def init_logger(name: str) -> _AphroditeLogger:
    """The main purpose of this function is to ensure that loggers are
    retrieved in such a way that we can be sure the root aphrodite logger has
