...
from aphrodite.ir.tolerances import DEFAULT_TOLERANCES, ToleranceSpec
from aphrodite.ir.util import hash_source, weak_cache
from aphrodite.logger import init_logger
from aphrodite.logging_utils import lazy, tensors_str_no_data

InputGenerator = Callable[..., tuple[Any, ...]]

aphrodite_ir_lib = Library("aphrodite_ir", "FRAGMENT")
...
def enable_torch_wrap(enable: bool = True):
    """
    Context manager to enable/disable torch custom op wrapping for Aphrodite IR ops.
...
def register_op(
    ...
    *,
    ...
    """
    Register a new Aphrodite IR op.
    ...
    @aphrodite.ir.register_op(name="custom_mul")
...
class IrOp:
    ...
    def __init__(self, name: str, native_impl: Callable):
        ...
        self.impls["native"] = IrOpImpl(self, "native", native_impl, supported=True, supports_args=None)
        ...
        # torch registration
        aphrodite_ir_lib.define(self.name + self._schema_str)
        # CompositeExplicitAutograd is not decomposed
        # by ATen IR normalization in AOTAutograd
        aphrodite_ir_lib.impl(self.name, self._inner_call, dispatch_key="CompositeExplicitAutograd")
        aphrodite_ir_lib._register_fake(self.name, self._fake_call)
        assert hasattr(torch.ops.aphrodite_ir, name)
        self.torch_op: torch._ops.OpOverload = getattr(torch.ops.aphrodite_ir, name).default
...
class IrOpImpl:
    ...
    def uuid(self):
        ...
        Used by aphrodite-compile hash mechanism and torch.compile lowering pass uuid to
        control the Aphrodite compile cache and AOTAutograd/Inductor caches respectively.
