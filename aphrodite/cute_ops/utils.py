# Adapted from
# https://github.com/Dao-AILab/quack/blob/48c702e0defa69c24529dc3ebbbf4a586e2853b1/quack/utils.py
...
def set_block_rank(
    ...
    *,
    ...
    """Map the given smem pointer to the address at another CTA rank in
    the cluster."""
...
def store_shared_remote(
    ...
    *,
    ...
    if cutlass.const_expr(isinstance(val, float)):
        val = Float32(val)
    assert isinstance(val, (Float32, cutlass.Int64)), (
        "val must be Float32 or Int64"
    )
    ...
    llvm.inline_asm(
        ...
        f"st.async.shared::cluster.mbarrier::complete_tx::bytes.{suffix} [$0], $1, [$2];",  # noqa: E501
...
def cluster_reduce(
    ...
    """reduction_buffer has shape (num_warps / warps_per_row, (warps_per_row, cluster_n))"""  # noqa: E501
...
def block_or_cluster_reduce(
    ...
    """Perform either block or cluster reduction based on whether mbar_ptr is
    provided."""
...
def online_softmax_reduce(
    ...
    if cutlass.const_expr(reduction_buffer is not None):
        ...
        if cutlass.const_expr(warps_per_row > 1 or cluster_n > 1):
            ...
            if cutlass.const_expr(mbar_ptr is None):
                if lane_idx == 0:
                    reduction_buffer[row_idx, col_idx] = f32x2_to_i64(
                        max_x, sum_exp_x
                    )
                cute.arch.barrier()
                ...
                if warp_idx == 0:
                    with cute.arch.elect_one():
                        ...
                        cute.arch.mbarrier_arrive_and_expect_tx(
                            mbar_ptr,
                            num_warps * cluster_n *
                            reduction_buffer.element_type.width // 8,
                        )
                if lane_idx < cluster_n:
                    store_shared_remote(
                ...
                for i in cutlass.range_constexpr(num_iter):
                    idx = lane_idx + i * cute.arch.WARP_SIZE
                    if idx < cute.size(reduction_buffer, mode=[1]):
                        max_x_single_warp[i], sum_exp_x_single_warp[i] = (
                            i64_to_f32x2(reduction_buffer[row_idx, idx])
                        )
    ...
    return (
        max_x,
        sum_exp_x,
        (exp_x if cutlass.const_expr(return_exp_x) else None),
    )
...
def silu(a: float | Float32, *, loc=None, ip=None) -> Float32:
    """
    silu(a) = a * sigmoid(a) = a * (1 + tanh(a / 2)) / 2 = (0.5 * a) * tanh(0.5 * a) + (0.5 * a)  # noqa: E501 # noqa: E501
...
def permute_gated_Cregs_b16(t: cute.Tensor) -> None:
    ...
    # 1 -> 0b11111, 2 -> 0b11110, 4 -> 0b11100, 8 -> 0b11000, 16 -> 0b10000,
    # 32 -> 0b00000
    width = 4
    ...
    for i in cutlass.range(cute.size(t_u32.shape) // 2, unroll_full=True):
        ...
        upper0 = cute.arch.shuffle_sync(upper0, offset=upper_idx,
                                        mask_and_clamp=mask_and_clamp)
        lower0 = cute.arch.shuffle_sync(lower0, offset=lower_idx,
                                        mask_and_clamp=mask_and_clamp)
        ...
        for rest_k in cutlass.range_constexpr(tXsX.shape[2]):
            if cutlass.const_expr(tXpX is not None):
                if not tXpX[rest_v, 0, rest_k]:
                    cute.autovec_copy(
                        tXrX_fill, tXsX[(None, rest_v), None, rest_k],
                    )
            else:
                cute.autovec_copy(
                    tXrX_fill, tXsX[(None, rest_v), None, rest_k],
...
def f32x2_to_i64(
    ...
    *,
    ...
    res = cutlass.Int64(
        vector.extract(
            vec_i64x1, dynamic_position=[], static_position=[0],
            loc=loc, ip=ip,
        )
    )
    return res
...
@dsl_user_op
def i64_to_f32x2(
    c: cutlass.Int64, *, loc=None, ip=None,
    ...
    res0 = Float32(
        vector.extract(
            vec_f32x2, dynamic_position=[], static_position=[0],
            loc=loc, ip=ip,
        )
    )
    res1 = Float32(
        vector.extract(
            vec_f32x2, dynamic_position=[], static_position=[1],
            loc=loc, ip=ip,
        )
    )
    return res0, res1
...
def domain_offset_i64(
    ...
    # HACK: we assume that applying the offset does not change the pointer
    # alignment
...
def warp_prefix_sum(
    val: cutlass.Int32,
    lane: Optional[cutlass.Int32] = None,
) -> cutlass.Int32:
...
def convert_layout_acc_mn(acc_layout: cute.Layout) -> cute.Layout:
    """
    For Sm80, convert ((2, 2), MMA_M, MMA_N, ...) to
    ((2, MMA_M), (2, MMA_N), ...).

    For Sm90, convert ((2, 2, V), MMA_M, MMA_N, ...) to
    ((2, MMA_M), (2, V, MMA_N), ...).
    """
    acc_layout_col_major = cute.make_layout(acc_layout.shape)
    acc_layout_mn = cute.make_layout(
        (
            (acc_layout_col_major.shape[0][1], acc_layout_col_major.shape[1]),  # MMA_M  # noqa: E501
            (
        ...
        stride=(
            (acc_layout_col_major.stride[0][1], acc_layout_col_major.stride[1]),  # MMA_M  # noqa: E501
            (
...
def sm90_get_smem_load_op(
    ...
    *,
    ...
    """
    Selects the largest vectorized smem load atom available subject to
    constraint of gmem layout.
    ...
    if elem_ty_c.width == 16:
        return cute.make_copy_atom(
            cute.nvgpu.warp.LdMatrix8x8x16bOp(is_m_major, 4),
            elem_ty_c,
            loc=loc,
            ip=ip,
        )
