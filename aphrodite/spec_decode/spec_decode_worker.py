...
from aphrodite.distributed.communication_op import (
    broadcast_tensor_dict, get_tp_group, tensor_model_parallel_gather)
...
def create_spec_worker(*args, **kwargs) -> "SpecDecodeWorker":
    ...
    speculative_config: SpeculativeConfig = aphrodite_config.speculative_config
    ...
    target_worker_config = copy.deepcopy(aphrodite_config)
    ...
    draft_worker_config = copy.deepcopy(aphrodite_config)
    draft_worker_config.model_config = speculative_config.draft_model_config
    draft_worker_config.quant_config = AphroditeConfig._get_quantization_config(
        draft_worker_config.model_config,
        aphrodite_config.load_config,
    )
    ...
    draft_worker_kwargs.update(
        aphrodite_config=draft_worker_config,
    ...
    * The scoring forward pass is done without an MQA kernel, which is
    ...
    def init_device(self) -> None:
        ...
        if self._enable_lm_head_weight_load:
            # NOTE: gather lm_head weight when tp enabled
    ...
    def execute_model(
        ...
        if all_prompt and execute_model_req.seq_group_metadata_list:
            assert num_lookahead_slots == 0, (
                ...
                "https://github.com/aphrodite-project/aphrodite-engine/issues")
    ...
    def _maybe_disable_speculative_tokens(
        ...
        for seq_group_metadata in seq_group_metadata_list:
            ...
            # TODO: We currently store spec decoding specific
    ...
    def _verify_tokens(
        ...
        # into spec and non spec sequences) and should be removed in the future.
        # It can be done by supporting per-sequence proposal lens.
