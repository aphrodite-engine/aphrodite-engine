...
def _attach_endpoint_plugins(app: FastAPI, supported_tasks: tuple["SupportedTask", ...]) -> None:
    ...
    shadow core routes with the same path. No-ops when no plugins are
    discovered or allowlisted.
    """
...
async def _init_endpoint_plugins_state(engine_client: EngineClient | None, state: State, args: Namespace) -> None:
    ...
    `build_app`. Some callers build their own bare `State` without going
    through `build_app`; absence is treated the same as no attached plugins.

    `engine_client` is `None` for the CPU-only render server, which has no
    engine. Plugins must handle a `None` engine client themselves.
    """
...
def build_app(
    ...
    # Fallback handlers for raw exceptions not yet migrated to AphroditeError.
...
async def init_render_app_state(
    ...
    :class:`~aphrodite.engine.protocol.EngineClient`; it bootstraps the
    preprocessing pipeline (renderer, input_processor)
    directly from the :class:`~aphrodite.config.AphroditeConfig`.
    """
    from aphrodite.entrypoints.chat_utils import load_chat_template
    ...
    # No `EngineClient` exists for the render server, so plugins get `None`.
...
async def build_and_serve(
    ...
    **uvicorn_kwargs,
    ...
    logger.info("Starting Aphrodite server on %s", listen_address)
...
async def build_and_serve_renderer(
    ...
    **uvicorn_kwargs,
    ...
    await init_render_app_state(aphrodite_config, app.state, args)
