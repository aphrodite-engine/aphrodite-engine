...
from aphrodite.entrypoints.openai.responses.harmony import harmony_to_response_output
...
class OpenAIServingResponses(GenerateBaseServing):
    ...
    def _validate_create_responses_input(self, request: ResponsesRequest) -> ErrorResponse | None:
        ...
        if request.store and not self.enable_store and request.background:
            return self.create_error_response(
                err_type="invalid_request_error",
                message=(
                    "This Aphrodite engine does not support `store=True` and "
                    "therefore does not support the background mode. To "
                    "enable these features, set the environment variable "
                    "`APHRODITE_ENABLE_RESPONSES_API_STORE=1` when launching "
                    "the Aphrodite server."
                ),
                status_code=HTTPStatus.BAD_REQUEST,
                param="background",
            )
    ...
    async def _render_resolved_response_inputs(
        ...
        *,
        ...
        if prev_response is not None:
            stored_messages = (
                self.msg_store[prev_response.id] if self.use_harmony else self.msg_store.get(prev_response.id)
            )
    ...
    async def _create_responses(
        ...
        for engine_input in engine_inputs:
            ...
            if self.use_harmony:
                ...
                if envs.APHRODITE_USE_EXPERIMENTAL_PARSER_CONTEXT:
            ...
            generator = self._generate_with_builtin_tools(
                ...
                context=context,
                tok_params=(
                    request.build_tok_params(self.model_config) if isinstance(context, HarmonyContext) else None
                ),
    ...
    async def _render_next_turn(
        ...
        if isinstance(render_result, ErrorResponse):
            raise APHRODITEValidationError(render_result.error.message)
    ...
    async def _generate_with_builtin_tools(
        ...
        while True:
            ...
            if isinstance(context, HarmonyContext):
                ...
                sampling_params.max_tokens = max_model_len - self._extract_prompt_len(engine_input)
                ...
                sampling_params.max_tokens = get_max_tokens(
                    ...
                    truncate_prompt_tokens=(-1 if context.request.truncation != "disabled" else None),
                )
    ...
    async def _process_harmony_streaming_events(
        ...
        async for ctx in result_generator:
            ...
            for segment in ctx.last_append_segments:
                if segment.delta:
                    for event in emit_content_delta_events(segment, state, ctx.function_tool_names):
                        yield _increment_sequence_number_and_return(event)

                elif completed_message := segment.completed_message:
                    # TODO: Fix browser emitted as MCP calls
                    for event in emit_previous_item_done_events(completed_message, state, ctx.function_tool_names):
                        yield _increment_sequence_number_and_return(event)

                    for event in emit_tool_action_events(completed_message, state, self.tool_server):
                        yield _increment_sequence_number_and_return(event)
