...
class OpenAIServingCompletion(GenerateBaseServing):
    ...
    def request_output_to_completion_response(
        ...
        if len(final_res_batch) == 1 and (request.n or 1) == 1:
            if self.enable_per_request_metrics:
                last_metrics = last_final_res.metrics if last_final_res is not None else None
                per_request_metrics = build_per_request_timing_metrics(last_metrics, num_generated_tokens)
