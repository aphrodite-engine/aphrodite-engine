...
from aphrodite.utils.serial_utils import numpy2base64
...
def _get_mm_token_counts(engine_input: EngineInput) -> dict[str, int]:
    ...
    """
    mm_placeholders = cast(MultiModalPlaceholders | None, engine_input.get("mm_placeholders"))
    return {modality: sum(p.length for p in ranges) for modality, ranges in (mm_placeholders or {}).items() if ranges}
...
def _make_prompt_tokens_details(
    ...
    if not enable_prompt_tokens_details:
        return None
    if num_cached_tokens is None and num_cache_creation_tokens is None and not mm_token_counts:
        return None
            ...
            if isinstance(sampling_params, BeamSearchParams):
                ...
                generator = self.engine_client.generate(
                    ...
                    reasoning_parser_kwargs={
                        "chat_template_kwargs": self._engine_chat_template_kwargs(chat_template_kwargs),
                    }
        ...
        try:
            async for res in result_generator:
                ...
                for output in res.outputs:
                    ...
                    if request.logprobs and (request.top_logprobs is not None or request.logprob_token_ids):
                    ...
                    if parser is not None:
                        ...
                        previous_reasoning_tokens[i] = parser.count_reasoning_tokens(tuple(generated_token_ids[i]))
                    ...
                    include_token_ids = request.return_token_ids and not hide_stream_metadata
                    ...
                    if output.finish_reason is None:
                        ...
                        choice_data = ChatCompletionResponseStreamChoice(
                            ...
                            logprobs=logprobs,
                            finish_reason=None,
                            token_ids=(as_list(output.token_ids) if include_token_ids else None),
                        )
                    ...
                    if include_continuous_usage:
                        ...
                        chunk.usage = UsageInfo(
                            ...
                            completion_tokens_details=(
                                _make_completion_tokens_details(previous_reasoning_tokens[i])
                                if self._include_reasoning_tokens_details
                                else None
                            ),
                        )
            ...
            if include_usage:
                ...
                final_usage = UsageInfo(
                    ...
                    completion_tokens=completion_tokens,
                    total_tokens=num_prompt_tokens + completion_tokens,
                    completion_tokens_details=_make_completion_tokens_details(sum(previous_reasoning_tokens))
                    if self._include_reasoning_tokens_details
                    else None,
                )
    ...
    async def chat_completion_full_generator(
        ...
        for output in final_res.outputs:
            ...
            is_named_tool_choice = (
                request.tool_choice is not None and type(request.tool_choice) is ChatCompletionNamedToolChoiceParam
            )
            ...
            if (not self.enable_auto_tools or not tool_parser_cls) and (
                ...
                message = self._create_chat_message(role=role, reasoning=reasoning, content=content)
                ...
                message = self._create_chat_message(
                    ...
                    reasoning=reasoning,
                    content=content or "",
                    tool_calls=[ToolCall(id=tc.id or make_tool_call_id(), function=tc) for tc in (tool_calls or [])],
                )
            ...
            # handle when there are tools and tool choice is auto
                ...
                if tool_calls:
                    message = self._create_chat_message(
                        ...
                        reasoning=reasoning,
                        content=content,
                        tool_calls=[ToolCall(id=tc.id or make_tool_call_id(), function=tc) for tc in tool_calls],
                    )
            ...
            routed_experts_b64 = numpy2base64(output.routed_experts) if output.routed_experts is not None else None
            ...
            choice_data = ChatCompletionResponseChoice(
                ...
                stop_reason=output.stop_reason,
                token_ids=(as_list(output.token_ids) if request.return_token_ids and not suppress_metadata else None),
        ...
        usage = UsageInfo(
            ...
            completion_tokens_details=_make_completion_tokens_details(total_reasoning_tokens)
            if self._include_reasoning_tokens_details
            else None,
        )
        ...
        if (request.n or 1) == 1:
            if self.enable_per_request_metrics:
                per_request_metrics = build_per_request_timing_metrics(final_res.metrics, num_generated_tokens)
