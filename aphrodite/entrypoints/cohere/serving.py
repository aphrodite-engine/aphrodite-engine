...
Aphrodite's renderer pipeline (``aphrodite.renderers``):

- For Cohere Command-family models, set ``--tokenizer-mode cohere`` and the
  :class:`aphrodite.renderers.cohere.CohereRenderer` will template the request
  ...
  (see :mod:`aphrodite.entrypoints.cohere.cohere_chat_message`).
...
from aphrodite.entrypoints.cohere.cohere_chat_message import (
    CitationSource as InternalCitationSource,
)
from aphrodite.entrypoints.cohere.cohere_chat_message import (
    CohereChatMessage,
)
from aphrodite.entrypoints.cohere.protocol import (
    CitationEndEvent,
    CitationStartEvent,
    CohereChatV2Request,
...
from aphrodite.renderers.cohere import (
...
def _emit(event: BaseModel) -> str:
    ...
    The typed event classes in ``aphrodite.entrypoints.cohere.protocol``
...
# Mapping of Aphrodite/OpenAI finish reasons to Cohere's enum.
...
def _map_finish_reason(reason: str | None, stop_reason: int | str | None = None) -> CohereFinishReason:
    # Aphrodite reports both EOS and matched stop strings as finish_reason="stop".
...
class CohereServingChatV2(OpenAIServingChat):
    ...
    renderer (:class:`aphrodite.renderers.cohere.CohereRenderer`) when the
    engine is started with ``--tokenizer-mode cohere``.

    Citations flow through ``CohereChatMessage`` / ``CohereDeltaMessage``
    subclasses (see :mod:`aphrodite.entrypoints.cohere.cohere_chat_message`) so
    ...
    async def create_chat_v2(
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug("Received Cohere v2 chat request %s", request.model_dump_json())
    ...
    def to_chat_completion_request(self, request: CohereChatV2Request) -> ChatCompletionRequest:
        ...
        :meth:`aphrodite.entrypoints.scale_out.render.serving.ServingRender.render_chat_request`
    ...
    def _engine_chat_template_kwargs(self, chat_template_kwargs: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        return {k: v for k, v in chat_template_kwargs.items() if k not in _API_SERVER_ONLY_TEMPLATE_KWARGS}
    ...
    @classmethod
    def _convert_v2_to_chat_completion(cls, request: CohereChatV2Request) -> ChatCompletionRequest:
    ...
    def _convert_tool_message(cls, msg: ToolChatMessageV2) -> dict[str, Any]:
        ...
        # emits for multi-part user content. Aphrodite's
        ...
        # which Aphrodite's shared ``_parse_chat_message_content_parts`` in
        # ``aphrodite/entrypoints/chat_utils.py`` rejects because the OpenAI
        ...
        # :func:`aphrodite.renderers.cohere._v2_tool_content_to_melody_block`
        ...
        for block in msg.content:
            if block.type == "text":
                ...
                content_parts.append(
                    {
                        "type": "text",
                        "text": json.dumps(block.document.model_dump(exclude_none=True)),
                    }
                )
        return {
            "role": "tool",
    ...
    def _apply_streaming_options(
        ...
        if request.stream:
            ...
            chat_req.stream_options = StreamOptions.model_validate({"include_usage": True})
    ...
    def _apply_cohere_template_kwargs(
        ...
        The :class:`aphrodite.renderers.cohere.CohereRenderer` consumes these
                    ...
                    documents.append(
                        {
                            ...
                            "data": (doc.data if isinstance(doc.data, dict) else {"text": doc.data}),
                        }
                    )
        ...
        # ``_melody_sources_to_aphrodite`` in the reasoning parser for the
        # consumer.
    ...
    @classmethod
    def _collect_message_citations(cls, request: CohereChatV2Request) -> dict[int, list[dict[str, Any]]]:
        ...
        """
        doc_positions = cls._build_doc_id_to_prompt_position(request.messages, request.documents)
    ...
    @classmethod
    def _walk_citable_positions(cls, messages: list, documents: list | None) -> list[_CitablePosition]:
        ...
        if documents:
            for idx, doc in enumerate(documents):
                ...
                if isinstance(doc, str):
                    ...
                    payload = dict(doc.data) if isinstance(doc.data, dict) else {"text": doc.data}
                ...
                ids: tuple[str, ...] = (explicit_id, fallback_id) if explicit_id else (fallback_id,)
                out.append(
                    _CitablePosition(
                        bucket=0,
                        result_idx=idx,
                        ids=ids,
                        source=InternalCitationSource(type="document", id=explicit_id, document=payload),
                    )
                )
            next_bucket = 1
                    ...
                    if isinstance(msg.content, str):
                        ...
                        out.append(
                            _CitablePosition(
                                ...
                                result_idx=base,
                                ids=(),
                                source=InternalCitationSource(type="tool", id=None, tool_output=payload),
                            )
                        )
    ...
    @classmethod
    def _build_doc_id_to_prompt_position(cls, messages: list, documents: list | None) -> dict[str, tuple[int, int]]:
    ...
    def _sdk_citation_to_melody(
        ...
        sources = [{"tool_call_index": bucket, "tool_result_indices": indices} for bucket, indices in grouped.items()]
    ...
    def _chat_completion_to_v2(
        ...
        if msg.reasoning:
            content_blocks.append({"type": ContentBlockType.THINKING, "thinking": msg.reasoning})
        if msg.content:
        ...
        if not self._is_reasoning_model and tool_calls and msg.reasoning:
            ...
            content_blocks = [blk for blk in content_blocks if blk.get("type") != ContentBlockType.THINKING]
    ...
    def _finalize_response_message(
        ...
        *,
        ...
        (:mod:`aphrodite.reasoning.cohere_command_reasoning_parser`) caches the
    ...
    def _extract_citations_if_any(self, msg: Any) -> list[Citation] | None:
        ...
        ``citations: list[aphrodite...Citation] | None`` field populated by
        ...
        :func:`_melody_sources_to_aphrodite` in
        :mod:`aphrodite.reasoning.cohere_command_reasoning_parser`) using
    ...
    def _to_wire_citation(self, citation: Any) -> Citation | None:
        ...
        try:
            ...
            logger.warning("Skipping malformed citation payload: %r", payload, exc_info=True)
            return None

    @classmethod
    def _build_position_to_source(cls, request: CohereChatV2Request) -> dict[tuple[int, int], InternalCitationSource]:
        ...
        :func:`_melody_sources_to_aphrodite` can attach real document ids /
    ...
    @classmethod
    def _collect_v2_tool_message_content(cls, request: CohereChatV2Request) -> dict[int, list[dict[str, Any]]]:
        ...
        :mod:`aphrodite.renderers.cohere` for why the map exists). Walks
        ...
        :func:`aphrodite.renderers.cohere._content_blocks`, so no forwarded
    ...
    def _handle_citation_deltas(
        ...
        :mod:`aphrodite.entrypoints.cohere.cohere_chat_message`) with
        sources whose ``id`` / ``type`` / ``document`` / ``tool_output``
        are already resolved against the request's document context
        (see :func:`_melody_sources_to_aphrodite` in the parser). This
        ...
        for c in citations:
            ...
            events.append(
                _emit(
                    CitationStartEvent(
                        index=idx,
                        delta={"message": {"citations": citation.model_dump(exclude_none=True)}},
                    )
                )
            )
