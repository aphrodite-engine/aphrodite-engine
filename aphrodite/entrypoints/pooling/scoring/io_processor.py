...
from aphrodite import PoolingParams, PoolingRequestOutput, TokensPrompt
from aphrodite.renderers import TokenizeParams
from aphrodite.renderers.hf import safe_apply_chat_template
...
def _apply_post_tokenization_to_token_type_ids(
    ...
    if max_length is not None and max_length < len(token_type_ids):
        if max_length == 0:
            ...
            side = tok_params.truncation_side or (tokenizer.truncation_side if tokenizer is not None else None)
            token_type_ids = token_type_ids[-max_length:] if side == "left" else token_type_ids[:max_length]
    ...
    if pad_length is not None and pad_length > len(token_type_ids):
        ...
        token_type_ids = token_type_ids + [pad_token_type_id] * (pad_length - len(token_type_ids))
...
class BiEncoderIOProcessor(ScoringIOProcessor):
    ...
    def get_request_factory_online(self, ctx: PoolingServeContext) -> Sequence[AnyRenderParam]:
        ...
        parsed_prompts = [parse_model_prompt(self.model_config, prompt) for prompt in prompts_seq]
...
class CrossEncoderIOProcessor(ScoringIOProcessor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        from aphrodite.model_executor.models.interfaces import supports_score_template
    ...
    def render(
        self,
        render_params: EncodeCMPLRenderParams | EncodeChatRenderParams | ScoringRenderParams,
    ) -> PoolingEngineInput:
        if "data_1" not in render_params:
            raise ValueError(f"Unsupported render_params type {render_params.__class__.__name__}")
        ...
        if token_type_ids := engine_prompt.pop("token_type_ids", None):
            ...
            compressed = compress_token_type_ids(
                _apply_post_tokenization_to_token_type_ids(self.tokenizer, tok_params, token_type_ids)
            )
            params.extra_kwargs = {
        ...
        engine_prompt_extras = (
            {k: v for k in ("mm_processor_kwargs", "cache_salt") if (v := prompt_extras.get(k)) is not None}
            if prompt_extras
            else None
        )
