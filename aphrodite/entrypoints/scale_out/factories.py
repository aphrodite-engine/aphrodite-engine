...
def register_scale_out_api_routers(
    ...
    if "render" in supported_tasks:
        dedicated_mode = "`aphrodite launch render`"
    elif getattr(app.state.args, "tokens_only", False):
        dedicated_mode = "`--tokens-only`"

    enabled = envs.APHRODITE_ENABLE_SCALE_OUT_ENDPOINTS
    if dedicated_mode is not None and enabled is False:
        raise ValueError(
            f"`APHRODITE_ENABLE_SCALE_OUT_ENDPOINTS=0` conflicts with {dedicated_mode}; unset it or set it to 1"
        )
    if dedicated_mode is None and not enabled:
        logger.info("Scale-out endpoints are disabled. Set APHRODITE_ENABLE_SCALE_OUT_ENDPOINTS=1 to enable them.")
        return
