...
class ServingDerender(BaseServing):
    ...
    def _validate_derender_bounds(
        ...
        if len(generate_responses) > max_n:
            return self.create_error_response(
                ...
                f"Set APHRODITE_MAX_N_SEQUENCES to increase this limit."
            )
    ...
    async def derender_chat_stream_response(
        ...
        try:
            ...
            return self.create_error_response(f"invalid stream_state: detokenization failed ({exc!r})")

        logger.debug(
            "derender_chat_stream request_id=%s model=%s delta_tokens=%d",
            request.generate_chunk.request_id,
            model_name,
            sum(len(c.token_ids) for c in request.generate_chunk.choices if c.token_ids),
        )
