...
from aphrodite.entrypoints.generate.base.protocol import StreamOptions, validate_cache_salt
...
class MultiModalFeatures(BaseModel):
    ...
    def _validate_parallel_fields(self) -> "MultiModalFeatures":
        ...
        if set(self.mm_placeholders) != modalities:
            raise ValueError("mm_hashes and mm_placeholders must use the same modalities")
        ...
        for modality in modalities:
            ...
            if num_hashes != num_placeholders:
                raise ValueError(f"{modality} mm_hashes and mm_placeholders must have the same length")
            if self.kwargs_data is not None and len(self.kwargs_data[modality]) != num_hashes:
                raise ValueError(f"{modality} kwargs_data and mm_hashes must have the same length")
        ...
        for (offset, end), (next_offset, _) in zip(flattened_ranges, flattened_ranges[1:]):
            if next_offset < end:
                raise ValueError("mm_placeholders must be globally non-overlapping and sorted")
        return self
        ...
        for ranges in self.features.mm_placeholders.values():
            for placeholder in ranges:
                if placeholder.offset + placeholder.length > prompt_len:
                    raise ValueError("mm_placeholders must remain within the token_ids sequence")
        return self
...
DerenderCompletionRequestUnion: TypeAlias = DerenderCompletionRequest | DerenderCompletionStreamRequest
