...
def redact_sensitive_args(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not any(key in _SENSITIVE_ARG_FIELDS for key in args):
        return args
    return {key: ("***" if key in _SENSITIVE_ARG_FIELDS else value) for key, value in args.items()}
...
def process_lora_modules(
    ...
    from aphrodite.entrypoints.openai.models.serving import LoRAModulePath
...
def log_version_and_model(lgr: Logger, version: str, model_name: str) -> None:
    if envs.APHRODITE_DISABLE_LOG_LOGO or (formatter := current_formatter_type(lgr)) is None:
        lgr.info("Aphrodite server version %s, serving model %s", version, model_name)
        return
