...
from aphrodite.entrypoints.launchers.api_server.entry import run_server, setup_server
from aphrodite.entrypoints.launchers.cli_args import (
    make_arg_parser,
    validate_parsed_serve_args,
)
from aphrodite.entrypoints.launchers.dp_supervisor import run_dp_supervisor
...
class ServeSubcommand(CLISubcommand):
    ...
    def cmd(args: argparse.Namespace) -> None:
        ...
        # Aphrodite currently ships the rebased core extension without the
        # split stable-libtorch quant op surface from latest upstream. When
        ...
        if not args.enforce_eager:
            ...
            if not hasattr(torch.ops._C, "per_token_group_fp8_quant"):
                logger.warning(
                    ...
                    "build; enabling eager mode for server startup."
                )
                args.enforce_eager = True

        if getattr(args, "grpc", False):
            from aphrodite.entrypoints.grpc_server import serve_grpc
...
def run_headless(args: argparse.Namespace):
    ...
    if parallel_config.node_rank_within_dp > 0:
        ...
        logger.info(
            "Launching Aphrodite (v%s) headless multiproc executor, "
    ...
    try:
        ...
        if shutdown_requested:
            timeout = aphrodite_config.shutdown_timeout
