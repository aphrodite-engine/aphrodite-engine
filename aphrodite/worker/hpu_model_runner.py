...
import aphrodite.hpu_extension.environment as environment
...
from aphrodite.hpu_extension.bucketing.common import get_bucketing_context
from aphrodite.hpu_extension.ops import LoraMask as LoraMask
from aphrodite.hpu_extension.profiler import (HabanaHighLevelProfiler,
...
class HpuModelAdapter:
    ...
    def __init__(self, model, aphrodite_config):
        self.model = model
        self.sampler = get_sampler()
        self.prefill_use_fusedsdpa = os.getenv('APHRODITE_PROMPT_USE_FUSEDSDPA',
                                               '0').lower() in ['1', 'true']
        self.aphrodite_config = aphrodite_config
        self.block_size = aphrodite_config.cache_config.block_size
        self.dtype = aphrodite_config.model_config.dtype
        enforce_eager = aphrodite_config.model_config.enforce_eager

        if not htorch.utils.internal.is_lazy() and not enforce_eager:
            if os.getenv('APHRODITE_REGIONAL_COMPILATION',
                         'true').lower() == 'true':
    ...
    def _set_block_mapping(self, metadata, batch_size, device, dtype):
        ...
        attn_bias = (torch.zeros_like(mask, dtype=dtype).masked_fill_(
            mask, -math.inf))
        if os.environ.get('APHRODITE_USE_FAKE_HPU',
    ...
    def forward(self, *args, **kwargs):
        ...
        with set_forward_context(attn_metadata, self.aphrodite_config,
                                 virtual_engine):
...
class HPUModelRunnerBase(ModelRunnerBase[TModelInputForHPU]):
    ...
    def __init__(
        ...
        ModelRunnerBase.__init__(self, aphrodite_config=aphrodite_config)
        ...
        self.use_contiguous_pa = envs.APHRODITE_USE_HPU_CONTIGUOUS_CACHE_FETCH
    ...
    def _set_gc_threshold(self) -> None:
        ...
        # We can either use APHRODITE_GC_THR_GEN[0-2] (this has higher priority)
        # to set particular generation threshold or use simpler
        # APHRODITE_GC_THR_MULTIPLIER to multiply default values.
        ...
        for i in range(len(default_gc_thrs)):
            requested_gc_thrs[i] = int(
                os.environ.get(f'APHRODITE_GC_THR_GEN{i}', default_gc_thrs[i]))
        if requested_gc_thrs == default_gc_thrs:
            gc_thr_multiplier = int(os.environ.get(
                'APHRODITE_GC_THR_MULTIPLIER', 2))
            requested_gc_thrs = [
                t * gc_thr_multiplier for t in default_gc_thrs
            ]
        gc.set_threshold(*requested_gc_thrs)

        self.skip_warmup = os.environ.get('APHRODITE_SKIP_WARMUP',
                                          'false').lower() == 'true'
        ...
        with HabanaMemoryProfiler() as m:
            ...
            with HabanaMemoryProfiler() as m_wrap:
                self.model = _maybe_wrap_in_hpu_graph(
                    self.model, aphrodite_config=self.aphrodite_config)
    ...
    def warmup_scenario(self,
        ...
        for _ in range(times):
            ...
            is_single_step = \
                self.aphrodite_config.scheduler_config.num_scheduler_steps == 1
    ...
    def warmup_model(self, kv_caches: List[torch.Tensor]) -> None:
        ...
        if profile := os.environ.get('APHRODITE_PT_PROFILE', None):
        ...
        with compile_only_mode_context(
            ...
            if not self.enforce_eager and htorch.utils.internal.is_lazy():
                ...
                prompt_graph_mem_ratio = float(
                    os.environ.get('APHRODITE_GRAPH_PROMPT_RATIO', '0.3'))
                ...
                msg = (
                    ...
                    f"(APHRODITE_GRAPH_PROMPT_RATIO={prompt_graph_mem_ratio})")
                logger.info(msg)
                prompt_strategy = os.environ.get(
                    'APHRODITE_GRAPH_PROMPT_STRATEGY', 'min_tokens')
                decode_strategy = os.environ.get(
                    'APHRODITE_GRAPH_DECODE_STRATEGY', 'max_bs')
...
class HPUModelRunner(HPUModelRunnerBase[ModelInputForHPUWithSamplingMetadata]):
    ...
    def execute_model(
        ...
        APHRODITE_DELAYED_SAMPLING = envs.APHRODITE_HPU_USE_DELAYED_SAMPLING
        use_delayed_sampling = APHRODITE_DELAYED_SAMPLING and not warmup_mode
        ...
        if model_input.is_first_multi_step:
            ...
            if num_steps == 1:
                ...
                return []
        return output if type(output) is list else [output]  # noqa: E721
