...
from aphrodite.modeling.sampling_metadata import set_random_seed
from aphrodite.prompt_adapter.request import PromptAdapterRequest
from aphrodite.worker.cache_engine import CacheEngine
from aphrodite.worker.hpu_model_runner import HPUModelRunner
...
class HPUWorker(LocalOrDistributedWorkerBase):
    ...
    def execute_model(
        ...
        # APHRODITE_HPU_LOG_STEP_GRAPH_COMPILATION     - will log graph compilations per engine step, only when there was any - highly recommended to use alongside PT_HPU_METRICS_GC_DETAILS! # noqa:E501
        # APHRODITE_HPU_LOG_STEP_GRAPH_COMPILATION_ALL - will log graph compilations per engine step, always, even if there were none # noqa:E501
        # APHRODITE_HPU_LOG_STEP_CPU_FALLBACKS         - will log cpu fallbacks per engine step, only when there was any # noqa:E501
        # APHRODITE_HPU_LOG_STEP_CPU_FALLBACKS_ALL     - will log cpu fallbacks per engine step, always, even if there were none # noqa:E501
        log_graph_compilation_all = os.environ.get(
            'APHRODITE_HPU_LOG_STEP_GRAPH_COMPILATION_ALL', '0') != '0'
        log_graph_compilation = os.environ.get(
            'APHRODITE_HPU_LOG_STEP_GRAPH_COMPILATION',
            '0') != '0' or log_graph_compilation_all
        log_cpu_fallbacks_all = os.environ.get(
            'APHRODITE_HPU_LOG_STEP_CPU_FALLBACKS_ALL', '0') != '0'
        log_cpu_fallbacks = os.environ.get(
            'APHRODITE_HPU_LOG_STEP_CPU_FALLBACKS',
        ...
        if (log_graph_compilation or log_cpu_fallbacks) and \
            ...
            if (log_graph_compilation and gc_local_metric.stats()[0][1]
                ...
                msg = ("APHRODITE_HPU_STEP_GRAPH_COMPILATION: "
            ...
            if (log_cpu_fallbacks and cpu_fallback_local_metric.stats()[0][1]
                ...
                msg = ("APHRODITE_HPU_STEP_CPU_FALLBACK: "
    ...
    def determine_num_available_blocks(self) -> Tuple[int, int]:
        ...
        graph_reserved_mem = (float(
            os.environ.get('APHRODITE_GRAPH_RESERVED_MEM', '0.1'))
        ...
        msg = (
            ...
            f"(APHRODITE_GRAPH_RESERVED_MEM={graph_reserved_mem}), "
