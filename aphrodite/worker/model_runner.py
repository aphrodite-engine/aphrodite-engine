...
from aphrodite.distributed.parallel_state import (
    get_tensor_model_parallel_rank, graph_capture)
...
from aphrodite.utils import (DeviceMemoryProfiler, GiB_bytes, PyObjectCache,
...
class ModelInputForGPUBuilder(ModelRunnerInputBuilderBase[ModelInputForGPU]):
    ...
    def _get_cuda_graph_pad_size(self,
        ...
        graph_batch_size = self.runner.aphrodite_config.pad_for_cudagraph(
            batch_size)
...
class GPUModelRunnerBase(ModelRunnerBase[TModelInputForGPU]):
    ...
    def __init__(
        ...
        self.max_batchsize_to_capture = \
            self.aphrodite_config.compilation_config.max_capture_size
    ...
    def load_model(self) -> None:
        ...
        with DeviceMemoryProfiler(self.device) as m:
            ...
            if self.lora_config:
                ...
                if supports_multimodal(self.model):
                    logger.warning(
                        "Regarding multimodal models, Aphrodite currently "
        ...
        if self.aphrodite_config.compilation_config.level ==\
    ...
    def save_tensorized_model(
        ...
        from aphrodite.modeling.model_loader import TensorizerLoader
    ...
    def capture_model(self, kv_caches: List[List[torch.Tensor]]) -> None:
        ...
        with self.attn_state.graph_capture(max_batch_size), graph_capture(
            ...
            for virtual_engine in range(
                ...
                # product.
                cudagraph_capture_sizes = self.aphrodite_config.compilation_config\
...
class ModelRunner(GPUModelRunnerBase[ModelInputForGPUWithSamplingMetadata]):
        ...
        **kwargs,
        ...
        if not bypass_model_exec:
            with set_forward_context(model_input.attn_metadata,
                                     self.aphrodite_config, virtual_engine):
    ...
    def need_recv_kv(self, model_input, kv_caches) -> bool:
        ...
        We need to receive KV when
            1. current Aphrodite instance is KV cache consumer/decode Aphrodite instance
            2. this batch is not a profiling run
            3. this batch is a prefill run

        Args:
            model_input: input to the model executable
            kv_caches: Aphrodite's paged memory
        """

        if self.aphrodite_config.kv_transfer_config is None:
            return False
        ...
        return self.aphrodite_config.kv_transfer_config.is_kv_consumer and (
            not is_profile_run) and is_prefill_run

    def need_send_kv(self, model_input, kv_caches) -> bool:
        """Check if we need to send kv-cache to the other worker.
        We need to send KV when
            1. current Aphrodite instance is KV cache producer/prefill Aphrodite instance
            2. this batch is not a profiling run
            3. this batch is a prefill run
        ...
        return self.aphrodite_config.kv_transfer_config.is_kv_producer and (
            not is_profile_run) and is_prefill_run
