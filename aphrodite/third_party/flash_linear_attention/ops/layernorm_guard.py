...
class LayerNormFwdKernel(AphroditeJitKernel["LayerNormFwdKernel.CompileKey"]):
...
def _layer_norm_fn_impl(
    ...
    backward pass, but Aphrodite is inference-only so there is no backward pass.
...
def layernorm_fn(
    ...
    return _layer_norm_fn_impl(x, weight, bias, z, eps, group_size, norm_before_gate, is_rms_norm, activation)
...
def rmsnorm_fn(
    ...
    return _layer_norm_fn_impl(x, weight, bias, z, eps, group_size, norm_before_gate, True, activation)
