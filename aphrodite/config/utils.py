...
"""Utility functions for Aphrodite config dataclasses."""
...
def config(
    ...
    **kwargs: Any,
    ...
    All config classes in Aphrodite should use this decorator.
...
def _update_config(config: ConfigT, overrides: Mapping[str, Any], config_path: str) -> ConfigT:
    ...
    for field_name, value in overrides.items():
        ...
        if is_dataclass(current_value):
            ...
            if isinstance(value, Mapping):
                ...
                raise ValueError(
                    f"Override for {field_path} must be a mapping or {expected_type_name}, got {type(value).__name__}"
                )
