...
from aphrodite.config.model_arch import (
...
from aphrodite.tasks import PoolingTask, ScoreType, SupportedTask
...
if TYPE_CHECKING:
    ...
    import aphrodite.model_executor.models as me_models
...
class ModelConfig:
    ...
    - "auto" will try to use the Aphrodite implementation, if it exists, and fall back to the
      Transformers implementation if no Aphrodite implementation is available.
    ...
    mm_hasher_algorithm: InitVar[MMHasherAlgorithm | None] = cast(InitVar[MMHasherAlgorithm | None], None)
    ...
    mm_device_do_normalize: InitVar[bool | None] = cast(InitVar[bool | None], None)
    mm_processor_device: InitVar[MMProcessorDevice | None] = cast(InitVar[MMProcessorDevice | None], None)
    ...
    def __post_init__(
        ...
        # disabled, but Aphrodite uses None for that. Convert 0 to None to avoid errors.
        ...
        if self._model_info.supports_multimodal:
            ...
            mm_processor_kwargs = MultiModalConfig.fold_mm_processor_device(mm_processor_kwargs, mm_processor_device)
            ...
            mm_config_kwargs = dict(
                ...
                mm_device_do_normalize=self._resolve_mm_device_do_normalize(mm_device_do_normalize),
            )
    ...
    def validate_model_config_after(self: "ModelConfig") -> "ModelConfig":
        ...
        if self.enable_prompt_embeds and self.is_encoder_decoder:
            ...
            raise ValueError("--enable-prompt-embeds is not supported with encoder-decoder models.")
        return self

    def _resolve_mm_device_do_normalize(self, mm_device_do_normalize: bool | None) -> bool:
        if mm_device_do_normalize is None:
            if envs.APHRODITE_USE_RUST_FRONTEND:
                logger.debug(
                    "APHRODITE_USE_RUST_FRONTEND is set. "
                ...
                mm_device_do_normalize = self._model_info.supports_mm_device_do_normalize
                logger.debug(
                    "mm_device_do_normalize is %s by default.",
                    "enabled" if mm_device_do_normalize else "disabled",
                )
        else:
            if mm_device_do_normalize and envs.APHRODITE_USE_RUST_FRONTEND:
                logger.warning(
                    "APHRODITE_USE_RUST_FRONTEND is set. "
                    "Rust frontend does not currently support mm_device_do_normalize, "
                    "forcing mm_device_do_normalize = False."
            ...
            if mm_device_do_normalize and not self._model_info.supports_mm_device_do_normalize:
                logger.warning("Model does not support mm_device_do_normalize, forcing mm_device_do_normalize = False.")
                mm_device_do_normalize = False
    ...
    def maybe_untie_word_embeddings(self) -> None:
        ...
        [maybe_retie_word_embeddings][aphrodite.model_executor.model_loader.weight_tying.maybe_retie_word_embeddings].
        ...
        # Both levels must be updated: `AphroditeConfig.with_hf_config` reads the top
        ...
        if self.quantization in me_quant.DEPRECATED_QUANTIZATION_METHODS:
            if self.allow_deprecated_quantization:
                ...
                raise ValueError(
                    ...
                    "and will be removed in future versions of Aphrodite. To bypass, "
                    "set `--allow-deprecated-quantization`.",
                    self.quantization,
                )

    def _verify_cuda_graph(self) -> None:
        quant_config = self.model_arch_config.quantization_config or {}
        if self.quantization == "bitsandbytes" and quant_config.get("load_in_8bit", False) and not self.enforce_eager:
            logger.warning("BitsAndBytes 8-bit models require eager execution; disabling CUDA graphs.")
    ...
    def get_diff_sampling_param(self) -> dict[str, Any]:
        ...
        config = {} if src == "aphrodite" else self.try_get_generation_config()
        ...
        if diff_sampling_param and src != "aphrodite":
            logger.warning_once(
                "Default Aphrodite sampling parameters have been overridden by %s: `%s`. "
                "If this is not intended, please relaunch Aphrodite instance "
                "with `--generation-config aphrodite`.",
    ...
    def is_hybrid(self) -> bool:
        ...
        return layer_types is None or not all(layer in ("attention", "full_attention") for layer in layer_types)
    ...
    def is_chunked_prefill_supported(self) -> bool:
        ...
        if pooler_config := self.pooler_config:
            ...
            # aphrodite currently does not have pooling models using hybrid,
