...
def _layout_from_name(layout_name: str) -> KVCacheLayout:
    ...
    try:
        ...
        raise ValueError(
            f"Unknown KV cache layout {layout_name!r}. Valid layouts: {[m.name for m in KVCacheLayout]}"
        ) from None
...
class CacheConfig:
    ...
    prefix_cache_retention_interval: int | None = Field(default_factory=_get_prefix_cache_retention_interval, ge=0)
    ...
    to None, which means no KV offloading is enabled. When set, Aphrodite will
    ...
    'native' (Aphrodite native CPU offloading), 'lmcache'.
    ...
    def _apply_block_size_default(self) -> "CacheConfig":
        ...
        # another pydantic model (e.g. AphroditeConfig). Guard against that.
