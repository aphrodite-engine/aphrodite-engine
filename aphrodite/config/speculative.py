...
if TYPE_CHECKING:
    ...
    import aphrodite.model_executor.layers.quantization as me_quant
else:
...
def _validate_qwen3_omni_dspark(
    ...
    if not isinstance(block_size, int) or isinstance(block_size, bool) or block_size <= 0:
        raise ValueError("Qwen3-Omni DSpark requires a positive integer block_size in the draft config.")
    ...
    if target_text_config is None:
        raise ValueError("Qwen3-Omni DSpark could not resolve the target thinker text_config.")
    ...
    for name, target_value in target_attention.items():
        if not isinstance(target_value, int) or isinstance(target_value, bool) or target_value <= 0:
            raise ValueError(
    ...
    target_layer_ids = _get_nested_config_value(draft_hf_config, "dflash_config", "target_layer_ids")
    ...
    if not isinstance(target_layer_ids, (list, tuple)) or not target_layer_ids:
        raise ValueError("Qwen3-Omni DSpark requires a non-empty target_layer_ids list in the draft config.")
    if any(not isinstance(layer_id, int) or isinstance(layer_id, bool) for layer_id in target_layer_ids):
        raise ValueError("Qwen3-Omni DSpark target_layer_ids must contain only integers.")
    if list(target_layer_ids) != sorted(set(target_layer_ids)):
        raise ValueError("Qwen3-Omni DSpark target_layer_ids must be unique and strictly increasing.")
    ...
    if not isinstance(markov_rank, int) or isinstance(markov_rank, bool) or markov_rank <= 0:
        raise ValueError("Qwen3-Omni DSpark requires a positive integer markov_rank in the draft config.")
    markov_head_type = getattr(draft_hf_config, "markov_head_type", None)
    if markov_head_type != "vanilla":
        raise ValueError(f"Qwen3-Omni DSpark currently requires markov_head_type='vanilla'; got {markov_head_type!r}.")
    ...
    if sample_from_anchor is not True or bonus_anchor is not False:
        raise ValueError("Qwen3-Omni DSpark requires sample_from_anchor=true and dspark_bonus_anchor=false.")
    ...
    if (
        ...
        raise ValueError(
            f"Qwen3-Omni DSpark input vocab_size must be a positive integer; got {draft_input_vocab_size}."
        )
    ...
    noise_token_id = _get_nested_config_value(draft_hf_config, "dflash_config", "mask_token_id")
    for name in (
        "mask_token_id",
    ...
    has_mrope = has_mrope or any(
        isinstance(rope_config, Mapping) and "mrope_section" in rope_config for rope_config in rope_configs
    )
    if has_mrope:
        raise ValueError(
            "Qwen3-Omni DSpark draft checkpoints must use logical 1-D RoPE and must not define mrope_section."
        )
...
class SpeculativeConfig:
    ...
    def hf_config_override(hf_config: PretrainedConfig) -> PretrainedConfig:
        ...
        if hf_config.model_type == "dots3_note":
            ...
            hf_config.update({"n_predict": n_predict, "architectures": ["Dots3NoteMTPModel"]})
        ...
        if hf_config.model_type == "deepseek_mtp":
            ...
            hf_config.update(
                {
                    "n_predict": n_predict,
                    "architectures": ["DeepseekV32MTPModel" if use_v32_mtp else "DeepSeekMTPModel"],
                }
            )
        ...
        if hf_config.model_type == "bailing_hybrid" or "BailingMoeV2_5ForCausalLM" in architectures:
            hf_config.model_type = "bailing_hybrid_mtp"
        if hf_config.model_type == "bailing_hybrid_mtp":
            n_predict = getattr(hf_config, "num_nextn_predict_layers", None)
            hf_config.update({"n_predict": n_predict, "architectures": ["BailingMoeV25MTPModel"]})
        ...
        if hf_config.model_type == "kimi_k3":
            ...
            hf_config.update({"n_predict": n_predict, "architectures": ["KimiK3MTPModel"]})
        ...
        if hf_config.model_type == "qwen4_exp_mtp":
            ...
            share_mtp_indices = getattr(text_config, "index_share_for_mtp_iteration", False)
            hf_config.update(
                {
        ...
        elif hf_config.model_type == "bailing_hybrid" or "BailingMoeV2_5ForCausalLM" in architectures:
            hf_config.model_type = "bailing_hybrid_mtp"
        if hf_config.model_type == "bailing_hybrid_mtp":
            n_predict = getattr(hf_config, "num_nextn_predict_layers", None)
            hf_config.update(
        ...
        if hf_config.model_type in ("intern_s2_preview", "interns2_mobius"):
            ...
            is_moe = is_mobius or (getattr(text_config, "model_type", None) == "qwen3_5_moe_text")
            if is_mobius:
            ...
            hf_config.model_type = "qwen3_5_mtp"
            n_predict = getattr(text_config, "mtp_num_hidden_layers", None)
            architecture = "InternS2MobiusMTP" if is_mobius else ("Qwen3_5MoeMTP" if is_moe else "Qwen3_5MTP")
            hf_config.update(
                {
                    "n_predict": n_predict,
            ...
            hf_config.update(
                {
                    ...
                    "chain_hidden_post_norm": mtp_config.get("chain_hidden_post_norm", False),
        ...
        if hf_config.model_type == "minimax_m3_vl" or initial_architecture == "MiniMaxM3SparseForConditionalGeneration":
            ...
            hf_config.update({"n_predict": n_predict, "architectures": ["MiniMaxM3MTP"]})
        elif hf_config.model_type == "minimax_m3_mtp" or initial_architecture == "MiniMaxM3MTP":
    ...
    def compose_draft_hf_overrides(
        ...
        (for example, test harnesses shrinking ``num_hidden_layers``) and
        must also reach the draft config. Dict overrides are target-specific
        key patches and are not applied to the draft.

        The composed override must stay picklable because the draft
        ``ModelConfig`` is sent to spawned engine-core processes. Bind the
        target via ``functools.partial`` over a module-referenceable static
        method instead of using a local closure.
        """
    ...
    def __post_init__(self):
        ...
        if self.model is None and self.num_speculative_tokens is not None:
            if self.method == "mtp":
                ...
                self.model = self.target_model_config.model_weights or self.target_model_config.model
        ...
        if self.method in ("ngram", "ngram_gpu"):
            ...
            if self.model is not None:
                ...
                # deepseek-ai/dspark_qwen3_8b_block7
                    ...
                    or "MuseGlimmerAssistantModel" in self.draft_model_config.architectures
                ):
                    self.method = "dflash"
                elif (
                    "dspark" in self.draft_model_config.model.lower()
                    or "Qwen3DSparkModel" in self.draft_model_config.architectures
                    or _QWEN3_OMNI_DSPARK_ARCHITECTURE in self.draft_model_config.architectures
                    ...
                    self.method = "mtp"
                    if self.num_speculative_tokens > 1 and self.draft_model_config.hf_config.model_type not in (
                        "step3p5_mtp",
                        "inkling_mtp",
                    ):
                        logger.warning(
                ...
                if (
                    ...
                    and _QWEN3_OMNI_DSPARK_ARCHITECTURE not in self.draft_model_config.architectures
                    ...
                    self.update_arch_()
                elif self.method == "dspark" and "Gemma4DSparkModel" in self.draft_model_config.architectures:
                    ...
                    if getattr(hf, "n_predict", None) is None and getattr(hf, "block_size", None) is not None:
                ...
                if self.method == "dspark":
                    ...
                    if dspark_draft_topk is None:
                        dspark_draft_topk = getattr(hf_config, "dspark_draft_topk", None)
                    if dspark_draft_topk is not None:
                        draft_vocab_size = getattr(hf_config, "draft_vocab_size", None) or hf_config.vocab_size
                        ...
                        if (
                            ...
                            raise ValueError(
                                "dspark_draft_topk is only supported by Qwen3DSparkModel and Qwen3OmniDSparkModel"
                            )
        ...
        if self.index_share_for_mtp_iteration is not None:
            if self.method != "mtp" or self.draft_model_config is None:
                raise ValueError("index_share_for_mtp_iteration is only supported with method='mtp'")
            self.draft_model_config.hf_config.index_share_for_mtp_iteration = self.index_share_for_mtp_iteration
    ...
    def _maybe_override_draft_max_position_embeddings(
        ...
        """Raise an EAGLE draft's max position embeddings to the target's.

        The proposer feeds the draft positions up to the target's
        max_model_len, while max_position_embeddings sizes the draft's rotary
        cache. A smaller checkpoint value makes that cache gather go out of
        bounds (#48894).
        ...
        """
        draft_max_position_embeddings = getattr(draft_hf_config, "max_position_embeddings", None)
        if draft_max_position_embeddings is None or draft_max_position_embeddings >= target_max_model_len:
            return
        logger.info(
    ...
    def _verify_args(self) -> Self:
        ...
        if self.use_heterogeneous_vocab and not self.uses_draft_model():
            raise ValueError("use_heterogeneous_vocab only works with method='draft_model'")
    ...
    def use_gemma4_mtp(self) -> bool:
        return (
            self.method == "mtp"
            and self.draft_model_config is not None
            and getattr(self.draft_model_config.hf_config, "model_type", None) == "gemma4_mtp"
        )
    ...
    def use_step3p5_mtp(self) -> bool:
        return (
            self.method == "mtp"
            and self.draft_model_config is not None
            and getattr(self.draft_model_config.hf_config, "model_type", None) == "step3p5_mtp"
        )
    ...
    def use_multi_module_mtp(self) -> bool:
        if self.method != "mtp" or self.draft_model_config is None:
            return False
        num_mtp_layers = getattr(self.draft_model_config.hf_config, "num_nextn_predict_layers", 1)
