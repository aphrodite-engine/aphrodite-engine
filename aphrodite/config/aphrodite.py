...
ROCM_DEFAULT_MRV1_ARCHITECTURES = frozenset(
    {
        ...
        # Retain Sonar's fallback for the Kimi-K3 ROCm profiling fault.
...
def default_breakable_cudagraph_architectures() -> frozenset[str]:
    ...
    if current_platform.is_rocm():
        ...
        # APHRODITE_USE_BREAKABLE_CUDAGRAPH=1.
        return frozenset()
...
def enable_act_fusion(cfg: "AphroditeConfig") -> bool:
    """
...
def enable_allreduce_rms_fusion(cfg: "AphroditeConfig") -> bool:
...
def enable_rope_kvcache_fusion(cfg: "AphroditeConfig") -> bool:
...
def enable_mla_dual_rms_norm_fusion(cfg: "AphroditeConfig") -> bool:
...
class AphroditeConfig:
    ...
    ec_manager_config: EncoderCacheManagerConfig = Field(default_factory=EncoderCacheManagerConfig)
    ...
    def compute_hash(self, include_version: bool = True) -> str:
        ...
        Args:
            include_version: Include the Aphrodite version in the hash.
        """
        factors: list[Any] = []
        ...
        if self.model_config:
            ...
            if (
                ...
                aphrodite_factors.append(self.model_config.multimodal_config.compute_hash())
        else:
        ...
        if self.offload_config:
            aphrodite_factors.append(self.offload_config.compute_hash())
        else:
            aphrodite_factors.append("None")
        if self.attention_config:
            aphrodite_factors.append(self.attention_config.compute_hash())
        else:
            aphrodite_factors.append("None")
        if self.lora_config:
        ...
        if self.profiler_config:
            aphrodite_factors.append(self.profiler_config.compute_hash())
        else:
            aphrodite_factors.append("None")
        ...
        if self.ec_transfer_config:
            aphrodite_factors.append(self.ec_transfer_config.compute_hash())
        else:
            aphrodite_factors.append("None")
    ...
    def is_mm_encoder_only(self) -> bool:
        mm_config = self.model_config.multimodal_config if self.model_config is not None else None
    ...
    def num_speculative_tokens(self) -> int:
        ...
        if self.diffusion_config is not None and self.diffusion_config.canvas_length is not None:
    ...
    def _maybe_enable_breakable_cudagraph(self) -> bool:
        if "APHRODITE_USE_BREAKABLE_CUDAGRAPH" not in os.environ and self._uses_breakable_cudagraph_by_default():
            os.environ["APHRODITE_USE_BREAKABLE_CUDAGRAPH"] = "1"
            logger.info_once(
                "Auto-enabling APHRODITE_USE_BREAKABLE_CUDAGRAPH=1. Set APHRODITE_USE_BREAKABLE_CUDAGRAPH=0 to opt out."
            )
    ...
    def with_hf_config(
        ...
        # in SomeVLTextConfig*. In Aphrodite, the lm_head belongs to the language_model, so
        ...
        if model_config.is_multimodal_model and hasattr(model_config.hf_config, "tie_word_embeddings"):
    ...
    def _maybe_override_dynamic_sd_cudagraph_mode(self) -> None:
        ...
        logger.warning_once(
            ...
            "PIECEWISE for reliability. Use APHRODITE_USE_V2_MODEL_RUNNER=1 "
    ...
    def __post_init__(self):
        ...
        if self.kv_transfer_config is not None and self.kv_transfer_config.has_connector("NixlConnector"):
            ...
            if self.model_config is not None:
                ...
                assert not (self.model_config.is_hybrid and dcp_size > 1), (
                    "PD with decode_context_parallel_size > 1 is not supported for hybrid Mamba/SSM models."
                )
        ...
        # "dummy" reads no weights at all, and the sharded formats read a Aphrodite
        ...
        if (
            ...
            from aphrodite.utils.deep_gemm import should_auto_disable_deep_gemm
            ...
            if should_auto_disable_deep_gemm(model_type):
                ...
                logger.warning_once(
                    ...
                    "To disable DeepGemm globally, set APHRODITE_USE_DEEP_GEMM=0.",
                    model_type,
                )
        ...
        if self.observability_config.per_request_spec_decode_metrics != "none" and self.speculative_config is None:
            raise ValueError(
        ...
        if self.is_mm_encoder_only and self.cache_config.enable_prefix_caching:
            ...
            logger.info("Disabling prefix caching: this instance runs the multi-modal encoder only.")
        ...
        if self.scheduler_config.disable_hybrid_kv_cache_manager is None:
            ...
            raise ValueError(
                ...
                "--no-disable-hybrid-kv-cache-manager flag to let Aphrodite decide"
                " automatically."
            )
    ...
    def _set_max_num_scheduled_tokens(self):
        ...
        if self.speculative_config is not None:
            ...
            if max_num_batched_tokens <= scheduled_token_delta:
                raise ValueError(
                    "AphroditeConfig does not have enough slots to schedule a token and"
    ...
    def _set_cudagraph_sizes(self):
        ...
        if (
            ...
            if max_cudagraph_capture_size is None:
                ...
                max_cudagraph_capture_size = min(max_num_seqs * decode_query_len * 2, default_max_graph_size)
                if decode_query_len > 1:
                    ...
                    speculative_config = self.speculative_config
                    if speculative_config is not None and speculative_config.uses_dynamic_speculative_decoding():
                        from aphrodite.v1.spec_decode.dynamic.utils import (
                        ...
                        try:
                            dense_schedule = build_dynamic_sd_schedule_lookup(
                                schedule,
                                aphrodite_max_batch_size=max_num_seqs,
                                aphrodite_num_speculative_tokens=self.num_speculative_tokens,
                            )
                        except ValueError:
                            pass
                        else:
                            ...
                            for batch_size, num_spec in enumerate(dense_schedule[1:], start=1):
            ...
            if self.compilation_config.cudagraph_capture_sizes is not None:
                ...
                cudagraph_capture_sizes += [size for size in uniform_decode_sizes if size <= max_num_tokens]
    ...
    def _resolve_allow_missing_mm_embeddings(self) -> None:
        ...
        mm_config.allow_missing_mm_embeddings = (ec_config is not None and ec_config.is_ec_consumer) or (
            kv_config is not None and kv_config.is_kv_consumer
        )
        if mm_config.allow_missing_mm_embeddings:
            logger.info_once("EC/KV consumer: pre-computed-embedding inputs may omit the embedding tensor.")
    ...
    def _resolve_mm_encoder_only(self) -> None:
        ...
        if mm_config is None:
            raise ValueError("An EC producer-only instance requires a multimodal model.")
    ...
    def _resolve_mm_processor_device(self) -> None:
        ...
        logger.info_once(
            "EPD encoder instance: running the multi-modal processor on %s. Override with --mm-processor-device=cpu.",
            device_type,
        )
    ...
    def _get_v2_model_runner_unsupported_features(self) -> list[str]:
        ...
        if self.compilation_config.pass_config.enable_sp and self.parallel_config.tensor_parallel_size > 1:
    ...
    def _validate_adaptive_verification(self) -> None:
        ...
        if self.compilation_config.cudagraph_mode == CUDAGraphMode.NONE:
            ...
            raise ValueError("Adaptive verification is not currently compatible with enforce_eager/cudagraph_mode=none")
        ...
        if self.parallel_config.pipeline_parallel_size > 1:
            ...
            raise ValueError("Adaptive verification is not currently compatible with pipeline parallelism")
    ...
    def _validate_batch_sharded_sampling(self) -> None:
        ...
        if tp_size <= 1:
            ...
            blockers.append(
                f"max_num_seqs ({self.scheduler_config.max_num_seqs}) is below tensor_parallel_size ({tp_size})"
            )
        ...
        if self.model_config is not None and self.model_config.return_sampling_mask:
            ...
            blockers.append("return_sampling_mask is set and the batch-sharded gather does not forward sampling masks")

        if self.speculative_config is not None and self.speculative_config.enable_adaptive_verification:
    ...
    def _get_dbo_unsupported_features(self) -> list[str]:
        ...
        # It should be enabled with explicit APHRODITE_USE_V2_MODEL_RUNNER environ.
        # Remove it when stable.
        if envs.APHRODITE_USE_V2_MODEL_RUNNER is None:
        ...
        if model_config is not None and (model_config.is_multimodal_model or model_config.is_encoder_decoder):
    ...
    def _validate_v1_model_runner(self) -> None:
        ...
        if unsupported:
            raise ValueError(f"Model Runner V1 does not support: {', '.join(unsupported)}")

    def adjust_dcp_kv_cache_interleave_size(self, kv_cache_config: "KVCacheConfig") -> None:
        ...
        if self.kv_transfer_config is None or not self.kv_transfer_config.has_connector("NixlConnector"):
            return
        ...
        local_block_size = min(g.kv_cache_spec.block_size for g in kv_cache_config.kv_cache_groups)
    ...
    def validate_block_size(self) -> None:
        ...
        nixl_pd_active = self.kv_transfer_config is not None and self.kv_transfer_config.has_connector("NixlConnector")
    ...
    @model_validator(mode="after")
    def validate_mamba_cached_kernel(self) -> "AphroditeConfig":
        ...
        if self.model_config is not None and not self.model_config.supports_replayssm:
            raise ValueError(f"--use-replayssm is not supported for architecture {self.model_config.architecture!r}")
        if self.cache_config.use_kda_recoverssm:
            ...
            if self.cache_config.mamba_cache_mode == "align" and not self.use_v2_model_runner:
                raise ValueError("RecoverSSM with align mode requires APHRODITE_USE_V2_MODEL_RUNNER=1")
            if self.parallel_config.pipeline_parallel_size > 1:
            ...
            if self.cache_config.mamba_cache_mode == "align":
                raise ValueError("FlashInfer ReplaySSM does not support --mamba-cache-mode align yet; use none")
        elif self.mamba_config.backend != MambaBackendEnum.TRITON:
            raise ValueError("--use-replayssm requires --mamba-backend triton or flashinfer")
        elif self.use_v2_model_runner:
            raise ValueError(
                "Triton ReplaySSM requires Model Runner V1; use --mamba-backend flashinfer or Model Runner V1"
            )
        if self.kv_transfer_config is not None and self.kv_transfer_config.is_kv_transfer_instance:
            raise ValueError(
...
def get_current_aphrodite_config() -> AphroditeConfig:
    if _current_aphrodite_config is None:
        raise AssertionError(
            "Current Aphrodite config is not set. This typically means "
            "get_current_aphrodite_config() was called outside of a "
            "set_current_aphrodite_config() context, or a CustomOp was instantiated "
            ...
            "'default_aphrodite_config' pytest fixture from tests/conftest.py."
        )
    return _current_aphrodite_config


def get_current_aphrodite_config_or_none() -> AphroditeConfig | None:
    return _current_aphrodite_config
