...
from aphrodite.config.utils import config, get_from_deprecated_env_if_set
...
[`MultiModalDummyOptionsBuiltins`][aphrodite.config.multimodal.MultiModalDummyOptionsBuiltins].
"""
...
class MultiModalConfig:
    ...
    mm_hasher_algorithm: MMHasherAlgorithm = Field(default_factory=_get_mm_hasher_algorithm)
    ...
    Derived, not user-settable: `AphroditeConfig.__post_init__` sets this to True
    ...
    def fold_mm_processor_device(
        ...
        `AphroditeConfig`, which is where the EC role needed to resolve it lives.
        ...
        device = "cpu" if mm_processor_device == "cpu" else (current_platform.device_type or "cpu")
    ...
    def validate_mm_processor_device(self, ec_config: ECTransferConfig | None) -> None:
        ...
        logger.info_once(
            "Running the multi-modal processor on %s. Override with --mm-processor-device=cpu.",
            device_type,
        )
