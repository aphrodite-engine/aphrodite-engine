...
from aphrodite.qaic.transformers.modeling_attn_mask_utils import (
    _create_causal_mask)
...
class QEffLlamaRotaryEmbedding(LlamaRotaryEmbedding):
    ...
    def __init__(
        ...
        if config is None:
            logger.warning_once(
                "`LlamaRotaryEmbedding` can now be fully parameterized by "
                "passing the model config through the `config` argument. All "
                "other arguments will be removed in v4.45"
            )
            self.rope_kwargs = {
                "rope_type": rope_type,
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            if self.layer_idx is None:
                raise ValueError(
                    "The cache structure has changed since version v4.36. If "
                    f"you are using {self.__class__.__name__} "
                    "for auto-regressive decoding with k/v caching, please "
                    "make sure to initialize the attention class "
                    "with a layer index."
                )
...
class QEffLlamaForCausalLM(LlamaForCausalLM):
    ...
    def forward(
        ...
        hidden_states = outputs[0][
            torch.arange(position_ids.shape[0]).view(-1, 1), logit_index
        ]
        if self.config.pretraining_tp > 1:
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask (`torch.FloatTensor`, *optional*):
                attention mask of size `(batch_size, sequence_length)` if
                flash attention is used or `(batch_size, 1,
                query_sequence_length, key_sequence_length)` if default
                attention is used.
...
class QEffLlamaModel(LlamaModel):
    ...
    def _update_causal_mask(
        ...
        # TODO: As of torch==2.2.0, the `attention_mask` passed to the model
        # in `generate` is 2D and of dynamic length even when the static
        ...
        # (`recording cudagraph tree for symint key 13`, etc.), which is VERY
        # slow. A workaround is `@torch.compiler.disable`, but this prevents
        # using `fullgraph=True`. See more context in
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            if attention_mask.max() != 0:
                raise ValueError(
                    "Custom 4D attention mask should be passed in inverted "
                    "form with max==0`"
                )
            ...
            if attention_mask is not None:
                ...
                causal_mask = _create_causal_mask(
                    position_ids=position_ids, target_length=target_length
                )
