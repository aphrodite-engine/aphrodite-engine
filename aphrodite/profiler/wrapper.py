...
class TorchProfilerWrapper(WorkerProfiler):
    ...
    def _maybe_add_version_metadata(self) -> None:
        """Stamp the Aphrodite version (which embeds the git commit) into the trace.
        ...
        if self.profiler.profiler is None:
            return
        try:
            self.profiler.add_metadata_json("aphrodite_version", json.dumps(aphrodite.version.__version__))
            self.profiler.add_metadata_json(
                "aphrodite_version_tuple",
                json.dumps([str(p) for p in aphrodite.version.__version_tuple__]),
            )
        except Exception as e:
            logger.warning("Failed to add Aphrodite version to profiler metadata: %s", e)
...
class ProtonProfilerWrapper(WorkerProfiler):
    ...
    def __init__(
        ...
        try:
            ...
            raise RuntimeError(
                "The Proton profiler requires a Triton installation with triton.profiler support."
            ) from exc
    ...
    def _require_triton_version(self, feature: str, minimum: Version) -> None:
        if self._triton_version is None or self._triton_version < minimum:
            raise RuntimeError(f"Proton {feature} requires Triton >= {minimum}; found {self._triton_version_string}.")
    ...
    def _validate_capabilities(self) -> None:
        if self._output_format is not None:
            ...
            if not supports_output_format:
                raise RuntimeError(
                    "The installed Triton Proton does not support selecting an output format during finalize."
                )

        if self._output_format == "hatchet_msgpack":
            self._require_triton_version("hatchet_msgpack output", _TRITON_PROTON_3_7_VERSION)
        if self._mode and self._mode.split(":", 1)[0] == "periodic_flushing":
            self._require_triton_version("periodic flushing", _TRITON_PROTON_3_7_VERSION)
