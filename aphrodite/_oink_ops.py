...
Aphrodite does not depend on the external Oink repository/package. When an external
plugin registers torch.library.custom_op entrypoints under the `oink::`
namespace (e.g. via Aphrodite's general_plugins mechanism) and
`APHRODITE_USE_OINK_OPS=1` is set, Aphrodite can route eligible calls to those ops.
...
def is_oink_available_for_device(device_index: int) -> bool:
    ...
    APHRODITE_USE_OINK_OPS=1, so if the ops are present they should be usable.
    """
