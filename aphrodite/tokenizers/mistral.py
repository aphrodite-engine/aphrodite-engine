...
def tekken_convert_ids_to_tokens(tokenizer: "Tekkenizer", ids: Sequence[int]) -> list[str | bytes]:
...
def tekken_convert_tokens_to_string(tokenizer: "Tekkenizer", tokens: Sequence[str | bytes]) -> str:
...
class MistralTokenizer(TokenizerLike):
    IS_MISTRAL_TOKENIZER = True  # used by aphrodite.utils.mistral
    ...
    def convert_ids_to_tokens(
        ...
        if self.tokenizer.is_special(SpecialTokens.args):
            non_skip_special_tokens_ids.add(self.tokenizer.get_special_token(SpecialTokens.args))
        ...
        for think_token in (SpecialTokens.begin_think, SpecialTokens.end_think):
            if self.tokenizer.is_special(think_token):
                non_skip_special_tokens_ids.add(self.tokenizer.get_special_token(think_token))
