...
from aphrodite.omni.utils.qwen3_force_align_processor import build_prompt, segment_words
...
POOLING_OUTPUT_DECODER_PATH = (
    "aphrodite.omni.model_executor.stage_input_processors.forced_aligner.decode_pooling_output"
)
...
def decode_pooling_output(logits: Any, request: Any, hf_config: Any) -> Any:
    ...
    from aphrodite.omni.utils.forced_aligner import _decode_timestamps

    ts_id = int(hf_config.timestamp_token_id)
    thinker_cfg = getattr(hf_config, "thinker_config", hf_config)
    classify_num = int(thinker_cfg.classify_num)
    seg_ms = float(hf_config.timestamp_segment_time)
