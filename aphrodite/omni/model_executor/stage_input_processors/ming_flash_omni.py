...
def _resolve_num_query_tokens(stage: Any) -> int | None:
    ...
    try:
        hf_config = stage.aphrodite_config.model_config.hf_config
...
def _slice_patch_hidden(
    ...
    if num_query_tokens is not None and image_end_token_id is not None:
        ...
        if num_query_tokens + 1 <= L and int(prompt_ids_t[-1].item()) == image_end_token_id:
...
def _build_talker_inputs(
    ...
    for i, source_output in enumerate(source_outputs):
        ...
        if original_prompt is not None:
            prompt_info = original_prompt.get("additional_information")
...
setattr(thinker2talker_token_only, "_is_sync_input", True)
