...
from aphrodite.omni.model_executor.models.mimo_audio.config_mimo_audio import TALKER_CODEC_PAD_TOKEN_ID
...
# ``aphrodite/omni/deploy/mimo_audio.yaml`` and the offline example
    ...
    if multimodal_output is None or not isinstance(multimodal_output, dict):
        if is_finished:
            ...
            if not isinstance(request_id, str):
                raise ValueError("MiMo audio request requires an external request ID")
