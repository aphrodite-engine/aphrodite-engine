...
def thinker2talker_token_only(
    ...
    if not isinstance(prompt, list):
        prompt = [prompt]
    multi_modal_data = {
        thinker_output.request_id: p.get("multi_modal_data", None) if p is not None else None
...
def thinker2talker_full_payload(
    ...
    finish-reason-aware stop-row trim: Sonar v1 appends the sampled
    ...
    def _ensure_list(x):
        if x is None:
            return []
        if hasattr(x, "_x"):
            # Sonar wraps cached token-id lists in ConstantList-like objects.
            return list(x._x)
    ...
    if stop_emission_drop == 0 and not status_name and output_token_ids:
        # Worker-side CachedRequestState has no `.status` field in Sonar v1;
