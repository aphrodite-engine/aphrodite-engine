...
from aphrodite.omni.model_executor.models.nemotron_voicechat.runtime_info import scalar_bool
...
def _request_info(request: Any) -> dict[str, Any]:
    ...
    for name in (
        ...
        if info is not None and hasattr(info, "entries"):
            from aphrodite.omni.engine.serialization import (
...
def thinker2talker_token_only(
    ...
    PAD prompt region. The thinker's Sonar prompt covers
    ...
    # starts at t=1) — the vendored talker paths ignore the Sonar positions.
    # The NATIVE talker (hf_overrides.use_native_talker) instead maps Sonar
...
def thinker2talker_async_chunk(
    ...
    **kwargs: Any,
    ...
    if is_duplex:
        ...
        # Sonar prompt = logical prompt + 1 placeholder (acoustic frame 0).
        logical_prompt_len = max(len(prompt_ids) - 1, 0)
        generated = generated_now
    pad_id = _DEFAULT_TEXT_PAD_ID
