...
from aphrodite.omni.data_entry_keys import (
    CodesStruct,
    EmbeddingsStruct,
    HiddenStatesStruct,
    IdsStruct,
...
from aphrodite.omni.model_executor.stage_input_processors.tts_utils import (
...
def _get_prefill_multimodal_output(
    ...
    return dict(prefill_mm) if isinstance(prefill_mm, Mapping) else None
...
def _get_qwen3_streaming_state(
    ...
    bridge_states = getattr(streaming_context, "bridge_states", None)
    if not isinstance(bridge_states, dict):
        raise ValueError("Streaming context requires bridge_states")
...
def thinker2talker_async_chunk(
    ...
    if chunk_id == 0:
        ...
        if transfer_manager.request_payload.get(request_id) is None:
            ...
            assert payload.embed is not None and payload.hidden_states is not None
...
def thinker2talker_token_only(
    ...
    for i, thinker_output in enumerate(source_outputs):
        ...
        next_prompt = OmniTokensPrompt(
            prompt_token_ids=[0] * prompt_len,
            multi_modal_data=None,
            mm_processor_kwargs=None,
        )
        if additional_information:
            next_prompt["additional_information"] = additional_information
        talker_inputs.append(next_prompt)
...
def talker2code2wav_async_chunk(
    ...
    if append_codes and isinstance(code_predictor_codes, torch.Tensor):
