...
"""MOSS-TTS-Nano single-stage model for Sonar Omni.
...
to avoid OOM from Sonar's post-init KV-cache profiling. Mirrors qwen3_tts
(PR #3117).
"""
...
from aphrodite.omni.model_executor.models.utils import (
...
def _hf_load_without_tp_warmup():
    ...
    Sonar initializes ``torch.distributed`` even at TP=1, so the crash fires.
...
# aphrodite/omni/entrypoints/openai/serving_speech.py).
...
class MossTTSNanoForGeneration(nn.Module):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        # See aphrodite.omni.model_executor.models.utils.reinit_rotary_inv_freq
        ...
        # Kept for API compat; Sonar worker init is single-threaded.
    ...
    def compute_logits(
        ...
        safe_id = 1 if vocab_size > 1 and eos_id != 1 else 0
