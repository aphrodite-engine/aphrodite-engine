...
from aphrodite.model_executor.models.qwen2_5_omni_thinker import (
...
from aphrodite.model_executor.models.qwen2_audio import Qwen2AudioProcessingInfo
from aphrodite.model_executor.models.qwen3_moe import Qwen3MoeForCausalLM
from aphrodite.model_executor.models.qwen3_moe import Qwen3MoeModel as _Qwen3MoeLLMModel
...
from aphrodite.model_executor.models.utils import WeightsMapper, _merge_multimodal_embeddings, maybe_prefix
from aphrodite.multimodal import MULTIMODAL_REGISTRY
from aphrodite.multimodal.inputs import MultiModalFeatureSpec, MultiModalKwargsItems
from aphrodite.multimodal.parse import AudioProcessorItems, MultiModalDataItems
...
from aphrodite.multimodal.utils import set_mm_embedding_modality
from aphrodite.omni.model_executor.models.qwen2_5_omni.qwen2_5_omni_thinker import (
...
from aphrodite.omni.quantization.component_config import (
...
class Qwen3MoeLLMForCausalLM(Qwen3MoeForCausalLM):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        # head. Sonar 0.23.0 made ParallelLMHead quantizable by NVFP4 (0.22's
        ...
        # weight fails to load. Mirrors Sonar's stock Qwen3MoeForCausalLM.
    ...
    def _call_hf_processor(
        ...
        if audios:
            ...
            if Version(TRANSFORMERS_VERSION) < Version("4.58.0"):
                ...
                # WhisperFeatureExtractor has a fixed sampling rate, and Sonar's
                ...
                if audio_sample_rate is not None:
                    ...
                    if audio_sample_rate != expected_sr:
                        logger.warning(
                            ...
                            "Sonar's audio loader already resampled to %dHz.",
...
class Qwen3OmniMoeThinkerForConditionalGeneration(
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        if isinstance(quant_config, ComponentQuantizationConfig):
            ...
            if quant_config.get_name() in PRE_QUANTIZED_METHODS:
                ...
                aphrodite_config = replace(aphrodite_config, quant_config=quant_config)
                audio_quant_config = None
                visual_quant_config = None
                language_quant_config = quant_config.resolve("language_model")
        else:
        ...
        with self._mark_language_model(aphrodite_config):
            lm_aphrodite_config = aphrodite_config.with_hf_config(
                thinker_config.text_config,
                architectures=["Qwen3MoeForCausalLM"],
            )
            if language_quant_config is not quant_config:
                lm_aphrodite_config = replace(lm_aphrodite_config, quant_config=language_quant_config)
    ...
    def _get_deepstack_input_embeds(
        ...
        # (Sonar gpu_model_runner). Tail positions have no vision deepstack and must
        ...
        if self.visual.deepstack_visual_indexes is not None and any(has_vision_embeddings):
            ...
            for index, embeddings in enumerate(multimodal_embeddings):
                ...
                if embeddings.shape[-1] != self.config.text_config.hidden_size:
                    ...
                    # tag since Sonar #46213, so carry it across the split.
    ...
    def forward(
        ...
        **kwargs: object,
        ...
        if capture_layer_indices is None and not return_hidden_states:
            # No capture requested. Return a bare tensor: stock Sonar's
            ...
            # global registry (e.g. plain `aphrodite serve` of the thinker).
            return hidden_states
        return hidden_states, captured_hidden_states
