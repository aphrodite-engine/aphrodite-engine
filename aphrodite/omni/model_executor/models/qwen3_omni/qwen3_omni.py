...
from aphrodite.model_executor.models.qwen3_asr_realtime import Qwen3ASRRealtimeBuffer
from aphrodite.model_executor.models.qwen3_omni_moe_thinker import (
...
from aphrodite.omni.data_entry_keys import Embeddings, HiddenStates, Ids, OmniPayload, OmniPayloadMeta
from aphrodite.omni.metrics import definitions as defs
from aphrodite.omni.model_executor.custom_process_mixin import CustomProcessMixin
from aphrodite.omni.model_executor.models.output_templates import OmniOutput
from aphrodite.omni.model_executor.models.qwen3_omni.quantization import (
    apply_outer_quant_config_mapping,
)
from aphrodite.omni.model_executor.models.qwen3_omni.qwen3_omni_moe_thinker import (
    Qwen3OmniMoeThinkerDummyInputsBuilder,
    Qwen3OmniMoeThinkerForConditionalGeneration,
    Qwen3OmniMoeThinkerMultiModalProcessor,
    Qwen3OmniMoeThinkerProcessingInfo,
)
from aphrodite.omni.model_executor.models.utils import add_prefix_to_loaded_weights, safe_tensor_reshape
...
from aphrodite.v1.outputs import SamplerOutput
...
class Qwen3OmniMoeForConditionalGeneration(
    ...
    Usage:
        Set `model_stage` in aphrodite_config to one of: "thinker", "talker", "code2wav"
    """

    # Sonar applies quantization-config name mapping before constructing this
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        config: Qwen3OmniMoeConfig = aphrodite_config.model_config.hf_config
        multimodal_config = aphrodite_config.model_config.multimodal_config

        # Keep aphrodite_config for later submodule init
        ...
        # startup (engine/stage_init_utils.py); a plain, non-staged Sonar run
        ...
        # such as the Sonar-text perf benchmark.
        #
        ...
        # accepted.
        self.model_stage = getattr(aphrodite_config.model_config, "model_stage", None) or "thinker"
        # Staged startup always injects model_stage; its absence means a plain
        # Sonar run with no talker stage downstream, so no one consumes captured
        # thinker layers and the forward must return what stock Sonar expects.
        self.is_staged_run = getattr(aphrodite_config.model_config, "model_stage", None) is not None

        if self.model_stage == "thinker":
            self.use_async_omni_output = True
            # Initialize thinker model (multimodal processing + text generation)
            # Create a new aphrodite_config with thinker_config as the hf_config
            thinker_aphrodite_config = aphrodite_config.with_hf_config(
                thinker_config, architectures=["Qwen3OmniMoeThinkerForConditionalGeneration"]
            )
            self.thinker = init_aphrodite_registered_model(
                aphrodite_config=thinker_aphrodite_config,
            ...
            # Create a new aphrodite_config with talker_config as the hf_config
            # This ensures the talker uses its own text_config (smaller vocab_size)
            talker_aphrodite_config = aphrodite_config.with_hf_config(
                talker_config, architectures=["Qwen3OmniMoeTalkerForConditionalGeneration"]
            )
            self.talker = init_aphrodite_registered_model(
                aphrodite_config=talker_aphrodite_config,
            ...
            # Create a new aphrodite_config with code2wav_config as the hf_config
            code2wav_aphrodite_config = aphrodite_config.with_hf_config(
                code2wav_config, architectures=["Qwen3OmniMoeCode2Wav"]
            )
            self.code2wav = init_aphrodite_registered_model(
                aphrodite_config=code2wav_aphrodite_config,
    ...
    def embed_input_ids(
        ...
        if self.model_stage == "code2wav":
            return (
                ...
                .reshape(-1, 1)
                .repeat(1, self.aphrodite_config.model_config.get_hidden_size())
            )
    ...
    def forward(
        ...
        **kwargs: object,
        ...
        if self.model_stage == "thinker":
            ...
            # plain Sonar run capturing would waste a clone per layer per step
            # and make the thinker return a tuple stock Sonar cannot handle.
    ...
    def generate_audio(
        ...
        if self.aphrodite_config.model_config.async_chunk:
    ...
    def _get_codec_frame_config(self) -> tuple[int, int]:
        ...
        model_cfg = getattr(self.aphrodite_config, "model_config", None)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        if self.code2wav and code2wav_weights:
            ...
            # enforce_eager setting, the same switch Sonar uses for outer graphs.
            try:
