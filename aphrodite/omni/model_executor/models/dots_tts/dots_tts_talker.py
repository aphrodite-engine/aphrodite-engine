...
"""dots.tts talker — Sonar-native AR base LM + audio side path.

Mirrors upstream rednote-hilab/dots.tts (pinned @ a393d2e):

  * Qwen2.5-1.5B base LM runs under Sonar PagedAttention (stock
    ``Qwen2Model``); weights come from the checkpoint's ``llm.model.*``
    namespace.  ``DotsTTSConfig`` hoists the LM fields to top-level
    attributes.
  * Everything else is side computation, invisible to the Sonar
...
Reference implementations: ``aphrodite/omni/model_executor/models/voxcpm2/``
(architectural template: Sonar-native base LM + side path + per-request
...
from aphrodite.model_executor.models.qwen2 import Qwen2Model
from aphrodite.model_executor.models.utils import AutoWeightsLoader, maybe_prefix
from aphrodite.omni.model_executor.models.dots_tts.dots_tts_dit import DiT
from aphrodite.omni.model_executor.models.dots_tts.dots_tts_patch_encoder import (
    VAESemanticEncoder,
)
from aphrodite.omni.model_executor.models.dots_tts.dots_tts_speaker_encoder import (
    SpeakerXVectorFeatures,
)
from aphrodite.omni.model_executor.models.dots_tts.dots_tts_vocoder import (
    AudioVAE,
    AudioVAEConfig,
)
...
class DotsTTSForConditionalGeneration(nn.Module):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        # sonar-omni capability flags — gpu_model_runner checks these to decide
        ...
        # bf16 dtype via Sonar's auto-cast and dtype-mismatch under
        ...
        # runtime.py:81 where only `model.core` is cast to bf16).  Sonar's
        ...
        latent_stats_path = self._resolve_latent_stats_path(aphrodite_config.model_config.model)
        ...
        # _deferred_cleanup_ids: requests Sonar has marked finished but whose
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        One Sonar step: base LM forward under PagedAttention, then slice
        ...
        Sonar only ever sees the returned hidden states; audio leaves
    ...
    def embed_input_ids(self, input_ids: torch.Tensor, **_: Any) -> torch.Tensor:
        ...
        Required by Sonar's ``is_text_generation_model`` Protocol check
        (interfaces_base._check_aphrodite_model_embed_input_ids).  Without this
    ...
    def on_requests_finished(self, finished_req_ids: Iterable[str]) -> None:
        ...
        Sonar scheduler calls this BEFORE forward() to notify that certain
    ...
    def _finish_decode(
        ...
        One Sonar step = one audio patch.  patch_encoder.decode_patch
        ...
        embeddings), so Sonar's span_len=1 decode matches naturally — no
        ...
        #    drains the high-stop logit on the next dispatch and Sonar
        ...
        if eos_logits[0, -1, 1].item() > 0.8:
            ...
            # request ends when Sonar sees this step's stop token, so drain
    ...
    def compute_logits(
        ...
        """Per-step stop signal logits (Sonar sampling protocol).
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        * **Qwen2 base LM**: keys under ``llm.model.``, handed off to Sonar's
        ...
        * **CAM++ speaker encoder**: keys from ``speaker_encoder.safetensors``
        ...
        if matched_llm:
            # Sonar's Qwen2Model.load_weights does its own fusion + tolerates
