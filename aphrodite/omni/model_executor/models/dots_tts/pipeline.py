...
Architecture: Qwen2.5-1.5B base LM (Sonar-native) → DiTAR / MeanFlow head
→ AudioVAE.decode → BigVGAN → 48 kHz mono.

Speaker conditioning via CAM++ x-vector injected in preprocess().

Modeled after voxcpm2's single-stage pipeline (same "Sonar-native base LM
    ...
    stages=(
        StagePipelineConfig(
            ...
            sampling_constraints={
                ...
                # talker emits logits[i, 1] = 1.0 when prob_stop > 0.8, Sonar
