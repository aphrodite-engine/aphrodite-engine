...
class HiggsAudioV3Code2Wav(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if aphrodite_config is not None:
            ...
            if config is None:
                raise TypeError("HiggsAudioV3Code2Wav: provide either `config` or `aphrodite_config`.")
            self.config = config
    ...
    def _ensure_codec_loaded(self) -> None:
        """Build the codec when Sonar's weight loader skipped ``load_weights``.
        ...
        if self.aphrodite_config is not None:
            try:
