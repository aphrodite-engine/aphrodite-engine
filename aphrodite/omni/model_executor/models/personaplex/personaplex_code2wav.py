...
in Sonar's safetensors loader, so they are loaded eagerly in ``load_weights``
part of the Sonar weights iterator (the codec owns its own checkpoint; see the
...
class PersonaPlexCode2Wav(nn.Module):
    ...
    Wraps the external Mimi decoder. The Sonar generation runner only calls a
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        self._max_codec_sessions = int(getattr(aphrodite_config.model_config, "duplex_max_sessions", 1))

        # The Mimi module is constructed in load_weights() (it owns its own
        # weight format) and assigned here so Sonar's memory profiler can see it.
    ...
    def embed_input_ids(self, input_ids: torch.Tensor, **_: Any) -> torch.Tensor:
        ...
        # the Sonar runner has a valid hidden-state placeholder.
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        The primary Sonar weights iterator carries no Code2Wav parameters (Mimi
        ...
        from aphrodite.omni.model_executor.models.personaplex.personaplex_mimi import (
