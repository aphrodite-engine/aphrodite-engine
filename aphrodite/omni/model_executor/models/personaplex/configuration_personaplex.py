...
:class:`~aphrodite.omni.model_executor.models.personaplex.configuration_helium.HeliumConfig`
...
from aphrodite.omni.model_executor.models.personaplex.configuration_helium import (
    HeliumConfig,
)
...
class PersonaPlexMimiConfig(PretrainedConfig):
    ...
    config only carries the scalars the sonar-omni serving layer needs to size
...
class PersonaPlexConfig(PretrainedConfig):
    ...
    that Sonar consumes (it exposes ``hidden_size`` / ``num_attention_heads`` and
    drives the talker's Sonar Llama backbone).
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # supply the primary (stage-0 talker) arch so Sonar's ModelConfig resolves a
    ...
    def get_text_config(self, **kwargs: Any) -> PretrainedConfig:
        # Sonar expects the text config to expose hidden_size / num_attention_heads.
        # For PersonaPlex the temporal-transformer (Helium) config is the text
        # model config that drives the talker's Sonar Llama backbone.
