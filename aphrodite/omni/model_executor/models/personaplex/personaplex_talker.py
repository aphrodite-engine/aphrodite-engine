...
"""PersonaPlex talker: the temporal transformer as a Sonar-native omni AR stage.
...
* the Helium temporal transformer (:class:`HeliumModel`) on Sonar paged attention,
...
from aphrodite.omni.model_executor.models.personaplex.modeling_helium import HeliumModel
from aphrodite.omni.model_executor.models.personaplex.personaplex_depformer import (
    PersonaPlexDepformer,
)
from aphrodite.omni.model_executor.models.personaplex.personaplex_embeddings import (
...
class PersonaPlexTalkerForConditionalGeneration(nn.Module):
    """Sonar-native PersonaPlex talker (temporal transformer + depformer).

    Plain ``nn.Module`` (not ``SupportsPP``): inheriting a Sonar Protocol puts
    ``Protocol`` in the MRO and breaks Sonar's runtime ``isinstance`` check for
    ``AphroditeModelForTextGeneration``, which would misclassify the talker as a
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        config: PersonaPlexConfig = aphrodite_config.model_config.hf_config  # type: ignore[assignment]
        self.config = config
        ...
        # Temporal backbone on Sonar paged attention (consumes inputs_embeds).
    ...
    def embed_input_ids(self, input_ids: torch.Tensor, **_: Any) -> torch.Tensor:
        """Placeholder embedding for Sonar's ``AphroditeModel`` protocol.
    ...
    def _duplex_stage0_runtime(self):
        ...
        if runtime is not None:
            return runtime
        from aphrodite.omni.model_executor.models.personaplex.duplex.stage0 import (
            PersonaPlexStage0DuplexRuntime,
        )

        model_path = str(getattr(self.aphrodite_config.model_config, "model", ""))
        device = str(next(self.parameters()).device)
        runtime = PersonaPlexStage0DuplexRuntime(
            self,
            model_path=model_path,
            device=device,
            max_sessions=int(getattr(self.aphrodite_config.model_config, "duplex_max_sessions", 1)),
        )
