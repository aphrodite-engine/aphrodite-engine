...
from aphrodite.omni.data_entry_keys import EmbeddingsStruct, OmniPayloadStruct, to_dict, to_struct
from aphrodite.omni.model_executor.models.cosyvoice3.tokenizer import get_qwen_tokenizer
from aphrodite.omni.model_executor.models.cosyvoice3.utils import (
...
from aphrodite.omni.transformers_utils.configs.cosyvoice3 import CosyVoice3Config
...
from aphrodite.v1.sample.ops.topk_topp_sampler import random_sample
...
class CosyVoice3MultiModalProcessor(BaseMultiModalProcessor[CosyVoice3MultiModalProcessingInfo]):
    ...
    def _build_runtime_components(self, model_dir: str, config: CosyVoice3Config) -> dict:
        ...
        if self._speaker_embedding_trt_enabled() and torch.cuda.is_available():
            try:
                from aphrodite.omni.model_executor.models.cosyvoice3.speaker_embedding_trt import (
                    get_campplus_trt,
                )
...
class CosyVoice3Model(
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.model_stage = aphrodite_config.model_config.model_stage
        model_dir = aphrodite_config.model_config.model
        if not os.path.isdir(model_dir):
            model_dir = hf_api().snapshot_download(model_dir)
        ...
        if self.model_stage == "cosyvoice3_talker":
            ...
            from aphrodite.omni.model_executor.models.cosyvoice3.cosyvoice3_talker import (
                APHRODITEQwen2Encoder,
                CosyVoice3LM,
            )

            llm_aphrodite_config = self._create_llm_aphrodite_config(aphrodite_config)
            llm = APHRODITEQwen2Encoder(aphrodite_config=llm_aphrodite_config, prefix="model")
            ...
            # KV cache is now managed externally by Sonar's PagedAttention
            # No need for self.llm_cache
            self.model = self.talker
        elif self.model_stage == "cosyvoice3_code2wav":
            # Initialize code2wav stage (flow matching + vocoder)
            from aphrodite.omni.model_executor.models.cosyvoice3.cosyvoice3_code2wav import CosyVoice3Code2Wav
    ...
    def _create_llm_aphrodite_config(self, parent_config: AphroditeConfig) -> AphroditeConfig:
        """Create AphroditeConfig for the inner Qwen2 LLM.

        This creates a modified AphroditeConfig with the Qwen2 HF config loaded from
    ...
    def compute_logits(self, hidden_states: torch.Tensor | OmniOutput) -> torch.Tensor | None:
        ...
        if self.model_stage == "cosyvoice3_talker":
            ...
            # token (6562) via logsumexp so that Sonar's stop_token_ids=[6562]
            ...
            # Pad to full vocab_size for Sonar token handling.
    ...
    def embed_multimodal(self, **kwargs: object):
        if self.model_stage == "cosyvoice3_talker":
            ...
            # Sonar's _execute_mm_encoder batches ALL multimodal items scheduled
    ...
    def embed_input_ids(
        ...
        if self.model_stage == "cosyvoice3_talker":
            if is_multimodal is not None and any(is_multimodal):
                ...
                # Sonar v1's multimodal placeholder range covers the FULL
                ...
                # Sonar v1 packs cached (decode) requests first, then new
    ...
    def _maybe_enable_code2wav_trt(self) -> None:
        ...
        if onnx_path is None:
            ...
            return
        try:
            from aphrodite.omni.model_executor.models.cosyvoice3.flow_estimator_trt import (
        ...
        **kwargs: object,
        ...
        if self.model_stage == "cosyvoice3_talker":
            ...
            if "speech_token" in kwargs:
                ...
                #
                # Sonar hands these mm fields to forward() as collated, padded
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        if self.model_stage == "cosyvoice3_talker":
            # Load weights for text to speech LM stage using Sonar's weight loading
            llm_weight_path = os.path.join(self.model_dir, "llm.pt")
            device = next(self.parameters()).device
            checkpoint = torch.load(llm_weight_path, map_location=device)

            # 1. Load Qwen2 model weights into Sonar's Qwen2Model
            # The checkpoint has prefix "llm.model.model." for the transformer weights
            # Sonar's Qwen2Model expects just the model structure without extra prefixes
            qwen_weights = []
            for name, weight in checkpoint.items():
                if name.startswith("llm.model.model."):
                    # Strip prefix: llm.model.model.X -> X (for Sonar's Qwen2Model)
                    aphrodite_name = name.replace("llm.model.model.", "")
                    qwen_weights.append((aphrodite_name, weight))

            # Use Sonar's built-in load_weights which handles stacked params
