...
class APHRODITEQwen2Encoder(torch.nn.Module):
    """Qwen2 encoder using Sonar's Qwen2Model with external KV cache management.

    This replaces the HuggingFace Qwen2ForCausalLM with Sonar's optimized implementation
    that uses PagedAttention and external KV cache via ForwardContext.
    """

    def __init__(self, aphrodite_config: AphroditeConfig, prefix: str = ""):
        super().__init__()
        self.model = Qwen2Model(aphrodite_config=aphrodite_config, prefix=prefix)
    ...
    def forward(self, inputs_embeds: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
        """Forward pass using Sonar's attention with external KV cache.
        ...
        """
        # Sonar model expects flattened tensors [total_tokens, hidden_size]
