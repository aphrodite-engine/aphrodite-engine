...
waveform. The backbone is a Sonar-native ``Qwen3_5ForCausalLM``; the codebook
...
# would flip Sonar's uses_mrope() to True.
...
class GepardConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # head0 is the token Sonar samples; STOP is a synthetic sentinel one past
        # its valid range.
        ...
        # NeMo NanoCodec, which runs outside Sonar.
        cc = codec or {}
        ...
        # head0 goes through Sonar's SamplingParams; the other 31 heads use this
    ...
    def _normalize_backbone(cls, backbone_config: dict | None) -> dict:
        ...
        ``config.json`` does not carry. Left in, they flip Sonar's
    ...
    def get_text_config(self, **kwargs) -> PretrainedConfig:
        """Return the Qwen3.5 backbone config, which Sonar runs the backbone on."""
        ...
        try:
            ...
            logger.warning(
                ...
                "PretrainedConfig for the backbone. Verify Sonar still resolves "
                "the Qwen3.5 model.",
                model_type,
                type(e).__name__,
                e,
