...
Single-stage AR TTS on a Sonar-native Qwen3.5 backbone. Each step samples one
32-channel FSQ frame; head0 is the Sonar-facing token and the other 31 are
side-channel. Generation ends on a learned binary stop head, not an EOS token.
The NeMo NanoCodec decodes committed frames to a waveform outside Sonar.
...
from aphrodite.model_executor.models.qwen3_5 import Qwen3_5ForCausalLM
from aphrodite.model_executor.models.utils import maybe_prefix
from aphrodite.omni.model_executor.models.gepard.configuration_gepard import GepardConfig
from aphrodite.omni.model_executor.models.gepard.nanocodec import NanoCodec
...
FIRST_CHUNK_FRAMES = int(os.environ.get("APHRODITE_GEPARD_FIRST_CHUNK_FRAMES", "10"))
CHUNK_FRAMES = int(os.environ.get("APHRODITE_GEPARD_CHUNK_FRAMES", "86"))
...
class GepardTalkerForConditionalGeneration(nn.Module):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        self._greedy = os.environ.get("APHRODITE_GEPARD_GREEDY", "0") == "1"
        ...
        self.model = Qwen3_5ForCausalLM(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, "model"))
        ...
        self._max_model_len: int = int(aphrodite_config.model_config.max_model_len or 0)
    ...
    def _sample_frame(
        ...
        here rather than by Sonar's sampler, so a request's seed only reaches
    ...
    def preprocess(self, input_ids: torch.Tensor, input_embeds: torch.Tensor | None = None, **req_infos: Any):
        ...
        # Sonar discards its sampled token.
        ...
        if is_prefill:
            ...
            # SamplingParams.seed only reaches Sonar's sampler, so it never
        ...
        **kwargs: Any,
        ...
        if self._pending_requests:
            ...
            for i, (req_id, _span, samples_frame, is_last_token) in enumerate(self._pending_requests):
                if not samples_frame:
                    # Partial prefill chunk: Sonar discards this row's token.
    ...
    def compute_logits(self, hidden_states: torch.Tensor | OmniOutput, sampling_metadata: Any = None):
        ...
        # the pipeline's sampling_constraints, without which Sonar never acts on
        # the sentinel.
        ...
        # so Sonar's sampler argmaxes the single finite entry.
        for i, (_req_id, head0, do_stop) in enumerate(self._results_queue):
            if i >= bsz:
                break
            if head0 < 0:
                logits[i, 0] = 0.0  # partial-prefill placeholder; Sonar discards it
                continue
