...
NeMo NanoCodec decodes the accumulated frames outside Sonar.

``compute_logits`` scatters head0 into Sonar's vocab-wide logits; head0's valid
range is 0..7, so the synthetic STOP sentinel is 8. ``stop_token_ids=[8]``
couples it to Sonar's stop machinery, with ``detokenize=False`` because head0 is
