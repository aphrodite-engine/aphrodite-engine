...
from aphrodite.model_executor.models.qwen2_vl import (
...
from aphrodite.omni.model_executor.model_loader.weight_utils import resolve_model_to_local_path
from aphrodite.omni.model_executor.models.output_templates import OmniOutput
from aphrodite.omni.transformers_utils.configs.ming_flash_omni import BailingMM2Config
from aphrodite.omni.transformers_utils.processors.ming import (
...
class MingFlashOmniThinkerProcessingInfo(Qwen2VLProcessingInfo):
    ...
    def get_target_channels(self) -> int:
        # See `_normalize_audio_tensor` in aphrodite/omni/transformers_utils/processors/ming.py
        return 1
    ...
    def get_feature_extractor(self, **kwargs: object):
        ...
        sonar-omni's subclass ``MingWhisperFeatureExtractor``. We accept both
...
class _MingMultiModalDataParser(MultiModalDataParser):
    ...
    On sonar-omni's serving path serving_chat emits
    ...
    the ``image`` key — so that:
      ...
      * Sonar's renderer validator sees consistent keys in
...
class MingFlashOmniThinkerMultiModalProcessor(BaseMultiModalProcessor[MingFlashOmniThinkerProcessingInfo]):
    ...
    def _apply_hf_processor_main(
        ...
        *,
        ...
        """Apply Ming-specific prompt rewrites before delegating to aphrodite's
        ...
        ``hf_processor_mm_kwargs`` together. Downstream, aphrodite's MM caching
        ...
        if isinstance(prompt, str) and is_image_gen:
            from aphrodite.omni.model_executor.models.ming_flash_omni.prompt_utils import (
    ...
    def _call_hf_processor(
        ...
        :meth:`_apply_hf_processor_main` — by the time we get here, aphrodite's
        ...
        if audios is not None:
            # Sonar's AudioProcessorItems provides raw numpy arrays (already resampled).
...
class MingFlashOmniThinkerForConditionalGeneration(
    ...
    def get_placeholder_str(cls, modality: str, i: int) -> str | None:
        # aphrodite/omni/transformers_utils/processors/ming.py
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        with self._mark_language_model(aphrodite_config):
            llm_aphrodite_config = aphrodite_config.with_hf_config(llm_config)
            self.language_model = BailingMoeV2ForCausalLM(
                aphrodite_config=llm_aphrodite_config, prefix=maybe_prefix(prefix, "llm")
            )
        ...
        self._load_image_gen_query_tokens(aphrodite_config.model_config.model)
