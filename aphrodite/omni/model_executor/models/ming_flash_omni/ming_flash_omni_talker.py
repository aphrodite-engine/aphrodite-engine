...
from aphrodite.omni.model_executor.models.common.ming.audio_vae import AudioVAE, AudioVAEConfig
from aphrodite.omni.model_executor.models.ming_tts.constants import SPEAKER_EMBEDDING_DIM
...
from aphrodite.omni.transformers_utils.configs.ming_flash_omni import MingFlashOmniTalkerConfig
...
class MingFlashOmniTalkerForConditionalGeneration(nn.Module, CustomProcessMixin):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        root_config = aphrodite_config.model_config.hf_config
        ...
        self._use_cuda_graphs = not aphrodite_config.model_config.enforce_eager
        ...
        self.voice_presets = VoicePresetRegistry(
            ...
            download_dir=aphrodite_config.load_config.download_dir,
            audio_vae=self.audio_vae,
            aggregator=self.aggregator,
    ...
    def _resolve_generation_params(self, additional_info: dict[str, Any]) -> _GenerationParams:
        ...
        if ming_task == "omni":
            ...
            instruction = additional_info.get("instruction")
    ...
    def _resolve_voice(self, additional_info: dict[str, Any]) -> _VoiceContext:
        spk_emb = additional_info.get("spk_emb")
        prompt_text = additional_info.get("prompt_text")
        prompt_wav_lat = additional_info.get("prompt_wav_lat")
        prompt_wav_emb = additional_info.get("prompt_wav_emb")
        already_projected = False

        voice_name = additional_info.get("voice_name")
    ...
    def _generate_latents(
        ...
        *,
        ...
        if not text_segments:
            # Sonar passes 1D input_ids; Qwen2Model expects (batch, seq).
    ...
    def _iter_talker_safetensors(self) -> Iterable[tuple[str, torch.Tensor]]:
        ...
        model_root = resolve_model_to_local_path(
            ...
            cache_dir=self.aphrodite_config.load_config.download_dir,
        )
