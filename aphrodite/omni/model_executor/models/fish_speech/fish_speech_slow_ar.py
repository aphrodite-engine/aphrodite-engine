...
Uses Sonar's ``Qwen3Model`` as the transformer backbone.  Adds:
  ...
  - ``preprocess`` / ``postprocess`` hooks for Sonar-omni's AR scheduler.
...
class FishSpeechSlowARForConditionalGeneration(nn.Module):
    """Sonar-AR Slow AR model for Fish Speech S2 Pro.
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        config: FishSpeechConfig = aphrodite_config.model_config.hf_config  # type: ignore[assignment]
        self.config = config
        ...
        from aphrodite.omni.attention.fish_kvcache_attn import is_fish_kvcache_attn_enabled

        if is_fish_kvcache_attn_enabled():
            from aphrodite.omni.attention.fish_kvcache_backend import install_fish_kvcache_attn_backend

            install_fish_kvcache_attn_backend(self.model)

        # Fish Speech uses interleaved (GPT-J) RoPE, not NeoX style.
        # Sonar's Qwen3Attention defaults to NeoX (is_neox_style=True).
        ...
        # dataclasses.replace: CompilationConfig / AphroditeConfig are pydantic
        ...
        # validator in aphrodite/config/compilation.py rejects it and the clone
        ...
        self._fast_ar_aphrodite_config = copy.copy(aphrodite_config)
        self._fast_ar_aphrodite_config.compilation_config = predictor_compilation
        from aphrodite.config.aphrodite import set_current_aphrodite_config as _set_cfg

        with _set_cfg(self._fast_ar_aphrodite_config):
            self.fast_ar = FishSpeechFastAR(
                aphrodite_config=self._fast_ar_aphrodite_config,
    ...
    def _fix_rope_style(self) -> None:
        ...
        but Sonar's Qwen3Attention defaults to NeoX style.  We rebuild the
