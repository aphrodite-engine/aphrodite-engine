...
from aphrodite.model_executor.layers.fused_moe import FusedMoEFactory as SharedFusedMoE
...
from aphrodite.omni.model_executor.models.hunyuan_image3.autoencoder_kl_3d import AutoencoderKLConv3D
from aphrodite.omni.model_executor.models.hunyuan_image3.hunyuan_base import (
    HunYuanMLP,
    HunYuanModel,
...
from aphrodite.omni.model_executor.models.hunyuan_image3.siglip2 import LightProjector, Siglip2VisionTransformer
...
from aphrodite.utils.tensor_schema import TensorSchema
...
class HunyuanModel(HunYuanModel):
    ...
    def _get_expert_weights_remapping(self) -> dict[str, tuple[str, int, int]]:
        """Return the Sonar Omni-local remapping for Hunyuan checkpoint weights."""
        if not _is_moe(self.config):
            return {}
        return {
...
class HunyuanImage3Processor:
    ...
    def __init__(self, tokenizer, hf_config, **kwargs: object):
        ...
        # type-homogeneous.
        from aphrodite.omni.diffusion.models.hunyuan_image3.hunyuan_image3_transformer import (
    ...
    def process_image(self, image_input: ImageInput):
        ...
        # concatenated; Sonar `flat_from_sizes("image", vae_pixel_size)` slices
...
class HunyuanImage3MultiModalProcessor(BaseMultiModalProcessor[HunyuanImage3ProcessingInfo]):
    ...
    def _get_mm_fields_config(
        ...
        # per-image flat length so Sonar can split the buffer back per image.
    ...
    def _get_prompt_updates(
        ...
        def get_replacement_image(item_idx: int) -> PromptUpdateDetails:
            ...
            # embed run so Sonar's prefix-LM mask treats it as a single
...
class HunyuanImage3SparseMoeBlock(HunYuanSparseMoeBlock):
    ...
    Sonar's stock ``HunYuanSparseMoeBlock`` instead builds the gate as a
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # aphrodite 0.20+ FusedMoE merges shared-experts internally and runs the
...
class HunyuanImage3RotaryEmbedding(nn.Module):
    ...
    Sonar's standard ``MRotaryEmbedding`` instead assigns *contiguous* blocks
...
class HunyuanImage3ForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsLoRA, SupportsPP, SupportsMRoPE):
    ...
    This is the main entry point for HunyuanImage3.0 in Sonar. It wraps:
    - HunyuanModel
    ...
    # Without this flag, Sonar silently falls back to "weights" (TP). See
    # _vit_encode_dp.
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.model = HunyuanModel(aphrodite_config=aphrodite_config, prefix="model")
        ...
        # If a future text-only stage is added, gate on aphrodite_config.model_config.model_stage.
        ...
        tokenizer = get_tokenizer(aphrodite_config.model_config.tokenizer)
        ...
        engine_output_type = getattr(aphrodite_config.model_config, "engine_output_type", None)
    ...
    def _patch_moe_blocks(self):
        ...
        Stock Sonar builds the router gate as a default-dtype (bf16)
        ...
        if not _is_moe(self.config):
            return
        enable_eplb = getattr(self.aphrodite_config.parallel_config, "enable_eplb", False)
        ccfg = self.aphrodite_config.compilation_config
        replaced = 0
        for layer_id, layer in enumerate(self.model.layers):
            ...
            if isinstance(mlp, HunYuanSparseMoeBlock) and not isinstance(mlp, HunyuanImage3SparseMoeBlock):
                ...
                # aphrodite/model_executor/layers/fused_moe/layer.py:327).
    ...
    def _replace_rotary_embeddings(self):
        """Replace Sonar's standard MRotaryEmbedding with the custom
    ...
    def _parse_and_validate_image_input(
        ...
        **kwargs: dict[str, Any],
        ...
        # vae_pixel_size is only metadata for Sonar's flat_from_sizes split;
    ...
    def get_mrope_input_positions(
        ...
        *,
        ...
        Maps the original model's build_2d_rope logic into Sonar's 3-dim
