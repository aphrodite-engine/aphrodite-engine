...
from aphrodite.omni.model_executor.models.omnivoice.fused_qkv_rope import fused_qkv_norm_rope
...
try:
    from aphrodite.triton_utils import HAS_TRITON, tl, tldevice, triton

    if not HAS_TRITON:
        raise ImportError("Triton is not available on this platform")

    rsqrt = tldevice.rsqrt
...
class _OmniVoiceCUDAGraphForward:
    ...
    isolates OmniVoice CUDA memory from other aphrodite modules.  Sequential replay
    ...
    def __init__(self, generator: OmniVoiceGenerator, capture_sizes: list[int]) -> None:
        ...
        # aphrodite modules while still allowing safe re-use across sequential replays.
    ...
    def _capture_for_key(
        ...
        # memory from other aphrodite modules (unlike get_global_graph_pool which
        ...
        with torch.no_grad(), torch.cuda.graph(graph, pool=self._pool_handle):
