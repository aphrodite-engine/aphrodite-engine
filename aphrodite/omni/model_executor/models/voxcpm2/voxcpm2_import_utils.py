...
1. ``APHRODITE_OMNI_VOXCPM_CODE_PATH`` env var (explicit source tree)
2. Sibling ``../VoxCPM/src`` relative to the sonar-omni repo root
...
def _iter_voxcpm2_src_candidates() -> list[Path]:
    ...
    env_path = os.environ.get("APHRODITE_OMNI_VOXCPM_CODE_PATH")
    if env_path:
...
def _import_voxcpm2_attrs(module_name: str, *attr_names: str) -> tuple[Any, ...]:
    ...
    raise ImportError(
        ...
        f"Or set APHRODITE_OMNI_VOXCPM_CODE_PATH to the VoxCPM source tree. "
