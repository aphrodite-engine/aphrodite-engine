...
class Qwen2_5OmniTalkerForConditionalGeneration(
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        super().__init__()
        config: Qwen2_5OmniTalkerConfig = aphrodite_config.model_config.hf_config
        ...
        if hasattr(config, "talker_config"):
            ...
            aphrodite_config.model_config.hf_text_config = aphrodite_config.model_config.hf_config.talker_config
        else:
            self.config = config
    ...
    def bad_word_processor(self, logits: torch.Tensor) -> torch.Tensor:
        ...
        if self.suppress_start_id and self.suppress_start_id < logits.size(-1):
            ...
            if hasattr(self.config, "tts_codec_end_token_id"):
                end_id = int(self.config.tts_codec_end_token_id)
        ...
        if hasattr(self.config, "tts_codec_start_token_id"):
            bos_id = int(self.config.tts_codec_start_token_id)
            logits[..., bos_id] = -1e9
        return logits
