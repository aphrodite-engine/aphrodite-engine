...
from aphrodite.omni.model_executor.models.common.nucleus_ras_sampling import ras_sample_one as _ras_sample_one
...
def _glm_tts_int_value(value: Any) -> int | None:
    """Extract a scalar integer from Sonar multimodal/runtime wrappers."""
...
class GLMTTSMultiModalProcessor(BaseMultiModalProcessor[GLMTTSMultiModalProcessingInfo]):
    ...
    def _call_hf_processor(
        ...
        if wav is None:
            ...
            # Sonar profiling/cache setup may call the processor without an
...
class GLMTTSForConditionalGeneration(nn.Module, SupportsMultiModal):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.model_stage = getattr(aphrodite_config.model_config, "model_stage", "glm_tts")

        # Load configuration
        config: GLMTTSConfig = aphrodite_config.model_config.hf_config  # type: ignore[assignment]
        self.config = config
        ...
        if self.model_stage == "glm_tts_dit":
            ...
            self._dit_gen = GLMTTSDiTForGeneration(aphrodite_config=aphrodite_config, prefix=prefix)
            ...
            # find and load it without triggering the Sonar#39699 bug where
        ...
        self.model_dir = resolve_glm_tts_model_dir(
            self.model_dir,
            tokenizer_path=getattr(aphrodite_config.model_config, "tokenizer", None),
        )
        ...
        # Sonar#39699 iterator bug. Keep the real stage-0 source of truth in
        ...
        trust_remote_code = bool(getattr(aphrodite_config.model_config, "trust_remote_code", False))
        ...
        # Update config with dynamic token IDs so Sonar uses correct eos_token
    ...
    def _model_dtype(self) -> torch.dtype:
        ...
        try:
            ...
            dtype = getattr(self.aphrodite_config.model_config, "dtype", None)
    ...
    def sample(
        ...
        Uses Sonar Sampler for logits processing (logit_bias_state handles
    ...
    def _iter_llm_safetensors(self) -> Iterable[tuple[str, torch.Tensor]]:
        ...
        model_root = download_weights_from_hf_specific(
            self.model_path,
            self.aphrodite_config.load_config.download_dir,
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        loaded_params = loader.load_weights(_glm_tts_weights(), mapper=self.hf_to_aphrodite_mapper)
