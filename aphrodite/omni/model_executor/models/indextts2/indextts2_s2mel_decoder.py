...
from aphrodite.omni.diffusion.model_loader.hub_prefetch import _repo_prefetch_lock
...
def _strip_weight_norm(root: nn.Module) -> list[str]:
    ...
    checkpoint-initialized for Sonar's loader validation.
    """
...
def _load_bigvgan(vocoder_name: str, device: torch.device, dtype: torch.dtype = torch.float32):
    ...
    try:
        with set_default_torch_dtype(torch.float32):
            ...
            # fp32. The Sonar model loader may otherwise leave the process-wide
...
class IndexTTS2S2MelDecoder(nn.Module):
        ...
        **kwargs: Any,
        ...
        if mel_codes is None or (self.use_gpt_latent and latent is None):
            if not request_infos:
                # Sonar profiles generation stages once with synthetic token
        ...
        # Keep the public Sonar Omni audio contract as normalized float. This is
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        Bypasses aphrodite's default .pt iterator (same reason as talker: raw
        ...
        if stripped:
            ...
            # parameter; register it as loaded so Sonar's initialization
    ...
    def _get_vocoder_graph(self, bigvgan: nn.Module, device: torch.device, dtype: torch.dtype):
        ...
        if self._vocoder_graph is None:
            from aphrodite.omni.model_executor.models.indextts2.bigvgan_cuda_graph import (
    ...
    def _enable_dit_cuda_graph(self, estimator: nn.Module) -> None:
        ...
        if graph_runner is not None:
            ...
            return
        from aphrodite.omni.model_executor.models.indextts2.dit_cuda_graph import CUDAGraphDiTRunner
