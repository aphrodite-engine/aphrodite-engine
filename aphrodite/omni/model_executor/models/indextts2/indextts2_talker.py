...
"""IndexTTS2 Stage 0: GPT-2 AR Talker with Sonar-native PagedAttention.
...
from aphrodite.model_executor.models.gpt2 import GPT2Block
...
class IndexTTS2TalkerForConditionalGeneration(nn.Module):
    """Sonar-native GPT-2 AR talker for IndexTTS2.
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.config: IndexTTS2Config = aphrodite_config.model_config.hf_config  # type: ignore[assignment]
        ...
        # --- Flags for Sonar Omni framework ---
        ...
        # --- GPT-2 transformer (Sonar-native with PagedAttention) ---
    ...
    def make_omni_output(self, model_outputs: torch.Tensor | OmniOutput, **kwargs: Any) -> OmniOutput:
        ...
        if self.use_gpt_latent:
            ...
            # Sonar already materializes sampled token IDs on CPU for scheduler
    ...
    def _compute_emotion_vector(
        ...
        *,
        ...
        if emo_vector is not None:
            if isinstance(emo_vector, torch.Tensor) or isinstance(emo_vector, np.ndarray):
    ...
    def _predict_emotion_from_text(self, text: str, device: torch.device) -> list[float] | None:
        ...
        model, tokenizer = load_qwen_emotion(
            self.model_path,
            device,
            trust_remote_code=self.aphrodite_config.model_config.trust_remote_code,
        )
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        Bypasses aphrodite's default .pt iterator because the model directory
        contains raw-tensor files (feat1.pt, feat2.pt) that are not state
        dicts and would crash ``pt_weights_iterator``.

        Weight mapping from gpt.pth checkpoint → Sonar model params:
