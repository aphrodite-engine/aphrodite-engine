...
from aphrodite.omni.model_executor.models.common.ming.audio_vae import AudioVAEConfig
...
class MingDenseConfig(PretrainedConfig):
    ...
    # config compatibility; deploy/ming_tts.yaml selects the Sonar Omni
...
class BailingMoeConfig(PretrainedConfig):
    ...
    community Sonar ``BailingMoeModel`` is duck-typed over these attributes.
...
def _coerce_bailing_moe_config(value: Any) -> BailingMoeConfig:
    ...
    # so Sonar's get_rope uses plain RoPE — it has no "3D" scaling type.
