...
from aphrodite.model_executor.models.bailing_moe import BailingMoeModel
from aphrodite.model_executor.models.qwen2 import Qwen2Model
from aphrodite.model_executor.models.utils import AutoWeightsLoader, WeightsMapper, maybe_prefix
from aphrodite.omni.model_executor.models.common.ming.aggregator import Aggregator
from aphrodite.omni.model_executor.models.common.ming.cfm_cudagraph import (
    CFMGraphExecutor,
    CFMSampler,
)
from aphrodite.omni.model_executor.models.common.ming.cfm_head import FlowLoss
...
class MingLLMModel(nn.Module):
    # Drop the tensor on purpose, otherwise upstream Sonar would reject.
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        super().__init__()
        self.ming_config = MingTTSConfig.from_hf_config(aphrodite_config.model_config.hf_config)
        self.ming_config.validate()
        self.aphrodite_config = aphrodite_config
        self.prefix = prefix
        self.fm_dtype = _resolve_ming_runtime_dtype(aphrodite_config)
        if self.ming_config.model_variant == "moe":
            # BailingMoeModel reads ``aphrodite_config.model_config.hf_config`` directly
            # (no get_text_config()), so re-wrap with the nested bailing_moe config.
            llm_config = aphrodite_config.model_config.hf_config.get_text_config()
            llm_aphrodite_config = aphrodite_config.with_hf_config(llm_config)
            self.model = BailingMoeModel(aphrodite_config=llm_aphrodite_config, prefix=maybe_prefix(prefix, "model"))
        else:
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        loaded_params = AutoWeightsLoader(self).load_weights(weights, mapper=self.hf_to_aphrodite_mapper)
...
def _resolve_ming_runtime_dtype(aphrodite_config: AphroditeConfig) -> torch.dtype:
    dtype = getattr(aphrodite_config.model_config, "dtype", None)
