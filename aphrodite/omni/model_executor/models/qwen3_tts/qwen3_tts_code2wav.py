...
from aphrodite.omni.model_executor.stage_input_processors.chunk_size_utils import parse_chunk_ramp
...
class Qwen3TTSCode2Wav(nn.Module):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self._async_chunk = bool(getattr(aphrodite_config.model_config, "async_chunk", False))
        ...
        self._batch_stats_enabled = os.environ.get("APHRODITE_OMNI_QWEN3_CODE2WAV_BATCH_STATS", "").lower() in (
            "1",
            "true",
            "yes",
            "on",
        )
        self._batch_stats_log_every = int(
            os.environ.get("APHRODITE_OMNI_QWEN3_CODE2WAV_BATCH_STATS_LOG_EVERY", "0") or 0
        )
        ...
        # Construct decoder from config so it is visible to Sonar's
    ...
    def embed_input_ids(self, input_ids: torch.Tensor, **_: Any) -> torch.Tensor:
        # This stage ignores token embeddings. Keep a stable dummy embedding for Sonar runner.
        if input_ids.numel() == 0:
            return torch.empty((0, 1), device=input_ids.device, dtype=torch.float32)
        return torch.zeros((input_ids.shape[0], 1), device=input_ids.device, dtype=torch.float32)

    def get_dummy_runtime_additional_information(self, num_reqs: int) -> list[dict[str, Any]]:
        """Provide request state metadata for Sonar's external graph dummy run."""
        return [
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # via Sonar's weight loader (handles sharded safetensors, index
