...
class CUDAGraphDecoderWrapper:
    ...
    def __init__(
        ...
        self._device = None
        self._stats_enabled = os.environ.get("APHRODITE_OMNI_QWEN3_CODE2WAV_CUDAGRAPH_STATS", "").lower() in (
            "1",
            "true",
            "yes",
            "on",
        )
        self._stats_log_every = int(os.environ.get("APHRODITE_OMNI_QWEN3_CODE2WAV_CUDAGRAPH_STATS_LOG_EVERY", "0") or 0)
        self._stats_total = 0
        self._stats_hits = 0
    ...
    def _get_cuda_memory_stats(device: torch.device) -> tuple[int, int, int] | None:
        ...
        try:
            return (
                int(torch.accelerator.memory_allocated(device)),
                int(torch.accelerator.memory_reserved(device)),
    ...
    def _decode(self, codes: torch.Tensor, *, clone_graph_output: bool) -> torch.Tensor:
        ...
        # active (e.g. Sonar's cudagraph_mode=FULL warmup on Stage 1). Fall back
