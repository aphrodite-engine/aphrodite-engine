...
from aphrodite.model_executor.models.qwen3 import Qwen3Model
from aphrodite.model_executor.models.utils import PPMissingLayer, WeightsMapper, maybe_prefix
...
from aphrodite.omni.utils.speaker_cache import (
    get_speaker_cache,
...
from .qwen3_tts_code_predictor_aphrodite import Qwen3TTSTalkerCodePredictorForConditionalGenerationAPHRODITE
...
# Only the classes actually needed by the Sonar AR Talker are kept here.
# ---------------------------------------------------------------------------
...
class Qwen3TTSTalkerForConditionalGeneration(nn.Module):
    """Sonar-AR talker: step-wise layer-0 codec decoding.
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.model_path = aphrodite_config.model_config.model
        self.config: Qwen3TTSConfig = aphrodite_config.model_config.hf_config  # type: ignore[assignment]
        ...
        cudagraph_mode = getattr(aphrodite_config.compilation_config, "cudagraph_mode", None)
        ...
        # Code predictor uses an isolated Sonar config so its KV cache doesn't
        ...
        # AphroditeConfig are pydantic dataclasses, so `replace` re-runs
        ...
        # piecewise-backend validator in aphrodite/config/compilation.py rejects it
        ...
        predictor_compilation = copy.copy(aphrodite_config.compilation_config)
        predictor_compilation.static_forward_context = {}
        self._code_predictor_aphrodite_config = copy.copy(aphrodite_config)
        self._code_predictor_aphrodite_config.compilation_config = predictor_compilation
        from aphrodite.config.aphrodite import set_current_aphrodite_config as _set_cfg

        with _set_cfg(self._code_predictor_aphrodite_config):
            self.code_predictor = Qwen3TTSTalkerCodePredictorForConditionalGenerationAPHRODITE(
                aphrodite_config=self._code_predictor_aphrodite_config,
        ...
        self._silence_ban_frames = max(0, int(getattr(aphrodite_config.model_config, "silence_ban_frames", 0) or 0))
        # Sonar only fills sampling_metadata.output_token_ids when penalties, bad
        ...
        model_dtype = getattr(aphrodite_config.model_config, "dtype", torch.bfloat16)
        ...
        raw_subtalker_sampling = getattr(aphrodite_config.model_config, "subtalker_sampling_params", None)
    ...
    def preprocess(
        ...
        **info_dict: Any,
        ...
        if is_prefill:
            ...
            # When inputs_embeds is set, token ids are ignored by the model but must stay in-vocab for Sonar bookkeeping.
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        loaded = loader.load_weights(_talker_and_collect_speaker(weights), mapper=self.hf_to_aphrodite_mapper)
        ...
        if speaker_weights:
            ...
            loaded |= loader.load_weights(speaker_weights, mapper=self.hf_to_aphrodite_mapper)
        else:
        ...
        model_loader = DefaultModelLoader(self.aphrodite_config.load_config)
        source = DefaultModelLoader.Source(
            model_or_path=self.model_path,
            revision=self.aphrodite_config.model_config.revision,
            subfolder="speech_tokenizer",
        )
    ...
    def talker_mtp(
        ...
        **kwargs: Any,
        ...
        # Residual predictor runs fixed-length (Q-1) steps via the Sonar-native code_predictor.
        max_steps = q - 1
        if max_steps <= 0:
