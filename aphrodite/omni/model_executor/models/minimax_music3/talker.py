...
Two things make this model unusual inside Sonar:
...
* **The decode input is not a token embedding.** After prefill, each step's
  ...
  sampled ``c0`` token id still flows through Sonar's normal machinery so that
...
class MiniMaxMusic3TalkerForConditionalGeneration(nn.Module):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        # Nothing here may survive a step: Sonar compacts and reorders the
    ...
    def prepare_runner_inputs(
        ...
        **_: Any,
        ...
        mapping for the step about to execute. Sonar compacts and reorders the
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        through Sonar. The audio embedding table and the depth decoder live in
