...
``Qwen3ForCausalLM`` under ``language_model/`` and loads through Sonar. The
...
those are things Sonar's loader knows how to map, so they are read directly.
...
# ``language_model/`` is pulled by Sonar's own loader, and the repo's top-level
...
def load_depth_decoder_weights(
    ...
    *,
    ...
    Args:
        aphrodite_config: The stage's config, used to locate the checkpoint.
