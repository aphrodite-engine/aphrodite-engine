...
from aphrodite.model_executor.models.bagel import BagelForConditionalGeneration
...
from aphrodite.omni.diffusion.models.bagel.autoencoder import (
    AutoEncoderParams,
    DiagonalGaussian,
    Encoder,
)
from aphrodite.omni.diffusion.models.bagel.bagel_transformer import (
    PositionEmbedding,
    TimestepEmbedder,
)
from aphrodite.omni.diffusion.models.bagel.pipeline_bagel import default_ae_params
from aphrodite.omni.model_executor.weight_loader import AutoWeightsLoader
from aphrodite.transformers_utils.processors.bagel import BagelProcessor
...
class OmniBagelProcessor(BagelProcessor):
    ...
    def __call__(self, text=None, images=None, **kwargs):
        ...
        if is_img2img and images is not None:
            ...
            from aphrodite.transformers_utils.processors.bagel import BagelProcessorKwargs
...
class OmniBagelForConditionalGeneration(BagelForConditionalGeneration):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        tok_name = getattr(aphrodite_config.model_config, "tokenizer", None) or aphrodite_config.model_config.model
    ...
    def _install_mot_modules(self, config):
        ...
        qwen2_model.norm_moe_gen = AphroditeRMSNorm(hidden_size, eps=rms_eps)
        ...
        for layer in qwen2_model.layers:
            ...
            attn.q_norm_moe_gen = AphroditeRMSNorm(head_dim, eps=rms_eps)
            attn.k_norm_moe_gen = AphroditeRMSNorm(head_dim, eps=rms_eps)
            ...
            layer.input_layernorm_moe_gen = AphroditeRMSNorm(hidden_size, eps=rms_eps)
            layer.post_attention_layernorm_moe_gen = AphroditeRMSNorm(hidden_size, eps=rms_eps)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
            ...
            if "latent_pos_embed.pos_embed" in mapped_name and tensor.ndim == 2:
                ...
                if current_param.shape != tensor.shape:
                    ...
                    if side * side == int(npos) and hdim == current_param.shape[1]:
                        ...
                        self.config.max_latent_size = int(side)
        ...
        loaded = loader.load_weights(filtered_weights, mapper=self.hf_to_aphrodite_mapper)
