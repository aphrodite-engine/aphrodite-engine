...
from aphrodite.omni.model_executor.models.moss_tts.modeling_moss_tts_local import (
    MossTTSRealtimeLocalTransformer,
)
from aphrodite.omni.model_executor.models.moss_tts.modeling_moss_tts_local_depth import (
...
class MossTTSDelayTalkerForGeneration(nn.Module):
    ...
    * Heads: (n_vq + 1) parallel linear heads over the final hidden state.
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        self.config: MossTTSDelayConfig = aphrodite_config.model_config.hf_config
        ...
        # Qwen3 backbone — weights live under ``language_model.*``. Sonar's
        # Qwen3Model reads num_hidden_layers/hidden_size/etc. from
        # aphrodite_config.model_config.hf_config, but MOSS nests the LLM config under
        ...
        # num_hidden_layers).
        lm_aphrodite_config = aphrodite_config.with_hf_config(lm_cfg, architectures=["Qwen3ForCausalLM"])
        lm_aphrodite_config.model_config.hf_text_config = lm_aphrodite_config.model_config.hf_config
        self.model = Qwen3Model(
            aphrodite_config=lm_aphrodite_config,
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        """Map HF weight names to Sonar Omni module names.
...
class MossTTSRealtimeTalkerForGeneration(nn.Module):
    ...
    * Per-step audio generation runs the small ``local_transformer`` (4-layer
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        super().__init__()
        self.aphrodite_config = aphrodite_config
        self.config: MossTTSRealtimeConfig = aphrodite_config.model_config.hf_config
        ...
        # Qwen3 backbone (uses the inner language_config). Sonar exposes its
        # internal weights under ``model.*`` so we keep that prefix and remap
        # the upstream ``language_model.*`` keys at load time.
        backbone_aphrodite_config = copy.copy(aphrodite_config)
        # Swap in the inner Qwen3 config so Sonar picks the right num_layers,
        # heads etc.; KV cache sizing already uses get_text_config().
        backbone_model_config = copy.copy(aphrodite_config.model_config)
        backbone_model_config.hf_config = lang_cfg
        backbone_model_config.hf_text_config = lang_cfg
        backbone_aphrodite_config.model_config = backbone_model_config
        self.model = Qwen3Model(
            aphrodite_config=backbone_aphrodite_config,
            prefix=_maybe_prefix(prefix, "model"),
        )
        ...
        # We still need a logits_processor so Sonar downstream stays happy with
    ...
    def embed_input_ids(self, input_ids: torch.Tensor, **_: Any) -> torch.Tensor:
        # Used by aphrodite dummy profiling; real prefill goes through preprocess().
    ...
    def preprocess(
        ...
        **info_dict: Any,
        ...
        # Decode step: text_token from the just-sampled Sonar logit, plus the
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # Qwen3 backbone weights need Sonar's stacked-params loader to fuse
...
class MossTTSLocalTalkerForGeneration(nn.Module):
    ...
    * Per-step audio generation runs ``MossTTSLocalDepthTransformer``
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        super().__init__()
        self.aphrodite_config = aphrodite_config
        self.config: MossTTSLocalConfig = aphrodite_config.model_config.hf_config
        ...
        # Qwen3 backbone. get_text_config() is sufficient for Sonar to size
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        """Map checkpoint weight names to Sonar Omni module names.
        ...
        if not self.aphrodite_config.model_config.enforce_eager:
