...
class _MossStreamingDecodeCompileAdapter(nn.Module):
    """Expose the BF16 codec streaming hot path to Sonar compile.

    The dtype is part of the class identity intentionally: Sonar's AOT cache
    ...
    def __init__(self, codec: nn.Module, *, aphrodite_config: AphroditeConfig) -> None:
        super().__init__()
        self.codec = codec
...
class CUDAGraphStreamingDecoderWrapper:
    ...
    def __init__(
        ...
        *,
        ...
        # Sonar owns Inductor compilation; this wrapper remains the sole owner
        # of CUDA Graph capture/replay because it understands persistent codec
        # state slots. Disable Sonar's CUDA Graph layer to avoid nested graphs.
        compile_config = copy.copy(aphrodite_config)
        compile_config.compilation_config = copy.copy(aphrodite_config.compilation_config)
        ...
        with set_current_aphrodite_config(compile_config):
            self._compiled_decode: nn.Module | None = _MossStreamingDecodeCompileAdapter(
                codec,
                aphrodite_config=compile_config,
            )
    ...
    def _capture(self, batch_size: int, frame_size: int, device: torch.device) -> bool:
        if self._compiled_decode is not None:
            try:
                ...
                logger.warning(
                    "Sonar compile/capture failed for MOSS-TTS (B,T)=(%d,%d); falling back to a plain CUDA Graph",
                    batch_size,
                    frame_size,
                    exc_info=True,
                )
