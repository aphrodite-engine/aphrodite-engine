...
from aphrodite.omni.model_executor.models.moss_tts.audio_tokenizer import (
    MossAudioTokenizerConfig,
    MossAudioTokenizerModel,
)
from aphrodite.omni.model_executor.models.moss_tts.audio_tokenizer_v2 import (
    MossAudioTokenizerModel as MossAudioTokenizerV2Model,
)
from aphrodite.omni.model_executor.models.moss_tts.configuration_moss_audio_tokenizer_v2 import (
    MossAudioTokenizerConfig as MossAudioTokenizerV2Config,
)
from aphrodite.omni.model_executor.models.moss_tts.cuda_graph_streaming_decoder_wrapper import (
    CUDAGraphStreamingDecoderWrapper,
)
from aphrodite.omni.model_executor.models.moss_tts.moss_codec_cudagraph import (
...
class MossTTSCodecDecoder(nn.Module):
    ...
    ``aphrodite_config.model_config.hf_config.codec_model_name_or_path``.
    """
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        scheduler_cfg = getattr(self.aphrodite_config, "scheduler_config", None)
        ...
        self._async_chunk = bool(getattr(self.aphrodite_config.model_config, "async_chunk", False))
    ...
    # ------------------------------------------------------------------
    # Sonar Omni stubs (codec has no AR loop)
    # ------------------------------------------------------------------
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        # ``input_ids`` is concatenated across all requests. Sonar Omni runners
    ...
    def _streaming_graph_batch_sizes_from_compilation_config(self) -> list[int]:
        if getattr(self.aphrodite_config.model_config, "enforce_eager", True):
            return []
        compilation_config = getattr(self.aphrodite_config, "compilation_config", None)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # same target-device/default-dtype contexts used by Sonar's
        ...
        with set_default_torch_dtype(torch.float32), device:
        ...
        # Sonar's track_weights_loading() compares the returned set against
