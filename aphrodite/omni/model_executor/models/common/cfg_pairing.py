...
#   inference_scripts_aphrodite/audiogen_scripts/aphrodite_cfg_patch.py
# Divergences from the official file are commented at the divergence site.
"""Model-neutral classifier-free-guidance request pairing for the Sonar v1 scheduler.
...
# aphrodite/omni/model_executor/stage_input_processors/audex.py). Models that mint
...
def _wait_queues(scheduler: Any) -> list[Any]:
    # Divergence from the official (Sonar 0.20-era) file: Sonar 0.24 parks
...
def _patch_scheduler(scheduler_cls: type | None = None) -> None:
    ...
    ``scheduler_cls`` defaults to Sonar's v1 ``Scheduler``; accepting a class
...
def apply_cfg_patches() -> None:
    """Make the Sonar v1 scheduler CFG-pair-aware. Idempotent, per process.
    ...
    logger.info("Applying CFG pairing scheduler patches to Sonar v1")
    _patch_scheduler()
