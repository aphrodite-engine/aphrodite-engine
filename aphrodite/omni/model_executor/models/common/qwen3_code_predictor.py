...
from aphrodite.omni.diffusion.layers.custom_op import CustomOp
...
# HuggingFace reference on CPU/CUDA. Sonar's fused kernels (RMSNorm,
...
class CodePredictorWrapper(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        prefix_graph_cfg = self._stage_connector_extra_config(aphrodite_config)
    ...
    @staticmethod
    def _stage_connector_extra_config(aphrodite_config: AphroditeConfig) -> dict:
        model_cfg = getattr(aphrodite_config, "model_config", None)
    ...
    def _warmup_buckets(self) -> None:
        ...
        max_bsz = self._aphrodite_config.scheduler_config.max_num_seqs
    ...
    def _capture_cuda_graphs(self) -> None:
        """Capture a CUDA graph per bucket using Sonar's global graph pool."""
    ...
    def _prepare_npu_weights(self) -> None:
        from aphrodite_ascend.utils import maybe_trans_nz
