...
class DyninOmniToken2Text(DyninOmniStageBase):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self._infer_sources = resolve_dynin_infer_sources(aphrodite_config=aphrodite_config)
    ...
    def _get_or_create_uni_prompting(
        ...
        if self._uni_prompting is not None:
            if max_text_len is None and hasattr(self._uni_prompting, "max_text_len"):
                try:
                    existing_max_text_len = int(self._uni_prompting.max_text_len)
            ...
            if cond_dropout_prob is None and hasattr(self._uni_prompting, "cond_dropout_prob"):
                try:
                    cond_dropout_prob = float(self._uni_prompting.cond_dropout_prob)
                except Exception:
                    pass
        ...
        try:
            ...
            if max_text_len_override is not None and hasattr(uni_prompting, "max_text_len"):
                try:
                    ...
                    if override_int > 0:
                        restore_values["max_text_len"] = uni_prompting.max_text_len
                        uni_prompting.max_text_len = override_int + 1
                except Exception:
                    pass
            yield
        finally:
    ...
    def _ensure_prompt_vq_model(self, runtime_info: dict[str, Any], ref_device: torch.device) -> Any:
        sources = resolve_dynin_infer_sources(aphrodite_config=self.aphrodite_config, runtime_info=runtime_info)
