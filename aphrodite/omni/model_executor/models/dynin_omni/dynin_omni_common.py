...
DEFAULT_VQ_AUDIO_SOURCE = "snu-aidas/emova_speech_tokenizer_aphrodite"
...
_DYNIN_CONFIG_CANDIDATE_RELPATHS = (
    ...
    "aphrodite/omni/model_executor/models/dynin_omni/configs/dynin_omni.yaml",
    "dynin_omni.yaml",
)
...
def resolve_hidden_size(
    *,
    ...
    for config_obj in (
        getattr(aphrodite_config.model_config, "hf_config", None),
        getattr(aphrodite_config.model_config, "hf_text_config", None),
    ):
...
def _resolve_config_path(aphrodite_config: AphroditeConfig, runtime_info: dict[str, Any]) -> str | None:
    for value, name in (
        ...
        (
            getattr(aphrodite_config.model_config, "dynin_config_path", None),
            "aphrodite_config.model_config.dynin_config_path",
        ),
    ):
    ...
    model_source = str(getattr(aphrodite_config.model_config, "model", "") or "")
    tokenizer_source = str(getattr(aphrodite_config.model_config, "tokenizer", "") or "")
    hf_config = getattr(aphrodite_config.model_config, "hf_config", None)
    hf_name_or_path = (
...
def resolve_dynin_infer_sources(
    *,
    ...
    base_model_source = str(getattr(aphrodite_config.model_config, "model", ""))
    base_model_path = Path(base_model_source).expanduser()
    local_aphrodite_model_source = str(base_model_path) if base_model_path.is_dir() else None
    ...
    config_path = _resolve_config_path(aphrodite_config, runtime_info)
    if config_path:
    ...
    if runtime_local_global is None and runtime_local_model is None and local_aphrodite_model_source is not None:
        model_local_files_only = True

    if local_aphrodite_model_source is not None:
        if not runtime_model_source:
            if model_source != local_aphrodite_model_source:
                logger.info(
                    "DYNIN infer model source overridden to local Sonar model path: %s (from %s)",
                    local_aphrodite_model_source,
                    model_source,
                )
            model_source = local_aphrodite_model_source
        if not runtime_tokenizer_source:
            tokenizer_source = local_aphrodite_model_source
