...
#   inference_scripts_aphrodite/audioqa_scripts/audex_2b_aphrodite/modeling_audex_aphrodite.py
#   inference_scripts_aphrodite/audioqa_scripts/audex_2b_aphrodite/audio_encoder.py
...
Mirrors the Qwen2-Audio pattern with the 2B dense backbone:
  * ``language_model``  = the sonar-omni ``NemotronDenseForCausalLM`` port
    (wrapped via ``init_aphrodite_registered_model``; resolved through the omni
    model registry).
...
tensor per audio item; Sonar's ``SupportsMultiModal.embed_input_ids`` merges
...
from aphrodite.omni.model_executor.models.audex.audex_omni_processing import (
...
class NemotronDenseAudexForConditionalGeneration(
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        Sonar streams *all* ``.safetensors`` in the model dir; route
...
class NemotronHAudexForConditionalGeneration(
    ...
    ``inference_scripts_aphrodite/audioqa_scripts/audex_30b_a3b_aphrodite/modeling_audex_aphrodite.py``:
    the same encoder/projector/processing as the dense 2B wrapper around
    Sonar's native hybrid Mamba+MoE ``NemotronHForCausalLM``, whose state
    ...
    @classmethod
    def get_mamba_state_shape_from_config(cls, aphrodite_config):
        from aphrodite.model_executor.models.nemotron_h import NemotronHForCausalLM

        return NemotronHForCausalLM.get_mamba_state_shape_from_config(aphrodite_config)

    @classmethod
    def get_mamba_state_dtype_from_config(cls, aphrodite_config):
        from aphrodite.model_executor.models.nemotron_h import NemotronHForCausalLM
