...
# Adapted from the NemotronDense Sonar plugin bundled with
# nvidia/Nemotron-Labs-Audex-2B (nemotron_dense_aphrodite_plugin), which is not
...
The thinker is a plain token-in/token-out Sonar model. Speech-codec token
extraction happens in the stage input processors
(``aphrodite/omni/model_executor/stage_input_processors/audex.py``), which read the
...
class NemotronDenseForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.model = NemotronDenseModel(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, "model"))
        if get_pp_group().is_last_rank:
