...
# aphrodite.transformers_utils.tokenizer no longer exists in upstream Sonar;
...
class ConditionalChatTTSConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Talker stage is a single-vocab codec LM: Sonar's Sampler and
...
class MiniCPMWhisperEncoder(WhisperEncoder):
    ...
    def forward(
        ...
        Example:
            ...
            ...     input_features=input_features, output_hidden_states=True, output_attentions=True, use_cache=True
            ... )
...
class MiniCPMO45OmniLLMProcessingInfo(BaseProcessingInfo):
    ...
    def get_data_parser(self) -> MultiModalDataParser:
        # Sonar >=0.16 expects ``BaseProcessingInfo.get_data_parser`` to construct
...
class MiniCPMO45OmniLLMForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP, SupportsMRoPE):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        if multimodal_config.get_limit_per_prompt("image"):
            ...
            if hasattr(aphrodite_config, "model_config") and hasattr(
                aphrodite_config.model_config, "_attn_implementation"
            ):
                config.vision_config._attn_implementation = aphrodite_config.model_config._attn_implementation
            else:
            ...
            self.vpm.embed_dim = self.vpm.embeddings.embed_dim
            self.vpm.patch_size = self.vpm.embeddings.patch_size
        else:
            self.vpm = None
        ...
        text_aphrodite_config = (
            aphrodite_config.with_hf_config(text_config)
            if callable(getattr(aphrodite_config, "with_hf_config", None))
            else aphrodite_config
        )
        self.llm = init_aphrodite_registered_model(
            aphrodite_config=text_aphrodite_config,
    ...
    def _parse_and_validate_vision_input(
        ...
        **kwargs: object,
        ...
        # Sonar 0.18 profiling batches pixel_values as a dense tensor; the HF path
        ...
        # Match Sonar MiniCPMV: count slices per image with len(ps), and flatten
