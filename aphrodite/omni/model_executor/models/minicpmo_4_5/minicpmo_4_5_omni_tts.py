...
Pipeline:
  ...
  4. Project last hidden through head_code; Sonar Sampler picks the codec id
...
from aphrodite.omni.engine.duplex.intermediate import get_tts_handoff
from aphrodite.omni.model_executor.models.minicpmo_4_5 import MINICPMO45_DUPLEX_CODEC_TOKENS_PER_CHUNK
...
class MiniCPMO45OmniTTSForConditionalGeneration(nn.Module, SupportsPP):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        super().__init__()
        from aphrodite.omni.model_executor.models.minicpmo_4_5.minicpmo_4_5_omni_llm import MiniCPMOConfig
    ...
    def _init_native_talker(self, prefix: str) -> None:
        ...
        talker_config = self.aphrodite_config.with_hf_config(llama_config, architectures=["LlamaForCausalLM"])
        talker_config.model_config.hf_text_config = llama_config
        self.tts_model = LlamaModel(
            aphrodite_config=talker_config,
    ...
    def preprocess(
        ...
        **info_dict: Any,
        ...
        """Build request-local prefill/decode embeddings for the Sonar runner."""
        del input_embeds
        ...
        if is_prefill or first_call:
            ...
            return (
                ...
                {
                    "audio_state": state,
                    # Prefill has no previous codec id. Sonar samples the first
        ...
        # Decode: Sonar's previous sampled codec id is this step's input.
    ...
    def _dummy_hidden_states(
        ...
        """Shape-correct zero tensor for aphrodite KV cache profiling.

        aphrodite's gpu_model_runner._dummy_run takes forward()'s return value as
    ...
    def compute_logits(self, hidden_states, *args, **kwargs):
        ...
        # sample() re-applies the decision on the sampled ids: Sonar's
    ...
    def _apply_codec_repetition_penalty(self, logits, sampling_metadata):
        """Score MiniCPMTTS.generate's windowed codec penalty, not Sonar's.

        Upstream taxes a code by ``penalty ** frequency`` over the last
        ``past_window`` frames only (``gen_logits`` builds
        ``CustomRepetitionPenaltyLogitsProcessorRepeat(penalty, num_code, 16)``).
        Sonar's is presence-based over the whole stream, so on a codec stream
    ...
    def _force_eos_on_sampled_ids(self, output: Any, force_eos: list[bool] | None) -> Any:
        ...
        The codec EOS is a stage ``stop_token_ids`` entry, so Sonar's
