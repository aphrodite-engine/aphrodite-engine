...
"""Native-Sonar building blocks for the NemotronVoiceChat talker.

The eager/CUDA-graph talker executes the vendored HF Gemma3 backbone inside the
per-request ``preprocess`` hook, next to (not inside) the Sonar engine. The
native path (``hf_overrides.use_native_talker``) instead runs the backbone as a
real Sonar model — PagedAttention KV cache (capacity and per-step attention cost
scale with the ACTUAL session length, no StaticCache buckets), Sonar's own decode
CUDA graphs, and, later, multi-session batching. This mirrors NVIDIA's own
deployment, whose EAR-TTS is served by a Sonar engine.
...
* ``synthesize_backbone_config``: the checkpoint stores the backbone as a NeMo
  ``tts_config.backbone_config`` dict; Sonar needs a real ``Gemma3TextConfig``.
...
* ``iter_backbone_weights``: remaps the checkpoint's
  ``tts_model.tts_model.backbone.*`` HF-Gemma3 tensors onto the names Sonar's
...
def iter_backbone_weights(
    ...
    *,
    ...
    """Yield Sonar-Gemma3 weights from the unified checkpoint stream.

    Checkpoint names: ``tts_model.tts_model.backbone.<hf gemma3 names>``.
    Sonar ``Gemma3ForCausalLM`` expects ``model.<hf gemma3 names>`` plus an
...
class MoGStepGraph:
    ...
    On the native path the backbone runs under Sonar's own decode CUDA graphs,
...
class UncondStream:
    ...
    On the native path the conditional stream's KV lives inside Sonar's paged
