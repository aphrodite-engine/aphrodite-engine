...
``LLM_AR`` stage driven per frame by the Sonar engine, with the vendored NeMo
...
NeMo — Sonar's sampler never touches the audio codes.
...
* Sonar prompt = one placeholder token (timeline position 0; NeMo's loop starts
...
class NemotronVoiceChatTalkerForConditionalGeneration(nn.Module):
    """Sonar AR stage wrapping the vendored NeMo EAR-TTS per-frame step."""
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        # Native-Sonar path (hf_overrides.use_native_talker, experimental): the
        # Gemma3 backbone runs as a real Sonar model on PagedAttention (KV and
        # per-step attention cost scale with the ACTUAL session length; Sonar's
        ...
        # on Sonar paged KV; the unconditional stream mirrors it on the vendored
        ...
        if self._use_native_backbone:
            ...
            from aphrodite.omni.model_executor.models.nemotron_voicechat.talker_native import (
            ...
            self.backbone = init_aphrodite_registered_model(
    ...
    # ------------------------------------------------------------------
    # Sonar model protocol (placeholder token loop).
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Native-Sonar path (experimental; see __init__).
    # ------------------------------------------------------------------
    ...
    def preprocess(
        ...
        **info_dict: Any,
        ...
        if request_id not in self._sessions:
            # The request id, not Sonar's per-segment prefill flag, is the
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        """
        from aphrodite.omni.model_executor.models.nemotron_voicechat.nemo_vendored.duplex_ear_tts import (
            DuplexEARTTS,
        )
        ...
        for name, tensor in weights:
            ...
            if self._use_native_backbone and name.startswith(backbone_prefix):
                # The backbone runs as a native Sonar Gemma3 model instead.
        ...
        if self._use_native_backbone:
            ...
            logger.info("NemotronVoiceChat native talker backbone loaded %d Sonar tensors", len(loaded))
        if not state:
            raise ValueError(
        ...
        if self._use_cuda_graphs:
            from aphrodite.omni.model_executor.models.nemotron_voicechat.talker_graph import (
                TalkerStepGraph,
            )
