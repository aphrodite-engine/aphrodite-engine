...
* the NemotronH (hybrid Mamba) backbone on Sonar paged execution via
  ``init_aphrodite_registered_model`` — this is where Sonar earns its keep;
* the separate ``lm_head`` (text channel, sampled by the Sonar engine) and
...
* The Sonar prompt covers ``logical_prompt_token_len + 1`` positions: the system
...
from aphrodite.model_executor.models.interfaces import HasInnerState, IsHybrid
from aphrodite.model_executor.models.utils import init_aphrodite_registered_model, maybe_prefix
from aphrodite.omni.model_executor.duplex_sampling import DuplexSamplingRow
from aphrodite.omni.model_executor.models.nemotron_voicechat.runtime_info import (
    merge_runtime_info,
    require_request_id,
)
...
def validate_timeline_fits_max_model_len(
    ...
    The Sonar request occupies ``logical_prompt_token_len + 1`` prefill
    ...
    aphrodite_prefill_len = logical_prompt_token_len + 1
    aphrodite_total_len = aphrodite_prefill_len + acoustic_frame_count
    if aphrodite_total_len > max_model_len:
        raise ValueError(
            "NemotronVoiceChat input exceeds the thinker stage's max_model_len: "
            f"aphrodite_prefill_len={aphrodite_prefill_len} + "
            f"acoustic_frame_count={acoustic_frame_count} = "
            f"aphrodite_total_len={aphrodite_total_len} > max_model_len={max_model_len} "
...
def compute_acoustic_frame_count(nemo_stt_cfg: dict[str, Any], num_samples: int) -> int:
    ...
    """
    from aphrodite.omni.model_executor.models.nemotron_voicechat.nemo_vendored.asr.subsampling import (
        calc_length,
    )
...
class NemotronVoiceChatThinkerForConditionalGeneration(nn.Module, HasInnerState, IsHybrid):
    """Sonar-native thinker (perception + fusion + NemotronH + dual heads).

    Declares the hybrid-Mamba interfaces (``IsHybrid``/``HasInnerState``) so
    Sonar configures the mamba state cache for the wrapped NemotronH backbone;
    ...
    def get_mamba_state_dtype_from_config(cls, aphrodite_config: AphroditeConfig) -> tuple[torch.dtype, torch.dtype]:
        ...
        return NemotronHForCausalLM.get_mamba_state_dtype_from_config(aphrodite_config)

    @classmethod
    def get_mamba_state_shape_from_config(
        cls, aphrodite_config: AphroditeConfig
    ) -> tuple[tuple[int, int], tuple[int, int, int]]:
        from aphrodite.model_executor.layers.mamba.mamba_utils import MambaStateShapeCalculator

        parallel_config = aphrodite_config.parallel_config
        hf_config = aphrodite_config.model_config.hf_config.get_text_config()
        ...
        return MambaStateShapeCalculator.mamba2_state_shape(
            ...
            num_spec=aphrodite_config.num_speculative_tokens,
        )
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        self._dtype = getattr(aphrodite_config.model_config, "dtype", torch.bfloat16)

        # NemotronH backbone (+ lm_head) on Sonar native layers.
        ...
        from aphrodite.omni.model_executor.models.nemotron_voicechat.nemo_vendored.perception import (
        ...
        max_num_seqs = int(getattr(aphrodite_config.scheduler_config, "max_num_seqs", 1))
        if max_num_seqs > 1:
    ...
    def compute_logits(self, hidden_states: torch.Tensor | OmniOutput, sampling_metadata: Any = None) -> torch.Tensor:
        ...
        # Sonar >= 0.25 dropped the sampling_metadata parameter.
    ...
    def _duplex_stable_frame(
        ...
        from aphrodite.omni.model_executor.models.nemotron_voicechat.duplex.input import (
            decode_pcm_f32le,
        )
    ...
    def _init_session(self, request_id: str, info: dict[str, Any], device: torch.device) -> dict[str, Any]:
        ...
        validate_timeline_fits_max_model_len(
            ...
            max_model_len=int(self.aphrodite_config.model_config.max_model_len),
        )
    ...
    def preprocess(
        ...
        **info_dict: Any,
        ...
        # Position-addressed frame index: decode of Sonar position P+k consumes
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        * ``stt_model.llm.*`` -> Sonar NemotronH (renamed to ``backbone.*``; the
        ...
        * everything else (``rnnt_*``, ``tts_model.*``) is skipped lazily.
        ...
        # The Sonar NemotronH model expects an input embedding; the checkpoint's
