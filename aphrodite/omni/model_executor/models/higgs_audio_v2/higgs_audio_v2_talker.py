...
"""Stage-0 talker for higgs-audio v2 (Sonar-native, DualFFN-aware).
...
- Backbone: Llama-3.2-3B (hidden=3072, 28 layers, GQA 24Q/8KV, head_dim=128,
  ...
  We reuse Sonar's compiled :class:`aphrodite.model_executor.models.llama.LlamaModel`
...
processor) to reproduce the upstream behavior under Sonar's compiled forward
path.
"""
...
from aphrodite.model_executor.models.llama import LlamaDecoderLayer, LlamaMLP
from aphrodite.omni.model_executor.models.higgs_audio_v2.configuration_higgs_audio_v2 import (
    HiggsAudioV2Config,
)
...
class HiggsAudioV2DecoderLayer(LlamaDecoderLayer):
    """One transformer block: Sonar-native attention + DualFFN-routed MLPs.

    Wraps a stock :class:`aphrodite.model_executor.models.llama.LlamaDecoderLayer`
    ...
    position split is straightforward. Sonar's compiled forward path passes
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        # for Sonar's compilation_config.static_forward_context.
    ...
    def _routed_norm(
        ...
        Matches the Sonar fused-residual pattern: takes (hidden, residual) and
        ...
        text positions through ``text_norm`` (Sonar compiled RMSNorm).

        For consistency we explicitly compute the residual sum once, then call
        the Sonar text_norm on text positions WITHOUT its residual argument
        ...
        if (~mask_flat).any():
            ...
            # Sonar compiled RMSNorm.forward(x) returns Tensor; some variants
    ...
    def forward(
        ...
        *,
        ...
        # Fused-residual pattern (compatible with Sonar's LlamaModel forward
...
class HiggsAudioV2TalkerForConditionalGeneration(nn.Module):
    ...
    The class wires together:
      1. Sonar-native ``LlamaModel`` backbone (PagedAttention scheduling).
      ...
      4. ``load_weights`` HF -> Sonar mapping (GQA reshape, llama3 RoPE,
         text/audio MLP split, audio code heads, audio token embeddings).

    The forward path interleaves DualFFN routing with Sonar's compiled
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        # use Sonar's ``make_layers`` helper to register attention layers
        ...
        from aphrodite.model_executor.models.utils import make_layers

        _, _, self.layers = make_layers(
            self.config.num_hidden_layers,
            lambda prefix: HiggsAudioV2DecoderLayer(aphrodite_config=aphrodite_config, prefix=prefix),
        ...
        # Sonar's logits processor wires sampling metadata into the lm_head.
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        """Map HF state dict names to the Sonar-native layout.

        The upstream tensor names live under ``model.layers.<L>...`` plus a few
        top-level entries. The Sonar-native layout uses fused QKV/MLP tensors,
        so we transform the relevant pairs at load time:

          * Attention: HF ``q_proj/k_proj/v_proj`` -> Sonar ``qkv_proj`` (packed
          ...
          * RoPE parameters are read from ``self.config.rope_parameters`` and
            consumed by Sonar's LlamaAttention without any weight transform
          ...
          * Text MLP: HF ``mlp.gate_proj`` + ``mlp.up_proj`` ->
            Sonar ``mlp.gate_up_proj`` (concatenated on dim 0).
          * Audio MLP: HF ``audio_mlp.gate_proj`` + ``audio_mlp.up_proj`` ->
            Sonar ``dual_ffns.<L>.audio_mlp.gate_up_proj`` (same shape transform).
        ...
        Sonar's PagedAttention scheduler so the loaded weights drive a working
        ...
        def _strip_model_prefix(name: str) -> str:
            """Sonar's DefaultModelLoader strips the leading ``model.`` from HF
    ...
    def _map_simple_name(self, hf_name: str) -> str | None:
        """Translate the simple (non-fused) HF parameter names to Sonar names."""
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if input_ids is not None and inputs_embeds is None:
            ...
            # Audio-feedback substitution: Sonar's AR runner calls forward()
    ...
    def compute_logits(
        ...
        Sonar AR runner pipes through ``.contiguous()`` and a generic
    ...
    def _maybe_apply_audio_feedback(self, hidden_states: torch.Tensor, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        Called from :meth:`forward` when ``inputs_embeds`` is ``None`` (Sonar's
    ...
    # aphrodite/omni/worker/gpu_ar_model_runner.py:_sample). The runner calls
    # ``self.model.sample(logits, sampling_metadata)`` when this flag is True;
    # we delegate to the stock Sonar sampler for now and document the
    ...
    def sample(self, logits: torch.Tensor, sampling_metadata: Any) -> Any:
        ...
        # back to ``audio_eos_token_id`` at ramp-down completion. The Sonar
