...
2. **Sonar stage runtime API** (used by the engine):
   ``__init__(*, aphrodite_config, prefix)``, ``embed_input_ids``,
   ...
   :class:`aphrodite.omni.model_executor.models.output_templates.OmniOutput`.

The kernel is the HiggsAudio codec helper in
``aphrodite/omni/model_executor/models/higgs_audio_v2/higgs_audio_decoder.py``; both
...
from aphrodite.omni.model_executor.models.higgs_audio_v2.higgs_audio_decoder import (
    HiggsAudioRVQ,
...
class HiggsAudioV2Code2Wav(nn.Module):
    ...
    Constructor accepts either the Sonar-runtime signature (``*, aphrodite_config,
    ...
    used we read the higgs_audio_v2 config out of ``aphrodite_config.model_config.hf_config``
    and the model_path off ``aphrodite_config.model_config.model`` so the codec can
    ...
    def __init__(
        ...
        *,
        ...
        if aphrodite_config is not None:
            ...
            self._model_path: str | None = aphrodite_config.model_config.model
            self.aphrodite_config: AphroditeConfig | None = aphrodite_config
        else:
            if config is None:
                raise TypeError(
                    "HiggsAudioV2Code2Wav: provide either positional `config` "
                    "(HiggsAudioV2Config) or keyword `aphrodite_config` (AphroditeConfig)."
                )
            self.config = config
            self._model_path = None
            self.aphrodite_config = None
    ...
    def embed_input_ids(self, input_ids: torch.Tensor, **_: Any) -> torch.Tensor:
        """Stage 1 ignores embeddings; Sonar's runner still needs a stable shape."""
    ...
    def load_weights(self, weights_or_model_dir, device: torch.device | None = None):
        ...
        - ``load_weights(weights_iterator)`` -- the Sonar engine path. The iterator
        ...
        if not isinstance(weights_or_model_dir, (str, bytes, os.PathLike)):
            ...
            if self._model_path is None:
                raise RuntimeError(
                    ...
                    "no model directory is known. Construct with aphrodite_config so the "
                    "constructor caches aphrodite_config.model_config.model."
                )
    ...
    def _resolve_hf_id_to_local(repo_id: str) -> str:
        ...
        """
        from aphrodite.transformers_utils.repo_utils import try_to_load_from_cache
    ...
    def decode_codes(self, audio_codes: torch.Tensor) -> torch.Tensor:
        ...
        # Match fc2's parameter dtype — Sonar may auto-cast the codec to the
