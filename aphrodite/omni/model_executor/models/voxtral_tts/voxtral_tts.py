...
from aphrodite.omni.model_executor.models.voxtral_tts.cuda_graph_acoustic_transformer_wrapper import (
    CUDAGraphAcousticTransformerWrapper,
)
from aphrodite.omni.model_executor.models.voxtral_tts.voxtral_tts_audio_generation import (
...
class VoxtralTTSForConditionalGeneration(
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        self.config = config
        self.repo_id = aphrodite_config.model_config.model
        ...
        if self.model_stage == "audio_generation":
            ...
            self.audio_generation = init_aphrodite_registered_model(
            ...
            if aphrodite_config.quant_config is not None:
                aphrodite_config = replace(aphrodite_config, quant_config=None)
                logger.info("Voxtral TTS FP8 routing: stage-1 audio_tokenizer=bf16")
            self.audio_tokenizer = init_aphrodite_registered_model(
                aphrodite_config=aphrodite_config,
                hf_config=config,
    ...
    def _enable_acoustic_transformer_cudagraph(self):
        ...
        if self.model_stage != "audio_generation" or not hasattr(self, "_aphrodite_config"):
            return

        model_cfg = getattr(self._aphrodite_config, "model_config", None)
