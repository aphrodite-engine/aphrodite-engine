...
from aphrodite.model_executor.models.interfaces import SupportsMultiModal, SupportsPP
from aphrodite.model_executor.models.qwen2 import Qwen2DecoderLayer, Qwen2MLP
from aphrodite.model_executor.models.qwen2_5_vl import (
    Qwen2_5_VLDummyInputsBuilder,
    Qwen2_5_VLForConditionalGeneration,
    Qwen2_5_VLMultiModalProcessor,
    Qwen2_5_VLProcessingInfo,
)
from aphrodite.model_executor.models.qwen2_vl import Qwen2VLMultiModalDataParser
from aphrodite.model_executor.models.qwen3 import Qwen3DecoderLayer, Qwen3MLP
...
from aphrodite.omni.model_executor.models.utils import add_prefix_to_loaded_weights, is_interleaved
from aphrodite.omni.transformers_utils.configs.mammoth_moda2 import Mammothmoda2Config
...
class MammothModa2ARProcessingInfo(Qwen2_5_VLProcessingInfo):
    ...
    def get_data_parser(self) -> Qwen2VLMultiModalDataParser:
        # Sonar >=0.16 expects the parser to be provided by ProcessingInfo,
...
class MammothModa2Qwen2ForCausalLM(nn.Module, SupportsPP):
    def __init__(
        ...
        *,
        ...
        # Sonar sampler/processor expects "last dimension of logits == model_config.get_vocab_size()",
    ...
    def forward(
        ...
        if get_pp_group().is_first_rank:
            ...
            # In Sonar v1 path, only inputs_embeds might be provided, with input_ids set to None.
...
class MammothModa2ARForConditionalGeneration(Qwen2_5_VLForConditionalGeneration):
    ...
    hf_to_aphrodite_mapper = WeightsMapper(
        orig_to_new_prefix={
            ...
            # Extra generation vocab (image tokens) weights: mapped separately to the Sonar language_model submodule.
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        mammoth_cfg = aphrodite_config.model_config.hf_config
        ar_hf_config = getattr(mammoth_cfg, "llm_config", mammoth_cfg)
        ar_aphrodite_config = aphrodite_config.with_hf_config(
            ar_hf_config, architectures=aphrodite_config.model_config.architectures
        )
        # Initialize multi-modal components like the vision tower first.
        super().__init__(aphrodite_config=ar_aphrodite_config, prefix=prefix)
        # Replace with the custom MoE language model.
        lm_hf_config = getattr(
            ar_aphrodite_config.model_config.hf_config, "text_config", ar_aphrodite_config.model_config.hf_config
        )
        self.language_model = init_aphrodite_registered_model(
            aphrodite_config=ar_aphrodite_config,
        ...
        # These are passed by the sonar-omni runner via kwargs, so caching them in the model is sufficient.
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        # sonar-omni runner passes sampling_metadata and runtime_additional_information
...
class MammothModa2Qwen3ARForConditionalGeneration(Qwen3VLForConditionalGeneration):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = "") -> None:
        ...
        ar_aphrodite_config = aphrodite_config.with_hf_config(
            ar_hf_config,
            architectures=aphrodite_config.model_config.architectures,
        )
        super().__init__(aphrodite_config=ar_aphrodite_config, prefix=prefix)
        text_config = ar_hf_config.text_config
        self.language_model = MammothModa2Qwen3ForCausalLM(
            aphrodite_config=ar_aphrodite_config.with_hf_config(text_config),
            prefix=maybe_prefix(prefix, "language_model"),
        )
        self.make_empty_intermediate_tensors = self.language_model.make_empty_intermediate_tensors
        self._last_runtime_additional_information: list[dict[str, Any]] | None = None
...
class MammothModa2ForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    # Ensure aphrodite/omni/worker/gpu_model_runner.py's `extract_multimodal_outputs` follows
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str = ""):
        ...
        if self.model_stage == "ar":
            ...
            self.ar = init_aphrodite_registered_model(
                aphrodite_config=aphrodite_config,
            ...
            self.dit = init_aphrodite_registered_model(
                aphrodite_config=aphrodite_config,
                prefix=maybe_prefix(prefix, "dit"),
                # NOTE: init_aphrodite_registered_model -> AphroditeConfig.with_hf_config requires a
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings=None) -> torch.Tensor:
        ...
        # However, Sonar's generation runner may still request token embeddings
        # to populate `inputs_embeds` buffers, so we provide a dummy tensor.
        if self.model_stage == "dit":
            hidden_size = int(self.aphrodite_config.model_config.get_hidden_size())
            try:
                target_dtype = next(self.model.parameters()).dtype
            except StopIteration:
                target_dtype = self.aphrodite_config.model_config.dtype
            return torch.zeros(
