...
:mod:`aphrodite.omni.utils.qwen3_force_align_processor`.
"""
...
from aphrodite.omni.utils import qwen3_force_align_processor as _processor

if TYPE_CHECKING:
    from aphrodite.omni.config.stage_config import DeployConfig, PipelineConfig
...
def _load_forced_aligner_yaml(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(raw, dict):
        raise ValueError("Forced aligner configuration must be a mapping.")
    config = raw.get("forced_aligner", raw)
    if not isinstance(config, dict):
        raise ValueError("The forced_aligner section must be a mapping.")
    return dict(config)
...
def inject_forced_aligner_stage(
    ...
    from aphrodite.omni.model_executor.stage_input_processors.forced_aligner import (
    ...
    _FA = "aphrodite.omni.model_executor.stage_input_processors.forced_aligner"
    new_id = len(pipeline.stages)
    aligner_ps = StagePipelineConfig(
        stage_id=new_id,
        model_stage="forced_aligner",
        # Pooling stage = LLM_AR stage run with runner="pooling" (Sonar derives
