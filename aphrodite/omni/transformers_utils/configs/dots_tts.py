...
the same Qwen2.5-1.5B backbone).  Sonar's ``Qwen2Model.__init__`` then reads
...
class DotsTTSConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # -- Hoist LM parameters to top-level for Sonar's Qwen2Model --
        lm = self.lm_config
        ...
        # Sonar's Qwen2Model reads ``max_window_layers`` and checks against
