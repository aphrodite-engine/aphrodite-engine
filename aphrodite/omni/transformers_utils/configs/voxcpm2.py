...
class VoxCPM2Config(PretrainedConfig):
    ...
    so that aphrodite's ``MiniCPMModel`` can consume them directly.

    aphrodite's MiniCPM **always** applies muP scaling (scale_emb, scale_depth,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # But aphrodite's MiniCPMModel ALWAYS applies scale_depth / sqrt(N).
        # Native applies scale_emb externally; aphrodite applies it in embed_input_ids.
        ...
        if raw_rope is not None:
            self.rope_scaling: dict | None = dict(raw_rope)
            ...
            # aphrodite's MiniCPMAttention reads config.rope_parameters (a dict
