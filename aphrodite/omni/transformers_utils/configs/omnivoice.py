...
"""OmniVoice configuration for Sonar Omni two-stage pipeline."""
...
class OmniVoiceConfig(PretrainedConfig):
    """Configuration for OmniVoice model in Sonar Omni.
    ...
    def get_text_config(self, **kwargs):
        """Return self so Sonar uses our top-level config (which has
    ...
    def __init__(self, **kwargs):
        ...
        # Expose LLM params at top level for Sonar ModelConfig compatibility
        # (Sonar expects num_attention_heads, hidden_size, etc. on the config)
        ...
        # TF32 matmuls: not bit-identical; opt-in (matches Sonar default-off), set OMNIVOICE_TF32=1 to enable.
