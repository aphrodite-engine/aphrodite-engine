...
The checkpoint root carries only::
    ...
    {"architectures": ["MiniMaxMusic3ForConditionalGeneration"], "model_type": "minimax_music3"}
...
root has no transformer of its own: they are metadata for Sonar's config
plumbing, not a description of the acoustic stage. The acoustic stage holds no
Sonar attention layers, so no KV cache is derived from them; it reads its real
