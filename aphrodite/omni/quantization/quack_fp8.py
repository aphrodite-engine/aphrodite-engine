...
def quack_enabled() -> bool:
    override = os.environ.get("APHRODITE_OMNI_USE_QUACK_FP8")
...
def _set_persistent_cache_dir() -> None:
    if os.environ.get("QUACK_CACHE_DIR"):
        return
    root = (
        os.environ.get("APHRODITE_CACHE_ROOT")
        or os.environ.get("XDG_CACHE_HOME")
        or os.path.join(os.path.expanduser("~"), ".cache")
    )
    os.environ["QUACK_CACHE_DIR"] = os.path.join(root, "aphrodite.omni", "quack")
...
def _scales_valid(scale_a: torch.Tensor, scale_b: torch.Tensor) -> bool:
    ...
    Sonar initializes a per-tensor FP8 scale to ``finfo(float32).min`` and fills it at
...
def install_quack_fp8_patch() -> None:
    ...
    setattr(apply_scaled_mm, "_omni_quack_patched", True)
    setattr(FlashInferFP8ScaledMMLinearKernel, "apply_scaled_mm", apply_scaled_mm)
