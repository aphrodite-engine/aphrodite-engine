...
"""Shared weight-attribute helper for Sonar-omni quantization configs.

``copy_missing_attrs`` used to live in upstream
``aphrodite.model_executor.layers.quantization.fp8`` but was removed from Sonar
(commit b5bcb3ce88, "[Refactor] Remove dead code in multiple files").
Sonar-omni's diffusion quant methods still rely on it, so it is re-homed here.
"""
