...
quantized linear. The four-bit GEMM uses Sonar's existing NVFP4 kernel
...
def _nvfp4_kernel() -> NvFp4LinearKernel:
    ...
    if not isinstance(kernel, _COMPATIBLE_NVFP4_KERNELS):
        raise RuntimeError(
            "The selected Sonar NVFP4 backend is incompatible with the "
...
class DiffusionSVDQuantConfig(QuantizationConfig):
    ...
    def get_quant_method(
        ...
        if is_layer_skipped(
            ...
            # Sonar 0.28: skip_with_substr was replaced by match_mode
...
class DiffusionSVDQuantLinearMethod(LinearMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        """Adapt the canonical row-major checkpoint to Sonar's NVFP4 ABI."""
        ...
        logger.info_once(
            "SVDQuant NVFP4 is using Sonar's compatibility path; the rank correction is not fused into the GEMM."
        )
