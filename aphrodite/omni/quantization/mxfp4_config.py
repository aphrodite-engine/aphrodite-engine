...
from aphrodite.model_executor.parameter import ModelWeightParameter, RowAphroditeParameter
...
def _register_rocm_mxfp4_op() -> None:
    """Register the aphrodite_omni::rocm_mxfp4_gemm custom op for torch.compile.
    ...
    @torch.library.custom_op("aphrodite_omni::rocm_mxfp4_gemm", mutates_args=())
...
class ROCmMxfp4LinearMethod(MXFPLinearMethodBase):
    ...
    Forward path:
      ...
      _quant_matmul: torch.ops.aphrodite_omni.rocm_mxfp4_gemm wraps activation quant
    ...
    def __init__(self, quant_config: DiffusionMXFP4Config) -> None:
        self.quant_config = quant_config
        self.out_dtype = torch.get_default_dtype()
        if not hasattr(torch.ops.aphrodite_omni, "rocm_mxfp4_gemm"):
    ...
    def _quant_matmul(
        ...
        output = torch.ops.aphrodite_omni.rocm_mxfp4_gemm(
            x_q,
...
class NPUMxfp4DualScaleLinearMethod(MXFPLinearMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        layer.mul_scale.ignore_warning = True
