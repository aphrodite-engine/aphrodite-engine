...
build_quant_config() delegates to Sonar's quantization registry.
...
# ---------------------------------------------------------------------------
# Stub the ``humming`` package so that Sonar's lazy import inside
...
def _register_humming_stubs() -> None:
    ...
    for name, mod in registry.items():
        ...
        if parent:
            setattr(registry[parent], attr, mod)
...
from aphrodite.model_executor.layers.quantization import (  # noqa: E402
    QUANTIZATION_METHODS,
    get_quantization_config,
)
from aphrodite.model_executor.layers.quantization.base_config import (  # noqa: E402
    QuantizationConfig,
)
...
def _build_torchao(**kw: Any) -> QuantizationConfig:
    ...
    from aphrodite.model_executor.layers.quantization.torchao import TorchAOConfig
...
def _build_single(method: str, **kwargs: Any) -> QuantizationConfig:
    ...
    Resolution: _OVERRIDES first, then Sonar registry via from_config().
    """
