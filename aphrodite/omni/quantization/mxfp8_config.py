...
from aphrodite.model_executor.model_loader.reload.meta import (
    CopyCounter as CopyNumelCounter,
)
from aphrodite.model_executor.model_loader.weight_utils import initialize_single_dummy_weight
from aphrodite.model_executor.parameter import ModelWeightParameter
from aphrodite.model_executor.utils import replace_parameter
from aphrodite.omni.platforms import current_omni_platform
from aphrodite.omni.quantization._copy_missing_attrs import (
    ...
    def get_quant_method(
        ...
        if isinstance(layer, LinearBase):
            ...
            if current_omni_platform.is_xpu():
                ...
                return AphroditeMxfp8OnlineLinearMethod()
            raise NotImplementedError(
...
# ---------------------------------------------------------------------------
# Sonar kernel-based methods (XPU, CUDA, ROCm — any platform with Mxfp8LinearKernel)
# ---------------------------------------------------------------------------
# Note: AphroditeMxfp8OfflineLinearMethod removed - XPU only supports AutoRound MXFP8 offline
#       (see IncMxfp8OfflineLinearMethod in inc_config.py)

try:
    from aphrodite.model_executor.layers.quantization.online.mxfp8 import (
        Mxfp8OnlineLinearMethod as _AphroditeMxfp8OnlineBase,
    )
except ImportError as e:
    raise ImportError(
        "Sonar MXFP8 support is not available. "
        "Online MXFP8 quantization requires Sonar with MXFP8 kernel support. "
        "Please upgrade Sonar or use a compatible build."
    ) from e


class AphroditeMxfp8OnlineLinearMethod(_AphroditeMxfp8OnlineBase):
    """Online MXFP8 linear method extending Sonar's implementation with 3D tensor support.

    Loads BF16 weights, quantizes to MXFP8 at load time using Sonar's kernel.
    Adds reshape logic to handle 3D tensors from diffusion models.

    Inherits from Sonar's Mxfp8OnlineLinearMethod and only overrides __init__ and apply().
    __init__ directly initializes the kernel without calling super().__init__() to avoid
    Sonar config dependency in unit tests. All other methods (create_weights,
    process_weights_after_loading) are inherited directly from Sonar.
    """

    def __init__(self) -> None:
        """Initialize kernel directly without calling parent __init__ to avoid config dependency.

        TODO: skipping super().__init__() couples this to Sonar internals. If the parent
        adds required initialization, consider a version-aware assertion or calling
        super().__init__() with a fallback config.
        """
        from aphrodite.model_executor.kernels.linear import init_mxfp8_linear_kernel
    ...
    def apply(
        ...
        Sonar's kernel expects 2D tensors (M, K), but diffusion models pass
