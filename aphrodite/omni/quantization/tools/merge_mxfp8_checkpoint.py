...
"""Merge W8A8 MXFP8 quantized Wan2.2 weights into HF Diffusers format for sonar-omni.
...
Based on the SGLang convert_wan_to_diffusers approach, extended to:
  - Inject `quantization_config` into each transformer's config.json so that
    sonar-omni's TransformerConfig.from_dict() auto-detects offline MXFP8 mode
...
Usage:
  python aphrodite/omni/quantization/tools/merge_mxfp8_checkpoint.py \\
