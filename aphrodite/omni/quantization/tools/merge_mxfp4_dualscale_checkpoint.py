...
into a single to_qkv layer in sonar-omni. The transformer's load_weights() handles this via
...
# ---------------------------------------------------------------------------
# Key rename: msModelSlim naming → Diffusers / sonar-omni naming
...
def _diffusers_to_aphrodite_ignored(diffusers_ignored: list[str]) -> list[str]:
    """Translate diffusers checkpoint prefixes to sonar-omni model parameter names.
...
def _collect_ignored_layers(merged: dict[str, Any], mxfp4_layer_prefixes: set[str]) -> list[str]:
    """Collect sonar-omni parameter name prefixes for BF16 fallback layers.

    A layer is BF16 if it has a .weight tensor but its prefix is not in
    mxfp4_layer_prefixes. Prefixes are returned in sonar-omni parameter naming
    ...
    return _diffusers_to_aphrodite_ignored(sorted(all_weight_prefixes - mxfp4_layer_prefixes))
...
def _convert_transformer(
    ...
    # Determine ignored_layers: BF16 fallback layer prefixes in sonar-omni parameter naming.
