# SPDX-License-Identifier: Apache-2.0
# SPDX-FileCopyrightText: Copyright contributors to the vLLM project
# SPDX-FileCopyrightText: Copyright contributors to the vLLM-Omni project
"""Pipeline registry and factory for sonar-omni.

``OMNI_PIPELINES`` maps each ``model_type`` to either a ``PipelineConfig``
instance or a resolver callable that accepts an optional HF config and returns
a ``PipelineConfig``.

To add a new pipeline:
    1. Define the ``PipelineConfig`` instance as a module-level variable in
       ``aphrodite/omni/.../pipeline.py``.
    2. If the model needs to support several configurations, e.g., because some
       stages are optional, implement a resolver that consumes the HF config
       and returns a ``PipelineConfig``.
    3. Update the registry to map the key to the new config object (in the case
       of new keys) or to the resolver func.

Out of tree pipeline configs or resolvers can also be registered with register_pipeline.

NOTE: Generic single-stage diffusion is selected by ``config.resolver`` when
no registered Omni pipeline matches.
"""

from __future__ import annotations

from collections.abc import Callable
from typing import TypeAlias

from transformers import PretrainedConfig

from aphrodite.logger import init_logger
from aphrodite.omni.config.stage_config import (
    PipelineConfig,
)
from aphrodite.omni.diffusion.models.pi0_pipeline_config import PI0_PIPELINE
from aphrodite.omni.model_executor.models.audex.pipeline import (
    AUDEX_S2S_PIPELINE,
    AUDEX_THINKER_ONLY_PIPELINE,
    AUDEX_TTA_PIPELINE,
    AUDEX_TTS_PIPELINE,
)
from aphrodite.omni.model_executor.models.aura_omni.pipeline import AURA_OMNI_PIPELINE
from aphrodite.omni.model_executor.models.bagel.pipeline import (
    BAGEL_PIPELINE,
    BAGEL_SINGLE_STAGE_PIPELINE,
    BAGEL_THINK_PIPELINE,
)
from aphrodite.omni.model_executor.models.cosyvoice3.pipeline import COSYVOICE3_PIPELINE
from aphrodite.omni.model_executor.models.covo_audio.pipeline import COVO_AUDIO_PIPELINE
from aphrodite.omni.model_executor.models.dots_tts.pipeline import DOTS_TTS_PIPELINE
from aphrodite.omni.model_executor.models.dreamzero.pipeline import DREAMZERO_PIPELINE
from aphrodite.omni.model_executor.models.dynin_omni.pipeline import DYNIN_OMNI_PIPELINE
from aphrodite.omni.model_executor.models.fish_speech.pipeline import FISH_SPEECH_PIPELINE
from aphrodite.omni.model_executor.models.gepard.pipeline import GEPARD_PIPELINE
from aphrodite.omni.model_executor.models.glm_image.pipeline import GLM_IMAGE_PIPELINE
from aphrodite.omni.model_executor.models.glm_tts.pipeline import GLM_TTS_PIPELINE
from aphrodite.omni.model_executor.models.gr00t.pipeline import GR00T_N1D7_PIPELINE
from aphrodite.omni.model_executor.models.higgs_audio_v2.pipeline import HIGGS_AUDIO_V2_PIPELINE
from aphrodite.omni.model_executor.models.higgs_audio_v3.pipeline import HIGGS_AUDIO_V3_PIPELINE
from aphrodite.omni.model_executor.models.hunyuan_image3.pipeline import (
    HUNYUAN_IMAGE3_AR_PIPELINE,
    HUNYUAN_IMAGE3_DIT_PIPELINE,
    HUNYUAN_IMAGE3_PIPELINE,
)
from aphrodite.omni.model_executor.models.hunyuan_video.pipeline import HUNYUAN_VIDEO_15_PIPELINE
from aphrodite.omni.model_executor.models.indextts2.pipeline import (
    INDEXTTS2_PIPELINE,
    INDEXTTS25_PIPELINE,
)
from aphrodite.omni.model_executor.models.joyai_vl_interaction.pipeline import (
    JOYAI_VL_INTERACTION_PIPELINE,
)
from aphrodite.omni.model_executor.models.lance.pipeline import LANCE_PIPELINE
from aphrodite.omni.model_executor.models.lingbot_world.pipeline import LINGBOT_WORLD_PIPELINE
from aphrodite.omni.model_executor.models.mammoth_moda2.pipeline import (
    MAMMOTH_MODA2_AR_PIPELINE,
    MAMMOTH_MODA2_PIPELINE,
)
from aphrodite.omni.model_executor.models.mimo_audio.pipeline import MIMO_AUDIO_PIPELINE
from aphrodite.omni.model_executor.models.ming_flash_omni.pipeline import (
    MING_FLASH_OMNI_IMAGE_PIPELINE,
    MING_FLASH_OMNI_PIPELINE,
    MING_FLASH_OMNI_THINKER_ONLY_PIPELINE,
    MING_FLASH_OMNI_TTS_PIPELINE,
)
from aphrodite.omni.model_executor.models.ming_tts.pipeline import (
    MING_TTS_MOE_PIPELINE,
    MING_TTS_PIPELINE,
)
from aphrodite.omni.model_executor.models.minicpmo_4_5.pipeline import MINICPMO_4_5_PIPELINE
from aphrodite.omni.model_executor.models.minimax_h3.pipeline import MINIMAX_H3_PIPELINE
from aphrodite.omni.model_executor.models.minimax_music3.pipeline import MINIMAX_MUSIC3_PIPELINE
from aphrodite.omni.model_executor.models.moss_tts.pipeline import (
    MOSS_TTS_LOCAL_PIPELINE,
    MOSS_TTS_PIPELINE,
    MOSS_TTS_REALTIME_PIPELINE,
)
from aphrodite.omni.model_executor.models.moss_tts_nano.pipeline import MOSS_TTS_NANO_PIPELINE
from aphrodite.omni.model_executor.models.nemotron_voicechat.pipeline import (
    NEMOTRON_VOICECHAT_PIPELINE,
)
from aphrodite.omni.model_executor.models.omnivoice.pipeline import OMNIVOICE_PIPELINE
from aphrodite.omni.model_executor.models.personaplex.pipeline import PERSONAPLEX_PIPELINE
from aphrodite.omni.model_executor.models.qwen2_5_omni.pipeline import (
    QWEN2_5_OMNI_PIPELINE,
    QWEN2_5_OMNI_THINKER_ONLY_PIPELINE,
)
from aphrodite.omni.model_executor.models.qwen3_omni.pipeline import (
    QWEN3_OMNI_THINKER_ONLY_PIPELINE,
    resolve_qwen3_omni_pipeline,
)
from aphrodite.omni.model_executor.models.qwen3_tts.pipeline import QWEN3_TTS_PIPELINE
from aphrodite.omni.model_executor.models.step_audio2.pipeline import (
    STEP_AUDIO2_ASR_PIPELINE,
    STEP_AUDIO2_PIPELINE,
)
from aphrodite.omni.model_executor.models.voxcpm2.pipeline import VOXCPM2_PIPELINE
from aphrodite.omni.model_executor.models.voxtral_tts.pipeline import VOXTRAL_TTS_PIPELINE
from aphrodite.omni.model_executor.models.wan2_2.pipeline import WAN2_2_TI2V_PIPELINE

logger = init_logger(__name__)

PipelineResolverFunc: TypeAlias = Callable[[PretrainedConfig | None], PipelineConfig | None]

# --- Multi-stage omni pipelines (LLM-centric; audio / video I/O) ---
OMNI_PIPELINES: dict[str, PipelineConfig | PipelineResolverFunc] = {
    "aura_omni": AURA_OMNI_PIPELINE,
    "joyai_vl_interaction": JOYAI_VL_INTERACTION_PIPELINE,
    "qwen2_5_omni": QWEN2_5_OMNI_PIPELINE,
    "qwen2_5_omni_thinker_only": QWEN2_5_OMNI_THINKER_ONLY_PIPELINE,
    "personaplex": PERSONAPLEX_PIPELINE,
    "nemotron_voicechat": NEMOTRON_VOICECHAT_PIPELINE,
    # Alias: lets bare `sonar-omni serve <NVIDIA-NemotronLabs-VoiceChat-11B dir>`
    # auto-detect through the path-basename fallback (the checkpoint config.json
    # has no model_type key).
    "nemotron_labs_voicechat": NEMOTRON_VOICECHAT_PIPELINE,
    "qwen3_omni_moe": resolve_qwen3_omni_pipeline,
    "qwen3_omni_moe_thinker_only": QWEN3_OMNI_THINKER_ONLY_PIPELINE,
    "qwen3_tts": QWEN3_TTS_PIPELINE,
    "step_audio_2": STEP_AUDIO2_PIPELINE,
    "step_audio_2_asr": STEP_AUDIO2_ASR_PIPELINE,
    "covo_audio": COVO_AUDIO_PIPELINE,
    "bagel": BAGEL_PIPELINE,
    "bagel_think": BAGEL_THINK_PIPELINE,
    "bagel_single_stage": BAGEL_SINGLE_STAGE_PIPELINE,
    "lance": LANCE_PIPELINE,
    "dreamzero": DREAMZERO_PIPELINE,
    "lingbot_world": LINGBOT_WORLD_PIPELINE,
    "Gr00tN1d7": GR00T_N1D7_PIPELINE,
    "pi0": PI0_PIPELINE,
    "gepard": GEPARD_PIPELINE,
    "glm_image": GLM_IMAGE_PIPELINE,
    "hunyuan_image_3_moe": HUNYUAN_IMAGE3_PIPELINE,
    "hunyuan_image3_ar": HUNYUAN_IMAGE3_AR_PIPELINE,
    "hunyuan_image3_dit": HUNYUAN_IMAGE3_DIT_PIPELINE,
    "hunyuan_video_15": HUNYUAN_VIDEO_15_PIPELINE,
    "wan2_2_ti2v": WAN2_2_TI2V_PIPELINE,
    "voxcpm2": VOXCPM2_PIPELINE,
    "dots_tts": DOTS_TTS_PIPELINE,
    "cosyvoice3": COSYVOICE3_PIPELINE,
    "audex_tts": AUDEX_TTS_PIPELINE,
    "audex_tta": AUDEX_TTA_PIPELINE,
    "audex_thinker_only": AUDEX_THINKER_ONLY_PIPELINE,
    "audex_s2s": AUDEX_S2S_PIPELINE,
    # Alias: the Nemotron-Labs-Audex-2B repo-root config.json reports
    # ``model_type: nemotron_labs_audex``; bare ``sonar-omni serve <repo>``
    # auto-detects through it and must land on the default (TTS) pipeline.
    "nemotron_labs_audex": AUDEX_TTS_PIPELINE,
    "mimo_audio": MIMO_AUDIO_PIPELINE,
    "ming_tts": MING_TTS_PIPELINE,
    "ming_tts_moe": MING_TTS_MOE_PIPELINE,
    "voxtral_tts": VOXTRAL_TTS_PIPELINE,
    "glm_tts": GLM_TTS_PIPELINE,
    "fish_qwen3_omni": FISH_SPEECH_PIPELINE,
    "ming_flash_omni": MING_FLASH_OMNI_PIPELINE,
    "ming_flash_omni_tts": MING_FLASH_OMNI_TTS_PIPELINE,
    "ming_flash_omni_thinker_only": MING_FLASH_OMNI_THINKER_ONLY_PIPELINE,
    "ming_flash_omni_image": MING_FLASH_OMNI_IMAGE_PIPELINE,
    "moss_tts_nano": MOSS_TTS_NANO_PIPELINE,
    "minimax_h3_disaggregated": MINIMAX_H3_PIPELINE,
    "omnivoice": OMNIVOICE_PIPELINE,
    "mammoth_moda2": MAMMOTH_MODA2_PIPELINE,
    "mammoth_moda2_ar": MAMMOTH_MODA2_AR_PIPELINE,
    "moss_tts_delay": MOSS_TTS_PIPELINE,
    "moss_tts_realtime": MOSS_TTS_REALTIME_PIPELINE,
    "moss_tts_local": MOSS_TTS_LOCAL_PIPELINE,
    "minicpmo_4_5": MINICPMO_4_5_PIPELINE,
    "minimax_music3": MINIMAX_MUSIC3_PIPELINE,
    "higgs_audio_v2": HIGGS_AUDIO_V2_PIPELINE,
    "higgs_multimodal_qwen3": HIGGS_AUDIO_V3_PIPELINE,
    "dynin_omni": DYNIN_OMNI_PIPELINE,
    "indextts2": INDEXTTS2_PIPELINE,
    "indextts2_5": INDEXTTS25_PIPELINE,
}


def register_pipeline(pipeline: PipelineConfig | PipelineResolverFunc, model_type: str | None = None):
    """Register an out of tree pipeline or PipelineResolverFunc to a model_type key.
    If a PipelineConfig is provided, model_type is optional, and pipeline.model_type
    will be used by default. If a callable is provided, model_type must be provided,
    since resolvers can return multiple different PipelineConfigs depending on the
    consumed config.
    """
    if isinstance(pipeline, PipelineConfig):
        model_type = model_type if model_type is not None else pipeline.model_type
    elif model_type is None:
        raise ValueError("Model type must be explicitly provided when registering a pipeline resolver")

    if model_type in OMNI_PIPELINES:
        logger.warning(f"Model type {model_type} is already registered; the old mapping will be clobbered")
    OMNI_PIPELINES[model_type] = pipeline


def resolve_pipeline_config(
    model_type: str,
    hf_config: PretrainedConfig | None = None,
) -> PipelineConfig | None:
    """Resolve a registry key to a concrete pipeline config."""
    if model_type not in OMNI_PIPELINES:
        logger.warning("Model type %s is not registered to OMNI_PIPELINES", model_type)
        return None
    pipeline = OMNI_PIPELINES[model_type]
    return pipeline(hf_config) if callable(pipeline) else pipeline
