...
from aphrodite.omni.config.composable_parallel.strategy_loader import load_strategy_specs
from aphrodite.omni.config.config_factory import StageConfigFactory, with_trust_remote_code_override
...
from aphrodite.omni.config.stage_config import PipelineConfig
from aphrodite.omni.config.yaml_util import create_config
...
class OmniConfigResolution:
    ...
    runtime ABI only until stage startup consumes ``AphroditeOmniConfig`` directly.
...
def _convert_dataclasses_to_dict(obj: Any) -> Any:
    ...
    # aphrodite.utils.Counter without importing either implementation.
...
def _build_registered_resolution(
    structured_config: AphroditeOmniConfig,
    *,
    cli_overrides: dict[str, Any],
