...
"""Translate a ``StrategySpec`` stack into Sonar Omni parallel sizing.
...
The distinction this module enforces — engine data parallelism (a true Sonar
...
from aphrodite.omni.config.composable_parallel.routing import (
    Broadcast,
    PartitionByHash,
    PipelineMicrobatch,
    RouteByStage,
...
from aphrodite.omni.config.composable_parallel.spec import MeshAxisKind, StrategySpec
...
# world dimensions Sonar realizes intra-engine; dense EP is a flag over the
# existing TP*DP ranks; stage_replica is an omni-coordinator-level fan-out of
# independent engine replicas (NOT a Sonar world dimension). The remaining kinds
...
_AXIS_TO_ENGINE_FIELD: dict[MeshAxisKind, str] = {
    MeshAxisKind.TP: "tensor_parallel_size",
    MeshAxisKind.DP: "data_parallel_size",
    MeshAxisKind.PP: "pipeline_parallel_size",
}
...
# data parallelism, realized by Sonar's own intra-engine DP load balancer.
_DEFAULT_L1_OWNER: dict[MeshAxisKind, L1Owner] = {
    MeshAxisKind.DP: "engine",
    MeshAxisKind.TP: "engine",
    MeshAxisKind.PP: "engine",
    MeshAxisKind.EP: "engine",
    MeshAxisKind.STAGE_REPLICA: "delegated",
}
...
class OmniParallelConfig:
    ...
    # This is NOT a Sonar world dimension; it is the per-stage deploy
    ...
    def world_size(self) -> int:
        ...
        # This product is what Sonar calls ``world_size_across_dp`` (tp * pp * dp).
...
def _validate_dp(spec: StrategySpec, owner: L1Owner) -> None:
    ...
    DP is realized intra-engine by Sonar's own DP load balancer, so it is
    ...
    *replica* balancer, a different layer). Key-stable (hash) request affinity is
    not something Sonar's DP LB guarantees, so it is rejected rather than silently
    dropped.
    """
    if owner != "engine":
        _fail(
            f"dp axis {spec.name!r} is engine data parallelism realized intra-engine by "
            f"Sonar's DP load balancer; l1_owner must be 'engine', got {owner!r}. For "
            "omni-coordinator-level request fan-out across independent replicas, use a "
            "'stage_replica' axis."
        )
    if _is_affinity_dp_routing(spec.routing):
        _not_implemented(
            f"dp axis {spec.name!r} requests key-stable (hash) routing, which Sonar's "
...
def _stage_replica_lb_policy(spec: StrategySpec, owner: L1Owner) -> str:
    ...
    A ``stage_replica`` axis is *not* a Sonar world dimension: it stands up N
