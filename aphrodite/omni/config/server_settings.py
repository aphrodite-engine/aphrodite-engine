...
class FileBackend(BaseModel):
    ...
    file_concurrency: int = Field(
        default=4, gt=0, validate_default=True, description="Maximum number of file operations permitted at a time"
    )
    file_ttl: int | None = Field(
        default=None, gt=0, description="Optional TTL (in seconds) configuration settings for locally stored files."
    )
    ttl_sweep_interval: int | None = Field(
        default=None, gt=0, description="Optional frequency (in seconds) to enforce file TTLs."
    )

    @field_validator("path", mode="before")
    @classmethod
    def _migrate_path(cls, value):
        return _resolve_deprecated_env("APHRODITE_OMNI_STORAGE_PATH", "APHRODITE_OMNI_SERVER_STORAGE__PATH", value)

    @field_validator("file_concurrency", mode="before")
    @classmethod
    def _migrate_file_concurrency(cls, value):
        return _resolve_deprecated_env(
            "APHRODITE_OMNI_STORAGE_MAX_CONCURRENCY", "APHRODITE_OMNI_SERVER_STORAGE__FILE_CONCURRENCY", value
        )
...
class ServerSettings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="APHRODITE_OMNI_SERVER_", env_nested_delimiter="__")
    storage: FileBackend = Field(default_factory=FileBackend)
    max_reference_upload_bytes: int = Field(default=64 * 1024 * 1024, gt=0)
    max_video_jobs: int = Field(default=32, gt=0)
