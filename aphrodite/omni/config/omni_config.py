...
"""Structured Sonar Omni configuration classes.

This module is additive for Phase 2 of RFC #4021.
``AphroditeOmniConfig.from_pipeline_config`` builds the structured view from an
...
from aphrodite.config import CacheConfig as AphroditeCacheConfig
from aphrodite.config import CompilationConfig as AphroditeCompilationConfig
from aphrodite.config import KVTransferConfig
from aphrodite.config import LoadConfig as AphroditeLoadConfig
from aphrodite.config import ParallelConfig as AphroditeParallelConfig
from aphrodite.config import ProfilerConfig as AphroditeProfilerConfig
from aphrodite.config import SchedulerConfig as AphroditeSchedulerConfig
from aphrodite.config.utils import config
from aphrodite.engine.arg_utils import EngineArgs as AphroditeEngineArgs
...
from aphrodite.omni.config.transport_fields import (
    CacheConfigTransportFields,
    ParallelConfigTransportFields,
    SchedulerConfigTransportFields,
)
...
class _StageEngineValues:
    ...
    compilation_config: Mapping[str, Any] | AphroditeCompilationConfig | None
    profiler_config: Mapping[str, Any] | AphroditeProfilerConfig | None
...
class OmniStageModelConfig:
    ...
    """Model architecture override for this stage."""
    ...
    """Attention backend to use for the draft model."""
    attention_config: Any = None
    """Attention configuration."""
    moe_backend: str = "auto"
    """Backend for MoE expert computation kernels."""
    hf_overrides: Any = None
    """If a dictionary, contains arguments to be forwarded to the Hugging Face config."""
    limit_mm_per_prompt: dict[str, Any] | None = None
    """Maximum media inputs of each modality accepted in one prompt."""
    ...
    """Maximum active streaming requests. Zero uses chunk-level round-robin scheduling."""
    session_mode: str = "turn"
    """Request lifecycle mode, either turn-based or duplex."""
    duplex_max_sessions: int = Field(default=1, ge=1)
    """Maximum simultaneous duplex sessions."""
    enable_sleep_mode: bool = False
    """Enable sleep mode for the engine (only cuda and hip platforms are supported)."""
    default_sampling_params: dict[str, Any] | None = None
    """Default sampling parameters for requests handled by this stage."""
    subtalker_sampling_params: dict[str, Any] | None = None
    """Sampling overrides for the audio code predictor."""
    silence_ban_frames: int = 0
    """Number of initial audio frames in which silence tokens are banned."""
    has_sampling_extra_args: bool = False
    """Whether the stage defines additional sampling arguments."""
    custom_voice_dir: str | None = None
    """Directory containing custom voice definitions."""
    task_type: str | None = None
    """Model-specific task selected at startup."""
    codec_frame_rate_hz: float | None = None
    """Number of audio codec frames per second."""
    enforce_eager: bool = False
    """Whether to always use eager-mode PyTorch."""
    ...
    """Load checkpoint weights with multiple threads."""
    num_weight_load_threads: int = Field(default=4, ge=1)
    """Number of threads used to load checkpoint weights."""
    disable_autocast: bool = False
    """Disable automatic mixed-precision casting in the model."""
    ...
    """Checkpoint subdirectory used by this stage."""
    tokenizer_subdir: str | None = None
    """Tokenizer subdirectory used by this stage."""
    requires_full_payload_input: bool = False
    """Require a complete upstream payload before starting this stage."""


@_enforce_keyword_only_init
@config(kw_only=True)
class OmniStageLoadConfig(_TrackExplicitConfigFields, AphroditeLoadConfig):
    """Sonar loading behavior plus Omni stage-specific tokenizer inputs."""
...
@_enforce_keyword_only_init
@config(kw_only=True)
class OmniStageCacheConfig(_TrackExplicitConfigFields, CacheConfigTransportFields):
    """Per-stage engine cache and memory behavior.

    This is separate from ``_DiffusionConfigProjection.cache_config``, which configures
    Sonar Omni diffusion-specific cache backends such as TeaCache and Cache-DiT.
    """

    kv_cache_memory_bytes: int | None = Field(default=None, ge=0)
    """Size of KV Cache per GPU in bytes."""
    ...
    # Hybrid-mamba SSM state dtype ("auto"/"float32"); Sonar CacheConfig field.
    mamba_ssm_cache_dtype: str | None = None
    """The data type to use for the Mamba cache (ssm state only, conv state will still be controlled by mamba_cache_dtype)."""

    @model_validator(mode="after")
    def _validate_cache_config(self) -> Self:
        if self._block_size_resolved:
            return self
        kwargs = {
            config_field.name: getattr(self, config_field.name)
            for config_field in fields(AphroditeCacheConfig)
            if config_field.init and getattr(self, config_field.name) is not None
        }
        validated = AphroditeCacheConfig(**kwargs)
        self.block_size = validated.block_size
        self.user_specified_block_size = validated.user_specified_block_size
        self.user_specified_mamba_block_size = validated.user_specified_mamba_block_size
...
@_enforce_keyword_only_init
@config(kw_only=True)
class OmniStageSchedulerConfig(_TrackExplicitConfigFields, SchedulerConfigTransportFields):
    ...
    """If set to False, disable async scheduling."""
    max_num_encoder_input_tokens: int | None = field(default=None, init=False)
    """Multimodal encoder compute budget, only used in V1."""
    encoder_cache_size: int | None = field(default=None, init=False)
...
class OmniStageConnectorConfig:
    ...
    """Transfer intermediate outputs between stages as chunks become ready."""
    omni_kv_config: dict[str, Any] | None = None
    """Cross-stage KV transfer configuration."""
    ...
    """Connector implementation and options for stage transfers."""
    output_connectors: dict[str, Any] | None = None
    """Connector settings for outgoing stage edges."""
    input_connectors: dict[str, Any] | None = None
    """Connector settings for incoming stage edges."""
...
class OmniStageRuntimeConfig:
    ...
    """The full name of the worker class to use."""
    devices: str | None = None
    """Device identifiers assigned to this stage."""
    num_replicas: int = Field(default=1, ge=1)
    """Number of independent replicas for this stage."""
    env: dict[str, Any] | None = None
    """Environment variables applied to stage workers."""
    num_gpus: int = Field(default=1, ge=1)
    """Number of GPUs assigned to this stage."""
    log_level: str = "info"
    """Minimum severity of emitted log messages."""
    log_stats: bool = False
    """Emit periodic stage statistics."""


@config
class _ParallelValidationConfig(AphroditeParallelConfig):
...
@_enforce_keyword_only_init
@config(kw_only=True)
class OmniStageParallelConfig(_TrackExplicitConfigFields, ParallelConfigTransportFields):
    ...
    worker_cls: str | None = None
    """The full name of the worker class to use."""
    data_parallel_index: int | None = field(default=None, init=False)
    ...
    def _validate_parallel_config(self) -> Self:
        ...
        kwargs = {
            ...
            for config_field in fields(AphroditeParallelConfig)
            if config_field.name in signature(_ParallelValidationConfig).parameters
            and getattr(self, config_field.name) is not None
        }
        kwargs["data_parallel_size_local"] = self.data_parallel_size
        kwargs["data_parallel_rank"] = 0
        _ParallelValidationConfig(**kwargs)
        return self
...
class OmniStageDiffusionParallelConfig(OmniStageParallelConfig):
    ...
    """If True, generate a boolean attention mask for zero-padded SP tokens when sequence length is not divisible by the SP world size."""
    hsdp_shard_size: int = -1
    """Number of GPUs to shard weights across within each replica group."""
...
class _DiffusionConfigProjection:
    ...
    """Diffusion model implementation class."""
    engine_backend: str | type = "default"
    """Backend used to execute diffusion requests."""
    diffusion_model_runner_cls: str | type | None = None
    """Diffusion model runner class override."""
    request_batch_max_wait_ms: float = 0.0
    """Maximum time in milliseconds to wait while forming a request batch."""
    streaming_output: bool = False
    """Return output chunks as they become available."""
    model_arch: str | None = None
    """Model architecture override for this stage."""
    task_type: str | None = None
    """Model-specific task selected at startup."""
    ...
    """Port used to establish NCCL communication."""
    ...
    """Port used for diffusion scheduler communication."""
    host: str | None = None
    """Network address on which the service listens."""
    port: int | None = None
    """Port on which the service listens."""
    model_config: dict[str, Any] = field(default_factory=dict)
    """Model configuration."""
    tf_model_config: Any = None
    """Transformer model configuration loaded from the checkpoint."""
    diffusion_attention_config: Any = None
    """Attention backend settings for diffusion execution."""
    cache_strategy: str = "none"
    """Strategy used to reuse diffusion intermediate results."""
    cache_backend: str = "none"
    """Implementation used for diffusion result caching."""
    cache_config: Any = field(default_factory=dict)
    """Cache configuration."""
    enable_cache_dit_summary: bool = False
    """Log a summary of Cache-DiT activity."""
    diffusion_kv_mode: DiffusionKVCacheMode = DiffusionKVCacheMode.DENSE_LEGACY
    """KV cache mode used by the diffusion scheduler."""
    diffusion_kv_max_rows_per_request: int | None = Field(default=None, ge=1, strict=True)
    """Maximum paged KV cache rows reserved for one diffusion request."""
    enable_prompt_embed_cache: bool = False
    """Reuse cached prompt embeddings across requests."""
    prompt_embed_cache_size: int = Field(default=32, ge=1)
    """Maximum number of prompt embedding cache entries."""
    enable_session_state_manager: bool = False
    """Retain diffusion state for requests in the same session."""
    diffusion_load_format: str = "default"
    """Format used to load diffusion model weights."""
    diffusers_load_kwargs: dict[str, Any] = field(default_factory=dict)
    """Additional arguments passed when loading a Diffusers pipeline."""
    diffusers_call_kwargs: dict[str, Any] = field(default_factory=dict)
    """Additional arguments passed when calling a Diffusers pipeline."""
    diffusers_pipeline_cls: Any = None
    """Diffusers pipeline class override."""
    lora_path: str | list[str] | None = None
    """Path to the diffusion LoRA adapter."""
    lora_scale: float = 1.0
    """Scale applied to the LoRA adapter output."""
    lora_backend: str = "peft"
    """Backend used to apply diffusion LoRA adapters."""
    max_cpu_loras: int | None = None
    """Maximum number of LoRAs to store in CPU memory."""
    output_type: str = "pil"
    """Representation returned by the diffusion pipeline."""
    diffusion_offload_config: dict[str, Any] | None = None
    """Settings for moving diffusion weights between CPU and GPU memory."""
    ...
    """Move model weights to CPU memory when they are not in use."""
    enable_layerwise_offload: bool = False
    """Move diffusion layers between CPU and GPU memory as they execute."""
    enable_distributed_layerwise_offload: bool = False
    """Distribute layer weight transfers across workers."""
    dlo_use_allgather: bool = True
    """Use all-gather for distributed layer weight transfers."""
    dlo_resident_layers: int = Field(default=0, ge=0)
    """Number of layers kept in GPU memory during distributed offload."""
    host_weight_runtime_mode: Literal["disabled", "preferred", "required"] = "disabled"
    """Runtime strategy for managing host-resident model weights."""
    host_weight_runtime_root: str | None = None
    """Root directory for host-resident weight storage."""
    dlo_host_registration_limit_gib: float = Field(default=0.0, ge=0)
    """Maximum registered host memory in GiB for distributed layer offload."""
    pin_cpu_memory: bool = True
    """Use pinned CPU memory for host-to-device transfers."""
    diffusion_compile_granularity: Literal["regional", "full"] = "regional"
    """Parts of the diffusion pipeline to compile."""
    diffusion_compile_dynamic: bool = Field(default=True, strict=True)
    """Compile diffusion execution with dynamic shapes."""
    fa_deterministic: bool = False
    """Use deterministic FlashAttention execution."""
    vae_use_slicing: bool = False
    """Decode VAE batches in smaller slices."""
    vae_use_tiling: bool = False
    """Decode VAE outputs in spatial tiles."""
    mask_strategy_file_path: str | None = None
    """Path to the diffusion attention mask strategy."""
    skip_time_steps: int = 15
    """Diffusion time steps excluded by the configured skip strategy."""
    VSA_sparsity: float = 0.0
    """Target sparsity for video sparse attention."""
    moba_config_path: str | None = None
    """Path to mixture-of-block attention settings."""
    boundary_ratio: float | None = None
    """Boundary used to switch between diffusion model phases."""
    flow_shift: float | None = None
    """Shift applied to diffusion flow-matching time steps."""
    diffusion_kv_cache_dtype: str | None = None
    """Data type used for diffusion KV cache entries."""
    diffusion_kv_cache_skip_steps: str | list[int] | tuple[int, ...] | set[int] | None = None
    """Selector for diffusion steps that retain unquantized keys and values."""
    diffusion_kv_cache_skip_layers: str | list[int] | tuple[int, ...] | set[int] | None = None
    """Selector for diffusion layers that retain unquantized keys and values."""
    diffusion_kv_cache_skip_step_indices: set[int] | None = None
    """Resolved diffusion step indices excluded from KV quantization."""
    diffusion_kv_cache_skip_layer_indices: set[int] | None = None
    """Resolved diffusion layer indices excluded from KV quantization."""
    moe_backend: str = "auto"
    """Backend for MoE expert computation kernels."""
    force_cutlass_fp8: bool = False
    """Require CUTLASS kernels for FP8 diffusion linear layers."""
    enable_diffusion_pipeline_profiler: bool = False
    """Record diffusion pipeline profiling information."""
    step_execution: bool = False
    """Schedule diffusion work one denoising step at a time."""
    supports_multimodal_inputs: bool = False
    """Whether this pipeline accepts media inputs."""
    max_multimodal_image_inputs: int | None = None
    """Maximum number of image inputs accepted by one request."""
    supports_mixed_reference_inputs: bool = False
    """Whether one request can combine different reference media types."""
    model_paths: dict[str, str] = field(default_factory=dict)
    """Resolved checkpoint paths for diffusion components."""
    ...
    """Whether model weights have been loaded."""
    override_transformer_cls_name: str | None = None
    """Transformer implementation class override."""
    worker_extension_cls: str | None = None
    """The full name of the worker extension class to use."""
    custom_pipeline_args: dict[str, Any] | None = None
    """Additional model-specific pipeline arguments."""
    additional_config: dict[str, Any] = field(default_factory=dict)
    """Additional config for specified platform."""
    kv_transfer_config: KVTransferConfig | None = None
    """The configurations for distributed KV cache transfer."""
    enable_stage_verification: bool = True
    """Verify stage configuration during startup."""
    prompt_file_path: str | None = None
    """Path to a file containing input prompts."""
    quantization_config: _QuantizationConfigType = None
    """User-facing quantization configuration."""
    extras: dict[str, Any] = field(default_factory=dict)
    """Additional backend-specific configuration values."""

    @field_validator("kv_transfer_config", mode="before")
    @classmethod
    def _normalize_kv_transfer_config(cls, value: Any) -> Any:
        from aphrodite.omni.diffusion.diffusion_kv.kv_connector import parse_kv_transfer_config

        return parse_kv_transfer_config(value)

    @classmethod
    def from_kwargs(cls, **kwargs: Any) -> _DiffusionConfigProjection:
        from aphrodite.omni.diffusion.data import normalize_omni_diffusion_kwargs
        from aphrodite.omni.diffusion.offloader.config import parse_diffusion_offload_config
    ...
    def __post_init__(self) -> None:
        # Keep diffusion imports lazy so importing aphrodite.omni.config does not
        # pull in the full diffusion stack unless a diffusion stage is built.
        from aphrodite.omni.diffusion.data import (
            AttentionConfig,
        ...
        from aphrodite.omni.diffusion.diffusion_kv.config import parse_diffusion_kv_cache_mode
...
def _upstream_engine_field_map(
    ...
    *,
    ...
    engine_fields = frozenset(config_field.name for config_field in fields(AphroditeEngineArgs))
    return {
...
_DIRECT_APHRODITE_CONFIG_ENGINE_FIELDS = frozenset({"compilation_config", "profiler_config"})
...
_COMMON_STAGE_ENGINE_FIELDS = (
    ...
    | _DIRECT_APHRODITE_CONFIG_ENGINE_FIELDS
)
...
def _global_stage_cli_fields() -> frozenset[str]:
    # Lazy import avoids aphrodite.omni.config -> omni_config -> engine.arg_utils ->
    # aphrodite.omni.config during package-level config imports.
    from aphrodite.omni.engine.arg_utils import OmniEngineArgs, orchestrator_field_names
    ...
    externally_consumed = (
        ...
        | frozenset(f.name for f in fields(cast(Any, AphroditeOmniOrchestratorConfig)))
...
def _orchestrator_cli_overrides(cli_overrides: Mapping[str, Any]) -> dict[str, Any]:
    overrides: dict[str, Any] = {}
    for config_field in fields(cast(Any, AphroditeOmniOrchestratorConfig)):
...
@config
class AphroditeOmniOrchestratorConfig:
    """Configuration consumed by the orchestrator process only."""

    stage_init_timeout: int = Field(default=300, ge=1)
    """Maximum time in seconds to initialize one stage."""
    init_timeout: int = Field(default=600, ge=1)
    """Maximum time in seconds to initialize the pipeline."""
    worker_backend: str = "multi_process"
    """Backend used to launch stage workers."""
    ray_address: str | None = None
    """Address of the Ray cluster used for workers."""
    deploy_config_path: str | None = None
    """Path to the deployment configuration file."""
    omni_master_address: str | None = None
    """Address of the Omni replica registration service."""
    omni_master_port: int | None = None
    """Port of the Omni replica registration service."""
    omni_dp_size_local: int = Field(default=1, ge=1)
    """Number of local Omni data-parallel replicas."""
    omni_lb_policy: str = "random"
    """Policy used to select a stage replica for each request."""
    omni_heartbeat_timeout: float = Field(default=30.0, gt=0.0)
    """Time in seconds before a missing replica heartbeat causes removal."""
    batch_timeout: int = Field(default=10, ge=0)
    """Maximum wait time in seconds for a stage batch."""
    ...
    """Initialize independent stages concurrently under device memory admission control."""


class _ResolvedProcessorPaths:
    _resolved_custom_process_input_func: str | None
    _resolved_custom_process_next_stage_input_func: str | None


@config(config=ConfigDict(arbitrary_types_allowed=True))
class BaseAphroditeOmniStageConfig(_ResolvedProcessorPaths):
    """Common structured config contract shared by all Omni stage realizations."""

    stage_pipeline_config: StagePipelineConfig = Field(...)
    """Stage topology and input/output relationships."""
    ...
    """Transfer settings for this stage."""
    runtime_config: OmniStageRuntimeConfig = field(default_factory=OmniStageRuntimeConfig)
    """Worker placement and runtime settings for this stage."""
    parallel_config: OmniStageParallelConfig = field(default_factory=OmniStageParallelConfig)
    """Parallel configuration."""
    compilation_config: AphroditeCompilationConfig | None = None
    """`torch.compile` and cudagraph capture configuration for the model."""
    profiler_config: AphroditeProfilerConfig | None = None
...
@config(config=ConfigDict(arbitrary_types_allowed=True))
class AphroditeOmniARStageConfig(BaseAphroditeOmniStageConfig):
    """Structured config for autoregressive LLM stages."""


@config(config=ConfigDict(arbitrary_types_allowed=True))
class AphroditeOmniGenerationStageConfig(BaseAphroditeOmniStageConfig):
    """Structured config for generation LLM stages."""


@config(config=ConfigDict(arbitrary_types_allowed=True))
class AphroditeOmniDiffusionStageConfig(BaseAphroditeOmniStageConfig):
    ...
    """Diffusion-specific model and execution settings."""


StageConfigType: TypeAlias = (
    AphroditeOmniARStageConfig | AphroditeOmniGenerationStageConfig | AphroditeOmniDiffusionStageConfig
)
    ...
    *,
    ...
    return cast(
        ...
        _with_resolved_processors(
            AphroditeOmniARStageConfig(**common_kwargs),
            input_proc,
            next_stage_proc,
        ),
    )
...
def _build_generation_stage_config(
    ...
    *,
    ...
    return cast(
        AphroditeOmniGenerationStageConfig,
        _with_resolved_processors(
            AphroditeOmniGenerationStageConfig(**common_kwargs),
            input_proc,
            next_stage_proc,
        ),
    )
...
def _build_diffusion_stage_config(
    ...
    *,
    ...
    return cast(
        AphroditeOmniDiffusionStageConfig,
        _with_resolved_processors(
            AphroditeOmniDiffusionStageConfig(**common_kwargs),
            input_proc,
            next_stage_proc,
        ),
    )
...
@config(config=ConfigDict(arbitrary_types_allowed=True))
class AphroditeOmniConfig:
    """Top-level structured Omni config built once from registry inputs."""

    pipeline_config: PipelineConfig = Field(...)
    """Topology and model roles for the full pipeline."""
    stage_configs: tuple[StageConfigType, ...] = Field(...)
    """Resolved configurations for each pipeline stage."""
    orchestrator_config: AphroditeOmniOrchestratorConfig = field(default_factory=AphroditeOmniOrchestratorConfig)
    """Startup and replica-management settings for the pipeline."""
    ...
    def from_pipeline_config(
        ...
        *,
        ...
        orchestrator_config = cast(Any, AphroditeOmniOrchestratorConfig)(
