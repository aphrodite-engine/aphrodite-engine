...
import aphrodite.omni.model_executor.models as me_models
...
class OmniModelArchConfigConvertor(ModelArchConfigConvertorBase):
    ...
    "thinker."-prefixed names that don't match sonar-omni's module names.
...
class OmniModelConfig(ModelConfig):
    """Configuration for Omni models, extending the base ModelConfig.
     ...
     This configuration class extends the base Sonar ModelConfig with
    ...
    The correct way to initialize this class is via Sonar config, as most
    of the logic for handling values is in the ModelConfig's __post_init__.

       Example:
         >>> config = OmniModelConfig.from_aphrodite_model_config(
         ...     aphrodite_config, stage_id=0, model_stage="thinker", model_arch="Qwen2_5OmniForConditionalGeneration"
         ... )
    """
    ...
    """Keep stage state between chunks of the same request."""
    ...
    """Role of this model stage, such as thinker or talker."""
    model_arch: str | None = None
    """Model architecture override for this stage."""
    worker_type: str | None = None
    """Worker execution mode for this stage."""
    engine_output_type: str | None = None
    """Output payload type used to select the stage output processor."""
    ...
    """Dotted path to the worker-side pooling output decoder."""
    hf_config_name: str | None = None
    """Name of the stage-specific section in the Hugging Face config."""
    custom_process_next_stage_input_func: str | None = None
    """Dotted path to the function that prepares inputs for the next stage."""
    ...
    """Connector implementation and options for this stage."""
    ...
    """Names of the stage's default additional sampling arguments."""
    ...
    def _validate_startup_task_type(self) -> None:
        ...
        if self.model_arch != "Qwen3TTSTalkerForConditionalGenerationARAPHRODITE" or self.task_type is None:
            return
    ...
    @classmethod
    def from_aphrodite_model_config(cls, model_config: ModelConfig, **omni_kwargs):
        """Create OmniModelConfig from an existing Sonar ModelConfig
        ...
        if (
            ...
            and omni_cfg.model_arch == "Qwen3TTSTalkerForConditionalGenerationARAPHRODITE"
        ):
