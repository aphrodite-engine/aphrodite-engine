...
"""Config factories for sonar-omni, e.g., StageConfigFactory."""
...
from aphrodite.omni.config.endpoint_policy import EndpointRestriction
from aphrodite.omni.config.omni_config import AphroditeOmniConfig
from aphrodite.omni.config.pipeline_registry import OMNI_PIPELINES, resolve_pipeline_config
...
from aphrodite.omni.diffusion.io_support import get_diffusion_output_type
...
def _materialize_object_storage_configs(model: str) -> str:
    ...
    Sonar's Run:AI streamer keeps ``s3://``/``gs://``/``az://`` URIs opaque until
    ...
    Pull the lightweight files once into Sonar's deterministic
...
def with_trust_remote_code_override(
    ...
    yaml per-stage value > Sonar default False): a non-None value becomes an
...
class StageConfigFactory:
    ...
    :func:`aphrodite.omni.config.resolver.resolve_omni_config`. Factory methods are
    ...
    Pipelines are declared in ``aphrodite/omni/config/pipeline_registry.py`` and
    ...
    def create_from_model(
        ...
        *,
        ...
        return AphroditeOmniConfig.from_pipeline_config(
            pipeline_cfg,
            user_deploy_config=user_deploy_config,
            deploy_config_path=deploy_config_path,
    ...
    def _resolve_legacy_from_registry(
        ...
        from aphrodite.omni.utils.forced_aligner import inject_forced_aligner_stage
    ...
    def _apply_strategy_specs(
        ...
        if not strategy_specs:
            return None
        from aphrodite.omni.config.composable_parallel import apply_strategy_specs
    ...
    def _reconcile_strategy_with_cli(
        ...
        if applied is None:
            return
        from aphrodite.omni.config.composable_parallel import check_device_layout
