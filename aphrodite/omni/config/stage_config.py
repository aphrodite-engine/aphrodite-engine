...
"""Stage configuration system for Sonar Omni."""
...
from aphrodite.omni.config.yaml_util import create_config, load_yaml_config, to_dict
from aphrodite.omni.core.sched.omni_ar_scheduler import OmniARAsyncScheduler, OmniARScheduler
from aphrodite.omni.core.sched.omni_generation_scheduler import OmniGenerationScheduler
...
def build_stage_runtime_overrides(
    ...
    *,
    ...
    if internal_keys is None:
        from aphrodite.omni.engine.arg_utils import SHARED_FIELDS, internal_blacklist_keys
...
def _scheduler_path(cls: type[APHRODITEScheduler] | None) -> str | None:
...
class StagePipelineConfig:
    ...
    # Rewrites the Stage-0 view of a raw prompt before Sonar input processing.
...
class PipelineConfig:
    ...
    # loaded from aphrodite/omni/deploy; None uses DeployConfig defaults.
...
class StageDeployConfig:
    ...
    # backbone) pin the SSM state dtype; projected onto Sonar CacheConfig.
...
def merge_pipeline_deploy(
    ...
    for ps in pipeline.stages:
        ...
        # an encoder. Do not make Sonar profile dummy multimodal inputs for them.
