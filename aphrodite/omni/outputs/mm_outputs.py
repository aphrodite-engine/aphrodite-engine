...
"""Multimodal output data structures for Sonar Omni.
...
from aphrodite.omni.outputs.output_modality import TensorAccumulationStrategy
from aphrodite.omni.outputs.utils import _is_tensor_list, _to_cpu
...
class MultimodalCompletionOutput(CompletionOutput):
    ...
    As a CompletionOutput subclass, compatible with all existing Sonar consumers.
    """

    if TYPE_CHECKING:
        cumulative_token_ids: list[int] = field(init=False)
        cumulative_text: str = field(init=False)
