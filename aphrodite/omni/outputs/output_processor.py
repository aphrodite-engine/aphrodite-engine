...
from aphrodite.omni.data_entry_keys import unflatten_payload
from aphrodite.omni.outputs.mm_outputs import MultimodalCompletionOutput, MultimodalPayload
from aphrodite.omni.outputs.multimodal_accumulation import (
    drain_delta_payload,
    is_non_final_delta_audio_chunk,
    replace_snapshot_keys,
)
from aphrodite.omni.outputs.output_modality import OutputModality, get_accumulation_strategy
from aphrodite.outputs import CompletionOutput, PoolingRequestOutput, RequestOutput
...
from aphrodite.v1.engine.output_processor import OutputProcessor as APHRODITEOutputProcessor
from aphrodite.v1.engine.output_processor import (
    OutputProcessorOutput,
    RequestOutputCollector,
    RequestState,
)
from aphrodite.v1.engine.parallel_sampling import ParentRequest
from aphrodite.v1.metrics.stats import IterationStats, RequestStateStats

logger = init_logger(__name__)


class _CumulativeCompletion(Protocol):
...
class OmniRequestState(RequestState):
    ...
    def _new_completion_output(
        ...
        cumulative_output = cast(_CumulativeCompletion, base_output)
        if self.detokenizer is not None:
            cumulative_output.cumulative_token_ids = list(self.detokenizer.output_token_ids)
        else:
            cumulative_output.cumulative_token_ids = list(token_ids)

        # Attach cumulative_text only at the final step for inter-stage use.
        if finish_reason is not None and self.detokenizer is not None:
            cumulative_output.cumulative_text = self.detokenizer.get_next_output_text(finished=True, delta=False)
        ...
        try:
            if self.mm_accumulated and not self.mm_accumulated.is_empty:
                ...
                output.cumulative_token_ids = cumulative_output.cumulative_token_ids
                if hasattr(base_output, "cumulative_text"):
                    output.cumulative_text = cumulative_output.cumulative_text
...
class MultimodalOutputProcessor(APHRODITEOutputProcessor):
    ...
    the base Sonar OutputProcessor for text handling.

    The data flow is:
    1. For each EngineCoreOutput with multimodal_output:
       - Capture into OmniRequestState.add_multimodal_tensor()
    2. Base Sonar OutputProcessor handles text detokenization
    ...
    def _native_text_metric_record(self, request_id: str) -> dict[str, Any]:
        return self._native_text_metrics_by_request.setdefault(
            request_id,
            {
                ...
                "aphrodite_ttft_ms": 0.0,
                "aphrodite_tpot_ms": 0.0,
                "aphrodite_itl_ms": 0.0,
                "aphrodite_itls_ms": [],
            },
        )
    ...
    def _update_stats_from_output(
        ...
        if was_prefilling:
            record["aphrodite_ttft_ms"] = max(float(native_stats.first_token_latency) * 1000.0, 0.0)
            return

        if previous_last_token_ts > 0:
            itl_ms = max((engine_core_timestamp - previous_last_token_ts) * 1000.0, 0.0)
            itls_ms = record.setdefault("aphrodite_itls_ms", [])
            itls_ms.append(itl_ms)
            record["aphrodite_itl_ms"] = sum(itls_ms) / float(len(itls_ms))
        record["aphrodite_tpot_ms"] = _mean_time_per_output_token_ms(native_stats)
    ...
    def _update_stats_from_finished(
        ...
        if finished_request.request_id != req_state.external_req_id:
            return
        self._native_text_metric_record(req_state.external_req_id)["aphrodite_tpot_ms"] = (
