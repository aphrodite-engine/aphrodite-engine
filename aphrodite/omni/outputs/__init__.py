...
class OmniRequestOutput(RequestOutput):
    ...
    Extends Sonar's ``RequestOutput`` so that omni outputs can flow directly
    through Sonar serving codepaths (which expect ``prompt_token_ids``,
    ...
    Attributes:
        ...
        metrics: Generation metrics. A plain dict for omni outputs; may carry
            Sonar's request stats object when copied from a raw RequestOutput.
    """
    ...
    diffusion_prompt: OmniPromptType | None = None
    ...
    def _copy_content_from(self, source: RequestOutput) -> None:
        ...
        *source* should be a Sonar ``RequestOutput`` or a subclass such as
    ...
    def from_stage_output(cls, source: RequestOutput, **kwargs: Any) -> "OmniRequestOutput":
        ...
        returned object.  *source* may be a Sonar ``RequestOutput``, another
    ...
    def from_diffusion(
        ...
        """
        text_prompt = prompt if isinstance(prompt, str) else None
        if isinstance(prompt, dict):
            candidate = prompt.get("prompt")
            ...
            result.update(
                {
                    ...
                    "prompt": self.diffusion_prompt if self.diffusion_prompt is not None else self.prompt,
