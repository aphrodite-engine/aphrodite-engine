...
``aphrodite:*{stage="thinker", ...}`` families exposed via the
...
from aphrodite.omni.metrics.utils import observe_audio_finalize, observe_diffusion_finalize
...
def observe_modality_at_finalize(
    ...
    *,
    ...
    upstream ``aphrodite:*{stage="thinker", ...}``. Caller should not need to
...
def observe_audio_streaming_finalize(
    ...
    *,
    ...
    Reuses the math from ``aphrodite.omni.benchmarks.audio_continuity`` so the
