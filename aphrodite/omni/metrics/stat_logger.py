...
``replica`` two-label scheme so the ~37 ``aphrodite:*`` metric families gain
...
from aphrodite.v1.metrics.perf import PerfMetricsProm
...
# StatLogger that owns them. Sonar runs a single Orchestrator/StatLogger per
...
class _RelabelMixin:
    ...
    ``aphrodite.v1.metrics.loggers.PrometheusStatLogger``:
...
class OmniPrometheusStatLogger(PrometheusStatLogger):
    ...
    ``replica`` so the ~37 ``aphrodite:*`` metric families gain per-replica
