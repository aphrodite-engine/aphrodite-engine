...
from aphrodite.omni.metrics.utils import _build_field_defs, _build_row, _format_table
...
class StageRequestStats:
    ...
    aphrodite_ttft_ms: float = 0.0
    aphrodite_tpot_ms: float = 0.0
    aphrodite_itl_ms: float = 0.0
    aphrodite_itls_ms: list[float] | None = None
...
class OrchestratorAggregator:
    ...
    def _merge_stage_metric_event(
        ...
        if current is None:
            current = {
                ...
                defs.APHRODITE_TTFT_MS: float(evt.aphrodite_ttft_ms),
                defs.APHRODITE_TPOT_MS: float(evt.aphrodite_tpot_ms),
                defs.APHRODITE_ITL_MS: float(evt.aphrodite_itl_ms),
                defs.APHRODITE_ITLS_MS: list(evt.aphrodite_itls_ms or []),
            }
            return current
        ...
        aphrodite_ttft_ms = float(evt.aphrodite_ttft_ms)
        current_aphrodite_ttft_ms = float(current.get(defs.APHRODITE_TTFT_MS, 0.0))
        if current_aphrodite_ttft_ms <= 0 < aphrodite_ttft_ms:
            current[defs.APHRODITE_TTFT_MS] = aphrodite_ttft_ms
        aphrodite_tpot_ms = float(evt.aphrodite_tpot_ms)
        if aphrodite_tpot_ms > 0:
            current[defs.APHRODITE_TPOT_MS] = aphrodite_tpot_ms
        aphrodite_itls = list(current.get(defs.APHRODITE_ITLS_MS) or [])
        aphrodite_itls.extend(list(evt.aphrodite_itls_ms or []))
        current[defs.APHRODITE_ITLS_MS] = aphrodite_itls
        current[defs.APHRODITE_ITL_MS] = sum(aphrodite_itls) / float(len(aphrodite_itls)) if aphrodite_itls else 0.0
    ...
    def on_stage_metrics(
        ...
        self.stage_total_tokens[stage_id] += int(stats.num_tokens_out)
        if stage_id == 0:
            self.stage_total_tokens[stage_id] += int(stats.num_tokens_in)
    ...
    def build_and_log_summary(self) -> dict[str, Any]:
        ...
        stage_wall_time_ms = [
            ...
            for first, last in zip(self.stage_first_ts, self.stage_last_ts)
        ]
