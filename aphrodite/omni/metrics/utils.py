...
if TYPE_CHECKING:
    from aphrodite.omni.metrics.modality import OmniModalityMetrics
    from aphrodite.omni.metrics.prometheus import OmniPrometheusMetrics
    from aphrodite.omni.metrics.stats import StageRequestStats

FAILURE_REASONS = frozenset({"client_abort", "client_disconnect", "stage_error", "unknown"})
DIFFUSION_METRICS_ONLY_REQUEST_ID = "__aphrodite_omni_diffusion_metrics__"
...
def extract_mm_output(mm_source: object) -> Mapping[str, Any]:
    ...
    Args:
        mm_source: Duck-typed container with optional ``multimodal_output`` and/or
            ``outputs`` (e.g. Sonar ``RequestOutput`` or ``OmniRequestOutput``).
...
def _field_getter(name: str, transform: Callable[[Any], Any] | None = None) -> Callable[[Any], Any]:
    def get_field(event: Any) -> Any:
...
def _build_field_defs(
    cls: type,
    exclude: set[str],
    transforms: dict[str, tuple[str, Callable[[Any], Any]]] | None = None,
    ...
    for f in fields(cls):
        ...
        if f.name in transforms:
            ...
            result.append((display_name, _field_getter(f.name, transform_fn)))
        else:
            result.append((f.name, _field_getter(f.name)))
    return result
