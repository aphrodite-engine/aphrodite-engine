...
from aphrodite.omni.experimental.fullduplex.joyvl.bridges.backend import OpenAIBackend
from aphrodite.omni.experimental.fullduplex.joyvl.bridges.delegation import (
    DelegationBridge,
...
from aphrodite.omni.experimental.fullduplex.joyvl.decision.output_parser import to_token_form
from aphrodite.omni.experimental.fullduplex.joyvl.memory.memory import Summarizer
...
def create_app(config: InteractionConfig) -> FastAPI:
    ...
    app = FastAPI(title="Sonar Omni Interaction Server", lifespan=lifespan)
    ...
    async def models() -> dict[str, Any]:
        return {"object": "list", "data": [{"id": config.main_model, "object": "model", "owned_by": "sonar-omni"}]}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Sonar Omni streaming interaction server")
