...
AR_DIFFUSION_MODEL_RUNNER_CLS = "aphrodite.omni.experimental.ar_diffusion.runner.ARDiffusionModelRunner"
...
class ARDiffusionEngine(DiffusionEngine):
    ...
    Sonar's paged KV stack (``KVCacheManager`` / ``BlockPool`` / ``BlockTables``)
    ...
    actual KV *body* is :class:`~aphrodite.omni.experimental.ar_diffusion.kv_cache.manager.ARDiffusionKVCache`, owned
    by :class:`~aphrodite.omni.experimental.ar_diffusion.runner.ARDiffusionModelRunner`. ``ARDiffusionEngine`` itself is the
