...
* **Chunk-window eviction** — a ``SlidingWindowSpec`` subclass whose unit is a
  *chunk* (``sliding_window = window_chunks * chunk_size``) plus a manager that
  ...
  replacement stay in Sonar's ``BlockPool``; only the token-skip math is here.
"""
...
from aphrodite.v1.kv_cache_interface import SlidingWindowSpec
from aphrodite.v1.kv_cache_spec_registry import register_kv_cache_spec
...
def allocate_kv_pool_with_views(
    ...
    """Allocate Sonar-style paged KV pools plus flat slot-write views.
