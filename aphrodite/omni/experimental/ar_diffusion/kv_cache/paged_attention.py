...
from aphrodite.omni.experimental.ar_diffusion.kv_cache.paged import compute_slot_mapping
...
def ar_diffusion_paged_attention(
    ...
    *,
    ...
    """Run non-causal paged attention over a Sonar block table.
    ...
    if not query_flat.is_cuda:
        ...
        # aphrodite.vllm_flash_attn contains CUDA-only extensions. ROCm's AITER and
        # upstream flash-attn expose the standard cu_seqlens_k API instead of
        # Sonar's seqused_k/fa_version API. The ROCm flash-attn paged kernel also
...
if not hasattr(torch.ops.aphrodite_omni, "ar_diffusion_paged_write_attn"):

    @torch.library.custom_op(
        "aphrodite_omni::ar_diffusion_paged_write_attn",
...
def paged_write_attn(
    ...
    return torch.ops.aphrodite_omni.ar_diffusion_paged_write_attn(
        query,
        k_curr,
        v_curr,
        k_act,
