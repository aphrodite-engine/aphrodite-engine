...
```bash
sonar-omni serve bosonai/higgs-audio-v3-tts-4b \
    --omni --trust-remote-code \
    --deploy-config aphrodite/omni/deploy/higgs_multimodal_qwen3_high_throughput.yaml
```
...
sets Stage 0 `enforce_eager: false` and explicitly enables Sonar
...
sonar-omni serve bosonai/higgs-audio-v3-tts-4b \
    --omni --trust-remote-code \
    --deploy-config aphrodite/omni/deploy/higgs_multimodal_qwen3_low_latency.yaml
```
...
`FLASHINFER` names the overall Sonar attention backend. Starting with Sonar
...
trace for the exact Higgs batch/context distribution. Until Sonar has a
...
- Keep `APHRODITE_USE_DEEP_GEMM=0` and `APHRODITE_MOE_USE_DEEP_GEMM=0` for this model
