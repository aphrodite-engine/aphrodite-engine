...
from aphrodite.inputs import EmbedsPrompt, PromptType, TextPrompt, TokensPrompt
from aphrodite.inputs.engine import MultiModalInput, TokensInput
...
class OmniTextPrompt(TextPrompt):
    ...
    use_system_prompt: NotRequired[str]
    system_prompt: NotRequired[str]
    _omni_request_artifact_dirs: NotRequired[list[str]]
...
class OmniMultiModalInput(MultiModalInput):
    prompt_embeds: NotRequired[torch.Tensor]
    additional_information: NotRequired[dict[str, Any]]
    model_intermediate_buffer: NotRequired[dict[str, Any]]
...
# Must ensure that all additional prompt types are inherited from Sonar prompt types
...
"""Omni singleton prompt type extending Sonar's SingletonPrompt."""
...
class OmniDiffusionSamplingParams:
    ...
    def from_params(cls, params: Any) -> "OmniDiffusionSamplingParams":
        ...
        Existing Omni params are returned unchanged. Plain Sonar SamplingParams
        ...
        if isinstance(params, SamplingParams):
            ...
            return cls(**mapped)
        raise TypeError(
            "Diffusion stage requires OmniDiffusionSamplingParams or aphrodite.SamplingParams, "
            f"got {type(params).__name__!r}."
        )


OmniSamplingParams: TypeAlias = SamplingParams | PoolingParams | OmniDiffusionSamplingParams
