...
EXTRAS = tuple(f"<|extra_{i}|>" for i in range(181))  # 205 - 19[len(QWEN_SPECIAL_TOKENS)] - 5
# align to qwen2.5 embedding size (152064)
EXTRAS += tuple(f"<|extra_margin_{i}|>" for i in range(152064 - 151846))
...
class MammothUTokenizer(PreTrainedTokenizer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.decoder: dict[int, bytes | str] = {v: k for k, v in self.mergeable_ranks.items()}
    ...
    def get_vocab(self) -> dict[bytes | str, int]:
        vocab: dict[bytes | str, int] = {token: rank for token, rank in self.mergeable_ranks.items()}
    ...
    @overload
    def convert_tokens_to_ids(self, tokens: bytes | str) -> int | None: ...

    @overload
    def convert_tokens_to_ids(self, tokens: list[bytes | str]) -> list[int | None]: ...

    def convert_tokens_to_ids(self, tokens: bytes | str | list[bytes | str]) -> int | None | list[int | None]:
        if isinstance(tokens, (str, bytes)):
            if isinstance(tokens, str):
                return self.special_tokens.get(tokens)
            return self.mergeable_ranks.get(tokens)

        ids: list[int | None] = []
        for token in tokens:
            if isinstance(token, str):
                ids.append(self.special_tokens.get(token))
            else:
    ...
    def save_vocabulary(self, save_directory: str, **kwargs) -> tuple[str, str]:
        """
        ...
        with open(special_file_path, "w", encoding="utf8") as w:
            for token in self.special_tokens:
                if token not in excluded_special_tokens:
                    print(token, file=w)
    ...
    def _convert_token_to_id(self, token: bytes | str) -> int:
        ...
        if isinstance(token, str) and token in self.special_tokens:
            return self.special_tokens[token]
        if isinstance(token, bytes) and token in self.mergeable_ranks:
    ...
    @overload
    def bytes_to_str(self, byte_tokens: dict[bytes, int]) -> dict[str, int]: ...

    @overload
    def bytes_to_str(self, byte_tokens: bytes) -> str: ...

    def bytes_to_str(self, byte_tokens: dict[bytes, int] | bytes) -> dict[str, int] | str:
