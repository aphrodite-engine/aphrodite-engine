...
from aphrodite.omni.data_entry_keys import MetaStruct, OmniPayloadStruct, unflatten_payload
from aphrodite.omni.request import OmniRequest as Request
...
class _LoadEntry:
    ...
    def external_req_id(self) -> str:
        return self.request.external_req_id or self.request.request_id
...
class OmniChunkTransferAdapter(OmniTransferAdapterBase):
    ...
    def __init__(self, aphrodite_config: Any):
        ...
        self.scheduler_max_num_seqs = aphrodite_config.scheduler_config.max_num_seqs
        (
            self._max_model_len,
        ...
        # waiting_queue.prepend_requests mid-step, which trips aphrodite's
    ...
    def _confirmed_num_computed_tokens(request: Request) -> int:
        # Sonar async scheduling advances num_computed_tokens with output
    ...
    def _refresh_generation_chunk_prefill_state(request: Request) -> None:
        if request.prompt_token_ids is None:
            raise ValueError("Streaming chunk prefill requires prompt token IDs")
    ...
    def save_async(
        ...
        """
        if request is None:
            raise ValueError("save_async requires a request")
        ...
        external_req_id = request.external_req_id or request.request_id
    ...
    def _preempt_non_active_running(self, waiting_queue: Any, running_queue: list[Request]) -> None:
        ...
        # aphrodite RequestQueue mid-step triggers
        #   "Cannot register new removed request after self.removed has
        #    been read"
        # in aphrodite.v1.sample.logits_processor.state when the persistent
