...
class MoriTransferEngineConnector(OmniConnectorBase):
                ...
                try:
                    ...
                    if isinstance(data, torch.Tensor):
                        ...
                        if src_view.device != dst_t.device:
                            ...
                            if src_view.is_cuda:
                                torch.cuda.current_stream(src_view.device).synchronize()
                            elif dst_t.is_cuda:
                                torch.cuda.current_stream(dst_t.device).synchronize()
                        else:
        ...
        try:
            ...
            if resp == TRANS_DONE:
                if self.pool.is_cuda:
                    torch.cuda.current_stream(self.pool.device).synchronize()
    ...
    def _zmq_listener_loop(self) -> None:
        sender_executor = self._sender_executor
        assert sender_executor is not None
