...
def get_omni_replica_id() -> int:
    ...
    try:
        replica_id = int(os.environ.get("APHRODITE_OMNI_REPLICA_ID", "0"))
    except (ValueError, TypeError):
        return 0
...
def normalize_layer_kv(
    ...
    *,
    ...
    In Sonar, different attention backends return paged-attention KV blocks
    with different layouts.  For example:

    * **FlashAttention** (``aphrodite/v1/attention/backends/flash_attn.py``)
      returns shape ``(2, num_blocks, block_size, num_kv_heads, head_size)``
      – the key/value dimension is at **dim 0**.
    * **FlashInfer** (``aphrodite/v1/attention/backends/flashinfer.py``)
    ...
    * **Packed tensor** ``[num_blocks, n_heads, block_size, 2 * head_dim]`` –
      Sonar >= 0.23 FlashAttention packs K and V into the last (content)
    ...
    * **Tuple** ``(key_tensor, value_tensor)`` – returned as-is after
    ...
    if isinstance(layer_kv, torch.Tensor):
        if (
            ...
            # Sonar >= 0.23 FlashAttention layout: (num_blocks, n_heads,
