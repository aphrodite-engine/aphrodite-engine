...
from aphrodite.omni.diffusion.sched.interface import KVPrefetchJob
...
class _TransferTopoConfig:
    ...
    sp_group: Any
    world: Any
    # The stage's tensor-model-parallel group (Sonar parallel state). Only the
...
class KVCacheTransferData:
    ...
    def _populate_caches(
        header: dict[str, Any], get_tensor: Callable[[dict[str, Any]], torch.Tensor]
    ) -> dict[str, Any]:
...
class OmniKVTransferManager:
    ...
    def __init__(self, config: OmniKVCacheConfig, *, async_prefetch: bool = False):
        ...
        recv_from: str | int | None = config.from_stage
        ...
        self._bg_copy_stream: object | None = None
    ...
    @classmethod
    def from_aphrodite_config(cls, aphrodite_config: Any, model_config: Any) -> "OmniKVTransferManager":
        """Create from aphrodite config with fallback to kv_transfer_config."""
        ...
        connector_cfg = cls._connector_cfg_from_kv_transfer(getattr(aphrodite_config, "kv_transfer_config", None))
    ...
    def _build_topo_config(self) -> _TransferTopoConfig:
        ...
        # The stage's TP group comes from Sonar parallel state (the TP-sharded
        ...
        try:
            ...
            tp_group, tp_size = None, 1  # Sonar TP not initialized
    ...
    def _resolve_sender_info(
        self, sender_info: dict[str, Any] | None, sender_stage_id: str | int | None = None
    ) -> dict[str, Any] | None:
        if not sender_info:
            return None
        ...
        for key in dict.fromkeys(preferred_keys):
            info = next((value for stage, value in sender_info.items() if stage == key), None)
