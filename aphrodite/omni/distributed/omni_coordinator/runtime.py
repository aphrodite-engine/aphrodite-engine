...
(matching Sonar's DPCoordinator pattern). Physical isolation prevents GIL
...
from aphrodite.omni.distributed.omni_coordinator.omni_coordinator import OmniCoordinator
...
def _get_coordinator_mp_context() -> multiprocessing.context.ForkContext | multiprocessing.context.SpawnContext:
    """Return the multiprocessing context used for OmniCoordinator.

    For the current sonar-omni startup path, ``spawn`` is too expensive: the
...
def _shutdown_proc(proc: multiprocessing.process.BaseProcess) -> None:
...
class OmniCoordinatorRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self._proc: multiprocessing.process.BaseProcess = ctx.Process(
