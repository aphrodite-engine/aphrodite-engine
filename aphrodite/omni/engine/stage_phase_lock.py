...
When ``AphroditeOmniOrchestratorConfig.parallel_stage_init`` is enabled, several
...
def resolve_driver_device_ids(aphrodite_config, local_dp_rank: int = 0) -> list[int]:
    """Physical GPU ids this engine-core driver owns, as a sorted list.

    A driver (one local Sonar DP rank) owns a contiguous slice of the visible
...
class DevicePhaseLock:
    ...
    def from_child(
        ...
        *,
        ...
        device_ids = resolve_driver_device_ids(aphrodite_config, local_dp_rank)
...
def wrap_executor_with_phase_locks(executor_class, locker: DevicePhaseLock):
    ...
    * ``compile_or_warm_up_model`` (kernel warmup + CUDA-graph capture) under
    ...
    into ``Executor.initialize_from_config``, but since Sonar v0.20.0 (aphrodite#39240)
