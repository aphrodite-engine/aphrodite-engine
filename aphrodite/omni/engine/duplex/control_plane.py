...
from aphrodite.omni.engine.duplex.lease import DuplexLeaseActivity, DuplexLeaseConfig
...
class DuplexControlPlane:
    ...
    async def reap_expired(self, now: float | None = None) -> int:
        ...
        for expiration_key, item in list(self._pending_expirations.items()):
            try:
            ...
            self._pending_expirations.pop(expiration_key, None)
            completed += 1
        return completed
