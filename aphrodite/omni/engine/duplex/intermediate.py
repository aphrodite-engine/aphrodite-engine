...
def set_ref_audio(buffer: DuplexIntermediateBuffer, waveform: object, sample_rate_hz: int) -> None:
    _mapping_field(buffer.setdefault("codes", {}), "codes")["ref"] = waveform
    _mapping_field(buffer.setdefault("meta", {}), "meta")["ref_audio_sr"] = int(sample_rate_hz)


def _mapping_field(value: object, key: str) -> dict[str, object]:
    if not isinstance(value, dict):
        raise TypeError(f"Duplex intermediate field {key!r} must be a dict")
    return value
...
def set_tts_handoff(buffer: DuplexIntermediateBuffer, token_ids: object | None, hidden_states: object | None) -> None:
    ...
    if token_ids is not None:
        _mapping_field(buffer.setdefault("ids", {}), "ids")["tts"] = token_ids
    if hidden_states is not None:
        _mapping_field(buffer.setdefault("hidden_states", {}), "hidden_states")["tts"] = hidden_states
