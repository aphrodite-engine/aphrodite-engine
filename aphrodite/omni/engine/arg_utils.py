...
from aphrodite.omni.config import OmniModelConfig
from aphrodite.omni.outputs.output_modality import OutputModality
from aphrodite.omni.platforms import current_omni_platform
from aphrodite.omni.plugins import load_omni_general_plugins
from aphrodite.omni.worker.omni_connector_validation import validate_worker_omni_connector
...
def _register_omni_hf_configs() -> None:
    try:
        ...
        from aphrodite.omni.model_executor.models.indextts2.configuration_indextts2 import (
            IndexTTS2Config,
            IndexTTS25Config,
        )
        from aphrodite.omni.model_executor.models.ming_tts.config_ming_tts import (
            MingDenseConfig,
            MingMoeConfig,
        )
        from aphrodite.omni.model_executor.models.moss_tts.configuration_moss_tts import (
            MossTTSLocalConfig,
            MossTTSRealtimeConfig,
        )
        from aphrodite.omni.model_executor.models.nemotron_voicechat.configuration_nemotron_voicechat import (
            NemotronVoiceChatConfig,
        )
        from aphrodite.omni.model_executor.models.personaplex.configuration_personaplex import (
            PersonaPlexConfig,
        )
        from aphrodite.omni.model_executor.models.qwen3_tts.configuration_qwen3_tts import (
            Qwen3TTSConfig,
        )
        from aphrodite.omni.transformers_utils.configs.cosyvoice3 import CosyVoice3Config
        from aphrodite.omni.transformers_utils.configs.glm_tts import GLMTTSConfig
        from aphrodite.omni.transformers_utils.configs.omnivoice import OmniVoiceConfig
        from aphrodite.omni.transformers_utils.configs.voxcpm2 import VoxCPM2Config
    ...
    # Register with both transformers AutoConfig and Sonar's config registry
    # so models with empty/missing config.json (e.g. CosyVoice3) can be
    # resolved when model_type is injected via hf_overrides.
    try:
        from aphrodite.transformers_utils.config import _CONFIG_REGISTRY
    except ImportError:
    ...
    for model_type, config_cls in [
        ...
        try:
            ...
            pass
        if config_registry is not None and model_type not in config_registry:
            config_registry[model_type] = config_cls


def register_omni_models_to_aphrodite():
    from aphrodite.model_executor.models import ModelRegistry
    from aphrodite.omni.model_executor.models.registry import _OMNI_MODELS

    _register_omni_hf_configs()

    # Unconditionally (re)register every omni arch into the upstream global
    # ModelRegistry: Sonar 0.27 added some omni archs (e.g.
    ...
    # overwrites those entries with the sonar-omni classes, which is the
    ...
    for arch, (mod_folder, mod_relname, cls_name) in _OMNI_MODELS.items():
        ModelRegistry.register_model(
            arch, f"aphrodite.omni.model_executor.models.{mod_folder}.{mod_relname}:{cls_name}"
        )

    # Register omni-specific reasoning parsers (e.g., step_audio).
    import aphrodite.omni.reasoning  # noqa: F401
...
class OmniEngineArgs(EngineArgs):
    ...
    def create_model_config(self) -> OmniModelConfig:
        ...
        # If model_arch is specified, inject it into hf_overrides so Sonar can
        ...
        if self.model_arch:
            ...
            # and point hf_config_path to it so Sonar reads model_type from it.
        ...
        # Build the Sonar config first, then use it to create the Omni config.
        try:
        ...
        omni_config = OmniModelConfig.from_aphrodite_model_config(
...
class OmniAsyncEngineArgs(AsyncEngineArgs, OmniEngineArgs):
    ...
    def add_cli_args(parser: FlexibleArgumentParser, async_args_only: bool = False) -> FlexibleArgumentParser:
        ...
        OmniEngineArgs._add_omni_specific_args(parser)
        return parser
...
# ============================================================================
#
# Sonar Omni's CLI flags live in three buckets:
#
#     ┌──────────────────┐    ┌──────────────────┐    ┌──────────────────┐
#     │ OrchestratorArgs │    │  OmniEngineArgs  │    │  (upstream aphrodite) │
#     │                  │    │                  │    │    server/api    │
