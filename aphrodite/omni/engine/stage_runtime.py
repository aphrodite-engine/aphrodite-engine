...
from aphrodite.omni.distributed.omni_connectors.utils.initialization import (
    resolve_omni_kv_config_for_stage,
)
from aphrodite.omni.distributed.omni_coordinator import (
...
from aphrodite.omni.engine.stage_client import StageClient, StagePoolClient
from aphrodite.omni.engine.stage_engine_core_client import StageEngineCoreClientBase
...
from aphrodite.omni.entrypoints.stage_utils import resolve_stage_physical_devices
from aphrodite.omni.entrypoints.utils import inject_omni_kv_config
from aphrodite.omni.outputs.output_metadata import FinalOutputModalityType
...
class StageRemoteFactoryContext:
    ...
    stage_cfg: Any
    base_metadata: Any
    aphrodite_config: Any | None = None
...
class StageRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        # (see AphroditeOmniOrchestratorConfig.parallel_stage_init). Default False
    ...
    def _scoped_spawn_device_env(self, physical_devices: str | None) -> Iterator[None]:
        ...
        from aphrodite.omni.engine.stage_engine_startup import scoped_spawn_device_env
    ...
    def _build_logical_stage_init_plans(
        ...
        for stage_idx, stage_cfg in enumerate(self._stage_configs):
            ...
            if base_metadata.stage_type != "diffusion":
                ...
                stage_aphrodite_config, executor_class = build_aphrodite_config(
                    stage_cfg,
                    self._model,
                    stage_connector_spec=stage_connector_spec,
            ...
            for replica_id in range(num_replicas):
                ...
                replicas.append(
                    ReplicaInitPlan(
                        ...
                        stage_aphrodite_config=stage_aphrodite_config,
    ...
    def _reject_unguardable_executors(self, stage_plans: Sequence[LogicalStageInitPlan]) -> None:
        ...
        for plan in stage_plans:
            for replica in plan.replicas:
                ...
                parallel_config = getattr(replica.stage_aphrodite_config, "parallel_config", None)
    ...
    def _run_stage_admission(self, stage_plans: Sequence[LogicalStageInitPlan]) -> None:
        ...
        from aphrodite.omni.engine.stage_admission import (
            ADMISSION_EXEMPT,
            AdmissionExempt,
            StageAdmissionError,
            check_admission,
    ...
    def _initialize_local_llm_replica(
        ...
        """Initialize one local LLM replica using Sonar's launch/attach pattern."""
        resources: StageReplicaResources | None = None
        stage_client = None
        lock_fds: list[int] = []
        try:
            ...
            aphrodite_config = plan.stage_aphrodite_config
            executor_class = plan.executor_class
            if aphrodite_config is None:
                raise RuntimeError(f"LLM stage {plan.metadata.stage_id} is missing aphrodite_config")
    ...
    def _assemble_stage_pools(
        ...
        for plan in stage_plans:
            ...
            if plan.replicas[0].metadata.stage_type != "diffusion":
                stage_aphrodite_config = plan.replicas[0].stage_aphrodite_config
                if stage_aphrodite_config is None:
                    raise RuntimeError(f"Stage {plan.stage_id} is missing aphrodite_config")
                output_processor = build_llm_stage_output_processor(
                    plan,
                    stage_aphrodite_config,
                    log_stats=self._log_stats,
                )
...
class DistStageRuntime(StageRuntime):
    ...
    def _start_omni_master_server(self, stage_plans: Sequence[LogicalStageInitPlan]) -> None:
        ...
        from aphrodite.omni.distributed.omni_coordinator import OmniCoordinatorRuntime
    ...
    def _capture_stage_factory_contexts(
        ...
        for plan in stage_plans:
            ...
            contexts[stage_id] = StageRemoteFactoryContext(
                ...
                aphrodite_config=template.stage_aphrodite_config,
    ...
    def _create_remote_replica_client(
        ...
        if ctx.stage_type == "diffusion":
            from aphrodite.omni.diffusion.stage_diffusion_client import StageDiffusionClient
        ...
        if ctx.aphrodite_config is None:
            raise RuntimeError(f"Remote LLM stage {stage_id} is missing aphrodite_config")
        if ctx.executor_class is None:
            raise RuntimeError(f"Remote LLM stage {stage_id} is missing executor_class")
        aphrodite_config = copy.deepcopy(ctx.aphrodite_config)
        aphrodite_config.parallel_config.data_parallel_size_local = 0
        resources = None
        try:
            ...
            llm_client = StageEngineCoreClientBase.make_async_mp_client(
                aphrodite_config=aphrodite_config,
