...
"""
Stage initialization helpers for Sonar Omni multi-stage runtime.
...
from aphrodite.omni.config.stage_config import StageType
from aphrodite.omni.diffusion.data import OmniDiffusionConfig
from aphrodite.omni.engine.arg_utils import OmniEngineArgs
from aphrodite.omni.entrypoints.stage_utils import _to_dict, set_stage_devices
from aphrodite.omni.entrypoints.utils import filter_dataclass_kwargs
from aphrodite.omni.inputs.data import OmniDiffusionSamplingParams, OmniSamplingParams
from aphrodite.omni.inputs.processor import OmniInputProcessor
from aphrodite.omni.outputs.output_processor import MultimodalOutputProcessor
from aphrodite.omni.platforms import current_omni_platform
from aphrodite.omni.quantization.inc_config import OmniINCConfig
...
class ReplicaInitPlan:
    ...
    stage_aphrodite_config: Any | None = None
...
# treating it as complete strips the Hub fallback Sonar would need later.
...
def _incomplete_stage_subdirs(
    ...
    Sonar cannot fetch missing weights for. ``weight_subdirs`` names the entries
...
def _resolve_model_tokenizer_paths(model: str, engine_args: dict[str, Any]) -> str:
    ...
    if is_runai_obj_uri(model):
        ...
        # ModelConfig, so the joins below are resolved by Sonar's streamer
...
class StageMetadata:
    ...
    default_sampling_params: OmniSamplingParams | PoolingParams
...
def extract_legacy_stage_metadata(stage_config: Any) -> StageMetadata:
    ...
    # A pooling stage is an LLM stage run with runner="pooling" (Sonar's
...
def extract_stage_metadata_from_omni_stage_config(
    stage_config: BaseAphroditeOmniStageConfig,
) -> StageMetadata:
...
def prepare_engine_environment() -> None:
    ...
    if os.environ.get("APHRODITE_WORKER_MULTIPROC_METHOD") != "spawn":
        ...
        logger.info("[stage_init] Set APHRODITE_WORKER_MULTIPROC_METHOD=spawn")
    try:
...
def get_stage_devices_per_replica(stage_cfg: Any, engine_args: Any | None = None) -> int:
    ...
    if getattr(stage_cfg, "stage_type", "llm") == "diffusion":
        ...
        try:
            from aphrodite.omni.diffusion.data import DiffusionParallelConfig
...
def _project_omni_stage_engine_args(
    ...
    engine_args: dict[str, Any] = {}
    is_diffusion = isinstance(stage_config, AphroditeOmniDiffusionStageConfig)
    if is_diffusion:
        diffusion_stage = cast(AphroditeOmniDiffusionStageConfig, stage_config)
    ...
    if is_diffusion:
        for diffusion_config, field_map in (
            ...
            engine_args.update(_project_omni_config_fields(diffusion_config, field_map=field_map))
    else:
        for upstream_config, field_map in (
            ...
            engine_args.update(_project_upstream_config_fields(upstream_config, field_map))
...
def _finalize_engine_args_dict(
    ...
    *,
    ...
    if audex_stage == "audex_xcodec":
        ...
        from aphrodite.omni.model_executor.models.audex.checkpoint import (
        ...
        # extras.
        from aphrodite.omni.model_executor.models.audex.checkpoint import ensure_audex_snapshot
    ...
    if engine_args_dict.get("model_stage") in ("audex_thinker", "audex_tta_thinker"):
        ...
        from aphrodite.omni.model_executor.models.audex.checkpoint import ensure_audiogen_weights
    ...
    if is_diffusion:
        from aphrodite.omni.diffusion.data import parse_attention_config
    ...
    if engine_args_dict.get("worker_type") == "generation":
        ...
        # prefix caching and can expose hybrid KV-cache layouts that Sonar's
...
def _check_stage_device_layout(stage_config: Any, engine_args_dict: dict[str, Any]) -> None:
    ...
    """
    from aphrodite.omni.config.composable_parallel import StrategyApplyError, check_device_layout

    runtime = getattr(stage_config, "runtime", None)
    devices = _get_attr_or_item(runtime, "devices", None) if runtime is not None else None
    if devices is None:
        # No explicit placement -> Sonar assigns devices itself; nothing to check.
        return
...
def build_aphrodite_config(
    ...
    """Build engine args, then create AphroditeConfig and executor_class.

    Returns:
        (aphrodite_config, executor_class)
    """
    ...
    # larger than HF max_position_embeddings by design. Sonar's validator
    ...
    if filtered_engine_args_dict.get("max_model_len") is not None and not os.environ.get(
        ...
        os.environ["APHRODITE_ALLOW_LONG_MAX_MODEL_LEN"] = "1"
        logger.debug(
            "Auto-set APHRODITE_ALLOW_LONG_MAX_MODEL_LEN=1 for stage %s (max_model_len=%s).",
            stage_config.stage_id,
            filtered_engine_args_dict["max_model_len"],
        )

    aphrodite_config = omni_engine_args.create_engine_config(
    ...
    upgraded = OmniINCConfig.maybe_upgrade(aphrodite_config.quant_config)
    if upgraded is not aphrodite_config.quant_config:
        aphrodite_config = replace(aphrodite_config, quant_config=upgraded)

    custom_voice_dir = engine_args_dict.get("custom_voice_dir")
    if custom_voice_dir:
        aphrodite_config.model_config.hf_config.custom_voice_dir = custom_voice_dir

    return aphrodite_config, executor_class


def build_llm_stage_output_processor(
    plan: LogicalStageInitPlan,
    stage_aphrodite_config: Any,
    ...
    if stage_aphrodite_config.model_config.skip_tokenizer_init:
        tokenizer = None
    else:
        tokenizer = cached_tokenizer_from_config(
            model_config=stage_aphrodite_config.model_config,
        )
...
def _build_token_only_renderer(stage_aphrodite_config: Any) -> BaseRenderer:
    return _TokenOnlyRenderer(stage_aphrodite_config, tokenizer=None)


def build_stage0_input_processor(stage_aphrodite_config: Any) -> OmniInputProcessor:
    """Build the shared stage-0 input processor."""

    patch_generation_config_if_needed(stage_aphrodite_config.model_config)
    if bool(getattr(stage_aphrodite_config.model_config, "skip_tokenizer_init", False)):
        input_processor = OmniInputProcessor(
            aphrodite_config=stage_aphrodite_config,
            renderer=_build_token_only_renderer(stage_aphrodite_config),
        )
    else:
        input_processor = OmniInputProcessor(aphrodite_config=stage_aphrodite_config)
...
def device_init_lock_path(device_id: int, lock_dir: str = "/tmp") -> str:
    ...
    """
    return os.path.join(lock_dir, f"aphrodite_omni_device_{device_id}_init.lock")
...
def open_device_lock_file(lock_file: str) -> tuple[int, bool]:
    ...
    * the mode passed to ``os.open`` is filtered by the creator's ``umask``, so
    ...
    tmp_fd, tmp_path = tempfile.mkstemp(dir=lock_dir, prefix=".aphrodite_omni_device_lock_")
    published = False
    try:
...
def load_omni_transfer_config_for_model(model: str, config_path: str | None) -> Any:
    ...
    config parser.
    """
    from aphrodite.omni.distributed.omni_connectors import load_omni_transfer_config
...
def get_stage_connector_spec(
    ...
    from aphrodite.omni.distributed.omni_connectors import get_stage_connector_config
...
def initialize_diffusion_stage(
    ...
    """
    from aphrodite.omni.diffusion.stage_diffusion_client import create_diffusion_client
...
def maybe_apply_cfg_scheduler_patches(aphrodite_config: Any) -> None:
    ...
    """
    model_config = getattr(aphrodite_config, "model_config", None)
    if model_config is None or not _stage_declares_cfg_pairs(model_config):
        return

    from aphrodite.omni.model_executor.models.common.cfg_pairing import apply_cfg_patches
