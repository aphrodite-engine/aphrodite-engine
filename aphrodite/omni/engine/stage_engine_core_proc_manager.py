...
This is a drop-in replacement for Sonar's :class:`CoreEngineProcManager` that
...
from aphrodite.omni.engine.stage_engine_core_proc import StageEngineCoreProc
...
# ``set_device_control_env_var`` was removed from upstream aphrodite.
# It was only required for non-CUDA DP; set to None so the existing
# guard at the call-site (aphrodite_config is not None and ... is not None)
    ...
    * ``omni_coordinator_address`` (if provided) and ``omni_stage_id`` are
    ...
    def __init__(
        ...
        *,
        ...
        # Intra-replica Sonar DP mesh (i.e. ``data_parallel_size`` ranks sharing
        ...
        # a Sonar DP group across replicas.
        has_intra_replica_dp = aphrodite_config.parallel_config.data_parallel_size > 1
        ...
        try:
            for proc, local_dp_rank in zip(self.processes, local_dp_ranks):
                ...
                if (
                    ...
                    and (not current_platform.is_cuda_alike() or aphrodite_config.parallel_config.use_ray)
                ):
