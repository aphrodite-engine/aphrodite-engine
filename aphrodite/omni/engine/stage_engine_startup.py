...
from aphrodite.omni.distributed.omni_connectors.utils import initialization
from aphrodite.omni.engine import stage_init_utils
...
from aphrodite.utils.network_utils import get_open_ports_list, zmq_socket_ctx
...
def _scoped_replica_compile_cache_env(stage_id: int | None, replica_id: int | None) -> Iterator[None]:
    """Give additional replicas an isolated APHRODITE_CACHE_ROOT at spawn time.

    Replicas of the same stage compile the same model concurrently; Sonar
    ...
    base = os.environ.get("APHRODITE_CACHE_ROOT") or os.path.expanduser("~/.cache/aphrodite")
    ...
    with _COMPILE_CACHE_ENV_LOCK:
        previous = os.environ.get("APHRODITE_CACHE_ROOT")
        os.environ["APHRODITE_CACHE_ROOT"] = isolated
        try:
            yield
        finally:
            if previous is None:
                os.environ.pop("APHRODITE_CACHE_ROOT", None)
            else:
                os.environ["APHRODITE_CACHE_ROOT"] = previous
...
def _launch_omni_core_engines(
    ...
    *,
    ...
    run_coordinator = aphrodite_config.needs_dp_coordinator and dp_rank == 0
    ...
    try:
        with zmq_socket_ctx(handshake_bind_address, zmq.ROUTER, bind=True) as handshake_socket:
            if omni_coordinator_address is not None:
                ...
                from aphrodite.omni.engine.stage_engine_core_proc_manager import StageEngineCoreProcManager
            ...
            wait_for_engine_startup(
                ...
                engines_to_handshake,
                parallel_config,
                parallel_config.data_parallel_size > 1 and aphrodite_config.model_config.is_moe,
                aphrodite_config.cache_config,
                CoreEngineLaunch(
...
def launch_stage_replica(
    ...
    *,
    ...
    from aphrodite.omni.engine.stage_engine_core_proc_manager import StageEngineCoreProcManager
    from aphrodite.utils.network_utils import get_open_zmq_ipc_path
...
def launch_headless_llm_replicas(
    *,
    ...
    dp_rank = parallel_config.data_parallel_rank if parallel_config.data_parallel_rank is not None else 0
    coordinator = None
    if aphrodite_config.needs_dp_coordinator and dp_rank == 0:
        coordinator = DPCoordinator(
            parallel_config,
            enable_wave_coordination=aphrodite_config.model_config.is_moe,
        )
    ...
    logger.info(
        "[Headless] Launching %d omni replica(s) (Sonar dp_size_local=%d each) for stage %d "
...
def launch_headless_diffusion_replica(
    *,
    ...
    from aphrodite.omni.diffusion import stage_diffusion_proc
    ...
    *,
    ...
    if omni_conn_cfg:
        if omni_from is None or omni_to is None:
            raise ValueError("KV connector configuration requires source and destination stages")
