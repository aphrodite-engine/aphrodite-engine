...
"""
Stage Engine Core Client for Sonar Omni multi-stage runtime.

Directly inherits from Sonar's AsyncMPClient to reuse EngineCore architecture.
"""
...
import aphrodite.v1.engine as _aphrodite_engine_module
import aphrodite.v1.engine.core_client as _aphrodite_core_client_module
from aphrodite.logger import init_logger
from aphrodite.omni.distributed.omni_connectors.utils.config import (
...
from aphrodite.omni.distributed.omni_connectors.utils.kv_utils import kv_zmq_port
from aphrodite.omni.engine import OmniEngineCoreOutput, OmniEngineCoreOutputs
from aphrodite.omni.engine.stage_client import StageClientBase
from aphrodite.omni.engine.stage_init_utils import StageMetadata
...
from aphrodite.v1.engine.core_client import AsyncMPClient, DPLBAsyncMPClient
from aphrodite.v1.engine.exceptions import EngineDeadError

if TYPE_CHECKING:
    from aphrodite.omni.inputs.data import OmniTokensPrompt
    from aphrodite.v1.engine import EngineCoreOutput
...
class StageEngineCoreClientBase(StageClientBase, AsyncMPClient):
    ...
    Fully reuses the underlying Sonar async MP client ``__init__`` for:
    - ZMQ setup, sockets
    - outputs_queue, output_queue_task
    - All utility methods (get_output_async, abort_requests_async, etc.)

    The stage engine subprocesses are owned by Sonar-style engine managers
    ...
    def __init__(
        ...
        *,
        ...
        Sonar's background resources, not by this client directly.
        """
        ...
        if metadata is not None:
            ...
            self.stage_type: str = metadata.stage_type
            ...
            self.engine_input_source: Sequence[int] = metadata.engine_input_source
            self.final_output = metadata.final_output
            if metadata.final_output_type not in (None, "text", "image", "audio", "video"):
                raise ValueError(f"Unsupported final output modality: {metadata.final_output_type!r}")
            self.final_output_type = cast(FinalOutputModalityType | None, metadata.final_output_type)
        ...
        self._omni_kv_config = getattr(getattr(aphrodite_config, "model_config", None), "omni_kv_config", None)
        self._stage_hf_config = getattr(getattr(aphrodite_config, "model_config", None), "hf_config", None)
        ...
        setattr(_aphrodite_engine_module, "EngineCoreOutput", OmniEngineCoreOutput)
        setattr(_aphrodite_engine_module, "EngineCoreOutputs", OmniEngineCoreOutputs)
        setattr(_aphrodite_core_client_module, "EngineCoreOutputs", OmniEngineCoreOutputs)
        logger.debug(
            "[StageEngineCoreClient] Patched EngineCoreOutputs -> %s",
            _aphrodite_core_client_module.EngineCoreOutputs,
        )
    ...
    def _call_custom_process_input(
        ...
        if target_model_config is not None:
            ...
            extra_kwargs["target_model_config"] = self.aphrodite_config.model_config
...
class StageEngineCoreClient(StageEngineCoreClientBase, AsyncMPClient):
    """Stage async client backed by Sonar's ``AsyncMPClient``."""


class DPLBStageEngineCoreClient(StageEngineCoreClientBase, DPLBAsyncMPClient):
    """Stage async client backed by Sonar's ``DPLBAsyncMPClient``."""
