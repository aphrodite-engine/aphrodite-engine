...
"""
Orchestrator for Sonar Omni multi-stage runtime.
...
from aphrodite.omni.config.stage_config import DuplexSessionRuntimeConfig
from aphrodite.omni.distributed.omni_connectors.utils.config import stage_receives_chunks
from aphrodite.omni.engine import OmniEngineCoreRequest
from aphrodite.omni.engine.cfg_companion_tracker import CfgCompanionTracker
from aphrodite.omni.engine.duplex.contracts import (
...
from aphrodite.omni.engine.duplex.control_plane import DuplexControlPlane
from aphrodite.omni.engine.duplex.lease import DuplexLeaseConfig
from aphrodite.omni.engine.duplex.messages import DuplexFence
from aphrodite.omni.engine.duplex.session import (
    DuplexSessionRuntimeManager,
    DuplexSessionRuntimeState,
)
from aphrodite.omni.engine.membership_controller import MembershipController
...
from aphrodite.omni.engine.orchestrator_monitor import create_orch_monitor, replica_key
from aphrodite.omni.engine.serialization import serialize_additional_information
from aphrodite.omni.engine.stage_client import StagePoolLLMClient
from aphrodite.omni.engine.stage_pool import StagePool, StageUnavailableError
from aphrodite.omni.errors import DEFAULT_CLIENT_ERROR_TYPE, OmniClientError
from aphrodite.omni.metrics import definitions as metric_defs
from aphrodite.omni.metrics.prometheus import OmniRequestCounter
from aphrodite.omni.metrics.stat_logger import OmniPrometheusStatLogger
from aphrodite.omni.metrics.utils import DIFFUSION_METRICS_ONLY_REQUEST_ID
from aphrodite.omni.outputs import OmniRequestOutput
from aphrodite.omni.outputs.duplex import attach_duplex_output_decision
...
# APHRODITE_OMNI_EVENT_DRIVEN_ORCH=1 switches the orchestration loop (and the
...
# replica awaits `client.get_output_async()` directly — the same pattern Sonar's
...
_EVENT_DRIVEN_ORCH_ENV = "APHRODITE_OMNI_EVENT_DRIVEN_ORCH"
...
class StreamingSegmentState:
    ...
    finished: bool = False
    # Tokens from the current raw segment boundary. The Sonar output processor
...
class _OrchestratorDuplexStagePort:
    ...
    def sampling_defaults(self) -> tuple[object, ...]:
        ...
        for pool in self._stage_pools:
            ...
            if client is None:
                raise StageUnavailableError(f"stage {pool.stage_id} has no live client")
    ...
    async def submit(self, submission: DuplexStageSubmission) -> DuplexStageSubmissionResult:
        ...
        if not isinstance(params, (SamplingParams, PoolingParams)):
            raise TypeError("Duplex stage requires sampling or pooling parameters")
        request = build_engine_core_request_from_tokens(
            request_id=context.request_id,
            prompt=dict(submission.prompt),
            params=params,
            model_config=self._stage_pools[context.stage_id].stage_aphrodite_config.model_config,
            resumable=True,
        )
...
class Orchestrator:
    ...
    def _init_metrics_state(
        ...
        that exposes ~37 upstream ``aphrodite:*`` families with per-(stage,
        ...
        tests with a minimal ``aphrodite_config`` still pass.

        ``log_stats=False`` short-circuits the wrap entirely so the
        ~65 upstream ``aphrodite:*`` families are not registered in the
        ...
        aphrodite_config_for_stats = next(
            (p.stage_aphrodite_config for p in stage_pools if p.stage_aphrodite_config is not None),
            None,
        )
        if aphrodite_config_for_stats is None:
            self._stat_logger = None
            return
        try:
            self._stat_logger = OmniPrometheusStatLogger(
                aphrodite_config=aphrodite_config_for_stats,
                stage_replica_map=stage_replica_map,
            )
        except Exception:
            # Minimal aphrodite_config in unit-test contexts can lack fields the
    ...
    async def _orchestration_loop_event_driven(self) -> None:
        ...
        Selected by ``APHRODITE_OMNI_EVENT_DRIVEN_ORCH=1``. One reader task per
        available LLM stage replica awaits ``client.get_output_async()``
        directly — the same pattern Sonar's own ``AsyncLLM`` output handler uses
    ...
    async def _apply_raw_terminal_stage_finish(
        ...
        ``is_segment_finished=False``, but Sonar's output processor may remove the
    ...
    def _maybe_clone_diffusion_params_for_cfg(self, request_id: str, params: Any) -> Any:
        ...
        from aphrodite.omni.inputs.data import OmniDiffusionSamplingParams
    ...
    def _stage_receives_async_chunks(self, stage_id: int) -> bool:
        ...
        pool = self.stage_pools[stage_id]
        model_config = getattr(pool.stage_aphrodite_config, "model_config", None)
    ...
    def _get_stage_input_processor(self, stage_id: int) -> Any:
        ...
        if processor is None:
            ...
            processor = build_stage0_input_processor(self.stage_pools[stage_id].stage_aphrodite_config)
        ...
        *,
        ...
        if self._next_stage_input_is_tokens(next_input):
            request = build_engine_core_request_from_tokens(
                ...
                prompt=next_input,
                params=params,
                model_config=next_pool.stage_aphrodite_config.model_config,
                mm_features=mm_features,
                resumable=resumable,
            )
            request.external_req_id = request.request_id
        ...
        # A pooling stage is driven by PoolingParams; Sonar validates them against
        # the stage's supported tasks, so advertise the model's pooling tasks (or
        # the configured task) instead of generate.
        if isinstance(params, PoolingParams):
            model_cfg = next_pool.stage_aphrodite_config.model_config
    ...
    async def _forward_to_next_stage_unguarded(
        ...
        *,
        ...
        for next_input in next_inputs:
            ...
            model_stage = getattr(getattr(next_pool.stage_aphrodite_config, "model_config", None), "model_stage", None)
    ...
    async def _prewarm_async_chunk_stages(
        ...
        for next_stage_id in range(1, req_state.final_stage_id + 1):
            ...
            if next_pool.stage_type == "diffusion":
                ...
                from aphrodite.omni.distributed.omni_connectors.adapter import compute_talker_prompt_ids_length
                ...
                # mismatch).
                prewarm_len = getattr(
                    getattr(next_pool.stage_aphrodite_config, "model_config", None),
                    "hf_config",
                    None,
                )
