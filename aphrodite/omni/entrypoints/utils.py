...
from aphrodite.omni.entrypoints.stage_utils import _to_dict
...
def coerce_param_message_types(params: list[OmniSamplingParams], is_streaming: bool):
    ...
    """
    # Coerce Sonar's default output kinds as needed to handle streaming
...
class PureDiffusionLauncherAdapter:
    """Sonar launcher compatibility shim for pure-diffusion mode.

    The upstream launcher's shutdown path reads
    ``app.state.engine_client.aphrodite_config.shutdown_timeout``
    (aphrodite/entrypoints/launcher.py), but ``AsyncOmni.aphrodite_config`` returns
    ...
    This adapter only overrides the ``aphrodite_config`` property with a minimal
    ...
    (``get_aphrodite_config()`` still returns ``None``) is unaffected.
    """
