...
from aphrodite.omni.engine.duplex.control_client import DuplexControlRequestError
from aphrodite.omni.engine.duplex.messages import DuplexFence
from aphrodite.omni.engine.duplex.runtime import duplex_data_plane_request_info
...
class NativeRuntimeBridgeMixin:
    ...
    @property
    def _native_handler(self) -> OmniDuplexSessionHandler:
        return cast("OmniDuplexSessionHandler", self)
    ...
    async def _open_runtime_session(self, session: DuplexSession, send_json) -> dict[str, object] | bool:
        ...
        if not self._native_handler._uses_serving_runtime_adapter(session.config):
            return True
        ...
        if contract_error is not None:
            ...
            return False
        open_session = getattr(self._native_handler._chat_service.engine_client, "open_duplex_session_async", None)
    ...
    async def _append_runtime_input(
        ...
        *,
        ...
        if not session.capabilities.supports_input_append:
            return True, False
        append_input = getattr(self._native_handler._chat_service.engine_client, "append_duplex_input_async", None)
        ...
        try:
            ...
            if self._callable_accepts_keyword(append_input, "collect_outputs"):
                append_kwargs["collect_outputs"] = bool(
                    getattr(self._native_handler._serving_runtime_adapter, "collect_outputs_on_append", False)
                )
        ...
        if request_id is not None:
            self._native_handler._require_serving_runtime_adapter().data_plane.begin_request(request_id)
    ...
    def _native_runtime_contract_error(self, session: DuplexSession) -> str | None:
        if session.capabilities.implementation_level != "model_native_duplex":
            return None
        engine_client = self._native_handler._chat_service.engine_client
        invalid: list[str] = []
    ...
    async def _start_native_data_plane_stream_task(
        ...
        *,
        ...
        native = self._native_handler._runtime_session_state(session)
        old_task = native.data_plane_task
        if old_task is not None and not old_task.done():
            if self._native_handler._session_auto_responds(session):
        ...
        async def _run() -> None:
            ...
            try:
                ...
                if close_reason is None and not self._native_handler._session_auto_responds(session):
                    ...
                    self._native_handler._require_serving_runtime_adapter().data_plane.close_stream(request_id)
    ...
    def _native_silence_unit_payload(self) -> dict[str, object]:
        samples = int(getattr(self._native_handler._serving_runtime_adapter, "silence_continuation_samples", 16000))
        audio = (
    ...
    async def _maybe_continue_native_response(
        ...
        *,
        ...
        if expected_epoch is not None and session.epoch != expected_epoch:
            return
        auto_response = self._native_handler._session_auto_responds(session)
    ...
    async def _signal_runtime_session(
        ...
        *,
        ...
        if not self._native_handler._uses_serving_runtime_adapter(session.config):
            return True
        signal_turn = getattr(self._native_handler._chat_service.engine_client, "signal_duplex_turn_async", None)
    ...
    async def _close_runtime_session(self, session: DuplexSession, *, reason: str, send_json=None) -> bool:
        if not self._native_handler._uses_serving_runtime_adapter(session.config):
            return True
        close_session = getattr(self._native_handler._chat_service.engine_client, "close_duplex_session_async", None)
    ...
    async def _send_native_duplex_events(
        ...
        *,
        ...
        request_id, _ = self._data_plane_request_info(result)
        if self._native_handler._require_serving_runtime_adapter().data_plane.is_terminal(request_id):
            return None, False
        if request_id is not None and session.active_request_id is None:
            session.bind_request(request_id)
        context = self._runtime_data_plane_context(session)
        for native_result in self._native_handler._require_serving_runtime_adapter().data_plane.project(
    ...
    async def _drain_native_data_plane_stream(
        ...
        *,
        ...
        collect_outputs = getattr(
            self._native_handler._chat_service.engine_client,
        ...
        while close_reason is None:
            ...
            if (
                ...
                and not self._native_handler._session_auto_responds(session)
            ):
    ...
    def _runtime_data_plane_context(self, session: DuplexSession) -> object:
        ...
        return self._native_handler._require_serving_runtime_adapter().data_plane_context(
            epoch=session.epoch,
            turn_id=session.turn_id,
            active_response_turn_id=session.active_response_turn_id,
            active_response_id=session.active_response_id,
            auto_responds=self._native_handler._session_auto_responds(session),
    ...
    async def _send_one_native_duplex_event(
        ...
        *,
        ...
        if isinstance(
            data_plane_request_id, str
        ) and self._native_handler._require_serving_runtime_adapter().data_plane.is_terminal(data_plane_request_id):
            return close_reason, emitted_response
        active_request_matches = session.active_request_id == data_plane_request_id or (
            self._native_handler._session_auto_responds(session) and session.active_request_id is None
        )
        ...
        if is_listen is True:
            ...
            if isinstance(data_plane_request_id, str) and not auto_response:
                self._native_handler._require_serving_runtime_adapter().data_plane.mark_terminal(data_plane_request_id)
            emitted_response = True
            payload = {
                "type": "response.listen",
                "session_id": session.session_id,
            ...
            if native_result.get("abort_data_plane_request") is True and isinstance(data_plane_request_id, str):
                await self._native_handler._abort_request_background(
                    session,
            ...
            if response_id is not None:
                if not self._native_handler._session_auto_responds(
                    session
                ) and self._native_response_continuations_remaining(session, response_id):
                    ...
                    return close_reason, emitted_response
                if auto_response:
                    self._native_handler._runtime_session_state(session).clear_continuation()
        ...
        if not has_text and not has_audio and not end_of_turn:
            ...
            if (
                tts_segment_ended
                and self._native_handler._session_auto_responds(session)
        ...
        if end_of_turn and not has_text and not has_audio and session.active_response_id is None:
            if isinstance(data_plane_request_id, str):
                if (
                    not self._native_handler._session_auto_responds(session)
                    and data_plane_request_id == session.active_request_id
                ):
                    session.clear_request()
                if not self._native_handler._session_auto_responds(session):
                    self._native_handler._require_serving_runtime_adapter().data_plane.mark_terminal(
                        data_plane_request_id
                    )
        ...
        if response_id is None:
            ...
            await send_json(
                self._native_handler._response_created_payload(
                    session,
                    response_id,
                    epoch=session.epoch,
                )
            )
        stage_metrics = native_result.get("stage_metrics")
        ...
        if end_of_turn:
            data_plane_request_id = native_result.get("data_plane_request_id")
            if isinstance(data_plane_request_id, str) and not self._native_handler._session_auto_responds(session):
                self._native_handler._require_serving_runtime_adapter().data_plane.close_stream(data_plane_request_id)
            if isinstance(data_plane_request_id, str):
                if (
                    not self._native_handler._session_auto_responds(session)
                    and data_plane_request_id == session.active_request_id
            ...
            should_commit = self._native_handler._should_commit_response_to_history(session, response_id)
            committed_message = session.end_response(
                commit_text=should_commit,
                preserve_request=self._native_handler._session_auto_responds(session),
            )
            model_turn_id = coerce_int(native_result.get("model_turn_id"))
            if model_turn_id is not None:
                session.complete_model_turn(model_turn_id)
    ...
    def _cleanup_duplex_session_state(self, session: DuplexSession) -> None:
        session_id = session.session_id
        pipeline = self._native_handler._server_vad_pipelines.pop(session_id, None)
        if pipeline is not None:
            pipeline.reset()
            self._native_handler._realtime_vad_metrics.session_finished()
        adapter = self._native_handler._serving_runtime_adapter
        if adapter is None:
            self._native_handler._serving_session_states.pop(session_id, None)
        else:
    ...
    def _encode_native_data_plane_audio(
        ...
        try:
            ...
            from aphrodite.omni.entrypoints.openai.protocol.audio import CreateAudio
            ...
            audio_response = self._native_handler._chat_service.create_audio(
                CreateAudio(
    ...
    def _attach_native_runtime_metadata(
        ...
        *,
        ...
        if metadata:
            payload["aphrodite.omni"] = metadata
