...
class DuplexSession:
    ...
    def accumulate_response_stage_metrics(
        ...
        interval_fields = (
            ...
            ("aphrodite_itls_ms", "aphrodite_itl_ms"),
        )
        handled_fields = {
            *additive_fields,
            *first_positive_fields,
        ...
        for raw_stage_id, raw_values in stage_metrics.items():
            ...
            for list_name, mean_name in interval_fields:
                ...
                if isinstance(values, list):
                    ...
                    assert isinstance(previous_values, list)
                    combined = list(previous_values)
                    combined.extend(
            ...
            tpot_ms = raw_values.get("aphrodite_tpot_ms")
            ...
            if isinstance(tpot_ms, int | float) and tpot_ms > 0:
                ...
                current["aphrodite_tpot_ms"] = self._response.stage_metric_tpot_weighted_ms[stage_id] / float(
                    self._response.stage_metric_tpot_weight[stage_id]
                )
    ...
    def truncate_history_item(
        ...
        *,
        ...
        if response_snapshot is not None:
            ...
            snapshot_message: dict[str, object] = copy.deepcopy(full_message)
            ...
            if not changed or self._message_text_len(snapshot_message) <= 0:
                self._conversation.pending_truncations_ms[item_id] = max(0, int(audio_end_ms))
                return False
            self._store_history_item_message(item_id, snapshot_message)
            if full_marks:
    ...
    def as_public_dict(self) -> dict[str, object]:
        ...
        for field_name in (
            ...
            field_value = self.config.extra_body.get(f"realtime_{field_name}")
            if isinstance(field_value, dict):
                payload[field_name] = dict(field_value)
        include = self.config.extra_body.get("realtime_include")
    ...
    def signal(
        ...
        if event_type == DuplexTurnEventType.USER_STARTED.value:
            ...
            if not isinstance(raw_played_ms, str | int | float):
                raise ValueError("played_ms must be a number")
