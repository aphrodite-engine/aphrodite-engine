...
from aphrodite.omni.entrypoints.duplex.audio import convert_output_audio
from aphrodite.omni.entrypoints.duplex.realtime_input import RealtimeInputTranslator
from aphrodite.omni.entrypoints.duplex.realtime_state import (
    _RealtimeResponseState,
)


class RealtimeOutputProjector(RealtimeInputTranslator):
    ...
    def _from_duplex_event(self, event: dict[str, Any]) -> list[dict[str, object]]:
        ...
        if event_type == "response.done":
            ...
            status = event_status if isinstance(event_status, str) else "completed"
        ...
        if event_type == "input.committed":
            ...
            # Acknowledgements carry their own identity; untracked silence
    ...
    def _realtime_audio_delta_events(
        ...
        for key in ("session_id", "epoch", "model_speak", "end_of_turn", "playback", "aphrodite.omni"):
            if key in event:
    ...
    def _rate_limits_updated_event() -> dict[str, object]:
        # Sonar Omni does not yet expose a Realtime-specific quota budget. Emit
        # the terminal event with an empty list so clients that sequence on
        # rate_limits.updated do not need a private Sonar branch.
