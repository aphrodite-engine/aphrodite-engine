...
from aphrodite.omni.engine.duplex.messages import DuplexFence, DuplexSessionLifecycleMessage
from aphrodite.omni.engine.duplex.runtime import duplex_resource_request_id
from aphrodite.omni.entrypoints.duplex.capability import (
    should_enable_duplex_endpoint,
)
from aphrodite.omni.entrypoints.duplex.chat_fallback import (
...
from aphrodite.omni.entrypoints.duplex.runtime_bridge import (
    NativeRuntimeBridgeMixin,
)
from aphrodite.omni.entrypoints.duplex.server_vad import (
...
from aphrodite.omni.entrypoints.duplex.session_runner import (
...
from aphrodite.omni.metrics.realtime import RealtimeVADMetrics
...
class OmniDuplexSessionHandler(
    ...
    def _apply_session_update(
        ...
        if isinstance(temperature, int | float):
            session.config.temperature = float(temperature)
        ...
        overlap_short_ack_ms = payload.get("overlap_short_ack_ms")
        if isinstance(overlap_short_ack_ms, int | float):
            session.config.overlap_short_ack_ms = max(0, int(overlap_short_ack_ms))
        overlap_barge_in_ms = payload.get("overlap_barge_in_ms")
        if isinstance(overlap_barge_in_ms, int | float):
            session.config.overlap_barge_in_ms = max(0, int(overlap_barge_in_ms))
        overlap_silence_rms = payload.get("overlap_silence_rms")
        if isinstance(overlap_silence_rms, int | float):
            session.config.overlap_silence_rms = max(0.0, float(overlap_silence_rms))
        ...
        if isinstance(extra, dict):
            session.config.extra_body.update(extra)
        ...
        input_audio_noise_reduction = payload.get("input_audio_noise_reduction")
        if isinstance(input_audio_noise_reduction, dict):
            session.config.extra_body["realtime_input_audio_noise_reduction"] = dict(input_audio_noise_reduction)
        ...
        if isinstance(audio, dict):
            session.config.extra_body["realtime_audio"] = dict(audio)
    ...
    async def _handle_playback_ack(self, session: DuplexSession, event: dict[str, object], send_json) -> None:
        ...
        if isinstance(item_id, str) and item_id:
            expected_response_item_id = f"item_{response_id}" if response_id is not None else None
            if (
                expected_response_item_id == item_id
