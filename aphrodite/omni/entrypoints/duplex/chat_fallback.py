...
from aphrodite.omni.entrypoints.duplex.protocol import DuplexSession
...
class ChatFallbackProjectorMixin:
    ...
    @property
    def _chat_handler(self) -> OmniDuplexSessionHandler:
    ...
    async def _run_response(self, session: DuplexSession, send_json) -> None:
        ...
        await send_json(
            self._chat_handler._response_created_payload(
                session,
                response_id,
                epoch=epoch,
        ...
        try:
            ...
            result = await self._chat_handler._chat_service.create_chat_completion(request, raw_request=None)
            ...
            if session.epoch == epoch:
                ...
                should_commit = self._chat_handler._should_commit_response_to_history(session, response_id)
                ...
                if final_stage_metrics:
                    done_payload["aphrodite.omni"] = {"stage_metrics": final_stage_metrics}
    ...
    def _build_chat_request(self, session: DuplexSession, request_id: str) -> ChatCompletionRequest:
        ...
        kwargs: dict[str, Any] = {
            "model": response_config.model or self._chat_handler._chat_service.model_config.model,
            "messages": messages,
            "stream": True,
        }
