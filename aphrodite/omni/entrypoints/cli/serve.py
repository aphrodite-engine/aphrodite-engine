...
"""
Omni serve command for Sonar Omni.
...
from aphrodite.entrypoints.launchers.cli_args import make_arg_parser, validate_parsed_serve_args
from aphrodite.entrypoints.serve.utils.api_utils import APHRODITE_SUBCMD_PARSER_EPILOG
from aphrodite.logger import init_logger
from aphrodite.omni.entrypoints.cli.logo import log_logo
from aphrodite.omni.entrypoints.openai.api_server import omni_run_server
...
Examples:
  ...
  aphrodite serve Qwen/Qwen2.5-Omni-7B --omni --port 8091

  # Start a diffusion model server
  aphrodite serve Qwen/Qwen-Image --omni --port 8091
...
def _ensure_aphrodite_platform():
    """Ensure Sonar's current_platform is valid before arg parsing.

    Upstream Sonar's argument parser now instantiates DeviceConfig during
    ...
    broken package metadata), Sonar's own platform auto-detection may fail
    ...
    """
    from aphrodite import platforms as aphrodite_platforms

    if aphrodite_platforms.current_platform.is_unspecified():
        from aphrodite.omni.platforms import current_omni_platform

        if not current_omni_platform.is_unspecified():
            aphrodite_platforms.current_platform = current_omni_platform
            logger.debug(
                "Replaced Sonar UnspecifiedPlatform with omni platform %s",
                type(current_omni_platform).__name__,
            )
        else:
            from aphrodite.platforms.cpu import CpuPlatform

            aphrodite_platforms.current_platform = CpuPlatform()
            logger.debug(
                "Both Sonar and omni platforms are unspecified, falling back to CpuPlatform for arg parsing",
            )


class OmniServeCommand(CLISubcommand):
    """The `serve` subcommand for the Sonar CLI."""
    ...
    def cmd(args: argparse.Namespace) -> None:
        if not isinstance(args, TrackingNamespace):
            tracked_keys = frozenset(getattr(args, "_cli_explicit_keys", vars(args)))
            args = TrackingNamespace(args, tracked_keys)
        if not os.environ.get("APHRODITE_DISABLE_LOG_LOGO"):
            os.environ["APHRODITE_DISABLE_LOG_LOGO"] = "1"
            log_logo()
    ...
    def validate(self, args: argparse.Namespace) -> None:
        ...
        # Sonar CLI args that omni does not honor: parallelism comes from the
        ...
        # Sonar equivalents on the command line would silently disagree with
        ...
        if omni_lb_policy is not None:
            from aphrodite.omni.distributed.omni_coordinator import LoadBalancingPolicy
        ...
        from aphrodite.omni.diffusion.utils.hf_utils import is_diffusion_model
    ...
    def subparser_init(self, subparsers: argparse._SubParsersAction) -> TrackingArgumentParser:
        serve_parser = subparsers.add_parser(
            ...
            usage="aphrodite serve [model_tag] --omni [options]",
        )

        _ensure_aphrodite_platform()
        serve_parser = make_arg_parser(serve_parser)
        if not isinstance(serve_parser, TrackingArgumentParser):
            raise TypeError("Omni CLI requires TrackingArgumentParser")
        ...
        omni_config_group = serve_parser.add_argument_group(
            title="OmniConfig", description="Configuration for Sonar Omni multi-stage and diffusion models."
        )

        omni_config_group.add_argument(
            "--omni",
            action="store_true",
            help="Enable Sonar Omni mode for multi-modal and diffusion models",
        )
        ...
        omni_config_group.add_argument(
            ...
            help=(
                ...
                "It overrides corresponding parameters in the standard Sonar Omni interface."
        ...
        omni_config_group.add_argument(
            ...
            help=(
                ...
                "Useful for model-specific sampling parameters not covered by the Sonar Omni interface."
                "During request time, it is overridden by corresponding parameters in the Sonar Omni interface."
        ...
        omni_config_group.add_argument(
            ...
            type=json.loads,
            default=None,
            help="Diffusion attention config. Accepts JSON or Sonar-style dotted flags. "
            "Examples: "
        ...
        # Sonar's --kv-cache-dtype (AR cache dtype, default "auto").
        omni_config_group.add_argument(
            "--diffusion-kv-cache-dtype",
            type=str,
            default=None,
            help="Diffusion attention KV cache dtype (e.g. fp8). Separate from Sonar --kv-cache-dtype.",
        )
        omni_config_group.add_argument(
...
def run_headless(args: TrackingNamespace) -> None:
    ...
    OmniCoordinator.
    """
    from aphrodite.omni.config.resolver import resolve_omni_config
    from aphrodite.omni.distributed.omni_connectors.utils.initialization import resolve_omni_kv_config_for_stage
    from aphrodite.omni.engine.stage_engine_startup import (
        get_headless_replica_devices,
        launch_headless_diffusion_replicas,
        launch_headless_llm_replicas,
    )
    from aphrodite.omni.engine.stage_init_utils import (
        build_aphrodite_config,
    ...
    aphrodite_config, executor_class = build_aphrodite_config(
        stage_cfg,
        model,
        stage_connector_spec=stage_connector_spec,
    ...
    if parallel_config.node_rank_within_dp > 0:
        ...
        logger.info(
            "Launching Sonar Omni (v%s) headless multiproc executor, "
            "with head node address %s for torch.distributed process group.",
            APHRODITE_VERSION,
            head_node_address,
        )

        executor = MultiprocExecutor(aphrodite_config, monitor_workers=False)
