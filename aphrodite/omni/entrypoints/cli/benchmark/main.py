...
"""Top-level ``aphrodite bench`` command integration for Sonar Omni.
...
exposes the command to the surrounding Sonar Omni CLI registration mechanism.
"""
...
class OmniBenchmarkSubcommand(CLISubcommand):
    """The `bench` subcommand for the Sonar CLI."""

    name = "bench"
    help = "Sonar-omni bench subcommand."
    ...
    def subparser_init(self, subparsers: argparse._SubParsersAction) -> TrackingArgumentParser:
        bench_parser = subparsers.add_parser(
            self.name, description=self.help, usage=f"aphrodite {self.name} <bench_type> [options]"
        )
        ...
        for cmd_cls in OmniBenchmarkSubcommandBase.__subclasses__():
            cmd_subparser = bench_subparsers.add_parser(
                ...
                usage=f"aphrodite {self.name} {cmd_cls.name} [--omni] [options]",
            )
