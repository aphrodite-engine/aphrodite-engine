...
"""Sonar Omni extensions for the ``aphrodite bench serve`` CLI.
...
Core functions:
    ...
    extend_omni_choices: Add Omni datasets and backends to choices defined by
        the upstream Sonar parser, including its shadow parser.
...
upstream Sonar registers its arguments. New feature arguments should normally be
...
def add_multi_stage_cli_args(parser: argparse.ArgumentParser) -> None:
    """Add CLI arguments for Sonar Omni multi-stage benchmarks."""
    group = parser.add_argument_group("Sonar Omni Multi-stage Benchmark Options")
    group.add_argument(
        "--print-stage",
        action="store_true",
        default=False,
...
def add_daily_omni_cli_args(parser: argparse.ArgumentParser) -> None:
    ...
    group.add_argument(
        ...
        "request. "
        "When using file URLs, you MUST start the Sonar server with "
...
def add_seed_tts_cli_args(parser: argparse.ArgumentParser) -> None:
    ...
    group.add_argument(
        ...
        "Sets SEED_TTS_WER_EVAL=1. Install: pip install 'sonar-omni[dev]'. "
...
def add_omni_args(parser: argparse.ArgumentParser) -> None:
    """Register all Sonar Omni serving benchmark arguments."""
