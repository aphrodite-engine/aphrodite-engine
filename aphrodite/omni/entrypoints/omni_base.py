...
from aphrodite.omni.engine.async_omni_engine import AsyncOmniEngine
...
from aphrodite.omni.entrypoints.utils import coerce_param_message_types, get_final_stage_id_for_e2e
from aphrodite.omni.errors import raise_client_error_or
from aphrodite.omni.metrics.modality import OmniModalityMetrics, observe_modality_at_finalize
from aphrodite.omni.metrics.prometheus import OmniPrometheusMetrics
from aphrodite.omni.metrics.stats import OrchestratorAggregator
from aphrodite.omni.metrics.transfer import OmniTransferMetrics
from aphrodite.omni.metrics.utils import (
...
from aphrodite.omni.utils.tracking_parser import TrackingNamespace
from aphrodite.transformers_utils.repo_utils import file_or_path_exists
...
if TYPE_CHECKING:
    from aphrodite.omni.engine.stage_pool import StagePool, StagePoolClient
...
def omni_snapshot_download(model_id: str) -> str:
    ...
    # ModelConfig. Sonar then materializes only config/tokenizer files locally
    ...
    # Read through ``aphrodite.envs`` so the flag keeps Sonar's semantics (only
    # "true", case-insensitive, enables ModelScope). Reading the raw variable
    # here made every non-empty value truthy, so ``APHRODITE_USE_MODELSCOPE=0``
...
class OmniBase(PDDisaggregationMixin):
    ...
    def _handle_output_message(
        ...
        if not isinstance(msg, OutputMessage):
            logger.warning("[%s] got unexpected msg type: %s", self.__class__.__name__, type(msg).__name__)
            return True, None, None, None
    ...
    def _stop_profile(self, stages: list[int] | None = None) -> list[Any]:
