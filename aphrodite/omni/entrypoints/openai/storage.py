...
from aphrodite.omni.config.server_settings import SERVER_SETTINGS_CONFIG, FileBackend
...
class LocalStorageManager(StorageBaseManager[FileStorageHandle]):
    ...
    async def save(self, data: bytes, file_name: str) -> SaveContext:
        async with self._io_semaphore:
            write = asyncio.create_task(asyncio.to_thread(self._save_sync, data, file_name))
            try:
                return await asyncio.shield(write)
            except asyncio.CancelledError:
                # Finish the write before the caller removes a cancelled job's file.
