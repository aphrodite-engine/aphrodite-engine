...
from aphrodite.omni.entrypoints.openai.tts_adapters.base import (
    ARTTSAdapter,
    PreparedRequest,
...
class _MossTTSAdapterBase(ARTTSAdapter):
    ...
    def _get_moss_ref_encoder(self):
        ...
        if cached is not None:
            return cached
        from aphrodite.omni.model_executor.models.moss_tts.reference_encoder import build_reference_encoder
    ...
    async def _encode_moss_references(
        ...
        *,
        ...
        """
        from aphrodite.omni.model_executor.models.moss_tts.reference_encoder import encode_request_references
        ...
        if not isinstance(request.ref_audio, str):
            raise ValueError("MOSS-TTS requires a single reference audio clip per speaker")
    ...
    async def _build_moss_tts_params(
        ...
        *,
        ...
        if v is None:  # moss_tts_nano
            ...
            if not isinstance(request.ref_audio, str):
                raise ValueError("MOSS-TTS Nano requires a single reference audio clip")
        ...
        if v == "realtime":
            ...
            if request.max_new_tokens is not None:
                params["max_new_frames"] = [request.max_new_tokens]
            if not isinstance(request.ref_audio, str):
                raise ValueError("MOSS-TTS Realtime requires a single reference audio clip")
            wav_list, sr, cache_key = await self._resolve_ref_audio(request.ref_audio)
            params["prompt_audio_array"] = [[wav_list, sr]]
            params["ref_audio_cache_key"] = cache_key
            return params
    ...
    async def build(
        ...
        if isinstance(tts_params.get("prompt_token_ids"), list):
            ...
            prompt = dict(tokens_input(prompt_token_ids=prompt_token_ids))
        else:
            prompt = dict(tokens_input(prompt_token_ids=[1]))
