...
class HiggsAudioV2Adapter(ARTTSAdapter):
    ...
    async def _build_higgs_audio_v2_params(self, request: "OpenAICreateSpeechRequest"):
        ...
        """
        from aphrodite.omni.model_executor.models.higgs_audio_v2.higgs_audio_v2_tokenizer import (
        ...
        if request.ref_audio is None:
            ...
            return dict(tokens_input(prompt_token_ids=prompt_token_ids))

        if not isinstance(request.ref_audio, str):
            raise ValueError("Higgs Audio v2 requires a single reference audio clip")
        ...
        prompt = dict(tokens_input(prompt_token_ids=out["prompt_token_ids"]))
        # Pass tensors at the top level of additional_information (NOT list-
        # wrapped). ``aphrodite.omni.data_entry_keys.serialize_payload`` routes
