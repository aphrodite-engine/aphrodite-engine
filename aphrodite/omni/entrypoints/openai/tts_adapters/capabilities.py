...
from aphrodite.omni.utils.speaker_cache import iter_custom_voice_profiles, load_validated_profile_tensors
...
def load_codec_frame_rate(engine_client: Any) -> float | None:
    ...
    try:
        configured_rate = getattr(engine_client.model_config.hf_config, "codec_frame_rate_hz", None)
        if configured_rate is not None:
            logger.info("Using codec frame rate from hf_config: %s Hz", configured_rate)
