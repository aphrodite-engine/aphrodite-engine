...
class HiggsAudioV3Adapter(ARTTSAdapter):
    ...
    async def _build_higgs_audio_v3_params(self, request: "OpenAICreateSpeechRequest"):
        ...
        if request.ref_audio is None:
            ...
            return dict(tokens_input(prompt_token_ids=prompt_ids))

        # Voice clone
        from aphrodite.omni.model_executor.models.higgs_audio_v3.higgs_audio_v3_tokenizer import (
            apply_delay_pattern,
            encode_reference_audio,
        )

        if not isinstance(request.ref_audio, str):
            raise ValueError("Higgs Audio v3 requires a single reference audio clip")
