...
from aphrodite.entrypoints.generate.base.serving import GenerateBaseServing as OpenAIServing
...
from aphrodite.omni.entrypoints.openai.audio_utils_mixin import AudioMixin, StreamingAudioResampler
...
from aphrodite.omni.entrypoints.openai.speech_usage import (
    SpeechOutputTokenCounter,
    build_speech_usage,
    qwen3_tts_input_token_details,
)
from aphrodite.omni.entrypoints.openai.tts_adapters import (
...
from aphrodite.omni.inputs.data import OmniCustomPrompt
from aphrodite.omni.outputs import OmniRequestOutput
from aphrodite.omni.utils.speaker_cache import get_speaker_cache
...
_SPEECH_USAGE_INPUT_TOKENS_HEADER = "X-APHRODITE-OMNI-INPUT-TOKENS"
_SPEECH_USAGE_OUTPUT_TOKENS_HEADER = "X-APHRODITE-OMNI-OUTPUT-TOKENS"
_SPEECH_USAGE_TOTAL_TOKENS_HEADER = "X-APHRODITE-OMNI-TOTAL-TOKENS"
_SPEECH_USAGE_INPUT_TEXT_TOKENS_HEADER = "X-APHRODITE-OMNI-INPUT-TEXT-TOKENS"
_SPEECH_USAGE_INPUT_AUDIO_TOKENS_HEADER = "X-APHRODITE-OMNI-INPUT-AUDIO-TOKENS"
...
class DiffusionSpeechPrompt(OmniCustomPrompt, total=False):
    input: str
...
class OmniOpenAIServingSpeech(OpenAIServing, AudioMixin):
    ...
    _diffusion_engine: "AsyncOmni"
    _diffusion_model_name: str
    _diffusion_stage_configs: list[object] | None
    ...
    def _init_speaker_storage(self) -> None:
        ...
        speaker_samples_dir = os.environ.get("SPEAKER_SAMPLES_DIR", os.path.expanduser("~/.cache/sonar-omni/speakers"))
    ...
    async def warmup(self) -> None:
        ...
        VoxCPM2 requires a real inference request with an active Sonar
    ...
    def _get_ref_audio_cache_key(
        ...
        path. Sonar's default is ``""`` (not ``None``); both mean local media
    ...
    async def _resolve_ref_audio(self, ref_audio_str: str) -> tuple[list[float], int, str]:
        ...
        Delegates to upstream Sonar's MediaConnector which handles http(s)
        ...
        for attempt in range(_REF_AUDIO_METADATA_FETCH_ATTEMPTS):
            ...
            wav_list, sr, artifact_key, duration = self._finalize_fetched_ref_audio(wav_np, int(fetched_sr))
    ...
    def _is_timestamps_output(res) -> bool:
        ...
        from aphrodite.omni.model_executor.stage_input_processors.forced_aligner import TIMESTAMPS_MODALITY
    ...
    async def _prepare_speech_generation(
        ...
        if request.extra_params is not None and sampling_params_list:
            ...
            if isinstance(stage0_params, PoolingParams):
                raise ValueError("Speech generation requires sampling parameters for the entry stage")
            for name in ("temperature", "top_p", "top_k"):
                if (value := request.extra_params.get(name)) is not None:
                    setattr(stage0_params, name, value)
            if stage0_params.extra_args is None:
                stage0_params.extra_args = {}
            stage0_params.extra_args.update(request.extra_params)
        ...
        try:
            ...
            async for res in generator:
                ...
                try:
                    ...
                    continue
                if step_key is None or step_audio is None:
                    continue
            ...
            if collect is not None and getattr(request, "word_timestamps", False):
                from aphrodite.omni.utils.forced_aligner import extract_word_timestamps
            ...
            if is_moss:
                ...
                # aphrodite 0.20 rebase non-stream requests resolve to FINAL_ONLY, so
        ...
        try:
            ...
            if request.ref_audio is not None:
                if not isinstance(request.ref_audio, str):
                    raise ValueError("Diffusion speech requires a single reference audio URL")
            ...
            prompt: DiffusionSpeechPrompt = {"input": request.input}
            ...
            if extra:
                ...
                sampling_params_list = copy.deepcopy(sampling_params_list)
                diffusion_params = sampling_params_list[0]
                if isinstance(diffusion_params, PoolingParams):
                    raise ValueError("Diffusion speech requires sampling parameters for the entry stage")
                if diffusion_params.extra_args is None:
                    diffusion_params.extra_args = {}
                diffusion_params.extra_args.update(extra)
