...
from aphrodite.omni.entrypoints.openai.protocol.videos import VideoGenerationRequest, VideoParams
...
from aphrodite.omni.inputs.data import (
...
from aphrodite.omni.outputs.output_metadata import DiffusionPayloadValue
...
class OmniStreamingVideoOutputHandler:
    ...
    async def _decode_image_reference(request: VideoGenerationRequest) -> Image.Image | None:
        if isinstance(request.image_reference, list):
            raise HTTPException(
                HTTPStatus.BAD_REQUEST.value,
                detail="Streaming video generation supports a single image_reference.",
            )
        try:
