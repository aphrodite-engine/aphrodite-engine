...
for text-to-image generation, with sonar-omni specific extensions.
"""
...
from aphrodite.omni.entrypoints.openai.image_api_utils import validate_layered_layers
...
class ImageGenerationRequest(BaseModel):
    ...
    Follows the OpenAI Images API specification with sonar-omni extensions
...
class ImageEditARDeltaChunk(BaseModel):
    ...
    metrics: dict[str, Any] | None = Field(
        default=None,
        description="Optional Sonar Omni per-stage metrics snapshot for benchmark clients.",
    )
