...
DEFAULT_AUDIO_FORMAT: Literal["wav"] = "wav"
...
class OpenAICreateSpeechRequest(BaseModel):
    ...
    def validate_speaker_embedding(
        ...
        if isinstance(v[0], list):
            ...
            return v
        if not all(not isinstance(x, list) and math.isfinite(x) for x in v):
            raise ValueError("'speaker_embedding' values must be finite (no NaN or Inf)")
        return v
    ...
    def normalize_references_alias(cls, data):
        ...
        sonar-omni's existing convention is ``ref_audio`` + ``ref_text``.
        ...
        (sonar-omni does not yet support multi-shot voice clone) or conflicts
