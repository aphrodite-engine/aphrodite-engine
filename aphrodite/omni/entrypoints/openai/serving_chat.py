...
from aphrodite.inputs.engine import EngineInput
from aphrodite.omni.diffusion.utils.param_utils import apply_declared_extra_args
from aphrodite.omni.entrypoints.async_omni import AsyncOmni
from aphrodite.omni.entrypoints.openai.diffusion_request_utils import (
    apply_normalized_diffusion_request_extra_args,
    normalize_diffusion_request_args,
)
from aphrodite.omni.entrypoints.openai.protocol.chat_completion import OmniChatCompletionResponse
from aphrodite.omni.entrypoints.utils import coerce_param_message_types
from aphrodite.omni.inputs.data import OmniDiffusionSamplingParams, OmniPromptType, OmniSamplingParams, OmniTextPrompt
from aphrodite.omni.metrics import definitions as _metric_defs
from aphrodite.omni.metrics.modality import (
    observe_audio_first_packet,
    observe_audio_streaming_finalize,
)
from aphrodite.omni.model_executor.models.minicpmo_4_5.pipeline import MINICPMO45_REFERENCE_AUDIO_KEY
from aphrodite.omni.model_extras import (
    get_extra_body_params,
    get_extra_output_params,
)
from aphrodite.parser.utils import (
...
from aphrodite.entrypoints.generate.base.serving import clamp_prompt_logprobs
...
from aphrodite.entrypoints.openai.chat_completion.serving import (
    OpenAIServingChat,
...
from aphrodite.entrypoints.serve.engine.protocol import ErrorInfo, ErrorResponse, UsageInfo
from aphrodite.entrypoints.serve.engine.typing import ChatLikeRequest
from aphrodite.entrypoints.serve.utils.api_utils import should_include_usage
from aphrodite.entrypoints.serve.utils.tool_calls_utils import maybe_filter_parallel_tool_calls
from aphrodite.inputs import PromptType
from aphrodite.logger import init_logger
from aphrodite.multimodal.media.connector import MediaConnector
from aphrodite.omni.entrypoints.openai.audio_utils_mixin import AudioMixin
from aphrodite.omni.entrypoints.openai.image_api_utils import (
    encode_image_base64_with_compression,
    validate_layered_layers,
)
from aphrodite.omni.entrypoints.openai.protocol import OmniChatCompletionStreamResponse
...
from aphrodite.omni.entrypoints.openai.utils import (
    get_stage_type,
...
from aphrodite.omni.lora.request import LoRARequest
from aphrodite.omni.outputs import OmniRequestOutput
from aphrodite.omni.outputs.output_metadata import DiffusionMetadataMapping, DiffusionMetadataValue
from aphrodite.omni.utils.audio import audio_chunk_pcm_bytes, audio_chunk_sample_rate
from aphrodite.parser.mistral import MistralToolCall
from aphrodite.reasoning import ReasoningParser
from aphrodite.renderers import BaseRenderer, merge_kwargs
from aphrodite.renderers.inputs import TokPrompt
from aphrodite.sampling_params import RequestOutputKind, SamplingParams
from aphrodite.tokenizers import TokenizerLike
from aphrodite.tokenizers import TokenizerLike as AnyTokenizer
from aphrodite.tokenizers.mistral import (
    MistralTokenizer,
...
from aphrodite.tool_parsers.streaming import extract_required_tool_call_streaming
...
class OmniOpenAIServingChat(OpenAIServingChat, AudioMixin):
    @property
    def _omni_engine(self) -> AsyncOmni:
        return cast(AsyncOmni, self.engine_client)
    ...
    # Harmony flag (always False for sonar-omni models)
    ...
    def tool_call_id_type(self) -> str:
        ...
        Upstream Sonar removed the stored ``tool_call_id_type`` attribute
        from ``OpenAIServingChat`` after the ParserManager refactor; the
        field is now resolved on demand via ``get_tool_call_id_type``.
        """
        try:
            from aphrodite.entrypoints.chat_utils import get_tool_call_id_type
    ...
    def _should_stream_with_auto_tool_parsing(self, request: ChatCompletionRequest) -> bool:
        ...
        This method existed in upstream Sonar commit 91df0fad4 (OpenAIServingChat)
        but was removed in the Harmony refactoring (PR #45171, #45104).  Omni's
        independently-maintained ``_omni_chat_completion_stream_generator`` still calls
    ...
    def for_diffusion(
        ...
        instance.engine_client = cast(EngineClient, diffusion_engine)
    ...
    def _normalize_diffusion_request_args(
        ...
        explicit_root_args = {
            key: getattr(request, key) for key in (explicit or ()) if key != "extra_body" and hasattr(request, key)
        }
    ...
    @classmethod
    def _filter_stage_metrics_detail(cls, metrics: dict[str, Any] | None, request: Any) -> PerRequestMetrics | None:
        if not metrics:
            return None
        if cls._truthy_extra_body_flag(request, "return_stage_metrics"):
            return PerRequestMetrics.model_validate(metrics)
        return None
    ...
    async def _create_chat_completion(
        ...
        try:
            ...
            if not self.use_harmony:
                ...
                conversation, rendered_prompts = await self._preprocess_chat(
                    request,
                    request.messages,
                ...
                conversation, rendered_prompts = self.online_renderer._make_request_with_harmony(
        ...
        engine_prompts: list[OmniPromptType] = [cast(OmniPromptType, prompt) for prompt in rendered_prompts]
        ...
        engine_output_modalities = [x for x in self._omni_engine.output_modalities if x is not None]
        output_modalities = getattr(request, "modalities", engine_output_modalities)
        output_modalities = output_modalities if output_modalities is not None else engine_output_modalities
        setattr(request, "modalities", output_modalities)

        if not isinstance(output_modalities, list) or not all(isinstance(m, str) for m in output_modalities):
        ...
        unsupported = set(output_modalities) - allowed_modalities
        if unsupported:
            return self.create_error_response(
        ...
        if output_modalities and ("image" in output_modalities):
            try:
                ...
                if engine_prompt_image is not None:
                    ...
                    # Provide multi_modal_uuids so that newer Sonar versions
        ...
        elif output_modalities and ("text" in output_modalities) and is_single_stage_diffusion(self.engine_client):
        ...
        generators: list[AsyncGenerator[OmniRequestOutput, None]] = []
        mm_token_counts: dict[str, int] | None = None
        try:
            for i, engine_prompt in enumerate(engine_prompts):
                if self.enable_prompt_tokens_details:
                    mm_token_counts = _get_mm_token_counts(cast(EngineInput, engine_prompt))
                ...
                sampling_params_list = coerce_param_message_types(sampling_params_list, bool(request.stream))
                if request.stream:
                ...
                generator = self._omni_engine.generate(
        ...
        # Streaming response
        if request.stream:
            return self._omni_chat_completion_stream_generator(
                request,
                result_generator,
                request_id,
                model_name,
        ...
        try:
            return await self._omni_chat_completion_full_generator(
                request,
                result_generator,
                request_id,
                model_name,
    ...
    async def _inject_audio_from_video_urls(
        ...
        for msg in messages:
            ...
            audios = await asyncio.gather(*(connector.fetch_audio_async(u) for u in video_urls))
    ...
    def _get_comprehension_stage_index(self) -> int:
        for idx, stage in enumerate(self._omni_engine.stage_configs):
    ...
    def _apply_request_overrides(
        ...
        for field_name in self._OPENAI_SAMPLING_FIELDS:
            if field_name not in (explicit_fields or ()):
                continue
    ...
    def _should_check_for_unstreamed_tool_arg_tokens(
        ...
        ``_omni_chat_completion_stream_generator`` still calls it, so we keep a
    ...
    def _build_sampling_params_list_from_request(
        ...
        """
        default_params_list = self._omni_engine.default_sampling_params_list
        ...
        for idx, default_params in enumerate(default_params_list):
            ...
            if idx == comprehension_idx:
                if not isinstance(default_params, SamplingParams):
                    raise TypeError("A chat comprehension stage requires autoregressive sampling parameters")
    ...
    def _log_omni_inputs(
        self,
        request_id: str,
        inputs: OmniPromptType,
        params_list: list[OmniSamplingParams] | None,
        lora_request: LoRARequest | None,
    ) -> None:
        if self.request_logger is None:
            return
        components = self._extract_prompt_components(cast(PromptType, inputs))
        self.request_logger.log_inputs(
            request_id,
            components.text,
            components.token_ids,
            components.embeds,
            params=params_list[0] if params_list and isinstance(params_list[0], SamplingParams) else None,
            lora_request=lora_request,
        )

    async def _omni_chat_completion_stream_generator(
        self,
        request: ChatCompletionRequest,
        ...
        try:
            async for omni_res in result_generator:
                ...
                if final_output_type == "text":
                    for output in res.outputs:
                        ...
                        # if the model is finished generating
                            ...
                            if (
                                ...
                                finish_reason_: str | None = "tool_calls"
                            else:
                    ...
                    # emit at request finalize. self._omni_engine.request_states
                    ...
                    req_state = next(
                        (s for s in self._omni_engine.request_states.values() if s.external_request_id == request_id),
                        None,
                    )
                    ...
                    if req_state is not None and req_state.first_audio_ts is None:
                        ...
                        if replica_id is None:
                            stage_pools = getattr(self._omni_engine.engine, "stage_pools", None)
                            ...
                            replica_id = (
                                ...
                                if stage_pools is not None
                                and omni_res.stage_id is not None
                                and 0 <= omni_res.stage_id < len(stage_pools)
                                else None
                            )
                        req_state.audio_emit_stage_id = omni_res.stage_id
                        req_state.audio_emit_replica_id = replica_id
                        if omni_res.stage_id is not None:
                            observe_audio_first_packet(
                                self._omni_engine.mod_metrics,
                    ...
                    role = self.get_chat_request_role(request)
                    image_choices: list[ChatCompletionResponseStreamChoice] = []
                    for choice in self._create_image_choice(omni_res, role, request, stream=True):
                        ...
                        image_choices.append(stream_choice)
                    chunk = OmniChatCompletionStreamResponse(
                        id=request_id,
                        object=chunk_object_type,
                        created=created_time,
    ...
    async def _omni_chat_completion_full_generator(
        self,
        request: ChatCompletionRequest,
        result_generator: AsyncIterator[OmniRequestOutput],
        request_id: str,
        ...
        for omni_outputs in final_outputs:
            choices_data: list[ChatCompletionResponseChoice] | ErrorResponse = []
        ...
        filtered_metrics = self._filter_stage_metrics_detail(response_metrics, request)
    ...
    def _create_text_choice(
        ...
        for output in final_res.outputs:
            ...
            # undetermined case that is still important to handle
                ...
                logger.error(
                    "Error in _omni_chat_completion_full_generator - cannot determine if tools should be extracted. "
    ...
    def _create_audio_choice(
        self, omni_outputs: OmniRequestOutput, role: str, request: ChatCompletionRequest, stream: Literal[False] = False
    ) -> list[ChatCompletionResponseChoice] | ErrorResponse: ...

    @overload
    def _create_audio_choice(
        self, omni_outputs: OmniRequestOutput, role: str, request: ChatCompletionRequest, stream: Literal[True]
    ) -> list[ChatCompletionResponseStreamChoice] | ErrorResponse: ...
    ...
    def _create_audio_choice(
        ...
        for output in final_res.outputs:
            if stream:
                ...
                choices.append(choice_data)
        return stream_choices if stream else choices
    ...
    def _create_image_choice(
        ...
        if len(image_contents) == 1 or len(image_contents) > 1:
    ...
    def _build_multistage_generation_inputs(
        ...
        *,
        ...
        if bot_task is not None or use_system_prompt is not None or custom_system_prompt is not None:
            from aphrodite.omni.diffusion.models.hunyuan_image3.prompt_utils import (
                build_prompt,
                build_prompt_tokens,
        ...
        if engine_prompt_data is not None:
            ...
            # Provide multi_modal_uuids so that newer Sonar versions can
        ...
        for idx, stage_cfg in enumerate(stage_configs):
            ...
            if stage_type == "llm" and ar_stop_token_ids is not None:
                if not isinstance(default_stage_params, SamplingParams):
                    raise TypeError("An LLM stage requires autoregressive sampling parameters")
            ...
            if comprehension_idx is not None and idx == comprehension_idx and height is not None and width is not None:
                if not isinstance(default_stage_params, SamplingParams):
                    raise TypeError("An AR stage requires autoregressive sampling parameters")
            ...
            if stage_type == "diffusion":
                if not isinstance(default_stage_params, OmniDiffusionSamplingParams):
                    raise TypeError("A diffusion stage requires diffusion sampling parameters")
                self._set_if_supported(
    ...
    def _prepare_diffusion_image_request(
        ...
        *,
        ...
        gen_prompt = OmniTextPrompt(prompt=prompt, modalities=["image"])
        if negative_prompt is not None:
            ...
            async for output in result_generator:
                ...
                if final_output_type == "text" and stage_id == 0:
                    for completion in output.outputs:
                        ...
                        if not text:
                            continue
                        chunk: ImageEditARDeltaChunk | ImageEditImageChunk | ImageEditStreamError = (
                            ImageEditARDeltaChunk(
                                delta=text,
        ...
        try:
            ...
            if final_output_type == "audio":
                ...
                if not image_contents:
                    content: str | list[dict[str, Any]] = "Image generation completed but no images were produced."
                else:
                    content = image_contents
