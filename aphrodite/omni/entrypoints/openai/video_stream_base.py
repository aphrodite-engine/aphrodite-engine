...
from aphrodite.omni.entrypoints.openai import video_stream_envs
from aphrodite.omni.entrypoints.openai.video_frame_filter import FrameSimilarityFilter
from aphrodite.omni.entrypoints.openai.video_stream_context import (
    text_only_message,
)
...
class OmniStreamingVideoHandler:
        ...
        try:
            ...
            async def _start_query_turn(*, query_text: str) -> None:
                ...
                query_prewarmed_frames = {
                    key: value for key, value in frame_pil_cache.items() if isinstance(value, tuple)
                }
    ...
    async def _process_query_engine(
        ...
        async_chunk_mode = video_stream_envs.APHRODITE_VIDEO_ASYNC_CHUNK
        streaming = async_chunk_mode == "on"
        audio_tail_tensors: list[Any] = []

        try:
            if self._engine_client is None:
                raise RuntimeError("Video streaming engine is not initialized")
    ...
    def _extract_audio_delta_b64(
        ...
        Two paths, selected at runtime by ``APHRODITE_VIDEO_AUDIO_DELTA_MODE``:
        ...
        if video_stream_envs.APHRODITE_VIDEO_AUDIO_DELTA_MODE == "slow":
    ...
    def _encode_audio_wav_b64(audio_np) -> str:
        ...
        return resp.audio_data.decode("ascii") if isinstance(resp.audio_data, bytes) else resp.audio_data
