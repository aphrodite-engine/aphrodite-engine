...
from aphrodite.entrypoints.chat_utils import ChatTemplateConfig, load_chat_template
...
from aphrodite.entrypoints.launchers.launcher import serve_http, terminate_if_errored
from aphrodite.entrypoints.mcp.tool_server import DemoToolServer, MCPToolServer, ToolServer
from aphrodite.entrypoints.openai.api_server import build_app as build_openai_app
from aphrodite.entrypoints.openai.api_server import setup_server as setup_openai_server
...
from aphrodite.entrypoints.pooling.classify.serving import ServingClassification
from aphrodite.entrypoints.pooling.embed.serving import ServingEmbedding as OpenAIServingEmbedding
from aphrodite.entrypoints.pooling.pooling.serving import ServingPooling
from aphrodite.entrypoints.pooling.scoring.serving import ServingScores
from aphrodite.entrypoints.scale_out.token_in_token_out.serving import ServingTokens
from aphrodite.entrypoints.serve.engine.protocol import ErrorResponse, ModelCard, ModelList, ModelPermission

# Sonar < 0.28 keeps create_error_response under serve.utils (it is not
# re-exported from aphrodite.entrypoints.serve); 0.28+ moved it under
...
try:
    ...
    from aphrodite.entrypoints.serve.exception_handling.error_response import create_error_response

# Sonar moved `base` from openai.basic.api_router to serve.instrumentator.basic.
# Keep a fallback for older/newer upstream layouts during rebase windows.
from aphrodite.entrypoints.launchers.utils.server_utils import get_uvicorn_log_config
...
from aphrodite.entrypoints.speech_to_text.realtime.serving import OpenAIServingRealtime
from aphrodite.entrypoints.speech_to_text.transcription.serving import (
    OpenAIServingTranscription,
)
from aphrodite.entrypoints.speech_to_text.translation.serving import (
    OpenAIServingTranslation,
)
from aphrodite.logger import init_logger
from aphrodite.omni.config.endpoint_policy import (
    remove_route_from_app,
    shutdown_unsupported_routes,
)
from aphrodite.omni.config.server_settings import SERVER_SETTINGS_CONFIG
from aphrodite.omni.diffusion.models.interface import ReferenceVideoDecodeSpec
from aphrodite.omni.entrypoints.async_omni import AsyncOmni
from aphrodite.omni.entrypoints.duplex.capability import should_enable_duplex_endpoint
from aphrodite.omni.entrypoints.duplex.serving import OmniDuplexSessionHandler
from aphrodite.omni.entrypoints.openai.batch_serving import OmniOpenAIServingChatBatch
from aphrodite.omni.entrypoints.openai.errors import InvalidInputReferenceError
from aphrodite.omni.entrypoints.openai.image_api_utils import (
    SUPPORTED_LAYERED_RESOLUTIONS,
    encode_image_base64_with_compression,
    parse_size,
    validate_layered_layers,
)
from aphrodite.omni.entrypoints.openai.protocol.audio import (
    BatchSpeechRequest,
...
from aphrodite.omni.entrypoints.openai.realtime_connection import RealtimeConnection
from aphrodite.omni.entrypoints.openai.serving_audio_generate import OmniOpenAIServingAudioGenerate
from aphrodite.omni.entrypoints.openai.serving_chat import OmniOpenAIServingChat
from aphrodite.omni.entrypoints.openai.serving_speech import OmniOpenAIServingSpeech
from aphrodite.omni.entrypoints.openai.serving_speech_stream import OmniStreamingSpeechHandler
from aphrodite.omni.entrypoints.openai.serving_video import (
    OmniOpenAIServingVideo,
    ReferenceAudio,
    ReferenceImage,
    ReferenceVideo,
)
from aphrodite.omni.entrypoints.openai.serving_video_output_stream import OmniStreamingVideoOutputHandler
from aphrodite.omni.entrypoints.openai.serving_video_stream import create_streaming_video_handler
from aphrodite.omni.entrypoints.openai.stage_params import (
    build_stage_sampling_params_list,
    get_default_sampling_params_list,
)
from aphrodite.omni.entrypoints.openai.storage import STORAGE_MANAGER, FileStorageHandle
from aphrodite.omni.entrypoints.openai.stores import VIDEO_STORE, VIDEO_TASKS
from aphrodite.omni.entrypoints.openai.utils import get_stage_type, parse_lora_request
from aphrodite.omni.entrypoints.openai.video_api_utils import (
    VideoFrames,
    decode_audio_url,
    decode_input_reference,
)
from aphrodite.omni.entrypoints.openpi.serving import ServingRealtimeRobotOpenPI
from aphrodite.omni.entrypoints.utils import PureDiffusionLauncherAdapter
from aphrodite.omni.errors import OmniClientError
from aphrodite.omni.inputs.data import OmniDiffusionSamplingParams, OmniTextPrompt
from aphrodite.omni.utils.forced_aligner import build_forced_aligner_config
from aphrodite.omni.utils.tracking_parser import TrackingArgumentParser, TrackingNamespace
...
from aphrodite.utils.system_utils import decorate_logs
...
async def _get_aphrodite_config(engine_client: EngineClient) -> Any:
    if hasattr(engine_client, "get_aphrodite_config"):
        return await engine_client.get_aphrodite_config()
    return getattr(engine_client, "aphrodite_config", None)


def _register_omni_exception_handlers(app) -> None:
    """Override upstream Sonar exception handlers with Omni-aware versions.
...
class _DiffusionServingModels:
    ...
    Sonar's /v1/models route expects `app.state.openai_serving_models` to expose
...
async def _warmup_duplex_realtime(app, args, warmup_frames: int) -> None:
    ...
    try:
        ...
        from aphrodite.omni.experimental.fullduplex.client import build_realtime_url

        url = (
            build_realtime_url(f"ws://127.0.0.1:{args.port}/v1/realtime", model_name, autostart=False)
            + "&aphrodite_omni_warmup=1"
        )
...
async def omni_run_server_worker(listen_address, sock, args, client_config=None, **uvicorn_kwargs) -> None:
    ...
    # In sonar-omni's multi-process architecture, create_engine_config() runs
    ...
    # (Upstream Sonar does this in EngineArgs.create_engine_config().)
    ...
    async with build_async_omni(
        ...
        # OMNI: Pass supported_tasks to build_app (required by upstream Sonar)
        ...
        aphrodite_config = await _get_aphrodite_config(engine_client)

        # Check if pure diffusion mode (aphrodite_config will be None)
        is_pure_diffusion = aphrodite_config is None
        if is_pure_diffusion:
            logger.info(
                "Starting Sonar API server (pure diffusion mode) on %s",
                listen_address,
            )
            # The Sonar 0.27 launcher's shutdown path reads
            # engine_client.aphrodite_config.shutdown_timeout; AsyncOmni.aphrodite_config
            ...
            # with a shim that only overrides aphrodite_config and forwards everything
            # else (get_aphrodite_config still returns None for pure-diffusion detection).
            ...
            logger.info(
                "Starting Sonar API server %d on %s",
                aphrodite_config.parallel_config._api_process_rank,
                listen_address,
            )
...
async def build_async_omni_from_stage_config(
    ...
    *,
    ...
    async_omni: AsyncOmni | None = None
...
async def omni_init_app_state(
    ...
    """
    # Get aphrodite_config from engine_client (following 0.14.0 pattern)
    aphrodite_config = await _get_aphrodite_config(engine_client)
    ...
    if aphrodite_config is None:
        # Try to get aphrodite_config from engine_client
        aphrodite_config = await _get_aphrodite_config(engine_client)
        if aphrodite_config is None:
            logger.warning("aphrodite_config is None, some features may not work correctly")

    state.aphrodite_config = aphrodite_config

    # Get supported tasks
    supported_tasks: set[SupportedTask] = {"generate"}
    if hasattr(engine_client, "get_supported_tasks"):
    ...
    default_mm_loras = (
        ...
        if aphrodite_config is not None and aphrodite_config.lora_config is not None
        else {}
    )
    lora_modules = process_lora_modules(args.lora_modules, default_mm_loras)

    if engine_client._omni_input_processor is None:
        if aphrodite_config is not None:
            ...
            logger.warning("Cannot initialize processors: aphrodite_config is None. OpenAIServingModels may fail.")
...
async def realtime_websocket(websocket: WebSocket):
    ...
    # connection marks itself with aphrodite_omni_warmup=1 and passes through).
    warmup_done = getattr(websocket.app.state, "duplex_warmup_done", None)
    if (
        warmup_done is not None
        and not warmup_done.is_set()
        and websocket.query_params.get("aphrodite_omni_warmup") != "1"
    ):
        try:
...
async def realtime_robot_openpi(websocket: WebSocket):
    ...
    from aphrodite.omni.entrypoints.openpi.connection import (
...
async def duplex_websocket(websocket: WebSocket):
    """WebSocket endpoint for Sonar Omni duplex session control."""
...
async def health(raw_request: Request) -> JSONResponse:
    ...
    Mirrors Sonar upstream's /health which catches EngineDeadError -> 503.
    """
...
# Remove existing models endpoint if present (from aphrodite imports)
    ...
    try:
        ...
        if len(stage_configs) > 1:
            ...
            if request.size is not None:
                ...
                _check_max_generated_image_size(app_state_args, requested_width, requested_height)
...
async def edit_images(
    ...
    # sonar-omni extensions for image editing
    mask_image: str | UploadFile | None = None,
    reference_image: str | UploadFile | None = None,
    # sonar-omni extensions for diffusion control
    ...
    # sonar-omni extension for per-request LoRA.
    lora: str | None = Form(None),  # Json string
    # sonar-omni extension for layered models (e.g., Qwen-Image-Layered)
    ...
    try:
        ...
        cot_output = None
        edit_prompt = OmniTextPrompt(prompt=prompt, modalities=["image"])
        if negative_prompt is not None:
            edit_prompt["negative_prompt"] = negative_prompt
        input_images_list = []
        ...
        edit_media: dict[str, Image.Image | list[Image.Image]] = {"image": pil_images}
        ...
        if mask_image is not None:
            ...
            edit_media["mask_image"] = loaded[0]

        if reference_image is not None:
            loaded = await _load_input_images([reference_image], normalize_rgb=normalize_edit_images_rgb)
            edit_media["reference_image"] = loaded[0]
        edit_prompt["multi_modal_data"] = edit_media
        ...
        # GLM-Image consumes target_h/target_w via mm_processor_kwargs.
        if width is not None and height is not None:
            edit_prompt["mm_processor_kwargs"] = {
                "target_h": height,
                "target_w": width,
            }
            # Backward-compatible fallback for processors reading top-level fields.
            edit_prompt["height"] = height
            edit_prompt["width"] = width
        ...
        if len(stage_configs) > 1:
            ...
            if isinstance(generation_result, ErrorResponse):
                raise HTTPException(
                    status_code=generation_result.error.code if generation_result.error else 400,
                    detail=generation_result.error.message,
                )
...
def _parse_lora_request(lora_body: dict[str, Any] | None):
    try:
...
def _build_hunyuan_edit_extra_args(
    *,
    ...
    if effective_use_system_prompt is None and bot_task is not None:
        from aphrodite.omni.diffusion.models.hunyuan_image3.prompt_utils import resolve_sys_type
...
def apply_stage_default_sampling_params(
    ...
    """
    Update a stage's sampling parameters with Sonar Omni defaults.
...
def _diffusion_model_classes(stage_configs: list[Any] | None) -> list[type]:
    ...
    from aphrodite.omni.diffusion.registry import DiffusionModelRegistry
...
VIDEO_SYNC_TIMEOUT_S = float(os.environ.get("APHRODITE_OMNI_VIDEO_SYNC_TIMEOUT", 600.0))
    ...
    try:
        for upload in uploads:
            ...
            if suffix not in {".mkv", ".mov", ".mp4", ".webm"}:
                suffix = ".mp4"
            fd, path = tempfile.mkstemp(prefix="aphrodite_omni_video_reference_", suffix=suffix)
            paths.append(path)
            with os.fdopen(fd, "wb") as output:
                size = 0
                while chunk := await upload.read(1024 * 1024):
                    size += len(chunk)
                    if size > SERVER_SETTINGS_CONFIG.max_reference_upload_bytes:
                        raise HTTPException(413, detail="Video reference exceeds the upload size limit.")
                    output.write(chunk)
...
async def _persist_uploaded_control_reference(
    ...
    *,
    ...
    fd, path = tempfile.mkstemp(prefix="aphrodite_omni_control_reference_", suffix=suffix)
    size = 0
    persisted = False
    try:
        with os.fdopen(fd, "wb") as output:
...
async def _read_upload_limited(upload: UploadFile, *, max_bytes: int | None = None) -> bytes:
    ...
    if max_bytes is None:
        max_bytes = SERVER_SETTINGS_CONFIG.max_reference_upload_bytes
    else:
        max_bytes = min(max_bytes, SERVER_SETTINGS_CONFIG.max_reference_upload_bytes)
...
async def _persist_uploaded_media_references(
    ...
    try:
        for upload in uploads:
            ...
            fd, path = tempfile.mkstemp(prefix="aphrodite_omni_reference_", suffix=suffix)
            paths.append(path)
            with os.fdopen(fd, "wb") as output:
...
async def _parse_video_form(
    ...
    if request.audio_reference is not None:
        try:
            ...
            existing_cleanup_paths = set(() if reference_audio is None else reference_audio.cleanup_paths)
            for path in audio_paths:
                if path not in existing_cleanup_paths and os.path.exists(path):
                    os.unlink(path)
...
async def create_video(
    ...
    accepted = await VIDEO_TASKS.try_start(
        ref.id,
        lambda: _run_video_generation_job(
            handler,
            request,
            ref.id,
            reference_image,
        ...
        limit=SERVER_SETTINGS_CONFIG.max_video_jobs,
    )
    if not accepted:
        await VIDEO_STORE.pop(ref.id)
        _cleanup_video_references(reference_video, reference_audio, control_path)
        raise HTTPException(503, detail="Video job queue is full. Please retry later.", headers={"Retry-After": "5"})
    return ref
...
async def omni_sleep(request: OmniSleepRequest, raw_request: Request):
    ...
    if not hasattr(engine_client, "sleep_stages"):
        raise HTTPException(status_code=501, detail="Engine does not support sleep")
    acks = await engine_client.sleep_stages(stage_ids=request.stage_ids, level=request.level)
...
async def omni_wakeup(request: OmniWakeupRequest, raw_request: Request):
    ...
    if not hasattr(engine_client, "wake_up_stages"):
        raise HTTPException(status_code=501, detail="Engine does not support wake_up")
    acks = await engine_client.wake_up_stages(stage_ids=request.stage_ids)
    for sid in request.stage_ids:
...
if __name__ == "__main__":
    parser = make_arg_parser(TrackingArgumentParser(description="Sonar Omni OpenAI-Compatible REST API server"))
    ...
    # args.model will be the default from Sonar's ModelConfig (currently
