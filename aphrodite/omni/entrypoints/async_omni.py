...
from aphrodite.omni.diffusion.data import CuMemTag, OmniACK, OmniSleepTask, OmniWakeTask
from aphrodite.omni.engine.duplex.lease import DuplexLeaseActivity
from aphrodite.omni.engine.duplex.messages import (
    DuplexFence,
    DuplexSessionLifecycleMessage,
)
from aphrodite.omni.engine.messages import ErrorMessage, OutputMessage
from aphrodite.omni.entrypoints.client_request_state import ClientRequestState
from aphrodite.omni.entrypoints.duplex_request_client import (
    DuplexRequestClient,
    DuplexRequestOutputPort,
)
from aphrodite.omni.entrypoints.omni_base import (
    OmniBase,
    OmniEngineDeadError,
)
from aphrodite.omni.errors import client_error_metadata
from aphrodite.omni.inputs.data import OmniSamplingParams
...
from aphrodite.outputs import CompletionOutput, PoolingRequestOutput
from aphrodite.plugins.io_processors import get_io_processor
from aphrodite.pooling_params import PoolingParams
from aphrodite.renderers.inputs.preprocess import extract_prompt_components
...
if TYPE_CHECKING:
    from aphrodite.omni.inputs.data import OmniInteractionPrompt, OmniPromptType
...
# (APHRODITE_OMNI_EVENT_DRIVEN_ORCH=1): a message wakes the drain immediately via
...
class AsyncOmni(EngineClient, OmniBase):
    ...
    def __init__(self, *args: Any, model: str = "", **kwargs: Any) -> None:
        ...
        self.tts_max_instructions_length = kwargs.get("tts_max_instructions_length")
        self._omni_input_processor: InputProcessor | None = self.engine.input_processor
        ...
        if stage_index is None:
            ...
            aphrodite_config = self.engine.stage_aphrodite_configs[stage_index]
            io_processor_plugin = aphrodite_config.model_config.io_processor_plugin
            renderer = self.renderer
            if renderer is None:
                from aphrodite.renderers import renderer_from_config

                renderer = renderer_from_config(aphrodite_config)
            self.io_processor = get_io_processor(aphrodite_config, renderer, io_processor_plugin)
    ...
    def _get_comprehension_stage_index(self) -> int | None:
        ...
        for idx, stage_client in enumerate(self.engine.stage_clients):
            stage_aphrodite_config = self.engine.stage_aphrodite_configs[idx]
            if stage_aphrodite_config is None:
                continue
    ...
    def renderer(self):
        ...
        if self._omni_input_processor is None:
            return None
        return self.input_processor.renderer

    @property
    def aphrodite_config(self):
        """Return the Sonar config for the comprehension stage when present."""
        stage_index = self._get_comprehension_stage_index()
        if stage_index is None:
            return None
        return self.engine.stage_aphrodite_configs[stage_index]

    async def get_aphrodite_config(self) -> Any:
        """Compatibility helper for call sites expecting async aphrodite config access."""
    ...
    def model_config(self):
        ...
        if aphrodite_config is None:
            return None
        return aphrodite_config.model_config
    ...
    def _get_unique_request_id(external_request_id: str):
        ...
        NOTE: in the upstream Sonar, this is done in the InputProcessor's
        `assign_request_id`.
        """
        uuid = random_uuid()
    ...
    async def generate(
        ...
        **Diffusion batching:**
        ...
        if session_id is not None:
            raise NotImplementedError("Use the duplex session API for Omni session requests")

        # Append a random UUID suffix to the request_id to ensure it is unique
        # and non-empty, similar to Sonar's input processor. The suffix is used
        ...
        try:
            ...
            if pd_pair is not None:
                ...
                req_sp_list[p_id] = self._prepare_prefill_sampling_params(request_id, prefill_params)
    ...
    async def _add_streaming_input_request(
        ...
        *,
        ...
        stage0_params = self._validate_streaming_input_sampling_params(sampling_params_list[0])
        req_state = self.request_states[request_id]
        has_submitted_first_chunk = False

        # NOTE: InputProcessor in Sonar should generally do this too, but for
    ...
    @staticmethod
    def _validate_streaming_input_sampling_params(params: OmniSamplingParams) -> SamplingParams:
        if (
    ...
    def _final_output_handler(self) -> None:
        ...
        # Event-driven drain (APHRODITE_OMNI_EVENT_DRIVEN_ORCH=1): block on the
        ...
        # event-driven loop (aphrodite/omni/engine/orchestrator.py).
        from aphrodite.omni.engine.orchestrator import _event_driven_orch_enabled
    ...
    async def _engine_core_rpc(
        ...
        *,
        ...
        (Sonar AsyncMPClient convention). Raises if any replica reports failure.
        """
    ...
    async def abort(self, request_id: str | Iterable[str]) -> None:
        ...
        # aborted. This is also what happens in this case in Sonar's output processor.
    ...
    async def pause_generation(
        ...
        *,
        ...
        """Pause generation, mirroring Sonar AsyncLLM.pause_generation.
        ...
        Sonar EngineCore.sleep). Call this API when you need pause *without*
        ...
        if ar_stage_ids:
            ...
            # Same API name as Sonar AsyncMPClient.pause_scheduler_async; routed
    ...
    async def start_profile(
        ...
        Uses Sonar-compatible profile(is_start=True, profile_prefix) interface.

        Args:
            profile_prefix: Optional prefix for the trace file names.
            stages: List of stage IDs to profile. If None, profiles all stages.
        """
        await self.collective_rpc(method="profile", args=(True, profile_prefix), stage_ids=stages)

    async def stop_profile(self, stages: list[int] | None = None) -> None:
        """Stop profiling specified stages.

        Uses Sonar-compatible profile(is_start=False) interface.

        Args:
            stages: List of stage IDs to profile. If None, stops all stages.
        """
        await self.collective_rpc(method="profile", args=(False, None), stage_ids=stages)
    ...
    async def reset_mm_cache(self) -> None:
        ...
        """
        processor = getattr(self, "_omni_input_processor", None)
    ...
    async def sleep(self, level: int = 2, mode: PauseMode = "abort") -> None:
        await self.sleep_stages(level=level, mode=mode)
    ...
    async def sleep_stages(
        ...
        offload/discard memory) — matching Sonar AsyncLLM.sleep.
    ...
    async def wake_up(self, tags: list[str] | None = None) -> None:
        await self.wake_up_stages(tags=tags)

    async def wake_up_stages(self, stage_ids: list[int] | None = None, tags: list[str] | None = None) -> list[OmniACK]:
    ...
    def input_processor(self) -> InputProcessor:
        if self._omni_input_processor is None:
            raise RuntimeError("This diffusion pipeline has no token input processor")
        return self._omni_input_processor

    @input_processor.setter
    def input_processor(self, processor: InputProcessor) -> None:
        self._omni_input_processor = processor
    ...
    async def get_tokenizer(self) -> TokenizerLike:
        ...
        if stage_index is not None:
            ...
            if tokenizer is not None:
                return tokenizer
        if self._omni_input_processor is None or self._omni_input_processor.tokenizer is None:
            raise RuntimeError("This pipeline has no tokenizer")
        return self._omni_input_processor.tokenizer
