...
# This monkey-patches aphrodite.benchmarks.datasets.get_samples before it's used
# Must be imported before any aphrodite.benchmarks module usage
import aphrodite.omni.benchmarks.patch.patch  # noqa: F401
from aphrodite.benchmarks.serve import main_async
from aphrodite.omni.benchmarks.omniinteract import omniinteract_output_lock
from aphrodite.omni.benchmarks.patch.patch import (
...
def _use_endpoint_backend_when_implicit(args: argparse.Namespace) -> None:
    ...
    if "backend" in explicit_keys:
        return
    # Upstream Sonar defaults --backend to "openai". Treat that non-explicit
