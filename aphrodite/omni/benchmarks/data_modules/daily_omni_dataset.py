...
Why ``BenchmarkDataset`` instead of ``HuggingFaceDataset``?
    Sonar's ``HuggingFaceDataset`` is a thin wrapper whose ``__init__`` always ends by calling
...
Usage:
    from aphrodite.omni.benchmarks.data_modules.daily_omni_dataset import DailyOmniDataset
...
try:
    from aphrodite.benchmarks.datasets import BenchmarkDataset, SampleRequest
except ImportError:
    # Fallback: if BenchmarkDataset not available, use base class from same module
    from aphrodite.benchmarks.datasets import HuggingFaceDataset as BenchmarkDataset
    from aphrodite.benchmarks.datasets import SampleRequest
from aphrodite.tokenizers import TokenizerLike
from aphrodite.tokenizers.hf import get_cached_tokenizer
...
def _extract_daily_omni_videos_tar(tar_path: Path, cache_key: str) -> Path:
    ...
    Cached under ``HF_HOME`` / ``aphrodite/omni/daily_omni_media/<cache_key>``. Reuses a previous
    extraction when the tarball fingerprint matches.
    """
    safe = cache_key.replace("/", "__").replace("\\", "_")
    staging_root = _daily_omni_hf_cache_root() / "aphrodite.omni" / "daily_omni_media" / safe
...
def ensure_daily_omni_hub_videos_dir(repo_id: str) -> Path:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install it (e.g. with Sonar) or provide --daily-omni-video-dir with a local extract."
        ) from e
...
class DailyOmniDataset(BenchmarkDataset):
    ...
    repo (env ``APHRODITE_DAILY_OMNI_MEDIA_REPO`` overrides the repo id; else ``dataset_path`` or
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #: Match Sonar ``HuggingFaceDataset`` / bench CLI ``--no-stream``.
    ...
    def _local_file_to_video_url_payload(self, video_path: Path) -> dict[str, Any]:
        ...
        Sonar rejects ``file://`` unless the server was started with
    ...
    def _lazy_ensure_hub_media_dir(self) -> None:
        ...
        if self.video_dir is not None:
            return
        repo = os.environ.get("APHRODITE_DAILY_OMNI_MEDIA_REPO", "").strip()
        if not repo:
    ...
    def _get_video_content(
        ...
        For ``file://`` URLs, start ``aphrodite serve`` with e.g.
    ...
    def _compose_minicpm_interleaved_multimodal(
        ...
        PIL frames and 1s audio ndarrays. Sonar Omni does not implement that expansion, so the
...
def load_daily_omni_dataset(
    ...
    **kwargs,
    ...
    Example:
        >>> from aphrodite.omni.benchmarks.data_modules.daily_omni_dataset import load_daily_omni_dataset
...
def get_daily_omni_statistics(
    ...
    Example:
        >>> from aphrodite.omni.benchmarks.data_modules.daily_omni_dataset import get_daily_omni_statistics
