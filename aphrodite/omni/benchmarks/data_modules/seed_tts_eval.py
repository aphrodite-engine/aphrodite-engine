...
Streaming PCM is decoded using ``APHRODITE_OMNI_BENCH_AUDIO_SAMPLE_RATE`` /
``APHRODITE_OMNI_BENCH_AUDIO_CHANNELS`` (default: 24 kHz mono).
"""
...
from aphrodite.omni.benchmarks.data_modules.seed_tts_dataset import SeedTTSSampleRequest
from aphrodite.omni.metrics.definitions import stream_pcm_format_from_env
...
def _missing_deps_message(lang: str) -> str | None:
    try:
        ...
        return f"Seed-TTS WER eval needs jiwer and zhon ({e!s}). Install: pip install 'sonar-omni[dev]'"
    try:
    ...
    if lang == "en":
        try:
            ...
            return f"English WER needs torch and transformers ({e!s}). Install: pip install 'sonar-omni[dev]'"
    else:
        try:
            import zhconv  # noqa: F401
            from funasr import AutoModel  # noqa: F401
        except ImportError as e:
            return f"Chinese WER needs funasr and zhconv ({e!s}). Install: pip install 'sonar-omni[dev]'"
    return None
