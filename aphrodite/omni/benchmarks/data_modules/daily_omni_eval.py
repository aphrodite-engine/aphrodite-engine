...
"""Daily-Omni multiple-choice accuracy scoring for Sonar Omni bench serve.
...
- Answer extraction defaults to the same rules as ``extract_choice_letter`` (strip after an
  ...
  ``DAILY_OMNI_EXTRACT_MODE=relaxed`` to use the older Sonar Omni heuristics (last ``answer:``,
  tail scan, etc.).
...
from aphrodite.omni.benchmarks.data_modules.daily_omni_dataset import DailyOmniSampleRequest
...
def _extract_predicted_choice_relaxed(text: str) -> str | None:
    """Legacy Sonar Omni heuristics (last ``answer:`` patterns, tail scan)."""
...
def compute_daily_omni_accuracy_metrics(
    ...
    **Denominators:** The open-source script excludes items that hit inference / I/O failures
    ...
    if include_per_item:
        metrics["daily_omni_eval_items"] = items
    return metrics
