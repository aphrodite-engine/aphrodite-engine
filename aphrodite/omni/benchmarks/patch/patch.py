...
from aphrodite.benchmarks import datasets
...
from aphrodite.omni.benchmarks.audio_continuity import compute_continuity_stats
from aphrodite.omni.benchmarks.data_modules.daily_omni_dataset import (
    DailyOmniDataset,
...
from aphrodite.omni.benchmarks.data_modules.omniinteract_dataset import (
...
from aphrodite.omni.benchmarks.data_modules.random_multi_modal_dataset import OmniRandomMultiModalDataset
from aphrodite.omni.benchmarks.data_modules.seed_tts_dataset import (
...
from aphrodite.omni.benchmarks.data_modules.sound_effect_dataset import SoundEffectDataset
from aphrodite.omni.benchmarks.data_modules.ttsd_dataset import TTSDDataset
from aphrodite.omni.benchmarks.omniinteract import (
    VIDEO_FPS,
...
from aphrodite.omni.metrics import definitions as defs
from aphrodite.omni.metrics.utils import coerce_bool, coerce_positive_float_scalar, coerce_positive_int_scalar
from aphrodite.tokenizers import TokenizerLike
from PIL import Image
from tqdm.asyncio import tqdm

if TYPE_CHECKING:
    from aphrodite.omni.clients.duplex import DuplexClient

# The duplex client library (aphrodite.omni.clients) is imported lazily inside the
# functions that drive Realtime sessions: this module is imported for its
# patching side effects by the CLI package (`sonar-omni serve` included), and
...
_AUDIO_CONTINUITY_THRESHOLD_ENV = "APHRODITE_OMNI_BENCH_AUDIO_CONTINUITY_THRESHOLD_S"
...
def _audio_continuity_threshold_s() -> float:
    ...
    Read from ``APHRODITE_OMNI_BENCH_AUDIO_CONTINUITY_THRESHOLD_S`` so users can
...
# Set from the ``--omni-request-timeout-s`` CLI flag by ``aphrodite bench serve``
...
def _attach_daily_omni_to_request_func_input(sample: SampleRequest, rfi: RequestFuncInput) -> None:
    ...
    if sample.omni_chat_messages is not None:
        rfi.omni_chat_messages = sample.omni_chat_messages
    else:
        rfi.mm_position = sample.omni_chat_mm_position
...
def _attach_seed_tts_to_request_func_input(sample: SampleRequest, rfi: RequestFuncInput) -> None:
    ...
    rfi.seed_tts_row = True
    sys_prompt = (sample.seed_tts_system_prompt or "").strip() or SEED_TTS_DEFAULT_OMNI_SYSTEM_PROMPT
    rfi.seed_tts_system_prompt = sys_prompt
    rfi.seed_tts_speech_extra = sample.seed_tts_speech_extra
    rfi.seed_tts_turns = sample.seed_tts_turns
    rfi.omni_chat_messages = [
...
def _attach_omniinteract_to_request_func_input(sample: SampleRequest, rfi: RequestFuncInput) -> None:
    if not isinstance(sample, OmniInteractSampleRequest):
        return
    rfi.omniinteract_case = sample.omniinteract_case
    rfi.omniinteract_options = sample.omniinteract_options
    rfi.omniinteract_prepared_input = sample.omniinteract_prepared_input
...
def _prepare_omniinteract_batch(input_requests: list[SampleRequest]) -> None:
    ...
    for sample in input_requests:
        ...
        if options is None or case is None:
            raise ValueError("OmniInteract requests require session options and a case")
...
def _daily_omni_repo_from_args(args) -> str | None:
    ...
    Sonar allows ``--dataset-path`` to be a local path while the real HF id is
...
def get_samples(args, tokenizer):
    ...
    if is_omniinteract:
        ...
        dataset = OmniInteractDataset(
            ...
            subsets=tuple(args.omniinteract_subsets),
            random_seed=args.seed,
            disable_shuffle=getattr(args, "disable_shuffle", False),
        )
        options = OmniInteractSessionOptions(
            output_root=Path(args.omniinteract_output_dir),
            timeout_s=float(args.omniinteract_timeout_s),
            media_timeout_s=float(args.omniinteract_media_timeout_s),
            ref_audio=str(args.omniinteract_ref_audio),
            require_response=bool(args.omniinteract_require_response),
            max_video_duration_s=float(args.omniinteract_max_video_duration_s),
        )
        ...
        from aphrodite.omni.clients.duplex import reference_audio_data_url
...
_serve_mod = sys.modules.get("aphrodite.benchmarks.serve")
        ...
        [
            {
                ...
                "itls_ms": stage0.get(defs.APHRODITE_ITLS_MS),
                "tpot_ms": stage0.get(defs.APHRODITE_TPOT_MS),
            }
        ],
...
async def async_request_openai_chat_omni_completions(
    ...
    payload = {
        ...
        "stream_options": {
            ...
            # NOTE: the Sonar StreamOptions field is "continuous_usage_stats",
            ...
            async with session.post(url=api_url, json=payload, headers=headers) as response:
                if response.status == 200:
                    ...
                    if response_format == "wav" and wav_pcm_buffer and wav_audio_params is not None:
                        ...
                        try:
                            from aphrodite.multimodal.audio import get_audio_duration
...
def _realtime_websocket_url(api_url: str) -> str:
    from aphrodite.omni.clients.duplex import build_realtime_url
...
async def _async_request_omniinteract(
    ...
    *,
    ...
    try:
        ...
        output.ttft = _metric_float(session_metrics.get("mean_ttft_ms")) / 1000.0
        output.audio_ttfp = _metric_float(session_metrics.get("mean_ttfp_ms")) / 1000.0
        output.audio_rtf = _metric_float(session_metrics.get("mean_rtf"))
        ...
        output.omniinteract_case = case
        output.omniinteract_case_result = case_result
    except Exception:
        output.success = False
        output.error = traceback.format_exc()
...
class _RealtimeTTSProbe:
    ...
    def __init__(self, url: str) -> None:
        from aphrodite.omni.clients.duplex import EventCollector
    ...
    async def configure(
        ...
        *,
        ...
        from aphrodite.omni.clients.duplex import AudioFormat, DuplexClient, SessionConfig
    ...
    async def acknowledge_playback(self) -> None:
        from aphrodite.omni.clients.duplex import acknowledge_collected_playback
...
async def async_request_openai_realtime_duplex(
    ...
    from aphrodite.omni.clients.duplex import summarize_session_request_metrics, wait_for_condition
    ...
    try:
        async with _RealtimeTTSProbe(_realtime_websocket_url(request_func_input.api_url)) as client:
            ...
            output.audio_rtf = _metric_float(session_metrics.get("mean_rtf"))
            output.audio_duration = (
                sum(_metric_float(metric.get("audio_duration_ms")) for metric in turn_metrics) / 1000.0
            )
...
from aphrodite.benchmarks import serve
from aphrodite.benchmarks.lib.ready_checker import wait_for_endpoint
from aphrodite.benchmarks.serve import TaskType, calculate_metrics_for_embeddings, get_request
from aphrodite.omni.benchmarks.metrics.metrics import (
...
def _merge_overrides(base: dict | None, overrides: dict | None) -> dict | None:
    ...
    Sonar 0.24 removed the private helper from ``aphrodite.benchmarks.serve``.
...
async def benchmark(
    ...
    if lora_modules:
        ...
        if lora_assignment == "round-robin":
            lora_iterator = iter([lora_modules_list[i % len(lora_modules_list)] for i in range(len(input_requests))])
        else:
            lora_iterator = iter([random.choice(lora_modules_list) for _ in range(len(input_requests))])
    ...
    # Ported from upstream Sonar v0.27.0 (aphrodite/benchmarks/serve.py), which added
    ...
    # Plain-Sonar backends (e.g. the Sonar-text perf config) return upstream
    ...
    from aphrodite.omni.benchmarks.data_modules.daily_omni_eval import (
    ...
    if _seed_tts_capture_pcm_for_wer():
        from aphrodite.omni.benchmarks.data_modules.seed_tts_eval import (
