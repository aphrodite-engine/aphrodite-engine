...
import aphrodite.omni.logger  # noqa: F401
from aphrodite.config import ModelConfig as _OriginalModelConfig
from aphrodite.inputs import TokensPrompt as _OriginalTokensPrompt
from aphrodite.model_executor.layers.rotary_embedding import (
    MRotaryEmbedding as _OriginalMRotaryEmbedding,
)
from aphrodite.omni.engine import OmniEngineCoreOutput, OmniEngineCoreOutputs, OmniEngineCoreRequest
from aphrodite.omni.inputs.data import OmniTokensPrompt
from aphrodite.omni.model_executor.layers.rotary_embedding import OmniMRotaryEmbedding
from aphrodite.omni.request import OmniRequest, OmniStreamingUpdate
from aphrodite.v1.engine import EngineCoreOutput as _OriginalEngineCoreOutput
from aphrodite.v1.engine import EngineCoreOutputs as _OriginalEngineCoreOutputs
from aphrodite.v1.engine import EngineCoreRequest as _OriginalEngineCoreRequest
from aphrodite.v1.request import Request as _OriginalRequest
from aphrodite.v1.request import RequestStatus
from aphrodite.v1.request import StreamingUpdate as _OriginalStreamingUpdate

_PATCH_LOGGER = logging.getLogger("aphrodite.omni.patch")
...
# (cond_token_attn_type: "joint_full" in config.json). Sonar gates this on
...
#
# WHY NOT model-level: is_mm_prefix_lm is checked in Sonar core (scheduler,
...
# original Sonar implementation unchanged.
#
...
# pydantic dataclass issue in aphrodite 0.19.0+. If Sonar changes
...
# pydantic dataclasses (aphrodite 0.19.0+).
...
setattr(_OriginalModelConfig, "is_mm_prefix_lm", _patched_cp)

# Sanity check: verify the patch is active. If Sonar changes the descriptor
...
assert _installed is _patched_cp, (
    ...
    "for HunyuanImage3 will not work. Check Sonar ModelConfig changes."
)
...
#
# WHY here (sonar-omni, not upstream): the root cause sits in ModelOpt; the
# analogous fix in Sonar's own PWAL is also pending. Both are planned as
# follow-up PRs. Until those land in sonar-omni's pinned Sonar, this load-time
# clamp keeps the user-visible failure from looking like a sonar-omni
...
# Sonar's own PWAL contains an in-place `masked_fill_` against `weight_scale`
# / `isnan` — the structure the upstream fix is expected to take when it is
# filed (planned as a follow-up PR after this one merges). Once sonar-omni's
# aphrodite pin moves to a release with that upstream fix, the override is
# skipped at import and this block can be deleted. NOTE: the heuristic only
# matches "Sonar PWAL clamps NaN in-place with `masked_fill_`"; if the
...
def _clamp_nvfp4_weight_scale_nans(layer) -> int:
    ...
    without monkey-patching + reloading the parent ``aphrodite.omni.patch``
    module.
    """
...
try:
    # Escape hatch — set APHRODITE_OMNI_SKIP_NVFP4_NAN_CLAMP=1 to skip installing
    ...
    if os.environ.get("APHRODITE_OMNI_SKIP_NVFP4_NAN_CLAMP", "").lower() in ("1", "true", "yes", "on"):
        raise ImportError("APHRODITE_OMNI_SKIP_NVFP4_NAN_CLAMP is set; skipping NaN-clamp install")
    from aphrodite.model_executor.layers.quantization.modelopt import (
        ModelOptLinearMethod as _OriginalModelOptNvFp4LinearMethod,
    )
    ...
    _original_nvfp4_pwal = getattr(_current_nvfp4_pwal, "_aphrodite_omni_wrapped_pwal", _current_nvfp4_pwal)
    ...
    def _patched_nvfp4_pwal(self, layer, *args, **kwargs):
        ...
        if self.spec.weight == kNvfp4Static and self.spec.activation == kNvfp4Dynamic:
    ...
    setattr(_patched_nvfp4_pwal, "_aphrodite_omni_wrapped_pwal", _original_nvfp4_pwal)

    if not _already_patched_upstream:
        setattr(_OriginalModelOptNvFp4LinearMethod, "process_weights_after_loading", _patched_nvfp4_pwal)
...
# implementation doesn't expose it in rope_parameters. Sonar's uses_mrope
...
for module_name, module in list(sys.modules.items()):
    # only do patch on module of aphrodite, pass others
    if "aphrodite" not in module_name:
        continue
    for name, original, replacement in (
        ("EngineCoreOutput", _OriginalEngineCoreOutput, OmniEngineCoreOutput),
        ("EngineCoreOutputs", _OriginalEngineCoreOutputs, OmniEngineCoreOutputs),
        ("TokensPrompt", _OriginalTokensPrompt, OmniTokensPrompt),
        ("MRotaryEmbedding", _OriginalMRotaryEmbedding, OmniMRotaryEmbedding),
        ("Request", _OriginalRequest, OmniRequest),
        ("StreamingUpdate", _OriginalStreamingUpdate, OmniStreamingUpdate),
        ("EngineCoreRequest", _OriginalEngineCoreRequest, OmniEngineCoreRequest),
    ):
        if getattr(module, name, None) == original:
            setattr(module, name, replacement)


# Patch: add qwen3_omni_moe to aphrodite's chat template fallback registry.
...
# aphrodite's resolve_chat_template falls back to MODEL_TYPE_TO_CHAT_TEMPLATE
# which has "qwen" but not "qwen3_omni_moe".  Register the same fallback.
def _patch_chat_template_registry():
    try:
        from aphrodite.transformers_utils.chat_templates.registry import (
...
def _patch_scaled_mm_fp8_contiguous_activation():
    ...
    """
    try:
        from aphrodite.model_executor.kernels.linear.scaled_mm.ScaledMMLinearKernel import (
...
def _patch_flashinfer_fp8_scaled_mm_output_shape():
    ...
    becomes a harmless no-op once Sonar honors ``output_shape`` itself.
    """
    try:
        from aphrodite.model_executor.kernels.linear.scaled_mm.flashinfer import (
...
def _patch_fp8_use_quack_fused_bias():
    try:
        from aphrodite.omni.quantization.quack_fp8 import install_quack_fp8_patch
...
def _patch_inductor_factorable_divisibility():
    ...
    if getattr(original, "_aphrodite_omni_factorable_divisibility", False):
        return
    ...
    statically_known_multiple_of._aphrodite_omni_factorable_divisibility = True
...
def _patch_cumem_free_callback_cuda() -> None:
    ...
    def _patched_free_callback(self, ptr: int) -> tuple:
        ...
        if data.is_asleep:
            ...
            result: tuple[int, int, int, list[int]] = (device, size, d_mem, [])
            _PATCH_LOGGER.debug(
    ...
    setattr(_patched_free_callback, "_omni_cumem_cuda_patched", True)
    setattr(CuMemAllocator, "_python_free_callback", _patched_free_callback)
    _PATCH_LOGGER.info(
