...
from aphrodite.omni.distributed.omni_connectors.factory import OmniConnectorFactory
from aphrodite.omni.distributed.omni_connectors.utils.config import (
    ConnectorSpec,
    get_stage_connector_role,
)
from aphrodite.omni.outputs import OmniConnectorOutput, OmniModelRunnerOutput
from aphrodite.omni.worker.payload_span import (
    get_tensor_span,
    merge_tensor_spans,
)
...
if TYPE_CHECKING:
    ...
    from aphrodite.omni.distributed.omni_connectors.connectors.base import (
        OmniConnectorBase,
    )
    from aphrodite.omni.distributed.omni_connectors.kv_transfer_manager import (
...
class OmniConnectorModelRunnerMixin:
    ...
    def init_omni_connectors(
        ...
        if self._kv_transfer_manager is not None:
            ...
            source_tp, target_tp, local_rank = effective_mapping
            if isinstance(source_tp, int) and isinstance(target_tp, int) and isinstance(local_rank, int):
                rank_cfg = {
                    "from_tp": source_tp,
                    "to_tp": target_tp,
        ...
        # ownership.
        self._local_stage_payload_cache: dict[str, OmniPayload] = {}
    ...
    def prune_inactive_requests(self, active_req_ids: Any) -> set[str]:
        ...
        for attr_name in (
            ...
            if isinstance(state, dict) or isinstance(state, set):
    ...
    def _extract_scheduling_metadata(cls, payload: OmniPayload) -> dict[str, Any]:
        ...
        if "next_stage_prompt_len" in meta:
            ...
            extracted["next_stage_prompt_len"] = payload.get("next_stage_prompt_len")
    ...
    def _get_model_config(self) -> Any:
        ...
        if model_config is not None:
            return model_config
        return getattr(getattr(self, "aphrodite_config", None), "model_config", None)
    ...
    def send_chunk(
        ...
        if not isinstance(raw_req_id, str):
            raise ValueError("Chunk requests must have a string request_id or req_id")
    ...
    def attach_omni_connector_output(self, result: Any | None) -> Any:
        ...
        from aphrodite.v1.outputs import EMPTY_MODEL_RUNNER_OUTPUT, ModelRunnerOutput

        base_output = result if result is not None else EMPTY_MODEL_RUNNER_OUTPUT
        if isinstance(base_output, OmniModelRunnerOutput):
            wrapped = copy(base_output)
        else:
            wrapped = OmniModelRunnerOutput(
                **{field.name: getattr(base_output, field.name) for field in fields(ModelRunnerOutput)}
            )
    ...
    def get_kv_remote_ranks(self) -> list[int]:
        ...
        Follows Sonar's ``TpKVTopology.get_target_remote_ranks()`` pattern:
