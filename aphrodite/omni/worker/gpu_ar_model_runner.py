...
"""AR GPU Model Runner for Sonar Omni.
...
from aphrodite.omni.utils.mm_outputs import (
    build_mm_cpu,
    partition_payload_list,
    snapshot_mm_payload,
)
from aphrodite.omni.worker.gpu_model_runner import OmniGPUModelRunner
from aphrodite.omni.worker.omni_connector_model_runner_mixin import (
    OmniConnectorModelRunnerMixin,
    needs_omni_connector,
)
from aphrodite.omni.worker.output.payload_build import build_omni_mm_payload
from aphrodite.omni.worker.runner_assisted_metadata import RunnerAssistedFullAttentionMetadataRequest
from aphrodite.omni.worker.sampling_utils import clamp_prompt_ids_to_penalty_padding, sanitize_min_tokens_stop_ids
from aphrodite.omni.worker.sparse_audio import resolve_sparse_mm_routing
...
class OmniAsyncGPUModelRunnerOutput(AsyncGPUModelRunnerOutput):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._omni_model_runner_output: OmniModelRunnerOutput | None = None
    ...
    def _build_model_runner_output_once(self) -> None:
        if self._omni_model_runner_output is not None:
            return
        ...
        with record_function_or_nullcontext("omni_async_output:get_output/build_model_runner_output"):
            self._omni_model_runner_output = self._model_runner_output_builder()
            self._model_runner_output = self._omni_model_runner_output
        self._model_runner_output_builder = None

    def _build_output_in_background(self) -> None:
        try:
            if self._cuda_device is not None:
                torch.accelerator.set_device_index(self._cuda_device)
...
class GPUARModelRunner(OmniGPUModelRunner, OmniConnectorModelRunnerMixin, DuplexSamplingRunnerMixin):
    ...
    def _build_model_sampler_output_token_ids(self) -> list[list[int]]:
        ...
        Sonar only populates sampling_metadata.output_token_ids when penalties or
    ...
    def _resolve_pooler_payload_req_ids(self, req_ids_output_copy: list[str]) -> tuple[str, list[str]]:
        ...
        engine_output_type = (cast(OmniModelConfig, self.model_config).engine_output_type or "").lower()
        if self._client_multimodal_output_keys() or engine_output_type == "audio" and not downstream_req_ids:
    ...
    def execute_model(
        ...
        with (
            ...
            from aphrodite.v1.kv_cache_interface import EncoderOnlyAttentionSpec
    ...
    def _build_multimodal_outputs(
        ...
        """
        if (
            cast(OmniModelConfig, self.model_config).engine_output_type == "text"
    ...
    def _should_return_omni_routed_experts(self) -> bool:
        ...
        if model_config is None:
            model_config = getattr(getattr(self, "aphrodite_config", None), "model_config", None)
        ...
        *,
        ...
        with record_function_or_nullcontext("omni_async_output:snapshot_cpu_payload"):
            async_payload_snapshot = _snapshot_tensor_payload_to_cpu_async(
                ...
                # NOTE: Sonar v0.24.0's GPUModelRunner no longer exposes a
