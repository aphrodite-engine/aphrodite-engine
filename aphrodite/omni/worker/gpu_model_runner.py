...
from aphrodite.model_executor.models.interfaces import SupportsMRoPE, supports_mrope
from aphrodite.model_executor.models.interfaces_base import AphroditeModelForPooling
from aphrodite.omni.core.prefix_cache import OmniTensorPrefixCache
from aphrodite.omni.engine.serialization import deserialize_additional_information
from aphrodite.omni.model_executor.layers.rotary_embedding.mrope import OmniMRotaryEmbedding as MRotaryEmbedding
from aphrodite.omni.model_executor.models.output_templates import OmniOutput
...
from aphrodite.v1.worker.gpu_model_runner import GPUModelRunner, PerLayerAttnMetadata
from aphrodite.v1.worker.ubatch_utils import maybe_create_ubatch_slices

if TYPE_CHECKING:
    from aphrodite.v1.core.sched.output import SchedulerOutput
    from aphrodite.v1.outputs import RoutedExpertsLists
else:
...
@dataclass
class OmniCachedRequestState(CachedRequestState):
    prompt_embeds_cpu: torch.Tensor | None = None
    additional_information_cpu: dict[str, Any] | None = None


def _filter_mrope_kwargs_for_model(model: SupportsMRoPE, kwargs: dict[str, Any]) -> dict[str, Any]:
    """Return only M-RoPE kwargs accepted by the model implementation."""
    method = model.get_mrope_input_positions
    try:
...
class OmniGPUModelRunner(GPUModelRunner):
    ...
    def _init_talker_mtp(self) -> None:
        ...
        if cudagraph_mode.has_full_cudagraphs() and talker_mtp_graph_safe:
            ...
            self.talker_mtp = graph_wrapper_cls(talker_mtp, self.aphrodite_config, runtime_mode=CUDAGraphMode.FULL)
        # TTS exposes mtp_hidden_size; Omni uses hf_text_config.hidden_size.
        hidden_size = int(getattr(self.model, "mtp_hidden_size", 0) or self.model_config.hf_text_config.hidden_size)
    ...
    def _prewarm_attention_capture_workspaces(self) -> None:
        ...
        if not capture_sizes:
            return
        from aphrodite.omni.attention.fish_kvcache_backend import prewarm_fish_kvcache_attn_capture_workspaces
    ...
    def _maybe_attach_attention_metadata_extensions(
        ...
        *,
        ...
        from aphrodite.omni.attention.fish_kvcache_backend import maybe_attach_fish_kvcache_seq_lens_upper_bound
    ...
    def _build_model_sampler_output_token_ids(self) -> list[list[int]]:
        ...
        Sonar only populates ``sampling_metadata.output_token_ids`` when penalties
    ...
    def _init_mrope_positions(self, req_state: CachedRequestState):
        ...
        Note:
            Upstream Sonar (commit 470229c37) added a fallback for the case
        ...
        for mm_feature in req_state.mm_features:
            ...
            if (t := mm_input.get("second_per_grid_ts")) is not None:
                assert isinstance(t, (torch.Tensor, float, int))
                second_per_grid_ts.append(float(t))
            ...
            if use_audio_in_video_value is not None:
                assert isinstance(use_audio_in_video_value, torch.Tensor)
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> Callable | None:
        ...
        for new_req_data in scheduler_output.scheduled_new_reqs:
            ...
            req_state = OmniCachedRequestState(
                req_id=req_id,
            ...
            try:
                ...
                if pe_cpu is not None:
                    req_state.prompt_embeds_cpu = pe_cpu
                    new_req_data.prompt_embeds = pe_cpu
            ...
            try:
                ...
                if info_payload is not None:
                    ...
                    if info_dict:
                        ...
                        req_state.additional_information_cpu = info_dict
            except Exception as e:
    ...
    def _dummy_run(
        ...
        Args:
            ...
            randomize_inputs: If True, randomize dummy input ids to balance
                expert selection. Sonar's kernel_warmup passes this when
    ...
    def _decode_and_store_request_payloads(
        ...
        for nr in new_reqs:
            ...
            if pe_cpu is not None:
                cast(OmniCachedRequestState, self.requests[req_id]).prompt_embeds_cpu = pe_cpu
            ...
            if info_dict:
                self.model_intermediate_buffer[req_id] = info_dict
                cast(OmniCachedRequestState, self.requests[req_id]).additional_information_cpu = info_dict
    ...
    def _preprocess(
        ...
        Note:
            Upstream Sonar (commit c621af169) added a conditional in the
        ...
        if hasattr(self.model, "has_preprocess") or hasattr(self.model, "enable_update_additional_information"):
            ...
            if not getattr(self.aphrodite_config.model_config, "async_chunk", False):
        ...
        if hasattr(self.model, "has_preprocess") and self.model.has_preprocess:
            ...
            for req_index, req_id in enumerate(self.input_batch.req_ids):
                ...
                req_infos["duplex_prompt_len"] = req_state.num_prompt_tokens if req_state is not None else None
                ...
                # reproducible: Sonar's own sampler seeding does not reach it.
    ...
    def _talker_mtp_forward(
        ...
        subtalker_params = getattr(self.aphrodite_config.model_config, "subtalker_sampling_params", None)
        ...
        with current_omni_platform.set_forward_context(
            None, self.aphrodite_config, cudagraph_runtime_mode=_cudagraph_mode, batch_descriptor=batch_desc
        ):
    ...
    def _update_intermediate_buffer(self, req_id: str, upd: dict) -> None:
        ...
        cast(OmniCachedRequestState, req_state).additional_information_cpu = existing
    ...
    def _replace_intermediate_buffer(self, req_id: str, upd: dict) -> None:
        ...
        cast(OmniCachedRequestState, req_state).additional_information_cpu = snapshot
    ...
    def _update_streaming_input_additional_info(self, new_req_data, req_id):
        ...
        self.requests[req_id].additional_information_cpu = merged_info
