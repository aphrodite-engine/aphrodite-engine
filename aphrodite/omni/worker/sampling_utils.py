...
from aphrodite.v1.sample.logits_processor import LogitsProcessors, MinTokensLogitsProcessor
...
def clamp_prompt_ids_to_penalty_padding(prompt_token_ids: torch.Tensor, logits_vocab: int) -> torch.Tensor:
    ...
    penalties (aphrodite/model_executor/layers/utils.py::
...
def sanitize_min_tokens_stop_ids(logitsprocs: LogitsProcessors, logits_vocab: int) -> None:
    ...
    Sonar's input processor unconditionally folds the stage tokenizer's EOS id
    ...
    for proc in logitsprocs.non_argmax_invariant:
        ...
        for _, _, stop_tok_ids, _ in min_toks.values():
            oob = [tok for tok in stop_tok_ids if tok < 0 or tok >= logits_vocab]
            if not oob:
                continue
