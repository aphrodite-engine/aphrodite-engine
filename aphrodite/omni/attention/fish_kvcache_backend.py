...
from aphrodite.omni.attention.fish_kvcache_attn import (
...
def _forward_with_fish_kvcache(
    ...
    if attn_metadata is not None and not attn_metadata.use_cascade:
        ...
        # Sonar >=0.23.0 lays out the KV cache as
        ...
        if can_use:
            assert dispatch_max_seq_len is not None
...
def install_fish_kvcache_attn_backend(model: Any) -> int:
    ...
    if not is_available():
        ...
        if is_fish_kvcache_attn_required():
            raise RuntimeError(
                f"APHRODITE_OMNI_FISH_KVCACHE_ATTN=required but Fish kvcache attention is unavailable: {load_error()!r}"
            )
        logger.warning(
