...
from aphrodite.platforms import Platform
from aphrodite.platforms.interface import PlatformEnum
...
class OmniPlatform(Platform):
    """
    Abstract base class for sonar-omni Platform.

    Inherits from Sonar's Platform and adds Omni-specific interfaces.
    This gives OmniPlatform all Sonar Platform capabilities plus
    ...
    def get_diffusion_worker_cls(cls) -> str:
        ...
        """
        return "aphrodite.omni.diffusion.worker.diffusion_worker.DiffusionWorker"
    ...
    def get_diffusion_model_runner_cls(cls) -> str:
        ...
        """
        return "aphrodite.omni.diffusion.worker.diffusion_model_runner.DiffusionModelRunner"
    ...
    def build_diffusion_kv_attn_metadata(cls, **kwargs: Any) -> dict[str, Any]:
        ...
        This default is the GPU/common path and uses Sonar's builder. NPU
        overrides it to build Ascend attention metadata without making the
        shared diffusion adapter import ``aphrodite_ascend``. ``seq_lens_cpu`` is an
    ...
    @classmethod
    def init_diffusion_worker_aphrodite_config(
        cls,
        aphrodite_config: Any,
    ) -> None:
        """Initialize platform-specific state for diffusion worker AphroditeConfig."""
        return None
    ...
    def get_profiler_cls(cls) -> str:
        ...
        """
        return "aphrodite.omni.profiler.omni_torch_profiler.OmniTorchProfilerWrapper"

    @classmethod
    def get_graph_wrapper_cls(cls) -> type:
        """Return the platform's full-graph wrapper class.

        Defaults to Sonar's CUDAGraphWrapper; NPU overrides with ACLGraphWrapper.
        """
    ...
    def set_forward_context(
        ...
        *,
        ...
        Defaults to Sonar's ``set_forward_context``; NPU overrides to dispatch
