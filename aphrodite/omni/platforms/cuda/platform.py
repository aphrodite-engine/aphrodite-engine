...
from aphrodite.platforms.cuda import CudaPlatformBase
...
class CudaOmniPlatform(OmniPlatform, CudaPlatformBase):
    ...
    Inherits all CUDA-specific implementations from Sonar's CudaPlatform,
    ...
    def has_flash_attn_4(cls) -> bool:
        ...
        from aphrodite.omni.diffusion.attention.backends.utils.fa import is_flash_attn_4_available
    ...
    def get_diffusion_attn_backend_cls(
        ...
        from aphrodite.omni.diffusion.envs import PACKAGES_CHECKER
        ...
        # "no kernel image". FA4 (flash_attn.cute / sonar-omni[fa4]) is the
        ...
        if selected_backend is not None:
            ...
            if backend_upper == "FLASH_ATTN" and not flash_attn_supported:
                if is_blackwell:
                    reason = (
                        ...
                        "(flash_attn.cute / sonar-omni[fa4]); FA2/FA3 kernels are Hopper-only"
                    )
            ...
            if backend_upper == "FLASHINFER_ATTN":
                from aphrodite.omni.diffusion.attention.backends.flashinfer_attn import FlashInferAttentionBackend
            ...
            if backend_upper == "CUDNN_ATTN":
                from aphrodite.omni.diffusion.attention.backends.cudnn_attn import CuDNNAttentionBackend
    ...
    def get_default_ir_op_priority(cls, aphrodite_config: AphroditeConfig) -> IrOpPriorityConfig:
        """Prefer ``aphrodite_c`` CUDA kernels over ``native`` for diffusion IR ops."""
        default = ["aphrodite_c", "native"]
