...
from aphrodite_ascend.attention.utils import using_paged_attention
from aphrodite_ascend.ops.rotary_embedding import update_cos_sin
from aphrodite_ascend.utils import enable_sp, lmhead_tp_enable
from aphrodite_ascend.worker.model_runner_v1 import SEQ_LEN_WITH_MAX_PA_WORKSPACE

import aphrodite.v1.worker.mamba_utils as mamba_utils
...
from aphrodite.logger import logger
from aphrodite.omni.core.sched.output import OmniCachedRequestData
from aphrodite.omni.outputs import OmniModelRunnerOutput
from aphrodite.omni.platforms.npu.worker.npu_ar_model_runner import ExecuteModelState, _ensure_tensor_values
from aphrodite.omni.platforms.npu.worker.npu_model_runner import NPUModelRunner, OmniNPUModelRunner
from aphrodite.omni.utils.mm_outputs import partition_payload_list
from aphrodite.omni.worker.mixins import maybe_unpad_input_ids
from aphrodite.omni.worker.omni_connector_model_runner_mixin import (
...
from aphrodite.v1.worker.gpu_model_runner import AsyncGPUModelRunnerOutput, PerLayerAttnMetadata
from aphrodite.v1.worker.ubatch_utils import maybe_create_ubatch_slices
...
class NPUGenerationModelRunner(OmniNPUModelRunner, OmniConnectorModelRunnerMixin):
    """Generation model runner for Sonar-omni on NPU (non-autoregressive)."""

    omni_execute_model_state: ExecuteModelState | None

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.omni_execute_model_state = None
    ...
    def _update_request_states(self, scheduler_output: SchedulerOutput):
        ...
        if not isinstance(cached_reqs, OmniCachedRequestData):
            raise TypeError("Async chunk generation requires OmniCachedRequestData")
        req_states = []
    ...
    def execute_model(
        ...
        if self.aphrodite_config.model_config.enable_return_routed_experts:
        ...
        if self.omni_execute_model_state is not None:
        ...
        with record_function_or_nullcontext("prepare input"):
            #  -------------------------------------- Omni-new -------------------------------------------------
            if self._async_chunk and num_scheduled_tokens:
            ...
            with self.synchronize_input_prep():
                ...
                if num_scheduled_tokens <= 0:
                    ...
                    return self.attach_omni_connector_output(
                        self.kv_connector_no_forward(scheduler_output, self.aphrodite_config)
                    )
                ...
                # '_update_states_after_model_execute', which is not overridden in Sonar-Ascend.
                # We simply utilize the implementation in Sonar.
                ...
                (attn_metadata, spec_decode_common_attn_metadata) = NPUModelRunner._build_attention_metadata(
                    self,
                    num_tokens=num_tokens_unpadded,
        ...
        with (
            ...
            self.omni_execute_model_state = ExecuteModelState(
                scheduler_output,
                None,
                spec_decode_metadata,
        ...
        if self.aphrodite_config.model_config.enable_return_routed_experts and hasattr(self, "_positions_cpu"):
    ...
    def sample_tokens(
        ...
        if self.omni_execute_model_state is None:
        ...
        if self._async_chunk:
            ...
            inter_stage_outputs = [payload for payload in per_req_payloads]
        ...
        if self.aphrodite_config.model_config.enable_return_routed_experts and hasattr(
    ...
    def _dummy_run(
        ...
        if create_mixed_batch:
            raise NotImplementedError(
                "create_mixed_batch is used for warmup deepgemm, aphrodite-ascend does not need it"
            )
        elif uniform_decode:
        ...
        # aphrodite-ascend does not support ubatch now
        ...
        with self.synchronize_input_prep():
            ...
            if self._should_build_dummy_attn_metadata(force_attention, is_profile, cudagraph_runtime_mode):
                ...
                if self.speculative_config and self.speculative_config.method == "mtp":
                    ...
                    if self.aphrodite_config.model_config.use_mla:
                ...
                if profile_seq_lens is not None:
                    ...
                    seq_lens = (
                        ...
                        if is_graph_capturing and using_paged_attention(num_tokens, self.aphrodite_config)
                        else max_query_len
                ...
                attn_metadata, _ = NPUModelRunner._build_attention_metadata(
                    self,
                    num_tokens=num_tokens_unpadded,
                    num_tokens_padded=num_tokens_padded,
                    num_reqs=num_reqs,
        ...
        with self.maybe_dummy_run_with_lora(
            ...
            # embeddings) even during the dummy profiling run that Sonar uses to
            ...
            drafter_dummy_run = getattr(self.drafter, "dummy_run", None)
            if callable(drafter_dummy_run) and not profile_cpp:
                drafter_dummy_run(
                    num_tokens=num_tokens_padded,
