...
from aphrodite_ascend.attention.utils import AscendCommonAttentionMetadata
...
from aphrodite_ascend.utils import enable_sp, global_stream
from aphrodite_ascend.worker.model_runner_v1 import graph_capture
...
from aphrodite.omni.data_entry_keys import flatten_payload
from aphrodite.omni.distributed.omni_connectors.kv_transfer_manager import OmniKVTransferManager
from aphrodite.omni.distributed.omni_connectors.utils.config import stage_sends_async_output
from aphrodite.omni.model_executor.duplex_sampling import DuplexSamplingRunnerMixin
...
from aphrodite.omni.utils.mm_outputs import build_mm_cpu, partition_payload_list, to_payload_element
...
class NPUARModelRunner(OmniNPUModelRunner, OmniConnectorModelRunnerMixin, DuplexSamplingRunnerMixin):
    ...
    omni_execute_model_state: ExecuteModelState | None = None
    sampling_done_event: torch.npu.Event | None
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Initialize KV cache manager (preserve aphrodite_config fallback behavior)
        self.kv_transfer_manager = OmniKVTransferManager.from_aphrodite_config(self.aphrodite_config, self.model_config)
    ...
    def _make_buffer(self, *size, dtype, numpy=True):
        ...
        from aphrodite.omni.distributed.ray_utils.utils import (
    ...
    def _build_multimodal_outputs(
        ...
        if getattr(self.model_config, "engine_output_type", None) == "text":
            return None
    ...
    def _resolve_pooler_payload_req_ids(self, req_ids_output_copy: list[str]) -> tuple[str, list[str]]:
        ...
        engine_output_type = (getattr(self.model_config, "engine_output_type", None) or "").lower()
    ...
    def execute_model(
            ...
            with self.synchronize_input_prep():
                    ...
                    with self.maybe_get_ec_connector_output(
                        ...
                        if kv_ids:
                            output = OmniModelRunnerOutput(
                                **{field.name: getattr(output, field.name) for field in fields(ModelRunnerOutput)},
                ...
                if num_scheduled_tokens <= 0:
                    ...
                    if not has_kv_transfer_group():
                        ...
                        output = self.kv_connector_no_forward(scheduler_output, self.aphrodite_config)
        ...
        with (
            ...
            hidden_states = self._model_forward(
                ...
                num_tokens_padded=num_tokens_padded, **model_kwargs
            )
    ...
    def sample_tokens(
        ...
        with record_function_or_nullcontext("draft_token"):
            ...
            # Sonar v0.18 defers KV connector finalization during target-model
        ...
        if self._async_chunk and stage_sends_async_output(self.model_config):
            ...
            pooler_inter = [payload for payload in pooler_output]
