...
import numpy as np
import torch
from aphrodite_ascend.ascend_forward_context import get_forward_context, set_ascend_forward_context
...
from aphrodite.omni.platforms.npu._310p import is_310p
...
from aphrodite.v1.worker.gpu_model_runner import PerLayerAttnMetadata

logger = init_logger(__name__)


if is_310p():
    from aphrodite_ascend._310p.model_runner_310p import NPUModelRunner310 as NPUModelRunner
else:
    from aphrodite_ascend.worker.model_runner_v1 import NPUModelRunner
...
class OmniNPUModelRunner(OmniGPUModelRunner, NPUModelRunner):
    ...
    def initialize_kv_cache(
        ...
        """
        if is_profiling:
            raise NotImplementedError("NPU Omni KV cache profiling is not supported")
        ...
        if self.omni_prefix_cache is None and self.cache_config.enable_prefix_caching:
            # Read num_blocks back off self.kv_cache_config: aphrodite-ascend
    ...
    def load_model(self, *args, **kwargs) -> None:
        if is_310p():
            from aphrodite.omni.platforms.npu._310p.patch import apply_model_patches

            apply_model_patches(self.model_config)
        NPUModelRunner.load_model(self, *args, **kwargs)
        # Initialize enable_sp cache to avoid get_current_aphrodite_config() error
        # in _pad_for_sequence_parallelism during execute_model.
        # This is a workaround for aphrodite-ascend not passing aphrodite_config to enable_sp().
        enable_sp(self.aphrodite_config)
        # ---------------------------------------Omni-new----------------------------------------------
    ...
    def _dummy_run(
        ...
        with self.maybe_dummy_run_with_lora(
            ...
            dummy_run = getattr(self.drafter, "dummy_run", None)
            if callable(dummy_run) and not profile_cpp:
                dummy_run(
                    num_tokens=num_tokens_padded,
    ...
    def _model_forward(
        ...
        **model_kwargs: Any,
        ...
        if num_tokens_padded is None:
            token_inputs = input_ids if input_ids is not None else inputs_embeds
            if token_inputs is None:
                raise ValueError("num_tokens_padded is required when no token inputs are provided")
            num_tokens_padded = token_inputs.shape[0]
    ...
    def _gather_runtime_additional_information(self):
        ...
        request id. The NPU runner inherits ``execute_model`` from aphrodite_ascend
