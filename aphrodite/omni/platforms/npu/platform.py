...
from aphrodite_ascend.platform import NPUPlatform
...
@cache
def _get_strict_ulysses_paged_backend() -> type[AttentionBackend]:
    """Return an Ascend backend that bypasses Sonar PCP dispatch."""

    from aphrodite_ascend.attention.attention_v1 import (
...
class NPUOmniPlatform(OmniPlatform, NPUPlatform):
    ...
    Inherits all NPU-specific implementations from aphrodite-ascend's NPUPlatform,
    ...
    def __init__(self) -> None:
        from aphrodite_ascend.utils import adapt_patch

        from aphrodite.omni.platforms.npu._310p import apply_patches as apply_310p_patches
        from aphrodite.omni.platforms.npu.models.minicpmo_4_5_code2wav import (
            apply_minicpmo_4_5_code2wav_patch,
        )
        from aphrodite.omni.platforms.npu.models.qwen3_tts_code2wav import (
            apply_qwen3_tts_code2wav_patch,
        )
        from aphrodite.omni.platforms.npu.models.qwen3_tts_tokenizer_v2 import (
    ...
    def set_device(cls, device: torch.device) -> None:
        ...
        # Register aphrodite_ascend custom ops (torch.ops._C_ascend.*).
        from aphrodite_ascend.utils import enable_custom_op
    ...
    def get_omni_ar_worker_cls(cls) -> str:
        return "aphrodite.omni.platforms.npu.worker.npu_ar_worker.NPUARWorker"

    @classmethod
    def get_omni_generation_worker_cls(cls) -> str:
        return "aphrodite.omni.platforms.npu.worker.npu_generation_worker.NPUGenerationWorker"

    @classmethod
    def init_diffusion_worker_aphrodite_config(cls, aphrodite_config: Any) -> None:
        from aphrodite_ascend.ascend_config import init_ascend_config
        from aphrodite_ascend.utils import adapt_patch

        # Omni's custom DiffusionWorker does not pass through Sonar-Ascend's
        ...
        adapt_patch()
        init_ascend_config(aphrodite_config)

    @classmethod
    def configure_diffusion_aphrodite_config(cls, aphrodite_config: Any, od_config: Any) -> None:
        ...
        if od_config.diffusion_kv_mode is not DiffusionKVCacheMode.PAGED_SCHEDULER:
            return
        from aphrodite_ascend.attention.attention_v1 import AscendAttentionBackend
        ...
        # Sonar's generic default is 16, while the Ascend FIA backend stores
        # cache pages as 128-token blocks. Set the Manager geometry
        # before KV specs are collected so Scheduler and Worker agree.
        aphrodite_config.cache_config.block_size = supported_sizes[0]
    ...
    @classmethod
    def get_diffusion_paged_kv_attn_backend(
        cls, attn_backend: type[AttentionBackend], *, ulysses_degree: int
    ) -> type[AttentionBackend]:
        """Keep strict Ulysses paged FIA out of Sonar's PCP implementation."""
    ...
    def get_diffusion_kv_block_tables_cls(cls) -> type:
        from aphrodite_ascend.worker.v2.block_table import AscendBlockTables
    ...
    def build_diffusion_kv_attn_metadata(cls, **kwargs: Any) -> dict[str, Any]:
        ...
        from aphrodite_ascend.attention.attention_v1 import AscendAttentionState
        from aphrodite_ascend.worker.v2.attn_utils import build_attn_metadata
    ...
    @classmethod
    def init_diffusion_model_runner_runtime(cls, aphrodite_config: Any, od_config: Any, device: torch.device) -> None:
        from aphrodite_ascend.ascend_forward_context import set_mc2_mask, set_mc2_tokens_capacity

        from aphrodite.omni.platforms.npu.models.minimax_h3 import (
        ...
        # construction races aphrodite/omni/__init__.py (patch before config) and
        ...
        apply_minimax_h3_qwen3vl_sdpa_patch()
        apply_minimax_h3_qwen3vl_swiglu_patch()
        set_mc2_tokens_capacity(aphrodite_config, od_config.max_num_seqs, 1)
        set_mc2_mask(aphrodite_config, device)
    ...
    def prepare_diffusion_op_runtime(cls, op_name: str, **kwargs: Any) -> None:
        ...
        from aphrodite.omni.platforms.npu.layers.fused_moe import prepare_fused_moe_runtime

        prepare_fused_moe_runtime()

    @classmethod
    def register_additional_diffusion_fused_moe_hooks(cls, moe_runner: Any) -> None:
        from aphrodite.omni.platforms.npu.layers.fused_moe import fused_moe_forward_context_pre_hook
    ...
    def reset_diffusion_fused_moe_forward_context(cls) -> None:
        from aphrodite.omni.platforms.npu.layers.fused_moe import reset_fused_moe_forward_context
    ...
    def get_diffusion_attn_backend_cls(
        ...
        if selected_backend is not None:
            ...
            if backend_upper in ("FLASH_ATTN", "RAINFUSION_ATTN") and find_spec("mindiesd"):
                ...
                # aphrodite-ascend custom op during model load/warmup). Import
    ...
    def get_profiler_cls(cls) -> str:
        return "aphrodite.omni.platforms.npu.profiler.NPUTorchProfilerWrapper"

    @classmethod
    def get_graph_wrapper_cls(cls) -> type:
        from aphrodite_ascend.compilation.acl_graph import ACLGraphWrapper
    ...
    def set_forward_context(
        ...
        *,
        ...
        from aphrodite_ascend.ascend_forward_context import set_ascend_forward_context
