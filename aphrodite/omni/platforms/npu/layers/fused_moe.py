...
import torch
from aphrodite_ascend.ascend_forward_context import MoECommType
from aphrodite_ascend.ops.fused_moe.moe_comm_method import _MoECommMethods
from aphrodite_ascend.utils import AscendDeviceType, get_ascend_device_type

import aphrodite.distributed.parallel_state as aphrodite_parallel_state
import aphrodite.forward_context as _aphrodite_fc
from aphrodite.config import AphroditeConfig
from aphrodite.distributed import get_dp_group, get_ep_group
from aphrodite.distributed.parallel_state import (
    init_model_parallel_group as aphrodite_init_model_parallel_group,
)
from aphrodite.omni.diffusion.distributed.parallel_state import (
    get_expert_parallel_group_ranks,
    get_world_group,
)
from aphrodite.omni.diffusion.forward_context import get_forward_context as omni_get_ctx


def _ensure_forward_context_attr(name: str, annotation: Any, default: Any) -> None:
    if name not in _aphrodite_fc.ForwardContext.__annotations__:
        _aphrodite_fc.ForwardContext.__annotations__[name] = annotation
    if not hasattr(_aphrodite_fc.ForwardContext, name):
        setattr(_aphrodite_fc.ForwardContext, name, default)


def _ensure_ascend_moe_customop_registered() -> None:
    """Register aphrodite-ascend's out-of-tree MoE layer implementations here.

    aphrodite-ascend registers its OOT layers from ``register_ascend_customop``,
    ...
    breaks outright: aphrodite-ascend's ``patch_fused_moe`` passes
    ...
    """
    from aphrodite_ascend.ops.fused_moe.fused_moe import AscendMoERunner
    from aphrodite_ascend.ops.fused_moe.routed_experts import AscendRoutedExperts

    from aphrodite.model_executor.custom_op import CustomOp, op_registry_oot
    ...
    for name, op_cls in ascend_moe_ops.items():
        # register_oot asserts on duplicates, so skip names aphrodite-ascend already
...
def _init_mc2_group_for_diffusion(
    ...
    import aphrodite_ascend.distributed.parallel_state as aphrodite_ascend_parallel_state

    if getattr(aphrodite_ascend_parallel_state, "_MC2", None) is not None:
        return
    aphrodite_ascend_parallel_state._MC2 = aphrodite_init_model_parallel_group(
        group_ranks,
        local_rank,
        backend,
        group_name="mc2",
...
def _select_moe_comm_method(aphrodite_config: AphroditeConfig) -> MoECommType | None:
    soc_version = get_ascend_device_type()
    if (
        not aphrodite_config.parallel_config.enable_expert_parallel
        or get_ep_group().world_size == 1
        or soc_version in {AscendDeviceType.A2}
    ):
...
def prepare_fused_moe_runtime() -> None:
    aphrodite_config = omni_get_ctx().aphrodite_config
    if aphrodite_config is None:
        raise RuntimeError("NPU MoE requires an active Aphrodite configuration")
    ...
    moe_comm_type = _select_moe_comm_method(aphrodite_config=aphrodite_config)
...
def reset_fused_moe_forward_context() -> None:
    try:
        forward_context = _aphrodite_fc.get_forward_context()
    ...
    setattr(forward_context, "max_tokens_across_dp", None)
    setattr(forward_context, "max_tokens_across_pcp", None)
...
def _set_max_tokens(forward_context: Any, hidden_states: torch.Tensor) -> None:
    ...
    pcp_group = getattr(aphrodite_parallel_state, "_PCP", None)
    ...
    forward_context.max_tokens_across_pcp = max(count for count in gathered_num_tokens if count is not None)
...
def fused_moe_forward_context_pre_hook(
    ...
    if aphrodite_config is None:
        raise RuntimeError("NPU MoE requires an active Aphrodite configuration")
    moe_comm_type = _select_moe_comm_method(aphrodite_config=aphrodite_config)
    setattr(forward_context, "moe_comm_type", moe_comm_type)
    setattr(forward_context, "moe_comm_method", _MoECommMethods.get(moe_comm_type))
