...
from aphrodite.omni.plugins import (
...
def cuda_omni_platform_plugin() -> str | None:
    ...
    return "aphrodite.omni.platforms.cuda.platform.CudaOmniPlatform" if is_cuda else None
...
def rocm_omni_platform_plugin() -> str | None:
    ...
    return "aphrodite.omni.platforms.rocm.platform.RocmOmniPlatform" if is_rocm else None
...
def npu_omni_platform_plugin() -> str | None:
    ...
    return "aphrodite.omni.platforms.npu.platform.NPUOmniPlatform" if is_npu else None
...
def xpu_omni_platform_plugin() -> str | None:
    ...
    try:
        ...
        if hasattr(torch, "xpu") and torch.xpu.is_available():
            is_xpu = True
            from aphrodite.omni.platforms.xpu import XPUOmniPlatform
    ...
    return "aphrodite.omni.platforms.xpu.platform.XPUOmniPlatform" if is_xpu else None
...
def musa_omni_platform_plugin() -> str | None:
    ...
    return "aphrodite.omni.platforms.musa.platform.MUSAOmniPlatform" if is_musa else None
...
def resolve_current_omni_platform_cls_qualname() -> str:
    ...
    if len(activated_oot_plugins) >= 2:
        ...
        platform_cls_qualname = "aphrodite.omni.platforms.interface.UnspecifiedOmniPlatform"
        logger.debug("No platform detected, Sonar Omni is running on UnspecifiedOmniPlatform")
