...
from aphrodite.omni.utils.mm_outputs import build_mm_cpu, to_payload_element
...
class OmniTensorPrefixCache:
    ...
    Sonar by leveraging the same slot mappings that the Sonar scheduler uses
    for the KV Cache.

    Conceptually, this means we are mapping Sonar's cache mapping:
    ...
    def _get_cached_block_ids(self, req_idx: int, input_batch: InputBatch) -> torch.Tensor:
        ...
        # NOTE: Sonar only caches full blocks
