...
from aphrodite.omni.core.sched.omni_scheduler_mixin import OmniSchedulerMixin
from aphrodite.omni.core.sched.utils import omni_routed_experts_for_request
from aphrodite.omni.engine import OmniEngineCoreOutput
from aphrodite.omni.engine.serialization import deserialize_additional_information
from aphrodite.omni.request import OmniRequest
from aphrodite.v1.core.sched.async_scheduler import AsyncScheduler as AsyncAPHRODITEScheduler
from aphrodite.v1.core.sched.output import SchedulerOutput
from aphrodite.v1.core.sched.request_queue import create_request_queue
from aphrodite.v1.core.sched.scheduler import Scheduler as APHRODITEScheduler
from aphrodite.v1.engine import EngineCoreEventType, EngineCoreOutput, EngineCoreOutputs
from aphrodite.v1.metrics.perf import PerfStats
...
from aphrodite.v1.structured_output.backend_types import StructuredOutputGrammar
...
class OmniARScheduler(OmniSchedulerMixin, APHRODITEScheduler):
    """Synchronous AutoRegressive scheduler for Sonar Omni. This class is also
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._omni_kv_config = getattr(self.aphrodite_config.model_config, "omni_kv_config", None)
        ...
        # KV-wait start ts for the aphrodite.omni:kv_wait_s metric; see
    ...
    def _get_omni_kv_config_value(self, key: str, default: Any = None) -> Any:
        ...
        if config is None and hasattr(self, "aphrodite_config"):
            config = getattr(self.aphrodite_config.model_config, "omni_kv_config", None)
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        # them. Upstream aphrodite raises RuntimeError on this status; omni allows
    ...
    def update_from_output(
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            request = cast(OmniRequest | None, self.requests.get(req_id))
            if request is not None:
                # Sonar 0.26: settle the in-flight tokens counted in schedule().
            ...
            # Sonar 0.27 (a0c092ee72) removed the async_tokens_to_discard
            ...
            if async_output_is_stale:
                ...
                setattr(request, "async_tokens_to_discard", max(0, stale_async_tokens - len(generated_token_ids)))
            ...
            if new_token_ids:
                ...
                if new_logprobs is not None and len(new_token_ids) < num_sampled_tokens:
                    ...
                    # with the emitted tokens, as upstream Sonar does by
            ...
            if new_token_ids and self.structured_output_manager.should_advance(request):
                ...
                assert isinstance(struct_output_request.grammar, StructuredOutputGrammar)
            ...
            if stopped:
                ...
                if not finished:
                    ...
                    if self.chunk_transfer_adapter:
                        if cast(OmniModelConfig, self.aphrodite_config.model_config).stage_id != 0:
                    ...
                    setattr(request, "async_tokens_to_discard", outstanding_async_tokens)
                ...
                elif status_before_stop == cast(RequestStatus, getattr(RequestStatus, "WAITING_FOR_CHUNK")):
            ...
            if self.chunk_transfer_adapter is not None and (
                ...
                self.chunk_transfer_adapter.save_async(
                    ...
                    segment_generation=boundary_generation if is_segment_finished else None,
                )
        ...
        if self.chunk_transfer_adapter is not None:
            for failed_request in failed_requests:
                self.chunk_transfer_adapter.cleanup_receiver(failed_request.request_id)
        ...
        if ec_connector is not None and ec_connector_output:
            ec_connector.update_connector_output(ec_connector_output)
    ...
    def _update_request_as_session(self, session: Request, update: StreamingUpdate) -> None:
        ...
        stage 0.
        """
        session = cast(OmniRequest, session)
        req_id = session.request_id
        self._new_prompt_len_snapshot[req_id] = len(update.prompt_token_ids or [])
        ...
        if outstanding_async_tokens > 0:
            ...
            setattr(session, "async_tokens_to_discard", 1)
            session.num_computed_tokens -= session.num_output_placeholders
            session.num_output_placeholders = 0
            session.spec_token_ids = []
        stage_id = cast(OmniModelConfig, self.aphrodite_config.model_config).stage_id
        ...
        if self.chunk_transfer_adapter and self.chunk_transfer_adapter.receives_chunks:
            if session.external_req_id is not None:
    ...
    def _free_request(
        ...
        request = cast(OmniRequest, request)
        ...
        inflight_prefills = getattr(self, "_inflight_prefills", None)
        ...
        if ec_connector is not None:
            ec_delay_free, ec_xfer_params = ec_connector.request_finished(request)
            ...
            if self._should_transfer_kv_for_request(request_id):
                ...
                if already_triggered:
                    ...
                    if request_id in self.requests_needing_kv_transfer:
                        ...
                        if add_info is not None and hasattr(add_info, "entries") and isinstance(add_info.entries, dict):
        ...
        if self._should_transfer_kv_for_request(req_id):
            ...
            try:
                ...
                if block_ids_tuple and len(block_ids_tuple) > 0:
                    ...
                    # Note: aphrodite_config might not be directly available, check scheduler_config or cache_config
...
class OmniARAsyncScheduler(OmniARScheduler, AsyncAPHRODITEScheduler):
