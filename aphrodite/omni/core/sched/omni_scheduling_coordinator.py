...
from aphrodite.omni.core.sched.output import OmniChunkRecvHandle
        ...
        for request in queue_snapshot:
            if request.status == RequestStatus.WAITING:
                ...
                if request.request_id in self.finished_requests:
                    continue
                request.status = cast(RequestStatus, getattr(RequestStatus, "WAITING_FOR_INPUT"))
            ...
            elif request.status == getattr(RequestStatus, "WAITING_FOR_INPUT"):
                if request.request_id in stage_recv_req_ids:
                    request.status = RequestStatus.WAITING
                    self._waiting_since.pop(request.request_id, None)
                else:
