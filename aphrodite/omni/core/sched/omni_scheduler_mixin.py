...
from aphrodite.distributed.kv_events import AllBlocksCleared, BlockRemoved, BlockStored, KVEventBatch
from aphrodite.distributed.kv_transfer.kv_connector.v1.metrics import KVConnectorStats
from aphrodite.logger import init_logger
from aphrodite.omni.core.sched.omni_scheduling_coordinator import (
    OmniSchedulingCoordinator,
    uses_full_payload_input_coordinator,
)
from aphrodite.omni.core.sched.output import (
    OmniChunkRecvHandle,
    OmniNewRequestData,
    OmniSchedulerOutput,
)
from aphrodite.omni.distributed.omni_connectors.transfer_adapter.chunk_transfer_adapter import (
...
from aphrodite.v1.core.sched.request_queue import RequestQueue
from aphrodite.v1.core.sched.utils import remove_all
...
if TYPE_CHECKING:
    from aphrodite.v1.core.sched.scheduler import Scheduler as _SchedulerBase
else:
...
# per-deployment via APHRODITE_OMNI_INPUT_WAIT_TIMEOUT_S; set exactly 0 to disable
...
_INPUT_WAIT_TIMEOUT_RAW = os.environ.get("APHRODITE_OMNI_INPUT_WAIT_TIMEOUT_S", "600")
try:
    DEFAULT_INPUT_WAIT_TIMEOUT_S: float = float(_INPUT_WAIT_TIMEOUT_RAW)
except ValueError:
    logger.warning(
        "Invalid APHRODITE_OMNI_INPUT_WAIT_TIMEOUT_S=%r; falling back to %s seconds.",
...
if not math.isfinite(DEFAULT_INPUT_WAIT_TIMEOUT_S):
    ...
    raise ValueError(
        f"APHRODITE_OMNI_INPUT_WAIT_TIMEOUT_S={_INPUT_WAIT_TIMEOUT_RAW!r} is not finite. "
...
if DEFAULT_INPUT_WAIT_TIMEOUT_S < 0:
    ...
    raise ValueError(
        f"APHRODITE_OMNI_INPUT_WAIT_TIMEOUT_S={_INPUT_WAIT_TIMEOUT_RAW!r} is negative. "
        "Use a positive number of seconds, or exactly 0 to disable the deadline."
    )
    ...
    logger.warning(
        "APHRODITE_OMNI_INPUT_WAIT_TIMEOUT_S=0: stage-input deadlines are DISABLED for "
...
class OmniSchedulerMixin(_SchedulerBase):
    ...
    def _init_omni_io_scheduling_state(self) -> None:
        ...
        self.chunk_transfer_adapter = (
            OmniChunkTransferAdapter(self.aphrodite_config) if getattr(model_config, "async_chunk", False) else None
        )
    ...
    def _maybe_decode_pooling_output(self, request: Request, pooler_output: Any) -> Any:
        ...
        return self._pooling_output_decoder(
            pooler_output,
            request,
            self.aphrodite_config.model_config.hf_config,
        )
    ...
    def _reset_streaming_session_replacement_state(self, session: Request) -> None:
        ...
        # (Sonar 0.27 a0c092ee72 removed async_tokens_to_discard). Seed in
    ...
    def _finish_streaming_session_update(self, session: Request, update: StreamingUpdate) -> None:
        ...
        omni_session = cast(OmniRequest, session)
        omni_update = cast(OmniStreamingUpdate, update)
        omni_session.additional_information = omni_update.additional_information or None
        omni_session.model_intermediate_buffer = getattr(
            update,
    ...
    def _reset_ready_async_chunk_replacements(self) -> None:
        ...
        for request_id in tuple(replaced_ids & ready_ids):
            ...
            watermark = getattr(adapter, "requests_num_chunks_sent", None)
            if watermark is not None:
                watermark.pop(cast(OmniRequest, request).external_req_id, None)
    ...
    def _process_pending_omni_inputs(self, model_mode: str) -> None:
        ...
        if self.chunk_transfer_adapter:
            self.chunk_transfer_adapter.process_pending_chunks(
                self.waiting,
                cast(list[OmniRequest], self.running),
                scheduler_requests=cast(dict[str, OmniRequest], self.requests),
            )
    ...
    def _process_pending_chunk_timeouts(self) -> None:
        ...
        Shares ``APHRODITE_OMNI_INPUT_WAIT_TIMEOUT_S`` with the full-payload net:
        ...
        *,
        ...
        if include_cached_payloads:
            self.chunk_transfer_adapter.postprocess_scheduler_output(
                scheduler_output,
                cast(dict[str, OmniRequest], self.requests),
            )
        else:
    ...
    def _resume_downstream_chunk_receiver(self, request: Request) -> None:
        ...
        if (
            ...
            or getattr(self.aphrodite_config.model_config, "session_mode", "turn") != "duplex"
    ...
    def _publish_kv_cache_events(self) -> None:
        ...
        if events:
            self.kv_event_publisher.publish(
                KVEventBatch(ts=time.time(), events=cast(list[BlockStored | BlockRemoved | AllBlocksCleared], events))
            )
    ...
    def _realign_request_status_to_queues(
        ...
        *,
        ...
        for rid in ids_to_align:
            ...
            # scheduler/connector queue. Sonar skips already-finished requests
    ...
    def _purge_finished_from_running(self, target_request_ids: set[str] | None = None) -> None:
        ...
        with idiomatic Sonar scheduler mutation; upstream
