...
from aphrodite.omni.diffusion.model_loader.host_weight_plan import HostWeightPlan, TensorBinding

logger = init_logger(__name__)

if TYPE_CHECKING:
    from aphrodite.omni.diffusion.data import DiffusionParallelConfig, OmniDiffusionConfig
    from aphrodite.omni.diffusion.model_loader.diffusers_loader import DiffusersPipelineLoader
    from aphrodite.omni.diffusion.model_loader.host_weights.identity_adapter import FinalLayoutIdentityContext
    from aphrodite.omni.diffusion.model_loader.host_weights.source_identity import (
        NodeSourceDigestCache,
        PreparedWeightSource,
    )
    from aphrodite.omni.host_weight_runtime import HostWeightRuntime
...
class HWRLoaderMixin:
    ...
    def _identity_fingerprint(value: object) -> str:
        from aphrodite.omni.host_weight_runtime import CanonicalJson
    ...
    def _prepare_hwr_sources(
        ...
        model: nn.Module,
        modules: object,
        sources: Sequence[DiffusersPipelineLoader.ComponentSource],
    ) -> tuple[PreparedWeightSource, ...]:
        from aphrodite.omni.diffusion.model_loader.host_weights import (
    ...
    def _build_hwr_context(
        ...
        *,
        ...
        contracts = [self._identity_value(dit.host_weight_restore_contract) for dit in getattr(modules, "dits", ())]
        loader_identity = FinalLayoutLoaderIdentity(
            implementation=ImplementationIdentity(
                implementation_id="sonar-omni.diffusion.diffusers-loader",
    ...
    def _resolve_hwr(
        ...
        *,
        ...
        from aphrodite.omni.host_weight_runtime import (
        ...
        from aphrodite.omni.host_weight_runtime.filesystem import detect_storage_class
        ...
        planned_prefixes = frozenset(
            prefix
            for binding in (target_bindings if isinstance(target_bindings, list) else ())
            if isinstance(binding, dict) and isinstance(prefix := binding.get("source_prefix"), str)
        )
    ...
    def load_fresh_canonical_model(self) -> nn.Module:
        ...
        try:
            return self.load_model(
                load_device=cast(str, request["load_device"]),
                load_format=cast("str | None", request["load_format"]),
                custom_pipeline_name=cast("str | type[nn.Module] | None", request["custom_pipeline_name"]),
                device=cast("torch.device | None", request["device"]),
            )
        finally:
    ...
    def _publish_hwr_after_load(self, model: nn.Module, modules: object, state: dict[str, object] | None) -> None:
        from aphrodite.omni.diffusion.model_loader.host_weights import FinalLayoutBF16Producer
        from aphrodite.omni.host_weight_runtime import PostLoadPublicationOutcome, ResolutionOutcome
    ...
    def _attach_offload_startup_state(self, model: nn.Module) -> None:
        ...
        if self.host_weight_plan is None:
            return
        from aphrodite.omni.diffusion.offloader.startup import OffloadStartupState, attach_offload_startup_state
