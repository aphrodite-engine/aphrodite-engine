...
from aphrodite.model_executor.layers.quantization.base_config import QuantizeMethodBase
...
from aphrodite.omni.diffusion.config import set_current_diffusion_config
from aphrodite.omni.diffusion.data import OmniDiffusionConfig
from aphrodite.omni.diffusion.distributed.hsdp import HSDPInferenceConfig, apply_hsdp_to_model
from aphrodite.omni.diffusion.lora.manager import LoRABackend
from aphrodite.omni.diffusion.model_loader.checkpoint_adapters import (
    get_checkpoint_adapter,
)
from aphrodite.omni.diffusion.model_loader.host_weight_loader import HWRLoaderMixin, _HWRCommitError
from aphrodite.omni.diffusion.model_loader.host_weight_plan import (
    HostWeightPlan,
    build_checkpoint_mmap_plan,
    has_online_quantization,
)
from aphrodite.omni.diffusion.models.diffusers_adapter.pipeline_diffusers_adapter import DiffusersAdapterPipeline
from aphrodite.omni.diffusion.offloader.component_utils import encoder_component_type
from aphrodite.omni.diffusion.offloader.config import (
    DIT_COMPONENT,
    TEXT_ENCODER_COMPONENT,
    OffloadStrategy,
    resolve_offload,
)
from aphrodite.omni.diffusion.offloader.module_collector import ModuleDiscovery
from aphrodite.omni.diffusion.offloader.offload_plan import get_offload_plan
from aphrodite.omni.diffusion.registry import initialize_model
from aphrodite.omni.model_executor.model_loader.weight_utils import download_weights_from_hf_specific
...
# download_gguf was removed from upstream Sonar (commit 6635279d8).
...
class DiffusersPipelineLoader(HWRLoaderMixin):
    ...
    def _prepare_weights(
        ...
        model_name_or_path = (
            maybe_download_from_modelscope(os.fspath(model_name_or_path), revision) or model_name_or_path
        )
    ...
    def _get_weights_iterator(
        ...
        if use_multithread:
            ...
            # Keep deterministic shard order before passing to Sonar helper.
    ...
    def _stream_online_quant_weights_to_cpu(
        ...
        Upstream Sonar's online layerwise loader materializes and quantizes a
        ...
        """
        from aphrodite.model_executor.model_loader.reload.layerwise import (
            get_layerwise_info,
        )
            ...
            if self.parallel_config.use_hsdp:
                ...
                if not self._force_canonical_load:
                    try:
                        ...
                        from aphrodite.omni.host_weight_runtime import RuntimeMode
    ...
    def _has_online_quant(model: nn.Module) -> bool:
        ...
        materialization onto the ``meta`` device (upstream Sonar
    ...
    def _unsupported_dlo_allgather_online_quant_methods(model: nn.Module) -> tuple[str, ...]:
        ...
        - online MXFP8: fp8 weight plus e8m0 block scale, either contiguous
          ...
          scale stored as a transposed view (Sonar kernel, .t() over a
        ...
        from aphrodite.omni.quantization.int8_config import (
            Int8OnlineLinearMethod,
            NPUInt8OnlineLinearMethod,
        )

        try:
            from aphrodite.omni.quantization.mxfp8_config import (
                AphroditeMxfp8OnlineLinearMethod,
            ...
            # MXFP8 requires a Sonar build with MXFP8 kernel support; treat it
    ...
    def _apply_skip_softmax_calibration(self, model: nn.Module) -> None:
        from aphrodite.omni.diffusion.attention.backends.trtllm_calibration import (
    ...
    def _process_weights_after_loading(self, model: nn.Module, target_device: torch.device) -> None:
        ...
        This handles Sonar's quantization methods that need to process weights
        after loading (e.g., FP8 online quantization from BF16/FP16 weights).
        """
        # Newer upstream Sonar online-quant methods (uses_meta_device=True) create
        ...
        # left untouched. Import lazily so older Sonar (no meta-device quant) is
        # unaffected.
        ...
        if has_online_quant:
            ...
            # model_config is only dereferenced by finalize for Sonar Attention /
        ...
        for _, module in model.named_modules():
            ...
            if has_online_quant:
                ...
                # processing and restore them afterward, mirroring upstream Sonar's
    ...
    def _load_model_with_hsdp(
        ...
        if self.quant_config is not None:
            from aphrodite.omni.diffusion.quantization.hsdp_fp8 import (
