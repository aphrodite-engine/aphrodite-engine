...
hit after the `transformers` v5 rebase (see Buildkite sonar-omni-rebase
...
``pipeline_qwen_image_edit_plus.py`` in build sonar-omni#7412 passes without
any prefetch), so the race is NOT a behavioural change in Sonar Omni itself.
Two environmental factors mask the race on main:

* ``origin/main`` is pinned (transitively, via Sonar main) to
...
* CI shares ``HF_HOME=/fsx/hf_cache`` across pipelines (both the
  ``sonar-omni`` and ``sonar-omni-rebase`` pipelines mount the same FS). That
...
# best-effort attempt left open (Buildkite sonar-omni-rebase #1858: both the
...
def _node_lock_dir() -> str:
    ...
    (``$HF_HOME/locks/sonar-omni-prefetch/``) so they share a filesystem -
    ...
    if hf_home:
        candidates.append(os.path.join(hf_home, "locks", "sonar-omni-prefetch"))
    xdg_cache = os.environ.get("XDG_CACHE_HOME")
    if xdg_cache:
        candidates.append(os.path.join(xdg_cache, "huggingface", "locks", "sonar-omni-prefetch"))
    candidates.append(os.path.join(os.path.expanduser("~"), ".cache", "huggingface", "locks", "sonar-omni-prefetch"))
    candidates.append(os.path.join("/tmp", "sonar-omni-prefetch-locks"))
    ...
    for cand in candidates:
        try:
            ...
            continue
    fallback = os.path.join("/tmp", "sonar-omni-prefetch-locks")
...
def _repo_prefetch_lock(model: str) -> Iterator[None]:
    ...
    * Multiple unrelated processes on the same node (concurrent CI jobs,
    ...
    try:
        ...
        if flock_held and fd is not None and fcntl is not None:
            with contextlib.suppress(OSError):
