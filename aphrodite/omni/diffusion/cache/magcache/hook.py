...
"""
Hook-based MagCache implementation for Sonar Omni.
...
from aphrodite.omni.diffusion.cache.magcache.state import MagCacheState
from aphrodite.omni.diffusion.cache.magcache.strategy import MagCacheStrategy, MagCacheStrategyRegistry
from aphrodite.omni.diffusion.hooks.base import HookRegistry, ModelHook, StateManager
...
class MagCacheHeadHook(ModelHook):
    ...
    def new_forward(self, module: torch.nn.Module, *args, **kwargs):
        ...
        if self.config.mag_calibrate:
            ...
            assert self.config.mag_ratios is not None
        ...
        if not should_compute:
            ...
            if isinstance(res, tuple):
                res = (res[0].to(hidden_states.device), res[1].to(hidden_states.device))
        ...
        if self._is_tail:
            ...
            if in_hidden is not None:
                if self._strategy is not None:
                    ...
                    assert not isinstance(in_hidden, tuple)
                    residual = out_hidden - in_hidden if out_hidden.shape == in_hidden.shape else out_hidden
    ...
    def new_forward(self, module: torch.nn.Module, *args, **kwargs):
        ...
        if self.is_tail:
            ...
            if self._strategy is not None:
                ...
                assert not isinstance(in_hidden, tuple)
                if (
                    out_hidden.shape == in_hidden.shape
                    or out_hidden.ndim == 3
                    and in_hidden.ndim == 3
                    and out_hidden.shape[2] == in_hidden.shape[2]
                ):
    ...
    def perform_calibration(
        ...
        if self._strategy is not None:
            ...
            if state.previous_residual is None:
                ...
                assert not isinstance(current_residual, tuple)
                assert not isinstance(state.previous_residual, tuple)
...
def apply_mag_cache_hook(
    ...
    if strategy is None:
        ...
        register_blocks = getattr(type(strategy), "register_blocks", None)
        if register_blocks is not None:
            register_blocks()
