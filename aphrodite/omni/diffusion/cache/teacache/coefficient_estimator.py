...
from aphrodite.omni.diffusion.cache.teacache.extractors import get_extractor
from aphrodite.omni.diffusion.data import OmniDiffusionConfig, TransformerConfig
from aphrodite.omni.diffusion.hooks import HookRegistry, ModelHook
...
class DataCollectionHook(ModelHook):
    ...
    def new_forward(self, module: torch.nn.Module, *args: Any, **kwargs: Any) -> Any:
        assert self.extractor_fn is not None
...
class LongCatAdapter(DefaultAdapter):
    """Adapter for LongCat Image - NOTE: currently this model needs the Sonar
    context to be correctly configured to actually run the estimation, since it
    uses Sonar norm layers etc.
    """
