...
from aphrodite.omni.diffusion.io_support import get_diffusion_output_type, supports_audio_output
...
def _metadata_from_legacy_payload(payload: Mapping[str, object]) -> DiffusionMetadata:
    ...
    if audio_sample_rate is not None:
        if not isinstance(audio_sample_rate, (int, float)):
            raise ValueError("audio_sample_rate must be numeric")
...
def format_diffusion_outputs(
    *,
    ...
    is_audio_output = od_config.model_class_name is not None and supports_audio_output(od_config.model_class_name)
    final_output_type = (
...
def _trajectory_payload(
    ...
    return cast(DiffusionTrajectoryPayload, trajectory)
