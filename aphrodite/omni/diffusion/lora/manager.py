...
from aphrodite.config.lora import LoRAConfig, MaxLoRARanks
...
from aphrodite.model_executor.layers.linear import MergedColumnParallelLinear, QKVParallelLinear
from aphrodite.omni.diffusion.lora.utils import (
    _expand_expected_modules_for_packed_layers,
    _match_target_modules,
    from_layer_diffusion,
)
from aphrodite.omni.lora.utils import stable_lora_int_id
...
class DiffusionLoRAManager:
    ...
    Reuses Sonar's LoRA infrastructure, adapted for diffusion pipelines.
    Uses LRU cache management similar to LRUCacheLoRAModelManager.
    """

    # Valid max allowed ranks for LoRA in Sonar
    _VALID_MAX_RANKS: list[MaxLoRARanks] = sorted(get_args(MaxLoRARanks))
    ...
    def __init__(
        ...
        Args:
            max_cached_adapters: Maximum number of LoRA adapters to keep in the
                CPU-side cache (LRU). This mirrors Sonar's `max_cpu_loras` and is
        ...
        # Sonar's get_supported_lora_modules() again would incorrectly yield
    ...
    def _compute_supported_lora_modules(self) -> set[str]:
        ...
        Sonar's get_supported_lora_modules() returns suffixes for LinearBase
    ...
    def _load_adapter(
        ...
        if loaded is None:
            ...
            lora_model = LoRAModel.from_local_checkpoint(
                ...
                device="cpu",  # consistent w/ aphrodite's behavior
                dtype=self.dtype,
    ...
    def _get_packed_modules_list(self, module: nn.Module) -> list[str]:
        """Return a packed_modules_list suitable for Sonar LoRA can_replace_layer().

        Diffusion transformers frequently use packed projection layers like
        QKVParallelLinear (fused QKV). Sonar's LoRA replacement logic relies on
    ...
    def _replace_layers_with_lora(self, peft_helper: PEFTHelper) -> None:
        ...
        lora_config = LoRAConfig(
            max_lora_rank=self._get_smallest_valid_max_rank(self._max_lora_rank),
            max_loras=1,
    ...
    @classmethod
    def _get_smallest_valid_max_rank(cls, min_rank: int) -> MaxLoRARanks:
