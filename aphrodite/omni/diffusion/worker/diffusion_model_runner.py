...
"""
Diffusion Model Runner for Sonar Omni.
...
from aphrodite.config import AphroditeConfig, LoadConfig
from aphrodite.logger import init_logger
from aphrodite.omni.diffusion.attention.layer import Attention
from aphrodite.omni.diffusion.cache.cachedit import cache_summary
from aphrodite.omni.diffusion.cache.prompt_embed_cache import (
    install_prompt_embed_cache,
    resolve_prompt_embed_cache_config,
)
from aphrodite.omni.diffusion.cache.selector import get_cache_backend
from aphrodite.omni.diffusion.compile import regionally_compile
from aphrodite.omni.diffusion.data import DiffusionOutput, OmniDiffusionConfig
...
from aphrodite.omni.diffusion.diffusion_kv.model_runner_backend import DiffusionKVModelRunnerBackend
from aphrodite.omni.diffusion.diffusion_kv.paged_attention_adapter import (
    DiffusionPagedAttentionMetadata,
    DiffusionPagedAttentionRow,
)
from aphrodite.omni.diffusion.distributed.parallel_state import get_classifier_free_guidance_rank
from aphrodite.omni.diffusion.forward_context import set_forward_context
from aphrodite.omni.diffusion.interaction.coordinator import InteractionCoordinator
from aphrodite.omni.diffusion.model_loader.diffusers_loader import DiffusersPipelineLoader
from aphrodite.omni.diffusion.models.interface import (
...
from aphrodite.omni.diffusion.offloader import enable_offload_backend
from aphrodite.omni.diffusion.offloader.config import TEXT_ENCODER_COMPONENT, resolve_offload
from aphrodite.omni.diffusion.registry import _NO_CACHE_ACCELERATION
...
from aphrodite.omni.diffusion.worker.input_batch import InputBatch, scatter_latents
from aphrodite.omni.diffusion.worker.request_batch import DiffusionRequestBatch
from aphrodite.omni.diffusion.worker.utils import (
    BatchRunnerOutput,
    RunnerOutput,
    StepRequestState,
...
from aphrodite.omni.worker.omni_connector_model_runner_mixin import OmniConnectorModelRunnerMixin
from aphrodite.utils.mem_utils import DeviceMemoryProfiler, GiB_bytes
...
def _dit_any_rank_failed(local_failed: bool) -> bool:
    ...
    if not torch.distributed.is_initialized():
        return local_failed
    from aphrodite.omni.diffusion.distributed.parallel_state import get_world_group

    group = get_world_group()
    if group.world_size == 1:
        return local_failed
    signal = torch.tensor(1 if local_failed else 0, dtype=torch.int32, device=group.device)
    torch.distributed.all_reduce(signal, op=torch.distributed.ReduceOp.MAX, group=group.device_group)
...
class DiffusionModelRunner(OmniConnectorModelRunnerMixin):
    ...
    def __init__(
        ...
        Args:
            aphrodite_config: Sonar configuration.
    ...
    def load_model(
        ...
        with get_memory_context(), DeviceMemoryProfiler() as m:
    ...
    def get_kv_cache_spec(self) -> dict[str, KVCacheSpec]:
        ...
        for module_path, module in self.pipeline.named_modules():
            if not isinstance(module, Attention):
                continue
            spec = module.get_kv_cache_spec(self.aphrodite_config)
            if spec is not None:
        ...
        with grad_context:
            ...
            with set_forward_context(
                ...
                if noise_pred is None and pipeline_interrupted:
                    ...
                    offset = 0
                    for req in states:
                        assert req.latents is not None, "Prepared request has no latents"
                ...
                if is_primary and record_output_peak_memory:
                    ...
                    for runner_output in runner_output_list:
                        if runner_output.result is None:
                            continue
                        output_state = states_by_id.get(runner_output.request_id)
                        if output_state is None:
                            continue
                        runner_output.result.peak_memory_mb = max(
                            runner_output.result.peak_memory_mb,
                            output_state.peak_memory_mb,
                        )
