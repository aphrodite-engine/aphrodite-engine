...
"""
Diffusion Worker for Sonar Omni.
...
from aphrodite.distributed.parallel_state import get_ep_group, get_tp_group
...
from aphrodite.omni.diffusion.diffusion_kv.kv_connector import (
    init_worker_kv_connector,
    shutdown_kv_connector,
)
from aphrodite.omni.diffusion.diffusion_kv.metadata import DiffusionKVMetadata
from aphrodite.omni.diffusion.distributed.parallel_state import (
...
from aphrodite.omni.diffusion.forward_context import set_forward_context
from aphrodite.omni.diffusion.ipc import DIFFUSION_RPC_RESULT_ENVELOPE, pack_diffusion_output_shm
from aphrodite.omni.diffusion.lora.manager import DiffusionLoRAManager, LoRABackend
from aphrodite.omni.diffusion.registry import get_diffusion_ir_op_priority_func
...
from aphrodite.omni.diffusion.worker.diffusion_model_runner import DiffusionModelRunner
from aphrodite.omni.diffusion.worker.utils import BaseRunnerOutput, BatchRunnerOutput
from aphrodite.omni.engine.stage_init_utils import set_death_signal
from aphrodite.omni.inputs.data import OmniInteractionPrompt
from aphrodite.omni.lora.request import LoRARequest
from aphrodite.omni.platforms import current_omni_platform
from aphrodite.omni.profiler import OmniTorchProfilerWrapper, create_omni_profiler
from aphrodite.omni.worker.gpu_memory_utils import get_process_gpu_memory
from aphrodite.profiler.wrapper import CudaProfilerWrapper, WorkerProfiler
from aphrodite.utils.import_utils import resolve_obj_by_qualname
from aphrodite.utils.mem_utils import GiB_bytes, MemorySnapshot, format_gib, memory_profiling
...
from aphrodite.v1.worker.utils import request_memory
...
def _setup_diffusion_worker_proc_title_and_log_prefix(
    ...
    set_process_title(name=process_name, prefix="Sonar Omni")
    decorate_logs(process_name)


@contextmanager
def _force_cutlass_fp8_linear_kernel(quant_config: object | None) -> Iterator[None]:
    import aphrodite.model_executor.layers.quantization.modelopt as aphrodite_modelopt

    if isinstance(quant_config, aphrodite_modelopt.ModelOptFp8Config) and quant_config.quant_method in {
        "FP8",
        ...
        if current_platform.is_cuda() and current_platform.has_device_capability(89):
            from aphrodite.model_executor.kernels.linear import CutlassFP8ScaledMMLinearKernel

            original_init_fp8_linear_kernel = aphrodite_modelopt.init_fp8_linear_kernel
            ...
            aphrodite_modelopt.init_fp8_linear_kernel = init_fp8_linear_kernel_with_cutlass
            logger.info("Using CUTLASS FP8 linear kernels for this ModelOpt FP8 diffusion stage.")
            try:
                yield
            finally:
                aphrodite_modelopt.init_fp8_linear_kernel = original_init_fp8_linear_kernel
            return

    yield


def _get_cumem_allocator_class() -> type["CuMemAllocator"]:
    from aphrodite.device_allocator.cumem import CuMemAllocator

    return CuMemAllocator


def _resolve_ir_op_priority(od_config: OmniDiffusionConfig, aphrodite_config: AphroditeConfig) -> Any:
    ir_op_priority = current_omni_platform.get_default_ir_op_priority(aphrodite_config)
    ir_op_priority_func = get_diffusion_ir_op_priority_func(od_config)
    if ir_op_priority_func is not None:
        ir_op_priority = ir_op_priority_func(ir_op_priority, aphrodite_config=aphrodite_config)
...
class DiffusionWorker:
    ...
    def __init__(
        ...
        if not (isinstance(runner_override, str) and runner_override):
            try:
                from aphrodite.omni.diffusion.diffusion_engine import DiffusionEngine
    ...
    def init_device(self) -> None:
        ...
        assert world_size is not None, "num_gpus must be resolved before worker initialization"
        rank = self.rank
        ...
        # Create aphrodite_config for parallel configuration. Pass explicit device_config
        # so DeviceConfig does not rely on current_platform in worker subprocesses.
        aphrodite_config = create_diffusion_aphrodite_config(self.device, self.od_config)
        # Since Sonar v0.20.0, IR wraps GPU ops. Set IR op priority preference to enforce GPU op fusion during wrapping.
        # Also need to log, because Sonar internally logs another line in AphroditeConfig.__post_init__. Avoid confusion.
        aphrodite_config.kernel_config.ir_op_priority = _resolve_ir_op_priority(self.od_config, aphrodite_config)
        if self.od_config.moe_backend != "auto":
            ...
            aphrodite_config.kernel_config.moe_backend = TypeAdapter[MoEBackend](MoEBackend).validate_python(
                self.od_config.moe_backend
            )
        logger.info(
            "Final IR op priority after setting Sonar Omni overrides: %s", aphrodite_config.kernel_config.ir_op_priority
        )
        self.aphrodite_config = aphrodite_config
        current_omni_platform.init_diffusion_worker_aphrodite_config(aphrodite_config)

        # Initialize distributed environment
        with (
            set_forward_context(aphrodite_config=self.aphrodite_config, omni_diffusion_config=self.od_config),
    ...
    def load_model(self, load_format: str = "default", custom_pipeline_name: str | None = None, **kwargs) -> None:
        ...
        assert self.aphrodite_config is not None, "Worker configuration not initialized"
    ...
    def determine_available_kv_memory(self, profile_requests: list[OmniDiffusionRequest]) -> list[int]:
        ...
        def determine_local_memory() -> int:
            ...
            override = self.aphrodite_config.cache_config.kv_cache_memory_bytes
            if override:
                # Match native Sonar: an explicit cache budget skips automatic
            ...
            logger.debug(
                ...
                self.rank,
                format_gib(self.init_snapshot.free_memory),
                self.aphrodite_config.cache_config.gpu_memory_utilization,
                format_gib(self.requested_memory),
            )
            logger.debug(
    ...
    def set_kv_cache_configs(
        ...
        self.aphrodite_config.model_config.max_model_len = resolved_max_model_len
        kv_cache_config = kv_cache_configs[self.rank]
        self.aphrodite_config.cache_config.num_gpu_blocks = kv_cache_config.num_blocks
        init_worker_kv_connector(self.aphrodite_config, kv_cache_config)
    ...
    def init_lora_manager(self) -> None:
        ...
        if isinstance(lora_path, list):
            raise ValueError("Dynamic LoRA management requires a single adapter path")
        lora_scale = self.od_config.lora_scale
        if isinstance(lora_scale, list):
            if len(lora_scale) != 1:
                raise ValueError("Dynamic LoRA management requires a single adapter scale")
        ...
        if lora_backend == LoRABackend.PEFT:
            self.lora_manager = DiffusionLoRAManager(
                ...
                max_cached_adapters=self.od_config.max_cpu_loras or 1,
                lora_path=lora_path,
    ...
    def execute_model(
        ...
        if isinstance(req, list):
            from aphrodite.omni.diffusion.distributed.parallel_state import get_data_parallel_rank

            dp_rank = get_data_parallel_rank()
            idx = dp_rank % len(req)
            new_req = req[idx]
            validate_new_request_data_identity(new_req)
            selected_req = new_req.req
        ...
        if self.lora_manager is not None:
            try:
                self.lora_manager.set_active_adapter(
                    selected_req.sampling_params.lora_request, selected_req.sampling_params.lora_scale
                )
            except Exception as exc:
                if selected_req.sampling_params.lora_request is not None:
                    raise
        ...
        with ctx:
            ...
            output = self.model_runner.execute_model(selected_req, **kwargs)
        if profiler:
            profiler.step()
    ...
    def sleep(self, level: int = 1) -> int:
        ...
        if level == 2 and self.model_runner is not None:
            ...
            assert model is not None, "Pipeline not loaded"
        ...
        try:
            ...
            if torch.distributed.is_initialized():
                try:
                    ...
                    pass
            task_id = task.get("task_id", "local") if isinstance(task, dict) else task.task_id
            return OmniACK(task_id=task_id, status="ERROR", error_msg=str(e))
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            try:
                ...
                try:
                    a2a_permute = sys.modules.get("aphrodite.omni.diffusion.distributed.a2a_permute")
...
class _CustomPipelineWorker(Protocol):
    model_runner: DiffusionModelRunner

    def load_model(
        self, load_format: str = "default", custom_pipeline_name: str | None = None, **kwargs: Any
    ) -> None: ...
...
class CustomPipelineWorkerExtension:
    def re_init_pipeline(self: _CustomPipelineWorker, custom_pipeline_args: dict[str, Any]) -> None:
        """
...
class WorkerProc:
    ...
    def __init__(
        ...
        self.mq: MessageQueue | None = MessageQueue.create_from_handle(broadcast_handle, gpu_id)
    ...
    def _enqueue_result(self, msg: Any) -> None:
        ...
        with self._result_mq_lock:
            assert self.result_mq is not None, "Result queue not initialized"
    ...
    def _return_result(self, output: Any, rpc_id: str | None = None) -> None:
        ...
        if not self.od_config.step_execution and isinstance(output, (DiffusionOutput, BatchRunnerOutput)):
            ...
            assert self._async_output_queue is not None, "Async output queue not initialized"
    ...
    def recv_message(self) -> Any:
        ...
        assert self.mq is not None, "Worker broadcast queue has closed"
    ...
    def _worker_busy_loop(self) -> None:
        ...
        while self._running:
            ...
            if isinstance(msg, dict) and msg.get("type") == "sleep":
                ...
                wake_task = OmniWakeTask(tags=msg.get("tags"), task_id=msg.get("task_id", "local"))
                ack = self.worker.handle_wake_task(wake_task)
                self._return_result(ack)
            # Route message based on type
                ...
                try:
                    if not isinstance(msg, (OmniDiffusionRequest, list)):
                        raise TypeError("Expected an OmniDiffusionRequest or a batch of requests")
    ...
    def worker_main(
        ...
        from aphrodite.omni.plugins import load_omni_general_plugins
...
class WorkerWrapperBase:
    ...
    def __init__(
        ...
        self.worker_extension_cls: str | type | None = worker_extension_cls
