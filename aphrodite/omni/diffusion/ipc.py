...
def _array_to_shm(array: np.ndarray) -> dict[str, Any]:
    ...
    shm_array = np.ndarray(array.shape, dtype=array.dtype, buffer=buffer[:nbytes])
...
def pack_diffusion_output_shm(
    ...
    if isinstance(result, DiffusionOutput):
        setattr(output, "result", _pack_diffusion_fields(result, d2h_stream=d2h_stream))
