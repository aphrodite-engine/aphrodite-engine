...
class DiffusionExecutor(ABC):
    ...
    def get_class(od_config: OmniDiffusionConfig) -> type[DiffusionExecutor]:
        ...
        # Mirror Sonar's `world_size == 1 -> "uni"` default
        # (aphrodite/config/parallel.py). A single-GPU diffusion deployment has
        ...
        if isinstance(distributed_executor_backend, type):
            ...
            from aphrodite.omni.diffusion.executor.multiproc_executor import MultiprocDiffusionExecutor

            executor_class = MultiprocDiffusionExecutor
        elif distributed_executor_backend == "uni":
            from aphrodite.omni.diffusion.executor.uniproc_executor import UniProcDiffusionExecutor
    ...
    def wait_output_ready(self, async_output_id: str) -> Future[DiffusionOutput]:
        raise NotImplementedError("This executor does not produce asynchronous outputs")
