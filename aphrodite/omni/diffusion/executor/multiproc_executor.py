...
from aphrodite.omni.diffusion.data import SHUTDOWN_MESSAGE, AsyncDiffusionOutput, AsyncOutputKind, DiffusionOutput
from aphrodite.omni.diffusion.executor.abstract import DiffusionExecutor
from aphrodite.omni.diffusion.ipc import DIFFUSION_RPC_RESULT_ENVELOPE, unpack_diffusion_output_shm
from aphrodite.omni.diffusion.offloader.config import (
    TEXT_ENCODER_COMPONENT,
    any_selected_component_uses_allgather,
    resolve_offload,
)
from aphrodite.omni.diffusion.sched.request_scheduler import build_request_batch_sampling_params_key
from aphrodite.omni.diffusion.utils.future_utils import try_set_exception, try_set_result
from aphrodite.omni.diffusion.worker import WorkerProc
from aphrodite.v1.engine.exceptions import EngineDeadError
from aphrodite.v1.executor.multiproc_executor import set_multiprocessing_worker_envs

if TYPE_CHECKING:
    from aphrodite.omni.diffusion.sched.interface import DiffusionSchedulerOutput
    from aphrodite.omni.diffusion.worker.utils import BaseRunnerOutput

logger = init_logger(__name__)

_DEQUEUE_TIMEOUT_S = 5.0
_DLO_DP_WAVE_TIMEOUT_S = float(os.environ.get("APHRODITE_OMNI_DLO_DP_WAVE_TIMEOUT", 600.0))
...
class MultiprocDiffusionExecutor(DiffusionExecutor):
    ...
    def _init_executor(self) -> None:
        ...
        self._broadcast_mq: MessageQueue | None = self._init_broadcast_queue(num_workers)
        assert self._broadcast_mq is not None
    ...
    def execute_request(self, scheduler_output: DiffusionSchedulerOutput) -> BaseRunnerOutput:
        ...
        """
        from aphrodite.omni.diffusion.worker.utils import BatchRunnerOutput, RunnerOutput
        ...
        from aphrodite.omni.diffusion.sched.interface import validate_new_request_data_identity
    ...
    def execute_batch(self, scheduler_output: DiffusionSchedulerOutput) -> BaseRunnerOutput:
        ...
        if isinstance(result, AsyncDiffusionOutput) and result.kind == AsyncOutputKind.COMPUTE_DONE:
            ...
            assert batch_id is not None, "COMPUTE_DONE must identify its asynchronous output"
    ...
    def collective_rpc(
        ...
        self._ensure_open()
        broadcast_mq = self._broadcast_mq
        assert broadcast_mq is not None, "Executor broadcast queue has closed"
            ...
            if msg.kind in (AsyncOutputKind.RPC_RESULT, AsyncOutputKind.COMPUTE_DONE):
                ...
                if per_req_map is not None:
                    ...
                    if msg.error:
                        ...
                        try:
                            unpack_diffusion_output_shm(msg.output)
                            if msg.output is None:
                                raise ValueError("OUTPUT_READY must contain an output or error")
                    ...
                    if batch_id:
                        with self._futures_lock:
                            ...
                            if pending is not None and not pending.done():
                                ...
                                output_future: concurrent.futures.Future[DiffusionOutput] = concurrent.futures.Future()
                                if exc is not None:
                                    output_future.set_exception(exc)
                                else:
                                    assert output_result is not None
                                    output_future.set_result(output_result)
                                self._completed_outputs[batch_id] = output_future
