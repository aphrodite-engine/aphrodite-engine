...
Mirrors Sonar's ``UniProcExecutor`` (``aphrodite/v1/executor/uniproc_executor.py``).
...
class UniProcDiffusionExecutor(DiffusionExecutor):
    ...
    def _init_executor(self) -> None:
        from aphrodite.omni.diffusion.worker.diffusion_worker import WorkerWrapperBase
        ...
        # size 1, Sonar's GroupCoordinator skips the device communicator and
    ...
    def collective_rpc(
        ...
        """
        self._ensure_open()
        assert self.driver_worker is not None, "Worker not initialized"
