...
import aphrodite.distributed.parallel_state as _aphrodite_ps
...
def _set_forward_context_num_tokens(num_tokens: int) -> None:
    """Update the token-count extension installed by platform MoE hooks."""
    if not _aphrodite_fc.is_forward_context_available():
        return
    forward_context = _aphrodite_fc.get_forward_context()
    if hasattr(forward_context, "num_tokens"):
        setattr(forward_context, "num_tokens", num_tokens)


def _set_forward_context_dp_metadata(num_tokens: int) -> None:
    """Populate Sonar MoE DP token metadata for diffusion-only DP mappings."""
    if not _aphrodite_fc.is_forward_context_available():
        return
    forward_context = _aphrodite_fc.get_forward_context()
    ...
    dp_group = getattr(_aphrodite_ps, "_DP", None)
    ...
    forward_context.dp_metadata = _aphrodite_fc.DPMetadata(torch.tensor(gathered_num_tokens, dtype=torch.int64))
...
class FusedMoE:
    ...
    def __new__(cls, *, prefix: str = "", **kwargs: Any) -> Any:
        ...
        # Current Sonar FusedMoE handles output reduction internally.
        kwargs.pop("reduce_results", None)
        # FusedMoE is now a factory function; call it to get a MoERunner.
        from aphrodite.model_executor.layers.fused_moe import FusedMoEFactory as _FusedMoE

        moe_runner = _FusedMoE(prefix=prefix, **kwargs)

        # Platform token counts and DP metadata follow each MoE input shape.
    ...
    def make_expert_params_mapping(
        ...
        Upstream Sonar refactored ``FusedMoE`` from a class (which had
        ...
        ``aphrodite.model_executor.layers.fused_moe``.
        """
