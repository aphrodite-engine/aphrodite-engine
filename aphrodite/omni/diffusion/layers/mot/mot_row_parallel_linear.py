...
from aphrodite.omni.diffusion.layers.mot.ops.mot_gemm import invoke_mot_gemm
...
class MoTRowParallelLinear(RowParallelLinear):
    ...
    vae  weights: on permanent submodule self.gen_exp (self.gen_exp.weight, ...),
                   ...
                   Sonar framework's process_weights_after_loading to automatically
    ...
    def __init__(
        ...
        *,
        ...
        # This is slightly unconventional but necessary. It allows Sonar's framework
        ...
        # layer.  Sonar's online quantizers inspect these attributes while
        ...
        # Enable gen_exp to be automatically discovered by Sonar framework's process_weights_after_loading
    ...
    def _mot_gemm_dispatch(
        ...
        # Any other backend (ROCm, XPU, TPU, CPU) uses the safe aphrodite fallback.
    ...
    def _mot_gemm_fp8_w8a8(self, x, text_idx, vae_idx, bias_t, bias_v):
        ...
        1) Activation quantization: reuse aphrodite's ops.scaled_fp8_quant
