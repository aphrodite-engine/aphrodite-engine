...
#
#  Usage pattern (mirrors Sonar fused_moe):
...
_ENV_CONFIG_FOLDER = "APHRODITE_TUNED_CONFIG_FOLDER"
...
def _load_mot_config_file(dtype_str: str) -> dict | None:
    ...
    Search order:
      1. ``$APHRODITE_TUNED_CONFIG_FOLDER/device_name=...,dtype=....json``
      2. ``aphrodite/omni/.../mot/configs/device_name=...,dtype=....json``
...
def invoke_mot_gemm(
    ...
    assert A.size(1) == K, "the weights' first dimension should matchinputtensor's last dimension (hidden_dim)"

    assert B_vae.size(0) == K and B_vae.size(1) == N, (
