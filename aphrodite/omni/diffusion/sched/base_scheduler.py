...
from aphrodite.omni.diffusion.diffusion_kv.manager import DiffusionKVAdmissionError, DiffusionKVCacheManager
...
class BaseScheduler(ABC):
    ...
    def initialize(
        ...
        *,
        ...
        if diffusion_kv_enabled:
            ...
            if kv_aphrodite_config is None:
                raise ValueError(
                    "paged_scheduler Diffusion KV requires the native AphroditeConfig used for cache sizing"
                )
            self._diffusion_kv_manager = DiffusionKVCacheManager(
                kv_cache_config,
                max_model_len=kv_aphrodite_config.model_config.max_model_len,
                scheduler_block_size=scheduler_block_size,
                hash_block_size=hash_block_size,
                max_in_flight_tokens=kv_aphrodite_config.max_in_flight_tokens,
            )
        else:
            if any(
                value is not None
        ...
        from aphrodite.omni.diffusion.diffusion_kv.kv_connector import create_scheduler_kv_connector
    ...
    def kv_connector(self):
        """Upstream Sonar Scheduler-role connector, when configured."""
    ...
    def close(self) -> None:
        if self._diffusion_kv_manager is not None:
            ...
            self._diffusion_kv_manager = None
        from aphrodite.omni.diffusion.diffusion_kv.kv_connector import shutdown_kv_connector
