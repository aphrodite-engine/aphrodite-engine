...
# We use the term "Sequence Parallelism" to align with Sonar Omni's existing terminology.
#
...
#   - Uses Sonar Omni's SequenceParallelGroupCoordinator instead of DeviceMesh
...
from aphrodite.omni.diffusion.distributed.sp_sharding import sp_gather, sp_shard
from aphrodite.omni.diffusion.hooks.base import HookRegistry, ModelHook
...
class SequenceParallelSplitHook(ModelHook):
    ...
    def _prepare_sp_input(
        ...
        def _maybe_validate_strict_divisibility(*, dim: int, seq_len: int) -> None:
            from aphrodite.omni.diffusion.forward_context import get_ulysses_mode
    ...
    def _shard_with_auto_pad(self, x: torch.Tensor, dim: int) -> torch.Tensor:
        ...
        """
        from aphrodite.omni.diffusion.attention.selector import get_attn_backend_for_capability
...
def apply_sequence_parallel(
    ...
    2. Attention parallelism (handled by Sonar Omni's Attention layer):
    ...
    Note:
        Sonar Omni's Attention layer automatically handles the internal
