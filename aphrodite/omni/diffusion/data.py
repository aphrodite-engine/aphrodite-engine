...
from aphrodite.omni.diffusion.diffusion_kv.config import (
...
from aphrodite.omni.diffusion.utils.network_utils import is_port_available
...
if TYPE_CHECKING:
    from aphrodite.config import KVTransferConfig, ProfilerConfig
...
def normalize_omni_diffusion_kwargs(raw_kwargs: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if "quantization" in config_kwargs and config_kwargs.get("quantization_config") is None:
        config_kwargs["quantization_config"] = config_kwargs.pop("quantization")
    else:
        config_kwargs.pop("quantization", None)

    # Renamed from kv_cache_* to avoid clashing with Sonar's --kv-cache-dtype.
...
def resolve_model_class_name(
    ...
    """
    from aphrodite.omni.diffusion.utils.hf_utils import get_diffusion_model_index, resolve_native_diffusion_model_class
    from aphrodite.transformers_utils.config import get_hf_file_to_dict
    ...
    from aphrodite.omni.diffusion.utils.hf_utils import _looks_like_hidream_o1
    ...
    if model_type == "vla":
        from aphrodite.omni.diffusion.utils.hf_utils import _looks_like_dreamzero
...
class OmniDiffusionConfig:
    ...
    # Native Sonar KV connector configuration. PR0 only assembles the connector;
    ...
    # has already resolved to Sonar's ModelOpt FP8 linear method.
    force_cutlass_fp8: bool = False

    # Diffusion attention KV cache dtype (not Sonar's --kv-cache-dtype for AR models).
    ...
    # Native Sonar KV-cache sizing inputs used only by paged_scheduler mode.
    # These mirror the structured stage cache/scheduler config until all
    # diffusion runtime construction consumes AphroditeOmniConfig directly.
    ...
    def __post_init__(self):
        ...
        # Match Sonar's config flow: parse entrypoint shorthands before the
        ...
        # align with ar stage behavior, see aphrodite/engine/arg_utils.py
    ...
    def enrich_config(self) -> None:
        ...
        """
        from aphrodite.omni.diffusion.utils.hf_utils import (
        ...
        try:
            ...
            if config_dict is not None:
                ...
                if self.diffusion_load_format == "diffusers":
                    ...
                    from aphrodite.omni.model_extras import get_transformer_config_subfolder
            ...
            if self.diffusion_load_format == "diffusers":
                ...
                if is_hidream_o1:
                    ...
                    # aphrodite/omni/diffusion/models/ming_flash_omni/pipeline_ming_imagegen.py.
...
class DiffusionOutput:
    ...
    # KV-recv wall-clock (ms) from the runner; feeds aphrodite.omni:diffusion_kv_load_s.
