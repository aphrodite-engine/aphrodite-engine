...
if current_omni_platform.is_rocm():
    ...
    try:
        from aphrodite._xpu_ops import xpu_ops  # noqa: F401
    ...
    if _is_blackwell():
        ...
        # Candidate 5: Sonar's encapsulated Flash Attention dispatcher.
        if flash_attn_varlen_func is None:
            try:
                from aphrodite.vllm_flash_attn import flash_attn_varlen_func as _fa_varlen
...
def _choose_aphrodite_flash_attn_version(
    device_major: int,
    requested: str | int | None,
    supported_versions: frozenset[int],
) -> int:
    """Choose a Sonar-bundled FA kernel without model-specific policy."""
    ...
    if 2 in supported_versions:
        return 2
    raise RuntimeError("No Sonar-bundled FlashAttention 2/3/4 kernel is available")


@cache
def resolve_aphrodite_flash_attn_version(requested: str | int | None = None) -> int:
    """Resolve an available Sonar-bundled FA2/FA3/FA4 implementation."""

    if not current_omni_platform.is_cuda():
        raise RuntimeError("Sonar-bundled versioned FlashAttention requires CUDA")

    from aphrodite.vllm_flash_attn.flash_attn_interface import is_fa_version_supported
    ...
    return _choose_aphrodite_flash_attn_version(capability.major, requested, supported_versions)


def aphrodite_flash_attn_varlen_with_lse(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    *,
    ...
    """Run packed Sonar FlashAttention and retain LSE for post-processing."""

    from aphrodite.vllm_flash_attn import flash_attn_varlen_func as aphrodite_flash_attn_varlen_func

    version = resolve_aphrodite_flash_attn_version(fa_version)
    out, lse = aphrodite_flash_attn_varlen_func(
        q,
        k,
        v,
        int(max_seqlen_q),
...
def is_flash_attn_installed() -> bool:
    ...
    # Try Sonar's flash attention wrapper
    try:
        from aphrodite.vllm_flash_attn import (  # noqa: F401
