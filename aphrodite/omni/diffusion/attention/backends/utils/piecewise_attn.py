...
def _run_homogeneous_paged_piecewise_plan(
    ...
    for segment, metadata in zip(plan.segments, segment_metadata, strict=True):
        ...
        segment_key = None if key_rows is None else key_rows[:, start:end].contiguous().reshape(-1, *key_rows.shape[2:])
        segment_value = (
            None if value_rows is None else value_rows[:, start:end].contiguous().reshape(-1, *value_rows.shape[2:])
        )
