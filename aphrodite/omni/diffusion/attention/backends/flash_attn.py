...
from aphrodite.omni.diffusion.attention.backends.abstract import AttentionBackend, AttentionImpl, AttentionMetadata
from aphrodite.omni.diffusion.attention.backends.sdpa import _maybe_reshape_attn_mask
...
class FlashAttentionImpl(AttentionImpl[AttentionMetadata]):
    ...
    def _forward_varlen_masked(
        ...
        from aphrodite.omni.diffusion.attention.backends.utils.fa import (
            _index_first_axis,
            _pad_input,
            _unpad_input,
            _upad_input,
    ...
    def _forward_varlen_dense(
        ...
        """Common wrapper for calling flash_attn_varlen_func for XPU and CUDA in Sonar.
    ...
    def forward_paged(self, paged_kv_context) -> torch.Tensor:
        ...
        boundary.  Its native layer wrapper keeps Sonar version-specific cache
        and kernel details out of Omni's common ``Attention`` layer. CUDA uses
        the native Sonar FlashAttention writer/kernel contract. Ascend writes
        ...
        # prewrites through Sonar-Ascend's normal-layout writer so piecewise FIA
    ...
    def forward_xpu(
        ...
        if not HAS_FLASH_ATTN:
            raise ImportError(
                "FlashAttentionBackend requires Flash Attention. "
                "Please assure aphrodite-xpu-kernels properly installed. "
                "Otherwise, use SDPA backend by setting DIFFUSION_ATTENTION_BACKEND=TORCH_SDPA"
            )
    ...
    def forward_fa_quant_npu(
        ...
        from aphrodite.omni.platforms.npu.quant.kv_quant_npu import fp8_rotate_quant_fa
