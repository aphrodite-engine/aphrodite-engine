...
from aphrodite.omni.diffusion.attention.backends.abstract import AttentionBackend, AttentionMetadata
from aphrodite.omni.diffusion.attention.backends.sdpa import SDPABackend, SDPAImpl
from aphrodite.omni.diffusion.attention.parallel import build_parallel_attention_strategy
from aphrodite.omni.diffusion.attention.parallel.base import NoParallelAttention
from aphrodite.omni.diffusion.attention.parallel.ring import RingParallelAttention
from aphrodite.omni.diffusion.attention.selector import get_attn_backend_for_role
from aphrodite.omni.diffusion.config import get_current_diffusion_config_or_none
from aphrodite.omni.diffusion.diffusion_kv.config import DiffusionKVCacheMode
from aphrodite.omni.diffusion.distributed.parallel_state import get_sp_group
from aphrodite.omni.diffusion.forward_context import (
    get_forward_context,
    get_ulysses_mode,
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        # ``prefix`` is also the stable layer identity used by Sonar's native
        ...
        if custom_attention is None:
            ...
            if (
                ...
                # Sonar resolves it to CUDA FlashAttention on GPU and Ascend native
                ...
                # paged execution still delegates to Sonar's native paged backend.
            ...
            self.sdpa_fallback: SDPAImpl | None = SDPABackend.get_impl_cls()(
                num_heads=num_heads,
                head_size=head_size,
                softmax_scale=softmax_scale,
                causal=causal,
    ...
    def get_kv_cache_spec(self, aphrodite_config: AphroditeConfig) -> KVCacheSpec | None:
        ...
        dtype = self.paged_kv_cache_dtype or aphrodite_config.model_config.dtype
    ...
    def _init_kv_cache_quantization(self, config) -> None:
        ...
        if dtype:
            ...
            if not self.attention.supports_kv_cache_dtype(dtype, platform_key):
                raise ValueError(
                    f"Attention implementation {type(self.attention).__name__} does not support "
    ...
    def _forward_impl(
        ...
        if use_paged_attention and not getattr(self.attn_backend, "supports_paged_kv", False):
            raise NotImplementedError(
                ...
                f"selected {self.attn_backend.get_name() if self.attn_backend is not None else 'custom attention'}"
            )
