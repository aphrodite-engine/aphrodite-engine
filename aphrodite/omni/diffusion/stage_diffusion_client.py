...
"""Stage Diffusion Client for Sonar Omni multi-stage runtime.
...
from aphrodite.omni.diffusion.stage_diffusion_proc import (
    StageDiffusionProc,
...
if TYPE_CHECKING:
    ...
    from aphrodite.omni.inputs.data import OmniDiffusionSamplingParams, OmniInteractionPrompt, OmniPromptType
...
def create_diffusion_client(
    ...
    if use_inline:
        from aphrodite.omni.diffusion.inline_stage_diffusion_client import InlineStageDiffusionClient
...
class StageDiffusionClient(StageClientBase):
    ...
    # ------------------------------------------------------------------
    # Process monitor (mirrors Sonar's MPClient.start_engine_core_monitor)
    # ------------------------------------------------------------------
    ...
    def _start_proc_monitor(self) -> None:
        ...
        # Mirrors MPClient.start_engine_core_monitor() in Sonar.
        assert self._proc_manager is not None
    ...
    def check_health(self) -> None:
        ...
        Mirrors the ``check_health`` protocol on Sonar's ``EngineClient``.
        """
        if self._engine_dead:
