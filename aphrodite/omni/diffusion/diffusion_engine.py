...
from aphrodite.omni.diffusion.diffusion_kv.config import DiffusionKVCacheMode, is_scheduler_paged_kv_mode
from aphrodite.omni.diffusion.diffusion_kv.initialization import initialize_diffusion_kv_control_plane
from aphrodite.omni.diffusion.executor.abstract import DiffusionExecutor
from aphrodite.omni.diffusion.io_support import (
...
from aphrodite.omni.diffusion.offloader.config import any_selected_component_uses_allgather
from aphrodite.omni.diffusion.output_formatter import (
    format_diffusion_outputs,
    format_empty_diffusion_outputs,
    normalize_diffusion_postprocess_output,
)
from aphrodite.omni.diffusion.registry import (
    DiffusionModelRegistry,
    get_diffusion_post_process_func,
    get_diffusion_pre_process_func,
)
from aphrodite.omni.diffusion.request import DUMMY_DIFFUSION_REQUEST_ID, OmniDiffusionRequest
from aphrodite.omni.diffusion.sched import BaseScheduler, RequestScheduler, StepScheduler
from aphrodite.omni.diffusion.sched.interface import DiffusionRequestStatus
from aphrodite.omni.diffusion.worker.utils import BaseRunnerOutput, BatchRunnerOutput, RunnerOutput
from aphrodite.omni.errors import client_error_from_metadata, is_client_error_status
...
_ASYNC_OUTPUT_TIMEOUT_ENV = "APHRODITE_OMNI_ASYNC_OUTPUT_TIMEOUT"
...
class DiffusionEngine:
    """The diffusion engine for Sonar Omni diffusion models."""
        ...
        try:
            ...
            if kv_control_plane is None:
                ...
                self._init_scheduler(
                    ...
                    kv_aphrodite_config=kv_aphrodite_config,
                )
    ...
    def _init_scheduler(
        ...
        *,
        ...
        kv_aphrodite_config: AphroditeConfig | None = None,
    ) -> None:
    ...
    def add_req_and_wait_for_response(self, request: OmniDiffusionRequest) -> DiffusionOutput:
        ...
        with self._rpc_lock:
            ...
            while True:
                ...
                if isinstance(runner_output, BatchRunnerOutput) and len(runner_output) != 1:
    ...
    def _make_dummy_request(
        ...
        *,
        ...
        if supports_audio_input:
            audio_sr = 16000
            multi_modal_data["audio"] = np.random.randn(audio_sr * 2).astype(np.float32)
