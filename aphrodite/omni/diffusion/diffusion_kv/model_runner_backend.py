...
from aphrodite.v1.worker import block_table as native_block_table
from aphrodite.v1.worker.gpu.attn_utils import init_attn_backend, init_kv_cache
...
class DiffusionKVModelRunnerBackend:
    ...
    def register_kv_cache_layers(
        ...
        previous_use_non_causal = self.aphrodite_config.attention_config.use_non_causal
        self.aphrodite_config.attention_config.use_non_causal = any(
        ...
        try:
            ...
            self.aphrodite_config.attention_config.use_non_causal = previous_use_non_causal
            raise
        ...
        set_attention_geometry = (
            getattr(self.aphrodite_config.model_config, "set_attention_geometry", None)
    ...
    def _get_max_model_len(self) -> int:
        max_len = getattr(self.aphrodite_config.model_config, "max_model_len", None)
    ...
    def initialize_kv_cache(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        unaligned_max_num_blocks_per_group = [
            group.kv_cache_spec.max_num_blocks_per_req(self.aphrodite_config, max_len) for group in kv_cache_groups
        ]
    ...
    def _validate_row(
        ...
        *,
        ...
        for group_index, (group_block_ids, kv_cache_group) in enumerate(zip(block_ids, groups, strict=True)):
            ...
            expected_count = spec.max_num_blocks_per_req(self.aphrodite_config, token_len)
            ...
            row_capacity = spec.max_num_blocks_per_req(self.aphrodite_config, max_len)
