...
through Sonar's native ``KVConnectorFactory``. PR0 only validates configuration
...
def create_scheduler_kv_connector(
    ...
    aphrodite_config = _build_native_aphrodite_config(od_config)
...
def init_worker_kv_connector(aphrodite_config: AphroditeConfig, kv_cache_config: KVCacheConfig) -> None:
    ...
    logger.info(
        ...
        aphrodite_config.kv_transfer_config.kv_connector,
...
def _build_native_aphrodite_config(od_config: OmniDiffusionConfig) -> AphroditeConfig:
    ...
    aphrodite_config = create_diffusion_aphrodite_config(torch.device("cpu"), od_config)
    aphrodite_config.kv_transfer_config = kv_transfer_config
