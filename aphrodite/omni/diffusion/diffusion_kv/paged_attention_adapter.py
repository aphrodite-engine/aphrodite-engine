...
from aphrodite.omni.diffusion.attention.backends.utils.piecewise_attn import (
    PagedPiecewisePlan,
    build_paged_piecewise_plan,
)
from aphrodite.omni.diffusion.forward_context import override_paged_kv_adapter
...
class DiffusionPagedAttentionLayerAdapter(AttentionLayerBase):
    """Register a diffusion layer with Sonar's native cache machinery.

    This object deliberately does *not* subclass ``aphrodite.Attention``.  The
    ...
    def __init__(
        ...
        *,
        ...
        try:
            # This is a portable Sonar backend request. The active platform
        ...
        with set_current_aphrodite_config(aphrodite_config):
            self.impl = self._create_native_impl(aphrodite_config)

    def get_attn_backend(self):
        return self.attn_backend

    def _create_native_impl(self, aphrodite_config: AphroditeConfig):
        ...
        if hasattr(impl, "aphrodite_flash_attn_version") and impl.aphrodite_flash_attn_version is None:
            raise RuntimeError(f"Native paged-attention kernel is unavailable for diffusion layer {self.layer_name!r}")
        return impl

    def get_kv_cache_spec(self, aphrodite_config: AphroditeConfig) -> AttentionSpec:
...
class DiffusionPagedAttentionAdapter:
    """Translate diffusion rows into Sonar metadata for an Omni paged backend."""
    ...
    def activate(
        ...
        try:
            with override_paged_kv_adapter(self), set_current_aphrodite_config(self.aphrodite_config):
                yield self
        finally:
            self._active_piecewise_plan = None
            self._active_piecewise_native_metadata = None
    ...
    def _get_piecewise_native_metadata(
        ...
        for packed_segment in plan.segments:
            ...
            if packed_segment.query_range is None:
                ...
                start, end = packed_segment.query_range
                positions = batch.positions[start:end]
                slot_mappings = batch.slot_mappings[..., start:end]
    ...
    def prepare_layer_context(
        ...
        *,
        ...
        expected_dtype = self.aphrodite_config.model_config.dtype
        ...
        # Sonar's CUDA writer consumes the source token strides directly, so
