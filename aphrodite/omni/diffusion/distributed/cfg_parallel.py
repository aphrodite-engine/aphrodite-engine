...
class CFGParallelMixin(metaclass=ABCMeta):
    ...
    def predict_noise_with_multi_branch_cfg(
        ...
        if do_true_cfg:
            ...
            # No CFG: only compute positive/conditional prediction
            conditional_pred = self.predict_noise(**branches_kwargs[0])
            if isinstance(conditional_pred, IntermediateTensors):
                raise TypeError("Multi-branch CFG requires final noise predictions")
            if output_slice is not None:
                conditional_pred = _unwrap(_slice_pred(_wrap(conditional_pred), output_slice))
