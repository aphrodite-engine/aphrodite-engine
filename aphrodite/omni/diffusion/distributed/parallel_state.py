...
import aphrodite.distributed.parallel_state as aphrodite_parallel_state
from aphrodite.logger import init_logger
from aphrodite.omni.diffusion import envs
from aphrodite.omni.diffusion.forward_context import get_forward_context
...
# get _TP&_EP from aphrodite.distributed.parallel_state
...
# with the same rank layout as Sonar EP.
...
def model_parallel_is_initialized():
    ...
    return (
        ...
        and _SP is not None
        and _PP is not None
        and aphrodite_parallel_state._TP is not None
    )
...
def init_model_parallel_group(
    ...
    local_rank: int,
    backend: str,
    parallel_mode: Literal["pipeline"],
    **kwargs,
) -> PipelineGroupCoordinator: ...
...
def init_model_parallel_group(
    ...
    local_rank: int,
    backend: str,
    parallel_mode: Literal["sequence"],
    **kwargs,
) -> SequenceParallelGroupCoordinator: ...
...
def init_aphrodite_model_parallel_group(
    group_ranks: list[list[int]],
    local_rank: int,
    backend: str,
    group_name: str,
    use_all2all: bool = False,
) -> aphrodite_parallel_state.GroupCoordinator:
    # Sonar 0.27's MoE oracle (make_unquantized_moe_kernel ->
    # get_ep_all2all_manager) asserts that the EP group's device communicator
    # carries an all2all_manager, and Sonar only builds one when the group is
    # created with use_all2all=True. Older Sonar has neither the kwarg nor the
    # requirement, so forward it only when supported.
    kwargs = {}
    if (
        use_all2all
        and "use_all2all" in inspect.signature(aphrodite_parallel_state.init_model_parallel_group).parameters
    ):
        kwargs["use_all2all"] = True
    return aphrodite_parallel_state.init_model_parallel_group(
        group_ranks=group_ranks,
        local_rank=local_rank,
        backend=backend,
...
def _initialize_model_parallel(
    ...
    aphrodite_parallel_state._DP = init_aphrodite_model_parallel_group(
        group_ranks=get_rank_groups("cfg-dp" if use_moe_parallel_mapping else "dp"),
        local_rank=get_world_group().local_rank,
        backend=backend,
        group_name="dp",
    )
    ...
    aphrodite_parallel_state._PP = init_aphrodite_model_parallel_group(
        group_ranks=get_rank_groups("pp"),
        local_rank=get_world_group().local_rank,
        backend=backend,
        group_name="pp",
    ...
    if use_moe_parallel_mapping:
        # Diffusion normally uses its own SP group. Map it to Sonar PCP only for
        # expert-parallel runtimes that rely on Sonar FusedMoE group semantics.
        # Sonar 0.24 MoE kernels require GroupCoordinator.device_communicator
        # and reduce_scatter(), which the diffusion SP coordinator intentionally
        # does not own. Keep the rank layout but build a Sonar coordinator.
        aphrodite_parallel_state._PCP = init_aphrodite_model_parallel_group(
            group_ranks=sp_group_ranks,
            local_rank=get_world_group().local_rank,
            backend=backend,
            group_name="pcp",
    ...
    assert aphrodite_parallel_state._TP is None, "Tensor parallel group is already initialized"
    tp_group_ranks = get_rank_groups("tp")
    aphrodite_parallel_state._TP = init_aphrodite_model_parallel_group(
    ...
    if use_moe_parallel_mapping:
        aphrodite_parallel_state._EP = init_aphrodite_model_parallel_group(
...
def initialize_model_parallel(
    ...
    groups created by this attempt, including Sonar-bound coordinators.
    """
    existing_groups = {
        "dp": _DP,
        "cfg": _CFG,
        ...
        "aphrodite_tp": aphrodite_parallel_state._TP,
        "aphrodite_dp": aphrodite_parallel_state._DP,
        "aphrodite_pp": aphrodite_parallel_state._PP,
        "aphrodite_ep": aphrodite_parallel_state._EP,
        "aphrodite_pcp": aphrodite_parallel_state._PCP,
    }
...
def destroy_model_parallel():
    ...
    if aphrodite_parallel_state._DP and aphrodite_parallel_state._DP is not _DP:
        aphrodite_parallel_state._DP.destroy()
    aphrodite_parallel_state._DP = None
    ...
    if aphrodite_parallel_state._PCP and aphrodite_parallel_state._PCP is not _SP:
        aphrodite_parallel_state._PCP.destroy()
    aphrodite_parallel_state._PCP = None

    if _SP:
        _SP.destroy()
    _SP = None

    if aphrodite_parallel_state._TP:
        aphrodite_parallel_state._TP.destroy()
    aphrodite_parallel_state._TP = None

    if aphrodite_parallel_state._EP:
        aphrodite_parallel_state._EP.destroy()
    aphrodite_parallel_state._EP = None
    _EXPERT_PARALLEL_GROUP_RANKS = None

    if aphrodite_parallel_state._PP and aphrodite_parallel_state._PP is not _PP:
        aphrodite_parallel_state._PP.destroy()
    aphrodite_parallel_state._PP = None
