...
def _distributed_tiled_decode(
    *,
    ...
    max_count = 0
    if rank == 0:
        assert count_gather is not None
...
def _distributed_patch_decode(
    *,
    ...
    max_w = 0
    if rank == 0:
        assert shape_gather is not None
        shapes = [tuple(int(x.item()) for x in t) for t in shape_gather]
...
def maybe_wrap_vae_decode_with_patch_parallelism(
    ...
    *,
    ...
    if getattr(vae, "_aphrodite_vae_patch_parallel_installed", False):
        return
    ...
    vae._aphrodite_vae_patch_parallel_installed = True  # type: ignore[attr-defined]
    vae._aphrodite_vae_patch_parallel_original_decode = vae.decode  # type: ignore[attr-defined]
