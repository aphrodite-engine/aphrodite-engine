...
void copy_rows(const at::Tensor& input, at::Tensor& out) {
  TORCH_CHECK(input.is_cuda() && out.is_cuda(),
              "a2a_permute: copy_rows requires CUDA tensors");
  TORCH_CHECK(input.device() == out.device(),
              "a2a_permute: copy_rows device mismatch");
  TORCH_CHECK(input.scalar_type() == out.scalar_type(),
              "a2a_permute: copy_rows dtype mismatch");
  TORCH_CHECK(input.dim() == 3, "a2a_permute: copy_rows input must be 3-D");
  TORCH_CHECK(out.sizes() == input.sizes(),
              "a2a_permute: copy_rows shape mismatch");
  TORCH_CHECK(out.is_contiguous(),
              "a2a_permute: copy_rows output must be contiguous");
  TORCH_CHECK(input.stride(2) == 1 && input.stride(1) == input.size(2),
              "a2a_permute: copy_rows requires contiguous rows");

  const int64_t row_elements = input.size(1) * input.size(2);
  TORCH_CHECK(input.stride(0) >= row_elements,
              "a2a_permute: copy_rows input rows overlap");
  const size_t rows = static_cast<size_t>(input.size(0));
  const size_t row_bytes =
      static_cast<size_t>(row_elements) * input.element_size();
  const size_t input_pitch =
      static_cast<size_t>(input.stride(0) * input.element_size());

  c10::cuda::CUDAGuard guard(input.device());
  auto stream = at::cuda::getCurrentCUDAStream();
  C10_CUDA_CHECK(cudaMemcpy2DAsync(out.data_ptr(), row_bytes, input.data_ptr(),
                                   input_pitch, row_bytes, rows,
                                   cudaMemcpyDeviceToDevice, stream));
}

// ---- kernel (verbatim from PR) ----------------------------------------------
__device__ inline void copy_bytes_vec16_aligned(const char* src, char* dst,
                                                size_t nbytes, size_t tid,
                                                size_t stride) {
...
__global__ void all_to_all_lsa_kernel(
    ncclWindow_t window, size_t base_src_byte_offset, unsigned char* out,
    int num_rows, size_t src_row_stride_bytes, size_t copy_row_bytes,
    size_t peer_stride_bytes, size_t dst_row_stride_bytes,
    ncclDevComm devComm) {
  const int peer_idx = blockIdx.x;
  const int local_block = blockIdx.y;
  const ncclCoopCta coop{};

  ncclLsaBarrierSession<ncclCoopCta> bar{coop, devComm, ncclTeamLsa(devComm),
                                         devComm.lsaBarrier,
  ...
  for (int i = local_block; i < num_rows; i += gridDim.y) {
    ...
    copy_bytes_vec16_aligned(reinterpret_cast<const char*>(src_row),
                             reinterpret_cast<char*>(dst_row), copy_row_bytes,
                             coop.thread_rank(), coop.size());
  }
  bar.sync(coop, cuda::memory_order_release);
}

// ---- own devcomm cache (replaces PR's keyed manager API) --------------------
ncclDevComm get_or_create_devcomm(ncclComm_t comm,
  ...
  TORCH_CHECK(res == ncclSuccess, "ncclDevCommCreate (a2a_permute) failed: ",
...
// ---- host entry -------------------------------------------------------------
void all_to_all_permute(const at::Tensor& input, at::Tensor& out,
                        int64_t scatter_dim, int64_t gather_dim,
                        std::string group_name) {
  TORCH_CHECK(input.stride(-1) == 1,
              "a2a_permute: innermost dim must be contiguous");
  ...
  TORCH_CHECK(tensor_leading_offset % 16 == 0,
              "a2a_permute: tensor byte offset must be 16B aligned");
  TORCH_CHECK(reinterpret_cast<uintptr_t>(input.data_ptr()) % 16 == 0,
              "a2a_permute: input ptr must be 16B aligned");
  TORCH_CHECK(reinterpret_cast<uintptr_t>(out.data_ptr()) % 16 == 0,
              "a2a_permute: out ptr must be 16B aligned");
  ...
  if (col_scatter) {
    ...
    if (input.dim() == 2) {
      ...
      TORCH_CHECK(total_cols % p == 0,
                  "a2a_permute: cols must divide group size");
      local_cols = static_cast<int>(total_cols / p);
    } else {
      TORCH_CHECK(input.dim() == 3, "a2a_permute: input must be 2-D or 3-D");
      TORCH_CHECK(input.size(1) == p,
                  "a2a_permute: 3-D input dim1 must equal group size");
    ...
    const bool ok3 = out.dim() == 3 && out.size(0) == p &&
                     out.size(1) == rows && out.size(2) == local_cols;
    const bool ok2 = out.dim() == 2 && out.size(0) == (int64_t)p * rows &&
    ...
    TORCH_CHECK(row_bytes % 16 == 0,
                "a2a_permute: local_cols*esize must be 16B-divisible");
    ctas_per_slot = std::max(
        1, std::min((rows * local_cols + elems_per_cta - 1) / elems_per_cta,
                    A2A_MAX_CTAS_PER_SLOT));
    const size_t esz = (size_t)esize;
    const size_t base_src =
        tensor_leading_offset + (size_t)my_rank * (size_t)local_cols * esz;
    all_to_all_lsa_kernel<<<dim3(p, ctas_per_slot), A2A_THREADS_PER_CTA, 0,
                            stream>>>(
        window, base_src, reinterpret_cast<unsigned char*>(out.data_ptr()),
        rows, (size_t)total_cols * esz, row_bytes,
        (size_t)rows * local_cols * esz, (size_t)local_cols * esz, devcomm);
    ...
    if (input.dim() == 2) {
      ...
      TORCH_CHECK(total_rows % p == 0,
                  "a2a_permute: rows must divide group size");
      local_rows = static_cast<int>(total_rows / p);
    } else {
      TORCH_CHECK(input.dim() == 3, "a2a_permute: input must be 2-D or 3-D");
      TORCH_CHECK(input.size(0) == p,
                  "a2a_permute: 3-D input dim0 must equal group size");
      ...
      TORCH_CHECK(input.stride(1) == c && input.stride(0) == s01,
                  "a2a_permute: 3-D input must be row-major contiguous");
    }
    const bool ok3 = out.dim() == 3 && out.size(0) == local_rows &&
                     out.size(1) == p && out.size(2) == cols;
    const bool ok2 = out.dim() == 2 && out.size(0) == local_rows &&
                     out.size(1) == (int64_t)p * cols;
    TORCH_CHECK(ok3 || ok2, "a2a_permute: bad out shape for (0,1)");
    const size_t row_bytes = (size_t)cols * (size_t)esize;
    TORCH_CHECK(row_bytes % 16 == 0,
                "a2a_permute: cols*esize must be 16B-divisible");
    ctas_per_slot = std::max(
        1, std::min((local_rows * cols + elems_per_cta - 1) / elems_per_cta,
                    A2A_MAX_CTAS_PER_SLOT));
    const size_t esz = (size_t)esize;
    const size_t cu = (size_t)cols;
    const size_t base_src =
        tensor_leading_offset + (size_t)(my_rank * local_rows) * cu * esz;
    all_to_all_lsa_kernel<<<dim3(p, ctas_per_slot), A2A_THREADS_PER_CTA, 0,
                            stream>>>(
        window, base_src, reinterpret_cast<unsigned char*>(out.data_ptr()),
        local_rows, cu * esz, row_bytes, cu * esz, (size_t)p * cu * esz,
        devcomm);
    C10_CUDA_KERNEL_LAUNCH_CHECK();
  }
}
...
TORCH_LIBRARY_FRAGMENT(a2ap, m) {
  ...
  m.def(
      "all_to_all_permute(Tensor input, Tensor(a!) out, int scatter_dim, int "
      "gather_dim, str group_name) -> ()");
}
