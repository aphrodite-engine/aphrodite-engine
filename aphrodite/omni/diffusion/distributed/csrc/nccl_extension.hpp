...
TORCH_API void nccl_put_with_signal(at::Tensor& tensor, int64_t signal,
                                    int64_t peer);
...
TORCH_API void nccl_reduce_scatter_offset(
    const at::Tensor& input, at::TensorList out, const std::string& group_name,
    int64_t dim, std::optional<at::IntArrayRef> offsets,
    std::optional<at::IntArrayRef> dst_ranks, const std::string& red_op);
...
// p, cols].
TORCH_API void nccl_all_to_all_permute(const at::Tensor& input, at::Tensor& out,
