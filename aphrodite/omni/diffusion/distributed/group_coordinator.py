...
class PipelineGroupCoordinator(GroupCoordinator):
    ...
    def __init__(
        ...
        self.recv_buffer: dict[str, dict[int, torch.Tensor]] = {}
        ...
        self.skip_tensor_recv_buffer: list[torch.Tensor] = []
    ...
    def set_recv_buffer(
        ...
        self.recv_buffer["latent"] = {
            idx: torch.zeros(*shape, dtype=self.dtype, device=self.device)
            for idx, shape in enumerate(patches_shape_list)
        }
        self.recv_buffer["latent"][-1] = torch.zeros(*feature_map_shape, dtype=self.dtype, device=self.device)
...
class SequenceParallelGroupCoordinator(GroupCoordinator):
    def __init__(
        ...
        **kwargs,
        ...
        allgather_group = kwargs.get("allgather_group")
