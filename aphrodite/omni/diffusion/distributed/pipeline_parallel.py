...
class PipelineParallelMixin:
    ...
    vae: torch.nn.Module
    predict_noise: Callable[..., Any]
    combine_cfg_noise: Callable[..., Any]
    ...
    def predict_noise_maybe_with_cfg(
        ...
        if get_pipeline_parallel_world_size() == 1:
            return cast(CFGParallelMixin, super()).predict_noise_maybe_with_cfg(
        ...
        if do_true_cfg and negative_kwargs is None:
            raise ValueError("CFG requires negative branch inputs")
        all_kwargs: list[dict[str, Any]]
        if cfg_parallel_ready:
            # Each PP pipeline carries exactly one CFG branch determined by cfg_rank.
            selected_kwargs = positive_kwargs if get_classifier_free_guidance_rank() == 0 else negative_kwargs
            assert selected_kwargs is not None
            all_kwargs = [selected_kwargs]
        else:
            ...
            if do_true_cfg:
                ...
                all_kwargs.append(negative_kwargs)
    ...
    def scheduler_step_maybe_with_cfg(
        ...
        """
        if get_pipeline_parallel_world_size() == 1:
            return cast(CFGParallelMixin, super()).scheduler_step_maybe_with_cfg(
                noise_pred, t, latents, do_true_cfg, per_request_scheduler, generator
            )

        pp_group = get_pp_group()
        if pp_group.is_last_rank:
            latents = cast(CFGParallelMixin, super()).scheduler_step_maybe_with_cfg(
                noise_pred, t, latents, do_true_cfg, per_request_scheduler, generator
            )
