...
#
# Native Sonar Omni port of the Boogu-Image transformer.
#
...
#   - diffusers Attention + custom processors replaced by the Sonar Omni
#     `Attention` layer (backend/SP/KV-quant handled by the framework).
#   - Q/K/V and FFN projections use Sonar parallel linears for TP support.
...
from aphrodite.omni.diffusion.models.utils import make_attention_mask
...
class BooguImageSelfAttention(nn.Module):
    ...
    the Sonar Omni `Attention` layer picks the kernel backend and handles
    ...
    def __init__(
        ...
        if prompt_tuning_configs.get("use_prompt_tuning", False):
            raise NotImplementedError(
                ...
                "native Sonar Omni BooguImageTransformer2DModel."
            )
