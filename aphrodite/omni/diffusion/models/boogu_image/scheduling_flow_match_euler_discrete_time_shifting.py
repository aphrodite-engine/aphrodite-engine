...
#
# Ported for Sonar Omni native Boogu-Image support from
# boogu/schedulers/scheduling_flow_match_euler_discrete_time_shifting.py
# (https://github.com/boogu-project/Boogu-Image). The scheduling logic is
# kept verbatim; only debug comments were removed and typing modernized.
#
...
class FlowMatchEulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    Timesteps run ascending 0 -> 1 (not descending sigmas as in stock
    diffusers), `set_timesteps` accepts a `num_tokens` argument for the
    dynamic-shift variants, and `step()` integrates with `t_next - t`
    directly.
            ...
            if self.config.do_shift:
                ...
                if self.config.dynamic_time_shift:
                    ...
                    if self.config.time_shift_version == "v1":
                        ...
                        # When input resolution is 320 * 320, m = 1, when input resolution is 512 * 512,
                        # m = 1.6, when input resolution is 1024 * 1024, m = 3.2
                        if num_tokens is not None and num_tokens > 0:
    ...
    def step(
        ...
        Returns:
            [`FlowMatchEulerDiscreteSchedulerOutput`] or `tuple`:
                If return_dict is `True`, [`FlowMatchEulerDiscreteSchedulerOutput`] is returned, otherwise a tuple is
