...
"""Native Sonar Omni pipeline for Boogu-Image-0.1.
...
- Upstream ``encode_instruction`` is exposed as ``encode_prompt`` (the
  Sonar Omni convention, also hooked by the prompt-embed cache).
- Text-to-image and single-reference TI2I inference share one native pipeline;
  CFG branches use Sonar Omni's shared two-branch/N-branch parallel helpers.
...
from aphrodite.omni.diffusion.models.boogu_image.boogu_image_transformer import (
    BooguImageDoubleStreamRotaryPosEmbed,
    BooguImageTransformer2DModel,
    RotaryFrequencyTables,
)
from aphrodite.omni.diffusion.models.boogu_image.image_processor import BooguImageProcessor
from aphrodite.omni.diffusion.models.boogu_image.scheduling_flow_match_euler_discrete_time_shifting import (
...
from aphrodite.omni.quantization.component_config import resolve_component_quant_config
...
class BooguImagePipeline(CFGParallelMixin, nn.Module, ProgressBarMixin, SupportsComponentDiscovery, SupportImageInput):
    ...
    Native Sonar Omni implementation. A request with a reference image (edit /
    ...
    def forward(self, req: DiffusionRequestBatch) -> list[DiffusionOutput]:
        ...
        if self._is_turbo:
            ...
            # as the Sonar-native spelling for the same normalized DMD schedule.
