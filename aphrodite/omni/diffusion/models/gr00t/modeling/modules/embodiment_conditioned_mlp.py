...
from aphrodite.omni.diffusion.utils.flow_matching import SinusoidalPositionalEncoding, swish
...
class CategorySpecificLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cat_ids: torch.Tensor) -> torch.Tensor:
        """Apply per-category linear projection.
...
class CategorySpecificMLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cat_ids: torch.Tensor) -> torch.Tensor:
        """Apply two-layer per-category MLP.
...
class MultiEmbodimentActionEncoder(nn.Module):
    ...
    def forward(self, actions: torch.Tensor, timesteps: torch.Tensor, cat_ids: torch.Tensor) -> torch.Tensor:
        """Encode actions with embodiment-conditioned MLP and time embedding.

        Args:
            actions: ``[B, T, action_dim]`` action tensor.
            timesteps: ``[B]`` timesteps (one scalar per batch item).
            cat_ids: ``[B]`` category / embodiment IDs.
