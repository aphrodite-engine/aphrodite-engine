...
Native Sonar Omni port of the NVlabs SANA-WM DiT. Modules are built eagerly at
...
from aphrodite.omni.diffusion.models.sana_wm.config import SanaWmConfig
from aphrodite.omni.diffusion.models.sana_wm.ucpe import (
    SanaWmCamGeometry,
    cam_prep_func,
...
def _shard_param_across_tp(param: torch.Tensor | None, dim: int = 0) -> None:
    """Attach Sonar's TP shard loader to a plain (non-parallel-layer) parameter.

    Sonar parallel layers narrow full checkpoint tensors to the local shard via
...
# video-latent recurrence is a different contract than Sonar's autoregressive
...
class SanaWmSelfAttention(nn.Module):
    ...
    def _reduce_scatter_cam_contrib(self, contrib: torch.Tensor, reference: torch.Tensor) -> torch.Tensor:
        ...
        ``get_tp_group().reduce_scatter()``, and Sonar Omni's diffusion
...
class SanaWmTransformer3DModel(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        if timestep.ndim == 0 or timestep.ndim == 1 and timestep.shape[0] == 1 and batch_size > 1:
    ...
    def load_weights(self, weights: Iterable[tuple[str, Any]]) -> set[str]:
        ...
        the TP-local shard at copy time for both Sonar parallel layers and the
        ...
        for source_name, tensor in weights:
            ...
            if callable(weight_loader):
                # Sonar parallel layers and the plain params marked by
