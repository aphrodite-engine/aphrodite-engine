...
"""LancePipeline — Lance (ByteDance) packaged for the Sonar Omni diffusion engine.
...
Bring-up status (verified on a B300 against ``bytedance-research/Lance``):
  ...
  * Two-stage (AR thinker + DiT): needs ``LanceConfig`` / ``LanceProcessor``
    registered in the ``aphrodite`` package; tracked in a separate PR.
"""
...
from aphrodite.omni.diffusion.models.bagel.pipeline_bagel import (
    BagelPipeline,
    add_special_tokens,
)
...
class LancePipeline(BagelPipeline):
    ...
    def _forward_t2v(self, req):
        ...
        from aphrodite.omni.diffusion.data import DiffusionOutput
        ...
        # t=0.4); without this sonar-omni keeps CFG on for late iters and the
    ...
    def _forward_i2v(self, req):
        ...
        # Upstream Lance i2v default is cfg_text_scale=4.0.  Note: sonar-omni's
        # bf16 attention trajectory (aphrodite-native kernels vs upstream's
    ...
    def _forward_video_edit(self, req):
        ...
        # denoise step).  sonar-omni's previous (0.4, 1.0) skipped CFG for the
        ...
        # sonar-omni's ViT grid was (T=23, H=40, W=54) for the car video versus
