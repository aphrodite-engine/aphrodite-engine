...
``time_embedder`` / ``latent_pos_embed`` connectors.  The deltas vs BAGEL are:
  ...
  * VAE is Wan2.2 (reused from the Sonar Omni WAN path) instead of the BAGEL AE,
...
``aphrodite.omni.diffusion.models.bagel.bagel_transformer`` and only the pipeline
wiring (ViT / VAE / checkpoint layout) is specialized here.
"""

from aphrodite.omni.diffusion.models.lance.pipeline_lance import (
    LancePipeline,
