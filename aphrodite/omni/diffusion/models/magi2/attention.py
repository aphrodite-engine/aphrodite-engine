...
MAGI-2 preview implementation.  This version uses Sonar's bundled
FlashAttention extension and Sonar Omni's existing Ulysses process group; the
...
from aphrodite.omni.diffusion.attention.backends.utils.fa import (
    aphrodite_flash_attn_varlen_with_lse,
    resolve_aphrodite_flash_attn_version,
)
...
def _resolve_flash_attn_version() -> int:
    ...
    version = resolve_aphrodite_flash_attn_version(requested)
...
def packed_attention_with_sink(
    ...
    *,
    ...
    if q.is_cuda:
        out, lse = aphrodite_flash_attn_varlen_with_lse(
            q,
            k,
            v,
            cu_seqlens_q=cu_q,
