...
Follows the same pattern as T5EncoderModel: uses Sonar's parallel linear layers
for tensor parallelism but simple scaled_dot_product_attention instead of
PagedAttention, so it can be used as a standalone encoder without AphroditeConfig.
...
class MistralEncoderModel(nn.Module):
    ...
    Accepts a HuggingFace Mistral3Config (or its text_config). Uses Sonar
