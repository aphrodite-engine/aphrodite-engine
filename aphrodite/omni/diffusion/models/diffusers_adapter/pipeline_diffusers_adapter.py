...
Sonar Omni to directly serve Diffusers models with near-zero per-model code.
...
from aphrodite.omni.diffusion.models.diffusers_adapter.pipeline_utils import (
    BasePipelineUtils,
    get_pipeline_utils_for_config,
    resolve_diffusers_pipeline_class,
)
from aphrodite.omni.diffusion.models.diffusers_adapter.quantization_utils import (
...
from aphrodite.omni.inputs.data import OmniPromptType, OmniTextPrompt
...
class DiffusersAdapterPipeline(nn.Module, DiffusionPipelineProfilerMixin):
        ...
        if (
            ...
            if self.od_config.diffusers_load_kwargs.get("dduf_file"):
                raise NotImplementedError(
                    ...
                    "Diffusers quantization config, or omit dduf_file for Sonar Omni "
    ...
    def _set_attention_backend(self) -> None:
        ...
        Roughly follow the logic in aphrodite/omni/diffusion/attention/backends/utils/fa.py,
        ...
        match attention_backend_config:
            ...
            case "CUDNN_ATTN" | "FLASHINFER_ATTN" | "TRTLLM_ATTN":
                raise ValueError(
                    ...
                    "provide a binding for this Sonar Omni backend. Select TORCH_SDPA or a Diffusers-native backend."
                )
            case _:
