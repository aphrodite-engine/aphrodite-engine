...
def ensure_supported_diffusers_quantization(quant_config: Any) -> None:
    """Validate that a Sonar Omni quantization config has a Diffusers mapping."""
...
def apply_diffusers_quantization_config(
    ...
    if "quantization_config" in load_kwargs:
        logger.warning(
            "Both Sonar Omni quantization_config and diffusers_load_kwargs.quantization_config "
...
def _ensure_no_ignored_layers(quant_config: Any) -> None:
    ...
    if ignored_layers:
        raise NotImplementedError(
            "Diffusers backend quantization conversion does not map Sonar "
...
def _build_torchao_pipeline_quant_config(
    ...
    """Build a Diffusers config after the caller has validated the Sonar config."""
...
def _validate_fp8_quant_config(quant_config: Any) -> None:
    if getattr(quant_config, "is_checkpoint_fp8_serialized", False):
        raise NotImplementedError(
            ...
            "TorchAO quantization; serialized Sonar fp8 checkpoints are not mapped."
        )
    ...
    if weight_block_size is not None:
        raise NotImplementedError(
            "Diffusers backend fp8 conversion does not map Sonar weight_block_size "
...
def _validate_int8_quant_config(quant_config: Any) -> None:
    if getattr(quant_config, "is_checkpoint_int8_serialized", False):
        raise NotImplementedError(
            ...
            "TorchAO quantization; serialized Sonar int8 checkpoints are not mapped."
        )
...
def _get_quantization_config_spec(method: str) -> _QuantizationConfigSpec:
    ...
    if spec is None:
        raise NotImplementedError(
            ...
            "or use a native Sonar Omni pipeline for this quantization method."
        )
    return spec
