...
from aphrodite.model_executor.layers.linear import ColumnParallelLinear, QKVParallelLinear, RowParallelLinear
...
from aphrodite.omni.diffusion.forward_context import build_local_sp_padding_mask, get_forward_context
from aphrodite.omni.diffusion.layers.adalayernorm import AdaLayerNorm
from aphrodite.omni.diffusion.layers.norm import LayerNorm, RMSNorm
from aphrodite.omni.diffusion.layers.rope import RotaryEmbeddingWan
...
class WanSelfAttention(nn.Module):
    """
    Optimized self-attention module using Sonar layers.
    """
    ...
    def __init__(
        ...
        # Fused QKV projection using Sonar's optimized layer
        ...
        # QK normalization using Sonar's RMSNorm
...
class WanCrossAttention(nn.Module):
    """
    Optimized cross-attention module using Sonar layers.
...
class WanTransformer3DModel(nn.Module):
    """
    Optimized Wan Transformer model for video generation using Sonar layers.

    This is an optimized version of the diffusers WanTransformer3DModel that uses
    Sonar's efficient QKVParallelLinear and RMSNorm implementations.
    ...
    def __init__(
        ...
        # 3. Transformer blocks — partitioned across PP stages via Sonar's `make_layers`.
