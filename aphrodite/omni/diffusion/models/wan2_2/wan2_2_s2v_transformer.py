...
"""
Wan2.2 Speech-to-Video (S2V) Transformer using sonar-omni ops.
...
from aphrodite.omni.diffusion.layers.rope import RotaryEmbeddingS2VGrid, RotaryEmbeddingWanS2V, WanS2VRotaryPosEmbed
...
class WanS2VSelfAttention(nn.Module):
    """S2V self-attention using sonar-omni ops (QKVParallelLinear + Attention)."""
...
class WanS2VCrossAttention(nn.Module):
    """S2V cross-attention using sonar-omni ops (ColumnParallelLinear + Attention)."""
...
class WanS2VTransformer3DModel(nn.Module):
    """
    Wan2.2 S2V Transformer using sonar-omni ops for the main transformer blocks.
