...
class Ovis2_5Processor(ProcessorMixin):
    ...
    def extra_special_tokens(self):
        ...
        return {key: self.tokenizer.convert_tokens_to_ids(token_name) for key, token_name in required_tokens.items()}
    ...
    def preprocess_multidata(
        ...
        images = [image.convert("RGB") if do_convert_rgb and image.mode != "RGB" else image for image in images]
        ...
        for image in images:
            ...
            image_pt = self.image_processor.preprocess(image, size=new_size)["pixel_values"][0]
