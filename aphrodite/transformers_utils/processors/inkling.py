...
def _validate_image_rescale(
    ...
    if rescale_image_frac is not None and (not math.isfinite(rescale_image_frac) or rescale_image_frac <= 0):
        raise ValueError(f"rescale_image_frac must be positive and finite or None, got {rescale_image_frac}")
    if rescale_image_max_upscaled_long_edge is None:
        return
    if rescale_image_max_upscaled_long_edge <= 0:
        raise ValueError(
            f"rescale_image_max_upscaled_long_edge must be positive or None, got {rescale_image_max_upscaled_long_edge}"
        )
    if rescale_image_frac is None or rescale_image_frac <= 1.0:
        raise ValueError(
            f"rescale_image_max_upscaled_long_edge requires rescale_image_frac > 1, got {rescale_image_frac}"
        )
...
class InklingProcessor:
    ...
    def __init__(
        ...
        self.audio_feature_extractor = audio_feature_extractor or InklingAudioFeatureExtractor()
