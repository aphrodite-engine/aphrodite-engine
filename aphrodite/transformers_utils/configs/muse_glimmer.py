...
"""MuseGlimmer model configuration for Aphrodite.

Native Aphrodite copy of the MuseGlimmer HuggingFace configs
(``configuration_muse_glimmer.py``). MuseGlimmer's
``model_type`` is not yet registered in released transformers, so Aphrodite ships
...
def _default_no_rope_layers(num_hidden_layers: int) -> list[int]:
    ...
    stride = 4
    return [0 if (num_hidden_layers - 1 - i) % stride == 0 else 1 for i in range(num_hidden_layers)]
...
class MuseGlimmerTextConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # bare rope_theta; normalize to rope_parameters for Aphrodite's get_rope.
        if rope_parameters is None:
            theta = rope_theta if rope_theta is not None else 500_000.0
            rope_parameters = {"rope_type": "default", "rope_theta": theta}
        self.rope_parameters = rope_parameters
        # Aphrodite reads rope_theta off the config in some codepaths.
        ...
        self.no_rope_layers = (
            no_rope_layers if no_rope_layers is not None else _default_no_rope_layers(num_hidden_layers)
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if layer_types is None:
            stride = 4
            layer_types = [
                "full_attention" if (i + 1) % stride == 0 or i == num_hidden_layers - 1 else "sliding_attention"
                for i in range(num_hidden_layers)
            ]
        self.layer_types = layer_types
...
class MuseGlimmerAssistantConfig(Qwen3Config):
    ...
    The head is Qwen3-shaped and runs on Aphrodite's generic ``qwen3_dflash``
    ...
    * ``vocab_size`` is absent from the checkpoint, so Qwen3's default of
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(vocab_size=vocab_size, use_sliding_window=use_sliding_window, **kwargs)
