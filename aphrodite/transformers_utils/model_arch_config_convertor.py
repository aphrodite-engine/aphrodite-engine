...
class ModelArchConfigConvertorBase:
    ...
    def get_per_layer_hf_configs(
        ...
        # its stack is what Aphrodite builds attention layers for.
        ...
        return [
            (
                ...
                self.hf_text_config.per_layer_config[i] if text_het else self.hf_text_config,
            )
            for i in range(len(source.per_layer_config))
        ]

    def get_architectures(self) -> list[str]:
        # Sometimes we get here from `aphrodite_config.with_hf_config(text_config)` where
    ...
    def get_head_size(self) -> int:
        if self.is_deepseek_mla():
            ...
            if not envs.APHRODITE_MLA_DISABLE:
    ...
    def _normalize_quantization_config(self, config: PretrainedConfig):
        ...
        if quant_cfg is None:
            ...
            is_legacy_modelopt = isinstance(modelopt_quant_cfg, dict) and "modelopt_quant_config" in modelopt_quant_cfg
    ...
    def convert(self, supports_multimodal: bool = True) -> ModelArchitectureConfig:
        ...
        return ModelArchitectureConfig.from_layers(
            [type(self)(*configs).convert_layer(supports_multimodal) for configs in per_layer]
        )

    def convert_layer(self, supports_multimodal: bool = True) -> ModelArchitectureConfig:
...
class BailingHybridV3MTPModelArchConfigConvertor(BailingHybridMTPModelArchConfigConvertor):
    pass
