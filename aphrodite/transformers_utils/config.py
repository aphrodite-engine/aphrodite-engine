...
from aphrodite.transformers_utils.repo_utils import is_mistral_model_repo
...
if Version(version("transformers")) < Version("5.0.0"):
    raise ImportError(
        "Support for Transformers v4 is deprecated and was removed in Aphrodite v0.24.0. "
...
# validation rejects them (e.g. GLM-5.2 `glm_moe_dsa` uses
...
def _register_config_class(model_type: str, config_class: type[PretrainedConfig]) -> None:
...
def _patch_hf_transformers_validate_rope():
    ...
    if hasattr(PretrainedConfig.validate_rope, "__aphrodite_patched__"):
        return
    ...
    patched_validate_rope.__aphrodite_patched__ = True  # type: ignore[attr-defined]
...
def _patch_hf_transformers_allowed_layer_types(
    ...
    """Extend transformers' ``ALLOWED_LAYER_TYPES`` for known checkpoint types."""
    global ALLOWED_LAYER_TYPES

    import transformers.configuration_utils as hf_configuration_utils

    missing = tuple(
        layer_type for layer_type in extra_layer_types if layer_type not in hf_configuration_utils.ALLOWED_LAYER_TYPES
    )
    if missing:
        hf_configuration_utils.ALLOWED_LAYER_TYPES += missing

    ALLOWED_LAYER_TYPES = hf_configuration_utils.ALLOWED_LAYER_TYPES
...
def _uses_mrope(config: PretrainedConfig) -> bool:
    ...
    return "mrope_section" in rope_parameters or any(
        isinstance(params, dict) and "mrope_section" in params for params in rope_parameters.values()
    )
...
def maybe_override_with_speculators(
    ...
    **kwargs,
    ...
    if isinstance(aphrodite_speculative_config, dict):
        speculative_config.update(aphrodite_speculative_config)
...
def get_pooling_config(
    ...
    normalize = bool(
        next(
            (item for item in modules_dict if item["type"] in _ST_NORMALIZE_MODULE_TYPES),
            False,
        )
    )

    if pooling:
        from aphrodite.config.pooler import SEQ_POOLING_TYPES, TOK_POOLING_TYPES
        ...
        if not {"seq_pooling_type", "tok_pooling_type"} & config.keys():
            logger.warning(
                ...
                "unless configured explicitly, Aphrodite will fall back to the model "
...
def get_hf_text_config(config: PretrainedConfig):
    ...
    if text_config is not config and not hasattr(text_config, "num_attention_heads"):
        raise ValueError(
            ...
            "between the model config and Aphrodite's expectations. Please "
            "ensure that the model config is compatible with Aphrodite."
        )
...
def try_get_safetensors_metadata(
    ...
    *,
    ...
    get_safetensors_metadata_partial = partial(hf_api().get_safetensors_metadata, model, revision=revision)
...
def get_safetensors_params_metadata(
    ...
    *,
    ...
    if repo_mt and (files_mt := repo_mt.files_metadata):
        return {
            param_name: asdict(info) for file_mt in files_mt.values() for param_name, info in file_mt.tensors.items()
        }
