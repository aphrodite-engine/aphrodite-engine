...
def test_unet_layer_by_layer():
    ...
    transformer_depth = [
        ...
        10,
        10,
    ]  # 6 blocks: 2 blocks per level, [0,0] for level 0, [2,2] for level 1, [10,10] for level 2
    context_dim = 2048
    ...
    if "conv_in" in diffusers_activations:
        ...
        print(
            f"  Stats: min={diff_act.min().item():.4f}, max={diff_act.max().item():.4f}, std={diff_act.std().item():.4f}"
        )
    ...
    if "input_blocks.0" in our_activations:
        ...
        print(
            f"  Stats: min={our_act.min().item():.4f}, max={our_act.max().item():.4f}, std={our_act.std().item():.4f}"
        )
    ...
    for diff_idx, our_indices in mapping:
        ...
        if diff_name in diffusers_activations:
            ...
            print(
                f"  Diffusers: min={diff_act.min().item():.4f}, max={diff_act.max().item():.4f}, std={diff_act.std().item():.4f}"
            )
            ...
            for our_idx in our_indices:
                ...
                if our_name in our_activations:
                    ...
                    print(
                        f"    {our_name}: {our_act.shape}, min={our_act.min().item():.4f}, max={our_act.max().item():.4f}, std={our_act.std().item():.4f}"
                    )
                    ...
                    if diff_act.shape == our_act.shape:
                        ...
                        print(
                            f"      Max diff: {max_diff:.6f}, Mean diff: {mean_diff:.6f}, Cosine sim: {cosine_sim:.6f}"
                        )
                        if max_diff > 0.1:
    ...
    if "mid_block" in diffusers_activations and "middle_block" in our_activations:
        ...
        if diff_act.shape == our_act.shape:
            ...
            max_diff = diff.max().item()
            mean_diff = diff.mean().item()
            cosine_sim = torch.nn.functional.cosine_similarity(diff_act.flatten(), our_act.flatten(), dim=0).item()
            ...
            print(
                f"  Ours:      min={our_act.min().item():.4f}, max={our_act.max().item():.4f}, std={our_act.std().item():.4f}"
            )
    ...
    if diffusers_output.shape == our_output.shape:
        ...
        print(
            f"  Diffusers: min={diffusers_output.min().item():.4f}, max={diffusers_output.max().item():.4f}, std={diffusers_output.std().item():.4f}"
        )
        print(
            f"  Ours:      min={our_output.min().item():.4f}, max={our_output.max().item():.4f}, std={our_output.std().item():.4f}"
        )
        print(f"  Max diff: {max_diff:.6f}, Mean diff: {mean_diff:.6f}, Cosine sim: {cosine_sim:.6f}")
    else:
