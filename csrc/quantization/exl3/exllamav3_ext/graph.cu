...
void Graph::capture_end() {
  ...
  for (int i = 0; i < num_nodes; ++i) node_needs_update[i] = false;
  ...
  while (true) {
    ...
    if (t == cudaGraphNodeTypeKernel) {
      ...
      for (; c < graph_sites.size(); c++) {
        ...
        graph_node_sites.push_back(
            std::tuple<int, int, int>(n, param_id, param_offset));
        if (param_id == GP_end) {
          c++;
          break;
        }
...
void Graph::record_param(void* kernel, int param_id, int param_offset) {
  graph_sites.push_back(
      std::tuple<void*, int, int>(kernel, param_id, param_offset));
}
  ...
  while (true) {
    if (std::get<1>(graph_node_sites[n]) == std::get<0>(params[p])) {
      if (std::get<0>(params[p]) != GP_end) {
        ...
        void** p_old_value =
            (void**)node_params[node_idx].kernelParams[param_offset];
...
void Graph::inspect_graph() {
  ...
  for (size_t i = 0; i < numNodes; ++i) {
    ...
    if (nodeType == cudaGraphNodeTypeKernel) {
      ...
      std::cout << "  Grid dimensions: (" << nodeParams.gridDim.x << ", "
                << nodeParams.gridDim.y << ", " << nodeParams.gridDim.z << ")"
                << std::endl;
      std::cout << "  Block dimensions: (" << nodeParams.blockDim.x << ", "
                << nodeParams.blockDim.y << ", " << nodeParams.blockDim.z << ")"
                << std::endl;
      std::cout << "  Shared memory: " << nodeParams.sharedMemBytes << " bytes"
                << std::endl;
