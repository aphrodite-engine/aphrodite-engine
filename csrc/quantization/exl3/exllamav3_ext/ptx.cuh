...
__device__ inline void ptx_mma_m8n8k4(const Vec<half2, 2>& frag_a,
  ...
  asm("mma.sync.aligned.m8n8k4.row.col.f32.f16.f16.f32 "
      ...
      : "r"(a[0]), "r"(a[1]), "r"(b[0]), "r"(b[1]), "f"(d[0]), "f"(d[1]),
        "f"(d[2]), "f"(d[3]), "f"(d[4]), "f"(d[5]), "f"(d[6]), "f"(d[7]));
}
...
// FP16 @ FP16 + FP32 -> FP32
__device__ inline void ptx_mma_m16n8k16(const FragA& frag_a,
                                        const FragB& frag_b, FragC& frag_c) {
  const uint32_t* a = reinterpret_cast<const uint32_t*>(&frag_a);
  const uint32_t* b = reinterpret_cast<const uint32_t*>(&frag_b);
  float* c = reinterpret_cast<float*>(&frag_c);
  const float* d = reinterpret_cast<const float*>(&frag_c);
...
// FP16 @ FP16 + FP16 -> FP16
__device__ inline void ptx_mma_m16n8k16(const FragA& frag_a,
                                        const FragB& frag_b, FragC_h& frag_c) {
  const uint32_t* a = reinterpret_cast<const uint32_t*>(&frag_a);
  const uint32_t* b = reinterpret_cast<const uint32_t*>(&frag_b);
...
// Load global to shared memory, predicated. Seems to produce incorrect code
// when compiling for Blackwell, but `if (...) cp_async(...)` compiles to a
// predicated instruction anyway

__device__ inline void cp_async_pred(void* smem_ptr, const void* glob_ptr,
                                     bool pred = true) {
  const int bytes = 16;
...
__device__ inline uint32_t mul_lo_u32(uint32_t x, uint32_t y) {
  uint32_t w;
  asm volatile("mul.lo.u32 %0, %1, %2;" : "=r"(w) : "r"(x), "r"(y));
  return w;
}

__device__ inline uint32_t mul_hi_u32(uint32_t x, uint32_t y) {
  uint32_t w;
  asm volatile("mul.hi.u32 %0, %1, %2;" : "=r"(w) : "r"(x), "r"(y));
  return w;
}
...
__device__ __forceinline__ void stg_wt_u128(uint4* p, const uint4 v) {
  asm volatile("st.global.wt.v4.u32 [%0], {%1,%2,%3,%4};" ::"l"(p), "r"(v.x),
               "r"(v.y), "r"(v.z), "r"(v.w));
}
...
__device__ __forceinline__ void stg_release_sys_u64(uint64_t* p, uint64_t v) {
  asm volatile("st.global.release.sys.u64 [%0], %1;" ::"l"(p), "l"(v)
               : "memory");
}
...
static __forceinline__ __device__ uint32_t bfe64(uint32_t lo, uint32_t hi,
...
__device__ inline void group_barrier(
    ...
    int* barrier_counters_sense  // length 2*max(group_id). odd positions are
                                 // flipped after sync (sense)
) {
  __syncthreads();

  if (threadIdx.x == 0) {
    cuda::atomic_ref<int, cuda::thread_scope_device> counter(
        barrier_counters_sense[group_id * 2]);
    cuda::atomic_ref<int, cuda::thread_scope_device> sense(
        barrier_counters_sense[group_id * 2 + 1]);
    ...
    if (old == group_size - 1) {
      ...
      while (sense.load(cuda::memory_order_acquire) == old_sense)
        __nanosleep(32);
    }
  }
