...
int DevCtx::get_num_sms(int device) {
  ...
  if (!num_sms[device])
    cuda_check(cudaDeviceGetAttribute(&num_sms[device],
...
int* DevCtx::get_locks(int device) {
  ...
  if (!locks[device]) {
    cudaSetDevice(device);
    size_t size =
        (MAX_TILES_C + MAX_BARRIERS * 2 + MOE_SCHED_INTS) * sizeof(int);
...
int g_get_cc(int device) { return DevCtx::instance().get_cc(device); }

int g_get_num_sms(int device) { return DevCtx::instance().get_num_sms(device); }
