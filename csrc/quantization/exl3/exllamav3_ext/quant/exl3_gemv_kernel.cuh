...
// FP16 @ FP16 + FP32 -> FP32
__device__ inline void ptx_mma_m8n8k16(const FragA_m8& frag_a,
                                       const FragB& frag_b, FragC_m8& frag_c) {
  ...
  asm("mma.sync.aligned.m16n8k16.row.col.f32.f16.f16.f32 "
      ...
      : "+f"(c[0]), "+f"(c[1]),
        "+f"(c[2])  // Todo: try to alias into m8 fragment
      : "r"(a[0]), "r"(a[1]), "r"(b[0]), "r"(b[1]));
}

__device__ inline float2 shfl_float2(float2 v, int src, int width) {
  return make_float2(__shfl_sync(0xffffffff, v.x, src, width),
                     __shfl_sync(0xffffffff, v.y, src, width));
}

template <int bits, bool c_fp32, int cb, int split_k>
__global__ __launch_bounds__(32 * TILESIZE_K / 16 * 1) void exl3_gemv_kernel(
    const half* __restrict__ A, const uint16_t* __restrict__ B,
    void* __restrict__ C, const int __grid_constant__ size_m,
    const int __grid_constant__ size_k, const int __grid_constant__ size_n
  ...
  const size_t str_B_gl_block_k =
      str_B_gl_block_n * (size_n / 16);  // B global: block stride, K
  const size_t str_B_gl_tile_n =
      str_B_gl_block_n * blocks_n;  // B global: tile stride, N
  const size_t str_B_gl_tile_k =
      str_B_gl_block_k * blocks_k;  // B global: tile stride, K

  const size_t str_B_sh_block_n =
      str_B_gl_block_n;  // B shared: block stride, N
  const size_t str_B_sh_block_k =
      str_B_sh_block_n * blocks_n;                  // B shared: block stride, K
  const size_t str_B_sh_tile_n = str_B_sh_block_k;  // B shared: tile stride, N
  const size_t str_B_sh_tile =
      str_B_sh_block_k * blocks_k;  // B shared: tile size
  ...
  const size_t str_C_gl_tile_n =
      TILESIZE_N * c_esize;  // C global: tile stride, N
...
#pragma unroll
  ...
  auto load_sh2fr_b = [&](int block_n) {
    dq_dispatch<bits, cb>((uint32_t*)B_sh_block, lane_id << 3,
                          frag_b[block_n][0], frag_b[block_n][1]);
  };

  // Load B fragment from gmem tile in warp
  auto load_gl2fr_b = [&](int block_n) {
    dq_dispatch<bits, cb>((uint32_t*)B_gl_block, lane_id << 3,
                          frag_b[block_n][0], frag_b[block_n][1]);
  };
...
#pragma unroll
  ...
  auto store_fr2gl_c = [&](int block_n) {
    ...
    if (frag_row < size_m) {
      if (c_fp32) {
        ...
        // Thread has c[i] and c[i+4], shuffle so threads hold c[i] and c[i+1]
        // in groups of four
        int base = lane_id & (~3);
        ...
        half2 c0 =
            __floats2half2_rn(frag_c[block_n][0][0], frag_c[block_n][0][1]);
        half2 c1 =
            __floats2half2_rn(frag_c[block_n][1][0], frag_c[block_n][1][1]);
  ...
  if constexpr (num_stages == 0) {
    for (int tile_k = 0; tile_k < tiles_k_slice; ++tile_k) {
      ...
      // Load B frag
      B_gl_tile =
          ((uint8_t*)B) + str_B_gl_tile_k * tile_k + str_B_gl_tile_n * tile_n;
...
#pragma unroll
    for (int tile_k = 0; tile_k < tiles_k_slice; ++tile_k) {
      // Load next B chunk
      B_gl_tile = ((uint8_t*)B) + str_B_gl_tile_k * (tile_k + 1) +
                  str_B_gl_tile_n * tile_n;
