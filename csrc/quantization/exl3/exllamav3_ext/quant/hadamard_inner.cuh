...
__device__ inline void shuffle_had_f4x32(float& h0, float& h1, float& h2,
...
__device__ inline void shuffle_had_f2x32(float& v, float& w,
                                         const int lane_id) {
#pragma unroll
  for (int i = 1; i < 32; i <<= 1) {
    uint64_t vw =
...
inline __device__ void had_hf_r_128_guad_inner(
    const half* __restrict__ input_ptr_g, const half* __restrict__ input_ptr_u,
    half* __restrict__ output_ptr, const half* __restrict__ post_scale_g,
    const half* __restrict__ post_scale_u, const half* __restrict__ pre_scale_d,
    const float r_scale, const float act_limit, const int act_function) {
  int t = threadIdx.x & 31;
  ...
  auto _gelu = [&](const half2& x) {
    ...
    xf.x = 0.5f * xf.x *
           (1.0f + tanh_opt(c * (xf.x + 0.044715f * xf.x * xf.x * xf.x)));
    xf.y = 0.5f * xf.y *
           (1.0f + tanh_opt(c * (xf.y + 0.044715f * xf.y * xf.y * xf.y)));
...
inline __device__ void had_hf_r_128_d_inner(const half* __restrict__ input_ptr,
                                            float* __restrict__ output_ptr,
                                            const half* __restrict__ post_scale,
                                            const float r_scale) {
  int t = threadIdx.x & 31;
