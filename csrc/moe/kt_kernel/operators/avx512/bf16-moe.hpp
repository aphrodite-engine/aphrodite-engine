/**
 * @Description  : AVX512 BF16 MoE operator (ported from avx2/bf16-moe.hpp)
 * @Author       : Claude
 * @Date         : 2026-03-18
 * @Version      : 1.0.0
 * @Copyright (c) 2024 by KVCache.AI, All Rights Reserved.
 *
 * BufferB::from_mat is row-major memcpy; GEMM consumes the row-major BF16
 * weights directly with AVX512 BF16 dot-product instructions.
 **/
#ifndef CPUINFER_OPERATOR_AVX512_BF16_MOE_H
#define CPUINFER_OPERATOR_AVX512_BF16_MOE_H

#include "avx512_bf16_gemm.hpp"
#include "moe_base.hpp"

template <class T = avx512::GemmKernelAVX512BF16>
class AVX512_BF16_MOE_TP : public AVX512_MOE_BASE<T, AVX512_BF16_MOE_TP<T>> {
  using Base = AVX512_MOE_BASE<T, AVX512_BF16_MOE_TP<T>>;
  using Base::config_;
  using Base::down_ba_;
  using Base::down_bb_;
  using Base::down_bc_;
 ...
 public:
  ...
  AVX512_BF16_MOE_TP() = default;

  AVX512_BF16_MOE_TP(GeneralMOEConfig config, int tp_part_idx_ = 0) : Base(config, tp_part_idx_) {}
  ...
  // GEMM dispatch uses avx512::gemm_bf16.
  void do_gate_up_gemm(bool do_up, int expert_idx, int ith, int nth, int qlen) {
    ...
    avx512::gemm_bf16(m, config_.intermediate_size, config_.hidden_size, *ba, *bb, *bc, ith, nth);
  }

  void do_down_gemm(int expert_idx, int ith, int nth, int qlen) {
    int m = m_local_num_[expert_idx];
    avx512::gemm_bf16(m, config_.hidden_size, config_.intermediate_size,
                      *down_ba_[expert_idx], *down_bb_[expert_idx], *down_bc_[expert_idx], ith, nth);
  }

  /**
   * Load BF16 weights from contiguous memory layout.
   * BufferB::from_mat is a trivial memcpy for AVX512 (no AMX transpose).
   */
  void load_weights() {
        ...
        [=, &w13_weight_ptrs, &w2_weight_ptrs, this](int task_id) {
          if (task_id < NUM_W13_TASKS * 2) {
            ...
            for (int row = row_start; row < row_end; row++) {
              ...
              bb->copy_row_slice_to(dst + expert_weight_off + (size_t)n_in_gpu * gpu_k_w13, row, 0, cpu_k_w13);
            }
          } else {
            ...
            for (int row = row_start; row < row_end; row++) {
              ...
              for (int k_start = 0; k_start < cpu_k_w2; k_start += gpu_k_w2) {
                ...
                bb->copy_row_slice_to(dst + (size_t)row * gpu_k_w2 + k_in_gpu, row, k_start, k_slice_len);
              }
            }
          }
        },
...
// ============================================================================
// TP_MOE specialization for AVX512_BF16_MOE_TP
// Handles per-expert pointer loading and TP weight splitting
// (Ported from amx/bf16-moe.hpp TP_MOE<AMX_BF16_MOE_TP<K>>)
// ============================================================================
template <typename K>
class TP_MOE<AVX512_BF16_MOE_TP<K>> : public TP_MOE<AVX512_MOE_BASE<K, AVX512_BF16_MOE_TP<K>>> {
 public:
  using Base = TP_MOE<AVX512_MOE_BASE<K, AVX512_BF16_MOE_TP<K>>>;
  using Base::Base;
...
#endif  // CPUINFER_OPERATOR_AVX512_BF16_MOE_H
