...
static inline __m256 erf_avx2(__m256 x) {
  ...
  __m256 t = _mm256_div_ps(_mm256_set1_ps(1.0f),
                           _mm256_fmadd_ps(p, ax, _mm256_set1_ps(1.0f)));
  ...
  __m256 exp_term =
      exp_avx2(_mm256_sub_ps(_mm256_setzero_ps(), _mm256_mul_ps(ax, ax)));
  __m256 y = _mm256_sub_ps(_mm256_set1_ps(1.0f),
                           _mm256_mul_ps(poly, exp_term));
  return _mm256_xor_ps(y, sign);
}

static inline __m256 gelu_fn(__m256 gate_val, __m256 up_val) {
  const __m256 inv_sqrt2 = _mm256_set1_ps(0.7071067811865476f);
  __m256 erf_val = erf_avx2(_mm256_mul_ps(gate_val, inv_sqrt2));
  __m256 act_val = _mm256_mul_ps(
      _mm256_mul_ps(_mm256_set1_ps(0.5f), gate_val),
      _mm256_add_ps(_mm256_set1_ps(1.0f), erf_val));
  return _mm256_mul_ps(act_val, up_val);
}
