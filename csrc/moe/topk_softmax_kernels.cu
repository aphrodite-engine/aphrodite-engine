...
#pragma unroll
    for (int mask = THREADS_PER_ROW / 2; mask > 0; mask /= 2) {
      float other_max_for_choice = APHRODITE_SHFL_XOR_SYNC_WIDTH(
          max_val_for_choice, mask, THREADS_PER_ROW);
      float other_max =
  ...
  switch (num_experts) {
    ...
    default: {
      TORCH_CHECK(workspace != nullptr,
                  "workspace must be provided for num_experts that are not a "
                  "power of 2 or multiple of 64.");
...
void dispatch_topk_launch(
    torch::Tensor& gating_output, torch::Tensor& topk_weights,
    torch::Tensor& topk_indices, torch::Tensor& token_expert_indices,
    torch::Tensor& softmax_workspace, int num_tokens, int num_experts, int topk,
    bool renormalize, std::optional<torch::Tensor> bias, cudaStream_t stream) {
  ...
  if (topk_indices.scalar_type() == at::ScalarType::Int) {
    aphrodite::moe::topkGatingKernelLauncher<int, ComputeType, SF>(
        ...
        renormalize, bias_ptr, stream);
  } else if (topk_indices.scalar_type() == at::ScalarType::UInt32) {
    aphrodite::moe::topkGatingKernelLauncher<uint32_t, ComputeType, SF>(
        reinterpret_cast<const ComputeType*>(gating_output.data_ptr()),
        topk_weights.data_ptr<float>(), topk_indices.data_ptr<uint32_t>(),
        token_expert_indices.data_ptr<int>(),
        softmax_workspace.data_ptr<float>(), num_tokens, num_experts, topk,
        renormalize, bias_ptr, stream);
  } else {
    ...
    aphrodite::moe::topkGatingKernelLauncher<int64_t, ComputeType, SF>(
        reinterpret_cast<const ComputeType*>(gating_output.data_ptr()),
        topk_weights.data_ptr<float>(), topk_indices.data_ptr<int64_t>(),
        token_expert_indices.data_ptr<int>(),
        softmax_workspace.data_ptr<float>(), num_tokens, num_experts, topk,
        renormalize, bias_ptr, stream);
  }
...
void topk_softmax(torch::Tensor& topk_weights,          // [num_tokens, topk]
                  ...
                  bool renormalize, std::optional<torch::Tensor> bias) {
  ...
  const auto workspace_options =
      gating_output.options().dtype(at::ScalarType::Float);
